...
RUN apt-get update \
    ...
    && echo "deb http://apt.llvm.org/$(lsb_release -cs)/ llvm-toolchain-$(lsb_release -cs)-16 main" | tee /etc/apt/sources.list.d/llvm.list >/dev/null \
    && apt-get update


RUN apt-get install -y git cmake python3-pip ninja-build antlr3 m4 clang-16 lld-16 libidn11-dev libaio1 libaio-dev llvm-16 \
