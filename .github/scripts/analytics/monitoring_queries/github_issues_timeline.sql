...
FROM (
    ...
    WHERE date_window >= CurrentUtcDate() - 365 * Interval("P1D")
) AS dt
CROSS JOIN (
    SELECT
    ...
    WHERE t.created_date >= CurrentUtcDate() - 365 * Interval("P1D")
) AS i
WHERE i.created_date <= dt.d
ORDER BY date DESC, i.issue_number;
