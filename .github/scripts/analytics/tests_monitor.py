...
def main():
    ...
    with ydb.Driver(
        ...
        default_start_date = datetime.date(2024, 9, 28)
        ...
        end_time = time.time()
        print(f'df prepared: {end_time - start_time}')
        start_time = time.time()
        ...
        end_time = time.time()
        print(f'df iteration: {end_time - start_time}')
        start_time = time.time()
        ...
        result = df[[
            'full_name','date_window',
            'suite_folder', 'test_name', 'days_ago_window', 'build_type', 'branch',
            'history', 'history_class', 'pass_count', 'mute_count', 'fail_count' , 'skip_count',
            'summary', 'owner','is_test_chunk', 'is_muted',
            'state', 'previous_state', 'state_change_date', 'days_in_state', 'success_rate'
        ]]

        end_time = time.time()
        print(f'df data processed {end_time - start_time}')
        start_time = time.time()
        print(f'monitor data collected, {len(result)} rows')
        prepared_for_update_rows = result.to_dict('records')
       
        end_time = time.time()
        print(f'preparing for upsert data: {end_time - start_time}')
        ...
        with ydb.SessionPool(driver) as pool:
            ...
            def chunk_data(data, chunk_size):
                """Генератор для разбиения данных на чанки."""
                for i in range(0, len(data), chunk_size):
                    yield data[i:i + chunk_size]

            chunk_size = 40000  # примерный размер чанка, не превышающий лимит

            # Подсчет общего количества чанков
            total_chunks = len(prepared_for_update_rows) // chunk_size + (1 if len(prepared_for_update_rows) % chunk_size != 0 else 0)
