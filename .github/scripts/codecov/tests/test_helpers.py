...
class WorkflowContractTest(unittest.TestCase):
    def test_codecov_flag_paths_match_suite_owned_prefixes(self) -> None:
        ...
        for kind in ("project", "patch"):
            # Psych parses YAML 1.1's unquoted `off` as false.
            self.assertIs(statuses[kind]["default"], False)
            for suite in SUITES:
                self.assertEqual(statuses[kind][suite]["flags"], [suite])
        for suite in SUITES:
            self.assertIs(statuses["patch"][suite]["informational"], True)
