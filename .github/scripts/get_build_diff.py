...
def main():
    ...
    if main_sizes_result and current_sizes_result:
        ...
        diff_perc = Decimal(bytes_diff * 100 / main_size_bytes).quantize(Decimal('.001'), rounding=ROUND_HALF_UP)
        ...
        comment = (f"[merge:{current_pr_commit_sha[:8]}]({current_url}) ydbd size {human_readable_size} **{sign}{human_readable_size_diff} {diff_perc}%%**  vs build [{branch}:{main_github_sha[:8]}]({main_url})\n\n"
