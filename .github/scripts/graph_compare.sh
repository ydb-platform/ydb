...
if [[ -z "$YA_MAKE_COMMAND" ]]; then
    YA_MAKE_COMMAND="./ya make -ttt --build release"
fi

$YA_MAKE_COMMAND ydb -k --cache-tests --build-all -Gj0 | jq '.graph[]' > $workdir/graph_base

echo Checkout head commit...
git checkout $2
echo Build graph for head commit...
$YA_MAKE_COMMAND ydb -k --cache-tests --build-all -Gj0  | jq '.graph[]' > $workdir/graph_head
