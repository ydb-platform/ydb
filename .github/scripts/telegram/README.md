...
Monitors the GitHub Actions queue and sends Telegram alerts when jobs are stuck (PR-check, Postmerge, Postcommit_cmake, etc.). Runs in **`.github/workflows/telegram_scheduled_notifications.yml`** (job **CI queue Telegram alerts**; same file also runs **Mute digest to Telegram** via `send_digest.py`).
