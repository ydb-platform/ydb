...
def generate_github_issue_title_and_body(test_data):
    ...
    body_template = (
        ...
        "1. Keep this issue open while tests are still unstable.\n"
        "2. If you fixed tests and want to start quarantine, close the issue manually.\n"
        "3. Bot may reopen the issue and mark tests as quarantine candidates until rule window is over.\n"
        "4. Final status is decided by coordinator rules from YDB (default/quarantine), not by issue text only.\n\n"
        "### Legacy note\n"
        "- Some old issues can contain legacy wording or outdated instructions.\n"
        "- Source of truth is machine-readable markers in issue body and mute coordinator state in YDB.\n\n"
