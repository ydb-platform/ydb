    ...
    if (data.by_chunk && data.xs_active_chunk && data.xs_active_chunk.length > 0) {{
      ...
      if (data.resources_overlay) {{
        ...
        Plotly.addTraces('cpuLayer', [
          {{ x: xDisp, y: ro.cpu_total_cores || [], mode: 'lines', name: 'CPU total (monitor)', line: {{ color: '#2563eb', width: 2 }} }},
          {{ x: xDisp, y: ro.cpu_ya_cores || [], mode: 'lines', name: 'CPU ya make (monitor)', line: {{ color: '#60a5fa', width: 2, dash: 'dash' }} }}
        ]);
        Plotly.addTraces('ramLayer', [
          {{ x: xDisp, y: ro.ram_gb || [], mode: 'lines', name: 'RAM total (monitor)', line: {{ color: '#16a34a', width: 2 }} }},
          {{ x: xDisp, y: ro.ram_ya_gb || [], mode: 'lines', name: 'RAM ya make (monitor)', line: {{ color: '#4ade80', width: 2, dash: 'dash' }} }}
        ]);
