  ...
  <div id="cpuSuggestionsSection" style="display: none; margin: 16px 0;">
    ...
    <div id="cpuHelpText" class="cpu-help">
      <ol>
        ...
        <li>Long test duration and timeouts drive <b>recommended_split</b>, <b>not</b> CPU. A single long test at/above the size threshold (<b>SMALL=60s, MEDIUM=600s, LARGE=3600s</b>) is flagged as a split hint only; it no longer bumps the CPU tier.</li>
    ...
    if (data.cpu_suggestions && data.cpu_suggestions.length > 0) {{
      ...
      function buildCpuUpdateScript(items) {{
        ...
        return [
          ...
          'SANITIZER_FOR_SCRIPT = ' + sanitizerPy + '  # from dashboard (ASAN/TSAN/MSAN)',
          '',
          ...
          '',
          'def _eval_condition_for_sanitizer(cond: str, sanitizer: str | None) -> bool:',
          "    expr = (cond or '').strip()",
          '    has_san = bool(sanitizer and str(sanitizer).strip() and str(sanitizer).strip().lower() not in ("none", "off", "false", "0"))',
          ...
          '',
          'def _find_requirements_line(lines: list[str], sanitizer: str | None) -> int | None:',
          ...
          '            if_res = _eval_condition_for_sanitizer(m_if.group(1), sanitizer)',
          '            current_active = parent_active and if_res',
          '            current_sanitizer_scope = parent_scope or _cond_mentions_sanitizer(m_if.group(1))',
          '            stack.append((parent_active, if_res, current_active, False, current_sanitizer_scope))',
          '            continue',
          '        if RE_ELSE.match(line):',
          '            if not stack:',
          '                continue',
          '            parent_active, if_res, prev_cur, seen_else, scope = stack.pop()',
          '            if seen_else:',
          '                stack.append((parent_active, if_res, prev_cur, seen_else, scope))',
          '                continue',
          ...
          '            continue',
          '        if current_active and RE_REQ_LINE.match(raw) and (not sanitizer or current_sanitizer_scope):',
          '            return i',
          '    return None',
          '',
          'def _find_sanitizer_block_insert_index(lines: list[str], sanitizer: str | None) -> int | None:',
          ...
          '        if m_if:',
          '            parent_active = current_active',
          '            if_res = _eval_condition_for_sanitizer(m_if.group(1), None)',
          '            current_active = parent_active and if_res',
          ...
          '',
          'def apply_one(repo_root: Path, suite_path: str, cpu: str, dry_run: bool, sanitizer: str | None) -> tuple[str, str]:',
          ...
          "    print('Sanitizer: ' + str(sanitizer or 'no (default branch)'))",
          "    print('Mode: ' + str(args.mode))",
          "    print('Total suites to update: ' + str(len(updates)))",
          '    changed = 0',
          '    for suite, cpu, action in updates:',
          '        suite_out, status = apply_one(repo_root, suite, cpu, args.dry_run, sanitizer)',
