"""Pure OLAP Now classify / compare rules (shared by generate.py + unit tests)."""
...
def tpcc_hard_band(side: dict | None) -> int:
    ...
    if st == "broken" or (side.get("n_broken") or 0) > 0:
        return 2
    if st in ("lat", "tpmc", "both", "regression") or (side.get("n_lat") or 0) or (side.get("n_tpmc") or 0):
        return 1
    return 0


def compare_delta_tpcc(prev: dict, now: dict, lat_tol: float = 10.0, tpmc_tol: float = 10.0) -> str:
