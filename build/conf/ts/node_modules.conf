...
_INJECT_PEERS_ARG=--inject-peers yes
_INJECT_PEERS=yes
_SUPPORTS_HERMETIC_NODE_MODULES=
_HERMETIC_NODE_MODULES_ARG=
_SQUASHFS_TOOLS_ARG=
...
_WITH_NODE_MODULES=
_WITH_NODE_MODULES_PROD=

### @usage: WITH_NODE_MODULES([PROD])
###
...
macro WITH_NODE_MODULES(PROD?"yes":"no") {
    SET(_WITH_NODE_MODULES yes)
    SET(_WITH_NODE_MODULES_PROD $PROD)
}

_TARBALLS_STORE=__tarballs__
_PREPARE_DEPS_INOUTS=${hide;output:"pre.pnpm-lock.yaml"} ${hide;output:"pnpm-lock.yaml"} ${hide;output:"pnpm-workspace.yaml"}
_PREPARE_DEPS_RESOURCES=
_PREPARE_DEPS_USE_RESOURCES_FLAG=
_PREPARE_DEPS_TS_PROTO_AUTO_FLAG=
_PREPARE_DEPS_CMD=$NOTS_TOOL $NOTS_TOOL_BASE_ARGS prepare-deps \
    --tarballs-store $_TARBALLS_STORE \
    $_PREPARE_DEPS_INOUTS \
    $_PREPARE_DEPS_RESOURCES \
    $_PREPARE_DEPS_USE_RESOURCES_FLAG \
    $_PREPARE_DEPS_TS_PROTO_AUTO_FLAG \
    $NOTS_TOOL_REQUIREMENTS__NO_UID__ \
...
macro INJECT_PEERS() {
    ENABLE(_INJECT_PEERS_COMPAT_NOOP)
}

macro USE_LEGACY_LINKED_PEERS() {
    SET(_INJECT_PEERS)
    SET(_INJECT_PEERS_ARG)
    MESSAGE(WARNING "USE_LEGACY_LINKED_PEERS enables the deprecated linked workspace packages mode. Please remove this macro and run 'ya tool nots install' in the module directory to migrate to injected workspace packages.")
}

### @usage: DISABLE_HERMETIC_NODE_MODULES()
###
### Restores the legacy node_modules installation flow for this target.
macro DISABLE_HERMETIC_NODE_MODULES() {
    SET(TS_DISABLE_HERMETIC_NODE_MODULES yes)
}
