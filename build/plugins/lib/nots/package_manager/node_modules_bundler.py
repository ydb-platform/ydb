...
from .utils import build_nm_path, remove_node_modules_volatile_metadata
...
@timeit
def bundle_node_modules(build_root, peers, node_modules_path, bundle_path, inject_peers=False, output_prefix=""):
    """
    ...
    # FBP-1435: dynamically generated values break the content uid.
    remove_node_modules_volatile_metadata(node_modules_path)

    def prefixed(path):
        return posixpath.join(output_prefix, path) if output_prefix else path

    paths_to_pack = []

    if inject_peers:
        paths_to_pack.append((node_modules_path, prefixed("node_modules")))
        archive.tar(set(paths_to_pack), bundle_path, compression_filter=None, compression_level=None, fixed_mtime=0)
        return

    paths_to_pack.append((node_modules_path, prefixed(".")))
    ...
    for p in peers:
        ...
        if not os.path.isdir(peer_nm_path):
            continue
        paths_to_pack.append((peer_nm_path, prefixed(peer_bundled_nm_path)))
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(peers_index_tmppath, "w") as peers_index:
            peers_index.write("\n".join(added_peers))
        paths_to_pack.append((peers_index_tmppath, prefixed(peers_index_relpath)))

        archive.tar(set(paths_to_pack), bundle_path, compression_filter=None, compression_level=None, fixed_mtime=0)


def extract_node_modules(build_root, node_modules_path, bundle_path, output_prefix=""):
    """
    ...
    def unprefix(name):
        ...
        if name.rstrip('/') == output_prefix:
            return ""
        if not name.startswith(output_prefix + '/'):
            raise ValueError(f"Archive entry {name!r} is outside output prefix {output_prefix!r}")
        return name[len(output_prefix) + 1 :]

    def entry_filter(entry):
        entry.pathname = unprefix(entry.pathname)
        if entry.ishardlink():
            entry.hardlink = os.fsencode(unprefix(entry.hardlink))
        return bool(entry.pathname)

    archive.extract_tar(
        bundle_path,
        node_modules_path,
        fail_on_duplicates=False,
        entry_filter=entry_filter if output_prefix else None,
    )
