...
@timeit
def bundle_node_modules(build_root, peers, node_modules_path, bundle_path, inject_peers=False):
    """
    ...
    # FBP-1435: так как варится динамически, то ломает content uid
    # todo: вместо удаление, сделать преобразование, чтобы файлы остались, но были более персистентны
    os.remove(os.path.join(node_modules_path, ".pnpm-workspace-state-v1.json"))
    os.remove(os.path.join(node_modules_path, ".modules.yaml"))

    paths_to_pack = []

    if inject_peers:
        paths_to_pack.append((node_modules_path, "node_modules"))
