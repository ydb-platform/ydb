...
def onyabs_generate_conf(unit, *args):
    flat, kv = sort_by_keywords({'MODE': 1, 'SCRIPT': 1, 'SRC': 1, 'TOOL': 1, 'CONF_DIR': 1, 'DEST': 1}, args)
...
def onyabs_generate_phantom_conf_patch(unit, *args):
    flat, kv = sort_by_keywords({'SRC': 1, 'DST': 1}, args)
    ...
    for f in flat:
        lhs = src + '/' + f
        rhs = dst + '/' + f
        unit.onpython(['mkdiff.py', lhs, rhs, 'IN', lhs, 'IN', rhs, 'STDOUT', f + ".patch"])
