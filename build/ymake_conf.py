    ...
    def __init__(self, tc, build):
        ...
        if self.tc.is_clang or self.tc.is_gcc and self.tc.version_at_least(8, 2):
            target_flags = select(default=[], selectors=[
                ...
                (target.is_linux and target.is_grace and ((self.tc.is_clang and self.tc.version_at_least(16)) or (self.tc.is_gcc and self.tc.version_at_least(13))), ['-mcpu=neoverse-v2+norng+crypto+sve2-sm4+sve2-aes+sve2-sha3']),
                (target.is_linux and target.is_grace, ['-march=armv9-a+crypto+sve2+sve2-bitperm+bf16+i8mm+fp16fml+sve2-aes+sve2-sha3+sve2-sm4+sha3+sm4+crc+dotprod+lse+rcpc+rdm+nossbs+noras+norng', '-mtune=neoverse-n2']),
