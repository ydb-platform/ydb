...
#ifndef DB_Crypto_EVPPKeyImpl_INCLUDED
#define DB_Crypto_EVPPKeyImpl_INCLUDED
...
namespace Crypto
{
    ...
    private:
        ...
        loadKey(K ** ppKey, PEM_read_FILE_Key_fn readFunc, F getFunc, const std::string & keyFile, const std::string & pass = "")
        {
            ...
            DB_poco_check_ptr(ppKey);
            DB_poco_assert_dbg(!*ppKey);
                ...
                if (pKey)
                {
                    ...
                    if (pFile)
                    {
                        ...
                        if (readFunc(pFile, &pKey, pCB, pPassword))
                        {
                            ...
                            {
                                DB_poco_assert_dbg(typeid(K *) == typeid(EVP_PKEY *));
                                *ppKey = (K *)pKey;
                            }
                            if (!*ppKey)
                                goto error;
        ...
        static bool loadKey(K ** ppKey, PEM_read_BIO_Key_fn readFunc, F getFunc, std::istream * pIstr, const std::string & pass = "")
        {
            ...
            if (pIstr)
            {
                ...
                DBPoco::StreamCopier::copyStream(*pIstr, ostr);
    ...
    {
        DB_poco_check_ptr(other._pEVPPKey);
        DB_poco_check_ptr(_pEVPPKey);
...
#endif // DB_Crypto_EVPPKeyImpl_INCLUDED
