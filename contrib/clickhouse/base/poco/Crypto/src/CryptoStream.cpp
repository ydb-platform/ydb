...
CryptoStreamBuf::CryptoStreamBuf(std::istream& istr, CryptoTransform* pTransform, std::streamsize bufferSize):
	DBPoco::BufferedStreamBuf(bufferSize, std::ios::in),
...
{
	DB_poco_check_ptr (pTransform);
	DB_poco_assert (bufferSize > 2 * pTransform->blockSize());
}


CryptoStreamBuf::CryptoStreamBuf(std::ostream& ostr, CryptoTransform* pTransform, std::streamsize bufferSize):
	DBPoco::BufferedStreamBuf(bufferSize, std::ios::out),
	_pTransform(pTransform),
	_pIstr(0),
	_pOstr(&ostr),
	_eof(false),
...
void CryptoStreamBuf::close()
{
	...
	else if (_pOstr)
	{
		...
		if (n > 0)
		{
			...
			if (!pOstr->good())
				throw DBPoco::IOException("Output stream failure");
		}
	}
}
