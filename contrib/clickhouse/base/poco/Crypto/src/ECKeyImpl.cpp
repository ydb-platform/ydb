...
#include "DBPoco/Crypto/ECKeyImpl.h"
	...
	const std::string& privateKeyPassphrase): KeyPairImpl("ec", KT_EC_IMPL), _pEC(0)
{
	...
	{
		checkEC(DBPoco::format("ECKeyImpl(%s, %s, %s)",
	...
	const std::string& privateKeyPassphrase): KeyPairImpl("ec", KT_EC_IMPL), _pEC(0)
{
	...
	{
		checkEC(DBPoco::format("ECKeyImpl(stream, stream, %s)",
...
{
	if (!_pEC) throw OpenSSLException(DBPoco::format("%s: %s", method, func));
	if (!EC_KEY_check_key(_pEC))
		throw OpenSSLException(DBPoco::format("%s: EC_KEY_check_key()", method));
}
