...
void EVPPKey::save(const std::string& publicKeyFile, const std::string& privateKeyFile, const std::string& privateKeyPassphrase) const
{
	if (!publicKeyFile.empty() && (publicKeyFile != privateKeyFile))
	{
		...
		if (!bio) throw DBPoco::IOException("Cannot create BIO for writing public key file", publicKeyFile);
		try
		{
			if (BIO_write_filename(bio, const_cast<char*>(publicKeyFile.c_str())))
			{
				...
				{
					throw DBPoco::WriteFileException("Failed to write public key to file", publicKeyFile);
				}
			}
			else throw DBPoco::CreateFileException("Cannot create public key file");
		}
		catch (...)
		{
			BIO_free(bio);
	...
	{
		BIO* bio = BIO_new(BIO_s_file());
		if (!bio) throw DBPoco::IOException("Cannot create BIO for writing private key file", privateKeyFile);
		try
		{
			if (BIO_write_filename(bio, const_cast<char*>(privateKeyFile.c_str())))
			{
				...
				if (!rc)
					throw DBPoco::FileException("Failed to write private key to file", privateKeyFile);
			}
			else throw DBPoco::CreateFileException("Cannot create private key file", privateKeyFile);
		}
		catch (...)
		{
			BIO_free(bio);
...
void EVPPKey::save(std::ostream* pPublicKeyStream, std::ostream* pPrivateKeyStream, const std::string& privateKeyPassphrase) const
{
	if (pPublicKeyStream && (pPublicKeyStream != pPrivateKeyStream))
	{
		...
		if (!bio) throw DBPoco::IOException("Cannot create BIO for writing public key");
		if (!PEM_write_bio_PUBKEY(bio, _pEVPPKey))
		{
			BIO_free(bio);
			throw DBPoco::WriteFileException("Failed to write public key to stream");
		}
		char* pData;
	...
	if (pPrivateKeyStream)
	{
		...
		{
			BIO_free(bio);
			throw DBPoco::FileException("Failed to write private key to stream");
		}
		char* pData;
		long size = BIO_get_mem_data(bio, &pData);
...
{
	DB_poco_check_ptr(pKey);
	DB_poco_check_ptr(pKey->impl());
