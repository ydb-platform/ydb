...
namespace
{
		...
		std::streamsize		 outputLength)
	{
		...
		DB_poco_assert_dbg(_pos <= maxSize);
		DB_poco_assert (outputLength >= rsaSize);
		int rc = 0;
		while (inputLength > 0)
		{
			// check how many data bytes we are missing to get the buffer full
			DB_poco_assert_dbg (maxSize >= _pos);
	...
	{
		DB_poco_assert (length >= blockSize());
		DB_poco_assert (_pos <= maxDataSize());
		int rc = 0;
		if (_pos > 0)
		{
		...
		std::streamsize		 outputLength)
	{
		...
		std::streamsize rsaSize = static_cast<std::streamsize>(blockSize());
		DB_poco_assert_dbg(_pos <= rsaSize);
		DB_poco_assert (outputLength >= rsaSize);
		int rc = 0;
		while (inputLength > 0)
		{
			// check how many data bytes we are missing to get the buffer full
			DB_poco_assert_dbg (rsaSize >= _pos);
