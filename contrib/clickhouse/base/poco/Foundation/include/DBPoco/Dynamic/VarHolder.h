...
#ifndef DB_Foundation_VarHolder_INCLUDED
#define DB_Foundation_VarHolder_INCLUDED
...
namespace Dynamic
{
    ...
    /// std::string, DBPoco::UTF16String, DateTime, LocalDateTime, Timestamp, std::vector<Var> and DynamicStruct.
    ///
    ...
    public:
        ...
        /// DB_POCO_NO_SOO is not defined), VarHolder will be
        /// instantiated in-place if it's size is smaller
        /// than DB_POCO_SMALL_OBJECT_SIZE.
        ...
        virtual void convert(DBPoco::UTF16String & val) const;
        /// Throws BadCastException. Must be overridden in a type
        /// specialization in order to support the conversion.
    ...
    protected:
        ...
        /// larger than DB_POCO_SMALL_OBJECT_SIZE, holder is instantiated on
        ...
        /// it is converted.
        {
            DB_poco_static_assert(std::numeric_limits<F>::is_specialized);
            DB_poco_static_assert(std::numeric_limits<T>::is_specialized);
            DB_poco_static_assert(std::numeric_limits<F>::is_signed);
            DB_poco_static_assert(std::numeric_limits<T>::is_signed);
        ...
        /// If upper limit is within the target data type limits, the conversion is performed.
        {
            DB_poco_static_assert(std::numeric_limits<F>::is_specialized);
            DB_poco_static_assert(std::numeric_limits<T>::is_specialized);
            DB_poco_static_assert(!std::numeric_limits<F>::is_integer);
            DB_poco_static_assert(std::numeric_limits<T>::is_integer);
            DB_poco_static_assert(!std::numeric_limits<T>::is_signed);
    ...
    inline void VarHolder::convert(DBPoco::UTF16String & /*val*/) const
    {
        throw BadCastException("Can not convert to DBPoco::UTF16String");
    }
    ...
    public:
        ...
        void convert(DBPoco::UTF16String & val) const
        {
            std::string str = NumberFormatter::format(_val);
            DBPoco::UnicodeConverter::convert(str, val);
        }
    ...
    public:
        ...
        void convert(DBPoco::UTF16String & val) const
        {
            DBPoco::UnicodeConverter::convert(_val, val);
        }
    ...
    {
    public:
        VarHolderImpl(const char * pVal) : _val(DBPoco::UnicodeConverter::to<UTF16String>(pVal)) { }

        VarHolderImpl(const DBPoco::UTF16String & val) : _val(val) { }

        ~VarHolderImpl() { }

        const std::type_info & type() const { return typeid(DBPoco::UTF16String); }
        ...
        const DBPoco::UTF16String & value() const
        {
            return _val;
        }
        ...
        UTF16Char & operator[](DBPoco::UTF16String::size_type n)
        {
            if (n < size())
                return _val.operator[](n);
        ...
        const UTF16Char & operator[](DBPoco::UTF16String::size_type n) const
        {
            if (n < size())
                return _val.operator[](n);
...
#else // if defined (DB_POCO_LONG_IS_64_BIT)
    ...
    public:
        ...
        void convert(std::string & val) const
        {
            val = DateTimeFormatter::format(_val, DBPoco::DateTimeFormat::ISO8601_FORMAT);
        }
...
#endif // DB_Foundation_VarHolder_INCLUDED
