...
#ifndef DB_Foundation_FIFOBuffer_INCLUDED
#define DB_Foundation_FIFOBuffer_INCLUDED
...
public:
    ...
    mutable DBPoco::BasicEvent<bool> writable;
    ...
    mutable DBPoco::BasicEvent<bool> readable;
    ...
    std::size_t peek(DBPoco::Buffer<T> & buffer, std::size_t length = 0) const
    /// Peeks into the data currently in the FIFO
    /// without actually extracting it.
    ...
    std::size_t read(DBPoco::Buffer<T> & buffer, std::size_t length = 0)
    /// Copies the data currently in the FIFO
    ...
    /// Returns the length of data written.
    {
        ...
        _used += len;
        DB_poco_assert(_used <= _buffer.size());
        if (_notify)
            notify(usedBefore);
    ...
    /// the used buffer size.
    {
        ...
        if (length > available())
            throw DBPoco::InvalidAccessException("Cannot extend buffer.");

        if (!isWritable())
            throw DBPoco::InvalidAccessException("Buffer not writable.");
...
#endif // DB_Foundation_FIFOBuffer_INCLUDED
