...
#ifndef DB_Foundation_PBKDF2Engine_INCLUDED
#define DB_Foundation_PBKDF2Engine_INCLUDED
...
public:
    ...
    PBKDF2Engine(const std::string & salt, unsigned c = 4096, DBPoco::UInt32 dkLen = PRF_DIGEST_SIZE) : _s(salt), _c(c), _dkLen(dkLen)
    {
...
protected:
    ...
    void f(DBPoco::UInt32 i)
    {
        PRF prf(_p);
        prf.update(_s);
        DBPoco::UInt32 iBE = DBPoco::ByteOrder::toBigEndian(i);
        prf.update(&iBE, sizeof(iBE));
        DBPoco::DigestEngine::Digest up = prf.digest();
        DBPoco::DigestEngine::Digest ux = up;
        DB_poco_assert_dbg(ux.size() == PRF_DIGEST_SIZE);
        for (unsigned k = 1; k < _c; k++)
        {
            prf.reset();
            prf.update(&up[0], up.size());
            DBPoco::DigestEngine::Digest u = prf.digest();
            DB_poco_assert_dbg(u.size() == PRF_DIGEST_SIZE);
...
#endif // DB_Foundation_PBKDF2Engine_INCLUDED
