...
#include "DBPoco/DirectoryIteratorStrategy.h"
...
bool TraverseBase::isDirectory(DBPoco::File& file)
{
	try
	{
...
const std::string ChildrenFirstTraverse::next(Stack* itStack, bool* isFinished)
{
	...
	DB_poco_check_ptr(isFinished);
	DB_poco_assert(!(*isFinished));
