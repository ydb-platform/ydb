...
#include "DBPoco/ArchiveStrategy.h"
#include "DBPoco/RotateStrategy.h"
#include "DBPoco/PurgeStrategy.h"
...
{
	DBPoco::File file(path + ".lz4");
	if (file.exists())
	{
		DBPoco::File::FileSize size = file.getSize();
		if (size > 0)
		{
			bool err = false;
			...
			{
				DBPoco::Buffer<char> buffer(4);
				DBPoco::FileInputStream istr(path + ".lz4");
