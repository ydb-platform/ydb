...
#include "DBPoco/Semaphore_POSIX.h"
#include <time.h>
#include <sys/time.h>
...
//
#ifndef DB_POCO_HAVE_MONOTONIC_PTHREAD_COND_TIMEDWAIT
		...
		#define DB_POCO_HAVE_MONOTONIC_PTHREAD_COND_TIMEDWAIT 1
	#endif
#endif
...
{
	DB_poco_assert (n >= 0 && max > 0 && n <= max);

	if (pthread_mutex_init(&_mutex, NULL))
		throw SystemException("cannot create semaphore (mutex)");

#if defined(DB_POCO_HAVE_MONOTONIC_PTHREAD_COND_TIMEDWAIT)
