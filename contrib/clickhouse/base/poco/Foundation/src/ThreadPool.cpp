...
{
	DB_poco_assert_dbg (stackSize >= 0);
...
ThreadPool::ThreadPool(int minCapacity,
    ...
    _globalProfilerCPUTimePeriodNs(globalProfilerCPUTimePeriodNs_)
{
	DB_poco_assert (minCapacity >= 1 && maxCapacity >= minCapacity && idleTime > 0);
...
void ThreadPool::addCapacity(int n)
{
	...
	DB_poco_assert (_maxCapacity + n >= _minCapacity);
	_maxCapacity += n;
	housekeep();
}
...
public:
	...
	ThreadPool* pool()
	{
		...
		if (!_pPool)
		{
			...
			if (DB_POCO_THREAD_STACK_SIZE > 0)
				_pPool->setStackSize(DB_POCO_THREAD_STACK_SIZE);
		}
		return _pPool;
	}
