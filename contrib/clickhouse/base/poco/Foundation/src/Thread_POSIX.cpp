...
#	endif
#endif
#if DB_POCO_OS == DB_POCO_OS_LINUX || DB_POCO_OS == DB_POCO_OS_ANDROID || DB_POCO_OS == DB_POCO_OS_MAC_OS_X || DB_POCO_OS == DB_POCO_OS_QNX
#	include <time.h>
#endif
...
{
#if (DB_POCO_OS == DB_POCO_OS_MAC_OS_X)
...
{
#if defined(DB_POCO_THREAD_PRIORITY_MIN)
	return DB_POCO_THREAD_PRIORITY_MIN;
#else
...
{
#if defined(DB_POCO_THREAD_PRIORITY_MAX)
	return DB_POCO_THREAD_PRIORITY_MAX;
#else
...
{
#if   DB_POCO_OS == DB_POCO_OS_LINUX || DB_POCO_OS == DB_POCO_OS_ANDROID || DB_POCO_OS == DB_POCO_OS_MAC_OS_X || DB_POCO_OS == DB_POCO_OS_QNX || DB_POCO_OS == DB_POCO_OS_VXWORKS
	DBPoco::Timespan remainingTime(1000*DBPoco::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	{
		struct timespec ts;
		...
		if (rc < 0 && errno == EINTR)
		{
			...
			DBPoco::Timespan waited = start.elapsed();
	...
	if (rc < 0 && remainingTime > 0) throw DBPoco::SystemException("Thread::sleep(): nanosleep() failed");
#else
	DBPoco::Timespan remainingTime(1000*DBPoco::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	...
	while (remainingTime > 0 && rc < 0 && errno == EINTR);
	if (rc < 0 && remainingTime > 0) throw DBPoco::SystemException("Thread::sleep(): select() failed");
#endif
}
...
int ThreadImpl::mapPrio(int prio, int policy)
{
	...
	case PRIO_HIGHEST_IMPL:
		return pmax;
	default:
		DB_poco_bugcheck_msg("invalid thread priority");
	}
