...
const Var Var::operator + (const Var& other) const
{
	...
	{
		if (isSigned())
			return add<DBPoco::Int64>(other);
		else
			return add<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator += (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = add<DBPoco::Int64>(other);
		else
			return *this = add<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator - (const Var& other) const
{
	...
	{
		if (isSigned())
			return subtract<DBPoco::Int64>(other);
		else
			return subtract<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator -= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = subtract<DBPoco::Int64>(other);
		else
			return *this = subtract<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator * (const Var& other) const
{
	...
	{
		if (isSigned())
			return multiply<DBPoco::Int64>(other);
		else
			return multiply<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator *= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = multiply<DBPoco::Int64>(other);
		else
			return *this = multiply<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator / (const Var& other) const
{
	...
	{
		if (isSigned())
			return divide<DBPoco::Int64>(other);
		else
			return divide<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator /= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = divide<DBPoco::Int64>(other);
		else
			return *this = divide<DBPoco::UInt64>(other);
	}
	else if (isNumeric())
		...
		default:
			{
				...
				{
					const DBPoco::Int64 number = NumberParser::parse64(str);
					return number;
				}
				else if (frac == 0 && isNumber && !isSigned)
				{
					const DBPoco::UInt64 number = NumberParser::parseUnsigned64(str);
					return number;
				}
...
{
	DB_poco_assert_dbg (pos < val.size() && val[pos] == '{');
	++pos;
	skipWhiteSpace(val, pos);
...
{
	DB_poco_assert_dbg (pos < val.size());
	if (val[pos] == '"')
	{
		return parseJSONString(val, pos);
	}
...
std::string Var::parseJSONString(const std::string& val, std::string::size_type& pos)
{
	...
	if (!done) throw DBPoco::DataFormatException("unterminated JSON string");
	return result;
}
