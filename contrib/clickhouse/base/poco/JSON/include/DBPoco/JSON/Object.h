...
#ifndef DB_JSON_Object_INCLUDED
#define DB_JSON_Object_INCLUDED
...
#include <sstream>
#include <vector>
#include "DBPoco/Dynamic/Struct.h"
...
namespace JSON
{
    ...
    ///
    ///    // copy/convert to DBPoco::DynamicStruct
    ///    DBPoco::DynamicStruct ds = *object;
    ...
    public:
        ...
        template <typename T>
        DBPoco::Nullable<T> getNullableValue(const std::string & key) const
        /// Retrieves the property with the given name and will
        /// try to convert the value to the given template type.
        ///
        /// The convert<T> method of Var is called
        ...
        static DBPoco::DynamicStruct makeStruct(const Object::Ptr & obj);
        /// Utility function for creation of struct.

        operator const DBPoco::DynamicStruct &() const;
        /// Cast operator to DBPoco::DynamiStruct.
    ...
    private:
        ...
        typedef DBPoco::DynamicStruct::Ptr StructPtr;
    ...
    inline const Dynamic::Var & Object::getValue(KeyList::const_iterator & it) const
    {
        ...
        if (itv != _values.end())
            ...
            throw DBPoco::NotFoundException();
    }
