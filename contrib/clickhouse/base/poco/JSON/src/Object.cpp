...
Object::Object(int options):
	_preserveInsOrder((options & DBPoco::JSON_PRESERVE_KEY_ORDER) != 0),
...
void Object::syncKeys(const KeyList& keys)
{
	if(_preserveInsOrder)
	{
		...
		for(KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it)
		{
			...
			DB_poco_assert (itv != _values.end());
			_keys.push_back(itv);
		}
	}
}
...
DBPoco::DynamicStruct Object::makeStruct(const Object::Ptr& obj)
{
	DBPoco::DynamicStruct ds;
	...
	for (; it != end; ++it)
	{
		...
		else if (obj->isArray(it))
		{
			...
			std::vector<DBPoco::Dynamic::Var> v = DBPoco::JSON::Array::makeArray(pArr);
...
{
	if (!_pStruct)
		_pStruct = new DBPoco::DynamicStruct;
	else
		_pStruct->clear();
}


Object::operator const DBPoco::DynamicStruct& () const
{
	if (!_values.size())
	{
		resetDynStruct();
	...
	else if (_modified)
	{
		...
		for (; it != end; ++it)
		{
			...
			{
				_pStruct->insert(it->first, DBPoco::JSON::Array::makeArray(getArray(it->first)));
			}
			else
			{
