...
#ifndef DB_Net_HTTPCredentials_INCLUDED
#define DB_Net_HTTPCredentials_INCLUDED
...
namespace Net
{
    ...
    ///     DBPoco::Net::HTTPCredentials creds("user", "s3cr3t");
    ///
    /// Second, send the HTTP request with DBPoco::Net::HTTPClientSession.
    ///     DBPoco::Net::HTTPClientSession session("pocoproject.org");
    ///     DBPoco::Net::HTTPRequest request(HTTPRequest::HTTP_GET, "/index.html", HTTPMessage::HTTP_1_1);
    ///     session.sendRequest(request);
    ///     DBPoco::Net::HTTPResponse;
    ///     std::istream& istr = session.receiveResponse(response);
    ///
    /// If the server responds with a 401 status, authenticate the
    /// request and resend it:
    ///     if (response.getStatus() == DBPoco::Net::HTTPResponse::HTTP_UNAUTHORIZED)
    ///     {
    ...
    public:
        ...
        static void extractCredentials(const DBPoco::URI & uri, std::string & username, std::string & password);
...
#endif // DB_Net_HTTPCredentials_INCLUDED
