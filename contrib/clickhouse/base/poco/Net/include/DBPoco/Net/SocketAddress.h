...
#ifndef DB_Net_SocketAddress_INCLUDED
#define DB_Net_SocketAddress_INCLUDED


#include <ostream>
#include "DBPoco/Net/Net.h"
#include "DBPoco/Net/SocketAddressImpl.h"
...
namespace Net
{
    ...
    public:
        ...
        SocketAddress(const IPAddress & hostAddress, DBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.

        explicit SocketAddress(DBPoco::UInt16 port);
        /// Creates a SocketAddress with unspecified (wildcard) IP address
        /// and given port number.

        SocketAddress(Family family, DBPoco::UInt16 port);
        /// Creates a SocketAddress with unspecified (wildcard) IP address
        /// of the given family, and given port number.

        SocketAddress(const std::string & hostAddress, DBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.
        ///
        /// The IP address must either be a domain name, or it must
        /// be in dotted decimal (IPv4) or hex string (IPv6) format.

        SocketAddress(Family family, const std::string & hostAddress, DBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.
        ///
        /// The IP address must either be a domain name, or it must
        /// be in dotted decimal (IPv4) or hex string (IPv6) format.
        ...
        SocketAddress(const struct sockaddr * addr, DB_poco_socklen_t length);
...
#elif defined(DB_POCO_HAVE_IPv6)
    ...
    protected:
        void init(const IPAddress & hostAddress, DBPoco::UInt16 portNumber);
        void init(const std::string & hostAddress, DBPoco::UInt16 portNumber);
        void init(Family family, const std::string & hostAddress, DBPoco::UInt16 portNumber);
        void init(Family family, const std::string & address);
        void init(const std::string & hostAndPort);
        DBPoco::UInt16 resolveService(const std::string & service);

    private:
        typedef DBPoco::Net::Impl::SocketAddressImpl Impl;
        typedef DBPoco::AutoPtr<Impl> Ptr;

        Ptr pImpl() const;

        void newIPv4();
        void newIPv4(const sockaddr_in *);
        void newIPv4(const IPAddress & hostAddress, DBPoco::UInt16 portNumber);

#if defined(DB_POCO_HAVE_IPv6)
        void newIPv6(const sockaddr_in6 *);
        void newIPv6(const IPAddress & hostAddress, DBPoco::UInt16 portNumber);
#endif
    ...
    inline SocketAddress::Ptr SocketAddress::pImpl() const
    {
        if (_pImpl)
            return _pImpl;
        throw DBPoco::NullPointerException("Pointer to SocketAddress implementation is NULL.");
    }


    inline void SocketAddress::newIPv4()
    {
        _pImpl = new DBPoco::Net::Impl::IPv4SocketAddressImpl;
    }


    inline void SocketAddress::newIPv4(const sockaddr_in * sockAddr)
    {
        _pImpl = new DBPoco::Net::Impl::IPv4SocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newIPv4(const IPAddress & hostAddress, DBPoco::UInt16 portNumber)
    {
        _pImpl = new DBPoco::Net::Impl::IPv4SocketAddressImpl(hostAddress.addr(), htons(portNumber));
    }
    ...
    {
        _pImpl = new DBPoco::Net::Impl::IPv6SocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newIPv6(const IPAddress & hostAddress, DBPoco::UInt16 portNumber)
    {
        _pImpl = new DBPoco::Net::Impl::IPv6SocketAddressImpl(hostAddress.addr(), htons(portNumber), hostAddress.scope());
    }
    ...
    {
        _pImpl = new DBPoco::Net::Impl::LocalSocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newLocal(const std::string & path)
    {
        _pImpl = new DBPoco::Net::Impl::LocalSocketAddressImpl(path.c_str(), path.size());
    }
...
Net_API DBPoco::BinaryWriter & operator<<(DBPoco::BinaryWriter & writer, const DBPoco::Net::SocketAddress & value);
Net_API DBPoco::BinaryReader & operator>>(DBPoco::BinaryReader & reader, DBPoco::Net::SocketAddress & value);
Net_API std::ostream & operator<<(std::ostream & ostr, const DBPoco::Net::SocketAddress & address);


#endif // DB_Net_SocketAddress_INCLUDED
