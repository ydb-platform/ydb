...
#ifndef DB_Net_SocketReactor_INCLUDED
#define DB_Net_SocketReactor_INCLUDED
...
namespace Net
{
    ...
    class Net_API SocketReactor : public DBPoco::Runnable
    ...
    public:
        ...
        explicit SocketReactor(const DBPoco::Timespan & timeout);
        ...
        void addEventHandler(const Socket & socket, const DBPoco::AbstractObserver & observer);
        /// Registers an event handler with the SocketReactor.
        ///
        /// Usage:
        ///     DBPoco::Observer<MyEventHandler, SocketNotification> obs(*this, &MyEventHandler::handleMyEvent);
        ///     reactor.addEventHandler(obs);

        bool hasEventHandler(const Socket & socket, const DBPoco::AbstractObserver & observer);
        /// Returns true if the observer is registered with SocketReactor for the given socket.

        void removeEventHandler(const Socket & socket, const DBPoco::AbstractObserver & observer);
    ...
    private:
        typedef DBPoco::AutoPtr<SocketNotifier> NotifierPtr;
        typedef DBPoco::AutoPtr<SocketNotification> NotificationPtr;
...
#endif // DB_Net_SocketReactor_INCLUDED
