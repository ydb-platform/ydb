...
typedef DBPoco::UInt32 punycode_uint;
...
#if defined(POCO_HAVE_LIBRESOLV)
static DBPoco::RWLock resolverLock;
#endif
...
{
#if defined(POCO_HAVE_LIBRESOLV)
	DBPoco::ScopedReadRWLock readLock(resolverLock);
#endif
...
{
#if defined(POCO_HAVE_LIBRESOLV)
	DBPoco::ScopedWriteRWLock writeLock(resolverLock);
	res_init();
#endif
}
...
std::string DNS::encodeIDNLabel(const std::string& label)
{
	...
	std::vector<DBPoco::UInt32> uniLabel;
	DBPoco::UTF8Encoding utf8;
	DBPoco::TextIterator it(label, utf8);
	DBPoco::TextIterator end(label);
	while (it != end)
	{
		int ch = *it;
		if (ch < 0) throw DNSException("Invalid UTF-8 character in IDN label", label);
		if (DBPoco::Unicode::isUpper(ch))
		{
			ch = DBPoco::Unicode::toLower(ch);
		}
		uniLabel.push_back(static_cast<DBPoco::UInt32>(ch));
		++it;
	}
	char buffer[64];
	std::size_t size = 64;
...
std::string DNS::decodeIDNLabel(const std::string& encodedIDN)
{
	...
	if (encodedIDN.compare(0, 4, "xn--") == 0)
	{
		...
		if (rc == punycode_success)
		{
			DBPoco::UTF32Encoding utf32;
			DBPoco::UTF8Encoding utf8;
			DBPoco::TextConverter converter(utf32, utf8);
...
#endif // DB_POCO_HAVE_IPv6 || defined(DB_POCO_HAVE_ADDRINFO)
}
