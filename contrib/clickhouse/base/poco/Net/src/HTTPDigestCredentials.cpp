...
#include "DBPoco/Net/HTTPDigestCredentials.h"
...
namespace
{
	std::string digest(DBPoco::DigestEngine& engine,
					   const std::string& a,
					   const std::string& b,
					   ...
					   const std::string& f = std::string())
	{
		...
		return DBPoco::DigestEngine::digestToHex(engine.digest());
	}

	std::string formatNonceCounter(int counter)
	{
		return DBPoco::NumberFormatter::formatHex(counter, 8);
	}
}
...
DBPoco::FastMutex HTTPDigestCredentials::_nonceMutex;
...
{
	DBPoco::FastMutex::ScopedLock lock(_nonceMutex);
...
void HTTPDigestCredentials::createAuthParams(const HTTPRequest& request, const HTTPAuthenticationParams& responseAuthParams)
{
	...
	{
		DBPoco::StringTokenizer tok(qop, ",", DBPoco::StringTokenizer::TOK_TRIM);
		bool qopSupported = false;
		for (DBPoco::StringTokenizer::Iterator it = tok.begin(); it != tok.end(); ++it)
		{
