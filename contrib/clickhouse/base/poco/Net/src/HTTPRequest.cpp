...
void HTTPRequest::setHost(const std::string& host, DBPoco::UInt16 port)
{
	std::string value;
...
void HTTPRequest::getCookies(NameValueCollection& cookies) const
{
	...
	while (it != end() && DBPoco::icompare(it->first, COOKIE) == 0)
	{
...
void HTTPRequest::read(std::istream& istr)
{
	...
	while (!DBPoco::Ascii::isSpace(ch) && ch != eof && method.length() < MAX_METHOD_LENGTH) { method += (char) ch; ch = istr.get(); }
	if (!DBPoco::Ascii::isSpace(ch)) throw MessageException("HTTP request method invalid or too long");
	while (DBPoco::Ascii::isSpace(ch)) ch = istr.get();
	while (!DBPoco::Ascii::isSpace(ch) && ch != eof && uri.length() < MAX_URI_LENGTH) { uri += (char) ch; ch = istr.get(); }
	if (!DBPoco::Ascii::isSpace(ch)) throw MessageException("HTTP request URI invalid or too long");
	while (DBPoco::Ascii::isSpace(ch)) ch = istr.get();
...
void HTTPRequest::getCredentials(const std::string& header, std::string& scheme, std::string& authInfo) const
{
	...
	if (has(header))
	{
		...
		while (it != end && !DBPoco::Ascii::isSpace(*it)) scheme += *it++;
		while (it != end && DBPoco::Ascii::isSpace(*it)) ++it;
