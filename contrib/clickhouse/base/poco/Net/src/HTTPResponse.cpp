...
void HTTPResponse::setDate(const DBPoco::Timestamp& dateTime)
{
	set(DATE, DateTimeFormatter::format(dateTime, DateTimeFormat::HTTP_FORMAT));
}

	
DBPoco::Timestamp HTTPResponse::getDate() const
{
...
void HTTPResponse::getCookies(std::vector<HTTPCookie>& cookies) const
{
	...
	while (it != end() && DBPoco::icompare(it->first, SET_COOKIE) == 0)
	{
...
void HTTPResponse::read(std::istream& istr)
{
	...
	while (DBPoco::Ascii::isSpace(ch)) ch = istr.get();
	if (ch == eof) throw MessageException("No HTTP response header");
	while (!DBPoco::Ascii::isSpace(ch) && ch != eof && version.length() < MAX_VERSION_LENGTH) { version += (char) ch; ch = istr.get(); }
	if (!DBPoco::Ascii::isSpace(ch)) throw MessageException("Invalid HTTP version string");
	while (DBPoco::Ascii::isSpace(ch)) ch = istr.get();
	while (!DBPoco::Ascii::isSpace(ch) && ch != eof && status.length() < MAX_STATUS_LENGTH) { status += (char) ch; ch = istr.get(); }
	if (!DBPoco::Ascii::isSpace(ch)) throw MessageException("Invalid HTTP status code");
	while (DBPoco::Ascii::isSpace(ch) && ch != '\r' && ch != '\n' && ch != eof) ch = istr.get();
	while (ch != '\r' && ch != '\n' && ch != eof && reason.length() < MAX_REASON_LENGTH) { reason += (char) ch; ch = istr.get(); }
	if (!DBPoco::Ascii::isSpace(ch)) throw MessageException("HTTP reason string too long");
	if (ch == '\r') ch = istr.get();
