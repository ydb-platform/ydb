...
std::ostream& operator << (std::ostream& os, const DBPoco::Net::NetworkInterface::MACAddress& mac)
{
	std::ios state(0);
	state.copyfmt(os);
	for (unsigned i = 0; i < mac.size(); ++i)
	{
		if (i > 0) os << DBPoco::Net::NetworkInterface::MAC_SEPARATOR;
...
class NetworkInterfaceImpl: public DBPoco::RefCountedObject
{
public:
...
{
#if !defined(DB_POCO_OS_FAMILY_WINDOWS) && !defined(POCO_VXWORKS)
	struct ifreq ifr;
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
	...
	else throw NotFoundException(DBPoco::format("No address with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::subnetMask(unsigned index) const
{
	...
	throw NotFoundException(DBPoco::format("No subnet mask with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::destAddress(unsigned index) const
{
	...
	throw NotFoundException(DBPoco::format("No address with index %u.", index));
}
...
#if   defined(DB_POCO_OS_FAMILY_BSD) || (DB_POCO_OS == DB_POCO_OS_QNX) || (DB_POCO_OS == DB_POCO_OS_SOLARIS)
//
...
#elif DB_POCO_OS == DB_POCO_OS_LINUX || DB_POCO_OS == DB_POCO_OS_ANDROID
//
// Linux
//
...
/// #include <sys/types.h>
#if DB_POCO_OS != DB_POCO_OS_ANDROID // Android doesn't have <ifaddrs.h>
/// #include <ifaddrs.h>
#endif
/// #include <net/if.h>
...
void setInterfaceParams(struct ifaddrs* iface, NetworkInterfaceImpl& impl)
{
	...
	if (addrStream.good())
	{
		...
		DBPoco::StringTokenizer tok(addr, ":");
		std::vector<unsigned char> mac;
		for (DBPoco::StringTokenizer::Iterator it = tok.begin(); it != tok.end(); ++it)
		{
			mac.push_back(static_cast<unsigned char>(DBPoco::NumberParser::parseHex(*it)));
		}
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
	...
	return result;
#else
	throw DBPoco::NotImplementedException("Not implemented in Android");
#endif
}
