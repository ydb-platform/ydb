...
#include "DBPoco/Net/POP3ClientSession.h"
#include "DBPoco/Net/MailMessage.h"
#include "DBPoco/Net/MailStream.h"
...
class DialogStreamBuf: public DBPoco::UnbufferedStreamBuf
{
public:
...
POP3ClientSession::POP3ClientSession(const std::string& host, DBPoco::UInt16 port):	
...
void POP3ClientSession::setTimeout(const DBPoco::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}

	
DBPoco::Timespan POP3ClientSession::getTimeout() const
{
...
int POP3ClientSession::messageCount()
{
	...
	while (it != end && DBPoco::Ascii::isSpace(*it)) ++it;
	while (it != end && DBPoco::Ascii::isDigit(*it)) count = count*10 + *it++ - '0';
	return count;
}
...
void POP3ClientSession::listMessages(MessageInfoVec& messages)
{
	...
	while (response != ".")
	{
		...
		std::string::const_iterator it  = response.begin();
		std::string::const_iterator end = response.end();
		while (it != end && DBPoco::Ascii::isDigit(*it)) info.id = info.id*10 + *it++ - '0';
		while (it != end && DBPoco::Ascii::isSpace(*it)) ++it;
		while (it != end && DBPoco::Ascii::isDigit(*it)) info.size = info.size*10 + *it++ - '0';
