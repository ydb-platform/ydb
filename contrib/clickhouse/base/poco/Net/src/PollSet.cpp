...
public:
	...
	void add(const Socket& socket, int mode)
	{
		...
		DB_poco_socket_t fd = sockImpl->sockfd();
	...
	void remove(const Socket& socket)
	{
		...
		DB_poco_socket_t fd = socket.impl()->sockfd();
		struct epoll_event ev;
		ev.events = 0;
		ev.data.ptr = 0;
	...
	PollSet::SocketModeMap poll(const DBPoco::Timespan& timeout)
	{
...
public:
	...
	PollSet::SocketModeMap poll(const DBPoco::Timespan& timeout)
	{
		PollSet::SocketModeMap result;
		{
			...
			for (std::map<DB_poco_socket_t, int>::iterator it = _addMap.begin(); it != _addMap.end(); ++it)
			{
				pollfd pfd;
				pfd.fd = it->first;
				pfd.events = 0;
...

		{
			...
			if (!_socketMap.empty())
			{
				for (std::vector<pollfd>::iterator it = _pollfds.begin(); it != _pollfds.end(); ++it)
				{
					std::map<DB_poco_socket_t, Socket>::const_iterator its = _socketMap.find(it->fd);
...
private:
	DBPoco::FastMutex _mutex;
	std::map<DB_poco_socket_t, Socket> _socketMap;
	std::map<DB_poco_socket_t, int> _addMap;
	std::set<DB_poco_socket_t> _removeSet;
...
public:
	...
	PollSet::SocketModeMap poll(const DBPoco::Timespan& timeout)
	{
...

		{
			...
			{
				DB_poco_socket_t fd = it->first.impl()->sockfd();
				if (fd != DB_POCO_INVALID_SOCKET && it->second)
				{
...
PollSet::SocketModeMap PollSet::poll(const DBPoco::Timespan& timeout)
{
