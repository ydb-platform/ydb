...
void RemoteSyslogChannel::log(const Message& msg)
{
	...
	m.reserve(1024);
	m += '<';
	DBPoco::NumberFormatter::append(m, getPrio(msg) + _facility);
	m += '>';
	if (_bsdFormat)
	{
		DBPoco::DateTimeFormatter::append(m, msg.getTime(), BSD_TIMEFORMAT);
		m += ' ';
		m += _host;
	}
	else
	{
		m += "1 "; // version
		DBPoco::DateTimeFormatter::append(m, msg.getTime(), SYSLOG_TIMEFORMAT);
		m += ' ';
		m += _host;
		m += ' ';
		m += _name;
		m += ' ';
		DBPoco::NumberFormatter::append(m, msg.getPid());
		m += ' ';
		m += msg.getSource();
		m += ' ';
...
void RemoteSyslogChannel::setProperty(const std::string& name, const std::string& value)
{
	...
	{
		std::string facility;
		if (DBPoco::icompare(value, 4, "LOG_") == 0)
			facility = DBPoco::toUpper(value.substr(4));
		else if (DBPoco::icompare(value, 4, "SYSLOG_") == 0)
			facility = DBPoco::toUpper(value.substr(7));
		else
			facility = DBPoco::toUpper(value);
...
{
	DBPoco::LoggingFactory::defaultFactory().registerChannelClass("RemoteSyslogChannel", new DBPoco::Instantiator<RemoteSyslogChannel, DBPoco::Channel>);
}
