...
using DBPoco::DigestEngine;
using DBPoco::HMACEngine;
using DBPoco::MD5Engine;
using DBPoco::SHA1Engine;
using DBPoco::DigestOutputStream;
...
SMTPClientSession::SMTPClientSession(const std::string& host, DBPoco::UInt16 port):
...
void SMTPClientSession::setTimeout(const DBPoco::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}

	
DBPoco::Timespan SMTPClientSession::getTimeout() const
{
...
void SMTPClientSession::loginUsingCRAM(const std::string& username, const std::string& method, DBPoco::DigestEngine& hmac)
{
	std::string response;
...
void SMTPClientSession::loginUsingLogin(const std::string& username, const std::string& password)
{
	...
	if (DBPoco::icompare(decodedResponse, 0, 8, "username") == 0) // username first (md5("Username:"))
	{
	...
	else if  (DBPoco::icompare(decodedResponse, 0, 8, "password") == 0) // password first (md5("Password:"))
	{
		status = sendCommand(passwordBase64.str(), response);
