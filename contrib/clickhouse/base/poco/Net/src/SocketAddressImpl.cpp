...
{
	DB_poco_assert (std::strlen(path) < sizeof(_pAddr->sun_path));

	_pAddr = new sockaddr_un;
	DB_poco_set_sun_len(_pAddr, std::strlen(path) + sizeof(struct sockaddr_un) - sizeof(_pAddr->sun_path) + 1);
...
{
	DB_poco_assert (length < sizeof(_pAddr->sun_path));

	_pAddr = new sockaddr_un;
	DB_poco_set_sun_len(_pAddr, length + sizeof(struct sockaddr_un) - sizeof(_pAddr->sun_path) + 1);
	_pAddr->sun_family = AF_UNIX;
