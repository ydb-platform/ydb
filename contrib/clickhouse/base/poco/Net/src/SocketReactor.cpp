...
#include "DBPoco/Net/SocketReactor.h"
#include "DBPoco/Net/SocketNotification.h"
#include "DBPoco/Net/SocketNotifier.h"
...
SocketReactor::SocketReactor(const DBPoco::Timespan& timeout):
	_stop(false),
	_timeout(timeout),
	_pReadableNotification(new ReadableNotification(this)),
	_pWritableNotification(new WritableNotification(this)),
...
void SocketReactor::setTimeout(const DBPoco::Timespan& timeout)
{
	_timeout = timeout;
}


const DBPoco::Timespan& SocketReactor::getTimeout() const
{
	return _timeout;
}


void SocketReactor::addEventHandler(const Socket& socket, const DBPoco::AbstractObserver& observer)
{
...
bool SocketReactor::hasEventHandler(const Socket& socket, const DBPoco::AbstractObserver& observer)
{
	NotifierPtr pNotifier;
	{
		FastMutex::ScopedLock lock(_mutex);
...
void SocketReactor::removeEventHandler(const Socket& socket, const DBPoco::AbstractObserver& observer)
{
	NotifierPtr pNotifier;
	{
		FastMutex::ScopedLock lock(_mutex);
