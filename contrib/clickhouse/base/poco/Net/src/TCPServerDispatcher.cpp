...
TCPServerDispatcher::TCPServerDispatcher(TCPServerConnectionFactory::Ptr pFactory, DBPoco::ThreadPool& threadPool, TCPServerParams::Ptr pParams):
    _rc(1),
    _pParams(pParams),
    _currentThreads(0),
    _totalConnections(0),
...
void TCPServerDispatcher::run()
{
    ...
    for (;;)
    {
            ...
            if (pNf && !_stopped)
            {
                ...
                if (pCNf)
                {
                    ...
                    if (!_stopped)
                    {
                        ...
                        DB_poco_check_ptr(pConnection.get());
                        pConnection->start();
                    }
        ...
        catch (DBPoco::Exception &exc) { ErrorHandler::handle(exc); }
