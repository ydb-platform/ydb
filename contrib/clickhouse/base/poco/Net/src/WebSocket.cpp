...
void WebSocket::shutdown(DBPoco::UInt16 statusCode, const std::string& statusMessage)
{
	DBPoco::Buffer<char> buffer(statusMessage.size() + 2);
	DBPoco::MemoryOutputStream ostr(buffer.begin(), buffer.size());
...
int WebSocket::receiveFrame(DBPoco::Buffer<char>& buffer, int& flags)
{
...
WebSocketImpl* WebSocket::connect(HTTPClientSession& cs, HTTPRequest& request, HTTPResponse& response, HTTPCredentials& credentials)
{
	...
	else if (response.getStatus() == HTTPResponse::HTTP_UNAUTHORIZED)
	{
		if (!credentials.empty())
		{
			...
			DBPoco::StreamCopier::copyStream(istr, null);
...
WebSocketImpl* WebSocket::completeHandshake(HTTPClientSession& cs, HTTPResponse& response, const std::string& key)
{
	...
	if (DBPoco::icompare(connection, "Upgrade") != 0)
	...
	if (DBPoco::icompare(upgrade, "websocket") != 0)
...
std::string WebSocket::createKey()
{
	...
	DBPoco::Base64Encoder base64(ostr);
	DBPoco::BinaryWriter writer(base64);
...
std::string WebSocket::computeAccept(const std::string& key)
{
	...
	DBPoco::SHA1Engine sha1;
	sha1.update(accept);
	DBPoco::DigestEngine::Digest d = sha1.digest();
	std::ostringstream ostr;
	DBPoco::Base64Encoder base64(ostr);
