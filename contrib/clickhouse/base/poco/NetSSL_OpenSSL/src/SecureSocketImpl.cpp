...
#define DB_POCO_BIO_set_nbio_accept(b,n) BIO_ctrl(b,BIO_C_SET_ACCEPT,1,(void*)((n)?"a":NULL))
...
{
	explicit RemainingTimeCounter(DBPoco::Timespan& remainingTime_) : remainingTime(remainingTime_) {};
...
private:
	DBPoco::Timespan& remainingTime;
	DBPoco::Timestamp start;
};

SecureSocketImpl::SecureSocketImpl(DBPoco::AutoPtr<SocketImpl> pSocketImpl, Context::Ptr pContext):
	_pSSL(nullptr),
	_pSocket(pSocketImpl),
	_pContext(pContext),
	_needHandshake(false)
{
	DB_poco_check_ptr (_pSocket);
	DB_poco_check_ptr (_pContext);
}
...
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	DB_poco_assert (!_pSSL);

	StreamSocket ss = _pSocket->acceptConnection(clientAddr);
	DBPoco::AutoPtr<SecureStreamSocketImpl> pSecureStreamSocketImpl = new SecureStreamSocketImpl(static_cast<StreamSocketImpl*>(ss.impl()), _pContext);
...
void SecureSocketImpl::connect(const SocketAddress& address, const DBPoco::Timespan& timeout, bool performHandshake)
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	if (_pSSL) reset();
	...
	DBPoco::Timespan receiveTimeout = _pSocket->getReceiveTimeout();
	DBPoco::Timespan sendTimeout = _pSocket->getSendTimeout();
...
void SecureSocketImpl::connectSSL(bool performHandshake)
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	DB_poco_assert (!_pSSL);
	DB_poco_assert (_pSocket->initialized());
	...
	try
	{
		...
		{
			int ret;
			DBPoco::Timespan remaining_time = getMaxTimeoutOrLimit();
			do
			{
...
int SecureSocketImpl::sendBytes(const void* buffer, int length, int flags)
{
	...
	if (rc <= 0)
	{
		...
		if (SSL_get_error(_pSSL, rc) == SSL_ERROR_SYSCALL && SocketImpl::lastError() == DB_POCO_ECONNRESET)
		{
			char c = 0;
			SSL_read(_pSSL, &c, 1);
		}
...
DBPoco::Timespan SecureSocketImpl::getMaxTimeoutOrLimit()
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	DBPoco::Timespan remaining_time = _pSocket->getReceiveTimeout();
	DBPoco::Timespan send_timeout = _pSocket->getSendTimeout();
	...
	///
	/// NOTE: we cannot use INT64_MAX (std::numeric_limits<DBPoco::Timespan::TimeDiff>::max()),
	...
	if (remaining_time == 0)
		remaining_time = DBPoco::Timespan(std::numeric_limits<int>::max());
	return remaining_time;
}

bool SecureSocketImpl::mustRetry(int rc, DBPoco::Timespan& remaining_time)
{
	if (remaining_time == 0)
		return false;
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	if (rc <= 0)
	{
		...
		case SSL_ERROR_WANT_READ:
			if (_pSocket->getBlocking())
			{
				if (_pSocket->pollImpl(remaining_time, DBPoco::Net::Socket::SELECT_READ))
					return true;
				else
					throw DBPoco::TimeoutException();
			}
			break;
		case SSL_ERROR_WANT_WRITE:
			if (_pSocket->getBlocking())
			{
				if (_pSocket->pollImpl(remaining_time, DBPoco::Net::Socket::SELECT_WRITE))
					return true;
				else
					throw DBPoco::TimeoutException();
			}
			break;
		/// NOTE: DB_POCO_EINTR is the same as SSL_ERROR_WANT_READ (at least in
		/// OpenSSL), so this likely dead code, but let's leave it for
		/// compatibility with other implementations
		case SSL_ERROR_SYSCALL:
			return socketError == DB_POCO_EAGAIN || socketError == DB_POCO_EINTR;
		default:
			return socketError == DB_POCO_EINTR;
		}
	}
	return false;
}
	...
	default:
		{
			...
			if (lastError == 0)
			{
				...
				{
					SecureStreamSocketImpl::error(DBPoco::format("The BIO reported an error: %d", rc));
				}
			}
			else
			{
