...
{
	DBPoco::FileOutputStream ostr(path);
	if (ostr.good())
	{
		DBPoco::OutputLineEndingConverter lec(ostr);
		save(lec);
		lec.flush();
		ostr.flush();
		if (!ostr.good()) throw DBPoco::WriteFileException(path);
	}
	else throw DBPoco::CreateFileException(path);
}
