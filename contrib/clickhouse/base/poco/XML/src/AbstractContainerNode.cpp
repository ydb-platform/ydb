...
#include "DBPoco/DOM/ElementsByTagNameList.h"
...
Node* AbstractContainerNode::replaceChild(Node* newChild, Node* oldChild)
{
	...
	else
	{
		...
		else
		{
			...
			if (pCur)
			{
				DB_poco_assert_dbg (pCur->_pNext == oldChild);
	...
	if (pNode && it != end)
	{
		if (*it == '[')
		{
			...
			else
			{
				...
				std::string index;
				DBPoco::UnicodeConverter::convert(xmlIndex, index);
				int i = DBPoco::NumberParser::parse(index);
#else
				int i = DBPoco::NumberParser::parse(xmlIndex);
#endif
				indexBound = true;
				bool ib;
