...
#include "DBPoco/SAX/DeclHandler.h"
...
void ParserEngine::setBillionLaughsAttackProtectionActivationThreshold(DBPoco::UInt64 activationThresholdBytes)
{
...
int ParserEngine::handleUnknownEncoding(void* encodingHandlerData, const XML_Char* name, XML_Encoding* info)
{
	...
	if (it != pThis->_encodings.end())
		...
		knownEncoding = DBPoco::TextEncoding::find(fromXMLString(encoding));
