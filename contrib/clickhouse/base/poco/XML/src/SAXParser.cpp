...
#include "DBPoco/SAX/SAXParser.h"
...
void SAXParser::addEncoding(const XMLString& name, DBPoco::TextEncoding* pEncoding)
{
...
void SAXParser::setProperty(const XMLString& propertyId, const XMLString& value)
{
	if (propertyId == XMLReader::PROPERTY_DECLARATION_HANDLER || propertyId == XMLReader::PROPERTY_LEXICAL_HANDLER)
		...
		_engine.setBillionLaughsAttackProtectionMaximumAmplification(static_cast<float>(DBPoco::NumberParser::parseFloat(value)));
	else if (propertyId == PROPERTY_BLA_ACTIVATION_THRESHOLD)
		_engine.setBillionLaughsAttackProtectionActivationThreshold(DBPoco::NumberParser::parseUnsigned64(value));
	else
