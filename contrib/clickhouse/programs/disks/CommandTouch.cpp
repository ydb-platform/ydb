...
public:
    ...
    void executeImpl(const CommandLineOptions & options, DisksClient & client) override
    {
        ...
        {
            LOG_WARNING(&DBPoco::Logger::get("CommandTouch"), "File already exists at path: {}", relative_path);
        }
        else if (disk_impl->existsDirectory(relative_path))
        {
            LOG_WARNING(&DBPoco::Logger::get("CommandTouch"), "Directory already exists at path: {}", relative_path);
        }
        else
        {
            LOG_INFO(&DBPoco::Logger::get("CommandTouch"), "Creating file at path: {}", relative_path);
