...
bool DisksApp::processQueryText(const String & text)
{
    ...
    for (const auto & subquery : subqueries)
    {
        ...
        if (error_string.has_value())
        {
            ...
            LOG_ERROR(&DBPoco::Logger::root(), "{}", error_string.value());
        }
        command = nullptr;
    }
...
int DisksApp::main(const std::vector<String> & /*args*/)
{
    ...
    if (config().has("save-logs"))
    {
        ...
        DBPoco::Logger::root().setLevel(DBPoco::Logger::parseLevel(log_level));
        ...
        DBPoco::AutoPtr<OwnFormattingChannel> log = new OwnFormattingChannel(pf, new DBPoco::FileChannel(log_path));
