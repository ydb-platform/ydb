...
void DiskWithPath::setPath(const String & any_path)
{
    if (isDirectory(any_path))
    {
        LOG_INFO(&DBPoco::Logger::get("DisksClient"), "Setting path to '{}' at disk '{}'", any_path, disk->getName());
...
DisksClient::DisksClient(const DBPoco::Util::AbstractConfiguration & config_, ContextPtr context_)
