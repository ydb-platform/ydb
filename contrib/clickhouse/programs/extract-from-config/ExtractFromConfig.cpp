...
{
    DBPoco::AutoPtr<DBPoco::ConsoleChannel> channel(new DBPoco::ConsoleChannel);
    DBPoco::AutoPtr<DBPoco::PatternFormatter> formatter(new DBPoco::PatternFormatter);
    formatter->setProperty("pattern", "%L%Y-%m-%d %H:%M:%S.%i <%p> %s: %t");
    DBPoco::AutoPtr<DBPoco::FormattingChannel> formatting_channel(new DBPoco::FormattingChannel(formatter, channel));
    DBPoco::Logger::root().setChannel(formatting_channel);
    DBPoco::Logger::root().setLevel(log_level);
}
...
static DB::ConfigurationPtr get_configuration(const std::string & config_path, bool process_zk_includes)
{
    ...
    {
        DB::ConfigurationPtr bootstrap_configuration(new DBPoco::Util::XMLConfiguration(config_xml));
    ...
    return DB::ConfigurationPtr(new DBPoco::Util::XMLConfiguration(config_xml));
}
