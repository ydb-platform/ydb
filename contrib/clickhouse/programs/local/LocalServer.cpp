...
#   error #include <azure/storage/common/internal/xml_wrapper.hpp>
#endif
...
DBPoco::Util::LayeredConfiguration & LocalServer::getClientConfiguration()
{
    return config();
}
...
void LocalServer::initialize(DBPoco::Util::Application & self)
{
    DBPoco::Util::Application::initialize(self);
...
static ConfigurationPtr getConfigurationFromXMLString(const char * xml_data)
{
    ...
    DBPoco::XML::InputSource input_source{ss};
    return {new DBPoco::Util::XMLConfiguration{&input_source}};
}
...
try
{
    ...
    /// Try to increase limit on number of open files.
    {
        ...
        if (getrlimit(RLIMIT_NOFILE, &rlim))
            throw DBPoco::Exception("Cannot getrlimit");
    ...
    if (!is_interactive)
    {
        ...
        DBPoco::ErrorHandler::set(&error_handler);
    }
...
void LocalServer::processConfig()
{
    ...
    {
        auto poco_logs_level = DBPoco::Logger::parseLevel(level);
        DBPoco::Logger::root().setLevel(poco_logs_level);
        DBPoco::AutoPtr<OwnPatternFormatter> pf = new OwnPatternFormatter;
        DBPoco::AutoPtr<OwnFormattingChannel> log = new OwnFormattingChannel(pf, new DBPoco::SimpleFileChannel(server_logs_file));
        DBPoco::Logger::root().setChannel(log);
    }
    else
    {
