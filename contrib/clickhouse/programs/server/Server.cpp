...
#    error #include <Server/KeeperTCPHandlerFactory.h>
#endif

#if USE_AZURE_BLOB_STORAGE
#   error #include <azure/storage/common/internal/xml_wrapper.hpp>
#   error #include <azure/core/diagnostics/logger.hpp>
#endif
...
DBPoco::Net::SocketAddress Server::socketBindListen(
...
Strings getListenHosts(const DBPoco::Util::AbstractConfiguration & config)
{
...
Strings getInterserverListenHosts(const DBPoco::Util::AbstractConfiguration & config)
{
...
bool getListenTry(const DBPoco::Util::AbstractConfiguration & config)
{
    bool listen_try = config.getBool("listen_try", false);
    if (!listen_try)
    {
        DBPoco::Util::AbstractConfiguration::Keys protocols;
...
int Server::run()
{
    ...
    {
        DBPoco::Util::HelpFormatter help_formatter(Server::options());
...
void Server::initialize(DBPoco::Util::Application & self)
{
    ...
    LOG_INFO(&logger(), "OS name: {}, version: {}, architecture: {}",
        DBPoco::Environment::osName(),
        DBPoco::Environment::osVersion(),
        DBPoco::Environment::osArchitecture());
}
...
void Server::defineOptions(DBPoco::Util::OptionSet & options)
{
    options.addOption(
        DBPoco::Util::Option("help", "h", "show help and exit")
            .required(false)
            .repeatable(false)
            .binding("help"));
    options.addOption(
        DBPoco::Util::Option("version", "V", "show version and exit")
            .required(false)
            .repeatable(false)
            .binding("version"));
...
void sanityChecks(Server & server)
{
    ...
    if (server.logger().is(DBPoco::Message::PRIO_TEST))
...
void loadStartupScripts(const DBPoco::Util::AbstractConfiguration & config, ContextMutablePtr context, DBPoco::Logger * log)
{
    try
    {
    ...
    [[ maybe_unused ]] int server_logs_level)
{
    ...
    static const std::unordered_map<AzureLogsLevel, std::pair<DBPoco::Message::Priority, DB::LogsLevel>> azure_to_server_mapping =
    {
        {AzureLogsLevel::Error, {DBPoco::Message::PRIO_DEBUG, LogsLevel::debug}},
        {AzureLogsLevel::Warning, {DBPoco::Message::PRIO_DEBUG, LogsLevel::debug}},
        {AzureLogsLevel::Informational, {DBPoco::Message::PRIO_TRACE, LogsLevel::trace}},
        {AzureLogsLevel::Verbose, {DBPoco::Message::PRIO_TEST, LogsLevel::test}},
    };

    static const std::map<DBPoco::Message::Priority, AzureLogsLevel> server_to_azure_mapping =
    {
        {DBPoco::Message::PRIO_DEBUG, AzureLogsLevel::Warning},
        {DBPoco::Message::PRIO_TRACE, AzureLogsLevel::Informational},
        {DBPoco::Message::PRIO_TEST, AzureLogsLevel::Verbose},
    };

    static const LoggerPtr azure_sdk_logger = getLogger("AzureSDK");

    auto it = server_to_azure_mapping.lower_bound(static_cast<DBPoco::Message::Priority>(server_logs_level));
...
#undef COLLECT_FLAG
    ...
    zkutil::EventPtr main_config_zk_changed_event = std::make_shared<DBPoco::Event>();
    ...
    if (config().has("keeper_server.server_id"))
    {
        ...
        auto config_getter = [this] () -> const DBPoco::Util::AbstractConfiguration &
        {
        ...
        for (const auto & listen_host : listen_hosts)
        {
            ...
            createServer(
                ...
                [&](UInt16 port) -> ProtocolServerAdapter
                {
                    ...
                    socket.setReceiveTimeout(DBPoco::Timespan(config().getUInt64("keeper_server.socket_receive_timeout_sec", DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC), 0));
                    socket.setSendTimeout(DBPoco::Timespan(config().getUInt64("keeper_server.socket_send_timeout_sec", DBMS_DEFAULT_SEND_TIMEOUT_SEC), 0));
            ...
            createServer(
                ...
                {
#if USE_SSL
                    DBPoco::Net::SecureServerSocket socket;
            ...
            [&](UInt16 port) -> ProtocolServerAdapter
            {
                ...
                DBPoco::Timespan keep_alive_timeout(config().getUInt("keep_alive_timeout", 10), 0);
                DBPoco::Net::HTTPServerParams::Ptr http_params = new DBPoco::Net::HTTPServerParams;
            ...
            createServer(config, host, port_name.c_str(), listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter
            {
                ...
                return ProtocolServerAdapter(
                    ...
                    std::make_unique<TCPServer>(
                        ...
                        server_pool,
                        socket,
                        new DBPoco::Net::TCPServerParams));
            });
        }
    }
    ...
    for (const auto & listen_host : listen_hosts)
    {
        ...
        if (server_type.shouldStart(ServerType::Type::MYSQL))
        {
            ...
            createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter
            {
                DBPoco::Net::ServerSocket socket;
                auto address = socketBindListen(config, socket, listen_host, port, /* secure = */ true);
                socket.setReceiveTimeout(DBPoco::Timespan());
                socket.setSendTimeout(settings[Setting::send_timeout]);
                return ProtocolServerAdapter(
                    listen_host,
                    port_name,
                    "MySQL compatibility protocol: " + address.toString(),
                    std::make_unique<TCPServer>(new MySQLHandlerFactory(*this, ProfileEvents::InterfaceMySQLReceiveBytes, ProfileEvents::InterfaceMySQLSendBytes), server_pool, socket, new DBPoco::Net::TCPServerParams));
            });
        }
...
#if USE_SSL
                    std::make_unique<TCPServer>(new PostgreSQLHandlerFactory(*this, DBPoco::Net::SSLManager::CFG_SERVER_PREFIX, ProfileEvents::InterfacePostgreSQLReceiveBytes, ProfileEvents::InterfacePostgreSQLSendBytes), server_pool, socket, new DBPoco::Net::TCPServerParams));
#else
                    std::make_unique<TCPServer>(new PostgreSQLHandlerFactory(*this, ProfileEvents::InterfacePostgreSQLReceiveBytes, ProfileEvents::InterfacePostgreSQLSendBytes), server_pool, socket, new DBPoco::Net::TCPServerParams));
#endif
            });
        }
        ...
        if (server_type.shouldStart(ServerType::Type::GRPC))
        {
            ...
            createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter
            {
                DBPoco::Net::SocketAddress server_address(listen_host, port);
                return ProtocolServerAdapter(
                    listen_host,
                    port_name,
    ...
    std::vector<ProtocolServerAdapter> & servers_to_start_before_tables)
{
    ...
    DBPoco::Util::AbstractConfiguration & previous_config = latest_config ? *latest_config : this->config();
