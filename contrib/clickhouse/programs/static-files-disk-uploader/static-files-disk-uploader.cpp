...
static void processFile(const fs::path & file_path, const fs::path & dst_path, bool test_mode, bool link, WriteBuffer & metadata_buf)
{
    ...
    else
    {
        ...
        if (test_mode)
            dst_buf = std::make_shared<WriteBufferFromHTTP>(HTTPConnectionGroupType::HTTP, DBPoco::URI(dst_file_path), DBPoco::Net::HTTPRequest::HTTP_PUT);
        else
...
static void processTableFiles(const fs::path & data_path, fs::path dst_path, bool test_mode, bool link)
{
    ...
    if (test_mode)
    {
        ...
        root_meta = std::make_shared<WriteBufferFromHTTP>(HTTPConnectionGroupType::HTTP, DBPoco::URI(files_root / ".index"), DBPoco::Net::HTTPRequest::HTTP_PUT);
    }
    else
    {
    ...
    for (fs::directory_iterator dir_it(data_path); dir_it != dir_end; ++dir_it)
    {
        if (dir_it->is_directory())
        {
            ...
            if (test_mode)
            {
                directory_meta = std::make_shared<WriteBufferFromHTTP>(HTTPConnectionGroupType::HTTP, DBPoco::URI(dst_path / directory_prefix / ".index"), DBPoco::Net::HTTPRequest::HTTP_PUT);
            }
            else
            {
                dst_path = fs::canonical(dst_path);
