...
AuthResult AccessControl::authenticate(const Credentials & credentials, const DBPoco::Net::IPAddress & address, const ClientInfo & client_info) const
{
    ...
    const String & user_name, const DBPoco::Net::IPAddress & address, const std::string & forwarded_address) const
{
    ...
    if (user_id && (user = tryRead<User>(*user_id)))
    {
        ...
        return quota_cache->getEnabledQuota(*user_id,
                                            ...
                                            std::make_shared<DBPoco::Net::IPAddress>(address),
                                            forwarded_address,
                                            quota_key,
