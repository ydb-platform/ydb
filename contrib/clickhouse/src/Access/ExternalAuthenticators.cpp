...
void parseLDAPSearchParams(LDAPClient::SearchParams & params, const DBPoco::Util::AbstractConfiguration & config, const String & prefix)
{
...
void parseLDAPServer(LDAPClient::Params & params, const DBPoco::Util::AbstractConfiguration & config, const String & name)
{
    if (name.empty())
...
void parseKerberosParams(GSSAcceptorContext::Params & params, const DBPoco::Util::AbstractConfiguration & config)
{
...
HTTPAuthClientParams parseHTTPAuthParams(const DBPoco::Util::AbstractConfiguration & config, const String & prefix)
{
    ...
    http_auth_params.timeouts = ConnectionTimeouts()
        .withConnectionTimeout(DBPoco::Timespan(connection_timeout_ms * 1000))
        .withReceiveTimeout(DBPoco::Timespan(receive_timeout_ms * 1000))
        .withSendTimeout(DBPoco::Timespan(send_timeout_ms * 1000));
...
void ExternalAuthenticators::setConfiguration(const DBPoco::Util::AbstractConfiguration & config, LoggerPtr log)
{
    std::lock_guard lock(mutex);
    resetImpl();
    ...
    DBPoco::Util::AbstractConfiguration::Keys http_auth_server_names;
    ...
    DBPoco::Util::AbstractConfiguration::Keys ldap_server_names;
