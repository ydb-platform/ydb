    ...
    try
    {
        ...
        for (auto it = addresses.begin(); it != addresses.end();)
        {
            ...
            if (static_cast<bool>(secure))
            {
                ...
                static_cast<DBPoco::Net::SecureStreamSocket*>(socket.get())->setLazyHandshake(true);
#else
...
void Connection::sendHello([[maybe_unused]] const DBPoco::Timespan & handshake_timeout)
{
...
#if USE_SSH
void Connection::performHandshakeForSSHAuth(const DBPoco::Timespan & handshake_timeout)
{
    ...
    std::function<void(const Progress &)>)
{
    ...
    if (settings)
    {
        ...
        std::string method = DBPoco::toUpper((*settings)[Setting::network_compression_method].toString());
