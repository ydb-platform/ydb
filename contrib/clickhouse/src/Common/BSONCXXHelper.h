...
#error #include <mongocxx/client.hpp>
...
static bsoncxx::types::bson_value::value fieldAsBSONValue(const Field & field, const DataTypePtr & type, const bool is_oid)
{
    ...
    switch (type_id)
    {
        ...
        case TypeIndex::UUID:
        {
            ...
            {
                uuid_numbers[0] = DB::byteswap(field.safeGet<UUID>().toUnderType().items[0]);
                uuid_numbers[1] = DB::byteswap(field.safeGet<UUID>().toUnderType().items[1]);
            } else
            {
