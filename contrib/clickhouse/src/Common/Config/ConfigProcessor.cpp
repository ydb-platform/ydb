...
bool ConfigProcessor::hasNodeWithAttribute(DBPoco::XML::Node * config_root, const std::string & attribute_name)
{
...
bool ConfigProcessor::hasNodeWithNameAndChildNodeWithAttribute(DBPoco::XML::Node * config_root, const std::string & node_name, const std::string & attribute_name)
{
    for (Node * node = config_root->firstChild(); node != nullptr; node = node->nextSibling())
    {
        if (node->nodeType() != Node::ELEMENT_NODE)
