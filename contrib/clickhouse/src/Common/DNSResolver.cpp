...
struct DNSResolver::AddressFilter
{
    ...
    void performAddressFiltering(DNSResolver::IPAddresses & addresses) const
    {
        ...
        std::erase_if(addresses, [dns_resolve_ipv6, dns_resolve_ipv4](const DBPoco::Net::IPAddress& address)
        {
            return (address.family() == DBPoco::Net::IPAddress::IPv6 && !dns_resolve_ipv6)
                || (address.family() == DBPoco::Net::IPAddress::IPv4 && !dns_resolve_ipv4);
        });
    }
...
DBPoco::Net::SocketAddress DNSResolver::resolveAddress(const std::string & host_and_port)
{
    ...
    addToNewHosts(host);
    return DBPoco::Net::SocketAddress(pickAddress(resolveIPAddressWithCache(host)), port);
}
...
std::vector<DBPoco::Net::SocketAddress> DNSResolver::resolveAddressList(const std::string & host, UInt16 port)
{
    ...
    std::vector<DBPoco::Net::IPAddress> ips = impl->disable_cache ? hostByName(host) : resolveIPAddressWithCache(host);
