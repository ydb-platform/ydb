...
public:
    ...
    bool is(DBPoco::Message::Priority priority) { return logger->is(priority); }
    LogFrequencyLimiterImpl * getChannel() {return this; }
    const String & name() const { return logger->name(); }

    void log(DBPoco::Message & message);
...
public:
    ...
    bool is(DBPoco::Message::Priority priority) { propagate_to_actual_log &= logger->is(priority); return true; }
