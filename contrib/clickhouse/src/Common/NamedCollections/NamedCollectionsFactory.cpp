...
namespace
{
    ...
    std::vector<std::string> listCollections(const DBPoco::Util::AbstractConfiguration & config)
    {
        DBPoco::Util::AbstractConfiguration::Keys collections_names;
    ...
    NamedCollectionsMap getNamedCollections(const DBPoco::Util::AbstractConfiguration & config)
    {
...
void NamedCollectionFactory::loadFromConfig(const DBPoco::Util::AbstractConfiguration & config, std::lock_guard<std::mutex> & lock)
{
...
void NamedCollectionFactory::reloadFromConfig(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(mutex);
    if (loadIfNot(lock))
        return;
