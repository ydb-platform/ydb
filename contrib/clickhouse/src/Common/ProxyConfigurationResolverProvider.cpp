...
namespace
{
    std::string getNoProxyHosts(const DBPoco::Util::AbstractConfiguration & configuration)
    {
        return configuration.getString("proxy.no_proxy", "");
    }

    bool isTunnelingDisabledForHTTPSRequestsOverHTTPProxy(
        const DBPoco::Util::AbstractConfiguration & configuration)
    {
        ...
        const DBPoco::Util::AbstractConfiguration & configuration)
    {
        ...
        auto endpoint = DBPoco::URI(configuration.getString(resolver_prefix + ".endpoint"));
    ...
    auto extractURIList(const String & config_prefix, const DBPoco::Util::AbstractConfiguration & configuration)
    {
        std::vector<String> keys;
        configuration.keys(config_prefix, keys);

        std::vector<DBPoco::URI> uris;
        for (const auto & key : keys)
        {
            if (startsWith(key, "uri"))
            {
                DBPoco::URI proxy_uri(configuration.getString(config_prefix + "." + key));
    ...
    bool hasRemoteResolver(const String & config_prefix, const DBPoco::Util::AbstractConfiguration & configuration)
    {
        return configuration.has(config_prefix + ".resolver");
    }

    bool hasListResolver(const String & config_prefix, const DBPoco::Util::AbstractConfiguration & configuration)
    {
