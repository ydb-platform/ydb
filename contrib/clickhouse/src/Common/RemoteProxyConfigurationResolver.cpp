...
std::string RemoteProxyHostFetcherImpl::fetch(const DBPoco::URI & endpoint, const ConnectionTimeouts & timeouts)
{
    auto request = DBPoco::Net::HTTPRequest(DBPoco::Net::HTTPRequest::HTTP_GET, endpoint.getPath(), DBPoco::Net::HTTPRequest::HTTP_1_1);
    ...
    std::string body;
    DBPoco::StreamCopier::copyToString(response_body_stream, body);

    if (response.getStatus() != DBPoco::Net::HTTPResponse::HTTP_OK)
        throw HTTPException(
