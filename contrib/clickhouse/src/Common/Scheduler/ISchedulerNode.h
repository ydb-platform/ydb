...
inline const DBPoco::Util::AbstractConfiguration & emptyConfig()
{
    static DBPoco::AutoPtr<DBPoco::Util::XMLConfiguration> config = new DBPoco::Util::XMLConfiguration();
    return *config;
}
...
struct SchedulerNodeInfo
{
    ...
    explicit SchedulerNodeInfo(const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix = {})
    {
...
{
public:
    explicit ISchedulerNode(EventQueue * event_queue_, const DBPoco::Util::AbstractConfiguration & config = emptyConfig(), const String & config_prefix = {})
