...
public:
    ...
    SchedulerNodePtr get(const String & name, EventQueue * event_queue, const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
    {
    ...
    {
        std::lock_guard lock{mutex};
        methods[name] = [] (EventQueue * event_queue, const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
        {
...
private:
    std::mutex mutex;
    using Method = std::function<SchedulerNodePtr(EventQueue * event_queue, const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)>;
