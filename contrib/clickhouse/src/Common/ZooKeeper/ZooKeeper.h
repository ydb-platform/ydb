...
///
/// DBPoco::Event objects are used for watches. The event is set only once on the first
/// watch notification.
...
class ZooKeeper
{
    ...
    */
    ZooKeeper(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB::ZooKeeperLog> zk_log_ = nullptr);
...
public:
    ...
    static Ptr create(const DBPoco::Util::AbstractConfiguration & config,
    ...
    bool configChanged(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name) const;
...
void validateZooKeeperConfig(const DBPoco::Util::AbstractConfiguration & config);

bool hasZooKeeperConfig(const DBPoco::Util::AbstractConfiguration & config);

String getZooKeeperConfigName(const DBPoco::Util::AbstractConfiguration & config);
