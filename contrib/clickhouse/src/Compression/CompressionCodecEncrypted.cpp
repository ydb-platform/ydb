...
size_t encrypt(std::string_view plaintext, char * ciphertext_and_tag, EncryptionMethod method, const String & key, const String & nonce)
{
    ...
    try
    {
        const EVP_CIPHER* cipher = EVP_get_cipherbyname(getMethod(method));
        if (cipher == nullptr)
            throw Exception::createDeprecated(lastErrorString(), ErrorCodes::OPENSSL_ERROR);
...
void CompressionCodecEncrypted::Configuration::loadImpl(
    const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix, EncryptionMethod method, std::unique_ptr<Params> & new_params)
{
...
bool CompressionCodecEncrypted::Configuration::tryLoad(const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
{
...
void CompressionCodecEncrypted::Configuration::load(const DBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
{
...
bool CompressionCodecEncrypted::Configuration::tryLoad(const DBPoco::Util::AbstractConfiguration & config [[maybe_unused]], const String & config_prefix [[maybe_unused]])
{
    return false;
}

/// if encryption is disabled, print warning about this.
void CompressionCodecEncrypted::Configuration::load(const DBPoco::Util::AbstractConfiguration & config [[maybe_unused]], const String & config_prefix [[maybe_unused]])
{
