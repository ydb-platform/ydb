...
{
    DBPoco::URI::QueryParameters params = {{"warehouse", warehouse}};
    ...
    DBPoco::Dynamic::Var json = parser.parse(json_str);
    const DBPoco::JSON::Object::Ptr & object = json.extract<DBPoco::JSON::Object::Ptr>();

    Config result;

    auto defaults_object = object->get("defaults").extract<DBPoco::JSON::Object::Ptr>();
    parseCatalogConfigurationSettings(defaults_object, result);

    auto overrides_object = object->get("overrides").extract<DBPoco::JSON::Object::Ptr>();
...
void RestCatalog::parseCatalogConfigurationSettings(const DBPoco::JSON::Object::Ptr & object, Config & result)
{
    if (!object)
        return;
...
std::string RestCatalog::retrieveAccessToken() const
{
    ...
    {
        url = DBPoco::URI(base_url / oauth_tokens_endpoint);

        DBPoco::URI::QueryParameters params = {
    ...
    {
        url = DBPoco::URI(oauth_server_uri);
    ...
    DBPoco::JSON::Parser parser;
    DBPoco::Dynamic::Var res_json = parser.parse(json_str);
    const DBPoco::JSON::Object::Ptr & object = res_json.extract<DBPoco::JSON::Object::Ptr>();
    ...
    const DB::HTTPHeaderEntries & headers) const
{
    ...
    DBPoco::URI url(base_url / endpoint);
    if (!params.empty())
        url.setQueryParameters(params);
    ...
    catch (const DB::HTTPException & e)
    {
        ...
        if (update_token_if_expired &&
            (status == DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_UNAUTHORIZED
             || status == DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_FORBIDDEN))
        {
...
DBPoco::URI::QueryParameters RestCatalog::createParentNamespaceParams(const std::string & base_namespace) const
{
...
RestCatalog::Namespaces RestCatalog::getNamespaces(const std::string & base_namespace) const
{
    ...
    catch (const DB::HTTPException & e)
    {
        ...
        if (e.code() == DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND)
...
RestCatalog::Namespaces RestCatalog::parseNamespaces(DB::ReadBuffer & buf, const std::string & base_namespace) const
{
    ...
    try
    {
        ...
        auto namespaces_object = object->get("namespaces").extract<DBPoco::JSON::Array::Ptr>();
        ...
        {
            auto current_namespace_array = namespaces_object->get(static_cast<int>(i)).extract<DBPoco::JSON::Array::Ptr>();
...
DB::Names RestCatalog::parseTables(DB::ReadBuffer & buf, const std::string & base_namespace, size_t limit) const
{
    ...
    try
    {
        ...
        auto identifiers_object = object->get("identifiers").extract<DBPoco::JSON::Array::Ptr>();
        ...
        {
            const auto current_table_json = identifiers_object->get(static_cast<int>(i)).extract<DBPoco::JSON::Object::Ptr>();
    ...
    TableMetadata & result) const
{
    ...
    auto metadata_object = object->get("metadata").extract<DBPoco::JSON::Object::Ptr>();
    if (!metadata_object)
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
    ...
    {
        auto config_object = object->get("config").extract<DBPoco::JSON::Object::Ptr>();
        if (!config_object)
