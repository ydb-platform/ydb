...
namespace
{
    bool hasValueAndItsNotNone(const std::string value, const DBPoco::JSON::Object::Ptr & object)
    {
...
std::pair<DBPoco::Dynamic::Var, std::string> UnityCatalog::getJSONRequest(const std::string & route, const DBPoco::URI::QueryParameters & params) const
{
    const auto & context = getContext();
    return makeHTTPRequestAndReadJSON(base_url / route, context, credentials, params, {auth_header});
}

std::pair<DBPoco::Dynamic::Var, std::string> UnityCatalog::postJSONRequest(const std::string & route, std::function<void(std::ostream &)> out_stream_callaback) const
{
    const auto & context = getContext();
    return makeHTTPRequestAndReadJSON(base_url / route, context, credentials, {}, {auth_header}, DBPoco::Net::HTTPRequest::HTTP_POST, out_stream_callaback);
}
...
void UnityCatalog::getCredentials(const std::string & table_id, TableMetadata & metadata) const
{
    ...
    switch (storage_type)
    {
        case StorageType::S3:
        {
            ...
            {
                const DBPoco::JSON::Object::Ptr & creds_object = object->getObject("aws_temp_credentials");
    ...
    try
    {
        ...
        if (hasValueAndItsNotNone("name", object) && object->get("name").extract<String>() == table_name)
        {
            ...
            if (result.requiresSchema())
            {
                ...
                {
                    const auto column_json = columns_json->get(static_cast<int>(i)).extract<DBPoco::JSON::Object::Ptr>();
                    ...
                    else
                    {
                        ...
                        data_type = DB::DeltaLakeMetadata::getFieldType(parsed_json_type.extract<DBPoco::JSON::Object::Ptr>(), "type", is_nullable);
                    }
...
DB::Names UnityCatalog::getTablesForSchema(const std::string & schema, size_t limit) const
{
    ...
    do
    {
        ...
        try
        {
            ...
            auto tables_object = object->get("tables").extract<DBPoco::JSON::Array::Ptr>();
            if (!tables_object)
                throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");

            for (size_t i = 0; i < tables_object->size(); ++i)
            {
                const auto current_table_json = tables_object->get(static_cast<int>(i)).extract<DBPoco::JSON::Object::Ptr>();
...
DataLake::ICatalog::Namespaces UnityCatalog::getSchemas(const std::string & base_prefix, size_t limit) const
{
    ...
    do
    {
        ...
        try
        {
            ...
            auto schemas_object = object->get("schemas").extract<DBPoco::JSON::Array::Ptr>();
            if (!schemas_object)
                throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");

            for (size_t i = 0; i < schemas_object->size(); ++i)
            {
                auto schema_info = schemas_object->get(static_cast<int>(i)).extract<DBPoco::JSON::Object::Ptr>();
