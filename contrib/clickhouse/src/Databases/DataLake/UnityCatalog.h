...
private:
    ...
    std::pair<DBPoco::Dynamic::Var, std::string> getJSONRequest(const std::string & route, const DBPoco::URI::QueryParameters & params = {}) const;
    std::pair<DBPoco::Dynamic::Var, std::string> postJSONRequest(const std::string & route, std::function<void(std::ostream &)> out_stream_callaback) const;
