...
void DiskBackup::setLastModified(const String & path, const DBPoco::Timestamp & timestamp)
{
    std::string replaced_path = replacePathPrefix(path);
...
DBPoco::Timestamp DiskBackup::getLastModified(const String & path) const
{
    std::string replaced_path = replacePathPrefix(path);
...
void DiskBackup::applyNewSettings(const DBPoco::Util::AbstractConfiguration &, ContextPtr, const String &, const DisksMap &)
{
