...
void DiskLocal::setLastModified(const String & path, const DBPoco::Timestamp & timestamp)
{
    FS::setModificationTime(fs::path(disk_path) / path, timestamp.epochTime());
}

DBPoco::Timestamp DiskLocal::getLastModified(const String & path) const
{
...
void DiskLocal::applyNewSettings(const DBPoco::Util::AbstractConfiguration & config, ContextPtr context, const String & config_prefix, const DisksMap & disk_map)
{
