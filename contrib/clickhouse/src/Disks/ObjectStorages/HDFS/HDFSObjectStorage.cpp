...
ObjectMetadata HDFSObjectStorage::getObjectMetadata(const std::string & path) const
{
    ...
    metadata.last_modified = DBPoco::Timestamp::fromEpochTime(file_info->mLastMod);
        ...
        else
        {
            children.emplace_back(std::make_shared<RelativePathWithMetadata>(
                ...
                ObjectMetadata{
                    ...
                    DBPoco::Timestamp::fromEpochTime(ls.file_info[i].mLastMod),
                    "",
                    {}}));
        }
...
std::unique_ptr<IObjectStorage> HDFSObjectStorage::cloneObjectStorage(
    const std::string &,
    const DBPoco::Util::AbstractConfiguration &,
