...
ReadBufferFromPocoSocketBase::ReadBufferFromPocoSocketBase(DBPoco::Net::Socket & socket_, size_t buf_size)
...
ReadBufferFromPocoSocketBase::ReadBufferFromPocoSocketBase(DBPoco::Net::Socket & socket_, const ProfileEvents::Event & read_event_, size_t buf_size)
...
bool ReadBufferFromPocoSocketBase::poll(size_t timeout_microseconds) const
{
    ...
    Stopwatch watch;
    bool res = socket.impl()->poll(timeout_microseconds, DBPoco::Net::Socket::SELECT_READ | DBPoco::Net::Socket::SELECT_ERROR);
    ProfileEvents::increment(ProfileEvents::NetworkReceiveElapsedMicroseconds, watch.elapsedMicroseconds());
    return res;
}
...
{
    socket.setReceiveTimeout(DBPoco::Timespan(receive_timeout_microseconds, 0));
}
