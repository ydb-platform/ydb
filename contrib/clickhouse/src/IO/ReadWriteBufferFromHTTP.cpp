...
bool isRetriableError(const DBPoco::Net::HTTPResponse::HTTPStatus http_status) noexcept
{
    static constexpr std::array non_retriable_errors{
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_BAD_REQUEST,
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_UNAUTHORIZED,
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND,
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_FORBIDDEN,
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_IMPLEMENTED,
        DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_METHOD_NOT_ALLOWED};

    return std::all_of(
        non_retriable_errors.begin(), non_retriable_errors.end(), [&](const auto status) { return http_status != status; });
}

DBPoco::URI getUriAfterRedirect(const DBPoco::URI & prev_uri, DBPoco::Net::HTTPResponse & response)
{
    chassert(DB::isRedirect(response.getStatus()));

    auto location = response.get("Location");
    auto location_uri = DBPoco::URI(location);
...
void ReadWriteBufferFromHTTP::prepareRequest(DBPoco::Net::HTTPRequest & request, std::optional<HTTPRange> range) const
{
    request.setHost(current_uri.getHost());

    if (out_stream_callback)
        request.setChunkedTransferEncoding(true);
    else if (method == DBPoco::Net::HTTPRequest::HTTP_POST)
...
std::optional<size_t> ReadWriteBufferFromHTTP::tryGetFileSize()
{
    if (!file_info)
    {
        ...
        catch (const DBPoco::Net::NetException &)
        {
            return std::nullopt;
        }
        catch (const DBPoco::IOException &)
        {
            return std::nullopt;
        }
    }
...
bool ReadWriteBufferFromHTTP::supportsReadAt()
{
    if (!file_info)
        file_info = getFileInfo();
    return method == DBPoco::Net::HTTPRequest::HTTP_GET && file_info->seekable;
}
...
void ReadWriteBufferFromHTTP::getHeadResponse(DBPoco::Net::HTTPResponse & response)
{
    doWithRetries(
        [&] ()
        {
            callWithRedirects(response, DBPoco::Net::HTTPRequest::HTTP_HEAD, {});
        },
        /*on_retry=*/ nullptr,
        /*mute_logging=*/ true);
}
...
ReadWriteBufferFromHTTP::ReadWriteBufferFromHTTP(
    ...
    , initial_uri(uri_)
    , method(!method_.empty() ? method_ : out_stream_callback_ ? DBPoco::Net::HTTPRequest::HTTP_POST : DBPoco::Net::HTTPRequest::HTTP_GET)
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callImpl(
    DBPoco::Net::HTTPResponse & response, const std::string & method_, const std::optional<HTTPRange> & range, bool allow_redirects) const
{
    if (remote_host_filter)
        remote_host_filter->checkURL(current_uri);

    DBPoco::Net::HTTPRequest request(method_, current_uri.getPathAndQuery(), DBPoco::Net::HTTPRequest::HTTP_1_1);
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callWithRedirects(
    DBPoco::Net::HTTPResponse & response, const String & method_, const std::optional<HTTPRange> & range)
{
    auto result = callImpl(response, method_, range, true);

    while (isRedirect(response.getStatus()))
    {
        DBPoco::URI uri_redirect = getUriAfterRedirect(current_uri, response);
        ++redirects;
...
std::unique_ptr<ReadBuffer> ReadWriteBufferFromHTTP::initialize()
{
    ...
    if (range.has_value() && response.getStatus() != DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT)
    {
        ...
        if (getOffset() != 0)
        {
            ...
            if (response.getStatus() == DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_OK)
            {
                ...
                throw HTTPException(
                    ...
                    DBPoco::Net::HTTPResponse::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE,
                    response.getReason(),
                    explanation);
            }
            throw Exception(
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            if (!impl)
            {
                ...
                {
                    if (http_skip_not_found_url && e.getHTTPStatus() == DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND)
                    {
                        next_result = false;
...
size_t ReadWriteBufferFromHTTP::readBigAt(char * to, size_t n, size_t offset, const std::function<bool(size_t)> & progress_callback) const
{
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            ...
            if (response.getStatus() != DBPoco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT &&
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::getFileInfo()
{
    ...
    catch (const HTTPException & e)
    {
        ...
        if (e.getHTTPStatus() >= 400 && e.getHTTPStatus() <= 499 &&
            e.getHTTPStatus() != DBPoco::Net::HTTPResponse::HTTP_TOO_MANY_REQUESTS &&
            e.getHTTPStatus() != DBPoco::Net::HTTPResponse::HTTP_REQUEST_TIMEOUT &&
            e.getHTTPStatus() != DBPoco::Net::HTTPResponse::HTTP_MISDIRECTED_REQUEST)
        {
            return HTTPFileInfo{};
        }
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::parseFileInfo(const DBPoco::Net::HTTPResponse & response, size_t requested_range_begin)
{
    HTTPFileInfo res;
