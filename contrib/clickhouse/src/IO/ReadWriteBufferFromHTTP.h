...
private:
    ...
    void prepareRequest(DBPoco::Net::HTTPRequest & request, std::optional<HTTPRange> range) const;
    ...
    void getHeadResponse(DBPoco::Net::HTTPResponse & response);
...
{
    DBPoco::URI uri;
    std::string method = DBPoco::Net::HTTPRequest::HTTP_GET;
...
public:
    explicit BuilderRWBufferFromHTTP(DBPoco::URI uri_)
        : uri(uri_)
    {}
...
/// NOLINTEND(bugprone-macro-parentheses)
    ...
    ReadWriteBufferFromHTTPPtr create(const DBPoco::Net::HTTPBasicCredentials & credentials_)
    {
