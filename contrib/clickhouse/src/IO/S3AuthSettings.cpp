...
bool setValueFromConfig(
    const DBPoco::Util::AbstractConfiguration & config, const std::string & path, typename S3AuthSettingsImpl::SettingFieldRef & field)
{
    if (!config.has(path))
        return false;
...
S3AuthSettings::S3AuthSettings(
    const DBPoco::Util::AbstractConfiguration & config, const DB::Settings & settings, const std::string & config_prefix)
    : S3AuthSettings()
{
