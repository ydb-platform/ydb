...
bool setValueFromConfig(
    const DBPoco::Util::AbstractConfiguration & config, const std::string & path, typename S3RequestSettingsImpl::SettingFieldRef & field)
{
    if (!config.has(path))
        return false;
...
void S3RequestSettings::normalizeSettings()
{
    if (!impl->storage_class_name.value.empty() && impl->storage_class_name.changed)
        impl->storage_class_name = DBPoco::toUpperInPlace(impl->storage_class_name.value);
}
