...
{
    current_address = std::make_shared<DBPoco::Net::SocketAddress>();
    initial_address = std::make_shared<DBPoco::Net::SocketAddress>();
}

std::optional<DBPoco::Net::SocketAddress> ClientInfo::getLastForwardedFor() const
{
    ...
    if (last[0] == '[')
        return DBPoco::Net::SocketAddress{DBPoco::Net::AddressFamily::IPv6, last};

    const auto colons = std::count(last.begin(), last.end(), ':');

    /// IPv6 address without port
    if (colons > 1)
        return DBPoco::Net::SocketAddress{DBPoco::Net::AddressFamily::IPv6, last, 0};

    /// IPv4 address with port
    if (colons == 1)
        return DBPoco::Net::SocketAddress{DBPoco::Net::AddressFamily::IPv4, last};

    /// IPv4 address without port
    return DBPoco::Net::SocketAddress{DBPoco::Net::AddressFamily::IPv4, last, 0};
}
...
void ClientInfo::read(ReadBuffer & in, UInt64 client_protocol_revision)
{
    ...
    initial_address = std::make_shared<DBPoco::Net::SocketAddress>(initial_address_string);
