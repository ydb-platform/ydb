...
struct ContextSharedPart : boost::noncopyable
{
    ...
    const DBPoco::Util::AbstractConfiguration & getConfigRefWithLock(const std::lock_guard<ContextSharedMutex> &) const TSA_REQUIRES(this->mutex)
    {
        return config ? *config : DBPoco::Util::Application::instance().config();
    }

    const DBPoco::Util::AbstractConfiguration & getConfigRef() const
    {
...
static VolumePtr createLocalSingleDiskVolume(const std::string & path, const DBPoco::Util::AbstractConfiguration & config_)
{
...
const DBPoco::Util::AbstractConfiguration & Context::getConfigRef() const
{
...
void Context::setExternalAuthenticatorsConfig(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::setCurrentQueryId(const String & query_id)
{
    ...
    ///  because DBPoco::UUIDGenerator::generateRandom method is using /dev/random, that is very expensive.
    ...
    if (query_id_to_set.empty())    /// If the user did not submit his query_id, then we generate it ourselves.
    {
        ...
        struct QueryUUID : DBPoco::UUID
        {
            QueryUUID(const char * bytes, DBPoco::UUID::Version version)
                : DBPoco::UUID(bytes, version) {}
        };

        query_id_to_set = QueryUUID(random.bytes, DBPoco::UUID::UUID_RANDOM).toString();
    }
...
void Context::loadOrReloadDictionaries(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::loadOrReloadUserDefinedExecutableFunctions(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::updateUncompressedCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateMarkCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updatePrimaryIndexCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateIndexUncompressedCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateIndexMarkCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateMMappedFileCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateQueryConditionCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateQueryResultCacheConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
bool checkZooKeeperConfigIsLocal(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_name)
{
...
void Context::updateKeeperConfiguration([[maybe_unused]] const DBPoco::Util::AbstractConfiguration & config)
{
#if USE_NURAFT
    std::lock_guard lock(shared->keeper_dispatcher_mutex);
    if (!shared->keeper_dispatcher)
...
void Context::updateInterserverCredentials(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setRemoteHostFilter(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setHTTPHeaderFilter(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::updateStorageConfiguration(const DBPoco::Util::AbstractConfiguration & config)
{
    {
        std::lock_guard lock(shared->storage_policies_mutex);
...
void Context::setApplicationType(ApplicationType type)
{
    ...
    if (type == ApplicationType::LOCAL || type == ApplicationType::SERVER || type == ApplicationType::DISKS)
        shared->server_settings.loadSettingsFromConfig(DBPoco::Util::Application::instance().config());

    if (type == ApplicationType::SERVER)
        shared->configureServerWideThrottling();
}

void Context::setDefaultProfiles(const DBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setHTTPClientInfo(const DBPoco::Net::HTTPRequest & request)
{
...
void Context::setCurrentAddress(const DBPoco::Net::SocketAddress & current_address)
{
    client_info.current_address = std::make_shared<DBPoco::Net::SocketAddress>(current_address);
    need_recalculate_access = true;
}
...
void Context::setInitialAddress(const DBPoco::Net::SocketAddress & initial_address)
{
    client_info.initial_address = std::make_shared<DBPoco::Net::SocketAddress>(initial_address);
}
...
DBPoco::Timespan HTTPContext::getReceiveTimeout() const
{
    return context->getSettingsRef()[Setting::http_receive_timeout];
}

DBPoco::Timespan HTTPContext::getSendTimeout() const
{
