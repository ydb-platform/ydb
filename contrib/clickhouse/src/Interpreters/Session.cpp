...
void Session::authenticate(const String & user_name, const String & password, const DBPoco::Net::SocketAddress & address, const Strings & external_roles_)
{
    authenticate(BasicCredentials{user_name, password}, address, external_roles_);
}

void Session::authenticate(const Credentials & credentials_, const DBPoco::Net::SocketAddress & address_, const Strings & external_roles_)
{
    if (session_context)
    ...
    if ((address == DBPoco::Net::SocketAddress{}) && (prepared_client_info->interface == ClientInfo::Interface::LOCAL))
        address = DBPoco::Net::SocketAddress{"127.0.0.1", 0};
    ...
    prepared_client_info->current_address = std::make_shared<DBPoco::Net::SocketAddress>(address);
}
...
void Session::onAuthenticationFailure(const std::optional<String> & user_name, const DBPoco::Net::SocketAddress & address_, const Exception & e)
{
    ...
    if (auto session_log = getSessionLog())
    {
        ...
        info_for_log.current_address = std::make_shared<DBPoco::Net::SocketAddress>(address_);
...
void Session::setHTTPClientInfo(const DBPoco::Net::HTTPRequest & request)
{
    if (session_context)
