        ...
        std::unordered_map<size_t, MongoDBPocoLegacyArrayInfo> & array_info, size_t column_idx, const DataTypePtr data_type)
    {
        ...
        if (which.isUInt8())
            ...
            parser = [](const DBPoco::MongoDB::Element & value, const std::string & name) -> Field
            {
                ...
                if (value.type() == DBPoco::MongoDB::ElementTraits<MongoUUID>::TypeId)
                {
...
DBPoco::MongoDB::Document::Vector MongoDBPocoLegacyCursor::nextDocuments(DBPoco::MongoDB::Connection & connection)
{
    if (is_wire_protocol_old)
    {
