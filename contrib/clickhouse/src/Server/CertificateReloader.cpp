...
void CertificateReloader::tryLoadClient(const DBPoco::Util::AbstractConfiguration & config)
{
    tryLoad(config, nullptr, DBPoco::Net::SSLManager::CFG_CLIENT_PREFIX);
}
...
std::list<CertificateReloader::MultiData>::iterator CertificateReloader::findOrInsert(SSL_CTX * ctx, const std::string & prefix)
{
    ...
    else
    {
        if (!ctx)
        {
            if (prefix == DBPoco::Net::SSLManager::CFG_CLIENT_PREFIX)
                ctx = DBPoco::Net::SSLManager::instance().defaultClientContext()->sslContext();
            else
