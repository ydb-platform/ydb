...
namespace
{
bool tryAddHTTPOptionHeadersFromConfig(HTTPServerResponse & response, const DBPoco::Util::LayeredConfiguration & config)
{
...
void processOptionsRequest(HTTPServerResponse & response, const DBPoco::Util::LayeredConfiguration & config)
{
...
HTTPHandlerConnectionConfig::HTTPHandlerConnectionConfig(const DBPoco::Util::AbstractConfiguration & config, const std::string & config_prefix)
{
    ...
    const ProfileEvents::Event & write_event)
{
    ...
    auto set_query_result = [&response, this] (const QueryResultDetails & details)
    {
        ...
        if (!(http_response_headers_override && http_response_headers_override->contains(DBPoco::Net::HTTPMessage::CONTENT_TYPE))
