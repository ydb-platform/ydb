...
struct SocketInterruptablePollWrapper
{
    ...
    explicit SocketInterruptablePollWrapper(const DBPoco::Net::StreamSocket & poco_socket_)
    ...
    PollResult poll(DBPoco::Timespan remaining_time, const ReadBufferFromPocoSocket & in)
    {
...
DBPoco::Timespan KeeperTCPHandler::receiveHandshake(int32_t handshake_length, bool & use_compression)
{
    ...
    return DBPoco::Timespan(timeout_ms * 1000);
}
...
void KeeperTCPHandler::runImpl()
{
    ...
    try
    {
        ...
        if (client_timeout.totalMilliseconds() == 0)
            client_timeout = DBPoco::Timespan(Coordination::DEFAULT_SESSION_TIMEOUT_MS * DBPoco::Timespan::MILLISECONDS);
        ...
        while (true)
        {
            ...
            if (result.has_requests && !close_received)
            {
                ...
                else if (received_op == Coordination::OpNum::Heartbeat)
                {
                    ...
                    operations[received_xid] = DBPoco::Timestamp();
...
void KeeperTCPHandler::updateStats(Coordination::ZooKeeperResponsePtr & response, const Coordination::ZooKeeperRequestPtr & request)
{
    ...
    {
        Int64 elapsed = (DBPoco::Timestamp() - operations[response->xid]);
        ...
        last_op.set(std::make_unique<LastOp>(LastOp{
            ...
            .last_response_time = DBPoco::Timestamp().epochMicroseconds() / 1000,
        }));
    }
