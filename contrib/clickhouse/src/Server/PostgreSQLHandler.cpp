...
#if USE_SSL
    params.privateKeyFile = config.getString(prefix + DBPoco::Net::SSLManager::CFG_PRIV_KEY_FILE, "");
    params.certificateFile = config.getString(prefix + DBPoco::Net::SSLManager::CFG_CERTIFICATE_FILE, params.privateKeyFile);
    if (!params.privateKeyFile.empty() && !params.certificateFile.empty())
    {
        auto ctx = DBPoco::Net::SSLManager::instance().defaultServerContext();
        params.caLocation = config.getString(prefix + DBPoco::Net::SSLManager::CFG_CA_LOCATION, ctx->getCAPaths().caLocation);

        params.verificationMode = DBPoco::Net::SSLManager::VAL_VER_MODE;
        if (config.hasProperty(prefix + DBPoco::Net::SSLManager::CFG_VER_MODE))
        {
            std::string mode = config.getString(prefix + DBPoco::Net::SSLManager::CFG_VER_MODE);
            params.verificationMode = DBPoco::Net::Utility::convertVerificationMode(mode);
        }

        params.verificationDepth = config.getInt(prefix + DBPoco::Net::SSLManager::CFG_VER_DEPTH, DBPoco::Net::SSLManager::VAL_VER_DEPTH);
        params.loadDefaultCAs
            = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_ENABLE_DEFAULT_CA, DBPoco::Net::SSLManager::VAL_ENABLE_DEFAULT_CA);
        params.cipherList = config.getString(prefix + DBPoco::Net::SSLManager::CFG_CIPHER_LIST, DBPoco::Net::SSLManager::VAL_CIPHER_LIST);
        params.cipherList
            = config.getString(prefix + DBPoco::Net::SSLManager::CFG_CYPHER_LIST, params.cipherList); // for backwards compatibility

        bool require_tlsv1 = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_REQUIRE_TLSV1, false);
        bool require_tlsv1_1 = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_REQUIRE_TLSV1_1, false);
        bool require_tlsv1_2 = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_REQUIRE_TLSV1_2, false);
        if (require_tlsv1_2)
            usage = DBPoco::Net::Context::TLSV1_2_SERVER_USE;
        else if (require_tlsv1_1)
            usage = DBPoco::Net::Context::TLSV1_1_SERVER_USE;
        else if (require_tlsv1)
            usage = DBPoco::Net::Context::TLSV1_SERVER_USE;
        else
            usage = DBPoco::Net::Context::SERVER_USE;

        params.dhParamsFile = config.getString(prefix + DBPoco::Net::SSLManager::CFG_DH_PARAMS_FILE, "");
        params.ecdhCurve = config.getString(prefix + DBPoco::Net::SSLManager::CFG_ECDH_CURVE, "");

        std::string disabled_protocols_list = config.getString(prefix + DBPoco::Net::SSLManager::CFG_DISABLE_PROTOCOLS, "");
        DBPoco::StringTokenizer dp_tok(
            disabled_protocols_list, ";,", DBPoco::StringTokenizer::TOK_TRIM | DBPoco::StringTokenizer::TOK_IGNORE_EMPTY);
        disabled_protocols = 0;
        for (const auto & token : dp_tok)
        {
            if (token == "sslv2")
                disabled_protocols |= DBPoco::Net::Context::PROTO_SSLV2;
            else if (token == "sslv3")
                disabled_protocols |= DBPoco::Net::Context::PROTO_SSLV3;
            else if (token == "tlsv1")
                disabled_protocols |= DBPoco::Net::Context::PROTO_TLSV1;
            else if (token == "tlsv1_1")
                disabled_protocols |= DBPoco::Net::Context::PROTO_TLSV1_1;
            else if (token == "tlsv1_2")
                disabled_protocols |= DBPoco::Net::Context::PROTO_TLSV1_2;
        }

        extended_verification = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_EXTENDED_VERIFICATION, false);
        prefer_server_ciphers = config.getBool(prefix + DBPoco::Net::SSLManager::CFG_PREFER_SERVER_CIPHERS, false);
    }
#endif
}
...
void PostgreSQLHandler::makeSecureConnectionSSL()
{
    ...
    if (!params.privateKeyFile.empty() && !params.certificateFile.empty())
    {
        ctx = DBPoco::Net::SSLManager::instance().getCustomServerContext(prefix);
        if (!ctx)
        {
            ctx = new DBPoco::Net::Context(usage, params);
            ...
            ctx = DBPoco::Net::SSLManager::instance().setCustomServerContext(prefix, ctx);
        }
    }
    ss = std::make_shared<DBPoco::Net::SecureStreamSocket>(DBPoco::Net::SecureStreamSocket::attach(socket(), ctx));    changeIO(*ss);
}
#else
void PostgreSQLHandler::makeSecureConnectionSSL() {}
#endif
