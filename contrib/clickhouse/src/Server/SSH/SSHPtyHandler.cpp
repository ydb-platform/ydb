...
public:
    ...
    (
        ...
        , log(&DBPoco::Logger::get("SSHChannelCallback"))
...
public:
    ...
    (
        ...
        IServer & server,
        const DBPoco::Net::SocketAddress & address_,
        const SSHPtyHandler::Options & options_
    )
        : server_context(server.context())
        , peer_address(address_)
        , log(&DBPoco::Logger::get("SSHSessionCallback"))
        , options(options_)
    {
...
SSHPtyHandler::SSHPtyHandler(
    ...
    , server(server_)
    , log(&DBPoco::Logger::get("SSHPtyHandler"))
