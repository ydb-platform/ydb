...
public:
    explicit SSHPtyHandlerFactory(
        IServer & server_, const DBPoco::Util::AbstractConfiguration & config)
        : server(server_), log(&DBPoco::Logger::get("SSHHandlerFactory"))
    {
    ...
    DBPoco::Net::TCPServerConnection * createConnection(const DBPoco::Net::StreamSocket & socket, TCPServer &) override
    {
