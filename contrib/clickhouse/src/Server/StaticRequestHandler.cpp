...
void StaticRequestHandler::handleRequest(HTTPServerRequest & request, HTTPServerResponse & response, const ProfileEvents::Event & /*write_event*/)
{
    ...
    if (request.getVersion() == DBPoco::Net::HTTPServerRequest::HTTP_1_1)
    ...
    try
    {
        ...
        if (request.getMethod() == DBPoco::Net::HTTPRequest::HTTP_POST && !request.getChunkedTransferEncoding() && !request.hasContentLength())
        ...
        response.setStatusAndReason(DBPoco::Net::HTTPResponse::HTTPStatus(status));
