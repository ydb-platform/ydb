        ...
        try
        {
            ...
            */
                ...
                && client_logs_level != LogsLevel::none)
            {
                ...
                query_state->logs_queue->max_priority = DBPoco::Logger::parseLevel(client_logs_level.toString());
...
void TCPHandler::initBlockOutput(QueryState & state, const Block & block)
{
    if (!state.block_out)
    {
        ...
        {
            std::string method = DBPoco::toUpper(query_settings[Setting::network_compression_method].toString());
