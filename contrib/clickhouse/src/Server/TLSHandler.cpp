...
DB::TLSHandler::TLSHandler(
    ...
    : DBPoco::Net::TCPServerConnection(socket)
#if USE_SSL
    , config(config_)
    , prefix(prefix_)
#endif
    , stack_data(stack_data_)
{
    ...
    if (!params.privateKeyFile.empty() && !params.certificateFile.empty())
    {
        ...
        DBPoco::StringTokenizer dp_tok(disabled_protocols_list, ";,", DBPoco::StringTokenizer::TOK_TRIM | DBPoco::StringTokenizer::TOK_IGNORE_EMPTY);
