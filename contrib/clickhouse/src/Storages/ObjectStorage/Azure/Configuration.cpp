...
#error #include <azure/storage/common/storage_credential.hpp>
...
#error #include <azure/identity/managed_identity_credential.hpp>
#error #include <azure/identity/workload_identity_credential.hpp>
...
{
    auto url = DBPoco::URI(connection_params.getConnectionURL());
...
void StorageAzureConfiguration::fromAST(ASTs & engine_args, ContextPtr context, bool with_structure)
{
    ...
    {
        return s == "auto" || FormatFactory::instance().getAllFormats().contains(DBPoco::toLower(s));
    };
