...
DataTypePtr getComplexTypeFromObject(const DBPoco::JSON::Object::Ptr & type)
{
    ...
    if (type_name == "struct")
    {
        ...
        Names element_names;
        auto fields = type->get("fields").extract<DBPoco::JSON::Array::Ptr>();
...
struct DeltaLakeMetadataImpl
{
        ...
        std::set<String> & result) const
    {
        ...
        while (!buf->eof())
        {
            ...
            {
                const auto metadata_object = object->get("metaData").extract<DBPoco::JSON::Object::Ptr>();
                if (!metadata_object)
                ...
                DBPoco::JSON::Parser p;
                DBPoco::Dynamic::Var fields_json = parser.parse(schema_object);
                const DBPoco::JSON::Object::Ptr & fields_object = fields_json.extract<DBPoco::JSON::Object::Ptr>();
                if (!fields_object)
            ...
            if (object->has("add"))
            {
                auto add_object = object->get("add").extract<DBPoco::JSON::Object::Ptr>();
                if (!add_object)
                ...
                if (it == file_partition_columns.end())
                {
                    ...
                    {
                        auto partition_values = add_object->get("partitionValues").extract<DBPoco::JSON::Object::Ptr>();
                        if (!partition_values)
            ...
            {
                auto remove_object = object->get("remove").extract<DBPoco::JSON::Object::Ptr>();
                if (!remove_object)
    ...
    NamesAndTypesList parseMetadata(const DBPoco::JSON::Object::Ptr & metadata_json) const
    {
        NamesAndTypesList schema;
        const auto fields = metadata_json->get("fields").extract<DBPoco::JSON::Array::Ptr>();
        for (size_t i = 0; i < fields->size(); ++i)
        {
            ...
            auto schema_metadata_object = field->get("metadata").extract<DBPoco::JSON::Object::Ptr>();
        ...
        DeltaLakePartitionColumns & file_partition_columns) const
    {
        ...
        for (size_t i = 0; i < path_column.size(); ++i)
        {
            ...
            {
                DBPoco::JSON::Parser parser;
                DBPoco::Dynamic::Var json = parser.parse(metadata);
                const DBPoco::JSON::Object::Ptr & object = json.extract<DBPoco::JSON::Object::Ptr>();
...
DataTypePtr DeltaLakeMetadata::getFieldType(const DBPoco::JSON::Object::Ptr & field, const String & type_key, bool is_nullable)
{
