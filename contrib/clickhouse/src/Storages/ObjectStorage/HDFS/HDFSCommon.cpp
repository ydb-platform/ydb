...
void HDFSBuilderWrapper::loadFromConfig(
    const DBPoco::Util::AbstractConfiguration & config, const String & prefix, [[maybe_unused]] bool isUser)
{
...
HDFSBuilderWrapper createHDFSBuilder(const String & uri_str, const DBPoco::Util::AbstractConfiguration & config)
{
    const DBPoco::URI uri(uri_str);
