...
#error #include <bsoncxx/json.hpp>
...
MongoDBConfiguration StorageMongoDB::getConfiguration(ASTs engine_args, ContextPtr context)
{
    ...
    if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, context))
    {
        ...
        else
        {
            ...
            DBPoco::URI::encode(named_collection->get<String>("password"), "!?#/'\",;:$&()[]*+=@", escaped_password);
            if (!user.empty())
    ...
    else
    {
        ...
        if (engine_args.size() >= 5 && engine_args.size() <= 7)
        {
            ...
            String auth_string;
            String escaped_password;
            DBPoco::URI::encode(checkAndGetLiteralArgument<String>(engine_args[4], "password"), "!?#/'\",;:$&()[]*+=@", escaped_password);
            if (!user.empty())
                auth_string = fmt::format("{}:{}@", user, escaped_password);
