...
/// But DBPoco::AbstractConfiguration converts them into "header", "header[1]", "header[2]".
...
IStorageURLBase::IStorageURLBase(
    ...
    : IStorage(table_id_)
    , uri(uri_)
    , compression_method(chooseCompressionMethod(DBPoco::URI(uri_).getPath(), compression_method_))
    ...
    Impl(const String & uri_, size_t max_addresses, const ActionsDAG::Node * predicate, const NamesAndTypesList & virtual_columns, const ContextPtr & context)
    {
        ...
        if (filter_dag)
        {
            ...
            for (const auto & uri : uris)
                paths.push_back(DBPoco::URI(uri).getPath());
...
void StorageURLSource::setCredentials(DBPoco::Net::HTTPBasicCredentials & credentials, const DBPoco::URI & request_uri)
{
    ...
    , need_only_count(need_only_count_)
{
    ...
    initialize = [=, this]()
    {
        ...
        std::pair<DBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> uri_and_buf;
        do
        {
...
std::pair<DBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> StorageURLSource::getFirstAvailableURIAndReadBuffer(
    ...
    bool delay_initialization)
{
    ...
    std::pair<DBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> last_skipped_empty_res;
    for (; option != end; ++option)
    {
        ...
        auto request_uri = DBPoco::URI(*option, context_->getSettingsRef()[Setting::enable_url_encoding]);
...
public:
    ...
    SinkPtr createSinkForPartition(const String & partition_id) override
    {
        ...
        context->getRemoteHostFilter().checkURL(DBPoco::URI(partition_path));
        ...
        Data next() override
        {
            ...
            do
            {
                ...
                uri_and_buf = StorageURLSource::getFirstAvailableURIAndReadBuffer(
                    ...
                    getContext(),
                    {},
                    DBPoco::Net::HTTPRequest::HTTP_GET,
                    {},
...
SinkToStoragePtr IStorageURLBase::write(const ASTPtr & query, const StorageMetadataPtr & metadata_snapshot, ContextPtr context, bool /*async_insert*/)
{
    if (http_method.empty())
        http_method = DBPoco::Net::HTTPRequest::HTTP_POST;
...
{
    context_->getRemoteHostFilter().checkURL(DBPoco::URI(uri));
    ...
    : StorageURL("", table_id_, format_name_, format_settings_, columns_, constraints_, String{}, context_, compression_method_)
{
    ...
    {
        DBPoco::URI poco_uri(uri_option);
...
void StorageURL::processNamedCollectionResult(Configuration & configuration, const NamedCollection & collection)
{
    ...
    if (!configuration.http_method.empty() && configuration.http_method != DBPoco::Net::HTTPRequest::HTTP_POST
        && configuration.http_method != DBPoco::Net::HTTPRequest::HTTP_PUT)
        throw Exception(
            ErrorCodes::BAD_ARGUMENTS,
...
StorageURL::Configuration StorageURL::getConfiguration(ASTs & args, const ContextPtr & local_context)
{
    ...
    if (configuration.format == "auto")
        configuration.format = FormatFactory::instance().tryGetFormatFromFileName(DBPoco::URI(configuration.url).getPath()).value_or("auto");
