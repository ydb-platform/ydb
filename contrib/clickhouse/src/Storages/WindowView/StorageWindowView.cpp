...
namespace
{
    ...
    inline UInt32 now()
    {
        return static_cast<UInt32>(DBPoco::Timestamp().epochMicroseconds() / 1000000);
    }
    ...
    void extractWindowArgument(const ASTPtr & ast, IntervalKind::Kind & kind, Int64 & num_units, String err_msg)
    {
        ...
        || !IntervalKind::tryParseString(DBPoco::toLower(arg->name.substr(10)), kind))
...
void StorageWindowView::threadFuncCleanup()
{
    ...
    if ((DBPoco::Timestamp().epochMicroseconds() - last_clean_timestamp_usec) > clean_interval_usec)
    {
        try
        {
            cleanup();
        ...
        last_clean_timestamp_usec = DBPoco::Timestamp().epochMicroseconds();
    }
}
...
void StorageWindowView::threadFuncFireProc()
{
    ...
    UInt64 timestamp_ms = static_cast<UInt64>(DBPoco::Timestamp().epochMicroseconds()) / 1000;
    if (!shutdown_called)
