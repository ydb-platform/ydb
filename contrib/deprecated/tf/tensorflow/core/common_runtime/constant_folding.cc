...
bool IsConstantFoldable(
    ...
    std::unordered_map<const Node*, std::vector<Tensor>>*
        shape_replacement_map) {
#if defined(ARCADIA_BUILD_ROOT) /* ImmutableConst is not constant-foldable */
  if (n->type_string() == "ImmutableConst") {
    // TODO(dbakshee): Make a separate NodeClass for ImmutableConst
    return false;
  }
...
Status ConstantFold(const ConstantFoldingOptions& opts,
  ...
  DumpGraph("Before ConstantFolding", graph);
  ...
  DumpGraph("After ConstantFolding", graph);
