...
static string SummarizeNode(const Node *node) {
...
void ExecutorState::Process(TaggedNode tagged_node, int64 scheduled_usec) {
  ...
  while (!inline_ready.empty()) {
    ...
    if (false && vlog_ && VLOG_IS_ON(1)) { // ARCADIA_BUILD_ROOT: 'false' removes failures of debug builds
...
#if defined(ARCADIA_BUILD_ROOT) /* Use SummarizeNode() while executing graph, not SummarizeNodeDef() */
            << SummarizeNode(node)
#else
            << SummarizeNode(*node)
