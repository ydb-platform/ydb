...
void AppendProtoDebugString(
  ...
  for (int i = 0; i < msg.value_size(); ++i) {
    o->AppendString("value", ProtobufStringToString(msg.value(i)));
  }
}
  ...
  while(true) {
    ...
    if (identifier == "value") {
      ...
      do {
        ...
        SetProtobufStringSwapAllowed(&str_value, msg->add_value());
...
void AppendProtoDebugString(
    ...
    const ::tensorflow::FloatList& msg) {
  for (int i = 0; i < msg.value_size(); ++i) {
    o->AppendNumeric("value", msg.value(i));
  }
}
...
void AppendProtoDebugString(
    ::tensorflow::strings::ProtoTextOutput* o,
    const ::tensorflow::Feature& msg) {
  if (msg.kind_case() == ::tensorflow::Feature::kBytesList) {
    o->OpenNestedMessage("bytes_list");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.bytes_list());
    o->CloseNestedMessage();
  }
  if (msg.kind_case() == ::tensorflow::Feature::kFloatList) {
    o->OpenNestedMessage("float_list");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.float_list());
    o->CloseNestedMessage();
  }
  if (msg.kind_case() == ::tensorflow::Feature::kInt64List) {
    o->OpenNestedMessage("int64_list");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.int64_list());
    o->CloseNestedMessage();
  }
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "bytes_list") {
      if (msg->kind_case() != 0) return false;
      ...
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_bytes_list())) return false;
    }
    else if (identifier == "float_list") {
      if (msg->kind_case() != 0) return false;
      ...
      ProtoSpaceAndComments(scanner);
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_float_list())) return false;
    }
    else if (identifier == "int64_list") {
      if (msg->kind_case() != 0) return false;
      ...
      ProtoSpaceAndComments(scanner);
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_int64_list())) return false;
    }
  }
}
...
bool ProtoParseFromScanner(
    ::tensorflow::strings::Scanner* scanner, bool nested, bool close_curly,
    ::tensorflow::protobuf::Map<string, ::tensorflow::Feature>* map) {
  string map_key;
  bool set_map_key = false;
  ::tensorflow::Feature map_value;
...
void AppendProtoDebugString(
    ...
    const ::tensorflow::Features& msg) {
  {
    ...
    for (const auto& e : msg.feature()) keys.push_back(e.first);
    std::stable_sort(keys.begin(), keys.end());
    for (const auto& key : keys) {
      o->OpenNestedMessage("feature");
      o->AppendString("key", ProtobufStringToString(key));
      o->OpenNestedMessage("value");
      ::tensorflow::internal::AppendProtoDebugString(o, msg.feature().at(key));
      o->CloseNestedMessage();
      o->CloseNestedMessage();
    }
  }
    ...
    if (identifier == "feature") {
      ...
      do {
        ...
        if (!ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->mutable_feature())) return false;
      } while (is_list && scanner->Peek() == ',');
      if (is_list && !scanner->OneLiteral("]").GetResult()) return false;
    }
  }
...
void AppendProtoDebugString(
    ::tensorflow::strings::ProtoTextOutput* o,
    const ::tensorflow::FeatureList& msg) {
  for (int i = 0; i < msg.feature_size(); ++i) {
    o->OpenNestedMessage("feature");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.feature(i));
    o->CloseNestedMessage();
  }
}
  ...
  while(true) {
    ...
    if (identifier == "feature") {
      ...
      do {
        ...
        ProtoSpaceAndComments(scanner);
        if (!::tensorflow::internal::ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->add_feature())) return false;
      } while (is_list && scanner->Peek() == ',');
      if (is_list && !scanner->OneLiteral("]").GetResult()) return false;
    }
  }
...
bool ProtoParseFromScanner(
    ::tensorflow::strings::Scanner* scanner, bool nested, bool close_curly,
    ::tensorflow::protobuf::Map<string, ::tensorflow::FeatureList>* map) {
  string map_key;
  bool set_map_key = false;
  ::tensorflow::FeatureList map_value;
  bool set_map_value = false;
  std::vector<bool> has_seen(2, false);
  while(true) {
    ProtoSpaceAndComments(scanner);
...
void AppendProtoDebugString(
  ...
  {
    std::vector<string> keys;
    for (const auto& e : msg.feature_list()) keys.push_back(e.first);
    std::stable_sort(keys.begin(), keys.end());
    for (const auto& key : keys) {
      o->OpenNestedMessage("feature_list");
      o->AppendString("key", ProtobufStringToString(key));
      o->OpenNestedMessage("value");
      ::tensorflow::internal::AppendProtoDebugString(o, msg.feature_list().at(key));
      o->CloseNestedMessage();
      o->CloseNestedMessage();
    }
  }
  ...
  while(true) {
    ...
    if (identifier == "feature_list") {
      ...
      do {
        ...
        ProtoSpaceAndComments(scanner);
        if (!ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->mutable_feature_list())) return false;
      } while (is_list && scanner->Peek() == ',');
      if (is_list && !scanner->OneLiteral("]").GetResult()) return false;
    }
  }
