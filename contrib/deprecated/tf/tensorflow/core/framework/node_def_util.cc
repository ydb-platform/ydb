...
#if defined(ARCADIA_BUILD_ROOT) /* SummarizeNode() shall log node, not node.def() */
string SummarizeNode(const Node& node) {
  string ret = strings::StrCat(node.def().name(), " = ", node.def().op(), "[");
  strings::StrAppend(&ret, SummarizeAttrsHelper(node.def(), node.def().device()));
  strings::StrAppend(&ret, "](");

  std::vector<const Edge*> edges;
  TF_CHECK_OK(node.input_edges(&edges));
  for (int i = 0; i < edges.size(); ++i) {
    if (i > 0) strings::StrAppend(&ret, ", ");
    strings::StrAppend(&ret, edges[i]->src()->name());
    int o = edges[i]->src_output();
    if (o > 0) strings::StrAppend(&ret, ":", o);
  }
  strings::StrAppend(&ret, ")");
  return ret;
}
