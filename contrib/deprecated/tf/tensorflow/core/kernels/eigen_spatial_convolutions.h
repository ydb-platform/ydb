...
/** Arcadia specific secion: begin */
  if (Eigen::PADDING_DANET == padding_type) {
      TensorIndex pad_rows = (out_height - 1) * row_stride + kernelRows - InputRows;
      TensorIndex pad_cols = (out_width - 1) * col_stride + kernelCols - InputCols;
      if (pad_rows < 0) pad_rows = 0;
      if (pad_cols < 0) pad_cols = 0;
      int shift = padding_type == Eigen::PADDING_DANET ? 1 : 0;
      TensorIndex pad_rows_before = (pad_rows + 1) / 2;
      TensorIndex pad_cols_before = (pad_cols + 1) / 2;
      return choose(
          ...
          input
              .extract_image_patches(kernelRows, kernelCols, row_stride, col_stride,
                                     ...
                                     1, 1,
                                     pad_rows_before, pad_rows - pad_rows_before,
                                     pad_cols_before, pad_cols - pad_cols_before,
                                     0)
              .reshape(pre_contract_dims)
              .contract(kernel.reshape(kernel_dims), contract_dims)
              .reshape(post_contract_dims));
  }
/** Arcadia specific secion: end */
