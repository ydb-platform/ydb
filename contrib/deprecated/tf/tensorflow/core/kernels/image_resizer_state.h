...
#if defined(ARCADIA_BUILD_ROOT) /* Define OpencvResize() */
#include "tensorflow/core/util/port.h"
#include <contrib/libs/opencv/include/opencv2/opencv.hpp>

namespace {

template <typename T> int _OpencvType(int C) { return -1; }
template <> int _OpencvType<float>(int C) { return CV_32FC(C); }
template <> int _OpencvType<unsigned char>(int C) { return CV_8UC(C); }

template <typename TI, typename TO>
inline bool OpencvResize(int method, int N, int C, const TI *i, int iH, int iW,
  TO *o, int oH, int oW) {
  if (N != 1 || C < 1 || C > CV_CN_MAX) {
    return false;
  }
  int itype = _OpencvType<TI>(C);
  int otype = _OpencvType<TO>(C);
  if (itype == -1 || otype == -1) {
    return false;
  }
  cv::Mat i_mat(iH, iW, itype, const_cast<TI*>(i));
  cv::Mat o_mat(oH, oW, otype, o);
  if (std::is_same<TI, TO>::value) {
    cv::resize(i_mat, o_mat, cv::Size(oW, oH), 0, 0, method);
  } else {
    cv::Mat tmp(oH, oW, itype);
    cv::resize(i_mat, tmp, cv::Size(oW, oH), 0, 0, method);
    tmp.convertTo(o_mat, otype, 1, 0);
  }
  return true;
}
