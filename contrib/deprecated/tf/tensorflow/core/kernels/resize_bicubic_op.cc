...
#if defined(ARCADIA_BUILD_ROOT) /* Amend ResizeBicubicOp() for compatibility with Danet if IsOpencvResize() */
    if (IsOpencvResize()) {
      const int N = input_data.dimension(0);
      const int C = input_data.dimension(3);
      const int iH = input_data.dimension(1);
      const int iW = input_data.dimension(2);
      const int oH = output_data.dimension(1);
      const int oW = output_data.dimension(2);
      const T *i = input_data.data();
      float *o = output_data.data();
      if (OpencvResize(cv::INTER_CUBIC, N, C, i, iH, iW, o, oH, oW))
        return;
    }
