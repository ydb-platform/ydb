...
#if defined(ARCADIA_BUILD_ROOT) /* Amend ResizeBilinearOp() for compatibility with Danet if IsOpencvResize() */
    if (IsOpencvResize()) {
      const int N = image_data.dimension(0);
      const int C = image_data.dimension(3);
      const int iH = image_data.dimension(1);
      const int iW = image_data.dimension(2);
      const int oH = output_data.dimension(1);
      const int oW = output_data.dimension(2);
      const T *i = image_data.data();
      float *o = output_data.data();
      if (OpencvResize(cv::INTER_LINEAR, N, C, i, iH, iW, o, oH, oW))
        return;
    }
