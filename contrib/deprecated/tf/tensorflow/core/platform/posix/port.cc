...
#include "absl/base/internal/sysinfo.h"  // Y_IGNORE
#endif
...
#include <unistd.h>
#ifdef TF_USE_SNAPPY
#include <contrib/libs/snappy/include/snappy.h>
#endif
...
int NumSchedulableCPUs() {
  const char* ename = "TF_NUM_THREADS";
  if (getenv(ename)) {
    int n = atoi(getenv(ename));
    if (n > 0) return n;
  }
#if defined(__linux__) && !defined(__ANDROID__)
  cpu_set_t cpuset;
...
bool Snappy_Compress(const char* input, size_t length, string* output) {
  ...
  size_t outlen;
  snappy::RawCompress(input, length, &*output->begin(), &outlen);
