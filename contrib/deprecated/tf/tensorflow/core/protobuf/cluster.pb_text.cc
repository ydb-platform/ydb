...
bool ProtoParseFromScanner(
    ...
    ::tensorflow::protobuf::Map<int32, string>* map) {
  int32 map_key;
...
void AppendProtoDebugString(
  ...
  o->AppendStringIfNotEmpty("name", ProtobufStringToString(msg.name()));
  {
    ...
    for (const auto& e : msg.tasks()) keys.push_back(e.first);
    std::stable_sort(keys.begin(), keys.end());
    for (const auto& key : keys) {
      o->OpenNestedMessage("tasks");
      o->AppendNumeric("key", key);
      o->AppendString("value", ProtobufStringToString(msg.tasks().at(key)));
    ...
    if (identifier == "name") {
      ...
      do {
        ...
        if (!ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->mutable_tasks())) return false;
...
void AppendProtoDebugString(
    ::tensorflow::strings::ProtoTextOutput* o,
    const ::tensorflow::ClusterDef& msg) {
  for (int i = 0; i < msg.job_size(); ++i) {
    o->OpenNestedMessage("job");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.job(i));
    o->CloseNestedMessage();
  }
}
    ...
    if (identifier == "job") {
      ...
      do {
        ...
        if (!::tensorflow::internal::ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->add_job())) return false;
      } while (is_list && scanner->Peek() == ',');
      if (is_list && !scanner->OneLiteral("]").GetResult()) return false;
    }
  }
