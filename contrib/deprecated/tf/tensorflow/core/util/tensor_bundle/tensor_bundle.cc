...
Status ReadStringTensor(io::InputBuffer* buffered_file, size_t num_elements,
  ...
  *actual_crc32c =
  ...
  for (size_t i = 0; i < num_elements; ++i) {
    ...
    size_t bytes_read = 0;
    TF_RETURN_IF_ERROR(
        buffered_file->ReadNBytes(string_length, &*buffer->begin(), &bytes_read));
...
Status BundleWriter::Add(StringPiece key, const Tensor& val) {
  ...
  const string key_string = string(key);
...
Status BundleWriter::AddSlice(StringPiece full_tensor_key,
  ...
  // full tensor.
  const string full_tensor_key_string = string(full_tensor_key);
...
static Status MergeOneBundle(Env* env, StringPiece prefix,
  ...
  for (; iter->Valid(); iter->Next()) {
    const string key = string(iter->key());
