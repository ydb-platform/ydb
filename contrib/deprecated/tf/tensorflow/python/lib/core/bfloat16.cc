...
#include "tensorflow/python/lib/core/safe_ptr.h"
    ...
    typedef PyArray_Descr PyArray_DescrProto;
#endif
...
PyArray_DescrProto NPyBfloat16_Descr = {
...
bool RegisterBfloat16Cast(int numpy_type, bool cast_is_safe) {
  PyArray_Descr* bfloat16_descr = PyArray_DescrFromType(npy_bfloat16_);
  if (!bfloat16_descr) {
      return false;
  }
  ...
  if (PyArray_RegisterCastFunc(PyArray_DescrFromType(numpy_type), npy_bfloat16_,
    ...
    Py_DECREF(bfloat16_descr);
    return false;
  }
  if (PyArray_RegisterCastFunc(bfloat16_descr, numpy_type,
                               NPyCast<bfloat16, T>) < 0) {
    Py_DECREF(bfloat16_descr);
    return false;
  }
  if (cast_is_safe && PyArray_RegisterCanCast(bfloat16_descr, numpy_type,
                                              NPY_NOSCALAR) < 0) {
    Py_DECREF(bfloat16_descr);
    return false;
  }
