...
Status ParseTransformParameters(const string& transforms_string,
  ...
  while (!remaining.empty()) {
    if (state == TRANSFORM_NAME) {
      ...
      func_parameters[string(parameter_name)].push_back(
...
string ExpandPath(const string& path_string) {
#if defined(PLATFORM_WINDOWS)
  return path_string;
#else
  ...
  string::size_type prefix = path_string.find_first_of('/');
  if (path_string.length() == 1 || prefix == 1) {
    ...
    string user(path_string, 1, (prefix == string::npos)
                                         ? string::npos
                                         : prefix - 1);
