...
#error #include "autotrace/autotrace.h"
#endif

#if defined(MAGICKCORE_RSVG_DELEGATE)
#include "librsvg/rsvg.h"
#if !defined(LIBRSVG_CHECK_VERSION)
#error #include "librsvg/rsvg-cairo.h"
#error #include "librsvg/librsvg-features.h"
...
static MagickBooleanType IsSVG(const unsigned char *magick,const size_t length)
{
  ...
  if (length < 14)
    return(MagickFalse);
  if (LocaleNCompare((const char *) magick+1,"!DOCTYPE svg",13) == 0)
    return(MagickTrue);
  return(MagickFalse);
}

  ...
  ExceptionInfo *exception)
{
  ...
  if ((fabs(image->x_resolution) > MagickEpsilon) &&
    ...
    rsvg_handle_set_dpi_x_y(svg_handle,image->x_resolution,image->y_resolution);
