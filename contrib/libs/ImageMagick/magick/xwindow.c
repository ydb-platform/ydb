...
#error #include <X11/Xproto.h>
#error #include <X11/Xlocale.h>
...
#include <sys/ipc.h>
#include <sys/shm.h>
#error #include <X11/extensions/XShm.h>
#endif
#if defined(MAGICKCORE_HAVE_SHAPE)
#error #include <X11/extensions/shape.h>
#endif

/*
  X defines.
...
#define XRedGamma(color) ClampToQuantum(red_gamma == 1.0 ? (double) \
  (color) : ((pow(((double) QuantumScale*(double) (color)),1.0* \
  ...
  const DiversityPacket *pixel)
{
  ...
  intensity=0.298839*(double) pixel->red+0.586811*(double) pixel->green+
    0.114350*(double) pixel->blue;
  return(intensity);
}
