...
void SetAttributes(const std::shared_ptr<arrow20::Field>& field, liborc::Type* type) {
...
Result<std::shared_ptr<DataType>> GetArrowType(const liborc::Type* type) {
  ...
  switch (kind) {
    ...
    case liborc::TIMESTAMP:
      ...
      // arrow20::TimestampArray. In the adapter implementations, we set both writer and
