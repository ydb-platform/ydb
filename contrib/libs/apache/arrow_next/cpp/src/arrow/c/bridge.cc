...
struct ArrayImporter {
  ...
  Result<std::shared_ptr<Array>> MakeArray() {
    ...
    return ::arrow20::MakeArray(data_);
  }
...
class ArrayStreamReader {
 ...
 protected:
  ...
  Result<std::shared_ptr<Array>> ImportArrayInternal(
      struct ArrowArray* array, std::shared_ptr<arrow20::DataType> type) {
    return ImportArray(array, type);
  }

  Result<std::shared_ptr<Array>> ImportArrayInternal(
      struct ArrowDeviceArray* array, std::shared_ptr<arrow20::DataType> type) {
