  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    ...
    if (value.is_array()) {
      ...
      return Status::TypeError("ExecBatch::ToRecordBatch value ", i, " with unsupported ",
                               "value kind ", ::arrow20::ToString(value.kind()));
    }
  }
    ...
    for (size_t i = 0; i < args_->size(); ++i) {
      ...
      if (arg.is_scalar()) {
        ...
        if (carr.num_chunks() > 0) {
          ...
          ::arrow20::internal::FillZeroLengthArray(carr.type().get(),
                                                 &span->values[i].array);
...
struct NullGeneralization {
  ...
  static type Get(const ExecValue& value) {
    ...
    if (!arrow20::internal::may_have_validity_bitmap(dtype_id)) {
      return ALL_VALID;
    }
    if (value.is_scalar()) {
...
ExecContext::ExecContext(MemoryPool* pool, ::arrow20::internal::Executor* executor,
