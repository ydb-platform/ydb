...
class ARROW_EXPORT ExecContext {
 public:
  ...
  explicit ExecContext(MemoryPool* pool = default_memory_pool(),
                       ::arrow20::internal::Executor* executor = NULLPTR,
  ...
  const ::arrow20::internal::CpuInfo* cpu_info() const;
...
struct ExecValue {
  ...
  const ExactType& scalar_as() const {
    return ::arrow20::internal::checked_cast<const ExactType&>(*this->scalar);
  }
