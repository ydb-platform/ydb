...
inline const compute::CastOptions* GetCastOptions(const Expression::Call& call) {
  ...
  return ::arrow20::internal::checked_cast<const compute::CastOptions*>(call.options.get());
}
...
inline const compute::MakeStructOptions* GetMakeStructOptions(
  ...
  return ::arrow20::internal::checked_cast<const compute::MakeStructOptions*>(
      call.options.get());
}
...
inline Result<std::shared_ptr<compute::Function>> GetFunction(
  ...
  const TypeHolder& to_type =
      ::arrow20::internal::checked_cast<const compute::CastOptions&>(*call.options).to_type;
