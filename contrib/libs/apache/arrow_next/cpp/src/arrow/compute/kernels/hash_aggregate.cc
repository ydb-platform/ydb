...
using ::arrow20::internal::FirstTimeBitmapWriter;
  ...
  Status Consume(const ExecSpan& batch) override {
    ...
    if (options_.mode == CountOptions::ALL) {
      ...
      if (options_.mode == CountOptions::ONLY_VALID) {  // ONLY_VALID
        if (input.type->id() != arrow20::Type::NA) {
...
struct GroupedMinMaxImpl final : public GroupedAggregator {
  ...
  Result<Datum> Finalize() override {
    ...
    if (!options_.skip_nulls) {
      ...
      arrow20::internal::BitmapAndNot(null_bitmap->data(), 0, has_nulls->data(), 0,
...
struct GroupedMinMaxImpl<Type,
  ...
  using Allocator = arrow20::stl::allocator<char>;
    ...
    for (size_t i = 0; i < values.size(); i++) {
      if (bit_util::GetBit(null_bitmap, i)) {
        ...
        if (value->size() >
            ...
            arrow20::internal::AddWithOverflow(
...
struct GroupedBooleanAggregator : public GroupedAggregator {
  ...
  Status Consume(const ExecSpan& batch) override {
    ...
    if (batch[0].is_array()) {
      ...
      if (input.MayHaveNulls()) {
        arrow20::internal::VisitBitBlocksVoid(
  ...
  Result<Datum> Finalize() override {
    ...
    if (!options_.skip_nulls) {
      ...
      if (null_bitmap) {
        arrow20::internal::BitmapAnd(null_bitmap->data(), /*left_offset=*/0,
...
struct GroupedAnyImpl : public GroupedBooleanAggregator<GroupedAnyImpl> {
  ...
  static void AdjustForMinCount(uint8_t* no_nulls, const uint8_t* seen,
                                int64_t num_groups) {
    arrow20::internal::BitmapOr(no_nulls, /*left_offset=*/0, seen, /*right_offset=*/0,
...
struct GroupedAllImpl : public GroupedBooleanAggregator<GroupedAllImpl> {
  ...
  static void AdjustForMinCount(uint8_t* no_nulls, const uint8_t* seen,
                                int64_t num_groups) {
    arrow20::internal::BitmapOrNot(no_nulls, /*left_offset=*/0, seen, /*right_offset=*/0,
                                 num_groups, /*out_offset=*/0, no_nulls);
  }
};
    ...
    if (options_.mode == CountOptions::ALL ||
      ...
      for (int64_t i = 0; i < list->length(); i++) {
        ...
        const int64_t null_count =
            slot_length - arrow20::internal::CountSetBits(values->null_bitmap()->data(),
