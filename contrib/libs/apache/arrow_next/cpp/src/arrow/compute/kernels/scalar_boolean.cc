...
struct AndOp : Commutative<AndOp> {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const ArraySpan& right,
                     ExecResult* out) {
    ArraySpan* out_span = out->array_span_mutable();
    ::arrow20::internal::BitmapAnd(left.buffers[1].data, left.offset, right.buffers[1].data,
...
struct KleeneAndOp : Commutative<KleeneAndOp> {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const Scalar& right,
    ...
    if (left.GetNullCount() == 0) {
      ::arrow20::internal::InvertBitmap(left.buffers[1].data, left.offset, left.length,
                                      out_span->buffers[0].data, out_span->offset);
    } else {
      ::arrow20::internal::BitmapAndNot(left.buffers[0].data, left.offset,
                                      left.buffers[1].data, left.offset, left.length,
                                      out_span->offset, out_span->buffers[0].data);
    }
    ::arrow20::internal::CopyBitmap(left.buffers[1].data, left.offset, left.length,
...
struct OrOp : Commutative<OrOp> {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const ArraySpan& right,
                     ExecResult* out) {
    ArraySpan* out_span = out->array_span_mutable();
    ::arrow20::internal::BitmapOr(left.buffers[1].data, left.offset, right.buffers[1].data,
                                right.offset, right.length, out_span->offset,
                                out_span->buffers[1].data);
    return Status::OK();
  }
...
struct KleeneOrOp : Commutative<KleeneOrOp> {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const Scalar& right,
    ...
    if (left.GetNullCount() == 0) {
      ...
      ::arrow20::internal::BitmapAnd(left.buffers[0].data, left.offset,
                                   left.buffers[1].data, left.offset, left.length,
                                   out_span->offset, out_span->buffers[0].data);
    }
    ::arrow20::internal::CopyBitmap(left.buffers[1].data, left.offset, left.length,
...
struct XorOp : Commutative<XorOp> {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const ArraySpan& right,
                     ExecResult* out) {
    ArraySpan* out_span = out->array_span_mutable();
    ::arrow20::internal::BitmapXor(left.buffers[1].data, left.offset, right.buffers[1].data,
                                 right.offset, right.length, out_span->offset,
                                 out_span->buffers[1].data);
    return Status::OK();
  }
...
struct AndNotOp {
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& left, const ArraySpan& right,
                     ExecResult* out) {
    ArraySpan* out_span = out->array_span_mutable();
    ::arrow20::internal::BitmapAndNot(left.buffers[1].data, left.offset,
...
struct KleeneAndNotOp {
  static Status Call(KernelContext* ctx, const Scalar& left, const ArraySpan& right,
    ...
    if (right.GetNullCount() == 0) {
      ::arrow20::internal::CopyBitmap(right.buffers[1].data, right.offset, right.length,
                                    out_span->buffers[0].data, out_span->offset);
    } else {
      ::arrow20::internal::BitmapAnd(right.buffers[0].data, right.offset,
                                   right.buffers[1].data, right.offset, right.length,
                                   out_span->offset, out_span->buffers[0].data);
    }
    ::arrow20::internal::InvertBitmap(right.buffers[1].data, right.offset, right.length,
                                    out_span->buffers[1].data, out_span->offset);
    return Status::OK();
  }
