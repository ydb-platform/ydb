...
struct CompareKernel {
  ...
  static Status Exec(KernelContext* ctx, const ExecSpan& batch, ExecResult* out) {
    ...
    if (!out_is_byte_aligned) {
      ::arrow20::internal::CopyBitmap(out_buffer, /*offset=*/0, batch.length,
    ...
    if (options.skip_nulls && initialize_output) {
      ...
      if (std::all_of(arrays.begin(), arrays.end(),
        ...
        for (const ArraySpan* arr : arrays) {
          ...
          if (!output->buffers[0]) {
            ...
            ::arrow20::internal::CopyBitmap(arr->buffers[0].data, arr->offset, batch.length,
                                          output->buffers[0]->mutable_data(),
                                          /*dest_offset=*/0);
          } else {
            ::arrow20::internal::BitmapOr(output->buffers[0]->data(), /*left_offset=*/0,
      ...
      for (const ArraySpan* arr : arrays) {
        ...
        if (!output->buffers[0]) {
          ...
          ::arrow20::internal::BitmapAnd(output->buffers[0]->data(), /*left_offset=*/0,
                                       arr->buffers[0].data, arr->offset, batch.length,
                                       /*out_offset=*/0,
                                       output->buffers[0]->mutable_data());
        }
