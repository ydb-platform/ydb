...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/compute/kernels/copy_data_internal.h"
      ...
      if (need_to_allocate) {  // NullHandling::COMPUTED_NO_PREALLOCATE
        ...
        if (cond.offset) {
          ARROW_ASSIGN_OR_RAISE(
              out_arr->buffers[0],
              arrow20::internal::CopyBitmap(ctx->memory_pool(), cond.buffers[0].data,
        ...
        arrow20::internal::CopyBitmap(cond.buffers[0].data, cond.offset, cond.length,
...
Status RunIfElseScalar(const BooleanScalar& cond, const ExecValue& left,
  ...
  if (valid_data.is_array()) {
    ...
    if (valid_array.MayHaveNulls()) {
      arrow20::internal::CopyBitmap(valid_array.buffers[0].data, valid_array.offset,
...
struct IfElseFunctor<Type, enable_if_boolean<Type>> {
  ...
  static Status Call(KernelContext* ctx, const BooleanScalar& cond, const ExecValue& left,
    ...
    return RunIfElseScalar(
        ...
        /*CopyArrayData*/
        [&](const ArraySpan& valid_array, ArraySpan* out_array) {
          arrow20::internal::CopyBitmap(valid_array.buffers[1].data, valid_array.offset,
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& cond, const ArraySpan& left,
    ...
    // out_buff = right & ~cond
    arrow20::internal::BitmapAndNot(right.buffers[1].data, right.offset,
    ...
    ARROW_ASSIGN_OR_RAISE(
        ...
        arrow20::internal::BitmapAnd(ctx->memory_pool(), left.buffers[1].data, left.offset,
                                   cond.buffers[1].data, cond.offset, cond.length, 0));

    arrow20::internal::BitmapOr(out_arr->buffers[1].data, out_arr->offset, temp_buf->data(),
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& cond, const Scalar& left,
    ...
    if (left_data) {
      arrow20::internal::BitmapOr(out_arr->buffers[1].data, out_arr->offset,
                                cond.buffers[1].data, cond.offset, cond.length,
                                out_arr->offset, out_arr->buffers[1].data);
    }
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& cond, const ArraySpan& left,
    ...
    // out_buff = left & cond
    arrow20::internal::BitmapAnd(left.buffers[1].data, left.offset, cond.buffers[1].data,
    ...
    if (right_data) {
      arrow20::internal::BitmapOrNot(out_arr->buffers[1].data, out_arr->offset,
                                   cond.buffers[1].data, cond.offset, cond.length,
                                   out_arr->offset, out_arr->buffers[1].data);
    }
  ...
  static Status Call(KernelContext* ctx, const ArraySpan& cond, const Scalar& left,
    ...
    if (left_data) {
      if (right_data) {
        ...
        // out_buf = cond
        arrow20::internal::CopyBitmap(cond.buffers[1].data, cond.offset, cond.length,
                                    out_buf, out_arr->offset);
      }
    } else {
      if (right_data) {
        // out_buf = ~cond
        arrow20::internal::InvertBitmap(cond.buffers[1].data, cond.offset, cond.length,
                                      out_buf, out_arr->offset);
      } else {
        // out_buf = zeros
...
struct IfElseFunctor<Type, enable_if_fixed_size_binary<Type>> {
  ...
  static const uint8_t* UnboxBinaryScalar(const Scalar& scalar) {
    return reinterpret_cast<const uint8_t*>(
        checked_cast<const arrow20::internal::PrimitiveScalarBase&>(scalar).view().data());
  }
    ...
    if (out_valid) {
      if (array.MayHaveNulls()) {
        if (length == 1) {
          ...
          arrow20::internal::CopyBitmap(array.buffers[0].data, array.offset + in_offset,
  ...
  for (const ExecValue& value : batch.values) {
    if (value.null_count() == 0) {
      ...
      if (in_null_count < 0.8 * batch.length) {
        ...
        arrow20::internal::BitmapOr(out_valid, out_offset, in_valid, in_offset,
...
Status ExecArrayScalarCoalesce(KernelContext* ctx, const ExecValue& left,
  ...
  if (right_scalar.is_valid || !left_valid) {
    ...
    arrow20::internal::CopyBitmap(left_valid, left_arr.offset, length, out_valid,
                                out_offset);
  }
  return Status::OK();
}
...
Status ExecBinaryCoalesce(KernelContext* ctx, const ExecValue& left,
  ...
  if (right_null_count == 0) {
    ...
    arrow20::internal::BitmapOr(left_valid, left_arr.offset, right_valid, right_arr.offset,
