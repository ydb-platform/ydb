...
struct CharacterPredicateUnicode {
  static bool Call(KernelContext*, const uint8_t* input, size_t input_string_ncodeunits,
    ...
    bool all;
    bool any = false;
    if (!ARROW_PREDICT_TRUE(arrow20::util::UTF8AllOf(
...
struct IsTitleUnicode {
  static bool Call(KernelContext*, const uint8_t* input, size_t input_string_ncodeunits,
    ...
    bool rule_3 = false;
    bool status =
        arrow20::util::UTF8AllOf(input, input + input_string_ncodeunits, &rules_1_and_2,
...
struct StringTransformCodepoint : public FunctionalCaseMappingTransform {
  int64_t Transform(const uint8_t* input, int64_t input_string_ncodeunits,
    ...
    if (ARROW_PREDICT_FALSE(
            !arrow20::util::UTF8Transform(input, input + input_string_ncodeunits, &output,
...
struct Utf8NormalizeBase {
  ...
  Result<int64_t> Decompose(std::string_view v, BufferBuilder* data_builder) {
    if (::arrow20::util::ValidateAscii(v)) {
...
struct UTF8TrimState {
  ...
  explicit UTF8TrimState(KernelContext* ctx, TrimOptions options)
    ...
    if (!ARROW_PREDICT_TRUE(
            arrow20::util::UTF8ForEach(options_.characters, [&](uint32_t c) {
              codepoints_.resize(
...
struct UTF8TrimTransform : public StringTransformBase {
  ...
  int64_t Transform(const uint8_t* input, int64_t input_string_ncodeunits,
    ...
    if (TrimLeft && !ARROW_PREDICT_TRUE(
                        arrow20::util::UTF8FindIf(begin, end, predicate, &begin_trimmed))) {
      return kStringTransformError;
    }
    if (TrimRight && begin_trimmed < end) {
      if (!ARROW_PREDICT_TRUE(arrow20::util::UTF8FindIfReverse(begin_trimmed, end,
...
struct Utf8ReplaceSliceTransform : ReplaceStringSliceTransformBase {
  ...
  int64_t Transform(const uint8_t* input, int64_t input_string_ncodeunits,
    ...
    if (opts.start >= 0) {
      // Count from left
      if (!arrow20::util::UTF8AdvanceCodepoints(begin, end, &begin_sliced, opts.start)) {
        return kStringTransformError;
      }
      if (opts.stop > options->start) {
        // Continue counting from left
        const int64_t length = opts.stop - options->start;
        if (!arrow20::util::UTF8AdvanceCodepoints(begin_sliced, end, &end_sliced, length)) {
          return kStringTransformError;
        }
      } else if (opts.stop < 0) {
        // Count from right
        if (!arrow20::util::UTF8AdvanceCodepointsReverse(begin_sliced, end, &end_sliced,
                                                       -opts.stop)) {
          return kStringTransformError;
        }
      } else {
      ...
      // Count from right
      if (!arrow20::util::UTF8AdvanceCodepointsReverse(begin, end, &begin_sliced,
                                                     -opts.start)) {
        return kStringTransformError;
      }
      if (opts.stop >= 0) {
        // Restart counting from left
        if (!arrow20::util::UTF8AdvanceCodepoints(begin, end, &end_sliced, opts.stop)) {
          return kStringTransformError;
        }
...
#define RETURN_IF_UTF8_ERROR(expr)    \
  ...
  int64_t SliceForward(const uint8_t* input, int64_t input_string_ncodeunits,
    ...
    if (opt.start >= 0) {
      ...
      RETURN_IF_UTF8_ERROR(
          arrow20::util::UTF8AdvanceCodepoints(begin, end, &begin_sliced, opt.start));
      if (opt.stop > opt.start) {
        // continue counting from begin_sliced
        const int64_t length = opt.stop - opt.start;
        RETURN_IF_UTF8_ERROR(
            arrow20::util::UTF8AdvanceCodepoints(begin_sliced, end, &end_sliced, length));
      } else if (opt.stop < 0) {
        // or from the end (but we will never need to < begin_sliced)
        RETURN_IF_UTF8_ERROR(arrow20::util::UTF8AdvanceCodepointsReverse(
      ...
      if (opt.stop > 0) {
        ...
        RETURN_IF_UTF8_ERROR(
            arrow20::util::UTF8AdvanceCodepoints(begin, end, &end_sliced, opt.stop));
    ...
    while (i < end_sliced) {
      ...
      RETURN_IF_UTF8_ERROR(arrow20::util::UTF8Decode(&i, &codepoint));
      dest = arrow20::util::UTF8Encode(dest, codepoint);
  ...
  int64_t SliceBackward(const uint8_t* input, int64_t input_string_ncodeunits,
    ...
    if (opt.start >= 0) {
      ...
      RETURN_IF_UTF8_ERROR(
          arrow20::util::UTF8AdvanceCodepoints(begin, end, &begin_sliced, opt.start + 1));
    } else {
    ...
    if (opt.stop >= 0) {
      ...
      RETURN_IF_UTF8_ERROR(
          arrow20::util::UTF8AdvanceCodepoints(begin, end, &end_sliced, length));
    } else {
      RETURN_IF_UTF8_ERROR(arrow20::util::UTF8AdvanceCodepointsReverse(
    ...
    while (i > end_sliced) {
      uint32_t codepoint = 0;
      // write a single codepoint
      RETURN_IF_UTF8_ERROR(arrow20::util::UTF8DecodeReverse(&i, &codepoint));
      dest = arrow20::util::UTF8Encode(dest, codepoint);
      // and skip the remainder
      int64_t skips = -opt.step - 1;
      while ((skips--) && (i > end_sliced)) {
...
struct SplitWhitespaceUtf8Finder : public StringSplitFinderBase<SplitOptions> {
  ...
  bool Find(const uint8_t* begin, const uint8_t* end, const uint8_t** separator_begin,
    ...
    while ((i < end)) {
      uint32_t codepoint = 0;
      *separator_begin = i;
      if (ARROW_PREDICT_FALSE(!arrow20::util::UTF8Decode(&i, &codepoint))) {
        return false;
      }
  ...
  bool FindReverse(const uint8_t* begin, const uint8_t* end,
    ...
    while ((i >= begin)) {
      uint32_t codepoint = 0;
      *separator_end = i + 1;
      if (ARROW_PREDICT_FALSE(!arrow20::util::UTF8DecodeReverse(&i, &codepoint))) {
        return false;
      }
      if (IsSpaceCharacterUnicode(codepoint)) {
        do {
