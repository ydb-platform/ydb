...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/compute/kernels/vector_selection_filter_internal.h"
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/compute/kernels/vector_selection_take_internal.h"
...
std::shared_ptr<arrow20::BooleanArray> MakeDropNullFilter(const Array& values) {
...
Result<Datum> DropNullRecordBatch(const std::shared_ptr<RecordBatch>& batch,
  ...
  for (const auto& column : batch->columns()) {
    ...
    if (column->null_bitmap_data()) {
      ::arrow20::internal::BitmapAnd(column->null_bitmap_data(), column->offset(),
...
Result<Datum> DropNullTable(const std::shared_ptr<Table>& table, ExecContext* ctx) {
  ...
  arrow20::RecordBatchVector filtered_batches;
