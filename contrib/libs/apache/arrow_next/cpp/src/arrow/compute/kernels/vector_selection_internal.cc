...
void VisitPlainxREEFilterOutputSegmentsImpl(
  ...
  const ArraySpan& filter_values = arrow20::ree_util::ValuesArray(filter);
  ...
  const arrow20::ree_util::RunEndEncodedArraySpan<FilterRunEndCType> filter_span(filter);
...
struct Selection {
  ...
  Status VisitTake(ValidVisitor&& visit_valid, NullVisitor&& visit_null) {
    ...
    arrow20::internal::OptionalBitIndexer indices_is_valid(is_valid, selection.offset);
    arrow20::internal::OptionalBitIndexer values_is_valid(values.buffers[0].data,
                                                        values.offset);

    const bool values_have_nulls = values.MayHaveNulls();
    arrow20::internal::OptionalBitBlockCounter bit_counter(is_valid, selection.offset,
                                                         selection.length);
    int64_t position = 0;
  ...
  Status VisitFilter(ValidVisitor&& visit_valid, NullVisitor&& visit_null) {
    ...
    arrow20::internal::OptionalBitBlockCounter values_valid_counter(
        values.buffers[0].data, values.offset, values.length);
    arrow20::internal::OptionalBitBlockCounter filter_valid_counter(
        filter_is_valid, filter_offset, selection.length);
    arrow20::internal::BitBlockCounter filter_counter(filter_data, filter_offset,
                                                    selection.length);

    int64_t in_position = 0;
    while (in_position < selection.length) {
      arrow20::internal::BitBlockCount filter_valid_block = filter_valid_counter.NextWord();
      arrow20::internal::BitBlockCount values_valid_block = values_valid_counter.NextWord();
      arrow20::internal::BitBlockCount filter_block = filter_counter.NextWord();
