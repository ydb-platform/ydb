...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/compute/kernels/gather_internal.h"
...
Result<std::shared_ptr<ArrayData>> GetTakeIndicesFromBitmapImpl(
  ...
  if (have_filter_nulls) {
    ...
    RETURN_NOT_OK(::arrow20::internal::VisitSetBitRuns(
...
Result<std::shared_ptr<ArrayData>> GetTakeIndicesFromREEBitmapImpl(
  ...
  const ::arrow20::ree_util::RunEndEncodedArraySpan<T> filter_span(filter);
...
struct FixedWidthTakeImpl {
  ...
  static Status Exec(KernelContext* ctx, const ArraySpan& values,
    ...
    arrow20::internal::Gather<kValueWidthInBits, IndexCType, WithFactor::value> gather{
