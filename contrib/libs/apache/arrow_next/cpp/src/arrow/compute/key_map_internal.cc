...
void SwissTable::init_slot_ids(const int num_keys, const uint16_t* selection,
  ...
  if (log_blocks_ == 0) {
    for (int i = 0; i < num_keys; ++i) {
      ...
      uint32_t match = ::arrow20::bit_util::GetBit(match_bitvector, id) ? 1 : 0;
...
void SwissTable::run_comparisons(const int num_keys,
  ...
  if (!optional_selection_ids && optional_selection_bitvector) {
    ...
    //
    int64_t num_matches = arrow20::internal::CountSetBits(optional_selection_bitvector,
                                                        /*offset=*/0, num_keys);
...
void SwissTable::find(const int num_keys, const uint32_t* hashes,
  ...
  int64_t num_matches = arrow20::internal::CountSetBits(inout_match_bitvector,
                                                      /*offset=*/0, num_keys);
  ...
  while (num_ids > 0) {
    ...
    for (int i = 0; i < num_ids_last_iteration; ++i) {
      ...
      if (!match_found) {
        ::arrow20::bit_util::ClearBit(inout_match_bitvector, id);
      } else {
        ids[num_ids++] = id;
      }
    }
...
Status SwissTable::map_new_keys_helper(
  ...
  for (num_processed = 0; num_processed < *inout_num_selected; ++num_processed) {
    ...
    if (!match_found) {
      ...
      ::arrow20::bit_util::ClearBit(match_bitvector, num_processed);
      ++num_inserted_new;
