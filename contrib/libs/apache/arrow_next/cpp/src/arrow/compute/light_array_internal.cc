...
Result<KeyColumnMetadata> ColumnMetadataFromDataType(
  ...
  const std::shared_ptr<DataType>& typ =
      is_extension ? arrow20::internal::checked_cast<const ExtensionType*>(type.get())
                         ->storage_type()
                   : type;

  if (typ->id() == Type::DICTIONARY) {
    auto bit_width =
        arrow20::internal::checked_cast<const FixedWidthType&>(*typ).bit_width();
  ...
  if (is_fixed_width(typ->id())) {
    return KeyColumnMetadata(
        true, arrow20::internal::checked_cast<const FixedWidthType&>(*typ).bit_width() / 8);
  }
...
std::shared_ptr<ArrayData> ResizableArrayData::array_data() const {
  auto valid_count =
      arrow20::internal::CountSetBits(buffers_[kValidityBuffer]->data(), /*bit_offset=*/0,
  ...
  if (column_metadata.is_fixed_length) {
    ...
    for (int i = 0; i < num_rows_to_append; ++i) {
      ...
      if (ARROW_PREDICT_FALSE(
              arrow20::internal::AddWithOverflow(sum, length, &new_sum_maybe_overflow))) {
