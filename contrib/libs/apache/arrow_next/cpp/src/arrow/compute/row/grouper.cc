...
struct GrouperFastImpl : public Grouper {
  ...
  static Result<std::unique_ptr<GrouperFastImpl>> Make(
    ...
    impl->encode_ctx_.hardware_flags =
        arrow20::internal::CpuInfo::GetInstance()->hardware_flags();
  ...
  Result<Datum> ConsumeImpl(const ExecSpan& batch, GrouperMode mode) {
    ...
    std::shared_ptr<arrow20::Buffer> group_ids, null_bitmap;
    ...
    for (uint32_t start_row = 0; start_row < num_rows;) {
      ...
      if (mode == GrouperMode::kLookup) {
        ...
        ::arrow20::internal::CopyBitmap(match_bitvector.mutable_data(), /*offset=*/0,
  ...
  Result<ExecBatch> GetUniques() override {
    ...
    for (size_t i = 0; i < num_columns; ++i) {
      ...
      auto valid_count = arrow20::internal::CountSetBits(
  ...
  static constexpr int minibatch_size_max_ = arrow20::util::MiniBatch::kMiniBatchLength;
  static constexpr int minibatch_size_min_ = 128;
  int minibatch_size_;

  ExecContext* ctx_;
  arrow20::util::TempVectorStack temp_stack_;
