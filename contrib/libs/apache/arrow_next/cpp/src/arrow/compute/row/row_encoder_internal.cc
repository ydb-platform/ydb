...
Status FixedWidthKeyEncoder::Encode(const ExecValue& data, int64_t batch_length,
  ...
  if (data.is_array()) {
    ...
    const auto& scalar = data.scalar_as<arrow20::internal::PrimitiveScalarBase>();
    if (scalar.is_valid) {
...
void RowEncoder::Init(const std::vector<TypeHolder>& column_types, ExecContext* ctx) {
  ...
  for (size_t i = 0; i < column_types.size(); ++i) {
    ...
    const TypeHolder& type =
        is_extension
            ? arrow20::internal::checked_cast<const ExtensionType*>(column_types[i].type)
                  ->storage_type()
            : column_types[i];

    if (is_extension) {
      extension_types_[i] = arrow20::internal::checked_pointer_cast<ExtensionType>(
...
Result<ExecBatch> RowEncoder::Decode(int64_t num_rows, const int32_t* row_ids) {
  ...
  for (size_t i = 0; i < encoders_.size(); ++i) {
    ...
    if (extension_types_[i] != nullptr) {
      ARROW_ASSIGN_OR_RAISE(out.values[i], ::arrow20::internal::GetArrayView(
