    ...
    return decoded_arrays_fut.Then(
        [state, bytes_parsed_or_skipped](
          ...
          ARROW_ASSIGN_OR_RAISE(auto decoded_arrays,
                                arrow20::internal::UnwrapOrRaise(maybe_decoded_arrays));
    ...
    Result<std::shared_ptr<RecordBatch>> DecodedArraysToBatch(
      ...
      if (schema == nullptr) {
        ...
        if (n_rows == 0) {
          ...
          return RecordBatch::Make(arrow20::schema(std::move(fields)), n_rows,
                                   std::move(arrays));
        }

        schema = arrow20::schema(std::move(fields));
      }
 ...
 protected:
  ...
  Result<std::shared_ptr<Table>> MakeTable() {
    ...
    for (int32_t i = 0; i < static_cast<int32_t>(column_builders_.size()); ++i) {
      ...
      fields.push_back(::arrow20::field(column.name, array->type()));
