...
class CSVWriterImpl : public ipc::RecordBatchWriter {
 public:
  static Result<std::shared_ptr<CSVWriterImpl>> Make(
    ...
    ARROW_ASSIGN_OR_RAISE(std::shared_ptr<Buffer> null_string,
                          arrow20::AllocateBuffer(options.null_string.length()));
  ...
  CSVWriterImpl(io::OutputStream* sink, std::shared_ptr<io::OutputStream> owned_sink,
      ...
      : sink_(sink),
        ...
        offsets_(0, 0, ::arrow20::stl::allocator<char*>(options.io_context.pool())),
 ...
 private:
  ...
  std::vector<int64_t, arrow20::stl::allocator<int64_t>> offsets_;
...
Status WriteCSV(const std::shared_ptr<RecordBatchReader>& reader,
                const WriteOptions& options, arrow20::io::OutputStream* output) {
