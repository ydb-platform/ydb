...
using ::arrow20::internal::IOErrorFromErrno;
#ifdef _WIN32
using ::arrow20::internal::IOErrorFromWinError;
#endif
using ::arrow20::internal::NativePathString;
using ::arrow20::internal::PlatformFilename;
...
Result<LocalFileSystemOptions> LocalFileSystemOptions::FromUri(
    const ::arrow20::util::Uri& uri, std::string* out_path) {
  ...
  for (const auto& [key, value] : params) {
    if (key == "use_mmap") {
      if (value.empty()) {
        ...
        ARROW_ASSIGN_OR_RAISE(options.use_mmap, ::arrow20::internal::ParseBoolean(value));
      }
      break;
    }
  }
...
bool LocalFileSystem::Equals(const FileSystem& other) const {
  if (other.type_name() != type_name()) {
    ...
    const auto& localfs = ::arrow20::internal::checked_cast<const LocalFileSystem&>(other);
 ...
 public:
  ...
  static Result<AsyncGenerator<FileInfoGenerator>> DiscoverDirectoryFiles(
    ...
    ARROW_ASSIGN_OR_RAISE(
        auto base_dir, arrow20::internal::PlatformFilename::FromString(selector.base_dir));
  ...
  static arrow20::Result<FileInfoGenerator> DiscoverDirectoriesFlattened(
      FileSelector selector, LocalFileSystemOptions fs_opts,
      const io::IOContext& io_context) {
 ...
 private:
  ...
  class DiscoveryImplIterator {
   ...
   public:
    ...
    Status Initialize() {
      auto result = arrow20::internal::ListDir(dir_fn_);
      if (!result.ok()) {
        auto status = result.status();
  ...
  static Status DoDiscovery(const PlatformFilename& dir_fn, int32_t nesting_depth,
    ...
    ARROW_RETURN_IF(discovery_state->producer.is_closed(),
                    arrow20::Status::Cancelled("Discovery cancelled"));
...
Status LocalFileSystem::CreateDir(const std::string& path, bool recursive) {
  ...
  ARROW_ASSIGN_OR_RAISE(auto fn, PlatformFilename::FromString(path));
  if (recursive) {
    return ::arrow20::internal::CreateDirTree(fn).status();
  } else {
    return ::arrow20::internal::CreateDir(fn).status();
  }
}
...
Status LocalFileSystem::DeleteDir(const std::string& path) {
  RETURN_NOT_OK(ValidatePath(path));
  ARROW_ASSIGN_OR_RAISE(auto fn, PlatformFilename::FromString(path));
  auto st = ::arrow20::internal::DeleteDirTree(fn, /*allow_not_found=*/false).status();
  if (!st.ok()) {
...
Status LocalFileSystem::DeleteDirContents(const std::string& path, bool missing_dir_ok) {
  ...
  ARROW_ASSIGN_OR_RAISE(auto fn, PlatformFilename::FromString(path));
  auto st = ::arrow20::internal::DeleteDirContents(fn, missing_dir_ok).status();
  if (!st.ok()) {
    std::stringstream ss;
...
Status LocalFileSystem::DeleteFile(const std::string& path) {
  RETURN_NOT_OK(ValidatePath(path));
  ARROW_ASSIGN_OR_RAISE(auto fn, PlatformFilename::FromString(path));
  return ::arrow20::internal::DeleteFile(fn, /*allow_not_found=*/false).status();
}
...
Result<std::shared_ptr<io::OutputStream>> OpenOutputStreamGeneric(const std::string& path,
  ...
  ARROW_ASSIGN_OR_RAISE(
      auto fd, ::arrow20::internal::FileOpenWritable(fn, write_only, truncate, append));
  int raw_fd = fd.Detach();
  auto maybe_stream = io::FileOutputStream::Open(raw_fd);
  if (!maybe_stream.ok()) {
    ARROW_UNUSED(::arrow20::internal::FileClose(raw_fd));
  }
  return maybe_stream;
}
...
static Result<std::shared_ptr<fs::FileSystem>> LocalFileSystemFactory(
    const arrow20::util::Uri& uri, const io::IOContext& io_context, std::string* out_path) {
  std::string path;
