...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/flight/types.h"  // IWYU pragma: keep
...
class ARROW_FLIGHT_EXPORT FlightWriteSizeStatusDetail : public arrow20::StatusDetail {
 public:
...
class ARROW_FLIGHT_EXPORT FlightStreamReader : public MetadataRecordBatchReader {
 public:
  ...
  virtual arrow20::Result<std::vector<std::shared_ptr<RecordBatch>>> ToRecordBatches(
      const StopToken& stop_token) = 0;

  using MetadataRecordBatchReader::ToTable;
  /// \brief Consume entire stream as a Table
  arrow20::Result<std::shared_ptr<Table>> ToTable(const StopToken& stop_token);
};
...
class ARROW_FLIGHT_EXPORT FlightClient {
 public:
  ...
  static arrow20::Result<std::unique_ptr<FlightClient>> Connect(const Location& location);
  ...
  /// \return Arrow result with the created FlightClient, OK status may not indicate that
  /// the connection was successful
  static arrow20::Result<std::unique_ptr<FlightClient>> Connect(
  ...
  /// successfully
  arrow20::Result<std::pair<std::string, std::string>> AuthenticateBasicToken(
  ...
  arrow20::Result<std::unique_ptr<ResultStream>> DoAction(const FlightCallOptions& options,
                                                        const Action& action);
  arrow20::Result<std::unique_ptr<ResultStream>> DoAction(const Action& action) {
  ...
  arrow20::Result<CancelFlightInfoResult> CancelFlightInfo(
  ...
  arrow20::Result<FlightEndpoint> RenewFlightEndpoint(
  ...
  arrow20::Result<std::vector<ActionType>> ListActions(const FlightCallOptions& options);
  arrow20::Result<std::vector<ActionType>> ListActions() {
  ...
  arrow20::Result<std::unique_ptr<FlightInfo>> GetFlightInfo(
  ...
  /// \param[in] options Per-RPC options
  /// \param[in] descriptor the dataset request
  arrow20::Future<FlightInfo> GetFlightInfoAsync(const FlightCallOptions& options,
                                               const FlightDescriptor& descriptor);
  arrow20::Future<FlightInfo> GetFlightInfoAsync(const FlightDescriptor& descriptor) {
  ...
  /// the requested query
  arrow20::Result<std::unique_ptr<PollInfo>> PollFlightInfo(
      const FlightCallOptions& options, const FlightDescriptor& descriptor);
  arrow20::Result<std::unique_ptr<PollInfo>> PollFlightInfo(
      const FlightDescriptor& descriptor) {
  ...
  arrow20::Result<std::unique_ptr<SchemaResult>> GetSchema(
      const FlightCallOptions& options, const FlightDescriptor& descriptor);
  ...
  arrow20::Result<std::unique_ptr<FlightListing>> ListFlights();
  ...
  arrow20::Result<std::unique_ptr<FlightStreamReader>> DoGet(
      const FlightCallOptions& options, const Ticket& ticket);
  arrow20::Result<std::unique_ptr<FlightStreamReader>> DoGet(const Ticket& ticket) {
    return DoGet({}, ticket);
  }
  ...
  arrow20::Result<DoPutResult> DoPut(const FlightCallOptions& options,
                                   const FlightDescriptor& descriptor,
                                   const std::shared_ptr<Schema>& schema);

  arrow20::Result<DoPutResult> DoPut(const FlightDescriptor& descriptor,
                                   const std::shared_ptr<Schema>& schema) {
  ...
  arrow20::Result<DoExchangeResult> DoExchange(const FlightCallOptions& options,
                                             const FlightDescriptor& descriptor);
  arrow20::Result<DoExchangeResult> DoExchange(const FlightDescriptor& descriptor) {
  ...
  ::arrow20::Result<SetSessionOptionsResult> SetSessionOptions(
  ...
  ::arrow20::Result<GetSessionOptionsResult> GetSessionOptions(
  ...
  ::arrow20::Result<CloseSessionResult> CloseSession(const FlightCallOptions& options,
