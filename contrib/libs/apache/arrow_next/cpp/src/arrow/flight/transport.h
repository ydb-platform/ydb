...
struct FlightData {
  ...
  ::arrow20::Result<std::unique_ptr<ipc::Message>> OpenMessage();
};
...
class ARROW_FLIGHT_EXPORT TransportDataStream {
 public:
  ...
  virtual arrow20::Result<bool> WriteData(const FlightPayload& payload);
...
class ARROW_FLIGHT_EXPORT ClientTransport {
 public:
  ...
  virtual arrow20::Result<std::pair<std::string, std::string>> AuthenticateBasicToken(
  ...
  virtual arrow20::Result<std::unique_ptr<SchemaResult>> GetSchema(
...
class ARROW_FLIGHT_EXPORT TransportRegistry {
 public:
  using ClientFactory = std::function<arrow20::Result<std::unique_ptr<ClientTransport>>()>;
  using ServerFactory = std::function<arrow20::Result<std::unique_ptr<ServerTransport>>(
      FlightServerBase*, std::shared_ptr<MemoryManager> memory_manager)>;

  TransportRegistry();
  ~TransportRegistry();

  arrow20::Result<std::unique_ptr<ClientTransport>> MakeClient(
      const std::string& scheme) const;
  arrow20::Result<std::unique_ptr<ServerTransport>> MakeServer(
...
class ARROW_FLIGHT_EXPORT AsyncRpc {
 public:
  ...
  /// Only needed for DoPut/DoExchange
  virtual void Write(arrow20::flight::FlightStreamChunk chunk) {}
  /// Only needed for DoPut/DoExchange
