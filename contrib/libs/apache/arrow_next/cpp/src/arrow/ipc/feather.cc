...
#include "contrib/libs/apache/arrow_next/cpp/src/generated/feather.fbs.h"
 ...
 public:
  ...
  Status ReadSchema() {
    ...
    for (int i = 0; i < static_cast<int>(metadata_->columns()->size()); ++i) {
      ...
      fields.push_back(::arrow20::field(col->name()->str(), type));
    }
    schema_ = ::arrow20::schema(std::move(fields));
    return Status::OK();
  }
...
#undef PRIMITIVE_CASE
  ...
  Status Read(const std::vector<int>& indices, std::shared_ptr<Table>* out) override {
    ...
    *out = Table::Make(::arrow20::schema(std::move(fields)), std::move(columns),
                       this->num_rows());
    return Status::OK();
  }
