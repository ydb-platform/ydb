...
class ARROW_EXPORT ChunkedArrayBuilder {
 public:
  ...
  virtual Status ReplaceTaskGroup(
      const std::shared_ptr<arrow20::internal::TaskGroup>& task_group) = 0;

 protected:
  explicit ChunkedArrayBuilder(
      const std::shared_ptr<arrow20::internal::TaskGroup>& task_group)
      : task_group_(task_group) {}

  std::shared_ptr<arrow20::internal::TaskGroup> task_group_;
};
...
ARROW_EXPORT Status MakeChunkedArrayBuilder(
    const std::shared_ptr<arrow20::internal::TaskGroup>& task_group, MemoryPool* pool,
