...
class ARROW_EXPORT Table {
 public:
  ...
  ///
  /// \param[in] reader the arrow20::RecordBatchReader that produces batches
  ...
  /// zero record batches
  ///
  /// \param[in] schema the arrow20::Schema for each batch
  /// \param[in] batches a std::vector of record batches
  static Result<std::shared_ptr<Table>> FromRecordBatches(
...
/// :func:`arrow20::PromoteTableToSchema` is used to unify schemas.
///
