...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/util/async_generator_fwd.h"
...
#include "contrib/libs/apache/arrow_next/cpp/src/arrow/util/queue.h"
 ...
 protected:
  ...
  struct Cleanup {
    ...
    ~Cleanup() {
      ...
      assert(state->worker_thread_id.load() != ::arrow20::internal::GetThreadId());
      Future<> finish_fut;
      {
        auto lock = state->mutex.Lock();
  ...
  static void WorkerTask(std::shared_ptr<State> state) {
    state->worker_thread_id.store(::arrow20::internal::GetThreadId());
