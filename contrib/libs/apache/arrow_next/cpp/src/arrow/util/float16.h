...
class ARROW_EXPORT Float16 {
 public:
  ...
  static Float16 FromLittleEndian(const uint8_t* src) {
    return FromBits(::arrow20::bit_util::FromLittleEndian(SafeLoadAs<uint16_t>(src)));
  }

  /// \brief Read a `Float16` from memory in big-endian byte order
  static Float16 FromBigEndian(const uint8_t* src) {
    return FromBits(::arrow20::bit_util::FromBigEndian(SafeLoadAs<uint16_t>(src)));
  }
...
// TODO: Not complete
template <>
class std::numeric_limits<arrow20::util::Float16> {
  using T = arrow20::util::Float16;
