...
inline void FormatYYYY_MM_DD(arrow20_vendored::date::year_month_day ymd, char** cursor) {
...
template <typename Duration>
void FormatHH_MM_SS(arrow20_vendored::date::hh_mm_ss<Duration> hms, char** cursor) {
...
// because of silent integer wraparound in the `arrow20_vendored::date` library.
//
...
// respectively (-32768 is an invalid year value in `arrow20_vendored::date`).
//
...
bool IsDateTimeInRange(Unit duration) {
  constexpr Unit kMinIncl =
      std::chrono::duration_cast<Unit>(arrow20_vendored::date::days{-12687428});
  constexpr Unit kMaxExcl =
      std::chrono::duration_cast<Unit>(arrow20_vendored::date::days{11248738});
...
const auto kEpoch = arrow20_vendored::date::sys_days{arrow20_vendored::date::jan / 1 / 1970};
...
class DateToStringFormatterMixin {
 ...
 protected:
  template <typename Appender>
  Return<Appender> FormatDays(arrow20_vendored::date::days since_epoch, Appender&& append) {
    arrow20_vendored::date::sys_days timepoint_days{since_epoch};
    ...
    detail::FormatYYYY_MM_DD(arrow20_vendored::date::year_month_day{timepoint_days},
                             &cursor);
    return append(detail::ViewDigitBuffer(buffer, cursor));
  }
};
...
class StringFormatter<Date32Type> : public DateToStringFormatterMixin {
 public:
  ...
  template <typename Appender>
  Return<Appender> operator()(value_type value, Appender&& append) {
    const auto since_epoch = arrow20_vendored::date::days{value};
...
class StringFormatter<Date64Type> : public DateToStringFormatterMixin {
 public:
  ...
  Return<Appender> operator()(value_type value, Appender&& append) {
    ...
    return FormatDays(std::chrono::duration_cast<arrow20_vendored::date::days>(since_epoch),
...
class StringFormatter<TimestampType> {
 public:
  ...
  Return<Appender> operator()(Duration, value_type value, Appender&& append) {
    ...
    // (the naive approach of using arrow20_vendored::date::floor() would
    ...
    detail::FormatHH_MM_SS(arrow20_vendored::date::make_time(since_midnight), &cursor);
    detail::FormatOneChar(' ', &cursor);
