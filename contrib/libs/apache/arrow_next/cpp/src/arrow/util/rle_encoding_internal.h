...
class RleDecoder {
 ...
 protected:
  ::arrow20::bit_util::BitReader bit_reader_;
...
class RleEncoder {
 public:
  ...
  static int MinBufferSize(int bit_width) {
    ...
    int max_literal_run_size = 1 + static_cast<int>(::arrow20::bit_util::BytesForBits(
    ...
    int max_repeated_run_size =
        ::arrow20::bit_util::BitReader::kMaxVlqByteLength +
        static_cast<int>(::arrow20::bit_util::BytesForBits(bit_width));
  ...
  static int MaxBufferSize(int bit_width, int num_values) {
    ...
    int num_runs = static_cast<int>(::arrow20::bit_util::CeilDiv(num_values, 8));
    ...
    int min_repeated_run_size =
        1 + static_cast<int>(::arrow20::bit_util::BytesForBits(bit_width));
...
inline int RleDecoder::GetSpaced(Converter converter, int batch_size, int null_count,
  ...
  arrow20::internal::BitRunReader bit_reader(valid_bits, valid_bits_offset,
                                           /*length=*/batch_size);
  arrow20::internal::BitRun valid_run = bit_reader.NextRun();
  while (values_read < batch_size) {
...
inline int RleDecoder::GetBatchSpaced(int batch_size, int null_count,
  ...
  arrow20::internal::BitBlockCounter block_counter(valid_bits, valid_bits_offset,
                                                 batch_size);

  int total_processed = 0;
  int processed = 0;
  arrow20::internal::BitBlockCount block;
...
bool RleDecoder::NextCounts() {
  ...
  if (is_literal) {
    ...
    if (!bit_reader_.GetAligned<T>(
            static_cast<int>(::arrow20::bit_util::CeilDiv(bit_width_, 8)), &value)) {
      return false;
    }
...
inline void RleEncoder::FlushRepeatedRun() {
  ...
  result &= bit_writer_.PutAligned(
      current_value_, static_cast<int>(::arrow20::bit_util::CeilDiv(bit_width_, 8)));
  DCHECK(result);
  num_buffered_values_ = 0;
  repeat_count_ = 0;
  CheckBufferFull();
