...
inline int SpacedCompress(const T* src, int num_values, const uint8_t* valid_bits,
  ...
  arrow20::internal::SetBitRunReader reader(valid_bits, valid_bits_offset, num_values);
  while (true) {
...
inline int SpacedExpand(T* buffer, int num_values, int null_count,
  ...
  arrow20::internal::ReverseSetBitRunReader reader(valid_bits, valid_bits_offset,
                                                 num_values);
  while (true) {
    const auto run = reader.NextRun();
    if (run.length == 0) {
