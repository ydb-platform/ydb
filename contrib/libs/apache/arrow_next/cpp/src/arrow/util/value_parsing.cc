...
bool StringToFloat(const char* s, size_t length, char decimal_point, float* out) {
  fast_float::parse_options options{
      fast_float::chars_format::general, decimal_point};
  const auto res =
      fast_float::from_chars_advanced(s, s + length, *out, options);
...
bool StringToFloat(const char* s, size_t length, char decimal_point, uint16_t* out) {
  ...
  float temp_out;
  const auto res =
      fast_float::from_chars_advanced(s, s + length, temp_out, options);
