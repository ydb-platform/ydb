...
static inline bool ParseYYYY_MM_DD(const char* s, Duration* since_epoch) {
  ...
  arrow20_vendored::date::year_month_day ymd{arrow20_vendored::date::year{year},
                                           arrow20_vendored::date::month{month},
                                           arrow20_vendored::date::day{day}};
  if (ARROW_PREDICT_FALSE(!ymd.ok())) return false;

  *since_epoch = std::chrono::duration_cast<Duration>(
      arrow20_vendored::date::sys_days{ymd}.time_since_epoch());
  return true;
}
...
static inline bool ParseTimestampStrptime(const char* buf, size_t length,
                                          ...
                                          int64_t* out) {
  // NOTE: strptime() is more than 10x faster than arrow20_vendored::date::parse().
  // The buffer may not be nul-terminated
  std::string clean_copy(buf, length);
  struct tm result;
  memset(&result, 0, sizeof(struct tm));
#ifdef _WIN32
  char* ret = arrow20_strptime(clean_copy.c_str(), format, &result);
#else
  ...
  arrow20_vendored::date::sys_seconds secs =
      arrow20_vendored::date::sys_days(arrow20_vendored::date::year(result.tm_year + 1900) /
