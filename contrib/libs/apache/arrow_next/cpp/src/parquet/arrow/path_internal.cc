...
using ::arrow20::TypedBufferBuilder;
...
bool LazyNoNulls(const Array& array) {
  ...
  return null_count == 0 ||
         ...
         // for null arrays.
         (null_count == ::arrow20::kUnknownNullCount &&
          array.null_bitmap_data() == nullptr);
}

struct PathWriteContext {
  PathWriteContext(::arrow20::MemoryPool* pool,
                   std::shared_ptr<::arrow20::ResizableBuffer> def_levels_buffer)
...
struct NullableTerminalNode {
  ...
  IterationResult Run(const ElementRange& range, PathWriteContext* context) {
    ...
    if (elements > 16) {  // 16 guarantees at least one unrolled loop.
      ::arrow20::internal::VisitBitsUnrolled(bitmap_, range.start + element_offset_,
                                           elements, bit_visitor);
    } else {
      ::arrow20::internal::VisitBits(bitmap_, range.start + element_offset_, elements,
                                   bit_visitor);
    }
    return kDone;
  }
...
class NullableNode {
 public:
  ...
  ::arrow20::internal::BitRunReader MakeReader(const ElementRange& range) {
    return ::arrow20::internal::BitRunReader(null_bitmap_, entry_offset_ + range.start,
                                           range.Size());
  }
  ...
  IterationResult Run(ElementRange* range, ElementRange* child_range,
    ...
    ::arrow20::internal::BitRun run = valid_bits_reader_.NextRun();
    if (!run.set) {
  ...
  ::arrow20::internal::BitRunReader valid_bits_reader_;
  int16_t def_level_if_null_;
...
class PathBuilder {
 public:
  ...
  template <typename T>
  ::arrow20::enable_if_t<std::is_base_of<::arrow20::FlatArray, T>::value, Status> Visit(
      const T& array) {
  ...
  template <typename T>
  ::arrow20::enable_if_t<std::is_same<::arrow20::ListArray, T>::value ||
                           std::is_same<::arrow20::LargeListArray, T>::value,
                       Status>
  Visit(const T& array) {
  ...
  Status Visit(const ::arrow20::DictionaryArray& array) {
  ...
  Status Visit(const ::arrow20::MapArray& array) {
    return Visit(static_cast<const ::arrow20::ListArray&>(array));
  }

  Status Visit(const ::arrow20::StructArray& array) {
    MaybeAddNullable(array);
  ...
  Status Visit(const ::arrow20::FixedSizeListArray& array) {
    MaybeAddNullable(array);
  ...
  Status Visit(const ::arrow20::ExtensionArray& array) {
...
Status PathBuilder::VisitInline(const Array& array) {
  return ::arrow20::VisitArrayInline(array, this);
}
...
class MultipathLevelBuilderImpl : public MultipathLevelBuilder {
 public:
  MultipathLevelBuilderImpl(std::shared_ptr<::arrow20::ArrayData> data,
  ...
  ::arrow20::Status Write(int leaf_index, ArrowWriteContext* context,
 ...
 private:
  ...
  std::shared_ptr<::arrow20::ArrayData> data_;
  std::unique_ptr<PathBuilder> path_builder_;
};

// static
::arrow20::Result<std::unique_ptr<MultipathLevelBuilder>> MultipathLevelBuilder::Make(
