...
using arrow20::Field;
using arrow20::Future;
using arrow20::Int32Array;
using arrow20::ListArray;
using arrow20::MemoryPool;
using arrow20::RecordBatchReader;
using arrow20::ResizableBuffer;
using arrow20::Result;
using arrow20::Status;
using arrow20::StructArray;
using arrow20::Table;
using arrow20::TimestampArray;

using arrow20::internal::checked_cast;
using arrow20::internal::Iota;

// Help reduce verbosity
using ParquetReader = parquet20::ParquetFileReader;

using parquet20::internal::RecordReader;
...
::arrow20::Result<std::shared_ptr<ArrayData>> ChunksToSingle(const ChunkedArray& chunked) {
  switch (chunked.num_chunks()) {
    case 0: {
      ARROW_ASSIGN_OR_RAISE(std::shared_ptr<Array> array,
                            ::arrow20::MakeArrayOfNull(chunked.type(), 0));
      return array->data();
    }
    case 1:
...
class ColumnReaderImpl : public ColumnReader {
 public:
  ...
  ::arrow20::Status NextBatch(int64_t batch_size,
                            std::shared_ptr<::arrow20::ChunkedArray>* out) final {
  ...
  virtual ::arrow20::Status LoadBatch(int64_t num_records) = 0;

  virtual ::arrow20::Status BuildArray(int64_t length_upper_bound,
                                     std::shared_ptr<::arrow20::ChunkedArray>* out) = 0;
...
class FileReaderImpl : public FileReader {
 public:
  ...
  Status GetFieldReaders(const std::vector<int>& column_indices,
                         ...
                         std::shared_ptr<::arrow20::Schema>* out_schema) {
    ...
    ::arrow20::FieldVector out_fields(field_indices.size());
    ...
    *out_schema = ::arrow20::schema(std::move(out_fields), manifest_.schema_metadata);
    return Status::OK();
  }
  ...
  Status GetSchema(std::shared_ptr<::arrow20::Schema>* out) override {
  ...
  Status ReadColumn(int i, const std::vector<int>& row_groups, ColumnReader* reader,
    ...
    ::arrow20::util::tracing::Span span;
    START_SPAN(span, "parquet20::arrow20::read_column",
  ...
  Future<std::shared_ptr<Table>> DecodeRowGroups(
      ...
      const std::vector<int>& column_indices, ::arrow20::internal::Executor* cpu_executor);
  ...
  ::arrow20::Result<::arrow20::AsyncGenerator<std::shared_ptr<::arrow20::RecordBatch>>>
...
class RowGroupRecordBatchReader : public ::arrow20::RecordBatchReader {
 public:
  RowGroupRecordBatchReader(::arrow20::RecordBatchIterator batches,
                            std::shared_ptr<::arrow20::Schema> schema)
      : batches_(std::move(batches)), schema_(std::move(schema)) {}

  ~RowGroupRecordBatchReader() override {}

  Status ReadNext(std::shared_ptr<::arrow20::RecordBatch>* out) override {
 ...
 private:
  ::arrow20::Iterator<std::shared_ptr<::arrow20::RecordBatch>> batches_;
...
class ColumnChunkReaderImpl : public ColumnChunkReader {
 public:
  ...
  Status Read(std::shared_ptr<::arrow20::ChunkedArray>* out) override {
...
class RowGroupReaderImpl : public RowGroupReader {
 public:
  ...
  Status ReadTable(const std::vector<int>& column_indices,
                   std::shared_ptr<::arrow20::Table>* out) override {
    return impl_->ReadRowGroup(row_group_index_, column_indices, out);
  }

  Status ReadTable(std::shared_ptr<::arrow20::Table>* out) override {
...
class LeafReader : public ColumnReaderImpl {
 public:
  LeafReader(std::shared_ptr<ReaderContext> ctx, std::shared_ptr<Field> field,
             ...
             ::parquet20::internal::LevelInfo leaf_info)
      : ctx_(std::move(ctx)),
        field_(std::move(field)),
        input_(std::move(input)),
        descr_(input_->descr()) {
    record_reader_ = RecordReader::Make(
        descr_, leaf_info, ctx_->pool, field_->type()->id() == ::arrow20::Type::DICTIONARY);
    NextRowGroup();
  }
  ...
  ::arrow20::Status BuildArray(int64_t length_upper_bound,
                             std::shared_ptr<::arrow20::ChunkedArray>* out) final {
    *out = out_;
    return Status::OK();
  }
...
class ListReader : public ColumnReaderImpl {
 public:
  ListReader(std::shared_ptr<ReaderContext> ctx, std::shared_ptr<Field> field,
             ::parquet20::internal::LevelInfo level_info,
  ...
  virtual ::arrow20::Result<std::shared_ptr<ChunkedArray>> AssembleArray(
      std::shared_ptr<ArrayData> data) {
    if (field_->type()->id() == ::arrow20::Type::MAP) {
      // Error out if data is not map-compliant instead of aborting in MakeArray below
      RETURN_NOT_OK(::arrow20::MapArray::ValidateChildData(data->child_data));
    }
    std::shared_ptr<Array> result = ::arrow20::MakeArray(data);
  ...
  Status BuildArray(int64_t length_upper_bound,
    ...
    ::parquet20::internal::ValidityBitmapInputOutput validity_io;
    ...
    offset_data[0] = 0;
    BEGIN_PARQUET_CATCH_EXCEPTIONS
    ::parquet20::internal::DefRepLevelsToList(def_levels, rep_levels, num_levels,
 ...
 private:
  std::shared_ptr<ReaderContext> ctx_;
  std::shared_ptr<Field> field_;
  ::parquet20::internal::LevelInfo level_info_;
...
class PARQUET_NO_EXPORT FixedSizeListReader : public ListReader<int32_t> {
 public:
  ...
  ::arrow20::Result<std::shared_ptr<ChunkedArray>> AssembleArray(
      std::shared_ptr<ArrayData> data) final {
    DCHECK_EQ(data->buffers.size(), 2);
    DCHECK_EQ(field()->type()->id(), ::arrow20::Type::FIXED_SIZE_LIST);
    const auto& type = checked_cast<::arrow20::FixedSizeListType&>(*field()->type());
...
class PARQUET_NO_EXPORT StructReader : public ColumnReaderImpl {
 ...
 private:
  ...
  const ::parquet20::internal::LevelInfo level_info_;
...
Status GetReader(const SchemaField& field, const std::shared_ptr<Field>& arrow_field,
  ...
  if (type_id == ::arrow20::Type::EXTENSION) {
  ...
  if (field.children.size() == 0) {
    ...
    *out = std::make_unique<LeafReader>(ctx, arrow_field, std::move(input),
                                        field.level_info);
  } else if (type_id == ::arrow20::Type::LIST || type_id == ::arrow20::Type::MAP ||
             type_id == ::arrow20::Type::FIXED_SIZE_LIST ||
             type_id == ::arrow20::Type::LARGE_LIST) {
    ...
    if (type_id == ::arrow20::Type::MAP) {
      if (reader_child_type->num_fields() != 2 ||
        ...
        list_field = list_field->WithType(::arrow20::list(child_reader->field()));
      } else if (!reader_child_type->field(1)->type()->Equals(
                     *schema_child_type.field(1)->type())) {
        list_field = list_field->WithType(std::make_shared<::arrow20::MapType>(
      ...
      *out = std::make_unique<ListReader<int32_t>>(ctx, list_field, field.level_info,
                                                   std::move(child_reader));
    } else if (type_id == ::arrow20::Type::LIST) {
      if (!reader_child_type->Equals(schema_child_type)) {
        list_field = list_field->WithType(::arrow20::list(reader_child_type));
      }

      *out = std::make_unique<ListReader<int32_t>>(ctx, list_field, field.level_info,
                                                   std::move(child_reader));
    } else if (type_id == ::arrow20::Type::LARGE_LIST) {
      if (!reader_child_type->Equals(schema_child_type)) {
        list_field = list_field->WithType(::arrow20::large_list(reader_child_type));
      }

      *out = std::make_unique<ListReader<int64_t>>(ctx, list_field, field.level_info,
                                                   std::move(child_reader));
    } else if (type_id == ::arrow20::Type::FIXED_SIZE_LIST) {
      if (!reader_child_type->Equals(schema_child_type)) {
        auto& fixed_list_type =
            checked_cast<const ::arrow20::FixedSizeListType&>(*list_field->type());
        int32_t list_size = fixed_list_type.list_size();
        list_field =
            list_field->WithType(::arrow20::fixed_size_list(reader_child_type, list_size));
      }
  ...
  } else if (type_id == ::arrow20::Type::STRUCT) {
    ...
    for (const auto& child : field.children) {
      ...
      std::shared_ptr<::arrow20::Field> child_field = child.field;
    ...
    auto filtered_field =
        ::arrow20::field(arrow_field->name(), ::arrow20::struct_(child_fields),
...
Result<std::unique_ptr<RecordBatchReader>> FileReaderImpl::GetRecordBatchReader(
  ...
  std::shared_ptr<::arrow20::Schema> batch_schema;
  ...
  if (readers.empty()) {
    ...
    auto max_sized_batch =
        ::arrow20::RecordBatch::Make(batch_schema, batch_size, ::arrow20::ArrayVector{});

    ::arrow20::RecordBatchVector batches;
    ...
    return std::make_unique<RowGroupRecordBatchReader>(
        ::arrow20::MakeVectorIterator(std::move(batches)), std::move(batch_schema));
  }
  ...
  using ::arrow20::RecordBatchIterator;
  ...
  ::arrow20::Iterator<RecordBatchIterator> batches = ::arrow20::MakeFunctionIterator(
      [readers, batch_schema, num_rows,
       this]() mutable -> ::arrow20::Result<RecordBatchIterator> {
        ::arrow20::ChunkedArrayVector columns(readers.size());
        ...
        for (const auto& column : columns) {
          if (column == nullptr || column->length() == 0) {
            return ::arrow20::IterationTraits<RecordBatchIterator>::End();
          }
        }
        ...
        if (!columns.empty()) {
          ...
          for (size_t i = 1; i < columns.size(); ++i) {
            if (columns[i]->length() != row_size) {
              return ::arrow20::Status::Invalid("columns do not have the same size");
            }
          }
        }

        auto table = ::arrow20::Table::Make(batch_schema, std::move(columns));
        auto table_reader = std::make_shared<::arrow20::TableBatchReader>(*table);

        // NB: explicitly preserve table so that table_reader doesn't outlive it
        return ::arrow20::MakeFunctionIterator(
            [table, table_reader] { return table_reader->Next(); });
      });

  return std::make_unique<RowGroupRecordBatchReader>(
      ::arrow20::MakeFlattenIterator(std::move(batches)), std::move(batch_schema));
}
...
class RowGroupGenerator {
 public:
  using RecordBatchGenerator =
      ::arrow20::AsyncGenerator<std::shared_ptr<::arrow20::RecordBatch>>;

  struct ReadRequest {
    ::arrow20::Future<RecordBatchGenerator> read;
    int64_t num_rows;
  };
  ...
  ::arrow20::Future<RecordBatchGenerator> operator()() {
    if (index_ >= row_groups_.size()) {
      return ::arrow20::AsyncGeneratorEnd<RecordBatchGenerator>();
    }
    index_++;
    FillReadahead();
 ...
 private:
  ...
  void FetchNext() {
    ...
    ::arrow20::Future<RecordBatchGenerator> row_group_read;
      ...
      row_group_read =
          ready.Then([cpu_executor = cpu_executor_, reader, row_group,
                      column_indices = std::move(
                          column_indices)]() -> ::arrow20::Future<RecordBatchGenerator> {
  ...
  static ::arrow20::Future<RecordBatchGenerator> SubmitRead(
      ::arrow20::internal::Executor* cpu_executor, std::shared_ptr<FileReaderImpl> self,
    ...
    return ::arrow20::DeferNotOk(cpu_executor->Submit(ReadOneRowGroup, cpu_executor, self,
                                                    row_group, column_indices));
  }

  static ::arrow20::Future<RecordBatchGenerator> ReadOneRowGroup(
      ::arrow20::internal::Executor* cpu_executor, std::shared_ptr<FileReaderImpl> self,
      const int row_group, const std::vector<int>& column_indices) {
    ...
    return self->DecodeRowGroups(self, {row_group}, column_indices, cpu_executor)
        .Then([batch_size](const std::shared_ptr<Table>& table)
                  -> ::arrow20::Result<RecordBatchGenerator> {
          ::arrow20::TableBatchReader table_reader(*table);
          table_reader.set_chunksize(batch_size);
          ARROW_ASSIGN_OR_RAISE(auto batches, table_reader.ToRecordBatches());
          return ::arrow20::MakeVectorGenerator(std::move(batches));
        });
  }

  std::shared_ptr<FileReaderImpl> arrow_reader_;
  ::arrow20::internal::Executor* cpu_executor_;
...
FileReaderImpl::GetRecordBatchGenerator(std::shared_ptr<FileReader> reader,
  ...
  ::arrow20::AsyncGenerator<RowGroupGenerator::RecordBatchGenerator> row_group_generator =
      RowGroupGenerator(::arrow20::internal::checked_pointer_cast<FileReaderImpl>(reader),
                        cpu_executor, row_group_indices, column_indices,
                        rows_to_readahead);
  ::arrow20::AsyncGenerator<std::shared_ptr<::arrow20::RecordBatch>> concatenated =
      ::arrow20::MakeConcatenatedGenerator(std::move(row_group_generator));
...
Future<std::shared_ptr<Table>> FileReaderImpl::DecodeRowGroups(
  ...
  std::vector<std::shared_ptr<ColumnReaderImpl>> readers;
  std::shared_ptr<::arrow20::Schema> result_schema;
  ...
  if (!cpu_executor) cpu_executor = ::arrow20::internal::GetCpuThreadPool();

  auto read_column = [row_groups, self, this](size_t i,
                                              std::shared_ptr<ColumnReaderImpl> reader)
      -> ::arrow20::Result<std::shared_ptr<::arrow20::ChunkedArray>> {
    std::shared_ptr<::arrow20::ChunkedArray> column;
    RETURN_NOT_OK(ReadColumn(static_cast<int>(i), row_groups, reader.get(), &column));
    return column;
  };
  auto make_table = [result_schema, row_groups, self,
                     this](const ::arrow20::ChunkedArrayVector& columns)
      -> ::arrow20::Result<std::shared_ptr<Table>> {
    int64_t num_rows = 0;
    if (!columns.empty()) {
  ...
  return ::arrow20::internal::OptionalParallelForAsync(reader_properties_.use_threads(),
...
Status FileReader::Make(::arrow20::MemoryPool* pool,
...
FileReaderBuilder::FileReaderBuilder()
    : pool_(::arrow20::default_memory_pool()),
      properties_(default_arrow_reader_properties()) {}

Status FileReaderBuilder::Open(std::shared_ptr<::arrow20::io::RandomAccessFile> file,
...
FileReaderBuilder* FileReaderBuilder::memory_pool(::arrow20::MemoryPool* pool) {
  pool_ = pool;
  return this;
}
...
Status OpenFile(std::shared_ptr<::arrow20::io::RandomAccessFile> file, MemoryPool* pool,
                std::unique_ptr<FileReader>* reader) {
  return OpenFile(std::move(file), pool).Value(reader);
}

Result<std::unique_ptr<FileReader>> OpenFile(
    std::shared_ptr<::arrow20::io::RandomAccessFile> file, MemoryPool* pool) {
...
Status FuzzReader(const uint8_t* data, int64_t size) {
  auto buffer = std::make_shared<::arrow20::Buffer>(data, size);
  Status st;
  for (auto batch_size : std::vector<std::optional<int>>{std::nullopt, 1, 13, 300}) {
    auto file = std::make_shared<::arrow20::io::BufferReader>(buffer);
    FileReaderBuilder builder;
    ArrowReaderProperties properties;
    if (batch_size) {
