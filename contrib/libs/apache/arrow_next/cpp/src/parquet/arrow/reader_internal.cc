...
using arrow20::Datum;
using arrow20::Decimal128;
using arrow20::Decimal128Array;
using arrow20::Decimal128Type;
using arrow20::Decimal256;
using arrow20::Decimal256Array;
using arrow20::Decimal256Type;
using arrow20::Field;
...
using ::arrow20::bit_util::FromBigEndian;
using ::arrow20::internal::checked_cast;
using ::arrow20::internal::checked_pointer_cast;
using ::arrow20::internal::SafeLeftShift;
using ::arrow20::util::Float16;
using ::arrow20::util::SafeLoadAs;

using parquet20::internal::BinaryRecordReader;
using parquet20::internal::DictionaryRecordReader;
...
Status MakeMinMaxScalar(const StatisticsType& statistics,
  ...
  *min = ::arrow20::MakeScalar(static_cast<CType>(statistics.min()));
  *max = ::arrow20::MakeScalar(static_cast<CType>(statistics.max()));
  return Status::OK();
}
...
Status MakeMinMaxTypedScalar(const StatisticsType& statistics,
                             ...
                             std::shared_ptr<::arrow20::Scalar>* min,
                             std::shared_ptr<::arrow20::Scalar>* max) {
  ARROW_ASSIGN_OR_RAISE(*min, ::arrow20::MakeScalar(type, statistics.min()));
  ARROW_ASSIGN_OR_RAISE(*max, ::arrow20::MakeScalar(type, statistics.max()));
  return Status::OK();
}

template <typename StatisticsType>
Status MakeMinMaxIntegralScalar(const StatisticsType& statistics,
                                const ::arrow20::DataType& arrow_type,
                                std::shared_ptr<::arrow20::Scalar>* min,
                                std::shared_ptr<::arrow20::Scalar>* max) {
...
Result<std::shared_ptr<::arrow20::Scalar>> FromBigEndianString(
  ...
  return ::arrow20::MakeScalar(std::move(arrow_type), decimal);
}
...
Status ByteArrayStatisticsAsScalars(const Statistics& statistics,
  ...
  if (statistics.descr()->physical_type() == Type::FIXED_LEN_BYTE_ARRAY) {
    type = ::arrow20::fixed_size_binary(statistics.descr()->type_length());
  } else {
    type = logical_type->type() == LogicalType::Type::STRING ? ::arrow20::utf8()
                                                             : ::arrow20::binary();
  }
  ARROW_ASSIGN_OR_RAISE(
      *min, ::arrow20::MakeScalar(type, Buffer::FromString(statistics.EncodeMin())));
  ARROW_ASSIGN_OR_RAISE(
      *max, ::arrow20::MakeScalar(type, Buffer::FromString(statistics.EncodeMax())));
...
Result<std::shared_ptr<ChunkedArray>> ViewOrCastChunkedArray(
  ...
  if (view_result.ok()) {
    ...
    ::arrow20::compute::ExecContext exec_context(pool);
    ARROW_ASSIGN_OR_RAISE(
        auto casted_datum,
        ::arrow20::compute::Cast(Datum(array), logical_value_type,
                               ::arrow20::compute::CastOptions(), &exec_context));
...
void ReconstructChunksWithoutNulls(::arrow20::ArrayVector* chunks) {
  for (size_t i = 0; i < chunks->size(); i++) {
    if ((*chunks)[i]->data()->buffers[0]) {
      std::shared_ptr<::arrow20::ArrayData> data = (*chunks)[i]->data();
      data->null_count = 0;
...
void AttachStatistics(::arrow20::ArrayData* data,
  ...
  if (data->null_count == ::arrow20::kUnknownNullCount && !statistics) {
    return;
  }

  auto array_statistics = std::make_shared<::arrow20::ArrayStatistics>();
  if (data->null_count != ::arrow20::kUnknownNullCount) {
  ...
  if (statistics) {
    ...
    if (statistics->HasMinMax()) {
      auto typed_statistics =
          static_cast<::parquet20::TypedStatistics<ParquetType>*>(statistics);
...
Status TransferInt(RecordReader* reader,
  ...
  ARROW_ASSIGN_OR_RAISE(auto data,
                        ::arrow20::AllocateBuffer(length * sizeof(ArrowCType), ctx->pool));
  ...
  auto array_data =
      ::arrow20::ArrayData::Make(field->type(), length, std::move(buffers), null_count);
...
std::shared_ptr<Array> TransferZeroCopy(
    RecordReader* reader, std::unique_ptr<::parquet20::ColumnChunkMetaData> metadata,
    const ReaderContext* ctx, const std::shared_ptr<Field>& field) {
  std::shared_ptr<::arrow20::ArrayData> data;
  if (field->nullable()) {
    std::vector<std::shared_ptr<Buffer>> buffers = {reader->ReleaseIsValid(),
                                                    reader->ReleaseValues()};
    data = std::make_shared<::arrow20::ArrayData>(field->type(), reader->values_written(),
  ...
  return ::arrow20::MakeArray(std::move(data));
}
...
Status TransferBool(RecordReader* reader,
  ...
  ARROW_ASSIGN_OR_RAISE(auto data, ::arrow20::AllocateBuffer(buffer_size, ctx->pool));
  ...
  for (int64_t i = 0; i < length; i++) {
    if (values[i]) {
      ::arrow20::bit_util::SetBit(data_ptr, i);
    }
  }

  std::shared_ptr<::arrow20::ArrayData> array_data;
  if (nullable) {
    array_data = ::arrow20::ArrayData::Make(::arrow20::boolean(), length,
  ...
  AttachStatistics<::arrow20::BooleanType, BooleanType>(array_data.get(),
...
Status TransferInt96(RecordReader* reader, MemoryPool* pool,
                     ...
                     const ::arrow20::TimeUnit::type int96_arrow_time_unit) {
  int64_t length = reader->values_written();
  auto values = reinterpret_cast<const Int96*>(reader->values());
  ARROW_ASSIGN_OR_RAISE(auto data,
                        ::arrow20::AllocateBuffer(length * sizeof(int64_t), pool));
...
Status TransferDate64(RecordReader* reader, MemoryPool* pool,
  ...
  if (field->nullable()) {
    *out = std::make_shared<::arrow20::Date64Array>(field->type(), length, std::move(data),
                                                  reader->ReleaseIsValid(),
                                                  reader->null_count());
  } else {
    *out =
        std::make_shared<::arrow20::Date64Array>(field->type(), length, std::move(data),
                                               /*null_bitmap=*/nullptr, /*null_count=*/0);
  }
  return Status::OK();
}
...
Status TransferDictionary(RecordReader* reader, MemoryPool* pool,
  ...
  *out = dict_reader->GetResult();
  ...
  if (!nullable) {
    ::arrow20::ArrayVector chunks = (*out)->chunks();
...
Status TransferBinary(RecordReader* reader, MemoryPool* pool,
  ...
  if (reader->read_dictionary()) {
    return TransferDictionary(
        reader, pool, ::arrow20::dictionary(::arrow20::int32(), logical_type_field->type()),
        logical_type_field->nullable(), out);
  }
  ::arrow20::compute::ExecContext ctx(pool);
  ::arrow20::compute::CastOptions cast_options;
  ...
  for (auto& chunk : chunks) {
    if (!chunk->type()->Equals(*logical_type_field->type())) {
      ...
      ARROW_ASSIGN_OR_RAISE(
          chunk,
          ::arrow20::compute::Cast(*chunk, logical_type_field->type(), cast_options, &ctx));
    }
  }
...
template <>
struct DecimalTypeTrait<::arrow20::Decimal128Array> {
  using value = ::arrow20::Decimal128;
};

template <>
struct DecimalTypeTrait<::arrow20::Decimal256Array> {
  using value = ::arrow20::Decimal256;
};
...
struct DecimalConverter<DecimalArrayType, FLBAType> {
  static inline Status ConvertToDecimal(const Array& array,
    ...
    const auto& fixed_size_binary_array =
        checked_cast<const ::arrow20::FixedSizeBinaryArray&>(array);

    // The byte width of each decimal value
    const int32_t type_length =
        checked_cast<const ::arrow20::DecimalType&>(*type).byte_width();
    ...
    const int32_t byte_width =
        checked_cast<const ::arrow20::FixedSizeBinaryType&>(*fixed_size_binary_array.type())
            .byte_width();
    // allocate memory for the decimal array
    ARROW_ASSIGN_OR_RAISE(auto data, ::arrow20::AllocateBuffer(length * type_length, pool));
...
struct DecimalConverter<DecimalArrayType, ByteArrayType> {
  static inline Status ConvertToDecimal(const Array& array,
                                        const std::shared_ptr<DataType>& type,
                                        MemoryPool* pool, std::shared_ptr<Array>* out) {
    const auto& binary_array = checked_cast<const ::arrow20::BinaryArray&>(array);
    const int64_t length = binary_array.length();

    const auto& decimal_type = checked_cast<const ::arrow20::DecimalType&>(*type);
...
template <
    ...
    typename = ::arrow20::enable_if_t<std::is_same<ParquetIntegerType, Int32Type>::value ||
...
static Status DecimalIntegerTransfer(RecordReader* reader, MemoryPool* pool,
  ...
  DCHECK(field->type()->id() == ::arrow20::Type::DECIMAL128 ||
         field->type()->id() == ::arrow20::Type::DECIMAL256);
  ...
  const auto& decimal_type = checked_cast<const ::arrow20::DecimalType&>(*field->type());
  const int64_t type_length = decimal_type.byte_width();

  ARROW_ASSIGN_OR_RAISE(auto data, ::arrow20::AllocateBuffer(length * type_length, pool));
  uint8_t* out_ptr = data->mutable_data();

  using ::arrow20::bit_util::FromLittleEndian;
  ...
  for (int64_t i = 0; i < length; ++i, out_ptr += type_length) {
    ...
    if constexpr (std::is_same_v<DecimalArrayType, Decimal128Array>) {
      ::arrow20::Decimal128 decimal(value);
      decimal.ToBytes(out_ptr);
    } else {
      ::arrow20::Decimal256 decimal(value);
      decimal.ToBytes(out_ptr);
    }
  }
...
/// \brief Convert an arrow20::BinaryArray to an arrow20::Decimal{128,256}Array
/// We do this by:
/// 1. Creating an arrow20::BinaryArray from the RecordReader's builder
/// 2. Allocating a buffer for the arrow20::Decimal{128,256}Array
...
Status TransferDecimal(RecordReader* reader, MemoryPool* pool,
  ...
  auto binary_reader = dynamic_cast<BinaryRecordReader*>(reader);
  DCHECK(binary_reader);
  ::arrow20::ArrayVector chunks = binary_reader->GetBuilderChunks();
  for (size_t i = 0; i < chunks.size(); ++i) {
...
Status TransferHalfFloat(RecordReader* reader, MemoryPool* pool,
                         const std::shared_ptr<Field>& field, Datum* out) {
  static const auto binary_type = ::arrow20::fixed_size_binary(2);
...
Status TransferColumnData(RecordReader* reader,
  ...
  switch (value_field->type()->id()) {
    ...
    case ::arrow20::Type::NA: {
      result = std::make_shared<::arrow20::NullArray>(reader->values_written());
      break;
    }
    case ::arrow20::Type::INT32:
      result = TransferZeroCopy<::arrow20::Int32Type, Int32Type>(
          reader, std::move(metadata), ctx, value_field);
      break;
    case ::arrow20::Type::INT64:
      result = TransferZeroCopy<::arrow20::Int64Type, Int64Type>(
          reader, std::move(metadata), ctx, value_field);
      break;
    case ::arrow20::Type::FLOAT:
      result = TransferZeroCopy<::arrow20::FloatType, FloatType>(
          reader, std::move(metadata), ctx, value_field);
      break;
    case ::arrow20::Type::DOUBLE:
      result = TransferZeroCopy<::arrow20::DoubleType, DoubleType>(
          reader, std::move(metadata), ctx, value_field);
      break;
    case ::arrow20::Type::BOOL:
      ...
      break;
      TRANSFER_INT32(UINT8, ::arrow20::UInt8Type);
      TRANSFER_INT32(INT8, ::arrow20::Int8Type);
      TRANSFER_INT32(UINT16, ::arrow20::UInt16Type);
      TRANSFER_INT32(INT16, ::arrow20::Int16Type);
      TRANSFER_INT32(UINT32, ::arrow20::UInt32Type);
      TRANSFER_INT64(UINT64, ::arrow20::UInt64Type);
      TRANSFER_INT32(DATE32, ::arrow20::Date32Type);
      TRANSFER_INT32(TIME32, ::arrow20::Time32Type);
      TRANSFER_INT64(TIME64, ::arrow20::Time64Type);
      TRANSFER_INT64(DURATION, ::arrow20::DurationType);
    ...
    case ::arrow20::Type::HALF_FLOAT: {
      ...
      if (descr->physical_type() != ::parquet20::Type::FIXED_LEN_BYTE_ARRAY) {
    ...
    case ::arrow20::Type::TIMESTAMP: {
      const ::arrow20::TimestampType& timestamp_type =
          checked_cast<::arrow20::TimestampType&>(*value_field->type());
      if (descr->physical_type() == ::parquet20::Type::INT96) {
        RETURN_NOT_OK(
