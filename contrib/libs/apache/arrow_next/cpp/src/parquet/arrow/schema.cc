...
using arrow20::Field;
using arrow20::FieldVector;
using arrow20::KeyValueMetadata;
using arrow20::Status;
using arrow20::internal::checked_cast;
using arrow20::internal::EndsWith;
using arrow20::internal::ToChars;

using ArrowType = arrow20::DataType;
using ArrowTypeId = arrow20::Type;

using parquet20::Repetition;
using parquet20::schema::GroupNode;
using parquet20::schema::Node;
using parquet20::schema::NodePtr;
using parquet20::schema::PrimitiveNode;

using ParquetType = parquet20::Type;
using parquet20::ConvertedType;
using parquet20::LogicalType;

using parquet20::internal::LevelInfo;
...
Status ListToNode(const std::shared_ptr<::arrow20::BaseListType>& type,
...
Status MapToNode(const std::shared_ptr<::arrow20::MapType>& type, const std::string& name,
...
Status StructToNode(const std::shared_ptr<::arrow20::StructType>& type,
                    const std::string& name, bool nullable, int field_id,
                    const WriterProperties& properties,
                    const ArrowWriterProperties& arrow_properties, NodePtr* out) {
...
static std::shared_ptr<const LogicalType> TimestampLogicalTypeFromArrowTimestamp(
    const ::arrow20::TimestampType& timestamp_type, ::arrow20::TimeUnit::type time_unit) {
...
static Status GetTimestampMetadata(const ::arrow20::TimestampType& type,
                                   const WriterProperties& properties,
  ...
  *logical_type = TimestampLogicalTypeFromArrowTimestamp(type, target_unit);
  ...
  if (coerce) {
    if (version == ::parquet20::ParquetVersion::PARQUET_1_0 ||
        version == ::parquet20::ParquetVersion::PARQUET_2_4) {
      switch (target_unit) {
        case ::arrow20::TimeUnit::MILLI:
        case ::arrow20::TimeUnit::MICRO:
          break;
        ...
        case ::arrow20::TimeUnit::SECOND:
          return Status::NotImplemented("For Parquet version ",
                                        ::parquet20::ParquetVersionToString(version),
  ...
  if ((version == ::parquet20::ParquetVersion::PARQUET_1_0 ||
       version == ::parquet20::ParquetVersion::PARQUET_2_4) &&
      type.unit() == ::arrow20::TimeUnit::NANO) {
    *logical_type =
        TimestampLogicalTypeFromArrowTimestamp(type, ::arrow20::TimeUnit::MICRO);
    return Status::OK();
  }
  ...
  if (type.unit() == ::arrow20::TimeUnit::SECOND) {
    *logical_type =
        TimestampLogicalTypeFromArrowTimestamp(type, ::arrow20::TimeUnit::MILLI);
    return Status::OK();
  }
...
std::shared_ptr<::arrow20::KeyValueMetadata> FieldIdMetadata(int field_id) {
  if (field_id >= 0) {
    return ::arrow20::key_value_metadata({FIELD_ID_KEY}, {ToChars(field_id)});
  } else {
    return nullptr;
  }
}
...
int FieldIdFromMetadata(
    const std::shared_ptr<const ::arrow20::KeyValueMetadata>& metadata) {
  if (!metadata) {
    return -1;
  }
  ...
  int field_id;
  if (::arrow20::internal::ParseValue<::arrow20::Int32Type>(
...
Status FieldToNode(const std::string& name, const std::shared_ptr<Field>& field,
  ...
  switch (field->type()->id()) {
    ...
    case ArrowTypeId::UINT32:
      if (properties.version() == ::parquet20::ParquetVersion::PARQUET_1_0) {
    ...
    case ArrowTypeId::FIXED_SIZE_BINARY: {
      ...
      const auto& fixed_size_binary_type =
          static_cast<const ::arrow20::FixedSizeBinaryType&>(*field->type());
      length = fixed_size_binary_type.byte_width();
    } break;
    case ArrowTypeId::DECIMAL128:
    case ArrowTypeId::DECIMAL256: {
      const auto& decimal_type = static_cast<const ::arrow20::DecimalType&>(*field->type());
    ...
    case ArrowTypeId::TIMESTAMP:
      RETURN_NOT_OK(
          GetTimestampMetadata(static_cast<::arrow20::TimestampType&>(*field->type()),
    ...
    case ArrowTypeId::TIME64: {
      type = ParquetType::INT64;
      auto time_type = static_cast<::arrow20::Time64Type*>(field->type().get());
      if (time_type->unit() == ::arrow20::TimeUnit::NANO) {
        logical_type =
    ...
    case ArrowTypeId::STRUCT: {
      auto struct_type = std::static_pointer_cast<::arrow20::StructType>(field->type());
    ...
    case ArrowTypeId::LIST: {
      auto list_type = std::static_pointer_cast<::arrow20::BaseListType>(field->type());
    ...
    case ArrowTypeId::DICTIONARY: {
      ...
      const ::arrow20::DictionaryType& dict_type =
          static_cast<const ::arrow20::DictionaryType&>(*field->type());
      std::shared_ptr<::arrow20::Field> unpacked_field = ::arrow20::field(
    ...
    case ArrowTypeId::EXTENSION: {
      auto ext_type = std::static_pointer_cast<::arrow20::ExtensionType>(field->type());
      ...
      std::shared_ptr<::arrow20::Field> storage_field = ::arrow20::field(
    ...
    case ArrowTypeId::MAP: {
      auto map_type = std::static_pointer_cast<::arrow20::MapType>(field->type());
...
bool IsDictionaryReadSupported(const ArrowType& type) {
  ...
  return type.id() == ::arrow20::Type::BINARY || type.id() == ::arrow20::Type::STRING;
}

// ----------------------------------------------------------------------
// Schema logic

::arrow20::Result<std::shared_ptr<ArrowType>> GetTypeForNode(
  ...
  if (ctx->properties.read_dictionary(column_index) &&
    ...
    return ::arrow20::dictionary(::arrow20::int32(), storage_type);
  }
  return storage_type;
}
...
Status GroupToStruct(const GroupNode& node, LevelInfo current_levels,
  ...
  auto struct_type = ::arrow20::struct_(arrow_fields);
  out->field = ::arrow20::field(node.name(), struct_type, node.is_optional(),
...
Status MapToSchemaField(const GroupNode& group, LevelInfo current_levels,
  ...
  key_value_field->field = ::arrow20::field(
      group.name(), ::arrow20::struct_({key_field->field, value_field->field}),
      /*nullable=*/false, FieldIdMetadata(key_value.field_id()));
  key_value_field->level_info = current_levels;

  out->field = ::arrow20::field(group.name(),
                              std::make_shared<::arrow20::MapType>(key_value_field->field),
...
Status ListToSchemaField(const GroupNode& group, LevelInfo current_levels,
  ...
  if (list_node.is_group()) {
    ...
    auto item_field = ::arrow20::field(list_node.name(), type, /*nullable=*/false,
                                     FieldIdMetadata(list_node.field_id()));
    RETURN_NOT_OK(
        PopulateLeaf(column_index, item_field, current_levels, ctx, out, child_field));
  }
  out->field = ::arrow20::field(group.name(), ::arrow20::list(child_field->field),
                              group.is_optional(), FieldIdMetadata(group.field_id()));
  out->level_info = current_levels;
  // At this point current levels contains the def level for this list,
  // we need to reset to the prior parent.
...
Status GroupToSchemaField(const GroupNode& node, LevelInfo current_levels,
  ...
  if (node.is_repeated()) {
    ...
    out->field = ::arrow20::field(node.name(), ::arrow20::list(out->children[0].field),
...
Status NodeToSchemaField(const Node& node, LevelInfo current_levels,
  ...
  if (node.is_group()) {
    ...
    if (node.is_repeated()) {
      ...
      int16_t repeated_ancestor_def_level = current_levels.IncrementRepeated();
      out->children.resize(1);
      auto child_field = ::arrow20::field(node.name(), type, /*nullable=*/false);
      RETURN_NOT_OK(PopulateLeaf(column_index, child_field, current_levels, ctx, out,
                                 &out->children[0]));

      out->field = ::arrow20::field(node.name(), ::arrow20::list(child_field),
                                  /*nullable=*/false, FieldIdMetadata(node.field_id()));
      out->level_info = current_levels;
      ...
      return PopulateLeaf(column_index,
                          ::arrow20::field(node.name(), type, node.is_optional(),
                                         FieldIdMetadata(node.field_id())),
...
Status GetOriginSchema(const std::shared_ptr<const KeyValueMetadata>& metadata,
                       ...
                       std::shared_ptr<::arrow20::Schema>* out) {
  if (metadata == nullptr) {
    *out = nullptr;
  ...
  auto decoded = ::arrow20::util::base64_decode(metadata->value(schema_index));
  auto schema_buf = std::make_shared<Buffer>(decoded);

  ::arrow20::ipc::DictionaryMemo dict_memo;
  ::arrow20::io::BufferReader input(schema_buf);

  ARROW_ASSIGN_OR_RAISE(*out, ::arrow20::ipc::ReadSchema(&input, &dict_memo));

  if (metadata->size() > 1) {
    // Copy the metadata without the schema key
    auto new_metadata = ::arrow20::key_value_metadata({}, {});
...
std::function<std::shared_ptr<::arrow20::DataType>(FieldVector)> GetNestedFactory(
    const ArrowType& origin_type, const ArrowType& inferred_type) {
  switch (inferred_type.id()) {
    case ::arrow20::Type::STRUCT:
      if (origin_type.id() == ::arrow20::Type::STRUCT) {
        return [](FieldVector fields) { return ::arrow20::struct_(std::move(fields)); };
      }
      break;
    case ::arrow20::Type::LIST:
      if (origin_type.id() == ::arrow20::Type::LIST) {
        return [](FieldVector fields) {
          DCHECK_EQ(fields.size(), 1);
          return ::arrow20::list(std::move(fields[0]));
        };
      }
      if (origin_type.id() == ::arrow20::Type::LARGE_LIST) {
        return [](FieldVector fields) {
          DCHECK_EQ(fields.size(), 1);
          return ::arrow20::large_list(std::move(fields[0]));
        };
      }
      if (origin_type.id() == ::arrow20::Type::FIXED_SIZE_LIST) {
        const auto list_size =
            checked_cast<const ::arrow20::FixedSizeListType&>(origin_type).list_size();
        return [list_size](FieldVector fields) {
          DCHECK_EQ(fields.size(), 1);
          return ::arrow20::fixed_size_list(std::move(fields[0]), list_size);
        };
      }
      break;
    default:
...
Result<bool> ApplyOriginalStorageMetadata(const Field& origin_field,
  ...
  if (num_children > 0 && origin_type->num_fields() == num_children) {
    ...
    if (factory) {
      ...
      if (modified) {
        ...
        ::arrow20::FieldVector modified_children(inferred_type->num_fields());
        for (int i = 0; i < inferred_type->num_fields(); ++i) {
  ...
  if (origin_type->id() == ::arrow20::Type::TIMESTAMP &&
      inferred_type->id() == ::arrow20::Type::TIMESTAMP) {
    // Restore time zone, if any
    const auto& ts_type = checked_cast<const ::arrow20::TimestampType&>(*inferred_type);
    const auto& ts_origin_type =
        checked_cast<const ::arrow20::TimestampType&>(*origin_type);
    ...
    if (ts_type.timezone() == "UTC" && !ts_origin_type.timezone().empty()) {
      if (ts_type.unit() == ts_origin_type.unit()) {
        ...
        auto ts_type_new = ::arrow20::timestamp(ts_type.unit(), ts_origin_type.timezone());
  ...
  if (origin_type->id() == ::arrow20::Type::DURATION &&
      inferred_type->id() == ::arrow20::Type::INT64) {
    // Read back int64 arrays as duration.
    inferred->field = inferred->field->WithType(origin_type);
    modified = true;
  }

  if (origin_type->id() == ::arrow20::Type::DICTIONARY &&
      inferred_type->id() != ::arrow20::Type::DICTIONARY &&
    ...
    const auto& dict_origin_type =
        checked_cast<const ::arrow20::DictionaryType&>(*origin_type);
    inferred->field = inferred->field->WithType(::arrow20::dictionary(
        dict_origin_type.index_type(), inferred_type, dict_origin_type.ordered()));
    modified = true;
  }

  if ((origin_type->id() == ::arrow20::Type::LARGE_BINARY &&
       inferred_type->id() == ::arrow20::Type::BINARY) ||
      (origin_type->id() == ::arrow20::Type::LARGE_STRING &&
       inferred_type->id() == ::arrow20::Type::STRING)) {
    // Read back binary-like arrays with the intended offset width.
    inferred->field = inferred->field->WithType(origin_type);
    modified = true;
  }

  if (origin_type->id() == ::arrow20::Type::DECIMAL256 &&
      inferred_type->id() == ::arrow20::Type::DECIMAL128) {
    inferred->field = inferred->field->WithType(origin_type);
    modified = true;
  }
...
Result<bool> ApplyOriginalMetadata(const Field& origin_field, SchemaField* inferred) {
  ...
  if (origin_type->id() == ::arrow20::Type::EXTENSION) {
    const auto& ex_type = checked_cast<const ::arrow20::ExtensionType&>(*origin_type);
    if (inferred_type->id() != ::arrow20::Type::EXTENSION &&
        ex_type.extension_name() == std::string("arrow.json") &&
        ::arrow20::extension::JsonExtensionType::IsSupportedStorageType(
            inferred_type->id())) {
      // Schema mismatch.
      //
      // Arrow extensions are DISABLED in Parquet.
      // origin_type is ::arrow20::extension::json()
      // inferred_type is ::arrow20::utf8()
      //
    ...
    } else if (inferred_type->id() == ::arrow20::Type::EXTENSION &&
               ex_type.extension_name() == std::string("arrow.json")) {
      // Potential schema mismatch.
      //
      // Arrow extensions are ENABLED in Parquet.
      // origin_type is arrow20::extension::json(...)
      // inferred_type is arrow20::extension::json(arrow20::utf8())
...
Status ToParquetSchema(const ::arrow20::Schema* arrow_schema,
                       const WriterProperties& properties,
                       const ArrowWriterProperties& arrow_properties,
  ...
  *out = std::make_shared<::parquet20::SchemaDescriptor>();
...
Status FromParquetSchema(
  ...
  if (manifest.origin_schema) {
    ...
    *out = ::arrow20::schema(fields, manifest.origin_schema->metadata());
  } else {
    *out = ::arrow20::schema(fields, key_value_metadata);
  }
  return Status::OK();
}
