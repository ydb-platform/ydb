...
PARQUET_EXPORT
::arrow20::Status FieldToNode(const std::shared_ptr<::arrow20::Field>& field,
                            const WriterProperties& properties,
                            const ArrowWriterProperties& arrow_properties,
                            schema::NodePtr* out);

PARQUET_EXPORT
::arrow20::Status ToParquetSchema(const ::arrow20::Schema* arrow_schema,
                                const WriterProperties& properties,
                                const ArrowWriterProperties& arrow_properties,
...
PARQUET_EXPORT
::arrow20::Status FromParquetSchema(
...
PARQUET_EXPORT
::arrow20::Status FromParquetSchema(const SchemaDescriptor* parquet_schema,
...
/// \brief Bridge between an arrow20::Field and parquet column indices.
...
struct PARQUET_EXPORT SchemaManifest {
  ...
  std::shared_ptr<::arrow20::Schema> origin_schema;
  std::shared_ptr<const ::arrow20::KeyValueMetadata> schema_metadata;
  ...
  ::arrow20::Status GetColumnField(int column_index, const SchemaField** out) const {
    auto it = column_index_to_field.find(column_index);
    if (it == column_index_to_field.end()) {
      return ::arrow20::Status::KeyError("Column index ", column_index,
  ...
  /// Coalesce a list of field indices (relative to the equivalent arrow20::Schema) which
  ...
  /// -- -- j  |
  /// -- -- -- k
  ::arrow20::Result<std::vector<int>> GetFieldIndices(
    ...
    for (int column_idx : column_indices) {
      if (column_idx < 0 || column_idx >= descr->num_columns()) {
        return ::arrow20::Status::IndexError("Column index ", column_idx, " is not valid");
      }
