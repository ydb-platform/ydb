...
using arrow20::ChunkedArray;
using arrow20::DataType;
using arrow20::DictionaryArray;
using arrow20::ExtensionArray;
using arrow20::ExtensionType;
using arrow20::Field;
using arrow20::FixedSizeBinaryArray;
using arrow20::ListArray;
using arrow20::MemoryPool;
using arrow20::NumericArray;
using arrow20::PrimitiveArray;
using arrow20::RecordBatch;
using arrow20::ResizableBuffer;
using arrow20::Result;
using arrow20::Status;
using arrow20::Table;
using arrow20::TimeUnit;

using arrow20::internal::checked_cast;

using parquet20::ParquetFileWriter;
using parquet20::ParquetVersion;
using parquet20::schema::GroupNode;
...
class ArrowColumnWriterV2 {
 public:
  ...
  static ::arrow20::Result<std::unique_ptr<ArrowColumnWriterV2>> Make(
...
class FileWriterImpl : public FileWriter {
 public:
  FileWriterImpl(std::shared_ptr<::arrow20::Schema> schema, MemoryPool* pool,
  ...
  Status WriteColumnChunk(const Array& data) override {
    ...
    auto chunk = ::arrow20::MakeArray(data.data());
    auto chunked_array = std::make_shared<::arrow20::ChunkedArray>(chunk);
  ...
  Status WriteColumnChunk(const std::shared_ptr<::arrow20::ChunkedArray>& data) override {
    return WriteColumnChunk(data, 0, data->length());
  }

  std::shared_ptr<::arrow20::Schema> schema() const override { return schema_; }
  ...
  Status WriteRecordBatch(const RecordBatch& batch) override {
    ...
    auto WriteBatch = [&](int64_t offset, int64_t size) {
      ...
      if (arrow_properties_->use_threads()) {
        ...
        RETURN_NOT_OK(::arrow20::internal::ParallelFor(
  ...
  ::arrow20::MemoryPool* memory_pool() const override {
  ...
  ::arrow20::Status AddKeyValueMetadata(
      const std::shared_ptr<const ::arrow20::KeyValueMetadata>& key_value_metadata)
      override {
...
Status FileWriter::Make(::arrow20::MemoryPool* pool,
                        std::unique_ptr<ParquetFileWriter> writer,
...
Status GetSchemaMetadata(const ::arrow20::Schema& schema, ::arrow20::MemoryPool* pool,
...
Result<std::unique_ptr<FileWriter>> FileWriter::Open(
    const ::arrow20::Schema& schema, ::arrow20::MemoryPool* pool,
  ...
  auto schema_ptr = std::make_shared<::arrow20::Schema>(schema);
...
Status WriteFileMetaData(const FileMetaData& file_metadata,
  ...
  PARQUET_CATCH_NOT_OK(::parquet20::WriteFileMetaData(file_metadata, sink));
  return Status::OK();
}

Status WriteMetaDataFile(const FileMetaData& file_metadata,
                         ::arrow20::io::OutputStream* sink) {
  PARQUET_CATCH_NOT_OK(::parquet20::WriteMetaDataFile(file_metadata, sink));
  return Status::OK();
}

Status WriteTable(const ::arrow20::Table& table, ::arrow20::MemoryPool* pool,
                  std::shared_ptr<::arrow20::io::OutputStream> sink, int64_t chunk_size,
                  std::shared_ptr<WriterProperties> properties,
                  std::shared_ptr<ArrowWriterProperties> arrow_properties) {
  std::unique_ptr<FileWriter> writer;
  ARROW_ASSIGN_OR_RAISE(
