...
class PARQUET_EXPORT FileWriter {
 public:
  static ::arrow20::Status Make(MemoryPool* pool, std::unique_ptr<ParquetFileWriter> writer,
  ...
  ///
  /// \since 11.0.0
  static ::arrow20::Result<std::unique_ptr<FileWriter>> Open(
      const ::arrow20::Schema& schema, MemoryPool* pool,
      std::shared_ptr<::arrow20::io::OutputStream> sink,
  ...
  virtual ::arrow20::Status WriteTable(
      const ::arrow20::Table& table, int64_t chunk_size = DEFAULT_MAX_ROW_GROUP_LENGTH) = 0;

  /// \brief Start a new row group.
  ///
  /// Returns an error if not all columns have been written.
  virtual ::arrow20::Status NewRowGroup() = 0;
  ...
  virtual ::arrow20::Status NewRowGroup(int64_t chunk_size) { return NewRowGroup(); }

  /// \brief Write ColumnChunk in row group using an array.
  virtual ::arrow20::Status WriteColumnChunk(const ::arrow20::Array& data) = 0;

  /// \brief Write ColumnChunk in row group using slice of a ChunkedArray
  virtual ::arrow20::Status WriteColumnChunk(
      const std::shared_ptr<::arrow20::ChunkedArray>& data, int64_t offset,
      int64_t size) = 0;

  /// \brief Write ColumnChunk in a row group using a ChunkedArray
  virtual ::arrow20::Status WriteColumnChunk(
      const std::shared_ptr<::arrow20::ChunkedArray>& data) = 0;

  /// \brief Start a new buffered row group.
  ///
  /// Returns an error if not all columns have been written.
  virtual ::arrow20::Status NewBufferedRowGroup() = 0;
  ...
  /// option in this case.
  virtual ::arrow20::Status WriteRecordBatch(const ::arrow20::RecordBatch& batch) = 0;
  ...
  virtual ::arrow20::Status AddKeyValueMetadata(
      const std::shared_ptr<const ::arrow20::KeyValueMetadata>& key_value_metadata) = 0;
...
PARQUET_EXPORT
::arrow20::Status WriteFileMetaData(const FileMetaData& file_metadata,
                                  ::arrow20::io::OutputStream* sink);

/// \brief Write metadata-only Parquet file to indicated Arrow OutputStream
PARQUET_EXPORT
::arrow20::Status WriteMetaDataFile(const FileMetaData& file_metadata,
                                  ::arrow20::io::OutputStream* sink);

/// \brief Write a Table to Parquet.
///
/// This writes one table in a single shot. To write a Parquet file with
/// multiple tables iteratively, see parquet20::arrow20::FileWriter.
///
...
/// \param properties general Parquet writer properties.
/// \param arrow_properties Arrow-specific writer properties.
::arrow20::Status PARQUET_EXPORT
WriteTable(const ::arrow20::Table& table, MemoryPool* pool,
           std::shared_ptr<::arrow20::io::OutputStream> sink,
