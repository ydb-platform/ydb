...
BlockSplitBloomFilter::BlockSplitBloomFilter(::arrow20::MemoryPool* pool)
    : pool_(pool),
...
void BlockSplitBloomFilter::Init(uint32_t num_bytes) {
  ...
  if ((num_bytes & (num_bytes - 1)) != 0) {
    num_bytes = static_cast<uint32_t>(::arrow20::bit_util::NextPower2(num_bytes));
  }
  ...
  PARQUET_ASSIGN_OR_THROW(data_, ::arrow20::AllocateBuffer(num_bytes_, pool_));
...
static ::arrow20::Status ValidateBloomFilterHeader(
  ...
  if (!header.hash.__isset.XXHASH) {
    return ::arrow20::Status::Invalid("Unsupported Bloom filter hash: ", header.hash, ".");
  }
  ...
  if (header.numBytes <= 0 ||
    ...
    return ::arrow20::Status::Invalid(ss.str());
  }
