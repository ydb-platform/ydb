...
using arrow20::internal::MultiplyWithOverflow;
  ...
  switch (encoding) {
    ...
    case Encoding::BIT_PACKED: {
      ...
      if (!bit_packed_decoder_) {
        bit_packed_decoder_ =
            std::make_unique<::arrow20::bit_util::BitReader>(data, num_bytes);
      } else {
...
void LevelDecoder::SetDataV2(int32_t num_bytes, int16_t max_level,
  ...
  if (!rle_decoder_) {
    rle_decoder_ =
        std::make_unique<::arrow20::util::RleDecoder>(data, num_bytes, bit_width_);
  } else {
...
class SerializedPageReader : public PageReader {
 ...
 private:
  ...
  std::unique_ptr<::arrow20::util::Codec> decompressor_;
...
std::shared_ptr<Page> SerializedPageReader::NextPage() {
  ...
  while (seen_num_values_ < total_num_values_) {
    ...
    if (properties_.page_checksum_verification() && current_page_header_.__isset.crc &&
      ...
      // verify crc
      uint32_t checksum =
          ::arrow20::internal::crc32(/* prev */ 0, page_buffer->data(), compressed_len);
...
class ColumnReaderImplBase {
 public:
  ...
  ColumnReaderImplBase(const ColumnDescriptor* descr, ::arrow20::MemoryPool* pool)
      : descr_(descr),
...
class TypedRecordReader : public TypedColumnReaderImpl<DType>,
 ...
 public:
  ...
  void ThrowAwayLevels(int64_t start_levels_position) {
    ...
    auto left_shift = [&](::arrow20::ResizableBuffer* buffer) {
...
class FLBARecordReader final : public TypedRecordReader<FLBAType>,
 ...
 public:
  FLBARecordReader(const ColumnDescriptor* descr, LevelInfo leaf_info,
                   ::arrow20::MemoryPool* pool, bool read_dense_for_nullable)
      : TypedRecordReader<FLBAType>(descr, leaf_info, pool, read_dense_for_nullable),
        ...
        type_(::arrow20::fixed_size_binary(byte_width_)),
        null_bitmap_builder_(pool),
        data_builder_(pool) {
    ARROW_DCHECK_EQ(descr_->physical_type(), Type::FIXED_LEN_BYTE_ARRAY);
  }

  ::arrow20::ArrayVector GetBuilderChunks() override {
    ...
    auto chunk = std::make_shared<::arrow20::FixedSizeBinaryArray>(
        type_, length, data_buffer, null_bitmap, null_count);
    return ::arrow20::ArrayVector({std::move(chunk)});
  }
  ...
  void UnsafeAppendSpaced(const FLBA* values, int64_t num_decoded,
    ...
    for (int64_t i = 0; i < num_decoded; i++) {
      if (::arrow20::bit_util::GetBit(valid_bits, valid_bits_offset + i)) {
        data_builder_.UnsafeAppend(values[i].ptr, byte_width_);
      } else {
 ...
 private:
  ...
  std::shared_ptr<::arrow20::DataType> type_;
  ::arrow20::TypedBufferBuilder<bool> null_bitmap_builder_;
  ::arrow20::BufferBuilder data_builder_;
};
...
class ByteArrayChunkedRecordReader final : public TypedRecordReader<ByteArrayType>,
 ...
 public:
  ByteArrayChunkedRecordReader(const ColumnDescriptor* descr, LevelInfo leaf_info,
    ...
    accumulator_.builder = std::make_unique<::arrow20::BinaryBuilder>(pool);
  }

  ::arrow20::ArrayVector GetBuilderChunks() override {
    ::arrow20::ArrayVector result = accumulator_.chunks;
    if (result.empty() || accumulator_.builder->length() > 0) {
      std::shared_ptr<::arrow20::Array> last_chunk;
...
/// ByteArrayDictionaryRecordReader reads into ::arrow20::dictionary(index: int32,
/// values: binary).
///
...
class ByteArrayDictionaryRecordReader final : public TypedRecordReader<ByteArrayType>,
 ...
 public:
  ...
  std::shared_ptr<::arrow20::ChunkedArray> GetResult() override {
    FlushBuilder();
    std::vector<std::shared_ptr<::arrow20::Array>> result;
    std::swap(result, result_chunks_);
    return std::make_shared<::arrow20::ChunkedArray>(std::move(result), builder_.type());
  }

  void FlushBuilder() {
    if (builder_.length() > 0) {
      std::shared_ptr<::arrow20::Array> chunk;
 ...
 private:
  ...
  ::arrow20::BinaryDictionary32Builder builder_;
  std::vector<std::shared_ptr<::arrow20::Array>> result_chunks_;
};
