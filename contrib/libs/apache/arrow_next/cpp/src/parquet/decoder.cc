...
using arrow20::VisitNullBitmapInline;
using arrow20::internal::AddWithOverflow;
using arrow20::internal::BitBlockCounter;
 ...
 public:
  ...
  int DecodeSpaced(T* buffer, int num_values, int null_count, const uint8_t* valid_bits,
    ...
    if (null_count > 0) {
      ...
      return ::arrow20::util::internal::SpacedExpand<T>(buffer, num_values, null_count,
...
class PlainBooleanDecoder : public DecoderImpl,
 ...
 private:
  std::unique_ptr<::arrow20::bit_util::BitReader> bit_reader_;
...
int PlainBooleanDecoder::Decode(uint8_t* buffer, int max_values) {
  ...
  ::arrow20::internal::CopyBitmap(this->data_, /*offset=*/total_num_values_ - num_values_,
...
struct ArrowBinaryHelper<ByteArrayType> {
  ...
  ArrowBinaryHelper(Accumulator* acc, int64_t length)
      : acc_(acc),
        ...
        chunk_space_remaining_(::arrow20::kBinaryMemoryLimit -
...
class PlainByteArrayDecoder : public PlainDecoder<ByteArrayType>,
 ...
 public:
  ...
  int DecodeArrow(int num_values, int null_count, const uint8_t* valid_bits,
                  int64_t valid_bits_offset,
                  ::arrow20::BinaryDictionary32Builder* builder) override {
    int result = 0;
...
class DictDecoderImpl : public DecoderImpl, virtual public DictDecoder<Type> {
 public:
  ...
  void SetData(int num_values, const uint8_t* data, int len) override {
    ...
    if (len == 0) {
      ...
      idx_decoder_ = ::arrow20::util::RleDecoder(data, len, /*bit_width=*/1);
      return;
    }
    ...
    idx_decoder_ = ::arrow20::util::RleDecoder(++data, --len, bit_width);
  }
  ...
  void InsertDictionary(::arrow20::ArrayBuilder* builder) override;

  int DecodeIndicesSpaced(int num_values, int null_count, const uint8_t* valid_bits,
                          int64_t valid_bits_offset,
                          ::arrow20::ArrayBuilder* builder) override {
    if (num_values > 0) {
    ...
    auto binary_builder = checked_cast<::arrow20::BinaryDictionary32Builder*>(builder);
    PARQUET_THROW_NOT_OK(
  ...
  int DecodeIndices(int num_values, ::arrow20::ArrayBuilder* builder) override {
    num_values = std::min(num_values, num_values_);
    if (num_values > 0) {
 ...
 protected:
  ...
  ::arrow20::util::RleDecoder idx_decoder_;
};
...
int DictDecoderImpl<FLBAType>::DecodeArrow(
    ...
    typename EncodingTraits<FLBAType>::DictAccumulator* builder) {
  auto value_type =
      checked_cast<const ::arrow20::DictionaryType&>(*builder->type()).value_type();
  auto byte_width =
      checked_cast<const ::arrow20::FixedSizeBinaryType&>(*value_type).byte_width();
...
template <typename Type>
void DictDecoderImpl<Type>::InsertDictionary(::arrow20::ArrayBuilder* builder) {
  ParquetException::NYI("InsertDictionary only implemented for BYTE_ARRAY types");
}

template <>
void DictDecoderImpl<ByteArrayType>::InsertDictionary(::arrow20::ArrayBuilder* builder) {
  auto binary_builder = checked_cast<::arrow20::BinaryDictionary32Builder*>(builder);

  // Make a BinaryArray referencing the internal dictionary data
  auto arr = std::make_shared<::arrow20::BinaryArray>(
...
class DictByteArrayDecoderImpl : public DictDecoderImpl<ByteArrayType>,
 ...
 private:
  Status DecodeArrowDense(int num_values, int null_count, const uint8_t* valid_bits,
    ...
    ::arrow20::internal::BitBlockCounter bit_blocks(valid_bits, valid_bits_offset,
                                                  num_values);
    int64_t position = 0;
  ...
  Status DecodeArrow(int num_values, int null_count, const uint8_t* valid_bits,
    ...
    RETURN_NOT_OK(builder->Reserve(num_values));
    ::arrow20::internal::BitmapReader bit_reader(valid_bits, valid_bits_offset, num_values);
...
class DeltaBitPackDecoder : public DecoderImpl, public TypedDecoderImpl<DType> {
 public:
  ...
  void SetData(int num_values, const uint8_t* data, int len) override {
    ...
    if (decoder_ == nullptr) {
      decoder_ = std::make_shared<::arrow20::bit_util::BitReader>(data, len);
    } else {
  ...
  void SetDecoder(int num_values, std::shared_ptr<::arrow20::bit_util::BitReader> decoder) {
    this->num_values_ = num_values;
 ...
 private:
  ...
  MemoryPool* pool_;
  std::shared_ptr<::arrow20::bit_util::BitReader> decoder_;
...
class RleBooleanDecoder : public DecoderImpl,
 ...
 public:
  ...
  void SetData(int num_values, const uint8_t* data, int len) override {
    ...
    num_bytes = ::arrow20::bit_util::FromLittleEndian(SafeLoadAs<uint32_t>(data));
    ...
    if (decoder_ == nullptr) {
      decoder_ = std::make_shared<::arrow20::util::RleDecoder>(decoder_data, num_bytes,
                                                             /*bit_width=*/1);
    } else {
 ...
 private:
  std::shared_ptr<::arrow20::util::RleDecoder> decoder_;
};
...
class ByteStreamSplitDecoderBase : public DecoderImpl, public TypedDecoderImpl<DType> {
 ...
 protected:
  int DecodeRaw(uint8_t* out_buffer, int max_values) {
    ...
    ::arrow20::util::internal::ByteStreamSplitDecode(data_, byte_width_, values_to_decode,
                                                   stride_, out_buffer);
  ...
  uint8_t* EnsureDecodeBuffer(int64_t min_values) {
    ...
    if (!decode_buffer_ || decode_buffer_->size() < size) {
      const auto alloc_size = ::arrow20::bit_util::NextPower2(size);
      PARQUET_ASSIGN_OR_THROW(decode_buffer_, ::arrow20::AllocateBuffer(alloc_size));
    }
