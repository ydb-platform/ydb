...
#include "contrib/libs/apache/arrow_next/cpp/src/parquet/encryption/file_key_material_store.h"
...
class PARQUET_EXPORT FileSystemKeyMaterialStore : public FileKeyMaterialStore {
 public:
  ...
  FileSystemKeyMaterialStore(const std::string& key_material_file_path,
                             const std::shared_ptr<::arrow20::fs::FileSystem>& file_system);
  ...
  static std::shared_ptr<FileSystemKeyMaterialStore> Make(
      ...
      const std::shared_ptr<::arrow20::fs::FileSystem>& file_system, bool use_tmp_prefix);
 ...
 private:
  ...
  std::shared_ptr<::arrow20::fs::FileSystem> file_system_;
