...
#include "contrib/libs/apache/arrow_next/cpp/src/parquet/test_util.h"

namespace parquet20 {
class ParquetFileReader;
namespace encryption::test {

using ::arrow20::internal::TemporaryDir;
...
inline ::arrow20::Result<std::unique_ptr<TemporaryDir>> temp_data_dir() {
...
class FileEncryptor {
 public:
  ...
  void EncryptFile(
      std::string file,
      std::shared_ptr<parquet20::FileEncryptionProperties> encryption_configurations);
...
class FileDecryptor {
 ...
 private:
  void CheckFile(
      parquet20::ParquetFileReader* file_reader,
      const std::shared_ptr<FileDecryptionProperties>& file_decryption_properties);
  void CheckPageIndex(
      parquet20::ParquetFileReader* file_reader,
      const std::shared_ptr<FileDecryptionProperties>& file_decryption_properties);
