...
#define END_PARQUET_CATCH_EXCEPTIONS                   \
  ...
  catch (const ::parquet20::ParquetStatusException& e) { \
    return e.status();                                 \
  }                                                    \
  catch (const ::parquet20::ParquetException& e) {       \
    return ::arrow20::Status::IOError(e.what());         \
  }
...
#define PARQUET_THROW_NOT_OK(s)                                 \
  do {                                                          \
    ::arrow20::Status _s = ::arrow20::internal::GenericToStatus(s); \
    if (!_s.ok()) {                                             \
      throw ::parquet20::ParquetStatusException(std::move(_s));   \
    }                                                           \
  } while (0)
...
class ParquetException : public std::exception {
 public:
  ...
  explicit ParquetException(Args&&... args)
      : msg_(::arrow20::util::StringBuilder(std::forward<Args>(args)...)) {}
...
class ParquetStatusException : public ParquetException {
 public:
  explicit ParquetStatusException(::arrow20::Status status)
      : ParquetException(status.ToString()), status_(std::move(status)) {}

  const ::arrow20::Status& status() const { return status_; }

 private:
  ::arrow20::Status status_;
};
...
class ParquetInvalidOrCorruptedFileException : public ParquetStatusException {
 public:
  ...
  explicit ParquetInvalidOrCorruptedFileException(Arg arg, Args&&... args)
      : ParquetStatusException(::arrow20::Status::Invalid(std::forward<Arg>(arg),
