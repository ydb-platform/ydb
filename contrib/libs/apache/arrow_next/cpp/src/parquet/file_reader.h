...
class PARQUET_EXPORT ParquetFileReader {
 public:
  ...
  struct PARQUET_EXPORT Contents {
    ...
    static ::arrow20::Future<std::unique_ptr<Contents>> OpenAsync(
        std::shared_ptr<::arrow20::io::RandomAccessFile> source,
        const ReaderProperties& props = default_reader_properties(),
        std::shared_ptr<FileMetaData> metadata = NULLPTR);
  ...
  static ::arrow20::Future<std::unique_ptr<ParquetFileReader>> OpenAsync(
      std::shared_ptr<::arrow20::io::RandomAccessFile> source,
      const ReaderProperties& props = default_reader_properties(),
      std::shared_ptr<FileMetaData> metadata = NULLPTR);
...
ReadMetaData(const std::shared_ptr<::arrow20::io::RandomAccessFile>& source);
