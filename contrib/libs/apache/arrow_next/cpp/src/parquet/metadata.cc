...
bool ColumnChunkMetaData::can_decompress() const {
  return ::arrow20::util::Codec::IsAvailable(compression());
}
...
std::unique_ptr<RowGroupMetaData> RowGroupMetaData::Make(
    ...
    const ReaderProperties& properties, const ApplicationVersion* writer_version,
    std::shared_ptr<InternalFileDecryptor> file_decryptor) {
  return std::unique_ptr<parquet20::RowGroupMetaData>(new RowGroupMetaData(
...
static void Scrub(std::string* s) {
  static ::arrow20::random::pcg64 rng;
...
class FileMetaData::FileMetaDataImpl {
 public:
  ...
  bool VerifySignature(const void* signature) {
    ...
    ::arrow20::util::span<const uint8_t> serialized_data_span(serialized_data,
                                                            serialized_len);

    // encrypt with nonce
    ::arrow20::util::span<const uint8_t> nonce(reinterpret_cast<const uint8_t*>(signature),
  ...
  void WriteTo(::arrow20::io::OutputStream* dst,
 ...
 private:
  ...
  void InitColumnOrders() {
    // update ColumnOrder
    std::vector<parquet20::ColumnOrder> column_orders;
...
void FileMetaData::WriteTo(::arrow20::io::OutputStream* dst,
                           const std::shared_ptr<Encryptor>& encryptor) const {
...
class FileCryptoMetaData::FileCryptoMetaDataImpl {
 public:
  ...
  void WriteTo(::arrow20::io::OutputStream* dst) const {
    ThriftSerializer serializer;
...
void FileCryptoMetaData::WriteTo(::arrow20::io::OutputStream* dst) const {
  impl_->WriteTo(dst);
}
...
std::string FileMetaData::SerializeToString() const {
  ...
  PARQUET_ASSIGN_OR_THROW(auto serializer, ::arrow20::io::BufferOutputStream::Create(0));
...
class ColumnChunkMetaDataBuilder::ColumnChunkMetaDataBuilderImpl {
 public:
  ...
  void WriteTo(::arrow20::io::OutputStream* sink) {
    ThriftSerializer serializer;
...
void ColumnChunkMetaDataBuilder::WriteTo(::arrow20::io::OutputStream* sink) {
  impl_->WriteTo(sink);
}
...
class FileMetaDataBuilder::FileMetaDataBuilderImpl {
 public:
  ...
  std::unique_ptr<FileMetaData> Finish(
    ...
    ToParquet(static_cast<parquet20::schema::GroupNode*>(schema_->schema_root().get()),
              &metadata_->schema);
