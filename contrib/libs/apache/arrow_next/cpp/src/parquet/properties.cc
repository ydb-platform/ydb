...
std::shared_ptr<ArrowInputStream> ReaderProperties::GetStream(
  ...
  if (buffered_stream_enabled_) {
    ...
    PARQUET_ASSIGN_OR_THROW(
        std::shared_ptr<::arrow20::io::InputStream> safe_stream,
        ::arrow20::io::RandomAccessFile::GetStream(source, start, num_bytes));
    PARQUET_ASSIGN_OR_THROW(
        auto stream, ::arrow20::io::BufferedInputStream::Create(buffer_size_, pool_,
    ...
    return std::make_shared<::arrow20::io::BufferReader>(data);
  }
}

::arrow20::internal::Executor* ArrowWriterProperties::executor() const {
  return executor_ != nullptr ? executor_ : ::arrow20::internal::GetCpuThreadPool();
}
