...
struct CompareHelper {
  ...
  static ::arrow20::enable_if_t<std::is_floating_point<T1>::value, T> Coalesce(T val,
                                                                             T fallback) {
    return std::isnan(val) ? fallback : val;
  }

  template <typename T1 = T>
  static ::arrow20::enable_if_t<!std::is_floating_point<T1>::value, T> Coalesce(
      T val, T fallback) {
    return val;
  }
...
template <typename T>
::arrow20::enable_if_t<std::is_integral<T>::value, optional<std::pair<T, T>>>
...
template <typename T>
::arrow20::enable_if_t<std::is_floating_point<T>::value, optional<std::pair<T, T>>>
CleanStatistic(std::pair<T, T> min_max, LogicalType::Type::type) {
  T min = min_max.first;
...
class TypedComparatorImpl
 ...
 public:
  ...
  std::pair<T, T> GetMinMax(const ::arrow20::Array& values) const override {
...
std::pair<ByteArray, ByteArray> TypedComparatorImpl<true, ByteArrayType>::GetMinMax(
    const ::arrow20::Array& values) const {
...
class TypedStatisticsImpl : public TypedStatistics<DType> {
 public:
  ...
  void Update(const ::arrow20::Array& values, bool update_counts) override {
    if (update_counts) {
