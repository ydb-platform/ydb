...
#include "contrib/libs/apache/arrow_next/cpp/src/parquet/stream_reader.h"
...
StreamReader& StreamReader::operator>>(optional<::arrow20::Decimal128>& v) {
  ...
  if (node->physical_type() == Type::FIXED_LEN_BYTE_ARRAY) {
    ...
    if (ReadOptional(&flba)) {
      PARQUET_ASSIGN_OR_THROW(v,
                              ::arrow20::Decimal128::FromBigEndian(flba.ptr, type_length));
    } else {
      v.reset();
    }
    ...
    ByteArray ba;
    if (ReadOptional(&ba)) {
      PARQUET_ASSIGN_OR_THROW(v, ::arrow20::Decimal128::FromBigEndian(ba.ptr, ba.len));
    } else {
      v.reset();
    }
  } else {
...
StreamReader& StreamReader::operator>>(::arrow20::Decimal128& v) {
  const auto& node = nodes_[column_index_];
  std::optional<::arrow20::Decimal128> maybe_v;
  *this >> maybe_v;
