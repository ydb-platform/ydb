...
// Mirrors parquet20::ConvertedType
...
// Mirrors parquet20::FieldRepetitionType
struct Repetition {
...
class PARQUET_EXPORT LogicalType {
 public:
  ...
  static std::shared_ptr<const LogicalType> FromConvertedType(
      const parquet20::ConvertedType::type converted_type,
      const parquet20::schema::DecimalMetadata converted_decimal_metadata = {false, -1,
                                                                           -1});
  ...
  static std::shared_ptr<const LogicalType> FromThrift(
      const parquet20::format::LogicalType& thrift_logical_type);
  ...
  bool is_compatible(parquet20::ConvertedType::type converted_type,
                     parquet20::schema::DecimalMetadata converted_decimal_metadata = {
                         false, -1, -1}) const;

  /// \brief If possible, return the legacy converted type (and decimal metadata if
  /// applicable) equivalent to this logical type.
  parquet20::ConvertedType::type ToConvertedType(
      parquet20::schema::DecimalMetadata* out_decimal_metadata) const;
  ...
  parquet20::format::LogicalType ToThrift() const;
...
// Data encodings. Mirrors parquet20::Encoding
struct Encoding {
  enum type {
    PLAIN = 0,
    PLAIN_DICTIONARY = 2,
...
static inline void Int96SetNanoSeconds(parquet20::Int96& i96, int64_t nanoseconds) {
...
static inline DecodedInt96 DecodeInt96Timestamp(const parquet20::Int96& i96) {
...
static inline int64_t Int96GetNanoSeconds(const parquet20::Int96& i96) {
  const auto decoded = DecodeInt96Timestamp(i96);
  return static_cast<int64_t>(decoded.days_since_epoch * kNanosecondsPerDay +
                              decoded.nanoseconds);
}

static inline int64_t Int96GetMicroSeconds(const parquet20::Int96& i96) {
  const auto decoded = DecodeInt96Timestamp(i96);
...
static inline int64_t Int96GetMilliSeconds(const parquet20::Int96& i96) {
  const auto decoded = DecodeInt96Timestamp(i96);
...
static inline int64_t Int96GetSeconds(const parquet20::Int96& i96) {
  const auto decoded = DecodeInt96Timestamp(i96);
