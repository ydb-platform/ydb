...
S57ClassContentExplorer::S57ClassContentExplorer(
    ...
    : poRegistrar(poRegistrarIn), papapszClassesFields(nullptr),
      iCurrentClass(-1), papszCurrentFields(nullptr), papszTempResult(nullptr)
{
}
...
bool S57ClassRegistrar::LoadInfo(const char *pszDirectory,
                                 const char *pszProfile, bool bReportErr)

{
    ...
    if (EQUAL(pszProfile, "Additional_Military_Layers"))
    {
        ...
        snprintf(szTargetFile, sizeof(szTargetFile), "s57objectclasses_%s.csv",
                 "aml");
    }
    ...
    {
        // Has been suppressed in GDAL data/
        snprintf(szTargetFile, sizeof(szTargetFile), "s57attributes_%s.csv",
                 "aml");
    }
    else if (EQUAL(pszProfile, "Inland_Waterways"))
    {
    ...
    while ((pszLine = ReadLine(fp)) != nullptr)
    {
        ...
        {
            CPLDebug("S57", "Duplicate/corrupt definition for attribute %d:%s",
                     iAttr, papszTokens[2]);
            CSLDestroy(papszTokens);
            continue;
        }
...

{
    ...
    if (papapszClassesFields == nullptr)
    {
        papapszClassesFields =
            (char ***)CPLCalloc(sizeof(void *), poRegistrar->nClasses);
    }
    ...
    if (papapszClassesFields[nNewIndex] == nullptr)
        papapszClassesFields[nNewIndex] = CSLTokenizeStringComplex(
            poRegistrar->apszClassesInfo[nNewIndex], ",", TRUE, TRUE);
...

{
    for (int i = 0; i < poRegistrar->nClasses; i++)
    {
        ...
        if (pszClassAcronym != nullptr &&
            strcmp(pszClassAcronym, pszAcronym) == 0)
            return true;
    }
