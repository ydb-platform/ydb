...
class OrcaService : public Service {
 public:
  struct Options {
    ...
    y_absl::Duration min_report_duration = y_absl::Seconds(30);

    Options() = default;
    Options& set_min_report_duration(y_absl::Duration duration) {
 ...
 private:
  ...
  const y_absl::Duration min_report_duration_;
  grpc::internal::Mutex mu_;
  // Contains the last serialized metrics from server_metric_recorder_.
  y_absl::optional<Slice> response_slice_ Y_ABSL_GUARDED_BY(mu_);
  // The update sequence number of metrics serialized in response_slice_.
  y_absl::optional<uint64_t> response_slice_seq_ Y_ABSL_GUARDED_BY(mu_);
};
