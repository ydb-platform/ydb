...
class ClientChannel::PromiseBasedCallData : public ClientChannel::CallData {
 public:
  ...
  ArenaPromise<y_absl::StatusOr<CallArgs>> MakeNameResolutionPromise(
      CallArgs call_args) {
    ...
    return [this, call_args = std::move(
                      call_args)]() mutable -> Poll<y_absl::StatusOr<CallArgs>> {
 ...
 private:
  ...
  void RetryCheckResolutionLocked()
      Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(&ClientChannel::resolution_mu_) override {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_client_channel_call_trace)) {
  ...
  Waker waker_ Y_ABSL_GUARDED_BY(&ClientChannel::resolution_mu_);
};
 ...
 public:
  ...
  static void Destroy(grpc_call_element* elem,
    ...
    if (GPR_LIKELY(subchannel_call != nullptr)) {
      ...
      ExecCtx::Run(DEBUG_LOCATION, then_schedule_closure, y_absl::OkStatus());
    }
  }
   ...
   public:
    ...
    ~WatcherWrapper() override {
      if (!IsWorkSerializerDispatchEnabled()) {
        ...
        parent->chand_->work_serializer_->Run(
            [parent]() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(
    ...
    void OnConnectivityStateChange(
      ...
      parent_->chand_->work_serializer_->Run(
          [this, state, status]() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(
              *parent_->chand_->work_serializer_) {
   ...
   private:
    void ApplyUpdateInControlPlaneWorkSerializer(grpc_connectivity_state state,
      ...
      watcher_->OnConnectivityStateChange(
          state,
          state == GRPC_CHANNEL_TRANSIENT_FAILURE ? status : y_absl::OkStatus());
    }
...
void ClientChannel::ExternalConnectivityWatcher::Notify(
  ...
  *state_ = state;
  ExecCtx::Run(DEBUG_LOCATION, on_complete_, y_absl::OkStatus());
...
void ClientChannel::ExternalConnectivityWatcher::Cancel() {
  ...
  ExecCtx::Run(DEBUG_LOCATION, on_complete_, y_absl::CancelledError());
  // Hop back into the work_serializer to clean up.
  // Note: The callback takes a ref in case the ref inside the state tracker
  // gets removed before the callback runs via a SHUTDOWN notification.
  Ref(DEBUG_LOCATION, "RemoveWatcherLocked()").release();
...
void ClientChannel::ExternalConnectivityWatcher::AddWatcherLocked() {
  Closure::Run(DEBUG_LOCATION, watcher_timer_init_, y_absl::OkStatus());
...
RefCountedPtr<LoadBalancingPolicy::Config> ChooseLbPolicy(
  ...
  Json config_json = Json::FromArray({Json::FromObject({
      {TString(*policy_name), Json::FromObject({})},
  })});
...
void ClientChannel::CreateResolverLocked() {
  ...
  UpdateStateLocked(GRPC_CHANNEL_CONNECTING, y_absl::Status(),
                    "started resolving");
...
void ClientChannel::StartTransportOpLocked(grpc_transport_op* op) {
  ...
  ExecCtx::Run(DEBUG_LOCATION, op->on_consumed, y_absl::OkStatus());
}
...
grpc_error_handle ClientChannel::CallData::ApplyServiceConfigToCallLocked(
  ...
  y_absl::Status call_config_status =
      (*config_selector)
          ->GetCallConfig(
...
class ClientChannel::LoadBalancedCall::Metadata
 ...
 public:
  ...
  y_absl::optional<y_absl::string_view> Lookup(y_absl::string_view key,
    ...
    if (batch_ == nullptr) return y_absl::nullopt;
...
y_absl::optional<y_absl::Status> ClientChannel::LoadBalancedCall::PickSubchannel(
  ...
  y_absl::AnyInvocable<void(RefCountedPtr<LoadBalancingPolicy::SubchannelPicker>)>
      set_picker;
  if (!IsWorkSerializerDispatchEnabled()) {
    set_picker =
        [&](RefCountedPtr<LoadBalancingPolicy::SubchannelPicker> picker) {
...
void ClientChannel::FilterBasedLoadBalancedCall::RecvTrailingMetadataReady(
  ...
  if (!self->failure_error_.ok()) {
    ...
    self->failure_error_ = y_absl::OkStatus();
  }
...
ClientChannel::PromiseBasedLoadBalancedCall::PromiseBasedLoadBalancedCall(
    ClientChannel* chand, y_absl::AnyInvocable<void()> on_commit,
    bool is_transparent_retry)
  ...
  return OnCancel(
      Map(TrySeq(
              ...
              [this]() -> Poll<y_absl::Status> {
                ...
                if (result == y_absl::nullopt) return Pending{};
      ...
      [lb_call = std::move(lb_call)]() {
        ...
        if (lb_call->call_attempt_tracer() != nullptr ||
          ...
          // CANCELLED in this case.
          lb_call->RecordCallCompletion(y_absl::CancelledError("call cancelled"),
                                        nullptr, nullptr, "");
        }
      });
}
