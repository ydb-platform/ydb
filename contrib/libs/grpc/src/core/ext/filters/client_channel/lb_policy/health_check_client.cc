...
void HealthProducer::HealthChecker::StartHealthStreamLocked() {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_health_check_client_trace)) {
    gpr_log(GPR_INFO,
            ...
            producer_.get(), this,
            TString(health_check_service_name_).c_str());
  }
...
void HealthProducer::HealthChecker::NotifyWatchersLocked(
    grpc_connectivity_state state, y_absl::Status status) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_health_check_client_trace)) {
    gpr_log(
        GPR_INFO,
...
void HealthProducer::HealthChecker::OnHealthWatchStatusChange(
  ...
  y_absl::Status use_status;
  if (!status.ok()) {
    TString address_str =
        grpc_sockaddr_to_uri(&producer_->subchannel_->address())
            .value_or("<unknown address type>");
    use_status = y_absl::Status(
        status.code(), y_absl::StrCat(address_str, ": ", status.message()));
  }
  work_serializer_->Schedule(
...
class HealthProducer::ConnectivityWatcher
 ...
 public:
  ...
  void OnConnectivityStateChange(
      ...
      grpc_connectivity_state state, const y_absl::Status& status) override {
...
void HealthWatcher::Notify(grpc_connectivity_state state, y_absl::Status status) {
  work_serializer_->Schedule(
