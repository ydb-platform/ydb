...
y_absl::string_view GetAddressFamily(const grpc_resolved_address& address) {
  const char* uri_scheme = grpc_sockaddr_get_uri_scheme(&address);
  return y_absl::string_view(uri_scheme == nullptr ? "other" : uri_scheme);
};
...
class AddressFamilyIterator {
 public:
  AddressFamilyIterator(y_absl::string_view scheme, size_t index)
...
void PickFirst::HealthWatcher::OnConnectivityStateChange(
    grpc_connectivity_state new_state, y_absl::Status status) {
...
void PickFirst::SubchannelList::SubchannelData::OnConnectivityStateChange(
  ...
  y_absl::optional<grpc_connectivity_state> old_state = connectivity_state_;
  ...
  if (p->selected_ == this) {
    ...
    if (p->latest_pending_subchannel_list_ != nullptr) {
      ...
      if (IsPickFirstHappyEyeballsEnabled()
        ...
        status = y_absl::UnavailableError(y_absl::StrCat(
        ...
        p->UpdateState(GRPC_CHANNEL_CONNECTING, y_absl::Status(),
...
void PickFirst::SubchannelList::SubchannelData::ProcessUnselectedReadyLocked() {
  ...
  if (p->enable_health_watch_) {
    ...
    p->UpdateState(GRPC_CHANNEL_READY, y_absl::Status(),
...
void PickFirst::SubchannelList::MaybeFinishHappyEyeballsPass() {
  ...
  if (policy_->subchannel_list_.get() == this) {
    ...
    y_absl::Status status = y_absl::UnavailableError(
        y_absl::StrCat((policy_->omit_status_message_prefix_
                          ? ""
                          : "failed to connect to all addresses; last error: "),
