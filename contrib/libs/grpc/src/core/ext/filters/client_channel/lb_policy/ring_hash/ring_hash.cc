...
RingHash::PickResult RingHash::Picker::Pick(PickArgs args) {
  ...
  if (hash_attribute == nullptr) {
    return PickResult::Fail(y_absl::InternalError("hash attribute not present"));
  }
...
RingHash::Ring::Ring(RingHash* ring_hash, RingHashLbConfig* config) {
  ...
  struct EndpointWeight {
    TString address;  // Key by endpoint's first address.
  ...
  for (size_t i = 0; i < endpoints.size(); ++i) {
    const TString& address_string = endpoint_weights[i].address;
...
void RingHash::UpdateAggregatedConnectivityStateLocked(
    bool entered_transient_failure, y_absl::Status status) {
