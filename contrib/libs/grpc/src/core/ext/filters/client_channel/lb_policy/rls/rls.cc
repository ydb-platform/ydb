...
class RlsLbConfig : public LoadBalancingPolicy::Config {
 public:
  ...
  const TString& rls_channel_service_config() const {
 ...
 private:
  ...
  class Cache {
   public:
    ...
    class Entry : public InternallyRefCounted<Entry> {
     public:
      ...
      Timestamp stale_time() const Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(&RlsLb::mu_) {
        return stale_time_;
      }
     ...
     private:
      class BackoffTimer : public InternallyRefCounted<BackoffTimer> {
       ...
       private:
        ...
        y_absl::optional<EventEngine::TaskHandle> backoff_timer_task_handle_
            Y_ABSL_GUARDED_BY(&RlsLb::mu_);
      };
      ...
      Timestamp backoff_time_ Y_ABSL_GUARDED_BY(&RlsLb::mu_) =
          Timestamp::InfPast();
      Timestamp backoff_expiration_time_ Y_ABSL_GUARDED_BY(&RlsLb::mu_) =
          Timestamp::InfPast();
      ...
      Timestamp data_expiration_time_ Y_ABSL_GUARDED_BY(&RlsLb::mu_) =
          Timestamp::InfPast();
      Timestamp stale_time_ Y_ABSL_GUARDED_BY(&RlsLb::mu_) = Timestamp::InfPast();

      Timestamp min_expiration_time_ Y_ABSL_GUARDED_BY(&RlsLb::mu_);
   ...
   private:
    ...
    void StartCleanupTimer() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(&RlsLb::mu_);
    ...
    y_absl::optional<EventEngine::TaskHandle> cleanup_timer_handle_;
  };
   ...
   private:
    ...
    class Throttle {
     ...
     private:
      ...
      std::deque<Timestamp> requests_ Y_ABSL_GUARDED_BY(&RlsLb::mu_);

      // Logged timestamps of failures.
      std::deque<Timestamp> failures_ Y_ABSL_GUARDED_BY(&RlsLb::mu_);
    };
  ...
  y_absl::StatusOr<EndpointAddressesList> addresses_;
...
y_absl::optional<Json> InsertOrUpdateChildPolicyField(const TString& field,
                                                    const TString& value,
                                                    const Json& config,
                                                    ValidationErrors* errors) {
  ...
  for (size_t i = 0; i < config.array().size(); ++i) {
    ...
    if (child_json.type() != Json::Type::kObject) {
      ...
      if (child.size() != 1) {
        ...
        const TString& child_name = child.begin()->first;
        ValidationErrors::ScopedField json_field(
            errors, y_absl::StrCat("[\"", child_name, "\"]"));
  ...
  if (errors->size() != original_num_errors) return y_absl::nullopt;
...
std::map<TString, TString> BuildKeyMap(
  ...
  if (!key_builder->host_key.empty()) {
    key_map[key_builder->host_key] = TString(host);
  }
  // Add service key.
...
y_absl::Status RlsLb::UpdateLocked(UpdateArgs args) {
  ...
  y_absl::StatusOr<EndpointAddressesList> old_addresses;
