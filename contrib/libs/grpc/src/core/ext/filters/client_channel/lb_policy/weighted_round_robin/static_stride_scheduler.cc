...
y_absl::optional<StaticStrideScheduler> StaticStrideScheduler::Make(
    y_absl::Span<const float> float_weights,
    y_absl::AnyInvocable<uint32_t()> next_sequence_func) {
  if (float_weights.empty()) return y_absl::nullopt;
  if (float_weights.size() == 1) return y_absl::nullopt;
  ...
  if (num_zero_weight_channels == n) return y_absl::nullopt;
