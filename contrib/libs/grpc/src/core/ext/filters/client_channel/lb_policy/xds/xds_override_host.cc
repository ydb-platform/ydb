 ...
 private:
  class SubchannelWrapper : public DelegatingSubchannel {
   public:
    ...
    void set_key(y_absl::string_view key) { key_ = TString(key); }
    const y_absl::optional<TString>& key() const { return key_; }
  ...
  class SubchannelEntry {
   ...
   private:
    y_absl::variant<WeakRefCountedPtr<SubchannelWrapper>,
  ...
  y_absl::StatusOr<EndpointAddressesList> UpdateAddressMap(
      y_absl::StatusOr<EndpointAddressesList> endpoints);
  ...
  void OnSubchannelConnectivityStateChange(y_absl::string_view subchannel_key)
      Y_ABSL_NO_THREAD_SAFETY_ANALYSIS;  // Called from within the
...
XdsOverrideHostLb::Picker::PickOverridenHost(
  ...
  if (cookie_address_list.empty()) return y_absl::nullopt;
  ...
  bool found_connecting = false;
  {
    ...
    for (y_absl::string_view address : y_absl::StrSplit(cookie_address_list, ',')) {
      RefCountedPtr<SubchannelWrapper> subchannel;
...
y_absl::StatusOr<EndpointAddressesList> XdsOverrideHostLb::UpdateAddressMap(
    y_absl::StatusOr<EndpointAddressesList> endpoints) {
  if (!endpoints.ok()) {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_xds_override_host_trace)) {
  ...
  std::map<const TString, AddressInfo> addresses_for_map;
  for (const auto& endpoint : *endpoints) {
    ...
    y_absl::Span<const TString> addresses_span = addresses;
    for (size_t i = 0; i < addresses.size(); ++i) {
      TString start = y_absl::StrJoin(addresses_span.subspan(0, i), ",");
      TString end = y_absl::StrJoin(addresses_span.subspan(i + 1), ",");
      RefCountedStringValue address_list(
          y_absl::StrCat(addresses[i], (start.empty() ? "" : ","), start,
                       (end.empty() ? "" : ","), end));
