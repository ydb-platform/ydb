...
constexpr y_absl::string_view kXdsWrrLocality = "xds_wrr_locality_experimental";
...
class XdsWrrLocalityLbConfig : public LoadBalancingPolicy::Config {
 public:
  ...
  y_absl::string_view name() const override { return kXdsWrrLocality; }
...
y_absl::Status XdsWrrLocalityLb::UpdateLocked(UpdateArgs args) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_wrr_locality_lb_trace)) {
  ...
  std::map<TString, uint32_t> locality_weights;
  ...
  for (const auto& p : locality_weights) {
    const TString& locality_name = p.first;
  ...
  if (!child_config.ok()) {
    ...
    y_absl::Status status = y_absl::InternalError(y_absl::StrCat(
