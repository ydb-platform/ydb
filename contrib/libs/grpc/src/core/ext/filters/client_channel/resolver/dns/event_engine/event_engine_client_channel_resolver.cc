...
class EventEngineClientChannelDNSResolver : public PollingResolver {
 ...
 private:
  ...
  class EventEngineDNSRequestWrapper
   ...
   private:
    void OnTimeout() Y_ABSL_LOCKS_EXCLUDED(on_resolved_mu_);
    void OnHostnameResolved(
        y_absl::StatusOr<std::vector<EventEngine::ResolvedAddress>> addresses);
    void OnSRVResolved(
        y_absl::StatusOr<std::vector<EventEngine::DNSResolver::SRVRecord>>
            srv_records);
    void OnBalancerHostnamesResolved(
        TString authority,
        y_absl::StatusOr<std::vector<EventEngine::ResolvedAddress>> addresses);
    void OnTXTResolved(y_absl::StatusOr<std::vector<TString>> service_config);
    ...
    y_absl::optional<Resolver::Result> OnResolvedLocked()
    ...
    // Lookup callbacks
    bool is_hostname_inflight_ Y_ABSL_GUARDED_BY(on_resolved_mu_) = false;
    bool is_srv_inflight_ Y_ABSL_GUARDED_BY(on_resolved_mu_) = false;
    bool is_txt_inflight_ Y_ABSL_GUARDED_BY(on_resolved_mu_) = false;
    // Output fields from requests.
    EndpointAddressesList addresses_ Y_ABSL_GUARDED_BY(on_resolved_mu_);
    EndpointAddressesList balancer_addresses_ Y_ABSL_GUARDED_BY(on_resolved_mu_);
    ValidationErrors errors_ Y_ABSL_GUARDED_BY(on_resolved_mu_);
    y_absl::StatusOr<TString> service_config_json_
        Y_ABSL_GUARDED_BY(on_resolved_mu_);
    // Other internal state
    size_t number_of_balancer_hostnames_initiated_
        Y_ABSL_GUARDED_BY(on_resolved_mu_) = 0;
    size_t number_of_balancer_hostnames_resolved_
        Y_ABSL_GUARDED_BY(on_resolved_mu_) = 0;
    bool orphaned_ Y_ABSL_GUARDED_BY(on_resolved_mu_) = false;
    y_absl::optional<EventEngine::TaskHandle> timeout_handle_
        Y_ABSL_GUARDED_BY(on_resolved_mu_);
...
EventEngineClientChannelDNSResolver::EventEngineDNSRequestWrapper::
  ...
  if (resolver_->enable_srv_queries_) {
    ...
    event_engine_resolver_->LookupSRV(
        ...
        y_absl::StrCat("_grpclb._tcp.", resolver_->name_to_resolve()));
  }
  if (resolver_->request_service_config_) {
    ...
    event_engine_resolver_->LookupTXT(
        [self = Ref(DEBUG_LOCATION, "OnTXTResolved")](
            y_absl::StatusOr<std::vector<TString>> service_config) mutable {
          ApplicationCallbackExecCtx callback_exec_ctx;
          ExecCtx exec_ctx;
          self->OnTXTResolved(std::move(service_config));
          self.reset();
        },
        y_absl::StrCat("_grpc_config.", resolver_->name_to_resolve()));
  }
...
void EventEngineClientChannelDNSResolver::EventEngineDNSRequestWrapper::
    OnHostnameResolved(y_absl::StatusOr<std::vector<EventEngine::ResolvedAddress>>
                           new_addresses) {
  y_absl::optional<Resolver::Result> result;
  {
    MutexLock lock(&on_resolved_mu_);
...
void EventEngineClientChannelDNSResolver::EventEngineDNSRequestWrapper::
  ...
  for (auto& srv_record : *srv_records) {
    ...
    event_engine_resolver_->LookupHostname(
        ...
        srv_record.host, ::ToString(srv_record.port));
  }
}
...
void EventEngineClientChannelDNSResolver::EventEngineDNSRequestWrapper::
  ...
  ValidationErrors::ScopedField field(
      &errors_, y_absl::StrCat("balancer lookup for ", authority));
  if (orphaned_) return;
...
void EventEngineClientChannelDNSResolver::EventEngineDNSRequestWrapper::
    OnTXTResolved(y_absl::StatusOr<std::vector<TString>> service_config) {
  y_absl::optional<Resolver::Result> result;
  {
    MutexLock lock(&on_resolved_mu_);
    // Make sure field destroys before cleanup.
    ...
    if (!service_config.ok()) {
      ...
      static constexpr y_absl::string_view kServiceConfigAttributePrefix =
          "grpc_config=";
      auto result = std::find_if(service_config->begin(), service_config->end(),
                                 [&](y_absl::string_view s) {
                                   return y_absl::StartsWith(
      ...
      if (result != service_config->end()) {
        ...
        service_config_json_ = y_absl::UnavailableError(y_absl::StrCat(
...
y_absl::optional<Resolver::Result> EventEngineClientChannelDNSResolver::
    EventEngineDNSRequestWrapper::OnResolvedLocked() {
  if (orphaned_) return y_absl::nullopt;
  ...
  if (addresses_.empty() && balancer_addresses_.empty()) {
    y_absl::Status status = errors_.status(
        y_absl::StatusCode::kUnavailable,
        y_absl::StrCat("errors resolving ", resolver_->name_to_resolve()));
    if (status.ok()) {
      // If no errors were returned, but the results are empty, we still need to
      // return an error. Validation errors may be empty.
      status = y_absl::UnavailableError("No results from DNS queries");
    }
