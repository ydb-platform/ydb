...
struct ServiceConfigChoice {
  std::vector<TString> client_language;
  int percentage = -1;
  std::vector<TString> client_hostname;
...
bool vector_contains(const std::vector<TString>& v,
                     const TString& value) {
...
y_absl::StatusOr<TString> ChooseServiceConfig(
