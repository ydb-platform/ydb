...
class FakeResolverResponseGenerator
 ...
 private:
  ...
  Mutex mu_;
  CondVar* resolver_set_cv_ Y_ABSL_GUARDED_BY(mu_) = nullptr;
  ...
  y_absl::optional<Resolver::Result> result_ Y_ABSL_GUARDED_BY(mu_);

  Mutex reresolution_mu_;
  CondVar* reresolution_cv_ Y_ABSL_GUARDED_BY(reresolution_mu_) = nullptr;
  bool reresolution_requested_ Y_ABSL_GUARDED_BY(reresolution_mu_) = false;
};
