...
void RetryGlobalConfig::JsonPostLoad(const Json& json, const JsonArgs& args,
  ...
  y_absl::string_view buf = it->second.string();
  ...
  if (decimal_point != y_absl::string_view::npos) {
    ...
    // Parse decimal value.
    if (!y_absl::SimpleAtoi(after_decimal, &decimal_value)) {
  ...
  if (!y_absl::SimpleAtoi(buf, &whole_value)) {
    errors->AddError("could not parse as a number");
    return;
  }
  milli_token_ratio_ =
