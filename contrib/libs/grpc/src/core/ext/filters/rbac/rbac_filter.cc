...
ArenaPromise<ServerMetadataHandle> RbacFilter::MakeCallPromise(
  ...
  if (method_params == nullptr) {
    return Immediate(ServerMetadataFromStatus(
        y_absl::PermissionDeniedError("No RBAC policy found.")));
  } else {
    ...
    if (authorization_engine
      ...
      return Immediate(ServerMetadataFromStatus(
          y_absl::PermissionDeniedError("Unauthorized RPC rejected")));
    }
  }
...
y_absl::StatusOr<RbacFilter> RbacFilter::Create(const ChannelArgs& args,
