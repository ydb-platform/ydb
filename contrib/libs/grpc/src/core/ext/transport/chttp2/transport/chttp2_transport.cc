...
TString HttpAnnotation::ToString() const {
  TString s = "HttpAnnotation type: ";
  switch (type_) {
    case Type::kStart:
      y_absl::StrAppend(&s, "Start");
      break;
    case Type::kHeadWritten:
      y_absl::StrAppend(&s, "HeadWritten");
      break;
    case Type::kEnd:
      y_absl::StrAppend(&s, "End");
      break;
    default:
      y_absl::StrAppend(&s, "Unknown");
  }
  y_absl::StrAppend(&s, " time: ", time_.ToString());
  if (transport_stats_.has_value()) {
    y_absl::StrAppend(&s, " transport:[", transport_stats_->ToString(), "]");
  }
  if (stream_stats_.has_value()) {
    y_absl::StrAppend(&s, " stream:[", stream_stats_->ToString(), "]");
  }
  return s;
}
  ...
  if (channel_args.GetBool(GRPC_ARG_ENABLE_CHANNELZ)
    ...
    t->channelz_socket =
        grpc_core::MakeRefCounted<grpc_core::channelz::SocketNode>(
            ...
            y_absl::StrCat(t->GetTransportName(), " ",
...
static void perform_stream_op_locked(void* stream_op,
  ...
  if (s->call_tracer) {
    s->call_tracer->RecordAnnotation(y_absl::StrFormat(
...
class PingClosureWrapper {
 public:
  ...
  ~PingClosureWrapper() {
    if (closure_ != nullptr) {
      grpc_core::ExecCtx::Run(DEBUG_LOCATION, closure_, y_absl::CancelledError());
    }
  }

  void operator()() {
    grpc_core::ExecCtx::Run(DEBUG_LOCATION, Take(), y_absl::OkStatus());
  }
...
Duration TarpitDuration(grpc_chttp2_transport* t) {
  return Duration::Milliseconds(y_absl::LogUniform<int>(
      y_absl::BitGen(), t->min_tarpit_duration_ms, t->max_tarpit_duration_ms));
}
...
static void read_action_parse_loop_locked(
  ...
  if (t->closed_with_error.ok()) {
    ...
    for (size_t i = 0;
         i < t->read_buffer.count && errors[1] == y_absl::OkStatus(); i++) {
      auto r = grpc_chttp2_perform_read(t.get(), t->read_buffer.slices[i],
                                        requests_started);
      if (auto* partial_read_size = y_absl::get_if<size_t>(&r)) {
        ...
        errors[1] = std::move(y_absl::get<y_absl::Status>(r));
      }
    }
...
static void post_benign_reclaimer(grpc_chttp2_transport* t) {
  if (!t->benign_reclaimer_registered) {
    ...
    t->memory_owner.PostReclaimer(
        ...
        [t = t->Ref()](
            y_absl::optional<grpc_core::ReclamationSweep> sweep) mutable {
          if (sweep.has_value()) {
            auto* tp = t.get();
...
y_absl::string_view grpc_chttp2_transport::GetTransportName() const {
  return "chttp2";
}
...
void grpc_chttp2_transport_start_reading(
  ...
  tp->combiner->Run(
      grpc_core::NewClosure([t = std::move(t), notify_on_receive_settings,
        ...
        read_action_locked(std::move(t), y_absl::OkStatus());
      }),
      y_absl::OkStatus());
}
