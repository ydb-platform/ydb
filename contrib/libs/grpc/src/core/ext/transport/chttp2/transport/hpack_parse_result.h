...
class HpackParseResult {
 public:
  ...
  static HpackParseResult FromStatus(HpackParseStatus status) {
    ...
    switch (status) {
      ...
      default:
        Crash(
            y_absl::StrCat("Invalid HpackParseStatus for FromStatus: ", status));
    }
  }
  ...
  static HpackParseResult FromStatusWithKey(HpackParseStatus status,
    ...
    if (r.state_ != nullptr) {
      r.state_->key = TString(key);
    }
    return r;
  }

  static HpackParseResult MetadataParseError(y_absl::string_view key) {
  ...
  static HpackParseResult HardMetadataLimitExceededByValueError(
      y_absl::string_view key, uint32_t value_length, uint32_t limit) {
  ...
  // Compute the y_absl::Status that goes along with this HpackParseResult.
  // (may be cached, so this is not thread safe)
  y_absl::Status Materialize() const;
 ...
 private:
  ...
  y_absl::Status BuildMaterialized() const;
  ...
  struct HpackParseResultState : public RefCounted<HpackParseResultState> {
    ...
    TString key;
    mutable y_absl::optional<y_absl::Status> materialized_status;
  };
