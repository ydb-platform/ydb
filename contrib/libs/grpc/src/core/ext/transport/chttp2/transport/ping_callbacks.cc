...
uint64_t Chttp2PingCallbacks::StartPing(y_absl::BitGenRef bitgen) {
  uint64_t id;
  do {
    id = y_absl::Uniform<uint64_t>(bitgen);
...
y_absl::optional<uint64_t> Chttp2PingCallbacks::OnPingTimeout(
  ...
  auto it = inflight_.find(most_recent_inflight_);
  if (it == inflight_.end()) return y_absl::nullopt;
