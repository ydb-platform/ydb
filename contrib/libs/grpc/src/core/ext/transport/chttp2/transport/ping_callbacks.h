...
class Chttp2PingCallbacks {
 public:
  ...
  using Callback = y_absl::AnyInvocable<void()>;
  ...
  GRPC_MUST_USE_RESULT uint64_t StartPing(y_absl::BitGenRef bitgen);
  ...
  y_absl::optional<uint64_t> OnPingTimeout(
 ...
 private:
  ...
  y_absl::flat_hash_map<uint64_t, InflightPing> inflight_;
