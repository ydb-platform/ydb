...
constexpr y_absl::string_view kFileWatcherPlugin = "file_watcher";
...
y_absl::string_view FileWatcherCertificateProviderFactory::Config::name() const {
...
y_absl::string_view FileWatcherCertificateProviderFactory::name() const {
  return kFileWatcherPlugin;
}
...
FileWatcherCertificateProviderFactory::CreateCertificateProvider(
  ...
  if (config->name() != name()) {
    gpr_log(GPR_ERROR, "Wrong config type Actual:%s vs Expected:%s",
            TString(config->name()).c_str(), TString(name()).c_str());
    return nullptr;
  }
