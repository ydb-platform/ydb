...
TString XdsClusterResource::ToString() const {
  ...
  contents.push_back(y_absl::StrCat("lb_policy_config=",
...
XdsClusterResource::Eds EdsConfigParse(
  ...
  if (eds_cluster_config == nullptr) {
    ...
    if (eds.eds_service_name.empty()) {
      ...
      if (y_absl::StartsWith(cluster_name, "xdstp:")) {
...
y_absl::StatusOr<std::shared_ptr<const XdsClusterResource>> CdsResourceParse(
    const XdsResourceType::DecodeContext& context,
    const envoy_config_cluster_v3_Cluster* cluster) {
