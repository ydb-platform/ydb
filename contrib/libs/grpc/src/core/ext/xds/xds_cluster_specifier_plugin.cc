...
y_absl::string_view XdsRouteLookupClusterSpecifierPlugin::ConfigProtoName()
    const {
...
Json XdsRouteLookupClusterSpecifierPlugin::GenerateLoadBalancingPolicyConfig(
  ...
  y_absl::string_view* serialized_plugin_config =
  ...
  if (json_size == static_cast<size_t>(-1)) {
    errors->AddError(y_absl::StrCat("failed to dump proto to JSON: ",
...
void XdsClusterSpecifierPluginRegistry::RegisterPlugin(
  ...
  y_absl::string_view name = plugin->ConfigProtoName();
