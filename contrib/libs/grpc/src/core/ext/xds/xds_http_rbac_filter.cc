...
Json ParseAuditLoggerConfigsToJson(
  ...
  for (size_t i = 0; i < size; ++i) {
    ValidationErrors::ScopedField field(
        errors, y_absl::StrCat(".logger_configs[", i, "]"));
...
y_absl::string_view XdsHttpRbacFilter::ConfigProtoName() const {
  return "envoy.extensions.filters.http.rbac.v3.RBAC";
}

y_absl::string_view XdsHttpRbacFilter::OverrideConfigProtoName() const {
