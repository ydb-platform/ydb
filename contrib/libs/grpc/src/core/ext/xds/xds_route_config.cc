...
XdsRouteConfigResource::Route::RouteAction::HashPolicy::Header::ToString()
    const {
  return y_absl::StrCat("Header ", header_name, "/",
...
TString XdsRouteConfigResource::Route::RouteAction::HashPolicy::ToString()
  ...
  TString type = Match(
      ...
      [](const ChannelId&) -> TString { return "ChannelId"; });
  return y_absl::StrCat("{", type, ", terminal=", terminal ? "true" : "false",
                      "}");
}
...
XdsRouteConfigResource::ClusterSpecifierPluginMap ClusterSpecifierPluginParse(
  ...
  for (size_t i = 0; i < num_cluster_specifier_plugins; ++i) {
    ...
    ValidationErrors::ScopedField field(
        errors, y_absl::StrCat(".cluster_specifier_plugins[", i, "].extension"));
    ...
    if (cluster_specifier_plugin_map.find(name) !=
      ...
      errors->AddError(y_absl::StrCat("duplicate name \"", name, "\""));
    } else {
...
y_absl::optional<StringMatcher> RoutePathMatchParse(
  ...
  if (envoy_config_route_v3_RouteMatch_has_prefix(match)) {
    ...
    // Empty path.
    if (path.empty()) return y_absl::nullopt;
    // Does not start with a slash.
    if (path[0] != '/') return y_absl::nullopt;
    ...
    if (path_elements.size() != 2) return y_absl::nullopt;
    // Empty service name.
    if (path_elements[0].empty()) return y_absl::nullopt;
    // Empty method name.
    if (path_elements[1].empty()) return y_absl::nullopt;
  ...
  if (!string_matcher.ok()) {
    errors->AddError(y_absl::StrCat("error creating path matcher: ",
...
void RouteHeaderMatchersParse(const envoy_config_route_v3_RouteMatch* match,
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (!header_matcher.ok()) {
      errors->AddError(y_absl::StrCat("cannot create header matcher: ",
...
XdsRouteConfigResource::TypedPerFilterConfig ParseTypedPerFilterConfig(
  ...
  while (true) {
    ...
    ValidationErrors::ScopedField field(errors, y_absl::StrCat("[", key, "]"));
    if (key.empty()) errors->AddError("filter name must be non-empty");
    const google_protobuf_Any* any = value_func(filter_entry);
    auto extension = ExtractXdsExtension(context, any, errors);
    if (!extension.has_value()) continue;
    auto* extension_to_use = &*extension;
    y_absl::optional<XdsExtension> nested_extension;
    ...
    y_absl::optional<XdsHttpFilterImpl::FilterConfig> filter_config =
...
y_absl::optional<XdsRouteConfigResource::Route::RouteAction> RouteActionParse(
    const XdsResourceType::DecodeContext& context,
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if ((header = envoy_config_route_v3_RouteAction_HashPolicy_header(
      ...
      if (regex_rewrite != nullptr) {
        ...
        TString regex = UpbStringToStdString(
        ...
        if (!header_policy.regex->ok()) {
          errors->AddError(y_absl::StrCat("errors compiling regex: ",
  ...
  if (envoy_config_route_v3_RouteAction_has_cluster(route_action_proto)) {
    ...
    if (it == cluster_specifier_plugin_map.end()) {
      errors->AddError(y_absl::StrCat("unknown cluster specifier plugin name \"",
                                    plugin_name, "\""));
    } else {
      ...
      if (it->second.empty()) return y_absl::nullopt;
    }
    route_action.action =
...
y_absl::optional<XdsRouteConfigResource::Route> ParseRoute(
    const XdsResourceType::DecodeContext& context,
    const envoy_config_route_v3_Route* route_proto,
    const y_absl::optional<XdsRouteConfigResource::RetryPolicy>&
        virtual_host_retry_policy,
    const XdsRouteConfigResource::ClusterSpecifierPluginMap&
        cluster_specifier_plugin_map,
    std::set<y_absl::string_view>* cluster_specifier_plugins_not_seen,
    ValidationErrors* errors) {
  XdsRouteConfigResource::Route route;
  // Parse route match.
  {
    ...
    if (query_parameters_size > 0) return y_absl::nullopt;
    // Parse matchers.
    auto path_matcher = RoutePathMatchParse(match, errors);
    if (!path_matcher.has_value()) return y_absl::nullopt;
  ...
  if (route_action_proto != nullptr) {
    ...
    if (!route_action.has_value()) return y_absl::nullopt;
    ...
    auto* cluster_specifier_action = y_absl::get_if<
        XdsRouteConfigResource::Route::RouteAction::ClusterSpecifierPluginName>(
        &route_action->action);
...
std::shared_ptr<const XdsRouteConfigResource> XdsRouteConfigResource::Parse(
  ...
  for (size_t i = 0; i < num_virtual_hosts; ++i) {
    ValidationErrors::ScopedField field(
        errors, y_absl::StrCat(".virtual_hosts[", i, "]"));
    ...
    for (size_t j = 0; j < num_routes; ++j) {
      ValidationErrors::ScopedField field(errors, y_absl::StrCat("[", j, "]"));
...
XdsResourceType::DecodeResult XdsRouteConfigResourceType::Decode(
  ...
  if (!errors.ok()) {
    y_absl::Status status =
        errors.status(y_absl::StatusCode::kInvalidArgument,
