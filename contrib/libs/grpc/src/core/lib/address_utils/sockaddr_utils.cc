...
#ifdef GRPC_HAVE_VSOCK
static y_absl::StatusOr<TString> grpc_sockaddr_to_uri_vsock_if_possible(
    const grpc_resolved_address* resolved_addr) {
  const grpc_sockaddr* addr =
      reinterpret_cast<const grpc_sockaddr*>(resolved_addr->addr);
  ...
  return y_absl::StrCat("vsock:", vsock_addr->svm_cid, ":", vsock_addr->svm_port);
}
#else
static y_absl::StatusOr<TString> grpc_sockaddr_to_uri_vsock_if_possible(
    const grpc_resolved_address* /* addr */) {
...
y_absl::StatusOr<TString> grpc_sockaddr_to_string(
  ...
  if (addr->sa_family == GRPC_AF_VSOCK) {
    ...
    out = y_absl::StrCat(addr_vm->svm_cid, ":", addr_vm->svm_port);
    return out;
  }
#endif
...
y_absl::StatusOr<TString> grpc_sockaddr_to_uri(
  ...
  if (scheme == nullptr) {
    return y_absl::InvalidArgumentError("Unknown address type");
  }
