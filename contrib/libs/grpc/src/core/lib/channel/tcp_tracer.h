...
class TcpTracerInterface {
 public:
  ...
  struct ConnectionMetrics {
    ...
    y_absl::optional<uint32_t> time_to_ack_usec;
    // Last socket error code. Only populated for CLOSED timestamps.
    y_absl::optional<uint32_t> socket_errno;
    // Peer's receive window after scaling (tcpi_snd_wnd).
    // Only available with SENDMSG timestamps.
    y_absl::optional<uint32_t> peer_rwnd;
    // Receive queue drops.
    y_absl::optional<uint32_t> rcvq_drops;
    // The NIC Rx delay reported by the remote host.
    y_absl::optional<uint32_t> nic_rx_delay_usec;
  };

  virtual ~TcpTracerInterface() = default;
  virtual void RecordEvent(Type type, y_absl::Time time, size_t byte_offset,
                           y_absl::optional<ConnectionMetrics> metrics) = 0;
};
