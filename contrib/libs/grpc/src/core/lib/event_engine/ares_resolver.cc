...
y_absl::Status AresStatusToAbslStatus(int status, y_absl::string_view error_msg) {
  switch (status) {
    case ARES_ECANCELLED:
      return y_absl::CancelledError(error_msg);
    case ARES_ENOTIMP:
      return y_absl::UnimplementedError(error_msg);
    case ARES_ENOTFOUND:
      return y_absl::NotFoundError(error_msg);
    default:
      return y_absl::UnknownError(error_msg);
  }
}
...
y_absl::Status SetRequestDNSServer(y_absl::string_view dns_server,
...
struct QueryArg {
  QueryArg(AresResolver* ar, int id, y_absl::string_view name)
...
struct HostnameQueryArg : public QueryArg {
  HostnameQueryArg(AresResolver* ar, int id, y_absl::string_view name, int p)
...
y_absl::StatusOr<grpc_core::OrphanablePtr<AresResolver>>
AresResolver::CreateAresResolver(
  ...
  if (status != ARES_SUCCESS) {
    ...
    return AresStatusToAbslStatus(
        status,
        y_absl::StrCat("Failed to init c-ares channel: ", ares_strerror(status)));
  }
  event_engine_grpc_ares_test_only_inject_config(&channel);
  polled_fd_factory->ConfigureAresChannelLocked(channel);
  if (!dns_server.empty()) {
    y_absl::Status status = SetRequestDNSServer(dns_server, &channel);
    if (!status.ok()) {
      return status;
    }
  }
    ...
    for (const auto& fd_node : fd_node_list_) {
      if (!fd_node->already_shutdown) {
        ...
        GPR_ASSERT(fd_node->polled_fd->ShutdownLocked(
            y_absl::CancelledError("AresResolver::Orphan")));
...
void AresResolver::LookupHostname(
  ...
  if (port_string.empty()) {
    if (default_port.empty()) {
      event_engine_->Run([callback = std::move(callback),
                          status = y_absl::InvalidArgumentError(y_absl::StrFormat(
  ...
  if (port_string == "http") {
    ...
    port = 443;
  } else if (!y_absl::SimpleAtoi(port_string, &port)) {
    event_engine_->Run([callback = std::move(callback),
                        status = y_absl::InvalidArgumentError(y_absl::StrCat(
  ...
  grpc_resolved_address addr;
  const TString hostport = grpc_core::JoinHostPort(host, port);
  ...
  if (IsIpv6LoopbackAvailable()) {
    ares_gethostbyname(channel_, TString(host).c_str(), AF_UNSPEC,
                       &AresResolver::OnHostbynameDoneLocked, resolver_arg);
  } else {
    ares_gethostbyname(channel_, TString(host).c_str(), AF_INET,
                       &AresResolver::OnHostbynameDoneLocked, resolver_arg);
  }
  CheckSocketsLocked();
...
void AresResolver::LookupSRV(
  ...
  ares_query(channel_, TString(host).c_str(), ns_c_in, ns_t_srv,
...
void AresResolver::LookupTXT(
  ...
  callback_map_.emplace(++id_, std::move(callback));
  auto* resolver_arg = new QueryArg(this, id_, host);
  ares_search(channel_, TString(host).c_str(), ns_c_in, ns_t_txt,
  ...
  while (!fd_node_list_.empty()) {
    ...
    if (!fd_node->already_shutdown) {
      ...
      fd_node->already_shutdown =
          fd_node->polled_fd->ShutdownLocked(y_absl::OkStatus());
    }
...
void AresResolver::OnReadable(FdNode* fd_node, y_absl::Status status) {
  grpc_core::MutexLock lock(&mutex_);
  ...
  if (status.ok() && !shutting_down_) {
    ...
    // If error is not y_absl::OkStatus() or the resolution was cancelled, it
...
void AresResolver::OnWritable(FdNode* fd_node, y_absl::Status status) {
  grpc_core::MutexLock lock(&mutex_);
...
void AresResolver::OnHostbynameDoneLocked(void* arg, int status,
  ...
  GPR_ASSERT(!nh.empty());
  GPR_ASSERT(
      y_absl::holds_alternative<EventEngine::DNSResolver::LookupHostnameCallback>(
          nh.mapped()));
  auto callback = y_absl::get<EventEngine::DNSResolver::LookupHostnameCallback>(
      std::move(nh.mapped()));
  if (status != ARES_SUCCESS) {
    TString error_msg =
        y_absl::StrFormat("address lookup failed for %s: %s",
...
void AresResolver::OnSRVQueryDoneLocked(void* arg, int status, int /*timeouts*/,
  ...
  GPR_ASSERT(!nh.empty());
  GPR_ASSERT(
      y_absl::holds_alternative<EventEngine::DNSResolver::LookupSRVCallback>(
          nh.mapped()));
  auto callback = y_absl::get<EventEngine::DNSResolver::LookupSRVCallback>(
      std::move(nh.mapped()));
  auto fail = [&](y_absl::string_view prefix) {
...
void AresResolver::OnTXTDoneLocked(void* arg, int status, int /*timeouts*/,
  ...
  GPR_ASSERT(!nh.empty());
  GPR_ASSERT(
      y_absl::holds_alternative<EventEngine::DNSResolver::LookupTXTCallback>(
          nh.mapped()));
  auto callback = y_absl::get<EventEngine::DNSResolver::LookupTXTCallback>(
      std::move(nh.mapped()));
  auto fail = [&](y_absl::string_view prefix) {
    TString error_message = y_absl::StrFormat(
        "%s for %s: %s", prefix, qa->query_name, ares_strerror(status));
  ...
  for (struct ares_txt_ext* part = reply; part != nullptr; part = part->next) {
    if (part->record_start) {
      ...
      y_absl::StrAppend(
          &result.back(),
          TString(reinterpret_cast<char*>(part->txt), part->length));
    }
  }
