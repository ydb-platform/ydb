  ...
  auto deadline_timer =
      RunAfter(timeout, [handle, that = std::static_pointer_cast<CFEventEngine>(
        ...
        that->CancelConnectInternal(
            handle, y_absl::DeadlineExceededError("Connect timed out"));
      });

  auto on_connect2 =
      [that = std::static_pointer_cast<CFEventEngine>(shared_from_this()),
       deadline_timer, handle,
       on_connect = std::move(on_connect)](y_absl::Status status) mutable {
...
bool CFEventEngine::CancelConnect(ConnectionHandle handle) {
  CancelConnectInternal(handle, y_absl::CancelledError("CancelConnect"));
...
void CFEventEngine::Run(y_absl::AnyInvocable<void()> closure) {
