...
y_absl::Status CFErrorToStatus(CFTypeUniqueRef<CFErrorRef> cf_error) {
  if (cf_error == nullptr) {
  ...
  return y_absl::Status(y_absl::StatusCode::kUnknown,
                      y_absl::StrFormat("(domain:%s, code:%ld, description:%s)",
                                      domain_buf, code, desc_buf));
}

y_absl::StatusOr<EventEngine::ResolvedAddress> CFReadStreamLocallAddress(
...
bool CFStreamEndpointImpl::CancelConnect(y_absl::Status status) {
...
void CFStreamEndpointImpl::Connect(
    y_absl::AnyInvocable<void(y_absl::Status)> on_connect,
    EventEngine::ResolvedAddress addr) {
...
void CFStreamEndpointImpl::Shutdown() {
  ...
  auto shutdownStatus =
      y_absl::Status(y_absl::StatusCode::kUnknown,
                   y_absl::StrFormat("Shutting down CFStreamEndpointImpl"));
...
bool CFStreamEndpointImpl::Read(
    y_absl::AnyInvocable<void(y_absl::Status)> on_read, SliceBuffer* buffer,
  ...
  read_event_.NotifyOn(new PosixEngineClosure(
      [that = Ref(), on_read = std::move(on_read),
       buffer](y_absl::Status status) mutable {
        if (status.ok()) {
...
bool CFStreamEndpointImpl::Write(
  ...
  write_event_.NotifyOn(new PosixEngineClosure(
      [that = Ref(), on_writable = std::move(on_writable),
       data](y_absl::Status status) mutable {
        if (status.ok()) {
