...
class GrpcPolledFd {
 public:
  ...
  virtual void RegisterForOnReadableLocked(
      y_absl::AnyInvocable<void(y_absl::Status)> read_closure) = 0;
  // Called when c-ares library is interested and there's no pending callback
  virtual void RegisterForOnWriteableLocked(
      y_absl::AnyInvocable<void(y_absl::Status)> write_closure) = 0;
  ...
  GRPC_MUST_USE_RESULT virtual bool ShutdownLocked(y_absl::Status error) = 0;
