...
class Epoll1EventHandle : public EventHandle {
 ...
 private:
  void HandleShutdownInternal(y_absl::Status why, bool releasing_fd);
...
void Epoll1EventHandle::OrphanHandle(PosixEngineClosure* on_done,
  ...
  if (!read_closure_->IsShutdown()) {
    was_shutdown = true;
    HandleShutdownInternal(y_absl::Status(y_absl::StatusCode::kUnknown, reason),
                           is_release_fd);
  }
  ...
  if (on_done != nullptr) {
    on_done->SetStatus(y_absl::OkStatus());
...
void Epoll1EventHandle::HandleShutdownInternal(y_absl::Status why,
                                               bool releasing_fd) {
...
EventHandle* Epoll1Poller::CreateHandle(int fd, y_absl::string_view /*name*/,
                                        bool track_err) {
...
void Epoll1EventHandle::ShutdownHandle(y_absl::Status why) {
