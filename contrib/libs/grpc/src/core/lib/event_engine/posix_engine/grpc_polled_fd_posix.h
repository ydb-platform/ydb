...
class GrpcPolledFdPosix : public GrpcPolledFd {
 public:
  GrpcPolledFdPosix(ares_socket_t as, EventHandle* handle)
      : name_(y_absl::StrCat("c-ares fd: ", static_cast<int>(as))),
        as_(as),
        handle_(handle) {}
  ...
  void RegisterForOnReadableLocked(
      y_absl::AnyInvocable<void(y_absl::Status)> read_closure) override {
    handle_->NotifyOnRead(new PosixEngineClosure(std::move(read_closure),
                                                 /*is_permanent=*/false));
  }

  void RegisterForOnWriteableLocked(
      y_absl::AnyInvocable<void(y_absl::Status)> write_closure) override {
  ...
  bool ShutdownLocked(y_absl::Status error) override {
