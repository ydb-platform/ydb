...
class TcpZerocopySendCtx {
 ...
 private:
  ...
  TcpZerocopySendRecord* send_records_ Y_ABSL_GUARDED_BY(mu_);
  TcpZerocopySendRecord** free_send_records_ Y_ABSL_GUARDED_BY(mu_);
  int max_sends_;
  int free_send_records_size_ Y_ABSL_GUARDED_BY(mu_);
  grpc_core::Mutex mu_;
  uint32_t last_send_ = 0;
  std::atomic<bool> shutdown_{false};
  bool enabled_ = false;
  size_t threshold_bytes_ = kDefaultSendBytesThreshold;
  y_absl::flat_hash_map<uint32_t, TcpZerocopySendRecord*> ctx_lookup_
      Y_ABSL_GUARDED_BY(mu_);
  bool memory_limited_ = false;
  bool is_in_write_ Y_ABSL_GUARDED_BY(mu_) = false;
  OptMemState zcopy_enobuf_state_ Y_ABSL_GUARDED_BY(mu_) = OptMemState::kOpen;
};
...
class PosixEndpointImpl : public grpc_core::RefCounted<PosixEndpointImpl> {
 ...
 private:
  ...
  void MaybePostReclaimer() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(read_mu_);
  void PerformReclamation() Y_ABSL_LOCKS_EXCLUDED(read_mu_);
  ...
  bool DoFlushZerocopy(TcpZerocopySendRecord* record, y_absl::Status& status);
  bool TcpFlushZerocopy(TcpZerocopySendRecord* record, y_absl::Status& status);
  bool TcpFlush(y_absl::Status& status);
  ...
  y_absl::Status TcpAnnotateError(y_absl::Status src_error);
  ...
  bool has_posted_reclaimer_ Y_ABSL_GUARDED_BY(read_mu_) = false;
  ...
  grpc_event_engine::experimental::SliceBuffer* incoming_buffer_
      Y_ABSL_GUARDED_BY(read_mu_) = nullptr;
  ...
  y_absl::AnyInvocable<void(y_absl::Status)> read_cb_ Y_ABSL_GUARDED_BY(read_mu_);
  y_absl::AnyInvocable<void(y_absl::Status)> write_cb_;
  ...
  y_absl::AnyInvocable<void(y_absl::StatusOr<int>)> on_release_fd_ = nullptr;
 ...
 public:
  ...
  void Shutdown(y_absl::AnyInvocable<void(y_absl::StatusOr<int> release_fd)>
    ...
    if (!shutdown_.exchange(true, std::memory_order_acq_rel)) {
      impl_->MaybeShutdown(y_absl::FailedPreconditionError("Endpoint closing"),
