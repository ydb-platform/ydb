...
class PosixEngineListenerImpl
 ...
 public:
  ...
  y_absl::Status HandleExternalConnection(int listener_fd, int fd,
 ...
 private:
    ...
    AsyncConnectionAcceptor(std::shared_ptr<EventEngine> engine,
        ...
        : engine_(std::move(engine)),
          ...
          notify_on_accept_(PosixEngineClosure::ToPermanentClosure(
              [this](y_absl::Status status) { NotifyOnAccept(status); })){};
    // Start listening for incoming connections on the socket.
    void Start();
    // Internal callback invoked when the socket has incoming connections to
    // process.
    void NotifyOnAccept(y_absl::Status status);
  ...
  // operation.
  ListenerAsyncAcceptors acceptors_ Y_ABSL_GUARDED_BY(mu_);
...
class PosixEngineListener : public PosixListenerWithFdSupport {
 public:
  ...
  y_absl::StatusOr<int> Bind(const grpc_event_engine::experimental::EventEngine::
