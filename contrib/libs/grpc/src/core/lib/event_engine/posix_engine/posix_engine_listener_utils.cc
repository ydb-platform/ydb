...
y_absl::StatusOr<int> GetUnusedPort() {
  ...
  if (bind(sock->Fd(), wild.address(), wild.size()) != 0) {
    ...
    return y_absl::FailedPreconditionError(
        y_absl::StrCat("bind(GetUnusedPort): ", std::strerror(errno)));
  }
  ...
  if (getsockname(sock->Fd(), const_cast<sockaddr*>(wild.address()), &len) !=
    ...
    close(sock->Fd());
    return y_absl::FailedPreconditionError(
        y_absl::StrCat("getsockname(GetUnusedPort): ", std::strerror(errno)));
  }
  close(sock->Fd());
  int port = ResolvedAddressGetPort(wild);
  if (port <= 0) {
    return y_absl::FailedPreconditionError("Bad port");
  }
  return port;
}
...
y_absl::Status PrepareSocket(const PosixTcpOptions& options,
  ...
  socket.port = 0;
  auto sock_cleanup = y_absl::MakeCleanup([&close_fd, fd]() -> void {
    if (close_fd && fd >= 0) {
      close(fd);
    }
  });
  ...
  if (listen(fd, GetMaxAcceptQueueSize()) < 0) {
    return y_absl::FailedPreconditionError(
        y_absl::StrCat("Error in listen: ", std::strerror(errno)));
  }
  ...
  if (getsockname(fd, const_cast<sockaddr*>(sockname_temp.address()), &len) <
      0) {
    return y_absl::FailedPreconditionError(
        y_absl::StrCat("Error in getsockname: ", std::strerror(errno)));
  }
...
y_absl::StatusOr<ListenerSocket> CreateAndPrepareListenerSocket(
...
y_absl::StatusOr<int> ListenerContainerAddAllLocalAddresses(
    ListenerSocketsContainer& listener_sockets, const PosixTcpOptions& options,
    int requested_port) {
#ifdef GRPC_HAVE_IFADDRS
  y_absl::Status op_status = y_absl::OkStatus();
  ...
  if (getifaddrs(&ifa) != 0 || ifa == nullptr) {
    return y_absl::FailedPreconditionError(
        y_absl::StrCat("getifaddrs: ", std::strerror(errno)));
  }
  for (ifa_it = ifa; ifa_it != nullptr; ifa_it = ifa_it->ifa_next) {
    ...
    TString addr_str = *ResolvedAddressToString(addr);
    gpr_log(GPR_DEBUG,
    ...
    if (!result.ok()) {
      op_status = y_absl::FailedPreconditionError(
  ...
  if (no_local_addresses) {
    return y_absl::FailedPreconditionError("No local addresses");
  }
...
y_absl::StatusOr<int> ListenerContainerAddWildcardAddresses(
    ListenerSocketsContainer& listener_sockets, const PosixTcpOptions& options,
    int requested_port) {
  ...
  y_absl::StatusOr<ListenerSocket> v6_sock;
  y_absl::StatusOr<ListenerSocket> v4_sock;
  int assigned_port = 0;
...
y_absl::StatusOr<ListenerSocketsContainer::ListenerSocket>
