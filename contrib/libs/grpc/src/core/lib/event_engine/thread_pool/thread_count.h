...
class LivingThreadCount {
 public:
  ...
  void Increment() Y_ABSL_LOCKS_EXCLUDED(mu_) {
    grpc_core::MutexLock lock(&mu_);
    ++living_count_;
    cv_.SignalAll();
  }
  void Decrement() Y_ABSL_LOCKS_EXCLUDED(mu_) {
    grpc_core::MutexLock lock(&mu_);
    --living_count_;
    cv_.SignalAll();
  }
  ...
  size_t count() Y_ABSL_LOCKS_EXCLUDED(mu_) {
    grpc_core::MutexLock lock(&mu_);
    return CountLocked();
  }

 private:
  size_t CountLocked() Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
  ...
  size_t living_count_ Y_ABSL_GUARDED_BY(mu_) = 0;
};
