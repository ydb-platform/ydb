...
void WorkStealingThreadPool::Run(y_absl::AnyInvocable<void()> callback) {
...
void WorkStealingThreadPool::ThreadState::SleepIfRunning() {
  ...
  y_absl::SleepFor(
      y_absl::Milliseconds(kTimeBetweenThrottledThreadStarts.millis()));
}
...
bool WorkStealingThreadPool::WorkSignal::WaitWithTimeout(
  ...
  grpc_core::MutexLock lock(&mu_);
  return cv_.WaitWithTimeout(&mu_, y_absl::Milliseconds(time.millis()));
}
