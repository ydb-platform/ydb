...
class WorkStealingThreadPool final : public ThreadPool {
 ...
 private:
  ...
  class WorkSignal {
   ...
   private:
    grpc_core::Mutex mu_;
    grpc_core::CondVar cv_ Y_ABSL_GUARDED_BY(mu_);
  };
  ...
  class TheftRegistry {
   public:
    ...
    void Enroll(WorkQueue* queue) Y_ABSL_LOCKS_EXCLUDED(mu_);
    // Disallow work stealing from the provided queue.
    void Unenroll(WorkQueue* queue) Y_ABSL_LOCKS_EXCLUDED(mu_);
    // Returns one closure from another thread, or nullptr if none are
    // available.
    EventEngine::Closure* StealOne() Y_ABSL_LOCKS_EXCLUDED(mu_);

   private:
    grpc_core::Mutex mu_;
    y_absl::flat_hash_set<WorkQueue*> queues_ Y_ABSL_GUARDED_BY(mu_);
  };
