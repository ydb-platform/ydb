...
class GrpcPolledFdWindows : public GrpcPolledFd {
 public:
  GrpcPolledFdWindows(std::unique_ptr<WinSocket> winsocket,
      ...
      : name_(y_absl::StrFormat("c-ares socket: %" PRIdPTR,
  ...
  int Connect(WSAErrorContext* wsa_error_ctx, const struct sockaddr* target,
    ...
    switch (socket_type_) {
      ...
      default:
        grpc_core::Crash(
            y_absl::StrFormat("Unknown socket_type_: %d", socket_type_));
    }
  }
 ...
 private:
  ...
  void ContinueRegisterForOnWriteableLocked() {
    ...
    switch (tcp_write_state_) {
      ...
      case WRITE_WAITING_FOR_VERIFICATION_UPON_RETRY:
        grpc_core::Crash(
            y_absl::StrFormat("Invalid tcp_write_state_: %d", tcp_write_state_));
    }
  }
  ...
  ares_ssize_t SendVTCP(WSAErrorContext* wsa_error_ctx, const struct iovec* iov,
    ...
    grpc_core::Crash(
        y_absl::StrFormat("Unknown tcp_write_state_: %d", tcp_write_state_));
  }
  ...
  y_absl::AnyInvocable<void(y_absl::Status)> read_closure_;
  y_absl::AnyInvocable<void(y_absl::Status)> write_closure_;
...
class CustomSockFuncs {
 public:
  static ares_socket_t Socket(int af, int type, int protocol, void* user_data) {
    ...
    if (type == SOCK_STREAM) {
      y_absl::Status error = PrepareSocket(s);
      if (!error.ok()) {
