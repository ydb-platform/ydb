...
class WindowsEventEngine : public EventEngine,
 ...
 private:
  ...
  struct ConnectionState {
    ...
    std::unique_ptr<WinSocket> socket Y_ABSL_GUARDED_BY(mu);
    EventEngine::ResolvedAddress address Y_ABSL_GUARDED_BY(mu);
    MemoryAllocator allocator Y_ABSL_GUARDED_BY(mu);
  };
  ...
  TaskHandleSet known_handles_ Y_ABSL_GUARDED_BY(task_mu_);
  grpc_core::Mutex connection_mu_ Y_ABSL_ACQUIRED_AFTER(ConnectionState::mu);
