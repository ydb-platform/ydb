...
class BasicWorkQueue : public WorkQueue {
 public:
  ...
  bool Empty() const override Y_ABSL_LOCKS_EXCLUDED(mu_);
  // Returns the size of the queue.
  size_t Size() const override Y_ABSL_LOCKS_EXCLUDED(mu_);
  ...
  EventEngine::Closure* PopMostRecent() override Y_ABSL_LOCKS_EXCLUDED(mu_);
  // Returns the most recent element from the queue, or nullptr if either empty
  ...
  //
  // This method may return nullptr even if the queue is not empty.
  EventEngine::Closure* PopOldest() override Y_ABSL_LOCKS_EXCLUDED(mu_);
  // Adds a closure to the queue.
  void Add(EventEngine::Closure* closure) override Y_ABSL_LOCKS_EXCLUDED(mu_);
  // Wraps an AnyInvocable and adds it to the the queue.
  void Add(y_absl::AnyInvocable<void()> invocable) override
      Y_ABSL_LOCKS_EXCLUDED(mu_);
  const void* owner() override { return owner_; }

 private:
  mutable grpc_core::Mutex mu_;
  std::deque<EventEngine::Closure*> q_ Y_ABSL_GUARDED_BY(mu_);
