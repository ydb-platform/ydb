...
class DirectoryReaderImpl : public DirectoryReader {
 public:
  explicit DirectoryReaderImpl(y_absl::string_view directory_path)
      : directory_path_(directory_path) {}
  y_absl::string_view Name() const override { return directory_path_; }
  y_absl::Status ForEach(y_absl::FunctionRef<void(y_absl::string_view)>) override;

 private:
  const TString directory_path_;
};
...
y_absl::Status DirectoryReaderImpl::ForEach(
    y_absl::FunctionRef<void(y_absl::string_view)> callback) {
  TString search_path = y_absl::StrCat(directory_path_, "/*");
  WIN32_FIND_DATAA find_data;
  HANDLE hFind = ::FindFirstFileA(search_path.c_str(), &find_data);
  if (hFind == INVALID_HANDLE_VALUE) {
    return y_absl::InternalError("Could not read crl directory.");
  }
  do {
