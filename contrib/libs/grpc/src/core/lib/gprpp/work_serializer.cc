...
class WorkSerializer::DispatchingWorkSerializer final
 ...
 private:
  ...
  using CallbackVector = y_absl::InlinedVector<CallbackWrapper, 1>;
  ...
  // instance is deleted.
  bool running_ Y_ABSL_GUARDED_BY(mu_) = false;
  bool orphaned_ Y_ABSL_GUARDED_BY(mu_) = false;
  Mutex mu_;
  ...
  CallbackVector incoming_ Y_ABSL_GUARDED_BY(mu_);
