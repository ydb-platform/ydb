    ...
    if (serr->ee_data >= elem->seq_no_) {
      switch (serr->ee_info) {
        ...
        case SCM_TSTAMP_ACK:
          ...
          // on the lifetime.
          g_timestamps_callback(elem->arg_, &(elem->ts_), y_absl::OkStatus());
...
void TracedBufferList::Shutdown(void* remaining, y_absl::Status shutdown_err) {
  MutexLock lock(&mu_);
  while (head_) {
    TracedBuffer* elem = head_;
