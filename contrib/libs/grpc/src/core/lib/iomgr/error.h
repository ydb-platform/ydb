...
// them. If the vector is empty, return y_absl::OkStatus().
...
bool grpc_error_get_str(grpc_error_handle error,
                        grpc_core::StatusStrProperty which, TString* str);
...
///
/// Edge Conditions -
/// 1) If either of \a src or \a child is y_absl::OkStatus(), returns a reference
/// to the other argument. 2) If both \a src and \a child are y_absl::OkStatus(),
/// returns y_absl::OkStatus(). 3) If \a src and \a child point to the same error,
