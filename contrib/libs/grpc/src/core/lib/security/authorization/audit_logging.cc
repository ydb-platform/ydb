...
AuditLoggerRegistry::AuditLoggerRegistry() {
  ...
  y_absl::string_view name = factory->name();
...
bool AuditLoggerRegistry::FactoryExists(y_absl::string_view name) {
  MutexLock lock(mu);
...
AuditLoggerRegistry::ParseConfig(y_absl::string_view name, const Json& json) {
  MutexLock lock(mu);
  auto it = registry->logger_factories_map_.find(name);
  if (it == registry->logger_factories_map_.end()) {
    return y_absl::NotFoundError(
        y_absl::StrFormat("audit logger factory for %s does not exist", name));
  }
