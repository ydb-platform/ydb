...
class AuditLoggerRegistry {
 public:
  ...
  static bool FactoryExists(y_absl::string_view name);

  static y_absl::StatusOr<std::unique_ptr<AuditLoggerFactory::Config>>
  ParseConfig(y_absl::string_view name, const Json& json);
 ...
 private:
  ...
  static AuditLoggerRegistry* registry Y_ABSL_GUARDED_BY(mu);

  // The key is owned by the factory.
  std::map<y_absl::string_view, std::unique_ptr<AuditLoggerFactory>>
      logger_factories_map_ Y_ABSL_GUARDED_BY(mu);
};
