...
constexpr y_absl::string_view kName = "stdout_logger";
...
void StdoutAuditLogger::Log(const AuditContext& context) {
  y_absl::FPrintF(stdout, kLogFormat, y_absl::FormatTime(y_absl::Now()),
                context.rpc_method(), context.principal(),
                context.policy_name(), context.matched_rule(),
                context.authorized() ? "true" : "false");
}

y_absl::string_view StdoutAuditLoggerFactory::Config::name() const {
  return kName;
}

TString StdoutAuditLoggerFactory::Config::ToString() const { return "{}"; }

y_absl::string_view StdoutAuditLoggerFactory::name() const { return kName; }

y_absl::StatusOr<std::unique_ptr<AuditLoggerFactory::Config>>
