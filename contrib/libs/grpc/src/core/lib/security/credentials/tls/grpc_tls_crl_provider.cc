...
namespace {
TString IssuerFromCrl(X509_CRL* crl) {
  if (crl == nullptr) {
    return "";
  }
...
y_absl::StatusOr<std::shared_ptr<Crl>> ReadCrlFromFile(
    const TString& crl_path) {
  y_absl::StatusOr<Slice> crl_slice = LoadFile(crl_path, false);
  if (!crl_slice.ok()) {
    return crl_slice.status();
  }
  y_absl::StatusOr<std::unique_ptr<Crl>> crl =
...
y_absl::StatusOr<std::unique_ptr<Crl>> Crl::Parse(y_absl::string_view crl_string) {
  if (crl_string.size() >= INT_MAX) {
    return y_absl::InvalidArgumentError("crl_string cannot be of size INT_MAX");
  }
  BIO* crl_bio =
...
y_absl::StatusOr<std::unique_ptr<CrlImpl>> CrlImpl::Create(X509_CRL* crl) {
  TString issuer = IssuerFromCrl(crl);
  if (issuer.empty()) {
    return y_absl::InvalidArgumentError("Issuer of crl cannot be empty");
  }
...
y_absl::StatusOr<std::shared_ptr<CrlProvider>> CreateStaticCrlProvider(
    y_absl::Span<const TString> crls) {
  y_absl::flat_hash_map<TString, std::shared_ptr<Crl>> crl_map;
  for (const auto& raw_crl : crls) {
    y_absl::StatusOr<std::unique_ptr<Crl>> crl = Crl::Parse(raw_crl);
    if (!crl.ok()) {
...
y_absl::StatusOr<std::shared_ptr<CrlProvider>> CreateDirectoryReloaderCrlProvider(
    y_absl::string_view directory, std::chrono::seconds refresh_duration,
    std::function<void(y_absl::Status)> reload_error_callback) {
  if (refresh_duration < std::chrono::seconds(60)) {
    return y_absl::InvalidArgumentError("Refresh duration minimum is 60 seconds");
  }
...
y_absl::Status DirectoryReloaderCrlProvider::Update() {
  y_absl::flat_hash_map<TString, std::shared_ptr<Crl>> new_crls;
  std::vector<TString> files_with_errors;
  y_absl::Status status = crl_directory_->ForEach([&](y_absl::string_view file) {
    TString file_path = y_absl::StrCat(crl_directory_->Name(), "/", file);
    // Build a map of new_crls to update to. If all files successful, do a
    // full swap of the map. Otherwise update in place.
    y_absl::StatusOr<std::shared_ptr<Crl>> crl = ReadCrlFromFile(file_path);
    if (!crl.ok()) {
      files_with_errors.push_back(
          y_absl::StrCat(file_path, ": ", crl.status().ToString()));
      return;
    }
  ...
  if (!files_with_errors.empty()) {
    ...
    return y_absl::UnknownError(y_absl::StrCat(
        "Errors reading the following files in the CRL directory: [",
        y_absl::StrJoin(files_with_errors, "; "), "]"));
  } else {
