...
struct MethodConfig {
  struct Name {
    y_absl::optional<TString> service;
    y_absl::optional<TString> method;
...
RefCountedPtr<ServiceConfig> ServiceConfigImpl::Create(
    const ChannelArgs& args, const Json& json, y_absl::string_view json_string,
    ValidationErrors* errors) {
  ...
  service_config->json_string_ = TString(json_string);
  ...
  if (method_configs.has_value()) {
    ...
    for (size_t i = 0; i < method_configs->size(); ++i) {
      ...
      ValidationErrors::ScopedField field(
          errors, y_absl::StrCat(".methodConfig[", i, "]"));
      ...
      for (size_t j = 0; j < method_config.names.size(); ++j) {
        ValidationErrors::ScopedField field(errors,
                                            y_absl::StrCat(".name[", j, "]"));
        TString path = method_config.names[j].Path();
        if (path.empty()) {
          ...
          if (value != nullptr) {
            errors->AddError(y_absl::StrCat("multiple method configs for path ",
