...
void Call::PublishToParent(Call* parent) {
  ...
  if (parent->Completed()) {
    CancelWithError(y_absl::CancelledError());
  }
}
    ...
    if (child != nullptr) {
      do {
        ...
        if (child->cancellation_is_inherited_) {
          ...
          child->CancelWithError(y_absl::CancelledError());
...
class FilterStackCall final : public Call {
 ...
 private:
  ...
  static TString PendingOpString(intptr_t pending_ops) {
    std::vector<y_absl::string_view> pending_op_strings;
    ...
    return y_absl::StrCat("{", y_absl::StrJoin(pending_op_strings, ","), "}");
  }
  struct BatchControl {
...
void FilterStackCall::DestroyCall(void* call, grpc_error_handle /*error*/) {
  ...
  c->status_error_.set(y_absl::OkStatus());
...
void FilterStackCall::ExecuteBatch(grpc_transport_stream_op_batch* batch,
  ...
  GRPC_CALL_COMBINER_START(call_combiner(), start_batch_closure,
                           y_absl::OkStatus(), "executing batch");
}
...
void FilterStackCall::RecvTrailingFilter(grpc_metadata_batch* b,
  ...
  if (!batch_error.ok()) {
    ...
    if (grpc_status.has_value()) {
      ...
      SetFinalStatus(y_absl::OkStatus());
    } else {
      gpr_log(GPR_DEBUG,
...
void FilterStackCall::BatchControl::PostCompletion() {
  ...
  batch_error_.set(y_absl::OkStatus());
...
class PromiseBasedCall : public Call,
 ...
 protected:
  ...
  TString PresentAndCompletionText(const char* caption, bool has,
                                       const Completion& completion) const {
    if (has) {
      if (completion.has_value()) {
        return y_absl::StrCat(caption, ":", CompletionString(completion), " ");
      } else {
        return y_absl::StrCat(caption,
      ...
      if (!completion.has_value()) {
        ...
        return y_absl::StrCat(caption, ":no-op:", CompletionString(completion),
                            " ");
      }
    }
  }
...
void PromiseBasedCall::Run() {
  ApplicationCallbackExecCtx callback_exec_ctx;
  ExecCtx exec_ctx;
  CancelWithError(y_absl::DeadlineExceededError("Deadline exceeded"));
  InternalUnref("deadline");
}
      ...
      [this, cancel_on_error,
       completion = AddOpToCompletion(completion, PendingOp::kReceiveMessage)](
        ...
        if (result.has_value()) {
          ...
          if (cancel_on_error) CancelWithError(y_absl::CancelledError());
          *recv_message_ = nullptr;
        } else {
          if (grpc_call_trace.enabled()) {
            gpr_log(GPR_INFO,
...
void CallContext::RunInContext(y_absl::AnyInvocable<void()> fn) {
...
class ClientPromiseBasedCall final : public PromiseBasedCall {
 public:
  ...
  TString DebugTag() const override {
    return y_absl::StrFormat("CLIENT_CALL[%p]: ", this);
  }
...
namespace {
TString MakeErrorString(const ServerMetadata* trailing_metadata) {
  TString out = y_absl::StrCat(
  ...
  if (auto annotations = trailing_metadata->get_pointer(GrpcStatusContext())) {
    y_absl::StrAppend(&out, "\nStatus Context:");
    for (const TString& annotation : *annotations) {
      y_absl::StrAppend(&out, "\n  ", annotation);
    }
  }
  return out;
...
ServerCallContext::MakeTopOfServerCallPromise(
    ...
    y_absl::FunctionRef<void(grpc_call*)>) {
  (void)call_;
...
grpc_call_error grpc_call_cancel(grpc_call* call, void* reserved) {
  ...
  grpc_core::ExecCtx exec_ctx;
  grpc_core::Call::FromC(call)->CancelWithError(y_absl::CancelledError());
  return GRPC_CALL_OK;
}
