...
class Server::RequestMatcherInterface {
 public:
  ...
  virtual ArenaPromise<y_absl::StatusOr<MatchResult>> MatchRequest(
...
void Server::MaybeFinishShutdown() {
  ...
  for (auto& shutdown_tag : shutdown_tags_) {
    Ref().release();
    grpc_cq_end_op(shutdown_tag.cq, shutdown_tag.tag, y_absl::OkStatus(),
...
void Server::ShutdownAndNotify(grpc_completion_queue* cq, void* tag) {
  ChannelBroadcaster broadcaster;
  {
    ...
    if (shutdown_published_) {
      grpc_cq_end_op(cq, tag, y_absl::OkStatus(), DonePublishedShutdown, nullptr,
  ...
  if (!path.has_value()) {
    return [] {
      return ServerMetadataFromStatus(
          y_absl::InternalError("Missing :path header"));
    };
  }
  auto host_ptr =
        ...
        maybe_read_first_message =
            Map(call_args.client_to_server_messages->Next(),
                [](NextResult<MessageHandle> msg)
                    -> y_absl::StatusOr<NextResult<MessageHandle>> {
                  return std::move(msg);
                });
    }
  } else {
        ...
        return GetContext<CallContext>()
            ...
            ->MakeTopOfServerCallPromise(
                ...
                [rc, cq_for_new_request](grpc_call* call) {
                  *rc->call = call;
                  grpc_cq_end_op(cq_for_new_request, rc->tag, y_absl::OkStatus(),
...
void Server::CallData::Publish(size_t cq_idx, RequestedCall* rc) {
  ...
  *rc->call = call_;
  ...
  grpc_cq_end_op(cq_new_, rc->tag, y_absl::OkStatus(), Server::DoneRequestEvent,
...
void Server::CallData::KillZombie() {
  GRPC_CLOSURE_INIT(&kill_zombie_closure_, KillZombieClosure, call_,
                    grpc_schedule_on_exec_ctx);
  ExecCtx::Run(DEBUG_LOCATION, &kill_zombie_closure_, y_absl::OkStatus());
}
...
void Server::CallData::StartNewRpc(grpc_call_element* elem) {
  ...
  switch (payload_handling) {
    case GRPC_SRM_PAYLOAD_NONE:
      PublishNewRpc(elem, y_absl::OkStatus());
      break;
    case GRPC_SRM_PAYLOAD_READ_INITIAL_BYTE_BUFFER: {
      grpc_op op;
...
void Server::CallData::RecvInitialMetadataReady(void* arg,
  ...
  if (calld->host_.has_value() && calld->path_.has_value()) {
    ...
    error = y_absl::UnknownError("Missing :authority or :path");
