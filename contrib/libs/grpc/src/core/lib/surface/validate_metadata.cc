...
ValidateMetadataResult ConformsTo(y_absl::string_view x,
...
y_absl::Status UpgradeToStatus(ValidateMetadataResult result) {
  if (result == ValidateMetadataResult::kOk) return y_absl::OkStatus();
  return y_absl::InternalError(ValidateMetadataResultToString(result));
}
