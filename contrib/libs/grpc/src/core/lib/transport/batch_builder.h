...
class BatchBuilder {
 ...
 private:
  ...
  struct Batch final {
    ...
    TString DebugPrefix(Activity* activity = Activity::current()) const {
      return y_absl::StrFormat("%s[connected] [batch %p] ", activity->DebugTag(),
                             this);
    }
  ...
  static ServerMetadataHandle CompleteSendServerTrailingMetadata(
      ...
      y_absl::Status send_result, bool actually_sent);
...
inline auto BatchBuilder::SendServerTrailingMetadata(
  ...
  auto promise = Map(pc->done_latch.WaitAndCopy(),
                     [pc, batch = batch->Ref()](y_absl::Status status) {
