...
struct GrpcTimeoutMetadata {
  ...
  static TString DisplayValue(ValueType x) { return x.ToString(); }
  static TString DisplayMemento(MementoType x) { return x.ToString(); }
};
...
struct GrpcAcceptEncodingMetadata {
  ...
  static y_absl::string_view DisplayValue(ValueType x) { return x.ToString(); }
  static y_absl::string_view DisplayMemento(MementoType x) {
...
struct XEnvoyPeerMetadata : public SimpleSliceBasedMetadata {
  static constexpr bool kRepeatable = false;
  using CompressionTraits = StableValueCompressor;
  static y_absl::string_view key() { return "x-envoy-peer-metadata"; }
};
...
struct LbCostBinMetadata {
  ...
  static TString DisplayMemento(MementoType x) { return DisplayValue(x); }
  static MementoType ParseMemento(Slice value,
                                  bool will_keep_past_request_lifetime,
                                  MetadataParseErrorFn on_error);
};
...
struct GrpcStreamNetworkState {
  static y_absl::string_view DebugKey() { return "GrpcStreamNetworkState"; }
  static constexpr bool kRepeatable = false;
  enum ValueType : uint8_t {
    kNotSentOnWire,
    kNotSeenByServer,
...
struct GrpcStatusFromWire {
  static y_absl::string_view DebugKey() { return "GrpcStatusFromWire"; }
  static constexpr bool kRepeatable = false;
...
struct GrpcTrailersOnly {
  static y_absl::string_view DebugKey() { return "GrpcTrailersOnly"; }
  static constexpr bool kRepeatable = false;
  using ValueType = bool;
  static y_absl::string_view DisplayValue(bool x) { return x ? "true" : "false"; }
};
...
struct GrpcRegisteredMethod {
  static y_absl::string_view DebugKey() { return "GrpcRegisteredMethod"; }
  static constexpr bool kRepeatable = false;
  using ValueType = void*;
  static TString DisplayValue(void* x);
};
...
struct GrpcTarPit {
  static y_absl::string_view DebugKey() { return "GrpcTarPit"; }
  static constexpr bool kRepeatable = false;
  using ValueType = Empty;
  static y_absl::string_view DisplayValue(Empty) { return "tarpit"; }
};
...
struct IsEncodableTrait<Trait, y_absl::void_t<decltype(Trait::key())>> {
...
struct EncodableTraits<y_absl::enable_if_t<IsEncodableTrait<Trait>::value, void>,
                       Trait, Traits...> {
  using List =
...
struct EncodableNameLookupKeyComparison {
  bool operator()(y_absl::string_view key) { return key == Trait::key(); }
};
...
using LogFn = y_absl::FunctionRef<void(y_absl::string_view, y_absl::string_view)>;
...
template <>
struct AdaptDisplayValueToLog<y_absl::string_view> {
  static TString ToString(y_absl::string_view value) {
...
struct AdaptDisplayValueToLog<Slice> {
  static TString ToString(Slice value) {
    return TString(value.as_string_view());
  }
};

template <>
struct AdaptDisplayValueToLog<const char*> {
  static TString ToString(const char* value) { return TString(value); }
};

template <>
struct AdaptDisplayValueToLog<StaticSlice> {
  static y_absl::string_view ToString(StaticSlice value) {
    return value.as_string_view();
  }
};

template <typename T, typename U, typename V>
GPR_ATTRIBUTE_NOINLINE void LogKeyValueTo(y_absl::string_view key, const T& value,
...
struct Value<Which, y_absl::enable_if_t<Which::kRepeatable == false &&
...
template <typename Which>
struct Value<Which, y_absl::enable_if_t<Which::kRepeatable == true &&
                                          IsEncodableTrait<Which>::value,
                                      void>> {
  Value() = default;
...
//   static y_absl::string_view DebugKey() { return "GRPC_XYZ"; }
//   // Can this metadata field be repeated?
//   static constexpr bool kRepeatable = ...;
//   // The type that's stored on MetadataBatch
//   using ValueType = ...;
