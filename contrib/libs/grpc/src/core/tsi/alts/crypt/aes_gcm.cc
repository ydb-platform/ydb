...
GsecKeyFactory::GsecKeyFactory(y_absl::Span<const uint8_t> key, bool is_rekey)
...
GsecKey::GsecKey(y_absl::Span<const uint8_t> key, bool is_rekey)
    : is_rekey_(is_rekey) {
  if (is_rekey_) {
...
y_absl::Span<const uint8_t> GsecKey::key() { return key_; }

y_absl::Span<const uint8_t> GsecKey::nonce_mask() { return nonce_mask_; }

y_absl::Span<uint8_t> GsecKey::kdf_counter() {
  return y_absl::MakeSpan(kdf_counter_);
}

y_absl::Span<uint8_t> GsecKey::aead_key() { return y_absl::MakeSpan(aead_key_); }

y_absl::Span<uint8_t> GsecKey::kdf_buffer() {
  return y_absl::MakeSpan(kdf_buffer_);
}
...
static grpc_status_code aes_gcm_derive_aead_key(
    y_absl::Span<uint8_t> dst, uint8_t* buf, y_absl::Span<const uint8_t> kdf_key,
    y_absl::Span<const uint8_t> kdf_counter) {
  unsigned char ctr = 1;
