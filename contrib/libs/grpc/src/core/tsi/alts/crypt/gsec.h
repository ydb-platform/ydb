...
class GsecKeyInterface {
 public:
  ...
  virtual y_absl::Span<const uint8_t> key() = 0;

  // All the accessors below should only be called when IsRekey() is true.
  virtual y_absl::Span<uint8_t> aead_key() = 0;
  virtual y_absl::Span<const uint8_t> nonce_mask() = 0;
  virtual y_absl::Span<uint8_t> kdf_counter() = 0;
  virtual y_absl::Span<uint8_t> kdf_buffer() = 0;
};
...
class GsecKeyFactory : public GsecKeyFactoryInterface {
 public:
  ...
  GsecKeyFactory(y_absl::Span<const uint8_t> key, bool is_rekey);
...
class GsecKey : public GsecKeyInterface {
 public:
  ...
  GsecKey(y_absl::Span<const uint8_t> key, bool is_rekey);
  ~GsecKey() override = default;

  bool IsRekey() override;
  y_absl::Span<const uint8_t> key() override;
  y_absl::Span<uint8_t> aead_key() override;
  y_absl::Span<const uint8_t> nonce_mask() override;
  y_absl::Span<uint8_t> kdf_counter() override;
  y_absl::Span<uint8_t> kdf_buffer() override;
