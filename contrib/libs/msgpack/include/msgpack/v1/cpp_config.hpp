...
struct unique_ptr : std::auto_ptr<T> {
    explicit unique_ptr(T* p = 0) noexcept : std::auto_ptr<T>(p) {}
    unique_ptr(unique_ptr& a) noexcept : std::auto_ptr<T>(a) {}
    template<class Y>
    unique_ptr (unique_ptr<Y>& a) noexcept : std::auto_ptr<T>(a) {}
};
