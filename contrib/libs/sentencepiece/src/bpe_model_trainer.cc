...
TString Trainer::Symbol::ToString() const {
...
util::Status Trainer::Train() {
  ...
  if (pretokenizer || !trainer_spec_.pretokenization_delimiter().empty()) {
    ...
    for (auto &w : sentences_) {
      if (pretokenizer) {
        w.first = y_absl::StrJoin(pretokenizer->PreTokenize(w.first),
                                TrainerInterface::kUPPBoundaryStr);
      } else if (!delimiter.empty()) {
        w.first = y_absl::StrReplaceAll(
  ...
  y_absl::flat_hash_set<TString> dup;
