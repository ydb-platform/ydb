...
#error #include <unicode/errorcode.h>
#error #include <unicode/locid.h>
#error #include <unicode/normlzr.h>
#error #include <unicode/numfmt.h>
#error #include <unicode/rbnf.h>
...
static constexpr std::string_view kDefaultNormalizerName = "nfkc";

#ifndef ENABLE_NFKC_COMPILE
static constexpr std::string_view kCompileError =
...
Builder::Chars UnicodeNormalize(UNormalizationMode mode,
  ...
  const TString utf8 = string_util::UnicodeTextToUTF8(input);
  CHECK(!utf8.empty());
...
bool IsValidNormalizerData(std::string_view blob_data) {
  NormalizerSpec spec;
...
util::Status Builder::CompileCharsMap(const CharsMap &chars_map,
  ...
  for (auto &p : normalized2pos) {
    ...
    const TString utf8_out = string_util::UnicodeTextToUTF8(p.first);
    CHECK_OR_RETURN(string_util::IsStructurallyValid(utf8_out));
    normalized += utf8_out;
    normalized += '\0';
  }

  std::vector<std::pair<TString, int>> kv;  // key-value of Trie.
  for (const auto &p : chars_map) {
    // The value of Trie stores the pointer to the normalized string.
    const TString utf8_in = string_util::UnicodeTextToUTF8(p.first);
...
// static
util::Status Builder::DecompileCharsMap(std::string_view blob,
...
// static
util::Status Builder::GetPrecompiledCharsMap(std::string_view name,
                                             TString *output) {
  CHECK_OR_RETURN(output);
...
#else   // DISABLE_EMBEDDED_DATA
  {
    const TString filename =
        y_absl::StrCat(util::JoinPath(GetDataDir(), name), ".bin");
...
// static
util::Status Builder::LoadCharsMap(std::string_view filename,
                                   CharsMap *chars_map) {
