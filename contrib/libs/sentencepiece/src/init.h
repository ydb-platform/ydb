...
#else
#error #include "third_party/protobuf-lite/google/protobuf/message_lite.h"
#endif

Y_ABSL_DECLARE_FLAG(int32_t, minloglevel);

namespace sentencepiece {
inline void ParseCommandLineFlags(const char *usage, int *argc, char ***argv,
                                  bool remove_arg = true) {
  y_absl::SetProgramUsageMessage(*argv[0]);
  const auto unused_args = y_absl::ParseCommandLine(*argc, *argv);
  ...
  logging::SetMinLogLevel(y_absl::GetFlag(FLAGS_minloglevel));
}
