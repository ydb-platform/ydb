...
std::string_view ModelInterface::unk_piece() const {
  RETURN_PIECE(unk_piece, "<unk>");
}

std::string_view ModelInterface::bos_piece() const {
  RETURN_PIECE(bos_piece, "<s>");
}

std::string_view ModelInterface::eos_piece() const {
  RETURN_PIECE(eos_piece, "</s>");
}

std::string_view ModelInterface::pad_piece() const {
  RETURN_PIECE(pad_piece, "<pad>");
}

#undef RETURN_PIECE

int ModelInterface::PieceToId(std::string_view piece) const {
...
void ModelInterface::InitializePieces() {
  ...
  std::set<std::string_view> user_defined_symbols;
...
std::vector<std::string_view> SplitIntoWords(std::string_view text,
  ...
  if (treat_ws_as_suffix) {  // put ws tokens at the end of non-ws sequences.
    ...
    while (begin < end) {
      ...
      const bool is_ws = std::string_view(begin, mblen) == kSpaceSymbol;
      ...
      result.back() =
          std::string_view(result.back().data(), result.back().size() + mblen);
      begin += mblen;
    ...
    while (begin < end) {
      const int mblen =
          std::min<int>(string_util::OneCharLen(begin), end - begin);
      bool is_ws = std::string_view(begin, mblen) == kSpaceSymbol;
...
TString ByteToPiece(unsigned char c) {
  return y_absl::StrFormat("<0x%02X>", c);
}

int PieceToByte(std::string_view piece) {
  using PieceToByteMap = y_absl::flat_hash_map<TString, unsigned char>;
  ...
  const auto it = kMap->find(TString(piece));
  if (it == kMap->end()) {
    return -1;
  } else {
    return it->second;
