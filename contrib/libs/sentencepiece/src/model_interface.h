...
std::vector<std::string_view> SplitIntoWords(
    std::string_view text, bool treat_ws_as_suffix = false,
    bool allow_ws_only_pieces = false);

// Converts byte (0-255) to piece (e.g., 58 -> "<0x3A>").
TString ByteToPiece(unsigned char c);
...
class ModelInterface {
 public:
  using PieceToIdMap = y_absl::flat_hash_map<std::string_view, int>;
  //                                           string_util::string_view_hash>;

  std::string_view unk_piece() const;
  std::string_view bos_piece() const;
  std::string_view eos_piece() const;
  std::string_view pad_piece() const;
  ...
  virtual EncodeResult Encode(std::string_view normalized) const = 0;

  // The same as above, but returns nbest result with score.
  virtual NBestEncodeResult NBestEncode(std::string_view normalized,
                                        int nbest_size) const {
    LOG(ERROR) << "Not implemented.";
    return NBestEncodeResult();
  }

  virtual EncodeResult SampleEncode(std::string_view normalized,
                                    float alpha) const {
    LOG(ERROR) << "Not implemented.";
    return EncodeResult();
  }
  ...
  virtual NBestEncodeResult SampleEncodeAndScore(std::string_view normalized,
  ...
  virtual float CalculateEntropy(std::string_view normalized,
                                 float alpha) const {
    LOG(ERROR) << "Not implemented.";
    return 0.0;
  }
  ...
  virtual const TString &IdToPiece(int id) const {
  ...
  virtual bool VerifyOutputsEquivalent(std::string_view expected,
