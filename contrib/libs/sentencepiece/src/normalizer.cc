...
util::Status Normalizer::Normalize(std::string_view input,
                                   TString *normalized,
  ...
  while (!input.empty()) {
    ...
    while (is_prev_space && y_absl::ConsumePrefix(&sp, " ")) {
    }
    ...
    if (!sp.empty()) {
      ...
      is_prev_space = y_absl::EndsWith(sp, " ");
    }
  ...
  if (spec_->remove_extra_whitespaces()) {
    ...
    while (y_absl::EndsWith(*normalized, space)) {
...
TString Normalizer::Normalize(std::string_view input) const {
...
// static
TString Normalizer::EncodePrecompiledCharsMap(
...
util::Status Normalizer::DecodePrecompiledCharsMap(
    ...
    std::string_view *normalized, TString *buffer) {
  ...
  *trie_blob = std::string_view(buffer->data(), trie_blob_size);
#else
...
TString PrefixMatcher::GlobalReplace(std::string_view w,
