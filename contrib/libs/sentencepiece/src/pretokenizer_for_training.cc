...
std::vector<TString> PretokenizerForTrainingInterface::PreTokenize(
    std::string_view text) const {
  return Postprocess(Tokenize(Preprocess(text)));
}

// static
TString PretokenizerForTrainingInterface::Preprocess(
    std::string_view text) {
  // Escapes kWSStr (_) as this character may not be processed by pre-tokenizer.
  return y_absl::StrReplaceAll(text, {{kWSStr, " "}});
}

// static
std::vector<TString> PretokenizerForTrainingInterface::Postprocess(
  ...
  for (auto &w : result) w = y_absl::StrReplaceAll(w, {{" ", kWSStr}});
