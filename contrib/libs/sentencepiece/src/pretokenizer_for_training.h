...
class PretokenizerForTrainingInterface {
 public:
  ...
  std::vector<TString> PreTokenize(std::string_view text) const;
  ...
  // the input text.
  virtual SentencePieceText Tokenize(std::string_view text) const = 0;

 private:
  static TString Preprocess(std::string_view text);
  static std::vector<TString> Postprocess(const SentencePieceText &spt);
};
