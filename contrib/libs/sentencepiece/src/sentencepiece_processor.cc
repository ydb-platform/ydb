...
std::vector<std::string_view> ToPieceArray(const std::vector<TString> &v) {
  std::vector<std::string_view> out(v.size());
...
const TString &ImmutableSentencePieceText_ImmutableSentencePiece::piece()
    const {
  return sp_->piece();
}

const TString &ImmutableSentencePieceText_ImmutableSentencePiece::surface()
    const {
  return sp_->surface();
}
...
const TString &ImmutableSentencePieceText::text() const {
  return spt_->text();
}
...
util::Status SentencePieceProcessor::Load(std::string_view filename) {
...
void SentencePieceProcessor::LoadOrDie(std::string_view filename) {
...
util::Status SentencePieceProcessor::SetVocabulary(
  ...
  const std::set<std::string_view> vocab(valid_vocab.begin(),
                                          valid_vocab.end());
...
util::Status SentencePieceProcessor::LoadVocabulary(std::string_view filename,
                                                    int threshold) {
  auto input = filesystem::NewReadableFile(filename);
  RETURN_IF_ERROR(input->status());

  TString line;
  std::vector<TString> vocab;

  while (input->ReadLine(&line)) {
    const std::vector<TString> v = y_absl::StrSplit(line, '\t');
    CHECK_GE_OR_RETURN(v.size(), 1);
    CHECK_OR_RETURN(!v[0].empty());
    int32_t freq = 1;
    if (v.size() >= 2) {
      CHECK_OR_RETURN(y_absl::SimpleAtoi(v[1], &freq))
...
util::Status SentencePieceProcessor::Encode(
    std::string_view input, std::vector<TString> *pieces) const {
...
util::Status SentencePieceProcessor::Encode(std::string_view input,
...
util::Status SentencePieceProcessor::NBestEncode(
    ...
    std::vector<std::vector<TString>> *pieces) const {
  CHECK_OR_RETURN_STATUS_STL(pieces);
  ...
  for (const auto &nbest : spt.nbests()) {
    std::vector<TString> &result = pieces->emplace_back();
...
util::Status SentencePieceProcessor::SampleEncode(
    std::string_view input, int nbest_size, float alpha,
    std::vector<TString> *pieces) const {
  CHECK_OR_RETURN_STATUS_STL(pieces);
...
util::Status SentencePieceProcessor::SampleEncode(std::string_view input,
...
util::Status SentencePieceProcessor::SampleEncodeAndScore(
    ...
    bool include_best,
    std::vector<std::pair<std::vector<TString>, float>> *pieces) const {
  CHECK_OR_RETURN_STATUS_STL(pieces);
...
util::Status SentencePieceProcessor::PopulateSentencePieceText(
  ...
  for (const auto &p : result) {
    ...
    if (IsControl(id)) {
      ...
      const auto surface =
          y_absl::ClippedSubstr(input, orig_begin, orig_end - orig_begin);
      ...
      if (is_unk && model_->ByteFallbackEnabled()) {
        ...
        if (is_prev_unk && is_unk) {
          ...
          sp->set_piece(sp->piece() + TString(w));
          sp->set_surface(sp->surface() + TString(surface));
          sp->set_end(orig_end);
        } else {
          auto *sp = spt->add_pieces();
          sp->set_piece(w.data(), w.size());
...
util::Status SentencePieceProcessor::SampleEncodeAndScore(
    std::string_view input, int samples, float alpha, bool wor,
...
util::Status SentencePieceProcessor::CalculateEntropy(std::string_view input,
                                                      float alpha,
                                                      float *entropy) const {
...
util::Status SentencePieceProcessor::Decode(
    const std::vector<TString> &pieces, SentencePieceText *spt) const {
...
util::Status SentencePieceProcessor::Decode(
  ...
  auto DecodeSentencePiece =
      [&](std::string_view piece, int id,
          bool is_bos_ws) -> std::pair<TString, bool> {
    if (IsControl(id)) {                 // <s>, </s>
      ...
      if (IdToPiece(id) == piece) {  // <unk>
        ...
        return std::make_pair(TString(piece), false);
      }
    }
    ...
    if (is_bos_ws &&
      ...
      has_bos_ws = y_absl::ConsumePrefix(&piece, kSpaceSymbol);
    ...
    return std::make_pair(y_absl::StrReplaceAll(piece, {{kSpaceSymbol, " "}}),
                          has_bos_ws);
  };

  spt->mutable_pieces()->Reserve(pieces.size());
  for (std::string_view w : pieces) {
    auto *sp = spt->add_pieces();
  ...
  TString *text = spt->mutable_text();
  auto SetSurface = [&](int index, std::string_view surface) {
    auto *sp = spt->mutable_pieces(index);
    sp->set_surface(TString(surface));
    sp->set_begin(text->size());
    sp->set_end(text->size() + surface.size());
    y_absl::StrAppend(text, surface);
  };
  ...
  auto ProcessBytePieces = [&](int token_index_begin,
    ...
    while (offset < bytes_len) {
      ...
      const bool is_valid = string_util::IsValidDecodeUTF8(
          std::string_view(bytes).substr(offset), &consumed);
...
util::Status SentencePieceProcessor::Normalize(std::string_view input,
...
util::Status SentencePieceProcessor::Normalize(
    std::string_view input, TString *normalized,
    std::vector<size_t> *norm_to_orig) const {
  CHECK_OR_RETURN(normalizer_);
  return normalizer_->Normalize(input, normalized, norm_to_orig);
}

TString SentencePieceProcessor::Normalize(std::string_view input) const {
  TString normalized;
...
const TString &SentencePieceProcessor::IdToPiece(int id) const {
  static const TString *kEmptyString = new TString;
...
int SentencePieceProcessor::unk_id() const {
  const int id = PieceToId(std::string_view(model_->unk_piece().data()));
  if (IsUnknown(id)) return id;
  return -1;
}

int SentencePieceProcessor::bos_id() const {
  const int id = PieceToId(std::string_view(model_->bos_piece().data()));
  if (IsControl(id)) return id;
  return -1;
}

int SentencePieceProcessor::eos_id() const {
  const int id = PieceToId(std::string_view(model_->eos_piece().data()));
  if (IsControl(id)) return id;
  return -1;
}

int SentencePieceProcessor::pad_id() const {
  const int id = PieceToId(std::string_view(model_->pad_piece().data()));
  if (IsControl(id)) return id;
  return -1;
}
  ...
  for (const auto &extra_option : extra_options) {
    switch (extra_option) {
      ...
      case EOS: {
        ...
        piece->set_id(PieceToId(std::string_view(model_->eos_piece().data())));
      ...
      case BOS: {
        ...
        piece->set_id(PieceToId(std::string_view(model_->bos_piece().data())));
...
util::Status SentencePieceProcessor::ParseExtraOptions(
    std::string_view _extra_option,
    std::vector<SentencePieceProcessor::ExtraOption> *extra_options) const {
  std::string_view extra_option(_extra_option.data(), _extra_option.size());
  ...
  static std::map<std::string_view, SentencePieceProcessor::ExtraOption>
  ...
  for (const auto &s : y_absl::StrSplit(extra_option, ':')) {
    ...
    if (it->second == SentencePieceProcessor::BOS) {
      CHECK_OR_RETURN(
          !IsUnknown(PieceToId(std::string_view(model_->bos_piece().data()))))
          << "id for `" << model_->bos_piece() << "` is not defined.";
    }
    if (it->second == SentencePieceProcessor::EOS) {
      CHECK_OR_RETURN(
          !IsUnknown(PieceToId(std::string_view(model_->eos_piece().data()))))
...
TString SentencePieceProcessor::serialized_model_proto() const {
...
namespace io {
util::Status LoadModelProto(std::string_view filename,
  ...
  if (!input->ReadAll(&serialized)) {
    return util::InternalError(y_absl::StrCat("could not read ", filename));
  }
  if (!model_proto->ParseFromArray(serialized.data(), serialized.size())) {
    return util::InternalError(
        y_absl::StrCat("could not parse ModelProto from ", filename));
  }

  return util::OkStatus();
}

util::Status SaveModelProto(std::string_view filename,
