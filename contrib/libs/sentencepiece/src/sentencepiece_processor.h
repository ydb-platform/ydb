...
namespace util {
// Redefine TString for serialized_proto interface as Python's string is
...
class ImmutableSentencePieceText_ImmutableSentencePiece {
 public:
  ...
  const TString &piece() const;
  const TString &surface() const;
  uint32_t id() const;
...
class SentencePieceProcessor {
 public:
  ...
  virtual util::Status Decode(const std::vector<TString> &pieces,
...
#define DEFINE_SPP_IMMUTABLE_PROTO_IMPL(FuncName, OutType, ...)      \
  ...
  virtual std::vector<TString> EncodeAsPieces(
      std::string_view input) const {
    DEFINE_SPP_DIRECT_FUNC_IMPL(Encode, std::vector<TString>, input);
  }
  ...
  virtual std::vector<std::vector<TString>> NBestEncodeAsPieces(
      std::string_view input, int nbest_size) const {
    DEFINE_SPP_DIRECT_FUNC_IMPL(
        NBestEncode, std::vector<std::vector<TString>>, input, nbest_size);
  }
  ...
  virtual std::vector<TString> SampleEncodeAsPieces(std::string_view input,
                                                        int nbest_size,
                                                        float alpha) const {
    DEFINE_SPP_DIRECT_FUNC_IMPL(SampleEncode, std::vector<TString>, input,
                                nbest_size, alpha);
  }
  ...
  // DEPRECATED: Remove this API and use std::vector<std::string_view>
  virtual TString DecodePieces(
      const std::vector<TString> &pieces) const {
    DEFINE_SPP_DIRECT_FUNC_IMPL(Decode, TString, pieces);
  }
  ...
  virtual TString DecodeIds(const std::vector<int> &ids) const {
    DEFINE_SPP_DIRECT_FUNC_IMPL(Decode, TString, ids);
  }
...
#undef DEFINE_SPP_IMMUTABLE_PROTO_IMPL
  ...
  // returns immutable model proto as TString.
...
//
//  auto model_proto = y_absl::make_unique<ModelProto>();
