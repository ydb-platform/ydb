...
util::Status SentencePieceTrainer::Train(const TrainerSpec &trainer_spec,
                                         ...
                                         TString *serialized_model_proto) {
...
util::Status SentencePieceTrainer::Train(
    ...
    SentenceIterator *sentence_iterator, TString *serialized_model_proto) {
  ...
  TString info =
      y_absl::StrCat(PrintProto(trainer_spec, "trainer_spec"),
...
// static
NormalizerSpec SentencePieceTrainer::GetNormalizerSpec(std::string_view name) {
  NormalizerSpec spec;
...
util::Status SentencePieceTrainer::MergeSpecsFromArgs(
  ...
  std::unordered_map<TString, TString> kwargs;
  for (auto arg : y_absl::StrSplit(args, " ")) {
    ...
    if (pos == std::string_view::npos) {
      ...
      key = TString(arg.substr(0, pos));
      value = TString(arg.substr(pos + 1));
    }
...
util::Status SentencePieceTrainer::MergeSpecsFromArgs(
  ...
  for (const auto &it : kwargs) {
    ...
    if (key == "normalization_rule_name") {
      ...
      int v = 0;
      CHECK_OR_RETURN(y_absl::SimpleAtoi(value, &v));
...
// static
util::Status SentencePieceTrainer::Train(std::string_view args,
                                         SentenceIterator *sentence_iterator,
                                         TString *serialized_model_proto) {
...
class VectorSentenceIterator : public SentenceIterator {
 public:
  explicit VectorSentenceIterator(const std::vector<TString> &values)
      : iter_(values.begin()), end_(values.end()) {}
  virtual ~VectorSentenceIterator() {}
  virtual bool done() const { return iter_ == end_; }
  void Next() override { ++iter_; }
  const TString &value() const override { return *iter_; }
  util::Status status() const override { return util::OkStatus(); }

 private:
  std::vector<TString>::const_iterator iter_;
  std::vector<TString>::const_iterator end_;
};
}  // namespace

// static
util::Status SentencePieceTrainer::Train(
    std::string_view args, const std::vector<TString> &sentences,
    TString *serialized_model_proto) {
...
util::Status SentencePieceTrainer::PopulateModelTypeFromString(
    std::string_view type, TrainerSpec *spec) {
  static const std::unordered_map<TString, TrainerSpec::ModelType>
      kModelTypeMap = {{"unigram", TrainerSpec::UNIGRAM},
                       {"bpe", TrainerSpec::BPE},
                       {"word", TrainerSpec::WORD},
                       {"char", TrainerSpec::CHAR}};
  const auto it = kModelTypeMap.find(y_absl::AsciiStrToLower(type));
...
util::Status SentencePieceNormalizer::Load(std::string_view filename) {
...
util::Status SentencePieceNormalizer::LoadFromRuleTSV(
  ...
  spec->set_normalization_rule_tsv(TString(filename));
  RETURN_IF_ERROR(SentencePieceTrainer::PopulateNormalizerSpec(spec));
  return Load(std::move(model_proto));
}

util::Status SentencePieceNormalizer::LoadFromRuleName(std::string_view name) {
  auto model_proto = std::make_unique<ModelProto>();
  auto *spec = model_proto->mutable_normalizer_spec();
  spec->set_name(TString(name));
  RETURN_IF_ERROR(SentencePieceTrainer::PopulateNormalizerSpec(spec));
  return Load(std::move(model_proto));
}

util::Status SentencePieceNormalizer::Normalize(std::string_view input,
...
TString SentencePieceNormalizer::Normalize(std::string_view input) const {
  TString normalized;
...
TString SentencePieceNormalizer::serialized_model_proto() const {
...
void ConvertToUnicodeAlignment(std::string_view orig, std::string_view norm,
                               std::vector<size_t> *norm_to_orig) {
  auto utf8_to_unicode_offsets = [](std::string_view str) {
