...
class SentencePieceTrainer {
 public:
  ...
  static util::Status Train(const TrainerSpec &trainer_spec,
                            ...
                            TString *serialized_model_proto = nullptr);
  ...
  // When `sentence_iterator` is passed, load sentences from the iterator.
  static util::Status Train(std::string_view args,
                            SentenceIterator *sentence_iterator = nullptr,
                            TString *serialized_model_proto = nullptr);
  ...
  static util::Status Train(
      const std::unordered_map<TString, TString> &kwargs,
      SentenceIterator *sentence_iterator = nullptr,
      TString *serialized_model_proto = nullptr);

  // The same as above, but passes the list of sentences.
  static util::Status Train(std::string_view args,
                            const std::vector<TString> &sentences,
                            TString *serialized_model_proto = nullptr);
  ...
  static util::Status MergeSpecsFromArgs(std::string_view args,
  ...
  static util::Status SetProtoField(std::string_view name,
  ...
  static util::Status PopulateModelTypeFromString(std::string_view type,
                                                  TrainerSpec *trainer_spec);
...
class SentencePieceNormalizer {
 public:
  ...
  virtual util::Status LoadFromRuleTSV(std::string_view filename);

  virtual util::Status LoadFromRuleName(std::string_view name);

  virtual util::Status Normalize(std::string_view input,
  ...
  virtual TString Normalize(std::string_view input) const;

  virtual NormalizerSpec *mutable_normalizer_spec() const;

  virtual TString serialized_model_proto() const;
