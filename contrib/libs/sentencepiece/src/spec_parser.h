...
#define PARSE_STRING(param_name)                   \
  if (name == #param_name) {                       \
    message->set_##param_name(TString(value)); \
    return util::OkStatus();                       \
  }

#define PARSE_REPEATED_STRING(param_name)                       \
  if (name == #param_name) {                                    \
    for (const TString &val : util::StrSplitAsCSV(value)) { \
...
#define PARSE_ENUM(param_name, map_name)                                      \
  if (name == #param_name) {                                                  \
    const auto it = map_name.find(y_absl::AsciiStrToUpper(value));              \
    if (it == map_name.end())                                                 \
      return util::StatusBuilder(util::StatusCode::kInvalidArgument, GTL_LOC) \
...
inline TString PrintProto(const TrainerSpec &message,
  ...
  static const std::map<TrainerSpec::ModelType, TString> kModelType_Map = {
...
inline TString PrintProto(const NormalizerSpec &message,
                              std::string_view name) {
  std::ostringstream os;
...
util::Status SentencePieceTrainer::SetProtoField(std::string_view name,
  ...
  static const std::map<TString, TrainerSpec::ModelType> kModelType_Map = {
