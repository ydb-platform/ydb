...
class SentenceSelector {
 public:
  ...
  bool Add(const std::pair<TString, int64_t> &sentence) {
    if (spec_->input_sentence_size() == 0) {
...
util::Status TrainerInterface::LoadSentences() {
  ...
  random::ReservoirSampler<TString> test_sentence_sampler(
  ...
  if (sentence_iterator_ == nullptr) {
    ...
    sentence_iterator_impl =
        std::make_unique<MultiFileSentenceIterator>(std::vector<TString>(
  ...
  for (; !sentence_iterator_->done(); sentence_iterator_->Next()) {
    int64_t freq = 1;
    TString sentence = sentence_iterator_->value();

    if (is_tsv) {
      const std::vector<TString> v = y_absl::StrSplit(sentence, '\t');
    ...
    if (sentence.find(kUNKStr) != TString::npos) {
...
END:
  ...
  // Normalize and removes empty string.
  {
    ...
    std::set<std::string_view> meta_pieces_set;
    ...
    for (size_t i = 0; i < sentences_.size(); ++i) {
      auto *s = &sentences_[i].first;
      CHECK_OR_RETURN(s->find(" ") == TString::npos)
  ...
  y_absl::flat_hash_map<char32, std::pair<bool, int64_t>> chars_count;
  for (const char32 c :
  ...
  for (const auto &w : sentences_) {
    for (const char32 c : string_util::UTF8ToUnicodeText(w.first)) {
      ...
      if (c == 0x0020) {
        ...
        CHECK_OR_RETURN(w.first.find(" ") == TString::npos)
...
void TrainerInterface::SplitSentencesByWhitespace() {
  LOG(INFO) << "Tokenizing input sentences with whitespace: "
            << sentences_.size();
  y_absl::flat_hash_map<TString, int64_t> tokens;
...
util::Status TrainerInterface::SaveModel(std::string_view filename) const {
...
util::Status TrainerInterface::SaveVocab(std::string_view filename) const {
...
util::Status TrainerInterface::InitMetaPieces() {
  ...
  auto insert_id = [&has_unk, this](int id, const TString &w) -> bool {
    if (id < 0) return true;
