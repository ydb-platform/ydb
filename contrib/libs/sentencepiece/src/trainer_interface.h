...
template <typename K, typename V>
std::vector<std::pair<K, V>> Sorted(const y_absl::flat_hash_map<K, V> &m) {
...
class MultiFileSentenceIterator : public SentenceIterator {
 public:
  explicit MultiFileSentenceIterator(const std::vector<TString> &files);
  ~MultiFileSentenceIterator() {}

  bool done() const override;
  void Next() override;
  const TString &value() const override { return value_; }
...
class TrainerInterface {
 public:
  using Sentence = std::pair<TString, int64_t>;
 ...
 protected:
  ...
  y_absl::flat_hash_map<char32, int64_t> required_chars_;

  // Final output pieces
  std::vector<std::pair<TString, float>> final_pieces_;
  ...
  // key is vocab id.
  std::map<int, std::pair<TString, ModelProto::SentencePiece::Type>>
      meta_pieces_;
 ...
 private:
  ...
  util::Status SaveSplits(std::string_view filename) const;

  // Saves model file.
  util::Status SaveModel(std::string_view filename) const;

  // Saves vocabulary file for NMT.
  util::Status SaveVocab(std::string_view filename) const;

  // Initializes `meta_pieces_` from TrainerSpec.
  util::Status InitMetaPieces();

  // Randomly sampled raw sentences for self-testing.
  std::vector<TString> self_test_samples_;
};
}  // namespace sentencepiece
