...
void Lattice::Clear() {
  begin_nodes_.clear();
  end_nodes_.clear();
  sentence_ = std::string_view("");
  surface_.clear();
  node_allocator_.Free();
}

void Lattice::SetSentence(std::string_view sentence) {
  Clear();
...
Lattice::Node *Lattice::Insert(int pos, int length) {
  ...
  node->piece = std::string_view(surface(pos), utf8_length);
...
Hypothesis *CloneHypAndDependents(
    ...
    y_absl::flat_hash_map<const Hypothesis *, Hypothesis *> *clone_map,
...
void Model::BuildTrie(std::vector<std::pair<std::string_view, int>> *pieces) {
  if (!status().ok()) return;
...
Model::Model(const ModelProto &model_proto) {
  ...
  std::vector<std::pair<std::string_view, int>> pieces;
...
EncodeResult Model::Encode(std::string_view normalized) const {
...
NBestEncodeResult Model::NBestEncode(std::string_view normalized,
...
EncodeResult Model::SampleEncode(std::string_view normalized,
...
NBestEncodeResult Model::SampleEncodeAndScore(std::string_view normalized,
...
float Model::CalculateEntropy(std::string_view normalized,
                              float inv_theta) const {
  Lattice lattice;
  lattice.SetSentence(normalized);
  PopulateNodes(&lattice);
...
bool Model::VerifyOutputsEquivalent(std::string_view expected,
                                    std::string_view actual) const {
  auto compute_unigram_model_score =
      [this](std::vector<std::string_view> output_pieces) {
        float total_score = 0;
        const float unk_score = min_score() - kUnkPenalty;
  ...
  const auto expected_score =
      compute_unigram_model_score(y_absl::StrSplit(expected, ' '));
  const auto actual_score =
      compute_unigram_model_score(y_absl::StrSplit(actual, ' '));
...
EncodeResult Model::EncodeOptimized(std::string_view normalized) const {
