...
class Lattice {
 public:
  ...
  struct Node {
    std::string_view piece;  // Sentence piece representation.
  ...
  // Sets new sentence.
  void SetSentence(std::string_view sentence);
...
class Model : public ModelInterface {
 public:
  ...
  NBestEncodeResult NBestEncode(std::string_view normalized,
                                int nbest_size) const override;

  EncodeResult SampleEncode(std::string_view normalized,
                            float theta) const override;

  NBestEncodeResult SampleEncodeAndScore(std::string_view normalized,
                                         float theta, int samples, bool wor,
                                         bool include_best) const override;

  float CalculateEntropy(std::string_view normalized,
                         float theta) const override;
  ...
  int PieceToId(std::string_view piece) const override;

  // Verifies if two outputs are equivalent by comparing their scores.
  bool VerifyOutputsEquivalent(std::string_view expected,
                               std::string_view actual) const override;
 ...
 protected:
  // Builds a Trie index.
  void BuildTrie(std::vector<std::pair<std::string_view, int>> *pieces);
  ...
  EncodeResult EncodeOptimized(std::string_view normalized) const;
