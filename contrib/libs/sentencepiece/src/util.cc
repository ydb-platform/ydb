...
namespace {
std::shared_ptr<const TString> *GetSharedDataDir() {
  static auto g_data_dir = std::make_shared<const TString>(INSTALL_DATADIR);
  return &g_data_dir;
}
}  // namespace
...
void SetDataDir(std::string_view data_dir) {
  auto shared_data_dir =
      std::make_shared<const TString>(TString(data_dir));
...
TString UnicodeCharToUTF8(const char32 c) { return UnicodeTextToUTF8({c}); }
