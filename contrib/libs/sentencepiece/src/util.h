...
inline bool lexical_cast(std::string_view arg, Target *result) {
  std::stringstream ss;
  return (ss << arg.data() && ss >> *result);
}

template <>
inline bool lexical_cast(std::string_view arg, bool *result) {
  const char *kTrue[] = {"1", "t", "true", "y", "yes"};
  const char *kFalse[] = {"0", "f", "false", "n", "no"};
  TString lower_value = TString(arg);
  std::transform(lower_value.begin(), lower_value.vend(), lower_value.begin(),
                 ::tolower);
...
template <>
inline bool lexical_cast(std::string_view arg, TString *result) {
  *result = TString(arg);
  return true;
}
...
template <typename T>
inline TString EncodePOD(const T &value) {
  TString s;
  s.resize(sizeof(T));
...
template <typename T>
inline TString IntToHex(T value) {
...
template <typename T>
inline T HexToInt(std::string_view value) {
  T n;
...
TString UnicodeCharToUTF8(const char32 c);

UnicodeText UTF8ToUnicodeText(std::string_view utf8);

TString UnicodeTextToUTF8(const UnicodeText &utext);
...
inline TString JoinPath(std::string_view path) {
  return TString(path.data(), path.size());
}

template <typename... T>
inline TString JoinPath(std::string_view first, const T &...rest) {
#ifdef OS_WIN
...
std::vector<TString> StrSplitAsCSV(std::string_view text);
...
#define DECLARE_ERROR(FUNC)                                \
  inline util::Status FUNC##Error(std::string_view str) { \
...
class StatusBuilder {
 public:
  ...
  operator Status() const { return Status(code_, TString(os_.str())); }
