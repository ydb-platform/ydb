  ...
  if (err) {
    ...
    if (is_arcadia) {
      return svn_error_quick_wrap(
             err,
             _("\n  Full SSH configuration documentation is here: "
               "https://wiki.yandex-team.ru/arcadia/starterguide/"
               "#nastrojjkakljuchejjdljadostupakrepozitoriju\n"
               "  To debug SSH connection problems, run "
               "SVN_SSH='ssh -v' svn info svn+ssh://arcadia.yandex.ru/arc\n"
               "  To get help you can send output of the above command to arcadia@yandex-team.ru"));
    } else {
      return svn_error_quick_wrap(
             err,
             _("To better debug SSH connection problems, remove the -q "
               "option from 'ssh' in the [tunnels] section of your "
               "Subversion configuration file. If -q option is not "
               "set, add -v option."));
    }
  }
