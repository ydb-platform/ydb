...
def exists(name):
    ...
    try:
        ...
        file_found = rs.find("resfs/file/contrib/python/Babel/py2/babel/locale-data/%s" % os.path.basename(resolve_locale_filename(name))) is not None
    except ImportError:
    ...
    if data is None:
        try:
            ...
            locale_identifiers.cache = data = [
                ...
                (os.path.splitext(filename) for filename in rs.iterkeys(prefix="resfs/file/contrib/python/Babel/py2/babel/locale-data"))
...
def load(name, merge_inherited=True):
    ...
    try:
        ...
        if not data:
            ...
            try:
                import library.python.resource as rs
                pickle_data = rs.find("resfs/file/contrib/python/Babel/py2/babel/locale-data/%s" % os.path.basename(resolve_locale_filename(name)))
