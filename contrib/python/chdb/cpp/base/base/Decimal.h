...
{
    template <typename T>
    struct hash<DB_CHDB::Decimal<T>>
    {
        size_t operator()(const DB_CHDB::Decimal<T> & x) const { return hash<T>()(x.value); }
    };

    template <>
    struct hash<DB_CHDB::Decimal128>
    {
        size_t operator()(const DB_CHDB::Decimal128 & x) const
        {
    ...
    template <>
    struct hash<DB_CHDB::DateTime64>
    {
        size_t operator()(const DB_CHDB::DateTime64 & x) const
        {
            return std::hash<DB_CHDB::DateTime64::NativeType>()(x);
        }
    };

    template <>
    struct hash<DB_CHDB::Decimal256>
    {
        size_t operator()(const DB_CHDB::Decimal256 & x) const
        {
            // FIXME temp solution
