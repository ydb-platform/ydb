...
namespace std
{
    ...
    template <>
    struct hash<DB_CHDB::IPv6>
    {
        size_t operator()(const DB_CHDB::IPv6 & x) const
        {
            return std::hash<std::string_view>{}(
                std::string_view(reinterpret_cast<const char *>(&x.toUnderType()), sizeof(DB_CHDB::IPv6::UnderlyingType)));
        }
    };

    template <>
    struct hash<DB_CHDB::IPv4>
    {
        size_t operator()(const DB_CHDB::IPv4 & x) const
        {
            return std::hash<DB_CHDB::IPv4::UnderlyingType>()(x.toUnderType());
        }
    };
}
