...
#    if defined(DEBUG_OR_SANITIZER_BUILD)
        ...
        #define chassert_1(x, ...) do { static_cast<bool>(x) ? void(0) : ::DB_CHDB::abortOnFailedAssertion(#x); } while (0)
        #define chassert_2(x, comment, ...) do { static_cast<bool>(x) ? void(0) : ::DB_CHDB::abortOnFailedAssertion(comment); } while (0)
