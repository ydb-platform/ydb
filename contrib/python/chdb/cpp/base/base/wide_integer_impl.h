...
struct IsWideInteger<wide_CHDB::integer<Bits, Signed>>
{
...
// numeric limits
template <size_t Bits, typename Signed>
class numeric_limits<wide_CHDB::integer<Bits, Signed>>
{
public:
    ...
    static constexpr wide_CHDB::integer<Bits, Signed> min() noexcept
    {
        if constexpr (is_same_v<Signed, signed>)
        {
            using T = wide_CHDB::integer<Bits, signed>;
            T res{};
            res.items[T::_impl::big(0)] = std::numeric_limits<typename wide_CHDB::integer<Bits, Signed>::signed_base_type>::min();
            return res;
        }
        return wide_CHDB::integer<Bits, Signed>(0);
    }

    static constexpr wide_CHDB::integer<Bits, Signed> max() noexcept
    {
        using T = wide_CHDB::integer<Bits, Signed>;
        T res{};
        res.items[T::_impl::big(0)] = is_same_v<Signed, signed>
            ? std::numeric_limits<typename wide_CHDB::integer<Bits, Signed>::signed_base_type>::max()
            : std::numeric_limits<typename wide_CHDB::integer<Bits, Signed>::base_type>::max();
        for (unsigned i = 1; i < wide_CHDB::integer<Bits, Signed>::_impl::item_count; ++i)
        {
            res.items[T::_impl::big(i)] = std::numeric_limits<typename wide_CHDB::integer<Bits, Signed>::base_type>::max();
        }
        return res;
    }

    static constexpr wide_CHDB::integer<Bits, Signed> lowest() noexcept { return min(); }
    static constexpr wide_CHDB::integer<Bits, Signed> epsilon() noexcept { return 0; }
    static constexpr wide_CHDB::integer<Bits, Signed> round_error() noexcept { return 0; }
    static constexpr wide_CHDB::integer<Bits, Signed> infinity() noexcept { return 0; }
    static constexpr wide_CHDB::integer<Bits, Signed> quiet_NaN() noexcept { return 0; }
    static constexpr wide_CHDB::integer<Bits, Signed> signaling_NaN() noexcept { return 0; }
    static constexpr wide_CHDB::integer<Bits, Signed> denorm_min() noexcept { return 0; }
};

// type traits
template <size_t Bits, typename Signed, size_t Bits2, typename Signed2>
struct common_type<wide_CHDB::integer<Bits, Signed>, wide_CHDB::integer<Bits2, Signed2>>
{
    using type = std::conditional_t<Bits == Bits2,
        ...
        std::conditional_t<Bits2<Bits, wide_CHDB::integer<Bits, Signed>, wide_CHDB::integer<Bits2, Signed2>>>;
};

template <size_t Bits, typename Signed, typename Arithmetic>
struct common_type<wide_CHDB::integer<Bits, Signed>, Arithmetic>
{
    static_assert(wide_CHDB::ArithmeticConcept<Arithmetic>());
...
struct common_type<Arithmetic, wide_CHDB::integer<Bits, Signed>> : common_type<wide_CHDB::integer<Bits, Signed>, Arithmetic>
{
};
...
struct integer<Bits, Signed>::_impl
{
    ...
    {
        /// NOTE: this can be called with DB_CHDB::Decimal, and in this case, result
        /// will be wrong
        if constexpr (std::is_signed_v<T>)
...
template <size_t Bits, typename Signed>
struct hash<wide_CHDB::integer<Bits, Signed>>
{
    std::size_t operator()(const wide_CHDB::integer<Bits, Signed> & lhs) const
    {
