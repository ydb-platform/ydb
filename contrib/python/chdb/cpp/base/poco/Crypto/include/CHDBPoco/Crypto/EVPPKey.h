...
#ifndef CHDB_Crypto_EVPPKeyImpl_INCLUDED
#define CHDB_Crypto_EVPPKeyImpl_INCLUDED
...
#include "CHDBPoco/Crypto/CryptoException.h"
...
namespace Crypto
{
    ...
    private:
                ...
                if (pKey)
                {
                    ...
                    if (pFile)
                    {
                        ...
                        if (readFunc(pFile, &pKey, pCB, pPassword))
                        {
                            ...
                            {
                                CHDB_poco_assert_dbg(typeid(K *) == typeid(EVP_PKEY *));
                                *ppKey = (K *)pKey;
                            }
                            if (!*ppKey)
                                goto error;
        ...
        static bool loadKey(K ** ppKey, PEM_read_BIO_Key_fn readFunc, F getFunc, std::istream * pIstr, const std::string & pass = "")
        {
            ...
            if (pIstr)
            {
                ...
                CHDBPoco::StreamCopier::copyStream(*pIstr, ostr);
    ...
    {
        CHDB_poco_check_ptr(other._pEVPPKey);
...
#endif // CHDB_Crypto_EVPPKeyImpl_INCLUDED
