	...
	_iv()
{
	...
	if (!_pCipher)
		throw CHDBPoco::NotFoundException("Cipher " + name + " was not found");

	_pDigest = EVP_get_digestbyname(digest.c_str());

	if (!_pDigest)
		throw CHDBPoco::NotFoundException("Digest " + name + " was not found");
...
CipherKeyImpl::Mode CipherKeyImpl::mode() const
{
	...
	throw CHDBPoco::IllegalStateException("Unexpected value of EVP_CIPHER_mode()");
}
...
{
	CHDBPoco::RandomInputStream random;
...
{
	CHDB_poco_assert(mode() == MODE_GCM || iv.size() == static_cast<ByteVec::size_type>(ivSize()));
	_iv = iv;
}
