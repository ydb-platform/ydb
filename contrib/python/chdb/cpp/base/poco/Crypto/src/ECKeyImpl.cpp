...
#include "CHDBPoco/Crypto/ECKeyImpl.h"
#include "CHDBPoco/Crypto/X509Certificate.h"
#include "CHDBPoco/Crypto/PKCS12Container.h"
	...
	const std::string& privateKeyPassphrase): KeyPairImpl("ec", KT_EC_IMPL), _pEC(0)
{
	...
	{
		checkEC(CHDBPoco::format("ECKeyImpl(%s, %s, %s)",
	...
	const std::string& privateKeyPassphrase): KeyPairImpl("ec", KT_EC_IMPL), _pEC(0)
{
	...
	{
		checkEC(CHDBPoco::format("ECKeyImpl(stream, stream, %s)",
...
{
	if (!_pEC) throw OpenSSLException(CHDBPoco::format("%s: %s", method, func));
	if (!EC_KEY_check_key(_pEC))
		throw OpenSSLException(CHDBPoco::format("%s: EC_KEY_check_key()", method));
}
