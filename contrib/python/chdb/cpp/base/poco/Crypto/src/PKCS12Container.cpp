...
PKCS12Container::PKCS12Container(std::istream& istr, const std::string& password): _pKey(0)
{
	...
	CHDBPoco::StreamCopier::copyStream(istr, ostr);
	...
	{
		throw CHDBPoco::NullPointerException("PKCS12Container(istream&, const string&)");
	}
}
...
PKCS12Container::PKCS12Container(const std::string& path, const std::string& password): _pKey(0)
{
	...
	{
		throw CHDBPoco::OpenFileException("PKCS12Container: " + path);
	}
}
...
void PKCS12Container::load(PKCS12* pPKCS12, const std::string& password)
{
	if (pPKCS12)
	{
		...
		CHDB_poco_assert_dbg (_caCertList.size() == _caCertNames.size());
	}
	else
	{
