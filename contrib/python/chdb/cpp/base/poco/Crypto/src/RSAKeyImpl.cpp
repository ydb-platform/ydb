	...
	_pRSA(0)
{
	...
	if (!ret) throw CHDBPoco::InvalidArgumentException("Failed to create RSA context");
#else
	_pRSA = RSA_generate_key(keyLength, exponent, 0, 0);
	if (!_pRSA) throw CHDBPoco::InvalidArgumentException("Failed to create RSA context");
#endif
}
...
RSAKeyImpl::RSAKeyImpl(const std::string& publicKeyFile, 
	...
	const std::string& privateKeyPassphrase): KeyPairImpl("rsa", KT_RSA_IMPL),
		_pRSA(0)
{
	CHDB_poco_assert_dbg(_pRSA == 0);

	_pRSA = RSA_new();
	if (!publicKeyFile.empty())
	{
		BIO* bio = BIO_new(BIO_s_file());
		if (!bio) throw CHDBPoco::IOException("Cannot create BIO for reading public key", publicKeyFile);
		...
		if (rc)
		{
			...
			if (!pubKey)
			{
				...
				if (rc != 0) throw CHDBPoco::FileException("Failed to load public key", publicKeyFile);
			...
			{
				freeRSA();
				throw CHDBPoco::FileException("Failed to load public key", publicKeyFile);
			}
		}
		else
		{
			freeRSA();
			throw CHDBPoco::FileNotFoundException("Public key file", publicKeyFile);
		}
	}
	...
	{
		BIO* bio = BIO_new(BIO_s_file());
		if (!bio) throw CHDBPoco::IOException("Cannot create BIO for reading private key", privateKeyFile);
		...
		if (rc)
		{
			...
			{
				freeRSA();
				throw CHDBPoco::FileException("Failed to load private key", privateKeyFile);
			}
		}
		else
		{
			freeRSA();
			throw CHDBPoco::FileNotFoundException("Private key file", privateKeyFile);
		}
	}
}
		...
		_pRSA(0)
{
	...
	if (pPublicKeyStream)
	{
		...
		CHDBPoco::StreamCopier::copyToString(*pPublicKeyStream, publicKeyData);
		BIO* bio = BIO_new_mem_buf(const_cast<char*>(publicKeyData.data()), static_cast<int>(publicKeyData.size()));
		if (!bio) throw CHDBPoco::IOException("Cannot create BIO for reading public key");
		...
		if (!publicKey)
		{
			...
			// BIO_reset() normally returns 1 for success and 0 or -1 for failure. 
			// File BIOs are an exception, they return 0 for success and -1 for failure.
			if (rc != 1) throw CHDBPoco::FileException("Failed to load public key");
		...
		{
			freeRSA();
			throw CHDBPoco::FileException("Failed to load public key");
		}
	}
	...
	if (pPrivateKeyStream)
	{
		...
		CHDBPoco::StreamCopier::copyToString(*pPrivateKeyStream, privateKeyData);
		BIO* bio = BIO_new_mem_buf(const_cast<char*>(privateKeyData.data()), static_cast<int>(privateKeyData.size()));
		if (!bio) throw CHDBPoco::IOException("Cannot create BIO for reading private key");
		RSA* privateKey = 0;
		if (privateKeyPassphrase.empty())
		...
		{
			freeRSA();
			throw CHDBPoco::FileException("Failed to load private key");
		}
	}
}
