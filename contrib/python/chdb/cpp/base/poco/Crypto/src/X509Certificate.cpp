...
{
	CHDB_poco_assert (!_pCert);

	std::stringstream certStream;
	CHDBPoco::StreamCopier::copyStream(istr, certStream);
	std::string cert = certStream.str();

	BIO *pBIO = BIO_new_mem_buf(const_cast<char*>(cert.data()), static_cast<int>(cert.size()));
	if (!pBIO) throw CHDBPoco::IOException("Cannot create BIO for reading certificate");
	_pCert = PEM_read_bio_X509(pBIO, 0, 0, 0);
	BIO_free(pBIO);

	if (!_pCert) throw CHDBPoco::IOException("Failed to load certificate from stream");
...
void X509Certificate::load(const std::string& path)
{
	...
	if (!pBIO) throw CHDBPoco::IOException("Cannot create BIO for reading certificate file", path);
	if (!BIO_read_filename(pBIO, const_cast<char *>(path.c_str())))
	{
		BIO_free(pBIO);
		throw CHDBPoco::OpenFileException("Cannot open certificate file for reading", path);
	}

	_pCert = PEM_read_bio_X509(pBIO, 0, 0, 0);
	BIO_free(pBIO);

	if (!_pCert) throw CHDBPoco::ReadFileException("Failed to load certificate from", path);
...
void X509Certificate::save(std::ostream& stream) const
{
	...
	if (!pBIO) throw CHDBPoco::IOException("Cannot create BIO for writing certificate");
	try
	{
		if (!PEM_write_bio_X509(pBIO, _pCert)) 
			throw CHDBPoco::IOException("Failed to write certificate to stream");
...
void X509Certificate::save(const std::string& path) const
{
	...
	{
		BIO_free(pBIO);
		throw CHDBPoco::CreateFileException("Cannot create certificate file", path);
	}
	try
	{
		if (!PEM_write_bio_X509(pBIO, _pCert)) 
			throw CHDBPoco::WriteFileException("Failed to write certificate to file", path);
	}
	catch (...)
	{
		BIO_free(pBIO);
...
CHDBPoco::DateTime X509Certificate::validFrom() const
{
...
CHDBPoco::DateTime X509Certificate::expiresOn() const
{
...
bool X509Certificate::issuedBy(const X509Certificate& issuerCertificate) const
{
	...
	if (!pIssuerPublicKey) throw CHDBPoco::InvalidArgumentException("Issuer certificate has no public key");
...
std::string X509Certificate::signatureAlgorithm() const
{
...
#else
	CHDB_poco_check_ptr(_pCert->sig_alg);
	...
	if (sigNID != NID_undef)
	{
		...
		else throw OpenSSLException(CHDBPoco::format("X509Certificate::"
...
X509Certificate::List X509Certificate::readPEM(const std::string& pemFileName)
{
	...
	if (!x) throw OpenSSLException(CHDBPoco::format("X509Certificate::readPEM(%s)", pemFileName));
	while(x)
	{
