...
#ifndef CHDB_Data_AbstractPreparator_INCLUDED
#define CHDB_Data_AbstractPreparator_INCLUDED
...
namespace Data
{
    ...
    public:
        ...
        AbstractPreparator(CHDBPoco::UInt32 length = 1u);
        /// Creates the AbstractPreparator.

        virtual ~AbstractPreparator();
        /// Destroys the AbstractPreparator.

        virtual void prepare(std::size_t pos, const CHDBPoco::Int8 &) = 0;
        /// Prepares an Int8.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::Int8> & val);
        /// Prepares an Int8 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::Int8> & val);
        /// Prepares an Int8 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::Int8> & val);
        /// Prepares an Int8 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::UInt8 &) = 0;
        /// Prepares an UInt8.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::UInt8> & val);
        /// Prepares an UInt8 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::UInt8> & val);
        /// Prepares an UInt8 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::UInt8> & val);
        /// Prepares an UInt8 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::Int16 &) = 0;
        /// Prepares an Int16.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::Int16> & val);
        /// Prepares an Int16 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::Int16> & val);
        /// Prepares an Int16 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::Int16> & val);
        /// Prepares an Int16 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::UInt16 &) = 0;
        /// Prepares an UInt16.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::UInt16> & val);
        /// Prepares an UInt16 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::UInt16> & val);
        /// Prepares an UInt16 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::UInt16> & val);
        /// Prepares an UInt16 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::Int32 &) = 0;
        /// Prepares an Int32.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::Int32> & val);
        /// Prepares an Int32 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::Int32> & val);
        /// Prepares an Int32 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::Int32> & val);
        /// Prepares an Int32 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::UInt32 &) = 0;
        /// Prepares an UInt32.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::UInt32> & val);
        /// Prepares an UInt32 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::UInt32> & val);
        /// Prepares an UInt32 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::UInt32> & val);
        /// Prepares an UInt32 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::Int64 &) = 0;
        /// Prepares an Int64.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::Int64> & val);
        /// Prepares an Int64 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::Int64> & val);
        /// Prepares an Int64 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::Int64> & val);
        /// Prepares an Int64 list.

        virtual void prepare(std::size_t pos, const CHDBPoco::UInt64 &) = 0;
        /// Prepares an UInt64.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::UInt64> & val);
        /// Prepares an UInt64 vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::UInt64> & val);
        /// Prepares an UInt64 deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::UInt64> & val);
        ...
        virtual void prepare(std::size_t pos, const CHDBPoco::Dynamic::Var &) = 0;
        /// Prepares a Var.

        virtual void prepare(std::size_t pos, const std::vector<CHDBPoco::Dynamic::Var> & val);
        /// Prepares a Var vector.

        virtual void prepare(std::size_t pos, const std::deque<CHDBPoco::Dynamic::Var> & val);
        /// Prepares a Var deque.

        virtual void prepare(std::size_t pos, const std::list<CHDBPoco::Dynamic::Var> & val);
        /// Prepares a Var list.

        void setLength(CHDBPoco::UInt32 length);
        /// Sets the length of prepared data.
        /// Needed only for data lengths greater than 1 (i.e. for
        /// bulk operations).

        CHDBPoco::UInt32 getLength() const;
    ...
    /// inlines
    ///
    inline void AbstractPreparator::setLength(CHDBPoco::UInt32 length)
    {
        _length = length;
    }


    inline CHDBPoco::UInt32 AbstractPreparator::getLength() const
    {
        return _length;
    }
...
#endif // CHDB_Data_AbstractPreparator_INCLUDED
