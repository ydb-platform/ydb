...
#ifndef CHDB_Data_BulkExtraction_INCLUDED
#define CHDB_Data_BulkExtraction_INCLUDED
...
namespace Data
{
    ...
    public:
        ...
        BulkExtraction(C & result, CHDBPoco::UInt32 limit, const Position & pos = Position(0))
            : AbstractExtraction(limit, pos.value(), true), _rResult(result), _default()
        {
            if (static_cast<CHDBPoco::UInt32>(result.size()) != limit)
                result.resize(limit);
        }

        BulkExtraction(C & result, const CValType & def, CHDBPoco::UInt32 limit, const Position & pos = Position(0))
        ...
        {
            CHDBPoco::UInt32 limit = getLimit();
    ...
    public:
        ...
        InternalBulkExtraction(C & result, Column<C> * pColumn, CHDBPoco::UInt32 limit, const Position & pos = Position(0))
    ...
    namespace Keywords
    {
        ...
        /// with std::vector bulk extraction support.
        {
            CHDBPoco::UInt32 size = static_cast<CHDBPoco::UInt32>(t.size());
            if (0 == size)
...
#endif // CHDB_Data_BulkExtraction_INCLUDED
