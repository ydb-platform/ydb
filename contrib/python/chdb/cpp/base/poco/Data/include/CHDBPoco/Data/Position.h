//
// PositionExtraction.h
//
// Library: Data
// Package: DataCore
// Module:  Position
//
// Definition of the PositionExtraction class.
//
// Copyright (c) 2006, Applied Informatics Software Engineering GmbH.
// and Contributors.
//
// SPDX-License-Identifier:	BSL-1.0
//


#ifndef CHDB_Data_Position_INCLUDED
#define CHDB_Data_Position_INCLUDED


#include "CHDBPoco/Data/Limit.h"


namespace CHDBPoco
{
namespace Data
{


    class Data_API Position
    /// Utility class wrapping unsigned integer. Used to
    /// indicate the recordset position in batch SQL statements.
    {
    public:
        Position(CHDBPoco::UInt32 value);
        /// Creates the Position.

        ~Position();
        /// Destroys the Position.

        CHDBPoco::UInt32 value() const;
        /// Returns the position value.

    private:
        Position();

        CHDBPoco::UInt32 _value;
    };


    ///
    /// inlines
    ///
    inline CHDBPoco::UInt32 Position::value() const
    {
        return _value;
    }


    namespace Keywords
    {


        template <typename T>
        inline Position from(const T & value)
        /// Convenience function for creation of position.
        {
            return Position(value);
        }


    } // namespace Keywords


}
} // namespace CHDBPoco::Data


#endif // CHDB_Data_Position_INCLUDED
