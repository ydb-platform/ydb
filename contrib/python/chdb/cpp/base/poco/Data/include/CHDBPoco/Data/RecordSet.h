...
#ifndef CHDB_Data_RecordSet_INCLUDED
#define CHDB_Data_RecordSet_INCLUDED


#include <limits>
#include <ostream>
#include "CHDBPoco/AutoPtr.h"
#include "CHDBPoco/Data/BulkExtraction.h"
#include "CHDBPoco/Data/Data.h"
#include "CHDBPoco/Data/Extraction.h"
#include "CHDBPoco/Data/LOB.h"
#include "CHDBPoco/Data/RowFilter.h"
#include "CHDBPoco/Data/RowIterator.h"
...
namespace Data
{
    ...
    public:
        ...
        CHDBPoco::Dynamic::Var value(std::size_t col, std::size_t row, bool checkFiltering = true) const;
        /// Returns the data value at column, row location.

        CHDBPoco::Dynamic::Var value(const std::string & name, std::size_t row, bool checkFiltering = true) const;
        /// Returns the data value at named column, row location.

        template <typename T>
        CHDBPoco::Dynamic::Var nvl(const std::string & name, const T & deflt = T()) const
        /// Returns the value in the named column of the current row
        /// if the value is not NULL, or deflt otherwise.
        {
            if (isNull(name))
                return CHDBPoco::Dynamic::Var(deflt);
            else
                return value(name, _currentRow);
        }

        template <typename T>
        CHDBPoco::Dynamic::Var nvl(std::size_t index, const T & deflt = T()) const
        ...
        CHDBPoco::Dynamic::Var value(const std::string & name);
        /// Returns the value in the named column of the current row.

        CHDBPoco::Dynamic::Var value(std::size_t index);
        /// Returns the value in the given column of the current row.

        CHDBPoco::Dynamic::Var operator[](const std::string & name);
        /// Returns the value in the named column of the current row.

        CHDBPoco::Dynamic::Var operator[](std::size_t index);
        /// Returns the value in the named column of the current row.
    ...
    private:
        ...
        /// Returns the position of the column with specified name.
        {
            ...
            for (; it != end; ++it)
            {
                ...
                if (pExtraction)
                {
                    ...
                    if (0 == CHDBPoco::icompare(name, col.name()))
                        return col.position();
                }
            }

            if (typeFound)
                throw NotFoundException(CHDBPoco::format("Column name: %s", name));
            else
                throw NotFoundException(CHDBPoco::format("Column type: %s, name: %s", std::string(typeid(T).name()), name));
        }
        ...
        /// Returns the reference to column at specified position.
        {
            ...
            if (0 == s || pos >= s)
                throw RangeException(CHDBPoco::format("Invalid column index: %z", pos));
            ...
            {
                throw CHDBPoco::BadCastException(
                    CHDBPoco::format("Type cast failed!\nColumn: %z\nTarget type:\t%s", pos, std::string(typeid(T).name())));
            }
        }
        ...
        void filter(const CHDBPoco::AutoPtr<RowFilter> & pFilter);
        /// Sets the filter for the RecordSet.

        const CHDBPoco::AutoPtr<RowFilter> & getFilter() const;
        ...
        RowIterator * _pEnd;
        RowMap _rowMap;
        CHDBPoco::AutoPtr<RowFilter> _pFilter;
    ...
    inline CHDBPoco::Dynamic::Var RecordSet::value(const std::string & name)
    {
        return value(name, _currentRow);
    }


    inline CHDBPoco::Dynamic::Var RecordSet::value(std::size_t index)
    {
        return value(index, _currentRow);
    }


    inline CHDBPoco::Dynamic::Var RecordSet::operator[](const std::string & name)
    {
        return value(name, _currentRow);
    }


    inline CHDBPoco::Dynamic::Var RecordSet::operator[](std::size_t index)
    {
        return value(index, _currentRow);
    }
    ...
    inline const CHDBPoco::AutoPtr<RowFilter> & RecordSet::getFilter() const
    {
        return _pFilter;
    }
...
#endif // CHDB_Data_RecordSet_INCLUDED
