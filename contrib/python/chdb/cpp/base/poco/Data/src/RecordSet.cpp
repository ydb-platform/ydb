...
#include "CHDBPoco/Data/RecordSet.h"
...
using namespace CHDBPoco::Data::Keywords;
using CHDBPoco::DateTime;
using CHDBPoco::UTF16String;
...
CHDBPoco::Dynamic::Var RecordSet::value(std::size_t col, std::size_t row, bool useFilter) const
{
	if (useFilter && isFiltered() && !isAllowed(row))
		throw InvalidAccessException("Row not allowed");

	if (isNull(col, row)) return CHDBPoco::Dynamic::Var();
...
CHDBPoco::Dynamic::Var RecordSet::value(const std::string& name, std::size_t row, bool useFilter) const
{
	if (useFilter && isFiltered() && !isAllowed(row))
		throw InvalidAccessException("Row not allowed");

	if (isNull(metaColumn(name).position(), row)) return CHDBPoco::Dynamic::Var();
...
{
	CHDB_poco_assert (extractions().size());
...
void RecordSet::filter(const CHDBPoco::AutoPtr<RowFilter>& pFilter)
{
	_pFilter = pFilter;
}
