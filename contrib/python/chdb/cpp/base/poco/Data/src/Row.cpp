...
CHDBPoco::Dynamic::Var& Row::get(std::size_t col)
{
	try
	{
...
{
	CHDB_poco_assert (pos <= _values.size());
	...
	else if ((_values[pos].type() == typeid(CHDBPoco::Int8))   ||
		(_values[pos].type() == typeid(CHDBPoco::UInt8))  ||
		(_values[pos].type() == typeid(CHDBPoco::Int16))  ||
		(_values[pos].type() == typeid(CHDBPoco::UInt16)) ||
		(_values[pos].type() == typeid(CHDBPoco::Int32))  ||
		(_values[pos].type() == typeid(CHDBPoco::UInt32)) ||
		(_values[pos].type() == typeid(CHDBPoco::Int64))  ||
		(_values[pos].type() == typeid(CHDBPoco::UInt64)) ||
...
{
	CHDB_poco_assert (oldPos <= _values.size());
	CHDB_poco_assert (newPos <= _values.size());
	...
	else if ((_values[newPos].type() == typeid(CHDBPoco::Int8)) ||
		(_values[newPos].type() == typeid(CHDBPoco::UInt8))     ||
		(_values[newPos].type() == typeid(CHDBPoco::Int16))     ||
		(_values[newPos].type() == typeid(CHDBPoco::UInt16))    ||
		(_values[newPos].type() == typeid(CHDBPoco::Int32))     ||
		(_values[newPos].type() == typeid(CHDBPoco::UInt32))    ||
		(_values[newPos].type() == typeid(CHDBPoco::Int64))     ||
		(_values[newPos].type() == typeid(CHDBPoco::UInt64))    ||
...
{
	std::vector<CHDBPoco::Dynamic::Var>::const_iterator it = _values.begin();
	std::vector<CHDBPoco::Dynamic::Var>::const_iterator end = _values.end();
...
bool Row::operator < (const Row& other) const
{
	...
	for (; it != end; ++it)
	{
		...
		case COMPARE_AS_INTEGER:
			if (_values[it->get<0>()].convert<CHDBPoco::Int64>() < 
				other._values[it->get<0>()].convert<CHDBPoco::Int64>())
				return true;
			else if (_values[it->get<0>()].convert<CHDBPoco::Int64>() != 
				other._values[it->get<0>()].convert<CHDBPoco::Int64>())
				return false;
			break;
