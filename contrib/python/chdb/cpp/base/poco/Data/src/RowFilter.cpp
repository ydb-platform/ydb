...
{
	CHDB_poco_check_ptr(pRecordSet);
	init();
	duplicate();
...
RowFilter::RowFilter(Ptr pParent, LogicOperator op): _pRecordSet(0),
	...
	_not(false)
{
	CHDB_poco_check_ptr(_pParent.get());
	init();
	duplicate();
...
bool RowFilter::isAllowed(std::size_t row) const
{
	...
	for (; it != end; ++it)
	{
		for (std::size_t col = 0; col < columns; ++col)
		{
			...
			CHDBPoco::Dynamic::Var ret;
			CompT compOp = 0;
			CHDBPoco::Dynamic::Var val = rs.value(col, row, false);
...
{
	CHDB_poco_check_ptr (_pRecordSet);
...
void RowFilter::doCompare(CHDBPoco::Dynamic::Var& ret,
	CHDBPoco::Dynamic::Var& val,
	CompT comp,
