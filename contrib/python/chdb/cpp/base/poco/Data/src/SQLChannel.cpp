...
{
	CHDB_poco_check_ptr (_pLogStatement);
...
void SQLChannel::initLogStatement()
{
	...
	std::string sql;
	CHDBPoco::format(sql, "INSERT INTO %s VALUES (?,?,?,?,?,?,?,?)", _table);
	*_pLogStatement << sql,
		use(_source),
		use(_name),
		use(_pid),
...
{
	CHDBPoco::LoggingFactory::defaultFactory().registerChannelClass("SQLChannel", 
		new CHDBPoco::Instantiator<SQLChannel, CHDBPoco::Channel>);
}
