...
SessionPool::SessionPool(const std::string& connector, const std::string& connectionString, int minSessions, int maxSessions, int idleTime):
	...
	_shutdown(false)
{
	CHDBPoco::TimerCallback<SessionPool> callback(*this, &SessionPool::onJanitorTimer);
...
void SessionPool::setProperty(const std::string& name, const CHDBPoco::Any& value)
{
	CHDBPoco::Mutex::ScopedLock lock(_mutex);
	if (_shutdown) throw InvalidAccessException("Session pool has been shut down.");
...
CHDBPoco::Any SessionPool::getProperty(const std::string& name)
{
...
void SessionPool::putBack(PooledSessionHolderPtr pHolder)
{
	...
	{
		CHDB_poco_bugcheck_msg("Unknown session passed to SessionPool::putBack()");
	}
}


void SessionPool::onJanitorTimer(CHDBPoco::Timer&)
{
	CHDBPoco::Mutex::ScopedLock lock(_mutex);
	if (_shutdown) return;
