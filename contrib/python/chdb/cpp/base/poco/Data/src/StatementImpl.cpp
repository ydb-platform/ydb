...
{
	CHDB_poco_assert (_state != ST_DONE);
	std::size_t count = 0;
...
void StatementImpl::fixupExtraction()
{
	...
	CHDBPoco::Data::AbstractExtractionVec::iterator it    = extractions().begin();
	CHDBPoco::Data::AbstractExtractionVec::iterator itEnd = extractions().end();
	for (; it != itEnd; ++it)
	{
...
void StatementImpl::resetExtraction()
{
	...
	CHDB_poco_assert (_curDataSet < _columnsExtracted.size());
	...
	for (int i = 0; i < count; ++i)
	{
		...
		switch (mc.type())
		{
			...
			case MetaColumn::FDT_WSTRING:
				addInternalExtract<CHDBPoco::UTF16String>(mc); break;
			...
			default:
				throw CHDBPoco::InvalidArgumentException("Data type not supported.");
		}
	}
}
...
{
	CHDB_poco_check_ptr (pExtraction);
...
std::size_t StatementImpl::columnsExtracted(int dataSet) const
{
	...
	{
		CHDB_poco_assert (dataSet >= 0 && dataSet < _columnsExtracted.size());
...
std::size_t StatementImpl::rowsExtracted(int dataSet) const
{
	...
	{
		CHDB_poco_assert (dataSet >= 0 && dataSet < _extractors.size());
...
std::size_t StatementImpl::subTotalRowCount(int dataSet) const
{
	...
	{
		CHDB_poco_assert (dataSet >= 0 && dataSet < _subTotalRowCount.size());
...
{
	std::string sql;
	CHDBPoco::format(sql, _ostr.str(), arguments);
	_ostr.str("");
	_ostr << sql;
}
