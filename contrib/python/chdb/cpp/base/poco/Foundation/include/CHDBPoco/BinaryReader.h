...
#ifndef CHDB_Foundation_BinaryReader_INCLUDED
#define CHDB_Foundation_BinaryReader_INCLUDED
...
public:
    ...
    /// to the currently set global encoding (see CHDBPoco::TextEncoding::global()).
...
#if !defined(CHDB_POCO_LONG_IS_64_BIT)
...
public:
    BasicMemoryBinaryReader(const Buffer<T> & data, StreamByteOrder byteOrder = NATIVE_BYTE_ORDER)
        : BinaryReader(_istr, byteOrder), _data(data), _istr(data.begin(), data.capacity())
    {
    }

    BasicMemoryBinaryReader(const Buffer<T> & data, TextEncoding & encoding, StreamByteOrder byteOrder = NATIVE_BYTE_ORDER)
        : BinaryReader(_istr, encoding, byteOrder), _data(data), _istr(data.begin(), data.capacity())
    {
    }

    ~BasicMemoryBinaryReader() { }

    const Buffer<T> & data() const { return _data; }

    const MemoryInputStream & stream() const { return _istr; }

    MemoryInputStream & stream() { return _istr; }
...
#endif // CHDB_Foundation_BinaryReader_INCLUDED
