...
#ifndef CHDB_Foundation_BinaryWriter_INCLUDED
#define CHDB_Foundation_BinaryWriter_INCLUDED
...
public:
    ...
    /// (see CHDBPoco::TextEncoding::global()) to the specified encoding.
    ...
    {
        CHDBPoco::UInt32 size(static_cast<CHDBPoco::UInt32>(value.size()));
...
public:
    BasicMemoryBinaryWriter(Buffer<T> & data, StreamByteOrder byteOrder = NATIVE_BYTE_ORDER)
        : BinaryWriter(_ostr, byteOrder), _data(data), _ostr(data.begin(), data.capacity())
    {
    }

    BasicMemoryBinaryWriter(Buffer<T> & data, TextEncoding & encoding, StreamByteOrder byteOrder = NATIVE_BYTE_ORDER)
        : BinaryWriter(_ostr, encoding, byteOrder), _data(data), _ostr(data.begin(), data.capacity())
    {
    }
    ...
    Buffer<T> & data() { return _data; }

    const Buffer<T> & data() const { return _data; }

    const MemoryOutputStream & stream() const { return _ostr; }

    MemoryOutputStream & stream() { return _ostr; }
...
#endif // CHDB_Foundation_BinaryWriter_INCLUDED
