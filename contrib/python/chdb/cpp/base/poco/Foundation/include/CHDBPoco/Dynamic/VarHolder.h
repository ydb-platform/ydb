...
#ifndef CHDB_Foundation_VarHolder_INCLUDED
#define CHDB_Foundation_VarHolder_INCLUDED
...
namespace Dynamic
{
    ...
    /// std::string, CHDBPoco::UTF16String, DateTime, LocalDateTime, Timestamp, std::vector<Var> and DynamicStruct.
    ///
    ...
    public:
        ...
        /// CHDB_POCO_NO_SOO is not defined), VarHolder will be
        /// instantiated in-place if it's size is smaller
        /// than CHDB_POCO_SMALL_OBJECT_SIZE.
        ...
        virtual void convert(CHDBPoco::UTF16String & val) const;
        /// Throws BadCastException. Must be overridden in a type
        /// specialization in order to support the conversion.
    ...
    protected:
        ...
        /// larger than CHDB_POCO_SMALL_OBJECT_SIZE, holder is instantiated on
...
#else
            CHDB_poco_check_ptr(pVarHolder);
        ...
        /// it is converted.
        {
            CHDB_poco_static_assert(std::numeric_limits<F>::is_specialized);
            CHDB_poco_static_assert(std::numeric_limits<T>::is_specialized);
            CHDB_poco_static_assert(std::numeric_limits<F>::is_signed);
            CHDB_poco_static_assert(std::numeric_limits<T>::is_signed);
        ...
        /// If upper limit is within the target data type limits, the conversion is performed.
        {
            CHDB_poco_static_assert(std::numeric_limits<F>::is_specialized);
            CHDB_poco_static_assert(std::numeric_limits<T>::is_specialized);
            CHDB_poco_static_assert(!std::numeric_limits<F>::is_integer);
            CHDB_poco_static_assert(std::numeric_limits<T>::is_integer);
            CHDB_poco_static_assert(!std::numeric_limits<T>::is_signed);
    ...
    inline void VarHolder::convert(CHDBPoco::UTF16String & /*val*/) const
    {
        throw BadCastException("Can not convert to CHDBPoco::UTF16String");
    }
    ...
    public:
        ...
        void convert(CHDBPoco::UTF16String & val) const
        {
            std::string str = NumberFormatter::format(_val);
            CHDBPoco::UnicodeConverter::convert(str, val);
        }
    ...
    public:
        ...
        void convert(CHDBPoco::UTF16String & val) const
        {
            CHDBPoco::UnicodeConverter::convert(_val, val);
        }
    ...
    {
    public:
        VarHolderImpl(const char * pVal) : _val(CHDBPoco::UnicodeConverter::to<UTF16String>(pVal)) { }

        VarHolderImpl(const CHDBPoco::UTF16String & val) : _val(val) { }

        ~VarHolderImpl() { }

        const std::type_info & type() const { return typeid(CHDBPoco::UTF16String); }
        ...
        const CHDBPoco::UTF16String & value() const
        {
            return _val;
        }
        ...
        UTF16Char & operator[](CHDBPoco::UTF16String::size_type n)
        {
            if (n < size())
                return _val.operator[](n);
        ...
        const UTF16Char & operator[](CHDBPoco::UTF16String::size_type n) const
        {
            if (n < size())
                return _val.operator[](n);
...
#else // if defined (CHDB_POCO_LONG_IS_64_BIT)
    ...
    public:
        ...
        void convert(std::string & val) const
        {
            val = DateTimeFormatter::format(_val, CHDBPoco::DateTimeFormat::ISO8601_FORMAT);
        }
...
#endif // CHDB_Foundation_VarHolder_INCLUDED
