...
#ifndef CHDB_Foundation_FIFOBuffer_INCLUDED
#define CHDB_Foundation_FIFOBuffer_INCLUDED
...
public:
    ...
    mutable CHDBPoco::BasicEvent<bool> writable;
    ...
    mutable CHDBPoco::BasicEvent<bool> readable;
    ...
    std::size_t peek(CHDBPoco::Buffer<T> & buffer, std::size_t length = 0) const
    /// Peeks into the data currently in the FIFO
    /// without actually extracting it.
    ...
    /// Returns the size of the copied data.
    {
        ...
        CHDB_poco_assert(_used >= readLen);
        _used -= readLen;
        if (0 == _used)
            _begin = 0;
        else
    ...
    std::size_t read(CHDBPoco::Buffer<T> & buffer, std::size_t length = 0)
    /// Copies the data currently in the FIFO
    ...
    /// Returns the length of data written.
    {
        ...
        _used += len;
        CHDB_poco_assert(_used <= _buffer.size());
        if (_notify)
            notify(usedBefore);
    ...
    /// the used buffer size.
    {
        ...
        if (length > available())
            throw CHDBPoco::InvalidAccessException("Cannot extend buffer.");

        if (!isWritable())
            throw CHDBPoco::InvalidAccessException("Buffer not writable.");
...
#endif // CHDB_Foundation_FIFOBuffer_INCLUDED
