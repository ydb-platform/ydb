...
#ifndef CHDB_Foundation_PBKDF2Engine_INCLUDED
#define CHDB_Foundation_PBKDF2Engine_INCLUDED
...
public:
    ...
    PBKDF2Engine(const std::string & salt, unsigned c = 4096, CHDBPoco::UInt32 dkLen = PRF_DIGEST_SIZE) : _s(salt), _c(c), _dkLen(dkLen)
    {
...
protected:
    ...
    void f(CHDBPoco::UInt32 i)
    {
        PRF prf(_p);
        prf.update(_s);
        CHDBPoco::UInt32 iBE = CHDBPoco::ByteOrder::toBigEndian(i);
        prf.update(&iBE, sizeof(iBE));
        CHDBPoco::DigestEngine::Digest up = prf.digest();
        CHDBPoco::DigestEngine::Digest ux = up;
        CHDB_poco_assert_dbg(ux.size() == PRF_DIGEST_SIZE);
        for (unsigned k = 1; k < _c; k++)
        {
            prf.reset();
            prf.update(&up[0], up.size());
            CHDBPoco::DigestEngine::Digest u = prf.digest();
            CHDB_poco_assert_dbg(u.size() == PRF_DIGEST_SIZE);
...
#endif // CHDB_Foundation_PBKDF2Engine_INCLUDED
