...
{
	CHDB_poco_assert (year >= 0 && year <= 9999);
	CHDB_poco_assert (month >= 1 && month <= 12);
	CHDB_poco_assert (day >= 1 && day <= daysOfMonth(year, month));
	CHDB_poco_assert (hour >= 0 && hour <= 23);
	CHDB_poco_assert (minute >= 0 && minute <= 59);
	CHDB_poco_assert (second >= 0 && second <= 60); // allow leap seconds
	CHDB_poco_assert (millisecond >= 0 && millisecond <= 999);
	CHDB_poco_assert (microsecond >= 0 && microsecond <= 999);
...
{
	CHDB_poco_assert (firstDayOfWeek >= 0 && firstDayOfWeek <= 6);
...
void DateTime::computeGregorian(double julianDay)
{
	...
	CHDB_poco_assert_dbg (_month >= 1 && _month <= 12);
	CHDB_poco_assert_dbg (_day >= 1 && _day <= daysOfMonth(_year, _month));
	CHDB_poco_assert_dbg (_hour >= 0 && _hour <= 23);
	CHDB_poco_assert_dbg (_minute >= 0 && _minute <= 59);
	CHDB_poco_assert_dbg (_second >= 0 && _second <= 59);
	CHDB_poco_assert_dbg (_millisecond >= 0 && _millisecond <= 999);
	CHDB_poco_assert_dbg (_microsecond >= 0 && _microsecond <= 999);
}
