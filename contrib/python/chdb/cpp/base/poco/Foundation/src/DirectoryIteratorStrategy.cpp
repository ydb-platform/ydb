...
#include "CHDBPoco/DirectoryIteratorStrategy.h"
...
bool TraverseBase::isDirectory(CHDBPoco::File& file)
{
	try
	{
...
const std::string ChildrenFirstTraverse::next(Stack* itStack, bool* isFinished)
{
	...
	CHDB_poco_check_ptr(isFinished);
	CHDB_poco_assert(!(*isFinished));
	...
	CHDB_poco_assert(!itStack->empty());
