...
#include "CHDBPoco/LoggingRegistry.h"
...
CHDBPoco::Logger::LoggerMap * _pLoggerMap = nullptr;
...
{
	void operator()(CHDBPoco::Logger * logger)
	{
		std::lock_guard<std::mutex> lock(getLoggerMutex());
...
int Logger::parseLevel(const std::string& level)
{
	...
	{
		int numLevel;
		if (CHDBPoco::NumberParser::tryParse(level, numLevel))
		{
