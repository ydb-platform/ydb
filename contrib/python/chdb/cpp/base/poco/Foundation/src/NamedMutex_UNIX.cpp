	...
	_name(name)
{
	...
	if (fd != -1)
		...
		throw SystemException(CHDBPoco::format("cannot create named mutex %s (lockfile)", fileName), _name);
	...
	throw SystemException(CHDBPoco::format("cannot create named mutex %s (semget() failed, errno=%d)", fileName, errno), _name);
