	...
	/// Used only internally.
{
	...
	CHDB_poco_assert_dbg (precision > 0);
	CHDB_poco_assert_dbg (str.length());
...
void floatToStr(char* buffer, int bufferSize, float value, int lowDec, int highDec)
{
	...
	DoubleToStringConverter dc(flags, CHDB_POCO_FLT_INF, CHDB_POCO_FLT_NAN, CHDB_POCO_FLT_EXP, lowDec, highDec, 0, 0);
...
void floatToFixedStr(char* buffer, int bufferSize, float value, int precision)
{
	...
	int flags = DoubleToStringConverter::UNIQUE_ZERO |
		DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
	DoubleToStringConverter dc(flags, CHDB_POCO_FLT_INF, CHDB_POCO_FLT_NAN, CHDB_POCO_FLT_EXP, -std::numeric_limits<float>::digits10, std::numeric_limits<float>::digits10, 0, 0);
...
void doubleToFixedStr(char* buffer, int bufferSize, double value, int precision)
{
	...
	int flags = DoubleToStringConverter::UNIQUE_ZERO |
		DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
	DoubleToStringConverter dc(flags, CHDB_POCO_FLT_INF, CHDB_POCO_FLT_NAN, CHDB_POCO_FLT_EXP, -std::numeric_limits<double>::digits10, std::numeric_limits<double>::digits10, 0, 0);
	dc.ToFixed(value, precision, &builder);
	builder.Finalize();
}
...
float strToFloat(const char* str)
{
	...
	StringToDoubleConverter converter(flags, 0.0, std::numeric_limits<float>::quiet_NaN(), CHDB_POCO_FLT_INF, CHDB_POCO_FLT_NAN);
...
double strToDouble(const char* str)
{
	...
	int flags = StringToDoubleConverter::ALLOW_LEADING_SPACES |
		StringToDoubleConverter::ALLOW_TRAILING_SPACES;
	StringToDoubleConverter converter(flags, 0.0, std::numeric_limits<double>::quiet_NaN(), CHDB_POCO_FLT_INF, CHDB_POCO_FLT_NAN);
