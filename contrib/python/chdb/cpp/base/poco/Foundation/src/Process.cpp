...
namespace
{
	std::vector<char> getEnvironmentVariablesBuffer(const CHDBPoco::Process::Env& env)
	{
		std::vector<char> envbuf;
		std::size_t pos = 0;

		for (CHDBPoco::Process::Env::const_iterator it = env.begin(); it != env.end(); ++it)
		{
...
{
	CHDB_poco_assert (inPipe == 0 || (inPipe != outPipe && inPipe != errPipe));
	std::string initialDirectory;
	Env env;
