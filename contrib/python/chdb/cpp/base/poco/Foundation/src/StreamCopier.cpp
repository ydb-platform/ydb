...
CHDBPoco::UInt64 StreamCopier::copyStream64(std::istream& istr, std::ostream& ostr, std::size_t bufferSize)
{
	CHDB_poco_assert (bufferSize > 0);

	Buffer<char> buffer(bufferSize);
	CHDBPoco::UInt64 len = 0;
	istr.read(buffer.begin(), bufferSize);
	std::streamsize n = istr.gcount();
	while (n > 0)
	{
...
CHDBPoco::UInt64 StreamCopier::copyToString64(std::istream& istr, std::string& str, std::size_t bufferSize)
{
	CHDB_poco_assert (bufferSize > 0);
...
CHDBPoco::UInt64 StreamCopier::copyStreamUnbuffered64(std::istream& istr, std::ostream& ostr)
{
	char c = 0;
	CHDBPoco::UInt64 len = 0;
	istr.get(c);
