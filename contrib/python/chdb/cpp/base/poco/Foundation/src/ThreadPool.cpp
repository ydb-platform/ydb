...
public:
	explicit PooledThread(
		...
		int stackSize = CHDB_POCO_THREAD_STACK_SIZE,
...
{
	CHDB_poco_assert_dbg (stackSize >= 0);
...
{
    DB_CHDB::ThreadStatus thread_status;
...
ThreadPool::ThreadPool(int minCapacity,
    ...
    _globalProfilerCPUTimePeriodNs(globalProfilerCPUTimePeriodNs_)
{
	CHDB_poco_assert (minCapacity >= 1 && maxCapacity >= minCapacity && idleTime > 0);
...
void ThreadPool::addCapacity(int n)
{
	...
	CHDB_poco_assert (_maxCapacity + n >= _minCapacity);
	_maxCapacity += n;
	housekeep();
}
...
public:
	...
	ThreadPool* pool()
	{
		...
		if (!_pPool)
		{
			...
			if (CHDB_POCO_THREAD_STACK_SIZE > 0)
				_pPool->setStackSize(CHDB_POCO_THREAD_STACK_SIZE);
		}
		return _pPool;
	}
