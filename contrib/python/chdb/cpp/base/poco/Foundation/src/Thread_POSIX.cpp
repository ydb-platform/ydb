...
#	endif
#endif
#if CHDB_POCO_OS == CHDB_POCO_OS_LINUX || CHDB_POCO_OS == CHDB_POCO_OS_ANDROID || CHDB_POCO_OS == CHDB_POCO_OS_MAC_OS_X || CHDB_POCO_OS == CHDB_POCO_OS_QNX
#	include <time.h>
#endif
...
{
#if defined(CHDB_POCO_THREAD_PRIORITY_MIN)
	return CHDB_POCO_THREAD_PRIORITY_MIN;
#else
...
{
#if defined(CHDB_POCO_THREAD_PRIORITY_MAX)
	return CHDB_POCO_THREAD_PRIORITY_MAX;
#else
...
{
#if   CHDB_POCO_OS == CHDB_POCO_OS_LINUX || CHDB_POCO_OS == CHDB_POCO_OS_ANDROID || CHDB_POCO_OS == CHDB_POCO_OS_MAC_OS_X || CHDB_POCO_OS == CHDB_POCO_OS_QNX || CHDB_POCO_OS == CHDB_POCO_OS_VXWORKS
	CHDBPoco::Timespan remainingTime(1000*CHDBPoco::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	{
		struct timespec ts;
		...
		if (rc < 0 && errno == EINTR)
		{
			...
			CHDBPoco::Timespan waited = start.elapsed();
	...
	if (rc < 0 && remainingTime > 0) throw CHDBPoco::SystemException("Thread::sleep(): nanosleep() failed");
#else
	CHDBPoco::Timespan remainingTime(1000*CHDBPoco::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	...
	while (remainingTime > 0 && rc < 0 && errno == EINTR);
	if (rc < 0 && remainingTime > 0) throw CHDBPoco::SystemException("Thread::sleep(): select() failed");
#endif
}
...
int ThreadImpl::mapPrio(int prio, int policy)
{
	...
	case PRIO_HIGHEST_IMPL:
		return pmax;
	default:
		CHDB_poco_bugcheck_msg("invalid thread priority");
	}
