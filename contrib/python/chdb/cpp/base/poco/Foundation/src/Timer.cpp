...
Timer::Timer(long startInterval, long periodicInterval): 
	...
	_pCallback(0)
{
	CHDB_poco_assert (startInterval >= 0 && periodicInterval >= 0);
}
...
void Timer::start(const AbstractTimerCallback& method, Thread::Priority priority, ThreadPool& threadPool)
{
	...
	if (_pCallback)
	{
		throw CHDBPoco::IllegalStateException("Timer already running");
	}
...
{
	CHDB_poco_assert (milliseconds >= 0);
	FastMutex::ScopedLock lock(_mutex);
	if (_pCallback)
	{
...
void Timer::run()
{
	...
	do
	{
		...
		else
		{
			...
			{
				CHDBPoco::ErrorHandler::handle(exc);
			}
