...
{
	static CHDBPoco::UInt32 offsetsFromUTF8[6] = {
...
std::string UTF8::unescape(const std::string::const_iterator& begin, const std::string::const_iterator& end)
{
	...
	while (it != end)
	{
		CHDBPoco::UInt32 ch = (CHDBPoco::UInt32) *it++;
		...
		if (ch == '\\')
		{
			...
			else if (*it == 'u')
			{
				...
				if( ch >= 0xD800 && ch <= 0xDBFF )
				{
					...
					if (dno > 0)
					{
						CHDBPoco::UInt32 temp = std::strtol(digs, NULL, 16);
