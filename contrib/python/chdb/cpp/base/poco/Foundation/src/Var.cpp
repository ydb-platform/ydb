...
const Var Var::operator + (const Var& other) const
{
	...
	{
		if (isSigned())
			return add<CHDBPoco::Int64>(other);
		else
			return add<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator += (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = add<CHDBPoco::Int64>(other);
		else
			return *this = add<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator - (const Var& other) const
{
	...
	{
		if (isSigned())
			return subtract<CHDBPoco::Int64>(other);
		else
			return subtract<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator -= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = subtract<CHDBPoco::Int64>(other);
		else
			return *this = subtract<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator * (const Var& other) const
{
	...
	{
		if (isSigned())
			return multiply<CHDBPoco::Int64>(other);
		else
			return multiply<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator *= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = multiply<CHDBPoco::Int64>(other);
		else
			return *this = multiply<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator / (const Var& other) const
{
	...
	{
		if (isSigned())
			return divide<CHDBPoco::Int64>(other);
		else
			return divide<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator /= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = divide<CHDBPoco::Int64>(other);
		else
			return *this = divide<CHDBPoco::UInt64>(other);
	}
	else if (isNumeric())
		...
		default:
			{
				...
				{
					const CHDBPoco::Int64 number = NumberParser::parse64(str);
					return number;
				}
				else if (frac == 0 && isNumber && !isSigned)
				{
					const CHDBPoco::UInt64 number = NumberParser::parseUnsigned64(str);
					return number;
				}
...
{
	CHDB_poco_assert_dbg (pos < val.size() && val[pos] == '{');
	++pos;
	skipWhiteSpace(val, pos);
...
{
	CHDB_poco_assert_dbg (pos < val.size());
	if (val[pos] == '"')
	{
		return parseJSONString(val, pos);
	}
	else
	{
		std::string result;
		while (pos < val.size() 
			&& !CHDBPoco::Ascii::isSpace(val[pos]) 
			&& val[pos] != ','
			&& val[pos] != ']'
			&& val[pos] != '}')
		{
...
std::string Var::parseJSONString(const std::string& val, std::string::size_type& pos)
{
	...
	if (!done) throw CHDBPoco::DataFormatException("unterminated JSON string");
	return result;
}
