...
Array::Array(int options): _modified(false),
	_escapeUnicode((options & CHDBPoco::JSON_ESCAPE_UNICODE) != 0)
{
}
...
{
	if (!_pArray)
		_pArray = new CHDBPoco::Dynamic::Array;
	else
		_pArray->clear();
}


Array::operator const CHDBPoco::Dynamic::Array& () const
{
	if (!_values.size())
	{
		resetDynArray();
	...
	else if (_modified)
	{
		...
		for (; it != end; ++it, ++index)
		{
			if (isObject(it))
			{
				_pArray->insert(_pArray->end(), CHDBPoco::JSON::Object::makeStruct(getObject(index)));
			}
			else if (isArray(it))
			{
...
CHDBPoco::Dynamic::Array Array::makeArray(const JSON::Array::Ptr& arr)
{
	CHDBPoco::Dynamic::Array vec;
	...
	for (; it != end; ++it, ++index)
	{
		if (arr->isObject(it))
		{
			...
			DynamicStruct str = CHDBPoco::JSON::Object::makeStruct(pObj);
			vec.insert(vec.end(), str);
		}
		else if (arr->isArray(it))
		{
			Array::Ptr pArr = arr->getArray(index);
			std::vector<CHDBPoco::Dynamic::Var> v = makeArray(pArr);
