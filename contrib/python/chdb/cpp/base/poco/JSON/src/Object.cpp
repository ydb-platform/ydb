...
Object::Object(int options):
	_preserveInsOrder((options & CHDBPoco::JSON_PRESERVE_KEY_ORDER) != 0),
...
void Object::syncKeys(const KeyList& keys)
{
	if(_preserveInsOrder)
	{
		...
		for(KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it)
		{
			...
			CHDB_poco_assert (itv != _values.end());
			_keys.push_back(itv);
		}
	}
}
...
CHDBPoco::DynamicStruct Object::makeStruct(const Object::Ptr& obj)
{
	CHDBPoco::DynamicStruct ds;
	...
	for (; it != end; ++it)
	{
		...
		else if (obj->isArray(it))
		{
			...
			std::vector<CHDBPoco::Dynamic::Var> v = CHDBPoco::JSON::Array::makeArray(pArr);
...
{
	if (!_pStruct)
		_pStruct = new CHDBPoco::DynamicStruct;
	else
		_pStruct->clear();
}


Object::operator const CHDBPoco::DynamicStruct& () const
{
	if (!_values.size())
	{
		resetDynStruct();
	...
	else if (_modified)
	{
		...
		for (; it != end; ++it)
		{
			...
			{
				_pStruct->insert(it->first, CHDBPoco::JSON::Array::makeArray(getArray(it->first)));
			}
			else
			{
