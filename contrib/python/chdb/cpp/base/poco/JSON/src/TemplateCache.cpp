...
Template::Ptr TemplateCache::getTemplate(const Path& path)
{
	if (_pLogger)
	{
		CHDB_poco_trace_f1(*_pLogger, "Trying to load %s", path.toString());
	}
	...
	if (_pLogger)
	{
		CHDB_poco_trace_f1(*_pLogger, "Path resolved to %s", templatePathname);
	}
	...
	if (it == _cache.end())
	{
		if (templateFile.exists())
		{
			if (_pLogger)
			{
				CHDB_poco_information_f1(*_pLogger, "Loading template %s", templatePath.toString());
			}
			...
			catch (JSONTemplateException& jte)
			{
				if (_pLogger)
				{
					CHDB_poco_error_f2(*_pLogger, "Template %s contains an error: %s", templatePath.toString(), jte.message());
				}
			}
		}
		else
		{
			if (_pLogger)
			{
				CHDB_poco_error_f1(*_pLogger, "Template file %s doesn't exist", templatePath.toString());
			}
	...
	else
	{
		...
		if (tpl->parseTime() < templateFile.getLastModified())
		{
			if (_pLogger)
			{
				CHDB_poco_information_f1(*_pLogger, "Reloading template %s", templatePath.toString());
			}
...
Path TemplateCache::resolvePath(const Path& path) const
{
	...
	for (std::vector<Path>::const_iterator it = _includePaths.begin(); it != _includePaths.end(); ++it)
	{
		...
		if (templateFile.exists())
		{
			if (_pLogger)
			{
				CHDB_poco_trace_f2(*_pLogger, "%s template file resolved to %s", path.toString(), templatePath.toString());
			}
			return templatePath;
		}
		if (_pLogger)
		{
			CHDB_poco_trace_f1(*_pLogger, "%s doesn't exist", templatePath.toString());
		}
	}
