...
#ifndef CHDB_Net_HTTPCredentials_INCLUDED
#define CHDB_Net_HTTPCredentials_INCLUDED


#include "CHDBPoco/Net/HTTPDigestCredentials.h"
...
namespace Net
{
    ...
    ///     CHDBPoco::Net::HTTPCredentials creds("user", "s3cr3t");
    ///
    /// Second, send the HTTP request with CHDBPoco::Net::HTTPClientSession.
    ///     CHDBPoco::Net::HTTPClientSession session("pocoproject.org");
    ///     CHDBPoco::Net::HTTPRequest request(HTTPRequest::HTTP_GET, "/index.html", HTTPMessage::HTTP_1_1);
    ///     session.sendRequest(request);
    ///     CHDBPoco::Net::HTTPResponse;
    ///     std::istream& istr = session.receiveResponse(response);
    ///
    /// If the server responds with a 401 status, authenticate the
    /// request and resend it:
    ///     if (response.getStatus() == CHDBPoco::Net::HTTPResponse::HTTP_UNAUTHORIZED)
    ///     {
    ...
    public:
        ...
        static void extractCredentials(const CHDBPoco::URI & uri, std::string & username, std::string & password);
...
#endif // CHDB_Net_HTTPCredentials_INCLUDED
