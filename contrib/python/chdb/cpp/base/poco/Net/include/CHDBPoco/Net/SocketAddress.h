...
#ifndef CHDB_Net_SocketAddress_INCLUDED
#define CHDB_Net_SocketAddress_INCLUDED
...
namespace Net
{
    ...
    public:
        ...
        SocketAddress(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.

        explicit SocketAddress(CHDBPoco::UInt16 port);
        /// Creates a SocketAddress with unspecified (wildcard) IP address
        /// and given port number.

        SocketAddress(Family family, CHDBPoco::UInt16 port);
        /// Creates a SocketAddress with unspecified (wildcard) IP address
        /// of the given family, and given port number.

        SocketAddress(const std::string & hostAddress, CHDBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.
        ///
        /// The IP address must either be a domain name, or it must
        /// be in dotted decimal (IPv4) or hex string (IPv6) format.

        SocketAddress(Family family, const std::string & hostAddress, CHDBPoco::UInt16 portNumber);
        /// Creates a SocketAddress from an IP address and given port number.
        ///
        /// The IP address must either be a domain name, or it must
        /// be in dotted decimal (IPv4) or hex string (IPv6) format.
        ...
        SocketAddress(const struct sockaddr * addr, CHDB_poco_socklen_t length);
...
#elif defined(CHDB_POCO_HAVE_IPv6)
    ...
    protected:
        void init(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber);
        void init(const std::string & hostAddress, CHDBPoco::UInt16 portNumber);
        void init(Family family, const std::string & hostAddress, CHDBPoco::UInt16 portNumber);
        void init(Family family, const std::string & address);
        void init(const std::string & hostAndPort);
        CHDBPoco::UInt16 resolveService(const std::string & service);

    private:
        typedef CHDBPoco::Net::Impl::SocketAddressImpl Impl;
        typedef CHDBPoco::AutoPtr<Impl> Ptr;

        Ptr pImpl() const;

        void newIPv4();
        void newIPv4(const sockaddr_in *);
        void newIPv4(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber);

#if defined(CHDB_POCO_HAVE_IPv6)
        void newIPv6(const sockaddr_in6 *);
        void newIPv6(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber);
#endif
    ...
    inline SocketAddress::Ptr SocketAddress::pImpl() const
    {
        if (_pImpl)
            return _pImpl;
        throw CHDBPoco::NullPointerException("Pointer to SocketAddress implementation is NULL.");
    }


    inline void SocketAddress::newIPv4()
    {
        _pImpl = new CHDBPoco::Net::Impl::IPv4SocketAddressImpl;
    }


    inline void SocketAddress::newIPv4(const sockaddr_in * sockAddr)
    {
        _pImpl = new CHDBPoco::Net::Impl::IPv4SocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newIPv4(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber)
    {
        _pImpl = new CHDBPoco::Net::Impl::IPv4SocketAddressImpl(hostAddress.addr(), htons(portNumber));
    }
    ...
    {
        _pImpl = new CHDBPoco::Net::Impl::IPv6SocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newIPv6(const IPAddress & hostAddress, CHDBPoco::UInt16 portNumber)
    {
        _pImpl = new CHDBPoco::Net::Impl::IPv6SocketAddressImpl(hostAddress.addr(), htons(portNumber), hostAddress.scope());
    }
    ...
    {
        _pImpl = new CHDBPoco::Net::Impl::LocalSocketAddressImpl(sockAddr);
    }


    inline void SocketAddress::newLocal(const std::string & path)
    {
        _pImpl = new CHDBPoco::Net::Impl::LocalSocketAddressImpl(path.c_str(), path.size());
    }
...
Net_API CHDBPoco::BinaryWriter & operator<<(CHDBPoco::BinaryWriter & writer, const CHDBPoco::Net::SocketAddress & value);
Net_API CHDBPoco::BinaryReader & operator>>(CHDBPoco::BinaryReader & reader, CHDBPoco::Net::SocketAddress & value);
Net_API std::ostream & operator<<(std::ostream & ostr, const CHDBPoco::Net::SocketAddress & address);


#endif // CHDB_Net_SocketAddress_INCLUDED
