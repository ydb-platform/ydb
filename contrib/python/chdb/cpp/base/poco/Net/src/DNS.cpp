...
typedef CHDBPoco::UInt32 punycode_uint;
...
#if defined(POCO_HAVE_LIBRESOLV)
static CHDBPoco::RWLock resolverLock;
#endif


HostEntry DNS::hostByName(const std::string& hostname, unsigned
#ifdef CHDB_POCO_HAVE_ADDRINFO
						  hintFlags
#endif
						 )
{
#if defined(POCO_HAVE_LIBRESOLV)
	CHDBPoco::ScopedReadRWLock readLock(resolverLock);
#endif
...
{
#if defined(POCO_HAVE_LIBRESOLV)
	CHDBPoco::ScopedWriteRWLock writeLock(resolverLock);
	res_init();
#endif
}
...
std::string DNS::encodeIDNLabel(const std::string& label)
{
	...
	std::vector<CHDBPoco::UInt32> uniLabel;
	CHDBPoco::UTF8Encoding utf8;
	CHDBPoco::TextIterator it(label, utf8);
	CHDBPoco::TextIterator end(label);
	while (it != end)
	{
		int ch = *it;
		if (ch < 0) throw DNSException("Invalid UTF-8 character in IDN label", label);
		if (CHDBPoco::Unicode::isUpper(ch))
		{
			ch = CHDBPoco::Unicode::toLower(ch);
		}
		uniLabel.push_back(static_cast<CHDBPoco::UInt32>(ch));
		++it;
	}
	char buffer[64];
	std::size_t size = 64;
...
std::string DNS::decodeIDNLabel(const std::string& encodedIDN)
{
	...
	if (encodedIDN.compare(0, 4, "xn--") == 0)
	{
		...
		if (rc == punycode_success)
		{
			CHDBPoco::UTF32Encoding utf32;
			CHDBPoco::UTF8Encoding utf8;
			CHDBPoco::TextConverter converter(utf32, utf8);
...
{
#if defined(CHDB_POCO_HAVE_IPv6) || defined(CHDB_POCO_HAVE_ADDRINFO)
	switch (code)
	{
	case EAI_AGAIN:
		throw DNSException("Temporary DNS error while resolving", arg);
...
#endif // CHDB_POCO_HAVE_IPv6 || defined(CHDB_POCO_HAVE_ADDRINFO)
}
