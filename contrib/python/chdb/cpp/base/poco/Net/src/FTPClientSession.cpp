...
#include "CHDBPoco/Net/FTPClientSession.h"
#include "CHDBPoco/Net/SocketAddress.h"
#include "CHDBPoco/Net/SocketStream.h"
#include "CHDBPoco/Net/ServerSocket.h"
...
void FTPClientSession::setTimeout(const CHDBPoco::Timespan& timeout)
{
	if (!isOpen())
...
CHDBPoco::Timespan FTPClientSession::getTimeout() const
{
	return _timeout;
}
...
void FTPClientSession::sendPORT(const SocketAddress& addr)
{
	...
	arg += ',';
	CHDBPoco::UInt16 port = addr.port();
...
void FTPClientSession::parseAddress(const std::string& str, SocketAddress& addr)
{
	...
	if (it != end) ++it;
	std::string host;
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) host += *it++;
	...
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) host += *it++;
	if (it != end && *it == ',') ++it;
	CHDBPoco::UInt16 portHi = 0;
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) { portHi *= 10; portHi += *it++ - '0'; }
	if (it != end && *it == ',') ++it;
	CHDBPoco::UInt16 portLo = 0;
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) { portLo *= 10; portLo += *it++ - '0'; }
...
void FTPClientSession::parseExtAddress(const std::string& str, SocketAddress& addr)
{
	...
	if (it != end && *it == delim) ++it;
	CHDBPoco::UInt16 port = 0;
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) { port *= 10; port += *it++ - '0'; }	
