...
{
	return icompare(header, 0, 5, "Basic") == 0 && (header.size() > 5 ? CHDBPoco::Ascii::isSpace(header[5]) : true);
}


bool HTTPCredentials::isDigestCredentials(const std::string& header)
{
	return icompare(header, 0, 6, "Digest") == 0 && (header.size() > 6 ? CHDBPoco::Ascii::isSpace(header[6]) : true);
}
...
void HTTPCredentials::extractCredentials(const CHDBPoco::URI& uri, std::string& username, std::string& password)
{
