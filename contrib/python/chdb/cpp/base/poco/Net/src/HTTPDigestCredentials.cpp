...
#include "CHDBPoco/StringTokenizer.h"


namespace
{
	std::string digest(CHDBPoco::DigestEngine& engine,
					   const std::string& a,
					   const std::string& b,
					   ...
					   const std::string& f = std::string())
	{
		...
		return CHDBPoco::DigestEngine::digestToHex(engine.digest());
	}

	std::string formatNonceCounter(int counter)
	{
		return CHDBPoco::NumberFormatter::formatHex(counter, 8);
	}
}
...
CHDBPoco::FastMutex HTTPDigestCredentials::_nonceMutex;
...
{
	CHDBPoco::FastMutex::ScopedLock lock(_nonceMutex);
...
void HTTPDigestCredentials::createAuthParams(const HTTPRequest& request, const HTTPAuthenticationParams& responseAuthParams)
{
	...
	{
		CHDBPoco::StringTokenizer tok(qop, ",", CHDBPoco::StringTokenizer::TOK_TRIM);
		bool qopSupported = false;
		for (CHDBPoco::StringTokenizer::Iterator it = tok.begin(); it != tok.end(); ++it)
		{
