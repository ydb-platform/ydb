...
const CHDBPoco::UInt8 ICMPv4PacketImpl::SOURCE_QUENCH_TYPE     = 4;
const CHDBPoco::UInt8 ICMPv4PacketImpl::REDIRECT_MESSAGE_TYPE  = 5;
const UInt8 ICMPv4PacketImpl::TIME_EXCEEDED_TYPE                 = 11;
const CHDBPoco::UInt8 ICMPv4PacketImpl::PARAMETER_PROBLEM_TYPE = 12;
...
void ICMPv4PacketImpl::initPacket()
{
	...
	icp->checksum = 0;
	icp->seq      = ++_seq;
	icp->id       = static_cast<UInt16>(CHDBPoco::Process::id());
...
struct timeval ICMPv4PacketImpl::time(CHDBPoco::UInt8* buffer, int length) const
{
	struct timeval tv;
...
ICMPv4PacketImpl::Header* ICMPv4PacketImpl::header(CHDBPoco::UInt8* buffer, int length) const
{
	CHDB_poco_check_ptr (buffer);
...
CHDBPoco::UInt8* ICMPv4PacketImpl::data(CHDBPoco::UInt8* buffer, int length) const
{
	return ((CHDBPoco::UInt8*) header(buffer, length)) + sizeof(Header);
}


bool ICMPv4PacketImpl::validReplyID(CHDBPoco::UInt8* buffer, int length) const
{
	Header *icp = header(buffer, length);
	return icp && (static_cast<CHDBPoco::UInt16>(Process::id()) == icp->id);
}
...
{
	CHDB_poco_assert (typeId >= ECHO_REPLY && typeId < MESSAGE_TYPE_LENGTH);
