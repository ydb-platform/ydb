...
using CHDBPoco::BinaryReader;
using CHDBPoco::BinaryWriter;
using CHDBPoco::toLower;
using CHDBPoco::trim;
using CHDBPoco::UInt8;
using CHDBPoco::UInt16;
using CHDBPoco::UInt32;
using CHDBPoco::Net::Impl::IPAddressImpl;
using CHDBPoco::Net::Impl::IPv4AddressImpl;
#if defined(CHDB_POCO_HAVE_IPv6)
using CHDBPoco::Net::Impl::IPv6AddressImpl;
#endif
...
IPAddress::IPAddress(Family family)
{
	if (family == IPv4)
		...
		newIPv6();
#endif
	else throw CHDBPoco::InvalidArgumentException("Invalid or unsupported address family passed to IPAddress()");
}
...
IPAddress::IPAddress(const void* addr, CHDB_poco_socklen_t length)
	: _pImpl(0)
{
	if (length == sizeof(struct in_addr))
		...
		newIPv6(addr);
#endif
	else throw CHDBPoco::InvalidArgumentException("Invalid address length passed to IPAddress()");
}


IPAddress::IPAddress(const void* addr, CHDB_poco_socklen_t length, CHDBPoco::UInt32 scope)
{
	if (length == sizeof(struct in_addr))
		newIPv4(addr);
#if defined(CHDB_POCO_HAVE_IPv6)
...
IPAddress::IPAddress(unsigned prefix, Family family)
{
	if (family == IPv4)
	{
		if (prefix <= 32)
			...
			throw CHDBPoco::InvalidArgumentException("Invalid prefix length passed to IPAddress()");
	}
#if defined(CHDB_POCO_HAVE_IPv6)
	else if (family == IPv6)
	{
...
IPAddress& IPAddress::operator = (const IPAddress& addr)
{
	if (&addr != this)
	{
		if (addr.family() == IPAddress::IPv4)
			...
			throw CHDBPoco::InvalidArgumentException("Invalid or unsupported address family");
	}
	return *this;
}
...
CHDBPoco::UInt32 IPAddress::scope() const
{
	return pImpl()->scope();
}
...
{
	CHDB_poco_socklen_t l1 = length();
	CHDB_poco_socklen_t l2 = a.length();
	if (l1 == l2)
    {
#if defined(CHDB_POCO_HAVE_IPv6)
        if ( scope() != a.scope() )
...
CHDB_poco_socklen_t IPAddress::length() const
{
	return pImpl()->length();
}
...
CHDBPoco::BinaryWriter& operator << (CHDBPoco::BinaryWriter& writer, const CHDBPoco::Net::IPAddress& value)
{
	writer << static_cast<CHDBPoco::UInt8>(value.length());
	writer.writeRaw(reinterpret_cast<const char*>(value.addr()), value.length());
	return writer;
}


CHDBPoco::BinaryReader& operator >> (CHDBPoco::BinaryReader& reader, CHDBPoco::Net::IPAddress& value)
{
	char buf[CHDBPoco::Net::IPAddress::MAX_ADDRESS_LENGTH];
	CHDBPoco::UInt8 length;
	reader >> length;
	reader.readRaw(buf, length);
	value = CHDBPoco::Net::IPAddress(buf, length);
	return reader;
}


std::ostream& operator << (std::ostream& ostr, const CHDBPoco::Net::IPAddress& addr)
{
	ostr << addr.toString();
	return ostr;
}
