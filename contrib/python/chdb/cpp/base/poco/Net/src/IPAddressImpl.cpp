...
#include "CHDBPoco/Net/IPAddressImpl.h"
...
using CHDBPoco::toLower;
using CHDBPoco::UInt8;
using CHDBPoco::UInt16;
using CHDBPoco::UInt32;
...
CHDB_poco_socklen_t IPv4AddressImpl::length() const
{
	return sizeof(_addr);
}
...
CHDBPoco::UInt32 IPv4AddressImpl::scope() const
{
	return 0;
}
...
{
	CHDB_poco_assert (pMask->af() == AF_INET && pSet->af() == AF_INET);
...
IPv6AddressImpl::IPv6AddressImpl(const void* addr, CHDBPoco::UInt32 scope): _scope(scope)
{
	std::memcpy(&_addr, addr, sizeof(_addr));
}
...
CHDB_poco_socklen_t IPv6AddressImpl::length() const
{
	return sizeof(_addr);
}
...
CHDBPoco::UInt32 IPv6AddressImpl::scope() const
{
	return _scope;
}
...
{
	throw CHDBPoco::NotImplementedException("mask() is only supported for IPv4 addresses");
}
...
IPv6AddressImpl IPv6AddressImpl::operator & (const IPv6AddressImpl& addr) const
{
	if (_scope != addr._scope)
		throw CHDBPoco::InvalidArgumentException("Scope ID of passed IPv6 address does not match with the source one.");
