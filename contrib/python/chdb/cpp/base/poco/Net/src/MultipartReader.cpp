...
#include "CHDBPoco/Net/MultipartReader.h"
...
MultipartStreamBuf::MultipartStreamBuf(std::istream& istr, const std::string& boundary):
	...
	_lastPart(false)
{
	CHDB_poco_assert (!boundary.empty() && boundary.length() < STREAM_BUFFER_SIZE - 6);
}
...
{
	CHDB_poco_assert_dbg (length >= _boundary.length() + 6);
...
void MultipartReader::guessBoundary()
{
	static const int eof = std::char_traits<char>::eof();
	int ch = _istr.get();
	while (CHDBPoco::Ascii::isSpace(ch))
		ch = _istr.get();
	if (ch == '-' && _istr.peek() == '-')
	{
		_istr.get();
