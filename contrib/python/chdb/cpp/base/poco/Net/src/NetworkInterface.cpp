...
#include "CHDBPoco/Net/NetworkInterface.h"


#ifdef CHDB_POCO_NET_HAS_INTERFACE
...
std::ostream& operator << (std::ostream& os, const CHDBPoco::Net::NetworkInterface::MACAddress& mac)
{
	std::ios state(0);
	state.copyfmt(os);
	for (unsigned i = 0; i < mac.size(); ++i)
	{
		if (i > 0) os << CHDBPoco::Net::NetworkInterface::MAC_SEPARATOR;
...
class NetworkInterfaceImpl: public CHDBPoco::RefCountedObject
{
public:
...
{
#if !defined(CHDB_POCO_OS_FAMILY_WINDOWS) && !defined(POCO_VXWORKS)
	struct ifreq ifr;
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
	...
	else throw NotFoundException(CHDBPoco::format("No address with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::subnetMask(unsigned index) const
{
	...
	throw NotFoundException(CHDBPoco::format("No subnet mask with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::destAddress(unsigned index) const
{
	...
	throw NotFoundException(CHDBPoco::format("No address with index %u.", index));
}
...
#if   defined(CHDB_POCO_OS_FAMILY_BSD) || (CHDB_POCO_OS == CHDB_POCO_OS_QNX) || (CHDB_POCO_OS == CHDB_POCO_OS_SOLARIS)
//
...
#elif CHDB_POCO_OS == CHDB_POCO_OS_LINUX || CHDB_POCO_OS == CHDB_POCO_OS_ANDROID
//
// Linux
//
...
/// #include <sys/types.h>
#if CHDB_POCO_OS != CHDB_POCO_OS_ANDROID // Android doesn't have <ifaddrs.h>
/// #include <ifaddrs.h>
#endif
/// #include <net/if.h>
...
void setInterfaceParams(struct ifaddrs* iface, NetworkInterfaceImpl& impl)
{
	...
	if (addrStream.good())
	{
		...
		CHDBPoco::StringTokenizer tok(addr, ":");
		std::vector<unsigned char> mac;
		for (CHDBPoco::StringTokenizer::Iterator it = tok.begin(); it != tok.end(); ++it)
		{
			mac.push_back(static_cast<unsigned char>(CHDBPoco::NumberParser::parseHex(*it)));
		}
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
	...
	return result;
#else
	throw CHDBPoco::NotImplementedException("Not implemented in Android");
#endif
}
...
#endif // CHDB_POCO_NET_HAS_INTERFACE
