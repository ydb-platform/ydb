...
#include "CHDBPoco/Net/OAuth10Credentials.h"
#include "CHDBPoco/Net/HTTPRequest.h"
#include "CHDBPoco/Net/HTMLForm.h"
#include "CHDBPoco/Net/NetException.h"
#include "CHDBPoco/Net/HTTPAuthenticationParams.h"
#include "CHDBPoco/SHA1Engine.h"
#include "CHDBPoco/HMACEngine.h"
#include "CHDBPoco/Base64Encoder.h"
#include "CHDBPoco/RandomStream.h"
...
OAuth10Credentials::OAuth10Credentials(const CHDBPoco::Net::HTTPRequest& request)
{
...
void OAuth10Credentials::authenticate(HTTPRequest& request, const CHDBPoco::URI& uri, SignatureMethod method)
{
...
void OAuth10Credentials::authenticate(HTTPRequest& request, const CHDBPoco::URI& uri, const CHDBPoco::Net::HTMLForm& params, SignatureMethod method)
{
...
bool OAuth10Credentials::verify(const HTTPRequest& request, const CHDBPoco::URI& uri)
{
	HTMLForm params;
...
bool OAuth10Credentials::verify(const HTTPRequest& request, const CHDBPoco::URI& uri, const CHDBPoco::Net::HTMLForm& params)
{
	if (request.hasCredentials())
	{
		std::string authScheme;
...
void OAuth10Credentials::signPlaintext(CHDBPoco::Net::HTTPRequest& request) const
{
	...
	if (!_realm.empty())
	{
		CHDBPoco::format(authorization, " realm=\"%s\",", _realm);
	}
	CHDBPoco::format(authorization, " oauth_consumer_key=\"%s\"", percentEncode(_consumerKey));
	CHDBPoco::format(authorization, ", oauth_signature=\"%s\"", percentEncode(signature));
	authorization += ", oauth_signature_method=\"PLAINTEXT\"";
	if (!_token.empty())
	{
		CHDBPoco::format(authorization, ", oauth_token=\"%s\"", percentEncode(_token));
	}
	if (!_callback.empty())
	{
		CHDBPoco::format(authorization, ", oauth_callback=\"%s\"", percentEncode(_callback));
	}
...
void OAuth10Credentials::signHMACSHA1(CHDBPoco::Net::HTTPRequest& request, const std::string& uri, const CHDBPoco::Net::HTMLForm& params) const
{
	...
	if (timestamp.empty())
	{
		timestamp = CHDBPoco::NumberFormatter::format(CHDBPoco::Timestamp().epochTime());
	}
	...
	CHDBPoco::format(authorization, " oauth_consumer_key=\"%s\"", percentEncode(_consumerKey));
	CHDBPoco::format(authorization, ", oauth_nonce=\"%s\"", percentEncode(nonce));
	CHDBPoco::format(authorization, ", oauth_signature=\"%s\"", percentEncode(signature));
	authorization += ", oauth_signature_method=\"HMAC-SHA1\"";
	CHDBPoco::format(authorization, ", oauth_timestamp=\"%s\"", timestamp);
	if (!_token.empty())
	{
		CHDBPoco::format(authorization, ", oauth_token=\"%s\"", percentEncode(_token));
	}
...
std::string OAuth10Credentials::createNonce() const
{
	...
	CHDBPoco::Base64Encoder base64Encoder(base64Nonce);
	CHDBPoco::RandomInputStream randomStream;
	for (int i = 0; i < 32; i++)
	{
	...
	return CHDBPoco::translate(nonce, "+/=", "");
}


std::string OAuth10Credentials::createSignature(const CHDBPoco::Net::HTTPRequest& request, const std::string& uri, const CHDBPoco::Net::HTMLForm& params, const std::string& nonce, const std::string& timestamp) const
{
	...
	for (CHDBPoco::Net::HTMLForm::ConstIterator it = params.begin(); it != params.end(); ++it)
	{
	...
	CHDBPoco::HMACEngine<CHDBPoco::SHA1Engine> hmacEngine(signingKey);
	hmacEngine.update(signatureBase);
	CHDBPoco::DigestEngine::Digest digest = hmacEngine.digest();
	std::ostringstream digestBase64;
	CHDBPoco::Base64Encoder base64Encoder(digestBase64);
...
{
	std::string encoded;
	CHDBPoco::URI::encode(str, "!?#/'\",;:$&()[]*+=@", encoded);
	return encoded;
}
