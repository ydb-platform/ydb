...
#include "CHDBPoco/Net/POP3ClientSession.h"
...
class DialogStreamBuf: public CHDBPoco::UnbufferedStreamBuf
{
public:
...
POP3ClientSession::POP3ClientSession(const std::string& host, CHDBPoco::UInt16 port):	
...
void POP3ClientSession::setTimeout(const CHDBPoco::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}

	
CHDBPoco::Timespan POP3ClientSession::getTimeout() const
{
...
int POP3ClientSession::messageCount()
{
	...
	while (it != end && CHDBPoco::Ascii::isSpace(*it)) ++it;
	while (it != end && CHDBPoco::Ascii::isDigit(*it)) count = count*10 + *it++ - '0';
	return count;
}
...
void POP3ClientSession::listMessages(MessageInfoVec& messages)
{
	...
	while (response != ".")
	{
		...
		std::string::const_iterator it  = response.begin();
		std::string::const_iterator end = response.end();
		while (it != end && CHDBPoco::Ascii::isDigit(*it)) info.id = info.id*10 + *it++ - '0';
		while (it != end && CHDBPoco::Ascii::isSpace(*it)) ++it;
		while (it != end && CHDBPoco::Ascii::isDigit(*it)) info.size = info.size*10 + *it++ - '0';
