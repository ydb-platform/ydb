...
#include "CHDBPoco/Net/PollSet.h"
...
public:
	...
	void add(const Socket& socket, int mode)
	{
		...
		CHDB_poco_socket_t fd = sockImpl->sockfd();
	...
	void remove(const Socket& socket)
	{
		...
		CHDB_poco_socket_t fd = socket.impl()->sockfd();
		struct epoll_event ev;
		ev.events = 0;
		ev.data.ptr = 0;
	...
	PollSet::SocketModeMap poll(const CHDBPoco::Timespan& timeout)
	{
...
public:
	...
	PollSet::SocketModeMap poll(const CHDBPoco::Timespan& timeout)
	{
		PollSet::SocketModeMap result;
		{
			...
			for (std::map<CHDB_poco_socket_t, int>::iterator it = _addMap.begin(); it != _addMap.end(); ++it)
			{
				pollfd pfd;
				pfd.fd = it->first;
				pfd.events = 0;
...

		{
			...
			if (!_socketMap.empty())
			{
				for (std::vector<pollfd>::iterator it = _pollfds.begin(); it != _pollfds.end(); ++it)
				{
					std::map<CHDB_poco_socket_t, Socket>::const_iterator its = _socketMap.find(it->fd);
...
private:
	CHDBPoco::FastMutex _mutex;
	std::map<CHDB_poco_socket_t, Socket> _socketMap;
	std::map<CHDB_poco_socket_t, int> _addMap;
	std::set<CHDB_poco_socket_t> _removeSet;
...
public:
	...
	PollSet::SocketModeMap poll(const CHDBPoco::Timespan& timeout)
	{
...

		{
			...
			{
				CHDB_poco_socket_t fd = it->first.impl()->sockfd();
				if (fd != CHDB_POCO_INVALID_SOCKET && it->second)
				{
...
PollSet::SocketModeMap PollSet::poll(const CHDBPoco::Timespan& timeout)
{
