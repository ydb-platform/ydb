...
void RemoteSyslogChannel::log(const Message& msg)
{
	...
	m.reserve(1024);
	m += '<';
	CHDBPoco::NumberFormatter::append(m, getPrio(msg) + _facility);
	m += '>';
	if (_bsdFormat)
	{
		CHDBPoco::DateTimeFormatter::append(m, msg.getTime(), BSD_TIMEFORMAT);
		m += ' ';
		m += _host;
	}
	else
	{
		m += "1 "; // version
		CHDBPoco::DateTimeFormatter::append(m, msg.getTime(), SYSLOG_TIMEFORMAT);
		m += ' ';
		m += _host;
		m += ' ';
		m += _name;
		m += ' ';
		CHDBPoco::NumberFormatter::append(m, msg.getPid());
		m += ' ';
		m += msg.getSource();
		m += ' ';
...
void RemoteSyslogChannel::setProperty(const std::string& name, const std::string& value)
{
	...
	{
		std::string facility;
		if (CHDBPoco::icompare(value, 4, "LOG_") == 0)
			facility = CHDBPoco::toUpper(value.substr(4));
		else if (CHDBPoco::icompare(value, 4, "SYSLOG_") == 0)
			facility = CHDBPoco::toUpper(value.substr(7));
		else
			facility = CHDBPoco::toUpper(value);
...
{
	CHDBPoco::LoggingFactory::defaultFactory().registerChannelClass("RemoteSyslogChannel", new CHDBPoco::Instantiator<RemoteSyslogChannel, CHDBPoco::Channel>);
}
