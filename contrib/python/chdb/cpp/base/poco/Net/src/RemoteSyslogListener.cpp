...
#include "CHDBPoco/Net/RemoteSyslogListener.h"
#include "CHDBPoco/Net/RemoteSyslogChannel.h"
...
#include "CHDBPoco/DateTimeParser.h"
...
class MessageNotification: public CHDBPoco::Notification
{
public:
	MessageNotification(const char* buffer, std::size_t length, const CHDBPoco::Net::SocketAddress& sourceAddress):
		_message(buffer, length),
		_sourceAddress(sourceAddress)
	{
	}

	MessageNotification(const std::string& message, const CHDBPoco::Net::SocketAddress& sourceAddress):
		_message(message),
		_sourceAddress(sourceAddress)
	{
	}
	...
	const CHDBPoco::Net::SocketAddress& sourceAddress() const
	{
...
private:
	std::string _message;
	CHDBPoco::Net::SocketAddress _sourceAddress;
};
...
class RemoteUDPListener: public CHDBPoco::Runnable
{
public:
	enum
	{
	...
	RemoteUDPListener(CHDBPoco::NotificationQueue& queue, CHDBPoco::UInt16 port);
	~RemoteUDPListener();
...
RemoteUDPListener::RemoteUDPListener(CHDBPoco::NotificationQueue& queue, CHDBPoco::UInt16 port):
	_queue(queue),
	_socket(CHDBPoco::Net::SocketAddress(CHDBPoco::Net::IPAddress(), port)),
	_stopped(false)
{
}
...
{
	CHDBPoco::Buffer<char> buffer(BUFFER_SIZE);
	CHDBPoco::Timespan waitTime(WAITTIME_MILLISEC* 1000);
	while (!_stopped)
	{
		try
		{
			...
			{
				CHDBPoco::Net::SocketAddress sourceAddress;
...
class SyslogParser: public CHDBPoco::Runnable
{
public:
	...
	SyslogParser(CHDBPoco::NotificationQueue& queue, RemoteSyslogListener* pListener);
	~SyslogParser();

	void parse(const std::string& line, CHDBPoco::Message& message);
	void run();
	void safeStop();

	static CHDBPoco::Message::Priority convert(RemoteSyslogChannel::Severity severity);
...
private:
	...
	void parseNew(const std::string& line, RemoteSyslogChannel::Severity severity, RemoteSyslogChannel::Facility fac, std::size_t& pos, CHDBPoco::Message& message);
	void parseBSD(const std::string& line, RemoteSyslogChannel::Severity severity, RemoteSyslogChannel::Facility fac, std::size_t& pos, CHDBPoco::Message& message);
...
SyslogParser::SyslogParser(CHDBPoco::NotificationQueue& queue, RemoteSyslogListener* pListener):
	_queue(queue),
	_stopped(false),
	_pListener(pListener)
{
	CHDB_poco_check_ptr (_pListener);
}
...
void SyslogParser::run()
{
	while (!_stopped)
	{
		try
		{
			CHDBPoco::AutoPtr<CHDBPoco::Notification> pNf(_queue.waitDequeueNotification(WAITTIME_MILLISEC));
			if (pNf)
			{
				CHDBPoco::AutoPtr<MessageNotification> pMsgNf = pNf.cast<MessageNotification>();
				CHDBPoco::Message message;
...
void SyslogParser::parse(const std::string& line, CHDBPoco::Message& message)
{
	...
	if (CHDBPoco::Ascii::isDigit(line[pos]))
	{
	...
	CHDB_poco_assert (pos == line.size());
}
...
{
	CHDB_poco_assert (pos < line.size());
	CHDB_poco_assert (line[pos] == '<');
	++pos;
	std::size_t start = pos;
	
	while (pos < line.size() && CHDBPoco::Ascii::isDigit(line[pos]))
		++pos;
	
	CHDB_poco_assert (line[pos] == '>');
	CHDB_poco_assert (pos - start > 0);
	std::string valStr = line.substr(start, pos - start);
	++pos; // skip the >

	int val = CHDBPoco::NumberParser::parse(valStr);
	CHDB_poco_assert (val >= 0 && val <= (RemoteSyslogChannel::SYSLOG_LOCAL7 + RemoteSyslogChannel::SYSLOG_DEBUG));
	
	CHDBPoco::UInt16 pri = static_cast<CHDBPoco::UInt16>(val);
...
void SyslogParser::parseNew(const std::string& line, RemoteSyslogChannel::Severity severity, RemoteSyslogChannel::Facility fac, std::size_t& pos, CHDBPoco::Message& message)
{
	CHDBPoco::Message::Priority prio = convert(severity);
	...
	pos = line.size();
	CHDBPoco::DateTime date;
	int tzd = 0;
	bool hasDate = CHDBPoco::DateTimeParser::tryParse(RemoteSyslogChannel::SYSLOG_TIMEFORMAT, timeStr, date, tzd);
	CHDBPoco::Message logEntry(msgId, messageText, prio);
	...
	int lval(0);
	CHDBPoco::NumberParser::tryParse(procId, lval);
	logEntry.setPid(lval);
	message.swap(logEntry);
}


void SyslogParser::parseBSD(const std::string& line, RemoteSyslogChannel::Severity severity, RemoteSyslogChannel::Facility fac, std::size_t& pos, CHDBPoco::Message& message)
{
	CHDBPoco::Message::Priority prio = convert(severity);
	// rest of the unparsed header is:
		...
		if (line[pos] == ' ')
		{
			...
			if (spaceCnt == 1)
			{
				...
				if (pos - start != 3)
				{
					...
					// assume hostName
					CHDBPoco::Message logEntry(line.substr(start, pos-start), line.substr(pos+1), prio);
					message.swap(logEntry);
					return;
				}
			}
			...
			{
				// a day value!
				if (!(CHDBPoco::Ascii::isDigit(line[pos-1]) && (CHDBPoco::Ascii::isDigit(line[pos-2]) || CHDBPoco::Ascii::isSpace(line[pos-2]))))
				{
	...
	bool hasDate = CHDBPoco::DateTimeParser::tryParse(RemoteSyslogChannel::BSD_TIMEFORMAT, timeStr, date, tzd);
	if (hasDate)
	{
		int m = date.month();
		int d = date.day();
		...
		int min = date.minute();
		int sec = date.second();
		date = CHDBPoco::DateTime(year, m, d, h, min, sec);
	}
	...
	std::string messageText(line.substr(pos));
	pos = line.size();
	CHDBPoco::Message logEntry(hostName, messageText, prio);
	logEntry.setTime(date.timestamp());
	message.swap(logEntry);
}
...
{
	std::size_t start = pos;
	while (pos < line.size() && !CHDBPoco::Ascii::isSpace(line[pos]))
		++pos;
	// skip space
	++pos;
...
std::string SyslogParser::parseStructuredData(const std::string& line, std::size_t& pos)
{
	...
	if (pos < line.size())
	{
		...
		if (pos < line.size() && CHDBPoco::Ascii::isSpace(line[pos])) ++pos;
	}
	return sd;
}
...
std::string SyslogParser::parseStructuredDataToken(const std::string& line, std::size_t& pos)
{
	...
	if (pos < line.size())
	{
		if (CHDBPoco::Ascii::isSpace(line[pos]) || line[pos] == '=' || line[pos] == '[' || line[pos] == ']')
		{
			tok += line[pos++];
		}
		else if (line[pos] == '"')
		...
		{
			while (pos < line.size() && !CHDBPoco::Ascii::isSpace(line[pos]) && line[pos] != '=')
			{
				tok += line[pos++];
			}
		}
...
CHDBPoco::Message::Priority SyslogParser::convert(RemoteSyslogChannel::Severity severity)
{
	switch (severity)
	{
	case RemoteSyslogChannel::SYSLOG_EMERGENCY:
		return CHDBPoco::Message::PRIO_FATAL;
	case RemoteSyslogChannel::SYSLOG_ALERT:
		return CHDBPoco::Message::PRIO_FATAL;
	case RemoteSyslogChannel::SYSLOG_CRITICAL:
		return CHDBPoco::Message::PRIO_CRITICAL;
	case RemoteSyslogChannel::SYSLOG_ERROR:
		return CHDBPoco::Message::PRIO_ERROR;
	case RemoteSyslogChannel::SYSLOG_WARNING:
		return CHDBPoco::Message::PRIO_WARNING;
	case RemoteSyslogChannel::SYSLOG_NOTICE:
		return CHDBPoco::Message::PRIO_NOTICE;
	case RemoteSyslogChannel::SYSLOG_INFORMATIONAL:
		return CHDBPoco::Message::PRIO_INFORMATION;
	case RemoteSyslogChannel::SYSLOG_DEBUG:
		return CHDBPoco::Message::PRIO_DEBUG;
	}
	throw CHDBPoco::LogicException("Illegal severity value in message");
}
...
RemoteSyslogListener::RemoteSyslogListener(CHDBPoco::UInt16 port):
	_pListener(0),
	_pParser(0),
	_port(port),
	_threads(1)
...
RemoteSyslogListener::RemoteSyslogListener(CHDBPoco::UInt16 port, int threads):
	_pListener(0),
	_pParser(0),
	_port(port),
	_threads(threads)
...
void RemoteSyslogListener::enqueueMessage(const std::string& messageText, const CHDBPoco::Net::SocketAddress& senderAddress)
{
...
void RemoteSyslogListener::setProperty(const std::string& name, const std::string& value)
{
	if (name == PROP_PORT)
	{
		int val = CHDBPoco::NumberParser::parse(value);
		if (val >= 0 && val < 65536)
			_port = static_cast<CHDBPoco::UInt16>(val);
		else
			throw CHDBPoco::InvalidArgumentException("Not a valid port number", value);
	}
	else if (name == PROP_THREADS)
	{
		...
		if (val > 0 && val < 16)
			...
			throw CHDBPoco::InvalidArgumentException("Invalid number of threads", value);
	}
	else 
	{
...
{
	if (name == PROP_PORT)
		return CHDBPoco::NumberFormatter::format(_port);
	else if (name == PROP_THREADS)
		return CHDBPoco::NumberFormatter::format(_threads);
	else	
...
{
	CHDBPoco::LoggingFactory::defaultFactory().registerChannelClass("RemoteSyslogListener", new CHDBPoco::Instantiator<RemoteSyslogListener, CHDBPoco::Channel>);
}
