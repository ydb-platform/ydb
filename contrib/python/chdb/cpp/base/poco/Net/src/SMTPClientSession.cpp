...
#include "CHDBPoco/Net/SMTPClientSession.h"
...
using CHDBPoco::DigestEngine;
using CHDBPoco::HMACEngine;
using CHDBPoco::MD5Engine;
using CHDBPoco::SHA1Engine;
using CHDBPoco::DigestOutputStream;
...
SMTPClientSession::SMTPClientSession(const std::string& host, CHDBPoco::UInt16 port):
...
void SMTPClientSession::setTimeout(const CHDBPoco::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}

	
CHDBPoco::Timespan SMTPClientSession::getTimeout() const
{
...
void SMTPClientSession::loginUsingCRAM(const std::string& username, const std::string& method, CHDBPoco::DigestEngine& hmac)
{
	std::string response;
...
void SMTPClientSession::loginUsingLogin(const std::string& username, const std::string& password)
{
	...
	if (CHDBPoco::icompare(decodedResponse, 0, 8, "username") == 0) // username first (md5("Username:"))
	{
	...
	else if  (CHDBPoco::icompare(decodedResponse, 0, 8, "password") == 0) // password first (md5("Password:"))
	{
		status = sendCommand(passwordBase64.str(), response);
