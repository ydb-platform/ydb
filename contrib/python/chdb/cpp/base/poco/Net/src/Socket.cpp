...
#include <sys/epoll.h>
#elif defined(CHDB_POCO_HAVE_FD_POLL)
...
int Socket::select(SocketList& readList, SocketList& writeList, SocketList& exceptList, const CHDBPoco::Timespan& timeout)
{
#if defined(POCO_HAVE_FD_EPOLL)
	...
	int epollfd = -1;
	{
		...
		{
			CHDB_poco_socket_t sockfd = it->sockfd();
			if (sockfd != CHDB_POCO_INVALID_SOCKET)
			{
		...
		{
			CHDB_poco_socket_t sockfd = reinterpret_cast<Socket*>(e->data.ptr)->sockfd();
			if (sockfd != CHDB_POCO_INVALID_SOCKET)
			{
	...
	do
	{
		...
		if (rc < 0 && SocketImpl::lastError() == CHDB_POCO_EINTR)
		{
	...
	while (rc < 0 && SocketImpl::lastError() == CHDB_POCO_EINTR);
...
#elif defined(CHDB_POCO_HAVE_FD_POLL)
	typedef CHDBPoco::SharedPtr<pollfd, CHDBPoco::ReferenceCounter, CHDBPoco::ReleaseArrayPolicy<pollfd> > SharedPollArray;
	...
	{
		CHDB_poco_socket_t fd = it->sockfd();
		if (fd != CHDB_POCO_INVALID_SOCKET)
		{
			if (int(fd) > nfd)
				nfd = int(fd);
			FD_SET(fd, &fdRead);
