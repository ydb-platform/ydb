...
#include "CHDBPoco/Net/SocketReactor.h"
#include "CHDBPoco/Net/SocketNotification.h"
#include "CHDBPoco/Net/SocketNotifier.h"
...
SocketReactor::SocketReactor(const CHDBPoco::Timespan& timeout):
	_stop(false),
	_timeout(timeout),
	_pReadableNotification(new ReadableNotification(this)),
	_pWritableNotification(new WritableNotification(this)),
...
void SocketReactor::setTimeout(const CHDBPoco::Timespan& timeout)
{
	_timeout = timeout;
}


const CHDBPoco::Timespan& SocketReactor::getTimeout() const
{
	return _timeout;
}


void SocketReactor::addEventHandler(const Socket& socket, const CHDBPoco::AbstractObserver& observer)
{
...
bool SocketReactor::hasEventHandler(const Socket& socket, const CHDBPoco::AbstractObserver& observer)
{
	NotifierPtr pNotifier;
	{
		FastMutex::ScopedLock lock(_mutex);
...
void SocketReactor::removeEventHandler(const Socket& socket, const CHDBPoco::AbstractObserver& observer)
{
	NotifierPtr pNotifier;
	{
		FastMutex::ScopedLock lock(_mutex);
