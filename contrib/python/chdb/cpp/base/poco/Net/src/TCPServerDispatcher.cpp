...
TCPServerDispatcher::TCPServerDispatcher(TCPServerConnectionFactory::Ptr pFactory, CHDBPoco::ThreadPool& threadPool, TCPServerParams::Ptr pParams):
    _rc(1),
    _pParams(pParams),
    _currentThreads(0),
    _totalConnections(0),
...
void TCPServerDispatcher::run()
{
    ...
    for (;;)
    {
            ...
            if (pNf && !_stopped)
            {
                ...
                if (pCNf)
                {
                    ...
                    if (!_stopped)
                    {
                        ...
                        CHDB_poco_check_ptr(pConnection.get());
                        pConnection->start();
                    }
        ...
        catch (CHDBPoco::Exception &exc) { ErrorHandler::handle(exc); }
