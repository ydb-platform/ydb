...
#include "CHDBPoco/Net/WebSocket.h"
#include "CHDBPoco/Net/WebSocketImpl.h"
#include "CHDBPoco/Net/HTTPServerRequestImpl.h"
#include "CHDBPoco/Net/HTTPServerResponse.h"
...
void WebSocket::shutdown(CHDBPoco::UInt16 statusCode, const std::string& statusMessage)
{
	CHDBPoco::Buffer<char> buffer(statusMessage.size() + 2);
	CHDBPoco::MemoryOutputStream ostr(buffer.begin(), buffer.size());
	CHDBPoco::BinaryWriter writer(ostr, CHDBPoco::BinaryWriter::NETWORK_BYTE_ORDER);
	writer << statusCode;
...
int WebSocket::receiveFrame(CHDBPoco::Buffer<char>& buffer, int& flags)
{
...
WebSocketImpl* WebSocket::connect(HTTPClientSession& cs, HTTPRequest& request, HTTPResponse& response, HTTPCredentials& credentials)
{
	...
	else if (response.getStatus() == HTTPResponse::HTTP_UNAUTHORIZED)
	{
		if (!credentials.empty())
		{
			...
			CHDBPoco::StreamCopier::copyStream(istr, null);
...
WebSocketImpl* WebSocket::completeHandshake(HTTPClientSession& cs, HTTPResponse& response, const std::string& key)
{
	...
	if (CHDBPoco::icompare(connection, "Upgrade") != 0)
	...
	if (CHDBPoco::icompare(upgrade, "websocket") != 0)
...
std::string WebSocket::createKey()
{
	...
	CHDBPoco::Base64Encoder base64(ostr);
	CHDBPoco::BinaryWriter writer(base64);
...
std::string WebSocket::computeAccept(const std::string& key)
{
	...
	CHDBPoco::SHA1Engine sha1;
	sha1.update(accept);
	CHDBPoco::DigestEngine::Digest d = sha1.digest();
	std::ostringstream ostr;
	CHDBPoco::Base64Encoder base64(ostr);
