...
{
	CHDB_poco_check_ptr(pStreamSocketImpl);
...
{
	CHDBPoco::Buffer<char> frame(length + MAX_HEADER_LENGTH);
	CHDBPoco::MemoryOutputStream ostr(frame.begin(), frame.size());
	CHDBPoco::BinaryWriter writer(ostr, CHDBPoco::BinaryWriter::NETWORK_BYTE_ORDER);

	if (flags == 0) flags = WebSocket::FRAME_BINARY;
	flags &= 0xff;
	writer << static_cast<CHDBPoco::UInt8>(flags);
	CHDBPoco::UInt8 lengthByte(0);
	if (_mustMaskPayload)
	{
	...
	if (length < 126)
	{
		lengthByte |= static_cast<CHDBPoco::UInt8>(length);
		writer << lengthByte;
	}
	else if (length < 65536)
	{
		lengthByte |= 126;
		writer << lengthByte << static_cast<CHDBPoco::UInt16>(length);
	}
	else
	{
		lengthByte |= 127;
		writer << lengthByte << static_cast<CHDBPoco::UInt64>(length);
	}
	if (_mustMaskPayload)
	{
		const CHDBPoco::UInt32 mask = _rnd.next();
...
int WebSocketImpl::receiveHeader(char mask[4], bool& useMask)
{
	...
	CHDB_poco_assert (n == 2);
	CHDBPoco::UInt8 flags = static_cast<CHDBPoco::UInt8>(header[0]);
	_frameFlags = flags;
	CHDBPoco::UInt8 lengthByte = static_cast<CHDBPoco::UInt8>(header[1]);
	...
	if (lengthByte == 127)
	{
		...
		CHDBPoco::MemoryInputStream istr(header + 2, 8);
		CHDBPoco::BinaryReader reader(istr, CHDBPoco::BinaryReader::NETWORK_BYTE_ORDER);
		CHDBPoco::UInt64 l;
		reader >> l;
	...
	else if (lengthByte == 126)
	{
		...
		CHDBPoco::MemoryInputStream istr(header + 2, 2);
		CHDBPoco::BinaryReader reader(istr, CHDBPoco::BinaryReader::NETWORK_BYTE_ORDER);
		CHDBPoco::UInt16 l;
		reader >> l;
		if (l > _maxPayloadSize) throw WebSocketException("Payload too big", WebSocket::WS_ERR_PAYLOAD_TOO_BIG);
...
{
	CHDB_poco_assert (maxPayloadSize > 0);
...
int WebSocketImpl::receiveBytes(void* buffer, int length, int)
{
	...
	if (payloadLength > length)
		throw WebSocketException(CHDBPoco::format("Insufficient buffer for payload size %d", payloadLength), WebSocket::WS_ERR_PAYLOAD_TOO_BIG);
	return receivePayload(reinterpret_cast<char*>(buffer), payloadLength, mask, useMask);
}


int WebSocketImpl::receiveBytes(CHDBPoco::Buffer<char>& buffer, int)
{
	char mask[4];
	bool useMask;
	int payloadLength = receiveHeader(mask, useMask);
...
{
	throw CHDBPoco::InvalidAccessException("Cannot acceptConnection() on a WebSocketImpl");
}


void WebSocketImpl::connect(const SocketAddress& address)
{
	throw CHDBPoco::InvalidAccessException("Cannot connect() a WebSocketImpl");
}


void WebSocketImpl::connect(const SocketAddress& address, const CHDBPoco::Timespan& timeout)
{
	throw CHDBPoco::InvalidAccessException("Cannot connect() a WebSocketImpl");
}
...
{
	throw CHDBPoco::InvalidAccessException("Cannot connectNB() a WebSocketImpl");
}


void WebSocketImpl::bind(const SocketAddress& address, bool reuseAddress)
{
	throw CHDBPoco::InvalidAccessException("Cannot bind() a WebSocketImpl");
}
...
{
	throw CHDBPoco::InvalidAccessException("Cannot bind6() a WebSocketImpl");
}
...
{
	throw CHDBPoco::InvalidAccessException("Cannot listen() on a WebSocketImpl");
}
...
{
	throw CHDBPoco::InvalidAccessException("Cannot sendTo() on a WebSocketImpl");
}


int WebSocketImpl::receiveFrom(void* buffer, int length, SocketAddress& address, int flags)
{
	throw CHDBPoco::InvalidAccessException("Cannot receiveFrom() on a WebSocketImpl");
}


void WebSocketImpl::sendUrgent(unsigned char data)
{
	throw CHDBPoco::InvalidAccessException("Cannot sendUrgent() on a WebSocketImpl");
}
...
void WebSocketImpl::setSendTimeout(const CHDBPoco::Timespan& timeout)
{
	_pStreamSocketImpl->setSendTimeout(timeout);
}


CHDBPoco::Timespan WebSocketImpl::getSendTimeout()
{
	return _pStreamSocketImpl->getSendTimeout();
}


void WebSocketImpl::setReceiveTimeout(const CHDBPoco::Timespan& timeout)
{
	_pStreamSocketImpl->setReceiveTimeout(timeout);
}


CHDBPoco::Timespan WebSocketImpl::getReceiveTimeout()
{
