...
Context::~Context()
{
	try
	{
		...
		CHDBPoco::Crypto::OpenSSLInitializer::uninitialize();
	}
	catch (...)
	{
...
{
	CHDBPoco::Crypto::OpenSSLInitializer::initialize();
	...
	try
	{
		...
		{
			CHDBPoco::File aFile(params.caLocation);
			if (aFile.isDirectory())
				errCode = SSL_CTX_load_verify_locations(_pSSLContext, 0, CHDBPoco::Path::transcode(params.caLocation).c_str());
			else
				errCode = SSL_CTX_load_verify_locations(_pSSLContext, CHDBPoco::Path::transcode(params.caLocation).c_str(), 0);
			if (errCode != 1)
			{
		...
		{
			errCode = SSL_CTX_use_PrivateKey_file(_pSSLContext, CHDBPoco::Path::transcode(params.privateKeyFile).c_str(), SSL_FILETYPE_PEM);
			if (errCode != 1)
			{
				std::string msg = Utility::getLastError();
		...
		{
			errCode = SSL_CTX_use_certificate_chain_file(_pSSLContext, CHDBPoco::Path::transcode(params.certificateFile).c_str());
			if (errCode != 1)
			{
...
void Context::useCertificate(const CHDBPoco::Crypto::X509Certificate& certificate)
{
...
void Context::addChainCertificate(const CHDBPoco::Crypto::X509Certificate& certificate)
{
...
void Context::usePrivateKey(const CHDBPoco::Crypto::RSAKey& key)
{
...
{
	CHDB_poco_assert (isForServerUse());
...
#endif
		default:
			throw CHDBPoco::InvalidArgumentException("Invalid or unsupported usage");
		}
	}
	if (!_pSSLContext)
	{
