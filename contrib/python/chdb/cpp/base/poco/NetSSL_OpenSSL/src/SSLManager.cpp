...
namespace
{
	static CHDBPoco::SingletonHolder<SSLManager> singleton;
}
...
Context::Ptr SSLManager::defaultClientContext()
{
	...
	if (!_ptrDefaultClientContext)
	{
		...
		catch (CHDBPoco::IllegalStateException&)
		{
...
void SSLManager::initDefaultContext(bool server)
{
	...
	CHDBPoco::Crypto::OpenSSLInitializer openSSLInitializer;
	initEvents(server);
	CHDBPoco::Util::AbstractConfiguration& config = appConfig();

#ifdef OPENSSL_FIPS
	bool fipsEnabled = config.getBool(CFG_FIPS_MODE, VAL_FIPS_MODE);
	if (fipsEnabled && !CHDBPoco::Crypto::OpenSSLInitializer::isFIPSEnabled())
	{
		CHDBPoco::Crypto::OpenSSLInitializer::enableFIPSMode(true);
	}
#endif
	...
	CHDBPoco::StringTokenizer dpTok(disabledProtocolsList, ";,", CHDBPoco::StringTokenizer::TOK_TRIM | CHDBPoco::StringTokenizer::TOK_IGNORE_EMPTY);
	int disabledProtocols = 0;
	for (CHDBPoco::StringTokenizer::Iterator it = dpTok.begin(); it != dpTok.end(); ++it)
	{
		if (*it == "sslv2")
...
void SSLManager::initPassphraseHandler(bool server)
{
	...
	else throw CHDBPoco::Util::UnknownOptionException(std::string("No passphrase handler known with the name ") + className);
}
...
void SSLManager::initCertificateHandler(bool server)
{
	...
	else throw CHDBPoco::Util::UnknownOptionException(std::string("No InvalidCertificate handler known with the name ") + className);
}
...
CHDBPoco::Util::AbstractConfiguration& SSLManager::appConfig()
{
	try
	{
		return CHDBPoco::Util::Application::instance().config();
	}
	catch (CHDBPoco::NullPointerException&)
	{
		throw CHDBPoco::IllegalStateException(
			"An application configuration is required to initialize the CHDBPoco::Net::SSLManager, "
...
void initializeSSL()
{
	CHDBPoco::Crypto::initializeCrypto();
}
...
void uninitializeSSL()
{
	...
	CHDBPoco::Crypto::uninitializeCrypto();
}
