...
#define CHDB_POCO_BIO_set_nbio_accept(b,n) BIO_ctrl(b,BIO_C_SET_ACCEPT,1,(void*)((n)?"a":NULL))
...
{
	explicit RemainingTimeCounter(CHDBPoco::Timespan& remainingTime_) : remainingTime(remainingTime_) {};
...
private:
	CHDBPoco::Timespan& remainingTime;
	CHDBPoco::Timestamp start;
};

SecureSocketImpl::SecureSocketImpl(CHDBPoco::AutoPtr<SocketImpl> pSocketImpl, Context::Ptr pContext):
	_pSSL(nullptr),
	_pSocket(pSocketImpl),
	_pContext(pContext),
	_needHandshake(false)
{
	CHDB_poco_check_ptr (_pSocket);
	CHDB_poco_check_ptr (_pContext);
}
...
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	CHDB_poco_assert (!_pSSL);

	StreamSocket ss = _pSocket->acceptConnection(clientAddr);
	CHDBPoco::AutoPtr<SecureStreamSocketImpl> pSecureStreamSocketImpl = new SecureStreamSocketImpl(static_cast<StreamSocketImpl*>(ss.impl()), _pContext);
...
void SecureSocketImpl::connect(const SocketAddress& address, const CHDBPoco::Timespan& timeout, bool performHandshake)
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	if (_pSSL) reset();
	...
	CHDBPoco::Timespan receiveTimeout = _pSocket->getReceiveTimeout();
	CHDBPoco::Timespan sendTimeout = _pSocket->getSendTimeout();
...
void SecureSocketImpl::connectSSL(bool performHandshake)
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	CHDB_poco_assert (!_pSSL);
	CHDB_poco_assert (_pSocket->initialized());
	...
	try
	{
		...
		{
			int ret;
			CHDBPoco::Timespan remaining_time = getMaxTimeoutOrLimit();
			do
			{
...
CHDBPoco::Timespan SecureSocketImpl::getMaxTimeoutOrLimit()
{
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	CHDBPoco::Timespan remaining_time = _pSocket->getReceiveTimeout();
	CHDBPoco::Timespan send_timeout = _pSocket->getSendTimeout();
	...
	///
	/// NOTE: we cannot use INT64_MAX (std::numeric_limits<CHDBPoco::Timespan::TimeDiff>::max()),
	...
	if (remaining_time == 0)
		remaining_time = CHDBPoco::Timespan(std::numeric_limits<int>::max());
	return remaining_time;
}

bool SecureSocketImpl::mustRetry(int rc, CHDBPoco::Timespan& remaining_time)
{
	if (remaining_time == 0)
		return false;
	std::lock_guard<std::recursive_mutex> lock(_mutex);
	if (rc <= 0)
	{
		...
		case SSL_ERROR_WANT_READ:
			if (_pSocket->getBlocking())
			{
				if (_pSocket->pollImpl(remaining_time, CHDBPoco::Net::Socket::SELECT_READ))
					return true;
				else
					throw CHDBPoco::TimeoutException();
			}
			break;
		case SSL_ERROR_WANT_WRITE:
			if (_pSocket->getBlocking())
			{
				if (_pSocket->pollImpl(remaining_time, CHDBPoco::Net::Socket::SELECT_WRITE))
					return true;
				else
					throw CHDBPoco::TimeoutException();
			}
			break;
		/// NOTE: CHDB_POCO_EINTR is the same as SSL_ERROR_WANT_READ (at least in
		/// OpenSSL), so this likely dead code, but let's leave it for
		/// compatibility with other implementations
		case SSL_ERROR_SYSCALL:
			return socketError == CHDB_POCO_EAGAIN || socketError == CHDB_POCO_EINTR;
		default:
			return socketError == CHDB_POCO_EINTR;
		}
	}
	return false;
}
	...
	default:
		{
			...
			if (lastError == 0)
			{
				...
				{
					SecureStreamSocketImpl::error(CHDBPoco::format("The BIO reported an error: %d", rc));
				}
			}
			else
			{
