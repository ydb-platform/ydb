...
#include "CHDBPoco/TemporaryFile.h"
...
X509Certificate::X509Certificate(std::istream& istr):
	CHDBPoco::Crypto::X509Certificate(istr)
{	
}


X509Certificate::X509Certificate(const std::string& path):
	CHDBPoco::Crypto::X509Certificate(path)
{
}


X509Certificate::X509Certificate(X509* pCert):
	CHDBPoco::Crypto::X509Certificate(pCert)
{
}


X509Certificate::X509Certificate(X509* pCert, bool shared):
	CHDBPoco::Crypto::X509Certificate(pCert, shared)
{
}


X509Certificate::X509Certificate(const CHDBPoco::Crypto::X509Certificate& cert):
	CHDBPoco::Crypto::X509Certificate(cert)
{
}


X509Certificate& X509Certificate::operator = (const CHDBPoco::Crypto::X509Certificate& cert)
{
...
bool X509Certificate::verify(const CHDBPoco::Crypto::X509Certificate& certificate, const std::string& hostName)
{		
		...
		for (std::set<std::string>::const_iterator it = dnsNames.begin(); !ok && it != dnsNames.end(); ++it)
		{
			try
			{
				...
				else
				{
					...
					{
						ok = CHDBPoco::icompare(*it, hostName) == 0;
					}
				}
			}
...
bool X509Certificate::matchWildcard(const std::string& wildcard, const std::string& hostName)
{
	...
	wildcardExpr += CHDBPoco::replace(wildcard, ".", "\\.");
	CHDBPoco::replaceInPlace(wildcardExpr, "*", ".*");
	CHDBPoco::replaceInPlace(wildcardExpr, "..*", ".*");
	CHDBPoco::replaceInPlace(wildcardExpr, "?", ".?");
	CHDBPoco::replaceInPlace(wildcardExpr, "..?", ".?");
	wildcardExpr += "$";

	CHDBPoco::RegularExpression expr(wildcardExpr, CHDBPoco::RegularExpression::RE_CASELESS);
