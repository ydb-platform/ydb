...
#ifndef CHDB_Util_Application_INCLUDED
#define CHDB_Util_Application_INCLUDED
...
#include "CHDBPoco/Util/OptionSet.h"
...
namespace Util
{
    ...
    ///
    /// The CHDB_POCO_APP_MAIN macro can be used to implement main(argc, argv).
    /// If POCO has been built with CHDB_POCO_WIN32_UTF8, CHDB_POCO_APP_MAIN supports
    /// Unicode command line arguments.
    {
    public:
        typedef std::vector<std::string> ArgVec;
        typedef CHDBPoco::AutoPtr<Subsystem> SubsystemPtr;
        ...
        const CHDBPoco::Timestamp & startTime() const;
        /// Returns the application start time (UTC).

        CHDBPoco::Timespan uptime() const;
    ...
    protected:
        ...
        void setLogger(CHDBPoco::Logger & logger);
        ...
        bool findFile(CHDBPoco::Path & path) const;
    ...
    private:
        ...
        void getApplicationPath(CHDBPoco::Path & path) const;
        void processOptions();
        bool findAppConfigFile(const std::string & appName, const std::string & extension, CHDBPoco::Path & path) const;
        bool findAppConfigFile(const Path & basePath, const std::string & appName, const std::string & extension, CHDBPoco::Path & path) const;

        typedef CHDBPoco::AutoPtr<LayeredConfiguration> ConfigPtr;
        ...
        OptionSet _options;
        bool _unixOptions;
        CHDBPoco::Logger * _pLogger;
        CHDBPoco::Timestamp _startTime;
    ...
    C & Application::getSubsystem() const
    {
        ...
        throw CHDBPoco::NotFoundException("The subsystem has not been registered", typeid(C).name());
    }
    ...
    inline CHDBPoco::Logger & Application::logger() const
    {
        CHDB_poco_check_ptr(_pLogger);
        return *_pLogger;
    }
    ...
    {
        CHDB_poco_check_ptr(_pInstance);
        return *_pInstance;
    }
    ...
    inline const CHDBPoco::Timestamp & Application::startTime() const
    {
        return _startTime;
    }


    inline CHDBPoco::Timespan Application::uptime() const
    {
        CHDBPoco::Timestamp now;
        CHDBPoco::Timespan uptime = now - _startTime;
...
//
#    define CHDB_POCO_APP_MAIN(App) \
        int main(int argc, char ** argv) \
        { \
            CHDBPoco::AutoPtr<App> pApp = new App; \
            try \
            { \
                pApp->init(argc, argv); \
            } \
            ...
            { \
                pApp->logger().log(exc); \
                return CHDBPoco::Util::Application::EXIT_CONFIG; \
            } \
            return pApp->run(); \
        }


#endif // CHDB_Util_Application_INCLUDED
