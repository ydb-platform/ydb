...
#include "CHDBPoco/Util/FilesystemConfiguration.h"
...
using CHDBPoco::Path;
using CHDBPoco::File;
using CHDBPoco::DirectoryIterator;
...
bool FilesystemConfiguration::getRaw(const std::string& key, std::string& value) const
{
	...
	if (f.exists())
	{
		...
		CHDBPoco::FileInputStream istr(p.toString());
		int c = istr.get();
...
void FilesystemConfiguration::setRaw(const std::string& key, const std::string& value)
{
	...
	p.setFileName("data");
	CHDBPoco::FileOutputStream ostr(p.toString());
