...
{
	CHDBPoco::FileInputStream fis(path);
	load(fis);
}
...
bool JSONConfiguration::getRaw(const std::string & key, std::string & value) const
{
	...
	CHDBPoco::DynamicAny result = query.find(key);
	if ( ! result.isEmpty() )
	{
...
void JSONConfiguration::setValue(const std::string& key, const CHDBPoco::DynamicAny& value)
{
...
#endif // CHDB_POCO_UTIL_NO_JSONCONFIGURATION
