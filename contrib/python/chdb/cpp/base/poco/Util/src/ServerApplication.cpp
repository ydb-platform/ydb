...
#if defined(POCO_VXWORKS) || CHDB_POCO_OS == CHDB_POCO_OS_ANDROID
CHDBPoco::Event ServerApplication::_terminate;
#endif
...
void ServerApplication::terminate()
{
	...
	_terminate.set();
#else
	CHDBPoco::Process::requestTermination(Process::id());
#endif
}
...
void ServerApplication::waitForTerminationRequest()
{
	...
	int sig;
	sigwait(&sset, &sig);
#else // CHDB_POCO_OS != CHDB_POCO_OS_ANDROID
	_terminate.wait();
#endif
}
...
void ServerApplication::beDaemon()
{
	...
	if (!fin) throw CHDBPoco::OpenFileException("Cannot attach stdin to /dev/null");
	FILE* fout = freopen("/dev/null", "r+", stdout);
	if (!fout) throw CHDBPoco::OpenFileException("Cannot attach stdout to /dev/null");
	FILE* ferr = freopen("/dev/null", "r+", stderr);
	if (!ferr) throw CHDBPoco::OpenFileException("Cannot attach stderr to /dev/null");
#else
	throw CHDBPoco::NotImplementedException("platform does not allow fork/exec");
#endif
}
...
void ServerApplication::handleUMask(const std::string& name, const std::string& value)
{
	...
	for (std::string::const_iterator it = value.begin(); it != value.end(); ++it)
	{
		...
		if (*it >= '0' && *it <= '7')
			...
			throw CHDBPoco::InvalidArgumentException("umask contains non-octal characters", value);
	}
	umask(mask);
}
...
{
	CHDBPoco::FileOutputStream ostr(value);
	if (ostr.good())
		ostr << CHDBPoco::Process::id() << std::endl;
	else
		throw CHDBPoco::CreateFileException("Cannot write PID to file", value);
	CHDBPoco::TemporaryFile::registerForDeletion(value);
}
