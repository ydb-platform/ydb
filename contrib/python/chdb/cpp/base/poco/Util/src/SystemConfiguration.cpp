...
bool SystemConfiguration::getRaw(const std::string& key, std::string& value) const
{
	...
	else if (key == NODEID)
	{
		try
		{
			CHDBPoco::Environment::NodeId id;
			CHDBPoco::Environment::nodeId(id);
			char result[13];
			std::snprintf(result, 
                sizeof(result),
	...
	{
		value = CHDBPoco::DateTimeFormatter::format(CHDBPoco::DateTime(), CHDBPoco::DateTimeFormat::ISO8601_FORMAT);
	}
	else if (key == PID)
	{
		value = "0";
		value = CHDBPoco::NumberFormatter::format(CHDBPoco::Process::id());
	}
...
{
	throw CHDBPoco::InvalidAccessException("Attempt to modify a system property", key);
}
...
{
	throw CHDBPoco::NotImplementedException("Removing a key in a SystemConfiguration");
}
