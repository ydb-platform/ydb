...
Node* AbstractContainerNode::replaceChild(Node* newChild, Node* oldChild)
{
	...
	else
	{
		...
		else
		{
			...
			if (pCur)
			{
				CHDB_poco_assert_dbg (pCur->_pNext == oldChild);
	...
	if (pNode && it != end)
	{
		if (*it == '[')
		{
			...
			else
			{
				...
				std::string index;
				CHDBPoco::UnicodeConverter::convert(xmlIndex, index);
				int i = CHDBPoco::NumberParser::parse(index);
#else
				int i = CHDBPoco::NumberParser::parse(xmlIndex);
#endif
				indexBound = true;
				bool ib;
