...
void ParserEngine::setBillionLaughsAttackProtectionActivationThreshold(CHDBPoco::UInt64 activationThresholdBytes)
{
...
{
	CHDB_poco_assert (!_context.empty());
...
int ParserEngine::handleUnknownEncoding(void* encodingHandlerData, const XML_Char* name, XML_Encoding* info)
{
	...
	if (it != pThis->_encodings.end())
		...
		knownEncoding = CHDBPoco::TextEncoding::find(fromXMLString(encoding));
