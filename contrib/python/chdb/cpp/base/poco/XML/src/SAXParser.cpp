...
#include "CHDBPoco/SAX/SAXParser.h"
#include "CHDBPoco/SAX/SAXException.h"
#include "CHDBPoco/SAX/EntityResolverImpl.h"
#include "CHDBPoco/SAX/InputSource.h"
#include "CHDBPoco/XML/NamespaceStrategy.h"
...
void SAXParser::addEncoding(const XMLString& name, CHDBPoco::TextEncoding* pEncoding)
{
...
void SAXParser::setProperty(const XMLString& propertyId, const XMLString& value)
{
	if (propertyId == XMLReader::PROPERTY_DECLARATION_HANDLER || propertyId == XMLReader::PROPERTY_LEXICAL_HANDLER)
		...
		_engine.setBillionLaughsAttackProtectionMaximumAmplification(static_cast<float>(CHDBPoco::NumberParser::parseFloat(value)));
	else if (propertyId == PROPERTY_BLA_ACTIVATION_THRESHOLD)
		_engine.setBillionLaughsAttackProtectionActivationThreshold(CHDBPoco::NumberParser::parseUnsigned64(value));
	else
