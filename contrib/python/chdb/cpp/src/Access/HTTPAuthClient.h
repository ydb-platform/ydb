...
public:
    ...
    Result authenticateRequest(CHDBPoco::Net::HTTPRequest & request) const
    {
        ...
        for (size_t attempt = 0; attempt < max_tries; ++attempt)
        {
            ...
            catch (const CHDBPoco::Exception &) // TODO: make retries smarter
            {
                if (last_attempt)
                    throw;
    ...
    const CHDBPoco::URI & getURI() const { return uri; }
...
public:
    ...
    {
        CHDBPoco::Net::HTTPRequest request{CHDBPoco::Net::HTTPRequest::HTTP_GET, this->getURI().getPathAndQuery()};
        CHDBPoco::Net::HTTPBasicCredentials basic_credentials{user_name, password};
