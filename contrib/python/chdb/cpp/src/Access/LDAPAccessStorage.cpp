...
LDAPAccessStorage::LDAPAccessStorage(const String & storage_name_, AccessControl & access_control_, const CHDBPoco::Util::AbstractConfiguration & config, const String & prefix)
...
void LDAPAccessStorage::setConfiguration(const CHDBPoco::Util::AbstractConfiguration & config, const String & prefix)
{
    std::lock_guard lock(mutex);
    ...
    if (has_roles)
    {
        CHDBPoco::Util::AbstractConfiguration::Keys role_names;
    ...
    if (has_role_mapping)
    {
        CHDBPoco::Util::AbstractConfiguration::Keys all_keys;
...
{
    std::lock_guard lock(mutex);
    CHDBPoco::JSON::Object params_json;

    params_json.set("server", ldap_server_name);

    CHDBPoco::JSON::Array common_role_names_json;
    ...
    CHDBPoco::JSON::Array role_mappings_json;
    for (const auto & role_mapping : role_search_params)
    {
        CHDBPoco::JSON::Object role_mapping_json;
    ...
    CHDBPoco::JSON::Stringifier::stringify(params_json, oss);
