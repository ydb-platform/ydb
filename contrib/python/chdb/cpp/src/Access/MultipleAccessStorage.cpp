...
StoragePtr MultipleAccessStorage::findExcludingStorage(AccessEntityType type, const String & name, DB_CHDB::MultipleAccessStorage::StoragePtr exclude) const
{
    auto storages = getStoragesInternal();
    for (const auto & storage : *storages)
    {
...
MultipleAccessStorage::authenticateImpl(const Credentials & credentials, const CHDBPoco::Net::IPAddress & address,
