...
struct AggregateFunctionFlameGraphTree
{
    ...
    static void append(DB_CHDB::PaddedPODArray<UInt64> & values, DB_CHDB::PaddedPODArray<UInt64> & offsets, std::vector<UInt64> & frame)
    {
...
static void insertData(DB_CHDB::PaddedPODArray<UInt8> & chars, DB_CHDB::PaddedPODArray<UInt64> & offsets, const char * pos, size_t length)
{
...
static void fillColumn(DB_CHDB::PaddedPODArray<UInt8> & chars, DB_CHDB::PaddedPODArray<UInt64> & offsets, const std::string & str)
{
    size_t start = 0;
    size_t end = 0;
...
void dumpFlameGraph(
    ...
    DB_CHDB::PaddedPODArray<UInt8> & chars,
    DB_CHDB::PaddedPODArray<UInt64> & offsets)
{
    DB_CHDB::WriteBufferFromOwnString out;

    std::unordered_map<uintptr_t, size_t> mapping;

#if defined(__ELF__) && !defined(OS_FREEBSD)
    const DB_CHDB::SymbolIndex & symbol_index = DB_CHDB::SymbolIndex::instance();
#endif
    ...
    for (const auto & trace : traces)
    {
        ...
        for (size_t i = 0; i < trace.frames.size(); ++i)
        {
            ...
            if (const auto * symbol = symbol_index.findSymbol(ptr))
                ...
                DB_CHDB::writePointerHex(ptr, out);
#else
            DB_CHDB::writePointerHex(ptr, out);
#endif
        }
...
struct AggregateFunctionFlameGraphData
{
    ...
    {
        DB_CHDB::dumpFlameGraph(tree.dump(max_depth, min_bytes), chars, offsets);
    }
};
