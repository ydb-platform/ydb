...
struct AggregateFunctionSparkbarData
{
    ...
    Y insert(const X & x, const Y & y)
    {
        ...
        if (!inserted)
        {
            ...
            {
                Y res;
                bool has_overfllow = common_CHDB::addOverflow(it->getMapped(), y, res);
    ...
    void render(ColumnString & to_column, const AggregateFunctionSparkbarData<X, Y> & data) const
    {
        ...
        for (const auto & point : data.points)
        {
            ...
            if constexpr (std::is_floating_point_v<Y>)
                ...
                has_overfllow = common_CHDB::addOverflow(histogram[index], point.getMapped(), res);
        ...
        for (auto & y : histogram)
        {
            ...
            {
                Y scaled;
                bool has_overflow = common_CHDB::mulOverflow<Y>(y, levels_num, scaled);
