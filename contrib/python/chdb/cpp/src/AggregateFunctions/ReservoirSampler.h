...
public:
    ...
    double quantileInterpolated(double level)
    {
        ...
        {
            if constexpr (DB_CHDB::is_decimal<T>)
    ...
    void merge(const ReservoirSampler<T, OnEmpty> & b)
    {
        if (sample_count != b.sample_count)
            throw CHDBPoco::Exception("Cannot merge ReservoirSampler's with different sample_count");
        sorted = false;
    ...
    {
        DB_CHDB::readBinaryLittleEndian(sample_count, buf);
        DB_CHDB::readBinaryLittleEndian(total_values, buf);
        ...
        DB_CHDB::readStringBinary(rng_string, buf);
        DB_CHDB::ReadBufferFromString rng_buf(rng_string);
        rng_buf >> rng;

        for (size_t i = 0; i < samples.size(); ++i)
            DB_CHDB::readBinaryLittleEndian(samples[i], buf);
    ...
    {
        DB_CHDB::writeBinaryLittleEndian(sample_count, buf);
        DB_CHDB::writeBinaryLittleEndian(total_values, buf);

        DB_CHDB::WriteBufferFromOwnString rng_buf;
        rng_buf << rng;
        DB_CHDB::writeStringBinary(rng_buf.str(), buf);

        for (size_t i = 0; i < std::min(sample_count, total_values); ++i)
            DB_CHDB::writeBinaryLittleEndian(samples[i], buf);
    }
...
private:
    ...
    using Array = DB_CHDB::PODArrayWithStackMemory<T, 64>;
    ...
    ResultType onEmpty() const
    {
        if (OnEmpty == ReservoirSamplerOnEmpty::THROW)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Quantile of empty ReservoirSampler");
        else
