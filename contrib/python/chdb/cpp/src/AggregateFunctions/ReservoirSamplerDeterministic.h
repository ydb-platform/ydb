...
public:
    ...
    void merge(const ReservoirSamplerDeterministic & b)
    {
        if (max_sample_size != b.max_sample_size)
            throw CHDBPoco::Exception("Cannot merge ReservoirSamplerDeterministic's with different max sample size");
        sorted = false;
    ...
    void read(DB_CHDB::ReadBuffer & buf)
    {
        size_t size = 0;
        ...
        if (unlikely(size > MAX_RESERVOIR_SIZE))
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::TOO_LARGE_ARRAY_SIZE,
    ...
    void write(DB_CHDB::WriteBuffer & buf) const
    {
        ...
        for (size_t i = 0; i < size; ++i)
        {
            ...
            DB_CHDB::transformEndianness<std::endian::little>(elem);
            DB_CHDB::writeString(reinterpret_cast<const char*>(&elem), sizeof(elem), buf);
        }
    }
...
private:
    ...
    using Array = DB_CHDB::PODArray<Element, 64>;
    ...
    void setSkipDegree(UInt8 skip_degree_)
    {
        ...
        if (skip_degree_ > detail::MAX_SKIP_DEGREE)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::MEMORY_LIMIT_EXCEEDED, "skip_degree exceeds maximum value");
    ...
    ResultType onEmpty() const
    {
        if (OnEmpty == ReservoirSamplerDeterministicOnEmpty::THROW)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Quantile of empty ReservoirSamplerDeterministic");
        else
