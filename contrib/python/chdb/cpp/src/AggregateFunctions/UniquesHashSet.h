...
public:
    ...
    {
        if (m_size > UNIQUES_HASH_MAX_SIZE)
            throw CHDBPoco::Exception("Cannot write UniquesHashSet: too large size_degree.");

        DB_CHDB::writeBinaryLittleEndian(skip_degree, wb);
        DB_CHDB::writeVarUInt(m_size, wb);

        if (has_zero)
        {
            HashValue x = 0;
            DB_CHDB::writeBinaryLittleEndian(x, wb);
        }

        for (size_t i = 0; i < buf_size(); ++i)
            if (buf[i])
                DB_CHDB::writeBinaryLittleEndian(buf[i], wb);
    }
    ...
    void read(DB_CHDB::ReadBuffer & rb)
    {
        ...
        DB_CHDB::readBinaryLittleEndian(skip_degree, rb);
        DB_CHDB::readVarUInt(m_size, rb);

        if (m_size > UNIQUES_HASH_MAX_SIZE)
            throw CHDBPoco::Exception("Cannot read UniquesHashSet: too large size_degree.");
        ...
        if (m_size <= 1)
        {
            ...
            {
                HashValue x = 0;
                DB_CHDB::readBinaryLittleEndian(x, rb);
                if (x == 0)
                    has_zero = true;
                else
                    reinsertImpl(x);
        ...
        else
        {
            ...
            for (size_t i = 0; i < m_size; ++i)
            {
                DB_CHDB::transformEndianness<std::endian::native, std::endian::little>(hs[i]);
                if (hs[i] == 0)
                    has_zero = true;
                else
                    reinsertImpl(hs[i]);
    ...
    void readAndMerge(DB_CHDB::ReadBuffer & rb)
    {
        ...
        DB_CHDB::readBinaryLittleEndian(rhs_skip_degree, rb);
        ...
        size_t rhs_size = 0;
        DB_CHDB::readVarUInt(rhs_size, rb);
    ...
    static void skip(DB_CHDB::ReadBuffer & rb)
    {
        size_t size = 0;

        rb.ignore();
        DB_CHDB::readVarUInt(size, rb);
    ...
    void writeText(DB_CHDB::WriteBuffer & wb) const
    {
        ...
        DB_CHDB::writeIntText(skip_degree, wb);
        wb.write(",", 1);
        DB_CHDB::writeIntText(m_size, wb);
    ...
    void readText(DB_CHDB::ReadBuffer & rb)
    {
        ...
        DB_CHDB::readIntText(skip_degree, rb);
        DB_CHDB::assertChar(',', rb);
        DB_CHDB::readIntText(m_size, rb);
