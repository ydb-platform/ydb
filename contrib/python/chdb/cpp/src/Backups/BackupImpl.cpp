...
void BackupImpl::writeBackupMetadata()
{
    ...
    auto timer = DB_CHDB::CurrentThread::getProfileEvents().timer(ProfileEvents::BackupWriteMetadataMicroseconds);
...
void BackupImpl::readBackupMetadata()
{
    ...
    auto timer = DB_CHDB::CurrentThread::getProfileEvents().timer(ProfileEvents::BackupReadMetadataMicroseconds);
    ...
    CHDBPoco::AutoPtr<CHDBPoco::XML::Document> config = dom_parser.parseMemory(str.data(), str.size());
    const CHDBPoco::XML::Node * config_root = getRootNode(config);
    ...
    for (const CHDBPoco::XML::Node * child = contents->firstChild(); child; child = child->nextSibling())
    {
        if (child->nodeName() == "file")
        {
            const CHDBPoco::XML::Node * file_config = child;
            BackupFileInfo info;
