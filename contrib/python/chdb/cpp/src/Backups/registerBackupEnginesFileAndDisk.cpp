...
namespace
{
    ...
    void checkDiskName(const String & disk_name, const CHDBPoco::Util::AbstractConfiguration & config)
    {
    ...
    void checkPath(fs::path & path, const CHDBPoco::Util::AbstractConfiguration & config, const fs::path & data_dir)
    {
        path = path.lexically_normal();
        if (path.empty())
