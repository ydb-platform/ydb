...
CHDBPoco::URI CatBoostLibraryBridgeHelper::getPingURI() const
{
...
CHDBPoco::URI CatBoostLibraryBridgeHelper::getMainURI() const
{
    auto uri = createBaseURI();
...
CHDBPoco::URI CatBoostLibraryBridgeHelper::createRequestURI(const String & method) const
{
    auto uri = getMainURI();
...
ExternalModelInfos CatBoostLibraryBridgeHelper::listModels()
{
    ...
    auto buf = BuilderRWBufferFromHTTP(createRequestURI(CATBOOST_LIST_METHOD))
                   .withConnectionGroup(HTTPConnectionGroupType::STORAGE)
                   .withMethod(CHDBPoco::Net::HTTPRequest::HTTP_POST)
                   .withTimeouts(http_timeouts)
                   .create(credentials);
