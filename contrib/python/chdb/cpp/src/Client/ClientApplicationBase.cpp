...
{
    return dynamic_cast<ClientApplicationBase&>(CHDBPoco::Util::Application::instance());
}
...
CHDBPoco::Util::LayeredConfiguration & ClientApplicationBase::getClientConfiguration()
{
    return config();
}
...
void ClientApplicationBase::init(int argc, char ** argv)
{
    ...
    if (options.count("log-level"))
        CHDBPoco::Logger::root().setLevel(options["log-level"].as<std::string>());
    ...
    fatal_channel_ptr = new CHDBPoco::SplitterChannel;
    fatal_console_channel_ptr = new CHDBPoco::ConsoleChannel;
    fatal_channel_ptr->addChannel(fatal_console_channel_ptr);
    if (options.count("client_logs_file"))
    {
        fatal_file_channel_ptr = new CHDBPoco::SimpleFileChannel(options["client_logs_file"].as<std::string>());
    ...
    {
        fatal_log = createLogger("ClientBase", fatal_channel_ptr.get(), CHDBPoco::Message::PRIO_FATAL);
