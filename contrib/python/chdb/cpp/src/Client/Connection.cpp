...
void Connection::connect(const ConnectionTimeouts & timeouts)
{
    try
    {
        ...
        for (auto it = addresses.begin(); it != addresses.end();)
        {
            ...
            {
#if USE_SSL
                socket = std::make_unique<CHDBPoco::Net::SecureStreamSocket>();
                ...
                static_cast<CHDBPoco::Net::SecureStreamSocket*>(socket.get())->setPeerHostName(host);
#else
            ...
            {
                socket = std::make_unique<CHDBPoco::Net::StreamSocket>();
            }
            ...
            try
            {
                ...
                {
                    socket->connectNB(*it);
                    while (!socket->poll(0, CHDBPoco::Net::Socket::SELECT_READ | CHDBPoco::Net::Socket::SELECT_WRITE | CHDBPoco::Net::Socket::SELECT_ERROR))
            ...
            catch (DB_CHDB::NetException &)
            {
                if (++it == addresses.end())
                    throw;
                continue;
            }
            catch (CHDBPoco::Net::NetException &)
            {
                if (++it == addresses.end())
                    throw;
                continue;
    ...
    catch (CHDBPoco::TimeoutException & e)
    {
        disconnect();
...
void Connection::performHandshakeForSSHAuth()
{
    String challenge;
    {
        ...
        if (in->eof())
            throw CHDBPoco::Net::NetException("Connection reset by peer");
...
void Connection::receiveHello(const CHDBPoco::Timespan & handshake_timeout)
{
    ...
    std::function<void(const Progress &)>)
{
    ...
    if (settings)
    {
        ...
        std::string method = CHDBPoco::toUpper(settings->network_compression_method.toString());
...
std::optional<CHDBPoco::Net::SocketAddress> Connection::getResolvedAddress() const
{
