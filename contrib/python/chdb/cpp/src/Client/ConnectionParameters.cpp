...
bool enableSecureConnection(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & connection_host)
{
...
ConnectionParameters::ConnectionParameters(const CHDBPoco::Util::AbstractConfiguration & config,
    ...
    , port(connection_port.value_or(getPortFromConfig(config, connection_host)))
{
    ...
    timeouts = ConnectionTimeouts()
            .withConnectionTimeout(
                CHDBPoco::Timespan(config.getInt("connect_timeout", DBMS_DEFAULT_CONNECT_TIMEOUT_SEC), 0))
            .withSendTimeout(
                CHDBPoco::Timespan(config.getInt("send_timeout", DBMS_DEFAULT_SEND_TIMEOUT_SEC), 0))
            .withReceiveTimeout(
                CHDBPoco::Timespan(config.getInt("receive_timeout", DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC), 0))
            .withTCPKeepAliveTimeout(
                CHDBPoco::Timespan(config.getInt("tcp_keep_alive_timeout", DEFAULT_TCP_KEEP_ALIVE_TIMEOUT), 0))
            .withHandshakeTimeout(
                CHDBPoco::Timespan(config.getInt("handshake_timeout_ms", DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC * 1000) * 1000))
            .withSyncRequestTimeout(
                CHDBPoco::Timespan(config.getInt("sync_request_timeout", DBMS_DEFAULT_SYNC_REQUEST_TIMEOUT_SEC), 0));
}
...
UInt16 ConnectionParameters::getPortFromConfig(const CHDBPoco::Util::AbstractConfiguration & config,
