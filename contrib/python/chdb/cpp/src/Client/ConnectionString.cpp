...
std::string uriDecode(const std::string & uri_encoded_string, bool plus_as_space)
{
    ...
    CHDBPoco::URI::decode(uri_encoded_string, decoded_string, plus_as_space);
    return decoded_string;
}

void getHostAndPort(const CHDBPoco::URI & uri, std::vector<std::vector<std::string>> & hosts_and_ports_arguments)
{
    ...
    std::vector<std::vector<std::string>> & hosts_and_ports_arguments)
{
    ...
    try
    {
        uri = CHDBPoco::URI(uri_string);
    }
    catch (const CHDBPoco::URISyntaxException & invalid_uri_exception)
    {
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS,
    ...
    std::vector<std::vector<std::string>> & hosts_and_ports_arguments)
{
    ...
    try
    {
        /** CHDBPoco::URI doesn't support several hosts in URI.
        ...
        CHDBPoco::URI::QueryParameters params = uri.getQueryParameters();
        ...
        if (!user_info.empty())
        {
            // CHDBPoco::URI doesn't decode user name/password by default.
