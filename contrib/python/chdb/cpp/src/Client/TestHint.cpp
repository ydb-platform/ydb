...
void TestHint::parse(Lexer & comment_lexer, bool is_leading_hint)
{
    ...
    for (Token token = comment_lexer.nextToken(); !token.isEnd(); token = comment_lexer.nextToken())
    {
        ...
        if (token.type == TokenType::BareWord && commands.contains(item))
        {
            ...
            if (item == "retry")
            {
                ...
                if (token.type != TokenType::Number)
                    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PARSE_TEXT, "Could not parse the number of retries: {}",
                ...
                if (token.type != TokenType::BareWord ||
                    ...
                    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PARSE_TEXT, "Expected 'until' or 'while' after the number of retries, got: {}",
                                        std::string_view(token.begin, token.end));
        ...
        else if (!is_leading_hint && token.type == TokenType::BareWord && command_errors.contains(item))
        {
            ...
            while (!token.isEnd())
            {
                ...
                if (token.type == TokenType::Number)
                {
                    ...
                    if (p == token.begin)
                        throw DB_CHDB::Exception(
                            DB_CHDB::ErrorCodes::CANNOT_PARSE_TEXT,
                ...
                {
                    int code = DB_CHDB::ErrorCodes::getErrorCodeByName(std::string_view(token.begin, token.end));
                    error_codes.push_back(code);
                }
                else
                    throw DB_CHDB::Exception(
    ...
    if (max_retries && server_errors.size() != 1)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PARSE_TEXT, "Expected one serverError after the 'retry N while|until' command");
}
