...
AvailableCollationLocales::AvailableCollationLocales()
{
    ...
    for (size_t i = 0; i < available_locales_count; ++i)
    {
        ...
        locales_map.emplace(CHDBPoco::toLower(locale_name), LocaleAndLanguage{locale_name, lang});
    }
...
bool AvailableCollationLocales::isCollationSupported(const std::string & locale_name) const
{
    ...
    return locales_map.contains(CHDBPoco::toLower(locale_name));
}

Collator::Collator(const std::string & locale_)
    : locale(CHDBPoco::toLower(locale_))
{
#if USE_ICU
    ...
    if (!AvailableCollationLocales::instance().isCollationSupported(locale))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::UNSUPPORTED_COLLATION_LOCALE, "Unsupported collation locale: {}", locale);
    ...
    {
        ucol_close(collator);
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::UNSUPPORTED_COLLATION_LOCALE, "Failed to open locale: {} with error: {}", locale, u_errorName(status));
    }
#else
    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::SUPPORT_IS_DISABLED,
...
int Collator::compare(const char * str1, size_t length1, const char * str2, size_t length2) const
{
    ...
    if (U_FAILURE(status))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::COLLATION_COMPARISON_FAILED, "ICU collation comparison failed with error code: {}",
