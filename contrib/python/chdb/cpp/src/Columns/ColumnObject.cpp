...
void ColumnObject::insertFromSharedDataAndFillRemainingDynamicPaths(const DB_CHDB::ColumnObject & src_object_column, std::vector<std::string_view> && src_dynamic_paths_for_shared_data, size_t start, size_t length)
{
...
void ColumnObject::serializePathAndValueIntoArena(DB_CHDB::Arena & arena, const char *& begin, StringRef path, StringRef value, StringRef & res) const
{
...
void ColumnObject::forEachSubcolumn(DB_CHDB::IColumn::MutableColumnCallback callback)
{
    for (auto & [_, column] : typed_paths)
        callback(column);
...
void ColumnObject::forEachSubcolumnRecursively(DB_CHDB::IColumn::RecursiveMutableColumnCallback callback)
{
    for (auto & [_, column] : typed_paths)
    {
        callback(*column);
...
void ColumnObject::getExtremes(DB_CHDB::Field & min, DB_CHDB::Field & max) const
{
    if (empty())
    {
        min = Object();
...
void ColumnObject::takeDynamicStructureFromSourceColumns(const DB_CHDB::Columns & source_columns)
{
    if (!empty())
