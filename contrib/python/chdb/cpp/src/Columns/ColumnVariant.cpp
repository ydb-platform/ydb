...
ColumnVariant::ColumnVariant(DB_CHDB::MutableColumnPtr local_discriminators_, DB_CHDB::MutableColumnPtr offsets_, DB_CHDB::MutableColumns && variants_) : ColumnVariant(std::move(local_discriminators_), std::move(offsets_), std::move(variants_), {})
{
}

ColumnVariant::ColumnVariant(DB_CHDB::MutableColumnPtr local_discriminators_, DB_CHDB::MutableColumnPtr offsets_, DB_CHDB::MutableColumns && variants_, const std::vector<Discriminator> & local_to_global_discriminators_)
{
...
ColumnVariant::Ptr ColumnVariant::create(const DB_CHDB::ColumnPtr & local_discriminators, const DB_CHDB::Columns & variants, const std::vector<Discriminator> & local_to_global_discriminators)
{
...
ColumnVariant::Ptr ColumnVariant::create(const DB_CHDB::ColumnPtr & local_discriminators, const DB_CHDB::ColumnPtr & offsets, const DB_CHDB::Columns & variants, const std::vector<Discriminator> & local_to_global_discriminators)
{
...
bool ColumnVariant::tryInsert(const DB_CHDB::Field & x)
{
    if (x.isNull())
    {
        insertDefault();
...
void ColumnVariant::insertFromImpl(const DB_CHDB::IColumn & src_, size_t n, const std::vector<ColumnVariant::Discriminator> * global_discriminators_mapping)
{
    const size_t num_variants = variants.size();
...
void ColumnVariant::insertRangeFromImpl(const DB_CHDB::IColumn & src_, size_t start, size_t length, const std::vector<ColumnVariant::Discriminator> * global_discriminators_mapping, const Discriminator * skip_discriminator)
{
    const size_t num_variants = variants.size();
...
void ColumnVariant::insertManyFromImpl(const DB_CHDB::IColumn & src_, size_t position, size_t length, const std::vector<ColumnVariant::Discriminator> * global_discriminators_mapping)
{
    const size_t num_variants = variants.size();
    const auto & src = assert_cast<const ColumnVariant &>(src_);
    if (!global_discriminators_mapping && src.variants.size() != num_variants)
...
#if !defined(DEBUG_OR_SANITIZER_BUILD)
void ColumnVariant::insertManyFrom(const DB_CHDB::IColumn & src_, size_t position, size_t length)
#else
void ColumnVariant::doInsertManyFrom(const DB_CHDB::IColumn & src_, size_t position, size_t length)
#endif
{
    insertManyFromImpl(src_, position, length, nullptr);
}

void ColumnVariant::insertFrom(const DB_CHDB::IColumn & src_, size_t n, const std::vector<ColumnVariant::Discriminator> & global_discriminators_mapping)
{
...
void ColumnVariant::insertManyFrom(const DB_CHDB::IColumn & src_, size_t position, size_t length, const std::vector<ColumnVariant::Discriminator> & global_discriminators_mapping)
{
    insertManyFromImpl(src_, position, length, &global_discriminators_mapping);
}

void ColumnVariant::insertIntoVariantFrom(DB_CHDB::ColumnVariant::Discriminator global_discr, const DB_CHDB::IColumn & src_, size_t n)
{
    Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
    getLocalDiscriminators().push_back(local_discr);
    getOffsets().push_back(variants[local_discr]->size());
...
void ColumnVariant::insertRangeIntoVariantFrom(DB_CHDB::ColumnVariant::Discriminator global_discr, const DB_CHDB::IColumn & src_, size_t start, size_t length)
{
    Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
    auto & local_discriminators_data = getLocalDiscriminators();
    local_discriminators_data.resize_fill(local_discriminators_data.size() + length, local_discr);
...
void ColumnVariant::insertManyIntoVariantFrom(DB_CHDB::ColumnVariant::Discriminator global_discr, const DB_CHDB::IColumn & src_, size_t position, size_t length)
{
    Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
    auto & local_discriminators_data = getLocalDiscriminators();
    local_discriminators_data.resize_fill(local_discriminators_data.size() + length, local_discr);
...
const char * ColumnVariant::deserializeVariantAndInsertFromArena(DB_CHDB::ColumnVariant::Discriminator global_discr, const char * pos)
{
    Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
    getLocalDiscriminators().push_back(local_discr);
    getOffsets().push_back(variants[local_discr]->size());
...
void ColumnVariant::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint, IColumn::Permutation & res, DB_CHDB::EqualRanges & equal_ranges) const
{
