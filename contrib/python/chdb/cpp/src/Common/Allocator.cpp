...
void * allocNoTrack(size_t size, size_t alignment)
{
    ...
    if (alignment <= MALLOC_MIN_ALIGNMENT)
    {
        ...
        if (nullptr == buf)
            throw DB_CHDB::ErrnoException(DB_CHDB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Allocator: Cannot malloc {}.", ReadableSize(size));
    }
    else
    {
        buf = nullptr;
        ...
        if (0 != res)
            throw DB_CHDB::ErrnoException(
                DB_CHDB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Cannot allocate memory (posix_memalign) {}.", ReadableSize(size));
...
void checkSize(size_t size)
{
    ...
    if (size >= 0x8000000000000000ULL)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Too large size ({}) passed to allocator. It indicates an error.", size);
}
...
void Allocator<clear_memory_, populate>::free(void * buf, size_t size)
{
    ...
    {
        DB_CHDB::tryLogCurrentException("Allocator::free");
        throw;
    }
}
