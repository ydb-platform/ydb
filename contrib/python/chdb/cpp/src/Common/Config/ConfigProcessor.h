...
#include <CHDBPoco/DOM/NodeList.h>
#include <CHDBPoco/DOM/NamedNodeMap.h>
#include <CHDBPoco/AutoPtr.h>
#include <CHDBPoco/DirectoryIterator.h>
...
namespace zkutil
{
    ...
    using EventPtr = std::shared_ptr<CHDBPoco::Event>;
}
...
using XMLDocumentPtr = CHDBPoco::AutoPtr<CHDBPoco::XML::Document>;
...
public:
    ...
    /// loadConfig* functions apply processConfig and create CHDBPoco::Util::XMLConfiguration.
...
private:
    ...
    LoggerPtr log;
    CHDBPoco::AutoPtr<CHDBPoco::Channel> channel_ptr;

    Substitutions substitutions;

    CHDBPoco::AutoPtr<CHDBPoco::XML::NamePool> name_pool;
    CHDBPoco::XML::DOMParser dom_parser;

    using NodePtr = CHDBPoco::AutoPtr<CHDBPoco::XML::Node>;

#if USE_SSL
    void decryptRecursive(CHDBPoco::XML::Node * config_root);

    /// Decrypt elements in config with specified encryption attributes
    void decryptEncryptedElements(LoadedConfig & loaded_config);
#endif

    void hideRecursive(CHDBPoco::XML::Node * config_root);
    XMLDocumentPtr hideElements(XMLDocumentPtr xml_tree);

    void mergeRecursive(XMLDocumentPtr config, CHDBPoco::XML::Node * config_root, const CHDBPoco::XML::Node * with_root);
