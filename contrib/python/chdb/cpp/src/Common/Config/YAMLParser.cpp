...
namespace
{
    ...
    CHDBPoco::AutoPtr<CHDBPoco::XML::Element> cloneXMLNode(const CHDBPoco::XML::Element & original_node)
    {
        CHDBPoco::AutoPtr<CHDBPoco::XML::Element> clone_node = original_node.ownerDocument()->createElement(original_node.nodeName());
        original_node.parentNode()->appendChild(clone_node);
        return clone_node;
    }

    void processNode(const YAML::Node & node, CHDBPoco::XML::Element & parent_xml_node)
    {
        ...
        switch (node.Type())
        {
            case YAML::NodeType::Scalar:
            {
                ...
                CHDBPoco::AutoPtr<CHDBPoco::XML::Text> xml_value = xml_document->createTextNode(value);
            ...
            case YAML::NodeType::Map:
            {
                for (const auto & key_value_pair : node)
                {
                    ...
                    else
                    {
                        ...
                        {
                            CHDBPoco::AutoPtr<CHDBPoco::XML::Element> xml_key = xml_document->createElement(key);
...
CHDBPoco::AutoPtr<CHDBPoco::XML::Document> YAMLParser::parse(const String& path)
{
    YAML::Node node_yml;
    try
    {
    ...
    CHDBPoco::AutoPtr<CHDBPoco::XML::Document> xml = new Document;
    CHDBPoco::AutoPtr<CHDBPoco::XML::Element> root_node = xml->createElement("clickhouse");
