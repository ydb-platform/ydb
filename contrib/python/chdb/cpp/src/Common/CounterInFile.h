...
public:
    ...
    Int64 add(Int64 delta, Callback && locked_callback, bool create_if_need = false)
    {
        ...
        {
            throw CHDBPoco::Exception("File " + path + " does not exist. "
        ...
        if (-1 == fd)
            DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot open file {}", path);

        try
        {
            int flock_ret = flock(fd, LOCK_EX);
            if (-1 == flock_ret)
                DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot lock file {}", path);

            if (!file_doesnt_exists)
            {
                DB_CHDB::ReadBufferFromFileDescriptor rb(fd, SMALL_READ_WRITE_BUFFER_SIZE);
                try
                {
                    DB_CHDB::readIntText(res, rb);
                }
                catch (const DB_CHDB::Exception & e)
                {
                    /// A more understandable error message.
                    if (e.code() == DB_CHDB::ErrorCodes::CANNOT_READ_ALL_DATA || e.code() == DB_CHDB::ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF)
                        throw DB_CHDB::Exception(e.code(), "File {} is empty. You must fill it manually with appropriate value.", path);
                    else
                        throw;
                }
            }
            ...
            if (delta || file_doesnt_exists)
            {
                ...
                DB_CHDB::WriteBufferFromFileDescriptor wb(fd, SMALL_READ_WRITE_BUFFER_SIZE);
                wb.seek(0, SEEK_SET);
                wb.truncate(0);
                DB_CHDB::writeIntText(res, wb);
                DB_CHDB::writeChar('\n', wb);
                wb.sync();
            }
        ...
        try
        {
            ...
            if (file_exists)
            {
                ...
                try
                {
                    ...
                    DB_CHDB::readIntText(current_value, rb);
                    char c;
                    DB_CHDB::readChar(c, rb);
                    if (rb.count() > 0 && c == '\n' && rb.eof())
                        broken = false;
                }
                catch (const DB_CHDB::Exception & e)
