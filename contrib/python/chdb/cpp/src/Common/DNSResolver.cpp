...
namespace std
{
template<> struct hash<CHDBPoco::Net::IPAddress>
{
    size_t operator()(const CHDBPoco::Net::IPAddress & address) const noexcept
    {
...
void splitHostAndPort(const std::string & host_and_port, std::string & out_host, UInt16 & out_port)
{
    ...
    unsigned port;
    if (CHDBPoco::NumberParser::tryParseUnsigned(port_str, port) && port <= 0xFFFF)
    {
...
DNSResolver::IPAddresses hostByName(const std::string & host)
{
    ...
    auto flags = CHDBPoco::Net::DNS::DNS_HINT_AI_ADDRCONFIG;

    DNSResolver::IPAddresses addresses;

    try
    {
        addresses = CHDBPoco::Net::DNS::hostByName(host, flags).addresses();
    }
    ...
    if (addresses.empty())
    {
        ...
        throw DB_CHDB::NetException(ErrorCodes::DNS_ERROR, "Not found address of host: {}", host);
    }
...
{
    CHDBPoco::Net::IPAddress ip;

    /// NOTE:
    /// - CHDBPoco::Net::DNS::resolveOne(host) doesn't work for IP addresses like 127.0.0.2
    /// - CHDBPoco::Net::IPAddress::tryParse() expect hex string for IPv6 (without brackets)
    if (host.starts_with('['))
    {
        assert(host.ends_with(']'));
        if (CHDBPoco::Net::IPAddress::tryParse(host.substr(1, host.size() - 2), ip))
            return DNSResolver::IPAddresses(1, ip);
    }
    else
    {
        if (CHDBPoco::Net::IPAddress::tryParse(host, ip))
            return DNSResolver::IPAddresses(1, ip);
    }
...
std::unordered_set<String> reverseResolveImpl(const CHDBPoco::Net::IPAddress & address)
{
    auto ptr_resolver = DB_CHDB::DNSPTRResolverProvider::get();

    if (address.family() == CHDBPoco::Net::IPAddress::Family::IPv4)
    {
...
std::unordered_set<String> reverseResolveWithCache(
    CacheBase<CHDBPoco::Net::IPAddress, std::unordered_set<std::string>> & cache, const CHDBPoco::Net::IPAddress & address)
{
    auto [result, _ ] = cache.getOrSet(address, [&address]() { return std::make_shared<std::unordered_set<String>>(reverseResolveImpl(address)); });
    return *result;
}

CHDBPoco::Net::IPAddress pickAddress(const DNSResolver::IPAddresses & addresses)
{
...
struct DNSResolver::Impl
{
    ...
    using AddressWithConsecutiveFailures = std::unordered_map<CHDBPoco::Net::IPAddress, UInt32>;

    CacheBase<std::string, DNSResolver::CacheEntry> cache_host{1024};
    CacheBase<CHDBPoco::Net::IPAddress, std::unordered_set<std::string>> cache_address{1024};
...
CHDBPoco::Net::IPAddress DNSResolver::resolveHost(const std::string & host)
{
...
CHDBPoco::Net::SocketAddress DNSResolver::resolveAddress(const std::string & host_and_port)
{
    if (impl->disable_cache)
        return CHDBPoco::Net::SocketAddress(host_and_port);
    ...
    addToNewHosts(host);
    return CHDBPoco::Net::SocketAddress(pickAddress(resolveIPAddressWithCache(impl->cache_host, host)), port);
}

CHDBPoco::Net::SocketAddress DNSResolver::resolveAddress(const std::string & host, UInt16 port)
{
    if (impl->disable_cache)
        return CHDBPoco::Net::SocketAddress(host, port);

    addToNewHosts(host);
    return CHDBPoco::Net::SocketAddress(pickAddress(resolveIPAddressWithCache(impl->cache_host, host)), port);
}

std::vector<CHDBPoco::Net::SocketAddress> DNSResolver::resolveAddressList(const std::string & host, UInt16 port)
{
    if (CHDBPoco::Net::IPAddress ip; CHDBPoco::Net::IPAddress::tryParse(host, ip))
        return std::vector<CHDBPoco::Net::SocketAddress>{{ip, port}};

    std::vector<CHDBPoco::Net::SocketAddress> addresses;

    if (!impl->disable_cache)
        addToNewHosts(host);

    std::vector<CHDBPoco::Net::IPAddress> ips = impl->disable_cache ? hostByName(host) : resolveIPAddressWithCache(impl->cache_host, host);
...
std::unordered_set<String> DNSResolver::reverseResolve(const CHDBPoco::Net::IPAddress & address)
{
    if (impl->disable_cache)
...
String DNSResolver::getHostName()
{
    ...
    if (!impl->host_name.has_value())
        impl->host_name.emplace(CHDBPoco::Net::DNS::hostName());

    return *impl->host_name;
}

static const String & cacheElemToString(const String & str) { return str; }
static String cacheElemToString(const CHDBPoco::Net::IPAddress & addr) { return addr.toString(); }
...
bool DNSResolver::updateCache(UInt32 max_consecutive_failures)
{
    ...
    {
        String updated_host_name = CHDBPoco::Net::DNS::hostName();
...
bool DNSResolver::updateAddress(const CHDBPoco::Net::IPAddress & address)
{
...
void DNSResolver::addToNewAddresses(const CHDBPoco::Net::IPAddress & address)
{
    std::lock_guard lock(impl->drop_mutex);
    UInt8 consecutive_failures = 0;
