...
CHDBPoco::DigestEngine::Digest calcSHA1(const std::string & path)
{
    std::ifstream stream(path);
    if (!stream)
        throw CHDBPoco::Exception("Error while opening file: '" + path + "'.");
    CHDBPoco::SHA1Engine digest_engine;
    CHDBPoco::DigestInputStream digest_stream(digest_engine, stream);
    digest_stream.ignore(std::numeric_limits<std::streamsize>::max());
    if (!stream.eof())
        throw CHDBPoco::Exception("Error while reading file: '" + path + "'.");
...
std::string determineDefaultTimeZone()
{
    ...
    try
    {
        ...
        CHDBPoco::SHA1Engine::Digest tzfile_sha1 = calcSHA1(tz_file_path.string());
    ...
    catch (const CHDBPoco::Exception & ex)
    {
        throw CHDBPoco::Exception(error_prefix + ex.message(), ex);
    }
    catch (const std::exception & ex)
    {
        throw CHDBPoco::Exception(error_prefix + ex.what());
    }

    throw CHDBPoco::Exception(error_prefix + "custom time zone file used.");
}
...
const DateLUTImpl & DateLUT::instance()
{
    ...
    if (DB_CHDB::CurrentThread::isInitialized())
    {
        std::string timezone_from_context;
        const DB_CHDB::ContextPtr query_context = DB_CHDB::CurrentThread::get().getQueryContext();
        ...
        const DB_CHDB::ContextPtr global_context = DB_CHDB::CurrentThread::get().getGlobalContext();
        if (global_context)
        {
...
std::string DateLUT::extractTimezoneFromContext(DB_CHDB::ContextPtr query_context)
{
