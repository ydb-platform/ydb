...
{
    auto & logger = CHDBPoco::Logger::root();
...
Exception::Exception(const MessageMasked & msg_masked, int code, bool remote_)
    : CHDBPoco::Exception(msg_masked.msg, code)
    , remote(remote_)
{
...
Exception::Exception(CreateFromPocoTag, const CHDBPoco::Exception & exc)
    : CHDBPoco::Exception(exc.displayText(), ErrorCodes::POCO_EXCEPTION)
{
    if (terminate_on_any_exception)
        std::_Exit(terminate_status_code);
    capture_thread_frame_pointers = getThreadFramePointers();
...
Exception::Exception(CreateFromSTDTag, const std::exception & exc)
    : CHDBPoco::Exception(demangle(typeid(exc).name()) + ": " + String(exc.what()), ErrorCodes::STD_EXCEPTION)
{
    if (terminate_on_any_exception)
        std::_Exit(terminate_status_code);
    capture_thread_frame_pointers = getThreadFramePointers();
...
static void tryLogCurrentExceptionImpl(CHDBPoco::Logger * logger, const std::string & start_of_message)
{
    if (!isLoggingEnabled())
        return;
    ...
    try
    {
        if (const auto * file_exception = dynamic_cast<const fs::filesystem_error *>(&e))
        {
            if (file_exception->code() == std::errc::no_space_on_device)
                ...
                msg += "\nCannot print extra info for CHDBPoco::Exception";
        }
        else if (const auto * errno_exception = dynamic_cast<const DB_CHDB::ErrnoException *>(&e))
        {
...
PreformattedMessage getCurrentExceptionMessageAndPattern(bool with_stacktrace, bool check_embedded_stacktrace /*= false*/, bool with_extra_info /*= true*/)
{
    ...
    catch (const CHDBPoco::Exception & e)
    {
        try
        {
            stream << "CHDBPoco::Exception. Code: " << ErrorCodes::POCO_EXCEPTION << ", e.code() = " << e.code()
                << ", " << e.displayText()
    ...
    try
    {
        ...
        if (check_embedded_stacktrace)
        {
            ...
            if (!with_stacktrace && has_embedded_stack_trace)
            {
                ...
                CHDBPoco::trimRightInPlace(text);
            }
        }
