...
void FstBuilder::add(std::string_view current_word, Output current_output)
{
    ...
    if (current_word_len > MAX_TERM_LENGTH)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Cannot build full-text index: The maximum term length is {}, this is exceeded by term {}", MAX_TERM_LENGTH, current_word_len);
