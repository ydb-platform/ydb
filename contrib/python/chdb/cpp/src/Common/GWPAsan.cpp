...
#    error #include <gwp_asan/common.h>
#    error #include <gwp_asan/crash_handler.h>
...
{
    explicit ScopedEndOfReportDecorator(CHDBPoco::LoggerPtr log_) : log(std::move(log_)) { }
    ~ScopedEndOfReportDecorator() { LOG_FATAL(log, "*** End GWP-ASan report ***"); }
    CHDBPoco::LoggerPtr log;
};

// Prints the provided error and metadata information.
void printHeader(gwp_asan::Error error, uintptr_t fault_address, const gwp_asan::AllocationMetadata * allocation_meta, CHDBPoco::LoggerPtr log)
{
