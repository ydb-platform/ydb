    ...
    public:
        ...
        void flushRequest() override
        {
            if (bool(request_stream))
            {
                ...
                if (auto * fixed_steam = dynamic_cast<CHDBPoco::Net::HTTPFixedLengthOutputStream *>(request_stream))
                {
                    request_stream_completed = fixed_steam->isComplete();
                }
                else if (auto * chunked_steam = dynamic_cast<CHDBPoco::Net::HTTPChunkedOutputStream *>(request_stream))
                {
                    chunked_steam->rdbuf()->close();
                    request_stream_completed = chunked_steam->isComplete();
                }
                else if (auto * http_stream = dynamic_cast<CHDBPoco::Net::HTTPOutputStream *>(request_stream))
                {
        ...
        std::ostream & sendRequest(CHDBPoco::Net::HTTPRequest & request) override
        {
            auto idle = idleTime();
        ...
        std::istream & receiveResponse(CHDBPoco::Net::HTTPResponse & response) override
        {
        ...
        ~PooledConnection() override
        {
            ...
            {
                if (auto * fixed_steam = dynamic_cast<CHDBPoco::Net::HTTPFixedLengthInputStream *>(response_stream))
                {
                    response_stream_completed = fixed_steam->isComplete();
                }
                else if (auto * chunked_steam = dynamic_cast<CHDBPoco::Net::HTTPChunkedInputStream *>(response_stream))
                {
                    response_stream_completed = chunked_steam->isComplete();
                }
                else if (auto * http_stream = dynamic_cast<CHDBPoco::Net::HTTPInputStream *>(response_stream))
                {
...
createConnectionPool(ConnectionGroup::Ptr group, std::string host, UInt16 port, bool secure, ProxyConfiguration proxy_configuration)
{
    ...
    {
#if USE_SSL
        return EndpointConnectionPool<CHDBPoco::Net::HTTPSClientSession>::create(
    ...
    {
        return EndpointConnectionPool<CHDBPoco::Net::HTTPClientSession>::create(
            group, std::move(host), port, secure, std::move(proxy_configuration));
    }
}
...
private:
    ...
    const CHDBPoco::Timespan wipe_timeout = CHDBPoco::Timespan(DEFAULT_WIPE_TIMEOUT_SECONDS, 0);
    ...
    CHDBPoco::Timestamp last_wipe_time TSA_GUARDED_BY(mutex);
...
protected:
    ...
    IExtendedPool::Ptr getPoolImpl(HTTPConnectionGroupType type, const CHDBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
        TSA_REQUIRES(mutex)
    {
    ...
    static bool useSecureConnection(const CHDBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
    {
    ...
    static std::tuple<std::string, UInt16, bool> getHostPortSecure(const CHDBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
    {
...
HTTPConnectionPools::getPool(HTTPConnectionGroupType type, const CHDBPoco::URI & uri, const ProxyConfiguration & proxy_configuration)
{
