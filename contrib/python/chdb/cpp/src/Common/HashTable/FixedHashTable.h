...
public:
    ...
    {
    public:
        explicit Reader(DB_CHDB::ReadBuffer & in_) : in(in_) {}
    ...
    void read(DB_CHDB::ReadBuffer & rb)
    {
        ...
        for (size_t i = 0; i < m_size; ++i)
        {
            ...
            DB_CHDB::readVarUInt(place_value, rb);
            Cell x;
            x.read(rb);
    ...
    void readText(DB_CHDB::ReadBuffer & rb)
    {
        ...
        for (size_t i = 0; i < m_size; ++i)
        {
            ...
            DB_CHDB::readText(place_value, rb);
            Cell x;
            DB_CHDB::assertChar(',', rb);
            x.readText(rb);
            new (&buf[place_value]) Cell(x, *this);
