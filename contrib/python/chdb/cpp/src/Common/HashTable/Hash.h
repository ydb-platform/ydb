...
inline size_t DefaultHash64(T key)
{
    ...
    else if constexpr (std::is_same_v<T, DB_CHDB::UUID> || std::is_same_v<T, DB_CHDB::IPv6>)
    {
        return intHash64(
...
DEFINE_HASH(Float32)
DEFINE_HASH(Float64)
DEFINE_HASH(DB_CHDB::UUID)
DEFINE_HASH(DB_CHDB::IPv4)
DEFINE_HASH(DB_CHDB::IPv6)
...
struct UUIDHash
{
    size_t operator()(DB_CHDB::UUID x) const
    {
...
{
    size_t operator()(DB_CHDB::UUID x) const { return DB_CHDB::UUIDHelpers::getHighBytes(x); }
};
...
template <>
struct DefaultHash<DB_CHDB::UUID> : public UUIDHash {};
