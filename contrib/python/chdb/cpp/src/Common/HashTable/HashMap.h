...
struct HashMapCell
{
    ...
    {
        DB_CHDB::writeBinary(value.first, wb);
        DB_CHDB::writeBinary(value.second, wb);
    }

    void writeText(DB_CHDB::WriteBuffer & wb) const
    {
        DB_CHDB::writeDoubleQuoted(value.first, wb);
        DB_CHDB::writeChar(',', wb);
        DB_CHDB::writeDoubleQuoted(value.second, wb);
    }

    /// Deserialization, in binary and text form.
    void read(DB_CHDB::ReadBuffer & rb)
    {
        DB_CHDB::readBinary(value.first, rb);
        DB_CHDB::readBinary(value.second, rb);
    }

    void readText(DB_CHDB::ReadBuffer & rb)
    {
        DB_CHDB::readDoubleQuoted(value.first, rb);
        DB_CHDB::assertChar(',', rb);
        DB_CHDB::readDoubleQuoted(value.second, rb);
    }
...
public:
    ...
    {
        DB_CHDB::PrefetchingHelper prefetching;
        size_t prefetch_look_ahead = DB_CHDB::PrefetchingHelper::getInitialLookAheadValue();
        ...
        for (auto it = this->begin(), end = this->end(); it != end; ++it, ++i)
        {
            ...
            {
                if (i == DB_CHDB::PrefetchingHelper::iterationsToMeasure())
                {
                    prefetch_look_ahead = prefetching.calcPrefetchLookAhead();
                    prefetch_it = advanceIterator(prefetch_it, prefetch_look_ahead - DB_CHDB::PrefetchingHelper::getInitialLookAheadValue());
                }
    ...
    const typename Cell::Mapped & ALWAYS_INLINE at(const Key & x) const
    {
        if (auto it = this->find(x); it != this->end())
            return it->getMapped();
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Cannot find element in HashMap::at method");
    }
