...
public:
    ...
    void readAndMerge(DB_CHDB::ReadBuffer & rb)
    {
        Cell::State::read(rb);
...
public:
    ...
    void writeAsSingleLevel(DB_CHDB::WriteBuffer & wb) const
    {
        DB_CHDB::writeVarUInt(this->size(), wb);
        ...
        for (size_t i = 0; i < Base::NUM_BUCKETS; ++i)
        {
            ...
            {
                if (zero_written)
                    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "No more than one zero value expected");
