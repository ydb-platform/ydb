...
struct HashTableNoState
{
    ...
    void write(DB_CHDB::WriteBuffer &) const         {}
    void writeText(DB_CHDB::WriteBuffer &) const     {}

    /// Deserialization, in binary and text form.
    void read(DB_CHDB::ReadBuffer &)                 {}
    void readText(DB_CHDB::ReadBuffer &)             {}
};
...
struct HashTableCell
{
    ...
    /// Serialization, in binary and text form.
    void write(DB_CHDB::WriteBuffer & wb) const         { DB_CHDB::writeBinaryLittleEndian(key, wb); }
    void writeText(DB_CHDB::WriteBuffer & wb) const     { DB_CHDB::writeDoubleQuoted(key, wb); }

    /// Deserialization, in binary and text form.
    void read(DB_CHDB::ReadBuffer & rb)        { DB_CHDB::readBinaryLittleEndian(key, rb); }
    void readText(DB_CHDB::ReadBuffer & rb)    { DB_CHDB::readDoubleQuoted(key, rb); }
...
struct ZeroValueStorage<false, Cell>
{
    ...
    void setHasZero() { throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "HashTable: logical error"); }
    void clearHasZero() {}
    void clearHasZeroFlag() {}
...
protected:
    ...
    {
        size_t size = 0;
        if (common_CHDB::mulOverflow(buffer_size, sizeof(Cell), size))
            throw DB_CHDB::Exception(
                DB_CHDB::ErrorCodes::CANNOT_ALLOCATE_MEMORY,
...
public:
    ...
    {
    public:
        explicit Reader(DB_CHDB::ReadBuffer & in_)
            : in(in_)
        {
        }
        ...
        const value_type & get() const
        {
            if (!is_initialized || is_eof)
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NO_AVAILABLE_DATA, "No available data");

            return cell.getValue();
        }

    private:
        DB_CHDB::ReadBuffer & in;
        Cell cell{};
        size_t read_count = 0;
        size_t size = 0;
        bool is_eof = false;
...
public:
    ...
    void write(DB_CHDB::WriteBuffer & wb) const
    {
        Cell::State::write(wb);
        DB_CHDB::writeVarUInt(m_size, wb);
    ...
    void writeText(DB_CHDB::WriteBuffer & wb) const
    {
        Cell::State::writeText(wb);
        DB_CHDB::writeText(m_size, wb);
    ...
    void read(DB_CHDB::ReadBuffer & rb)
    {
        Cell::State::read(rb);
        ...
        size_t new_size = 0;
        DB_CHDB::readVarUInt(new_size, rb);
    ...
    void readText(DB_CHDB::ReadBuffer & rb)
    {
