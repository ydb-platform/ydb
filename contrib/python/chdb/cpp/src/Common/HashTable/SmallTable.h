    ...
    public:
        ...
        bool next()
        {
            if (!is_initialized)
            {
                ...
                if (size > capacity)
                    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::INCORRECT_DATA, "Illegal size");
...
public:
    ...
    void read(DB_CHDB::ReadBuffer & rb)
    {
        ...
        if (new_size > 1000'000)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::TOO_LARGE_ARRAY_SIZE, "The size of serialized small table is suspiciously large: {}", new_size);
