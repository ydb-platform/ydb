...
HostResolver::HostResolver(String host_, CHDBPoco::Timespan history_)
    : host(std::move(host_))
    , history(history_)
...
HostResolver::HostResolver(
    ResolveFunction && resolve_function_, String host_, CHDBPoco::Timespan history_)
...
void HostResolver::setSuccess(const CHDBPoco::Net::IPAddress & address)
{
    std::lock_guard lock(mutex);
...
void HostResolver::setFail(const CHDBPoco::Net::IPAddress & address)
{
    CHDBPoco::Timestamp now;
...
CHDBPoco::Net::IPAddress HostResolver::selectBest()
{
...
HostResolver::Records::iterator HostResolver::find(const CHDBPoco::Net::IPAddress & addr) TSA_REQUIRES(mutex)
{
    auto it = std::lower_bound(
        records.begin(), records.end(), addr, [](const Record & rec, const CHDBPoco::Net::IPAddress & value) { return rec.address < value; });
...
void HostResolver::updateImpl(CHDBPoco::Timestamp now, std::vector<CHDBPoco::Net::IPAddress> & next_gen)
    TSA_REQUIRES(mutex)
{
    ...
    for (auto & record : merged)
    {
        ...
        auto banned_until = now - CHDBPoco::Timespan(history.totalMicroseconds() * (1ull << (record.consecutive_fail_count - 1)));
