...
// - resolve schedule
//    Addresses are resolved through `DB_CHDB::DNSResolver::instance()`.
...
public:
    ...
    private:
        ...
        Entry(HostResolver & pool_, CHDBPoco::Net::IPAddress address_)
...
protected:
    explicit HostResolver(
        String host_,
        CHDBPoco::Timespan history_ = CHDBPoco::Timespan(DEFAULT_RESOLVE_TIME_HISTORY_SECONDS, 0));

    using ResolveFunction = std::function<std::vector<CHDBPoco::Net::IPAddress> (const String & host)>;
    HostResolver(ResolveFunction && resolve_function_,
                    String host_,
                    CHDBPoco::Timespan history_);

    friend class Entry;
    WeakPtr getWeakFromThis();

    void setSuccess(const CHDBPoco::Net::IPAddress & address);
    void setFail(const CHDBPoco::Net::IPAddress & address);

    struct Record
    {
        Record(CHDBPoco::Net::IPAddress address_, CHDBPoco::Timestamp resolve_time_)
            : address(address_)
        ...
        CHDBPoco::Net::IPAddress address;
        CHDBPoco::Timestamp resolve_time;
        size_t usage = 0;
        bool failed = false;
        CHDBPoco::Timestamp fail_time = 0;
        ...
        bool setFail(const CHDBPoco::Timestamp & now)
        {
            bool was_ok = !failed;
    ...
    CHDBPoco::Net::IPAddress selectBest() TSA_REQUIRES(mutex);
    Records::iterator find(const CHDBPoco::Net::IPAddress & address) TSA_REQUIRES(mutex);
    bool isUpdateNeeded();

    void updateImpl(CHDBPoco::Timestamp now, std::vector<CHDBPoco::Net::IPAddress> & next_gen) TSA_REQUIRES(mutex);
    ...
    CHDBPoco::Timespan getRecordHistoryTime(const Record&) const;

    const String host;
    const CHDBPoco::Timespan history;
    ...
    CHDBPoco::Timestamp last_resolve_time TSA_GUARDED_BY(mutex) = CHDBPoco::Timestamp::TIMEVAL_MIN;
    Records records TSA_GUARDED_BY(mutex);

    CHDBPoco::Logger * log = &CHDBPoco::Logger::get("ConnectionPool");
};
