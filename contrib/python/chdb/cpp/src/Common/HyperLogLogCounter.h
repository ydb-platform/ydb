...
private:
    ...
    using Value = UInt64;
    using RankStore = DB_CHDB::CompactArray<HashValueType, rank_width, bucket_count>;
...
public:
    ...
    void read(DB_CHDB::ReadBuffer & in)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < denominator_copy.size(); i += (sizeof(UInt32) / sizeof(char)))
            {
                ...
                DB_CHDB::transformEndianness<std::endian::native, std::endian::little>(*cur);
            }
            ...
            DB_CHDB::transformEndianness<std::endian::native, std::endian::little>(zeros);
        }
    }

    void readAndMerge(DB_CHDB::ReadBuffer & in)
    {
    ...
    void write(DB_CHDB::WriteBuffer & out) const
    {
       ...
       else
       {
            ...
            {
                UInt32 * cur = reinterpret_cast<UInt32 *>(&denominator_copy[i]);
                DB_CHDB::transformEndianness<std::endian::little, std::endian::native>(*cur);
            }
            out.write(denominator_copy.begin(), denom_size);

            auto zeros_copy = zeros;
            DB_CHDB::transformEndianness<std::endian::little, std::endian::native>(zeros_copy);
    ...
    void readText(DB_CHDB::ReadBuffer & in)
    {
    ...
    static void skipText(DB_CHDB::ReadBuffer & in)
    {
        UInt8 dummy;
        for (size_t i = 0; i < RankStore::size(); ++i)
        {
            if (i != 0)
                DB_CHDB::assertChar(',', in);
            DB_CHDB::readIntText(dummy, in);
        }
    }

    void writeText(DB_CHDB::WriteBuffer & out) const
    {
...
private:
    ...
    double fixRawEstimate(double raw_estimate) const
    {
        ...
        else if (mode == HyperLogLogMode::FullFeatured)
        {
            ...
            throw CHDBPoco::Exception("Internal error", DB_CHDB::ErrorCodes::LOGICAL_ERROR);
    }
