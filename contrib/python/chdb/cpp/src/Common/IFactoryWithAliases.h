...
protected:
    ...
    String getAliasToOrName(const String & name) const
    {
        ...
        else if (String name_lowercase = CHDBPoco::toLower(name); case_insensitive_aliases.contains(name_lowercase))
...
public:
    ...
    void registerAlias(const String & alias_name, const String & real_name, Case case_sensitiveness = Sensitive)
    {
        ...
        String real_name_lowercase = CHDBPoco::toLower(real_name);
    ...
    {
        String alias_name_lowercase = CHDBPoco::toLower(alias_name);
    ...
    {
        String name_lowercase = CHDBPoco::toLower(name);
    ...
    const String & aliasTo(const String & name) const
    {
        if (auto it = aliases.find(name); it != aliases.end())
            return it->second;
        else if (auto jt = case_insensitive_aliases.find(CHDBPoco::toLower(name)); jt != case_insensitive_aliases.end())
            return jt->second;
    ...
    {
        auto it = case_insensitive_name_mapping.find(CHDBPoco::toLower(name));
