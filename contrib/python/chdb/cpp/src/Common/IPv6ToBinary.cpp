...
void IPv6ToRawBinary(const CHDBPoco::Net::IPAddress & address, char * res)
{
    if (CHDBPoco::Net::IPAddress::IPv6 == address.family())
    {
        memcpy(res, address.addr(), 16);
    }
    else if (CHDBPoco::Net::IPAddress::IPv4 == address.family())
    {
...
std::array<char, 16> IPv6ToBinary(const CHDBPoco::Net::IPAddress & address)
{
