...
using LoggerPtr = std::shared_ptr<CHDBPoco::Logger>;
using LoggerRawPtr = CHDBPoco::Logger *;
...
{
    return CHDBPoco::Logger::getShared(name, false /*should_be_owned_by_shared_ptr_if_created*/);
}
...
/** Create Logger with specified name, channel and logging level.
  ...
  * Logger is destroyed, when last shared ptr that refers to Logger with specified name is destroyed.
  */
LoggerPtr createLogger(const std::string & name, CHDBPoco::Channel * channel, CHDBPoco::Message::Priority level = CHDBPoco::Message::PRIO_INFORMATION);

/** Create raw CHDBPoco::Logger that will not be destroyed before program termination.
...
LoggerRawPtr createRawLogger(const std::string & name, CHDBPoco::Channel * channel, CHDBPoco::Message::Priority level = CHDBPoco::Message::PRIO_INFORMATION);
