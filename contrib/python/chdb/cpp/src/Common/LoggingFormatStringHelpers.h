...
public:
    ...
    bool is(CHDBPoco::Message::Priority priority) { return logger->is(priority); }
    LogFrequencyLimiterIml * getChannel() {return this; }
    const String & name() const { return logger->name(); }

    void log(CHDBPoco::Message & message);
...
public:
    ...
    bool is(CHDBPoco::Message::Priority priority) { propagate_to_actual_log &= logger->is(priority); return true; }
