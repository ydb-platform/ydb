...
private:
    ...
    public:
        explicit RegexpCharClassFunction(Regexp * re_)
        {
            ...
            chassert(cc);
            if (cc->empty())
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "kRegexpCharClass is empty");
...
public:
    ...
    {
        if (root == nullptr)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "no root has been set");

        if (generators.empty())
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "no generators");
...
private:
    ...
    {
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "ShortVisit should not be called");
    }
