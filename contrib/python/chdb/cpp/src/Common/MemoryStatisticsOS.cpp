...
MemoryStatisticsOS::Data MemoryStatisticsOS::get() const
{
    ...
    if (sizeof(struct kinfo_proc) != len)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::SYSTEM_ERROR, "Kernel returns structure of {} bytes instead of expected {}",
            len, sizeof(struct kinfo_proc));

    if (sizeof(struct kinfo_proc) != kp.ki_structsize)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::SYSTEM_ERROR, "Kernel structure size ({}) does not match expected ({}).",
