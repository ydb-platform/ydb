...
inline std::string_view toDescription(OvercommitResult result)
{
    ...
    case OvercommitResult::MEMORY_FREED:
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "OvercommitResult::MEMORY_FREED shouldn't be asked for description");
...
void AllocationTrace::onAllocImpl(void * ptr, size_t size) const
{
    ...
    DB_CHDB::TraceSender::send(DB_CHDB::TraceType::MemorySample, StackTrace(), {.size = Int64(size), .ptr = ptr});
}
...
void MemoryTracker::injectFault() const
{
    ...
    throw DB_CHDB::Exception(
        DB_CHDB::ErrorCodes::MEMORY_LIMIT_EXCEEDED,
...
{
    if (size < 0)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Negative size ({}) is passed to MemoryTracker. It is a bug.", size);
    ...
    {
        MemoryTrackerBlockerInThread untrack_lock(VariableContext::Global);
        DB_CHDB::TraceSender::send(DB_CHDB::TraceType::Memory, StackTrace(), {.size = size});
    ...
    {
        MemoryTrackerBlockerInThread untrack_lock(VariableContext::Global);
        DB_CHDB::TraceSender::send(DB_CHDB::TraceType::MemoryPeak, StackTrace(), {.size = will_be});
    }
...
void MemoryTracker::setParent(MemoryTracker * elem)
{
    ...
    if (level == VariableContext::Thread && DB_CHDB::current_thread)
        DB_CHDB::current_thread->flushUntrackedMemory();
