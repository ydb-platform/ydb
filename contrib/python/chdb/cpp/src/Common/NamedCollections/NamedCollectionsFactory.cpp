...
namespace
{
    ...
    std::vector<std::string> listCollections(const CHDBPoco::Util::AbstractConfiguration & config)
    {
        CHDBPoco::Util::AbstractConfiguration::Keys collections_names;
    ...
    NamedCollectionsMap getNamedCollections(const CHDBPoco::Util::AbstractConfiguration & config)
    {
...
void NamedCollectionFactory::loadFromConfig(const CHDBPoco::Util::AbstractConfiguration & config, std::lock_guard<std::mutex> & lock)
{
...
void NamedCollectionFactory::reloadFromConfig(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(mutex);
    if (loadIfNot(lock))
        return;
            ...
            catch (const Coordination::Exception & e)
            {
                if (Coordination::isHardwareError(e.code))
                {
                    LOG_INFO(log, "Lost ZooKeeper connection, will try to connect again: {}",
                            DB_CHDB::getCurrentExceptionMessage(true));
