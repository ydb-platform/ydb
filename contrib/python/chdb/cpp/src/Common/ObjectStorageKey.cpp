...
{
    if (!is_relative)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "object key has no prefix, key: {}", key);
...
{
    if (!is_relative)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "object key has no suffix, key: {}", key);
    return suffix;
}
