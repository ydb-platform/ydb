...
catch (...)
{
    ...
    alternatives.clear();
    LOG_ERROR(getLogger("OptimizeRegularExpression"), "Analyze RegularExpression failed, got error: {}", DB_CHDB::getCurrentExceptionMessage(false));
}
...
OptimizedRegularExpression::OptimizedRegularExpression(const std::string & regexp_, int options)
{
    ...
    if (options & (~(RE_CASELESS | RE_NO_CAPTURE | RE_DOT_NL)))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_COMPILE_REGEXP, "OptimizedRegularExpression: Unsupported option.");
    ...
    if (!is_trivial)
    {
        ...
        if (!re2->ok())
        {
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_COMPILE_REGEXP,
        ...
        if (!is_no_capture)
        {
            ...
            if (number_of_subpatterns > MAX_SUBPATTERNS)
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_COMPILE_REGEXP, "OptimizedRegularExpression: too many subpatterns in regexp: {}", regexp_);
        }
    }
...
unsigned OptimizedRegularExpression::match(const char * subject, size_t subject_size, MatchVec & matches, unsigned limit) const
{
    ...
    else
    {
        ...
        DB_CHDB::PODArrayWithStackMemory<std::string_view, 128> pieces(limit);
