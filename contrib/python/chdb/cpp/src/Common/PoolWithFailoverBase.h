        ...
        const GetPriorityFunc & get_priority)
{
    ...
    if (usable_count < min_entries)
        throw DB_CHDB::NetException(DB_CHDB::ErrorCodes::ALL_CONNECTION_TRIES_FAILED,
    ...
    else if (up_to_date_count >= min_entries)
    {
        if (try_results.size() < up_to_date_count)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Could not find enough connections for up-to-date results. Got: {}, needed: {}", try_results.size(), up_to_date_count);

        /// There is enough up-to-date entries.
        try_results.resize(up_to_date_count);
    }
    else
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::ALL_REPLICAS_ARE_STALE,
