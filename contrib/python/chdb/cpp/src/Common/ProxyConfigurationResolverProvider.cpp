...
namespace
{
    std::string getNoProxyHosts(const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
        return configuration.getString("proxy.no_proxy", "");
    }

    bool isTunnelingDisabledForHTTPSRequestsOverHTTPProxy(
        const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
        ...
        const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
        ...
        auto endpoint = CHDBPoco::URI(configuration.getString(resolver_prefix + ".endpoint"));
    ...
    auto extractURIList(const String & config_prefix, const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
        std::vector<String> keys;
        configuration.keys(config_prefix, keys);

        std::vector<CHDBPoco::URI> uris;
        for (const auto & key : keys)
        {
            if (startsWith(key, "uri"))
            {
                CHDBPoco::URI proxy_uri(configuration.getString(config_prefix + "." + key));
    ...
    bool hasRemoteResolver(const String & config_prefix, const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
        return configuration.has(config_prefix + ".resolver");
    }

    bool hasListResolver(const String & config_prefix, const CHDBPoco::Util::AbstractConfiguration & configuration)
    {
