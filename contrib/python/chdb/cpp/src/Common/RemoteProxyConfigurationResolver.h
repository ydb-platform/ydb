...
struct RemoteProxyHostFetcher
{
    ...
    virtual std::string fetch(const CHDBPoco::URI & endpoint, const ConnectionTimeouts & timeouts) = 0;
};

struct RemoteProxyHostFetcherImpl : public RemoteProxyHostFetcher
{
    std::string fetch(const CHDBPoco::URI & endpoint, const ConnectionTimeouts & timeouts) override;
};
