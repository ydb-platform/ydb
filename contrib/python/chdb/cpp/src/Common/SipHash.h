    ...
    ALWAYS_INLINE void update(const T & x)
    {
        if constexpr (std::endian::native == std::endian::big)
        {
            ...
            if constexpr (!std::is_same_v<Transform, void>)
                ...
                DB_CHDB::transformEndianness<std::endian::little>(transformed_x);
    ...
    UInt128 get128Reference()
    {
        if (!is_reference_128)
            throw DB_CHDB::Exception(
                DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Can't call get128Reference when is_reference_128 is not set");
        finalize();
