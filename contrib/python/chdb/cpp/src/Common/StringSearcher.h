...
public:
        ...
        , needle_size(needle_size_)
    {
        ...
        else
        {
            ...
            {
                uint32_t first_l_u32 = CHDBPoco::Unicode::toLower(*first_u32);
                uint32_t first_u_u32 = CHDBPoco::Unicode::toUpper(*first_u32);
        ...
        for (size_t i = 0; i < N;)
        {
            ...
            if (c_u32)
            {
                int c_l_u32 = CHDBPoco::Unicode::toLower(*c_u32);
                int c_u_u32 = CHDBPoco::Unicode::toUpper(*c_u32);
    ...
    ALWAYS_INLINE bool compareTrivial(const CharT * haystack_pos, const CharT * const haystack_end, const uint8_t * needle_pos) const
    {
        while (haystack_pos < haystack_end && needle_pos < needle_end)
        {
            ...
            if (CHDBPoco::Unicode::toLower(*haystack_code_point) != CHDBPoco::Unicode::toLower(*needle_code_point))
                break;
