...
public:
    ...
    DB_CHDB::OpenTelemetry::TracingContextOnThread thread_trace_context;
    ...
    JobWithPriority(
        ...
        const DB_CHDB::OpenTelemetry::TracingContextOnThread & thread_trace_context_,
        ...
        thread_trace_context(thread_trace_context_), enable_job_stack_trace(capture_frame_pointers)
    {
        ...
        frame_pointers = DB_CHDB::Exception::getThreadFramePointers();
...
ReturnType ThreadPoolImpl<Thread>::scheduleImpl(Job job, Priority priority, std::optional<uint64_t> wait_microseconds, bool propagate_opentelemetry_tracing_context)
{
    auto on_error = [&](const std::string & reason)
    {
        if constexpr (std::is_same_v<ReturnType, void>)
        {
            ...
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_SCHEDULE_TASK,
...

    {
        ...
        jobs.emplace(std::move(job),
                     ...
                     propagate_opentelemetry_tracing_context ? DB_CHDB::OpenTelemetry::CurrentContext() : DB_CHDB::OpenTelemetry::TracingContextOnThread(),
                     /// capture_frame_pointers
                     DB_CHDB::Exception::enable_job_stack_trace);
...
void ThreadPoolImpl<Thread>::worker(typename std::list<Thread>::iterator thread_it)
{
    ...
    while (true)
    {
        ...
        DB_CHDB::OpenTelemetry::TracingContextHolder thread_trace_context("ThreadPool::worker()", job_data->thread_trace_context);

        /// Run the job.
        try
        {
            if (DB_CHDB::Exception::enable_job_stack_trace)
                DB_CHDB::Exception::setThreadFramePointers(std::move(job_data->frame_pointers));
