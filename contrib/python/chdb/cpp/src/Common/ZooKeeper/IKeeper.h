...
class Exception : public DB_CHDB::Exception
{
private:
    ...
    Exception(T && message, Error code_) : DB_CHDB::Exception(std::forward<T>(message), DB_CHDB::ErrorCodes::KEEPER_EXCEPTION, /* remote_= */ false), code(code_)
    {
...
public:
    ...
    Exception(Error code_, FormatStringHelper<Args...> fmt, Args &&... args)
        : DB_CHDB::Exception(DB_CHDB::ErrorCodes::KEEPER_EXCEPTION, std::move(fmt), std::forward<Args>(args)...)
        , code(code_)
    {
        incrementErrorMetrics(code);
    }
...
public:
    ...
    virtual bool isFeatureEnabled(DB_CHDB::KeeperFeatureFlag feature_flag) const = 0;

    virtual const DB_CHDB::KeeperFeatureFlags * getKeeperFeatureFlags() const { return nullptr; }
