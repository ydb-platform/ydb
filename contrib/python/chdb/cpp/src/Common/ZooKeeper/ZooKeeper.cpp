...
void ZooKeeper::updateAvailabilityZones()
{
    ...
    for (const auto & node : shuffled_hosts)
    {
        ...
        {
            DB_CHDB::tryLogCurrentException(log, "Failed to get availability zone for " + node.host);
        }
    }
...
void ZooKeeper::init(ZooKeeperArgs args_, std::unique_ptr<Coordination::IKeeper> existing_impl)
{
    ...
    else if (args.implementation == "zookeeper")
    {
        ...
        if (availability_zones.empty())
        {
            ...
            LOG_TEST(log, "Availability zones from config: [{}], client: {}",
                fmt::join(collections::map(availability_zones, [](auto s){ return DB_CHDB::quoteString(s); }), ", "),
                DB_CHDB::quoteString(args.client_availability_zone));
        ...
        if (connected_to_suboptimal_node && may_benefit_from_reconnecting)
        {
            ...
            auto reconnect_task_holder = DB_CHDB::Context::getGlobalContextInstance()->getSchedulePool().createTask("ZKReconnect", [this, optimal_host = shuffled_hosts[0]]()
            {
                try
                {
                ...
                {
                    LOG_WARNING(log, "Failed to connect to a more optimal ZooKeeper, will try again later: {}", DB_CHDB::getCurrentExceptionMessage(/*with_stacktrace*/ false));
                    (*reconnect_task)->scheduleAfter(getSecondsUntilReconnect(args) * 1000);
                }
            });
            reconnect_task = std::make_unique<DB_CHDB::BackgroundSchedulePoolTaskHolder>(std::move(reconnect_task_holder));
    ...
    {
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NOT_IMPLEMENTED, "Unknown implementation of coordination service: {}", args.implementation);
    }
...
ZooKeeper::ZooKeeper(const ZooKeeperArgs & args_, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_)
...
ZooKeeper::ZooKeeper(const ZooKeeperArgs & args_, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_, Strings availability_zones_, std::unique_ptr<Coordination::IKeeper> existing_impl)
    : availability_zones(std::move(availability_zones_)), zk_log(std::move(zk_log_))
{
    if (availability_zones.size() != args_.hosts.size())
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Argument sizes mismatch: availability_zones count {} and hosts count {}",
                            availability_zones.size(), args_.hosts.size());
    init(args_, std::move(existing_impl));
}


ZooKeeper::ZooKeeper(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_)
    : zk_log(std::move(zk_log_))
{
...
bool ZooKeeper::configChanged(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name) const
{
...
void ZooKeeper::createAncestors(const std::string & path)
{
    ...
    if (isFeatureEnabled(DB_CHDB::KeeperFeatureFlag::CREATE_IF_NOT_EXISTS))
    {
...
{
    zkutil::EventPtr eph_node_disappeared = std::make_shared<CHDBPoco::Event>();
    String content;
    Coordination::Stat stat;
    ...
    else
    {
        ...
        if (!eph_node_disappeared->tryWait(timeout_ms))
            throw DB_CHDB::Exception(
                DB_CHDB::ErrorCodes::LOGICAL_ERROR,
...
ZooKeeperPtr ZooKeeper::create(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_)
{
...
{
    String session_path = fs::path(args.sessions_path) / args.zookeeper_name / toString(DB_CHDB::ServerUUID::get());
    Coordination::Stat stat;
...
bool ZooKeeper::isFeatureEnabled(DB_CHDB::KeeperFeatureFlag feature_flag) const
{
...
void ZooKeeper::setZooKeeperLog(std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_)
{
    zk_log = std::move(zk_log_);
...
{
    if (responses.empty())
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Responses for multi transaction is empty");
    ...
    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "There is no failed OpResult");
}
...
std::string normalizeZooKeeperPath(std::string zookeeper_path, bool check_starts_with_slash, LoggerPtr log)
{
    ...
    if (!zookeeper_path.empty() && zookeeper_path.front() != '/')
    {
        ...
        if (check_starts_with_slash)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path must starts with '/', got '{}'", zookeeper_path);
        if (log)
...
String extractZooKeeperName(const String & path)
{
    ...
    if (path.empty())
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path should not be empty");
    if (path[0] == '/')
    ...
    if (pos != String::npos && pos < path.find('/'))
    {
        ...
        if (zookeeper_name.empty())
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Zookeeper path should start with '/' or '<auxiliary_zookeeper_name>:/'");
...
void validateZooKeeperConfig(const CHDBPoco::Util::AbstractConfiguration & config)
{
    if (config.has("zookeeper") && config.has("keeper"))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG, "Both ZooKeeper and Keeper are specified");
}

bool hasZooKeeperConfig(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
String getZooKeeperConfigName(const CHDBPoco::Util::AbstractConfiguration & config)
{
    ...
    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NO_ELEMENTS_IN_CONFIG, "There is no Zookeeper configuration in server config");
}
