...
struct ShuffleHost
{
    ...
    mutable std::optional<CHDBPoco::Net::SocketAddress> address;
...
using GetPriorityForLoadBalancing = DB_CHDB::GetPriorityForLoadBalancing;
        ...
        return std::visit(
            [&]<typename TResponses>(const TResponses & resp) -> size_t
            {
                if constexpr (std::same_as<TResponses, std::monostate>)
                    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "No responses set for MultiRead");
                else
                    return resp.size();
            },
            responses);
...
///
/// CHDBPoco::Event objects are used for watches. The event is set only once on the first
/// watch notification.
...
class ZooKeeper
{
    ...
    friend class DB_CHDB::ZooKeeperWithFaultInjection;

    explicit ZooKeeper(const ZooKeeperArgs & args_, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_ = nullptr);

    /// Allows to keep info about availability zones when starting a new session
    ZooKeeper(const ZooKeeperArgs & args_, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_, Strings availability_zones_, std::unique_ptr<Coordination::IKeeper> existing_impl);
    ...
    */
    ZooKeeper(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name, std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_ = nullptr);
...
public:
    ...
    static Ptr create(const CHDBPoco::Util::AbstractConfiguration & config,
                      const std::string & config_name,
                      std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_);
    ...
    bool configChanged(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name) const;

    /// Returns true, if the session has expired.
    bool expired();

    bool isFeatureEnabled(DB_CHDB::KeeperFeatureFlag feature_flag) const;
    ...
    void setZooKeeperLog(std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log_);
    ...
    const DB_CHDB::KeeperFeatureFlags * getKeeperFeatureFlags() const { return impl->getKeeperFeatureFlags(); }
...
private:
    ...
    {
        if (isFeatureEnabled(DB_CHDB::KeeperFeatureFlag::MULTI_READ))
        {
    ...
    std::shared_ptr<DB_CHDB::ZooKeeperLog> zk_log;

    AtomicStopwatch session_uptime;

    int32_t session_node_version;

    std::unique_ptr<DB_CHDB::BackgroundSchedulePoolTaskHolder> reconnect_task;
};
...
public:
    ...
    ~EphemeralNodeHolder()
    {
        ...
        {
            ProfileEvents::increment(ProfileEvents::CannotRemoveEphemeralNode);
            DB_CHDB::tryLogCurrentException(__PRETTY_FUNCTION__, "Cannot remove " + path);
        }
    }
...
{
    if (client.isFeatureEnabled(DB_CHDB::KeeperFeatureFlag::CHECK_NOT_EXISTS))
    {
