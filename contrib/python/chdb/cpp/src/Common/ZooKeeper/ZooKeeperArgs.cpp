...
ZooKeeperArgs::ZooKeeperArgs(const CHDBPoco::Util::AbstractConfiguration & config, const String & config_name)
{
    ...
    for (size_t i = 0; i < hosts.size(); ++i)
    {
        ...
        get_priority_load_balancing.hostname_prefix_distance[i] = DB_CHDB::getHostNamePrefixDistance(local_hostname, node_host);
        get_priority_load_balancing.hostname_levenshtein_distance[i] = DB_CHDB::getHostNameLevenshteinDistance(local_hostname, node_host);
    }
}
...
void ZooKeeperArgs::initFromKeeperServerSection(const CHDBPoco::Util::AbstractConfiguration & config)
{
    ...
    for (const auto * load_balancing_key : load_balancing_keys)
    {
            ...
            config.has(load_balancing_config))
        {
            ...
            auto load_balancing = magic_enum::enum_cast<DB_CHDB::LoadBalancing>(CHDBPoco::toUpper(load_balancing_str));
            if (!load_balancing)
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Unknown load balancing: {}", load_balancing_str);
            get_priority_load_balancing = DB_CHDB::GetPriorityForLoadBalancing(*load_balancing, thread_local_rng() % hosts.size());
            break;
        }
    }
    ...
    if (prefer_local_availability_zone)
        client_availability_zone = DB_CHDB::PlacementInfo::PlacementInfo::instance().getAvailabilityZone();
}

void ZooKeeperArgs::initFromKeeperSection(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name)
{
    ...
    std::optional<DB_CHDB::LoadBalancing> load_balancing;
    ...
    for (const auto & key : keys)
    {
        ...
        else if (key == "zookeeper_load_balancing" || key == "keeper_load_balancing")
        {
            ...
            /// Use magic_enum to avoid dependency from dbms (`SettingFieldLoadBalancingTraits::fromString(...)`)
            load_balancing = magic_enum::enum_cast<DB_CHDB::LoadBalancing>(CHDBPoco::toUpper(load_balancing_str));
            if (!load_balancing)
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Unknown load balancing: {}", load_balancing_str);
        }
