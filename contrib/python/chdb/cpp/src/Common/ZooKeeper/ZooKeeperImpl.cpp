    ...
    CHDBPoco::Timespan connection_timeout)
{
    ...
    for (const auto & node : nodes)
    {
        try
        {
            const CHDBPoco::Net::SocketAddress host_socket_addr{node.host};
        ...
        catch (const CHDBPoco::Net::HostNotFoundException & e)
        {
...
void ZooKeeper::initFeatureFlags()
{
    ...
    DB_CHDB::KeeperApiVersion keeper_api_version{DB_CHDB::KeeperApiVersion::ZOOKEEPER_COMPATIBLE};
    ...
    DB_CHDB::ReadBufferFromOwnString buf(*keeper_api_version_string);
    uint8_t keeper_version{0};
    DB_CHDB::readIntText(keeper_version, buf);
    keeper_api_version = static_cast<DB_CHDB::KeeperApiVersion>(keeper_version);
