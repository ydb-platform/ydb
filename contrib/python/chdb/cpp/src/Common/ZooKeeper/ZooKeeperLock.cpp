...
void ZooKeeperLock::unlock()
{
    ...
    if (result && stat.ephemeralOwner == zookeeper->getClientID())
    {
        ...
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Lock is lost, it has another owner. Path: {}, message: {}, owner: {}, our id: {}",
                        lock_path, lock_message, stat.ephemeralOwner, zookeeper->getClientID());
    else if (throw_if_lost)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Lock is lost, node does not exist. Path: {}, message: {}, our id: {}",
