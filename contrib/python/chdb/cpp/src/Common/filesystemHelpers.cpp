...
String getBlockDeviceId([[maybe_unused]] const String & path)
{
    ...
    if (lstat(path.c_str(), &sb))
        DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_STAT, path, "Cannot lstat {}", path);
    WriteBufferFromOwnString ss;
    ss << major(sb.st_dev) << ":" << minor(sb.st_dev);
    return ss.str();
#else
    throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "The function getDeviceId is supported on Linux only");
#endif
}
...
#else
    throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "The function getDeviceType is supported on Linux only");
#endif
}
...
std::filesystem::path getMountPoint(std::filesystem::path absolute_path)
{
    ...
    const auto get_device_id = [](const std::filesystem::path & p)
    {
        ...
        if (stat(p.c_str(), &st))   /// NOTE: man stat does not list EINTR as possible error
            DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::SYSTEM_ERROR, p.string(), "Cannot stat {}", p.string());
        return st.st_dev;
    };
...
String getFilesystemName([[maybe_unused]] const String & mount_point)
{
    ...
    if (!mounted_filesystems)
        throw DB_CHDB::Exception(ErrorCodes::SYSTEM_ERROR, "Cannot open /etc/mtab to get name of filesystem");
    mntent fs_info;
    ...
    if (fs_info.mnt_dir != mount_point)
        throw DB_CHDB::Exception(ErrorCodes::SYSTEM_ERROR, "Cannot find name of filesystem by mount point {}", mount_point);
    return fs_info.mnt_fsname;
#else
    throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "The function getFilesystemName is supported on Linux only");
#endif
}
...
size_t getSizeFromFileDescriptor(int fd, const String & file_name)
{
    ...
    if (-1 == res)
    {
        DB_CHDB::ErrnoException::throwFromPath(
            DB_CHDB::ErrorCodes::CANNOT_FSTAT, file_name, "Cannot execute fstat{}", file_name.empty() ? "" : " file: " + file_name);
    }
    return buf.st_size;
}
...
Int64 getINodeNumberFromPath(const String & path)
{
    ...
    {
        DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_STAT, path, "Cannot execute stat for file {}", path);
    }
...
bool createFile(const std::string & path)
{
    ...
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_CREATE_FILE, path, "Cannot create file: {}", path);
}
...
bool canRead(const std::string & path)
{
    ...
    if (errno == EACCES)
        return false;
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::PATH_ACCESS_DENIED, path, "Cannot check read access to file: {}", path);
}
...
bool canWrite(const std::string & path)
{
    ...
    if (errno == EACCES)
        return false;
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::PATH_ACCESS_DENIED, path, "Cannot check write access to file: {}", path);
}
...
bool canExecute(const std::string & path)
{
    ...
    if (errno == EACCES)
        return false;
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::PATH_ACCESS_DENIED, path, "Cannot check execute access to file: {}", path);
}
...
{
    return CHDBPoco::Timestamp::fromEpochTime(getModificationTime(path));
}
...
void setModificationTime(const std::string & path, time_t time)
{
    ...
    if (utime(path.c_str(), &tb) != 0)
        DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::PATH_ACCESS_DENIED, path, "Cannot set modification time to file: {}", path);
}
