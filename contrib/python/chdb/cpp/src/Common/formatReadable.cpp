...
static void formatReadable(double size, DB_CHDB::WriteBuffer & out,
    int precision, const char ** units, size_t units_size, double delimiter)
{
    ...
    const auto & converter = DB_CHDB::DoubleConverter<false>::instance();
    ...
    if (!result)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Cannot print float or double number");
...
void formatReadableSizeWithBinarySuffix(double value, DB_CHDB::WriteBuffer & out, int precision)
{
...
void formatReadableSizeWithDecimalSuffix(double value, DB_CHDB::WriteBuffer & out, int precision)
{
...
void formatReadableQuantity(double value, DB_CHDB::WriteBuffer & out, int precision)
{
