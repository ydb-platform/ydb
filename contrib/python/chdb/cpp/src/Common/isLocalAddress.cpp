...
struct NetworkInterfaces : public boost::noncopyable
{
    ...
    bool hasAddress(const CHDBPoco::Net::IPAddress & address) const
    {
        ifaddrs * iface;
        for (iface = ifaddr; iface != nullptr; iface = iface->ifa_next)
        {
            ...
            std::optional<CHDBPoco::Net::IPAddress> interface_address;
            switch (family)
            {
...
bool isLocalAddress(const CHDBPoco::Net::IPAddress & address)
{
    ...
    {
        if (address.family() == CHDBPoco::Net::AddressFamily::IPv4)
        {
        ...
        else if (address.family() == CHDBPoco::Net::AddressFamily::IPv6)
        {
            return true;
        }
    }
