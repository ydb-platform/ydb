...
void setThreadName(const char * name, bool truncate)
{
    ...
    if (!truncate && name_len > THREAD_NAME_SIZE - 1)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::PTHREAD_ERROR, "Thread name cannot be longer than 15 bytes");
    ...
    //    throw DB_CHDB::ErrnoException(DB_CHDB::ErrorCodes::PTHREAD_ERROR, "Cannot set thread name with prctl(PR_SET_NAME, ...)");
...
const char * getThreadName()
{
    ...
    if (pthread_getname_np(pthread_self(), thread_name, THREAD_NAME_SIZE))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::PTHREAD_ERROR, "Cannot get thread name with pthread_getname_np()");
#elif defined(OS_FREEBSD)
...
//        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::PTHREAD_ERROR, "Cannot get thread name with pthread_get_name_np()");
#else
    if (0 != prctl(PR_GET_NAME, thread_name, 0, 0, 0))
        if (errno != ENOSYS && errno != EPERM)    /// It's ok if the syscall is unsupported or not allowed in some environments.
            throw DB_CHDB::ErrnoException(DB_CHDB::ErrorCodes::PTHREAD_ERROR, "Cannot get thread name with prctl(PR_GET_NAME)");
#endif
