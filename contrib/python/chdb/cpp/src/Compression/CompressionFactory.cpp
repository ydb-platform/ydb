...
CompressionCodecPtr CompressionCodecFactory::get(const String & family_name, std::optional<int> level) const
{
    if (level)
    {
        ...
        return get(makeASTFunction("CODEC", makeASTFunction(CHDBPoco::toUpper(family_name), level_literal)), {});
    }
    else
    {
        auto identifier = std::make_shared<ASTIdentifier>(CHDBPoco::toUpper(family_name));
...
CompressionCodecPtr CompressionCodecFactory::get(const String & compression_codec) const
{
    ...
    auto ast = parseQuery(codec_parser, "(" + CHDBPoco::toUpper(compression_codec) + ")", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
