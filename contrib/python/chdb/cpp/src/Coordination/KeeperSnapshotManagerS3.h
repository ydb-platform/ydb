...
public:
    ...
    void updateS3Configuration(const CHDBPoco::Util::AbstractConfiguration & config, const MultiVersion<Macros>::Version & macros);
    void uploadSnapshot(const SnapshotFileInfoPtr & file_info, bool async_upload = true);

    /// 'macros' are used to substitute macros in endpoint of disks
    void startup(const CHDBPoco::Util::AbstractConfiguration & config, const MultiVersion<Macros>::Version & macros);
    void shutdown();
private:
...
public:
    ...
    void updateS3Configuration(const CHDBPoco::Util::AbstractConfiguration &, const MultiVersion<Macros>::Version &) {}
    void uploadSnapshot(const SnapshotFileInfoPtr &, [[maybe_unused]] bool async_upload = true) {}

    void startup(const CHDBPoco::Util::AbstractConfiguration &, const MultiVersion<Macros>::Version &) {}
