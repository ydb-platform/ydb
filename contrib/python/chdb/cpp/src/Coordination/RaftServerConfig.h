...
template <>
struct fmt::formatter<DB_CHDB::RaftServerConfig> : fmt::formatter<string_view>
{
    constexpr auto format(const DB_CHDB::RaftServerConfig & server, format_context & ctx)
    {
...
template <>
struct fmt::formatter<DB_CHDB::ClusterUpdateAction> : fmt::formatter<string_view>
{
    constexpr auto format(const DB_CHDB::ClusterUpdateAction & action, format_context & ctx)
    {
        if (const auto * add = std::get_if<DB_CHDB::AddRaftServer>(&action))
            return fmt::format_to(ctx.out(), "(Add server {})", add->id);
        if (const auto * remove = std::get_if<DB_CHDB::RemoveRaftServer>(&action))
            return fmt::format_to(ctx.out(), "(Remove server {})", remove->id);
        if (const auto * update = std::get_if<DB_CHDB::UpdateRaftServerPriority>(&action))
