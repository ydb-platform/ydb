...
#error #include <rocksdb/convenience.h>
#error #include <rocksdb/options.h>
#error #include <rocksdb/status.h>
#error #include <rocksdb/table.h>
#error #include <rocksdb/snapshot.h>
...
public:
    ...
    void initialize(const KeeperContextPtr & context)
    {
        ...
        rocksdb::DB * db;
        auto status = rocksdb::DB_CHDB::Open(*options, rocksdb_dir, &db);
        if (!status.ok())
        {
