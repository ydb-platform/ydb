    ...
    static NO_INLINE UInt8 apply(A a, B b, CompareInt scale [[maybe_unused]])
    {
        ...
        if constexpr (_check_overflow)
        {
            ...
            if constexpr (scale_left)
                overflow |= common_CHDB::mulOverflow(x, scale, x);
            if constexpr (scale_right)
                overflow |= common_CHDB::mulOverflow(y, scale, y);
        ...
        {
            if constexpr (scale_left)
                x = common_CHDB::mulIgnoreOverflow(x, scale);
            if constexpr (scale_right)
                y = common_CHDB::mulIgnoreOverflow(y, scale);
        }
