...
{
    T multiplied = 0;
    if (common_CHDB::mulOverflow(x, multiplier, multiplied))
    {
    ...
    if (common_CHDB::addOverflow(multiplied, delta, result))
    {
        if constexpr (throw_on_error)
            throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
        return false;
