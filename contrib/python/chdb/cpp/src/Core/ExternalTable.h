...
{
public:
    ExternalTablesHandler(ContextMutablePtr context_, const CHDBPoco::Net::NameValueCollection & params_) : WithMutableContext(context_), params(params_) {}

    void handlePart(const CHDBPoco::Net::MessageHeader & header, ReadBuffer & stream) override;

private:
    const CHDBPoco::Net::NameValueCollection & params;
};
