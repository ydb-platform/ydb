...
static String generateScramble()
{
    ...
    CHDBPoco::RandomInputStream generator;
...
Native41::Native41(const String & password_, const String & scramble_)
{
    ...
    CHDBPoco::SHA1Engine engine1;
    engine1.update(password_);
    const CHDBPoco::SHA1Engine::Digest & password_sha1 = engine1.digest();

    CHDBPoco::SHA1Engine engine2;
    engine2.update(password_sha1.data(), password_sha1.size());
    const CHDBPoco::SHA1Engine::Digest & password_double_sha1 = engine2.digest();

    CHDBPoco::SHA1Engine engine3;
    engine3.update(scramble_.data(), scramble_.size());
    engine3.update(password_double_sha1.data(), password_double_sha1.size());
    const CHDBPoco::SHA1Engine::Digest & digest = engine3.digest();
...
void Native41::authenticate(
    ...
    std::shared_ptr<PacketEndpoint> packet_endpoint, bool, const CHDBPoco::Net::SocketAddress & address)
{
    if (!auth_response)
    {
    ...
    if (auth_response->size() != CHDBPoco::SHA1Engine::DIGEST_SIZE)
        throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Wrong size of auth response. Expected: {} bytes, received: {} bytes.",
            std::to_string(CHDBPoco::SHA1Engine::DIGEST_SIZE), std::to_string(auth_response->size()));
...
void Sha256Password::authenticate(
    const String & user_name, Session & session, std::optional<String> auth_response,
    std::shared_ptr<PacketEndpoint> packet_endpoint, bool is_secure_connection, const CHDBPoco::Net::SocketAddress & address)
{
    if (!auth_response)
    {
        packet_endpoint->sendPacket(AuthSwitchRequest(getName(), scramble), true);
