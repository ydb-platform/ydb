...
namespace std
{
    template <> struct tuple_size<DB_CHDB::NameAndTypePair> : std::integral_constant<size_t, 2> {};
    template <> struct tuple_element<0, DB_CHDB::NameAndTypePair> { using type = String; };
    template <> struct tuple_element<1, DB_CHDB::NameAndTypePair> { using type = DB_CHDB::DataTypePtr; };
}
