...
{
public:
    using ReplicasConfigurationByPriority = std::map<size_t, std::vector<DB_CHDB::StoragePostgreSQL::Configuration>>;
    ...
    explicit PoolWithFailover(
        const DB_CHDB::StoragePostgreSQL::Configuration & configuration,
        size_t pool_size,
        size_t pool_wait_timeout,
        size_t max_tries_,
        bool auto_close_connection_,
