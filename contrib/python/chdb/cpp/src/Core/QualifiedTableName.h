...
template <> struct hash<DB_CHDB::QualifiedTableName>
{
    using argument_type = DB_CHDB::QualifiedTableName;
...
{
    template <>
    struct formatter<DB_CHDB::QualifiedTableName>
    {
        ...
        auto format(const DB_CHDB::QualifiedTableName & name, FormatContext & ctx)
        {
            return fmt::format_to(ctx.out(), "{}.{}", DB_CHDB::backQuoteIfNeed(name.database), DB_CHDB::backQuoteIfNeed(name.table));
        }
    };
}
