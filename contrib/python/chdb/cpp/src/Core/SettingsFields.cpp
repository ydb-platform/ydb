...
void SettingFieldNumber<T>::writeBinary(WriteBuffer & out) const
{
    ...
    else
    {
        ...
        writeStringBinary(::DB_CHDB::toString(value), out);
    }
}
...
void SettingFieldNumber<T>::readBinary(ReadBuffer & in)
{
    ...
    else
    {
        ...
        *this = ::DB_CHDB::parseFromString<T>(str);
    }
}
...
{
    if (is_auto)
        return "'auto(" + ::DB_CHDB::toString(value) + ")'";
    else
        return ::DB_CHDB::toString(value);
}
...
namespace
{
    CHDBPoco::Timespan::TimeDiff float64AsSecondsToTimespan(Float64 d)
    {
...
#pragma clang diagnostic ignored "-Wimplicit-const-int-float-conversion"
        if (d * 1000000 > std::numeric_limits<CHDBPoco::Timespan::TimeDiff>::max() || d * 1000000 < std::numeric_limits<CHDBPoco::Timespan::TimeDiff>::min())
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS, "Cannot convert seconds to microseconds: the setting's value in seconds is too big: {}", d);
#pragma clang diagnostic pop

        return static_cast<CHDBPoco::Timespan::TimeDiff>(d * 1000000);
    }
...
SettingFieldSeconds::SettingFieldTimespan(const Field & f)
    : SettingFieldTimespan(CHDBPoco::Timespan{float64AsSecondsToTimespan(fieldToNumber<Float64>(f))})
{
}
...
{
    *this = CHDBPoco::Timespan{float64AsSecondsToTimespan(fieldToNumber<Float64>(f))};
    return *this;
}
...
{
    return ::DB_CHDB::toString(static_cast<Float64>(value.totalMicroseconds()) / microseconds_per_unit);
}

template <>
String SettingFieldMilliseconds::toString() const
{
    return ::DB_CHDB::toString(operator UInt64());
}
...
void SettingFieldSeconds::parseFromString(const String & str)
{
    ...
    *this = CHDBPoco::Timespan{static_cast<CHDBPoco::Timespan::TimeDiff>(n * microseconds_per_unit)};
}
...
{
    DB_CHDB::writeBinary(value, out);
}

void SettingFieldMap::readBinary(ReadBuffer & in)
{
    Map map;
    DB_CHDB::readBinary(map, in);
    *this = map;
}
...
void SettingFieldTimezone::validateTimezone(const std::string & tz_str)
{
    ...
    if (!tz_str.empty() && !cctz::load_time_zone(tz_str, &validated_tz))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Invalid time zone: {}", tz_str);
}
...
SettingFieldNonZeroUInt64::SettingFieldNonZeroUInt64(const DB_CHDB::Field & f) : SettingFieldUInt64(f)
{
    checkValueNonZero();
}
...
SettingFieldNonZeroUInt64 & SettingFieldNonZeroUInt64::operator=(const DB_CHDB::Field & f)
{
