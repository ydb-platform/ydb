...
struct SettingFieldTimespan
{
    ...
    CHDBPoco::Timespan value;
    bool changed = false;

    explicit SettingFieldTimespan(CHDBPoco::Timespan x = {}) : value(x) {}

    template <class Rep, class Period = std::ratio<1>>
    explicit SettingFieldTimespan(const std::chrono::duration<Rep, Period> & x)
        : SettingFieldTimespan(CHDBPoco::Timespan{static_cast<CHDBPoco::Timespan::TimeDiff>(std::chrono::duration_cast<std::chrono::microseconds>(x).count())}) {}

    explicit SettingFieldTimespan(UInt64 x) : SettingFieldTimespan(CHDBPoco::Timespan{static_cast<CHDBPoco::Timespan::TimeDiff>(x * microseconds_per_unit)}) {}
    explicit SettingFieldTimespan(const Field & f);

    SettingFieldTimespan & operator =(CHDBPoco::Timespan x) { value = x; changed = true; return *this; }

    template <class Rep, class Period = std::ratio<1>>
    SettingFieldTimespan & operator =(const std::chrono::duration<Rep, Period> & x) { *this = CHDBPoco::Timespan{static_cast<CHDBPoco::Timespan::TimeDiff>(std::chrono::duration_cast<std::chrono::microseconds>(x).count())}; return *this; }

    SettingFieldTimespan & operator =(UInt64 x) { *this = CHDBPoco::Timespan{static_cast<CHDBPoco::Timespan::TimeDiff>(x * microseconds_per_unit)}; return *this; }
    SettingFieldTimespan & operator =(const Field & f);

    operator CHDBPoco::Timespan() const { return value; } /// NOLINT
    ...
    CHDBPoco::Timespan::TimeDiff totalMicroseconds() const { return value.totalMicroseconds(); }
    CHDBPoco::Timespan::TimeDiff totalMilliseconds() const { return value.totalMilliseconds(); }
    CHDBPoco::Timespan::TimeDiff totalSeconds() const { return value.totalSeconds(); }
...
struct SettingFieldURI
{
    ...
    explicit SettingFieldURI(const CHDBPoco::URI & uri = {}) : value(uri) {}
    explicit SettingFieldURI(const String & str) : SettingFieldURI(CHDBPoco::URI{str}) {}
    explicit SettingFieldURI(const char * str) : SettingFieldURI(CHDBPoco::URI{str}) {}
    explicit SettingFieldURI(const Field & f) : SettingFieldURI(f.safeGet<String>()) {}

    SettingFieldURI & operator =(const CHDBPoco::URI & x) { value = x; changed = true; return *this; }
    SettingFieldURI & operator =(const String & str) { *this = CHDBPoco::URI{str}; return *this; }
    SettingFieldURI & operator =(const char * str) { *this = CHDBPoco::URI{str}; return *this; }
    SettingFieldURI & operator =(const Field & f) { *this = f.safeGet<const String &>(); return *this; }

    operator const CHDBPoco::URI &() const { return value; } /// NOLINT
