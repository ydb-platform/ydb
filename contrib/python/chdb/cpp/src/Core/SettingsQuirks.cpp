...
{
    if (CHDBPoco::Environment::os() != CHDB_POCO_OS_LINUX)
        return true;

    DB_CHDB::VersionNumber linux_version(CHDBPoco::Environment::osVersion());

    /// the check is correct since there will be no more 5.5.x releases.
    if (linux_version >= DB_CHDB::VersionNumber{5, 5, 0} && linux_version < DB_CHDB::VersionNumber{5, 6, 13})
    {
        if (log)
