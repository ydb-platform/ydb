...
private:
    ...
    void writeImpl(const T & data, time_t timestamp, const std::string & custom_root_path)
    {
        ...
        try
        {
            CHDBPoco::Net::SocketAddress socket_address(host, port);
            CHDBPoco::Net::StreamSocket socket(socket_address);
            socket.setSendTimeout(CHDBPoco::Timespan(static_cast<CHDBPoco::Int64>(timeout * 1000000)));
            CHDBPoco::Net::SocketStream str(socket);

            out(str, data, timestamp, custom_root_path);
        }
        catch (const CHDBPoco::Exception & e)
        {
            LOG_WARNING(&CHDBPoco::Util::Application::instance().logger(), "Fail to write to Graphite {}:{}. e.what() = {}, e.message() = {}", host, port, e.what(), e.message());
        }
    }
