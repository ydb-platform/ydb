...
void setExtras(bool anonymize, const std::string & server_data_path)
{
    ...
    DB_CHDB::UUID server_uuid = DB_CHDB::ServerUUID::get();
    if (server_uuid != DB_CHDB::UUIDHelpers::Nil)
    {
        std::string server_uuid_str = DB_CHDB::toString(server_uuid);
...
void SentryWriter::initializeInstance(CHDBPoco::Util::LayeredConfiguration & config)
{
...
SentryWriter::SentryWriter(CHDBPoco::Util::LayeredConfiguration & config)
{
    bool enabled = false;
    ...
    if (enabled)
    {
        server_data_path = config.getString("path", DB_CHDB::DBMS_DEFAULT_PATH);
    ...
    if (initialized)
    {
        ...
        switch (type)
        {
            ...
            case EXCEPTION:
            {
                ...
                sentry_set_tag("exception", DB_CHDB::ErrorCodes::getName(code).data());
            ...
            const String & build_id_hex = DB_CHDB::SymbolIndex::instance().getBuildIDHex();
...
void SentryWriter::initializeInstance(CHDBPoco::Util::LayeredConfiguration &) {}
SentryWriter * SentryWriter::getInstance() { return nullptr; }
void SentryWriter::resetInstance() {}

SentryWriter::SentryWriter(CHDBPoco::Util::LayeredConfiguration &) {}
