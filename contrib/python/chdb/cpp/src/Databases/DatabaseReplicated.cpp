...
void DatabaseReplicated::fillClusterAuthInfo(String collection_name, const CHDBPoco::Util::AbstractConfiguration & config_ref)
{
...
{
    return DB_CHDB::getMetadataHash(table_name, readMetadataFile(table_name));
}
...
void DatabaseReplicated::recoverLostReplica(const ZooKeeperPtr & current_zookeeper, UInt32 our_log_ptr, UInt32 & max_log_ptr)
{
    ...
    auto rename_table = [&](String from, String to)
    {
        ...
        new_digest -= DB_CHDB::getMetadataHash(from, statement);
        new_digest += DB_CHDB::getMetadataHash(to, statement);
                                     ...
                                     const String & to_table_name, bool exchange, bool dictionary)
{
    ...
    std::lock_guard lock{metadata_mutex};
    UInt64 new_digest = tables_metadata_digest;
    new_digest -= DB_CHDB::getMetadataHash(table_name, statement);
    new_digest += DB_CHDB::getMetadataHash(to_table_name, statement);
    if (exchange)
    {
        new_digest -= DB_CHDB::getMetadataHash(to_table_name, statement_to);
        new_digest += DB_CHDB::getMetadataHash(table_name, statement_to);
    }
                       ...
                       ContextPtr query_context)
{
    ...
    std::lock_guard lock{metadata_mutex};
    UInt64 new_digest = tables_metadata_digest;
    new_digest += DB_CHDB::getMetadataHash(query.getTable(), statement);
    if (txn && !txn->isCreateOrReplaceQuery() && !is_recovering)
        txn->addOp(zkutil::makeSetRequest(replica_path + "/digest", toString(new_digest), -1));
                                          ...
                                          const String & statement, ContextPtr query_context)
{
    ...
    new_digest += DB_CHDB::getMetadataHash(table_id.table_name, statement);
    if (txn && !is_recovering)
        txn->addOp(zkutil::makeSetRequest(replica_path + "/digest", toString(new_digest), -1));
