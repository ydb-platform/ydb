...
namespace
{
    ...
    struct IPRecord
    {
        CHDBPoco::Net::IPAddress addr;
        UInt8 prefix;
        size_t row;
        bool isv6;

        IPRecord(const CHDBPoco::Net::IPAddress & addr_, UInt8 prefix_, size_t row_)
            : addr(addr_)
            , prefix(prefix_)
            , row(row_)
            , isv6(addr.family() == CHDBPoco::Net::IPAddress::IPv6)
        {
        }
...
static std::pair<CHDBPoco::Net::IPAddress, UInt8> parseIPFromString(const std::string_view addr_str)
{
    try
    {
        ...
        {
            CHDBPoco::Net::IPAddress addr{std::string(addr_str.substr(0, pos))};

            uint8_t prefix = 0;
            const auto * addr_str_end = addr_str.data() + addr_str.size();
            auto [p, ec] = std::from_chars(addr_str.data() + pos + 1, addr_str_end, prefix);
            if (p != addr_str_end)
                throw DB_CHDB::Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "Extra characters at the end of IP address");
            if (ec != std::errc())
                throw DB_CHDB::Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Mask for IP address is not a valid number");

            addr = addr & CHDBPoco::Net::IPAddress(prefix, addr.family());
            return {addr, prefix};
        }

        CHDBPoco::Net::IPAddress addr{std::string(addr_str)};
        return {addr, addr.length() * 8};
    }
    catch (CHDBPoco::Exception & ex)
    {
        throw DB_CHDB::Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "Can't parse address \"{}\": {}",
...
{
    return CHDBPoco::ByteOrder::fromNetwork(*reinterpret_cast<const UInt32 *>(addr));
}
...
inline static void mapIPv4ToIPv6(UInt32 addr, uint8_t * buf)
{
    ...
    buf[10] = '\xFF';
    buf[11] = '\xFF';
    addr = CHDBPoco::ByteOrder::toNetwork(addr);
...
void IPAddressDictionary::loadData()
{
    ...
    while (executor.pull(block))
    {
        ...
        for (const auto row : collections::range(0, rows))
        {
            ...
            has_ipv6 = has_ipv6 || (addr.family() == CHDBPoco::Net::IPAddress::IPv6);
