...
MongoDBDictionarySource::MongoDBDictionarySource(
    ...
    , connection{std::make_shared<CHDBPoco::MongoDB::Connection>()}
{
    ...
    if (!uri.empty())
    {
        ...
        CHDBPoco::URI poco_uri(connection->uri());
    ...
    else
    {
        ...
        if (!user.empty())
        {
            CHDBPoco::MongoDB::Database poco_db(db);
            if (!poco_db.authenticate(*connection, user, password, method.empty() ? CHDBPoco::MongoDB::Database::AUTH_SCRAM_SHA1 : method))
...
{
    return QueryPipeline(std::make_shared<MongoDBSource>(connection, db, collection, CHDBPoco::MongoDB::Document{}, sample_block, max_block_size));
}
...
QueryPipeline MongoDBDictionarySource::loadIds(const std::vector<UInt64> & ids)
{
    ...
    CHDBPoco::MongoDB::Document query;

    /** NOTE: While building array, CHDBPoco::MongoDB requires passing of different unused element names, along with values.
      * In general, CHDBPoco::MongoDB is quite inefficient and bulky.
      */

    CHDBPoco::MongoDB::Array::Ptr ids_array(new CHDBPoco::MongoDB::Array);
    for (const UInt64 id : ids)
        ids_array->add(DB_CHDB::toString(id), static_cast<Int32>(id));
...
QueryPipeline MongoDBDictionarySource::loadKeys(const Columns & key_columns, const std::vector<size_t> & requested_rows)
{
    ...
    CHDBPoco::MongoDB::Document query;
    CHDBPoco::MongoDB::Array::Ptr keys_array(new CHDBPoco::MongoDB::Array);

    for (const auto row_idx : requested_rows)
    {
        auto & key = keys_array->addNewDocument(DB_CHDB::toString(row_idx));
        ...
        for (size_t attribute_index = 0; attribute_index < key_attributes.size(); ++attribute_index)
        {
            ...
            switch (key_attribute.underlying_type)
            {
                ...
                case AttributeUnderlyingType::String:
                {
                    ...
                    {
                        CHDBPoco::MongoDB::ObjectId::Ptr loaded_id(new CHDBPoco::MongoDB::ObjectId(loaded_str));
