...
{
public:
    DiskEncrypted(const String & name_, const CHDBPoco::Util::AbstractConfiguration & config_, const String & config_prefix_, const DisksMap & map_);
    ...
    void setLastModified(const String & path, const CHDBPoco::Timestamp & timestamp) override
    {
        auto tx = createEncryptedTransaction();
    ...
    CHDBPoco::Timestamp getLastModified(const String & path) const override
    {
        auto wrapped_path = wrappedPath(path);
    ...
    void applyNewSettings(const CHDBPoco::Util::AbstractConfiguration & config, ContextPtr context, const String & config_prefix, const DisksMap & map) override;
