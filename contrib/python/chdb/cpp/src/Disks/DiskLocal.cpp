...
void DiskLocal::setLastModified(const String & path, const CHDBPoco::Timestamp & timestamp)
{
    FS::setModificationTime(fs::path(disk_path) / path, timestamp.epochTime());
}

CHDBPoco::Timestamp DiskLocal::getLastModified(const String & path) const
{
...
{
    DB_CHDB::createHardLink(fs::path(disk_path) / src_path, fs::path(disk_path) / dst_path);
}
...
void DiskLocal::applyNewSettings(const CHDBPoco::Util::AbstractConfiguration & config, ContextPtr context, const String & config_prefix, const DisksMap & disk_map)
{
...
DiskLocal::DiskLocal(const String & name_, const String & path_, UInt64 keep_free_space_bytes_,
                     const CHDBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
...
struct stat DiskLocal::stat(const String & path) const
{
    ...
    if (::stat(full_path.string().c_str(), &st) == 0)
        return st;
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::CANNOT_STAT, path, "Cannot stat file: {}", path);
}
...
void DiskLocal::chmod(const String & path, mode_t mode)
{
    ...
    if (::chmod(full_path.string().c_str(), mode) == 0)
        return;
    DB_CHDB::ErrnoException::throwFromPath(DB_CHDB::ErrorCodes::PATH_ACCESS_DENIED, path, "Cannot chmod file: {}", path);
}
