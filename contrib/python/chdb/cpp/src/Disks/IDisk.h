...
public:
    /// Default constructor.
    IDisk(const String & name_, const CHDBPoco::Util::AbstractConfiguration & config, const String & config_prefix)
        : name(name_)
        , copying_thread_pool(
    ...
    virtual void setLastModified(const String & path, const CHDBPoco::Timestamp & timestamp) = 0;

    /// Get last modified time of file or directory at `path`.
    virtual CHDBPoco::Timestamp getLastModified(const String & path) const = 0;
    ...
    virtual void applyNewSettings(const CHDBPoco::Util::AbstractConfiguration & config, ContextPtr context, const String & config_prefix, const DisksMap & map);
