...
void DiskObjectStorage::setLastModified(const String & path, const CHDBPoco::Timestamp & timestamp)
{
    auto transaction = createObjectStorageTransaction();
...
CHDBPoco::Timestamp DiskObjectStorage::getLastModified(const String & path) const
{
...
void DiskObjectStorage::applyNewSettings(
    const CHDBPoco::Util::AbstractConfiguration & config, ContextPtr context_, const String & /*config_prefix*/, const DisksMap & disk_map)
{
...
void DiskObjectStorage::restoreMetadataIfNeeded(
    const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_prefix, ContextPtr context)
{
    if (send_metadata)
    {
