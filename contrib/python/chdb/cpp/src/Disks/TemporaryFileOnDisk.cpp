    ...
    , metric_increment(CurrentMetrics::TotalTemporaryFiles)
{
    ...
    /// That's why we must not use CHDBPoco::TemporaryFile::tempName() here (CHDBPoco::TemporaryFile::tempName() can return the same names for different processes on different nodes).
