...
public:
    ...
    virtual size_t getSize() const { throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NOT_IMPLEMENTED, "Function getSize() is not implemented"); }
    virtual void setSize(size_t) { throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NOT_IMPLEMENTED, "Function setSize() is not implemented"); }
    virtual String str() const { throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NOT_IMPLEMENTED, "Function str() is not implemented"); }
...
public:
    NumpyDataTypeInt(Endianness endianness_, size_t size_, bool is_signed_) : NumpyDataType(endianness_), size(size_), is_signed(is_signed_)
    {
        switch (size)
        {
            ...
            default:
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Incorrect int type with size {}", size);
        }
    }
    ...
    {
        DB_CHDB::WriteBufferFromOwnString buf;
...
public:
    NumpyDataTypeFloat(Endianness endianness_, size_t size_) : NumpyDataType(endianness_), size(size_)
    {
        switch (size)
        {
            ...
            default:
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Numpy float type with size {} is not supported", size);
        }
    }
