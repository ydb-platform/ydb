...
const ColumnConst & checkAndGetColumnConst(const IColumn & column)
{
    ...
    if (!checkColumn<Type>(&data_column))
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Unexpected const column type: expected {}, got {}", demangle(typeid(Type).name()), demangle(typeid(data_column).name()));
