...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        {
#if USE_SSL
            std::unique_ptr<CHDBPoco::Crypto::X509Certificate> x509_cert;
            if (!certificate.empty())
                x509_cert = std::make_unique<CHDBPoco::Crypto::X509Certificate>(certificate);

            const X509 * cert = x509_cert ?
                x509_cert->certificate() :
                SSL_CTX_get0_certificate(CHDBPoco::Net::SSLManager::instance().defaultServerContext()->sslContext());
