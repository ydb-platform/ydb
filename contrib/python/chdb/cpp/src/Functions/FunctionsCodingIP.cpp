...
private:
    ...
    ColumnPtr executeTyped(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const
    {
        ...
        if (const ColumnType * col = typeid_cast<const ColumnType *>(column.get()))
        {
            ...
            for (size_t i = 0; i < input_rows_count; ++i)
            {
                DB_CHDB::formatIPv4(reinterpret_cast<const unsigned char*>(&vec_in[i]), sizeof(ArgType), pos, mask_tail_octets, "xxx");
                offsets_res[i] = pos - begin;
            }
...
#else
            unalignedStoreLittleEndian<UInt64>(buf + 8, 0x00000000FFFF0000ull | (static_cast<UInt64>(DB_CHDB::byteswap(in)) << 32));
#endif
    }
};
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            vec_res[i] = DB_CHDB::parseIPv4whole(reinterpret_cast<const char *>(&vec_src[prev_offset]), reinterpret_cast<unsigned char *>(&result));
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            vec_res[i] = DB_CHDB::parseIPv6whole(reinterpret_cast<const char *>(&vec_src[prev_offset]),
