...
std::pair<int, int> determineBinaryStartIndexWithIncrement(ptrdiff_t num_bytes, Representation representation)
{
    ...
    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "{} is not handled yet", magic_enum::enum_name(representation));
}
...
public:
        ...
        : first_half_binary_representation(variant == Variant::Microsoft ? Representation::LittleEndian : Representation::BigEndian)
    {
        if (variant != Variant::Default && variant != Variant::Microsoft)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "{} is not handled yet", magic_enum::enum_name(variant));
    }
...
void checkArgumentCount(const DB_CHDB::DataTypes & arguments, const std::string_view function_name)
{
    if (const auto argument_count = std::ssize(arguments); argument_count < 1 || argument_count > 2)
        throw DB_CHDB::Exception(
            DB_CHDB::ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
            "Number of arguments for function {} doesn't match: passed {}, should be 1 or 2",
            function_name,
            argument_count);
}

void checkFormatArgument(const DB_CHDB::DataTypes & arguments, const std::string_view function_name)
{
    if (const auto argument_count = std::ssize(arguments);
        argument_count > 1 && !DB_CHDB::WhichDataType(arguments[1]).isInt8() && !DB_CHDB::WhichDataType(arguments[1]).isUInt8())
        throw DB_CHDB::Exception(
            DB_CHDB::ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
            "Illegal type {} of second argument of function {}, expected Int8 or UInt8 type",
            arguments[1]->getName(),
            function_name);
}

UUIDSerializer::Variant parseVariant(const DB_CHDB::ColumnsWithTypeAndName & arguments)
{
    if (arguments.size() < 2)
    ...
    if (!as_enum)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Expected UUID variant, got {}", representation);
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        if (const auto * col_in = checkAndGetColumn<ColumnUUID>(column.get()))
        {
            ...
            for (size_t i = 0; i < input_rows_count; ++i)
            {
                uint64_t hiBytes = DB_CHDB::UUIDHelpers::getHighBytes(uuids[i]);
                uint64_t loBytes = DB_CHDB::UUIDHelpers::getLowBytes(uuids[i]);
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        if (const auto * col_in = checkAndGetColumn<ColumnUUID>(column.get()))
        {
            ...
            for (size_t i = 0; i < input_rows_count; ++i)
            {
                const uint64_t hiBytes = DB_CHDB::UUIDHelpers::getHighBytes(uuids[i]);
