            ...
            for (auto & arg : args)
            {
                ...
                for (size_t j = 0; j < len; ++j)
                {
                    ...
                    if constexpr (op_type == OpTypes::ADD)
                    {
                        ...
                        if (!inserted)
                            it->second = common_CHDB::addIgnoreOverflow(it->second, value);
                    }
                    else
                    {
                        static_assert(op_type == OpTypes::SUBTRACT);
                        const auto [it, inserted] = summing_map.insert({key, first ? value : common_CHDB::negateIgnoreOverflow(value)});
                        if (!inserted)
                            it->second = common_CHDB::subIgnoreOverflow(it->second, value);
                    }
                }
