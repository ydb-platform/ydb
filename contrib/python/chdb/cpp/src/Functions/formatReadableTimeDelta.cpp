...
private:
    ...
    {
        DB_CHDB::DoubleConverter<true>::BufferType buffer;
        double_conversion::StringBuilder builder{buffer, sizeof(buffer)};

        if (!DB_CHDB::DoubleConverter<false>::instance().ToFixed(fractional_part, 9, &builder))
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Cannot print double number: {}", fractional_part);
