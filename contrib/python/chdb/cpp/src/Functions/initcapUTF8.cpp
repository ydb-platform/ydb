...
struct InitcapUTF8Impl
{
    ...
    static void processCodePoint(const UInt8 *& src, const UInt8 * src_end, UInt8 *& dst, bool& prev_alphanum)
    {
        ...
        if (src_code_point)
        {
            bool alpha = CHDBPoco::Unicode::isAlpha(*src_code_point);
            bool alphanum = alpha || CHDBPoco::Unicode::isDigit(*src_code_point);

            int dst_code_point = *src_code_point;
            if (alphanum && !prev_alphanum)
            {
                if (alpha)
                    dst_code_point = CHDBPoco::Unicode::toUpper(*src_code_point);
            }
            else if (alpha)
            {
                dst_code_point = CHDBPoco::Unicode::toLower(*src_code_point);
            }
