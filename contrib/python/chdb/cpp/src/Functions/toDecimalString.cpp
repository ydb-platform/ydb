...
private:
                      ...
                      size_t input_rows_count) const
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            if (vec_precision[i] > max_digits)
                throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER,
    ...
    static void format(T value, DB_CHDB::WriteBuffer & out, UInt8 precision)
    {
        ...
        DB_CHDB::DoubleConverter<false>::BufferType buffer;
        double_conversion::StringBuilder builder{buffer, sizeof(buffer)};

        const auto result = DB_CHDB::DoubleConverter<false>::instance().ToFixed(value, precision, &builder);

        if (!result)
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Error processing number: {}", value);
