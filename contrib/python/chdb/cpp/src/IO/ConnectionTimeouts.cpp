...
CHDBPoco::Timespan ConnectionTimeouts::saturate(CHDBPoco::Timespan timespan, CHDBPoco::Timespan limit)
{
...
ConnectionTimeouts ConnectionTimeouts::getHTTPTimeouts(const Settings & settings, CHDBPoco::Timespan http_keep_alive_timeout)
{
    return ConnectionTimeouts()
...
private:
    ...
    static constexpr CHDBPoco::Timestamp::TimeDiff first_byte_ms[known_methods_count][2] =
    {
        /* GET */ {200, 200},
        /* POST */ {200, 200},
        /* DELETE */ {200, 200},
    ...
    static constexpr CHDBPoco::Timestamp::TimeDiff rest_bytes_ms[known_methods_count][2] =
    {
        /* GET */ {500, 500},
        /* POST */ {1000, 30000},
        /* DELETE */ {1000, 10000},
    ...
    static_assert(sizeof(first_byte_ms) == known_methods_count * sizeof(CHDBPoco::Timestamp::TimeDiff) * 2);
...
public:
    static std::pair<CHDBPoco::Timespan, CHDBPoco::Timespan> getSendReceiveTimeout(const String & method, bool first_byte)
    {
        auto idx = getMethodIndex(method);

        if (first_byte)
            return std::make_pair(
                CHDBPoco::Timespan(first_byte_ms[idx][0] * 1000),
                CHDBPoco::Timespan(first_byte_ms[idx][1] * 1000)
            );

        return std::make_pair(
            CHDBPoco::Timespan(rest_bytes_ms[idx][0] * 1000),
            CHDBPoco::Timespan(rest_bytes_ms[idx][1] * 1000)
        );
    }
};
...
void setTimeouts(CHDBPoco::Net::HTTPClientSession & session, const ConnectionTimeouts & timeouts)
{
...
ConnectionTimeouts getTimeouts(const CHDBPoco::Net::HTTPClientSession & session)
{
    return ConnectionTimeouts()
