...
{
    CHDBPoco::Timespan connection_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_CONNECT_TIMEOUT_SEC, 0);
    CHDBPoco::Timespan secure_connection_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_CONNECT_TIMEOUT_SEC, 0);

    CHDBPoco::Timespan send_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_SEND_TIMEOUT_SEC, 0);
    CHDBPoco::Timespan receive_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC, 0);

    CHDBPoco::Timespan tcp_keep_alive_timeout = CHDBPoco::Timespan(DEFAULT_TCP_KEEP_ALIVE_TIMEOUT, 0);
    CHDBPoco::Timespan http_keep_alive_timeout = CHDBPoco::Timespan(DEFAULT_HTTP_KEEP_ALIVE_TIMEOUT, 0);
    size_t http_keep_alive_max_requests = DEFAULT_HTTP_KEEP_ALIVE_MAX_REQUEST;


    /// Timeouts for HedgedConnections
    CHDBPoco::Timespan hedged_connection_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC, 0);
    CHDBPoco::Timespan receive_data_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC, 0);
    /// Timeout for receiving HELLO packet
    CHDBPoco::Timespan handshake_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC, 0);
    /// Timeout for synchronous request-result protocol call (like Ping or TablesStatus)
    CHDBPoco::Timespan sync_request_timeout = CHDBPoco::Timespan(DBMS_DEFAULT_SYNC_REQUEST_TIMEOUT_SEC, 0);

    ConnectionTimeouts() = default;

    static CHDBPoco::Timespan saturate(CHDBPoco::Timespan timespan, CHDBPoco::Timespan limit);
    ConnectionTimeouts getSaturated(CHDBPoco::Timespan limit) const;
    ...
    static ConnectionTimeouts getHTTPTimeouts(const Settings & settings, CHDBPoco::Timespan http_keep_alive_timeout);
...
#define DECLARE_BUILDER_FOR_MEMBER(member, setter_func) \
    ...
    ConnectionTimeouts & setter_func(CHDBPoco::Timespan span); \
...
#undef DECLARE_BUILDER_FOR_MEMBER
    ...
    ConnectionTimeouts & withConnectionTimeout(CHDBPoco::Timespan span);
...
#define DEFINE_BUILDER_FOR_MEMBER(member, setter_func) \
    ...
    { \
        return setter_func(CHDBPoco::Timespan(seconds, 0)); \
    } \
    inline ConnectionTimeouts & ConnectionTimeouts::setter_func(CHDBPoco::Timespan span) \
    { \
        member = span; \
        return *this; \
    } \
...
inline ConnectionTimeouts ConnectionTimeouts::getSaturated(CHDBPoco::Timespan limit) const
{
...
{
    return withConnectionTimeout(CHDBPoco::Timespan(seconds, 0));
}

inline ConnectionTimeouts & ConnectionTimeouts::withConnectionTimeout(CHDBPoco::Timespan span)
{
