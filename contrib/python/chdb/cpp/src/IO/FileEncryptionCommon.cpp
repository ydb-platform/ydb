...
InitVector InitVector::random()
{
    ...
    if (ret != 1)
        throw Exception(DB_CHDB::ErrorCodes::OPENSSL_ERROR, "OpenSSL error code: {}", ERR_get_error());
    return InitVector{counter};
}
...
void Header::read(ReadBuffer & in)
{
    ...
    if (std::endian::native != endian)
        algorithm_u16 = DB_CHDB::byteswap(algorithm_u16);
    ...
    if (version < 2)
    {
        ...
        if (std::endian::native != endian)
            key_id = DB_CHDB::byteswap(key_id);
