...
void PeekableReadBuffer::checkStateCorrect() const
{
    if (checkpoint)
    {
        ...
        {
            if (!peeked_size)
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Checkpoint in empty own buffer");
            if (currentlyReadFromOwnMemory() && pos < *checkpoint)
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in own buffer before checkpoint in own buffer");
            if (!currentlyReadFromOwnMemory() && pos < sub_buf->position())
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in subbuffer less than sub_buf->position()");
        }
        else
        {
            if (peeked_size)
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Own buffer is not empty");
            if (currentlyReadFromOwnMemory())
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in own buffer before checkpoint in subbuffer");
            if (pos < *checkpoint)
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in subbuffer before checkpoint in subbuffer");
        }
    }
    else
    {
    ...
    if (currentlyReadFromOwnMemory() && !peeked_size)
        throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Pos in empty own buffer");
}
...
{
    if (!checkpoint)
        throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "There is no checkpoint");
    checkStateCorrect();
