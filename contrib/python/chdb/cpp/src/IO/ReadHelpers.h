...
ReturnType readIntTextImpl(T & x, ReadBuffer & buf)
{
            ...
            case '9':
            {
                ...
                if constexpr (check_overflow == ReadIntTextCheckOverflow::CHECK_OVERFLOW && !is_big_int_v<T>)
                {
                    ...
                    if (buf.count() - initial_pos + 1 >= std::numeric_limits<T>::max_digits10)
                    {
                        ...
                        {
                            T signed_res = -res;
                            if (common_CHDB::mulOverflow<T>(signed_res, 10, signed_res) ||
                                common_CHDB::subOverflow<T>(signed_res, (*buf.position() - '0'), signed_res))
                                return ReturnType(false);
                        ...
                        else
                        {
                            T signed_res = res;
                            if (common_CHDB::mulOverflow<T>(signed_res, 10, signed_res) ||
                                common_CHDB::addOverflow<T>(signed_res, (*buf.position() - '0'), signed_res))
                                return ReturnType(false);
    ...
    if constexpr (is_signed_v<T>)
    {
        if (negative)
        {
            ...
            {
                if (common_CHDB::mulOverflow<UnsignedT, Int8, T>(res, -1, x))
                    return ReturnType(false);
            }
            else
                x = -res;
...
inline ReturnType readDateTimeTextImpl(DateTime64 & datetime64, UInt32 scale, ReadBuffer & buf, const DateLUTImpl & date_lut, const char * allowed_date_delimiters = nullptr, const char * allowed_time_delimiters = nullptr)
{
    ...
    DB_CHDB::DecimalUtils::DecimalComponents<DateTime64> components{static_cast<DateTime64::NativeType>(whole), 0};
    ...
    else if (whole >= 10413792000LL)
    {
        ...
        components.fractional =  components.whole % common_CHDB::exp10_i32(scale);
        components.whole = components.whole / common_CHDB::exp10_i32(scale);
    }
...
inline ReturnType readTimeTextImpl(Decimal64 & time64, UInt32 scale, ReadBuffer & buf)
{
    ...
    DB_CHDB::DecimalUtils::DecimalComponents<Decimal64> components{static_cast<Decimal64::NativeType>(whole), 0};
...
void readBinary(std::vector<T> & x, ReadBuffer & buf)
{
    ...
    if (size > DEFAULT_MAX_STRING_SIZE)
        throw CHDBPoco::Exception("Too large vector size.");
