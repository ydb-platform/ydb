...
bool isRetriableError(const CHDBPoco::Net::HTTPResponse::HTTPStatus http_status) noexcept
{
    static constexpr std::array non_retriable_errors{
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_BAD_REQUEST,
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_UNAUTHORIZED,
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND,
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_FORBIDDEN,
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_IMPLEMENTED,
        CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_METHOD_NOT_ALLOWED};

    return std::all_of(
        non_retriable_errors.begin(), non_retriable_errors.end(), [&](const auto status) { return http_status != status; });
}

CHDBPoco::URI getUriAfterRedirect(const CHDBPoco::URI & prev_uri, CHDBPoco::Net::HTTPResponse & response)
{
    chassert(DB_CHDB::isRedirect(response.getStatus()));

    auto location = response.get("Location");
    auto location_uri = CHDBPoco::URI(location);
...
class ReadBufferFromSessionResponse : public DB_CHDB::ReadBufferFromIStream
{
private:
    DB_CHDB::HTTPSessionPtr session;

public:
    ReadBufferFromSessionResponse(DB_CHDB::HTTPSessionPtr && session_, std::istream & rstr, size_t size)
...
void ReadWriteBufferFromHTTP::prepareRequest(CHDBPoco::Net::HTTPRequest & request, std::optional<HTTPRange> range) const
{
    request.setHost(current_uri.getHost());

    if (out_stream_callback)
        request.setChunkedTransferEncoding(true);
    else if (method == CHDBPoco::Net::HTTPRequest::HTTP_POST)
...
std::optional<size_t> ReadWriteBufferFromHTTP::tryGetFileSize()
{
    if (!file_info)
    {
        ...
        catch (const CHDBPoco::IOException &)
        {
            return std::nullopt;
        }
    }
...
bool ReadWriteBufferFromHTTP::supportsReadAt()
{
    if (!file_info)
        file_info = getFileInfo();
    return method == CHDBPoco::Net::HTTPRequest::HTTP_GET && file_info->seekable;
}
...
void ReadWriteBufferFromHTTP::getHeadResponse(CHDBPoco::Net::HTTPResponse & response)
{
    doWithRetries(
        [&] ()
        {
            callWithRedirects(response, CHDBPoco::Net::HTTPRequest::HTTP_HEAD, {});
        },
        /*on_retry=*/ nullptr,
        /*mute_logging=*/ true);
}
...
ReadWriteBufferFromHTTP::ReadWriteBufferFromHTTP(
    ...
    , initial_uri(uri_)
    , method(!method_.empty() ? method_ : out_stream_callback_ ? CHDBPoco::Net::HTTPRequest::HTTP_POST : CHDBPoco::Net::HTTPRequest::HTTP_GET)
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callImpl(
    CHDBPoco::Net::HTTPResponse & response, const std::string & method_, const std::optional<HTTPRange> & range, bool allow_redirects) const
{
    if (remote_host_filter)
        remote_host_filter->checkURL(current_uri);

    CHDBPoco::Net::HTTPRequest request(method_, current_uri.getPathAndQuery(), CHDBPoco::Net::HTTPRequest::HTTP_1_1);
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callWithRedirects(
    CHDBPoco::Net::HTTPResponse & response, const String & method_, const std::optional<HTTPRange> & range)
{
    auto result = callImpl(response, method_, range, true);

    while (isRedirect(response.getStatus()))
    {
        CHDBPoco::URI uri_redirect = getUriAfterRedirect(current_uri, response);
        ++redirects;
                                            ...
                                            bool mute_logging) const
{
    ...
    for (size_t attempt = 1; attempt <= read_settings.http_max_tries; ++attempt)
    {
        ...
        catch (CHDBPoco::Net::NetException & e)
        {
        ...
        catch (CHDBPoco::Exception & e)
        {
            if (e.code() == CHDB_POCO_EMFILE)
                is_retriable = false;
...
std::unique_ptr<ReadBuffer> ReadWriteBufferFromHTTP::initialize()
{
    ...
    if (range.has_value() && response.getStatus() != CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT)
    {
        ...
        if (getOffset() != 0)
        {
            ...
            if (response.getStatus() == CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_OK)
            {
                ...
                throw HTTPException(
                    ...
                    CHDBPoco::Net::HTTPResponse::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE,
                    reason,
                    "");
            }
            else
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            if (!impl)
            {
                ...
                {
                    if (http_skip_not_found_url && e.getHTTPStatus() == CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND)
                    {
                        next_result = false;
...
size_t ReadWriteBufferFromHTTP::readBigAt(char * to, size_t n, size_t offset, const std::function<bool(size_t)> & progress_callback) const
{
    ...
    doWithRetries(
        /*callable=*/ [&] ()
        {
            ...
            if (response.getStatus() != CHDBPoco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT &&
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::getFileInfo()
{
    ...
    catch (const HTTPException & e)
    {
        ...
        if (e.getHTTPStatus() >= 400 && e.getHTTPStatus() <= 499 &&
            e.getHTTPStatus() != CHDBPoco::Net::HTTPResponse::HTTP_TOO_MANY_REQUESTS &&
            e.getHTTPStatus() != CHDBPoco::Net::HTTPResponse::HTTP_REQUEST_TIMEOUT &&
            e.getHTTPStatus() != CHDBPoco::Net::HTTPResponse::HTTP_MISDIRECTED_REQUEST)
        {
            return HTTPFileInfo{};
        }
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::parseFileInfo(const CHDBPoco::Net::HTTPResponse & response, size_t requested_range_begin)
{
    HTTPFileInfo res;
