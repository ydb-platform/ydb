...
private:
    ...
    const CHDBPoco::URI initial_uri;
    ...
    CHDBPoco::URI current_uri;
    size_t redirects = 0;

    std::string content_encoding;
    std::unique_ptr<ReadBuffer> impl;

    std::vector<CHDBPoco::Net::HTTPCookie> cookies;
    ...
    void prepareRequest(CHDBPoco::Net::HTTPRequest & request, std::optional<HTTPRange> range) const;
    ...
    void getHeadResponse(CHDBPoco::Net::HTTPResponse & response);
...
public:
    ...
    static HTTPFileInfo parseFileInfo(const CHDBPoco::Net::HTTPResponse & response, size_t requested_range_begin);
};
...
{
    CHDBPoco::URI uri;
    std::string method = CHDBPoco::Net::HTTPRequest::HTTP_GET;
...
public:
    explicit BuilderRWBufferFromHTTP(CHDBPoco::URI uri_)
        : uri(uri_)
    {}
...
/// NOLINTEND(bugprone-macro-parentheses)
    ...
    ReadWriteBufferFromHTTPPtr create(const CHDBPoco::Net::HTTPBasicCredentials & credentials_)
    {
