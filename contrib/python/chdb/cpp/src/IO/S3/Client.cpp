...
/// doRequest calls virtuall function from Aws::S3::Client while DB_CHDB::S3::Client has not virtual calls for each request type
...
Client::doRequestWithRetryNetworkErrors(RequestType & request, RequestFn request_fn) const
{
    ...
    auto with_retries = [this, request_fn_ = std::move(request_fn)] (const RequestType & request_)
    {
        ...
        for (Int64 attempt_no = 0; attempt_no < max_attempts; ++attempt_no)
        {
            ...
            catch (CHDBPoco::Net::ConnectionResetException &)
            {
    ...
    const String & protocol)
{
    ...
    chassert(context);
    auto proxy_configuration_resolver = DB_CHDB::ProxyConfigurationResolverProvider::get(DB_CHDB::ProxyConfiguration::protocolFromString(protocol), context->getConfigRef());

    auto per_request_configuration = [=] () { return proxy_configuration_resolver->resolve(); };
    auto error_report = [=] (const DB_CHDB::ProxyConfiguration & req) { proxy_configuration_resolver->errorReport(req); };
