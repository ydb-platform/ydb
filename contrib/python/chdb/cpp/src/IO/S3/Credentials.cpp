...
{
    CHDBPoco::URI uri(getAWSMetadataEndpoint() + EC2_AVAILABILITY_ZONE_RESOURCE);
    CHDBPoco::Net::HTTPClientSession session(uri.getHost(), uri.getPort());
    session.setTimeout(CHDBPoco::Timespan(AVAILABILITY_ZONE_REQUEST_TIMEOUT_SECONDS, 0));

    CHDBPoco::Net::HTTPResponse response;
    CHDBPoco::Net::HTTPRequest request(CHDBPoco::Net::HTTPRequest::HTTP_GET, uri.getPath());
    session.sendRequest(request);

    std::istream & rs = session.receiveResponse(response);
    if (response.getStatus() != CHDBPoco::Net::HTTPResponse::HTTP_OK)
        throw DB_CHDB::Exception(ErrorCodes::AWS_ERROR, "Failed to get AWS availability zone. HTTP response code: {}", response.getStatus());
    String response_data;
    CHDBPoco::StreamCopier::copyToString(rs, response_data);
    return response_data;
}

String getGCPAvailabilityZoneOrException()
{
    CHDBPoco::URI uri(String(GCP_METADATA_SERVICE_ENDPOINT) + "/computeMetadata/v1/instance/zone");
    CHDBPoco::Net::HTTPClientSession session(uri.getHost(), uri.getPort());
    session.setTimeout(CHDBPoco::Timespan(AVAILABILITY_ZONE_REQUEST_TIMEOUT_SECONDS, 0));
    ...
    std::istream & rs = session.receiveResponse(response);
    if (response.getStatus() != CHDBPoco::Net::HTTPResponse::HTTP_OK)
        throw DB_CHDB::Exception(ErrorCodes::GCP_ERROR, "Failed to get GCP availability zone. HTTP response code: {}", response.getStatus());
    String response_data;
    CHDBPoco::StreamCopier::copyToString(rs, response_data);
    Strings zone_info;
    ...
    if (zone_info.size() != 4)
        throw DB_CHDB::Exception(ErrorCodes::GCP_ERROR, "Invalid format of GCP zone information, expect projects/<project-number>/zones/<zone-value>");
    return zone_info[3];
}
...
String getRunningAvailabilityZone()
{
    ...
    catch (...)
    {
        ...
        catch (...)
        {
            ...
            throw DB_CHDB::Exception(ErrorCodes::UNSUPPORTED_METHOD,
...
AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider(
    DB_CHDB::S3::PocoHTTPClientConfiguration & aws_client_configuration, uint64_t expiration_window_seconds_)
...
SSOCredentialsProvider::SSOCredentialsProvider(DB_CHDB::S3::PocoHTTPClientConfiguration aws_client_configuration_, uint64_t expiration_window_seconds_)
...
S3CredentialsProviderChain::S3CredentialsProviderChain(
        const DB_CHDB::S3::PocoHTTPClientConfiguration & configuration,
        ...
        CredentialsConfiguration credentials_configuration)
{
    ...
    if (credentials_configuration.use_environment_credentials)
    {
        ...
        {
            DB_CHDB::S3::PocoHTTPClientConfiguration aws_client_configuration = DB_CHDB::S3::ClientFactory::instance().createClientConfiguration(
...
{
    throw DB_CHDB::Exception(ErrorCodes::UNSUPPORTED_METHOD, "Does not support availability zone detection for non-cloud environment");
}
