...
PocoHTTPClientConfiguration::PocoHTTPClientConfiguration(
        std::function<DB_CHDB::ProxyConfiguration()> per_request_configuration_,
        ...
        std::function<void(const DB_CHDB::ProxyConfiguration &)> error_report_)
...
void PocoHTTPClientConfiguration::updateSchemeAndRegion()
{
    if (!endpointOverride.empty())
    {
        ...
        CHDBPoco::URI uri(endpointOverride);
...
ConnectionTimeouts getTimeoutsFromConfiguration(const PocoHTTPClientConfiguration & client_configuration)
{
    return ConnectionTimeouts()
        .withConnectionTimeout(CHDBPoco::Timespan(client_configuration.connectTimeoutMs * 1000))
        .withSendTimeout(CHDBPoco::Timespan(client_configuration.requestTimeoutMs * 1000))
        .withReceiveTimeout(CHDBPoco::Timespan(client_configuration.requestTimeoutMs * 1000))
        .withTCPKeepAliveTimeout(CHDBPoco::Timespan(
            client_configuration.enableTcpKeepAlive ? client_configuration.tcpKeepAliveIntervalMs * 1000 : 0))
        .withHTTPKeepAliveTimeout(CHDBPoco::Timespan(client_configuration.http_keep_alive_timeout, 0))
...
String getMethod(const Aws::Http::HttpRequest & request)
{
    switch (request.GetMethod())
    {
        ...
        case Aws::Http::HttpMethod::HTTP_DELETE:
            return CHDBPoco::Net::HTTPRequest::HTTP_DELETE;
        case Aws::Http::HttpMethod::HTTP_PUT:
            return CHDBPoco::Net::HTTPRequest::HTTP_PUT;
        case Aws::Http::HttpMethod::HTTP_HEAD:
            return CHDBPoco::Net::HTTPRequest::HTTP_HEAD;
        case Aws::Http::HttpMethod::HTTP_PATCH:
            return CHDBPoco::Net::HTTPRequest::HTTP_PATCH;
    }
}
    ...
    Aws::Utils::RateLimits::RateLimiterInterface *) const
{
    ...
    try
    {
        ...
        for (unsigned int attempt = 0; attempt <= s3_max_redirects; ++attempt)
        {
            ...
            CHDBPoco::Net::HTTPRequest poco_request(CHDBPoco::Net::HTTPRequest::HTTP_1_1);
            ...
            /** According to RFC-2616, Request-URI is allowed to be encoded.
              ...
              * Effectively, `CHDBPoco::URI` chooses smaller subset of characters to encode,
            ...
            const std::string reserved = "?#:;+@&=%"; /// CHDBPoco::URI::RESERVED_QUERY_PARAM without '/' plus percent sign.
            CHDBPoco::URI::encode(target_uri.getPath(), reserved, path_and_query);
            ...
            CHDBPoco::Net::HTTPResponse poco_response;
            ...
            if (request.GetContentBody())
            {
                ...
                auto size = CHDBPoco::StreamCopier::copyStream(*request.GetContentBody(), request_body_stream);
                if (enable_s3_requests_logging)
            ...
            if (poco_response.getStatus() == CHDBPoco::Net::HTTPResponse::HTTP_TEMPORARY_REDIRECT)
            {
                auto location = poco_response.get("location");
                remote_host_filter.checkURL(CHDBPoco::URI(location));
                uri = location;
                if (enable_s3_requests_logging)
