...
HTTPHeaderEntries getHTTPHeaders(const std::string & config_elem, const CHDBPoco::Util::AbstractConfiguration & config)
{
    ...
    CHDBPoco::Util::AbstractConfiguration::Keys subconfig_keys;
...
AuthSettings::AuthSettings(const DB_CHDB::Settings & settings)
{
    updateFromSettings(settings, /* if_changed */false);
}

void AuthSettings::updateFromSettings(const DB_CHDB::Settings & settings, bool if_changed)
{
    for (auto & field : allMutable())
    {
...
RequestSettings::RequestSettings(const DB_CHDB::Settings & settings, bool validate_settings)
{
...
void RequestSettings::updateFromSettings(
    const DB_CHDB::Settings & settings, bool if_changed, bool validate_settings)
{
    for (auto & field : allMutable())
    {
        const auto setting_name = "s3_" + field.getName();
...
void RequestSettings::normalizeSettings()
{
    if (!storage_class_name.value.empty() && storage_class_name.changed)
        storage_class_name = CHDBPoco::toUpperInPlace(storage_class_name.value);
}

void RequestSettings::finishInit(const DB_CHDB::Settings & settings, bool validate_settings)
{
    normalizeSettings();
    if (validate_settings)
        validateUploadSettings();
...
void RequestSettings::validateUploadSettings()
{
    ...
    if (common_CHDB::mulOverflow(max_upload_part_size.value, upload_part_size_multiply_factor.value, maybe_overflow))
        throw Exception(
                        ErrorCodes::INVALID_SETTING_VALUE,
