...
struct AuthSettings : public BaseSettings<AuthSettingsTraits>
{
    ...
    AuthSettings(
        ...
        const DB_CHDB::Settings & settings,
        const std::string & config_prefix);

    explicit AuthSettings(const DB_CHDB::Settings & settings);

    explicit AuthSettings(const DB_CHDB::NamedCollection & collection);

    void updateFromSettings(const DB_CHDB::Settings & settings, bool if_changed);
...
struct RequestSettings : public BaseSettings<RequestSettingsTraits>
{
    ...
    /// Create request settings from DB_CHDB::Settings.
    explicit RequestSettings(const DB_CHDB::Settings & settings, bool validate_settings = true);
    ...
    void updateFromSettings(const DB_CHDB::Settings & settings, bool if_changed, bool validate_settings = true);
...
private:
    void finishInit(const DB_CHDB::Settings & settings, bool validate_settings);
    void normalizeSettings();
};

HTTPHeaderEntries getHTTPHeaders(const std::string & config_elem, const CHDBPoco::Util::AbstractConfiguration & config);

ServerSideEncryptionKMSConfig getSSEKMSConfig(const std::string & config_elem, const CHDBPoco::Util::AbstractConfiguration & config);
