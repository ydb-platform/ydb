...
DB_CHDB::AggregatedDataVariants::Type convertToTwoLevelTypeIfPossible(DB_CHDB::AggregatedDataVariants::Type type)
{
    using Type = DB_CHDB::AggregatedDataVariants::Type;
    switch (type)
    {
#define M(NAME) \
    case Type::NAME: \
...
void initDataVariantsWithSizeHint(
    DB_CHDB::AggregatedDataVariants & result, DB_CHDB::AggregatedDataVariants::Type method_chosen, const DB_CHDB::Aggregator::Params & params)
{
...
void updateStatistics(const DB_CHDB::ManyAggregatedDataVariants & data_variants, const DB_CHDB::StatsCollectingParams & params)
{
    ...
    DB_CHDB::getHashTablesStatistics().update(sum_of_sizes, *median_size, params);
}

DB_CHDB::ColumnNumbers calculateKeysPositions(const DB_CHDB::Block & header, const DB_CHDB::Aggregator::Params & params)
{
    DB_CHDB::ColumnNumbers keys_positions(params.keys_size);
...
UInt64 calculateCacheKey(const DB_CHDB::ASTPtr & select_query)
{
    if (!select_query)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Query ptr cannot be null");

    const auto & select = select_query->as<DB_CHDB::ASTSelectQuery &>();
