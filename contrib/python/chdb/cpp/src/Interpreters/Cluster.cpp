...
inline bool isLocalImpl(const Cluster::Address & address, const CHDBPoco::Net::SocketAddress & resolved_address, UInt16 clickhouse_port)
{
...
std::optional<CHDBPoco::Net::SocketAddress> Cluster::Address::getResolvedAddress() const
{
    try
    {
...
{
    return escapeForFileName(host_name) + ':' + DB_CHDB::toString(port);
}
...
Clusters::Clusters(const CHDBPoco::Util::AbstractConfiguration & config, const Settings & settings, MultiVersion<Macros>::Version macros, const String & config_prefix)
{
    this->macros_ = macros;
...
void Clusters::updateClusters(const CHDBPoco::Util::AbstractConfiguration & new_config, const Settings & settings, const String & config_prefix, CHDBPoco::Util::AbstractConfiguration * old_config)
{
    CHDBPoco::Util::AbstractConfiguration::Keys new_config_keys;
    ...
    CHDBPoco::Util::AbstractConfiguration::Keys deleted_keys;
    if (old_config)
    {
        ::sort(new_config_keys.begin(), new_config_keys.end());

        CHDBPoco::Util::AbstractConfiguration::Keys old_config_keys;
...
Cluster::Cluster(const CHDBPoco::Util::AbstractConfiguration & config,
                 ...
                 const String & cluster_name) : name(cluster_name)
{
    ...
    for (const auto & key : config_keys)
    {
        ...
        else if (startsWith(key, "shard"))
        {
            ...
            CHDBPoco::Util::AbstractConfiguration::Keys replica_keys;
...
CHDBPoco::Timespan Cluster::saturate(CHDBPoco::Timespan v, CHDBPoco::Timespan limit)
{
