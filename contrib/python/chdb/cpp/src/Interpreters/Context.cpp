...
struct ContextSharedPart : boost::noncopyable
{
    ...
    const CHDBPoco::Util::AbstractConfiguration & getConfigRefWithLock(const std::lock_guard<ContextSharedMutex> &) const TSA_REQUIRES(this->mutex)
    {
        return config ? *config : CHDBPoco::Util::Application::instance().config();
    }
...
{
    DB_CHDB::tryLogCurrentException(log, fmt::format(
...
static VolumePtr createLocalSingleDiskVolume(const std::string & path, const CHDBPoco::Util::AbstractConfiguration & config_)
{
...
const CHDBPoco::Util::AbstractConfiguration & Context::getConfigRef() const
{
...
void Context::setExternalAuthenticatorsConfig(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
StoragePtr Context::executeTableFunction(const ASTPtr & table_expression, const ASTSelectQuery * select_query_hint)
{
    ...
    if (!res)
    {
        ...
        if (select_query_hint && use_structure_from_insertion_table_in_table_functions && table_function_ptr->needStructureHint() && hasInsertionTable())
        {
            ...
            DB_CHDB::ColumnsDescription structure_hint;
...
void Context::setCurrentQueryId(const String & query_id)
{
    ...
    ///  because CHDBPoco::UUIDGenerator::generateRandom method is using /dev/random, that is very expensive.
    ...
    if (query_id_to_set.empty())    /// If the user did not submit his query_id, then we generate it ourselves.
    {
        ...
        struct QueryUUID : CHDBPoco::UUID
        {
            QueryUUID(const char * bytes, CHDBPoco::UUID::Version version)
                : CHDBPoco::UUID(bytes, version) {}
        };

        query_id_to_set = QueryUUID(random.bytes, CHDBPoco::UUID::UUID_RANDOM).toString();
    }
...
void Context::tryCreateEmbeddedDictionaries(const CHDBPoco::Util::AbstractConfiguration & config) const
{
    if (!config.getBool("dictionaries_lazy_load", true))
        static_cast<void>(getEmbeddedDictionariesImpl(true));
}

void Context::loadOrReloadDictionaries(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::loadOrReloadUserDefinedExecutableFunctions(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::updateUncompressedCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateMarkCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateIndexUncompressedCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateIndexMarkCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateMMappedFileCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
void Context::updateQueryCacheConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    std::lock_guard lock(shared->mutex);
...
bool checkZooKeeperConfigIsLocal(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_name)
{
...
void Context::updateKeeperConfiguration([[maybe_unused]] const CHDBPoco::Util::AbstractConfiguration & config)
{
#if USE_NURAFT
    std::lock_guard lock(shared->keeper_dispatcher_mutex);
    if (!shared->keeper_dispatcher)
...
void Context::updateInterserverCredentials(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setRemoteHostFilter(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setHTTPHeaderFilter(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::updateStorageConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
{
    {
        std::lock_guard lock(shared->storage_policies_mutex);
...
void Context::setApplicationType(ApplicationType type)
{
    ...
    if (type == ApplicationType::LOCAL || type == ApplicationType::SERVER || type == ApplicationType::DISKS)
        shared->server_settings.loadSettingsFromConfig(CHDBPoco::Util::Application::instance().config());

    if (type == ApplicationType::SERVER)
        shared->configureServerWideThrottling();
}

void Context::setDefaultProfiles(const CHDBPoco::Util::AbstractConfiguration & config)
{
...
void Context::setHTTPClientInfo(const CHDBPoco::Net::HTTPRequest & request)
{
...
void Context::setCurrentAddress(const CHDBPoco::Net::SocketAddress & current_address)
{
...
void Context::setInitialAddress(const CHDBPoco::Net::SocketAddress & initial_address)
{
...
CHDBPoco::Timespan HTTPContext::getReceiveTimeout() const
{
return context->getSettingsRef().http_receive_timeout;
}

CHDBPoco::Timespan HTTPContext::getSendTimeout() const
{
