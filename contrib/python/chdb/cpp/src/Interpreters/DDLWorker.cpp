...
bool DDLWorker::tryExecuteQuery(DDLTaskBase & task, const ZooKeeperPtr & zookeeper)
{
    ...
    catch (...)
    {
        ...
        /// We don't know what exactly happened, but maybe it's CHDBPoco::NetException or std::bad_alloc,
...
bool DDLWorker::canRemoveQueueEntry(const String & entry_name, const Coordination::Stat & stat)
{
    ...
    bool node_lifetime_is_expired = zookeeper_time_seconds + task_max_lifetime < CHDBPoco::Timestamp().epochTime();
...
String DDLWorker::enqueueQuery(DDLLogEntry & entry)
{
    ...
    if (max_pushed_entry_metric)
    {
        ...
        DB_CHDB::ReadBufferFromString in(str_buf);
...
void DDLWorker::runCleanupThread()
{
    ...
    while (!stop_flag)
    {
        try
        {
            ...
            Int64 current_time_seconds = CHDBPoco::Timestamp().epochTime();
