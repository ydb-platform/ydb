...
bool DatabaseCatalog::isTableExist(const DB_CHDB::StorageID & table_id, ContextPtr context_) const
{
    if (table_id.hasUUID())
...
void DatabaseCatalog::loadMarkedAsDroppedTables()
{
    ...
    CHDBPoco::DirectoryIterator dir_end;
    for (CHDBPoco::DirectoryIterator it(path); it != dir_end; ++it)
    {
...
bool DatabaseCatalog::maybeRemoveDirectory(const String & disk_name, const DiskPtr & disk, const String & unused_dir)
{
    ...
    try
    {
        ...
        if (st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO))
        {
            ...
            disk->setLastModified(unused_dir, CHDBPoco::Timestamp::fromEpochTime(current_time));
