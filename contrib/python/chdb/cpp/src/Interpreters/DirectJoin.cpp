    ...
    const Block & source_sample_block, const Block & result_sample_block, MutableColumns && columns, const PaddedPODArray<UInt8> & null_map)
{
    ...
    for (const auto & out_sample_col : result_sample_block)
    {
        ...
        {
            throw DB_CHDB::Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Can't find column '{}'", out_sample_col.name);
        }
    ...
    , log(getLogger("DirectKeyValueJoin"))
{
    ...
    {
        throw DB_CHDB::Exception(ErrorCodes::UNSUPPORTED_JOIN_KEYS, "Not supported by direct JOIN");
    }
    ...
    {
        throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "Strictness {} and kind {} is not supported by direct JOIN",
...
{
    throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Unreachable code reached");
}
