...
public:
    ...
    static DictionaryStructure getDictionaryStructure(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & key_in_config = "dictionary");
...
protected:
    LoadableMutablePtr createObject(const std::string & name, const CHDBPoco::Util::AbstractConfiguration & config,
                                    const std::string & key_in_config, const std::string & repository_name) const override;

    bool doesConfigChangeRequiresReloadingObject(const CHDBPoco::Util::AbstractConfiguration & old_config, const String & old_key_in_config,
                                                 const CHDBPoco::Util::AbstractConfiguration & new_config, const String & new_key_in_config) const override;

    void updateObjectFromConfigWithoutReloading(
        IExternalLoadable & object, const CHDBPoco::Util::AbstractConfiguration & config, const String & key_in_config) const override;
