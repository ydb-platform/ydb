...
std::optional<CHDBPoco::Timestamp> ExternalLoaderXMLConfigRepository::getUpdateTime(const std::string & definition_entity_name)
{
...
std::set<std::string> ExternalLoaderXMLConfigRepository::getAllLoadablesDefinitionNames()
{
    ...
    for (const auto & pattern : patterns_copy)
    {
        ...
        if (pattern[0] != '/')
        {
            ...
            CHDBPoco::Glob::glob(absolute_path, files, 0);
            if (!files.empty())
                continue;
        }

        CHDBPoco::Glob::glob(pattern, files, 0);
    }
...
CHDBPoco::AutoPtr<CHDBPoco::Util::AbstractConfiguration> ExternalLoaderXMLConfigRepository::load(
