...
std::optional<HashTablesStatistics::Entry> HashTablesStatistics::getSizeHint(const Params & params)
{
    if (!params.isCollectionAndUseEnabled())
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Collection and use of the statistics should be enabled.");
...
std::optional<HashTablesCacheStatistics> HashTablesStatistics::getCacheStats() const
{
    ...
    if (hash_table_stats)
    {
        ...
        return DB_CHDB::HashTablesCacheStatistics{.entries = hash_table_stats->count(), .hits = hits, .misses = misses};
    }
    return std::nullopt;
}
...
std::optional<HashTablesStatistics::Entry> getSizeHint(const DB_CHDB::StatsCollectingParams & stats_collecting_params, size_t tables_cnt)
{
    ...
    {
        if (auto hint = DB_CHDB::getHashTablesStatistics().getSizeHint(stats_collecting_params))
        {
