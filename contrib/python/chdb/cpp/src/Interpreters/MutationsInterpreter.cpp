...
void MutationsInterpreter::prepare(bool dry_run)
{
    ...
    for (const auto & command : commands)
    {
        ...
        else if (command.type == MutationCommand::READ_COLUMN)
        {
            ...
            throw Exception(ErrorCodes::UNKNOWN_MUTATION_COMMAND, "Unknown mutation command type: {}", DB_CHDB::toString<int>(command.type));
    }
    ...
    bool can_execute_) const
{
    ...
    if (!can_execute_)
    {
        ...
        auto callback = []()
        {
            return DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Cannot execute a mutation because can_execute flag set to false");
        };
...
{
    return DB_CHDB::getPartitionAndPredicateExpressionForMutationCommand(command, source.getStorage(), context);
}
