...
std::shared_ptr<DB_CHDB::DataTypeEnum8> TypeEnum = std::make_shared<DB_CHDB::DataTypeEnum8>(DB_CHDB::DataTypeEnum8::Values{
...
void dumpToMapColumn(const Counters::Snapshot & counters, DB_CHDB::IColumn * column, bool nonzero_only)
{
...
static void dumpProfileEvents(ProfileEventsSnapshot const & snapshot, DB_CHDB::MutableColumns & columns, String const & host_name)
{
    size_t rows = 0;
...
static void dumpMemoryTracker(ProfileEventsSnapshot const & snapshot, DB_CHDB::MutableColumns & columns, String const & host_name)
{
    size_t i = 0;
    columns[i++]->insertData(host_name.data(), host_name.size());
    columns[i++]->insert(static_cast<UInt64>(snapshot.current_time));
