...
void Session::authenticate(const String & user_name, const String & password, const CHDBPoco::Net::SocketAddress & address)
{
    authenticate(BasicCredentials{user_name, password}, address);
}

void Session::authenticate(const Credentials & credentials_, const CHDBPoco::Net::SocketAddress & address_)
{
    if (session_context)
    ...
    if ((address == CHDBPoco::Net::SocketAddress{}) && (prepared_client_info->interface == ClientInfo::Interface::LOCAL))
        address = CHDBPoco::Net::SocketAddress{"127.0.0.1", 0};
...
void Session::onAuthenticationFailure(const std::optional<String> & user_name, const CHDBPoco::Net::SocketAddress & address_, const Exception & e)
{
...
void Session::setHTTPClientInfo(const CHDBPoco::Net::HTTPRequest & request)
{
    if (session_context)
