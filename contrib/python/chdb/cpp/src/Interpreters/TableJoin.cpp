...
void TableJoin::inferJoinKeyCommonType(const LeftNamesAndTypes & left, const RightNamesAndTypes & right, bool allow_right, bool require_strict_keys_match)
{
    ...
    {
        if (clauses.size() != 1)
            throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "ASOF join over multiple keys is not supported");
    }
    ...
    forAllKeys(clauses, [&](const auto & left_key_name, const auto & right_key_name)
    {
        ...
        try
        {
            ...
            common_type = DB_CHDB::getLeastSupertype(DataTypes{ltype, rtype});
        }
        catch (DB_CHDB::Exception & ex)
        {
            throw DB_CHDB::Exception(ErrorCodes::TYPE_MISMATCH,
...
void TableJoin::assertHasOneOnExpr() const
{
    if (!oneDisjunct())
    {
        ...
        throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Expected to have only one join clause, got {}: [{}], query: '{}'",
