...
OwnJSONPatternFormatter::OwnJSONPatternFormatter(CHDBPoco::Util::AbstractConfiguration & config)
{
...
void OwnJSONPatternFormatter::formatExtended(const DB_CHDB::ExtendedLogMessage & msg_ext, std::string & text) const
{
    DB_CHDB::WriteBufferFromString wb(text);

    DB_CHDB::FormatSettings settings;
    ...
    if (!source_line.empty())
    {
        ...
        DB_CHDB::writeChar(':', wb);
        DB_CHDB::writeChar('\"', wb);
        DB_CHDB::writeIntText(msg.getSourceLine(), wb);
        DB_CHDB::writeChar('\"', wb);
    }
    DB_CHDB::writeChar('}', wb);
}

void OwnJSONPatternFormatter::format(const CHDBPoco::Message & msg, std::string & text)
{
