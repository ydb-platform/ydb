...
void OwnSplitChannel::log(const CHDBPoco::Message & msg)
{
    if (!isLoggingEnabled())
        return;
...
void OwnSplitChannel::tryLogSplit(const CHDBPoco::Message & msg)
{
...
void OwnSplitChannel::logSplit(const CHDBPoco::Message & msg)
{
...
void OwnSplitChannel::addChannel(CHDBPoco::AutoPtr<CHDBPoco::Channel> channel, const std::string & name)
{
...
void OwnSplitChannel::setLevel(const std::string & name, int level)
{
     ...
     if (it != channels.end())
     {
         if (auto * channel = dynamic_cast<DB_CHDB::OwnFormattingChannel *>(it->second.first.get()))
