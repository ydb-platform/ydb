...
namespace
{
constexpr DB_CHDB::TokenType determineClosingPair(const DB_CHDB::TokenType token_type)
{
    if (token_type == DB_CHDB::TokenType::OpeningCurlyBrace)
        return DB_CHDB::TokenType::ClosingCurlyBrace;
    else if (token_type == DB_CHDB::TokenType::OpeningRoundBracket)
        return DB_CHDB::TokenType::ClosingRoundBracket;
    else if (token_type == DB_CHDB::TokenType::OpeningSquareBracket)
        return DB_CHDB::TokenType::ClosingSquareBracket;

    throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NOT_IMPLEMENTED, "Unhandled token: {}", magic_enum::enum_name(token_type));
}

constexpr bool isClosingBracket(const DB_CHDB::TokenType token_type)
{
    return token_type == DB_CHDB::TokenType::ClosingCurlyBrace || token_type == DB_CHDB::TokenType::ClosingRoundBracket
        || token_type == DB_CHDB::TokenType::ClosingSquareBracket;
}

constexpr bool isOpeningBracket(const DB_CHDB::TokenType token_type)
{
    return token_type == DB_CHDB::TokenType::OpeningCurlyBrace || token_type == DB_CHDB::TokenType::OpeningRoundBracket
        || token_type == DB_CHDB::TokenType::OpeningSquareBracket;
}
}
...
String IParserKQLFunction::getArgument(const String & function_name, DB_CHDB::IParser::Pos & pos, const ArgumentState argument_state)
{
...
std::vector<std::string> IParserKQLFunction::getArguments(
    const String & function_name, DB_CHDB::IParser::Pos & pos, const ArgumentState argument_state, const Interval & argument_count_interval)
{
...
std::optional<String>
IParserKQLFunction::getOptionalArgument(const String & function_name, DB_CHDB::IParser::Pos & pos, const ArgumentState argument_state)
{
    if (const auto type = pos->type; type != DB_CHDB::TokenType::Comma && type != DB_CHDB::TokenType::OpeningRoundBracket)
        return {};

    ++pos;
    if (const auto type = pos->type; type == DB_CHDB::TokenType::ClosingRoundBracket || type == DB_CHDB::TokenType::ClosingSquareBracket)
        return {};
    ...
    std::stack<DB_CHDB::TokenType> scopes;
    while (isValidKQLPos(pos) && (!scopes.empty() || (pos->type != DB_CHDB::TokenType::Comma && pos->type != DB_CHDB::TokenType::ClosingRoundBracket)))
    {
        ...
        else if (isClosingBracket(token_type))
        {
            if (scopes.empty() || determineClosingPair(scopes.top()) != token_type)
                throw Exception(
                    DB_CHDB::ErrorCodes::SYNTAX_ERROR, "Unmatched token: {} when parsing {}", magic_enum::enum_name(token_type), function_name);
    ...
    const std::string_view function_name, const std::span<const std::string_view> params, uint32_t max_depth, uint32_t max_backtracks)
{
    ...
    return DB_CHDB::IParserKQLFunction::getExpression(tokens_pos);
}
    ...
    if (pos->type == TokenType::BareWord)
    {
        ...
        else
        {
            if (!fun)
            {
                ...
                if (pos->type == TokenType::OpeningRoundBracket)
                {
                    if (CHDBPoco::toLower(arg) != "and" && CHDBPoco::toLower(arg) != "or")
