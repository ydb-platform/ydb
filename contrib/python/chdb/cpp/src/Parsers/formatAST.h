...
template<>
struct fmt::formatter<DB_CHDB::ASTPtr>
{
    ...
    auto format(const DB_CHDB::ASTPtr & ast, FormatContext & context)
    {
        return fmt::format_to(context.out(), "{}", DB_CHDB::serializeAST(*ast));
    }
};
