...
void IRowOutputFormat::consume(DB_CHDB::Chunk chunk)
{
...
void IRowOutputFormat::consumeTotals(DB_CHDB::Chunk chunk)
{
    if (!supportTotals())
        return;
...
void IRowOutputFormat::consumeExtremes(DB_CHDB::Chunk chunk)
{
    if (!supportExtremes())
        return;
...
void IRowOutputFormat::writeMinExtreme(const DB_CHDB::Columns & columns, size_t row_num)
{
    write(columns, row_num);
}

void IRowOutputFormat::writeMaxExtreme(const DB_CHDB::Columns & columns, size_t row_num)
{
    write(columns, row_num);
}

void IRowOutputFormat::writeTotals(const DB_CHDB::Columns & columns, size_t row_num)
{
    write(columns, row_num);
}
