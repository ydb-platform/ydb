...
arrow20::Compression::type getArrowCompression(FormatSettings::ArrowCompression method)
{
    switch (method)
    {
        ...
        case FormatSettings::ArrowCompression::ZSTD:
            return arrow20::Compression::type::ZSTD;
        case FormatSettings::ArrowCompression::LZ4_FRAME:
            return arrow20::Compression::type::LZ4_FRAME;
    }
}
...
void ArrowBlockOutputFormat::prepareWriter(const std::shared_ptr<arrow20::Schema> & schema)
{
    arrow_ostream = std::make_shared<ArrowBufferedOutputStream>(out);
    arrow20::Result<std::shared_ptr<arrow20::ipc::RecordBatchWriter>> writer_status;
    arrow20::ipc::IpcWriteOptions options = arrow20::ipc::IpcWriteOptions::Defaults();
    options.codec = *arrow20::util::Codec::Create(getArrowCompression(format_settings.arrow.output_compression_method));
    options.emit_dictionary_deltas = true;

    // TODO: should we use arrow20::ipc::IpcOptions::alignment?
    if (stream)
        writer_status = arrow20::ipc::MakeStreamWriter(arrow_ostream.get(), schema, options);
    else
        writer_status = arrow20::ipc::MakeFileWriter(arrow_ostream.get(), schema,options);
