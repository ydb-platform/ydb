...
arrow20::Status ArrowBufferedOutputStream::Write(const void * data, int64_t length)
{
    ...
    catch (...)
    {
        ...
        return arrow20::Status::IOError(message);
    }
}
...
{
    ARROW_ASSIGN_OR_RAISE(auto buffer, arrow20::AllocateResizableBuffer(nbytes, ArrowMemoryPool::instance()))
...
arrow20::Future<std::shared_ptr<arrow20::Buffer>> RandomAccessFileFromSeekableReadBuffer::ReadAsync(const arrow20::io::IOContext &, int64_t position, int64_t nbytes)
{
...
arrow20::Result<int64_t> RandomAccessFileFromRandomAccessReadBuffer::GetSize()
{
    return file_size;
}

arrow20::Result<int64_t> RandomAccessFileFromRandomAccessReadBuffer::ReadAt(int64_t position, int64_t nbytes, void* out)
{
    try
    {
...
arrow20::Result<std::shared_ptr<arrow20::Buffer>> RandomAccessFileFromRandomAccessReadBuffer::ReadAt(int64_t position, int64_t nbytes)
{
    ARROW_ASSIGN_OR_RAISE(auto buffer, arrow20::AllocateResizableBuffer(nbytes, ArrowMemoryPool::instance()))
...
arrow20::Future<std::shared_ptr<arrow20::Buffer>> RandomAccessFileFromRandomAccessReadBuffer::ReadAsync(const arrow20::io::IOContext&, int64_t position, int64_t nbytes)
{
    return arrow20::Future<std::shared_ptr<arrow20::Buffer>>::MakeFinished(ReadAt(position, nbytes));
}

arrow20::Status RandomAccessFileFromRandomAccessReadBuffer::Close()
{
    chassert(is_open);
    is_open = false;
    return arrow20::Status::OK();
...
arrow20::Status RandomAccessFileFromRandomAccessReadBuffer::Seek(int64_t) { return arrow20::Status::NotImplemented(""); }
arrow20::Result<int64_t> RandomAccessFileFromRandomAccessReadBuffer::Tell() const { return arrow20::Status::NotImplemented(""); }
arrow20::Result<int64_t> RandomAccessFileFromRandomAccessReadBuffer::Read(int64_t, void*) { return arrow20::Status::NotImplemented(""); }
arrow20::Result<std::shared_ptr<arrow20::Buffer>> RandomAccessFileFromRandomAccessReadBuffer::Read(int64_t) { return arrow20::Status::NotImplemented(""); }
...
arrow20::Status ArrowMemoryPool::Allocate(int64_t size, int64_t alignment, uint8_t ** out)
{
    if (size == 0)
    {
        *out = arrow20::memory_pool::internal::kZeroSizeArea;
        return arrow20::Status::OK();
    }
    ...
    {
        return arrow20::Status::OutOfMemory("allocation of size ", size, " failed");
    }

    return arrow20::Status::OK();
}

arrow20::Status ArrowMemoryPool::Reallocate(int64_t old_size, int64_t new_size, int64_t alignment, uint8_t ** ptr)
{
    if (old_size == 0)
    {
        chassert(*ptr == arrow20::memory_pool::internal::kZeroSizeArea);
        return Allocate(new_size, alignment, ptr);
    }
    if (new_size == 0)
    {
        Free(*ptr, old_size, alignment);
        *ptr = arrow20::memory_pool::internal::kZeroSizeArea;
        return arrow20::Status::OK();
    }
    ...
    {
        return arrow20::Status::OutOfMemory("reallocation of size ", new_size, " failed");
    }
...
void ArrowMemoryPool::Free(uint8_t * buffer, int64_t size, int64_t /*alignment*/)
{
    if (size == 0)
    {
        chassert(buffer == arrow20::memory_pool::internal::kZeroSizeArea);
        return;
    }
