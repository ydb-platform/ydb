...
#define FOR_ARROW_NUMERIC_TYPES(M) \
        M(arrow20::Type::UINT8, UInt8) \
        M(arrow20::Type::INT8, Int8) \
        M(arrow20::Type::INT16, Int16) \
        M(arrow20::Type::UINT64, UInt64) \
        M(arrow20::Type::INT64, Int64) \
        M(arrow20::Type::DURATION, Int64) \
        M(arrow20::Type::HALF_FLOAT, Float32) \
        M(arrow20::Type::FLOAT, Float32) \
        M(arrow20::Type::DOUBLE, Float64)

#define FOR_ARROW_INDEXES_TYPES(M) \
        M(arrow20::Type::UINT8, UInt8) \
        M(arrow20::Type::INT8, UInt8) \
        M(arrow20::Type::UINT16, UInt16) \
        M(arrow20::Type::INT16, UInt16) \
        M(arrow20::Type::UINT32, UInt32) \
        M(arrow20::Type::INT32, UInt32) \
        M(arrow20::Type::UINT64, UInt64) \
        M(arrow20::Type::INT64, UInt64)
...
static ColumnWithTypeAndName readColumnWithNumericData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithStringData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithFixedStringData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
    const auto * fixed_type = assert_cast<arrow20::FixedSizeBinaryType *>(arrow_column->type().get());
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
        arrow20::FixedSizeBinaryArray & chunk = dynamic_cast<arrow20::FixedSizeBinaryArray &>(*(arrow_column->chunk(chunk_i)));
...
static ColumnWithTypeAndName readColumnWithBigIntegerFromFixedBinaryData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name, const DataTypePtr & column_type)
{
    const auto * fixed_type = assert_cast<arrow20::FixedSizeBinaryType *>(arrow_column->type().get());
    size_t fixed_len = fixed_type->byte_width();
...
static ColumnWithTypeAndName readColumnWithBigNumberFromBinaryData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name, const DataTypePtr & column_type)
{
    size_t total_size = 0;
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
        auto & chunk = dynamic_cast<arrow20::BinaryArray &>(*(arrow_column->chunk(chunk_i)));
        const size_t chunk_length = chunk.length();
        ...
        for (size_t i = 0; i != chunk_length; ++i)
        {
            ...
            if (!chunk.IsNull(i) && chunk.value_length(i) != sizeof(ValueType))
                return readColumnWithStringData<arrow20::BinaryArray>(arrow_column, column_name);
...
static ColumnWithTypeAndName readColumnWithBooleanData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithDate32Data(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name,
...
static ColumnWithTypeAndName readColumnWithDate64Data(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithTimestampData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
    const auto & arrow_type = static_cast<const arrow20::TimestampType &>(*(arrow_column->type()));
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
        const auto & chunk = dynamic_cast<const arrow20::TimestampArray &>(*(arrow_column->chunk(chunk_i)));
        for (size_t value_i = 0, length = static_cast<size_t>(chunk.length()); value_i < length; ++value_i)
        {
...
static ColumnWithTypeAndName readColumnWithTimeData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithTime32Data(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
    return readColumnWithTimeData<arrow20::Time32Type, arrow20::Time32Array>(arrow_column, column_name);
}

static ColumnWithTypeAndName readColumnWithTime64Data(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
    return readColumnWithTimeData<arrow20::Time64Type, arrow20::Time64Array>(arrow_column, column_name);
}

template <typename DecimalType, typename DecimalArray>
static ColumnWithTypeAndName readColumnWithDecimalDataImpl(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name, DataTypePtr internal_type)
{
    auto internal_column = internal_type->createColumn();
...
static ColumnWithTypeAndName readColumnWithDecimalData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnPtr readByteMapFromArrowColumn(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column)
{
...
template <>
struct ArrowOffsetArray<arrow20::ListArray>
{
    using type = arrow20::Int32Array;
};

template <>
struct ArrowOffsetArray<arrow20::LargeListArray>
{
    using type = arrow20::Int64Array;
};

template <typename ArrowListArray>
static ColumnPtr readOffsetsFromArrowListColumn(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column)
{
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
         ...
         * monotonically increasing, which will cause inconsistencies with the batch data length on `DB_CHDB::ColumnArray`.
         *
 ...
 * */
template <typename NumericType, typename VectorType = ColumnVector<NumericType>>
static ColumnWithTypeAndName readColumnWithIndexesDataImpl(std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name, Int64 default_value_index, NumericType dict_size, bool is_nullable)
{
    auto internal_type = std::make_shared<DataTypeNumber<NumericType>>();
    auto internal_column = internal_type->createColumn();
    auto & column_data = static_cast<VectorType &>(*internal_column).getData();
...
static ColumnPtr readColumnWithIndexesData(std::shared_ptr<arrow20::ChunkedArray> & arrow_column, Int64 default_value_index, UInt64 dict_size, bool is_nullable)
{
...
template <typename ArrowListArray>
static std::shared_ptr<arrow20::ChunkedArray> getNestedArrowColumn(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column)
{
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
        ...
        /*
         * It seems like arrow20::ListArray::values() (nested column data) might or might not be shared across chunks.
         * Therefore, simply appending arrow20::ListArray::values() could lead to duplicated data to be appended.
         * To properly handle this, arrow20::ListArray::values() needs to be sliced based on the chunk offsets.
         * arrow20::ListArray::Flatten does that. More info on: https://lists.apache.org/thread/rrwfb9zo2dc58dhd9rblf20xd7wmy7jm and
...
static ColumnWithTypeAndName readIPv6ColumnFromBinaryData(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
    size_t total_size = 0;
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
...
static ColumnWithTypeAndName readIPv4ColumnWithInt32Data(const std::shared_ptr<arrow20::ChunkedArray> & arrow_column, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnFromArrowColumn(
    const std::shared_ptr<arrow20::ChunkedArray> & arrow_column,
    ...
    const ReadColumnFromArrowColumnSettings & settings)
{
    switch (arrow_column->type()->id())
    {
        ...
        case arrow20::Type::LARGE_STRING:
            return readColumnWithStringData<arrow20::LargeBinaryArray>(arrow_column, column_name);
        ...
        case arrow20::Type::DECIMAL128:
            return readColumnWithDecimalData<arrow20::Decimal128Array>(arrow_column, column_name);
        case arrow20::Type::DECIMAL256:
            return readColumnWithDecimalData<arrow20::Decimal256Array>(arrow_column, column_name);
        case arrow20::Type::MAP:
        {
            ...
            auto arrow_nested_column = getNestedArrowColumn<arrow20::ListArray>(arrow_column);
            ...
            auto offsets_column = readOffsetsFromArrowListColumn<arrow20::ListArray>(arrow_column);
        ...
        {
            bool is_large_list = arrow_column->type()->id() == arrow20::Type::LARGE_LIST;
            DataTypePtr nested_type_hint;
            if (type_hint)
            {
            ...
            if (is_large_list)
            {
                auto * arrow_large_list_type = assert_cast<arrow20::LargeListType *>(arrow_column->type().get());
                is_nested_nullable_column = arrow_large_list_type->value_field()->nullable();
            }
            else
            {
                auto * arrow_list_type = assert_cast<arrow20::ListType *>(arrow_column->type().get());
                is_nested_nullable_column = arrow_list_type->value_field()->nullable();
            }

            auto arrow_nested_column = is_large_list ? getNestedArrowColumn<arrow20::LargeListArray>(arrow_column) : getNestedArrowColumn<arrow20::ListArray>(arrow_column);
            auto nested_column = readColumnFromArrowColumn(arrow_nested_column,
                column_name,
                dictionary_infos,
                nested_type_hint,
            ...
            auto offsets_column = is_large_list ? readOffsetsFromArrowListColumn<arrow20::LargeListArray>(arrow_column) : readOffsetsFromArrowListColumn<arrow20::ListArray>(arrow_column);
    ...
    const ReadColumnFromArrowColumnSettings & settings)
{
    ...
    if (read_as_nullable_column &&
        arrow_column->type()->id() != arrow20::Type::LIST &&
        arrow_column->type()->id() != arrow20::Type::LARGE_LIST &&
        arrow_column->type()->id() != arrow20::Type::MAP &&
        arrow_column->type()->id() != arrow20::Type::STRUCT &&
        arrow_column->type()->id() != arrow20::Type::DICTIONARY)
    {
        DataTypePtr nested_type_hint;
        if (type_hint)
...
static std::shared_ptr<arrow20::ChunkedArray> createArrowColumn(const std::shared_ptr<arrow20::Field> & field, const String & format_name)
{
    ...
    return std::make_shared<arrow20::ChunkedArray>(arrow20::ArrayVector{arrow_array});
}
...
Chunk ArrowColumnToCHColumn::arrowTableToCHChunk(const std::shared_ptr<arrow20::Table> & table, size_t num_rows, BlockMissingValues * block_missing_values)
{
