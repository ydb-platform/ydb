...
public:
    ...
    std::unordered_map<std::string, std::pair<int, int>>
        calculateFieldIndices(const arrow20::Schema & schema)
    {
    ...
    std::vector<int> findRequiredIndices(const Block & header, const arrow20::Schema & schema)
    {
    ...
    size_t countIndicesForType(std::shared_ptr<arrow20::DataType> type)
    {
...
private:
    ...
    void calculateFieldIndices(const arrow20::Field & field,
        ...
        std::unordered_map<std::string, std::pair<int, int>> & result, const std::string & name_prefix = "")
    {
        ...
        if (field_type->id() == arrow20::Type::STRUCT)
        {
            auto * struct_type = static_cast<arrow20::StructType *>(field_type.get());
        ...
        else if (field_type->id() == arrow20::Type::LIST)
        {
            // It is a nested table.
            const auto * list_type = static_cast<arrow20::ListType *>(field_type.get());
        ...
        else if (field_type->id() == arrow20::Type::MAP)
        {
            const auto * map_type = static_cast<arrow20::MapType *>(field_type.get());
            auto index_snapshot = current_start_index;
        ...
        std::vector<int> & required_indices)
    {
        ...
        if (const DB_CHDB::DataTypeTuple * type_tuple = typeid_cast<const DB_CHDB::DataTypeTuple *>(nested_type.get()))
        {
        ...
        else if (const auto * type_array = typeid_cast<const DB_CHDB::DataTypeArray *>(nested_type.get()))
        {
            findRequiredIndices(name, type_array->getNestedType(), field_indices, added_indices, required_indices);
            return;
        }
        else if (const auto * type_map = typeid_cast<const DB_CHDB::DataTypeMap *>(nested_type.get()))
        {
