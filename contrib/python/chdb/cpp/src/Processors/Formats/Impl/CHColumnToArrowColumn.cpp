...
namespace DB_CHDB
{
    ...
    static const std::initializer_list<std::pair<String, std::shared_ptr<arrow20::DataType>>> internal_type_to_arrow_type =
    {
        ...
        {"Int128", arrow20::fixed_size_binary(sizeof(Int128))},
        {"UInt128", arrow20::fixed_size_binary(sizeof(UInt128))},
        {"Int256", arrow20::fixed_size_binary(sizeof(Int256))},
        {"UInt256", arrow20::fixed_size_binary(sizeof(UInt256))},
    };
        ...
        size_t end)
    {
        ...
        arrow20::BooleanBuilder & builder = assert_cast<arrow20::BooleanBuilder &>(*array_builder);
        arrow20::Status status;
        ...
        size_t end)
    {
        ...
        for (size_t value_i = start; value_i < end; ++value_i)
        {
            ...
            else
            {
                ...
                if (need_rescale)
                {
                    if (common_CHDB::mulOverflow(value, rescale_multiplier, value))
    ...
    static void checkIfIndexesTypeIsExceeded(const std::shared_ptr<arrow20::DataType> & arrow_type, size_t dict_size)
    {
        const auto & dict_indexes_arrow_type = assert_cast<const arrow20::DictionaryType *>(arrow_type.get())->index_type();
        /// We can use Int32/UInt32/Int64/UInt64 type for indexes.
        const auto * indexes_int32_type = typeid_cast<const arrow20::Int32Type *>(dict_indexes_arrow_type.get());
        const auto * indexes_uint32_type = typeid_cast<const arrow20::UInt32Type *>(dict_indexes_arrow_type.get());
        const auto * indexes_int64_type = typeid_cast<const arrow20::UInt32Type *>(dict_indexes_arrow_type.get());
        ...
        std::unordered_map<String, MutableColumnPtr> & dictionary_values)
    {
        ...
        arrow20::MemoryPool* pool = ArrowMemoryPool::instance();
        ...
        std::shared_ptr<arrow20::Array> arrow_dict_array;
...
#undef DISPATCH
        ...
        size_t end)
    {
        ...
        arrow20::FixedSizeBinaryBuilder & builder = assert_cast<arrow20::FixedSizeBinaryBuilder &>(*array_builder);

        PaddedPODArray<UInt8> arrow_null_bytemap = revertNullByteMap(null_bytemap, start, end);
        const UInt8 * arrow_null_bytemap_raw_ptr = arrow_null_bytemap.empty() ? nullptr : arrow_null_bytemap.data();

        const uint8_t * data_start = reinterpret_cast<const uint8_t *>(internal_data.data() + start * fixed_length);
        arrow20::Status status = builder.AppendValues(data_start, end - start, reinterpret_cast<const uint8_t *>(arrow_null_bytemap_raw_ptr));
        checkStatus(status, write_column->getName(), format_name);
    }
        ...
        size_t end)
    {
        ...
        auto & builder = assert_cast<arrow20::UInt32Builder &>(*array_builder);

        PaddedPODArray<UInt8> arrow_null_bytemap = revertNullByteMap(null_bytemap, start, end);
        const UInt8 * arrow_null_bytemap_raw_ptr = arrow_null_bytemap.empty() ? nullptr : arrow_null_bytemap.data();
        arrow20::Status status = builder.AppendValues(&(internal_data.data() + start)->toUnderType(), end - start, reinterpret_cast<const uint8_t *>(arrow_null_bytemap_raw_ptr));
        checkStatus(status, write_column->getName(), format_name);
    }
        ...
        switch (column_type->getTypeId())
        {
            ...
            case TypeIndex::String:
            {
                if (settings.output_string_as_string)
                    fillArrowArrayWithStringColumnData<ColumnString, arrow20::StringBuilder>(column, null_bytemap, format_name, array_builder, start, end);
                else
                    fillArrowArrayWithStringColumnData<ColumnString, arrow20::BinaryBuilder>(column, null_bytemap, format_name, array_builder, start, end);
                break;
            }
            case TypeIndex::FixedString:
            {
                if (settings.output_fixed_string_as_fixed_byte_array)
                    ...
                    fillArrowArrayWithStringColumnData<ColumnFixedString, arrow20::StringBuilder>(column, null_bytemap, format_name, array_builder, start, end);
                else
                    fillArrowArrayWithStringColumnData<ColumnFixedString, arrow20::BinaryBuilder>(column, null_bytemap, format_name, array_builder, start, end);
                break;
            }
            case TypeIndex::IPv6:
            ...
            case TypeIndex::Array:
                fillArrowArrayWithArrayColumnData<arrow20::ListBuilder>(column_name, column, column_type, null_bytemap, array_builder, format_name, start, end, settings, dictionary_values);
                break;
            ...
            case TypeIndex::Map:
            {
                ...
                fillArrowArrayWithArrayColumnData<arrow20::MapBuilder>(column_name, column_array, array_type, null_bytemap, array_builder, format_name, start, end, settings, dictionary_values);
                break;
            }
            case TypeIndex::Decimal32:
                fillArrowArrayWithDecimalColumnData<DataTypeDecimal32, Int128, arrow20::Decimal128, arrow20::Decimal128Builder>(column, null_bytemap, array_builder, format_name, start, end);
                break;
            case TypeIndex::Decimal64:
                fillArrowArrayWithDecimalColumnData<DataTypeDecimal64, Int128, arrow20::Decimal128, arrow20::Decimal128Builder>(column, null_bytemap, array_builder, format_name, start, end);
                break;
            case TypeIndex::Decimal128:
                fillArrowArrayWithDecimalColumnData<DataTypeDecimal128, Int128, arrow20::Decimal128, arrow20::Decimal128Builder>(column, null_bytemap, array_builder, format_name, start, end);
                break;
            case TypeIndex::Decimal256:
                fillArrowArrayWithDecimalColumnData<DataTypeDecimal256, Int256, arrow20::Decimal256, arrow20::Decimal256Builder>(column, null_bytemap, array_builder, format_name, start, end);
                break;
            ...
            case TypeIndex::UInt8:
            {
                if (isBool(column_type))
                    ...
                    fillArrowArrayWithNumericColumnData<UInt8, arrow20::UInt8Builder>(column, null_bytemap, format_name, array_builder, start, end);
                break;
            }
            case TypeIndex::Enum8:
                fillArrowArrayWithNumericColumnData<Int8, arrow20::Int8Builder>(column, null_bytemap, format_name, array_builder, start, end);
                break;
            case TypeIndex::Enum16:
                fillArrowArrayWithNumericColumnData<Int16, arrow20::Int16Builder>(column, null_bytemap, format_name, array_builder, start, end);
                break;
...
#undef DISPATCH
    ...
    static std::shared_ptr<arrow20::DataType> getArrowTypeForLowCardinalityIndexes(ColumnPtr indexes_column, const CHColumnToArrowColumn::Settings & settings)
    {
        switch (indexes_column->getDataType())
        {
            ...
            case TypeIndex::UInt32:
                if (!settings.use_64_bit_indexes_for_dictionary)
                    return settings.use_signed_indexes_for_dictionary ? arrow20::int32() : arrow20::uint32();
                [[fallthrough]];
            case TypeIndex::UInt64:
                return settings.use_signed_indexes_for_dictionary ? arrow20::int64() : arrow20::uint64();
            default:
        ...
        DataTypePtr column_type, ColumnPtr column, const std::string & column_name, const std::string & format_name, const CHColumnToArrowColumn::Settings & settings, bool * out_is_column_nullable)
    {
        ...
        if (isTuple(column_type))
        {
            ...
            for (size_t i = 0; i != nested_types.size(); ++i)
            {
                ...
                nested_fields.push_back(std::make_shared<arrow20::Field>(nested_names[i], nested_arrow_type, *out_is_column_nullable));
            }
            return arrow20::struct_(nested_fields);
        }
        ...
        if (isFixedString(column_type) && settings.output_fixed_string_as_fixed_byte_array)
        {
            ...
            return arrow20::fixed_size_binary(static_cast<int32_t>(fixed_length));
        }
        ...
        if (isIPv6(column_type))
            return arrow20::fixed_size_binary(sizeof(IPv6));
    ...
    void CHColumnToArrowColumn::chChunkToArrowTable(
        ...
        size_t columns_num)
    {
        std::shared_ptr<arrow20::Schema> arrow_schema;
        std::vector<arrow20::ArrayVector> table_data(columns_num);
        ...
        for (const auto & chunk : chunks)
        {
            ...
            for (size_t column_i = 0; column_i < columns_num; ++column_i)
            {
                ...
                if (!is_arrow_fields_initialized)
                {
                    ...
                    auto arrow_type = getArrowType(
                        ...
                        settings,
                        &is_column_nullable);
                    arrow_fields.emplace_back(std::make_shared<arrow20::Field>(header_column.name, arrow_type, is_column_nullable));
                }
            ...
            if (!arrow_schema)
                arrow_schema = std::make_shared<arrow20::Schema>(arrow_fields);
        }

        std::vector<std::shared_ptr<arrow20::ChunkedArray>> columns;
        columns.reserve(columns_num);
        for (size_t column_i = 0; column_i < columns_num; ++column_i)
            columns.emplace_back(std::make_shared<arrow20::ChunkedArray>(table_data.at(column_i)));

        res = arrow20::Table::Make(arrow_schema, columns);
    }
