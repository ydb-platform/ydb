...
static const orc_chdb::Type * getORCTypeByName(const orc_chdb::Type & schema, const String & name, bool ignore_case)
{
...
static DataTypePtr parseORCType(const orc_chdb::Type * orc_type, bool skip_columns_with_unsupported_types, bool & skipped)
{
    ...
    {
        case orc_chdb::TypeKind::BOOLEAN:
            return DataTypeFactory::instance().get("Bool");
        case orc_chdb::TypeKind::BYTE:
            return std::make_shared<DataTypeInt8>();
        case orc_chdb::TypeKind::SHORT:
            return std::make_shared<DataTypeInt16>();
        case orc_chdb::TypeKind::INT:
            return std::make_shared<DataTypeInt32>();
        case orc_chdb::TypeKind::LONG:
            return std::make_shared<DataTypeInt64>();
        case orc_chdb::TypeKind::FLOAT:
            return std::make_shared<DataTypeFloat32>();
        case orc_chdb::TypeKind::DOUBLE:
            return std::make_shared<DataTypeFloat64>();
        case orc_chdb::TypeKind::DATE:
            return std::make_shared<DataTypeDate32>();
        case orc_chdb::TypeKind::TIMESTAMP:
            return std::make_shared<DataTypeDateTime64>(9);
        case orc_chdb::TypeKind::TIMESTAMP_INSTANT:
            return std::make_shared<DataTypeDateTime64>(9, "UTC");
        case orc_chdb::TypeKind::VARCHAR:
        case orc_chdb::TypeKind::BINARY:
        case orc_chdb::TypeKind::STRING:
            return std::make_shared<DataTypeString>();
        case orc_chdb::TypeKind::CHAR:
            return std::make_shared<DataTypeFixedString>(orc_type->getMaximumLength());
        case orc_chdb::TypeKind::DECIMAL: {
...
static std::optional<orc_chdb::PredicateDataType> convertORCTypeToPredicateType(const orc_chdb::Type & orc_type)
{
    switch (orc_type.getKind())
    {
        case orc_chdb::BOOLEAN:
            return orc_chdb::PredicateDataType::BOOLEAN;
        case orc_chdb::BYTE:
        case orc_chdb::SHORT:
        case orc_chdb::INT:
        case orc_chdb::LONG:
            return orc_chdb::PredicateDataType::LONG;
        case orc_chdb::FLOAT:
        case orc_chdb::DOUBLE:
            return orc_chdb::PredicateDataType::FLOAT;
        case orc_chdb::VARCHAR:
        case orc_chdb::CHAR:
        case orc_chdb::STRING:
            return orc_chdb::PredicateDataType::STRING;
        case orc_chdb::DATE:
            return orc_chdb::PredicateDataType::DATE;
        case orc_chdb::TIMESTAMP:
            return orc_chdb::PredicateDataType::TIMESTAMP;
        case orc_chdb::DECIMAL:
            return orc_chdb::PredicateDataType::DECIMAL;
        default:
            return {};
    }
}
...
static std::optional<orc_chdb::Literal>
convertFieldToORCLiteral(const orc_chdb::Type & orc_type, const Field & field, DataTypePtr type_hint = nullptr)
{
    try
    {
        ...
        switch (orc_type.getKind())
        {
            ...
            case orc_chdb::STRING: {
                String str;
                if (field.tryGet(str))
                    return orc_chdb::Literal(str.data(), str.size());
                break;
            }
            case orc_chdb::DATE: {
                Int64 val;
                if (field.tryGet(val))
                    return orc_chdb::Literal(orc_chdb::PredicateDataType::DATE, val);
                break;
            }
            case orc_chdb::TIMESTAMP: {
                ...
                if (field.tryGet(ts))
                {
                    ...
                    return orc_chdb::Literal(secs, nanos);
                }
                break;
            }
            case orc_chdb::DECIMAL: {
                ...
                if (precision <= DecimalUtils::max_precision<Decimal32>)
                {
                    ...
                    if (field.tryGet(val))
                    {
                        ...
                        return orc_chdb::Literal(
                            orc_chdb::Int128(right), static_cast<Int32>(orc_type.getPrecision()), static_cast<Int32>(orc_type.getScale()));
                    }
                }
                ...
                else if (precision <= DecimalUtils::max_precision<Decimal128>)
                {
                    ...
                    if (field.tryGet(val))
                    {
                        ...
                        return orc_chdb::Literal(
                            orc_chdb::Int128(high, low), static_cast<Int32>(orc_type.getPrecision()), static_cast<Int32>(orc_type.getScale()));
                    }
                }
                break;
            }
...
static void buildORCSearchArgumentImpl(
    ...
    orc_chdb::SearchArgumentBuilder & builder,
    const FormatSettings & format_settings)
{
    ...
    switch (curr.function)
    {
        ...
        case KeyCondition::RPNElement::FUNCTION_IS_NOT_NULL:
        {
            ...
            {
                builder.literal(orc_chdb::TruthValue::YES_NO_NULL);
                break;
            }
            ...
            else if (contains_in_range)
            {
                ...
                {
                    /// Transform whole range orc_chdb::TruthValue::YES_NULL
                    builder.literal(orc_chdb::TruthValue::YES_NULL);
                }
                ...
                {
                    std::optional<orc_chdb::Literal> left_literal;
                    if (has_left_bound)
                        left_literal = convertFieldToORCLiteral(*orc_type, range.left);

                    std::optional<orc_chdb::Literal> right_literal;
                    if (has_right_bound)
            ...
            else if (contains_in_set)
            {
                ...
                bool fail = false;
                std::vector<orc_chdb::Literal> literals;
                ...
                if (literals.empty())
                    builder.literal(orc_chdb::TruthValue::YES);
                else if (fail)
                    builder.literal(orc_chdb::TruthValue::YES_NO_NULL);
                else
        ...
        {
            builder.literal(orc_chdb::TruthValue::NO);
            rpn_stack.pop_back();
            break;
        }
...
std::unique_ptr<orc_chdb::SearchArgument>
buildORCSearchArgument(const KeyCondition & key_condition, const Block & header, const orc_chdb::Type & schema, const FormatSettings & format_settings)
{
    auto rpn_stack = key_condition.getRPN();
    if (rpn_stack.empty())
        return nullptr;

    auto builder = orc_chdb::SearchArgumentFactory::newBuilder();
    buildORCSearchArgumentImpl(key_condition, header, schema, rpn_stack, *builder, format_settings);
    return builder->build();
}

static void getFileReader(
    ReadBuffer & in, std::unique_ptr<orc_chdb::Reader> & file_reader, const FormatSettings & format_settings, std::atomic<int> & is_stopped)
{
    if (is_stopped)
        return;

    orc_chdb::ReaderOptions options;
    auto input_stream = asORCInputStream(in, format_settings, is_stopped);
    file_reader = orc_chdb::createReader(std::move(input_stream), options);
}

static const orc_chdb::Type * traverseDownORCTypeByName(
    const std::string & target,
    const orc_chdb::Type * orc_type,
    DataTypePtr & type,
    bool ignore_case)
{
    if (target.empty())
    ...
    auto search_struct_field = [&](const std::string & target_, const orc_chdb::Type * type_) -> std::pair<std::string, const orc_chdb::Type *>
    {
    ...
    if (orc_chdb::STRUCT == orc_type->getKind())
    {
    ...
    else if (orc_chdb::LIST == orc_type->getKind())
    {
        ...
        if (array_type && orc_chdb::STRUCT == orc_nested_type->getKind())
        {
...
static void updateIncludeTypeIds(
    DataTypePtr type, const orc_chdb::Type * orc_type, bool ignore_case, std::unordered_set<UInt64> & include_typeids)
{
...
bool NativeORCBlockInputFormat::prepareStripeReader()
{
    ...
    orc_chdb::RowReaderOptions row_reader_options;
...
NamesAndTypesList NativeORCSchemaReader::readSchema()
{
    ...
    for (size_t i = 0; i < schema.getSubtypeCount(); ++i)
    {
        ...
        const orc_chdb::Type * orc_type = schema.getSubtype(i);
...
void ORCColumnToCHColumn::orcTableToCHChunk(
    Chunk & res, const orc_chdb::Type * schema, const orc_chdb::ColumnVectorBatch * table, size_t num_rows, BlockMissingValues * block_missing_values)
{
    const auto * struct_batch = dynamic_cast<const orc_chdb::StructVectorBatch *>(table);
    if (!struct_batch)
...
static ColumnPtr readByteMapFromORCColumn(const orc_chdb::ColumnVectorBatch * orc_column)
{
...
static const orc_chdb::ColumnVectorBatch * getNestedORCColumn(const orc_chdb::ListVectorBatch * orc_column)
{
...
readColumnWithBooleanData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
    const auto * orc_bool_column = dynamic_cast<const orc_chdb::LongVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName
readColumnWithNumericData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
...
template <typename NumericType, typename BatchType, typename VectorType = ColumnVector<NumericType>>
static ColumnWithTypeAndName
readColumnWithNumericDataCast(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
    auto internal_type = std::make_shared<DataTypeNumber<NumericType>>();
    auto internal_column = internal_type->createColumn();
    auto & column_data = static_cast<VectorType &>(*internal_column).getData();
...
static ColumnWithTypeAndName
readColumnWithStringData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
    ...
    const auto * orc_str_column = dynamic_cast<const orc_chdb::StringVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName
readColumnWithFixedStringData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type * orc_type, const String & column_name)
{
...
static ColumnWithTypeAndName readColumnWithDecimalDataCast(
    const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name, DataTypePtr internal_type)
{
    using NativeType = typename DecimalType::NativeType;
    static_assert(std::is_same_v<BatchType, orc_chdb::Decimal128VectorBatch> || std::is_same_v<BatchType, orc_chdb::Decimal64VectorBatch>);
    ...
    for (size_t i = 0; i < orc_decimal_column->numElements; ++i)
    {
        ...
        if constexpr (std::is_same_v<BatchType, orc_chdb::Decimal128VectorBatch>)
        {
            Int128 int128_value;
...
static ColumnWithTypeAndName
readIPv6ColumnFromBinaryData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type * orc_type, const String & column_name)
{
    const auto * orc_str_column = dynamic_cast<const orc_chdb::StringVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName
readIPv4ColumnWithInt32Data(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
    const auto * orc_int_column = dynamic_cast<const orc_chdb::LongVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName readColumnWithBigNumberFromBinaryData(
    const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name, const DataTypePtr & column_type)
{
    const auto * orc_str_column = dynamic_cast<const orc_chdb::StringVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName readColumnWithDateData(
    const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name, const DataTypePtr & type_hint)
{
...
static ColumnWithTypeAndName
readColumnWithTimestampData(const orc_chdb::ColumnVectorBatch * orc_column, const orc_chdb::Type *, const String & column_name)
{
    const auto * orc_ts_column = dynamic_cast<const orc_chdb::TimestampVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName readColumnFromORCColumn(
    const orc_chdb::ColumnVectorBatch * orc_column,
    const orc_chdb::Type * orc_type,
    ...
    DataTypePtr type_hint = nullptr)
{
    ...
    if (!inside_nullable && (orc_column->hasNulls || (type_hint && type_hint->isNullable()))
        && (orc_type->getKind() != orc_chdb::LIST && orc_type->getKind() != orc_chdb::MAP && orc_type->getKind() != orc_chdb::STRUCT))
    {
    ...
    switch (orc_type->getKind())
    {
        ...
        case orc_chdb::BYTE:
            return readColumnWithNumericDataCast<Int8, orc_chdb::LongVectorBatch>(orc_column, orc_type, column_name);
        case orc_chdb::SHORT:
            return readColumnWithNumericDataCast<Int16, orc_chdb::LongVectorBatch>(orc_column, orc_type, column_name);
        case orc_chdb::INT: {
            ...
            return readColumnWithNumericDataCast<Int32, orc_chdb::LongVectorBatch>(orc_column, orc_type, column_name);
        }
        case orc_chdb::LONG:
            return readColumnWithNumericData<Int64, orc_chdb::LongVectorBatch>(orc_column, orc_type, column_name);
        case orc_chdb::FLOAT:
            return readColumnWithNumericDataCast<Float32, orc_chdb::DoubleVectorBatch>(orc_column, orc_type, column_name);
        case orc_chdb::DOUBLE:
            return readColumnWithNumericData<Float64, orc_chdb::DoubleVectorBatch>(orc_column, orc_type, column_name);
        case orc_chdb::DATE:
            return readColumnWithDateData(orc_column, orc_type, column_name, type_hint);
        case orc_chdb::TIMESTAMP: [[fallthrough]];
        case orc_chdb::TIMESTAMP_INSTANT:
        ...
        case orc_chdb::DECIMAL: {
            ...
            if (precision <= DecimalUtils::max_precision<Decimal32>)
                return readColumnWithDecimalDataCast<Decimal32, orc_chdb::Decimal64VectorBatch>(orc_column, orc_type, column_name, interal_type);
            else if (precision <= DecimalUtils::max_precision<Decimal64>)
                return readColumnWithDecimalDataCast<Decimal64, orc_chdb::Decimal64VectorBatch>(orc_column, orc_type, column_name, interal_type);
            else if (precision <= DecimalUtils::max_precision<Decimal128>)
                return readColumnWithDecimalDataCast<Decimal128, orc_chdb::Decimal128VectorBatch>(
        ...
        case orc_chdb::MAP: {
            ...
            const auto * orc_map_column = dynamic_cast<const orc_chdb::MapVectorBatch *>(orc_column);
        ...
        case orc_chdb::LIST: {
            ...
            const auto * orc_list_column = dynamic_cast<const orc_chdb::ListVectorBatch *>(orc_column);
        ...
        case orc_chdb::STRUCT: {
            ...
            const auto * orc_struct_column = dynamic_cast<const orc_chdb::StructVectorBatch *>(orc_column);
            for (size_t i = 0; i < orc_type->getSubtypeCount(); ++i)
            {
