...
class ORCInputStream : public orc_chdb::InputStream
{
public:
...
std::unique_ptr<orc_chdb::InputStream> asORCInputStream(ReadBuffer & in, const FormatSettings & settings, std::atomic<int> & is_cancelled);

// Reads the whole file into a memory buffer, owned by the returned RandomAccessFile.
std::unique_ptr<orc_chdb::InputStream> asORCInputStreamLoadIntoMemory(ReadBuffer & in, std::atomic<int> & is_cancelled);

std::unique_ptr<orc_chdb::SearchArgument> buildORCSearchArgument(
    const KeyCondition & key_condition, const Block & header, const orc_chdb::Type & schema, const FormatSettings & format_settings);
...
private:
    ...
    std::unique_ptr<orc_chdb::Reader> file_reader;
    std::unique_ptr<orc_chdb::RowReader> stripe_reader;
    std::unique_ptr<ORCColumnToCHColumn> orc_column_to_ch_column;

    std::shared_ptr<orc_chdb::SearchArgument> sarg;
    ...
    std::unique_ptr<orc_chdb::StripeInformation> current_stripe_info;
...
{
public:
    using ORCColumnPtr = const orc_chdb::ColumnVectorBatch *;
    using ORCTypePtr = const orc_chdb::Type *;
    ...
    void orcTableToCHChunk(
        Chunk & res,
        const orc_chdb::Type * schema,
        const orc_chdb::ColumnVectorBatch * table,
        size_t num_rows,
