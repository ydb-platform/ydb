...
Chunk ORCBlockInputFormat::read()
{
    ...
    if (need_only_count)
        throw Exception(500, "Not implemented");
    ...
    auto table_result = arrow20::Table::FromRecordBatches({batch});
    if (!table_result.ok())
        throw Exception(
    ...
    std::atomic<int> & is_stopped)
{
    ...
    auto result = arrow20::adapters::orc::ORCFileReader::Open(arrow_file, ArrowMemoryPool::instance());
    if (!result.ok())
