...
/// orc_chdb::Writer writes only in orc_chdb::OutputStream
class ORCOutputStream : public orc_chdb::OutputStream
{
public:
...
private:
    ...
    std::unique_ptr<orc_chdb::Type> getORCType(const DataTypePtr & type);
    ...
    void writeNumbers(orc_chdb::ColumnVectorBatch & orc_column, const IColumn & column, const PaddedPODArray<UInt8> * null_bytemap, ConvertFunc convert);
    ...
    void writeDecimals(orc_chdb::ColumnVectorBatch & orc_column, const IColumn & column, DataTypePtr & type,
                        const PaddedPODArray<UInt8> * null_bytemap, ConvertFunc convert);

    template <typename ColumnType>
    void writeStrings(orc_chdb::ColumnVectorBatch & orc_column, const IColumn & column, const PaddedPODArray<UInt8> * null_bytemap);
    ...
    void writeDateTimes(orc_chdb::ColumnVectorBatch & orc_column, const IColumn & column, const PaddedPODArray<UInt8> * null_bytemap,
                        GetSecondsFunc get_seconds, GetNanosecondsFunc get_nanoseconds);

    void writeColumn(orc_chdb::ColumnVectorBatch & orc_column, const IColumn & column, DataTypePtr & type, const PaddedPODArray<UInt8> * null_bytemap);
    ...
    std::unique_ptr<orc_chdb::Writer> writer;
    std::unique_ptr<orc_chdb::Type> schema;
    orc_chdb::WriterOptions options;
};
