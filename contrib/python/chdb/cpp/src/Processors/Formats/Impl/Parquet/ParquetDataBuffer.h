...
template <> struct ToArrowDecimal<Decimal<wide_CHDB::integer<128, signed>>>
{
    using ArrowDecimal = arrow20::Decimal128;
};

template <> struct ToArrowDecimal<Decimal<wide_CHDB::integer<256, signed>>>
{
    using ArrowDecimal = arrow20::Decimal256;
};
...
public:
    ...
    void ALWAYS_INLINE readDateTime64FromInt96(DateTime64 & dst)
    {
        ...
        parquet20::Int96 tmp;
        readValue(tmp);
        auto decoded = parquet20::DecodeInt96Timestamp(tmp);
    ...
    void ALWAYS_INLINE readString(ColumnString & column, size_t cursor)
    {
        ...
        checkAvaible(4);
        auto value_len = ::arrow20::util::SafeLoadAs<Int32>(getArrowData());
