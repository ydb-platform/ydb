...
ColumnPtr readDictPage(
    const parquet20::DictionaryPage & page,
    const parquet20::ColumnDescriptor & col_des,
    ...
    const DataTypePtr & data_type)
{
    ...
    if (col_des.physical_type() == parquet20::Type::INT64)
    {
...
std::unique_ptr<ParquetDataValuesReader> createPlainReader(
    ...
    ParquetDataBuffer buffer)
{
    if (std::is_same_v<TColumn, ColumnDecimal<DateTime64>> && col_des.physical_type() == parquet20::Type::INT96)
...
ParquetLeafColReader<TColumn>::ParquetLeafColReader(
    const parquet20::ColumnDescriptor & col_descriptor_,
    DataTypePtr base_type_,
    std::unique_ptr<parquet20::ColumnChunkMetaData> meta_,
    std::unique_ptr<parquet20::PageReader> reader_)
    : col_descriptor(col_descriptor_)
    , base_data_type(base_type_)
    , col_chunk_meta(std::move(meta_))
    , parquet_page_reader(std::move(reader_))
    , log(&CHDBPoco::Logger::get("ParquetLeafColReader"))
{
}
...
void ParquetLeafColReader<TColumn>::readPage()
{
    ...
    {
        case parquet20::PageType::DATA_PAGE:
            readPageV1(*std::static_pointer_cast<parquet20::DataPageV1>(cur_page));
            break;
        case parquet20::PageType::DATA_PAGE_V2:
            readPageV2(*std::static_pointer_cast<parquet20::DataPageV2>(cur_page));
            break;
        case parquet20::PageType::DICTIONARY_PAGE:
        {
            const parquet20::DictionaryPage & dict_page = *std::static_pointer_cast<parquet20::DictionaryPage>(cur_page);
            if (unlikely(
                dict_page.encoding() != parquet20::Encoding::PLAIN_DICTIONARY
                && dict_page.encoding() != parquet20::Encoding::PLAIN))
            {
                throw Exception(
...
template <typename TColumn>
void ParquetLeafColReader<TColumn>::readPageV1(const parquet20::DataPageV1 & page)
{
    static parquet20::LevelDecoder repetition_level_decoder;
    ...
    if (page.definition_level_encoding() != parquet20::Encoding::RLE && col_descriptor.max_definition_level() != 0)
    {
    ...
    {
        auto bit_width = arrow20::bit_util::Log2(col_descriptor.max_definition_level() + 1);
        auto num_bytes = ::arrow20::util::SafeLoadAs<int32_t>(buffer);
        auto bit_reader = std::make_unique<arrow20::bit_util::BitReader>(buffer + 4, num_bytes);
        num_bytes += 4;
        buffer += num_bytes;
        max_size -= num_bytes;
    ...
    switch (page.encoding())
    {
        ...
        case parquet20::Encoding::RLE_DICTIONARY:
        case parquet20::Encoding::PLAIN_DICTIONARY:
        {
            ...
            auto bit_reader = std::make_unique<arrow20::bit_util::BitReader>(++buffer, --max_size);
        ...
        case parquet20::Encoding::BYTE_STREAM_SPLIT:
        case parquet20::Encoding::DELTA_BINARY_PACKED:
        case parquet20::Encoding::DELTA_LENGTH_BYTE_ARRAY:
        case parquet20::Encoding::DELTA_BYTE_ARRAY:
...
template <typename TColumn>
void ParquetLeafColReader<TColumn>::readPageV2(const parquet20::DataPageV2 & /*page*/)
{
