...
#define THROW_PARQUET_EXCEPTION(s)                                            \
    ...
    {                                                                         \
        try { (s); }                                                          \
        catch (const ::parquet20::ParquetException & e)                         \
        {                                                                     \
...
std::unique_ptr<parquet20::ParquetFileReader> createFileReader(
    std::shared_ptr<::arrow20::io::RandomAccessFile> arrow_file,
    parquet20::ReaderProperties reader_properties,
    std::shared_ptr<parquet20::FileMetaData> metadata = nullptr)
{
    std::unique_ptr<parquet20::ParquetFileReader> res;
    THROW_PARQUET_EXCEPTION(res = parquet20::ParquetFileReader::Open(
            std::move(arrow_file),
            reader_properties,
            metadata));
    return res;
...
private:
    const parquet20::ArrowReaderProperties & arrow_properties;
    const parquet20::ColumnDescriptor & col_descriptor;
    DataTypePtr ch_type;
    std::unique_ptr<parquet20::ColumnChunkMetaData> meta;
    std::unique_ptr<parquet20::PageReader> page_reader;


    UInt32 getScaleFromLogicalTimestamp(parquet20::LogicalType::TimeUnit::unit tm_unit);
    UInt32 getScaleFromArrowTimeUnit(arrow20::TimeUnit::type tm_unit);
    ...
    std::unique_ptr<ParquetColumnReader> fromInt32INT(const parquet20::IntLogicalType & int_type);
    std::unique_ptr<ParquetColumnReader> fromInt64INT(const parquet20::IntLogicalType & int_type);
...
UInt32 ColReaderFactory::getScaleFromLogicalTimestamp(parquet20::LogicalType::TimeUnit::unit tm_unit)
{
    switch (tm_unit)
    {
        case parquet20::LogicalType::TimeUnit::MILLIS:
            return 3;
        case parquet20::LogicalType::TimeUnit::MICROS:
            return 6;
        case parquet20::LogicalType::TimeUnit::NANOS:
            return 9;
        default:
...
UInt32 ColReaderFactory::getScaleFromArrowTimeUnit(arrow20::TimeUnit::type tm_unit)
{
    switch (tm_unit)
    {
...
std::unique_ptr<ParquetColumnReader> ColReaderFactory::fromInt32()
{
    ...
    {
        case parquet20::LogicalType::Type::INT:
            return fromInt32INT(dynamic_cast<const parquet20::IntLogicalType &>(*col_descriptor.logical_type()));
        case parquet20::LogicalType::Type::NONE:
            return makeLeafReader<DataTypeInt32>();
        case parquet20::LogicalType::Type::DATE:
            return makeLeafReader<DataTypeDate32>();
        case parquet20::LogicalType::Type::DECIMAL:
...
std::unique_ptr<ParquetColumnReader> ColReaderFactory::fromInt64()
{
    ...
    {
        case parquet20::LogicalType::Type::INT:
            return fromInt64INT(dynamic_cast<const parquet20::IntLogicalType &>(*col_descriptor.logical_type()));
        case parquet20::LogicalType::Type::NONE:
            return makeLeafReader<DataTypeInt64>();
        case parquet20::LogicalType::Type::TIMESTAMP:
        {
            const auto & tm_type = dynamic_cast<const parquet20::TimestampLogicalType &>(*col_descriptor.logical_type());
...
std::unique_ptr<ParquetColumnReader> ColReaderFactory::fromByteArray()
{
    switch (col_descriptor.logical_type()->type())
    {
        case parquet20::LogicalType::Type::STRING:
        case parquet20::LogicalType::Type::NONE:
...
std::unique_ptr<ParquetColumnReader> ColReaderFactory::fromInt32INT(const parquet20::IntLogicalType & int_type)
{
...
std::unique_ptr<ParquetColumnReader> ColReaderFactory::fromInt64INT(const parquet20::IntLogicalType & int_type)
{
    switch (int_type.bit_width())
    {
        case 64:
...
ParquetRecordReader::ParquetRecordReader(
    ...
    std::shared_ptr<parquet20::FileMetaData> metadata)
...
{
    log = &CHDBPoco::Logger::get("ParquetRecordReader");

    std::unordered_map<String, parquet20::schema::NodePtr> parquet_columns;
...
Int64 ParquetRecordReader::getTotalRows(const parquet20::FileMetaData & meta_data)
{
    Int64 res = 0;
