...
struct StatisticsFixedStringRef
{
    ...
    void add(parquet20::FixedLenByteArray a)
    {
...
{
    parquet20::ByteArray min;
    parquet20::ByteArray max;

    void add(parquet20::ByteArray x)
    {
        addMin(x);
        addMax(x);
    }
    ...
    void addMin(parquet20::ByteArray x)
    {
        if (min.ptr == nullptr || compare(x, min) < 0)
            min = x;
    }

    void addMax(parquet20::ByteArray x)
    {
        if (max.ptr == nullptr || compare(x, max) > 0)
            max = x;
    }

    static int compare(parquet20::ByteArray a, parquet20::ByteArray b)
    {
...
struct ConverterString
{
    ...
    PODArray<parquet20::ByteArray> buf;

    explicit ConverterString(const ColumnPtr & c) : column(assert_cast<const ColumnString &>(*c)) {}

    const parquet20::ByteArray * getBatch(size_t offset, size_t count)
    {
        buf.resize(count);
        for (size_t i = 0; i < count; ++i)
        {
            StringRef s = column.getDataAt(offset + i);
            buf[i] = parquet20::ByteArray(static_cast<UInt32>(s.size), reinterpret_cast<const uint8_t *>(s.data));
        }
        return buf.data();
    }
};
...
struct ConverterFixedString
{
    ...
    PODArray<parquet20::FixedLenByteArray> buf;

    explicit ConverterFixedString(const ColumnPtr & c) : column(assert_cast<const ColumnFixedString &>(*c)) {}

    const parquet20::FixedLenByteArray * getBatch(size_t offset, size_t count)
    {
        buf.resize(count);
        for (size_t i = 0; i < count; ++i)
...
struct ConverterFixedStringAsString
{
    ...
    const parquet20::ByteArray * getBatch(size_t offset, size_t count)
    {
        buf.resize(count);
        for (size_t i = 0; i < count; ++i)
            buf[i] = parquet20::ByteArray(static_cast<UInt32>(column.getN()), reinterpret_cast<const uint8_t *>(column.getChars().data() + (offset + i) * column.getN()));
        return buf.data();
    }
};
...
struct ConverterDecimal
{
    ...
    PODArray<parquet20::FixedLenByteArray> ptr_buf;
    ...
    ColumnChunkWriteState & s, const WriteOptions & options, WriteBuffer & out, Converter && converter)
{
    ...
    std::optional<parquet20::ColumnDescriptor> fixed_string_descr;
    if constexpr (std::is_same_v<ParquetDType, parquet20::FLBAType>)
    {
        /// This just communicates one number to MakeTypedEncoder(): the fixed string length.
        fixed_string_descr.emplace(parquet20::schema::PrimitiveNode::Make(
            "", parquet20::Repetition::REQUIRED, parquet20::Type::FIXED_LEN_BYTE_ARRAY,
            parquet20::ConvertedType::NONE, static_cast<int>(converter.fixedStringSize())), 0, 0);
    ...
    auto encoder = parquet20::MakeTypedEncoder<ParquetDType>(
        // ignored if using dictionary
        static_cast<parquet20::Encoding::type>(encoding),
    ...
    auto flush_page = [&](size_t def_count, size_t data_count)
    {
        ...
        std::shared_ptr<parquet20::Buffer> values = encoder->FlushValues(); // resets it for next page
    ...
    {
        auto * dict_encoder = dynamic_cast<parquet20::DictEncoder<ParquetDType> *>(encoder.get());
...
#pragma clang loop vectorize(disable)
            ...
            if (use_dictionary && is_dict_too_big())
            {
                ...
                encoder = parquet20::MakeTypedEncoder<ParquetDType>(
                    static_cast<parquet20::Encoding::type>(encoding), /* use_dictionary */ false,
...
void writeColumnChunkBody(ColumnChunkWriteState & s, const WriteOptions & options, WriteBuffer & out)
{
    ...
    switch (s.primitive_column->getDataType())
    {
        ...
        #define N(source_type, parquet_dtype) \
            writeColumnImpl<parquet20::parquet_dtype>(s, options, out, \
                ConverterNumeric<ColumnVector<source_type>, parquet20::parquet_dtype::c_type>( \
                    s.primitive_column))

        case TypeIndex::UInt8:
            if (s.is_bool)
                writeColumnImpl<parquet20::BooleanType>(s, options, out,
        ...
        case TypeIndex::Float32:
            writeColumnImpl<parquet20::FloatType>(
                s, options, out, ConverterNumeric<ColumnVector<Float32>, Float32, Float32>(
                    s.primitive_column));
            break;

        case TypeIndex::Float64:
            writeColumnImpl<parquet20::DoubleType>(
        ...
        case TypeIndex::DateTime64:
            if (s.datetime64_multiplier == 1)
                writeColumnImpl<parquet20::Int64Type>(
