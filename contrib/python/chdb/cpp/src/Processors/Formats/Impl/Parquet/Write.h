...
struct WriteOptions
{
    ...
    parquet20::format::Encoding::type encoding = parquet20::format::Encoding::PLAIN;
...
struct ColumnChunkWriteState
{
    ...
    parquet20::format::ColumnChunk column_chunk;
...
using SchemaElements = std::vector<parquet20::format::SchemaElement>;
...
parquet20::format::ColumnChunk finalizeColumnChunkAndWriteFooter(
    size_t offset_in_file, ColumnChunkWriteState s, const WriteOptions & options, WriteBuffer & out);

parquet20::format::RowGroup makeRowGroup(std::vector<parquet20::format::ColumnChunk> column_chunks, size_t num_rows);

void writeFileFooter(std::vector<parquet20::format::RowGroup> row_groups, SchemaElements schema, const WriteOptions & options, WriteBuffer & out);
