...
///  * We parse the value from the encoded byte string instead of casting the parquet20::Statistics
///    to parquet20::TypedStatistics and taking the value from there.
...
static Field decodePlainParquetValueSlow(const std::string & data, parquet20::Type::type physical_type, const parquet20::ColumnDescriptor & descr, TypeIndex type_hint)
{
    using namespace parquet20;

    auto decode_integer = [&](bool signed_) -> UInt64 {
        size_t size;
        switch (physical_type)
        {
            case parquet20::Type::type::BOOLEAN: size = 1; break;
            case parquet20::Type::type::INT32: size = 4; break;
            case parquet20::Type::type::INT64: size = 8; break;
...
static std::vector<Range> getHyperrectangleForRowGroup(const parquet20::FileMetaData & file, int row_group_idx, const Block & header, const FormatSettings & format_settings)
{
    ...
    std::unique_ptr<parquet20::RowGroupMetaData> row_group = file.RowGroup(row_group_idx);

    std::unordered_map<std::string, std::shared_ptr<parquet20::Statistics>> name_to_statistics;
...
void ParquetBlockInputFormat::initializeIfNeeded()
{
    ...
    metadata = parquet20::ReadMetaData(arrow_file);

    std::shared_ptr<arrow20::Schema> schema;
    THROW_ARROW_NOT_OK(parquet20::arrow20::FromParquetSchema(metadata->schema(), &schema));
...
void ParquetBlockInputFormat::initializeRowGroupBatchReader(size_t row_group_batch_idx)
{
    ...
    parquet20::ReaderProperties reader_properties(ArrowMemoryPool::instance());
    ...
    //     (in arrow20::io::internal::ReadRangeCache) until the whole row group reading is done.
    ...
    auto cache_options = arrow20::io::CacheOptions::LazyDefaults();
    ...
    if (metadata->writer_version().VersionLt(parquet20::ApplicationVersion::PARQUET_816_FIXED_VERSION()))
...
void ParquetBlockInputFormat::decodeOneChunk(size_t row_group_batch_idx, std::unique_lock<std::mutex> & lock)
{
    ...
    else
    {
        ...
        auto tmp_table = arrow20::Table::FromRecordBatches({*batch});
