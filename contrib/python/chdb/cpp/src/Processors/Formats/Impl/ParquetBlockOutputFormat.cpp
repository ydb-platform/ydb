...
namespace
{
    ...
    parquet20::ParquetVersion::type getParquetVersion(const FormatSettings & settings)
    {
        switch (settings.parquet.output_version)
        {
            case FormatSettings::ParquetVersion::V1_0:
                return parquet20::ParquetVersion::PARQUET_1_0;
            case FormatSettings::ParquetVersion::V2_4:
                return parquet20::ParquetVersion::PARQUET_2_4;
            case FormatSettings::ParquetVersion::V2_6:
                return parquet20::ParquetVersion::PARQUET_2_6;
            case FormatSettings::ParquetVersion::V2_LATEST:
                return parquet20::ParquetVersion::PARQUET_2_LATEST;
        }
    }

    parquet20::Compression::type getParquetCompression(FormatSettings::ParquetCompression method)
    {
        if (method == FormatSettings::ParquetCompression::NONE)
            return parquet20::Compression::type::UNCOMPRESSED;

#if USE_SNAPPY
        if (method == FormatSettings::ParquetCompression::SNAPPY)
            return parquet20::Compression::type::SNAPPY;
#endif

#if USE_BROTLI
        if (method == FormatSettings::ParquetCompression::BROTLI)
            return parquet20::Compression::type::BROTLI;
#endif

        if (method == FormatSettings::ParquetCompression::ZSTD)
            return parquet20::Compression::type::ZSTD;

        if (method == FormatSettings::ParquetCompression::LZ4)
            return parquet20::Compression::type::LZ4;

        if (method == FormatSettings::ParquetCompression::GZIP)
            return parquet20::Compression::type::GZIP;
...
void ParquetBlockOutputFormat::writeUsingArrow(std::vector<Chunk> chunks)
{
    ...
    if (!file_writer)
    {
        ...
        parquet20::ArrowWriterProperties::Builder writer_props_builder;
...
void ParquetBlockOutputFormat::writeRowGroupInOneThread(Chunk chunk)
{
    ...
    std::vector<parquet20::format::ColumnChunk> column_chunks;
...
void ParquetBlockOutputFormat::reapCompletedRowGroups(std::unique_lock<std::mutex> & lock)
{
    while (!row_groups.empty() && row_groups.front().tasks_in_flight == 0 && !is_stopped)
    {
        ...
        std::vector<parquet20::format::ColumnChunk> metadata;
