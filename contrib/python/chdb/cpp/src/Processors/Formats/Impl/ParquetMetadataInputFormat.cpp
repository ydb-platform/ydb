...
Chunk ParquetMetadataInputFormat::read()
{
    ...
    for (const auto & name : header.getNames())
    {
        ...
        else if (name == names[3])
        {
            ...
            String version = metadata->version() == parquet20::ParquetVersion::PARQUET_1_0 ? "1.0" : "2.6";
...
void ParquetMetadataInputFormat::fillColumnsMetadata(const std::shared_ptr<parquet20::FileMetaData> & metadata, MutableColumnPtr & column)
{
...
void ParquetMetadataInputFormat::fillRowGroupsMetadata(const std::shared_ptr<parquet20::FileMetaData> & metadata, MutableColumnPtr & column)
{
...
void ParquetMetadataInputFormat::fillColumnChunksMetadata(const std::unique_ptr<parquet20::RowGroupMetaData> & row_group_metadata, IColumn & column)
{
    auto & array_column = assert_cast<ColumnArray &>(column);
    auto & tuple_column = assert_cast<ColumnTuple &>(array_column.getData());
...
template <typename T>
static void getMinMaxNumberStatistics(const std::shared_ptr<parquet20::Statistics> & statistics, String & min, String & max)
{
    const auto & typed_statistics = dynamic_cast<parquet20::TypedStatistics<T> &>(*statistics);
    min = std::to_string(typed_statistics.min());
    max = std::to_string(typed_statistics.max());
}

void ParquetMetadataInputFormat::fillColumnStatistics(const std::shared_ptr<parquet20::Statistics> & statistics, IColumn & column, int32_t type_length)
{
    ...
    /// min/max
    if (statistics->HasMinMax() && statistics->physical_type() != parquet20::Type::type::UNDEFINED)
    {
        String min;
        String max;
        switch (statistics->physical_type())
        {
            ...
            {
                getMinMaxNumberStatistics<parquet20::FloatType>(statistics, min, max);
                break;
            }
            case parquet20::Type::type::DOUBLE:
            {
                getMinMaxNumberStatistics<parquet20::DoubleType>(statistics, min, max);
                break;
            }
            case parquet20::Type::type::INT32:
            {
                getMinMaxNumberStatistics<parquet20::Int32Type>(statistics, min, max);
                break;
            }
            case parquet20::Type::type::INT64:
            {
                getMinMaxNumberStatistics<parquet20::Int64Type>(statistics, min, max);
                break;
            }
            case parquet20::Type::type::INT96:
            {
                const auto & int96_statistics = dynamic_cast<parquet20::TypedStatistics<parquet20::Int96Type> &>(*statistics);
                min = parquet20::Int96ToString(int96_statistics.min());
                max = parquet20::Int96ToString(int96_statistics.max());
                break;
            }
            case parquet20::Type::type::BOOLEAN:
            {
                getMinMaxNumberStatistics<parquet20::BooleanType>(statistics, min, max);
                break;
            }
            case parquet20::Type::type::BYTE_ARRAY:
            {
                const auto & byte_array_statistics = dynamic_cast<parquet20::ByteArrayStatistics &>(*statistics);
                min = parquet20::ByteArrayToString(byte_array_statistics.min());
                max = parquet20::ByteArrayToString(byte_array_statistics.max());
                break;
            }
            case parquet20::Type::type::FIXED_LEN_BYTE_ARRAY:
            {
                const auto & flba_statistics = dynamic_cast<parquet20::FLBAStatistics &>(*statistics);
                min = parquet20::FixedLenByteArrayToString(flba_statistics.min(), type_length);
                max = parquet20::FixedLenByteArrayToString(flba_statistics.max(), type_length);
                break;
            }
