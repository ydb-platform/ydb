...
namespace arrow20 { class Buffer; class RecordBatchReader;}
...
private:
    ...
    void fillColumnsMetadata(const std::shared_ptr<parquet20::FileMetaData> & metadata, MutableColumnPtr & column);
    void fillRowGroupsMetadata(const std::shared_ptr<parquet20::FileMetaData> & metadata, MutableColumnPtr & column);
    void fillColumnChunksMetadata(const std::unique_ptr<parquet20::RowGroupMetaData> & row_group_metadata, IColumn & column);
    void fillColumnStatistics(const std::shared_ptr<parquet20::Statistics> & statistics, IColumn & column, int32_t type_length);
