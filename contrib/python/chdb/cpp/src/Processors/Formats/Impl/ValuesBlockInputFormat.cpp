...
bool ValuesBlockInputFormat::parseExpression(IColumn & column, size_t column_idx)
{
    ...
    if (shouldDeduceNewTemplate(column_idx))
    {
        if (templates[column_idx])
            throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Template for column {} already exists and it was not evaluated yet",
...
void ValuesSchemaReader::transformTypesIfNeeded(DB_CHDB::DataTypePtr & type, DB_CHDB::DataTypePtr & new_type)
{
