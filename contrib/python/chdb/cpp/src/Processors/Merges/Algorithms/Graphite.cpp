...
const String & ruleTypeStr(RuleType rule_type)
{
    ...
    {
        throw Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "invalid rule type: {}", std::to_string(rule_type));
    }
}
...
RuleType ruleType(const String & s)
{
    if (s == "all")
        ...
        throw Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "invalid rule type: {}", s);
}
...
static bool compareRetentions(const Retention & a, const Retention & b)
{
    ...
    throw Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS, "Age and precision should only grow up: {}:{} vs {}:{}",
    ...
    ContextPtr context)
{
    ...
    for (const auto & key : keys)
    {
        ...
        else if (startsWith(key, "retention"))
        {
            ...
            throw Exception(DB_CHDB::ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Unknown element in config: {}", key);
    }
    ...
    if (!pattern.function && pattern.retentions.empty())
        throw Exception(DB_CHDB::ErrorCodes::NO_ELEMENTS_IN_CONFIG,
    ...
    {
        throw Exception(DB_CHDB::ErrorCodes::BAD_ARGUMENTS,
