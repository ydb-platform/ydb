...
const DB_CHDB::DataStream & getChildOutputStream(DB_CHDB::QueryPlan::Node & node)
{
    if (node.children.size() != 1)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR, "Node \"{}\" is expected to have only one child.", node.step->getName());
...
size_t tryExecuteFunctionsAfterSorting(QueryPlan::Node * parent_node, QueryPlan::Nodes & nodes)
{
    ...
    parent_step = std::make_unique<DB_CHDB::ExpressionStep>(child_step->getOutputStream(), std::move(unneeded_for_sorting));
