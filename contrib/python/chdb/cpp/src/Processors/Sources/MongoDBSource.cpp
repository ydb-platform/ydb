...
#error #include <CHDBPoco/MongoDB/Binary.h>
...
#error #include <CHDBPoco/MongoDB/OpMsgCursor.h>
#error #include <CHDBPoco/MongoDB/ObjectId.h>
...
namespace
{
    ...
    using ObjectId = CHDBPoco::MongoDB::ObjectId;
    using MongoArray = CHDBPoco::MongoDB::Array;
    using MongoUUID = CHDBPoco::MongoDB::Binary::Ptr;


    UUID parsePocoUUID(const CHDBPoco::UUID & src)
    {
        UUID uuid;

        std::array<CHDBPoco::UInt8, 6> src_node = src.getNode();
        UInt64 node = 0;
    ...
    template <typename T>
    Field getNumber(const CHDBPoco::MongoDB::Element & value, const std::string & name)
    {
        switch (value.type())
        {
            case CHDBPoco::MongoDB::ElementTraits<Int32>::TypeId:
                return static_cast<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<Int32> &>(value).value());
            case CHDBPoco::MongoDB::ElementTraits<CHDBPoco::Int64>::TypeId:
                return static_cast<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<CHDBPoco::Int64> &>(value).value());
            case CHDBPoco::MongoDB::ElementTraits<Float64>::TypeId:
                return static_cast<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<Float64> &>(value).value());
            case CHDBPoco::MongoDB::ElementTraits<bool>::TypeId:
                return static_cast<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<bool> &>(value).value());
            case CHDBPoco::MongoDB::ElementTraits<CHDBPoco::MongoDB::NullValue>::TypeId:
                return Field();
            case CHDBPoco::MongoDB::ElementTraits<String>::TypeId:
                return parse<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<String> &>(value).value());
            default:
        ...
        std::unordered_map<size_t, MongoDBArrayInfo> & array_info, size_t column_idx, const DataTypePtr data_type)
    {
        ...
        if (which.isUInt8())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<UInt8>(value, name); };
        else if (which.isUInt16())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<UInt16>(value, name); };
        else if (which.isUInt32())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<UInt32>(value, name); };
        else if (which.isUInt64())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<UInt64>(value, name); };
        else if (which.isInt8())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Int8>(value, name); };
        else if (which.isInt16())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Int16>(value, name); };
        else if (which.isInt32())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Int32>(value, name); };
        else if (which.isInt64())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Int64>(value, name); };
        else if (which.isFloat32())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Float32>(value, name); };
        else if (which.isFloat64())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field { return getNumber<Float64>(value, name); };
        else if (which.isString() || which.isFixedString())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field
            {
                if (value.type() == CHDBPoco::MongoDB::ElementTraits<ObjectId::Ptr>::TypeId)
                {
                    String string_id = value.toString();
                    return Field(string_id.data(), string_id.size());
                }
                else if (value.type() == CHDBPoco::MongoDB::ElementTraits<String>::TypeId)
                {
                    String string = static_cast<const CHDBPoco::MongoDB::ConcreteElement<String> &>(value).value();
            ...
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field
            {
                if (value.type() != CHDBPoco::MongoDB::ElementTraits<CHDBPoco::Timestamp>::TypeId)
                ...
                return static_cast<UInt16>(DateLUT::instance().toDayNum(
                    static_cast<const CHDBPoco::MongoDB::ConcreteElement<CHDBPoco::Timestamp> &>(value).value().epochTime()));
            };
            ...
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field
            {
                ...
                return static_cast<UInt32>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<CHDBPoco::Timestamp> &>(value).value().epochTime());
            };
        else if (which.isUUID())
            parser = [](const CHDBPoco::MongoDB::Element & value, const std::string & name) -> Field
            {
                if (value.type() == CHDBPoco::MongoDB::ElementTraits<String>::TypeId)
                {
                    String string = static_cast<const CHDBPoco::MongoDB::ConcreteElement<String> &>(value).value();
                    return parse<UUID>(string);
                }
                else if (value.type() == CHDBPoco::MongoDB::ElementTraits<MongoUUID>::TypeId)
                {
                    const CHDBPoco::UUID & poco_uuid = static_cast<const CHDBPoco::MongoDB::ConcreteElement<MongoUUID> &>(value).value()->uuid();
    ...
    template <typename T>
    void insertNumber(IColumn & column, const CHDBPoco::MongoDB::Element & value, const std::string & name)
    {
        switch (value.type())
        {
            case CHDBPoco::MongoDB::ElementTraits<Int32>::TypeId:
                assert_cast<ColumnVector<T> &>(column).getData().push_back(
                    static_cast<const CHDBPoco::MongoDB::ConcreteElement<Int32> &>(value).value());
                break;
            case CHDBPoco::MongoDB::ElementTraits<CHDBPoco::Int64>::TypeId:
                assert_cast<ColumnVector<T> &>(column).getData().push_back(
                    static_cast<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<CHDBPoco::Int64> &>(value).value()));
                break;
            case CHDBPoco::MongoDB::ElementTraits<Float64>::TypeId:
                assert_cast<ColumnVector<T> &>(column).getData().push_back(static_cast<T>(
                    static_cast<const CHDBPoco::MongoDB::ConcreteElement<Float64> &>(value).value()));
                break;
            case CHDBPoco::MongoDB::ElementTraits<bool>::TypeId:
                assert_cast<ColumnVector<T> &>(column).getData().push_back(
                    static_cast<const CHDBPoco::MongoDB::ConcreteElement<bool> &>(value).value());
                break;
            case CHDBPoco::MongoDB::ElementTraits<CHDBPoco::MongoDB::NullValue>::TypeId:
            ...
            case CHDBPoco::MongoDB::ElementTraits<String>::TypeId:
                assert_cast<ColumnVector<T> &>(column).getData().push_back(
                    parse<T>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<String> &>(value).value()));
                break;
            default:
                throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch, expected a number, got type id = {} for column {}",
                    toString(value.type()), name);
    ...
    void insertValue(
        ...
        const CHDBPoco::MongoDB::Element & value,
        ...
        switch (type)
        {
            ...
            case ValueType::vtDateTime:
            {
                ...
                assert_cast<ColumnUInt32 &>(column).getData().push_back(
                    static_cast<UInt32>(static_cast<const CHDBPoco::MongoDB::ConcreteElement<CHDBPoco::Timestamp> &>(value).value().epochTime()));
                break;
            }
            ...
            {
                if (value.type() != CHDBPoco::MongoDB::ElementTraits<MongoArray::Ptr>::TypeId)
                ...
                auto array = static_cast<const CHDBPoco::MongoDB::ConcreteElement<MongoArray::Ptr> &>(value).value();

                std::vector<std::pair<const CHDBPoco::MongoDB::Element *, size_t>> arrays;
                ...
                while (!arrays.empty())
                {
                    ...
                    auto parent = static_cast<const CHDBPoco::MongoDB::ConcreteElement<MongoArray::Ptr> &>(*parent_ptr).value();
                    ...
                    CHDBPoco::MongoDB::Element::Ptr child = parent->get(static_cast<int>(child_idx));
                    arrays.back().second += 1;

                    if (child->type() == CHDBPoco::MongoDB::ElementTraits<MongoArray::Ptr>::TypeId)
                    {
                        arrays.emplace_back(child.get(), 0);
                    }
                    else if (child->type() == CHDBPoco::MongoDB::ElementTraits<CHDBPoco::MongoDB::NullValue>::TypeId)
                    {
                        if (dimension_idx + 1 == expected_dimensions)
...
{
    CHDBPoco::MongoDB::Database db(database_name_);
    CHDBPoco::MongoDB::Document::Ptr doc = db.queryServerHello(connection_, false);

    if (doc->exists("maxWireVersion"))
    {
        auto wire_version = doc->getInteger("maxWireVersion");
        return wire_version < CHDBPoco::MongoDB::Database::WireVersion::VER_36;
    }
...
{
    CHDBPoco::MongoDB::Document projection;
    ...
    {
        old_cursor = std::make_unique<CHDBPoco::MongoDB::Cursor>(database, collection);
        old_cursor->query().selector() = query;
        old_cursor->query().returnFieldSelector() = projection;
    }
    else
    {
        new_cursor = std::make_unique<CHDBPoco::MongoDB::OpMsgCursor>(database, collection);
        new_cursor->query().setCommandName(CHDBPoco::MongoDB::OpMsgMessage::CMD_FIND);
        new_cursor->query().body().addNewDocument("filter") = query;
        new_cursor->query().body().addNewDocument("projection") = projection;
    }
}

CHDBPoco::MongoDB::Document::Vector MongoDBCursor::nextDocuments(CHDBPoco::MongoDB::Connection & connection)
{
    if (is_wire_protocol_old)
    {
    ...
    while (num_rows < max_block_size)
    {
        ...
        for (auto & document : documents)
        {
                ...
                && document->exists("code") && document->getInteger("ok") == 0)
            {
                ...
                const CHDBPoco::MongoDB::Element::Ptr value = document->get("$err");
                auto message = static_cast<const CHDBPoco::MongoDB::ConcreteElement<String> &>(*value).value();
            ...
            for (const auto idx : collections::range(0, size))
            {
                ...
                const CHDBPoco::MongoDB::Element::Ptr value = document->get(name);

                if (value.isNull() || value->type() == CHDBPoco::MongoDB::ElementTraits<CHDBPoco::MongoDB::NullValue>::TypeId)
                {
                    insertDefaultValue(*columns[idx], *description.sample_block.getByPosition(idx).column);
                }
                else
