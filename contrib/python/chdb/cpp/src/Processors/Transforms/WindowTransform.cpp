...
template <>
struct fmt::formatter<DB_CHDB::RowNumber>
{
    ...
    auto format(const DB_CHDB::RowNumber & x, FormatContext & ctx)
    {
    ...
    bool offset_is_preceding)
{
    ...
    if (offset_is_preceding)
        is_overflow = common_CHDB::subOverflow(reference_value, offset, reference_value);
    else
        is_overflow = common_CHDB::addOverflow(reference_value, offset, reference_value);
