...
namespace
{
    ...
    String loadAvailabilityZoneFromFile(const CHDBPoco::Util::AbstractConfiguration & config)
    {
        ...
        CHDBPoco::trimInPlace(availability_zone_from_file);
...
void PlacementInfo::initialize(const CHDBPoco::Util::AbstractConfiguration & config)
try
{
    if (!config.has(DB_CHDB::PlacementInfo::PLACEMENT_CONFIG_PREFIX))
    {
        availability_zone = "";
        initialized = true;
        return;
