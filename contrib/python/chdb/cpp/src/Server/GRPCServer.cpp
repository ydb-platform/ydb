...
namespace
{
    ...
    void initGRPCLogging(const CHDBPoco::Util::AbstractConfiguration & config)
    {
        ...
        std::call_once(once_flag, [&config]
        {
            ...
            else if (logger->is(CHDBPoco::Message::PRIO_DEBUG))
            {
                gpr_set_log_verbosity(GPR_LOG_SEVERITY_DEBUG);
            }
            else if (logger->is(CHDBPoco::Message::PRIO_INFORMATION))
            {
    ...
    {
        CHDBPoco::FileInputStream ifs(filepath);
        String res;
        CHDBPoco::StreamCopier::copyToString(ifs, res);
        return res;
    }

    /// Makes credentials based on the server config.
    std::shared_ptr<grpc::ServerCredentials> makeCredentials(const CHDBPoco::Util::AbstractConfiguration & config)
    {
...
#else
            throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::SUPPORT_IS_DISABLED, "Can't use SSL in grpc, because ClickHouse was built without SSL library");
#endif
        }
    ...
    struct TransportCompression
    {
        ...
        static TransportCompression fromConfiguration(const CHDBPoco::Util::AbstractConfiguration & config)
        {
            TransportCompression res;
    ...
    std::chrono::steady_clock::duration getSessionTimeout(const GRPCQueryInfo & query_info, const CHDBPoco::Util::AbstractConfiguration & config)
    {
    ...
    public:
        ...
        CHDBPoco::Net::SocketAddress getClientAddress() const
        {
            ...
            auto family = ipv4 ? CHDBPoco::Net::AddressFamily::Family::IPv4 : CHDBPoco::Net::AddressFamily::Family::IPv6;

            uri_encoded_peer= uri_encoded_peer.substr(prefix.length());

            String peer;
            CHDBPoco::URI::decode(uri_encoded_peer, peer);

            return CHDBPoco::Net::SocketAddress{family, peer};
        }
    ...
    void Call::run()
    {
        ...
        catch (CHDBPoco::Exception & exception)
        {
    ...
    void Call::executeQuery()
    {
        ...
        CHDBPoco::Net::SocketAddress user_address = responder->getClientAddress();
        ...
        if (client_logs_level != LogsLevel::none)
        {
            ...
            logs_queue->max_priority = CHDBPoco::Logger::parseLevel(client_logs_level.toString());
        ...
        io = ::DB_CHDB::executeQuery(query, query_context).second;
    }
    ...
    void Call::addLogsToResult()
    {
        ...
        static_assert(::clickhouse::grpc::LOG_FATAL       == static_cast<int>(CHDBPoco::Message::PRIO_FATAL));
        static_assert(::clickhouse::grpc::LOG_CRITICAL    == static_cast<int>(CHDBPoco::Message::PRIO_CRITICAL));
        static_assert(::clickhouse::grpc::LOG_ERROR       == static_cast<int>(CHDBPoco::Message::PRIO_ERROR));
        static_assert(::clickhouse::grpc::LOG_WARNING     == static_cast<int>(CHDBPoco::Message::PRIO_WARNING));
        static_assert(::clickhouse::grpc::LOG_NOTICE      == static_cast<int>(CHDBPoco::Message::PRIO_NOTICE));
        static_assert(::clickhouse::grpc::LOG_INFORMATION == static_cast<int>(CHDBPoco::Message::PRIO_INFORMATION));
        static_assert(::clickhouse::grpc::LOG_DEBUG       == static_cast<int>(CHDBPoco::Message::PRIO_DEBUG));
        static_assert(::clickhouse::grpc::LOG_TRACE       == static_cast<int>(CHDBPoco::Message::PRIO_TRACE));
...
GRPCServer::GRPCServer(IServer & iserver_, const CHDBPoco::Net::SocketAddress & address_to_listen_)
    : iserver(iserver_)
...
void GRPCServer::start()
{
    ...
    {
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::NETWORK_ERROR, "Can't start grpc server, there is a port conflict");
    }
