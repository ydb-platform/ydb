...
{
public:
    void handlePart(const CHDBPoco::Net::MessageHeader &, ReadBuffer &) override {}
};
...
HTMLForm::HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody, PartHandler & handler)
    : HTMLForm(settings)
{
...
HTMLForm::HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody) : HTMLForm(settings)
{
    load(request, requestBody);
}


HTMLForm::HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request) : HTMLForm(settings, CHDBPoco::URI(request.getURI()))
{
}

HTMLForm::HTMLForm(const Settings & settings, const CHDBPoco::URI & uri) : HTMLForm(settings)
{
...
void HTMLForm::load(const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody, PartHandler & handler)
{
    clear();

    CHDBPoco::URI uri(request.getURI());
    ...
    if (request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_POST || request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_PUT)
    {
        std::string media_type;
        NameValueCollection params;
        CHDBPoco::Net::MessageHeader::splitParameters(request.getContentType(), media_type, params);
        encoding = media_type;
...
void HTMLForm::load(const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody)
{
    NullPartHandler nah;
...
void HTMLForm::readQuery(ReadBuffer & in)
{
    ...
    while (true)
    {
        if (max_fields_number > 0 && fields == max_fields_number)
            throw CHDBPoco::Net::HTMLFormException("Too many form fields");
        ...
        while (in.read(ch) && ch != '=' && ch != '&')
        {
            ...
            if (name.size() < max_field_name_size)
                ...
                throw CHDBPoco::Net::HTMLFormException("Field name too long");
        }
        ...
        if (ch == '=')
        {
            while (in.read(ch) && ch != '&')
            {
                ...
                if (value.size() < max_field_value_size)
                    ...
                    throw CHDBPoco::Net::HTMLFormException("Field value too long");
            }
        }

        // Remove UTF-8 BOM from first name, if present
        if (is_first)
            CHDBPoco::UTF8::removeBOM(name);

        std::string decoded_name;
        std::string decoded_value;
        CHDBPoco::URI::decode(name, decoded_name);
...
void HTMLForm::readMultipart(ReadBuffer & in_, PartHandler & handler)
{
    ...
    if (!in.skipToNextBoundary())
        throw CHDBPoco::Net::HTMLFormException("No boundary line found");
    ...
    while (!in.eof())
    {
        ...
        CHDBPoco::Net::MessageHeader header;
        ...
        {
            std::string unused;
            CHDBPoco::Net::MessageHeader::splitParameters(header.get("Content-Disposition"), unused, params);
        }
