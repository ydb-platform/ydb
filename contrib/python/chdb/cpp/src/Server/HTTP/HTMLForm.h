...
class HTMLForm : public CHDBPoco::Net::NameValueCollection, private boost::noncopyable
{
public:
    class PartHandler;
    ...
    HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody, PartHandler & handler);

    /// Creates a HTMLForm from the given HTTP request.
    /// Uploaded files are silently discarded.
    HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody);
    ...
    explicit HTMLForm(const Settings & settings, const CHDBPoco::Net::HTTPRequest & request);

    explicit HTMLForm(const Settings & settings, const CHDBPoco::URI & uri);

    template <typename T>
    T getParsed(const std::string & key, T default_value)
    {
        auto it = find(key);
        return (it != end()) ? DB_CHDB::parse<T>(it->second) : default_value;
    }

    /// Reads the form data from the given HTTP request.
    /// Uploaded files are passed to the given PartHandler.
    void load(const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody, PartHandler & handler);

    /// Reads the form data from the given HTTP request.
    /// Uploaded files are silently discarded.
    void load(const CHDBPoco::Net::HTTPRequest & request, ReadBuffer & requestBody);
...
private:
    ...
    {
        std::string name;
        std::unique_ptr<CHDBPoco::Net::PartSource> source;
    };
...
public:
    ...
    virtual void handlePart(const CHDBPoco::Net::MessageHeader &, ReadBuffer &) = 0;
};
