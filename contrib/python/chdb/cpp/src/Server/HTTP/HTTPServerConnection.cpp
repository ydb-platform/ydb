...
{
    CHDB_poco_check_ptr(factory);
}

void HTTPServerConnection::run()
{
    std::string server = params->getSoftwareVersion();
    CHDBPoco::Net::HTTPServerSession session(socket(), params);
    ...
    while (!stopped && tcp_server.isOpen() && session.hasMoreRequests() && session.connected())
    {
        try
        {
            ...
            if (!stopped && tcp_server.isOpen() && session.connected())
            {
                ...
                try
                {
                    ...
                    {
                        sendErrorResponse(session, CHDBPoco::Net::HTTPResponse::HTTP_SERVICE_UNAVAILABLE);
                        break;
                    }
                    ...
                    if (handler)
                    {
                        if (request.getExpectContinue() && response.getStatus() == CHDBPoco::Net::HTTPResponse::HTTP_OK)
                        ...
                        sendErrorResponse(session, CHDBPoco::Net::HTTPResponse::HTTP_NOT_IMPLEMENTED);
                }
                catch (CHDBPoco::Exception &)
                {
                    if (!response.sent())
                    {
                        try
                        {
                            sendErrorResponse(session, CHDBPoco::Net::HTTPResponse::HTTP_INTERNAL_SERVER_ERROR);
                        }
        ...
        catch (const CHDBPoco::Net::NoMessageException &)
        {
            break;
        }
        catch (const CHDBPoco::Net::MessageException &)
        {
            sendErrorResponse(session, CHDBPoco::Net::HTTPResponse::HTTP_BAD_REQUEST);
        }
        catch (const CHDBPoco::Net::NetException & e)
        {
            /// Do not spam logs with messages related to connection reset by peer.
            if (e.code() == CHDB_POCO_ENOTCONN)
            {
        ...
        catch (const CHDBPoco::Exception &)
        {
            if (session.networkException())
            {
                session.networkException()->rethrow();
...
// static
void HTTPServerConnection::sendErrorResponse(CHDBPoco::Net::HTTPServerSession & session, CHDBPoco::Net::HTTPResponse::HTTPStatus status)
{
    HTTPServerResponse response(session);
    response.setVersion(CHDBPoco::Net::HTTPMessage::HTTP_1_1);
