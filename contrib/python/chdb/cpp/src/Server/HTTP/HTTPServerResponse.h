...
protected:
    void nextImpl() override
    {
        ...
        CHDBPoco::NumberFormatter::appendHex(chunk_header, offset());
        chunk_header.append("\r\n", 2);
...
{
public:
    explicit HTTPWriteBufferFixedLength(CHDBPoco::Net::Socket & socket_, size_t fixed_length_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, buf_size)
    {
        fixed_length = fixed_length_;
    }
    explicit HTTPWriteBufferFixedLength(CHDBPoco::Net::Socket & socket_, size_t fixed_length_, const ProfileEvents::Event & write_event_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
...
{
public:
    explicit HTTPWriteBuffer(CHDBPoco::Net::Socket & socket_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, buf_size)
    {
    }
    explicit HTTPWriteBuffer(CHDBPoco::Net::Socket & socket_, const ProfileEvents::Event & write_event_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE)
        : WriteBufferFromPocoSocket(socket_, write_event_, buf_size)
    {
    }
...
{
public:
    explicit HTTPServerResponse(CHDBPoco::Net::HTTPServerSession & session, const ProfileEvents::Event & write_event_ = ProfileEvents::end());
    ...
    CHDBPoco::Net::StreamSocket & getSocket() { return session.socket(); }

    void attachRequest(HTTPServerRequest * request_) { request = request_; }

    const CHDBPoco::Net::HTTPServerSession & getSession() const { return session; }

private:
    CHDBPoco::Net::HTTPServerSession & session;
