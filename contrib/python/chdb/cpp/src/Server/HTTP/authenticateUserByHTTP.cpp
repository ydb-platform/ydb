    ...
    LoggerPtr log)
{
    ...
    else if (has_http_credentials)
    {
        ...
        if (CHDBPoco::icompare(scheme, "Basic") == 0)
        {
            CHDBPoco::Net::HTTPBasicCredentials credentials(auth_info);
            user = credentials.getUsername();
            password = credentials.getPassword();
            checkUserNameNotEmpty(user, "Authorization HTTP header");
        }
        else if (CHDBPoco::icompare(scheme, "Negotiate") == 0)
        {
...
#pragma clang diagnostic pop
    ...
    try
    {
        if (!forwarded_address.empty() && global_context->getConfigRef().getBool("auth_use_forwarded_address", false))
            session.authenticate(*current_credentials, CHDBPoco::Net::SocketAddress(forwarded_address, request.clientAddress().port()));
        else
