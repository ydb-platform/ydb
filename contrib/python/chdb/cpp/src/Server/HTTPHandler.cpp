...
namespace
{
bool tryAddHTTPOptionHeadersFromConfig(HTTPServerResponse & response, const CHDBPoco::Util::LayeredConfiguration & config)
{
...
void processOptionsRequest(HTTPServerResponse & response, const CHDBPoco::Util::LayeredConfiguration & config)
{
    ...
    const ProfileEvents::Event & write_event)
{
    ...
    auto set_query_result = [&response, this] (const QueryResultDetails & details)
    {
        ...
        if (!(http_response_headers_override && http_response_headers_override->contains(CHDBPoco::Net::HTTPMessage::CONTENT_TYPE))
    ...
    const std::string & config_prefix)
{
    ...
    CHDBPoco::Util::AbstractConfiguration::Keys headers_name;
