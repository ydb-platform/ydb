...
public:
    ...
    void addFiltersFromConfig(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & prefix)
    {
        CHDBPoco::Util::AbstractConfiguration::Keys filters_type;
    ...
    void allowGetAndHeadRequest()
    {
        ...
        {
            return request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_GET
                || request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_HEAD;
        });
    }
    ...
    void allowPostAndGetParamsAndOptionsRequest()
    {
        addFilter([](const auto & request)
        {
            return (request.getURI().find('?') != std::string::npos
                && (request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_GET
                || request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_HEAD))
                || request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_OPTIONS
                || request.getMethod() == CHDBPoco::Net::HTTPRequest::HTTP_POST;
        });
    }
