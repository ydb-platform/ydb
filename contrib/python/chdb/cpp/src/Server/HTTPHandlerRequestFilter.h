...
static inline auto methodsFilter(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_path)
{
    std::vector<String> methods;
    CHDBPoco::StringTokenizer tokenizer(config.getString(config_path), ",");

    for (const auto & iterator : tokenizer)
        methods.emplace_back(CHDBPoco::toUpper(CHDBPoco::trim(iterator)));
...
static inline auto urlFilter(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_path)
{
...
static inline auto headersFilter(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & prefix)
{
