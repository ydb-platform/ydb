...
void KeeperReadinessHandler::handleRequest(HTTPServerRequest & /*request*/, HTTPServerResponse & response, const ProfileEvents::Event & /*write_event*/)
{
    try
    {
        ...
        CHDBPoco::JSON::Object json, details;
        ...
        CHDBPoco::JSON::Stringifier::stringify(json, oss);

        if (!status)
            response.setStatusAndReason(CHDBPoco::Net::HTTPResponse::HTTP_SERVICE_UNAVAILABLE);
