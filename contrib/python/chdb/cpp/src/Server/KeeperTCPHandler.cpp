...
struct SocketInterruptablePollWrapper
{
    ...
    explicit SocketInterruptablePollWrapper(const CHDBPoco::Net::StreamSocket & poco_socket_)
    ...
    PollResult poll(CHDBPoco::Timespan remaining_time, const ReadBufferFromPocoSocket & in)
    {
...
CHDBPoco::Timespan KeeperTCPHandler::receiveHandshake(int32_t handshake_length, bool & use_compression)
{
    ...
    return CHDBPoco::Timespan(timeout_ms * 1000);
}
...
void KeeperTCPHandler::runImpl()
{
    ...
    try
    {
        ...
        if (client_timeout.totalMilliseconds() == 0)
            client_timeout = CHDBPoco::Timespan(Coordination::DEFAULT_SESSION_TIMEOUT_MS * CHDBPoco::Timespan::MILLISECONDS);
        ...
        while (true)
        {
            ...
            if (result.has_requests && !close_received)
            {
                ...
                else if (received_op == Coordination::OpNum::Heartbeat)
                {
                    ...
                    operations[received_xid] = CHDBPoco::Timestamp();
...
void KeeperTCPHandler::updateStats(Coordination::ZooKeeperResponsePtr & response, const Coordination::ZooKeeperRequestPtr & request)
{
    ...
    {
        Int64 elapsed = (CHDBPoco::Timestamp() - operations[response->xid]);
        ...
        last_op.set(std::make_unique<LastOp>(LastOp{
            ...
            .last_response_time = CHDBPoco::Timestamp().epochMicroseconds() / 1000,
        }));
    }
