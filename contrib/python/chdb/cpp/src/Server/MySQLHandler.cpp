...
#if USE_SSL
using CHDBPoco::Net::SecureStreamSocket;
using CHDBPoco::Net::SSLManager;
#endif
...
void MySQLHandler::authenticate(const String & user_name, const String & auth_plugin_name, const String & initial_auth_response)
{
    try
    {
        ...
        if (session->getAuthenticationTypeOrLogInFailure(user_name) == DB_CHDB::AuthenticationType::SHA256_PASSWORD)
        {
            authPluginSSL();
        }
...
void MySQLHandler::comStmtPrepare(DB_CHDB::ReadBuffer & payload)
{
    String statement;
