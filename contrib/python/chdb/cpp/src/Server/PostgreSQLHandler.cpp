...
void PostgreSQLHandler::changeIO(CHDBPoco::Net::StreamSocket & socket)
{
...
void PostgreSQLHandler::run()
{
    ...
    catch (const CHDBPoco::Exception &exc)
    {
        log->log(exc);
    }
...
void PostgreSQLHandler::makeSecureConnectionSSL()
{
    ...
    ss = std::make_shared<CHDBPoco::Net::SecureStreamSocket>(
        CHDBPoco::Net::SecureStreamSocket::attach(socket(), CHDBPoco::Net::SSLManager::instance().defaultServerContext()));
    changeIO(*ss);
}
#else
void PostgreSQLHandler::makeSecureConnectionSSL() {}
...
bool PostgreSQLHandler::isEmptyQuery(const String & query)
{
    ...
    CHDBPoco::RegularExpression regex(R"(\A\s*\z)");
