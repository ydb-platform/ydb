...
void validateClientInfo(const ClientInfo & session_client_info, const ClientInfo & client_info)
{
    ...
    {
        throw Exception(
            DB_CHDB::ErrorCodes::CLIENT_INFO_DOES_NOT_MATCH,
...
TCPHandler::TCPHandler(
    ...
    : CHDBPoco::Net::TCPServerConnection(socket_)
    , server(server_)
...
void TCPHandler::runImpl()
{
    ...
    while (tcp_server.isOpen())
    {
         ...
         */
        std::unique_ptr<DB_CHDB::Exception> exception;
        ...
        try
        {
                ...
                && client_logs_level != LogsLevel::none)
            {
                ...
                state.logs_queue->max_priority = CHDBPoco::Logger::parseLevel(client_logs_level.toString());
        ...
        catch (const CHDBPoco::Net::NetException & e)
        {
             ...
             */
            state.io.onException();
            exception = std::make_unique<DB_CHDB::Exception>(Exception::CreateFromPocoTag{}, e);
        }
...
// tests.
        ...
        {
            state.io.onException();
            exception = std::make_unique<DB_CHDB::Exception>(Exception::CreateFromSTDTag{}, e);
        ...
        {
            state.io.onException();
            exception = std::make_unique<DB_CHDB::Exception>(Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Unknown exception"));
        }
...
std::string formatHTTPErrorResponseWhenUserIsConnectedToWrongPort(const CHDBPoco::Util::AbstractConfiguration& config)
{
...
void TCPHandler::receiveHello()
{
    ...
    if (dynamic_cast<CHDBPoco::Net::SecureStreamSocketImpl*>(socket().impl()))
    {
        CHDBPoco::Net::SecureStreamSocket secure_socket(socket());
        if (secure_socket.havePeerCertificate())
        {
            ...
            {
                if (e.code() != DB_CHDB::ErrorCodes::AUTHENTICATION_FAILED)
                    throw;
...
void TCPHandler::initBlockOutput(const Block & block)
{
    if (!state.block_out)
    {
        ...
        {
            std::string method = CHDBPoco::toUpper(query_settings.network_compression_method.toString());
...
void TCPHandler::run()
{
    ...
    catch (CHDBPoco::Exception & e)
    {
        ...
        {
            LOG_DEBUG(log, "CHDBPoco::Exception. Code: {}, e.code() = {}, e.displayText() = {}, e.what() = {}", ErrorCodes::POCO_EXCEPTION, e.code(), e.displayText(), e.what());
        }
        else
            throw;
    }
...
CHDBPoco::Net::SocketAddress TCPHandler::getClientAddress(const ClientInfo & client_info)
{
    ...
    if (!forwarded_address.empty() && server.config().getBool("auth_use_forwarded_address", false))
        return CHDBPoco::Net::SocketAddress(forwarded_address, socket().peerAddress().port());
    else
