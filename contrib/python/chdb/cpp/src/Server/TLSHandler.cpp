...
DB_CHDB::TLSHandler::TLSHandler(
    ...
    : CHDBPoco::Net::TCPServerConnection(socket)
#if USE_SSL
    , config(config_)
    , prefix(prefix_)
#endif
    , stack_data(stack_data_)
{
    ...
    if (!params.privateKeyFile.empty() && !params.certificateFile.empty())
    {
        ...
        if (config.hasProperty(prefix + SSLManager::CFG_VER_MODE))
        {
            ...
            params.verificationMode = CHDBPoco::Net::Utility::convertVerificationMode(mode);
        }
        ...
        CHDBPoco::StringTokenizer dp_tok(disabled_protocols_list, ";,", CHDBPoco::StringTokenizer::TOK_TRIM | CHDBPoco::StringTokenizer::TOK_IGNORE_EMPTY);
