...
void SchemaCache::addNumRows(const DB_CHDB::SchemaCache::Key & key, size_t num_rows)
{
    std::lock_guard lock(mutex);
...
std::optional<ColumnsDescription> SchemaCache::tryGetColumns(const DB_CHDB::SchemaCache::Key & key, DB_CHDB::SchemaCache::LastModificationTimeGetter get_last_mod_time)
{
...
std::optional<size_t> SchemaCache::tryGetNumRows(const DB_CHDB::SchemaCache::Key & key, DB_CHDB::SchemaCache::LastModificationTimeGetter get_last_mod_time)
{
    auto schema_info = tryGetImpl(key, get_last_mod_time);
    if (!schema_info)
        return std::nullopt;
