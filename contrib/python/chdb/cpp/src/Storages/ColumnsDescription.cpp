...
void ColumnDescription::writeText(WriteBuffer & buf) const
{
    ...
    {
        writeChar('\t', buf);
        DB_CHDB::writeText(DB_CHDB::toString(default_desc.kind), buf);
        writeChar('\t', buf);
    ...
    {
        writeChar('\t', buf);
        DB_CHDB::writeText("SETTINGS ", buf);
        DB_CHDB::writeText("(", buf);
        ASTSetQuery ast;
...
String ColumnsDescription::toString() const
{
    ...
    DB_CHDB::writeText(columns.size(), buf);
