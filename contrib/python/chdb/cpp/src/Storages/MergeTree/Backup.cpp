    ...
    const NameSet & files_to_copy_instead_of_hardlinks)
{
    ...
    if (level >= 1000)
        throw DB_CHDB::Exception(DB_CHDB::ErrorCodes::TOO_DEEP_RECURSION, "Too deep recursion");
    ...
    DiskTransactionPtr disk_transaction)
{
    ...
    {
        throw DB_CHDB::Exception(ErrorCodes::DIRECTORY_ALREADY_EXISTS, "Directory {} already exists and is not empty.",
                            DB_CHDB::fullPath(dst_disk, destination_path));
    }
    ...
    while (true)
    {
        ...
        catch (const DB_CHDB::ErrnoException & e)
        {
