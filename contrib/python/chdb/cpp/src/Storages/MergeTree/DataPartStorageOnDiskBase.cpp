...
String DataPartStorageOnDiskBase::getPartDirForPrefix(const String & prefix, bool detached, int try_no) const
{
    ...
    if (try_no)
        res += "_try" + DB_CHDB::toString(try_no);
...
CHDBPoco::Timestamp DataPartStorageOnDiskBase::getLastModified() const
{
    ...
    bool allow_backup_broken_projection) const
{
    ...
    auto * log = &CHDBPoco::Logger::get("DataPartStorageOnDiskBase::backup");
    ...
    bool fsync_part_dir)
{
    ...
    {
        disk.setLastModified(from, CHDBPoco::Timestamp::fromEpochTime(time(nullptr)));
