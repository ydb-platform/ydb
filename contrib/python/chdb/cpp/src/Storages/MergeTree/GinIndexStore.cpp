...
UInt32 GinIndexStore::getNextSegmentIDRange(const String & file_name, size_t n)
{
    ...
    {
        /// Create file
        std::unique_ptr<DB_CHDB::WriteBufferFromFileBase> ostr = this->data_part_storage_builder->writeFile(file_name, DBMS_DEFAULT_BUFFER_SIZE, {});
    ...
    UInt32 result = 0;
    {
        std::unique_ptr<DB_CHDB::ReadBufferFromFileBase> istr = this->storage->readFile(file_name, {}, std::nullopt, std::nullopt);
...
UInt32 GinIndexStore::getNumOfSegments()
{
    ...
    UInt32 result = 0;
    {
        std::unique_ptr<DB_CHDB::ReadBufferFromFileBase> istr = this->storage->readFile(segment_id_file_name, {}, std::nullopt, std::nullopt);
