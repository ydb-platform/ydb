...
inline void checkNoOldLeaders(LoggerPtr log, ZooKeeper & zookeeper, const String path)
{
    ...
    while (num_tries--)
    {
        ...
        else
        {
            ...
            if (!identifier.ends_with(suffix))
                throw CHDBPoco::Exception(fmt::format("Found leader replica ({}) with too old version (< 20.6). Stop it before upgrading", identifier));
    ...
    throw CHDBPoco::Exception("Cannot check that no old leaders exist");
}
