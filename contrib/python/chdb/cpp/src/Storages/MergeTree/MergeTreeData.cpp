...
template <>
struct fmt::formatter<DB_CHDB::DataPartPtr> : fmt::formatter<std::string>
{
    template <typename FormatCtx>
    auto format(const DB_CHDB::DataPartPtr & part, FormatCtx & ctx) const
    {
    ...
    for (size_t i = 0; i < old_and_new_names.size(); ++i)
    {
        try
        {
            ...
            if (old_name.empty() || new_name.empty())
                throw DB_CHDB::Exception(ErrorCodes::LOGICAL_ERROR, "Empty part name. Most likely it's a bug.");
...
void MergeTreeData::delayInsertOrThrowIfNeeded(CHDBPoco::Event * until, const ContextPtr & query_context, bool allow_throw) const
{
    const auto settings = getSettings();
...
void MergeTreeData::delayMutationOrThrowIfNeeded(CHDBPoco::Event * until, const ContextPtr & query_context) const
{
    const auto settings = getSettings();
    const auto & query_settings = query_context->getSettingsRef();
    ...
    const PartitionCommands & commands, const StorageMetadataPtr & /*metadata_snapshot*/, const Settings & settings, ContextPtr local_context) const
{
    ...
    for (const auto & command : commands)
    {
        if (command.type == PartitionCommand::DROP_DETACHED_PARTITION
            ...
            throw DB_CHDB::Exception(ErrorCodes::SUPPORT_IS_DISABLED,
            ...
            else
            {
                ...
                if (partition_ast && partition_ast->all)
                {
                    if (command.type != PartitionCommand::DROP_PARTITION && command.type != PartitionCommand::ATTACH_PARTITION)
                        throw DB_CHDB::Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Only support DROP/DETACH/ATTACH PARTITION ALL currently");
                }
                else
                {
...
void MergeTreeData::validateDetachedPartName(const String & name)
{
    if (name.find('/') != std::string::npos || name == "." || name == "..")
        throw DB_CHDB::Exception(ErrorCodes::INCORRECT_FILE_NAME, "Invalid part name '{}'", name);

    if (startsWith(name, "attaching_") || startsWith(name, "deleting_"))
        throw DB_CHDB::Exception(ErrorCodes::BAD_DATA_PART_NAME, "Cannot drop part {}: "
...
DiskPtr MergeTreeData::getDiskForDetachedPart(const String & part_name) const
{
    if (auto disk = tryGetDiskForDetachedPart(part_name))
        return disk;
    throw DB_CHDB::Exception(ErrorCodes::BAD_DATA_PART_NAME, "Detached part \"{}\" not found", part_name);
}
...
{
    return DB_CHDB::getConcreteObjectColumns(
...
void MergeTreeData::updateObjectColumns(const DataPartPtr & part, const DataPartsLock & /*lock*/)
{
    ...
    DB_CHDB::updateObjectColumns(object_columns, columns, part->getColumns());
}
        ...
        const MergeTreeTransactionPtr & txn)
{
    ...
    DB_CHDB::IMergeTreeDataPart::TTLInfos move_ttl_infos;
