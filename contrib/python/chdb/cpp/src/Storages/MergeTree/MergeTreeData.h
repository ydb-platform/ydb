...
public:
    ...
    void delayInsertOrThrowIfNeeded(CHDBPoco::Event * until, const ContextPtr & query_context, bool allow_throw) const;
    ...
    void delayMutationOrThrowIfNeeded(CHDBPoco::Event * until, const ContextPtr & query_context) const;
    ...
    {
        return DB_CHDB::extractKeyExpressionList(node);
    }
    ...
    class PartMutationBackoffPolicy
    {
        struct PartMutationInfo
        {
            ...
            explicit PartMutationInfo(size_t max_postpone_time_ms_)
                            : retry_count(0ull)
                            , latest_fail_time_us(static_cast<size_t>(CHDBPoco::Timestamp().epochMicroseconds()))
            ...
            size_t getNextMinExecutionTimeUsResolution() const
            {
                if (max_postpone_time_ms == 0)
                    return static_cast<size_t>(CHDBPoco::Timestamp().epochMicroseconds());
            ...
            bool partCanBeMutated() const
            {
                ...
                auto current_time_us = static_cast<size_t>(CHDBPoco::Timestamp().epochMicroseconds());
