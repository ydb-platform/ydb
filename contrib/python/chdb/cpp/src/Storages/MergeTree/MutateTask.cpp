...
static UInt64 getExistingRowsCount(const Block & block)
{
    ...
    if (!row_exists_col)
    {
        LOG_WARNING(&CHDBPoco::Logger::get("MutationHelpers::getExistingRowsCount"), "_row_exists column type is not UInt8");
        return block.rows();
    }
    ...
    bool sync)
{
...

    {
        ...
        DB_CHDB::writeText(queryToString(codec->getFullCodecDesc()), *out_comp);
...

    {
        ...
        DB_CHDB::writeText(metadata_snapshot->getMetadataVersion(), *out_metadata);
