...
{
    CHDBPoco::JSON::Object json;

    json.set(JSON_KEY_CREATE_TIME, DB_CHDB::toString(create_time));
    json.set(JSON_KEY_UPDATE_TIME, DB_CHDB::toString(update_time));
    json.set(JSON_KEY_TASK_UUID, DB_CHDB::toString(task_uuid));
    json.set(JSON_KEY_PART_NAME, part_name);
    json.set(JSON_KEY_PART_UUID, DB_CHDB::toString(part_uuid));
    json.set(JSON_KEY_TO_SHARD, to_shard);
    json.set(JSON_KEY_DST_PART_NAME, dst_part_name);
    json.set(JSON_KEY_STATE, state.toString());
    json.set(JSON_KEY_ROLLBACK, DB_CHDB::toString(rollback));
    json.set(JSON_KEY_LAST_EX_MSG, last_exception_msg);
    json.set(JSON_KEY_NUM_TRIES, DB_CHDB::toString(num_tries));
    ...
    CHDBPoco::JSON::Stringifier::stringify(json, oss, 0, -1, CHDBPoco::JSON_WRAP_STRINGS | CHDBPoco::JSON_ESCAPE_UNICODE);
...
void PartMovesBetweenShardsOrchestrator::Entry::fromString(const String & buf)
{
    ...
    auto json = parser.parse(buf).extract<CHDBPoco::JSON::Object::Ptr>();
