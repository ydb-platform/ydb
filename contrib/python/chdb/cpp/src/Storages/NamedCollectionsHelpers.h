...
MutableNamedCollectionPtr tryGetNamedCollectionWithOverrides(const CHDBPoco::Util::AbstractConfiguration & config, const std::string & config_prefix, ContextPtr context);
...
template <typename T>
struct fmt::formatter<DB_CHDB::NamedCollectionValidateKey<T>>
{
    ...
    auto format(const DB_CHDB::NamedCollectionValidateKey<T> & elem, FormatContext & context)
    {
