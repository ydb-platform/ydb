...
struct DeltaLakeMetadataImpl
{
        ...
        std::set<String> & result)
    {
        ...
        while (!buf->eof())
        {
            ...
            CHDBPoco::Dynamic::Var json = parser.parse(json_str);
            CHDBPoco::JSON::Object::Ptr object = json.extract<CHDBPoco::JSON::Object::Ptr>();
            ...
            {
                const auto metadata_object = object->get("metaData").extract<CHDBPoco::JSON::Object::Ptr>();
                const auto schema_object = metadata_object->getValue<String>("schemaString");

                CHDBPoco::JSON::Parser p;
                CHDBPoco::Dynamic::Var fields_json = parser.parse(schema_object);
                const CHDBPoco::JSON::Object::Ptr & fields_object = fields_json.extract<CHDBPoco::JSON::Object::Ptr>();
            ...
            if (object->has("add"))
            {
                auto add_object = object->get("add").extract<CHDBPoco::JSON::Object::Ptr>();
                ...
                if (it == file_partition_columns.end())
                {
                    ...
                    {
                        auto partition_values = add_object->get("partitionValues").extract<CHDBPoco::JSON::Object::Ptr>();
            ...
            {
                auto path = object->get("remove").extract<CHDBPoco::JSON::Object::Ptr>()->getValue<String>("path");
                result.erase(fs::path(configuration->getPath()) / path);
            }
        }
    }

    NamesAndTypesList parseMetadata(const CHDBPoco::JSON::Object::Ptr & metadata_json)
    {
        NamesAndTypesList schema;
        const auto fields = metadata_json->get("fields").extract<CHDBPoco::JSON::Array::Ptr>();
        for (size_t i = 0; i < fields->size(); ++i)
        {
            ...
            auto schema_metadata_object = field->get("metadata").extract<CHDBPoco::JSON::Object::Ptr>();
    ...
    DataTypePtr getFieldType(const CHDBPoco::JSON::Object::Ptr & field, const String & type_key, bool is_nullable)
    {
    ...
    DataTypePtr getComplexTypeFromObject(const CHDBPoco::JSON::Object::Ptr & type)
    {
        ...
        if (type_name == "struct")
        {
            ...
            Names element_names;
            auto fields = type->get("fields").extract<CHDBPoco::JSON::Array::Ptr>();
    ...
    #define THROW_ARROW_NOT_OK(status)                                    \
        do                                                                \
        {                                                                 \
            if (const ::arrow20::Status & _s = (status); !_s.ok())          \
        ...
        DataLakePartitionColumns & file_partition_columns)
    {
        ...
        for (size_t i = 0; i < path_column.size(); ++i)
        {
            ...
            {
                CHDBPoco::JSON::Parser parser;
                CHDBPoco::Dynamic::Var json = parser.parse(metadata);
                const CHDBPoco::JSON::Object::Ptr & object = json.extract<CHDBPoco::JSON::Object::Ptr>();
