...
void registerStorageJoin(StorageFactory & factory)
{
    auto creator_fn = [](const StorageFactory::Arguments & args)
    {
        ...
        {
            const String strictness_str = CHDBPoco::toLower(*opt_strictness_id);
        ...
        {
            const String kind_str = CHDBPoco::toLower(*opt_kind_id);
...
public:
        ...
        , sample_block(std::move(sample_block_))
    {
        if (!join->getTableJoin().oneDisjunct())
            throw DB_CHDB::Exception(ErrorCodes::NOT_IMPLEMENTED, "StorageJoin does not support OR for keys in JOIN ON section");
