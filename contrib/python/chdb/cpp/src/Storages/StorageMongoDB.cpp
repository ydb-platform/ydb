...
#error #include <CHDBPoco/MongoDB/Cursor.h>
#error #include <CHDBPoco/MongoDB/Database.h>
...
void StorageMongoDB::connectIfNotConnected()
{
    ...
    if (!connection)
    {
        ...
        connection = std::make_shared<CHDBPoco::MongoDB::Connection>(uri, factory);
    }
    ...
    if (!authenticated)
    {
        ...
        {
            CHDBPoco::MongoDB::Database poco_db(auth_db);
            if (!poco_db.authenticate(*connection, username, password, CHDBPoco::MongoDB::Database::AUTH_SCRAM_SHA1))
...
public:
    ...
    {
        CHDBPoco::MongoDB::Database db(db_name);
        CHDBPoco::MongoDB::Document::Vector documents;
        ...
        {
            CHDBPoco::MongoDB::Document::Ptr document = new CHDBPoco::MongoDB::Document();
        ...
        {
            CHDBPoco::SharedPtr<CHDBPoco::MongoDB::InsertRequest> insert_request = db.createInsertRequest(collection_name);
            insert_request->documents() = std::move(documents);
            connection->sendRequest(*insert_request);
        }
        else
        {
            CHDBPoco::SharedPtr<CHDBPoco::MongoDB::OpMsgMessage> insert_request = db.createOpMsgMessage(collection_name);
            insert_request->setCommandName(CHDBPoco::MongoDB::OpMsgMessage::CMD_INSERT);
            insert_request->documents() = std::move(documents);
            connection->sendRequest(*insert_request);
        }
    }
...
private:
    ...
    void insertValueIntoMongoDB(
        CHDBPoco::MongoDB::Document & document,
        ...
        size_t idx)
    {
        ...
        if (which.isArray())
        {
            ...
            CHDBPoco::MongoDB::Array::Ptr array = new CHDBPoco::MongoDB::Array();
            for (size_t i = 0; i + offset < next_offset; ++i)
            {
                insertValueIntoMongoDB(*array, CHDBPoco::NumberFormatter::format(i), *nested_type, nested_column, i + offset);
            }
        ...
        if (which.isNativeUInt())
            document.add(name, static_cast<CHDBPoco::Int64>(column.getUInt(idx)));
        else if (which.isNativeInt())
            document.add(name, static_cast<CHDBPoco::Int64>(column.getInt(idx)));
        else if (which.isFloat32())
            document.add(name, static_cast<Float64>(column.getFloat32(idx)));
        else if (which.isFloat64())
            document.add(name, column.getFloat64(idx));
        else if (which.isDate())
            document.add(name, CHDBPoco::Timestamp(DateLUT::instance().fromDayNum(DayNum(column.getUInt(idx))) * 1000000));
        else if (which.isDateTime())
            document.add(name, CHDBPoco::Timestamp(column.getUInt(idx) * 1000000));
        else
        {
    ...
    size_t /*num_streams*/)
{
    ...
    return Pipe(std::make_shared<MongoDBSource>(connection, database_name, collection_name, CHDBPoco::MongoDB::Document{}, sample_block, max_block_size));
}
