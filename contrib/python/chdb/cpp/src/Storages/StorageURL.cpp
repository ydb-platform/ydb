...
/// But CHDBPoco::AbstractConfiguration converts them into "header", "header[1]", "header[2]".
...
IStorageURLBase::IStorageURLBase(
    ...
    : IStorage(table_id_)
    , uri(uri_)
    , compression_method(chooseCompressionMethod(CHDBPoco::URI(uri_).getPath(), compression_method_))
    ...
    Impl(const String & uri_, size_t max_addresses, const ActionsDAG::Node * predicate, const NamesAndTypesList & virtual_columns, const ContextPtr & context)
    {
        ...
        if (filter_dag)
        {
            ...
            for (const auto & uri : uris)
                paths.push_back(CHDBPoco::URI(uri).getPath());
...
void StorageURLSource::setCredentials(CHDBPoco::Net::HTTPBasicCredentials & credentials, const CHDBPoco::URI & request_uri)
{
    ...
    , need_only_count(need_only_count_)
{
    ...
    initialize = [=, this]()
    {
        ...
        std::pair<CHDBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> uri_and_buf;
        do
        {
...
std::pair<CHDBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> StorageURLSource::getFirstAvailableURIAndReadBuffer(
    ...
    bool delay_initialization)
{
    ...
    std::pair<CHDBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> last_skipped_empty_res;
    for (; option != end; ++option)
    {
        ...
        auto request_uri = CHDBPoco::URI(*option, context_->getSettingsRef().enable_url_encoding);
    ...
    : SinkToStorage(sample_block)
{
    ...
    auto poco_uri = CHDBPoco::URI(uri);
...
public:
    ...
    SinkPtr createSinkForPartition(const String & partition_id) override
    {
        ...
        context->getRemoteHostFilter().checkURL(CHDBPoco::URI(partition_path));
...
{
    return CHDBPoco::Net::HTTPRequest::HTTP_GET;
}
        ...
        Data next() override
        {
            ...
            do
            {
                ...
                uri_and_buf = StorageURLSource::getFirstAvailableURIAndReadBuffer(
                    ...
                    getContext(),
                    {},
                    CHDBPoco::Net::HTTPRequest::HTTP_GET,
                    {},
...
SinkToStoragePtr IStorageURLBase::write(const ASTPtr & query, const StorageMetadataPtr & metadata_snapshot, ContextPtr context, bool /*async_insert*/)
{
    if (http_method.empty())
        http_method = CHDBPoco::Net::HTTPRequest::HTTP_POST;
...
{
    context_->getRemoteHostFilter().checkURL(CHDBPoco::URI(uri));
    ...
    : StorageURL("", table_id_, format_name_, format_settings_, columns_, constraints_, String{}, context_, compression_method_)
{
    ...
    {
        CHDBPoco::URI poco_uri(uri_option);
...
void StorageURL::processNamedCollectionResult(Configuration & configuration, const NamedCollection & collection)
{
    ...
    if (!configuration.http_method.empty() && configuration.http_method != CHDBPoco::Net::HTTPRequest::HTTP_POST
        && configuration.http_method != CHDBPoco::Net::HTTPRequest::HTTP_PUT)
        throw Exception(
            ErrorCodes::BAD_ARGUMENTS,
...
StorageURL::Configuration StorageURL::getConfiguration(ASTs & args, const ContextPtr & local_context)
{
    ...
    if (configuration.format == "auto")
        configuration.format = FormatFactory::instance().tryGetFormatFromFileName(CHDBPoco::URI(configuration.url).getPath()).value_or("auto");
