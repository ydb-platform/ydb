...
public:
    ...
    static std::optional<time_t> tryGetLastModificationTime(
        const String & url,
        const HTTPHeaderEntries & headers,
        const CHDBPoco::Net::HTTPBasicCredentials & credentials,
        const ContextPtr & context);
...
public:
    ...
    static void setCredentials(CHDBPoco::Net::HTTPBasicCredentials & credentials, const CHDBPoco::URI & request_uri);

    static std::pair<CHDBPoco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> getFirstAvailableURIAndReadBuffer(
...
public:
    StorageURLSink(
        ...
        CompressionMethod compression_method,
        const HTTPHeaderEntries & headers = {},
        const String & method = CHDBPoco::Net::HTTPRequest::HTTP_POST);
