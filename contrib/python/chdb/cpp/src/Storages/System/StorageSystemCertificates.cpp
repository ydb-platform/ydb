...
static void enumCertificates(const std::string & dir, bool def, MutableColumns & res_columns)
{
    ...
    for (auto const& dir_entry : std::filesystem::directory_iterator(p))
    {
        ...
        CHDBPoco::Crypto::X509Certificate cert(dir_entry.path());
...
{
#if USE_SSL
    const auto & ca_paths = CHDBPoco::Net::SSLManager::instance().defaultServerContext()->getCAPaths();

    if (!ca_paths.caLocation.empty())
    {
        CHDBPoco::File afile(ca_paths.caLocation);
        if (afile.exists())
        {
            ...
            {
                auto certs = CHDBPoco::Crypto::X509Certificate::readPEM(afile.path());
    ...
    {
        CHDBPoco::File afile(ca_paths.caDefaultFile);
        if (afile.exists())
        {
            auto certs = CHDBPoco::Crypto::X509Certificate::readPEM(ca_paths.caDefaultFile);
            for (const auto & cert : certs)
