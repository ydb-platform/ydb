...
namespace
{
    ...
    inline UInt32 now()
    {
        return static_cast<UInt32>(CHDBPoco::Timestamp().epochMicroseconds() / 1000000);
    }
    ...
    void extractWindowArgument(const ASTPtr & ast, IntervalKind::Kind & kind, Int64 & num_units, String err_msg)
    {
        ...
        || !IntervalKind::tryParseString(CHDBPoco::toLower(arg->name.substr(10)), kind))
...
static void extractDependentTable(ContextPtr context, ASTPtr & query, String & select_database_name, String & select_table_name)
{
    ...
    else if (auto * ast_select = subquery->as<ASTSelectWithUnionQuery>())
    {
        ...
        throw Exception(DB_CHDB::ErrorCodes::LOGICAL_ERROR,
...
void StorageWindowView::threadFuncCleanup()
{
    ...
    if ((CHDBPoco::Timestamp().epochMicroseconds() - last_clean_timestamp_usec) > clean_interval_usec)
    {
        try
        {
            cleanup();
        ...
        last_clean_timestamp_usec = CHDBPoco::Timestamp().epochMicroseconds();
    }
}
...
void StorageWindowView::threadFuncFireProc()
{
    ...
    UInt64 timestamp_ms = static_cast<UInt64>(CHDBPoco::Timestamp().epochMicroseconds()) / 1000;
    if (!shutdown_called)
