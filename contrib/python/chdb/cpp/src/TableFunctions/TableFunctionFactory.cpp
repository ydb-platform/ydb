    ...
    const std::string & name, Value value, Case case_sensitiveness)
{
    ...
    if (case_sensitiveness == Case::Insensitive
        && !case_insensitive_table_functions.emplace(CHDBPoco::toLower(name), value).second)
        ...
        ContextPtr) const
{
    ...
    {
        it = case_insensitive_table_functions.find(CHDBPoco::toLower(name));
...
std::optional<TableFunctionProperties> TableFunctionFactory::tryGetPropertiesImpl(const String & name_param) const
{
    ...
    if (auto jt = case_insensitive_table_functions.find(CHDBPoco::toLower(name)); jt != case_insensitive_table_functions.end())
        found = jt->second;
