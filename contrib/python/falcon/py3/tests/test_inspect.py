...
def make_app():
    ...
    app.add_static_route('/fal', yatest.common.test_source_path('../falcon'))
    app.add_static_route(
        '/tes', yatest.common.test_source_path('../tests'), fallback_filename='conftest.py'
    )
    return app
...
class TestInspectApp:
    ...
    def test_routes(self, asgi):
        ...
        self.check_route(
            asgi, routes[2], '/bar', 'OtherResponder', ['POST'], 'on_{}_id'
        )
    ...
    def test_error_handler(self, asgi):
        ...
        assert (
            errors[-1].name == 'my_error_handler_async' if asgi else 'my_error_handler'
        )
