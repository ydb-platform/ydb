...
class RecommendationSubscriptionServiceGrpcAsyncIOTransport(
    RecommendationSubscriptionServiceTransport
):
    """gRPC AsyncIO backend transport for RecommendationSubscriptionService.
    ...
    def mutate_recommendation_subscription(
        ...
        r"""Return a callable for the mutate recommendation
        ...
        Returns:
            Callable[[~.MutateRecommendationSubscriptionRequest],
                    Awaitable[~.MutateRecommendationSubscriptionResponse]]:
        ...
        if "mutate_recommendation_subscription" not in self._stubs:
            self._stubs["mutate_recommendation_subscription"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v20.services.RecommendationSubscriptionService/MutateRecommendationSubscription",
                    request_serializer=recommendation_subscription_service.MutateRecommendationSubscriptionRequest.serialize,
                    response_deserializer=recommendation_subscription_service.MutateRecommendationSubscriptionResponse.deserialize,
                )
            )
        return self._stubs["mutate_recommendation_subscription"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.mutate_recommendation_subscription: self._wrap_method(
                self.mutate_recommendation_subscription,
