...
from .base import TravelAssetSuggestionServiceTransport, DEFAULT_CLIENT_INFO
...
class TravelAssetSuggestionServiceGrpcAsyncIOTransport(
    ...
    """gRPC AsyncIO backend transport for TravelAssetSuggestionService.
    ...
    def suggest_travel_assets(
        ...
        r"""Return a callable for the suggest travel assets method over gRPC.

        Returns Travel Asset suggestions. Asset
        suggestions are returned on a best-effort basis. There
        are no guarantees that all possible asset types will be
        returned for any given hotel property.

        Returns:
            Callable[[~.SuggestTravelAssetsRequest],
                    Awaitable[~.SuggestTravelAssetsResponse]]:
        ...
        if "suggest_travel_assets" not in self._stubs:
            self._stubs["suggest_travel_assets"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v20.services.TravelAssetSuggestionService/SuggestTravelAssets",
                    request_serializer=travel_asset_suggestion_service.SuggestTravelAssetsRequest.serialize,
                    response_deserializer=travel_asset_suggestion_service.SuggestTravelAssetsResponse.deserialize,
                )
            )
        return self._stubs["suggest_travel_assets"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.suggest_travel_assets: self._wrap_method(
                self.suggest_travel_assets,
