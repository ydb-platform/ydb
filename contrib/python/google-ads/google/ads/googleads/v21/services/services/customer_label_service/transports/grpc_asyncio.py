...
from .base import CustomerLabelServiceTransport, DEFAULT_CLIENT_INFO
...
class CustomerLabelServiceGrpcAsyncIOTransport(CustomerLabelServiceTransport):
    """gRPC AsyncIO backend transport for CustomerLabelService.
    ...
    def mutate_customer_labels(
        ...
        r"""Return a callable for the mutate customer labels method over gRPC.

        Creates and removes customer-label relationships. Operation
        ...
        Returns:
            Callable[[~.MutateCustomerLabelsRequest],
                    Awaitable[~.MutateCustomerLabelsResponse]]:
        ...
        if "mutate_customer_labels" not in self._stubs:
            self._stubs["mutate_customer_labels"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v21.services.CustomerLabelService/MutateCustomerLabels",
                    request_serializer=customer_label_service.MutateCustomerLabelsRequest.serialize,
                    response_deserializer=customer_label_service.MutateCustomerLabelsResponse.deserialize,
                )
            )
        return self._stubs["mutate_customer_labels"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.mutate_customer_labels: self._wrap_method(
                self.mutate_customer_labels,
