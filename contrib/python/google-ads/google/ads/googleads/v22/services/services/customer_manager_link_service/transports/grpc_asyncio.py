    ...
    async def intercept_unary_unary(
        ...
        if logging_enabled:  # pragma: NO COVER
            ...
            _LOGGER.debug(
                ...
                extra={
                    "serviceName": "google.ads.googleads.v22.services.CustomerManagerLinkService",
...
class CustomerManagerLinkServiceGrpcAsyncIOTransport(
    CustomerManagerLinkServiceTransport
):
    """gRPC AsyncIO backend transport for CustomerManagerLinkService.
    ...
    def mutate_customer_manager_link(
        ...
        Returns:
            Callable[[~.MutateCustomerManagerLinkRequest],
                    Awaitable[~.MutateCustomerManagerLinkResponse]]:
        ...
        if "mutate_customer_manager_link" not in self._stubs:
            self._stubs["mutate_customer_manager_link"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v22.services.CustomerManagerLinkService/MutateCustomerManagerLink",
                    request_serializer=customer_manager_link_service.MutateCustomerManagerLinkRequest.serialize,
                    response_deserializer=customer_manager_link_service.MutateCustomerManagerLinkResponse.deserialize,
                )
            )
        return self._stubs["mutate_customer_manager_link"]
    ...
    def move_manager_link(
        ...
        Moves a client customer to a new manager customer. This
        simplifies the complex request that requires two operations to
        move a client customer to a new manager, for example:
        ...
        Returns:
            Callable[[~.MoveManagerLinkRequest],
                    Awaitable[~.MoveManagerLinkResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "move_manager_link" not in self._stubs:
            self._stubs["move_manager_link"] = self._logged_channel.unary_unary(
                "/google.ads.googleads.v22.services.CustomerManagerLinkService/MoveManagerLink",
                request_serializer=customer_manager_link_service.MoveManagerLinkRequest.serialize,
                response_deserializer=customer_manager_link_service.MoveManagerLinkResponse.deserialize,
            )
        return self._stubs["move_manager_link"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.mutate_customer_manager_link: self._wrap_method(
                self.mutate_customer_manager_link,
                default_timeout=None,
                client_info=client_info,
            ),
            self.move_manager_link: self._wrap_method(
                self.move_manager_link,
                default_timeout=None,
                client_info=client_info,
            ),
        }
...
__all__ = ("CustomerManagerLinkServiceGrpcAsyncIOTransport",)
