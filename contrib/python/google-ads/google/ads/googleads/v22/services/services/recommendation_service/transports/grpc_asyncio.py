...
class RecommendationServiceGrpcAsyncIOTransport(RecommendationServiceTransport):
    """gRPC AsyncIO backend transport for RecommendationService.
    ...
    def apply_recommendation(
        ...
        Returns:
            Callable[[~.ApplyRecommendationRequest],
                    Awaitable[~.ApplyRecommendationResponse]]:
        ...
        if "apply_recommendation" not in self._stubs:
            self._stubs["apply_recommendation"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v22.services.RecommendationService/ApplyRecommendation",
    ...
    def dismiss_recommendation(
        ...
        Returns:
            Callable[[~.DismissRecommendationRequest],
                    Awaitable[~.DismissRecommendationResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        if "dismiss_recommendation" not in self._stubs:
            self._stubs["dismiss_recommendation"] = (
                self._logged_channel.unary_unary(
    ...
    def generate_recommendations(
        ...
        r"""Return a callable for the generate recommendations method over gRPC.

        Generates Recommendations based off the requested
        ...
        Returns:
            Callable[[~.GenerateRecommendationsRequest],
                    Awaitable[~.GenerateRecommendationsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "generate_recommendations" not in self._stubs:
            self._stubs["generate_recommendations"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v22.services.RecommendationService/GenerateRecommendations",
                    request_serializer=recommendation_service.GenerateRecommendationsRequest.serialize,
                    response_deserializer=recommendation_service.GenerateRecommendationsResponse.deserialize,
                )
            )
        return self._stubs["generate_recommendations"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.apply_recommendation: self._wrap_method(
                self.apply_recommendation,
                default_timeout=None,
                client_info=client_info,
            ),
            self.dismiss_recommendation: self._wrap_method(
                self.dismiss_recommendation,
                default_timeout=None,
                client_info=client_info,
            ),
            self.generate_recommendations: self._wrap_method(
