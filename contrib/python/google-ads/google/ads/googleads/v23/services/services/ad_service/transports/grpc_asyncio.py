    ...
    async def intercept_unary_unary(
        ...
        if logging_enabled:  # pragma: NO COVER
            ...
            _LOGGER.debug(
                ...
                extra={
                    "serviceName": "google.ads.googleads.v23.services.AdService",
...
class AdServiceGrpcAsyncIOTransport(AdServiceTransport):
    """gRPC AsyncIO backend transport for AdService.
    ...
    def mutate_ads(
        ...
        [ad_service.MutateAdsRequest], Awaitable[ad_service.MutateAdsResponse]
    ]:
        r"""Return a callable for the mutate ads method over gRPC.

        Updates ads. Operation statuses are returned. Updating ads is
        ...
        Returns:
            Callable[[~.MutateAdsRequest],
                    Awaitable[~.MutateAdsResponse]]:
        ...
        if "mutate_ads" not in self._stubs:
            self._stubs["mutate_ads"] = self._logged_channel.unary_unary(
                "/google.ads.googleads.v23.services.AdService/MutateAds",
                request_serializer=ad_service.MutateAdsRequest.serialize,
                response_deserializer=ad_service.MutateAdsResponse.deserialize,
            )
        return self._stubs["mutate_ads"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.mutate_ads: self._wrap_method(
                self.mutate_ads,
