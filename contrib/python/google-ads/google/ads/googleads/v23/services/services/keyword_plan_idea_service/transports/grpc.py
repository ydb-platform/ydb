...
class KeywordPlanIdeaServiceGrpcTransport(KeywordPlanIdeaServiceTransport):
    ...
    def generate_keyword_ideas(
        ...
        if "generate_keyword_ideas" not in self._stubs:
            self._stubs["generate_keyword_ideas"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.KeywordPlanIdeaService/GenerateKeywordIdeas",
    ...
    def generate_keyword_historical_metrics(
        ...
        if "generate_keyword_historical_metrics" not in self._stubs:
            self._stubs["generate_keyword_historical_metrics"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.KeywordPlanIdeaService/GenerateKeywordHistoricalMetrics",
    ...
    def generate_ad_group_themes(
        ...
        r"""Return a callable for the generate ad group themes method over gRPC.
        ...
        Returns:
            Callable[[~.GenerateAdGroupThemesRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "generate_ad_group_themes" not in self._stubs:
            self._stubs["generate_ad_group_themes"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.KeywordPlanIdeaService/GenerateAdGroupThemes",
                    request_serializer=keyword_plan_idea_service.GenerateAdGroupThemesRequest.serialize,
                    response_deserializer=keyword_plan_idea_service.GenerateAdGroupThemesResponse.deserialize,
                )
            )
        return self._stubs["generate_ad_group_themes"]
    ...
    def generate_keyword_forecast_metrics(
        ...
        r"""Return a callable for the generate keyword forecast
        metrics method over gRPC.

        Returns metrics (such as impressions, clicks, total cost) of a
        ...
        Returns:
            Callable[[~.GenerateKeywordForecastMetricsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "generate_keyword_forecast_metrics" not in self._stubs:
            self._stubs["generate_keyword_forecast_metrics"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.KeywordPlanIdeaService/GenerateKeywordForecastMetrics",
                    request_serializer=keyword_plan_idea_service.GenerateKeywordForecastMetricsRequest.serialize,
                    response_deserializer=keyword_plan_idea_service.GenerateKeywordForecastMetricsResponse.deserialize,
                )
            )
        return self._stubs["generate_keyword_forecast_metrics"]
