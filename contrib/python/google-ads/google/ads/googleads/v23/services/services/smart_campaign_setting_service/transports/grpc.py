...
from .base import SmartCampaignSettingServiceTransport, DEFAULT_CLIENT_INFO
...
class SmartCampaignSettingServiceGrpcTransport(
    SmartCampaignSettingServiceTransport
):
    """gRPC backend transport for SmartCampaignSettingService.
    ...
    def get_smart_campaign_status(
        ...
        r"""Return a callable for the get smart campaign status method over gRPC.

        Returns the status of the requested Smart campaign.

        Returns:
            Callable[[~.GetSmartCampaignStatusRequest],
        ...
        if "get_smart_campaign_status" not in self._stubs:
            self._stubs["get_smart_campaign_status"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.SmartCampaignSettingService/GetSmartCampaignStatus",
                    request_serializer=smart_campaign_setting_service.GetSmartCampaignStatusRequest.serialize,
                    response_deserializer=smart_campaign_setting_service.GetSmartCampaignStatusResponse.deserialize,
                )
            )
        return self._stubs["get_smart_campaign_status"]
    ...
    def mutate_smart_campaign_settings(
        ...
        r"""Return a callable for the mutate smart campaign settings method over gRPC.

        Updates Smart campaign settings for campaigns.

        Returns:
            Callable[[~.MutateSmartCampaignSettingsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "mutate_smart_campaign_settings" not in self._stubs:
            self._stubs["mutate_smart_campaign_settings"] = (
                self._logged_channel.unary_unary(
                    "/google.ads.googleads.v23.services.SmartCampaignSettingService/MutateSmartCampaignSettings",
                    request_serializer=smart_campaign_setting_service.MutateSmartCampaignSettingsRequest.serialize,
                    response_deserializer=smart_campaign_setting_service.MutateSmartCampaignSettingsResponse.deserialize,
                )
            )
...
__all__ = ("SmartCampaignSettingServiceGrpcTransport",)
