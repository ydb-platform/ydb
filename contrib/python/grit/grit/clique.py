...
class UberClique(object):
  ...
  def __init__(self):
    ...
    # A source language for all the cliques in the uberclique.
    # GRD files may not have 'source_lang_id' attribute. So cliques have
    # no chance to get explicit source language. Assume that default source
    # language is English.
    self.source_language_ = 'en'
  ...
  def SourceLanguage(self):
    return self.source_language_

  def SetSourceLanguage(self, source_language):
    self.source_language_ = source_language
...
class OneOffCustomType(CustomType):
  ...
  '''
  def __init__(self, expression, source_language):
    self.expr = expression
    self.source_language = source_language
  def Validate(self, message):
    return self.ValidateAndModify(self.source_language, message)
...
class MessageClique(object):
  ...
  def __init__(self, uber_clique, message, translateable=True,
    ...
    self.clique = { self.uber_clique.SourceLanguage() : message }
  ...
  def GetMessage(self):
    ...
    return self.clique[self.uber_clique.SourceLanguage()]
  ...
  def AddTranslation(self, translation, language):
    ...
    assert not language in self.clique, \
      "{0} is already in clique {1}!".format(language, translation.GetId())
    ...
    original = self.MessageForLanguage(
      self.uber_clique.SourceLanguage(), False)
