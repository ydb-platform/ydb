...
from grit.gather import chrome_scaled_image_exceptions
...
def _GetImageSizeFromIHDRChunk(chunk):
  return (struct.unpack('>I', chunk[8:12])[0],
          struct.unpack('>I', chunk[12:16])[0])


def _GetPngImageSize(png_file_name):
  data = util.ReadFile(png_file_name, util.BINARY)
  for chunk in _ChunkifyPNG(data):
    type = chunk[4:8]
    if type == _FIRST_CHUNK:
      return _GetImageSizeFromIHDRChunk(chunk)
  return (0,0)
...
class ChromeScaledImage(interface.GathererBase):
  ...
  has_valid_resolution_ = False

  def _CheckResolution(self, fname_100_percent, fname_target, scale):
    ''' Check has scaled image valid resolution '''
    if (os.path.abspath(fname_100_percent) ==
            os.path.abspath(fname_target)):
      return scale == 100
    if not os.path.exists(fname_100_percent):
      print('Error: 100% scale image not found for {}.'.format(self.extkey))
      return False
    original_size = _GetPngImageSize(fname_100_percent)
    target_size = _GetPngImageSize(fname_target)

    if (original_size[0] * scale != target_size[0] * 100 or
        original_size[1] * scale != target_size[1] * 100):

      exceptions = [
        # FIXME(mblsha/andrave): BROWSER-29996
        'IDR_NEWTAB_BUTTON_MASK',
        'IDR_NEWTAB_BUTTON_H',
        'IDR_OMNIBOX_ARROW_L',
        'IDR_OMNIBOX_ARROW_R',
        'IDR_OMNIBOX_ARROW_YANDEX_L',
        'IDR_OMNIBOX_ARROW_YANDEX_R',
        'IDR_OMNIBOX_TURBO_ARROW_BLUE',
        'IDR_OMNIBOX_TURBO_ARROW_GRAY',
        'IDR_THEME_TOOLBAR',

        # TODO(gekalas): This one needs ST issue to be fixed
        'IDR_EXTENSION_ACTION_PLAIN_BACKGROUND',
        'IDR_AUTOFILL_TOOLTIP_ICON',
        'IDR_AUTOFILL_TOOLTIP_ICON_H',
        'IDR_AUTOFILL_TOOLTIP_ICON_P',
        'IDR_AUTOFILL_PR_DISCOVER',
        'IDR_AUTOFILL_PR_GENERIC',
        'IDR_AUTOFILL_PR_AMEX',
        'IDR_AUTOFILL_PR_DINERS',
        'IDR_AUTOFILL_PR_JCB',
        'IDR_AUTOFILL_PR_MASTERCARD',
        'IDR_AUTOFILL_PR_MIR',
        'IDR_AUTOFILL_PR_UNIONPAY',
        'IDR_AUTOFILL_PR_VISA',
        'IDR_PHYSICAL_WEB_LOGO_WITH_PADDING',
      ]

      message = ('{0}% image {1} actual size is {2}x{3}'
                 ', but should be {4}x{5} '
                 '(100% size is {6}x{7}) (file name: {8})'
                ).format(scale, self.extkey,
                         target_size[0], target_size[1],
                         original_size[0] * scale / 100,
                         original_size[1] * scale / 100,
                         original_size[0], original_size[1],
                         fname_target)

      if self.extkey in exceptions:
        # print >> sys.stderr, ('Warning: ' + message)
        return False

      raise exception.InvalidHiDPIImageSize('Error: ' + message)
    return True

  def _GetTargetImageFileRealPath(self, scale, layout):
    dir = '%s_%s_percent' % (layout, scale)
    path = os.path.join(dir, self.rc_file)
    return self.grd_node.ToRealPath(path)

  def _CheckLayoutConsistent(self, layout):
    for scale in [100, 200]:
      path = self._GetTargetImageFileRealPath(scale, layout)
      if not os.path.exists(path):
        message = ('{0}% scale image not found for {1} in layout {2} '
                   '(expected file name: {3})'
                   ).format(scale, self.extkey, layout, path)
        if (scale == 200 and
            self.extkey in chrome_scaled_image_exceptions._NO_SCALE_200):
          # print >> sys.stderr, ('Warning: ' + message)
          pass
        else:
          raise exception.InvalidHiDPIImageSize('Error: ' + message)
  ...
  def _FindInputFile(self):
    ...
    for layout in layouts:
      if not try_low_res:
        self._CheckLayoutConsistent(layout)
      file_100_percent = os.path.join('%s_100_percent' % (layout),
                                      self.rc_file)
      for scale in scales:
        dir = '%s_%s_percent' % (layout, scale)
        path = os.path.join(dir, self.rc_file)
        if os.path.exists(self.grd_node.ToRealPath(path)):
          self.has_valid_resolution_ = self._CheckResolution(
                       self.grd_node.ToRealPath(file_100_percent),
                       self.grd_node.ToRealPath(path),
                       scale)
  ...
  def HasValidResolution(self):
    return self.has_valid_resolution_
