...
class GrdContentHandler(xml.sax.handler.ContentHandler):
  def __init__(self, stop_after, debug, dir, defines, tags_to_ignore,
               target_platform, source, grd_filename, validate_unique_ids=True):
    # Invariant of data:
    ...
    self.grd_filename = grd_filename
    self.validate_unique_ids = validate_unique_ids
    self.source = source
  ...
  def startElement(self, name, attrs):
    ...
    if isinstance(node, misc.GritNode) and \
            not hasattr(self.grd_filename, 'read'):
      node.SetGrdFilename(util.normpath(os.path.join(os.getcwd(),
          self.grd_filename)))
    if isinstance(node, misc.GritNode):
      node.SetValidateUniqueIds(self.validate_unique_ids)
...
def Parse(filename_or_stream, dir=None, stop_after=None, first_ids_file=None,
          ...
          predetermined_ids_file=None, validate_unique_ids=True):
  ...
  handler = GrdContentHandler(stop_after=stop_after, debug=debug, dir=dir,
                              ...
                              grd_filename=filename_or_stream,
  ...
  if isinstance(handler.root, misc.GritNode):
    ...
    if first_ids_file:
      ...
      REL_PATH_SUFFIX = ':REL'  # path is specified relatively to the cwd
      if not (first_ids_file.startswith(GRIT_DIR_PREFIX)
          and first_ids_file[len(GRIT_DIR_PREFIX)] in ['/', '\\']
          or first_ids_file.endswith(REL_PATH_SUFFIX)):
