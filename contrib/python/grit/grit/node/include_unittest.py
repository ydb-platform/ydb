...
def checkIsGzipped(filename, compress_attr):
  test_data_root = yatest.common.source_path('contrib/python/grit/grit/testdata')
...
class IncludeNodeUnittest(unittest.TestCase):
  ...
  def testSkipInResourceMap(self):
    root = util.ParseGrdForUnittest('''
        ...
        </includes>''', base_dir = yatest.common.source_path('contrib/python/grit/grit/testdata'))
  ...
  def testAcceptsPreprocess(self):
    root = util.ParseGrdForUnittest(
        ...
        base_dir=yatest.common.source_path('contrib/python/grit/grit/testdata'))
    inc, = root.GetChildrenOfType(include.IncludeNode)
