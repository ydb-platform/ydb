...
class GritNode(base.Node):
  ...
  def HandleAttribute(self, attrib, value):
    super(GritNode, self).HandleAttribute(attrib, value)
    if attrib == 'source_lang_id':
      self.UberClique().SetSourceLanguage(value)
  ...
  def GetFirstIdsFile(self):
    ...
    elif path.endswith(REL_PATH_SUFFIX):
      return path[:-len(REL_PATH_SUFFIX)]
    else:
  ...
  def SetValidateUniqueIds(self, validate_unique_ids):
  ...
  def GetGrdFilename(self):
    return self.grd_filename if hasattr(self, 'grd_filename') else None

  def SetGrdFilename(self, grd_filename):
