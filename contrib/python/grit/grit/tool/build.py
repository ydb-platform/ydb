...
Options:
  ...
  --black-list BLACKLISTFILE
                    Path to a file containing the string names of the resources
                    to drop.  Anything not listed is processed as usual.
...
'''
  ...
  def Run(self, opts, args):
    ...
    for (key, val) in own_opts:
      if key == '-a':
        ...
        blacklist_filenames.append(val)
    ...
    if blacklist_filenames:
      self.blacklist_names = set()
      for blacklist_filename in blacklist_filenames:
        self.VerboseOut('Using blacklist: %s\n' % blacklist_filename);
        blacklist_contents = util.ReadFile(blacklist_filename, 'utf-8')
        self.blacklist_names.update(blacklist_contents.strip().split('\n'))
  ...
  def __init__(self, defines=None):
    ...
    # The set of names that are blacklisted to actually not to be included
    # in the output.
    self.blacklist_names = None
  ...
  @staticmethod
  def AddBlacklistTags(start_node, blacklist_names):
    # Walk the tree of nodes added attributes for the nodes that shouldn't
    # be written into the target files (skip markers).
    from grit.node import include
    from grit.node import message
    ...
    for node in start_node:
      ...
      if (isinstance(node, include.IncludeNode) or
          ...
          isinstance(node, structure.StructureNode)):
        text_ids = node.GetTextualIds()
        # Mark the item to be skipped if it was in the blacklist.
        if text_ids and text_ids[0] in blacklist_names:
          node.SetWhitelistMarkedAsSkip(True)
  ...
  def Process(self):
    ...
    # If there are blacklisted names, tag the tree once up front, this way
    # while looping through the actual output, it is just an attribute check.
    if self.blacklist_names:
      self.AddBlacklistTags(self.res, self.blacklist_names)
    ...
    for output in self.res.GetOutputFiles():
      ...
      # BROWSER-44579: Cannot compile same #defines with different content.
      comment_out_h_file = (output.GetType() == 'rc_header' and
                            'google_chrome' in output.GetOutputFilename())

      # Iterate in-order through entire resource tree, calling formatters on
      # the entry into a node and on exit out of it.
      with tmpfile:
        if comment_out_h_file:
          tmpfile.writelines('#if 0  // BROWSER-44579\n')
