...
class SearchPaths:
    def __init__(
        ...
        self.python_path = tuple(python_path)
        # from $MYPYPATH or config variable
        self.mypy_path = tuple(mypy_path)
        # from get_site_packages_dirs()
        self.package_path = tuple(package_path)
        # paths in typeshed
        self.typeshed_path = tuple(typeshed_path)
...
class FindModuleCache:
    ...
    def _find_module(self, id: str, use_typeshed: bool) -> tuple[ModuleSearchResult, bool]:
        ...
        # In Arcadia, third-party stubs have high priority
        candidate_base_dirs += third_party_stubs_dirs
        ...
        for base_dir, verify in candidate_base_dirs:
            ...
            if base_dir == '':
                # The path in binary has no separator at the beginning
                base_path = components[-1]
            has_init = False
            dir_prefix = base_dir
...
def default_lib_path(
    ...
    if custom_typeshed_dir:
        ...
        if os.path.isdir(auto):
            data_dir = auto
        typeshed_dir = "typeshed/stdlib"
        mypy_extensions_dir = "typeshed/stubs/mypy-extensions"
...
def load_stdlib_py_versions(custom_typeshed_dir: str | None) -> StdlibVersions:
    ...
    f = ress.resfs_read(versions_path).decode()
    if True:
        for line in f.split('\n'):
