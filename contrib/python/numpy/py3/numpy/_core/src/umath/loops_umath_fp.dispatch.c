#line 1 "../numpy/_core/src/umath/loops_umath_fp.dispatch.c.src"
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_f64 out = __svml_exp28_ha(x_exnan);
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_f64 out = __svml_log28_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(1));
        npyv_f64 out = __svml_log108_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_log108_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(1));
        npyv_f64 out = __svml_expm18_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_expm18_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_log1p8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_log1p8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_cbrt8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_tan8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_asin8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_asin8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_acos8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_acos8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_atan8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_atan8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_sinh8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_sinh8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_cosh8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_cosh8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_asinh8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_asinh8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(1));
        npyv_f64 out = __svml_acosh8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_acosh8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src += ssrc*vstep, dst += sdst*vstep) {
        ...
        npyv_b64 nnan_mask = npyv_notnan_f64(x);
        npyv_f64 x_exnan = npyv_select_f64(nnan_mask, x, npyv_setall_f64(0));
        npyv_f64 out = __svml_atanh8_ha(x_exnan);
        out = npyv_select_f64(nnan_mask, out, x);
    #else
        npyv_f64 out = __svml_atanh8_ha(x);
    #endif
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
                        ...
                        npyv_lanetype_f64 *dst, npy_intp sdst, npy_intp len)
{
    ...
    for (; len > 0; len -= vstep, src1 += ssrc1*vstep, src2 += ssrc2*vstep, dst += sdst*vstep) {
        ...
        npyv_f64 out = __svml_atan28_ha(x1, x2);
        if (sdst == 1) {
            npyv_store_till_f64(dst, len, out);
        } else {
            npyv_storen_till_f64(dst, sdst, len, out);
