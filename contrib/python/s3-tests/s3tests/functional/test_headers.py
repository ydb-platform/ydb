...
from s3tests.functional import (
    _make_raw_request,
...
def setup():
    ...
    if hasattr(S3Connection, 'fill_in_auth'):
        ...
        if boto.connection.HTTPRequest.authorize != _our_authorize:
...
def test_object_create_bad_amz_date_invalid_aws4():
    ...
    assert e.error_code in ('AccessDenied', 'SignatureDoesNotMatch', 'MalformedDate')
...
def test_bucket_create_bad_authorization_invalid_aws4():
    ...
    eq(e.status, 403)
    eq(e.reason.lower(), 'forbidden') # some proxies vary the case
    eq(e.error_code, 'AccessDenied')
