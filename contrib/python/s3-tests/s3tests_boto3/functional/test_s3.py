...
from s3tests_boto3.functional.policy import Policy, Statement, make_json_policy
...
def _create_objects(bucket=None, bucket_name=None, keys=None):
    """
...
def _multipart_upload(bucket_name, key, size, part_size=5*1024*1024, client=None, content_type=None, metadata=None, resend_parts=None):
    """
    ...
    if client == None:
        client = get_client()
    resend_parts = [] if not resend_parts else resend_parts
...
def test_object_copy_versioning_multipart_upload():
    ...
    eq(CaseInsensitiveDict(key1_metadata), CaseInsensitiveDict(response['Metadata']))
    eq(content_type, response['ContentType'])
...
def test_multipart_upload():
    ...
    eq(CaseInsensitiveDict(response['Metadata']), CaseInsensitiveDict(metadata))
    body = _get_body(response)
...
def test_versioning_bucket_create_suspend():
    ...
    # Our custom behavior. When suspended attempting to apply to bucket 
    # with disabled versioning, nothing happens. The "Suspended" state 
    # will only be set when the bucket's versioning is enabled.
    check_configure_versioning_retry(bucket_name, "Suspended", None)
    check_configure_versioning_retry(bucket_name, "Enabled", "Enabled")
    check_configure_versioning_retry(bucket_name, "Enabled", "Enabled")
