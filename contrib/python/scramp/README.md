...
* [Standards](#standards)
* [Missing `tls-exporter` channel binding support](#missing-tls-exporter-channel-binding-support)
* [API Docs](#api-docs)
...
## Missing `tls-exporter` channel binding support

RFC 9266 defines `tls-exporter` as the channel binding type for TLS 1.3, using the
keying-material exporter from [RFC 5705](https://www.rfc-editor.org/rfc/rfc5705)
(TLS ≤1.2) / [RFC 8446 §7.5](https://www.rfc-editor.org/rfc/rfc8446#section-7.5)
(TLS 1.3), i.e. OpenSSL's `SSL_export_keying_material()`.

Scramp can't implement `tls-exporter` because Python doesn't support it in
 [`SSLSocket.get_channel_binding()`](https://docs.python.org/3/library/ssl.html#ssl.SSLSocket.get_channel_binding).
Only the older `tls-unique`/`tls-server-end-point` are supported.

* Relevant Python issue: [python/cpython#25255](https://github.com/python/cpython/pull/25255)
* Relevant [Scramp issue](https://codeberg.org/tlocke/scramp/issues/9).

Once `tls-exporter` is supported by `SSLSocket.get_channel_binding()`, we can
support `tls-exporter` in Scramp.
...
Note that `ScramServer` doesn't enforce a maximum message or field length, so that's something you
might want to consider doing if you're embedding it in a network-facing server.
...
`podman run -e GITHUB_AUTH_TOKEN=<auth_token> gcr.io/openssf/scorecard:stable --repo=github.com/tlocke/scramp`
...
- Check message attribute values don't have invalid characters.


### Version 1.4.14, 2026-07-25

- Make sure all Salt and Nonce errors can be handled with the protocol.
- Add note in README on tls-exporter.
