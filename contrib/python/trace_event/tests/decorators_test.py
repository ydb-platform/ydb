...
from trace_event_impl import decorators
...
class DecoratorTests(TraceTest):
  def _get_decorated_method_name(self, f):
    ...
    self.assertEqual(events[0]["name"], events[1]["name"])
    return events[1]["name"]


  def test_func_names_work(self):
    self.assertEqual('__tests__.decorators_test.traced_func', self._get_decorated_method_name(traced_func))

  def test_method_names_work(self):
    ctt = ClassToTest()
    self.assertEqual('ClassToTest.method1', self._get_decorated_method_name(ctt.method1))
    self.assertEqual('ClassToTest.method2', self._get_decorated_method_name(ctt.method2))
