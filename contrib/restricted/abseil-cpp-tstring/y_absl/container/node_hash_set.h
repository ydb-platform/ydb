...
#include "y_absl/container/internal/node_slot_policy.h"
...
// types.
//
// Using `y_absl::node_hash_set` at interface boundaries in dynamically loaded
...
class Y_ABSL_INTERNAL_ATTRIBUTE_OWNER node_hash_set
...
template <class T>
struct NodeHashSetPolicy
    : y_absl::container_internal::node_slot_policy<T&, NodeHashSetPolicy<T>> {
  using key_type = T;
  using init_type = T;
