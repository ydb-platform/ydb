...
// Common code shared between y_absl/hash/hash_test.cc and
// y_absl/hash/hash_instantiated_test.cc.

#ifndef Y_ABSL_HASH_INTERNAL_HASH_TEST_H_
#define Y_ABSL_HASH_INTERNAL_HASH_TEST_H_
...
class TypeErasedValue {
 public:
  ...
  friend H AbslHashValue(H hash_state, const TypeErasedValue& v) {
    v.HashValue(y_absl::HashState::Create(&hash_state));
    return hash_state;
  }

  void HashValue(y_absl::HashState state) const {
    y_absl::HashState::combine(std::move(state), n_);
  }
...
// Helper trait to verify if T is hashable. We use y_absl::Hash's poison status to
// detect it.
template <typename T>
using is_hashable = std::is_default_constructible<y_absl::Hash<T>>;

}  // namespace hash_test_internal
Y_ABSL_NAMESPACE_END
}  // namespace y_absl

#endif  // Y_ABSL_HASH_INTERNAL_HASH_TEST_H_
