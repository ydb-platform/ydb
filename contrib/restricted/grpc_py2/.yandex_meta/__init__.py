...
def post_install(self):
    def fix_protos(m):
        ...
        for s in srcs:
            if s.endswith(".proto"):
                protos.add(s)
                m.PEERDIR.add(P.join("contrib/restricted/grpc_py2/src", P.dirname(s)))
    ...
    # fix path for protos
    with self.yamakes["grpc++_reflection"] as m:
        m.PEERDIR.remove("contrib/restricted/grpc_py2/src/protos/src/proto/grpc/reflection/v1alpha")
        m.PEERDIR.add("contrib/restricted/grpc_py2/src/proto/grpc/reflection/v1alpha")
        m.ADDINCL.remove("contrib/restricted/grpc_py2/protos")

    with self.yamakes["grpcpp_channelz"] as m:
        m.PEERDIR.remove("contrib/restricted/grpc_py2/src/protos/src/proto/grpc/channelz")
        m.PEERDIR.add("contrib/restricted/grpc_py2/src/proto/grpc/channelz")
        m.ADDINCL.remove("contrib/restricted/grpc_py2/protos")

    # fix induced deps
    for name, module in self.yamakes.items():
        addincls = getattr(module, "ADDINCL", None)
        source_addincl = ArcPath("contrib/restricted/grpc_py2", build=False)
        build_addincl = ArcPath("contrib/restricted/grpc_py2", build=True)
    ...
    self.yamakes["."].LD_PLUGIN = ["hide_grpc.py"]


grpc = CMakeNinjaNixProject(
    owners=["g:cpp-contrib"],
    arcdir="contrib/restricted/grpc_py2",
    nixattr="grpc",
    license="Apache-2.0",
    keep_paths=[
