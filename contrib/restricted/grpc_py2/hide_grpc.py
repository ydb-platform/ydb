...
def rename_syms(where, ret, libs):
    ...
    args = [
        where + 'llvm-objcopy',
        '--redefine-sym',
        p + 'init4grpc7_cython6cygrpc=init4grpc7_cython6cygrpc',
        '--redefine-sym',
        p + '_init4grpc7_cython6cygrpc=_init4grpc7_cython6cygrpc',
        ret,
    ]
...
def fix_py2(cmd, have_comand_files=False, prefix='lib', suffix='a'):
    ...
    if 'contrib-restricted-grpc_py2' not in str(args):
        return cmd

    py2_libs = ['contrib-restricted-grpc_py2', 'grpc_py2-third_party-address_sorting', 'grpc_py2-third_party-upb', 'pypython-grpcio-py2']
    py2_libs = [ prefix + x + '.' + suffix for x in py2_libs]
    ...
    name = rename_syms(where, 'libpy2_grpc.' + suffix, lib)
    ...
    for file in cmd:
        if is_cmdfile_arg(file):
            ...
            if not 'contrib-restricted-grpc_py2' in str(args):
                continue
