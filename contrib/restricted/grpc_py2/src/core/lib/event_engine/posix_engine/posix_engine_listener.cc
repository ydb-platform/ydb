  ...
  for (;;) {
    ...
    auto endpoint = CreatePosixEndpoint(
        ...
        listener_->memory_allocator_factory_->CreateMemoryAllocator(
            y_absl::StrCat("endpoint-tcp-server-connection: ", *peer_name)),
    ...
    listener_->on_accept_(
        ...
        /*memory_allocator=*/
        listener_->memory_allocator_factory_->CreateMemoryAllocator(
            y_absl::StrCat("on-accept-tcp-server-connection: ", *peer_name)),
        /*pending_data=*/nullptr);
  }
  GPR_UNREACHABLE_CODE(return);
}

y_absl::Status PosixEngineListenerImpl::HandleExternalConnection(
  ...
  if (fd < 0) {
    return y_absl::UnknownError(
        y_absl::StrCat("HandleExternalConnection: Invalid peer socket: ", fd));
  }
  PosixSocketWrapper sock(fd);
  (void)sock.SetSocketNoSigpipeIfPossible();
  ...
  if (!peer_name.ok()) {
    return y_absl::UnknownError(
        y_absl::StrCat("HandleExternalConnection: peer not connected: ",
  ...
  auto endpoint = CreatePosixEndpoint(
      ...
      /*allocator=*/
      memory_allocator_factory_->CreateMemoryAllocator(y_absl::StrCat(
...
void PosixEngineListenerImpl::AsyncConnectionAcceptor::Shutdown() {
  ...
  handle_->ShutdownHandle(y_absl::InternalError("Shutting down acceptor"));
  Unref();
}

y_absl::Status PosixEngineListenerImpl::Start() {
  y_absl::MutexLock lock(&this->mu_);
