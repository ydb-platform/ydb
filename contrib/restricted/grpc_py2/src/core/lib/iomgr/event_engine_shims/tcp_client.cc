...
int64_t event_engine_tcp_client_connect(
  ...
  EventEngine::ConnectionHandle handle = engine_ptr->Connect(
      [on_connect,
       endpoint](y_absl::StatusOr<std::unique_ptr<EventEngine::Endpoint>> ep) {
        grpc_core::ApplicationCallbackExecCtx app_ctx;
        grpc_core::ExecCtx exec_ctx;
        y_absl::Status conn_status = ep.ok() ? y_absl::OkStatus() : ep.status();
        if (ep.ok()) {
      ...
      resource_quota != nullptr
          ? resource_quota->memory_quota()->CreateMemoryOwner(
                y_absl::StrCat("tcp-client:", addr_uri.value()))
