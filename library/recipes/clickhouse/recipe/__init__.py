...
def start(argv):
    ...
    http_port = opts.get('http-port', http_port)
    native_port = opts.get('native-port', native_port)
...
def _parse_argv(argv):
    opts, argv = getopt.getopt(
        argv, '', ['config=', 'execute=', 'execute-file=', 'insert-csv=', 'expand-vars', 'prefix=', 'native-port=', 'http-port=']
    )
    opt_dict = {}
    for opt, arg in opts:
        if opt == '--config':
            ...
            opt_dict['prefix'] = arg
        elif opt == '--native-port':
            opt_dict['native-port'] = int(arg)
        elif opt == '--http-port':
            opt_dict['http-port'] = int(arg)
...
def _prepare_clickhouse_config(opts):
    ...
    if os.path.exists(work_config_dir):
