...
def invoke_grpc(func, *params, explicit_host=None, endpoint=None, stub_factory=kikimr_grpc.TGRpcServerStub,
                endpoints=None, authenticated=False, result_handler=None, report_unimplemented=False):
    options = [
    ...
    def work(channel):
        try:
            ...
            kwargs = {}
            if authenticated and connection_params.token is not None:
                kwargs['metadata'] = (('x-ydb-auth-ticket', connection_params.token),)
            res = getattr(stub, func)(*params, **kwargs)
...
def invoke_distributed_storage_request(request_type, request, result_handler=None):
    return invoke_grpc(request_type, request,
                       stub_factory=distributed_storage_grpc_server.DistributedStorageServiceStub,
                       authenticated=True, result_handler=result_handler, report_unimplemented=True)
