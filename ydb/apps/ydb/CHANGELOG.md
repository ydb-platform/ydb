...
* Resource paths in the YDB CLI are relative to the database unless they start with `/`. The CLI expands paths when the database path is absolute; otherwise resolution requires server support for relative paths. `ydb tools restore` supports relative destinations whose parent directories do not yet exist.
