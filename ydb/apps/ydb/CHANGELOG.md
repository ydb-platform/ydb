...
* Resource paths in the YDB CLI are resolved by the server. Only paths starting with `/` are absolute; all other paths are relative to the database. `ydb tools restore` supports relative destinations whose parent directories do not yet exist.
