...
struct TEvBlobStorage {
    ...
    struct TEvPut : public TEventLocal<TEvPut, EvPut> {
        ...
        const TContiguousData Buffer; //FIXME(innokentii) const members prevent usage of move-semantics elsewhere
        ...
        TEvPut(const TLogoBlobID &id, TContiguousData &&buffer, TInstant deadline,
        ...
        TEvPut(const TLogoBlobID &id, const TString &buffer, TInstant deadline,
               NKikimrBlobStorage::EPutHandleClass handleClass = NKikimrBlobStorage::TabletLog,
               ETactic tactic = TacticDefault)
            : TEvPut(id, TContiguousData(buffer), deadline, handleClass, tactic)
        {}
    ...
    public:
        struct TDiff {
            ...
            void Set(const TString &buffer, ui32 offset) {
                Buffer = TContiguousData(buffer);
                Offset = offset;
                Y_VERIFY_S(buffer.Size(), "EvPatchDiff invalid: Diff size must be non-zero");
            }

            void Set(const TContiguousData &buffer, ui32 offset) {
                Buffer = buffer;
                Offset = offset;
                Y_VERIFY_S(buffer.Size(), "EvPatchDiff invalid: Diff size must be non-zero");
            }
