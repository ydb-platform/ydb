...
class TBoardLookupActor : public TActorBootstrapped<TBoardLookupActor> {
    ...
    bool CheckConfigVersion(const TActorId &sender, const auto *msg) {
        ...
        if (ClusterStateGeneration < msgGeneration || (ClusterStateGeneration == msgGeneration && ClusterStateGuid != msgGuid)) {
            LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::BOARD_LOOKUP,"LookupReplica TEvNodeWardenNotifyConfigMismatch: Info->ClusterStateGeneration=" << ClusterStateGeneration << " msgGeneration=" << msgGeneration <<" Info->ClusterStateGuid=" << ClusterStateGuid << " msgGuid=" << msgGuid);
    ...
    void PassAway() override {
        LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::BOARD_LOOKUP,"TBoardLookupActor::PassAway");
        if (Subscriber) {
    ...
    void Handle(TEvStateStorage::TEvResolveReplicasList::TPtr &ev) {
        ...
        if (msg->ReplicaGroups.empty()) {
            LOG_ERROR_S(*TlsActivationContext, NKikimrServices::BOARD_LOOKUP,"lookup on unconfigured statestorage board service");
            return NotAvailable();
        }
        ...
        for (ui32 replicaGroupIdx : xrange(msg->ReplicaGroups.size())) {
            ...
            LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::BOARD_LOOKUP,"Handle TEvResolveReplicasList: Mode: " << (ui32)Mode << " groupIdx: " << replicaGroupIdx << " Group: " << replicaGroups.ToString() << " Path: " << Path);
        }
    ...
    void Handle(TEvStateStorage::TEvReplicaBoardInfo::TPtr &ev) {
        ...
        auto &replica = ReplicaGroups[groupIdx].Replicas[idx];
        LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::BOARD_LOOKUP,"Handle TEvReplicaBoardInfo: groupIdx: " << groupIdx << " idx: " << idx << " reconnectNumber: "
