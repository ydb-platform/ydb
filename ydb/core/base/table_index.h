...
namespace NFulltext {
    ...
    // Deterministic names of the auto-provisioned unique secondary index over [__rowId] and
    // of the sequence that generates __rowId values. Deterministic so a second fulltext index
    // on the same table reuses the same infrastructure instead of creating duplicates.
    inline constexpr const char* RowIdUniqueIndexName = "uniq___rowId";
    inline constexpr const char* RowIdSequenceName = "_seq___rowId";
