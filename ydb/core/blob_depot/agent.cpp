...
namespace NKikimr::NBlobDepot {
    ...
    void TBlobDepot::Handle(TEvTabletPipe::TEvServerConnected::TPtr ev) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvServerConnected", {"Marker", "BDT01"},
            {"Id", GetLogId()},
    ...
    void TBlobDepot::Handle(TEvTabletPipe::TEvServerDisconnected::TPtr ev) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvServerDisconnected", {"Marker", "BDT02"},
            {"Id", GetLogId()},
    ...
    void TBlobDepot::Handle(TEvBlobDepot::TEvRegisterAgent::TPtr ev) {
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvRegisterAgent", {"Marker", "BDT03"},
            {"Id", GetLogId()},
            {"Msg", req},
            {"NodeId", nodeId},
    ...
    void TBlobDepot::Handle(TEvBlobDepot::TEvAllocateIds::TPtr ev) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvAllocateIds", {"Marker", "BDT04"},
            {"Id", GetLogId()},
            {"Msg", ev->Get()->Record},
        ...
        if (PickChannels(record->GetChannelKind(), channels)) {
            ...
            for (const auto& range : givenIdRange->GetChannelRanges()) {
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT, "IssueNewRange", {"Marker", "BDT05"},
                    {"Id", GetLogId()},
    ...
    void TBlobDepot::ResetAgent(TAgent& agent) {
        for (auto& [channel, agentGivenIdRange] : agent.GivenIdRanges) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "ResetAgent", {"Marker", "BDT06"},
                {"Id", GetLogId()},
                {"AgentId", agent.Connection->NodeId},
                {"Channel", int(channel)},
