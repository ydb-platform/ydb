...
namespace NKikimr::NBlobDepot {
    ...
    void TBlobDepotAgent::TBlobMappingCache::HandleResolveResult(ui64 tag, const NKikimrBlobDepot::TEvResolveResult& msg,
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleResolveResult", {"Marker", "BDA28"},
            {"AgentId", Agent.LogId},
            {"Cookie", tag},
            {"Msg", msg});
        ...
        auto process = [&](const auto& item, bool nodata) {
            ...
            if (item.HasErrorReason() || item.GetValueChain().empty() || nodata) {
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleResolveResult error", {"Marker", "BDA43"},
                    {"AgentId", Agent.LogId},
                    {"Item", item},
                    {"NoData", nodata},
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleResolveResult success", {"Marker", "BDA44"},
                    {"AgentId", Agent.LogId},
                    {"Item", item},
                    {"NoData", nodata},
    ...
    const TResolvedValue *TBlobDepotAgent::TBlobMappingCache::ResolveKey(TString key, TQuery *query,
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "ResolveKey", {"Marker", "BDA45"},
            {"AgentId", Agent.LogId},
    ...
    void TBlobDepotAgent::TBlobMappingCache::ProcessResponse(ui64 tag, TRequestContext::TPtr context, TResponse response) {
        if (auto *p = std::get_if<TEvBlobDepot::TEvResolveResult*>(&response)) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "TBlobMappingCache::TTabletDisconnected", {"Marker", "BDA38"},
                {"AgentId", Agent.LogId},
                {"Cookie", tag});
            auto& resolveContext = context->Obtain<TResolveContext>();
