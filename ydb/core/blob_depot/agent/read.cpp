...
namespace NKikimr::NBlobDepot {
    ...
    bool TBlobDepotAgent::TQuery::IssueRead(TReadArg&& arg, TString& error) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "IssueRead", {"Marker", "BDA34"},
            {"AgentId", Agent.LogId},
            {"QueryId", GetQueryId()},
            {"ReadId", arg.Tag},
        ...
        for (const auto& value : arg.Value.Chain) {
            ...
            if (end <= begin || itemLen < end) {
                ...
                YDBLOG_COMP_CRIT(BLOB_DEPOT_AGENT, error, {"Marker", "BDA24"},
                    {"AgentId", Agent.LogId},
                    {"QueryId", GetQueryId()},
                    {"ReadId", arg.Tag},
                    {"Key", Agent.PrettyKey(arg.Key)},
        ...
        if (size) {
            ...
            YDBLOG_COMP_ERROR(BLOB_DEPOT_AGENT, error, {"Marker", "BDA25"},
                {"AgentId", Agent.LogId},
                {"QueryId", GetQueryId()},
                {"ReadId", arg.Tag},
                {"Key", Agent.PrettyKey(arg.Key)},
        ...
        for (const auto& [groupId, queries] : queriesPerGroup) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "issuing TEvGet", {"Marker", "BDA39"},
                {"AgentId", Agent.LogId},
                {"QueryId", GetQueryId()},
        ...
        for (TS3ReadItem& item : s3items) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "starting S3 read", {"Marker", "BDA57"},
                {"AgentId", Agent.LogId},
                {"QueryId", GetQueryId()},
                {"ReadId", context->GetTag()},
                {"Key", item.Key},
    ...
    void TBlobDepotAgent::TQuery::HandleGetResult(const TRequestContext::TPtr& context, TEvBlobStorage::TEvGetResult& msg) {
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleGetResult", {"Marker", "BDA41"},
            {"AgentId", Agent.LogId},
            {"QueryId", GetQueryId()},
        ...
        for (ui32 i = 0; i < msg.ResponseSz; ++i) {
            ...
            if (blob.Status == NKikimrProto::NODATA) {
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "issuing extra resolve", {"Marker", "BDA48"},
                    {"Agent", Agent.LogId},
                    {"QueryId", GetQueryId()},
                    {"ReadId", readContext.GetTag()},
                    {"Key", Agent.PrettyKey(readContext.ReadArg.Key)},
    ...
    void TBlobDepotAgent::TQuery::HandleResolveResult(const TRequestContext::TPtr& context, TEvBlobDepot::TEvResolveResult& msg) {
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleResolveResult", {"Marker", "BDA42"},
            {"AgentId", Agent.LogId},
            {"QueryId", GetQueryId()},
            {"ReadId", readContext.GetTag()},
            {"Key", Agent.PrettyKey(readContext.ReadArg.Key)},
        ...
        if (msg.Record.GetStatus() != NKikimrProto::OK) {
            ...
            if (TResolvedValue value(item); value.Supersedes(readContext.ReadArg.Value)) { // value chain has changed, we have to try again
                ...
                YDBLOG_COMP_CRIT(BLOB_DEPOT_AGENT, "failed to read blob: data seems to be lost", {"Marker", "BDA40"},
                    {"AgentId", Agent.LogId},
                    {"QueryId", GetQueryId()},
                    {"ReadId", readContext.GetTag()},
                    {"Key", Agent.PrettyKey(readContext.ReadArg.Key)},
    ...
    void TBlobDepotAgent::TQuery::IssueCheckIntegrity(TReadArg&& arg) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "IssueCheckIntegrity", {"Marker", "BDA62"},
            {"AgentId", Agent.LogId},
            {"QueryId", GetQueryId()},
            {"Key", Agent.PrettyKey(arg.Key)},
            {"Value", arg.Value});
        ...
        for (const auto& value : checkContext->ReadArg.Value.Chain) {
            if (value.Blob) {
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "issuing TEvCheckIntegrity", {"Marker", "BDA63"},
                    {"AgentId", Agent.LogId},
                    {"QueryId", GetQueryId()},
    ...
    void TBlobDepotAgent::TQuery::HandleCheckIntegrityResult(const TRequestContext::TPtr& context,
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "HandleCheckIntegrityResult", {"Marker", "BDA64"},
            {"AgentId", Agent.LogId},
            {"QueryId", GetQueryId()},
            {"Key", Agent.PrettyKey(checkContext.ReadArg.Key)},
            {"Msg", msg});
