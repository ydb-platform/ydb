...
namespace NKikimr::NBlobDepot {
    ...
    bool TBlobDepotAgent::TQuery::IssueRead(TReadArg&& arg, TString& error) {
        ...
        for (TS3ReadItem& item : s3items) {
            class TGetActor : public TActorBootstrapped<TGetActor> {
                size_t OutputOffset;
                std::weak_ptr<TReadContext> ReadContext;
                TQuery* const Query;

            public:
                TGetActor(size_t outputOffset, std::weak_ptr<TReadContext> readContext, TQuery *query)
                ...
                void Finish(std::optional<TString> data) {
                    if (auto context = ReadContext.lock(); context && !context->Terminated && !context->StopProcessingParts) {
                        if (data) {
                            auto& buffer = context->Buffer;
                            buffer.Write(OutputOffset, TRope(std::move(*data)));
                        } else {
                            context->EndWithError(Query, NKikimrProto::ERROR, "failed to fetch data from S3");
                        }
