    ...
    TBlobDepotAgent::TQuery *TBlobDepotAgent::CreateQuery<TEvBlobStorage::EvGet>(std::unique_ptr<IEventHandle> ev,
        ...
        public:
            ...
            void Initiate() override {
                ...
                for (ui32 i = 0; i < Request.QuerySize; ++i) {
                    ...
                    if (const TResolvedValue *value = Agent.BlobMappingCache.ResolveKey(blobId, this,
                        ...
                        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "resolve pending", {"Marker", "BDA29"},
                            {"AgentId", Agent.LogId},
                            {"QueryId", GetQueryId()},
                            {"QueryIdx", i},
                            {"BlobId", query.Id});
            ...
            bool ProcessSingleResult(ui32 queryIdx, const TKeyResolved& result) {
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "ProcessSingleResult", {"Marker", "BDA27"},
                    {"AgentId", Agent.LogId},
                    {"QueryId", GetQueryId()},
                    {"QueryIdx", queryIdx},
                    {"Result", result});
            ...
            void OnRead(ui64 tag, TReadOutcome&& outcome) override {
                YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "OnRead", {"Marker", "BDA35"},
                    {"AgentId", Agent.LogId},
                    {"QueryId", GetQueryId()},
                    {"Tag", tag},
                    {"Outcome", outcome});
            ...
            void ProcessResponse(ui64 /*id*/, TRequestContext::TPtr context, TResponse response) override {
                if (auto *p = std::get_if<TKeyResolved>(&response)) {
                    ...
                    if (auto *resolveContext = dynamic_cast<TResolveKeyContext*>(context.get())) {
                        YDBLOG_COMP_DEBUG(BLOB_DEPOT_AGENT, "TTabletDisconnected", {"Marker", "BDA26"},
                            {"AgentId", Agent.LogId},
                            {"QueryId", GetQueryId()},
                            {"QueryIdx", resolveContext->QueryIdx});
