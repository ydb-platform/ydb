        ...
        public:
            ...
            void HandlePrepareWriteS3Result(TRequestContext::TPtr /*context*/, NKikimrBlobDepot::TEvPrepareWriteS3Result& msg) {
                ...
                if (item.GetStatus() == NKikimrProto::OK) {
                    ...
                    public:
                        TWriteActor(TString key, TString data, TActorId wrapperId, std::weak_ptr<TLifetimeToken> lifetimeToken,
                                TPutQuery *putQuery)
                            : Key(std::move(key))
                            , Data(std::move(data))
                            , WrapperId(wrapperId)
                        ...
                        void Bootstrap() {
                            Send(WrapperId, new NWrappers::TEvExternalStorage::TEvPutObjectRequest(
                                Aws::S3::Model::PutObjectRequest().WithKey(std::move(Key)),
                                std::move(Data)));
                    ...
                    TString key = TStringBuilder()
                        << Agent.S3BasePath
                        << '/' << Agent.TabletId
                        << '/' << Request.Id.ToString()
                        << '@' << locator->GetGeneration()
                        << '.' << locator->GetKeyId();

                    LifetimeToken = std::make_shared<TLifetimeToken>();
                    WriterActorId = Agent.RegisterWithSameMailbox(new TWriteActor(std::move(key),
                        Request.Buffer.ExtractUnderlyingContainerOrCopy<TString>(), Agent.S3WrapperId,
                        LifetimeToken, this));
                } else {
                    EndWithError(item.GetStatus(), item.GetErrorReason());
                }
