...
namespace NKikimr::NBlobDepot {
    ...
    public:
        ...
        void Bootstrap() {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TResolveDecommitActor::Bootstrap", {"Marker", "BDT42"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie});
        ...
        void IssueRange(ui64 tabletId, TLogoBlobID from, TLogoBlobID to, bool mustRestoreFirst) {
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "going to TEvRange", {"Marker", "BDT50"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"TabletId", tabletId},
        ...
        void Handle(TEvBlobStorage::TEvRangeResult::TPtr ev) {
            auto& msg = *ev->Get();
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvRangeResult", {"Marker", "BDT55"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"Msg", msg},
                {"GetsInFlight", GetsInFlight},
                {"RangesInFlight", RangesInFlight},
                {"TxInFlight", TxInFlight},
                {"PutsInFlight", PutsInFlight},
                {"GetQ.size", GetQ.size()});
        ...
        void IssueGet(TLogoBlobID id, bool mustRestoreFirst) {
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "going to TEvGet", {"Marker", "BDT86"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"BlobId", id},
        ...
        void Handle(TEvBlobStorage::TEvGetResult::TPtr ev) {
            auto& msg = *ev->Get();
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvGetResult", {"Marker", "BDT87"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"Msg", msg},
        ...
        void IssuePut(TKey key, TRope&& buffer, bool keep, bool doNotKeep) {
            ...
            if (Self->PickChannels(NKikimrBlobDepot::TChannelKind::Data, channels)) {
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT, "going to TEvPut", {"Marker", "BDT91"},
                    {"Id", Self->GetLogId()},
                    {"Sender", Ev->Sender},
                    {"Cookie", Ev->Cookie},
                    {"Key", key},
        ...
        void Handle(TEvBlobStorage::TEvPutResult::TPtr ev) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "got TEvPutResult", {"Marker", "BDT88"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"Msg", msg},
        ...
        void HandleTxComplete() {
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "HandleTxComplete", {"Marker", "BDT84"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"GetsInFlight", GetsInFlight},
        ...
        void CheckIfDone() {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "request succeeded", {"Marker", "BDT92"},
                {"Id", Self->GetLogId()},
                {"Sender", Ev->Sender},
                {"Cookie", Ev->Cookie},
                {"ResolutionErrors.size", ResolutionErrors.size()},
        ...
        STATEFN(StateFunc) {
            ...
            switch (const ui32 type = ev->GetTypeRewrite()) {
                ...
                default:
                    ...
                    YDBLOG_COMP_CRIT(BLOB_DEPOT, "unexpected event", {"Marker", "BDT90"},
                        {"Id", Self->GetLogId()},
                        {"Type", type});
                    break;
            }
