...
namespace NKikimr::NBlobDepot {
    ...
    public:
        ...
        bool Execute(TTransactionContext& txc, const TActorContext&) override {
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TTxResolve::Execute", {"Marker", "BDT22"},
                {"Id", Self->GetLogId()},
        ...
        void Complete(const TActorContext&) override {
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TTxResolve::Complete", {"Marker", "BDT30"},
                {"Id", Self->GetLogId()},
                {"Sender", Request->Sender},
                {"Cookie", Request->Cookie},
        ...
        void IssueResponseItem(std::optional<ui64> cookie, const TKey& key, const TValue& value, bool reliablyWritten) {
            ...
            if (!ResolutionErrors.empty() && ResolutionErrors.contains(key.GetBlobId())) {
                ...
                if (!item.ValueChainSize()) {
                    YDBLOG_COMP_WARN(BLOB_DEPOT, "empty ValueChain on Resolve", {"Marker", "BDT48"},
                        {"Id", Self->GetLogId()},
                        {"Key", key},
                        {"Value", value},
                        {"Item", item},
    ...
    void TData::Handle(TEvBlobDepot::TEvResolve::TPtr ev) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvResolve", {"Marker", "BDT21"},
            {"Id", Self->GetLogId()},
            {"Msg", ev->Get()->ToString()},
            {"Sender", ev->Sender},
            {"Cookie", ev->Cookie},
