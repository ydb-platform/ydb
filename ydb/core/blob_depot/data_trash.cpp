...
namespace NKikimr::NBlobDepot {
    ...
    void TData::HandleTrash(TRecordsPerChannelGroup& record) {
        ...
        if (record.HardGenStep < hardGenStep) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "issuing hard barrier TEvCollectGarbage", {"Marker", "BDT85"},
                {"Id", Self->GetLogId()},
        ...
        if (trashInFlight.empty()) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "issuing TEvCollectGarbage", {"Marker", "BDT11"},
                {"Id", Self->GetLogId()},
                {"Channel", int(record.Channel)},
                {"GroupId", record.GroupId},
                {"Msg", ev->ToString()},
    ...
    void TData::Handle(TEvBlobStorage::TEvCollectGarbageResult::TPtr ev) {
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TEvCollectGarbageResult", {"Marker", "BDT12"},
            {"Id", Self->GetLogId()},
