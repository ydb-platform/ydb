...
namespace NKikimr::NBlobDepot {
    ...
    void TData::TUncertaintyResolver::PushResultWithUncertainties(TResolveResultAccumulator&& result,
        ...
        for (const TKey& key : uncertainties) {
            if (const TValue *value = Self->Data->FindKey(key); value && value->IsWrittenUncertainly()) {
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT, "uncertain key", {"Marker", "BDT61"},
                    {"Id", Self->GetLogId()},
                ...
                YDBLOG_COMP_DEBUG(BLOB_DEPOT, "racing uncertain key", {"Marker", "BDT62"},
                    {"Id", Self->GetLogId()},
                    {"Sender", entry->Result.GetSender()},
                    {"Cookie", entry->Result.GetCookie()},
                    {"Key", key});
        ...
        if (entry->NumUncertainKeys == 0) {
            ...
            YDBLOG_COMP_DEBUG(BLOB_DEPOT, "uncertainty resolver finished with noop", {"Marker", "BDT63"},
                {"Id", Self->GetLogId()},
                {"Sender", entry->Result.GetSender()},
                {"Cookie", entry->Result.GetCookie()});
    ...
    void TData::TUncertaintyResolver::MakeKeyCertain(const TKey& key) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::MakeKeyCertain", {"Marker", "BDT70"},
            {"Id", Self->GetLogId()},
            {"Key", key});
    ...
    void TData::TUncertaintyResolver::DropBlobs(const std::vector<TLogoBlobID>& blobIds) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::DropBlobs", {"Marker", "BDT71"},
            {"Id", Self->GetLogId()},
            {"BlobIds", blobIds});
    ...
    void TData::TUncertaintyResolver::DropKey(const TKey& key) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::DropKey", {"Marker", "BDT72"},
            {"Id", Self->GetLogId()},
            {"Key", key});
    ...
    void TData::TUncertaintyResolver::Handle(TEvBlobStorage::TEvGetResult::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::Handle(TEvGetResult)", {"Marker", "BDT73"},
            {"Id", Self->GetLogId()},
            {"Msg", msg});
    ...
    void TData::TUncertaintyResolver::FinishBlob(TLogoBlobID id, EKeyBlobState state, const TString& errorReason) {
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::FinishBlob", {"Marker", "BDT64"},
            {"Id", Self->GetLogId()},
            {"BlobId", id},
            {"State", state},
            ...
            switch (state) {
                case EKeyBlobState::INITIAL: {
                    ...
                    if (blob.KeysWaitingForThisBlob.size() == 1) {
                        ...
                        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver sending Get", {"Marker", "BDT65"},
                            {"Id", Self->GetLogId()},
                            {"BlobId", id},
                            {"Key", key},
                            {"GroupId", groupId});
    ...
    void TData::TUncertaintyResolver::FinishKey(const TKey& key, NKikimrProto::EReplyStatus status,
        ...
        YDBLOG_COMP_DEBUG(BLOB_DEPOT, "TUncertaintyResolver::FinishKey", {"Marker", "BDT66"},
            {"Id", Self->GetLogId()},
            {"Key", key},
            {"Status", status});
        ...
        for (auto& request : keyContext.DependentRequests) {
            ...
            if (--request->NumUncertainKeys == 0) { // we can finish the request
                YDBLOG_COMP_DEBUG(BLOB_DEPOT, "uncertainty resolver finished", {"Marker", "BDT67"},
                    {"Id", Self->GetLogId()},
