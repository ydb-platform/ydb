...
class TEventHolder {
    ...
    // exactly one of the two is set: either we keep the original event object and serialize it (if needed at all)
    // when it is actually sent, or the event has arrived already serialized and we just keep the buffer
...
public:
        ...
        , Tracker(std::move(ev->Get()->MessageRelevanceTracker))
    {
        ...
        if (ev->HasEvent()) {
            // keep the event object itself; it is serialized in SendToVDisk, when the MsgQoS fields are known and
            // the item is known to be actually going out -- items that expire in the queue or get pruned while
            // waiting never pay for serialization at all
            ByteSize = ev->Get()->GetCachedByteSize();
            Event.reset(ev->ReleaseBase().Release());
        } else {
            // the event has arrived in serialized form, there is no object to keep
