    ...
    public:
        ...
        void HandleProxyRequest(TAutoPtr<TEventHandle<TEvent>>& ev) {
            ...
            for (size_t i = 0; i < state.PileSize(); ++i) {
                ...
                std::unique_ptr<IEventBase> eventToSend = PrepareEvent(groupPileInfo, ev, i + 1 == state.PileSize());
                if (!eventToSend) {
                    continue;
                }
        ...
        std::unique_ptr<IEventBase> PrepareEvent(const NKikimrBridge::TGroupState::TPile& pile,
                TAutoPtr<TEventHandle<TEvent>>& ev, bool last) {
            auto& msg = *ev->Get();
            ...
            switch (pile.GetStage()) {
                ...
                case NKikimrBridge::TGroupState::WRITE_KEEP:
                    if constexpr (std::is_same_v<TEvent, TEvBlobStorage::TEvCollectGarbage>) {
                        ...
                        res = std::make_unique<TEvBlobStorage::TEvCollectGarbage>(msg.TabletId, msg.RecordGeneration,
                            msg.Channel, false, 0, 0, msg.Keep.Release(), nullptr, msg.Deadline);
                    }
                    [[fallthrough]];
            ...
            if (!res) {
                res.reset(last ? new TEvent(TEvBlobStorage::CloneEventPolicy, msg) : ev->Release().Release());
            }
