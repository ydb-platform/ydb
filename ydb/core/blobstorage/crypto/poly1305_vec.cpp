/*
 * Public Domain Poly1305Vec from Andrew Moon
 * Based on Poly1305Vec-donna.c, Poly1305Vec-donna-32.h and Poly1305Vec-donna.h from:
 *   https://github.com/floodyberry/Poly1305Vec-donna
 */
...
static uint128_t INLINE add128_64(uint128_t a, uint64_t b)
{
    return a + b;
}
...
static uint64_t INLINE lo128(uint128_t a)
{
    return (uint64_t)a;
}
...
constexpr size_t Poly1305Vec::KEY_SIZE;
constexpr size_t Poly1305Vec::MAC_SIZE;
...
void Poly1305Vec::Update(const ui8* msg, size_t size)
{
    poly1305_state_internal *st = poly1305_aligned_state(&state);
    size_t want;
    ...
    if (!st->started) {
        if ((st->leftover == 0) && (size > 32)) {
            poly1305_first_block(st, msg);
            msg += 32;
            size -= 32;
        } else {
            want = poly1305_min(32 - st->leftover, size);
            poly1305_block_copy(st->buffer + st->leftover, msg, want);
            size -= want;
            msg += want;
            st->leftover += want;
            if ((st->leftover < 32) || (size == 0))
                return;
    ...
    /* handle leftover */
    if (st->leftover) {
        want = poly1305_min(64 - st->leftover, size);
        poly1305_block_copy(st->buffer + st->leftover, msg, want);
        size -= want;
        msg += want;
        st->leftover += want;
    ...
    if (size >= 64) {
        want = (size & ~63);
        poly1305_blocks(st, msg, want);
        msg += want;
        size -= want;
    }

    if (size) {
        poly1305_block_copy(st->buffer + st->leftover, msg, size);
        st->leftover += size;
    }
}

void Poly1305Vec::Finish(ui8 mac[MAC_SIZE])
{
    poly1305_state_internal *st = poly1305_aligned_state(&state);
