...
namespace NKikimr::NDDisk {
    ...
    void TDDiskActor::InitPDiskInterface() {
        ...
        YDBLOG_DEBUG("TDDiskActor::InitPDiskInterface",
            {"Marker", "BSDD01"},
            {"DDiskId", DDiskId},
    ...
    void TDDiskActor::Handle(NPDisk::TEvYardInitResult::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_INFO("TDDiskActor::Handle(TEvYardInitResult)",
            {"Marker", "BSDD02"},
            {"DDiskId", DDiskId},
            {"Msg", msg.ToString()});
        ...
        if (!DiskFd.IsOpen()) {
            YDBLOG_INFO("TDDiskActor::Handle(TEvYardInitResult) DiskFd is invalid, all further I/O will be routed through PDisk",
                {"Marker", "BSDD17"},
                {"DDiskId", DDiskId},
                {"PDiskActorId", BaseInfo.PDiskActorID});
        }
    ...
    void TDDiskActor::Handle(NPDisk::TEvReadLogResult::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_DEBUG("TDDiskActor::Handle(TEvReadLogResult)",
            {"Marker", "BSDD03"},
            {"DDiskId", DDiskId},
            {"Msg", msg.ToString()});
    ...
    void TDDiskActor::CreatePersistentBuffer() {
        ...
        YDBLOG_DEBUG("TDDiskActor::CreatePersistentBuffer()",
            {"Marker", "BSDD03"},
            {"DDiskId", DDiskId},
        ...
        if (!UringRouter) {
            if (!Config.ForcePDiskFallback && DiskFd != INVALID_FHANDLE && DiskFormat && NPDisk::TUringRouter::Probe(config)) {
                ...
                if (const auto result = UringRouter->RegisterFile(); !result) {
                    YDBLOG_WARN("TDDiskActor::InitUring failed to register fixed file for io_uring",
                        {"Marker", "BSDD18"},
                        {"DDiskId", DDiskId},
                        {"Errno", result.error()});
                }
        ...
        if (UringRouter) {
            ...
            *Counters.DirectIO.FallbackPDiskCount = 0;
            if (actualFavor != requestedFavor) {
                YDBLOG_WARN("TDDiskActor::InitUring io_uring mode fallback",
                    {"Marker", "BSDD19"},
                    {"DDiskId", DDiskId},
                    {"RequestedFavor", requestedFavor},
                    {"ActualFavor", actualFavor});
            }
            YDBLOG_INFO("TDDiskActor::InitUring started io_uring with config",
                {"Marker", "BSDD20"},
                {"DDiskId", DDiskId},
    ...
    void TDDiskActor::Handle(NPDisk::TEvLogResult::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_DEBUG("TDDiskActor::Handle(TEvLogResult)",
            {"Marker", "BSDD05"},
            {"DDiskId", DDiskId},
            {"Msg", msg.ToString()});
