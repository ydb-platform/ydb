...
namespace NKikimr::NDDisk {
    ...
    bool TPersistentBufferBarriersManager::Compact(std::vector<ui64>& oldLsns, std::vector<ui64>& newLsns, TPersistentBufferHeader& header) {
        ...
        for (; it != lsns.end(); ++it) {
            ...
            while (delta >= 0x80) {
                ...
                header.Erase.CompactLsns[resPos++] = static_cast<char>((delta & 0x7F) | 0x80);
                delta >>= 7;
            }
            if (resPos >= TPersistentBufferHeader::ErasesBufferSize) {
                return false;
    ...
    bool TPersistentBufferBarriersManager::AddErase(const TPersistentBufferHeader* header, ui32 chunkIdx, ui32 sectorIdx) {
        ...
        erase.Lsns = Uncompact(header->Erase.CompactLsns, header->Flags & TPersistentBufferHeader::IS_ERASE_COMPACT);
