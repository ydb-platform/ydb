...
namespace NKikimr::NStorage {
    ...
    void TDistributedConfigKeeper::Bootstrap() {
        YDBLOG_COMP_DEBUG(BS_NODE, "Bootstrap", {"Marker", "NWDC00"});
    ...
    STFUNC(TDistributedConfigKeeper::StateWaitForInit) {
        YDBLOG_COMP_DEBUG(BS_NODE, "StateWaitForInit event", {"Marker", "NWDC53"},
    ...
    STFUNC(TDistributedConfigKeeper::StateFunc) {
        ...
        Y_DEFER {
            if (auto duration = TDuration::Seconds(timer.Passed()); duration >= TDuration::MilliSeconds(5)) {
                YDBLOG_COMP_WARN(BS_NODE, "StateFunc too long", {"Marker", "NWDC01"},
                    {"Type", type},
                    {"Duration", duration});
            }
        };
        YDBLOG_COMP_DEBUG(BS_NODE, "StateFunc", {"Marker", "NWDC15"},
            {"Type", ev->GetTypeRewrite()},
            {"Sender", ev->Sender},
