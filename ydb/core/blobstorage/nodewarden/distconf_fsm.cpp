...
namespace NKikimr::NStorage {
    ...
    void TDistributedConfigKeeper::HandleRetryCollectConfigsAndPropose(STATEFN_SIG) {
        YDBLOG_COMP_DEBUG(BS_NODE, "HandleRetryCollectConfigsAndPropose", {"Marker", "NWDC84"},
            {"Cookie", ev->Cookie},
    ...
    void TDistributedConfigKeeper::BecomeRoot() {
        YDBLOG_COMP_DEBUG(BS_NODE, "BecomeRoot", {"Marker", "NWDC85"},
            {"RootState", RootState},
    ...
    void TDistributedConfigKeeper::HandleErrorTimeout() {
        YDBLOG_COMP_DEBUG(BS_NODE, "Error timeout hit", {"Marker", "NWDC20"});
        Y_ABORT_UNLESS(RootState == ERootState::ERROR_TIMEOUT);
        Y_ABORT_UNLESS(!Scepter);
    ...
    void TDistributedConfigKeeper::ProcessGather(TEvGather *res) {
        YDBLOG_COMP_DEBUG(BS_NODE, "ProcessGather", {"Marker", "NWDC27"},
            {"RootState", RootState},
            {"Res", *res});
    ...
    TDistributedConfigKeeper::TProcessCollectConfigsResult TDistributedConfigKeeper::ProcessCollectConfigs(
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "ProcessCollectConfigs", {"Marker", "NWDC31"},
            {"RootState", RootState},
        ...
        for (const auto& node : res->GetNodes()) {
            if (node.HasBaseConfig()) {
                ...
                if (!CheckFingerprint(baseConfig)) {
                    YDBLOG_COMP_CRIT(BS_NODE, "BaseConfig fingerprint error", {"Marker", "NWDC57"},
                        {"NodeRecord", node});
        ...
        if (baseConfigs.size() > 1) {
            YDBLOG_COMP_CRIT(BS_NODE, "Multiple nonintersecting node sets have quorum of BaseConfig", {"Marker", "NWDC08"},
        ...
        for (auto&& [field, isCommitted] : {
            ...
            for (const TEvGather::TCollectConfigs::TPersistentConfig& item : *field) {
                ...
                if (!CheckFingerprint(config)) {
                    YDBLOG_COMP_ERROR(BS_NODE, "PersistentConfig fingerprint error", {"Marker", "NWDC58"},
                        {"ConfigRecord", config});
            ...
            for (auto&& [candidateCommitted, havingDisksPtr] : {
                ...
                if (HasConfigQuorum(r.Config, *havingDisksPtr, BridgePileNameMap, TBridgePileId(), *Cfg, false)) {
                    ...
                    if (configPtr && configPtr->GetFingerprint() != r.Config.GetFingerprint()) {
                        YDBLOG_COMP_ERROR(BS_NODE, "Persistent config quorum with different fingerprints", {"Marker", "NWDC37"},
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "ProcessCollectConfigs", {"Marker", "NWDC59"},
    ...
    void TDistributedConfigKeeper::ProcessProposeStorageConfig(TEvGather::TProposeStorageConfig *res) {
        ...
        if (TStringStream err; HasConfigQuorum(proposition.StorageConfig, successfulDisks, BridgePileNameMap, TBridgePileId(),
            ...
            YDBLOG_COMP_DEBUG(BS_NODE, "no quorum for ProposedStorageConfig", {"Marker", "NWDC47"},
                {"Record", *res},
        ...
        switch (task.Request.GetRequestCase()) {
            ...
            case TEvScatter::kProposeStorageConfig:
                if (PersistProposedStorageConfigInFlight) {
                    ...
                    YDBLOG_COMP_ERROR(BS_NODE, "ProposedStorageConfig generation mismatch", {"Marker", "NWDC49"},
                    ...
                    auto callback = [this, cookie](TEvPrivate::TEvStorageConfigStored& msg) {
                        ...
                        if (auto it = ScatterTasks.find(cookie); it != ScatterTasks.end()) {
                            ...
                            YDBLOG_COMP_DEBUG(BS_NODE, "ProposeStorageConfig TEvStorageConfigStored", {"Marker", "NWDC48"},
                                {"Cookie", cookie},
                                {"Status", *status});
                        } else {
                            YDBLOG_COMP_DEBUG(BS_NODE, "ProposeStorageConfig TEvStorageConfigStored no scatter task", {"Marker", "NWDC45"},
                                {"Cookie", cookie});
                        }
    ...
    std::optional<TString> TDistributedConfigKeeper::StartProposition(NKikimrBlobStorage::TStorageConfig *configToPropose,
        ...
        YDBLOG_COMP_INFO(BS_NODE, "StartProposition", {"Marker", "NWDC60"},
            {"ConfigToPropose", *configToPropose},
    ...
    void TDistributedConfigKeeper::CheckForConfigUpdate() {
        ...
        if (NKikimrBlobStorage::TStorageConfig config(*StorageConfig); UpdateConfig(&config)) {
            YDBLOG_COMP_DEBUG(BS_NODE, "CheckForConfigUpdate", {"Marker", "NWDC63"},
                {"Config", config});
            Invoke(TProposeConfig{
                .Config = std::move(config),
            });
        } else {
            YDBLOG_COMP_DEBUG(BS_NODE, "CheckForConfigUpdate: no update", {"Marker", "NWDC83"});
        }
    }
