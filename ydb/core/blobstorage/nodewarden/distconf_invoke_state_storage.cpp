...
namespace NKikimr::NStorage {
    ...
    void TInvokeRequestHandlerActor::SelfHealStateStorage(ui32 waitForConfigStep, bool forceHeal, bool pileupReplicas, ui32 overrideReplicasInRingCount, ui32 overrideRingsCount, ui32 replicasSpecificVolume) {
        RunCommonChecks();
        YDBLOG_COMP_DEBUG(BS_NODE, "TInvokeRequestHandlerActor::SelfHealStateStorage", {"Marker", "NW105"},
        ...
        auto needReconfig = [&](auto clearFunc, auto ssMutableFunc, auto buildFunc) {
            ...
            if (oldSSInfo->RingGroups == newSSInfo->RingGroups) {
                (targetConfig.*clearFunc)();
                 YDBLOG_COMP_DEBUG(BS_NODE, "needReconfig clear config", {"Marker", "NW104"},
                     {"CurrentConfig", ss},
        ...
        if (nodesToReplace.size() == 1 && needReconfigSS != ReconfigType::FULL && needReconfigSSB != ReconfigType::FULL && needReconfigSB != ReconfigType::FULL) {
            YDBLOG_COMP_DEBUG(BS_NODE, "Need to reconfig one node " << nodesToReplace.begin()->first << " to " << std::get<2>(nodesToReplace.begin()->second), {"Marker", "NW100"},
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "Need to reconfig, starting StateStorageSelfHealActor", {"Marker", "NW101"},
            {"CurrentConfig", currentConfig},
            {"TargetConfig", targetConfig});
    ...
    void TInvokeRequestHandlerActor::ReconfigStateStorage(const NKikimrBlobStorage::TStateStorageConfig& cmd) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "TInvokeRequestHandlerActor::ReconfigStateStorage", {"Marker", "NW67"},
...
#define PROCESS(NAME) \
    ...
    void TInvokeRequestHandlerActor::ReassignStateStorageNode(const TQuery::TReassignStateStorageNode& cmd) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "TInvokeRequestHandlerActor::ReassignStateStorageNode", {"Marker", "NW67"},
            {"config", config});
...
#define F(NAME) \
        ...
        F(SchemeBoard)
#undef F
        YDBLOG_COMP_DEBUG(BS_NODE, "TInvokeRequestHandlerActor::ReassignStateStorageNode new config ", {"Marker", "NW67"},
            {"config", config});
        StartProposition(&config);
    }
