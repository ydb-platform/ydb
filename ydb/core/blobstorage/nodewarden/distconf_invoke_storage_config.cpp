...
namespace NKikimr::NStorage {
    ...
    void TInvokeRequestHandlerActor::FetchStorageConfig(const TQuery::TFetchStorageConfig& request) {
        ...
        if (request.RequestorHostSize()) {
            ...
            if (!found) {
                YDBLOG_COMP_DEBUG(BS_NODE, "FetchStorageConfig: requestor put to pending queue", {"Marker", "NWDC62"},
                    {"SelfId", SelfId()},
                    {"Request", request});
                DetachQuery();
    ...
    void TInvokeRequestHandlerActor::Handle(TEvNodeWardenStorageConfig::TPtr ev) {
        ...
        for (const auto& node : config.GetAllNodes()) {
            if (requestorHosts.contains(node.GetHost()) && node.GetPort() == cmd.GetRequestorPort()) {
                YDBLOG_COMP_DEBUG(BS_NODE, "FetchStorageConfig: TEvNodeWardenStorageConfig received, host found", {"Marker", "NWDC82"},
                    {"SelfId", SelfId()});
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "FetchConfigConfig: TEvNodeWardenStorageConfig received, host still not found", {"Marker", "NWDC86"},
            {"SelfId", SelfId()});
    }

    void TInvokeRequestHandlerActor::HandleWakeup() {
        YDBLOG_COMP_DEBUG(BS_NODE, "FetchStorageConfig: timed out", {"Marker", "NWDC87"},
            {"SelfId", SelfId()});
    ...
    void TInvokeRequestHandlerActor::Handle(TEvTabletPipe::TEvClientConnected::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_COMP_DEBUG(BS_NODE, "received TEvClientConnected", {"Marker", "NWDC65"},
            {"SelfId", SelfId()},
            {"Status", msg.Status},
    ...
    void TInvokeRequestHandlerActor::Handle(TEvTabletPipe::TEvClientDestroyed::TPtr ev) {
        auto& msg = *ev->Get();
        YDBLOG_COMP_DEBUG(BS_NODE, "received TEvClientDestroyed", {"Marker", "NWDC79"},
            {"SelfId", SelfId()},
            {"ClientId", msg.ClientId},
            {"ServerId", msg.ServerId});
    ...
    void TInvokeRequestHandlerActor::Handle(TEvBlobStorage::TEvControllerConfigResponse::TPtr ev) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "received TEvControllerConfigResponse", {"Marker", "NWDC80"},
            {"SelfId", SelfId()},
            {"Response", response});
    ...
    void TInvokeRequestHandlerActor::Handle(TEvBlobStorage::TEvControllerDistconfResponse::TPtr ev) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "received TEvControllerDistconfResponse", {"Marker", "NWDC81"},
            {"SelfId", SelfId()},
            {"Record", getRecord()});
    ...
    void TInvokeRequestHandlerActor::Handle(TEvBlobStorage::TEvControllerValidateConfigResponse::TPtr ev) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "received TEvControllerValidateConfigResponse", {"Marker", "NWDC77"},
            {"SelfId", SelfId()},
