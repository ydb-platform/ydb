...
void TNodeWarden::RemoveDrivesWithBadSerialsAndReport(TVector<NPDisk::TDriveData>& drives, TStringStream& details) {
    ...
    for (auto it = toRemove; it != drives.end(); ++it) {
        ...
        YDBLOG_COMP_WARN(BS_NODE, "Bad serial number", {"Marker", "NW03"},
            {"Path", path},
...
TVector<NPDisk::TDriveData> TNodeWarden::ListLocalDrives() {
    ...
    try {
        ...
        if (google::protobuf::TextFormat::ParseFromString(raw, &MockDevicesConfig)) {
            ...
            YDBLOG_COMP_WARN(BS_NODE, "Error parsing mock devices protobuf from file", {"Marker", "NW01"},
                {"Path", MockDevicesPath});
        }
    } catch (...) {
        YDBLOG_COMP_INFO(BS_NODE, "Unable to find mock devices file", {"Marker", "NW90"},
            {"Path", MockDevicesPath});
    }
...
void TNodeWarden::StartInvalidGroupProxy() {
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "StartInvalidGroupProxy", {"Marker", "NW11"},
        {"GroupId", groupId});
...
void TNodeWarden::StopInvalidGroupProxy() {
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "StopInvalidGroupProxy", {"Marker", "NW15"},
        {"GroupId", groupId});
...
void TNodeWarden::StartRequestReportingThrottler() {
    YDBLOG_COMP_DEBUG(BS_NODE, "StartRequestReportingThrottler", {"Marker", "NW62"});
...
void TNodeWarden::PassAway() {
    YDBLOG_COMP_DEBUG(BS_NODE, "PassAway", {"Marker", "NW25"});
...
void TNodeWarden::Bootstrap() {
    YDBLOG_COMP_DEBUG(BS_NODE, "Bootstrap", {"Marker", "NW26"});
    ...
    EnqueueSyncOp([this, cfg = Cfg](const TActorContext&) {
        ...
        return [=, this] {
            ...
            try {
                ...
                YDBLOG_COMP_INFO(BS_NODE, "Bootstrap", {"Marker", "NW07"},
                    {"Cache", proto});
                ...
                YDBLOG_COMP_INFO(BS_NODE, "Bootstrap failed to fetch cache", {"Marker", "NW16"},
...
void TNodeWarden::Handle(NPDisk::TEvSlayResult::TPtr ev) {
    ...
    YDBLOG_COMP_INFO(BS_NODE, "Handle(NPDisk::TEvSlayResult)", {"Marker", "NW28"},
        {"Msg", msg.ToString()},
    ...
    switch (msg.Status) {
        ...
        case NKikimrProto::ERROR:
            SlayInFlight.erase(it);
            YDBLOG_COMP_ERROR(BS_NODE, "Handle(NPDisk::TEvSlayResult) error", {"Marker", "NW29"},
                {"Msg", msg.ToString()});
...
void TNodeWarden::Handle(NPDisk::TEvChangeExpectedSlotCountResult::TPtr ev) {
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "Handle(NPDisk::TEvChangeExpectedSlotCountResult)", {"Marker", "NW108"},
        {"Msg", msg.ToString()});

    // For now, just log the result. In the future, we might want to track this or take action based on the result.
    if (msg.Status != NKikimrProto::OK) {
        YDBLOG_COMP_ERROR(BS_NODE, "ChangeExpectedSlotCount failed", {"Marker", "NW109"},
            {"Status", msg.Status},
...
void TNodeWarden::ProcessShredStatus(ui64 cookie, ui64 generation, std::optional<TString> error) {
    ...
    YDBLOG_COMP_DEBUG(BS_SHRED, "processing shred result from PDisk", {"Marker", "BSSN00"},
        {"Cookie", cookie},
        {"PDiskId", key},
...
void TNodeWarden::PersistConfig(std::optional<TString> mainYaml, ui64 mainYamlVersion, std::optional<TString> storageYaml,
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "persisting new configurations", {"Marker", "NW63"},
    ...
    EnqueueSyncOp([this, saveCtx](const TActorContext&) {
        ...
        try {
            ...
            YDBLOG_COMP_ERROR(BS_NODE, "Failed to create config store path", {"Marker", "NW91"},
                {"Error", e.what()});
            success = false;
        }
            ...
            try {
...

                {
                    ...
                    if (Chmod(tempPath.c_str(), S_IRUSR | S_IRGRP | S_IROTH) != 0) {
                        YDBLOG_COMP_ERROR(BS_NODE, "Failed to set permissions for temporary file", {"Marker", "NW92"},
                ...
                if (!NFs::Rename(tempPath, configPath)) {
                    YDBLOG_COMP_ERROR(BS_NODE, "Failed to rename temporary file", {"Marker", "NW53"},
                        {"Error", LastSystemErrorText()});
                    success = false;
                    return false;
                }
                ...
                YDBLOG_COMP_ERROR(BS_NODE, "Failed to save config file", {"Marker", "NW93"},
                    {"Error", e.what()});
                success = false;
                return false;
            }
        ...
        if (success && saveCtx->UpdateMain) {
            ...
            if (success) {
                YDBLOG_COMP_INFO(BS_NODE, "Yaml config saved", {"Marker", "NW94"});
            }
        }
        ...
        if (saveCtx->DeleteStorage) {
            ...
            if (success) {
                YDBLOG_COMP_INFO(BS_NODE, "Storage config saved", {"Marker", "NW95"});
            }
        }
...
void TNodeWarden::Handle(TEvBlobStorage::TEvControllerNodeServiceSetUpdate::TPtr ev) {
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "TEvControllerNodeServiceSetUpdate", {"Marker", "NW52"},
        {"Record", record});
    ...
    if (record.HasServiceSet()) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "Handle(TEvBlobStorage::TEvControllerNodeServiceSetUpdate)", {"Marker", "NW17"},
            {"Msg", record});
        ...
        for (ui32 pdiskId : request.GetPDiskIds()) {
            ...
            if (const auto it = LocalPDisks.find(key); it != LocalPDisks.end()) {
                ...
                auto issueShredRequestToPDisk = [&] {
                    ...
                    YDBLOG_COMP_DEBUG(BS_SHRED, "sending shred query to PDisk", {"Marker", "BSSN01"},
                        {"Cookie", cookie},
                        {"PDiskId", key},
                        {"ShredGeneration", generation});
                };
...
void TNodeWarden::SendVDiskReport(TVSlotId vslotId, const TVDiskID &vDiskId,
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "SendVDiskReport", {"Marker", "NW32"},
        {"VSlotId", vslotId},
        {"VDiskId", vDiskId},
        {"Phase", phase});
...
void TNodeWarden::Handle(TEvBlobStorage::TEvControllerUpdateDiskStatus::TPtr ev) {
    YDBLOG_COMP_TRACE(BS_NODE, "Handle(TEvBlobStorage::TEvControllerUpdateDiskStatus)", {"Marker", "NW38"});
...
void TNodeWarden::Handle(TEvPrivate::TEvSendDiskMetrics::TPtr&) {
    YDBLOG_COMP_TRACE(BS_NODE, "Handle(TEvPrivate::TEvSendDiskMetrics)", {"Marker", "NW39"});
...
void TNodeWarden::Handle(TEvPrivate::TEvUpdateNodeDrives::TPtr&) {
    YDBLOG_COMP_TRACE(BS_NODE, "Handle(TEvPrivate::UpdateNodeDrives)", {"Marker", "NW88"});
...
void TNodeWarden::Handle(TEvPrivate::TEvRetrySaveConfig::TPtr& ev) {
    YDBLOG_COMP_TRACE(BS_NODE, "Handle(TEvRetrySaveConfig)", {"Marker", "NW97"});
...
void TNodeWarden::SendDiskMetrics(bool reportMetrics) {
    YDBLOG_COMP_TRACE(BS_NODE, "SendDiskMetrics", {"Marker", "NW45"},
...
void TNodeWarden::Handle(TEvStatusUpdate::TPtr ev) {
    YDBLOG_COMP_DEBUG(BS_NODE, "Handle(TEvStatusUpdate)", {"Marker", "NW47"});
    auto *msg = ev->Get();
        ...
        if (!equals(InferPDiskSlotCountSettings, inferSettings)) {
            ...
            for (auto& [key, localPDisk] : LocalPDisks) {
                ...
                if (newExpectedSlotCount != localPDisk.ExpectedSlotCount ||
                    ...
                    YDBLOG_COMP_DEBUG(BS_NODE, "SendChangeExpectedSlotCount from config notification", {"Marker", "NW112"},
                        {"PDiskId", key.PDiskId},
...
bool NKikimr::ObtainTenantKey(TEncryptionKey *key, const NKikimrProto::TKeyConfig& keyConfig) {
    if (keyConfig.KeysSize()) {
        ...
        YDBLOG_COMP_INFO(BS_NODE, "No Keys in KeyConfig! Encrypted group DsProxies will not start", {"Marker", "NW66"});
        return false;
    }
}
...
bool NKikimr::ObtainPDiskKey(NPDisk::TMainKey *mainKey, const NKikimrProto::TKeyConfig& keyConfig) {
    ...
    *mainKey = NPDisk::TMainKey{};
    ...
    if (!keysSize) {
        YDBLOG_COMP_INFO(BS_NODE, "No Keys in PDiskKeyConfig! Encrypted pdisks will not start", {"Marker", "NW69"});
