...
void TNodeWarden::EstablishPipe() {
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "EstablishPipe", {"Marker", "NW21"},
...
void TNodeWarden::Handle(TEvTabletPipe::TEvClientConnected::TPtr ev) {
    ...
    if (msg->Status != NKikimrProto::OK) {
        YDBLOG_COMP_ERROR(BS_NODE, "TEvTabletPipe::TEvClientConnected", {"Marker", "NW71"},
            {"Status", msg->Status},
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "TEvTabletPipe::TEvClientConnected OK", {"Marker", "NW05"},
            {"ClientId", msg->ClientId},
            {"ServerId", msg->ServerId},
            {"TabletId", msg->TabletId},
            {"PipeClientId", PipeClientId});
...
void TNodeWarden::Handle(TEvTabletPipe::TEvClientDestroyed::TPtr ev) {
    ...
    YDBLOG_COMP_ERROR(BS_NODE, "Handle(TEvTabletPipe::TEvClientDestroyed)", {"Marker", "NW42"},
        {"ClientId", msg->ClientId},
        {"ServerId", msg->ServerId},
        {"TabletId", msg->TabletId},
        {"PipeClientId", PipeClientId});
...
void TNodeWarden::SendRegisterNode() {
    YDBLOG_COMP_DEBUG(BS_NODE, "SendRegisterNode", {"Marker", "NW20"});
...
void TNodeWarden::SendInitialGroupRequests() {
    ...
    if (!groupIds.empty()) {
        YDBLOG_COMP_DEBUG(BS_NODE, "SendInitialGroupRequests", {"Marker", "NW22"},
