...
void TNodeWarden::ApplyServiceSet(const NKikimrBlobStorage::TNodeWardenServiceSet &serviceSet, bool isStatic,
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "ApplyServiceSet", {"Marker", "NW18"},
        {"IsStatic", isStatic},
...
#define FETCH_CONFIG(PART, PROTO) \
    ...
    YDBLOG_COMP_DEBUG(BS_NODE, "ApplyStateStorageConfig", {"Marker", "NW55"},
                ...
                for (const auto& ring : ringGroup.Rings) {
                    for (const auto& replicaId : ring.Replicas) {
                        if (replicaId.NodeId() == LocalNodeId) {
                            YDBLOG_COMP_INFO(BS_NODE, "Local replica found", {"Marker", "NW54"},
                                {"Component", comp},
                                {"ReplicaId", replicaId});
                ...
                for (ui32 index = 0; index < ring.Replicas.size(); ++index) {
                    if (const TActorId& replicaId = ring.Replicas[index]; replicaId.NodeId() == LocalNodeId) {
                        if (!localActorIds.contains(replicaId) && !newActorIds.contains(replicaId)) {
                            YDBLOG_COMP_INFO(BS_NODE, "starting state storage new replica", {"Marker", "NW08"},
                                {"Component", comp},
                                {"ReplicaId", replicaId},
                                {"Index", index},
                                {"Config", *info});
    ...
    // reconfigure proxy
    YDBLOG_COMP_INFO(BS_NODE, "updating state storage proxy configuration", {"Marker", "NW50"});
    ...
    for (const auto& replicaId : localActorIds) {
        if (!newActorIds.contains(replicaId)) {
            YDBLOG_COMP_INFO(BS_NODE, "terminating useless state storage replica", {"Marker", "NW43"},
                {"ReplicaId", replicaId});
...
void TNodeWarden::Handle(TEvNodeWardenNotifyConfigMismatch::TPtr ev) {
    ...
    auto *msg = ev->Get();
    YDBLOG_COMP_INFO(BS_NODE, "TEvNodeWardenNotifyConfigMismatch: NodeId: " << msg->NodeId
        << " ClusterStateGeneration: " << msg->ClusterStateGeneration << " ClusterStateGuid: " << msg->ClusterStateGuid, {"Marker", "NW51"});
}
