...
namespace NKikimr::NStorage {
    ...
    void TNodeWarden::DestroyLocalVDisk(TVDiskRecord& vdisk) {
        YDBLOG_COMP_INFO(BS_NODE, "DestroyLocalVDisk", {"Marker", "NW35"},
    ...
    void TNodeWarden::PoisonLocalVDisk(TVDiskRecord& vdisk) {
        YDBLOG_COMP_INFO(BS_NODE, "PoisonLocalVDisk", {"Marker", "NW00"},
            {"VDiskId", vdisk.GetVDiskId()},
            {"VSlotId", vdisk.GetVSlotId()},
    ...
    void TNodeWarden::StartLocalVDiskActor(TVDiskRecord& vdisk) {
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "StartLocalVDiskActor", {"Marker", "NW23"},
        ...
        YDBLOG_COMP_DEBUG(BS_NODE, "StartLocalVDiskActor done", {"Marker", "NW24"},
            {"VDiskId", vdisk.GetVDiskId()},
            {"VSlotId", vslotId},
            {"PDiskGuid", pdiskGuid},
            {"DDisk", ddisk},
    ...
    void TNodeWarden::Slay(TVDiskRecord& vdisk) {
        const TVSlotId vslotId = vdisk.GetVSlotId();
        YDBLOG_COMP_INFO(BS_NODE, "Slay", {"Marker", "NW33"},
            {"VDiskId", vdisk.GetVDiskId()},
            {"VSlotId", vdisk.GetVSlotId()},
            {"SlayInFlight", SlayInFlight.contains(vslotId)});
        if (!SlayInFlight.contains(vslotId)) {
    ...
    void TNodeWarden::Handle(TEvBlobStorage::TEvDropDonor::TPtr ev) {
        ...
        YDBLOG_COMP_INFO(BS_NODE, "TEvDropDonor", {"Marker", "NW34"},
            {"VSlotId", vslotId},
            {"VDiskId", msg->VDiskId});
