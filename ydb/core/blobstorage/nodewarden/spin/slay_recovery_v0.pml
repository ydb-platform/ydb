 ...
 * Version v0 models NodeWarden before action-aware SlayInFlight recovery.
 *
...
/* v0 NOTREADY schedules a raw TEvSlay to the PDisk service rather than a
 * callback to NodeWarden.  These events remain semantically active even after
 * their round becomes stale at NodeWarden. */
chan old_direct_slay_timer = [RAW_SLAY_TIMER_CAP] of {
    byte, byte, byte
};
        ...
        nw_processing_event = true;
            ...
            :: message == SERVICE_SET_WIPE ->
                ...
                :: second == nw_epoch && third == nw_known_bsc_epoch ->
                    ...
                    :: !nw_slay_inflight ->
                        ...
                        /* Old SlayInFlight contains only owner round. */
            ...
            :: message == SERVICE_SET_DESTROY ->
                ...
                :: second == nw_epoch && third == nw_known_bsc_epoch ->
                    ...
                    :: !nw_slay_inflight ->
                        ...
                        send_slay(first)
                    :: nw_slay_inflight ->
                        printf("[NodeWarden] old code keeps the existing round only\n")
                    fi;
            ...
            :: message == PDISK_RESTARTED ->
                ...
                :: nw_pdisk_restart_inflight ->
                    ...
                    :: nw_local_vdisk && nw_slay_inflight ->
                        printf("[NodeWarden] old callback replays only LocalVDisks\n");
                        send_slay(nw_current_generation)
                    :: !nw_local_vdisk && nw_slay_inflight ->
                        printf("[ERROR] deleted VDisk is absent from LocalVDisks; no replay\n")
                    :: else -> skip
                    fi
                    if
                    :: nw_local_vdisk && !nw_slay_inflight ->
            ...
            :: message == RETRY_SLAY ->
                ...
                :: second == nw_epoch && nw_slay_inflight && first == nw_round ->
                    ...
                    /* v0 never schedules TEvRetrySlay; it uses the raw timer
                     * process below.  Retain defensive stale-event handling. */
                    release_round(first)
                :: else ->
            ...
            :: message == SLAY_NOTREADY ->
                ...
                :: else ->
                    ...
                    nw_current_result_available = false;
                    nw_current_attempt_outstanding = false;
                    /* Old code allocates and stores the replacement owner
                     * round before its delayed direct TEvSlay is delivered. */
                    nfull(old_direct_slay_timer);
                    {
                        byte old_round;
                        byte fresh_round;
                        old_round = nw_round;
                        allocate_fresh_round(fresh_round);
                        nw_round = fresh_round;
                        release_round(old_round); /* SlayInFlight old round. */
                        release_round(first);     /* NOTREADY message. */
                        retain_round(fresh_round);
                        old_direct_slay_timer!fresh_round(second, third)
                    }
                fi
            ...
            :: message == SLAY_OK || message == SLAY_ALREADY || message == SLAY_RACE ->
                ...
                :: else ->
                    ...
                    nw_current_result_available = false;
                    nw_current_attempt_outstanding = false;
                    completed_generation = second;
                    if
                    :: nw_local_vdisk -> completed_action = ACTION_WIPE
                    :: !nw_local_vdisk -> completed_action = ACTION_DESTROY
                    fi
...
/* v0 delayed Slay bypasses NodeWarden.  It is deliberately delivered even
 * when its round is stale: it may still remove the owner or produce RACE. */
proctype OldDirectSlayTimer() {
    byte round;
    byte generation;
    byte sender_nw_epoch;

end_old_direct_slay_timer:
    do
    :: atomic {
        len(old_direct_slay_timer) > 0 && nfull(pdisk_actor_mailbox);
        old_direct_slay_timer?round(generation, sender_nw_epoch);
        if
        :: sender_nw_epoch == nw_epoch && nw_slay_inflight && round == nw_round ->
            nw_last_issue_pdisk_epoch = pdisk_epoch;
            nw_current_result_available = false;
            nw_current_attempt_outstanding = true
        :: else -> skip
        fi;
        /* Transfer the raw-timer reference to the PDisk actor mailbox. */
        printf("[v0 raw timer] deliver TEvSlay roundToken=%d pdiskEpoch=%d\n",
            round, pdisk_epoch);
        pdisk_actor_mailbox!SLAY(round, generation, pdisk_epoch, sender_nw_epoch)
    }
    od
}
