...
proctype NodeWarden() {
    ...
    :: bsc_to_nw?SERVICE_SET_DESTROY(request_id) ->
        atomic {
            ...
            :: !nw_slay_inflight ->
                ...
                send_slay(request_id)
            :: else ->
                /* Old code cannot retain or upgrade the operation action. */
                printf("[NodeWarden] ignore DESTROY because a slay is already in flight\n")
            fi;
    ...
    :: atomic {
        ...
        :: nw_pdisk_restart_inflight ->
            ...
            /* Old restart recovery only sees VDisks still in LocalVDisks. */
            if
            :: nw_local_vdisk && nw_slay_inflight ->
                printf("[NodeWarden] PDisk restarted; replay through LocalVDisks\n");
                send_slay(nw_current_vdisk_id)
            :: nw_slay_inflight ->
                printf("[ERROR] PDisk restarted, but DELETE slay is orphaned outside LocalVDisks\n")
            :: else ->
                printf("[NodeWarden] PDisk restarted after slay completion\n")
            fi
    ...
    :: pdisk_to_nw?SLAY_OK(reply_round, reply_id) ->
        ...
        :: nw_slay_inflight && reply_round == nw_round ->
            atomic {
                /* Old code infers action from LocalVDisks at result time. */
                if
                :: !nw_local_vdisk ->
                    printf("[NodeWarden] infer DESTROY from missing LocalVDisk\n");
                    nw_to_bsc!DESTROYED(reply_id)
                :: nw_local_vdisk ->
                    printf("[NodeWarden] infer WIPE from existing LocalVDisk\n");
                    nw_to_bsc!WIPED(reply_id);
