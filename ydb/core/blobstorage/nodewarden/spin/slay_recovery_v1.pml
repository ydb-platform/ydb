 ...
 *
 * Version v1: fixed NodeWarden. Action-aware state is replayed directly from
 * SlayInFlight, independently of LocalVDisks.
 */
mtype = {
    SERVICE_SET_DESTROY,
    POISON,
...
proctype NodeWarden() {
    ...
    :: bsc_to_nw?SERVICE_SET_DESTROY ->
        ...
        :: !nw_slay_inflight ->
            atomic {
                ...
                nw_round++;
                /* New state retains VDiskId and action after record removal. */
                nw_has_slay_identity = true;
    ...
    :: atomic {
        ...
        :: nw_pdisk_restart_inflight ->
            ...
            /* Fixed code iterates SlayInFlight, whose state owns the identity. */
            if
            :: nw_slay_inflight && nw_has_slay_identity ->
                nw_round++;
                printf("[NodeWarden] PDisk restarted; replay TEvSlay from SlayInFlight round=%d\n", nw_round);
                nw_to_pdisk!SLAY(nw_round)
            :: else ->
