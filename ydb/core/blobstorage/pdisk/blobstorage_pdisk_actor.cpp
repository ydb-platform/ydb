...
public:
    ...
    void InitHandle(NPDisk::TEvYardResize::TPtr &ev) {
        Y_UNUSED(ev);
        // const NPDisk::TEvChangeSlotSizeUnits &evChangeSlotSizeUnits = *ev->Get();
        // PDisk->Mon.YardSlay.CountRequest();
        // TStringStream str;
        // str << PCtx->PDiskLogPrefix << "is still initializing, please wait";
        // Send(ev->Sender, new NPDisk::TEvSlayResult(NKikimrProto::NOTREADY, 0,
        //             evChangeSlotSizeUnits.VDiskId, evChangeSlotSizeUnits.SlayOwnerRound, evSlay.PDiskId, evChangeSlotSizeUnits.VSlotId, str.Str()));
        // PDisk->Mon.YardSlay.CountResponse();
    }
    ...
    void ErrorHandle(NPDisk::TEvCheckSpace::TPtr &ev) {
        ...
        Send(ev->Sender, new NPDisk::TEvCheckSpaceResult(NKikimrProto::CORRUPTED, 0, 0, 0, 0, 0, 0, StateErrorReason));
