...
TEncryptionThread::TEncryptionThread(TPDiskMon* const mon, TString name)
    : Name(name),
    Counter(mon->PDiskGroup->GetCounter(Name + "CPU", true))
{}
...
TEncryptionThreads::TEncryptionThreads(TPDiskMon* const mon, size_t threadsCount) : Mon(mon) {
    SetThreadCount(threadsCount);
}

void TEncryptionThreads::SetThreadCount(size_t threadsCount) {
    threadsCount = std::min(threadsCount, TEncryptionThreads::MAX_THREAD_COUNT);
    for (size_t i = 0; i < threadsCount; i++) {
        Threads.push_back(MakeHolder<TEncryptionThread>(Mon, TStringBuilder() << "PdEncrypt_" << i));
        Threads.back()->Start();
    }
