...
public:
    TEncryptionThread(TPDiskMon* const mon, TString name);
...
public:
    static constexpr size_t MAX_THREAD_COUNT = 3;
    TVector<THolder<TEncryptionThread>> Threads;
    size_t AvailableThreads;
    TPDiskMon* const Mon;

    TEncryptionThreads(TPDiskMon* const mon, size_t threadsCount);
