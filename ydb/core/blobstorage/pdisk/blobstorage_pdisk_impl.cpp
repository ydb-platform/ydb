...
bool TPDisk::ChunkWritePiece(TChunkWrite *evChunkWrite, ui32 pieceShift, ui32 pieceSize) {
    ...
    if (!encrypted) {
        ...
        // Cerr << __PRETTY_FUNCTION__ << Endl << TString((char*)buff, newSize).Quote() << Endl;
        BlockDevice->PwriteAsync(buff, newSize, diskOffset, comp.Release(), evChunkWrite->ReqId, {});
...
TPDisk::EChunkReadPieceResult TPDisk::ChunkReadPiece(TIntrusivePtr<TChunkRead> &read, ui64 pieceCurrentSector,
    ...
    P_LOG(PRI_NOTICE, BPD01, "Read chunk", (Encrypted, read->ChunkEncrypted), (ChunkIdx, read->ChunkIdx),
    ...
    switch (request->GetType()) {
        ...
        case ERequestType::RequestChunkRead:
        {
            ...
            *Mon.QueueBytes -= read->Size;
            ...
            //if (state.CommitState == TChunkState::DATA_RESERVED) {
            //    err << "chunk is empty and has CommitState# " << state.CommitState;
            //    SendChunkReadError(read, err, NKikimrProto::CORRUPTED);
            //    return false;
            //}
            ui64 offset = 0;
            ...
            request->JobKind = NSchLab::JobKindRead;
            read->ChunkEncrypted = !Cfg->PlainDataChunks;
            if (!read->ChunkEncrypted) {
        ...
        case ERequestType::RequestChunkWrite:
        {
            ...
            *Mon.QueueBytes -= size;
            ...
            ev.ChunkEncrypted = !Cfg->PlainDataChunks;
