...
bool TPDisk::ChunkWritePiece(TChunkWrite *evChunkWrite, ui32 pieceShift, ui32 pieceSize) {
    ...
    ui64 sectorOffset = 0;
    ui64 lastSectorIdx;
    if (!ParseSectorOffset(Format, PCtx->ActorSystem, PDiskId, evChunkWrite->Offset + evChunkWrite->BytesWritten,
...
TPDisk::EChunkReadPieceResult TPDisk::ChunkReadPiece(TIntrusivePtr<TChunkRead> &read, ui64 pieceCurrentSector,
    ...
    ui64 lastSector;
    ui64 sectorOffset;
    bool isOk = ParseSectorOffset(Format, PCtx->ActorSystem, PDiskId,
...
void TPDisk::ChunkLock(TChunkLock &evChunkLock) {
    ...
    P_LOG(PRI_INFO, BPD01, "Locked chunks from owner", (ChunksCount, result->LockedChunks.size()), (Owner, (ui32)owner));
    ...
    switch (request->GetType()) {
        ...
        case ERequestType::RequestChunkRead:
        {
            ...
            *Mon.QueueBytes -= read->Size;
            ...
            ui64 offset = 0;
            if (!ParseSectorOffset(Format, PCtx->ActorSystem, PDiskId, read->Offset, read->Size,
