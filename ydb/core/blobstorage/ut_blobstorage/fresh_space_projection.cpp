...
struct TFreshProjectionOosEnv {
    ...
    explicit TFreshProjectionOosEnv(TOosSettings oos = {})
        : Oos(std::move(oos))
        , Env({
            .NodeCount = 3,
    ...
    // Stands in for the writes a system tablet does: same group, same fail model,
    // SYSTEM DataKind.
...
namespace {
    ...
    void CheckFreshAbortReleasesChunks(bool restartBeforeRetry) {
        ...
        env.Runtime->FilterFunction = [&](ui32 nodeId, std::unique_ptr<IEventHandle>& ev) {
            switch (ev->GetTypeRewrite()) {
                ...
                case TEvBlobStorage::EvHullCommitFinished:
                    if (ev->Get<THullCommitFinished>()->Type == THullCommitFinished::CommitFreshAborted) {
                        ++cleanupCommits;
                    }
                    break;
            }
        ...
        UNIT_ASSERT_VALUES_EQUAL(cleanupCommits, 3);
        ...
        env.Sim(TDuration::Seconds(30));
        // Cleanup commits must preserve the old Fresh records until a successful
...
Y_UNIT_TEST_SUITE(VDiskFreshSpaceProjection) {
    ...
    Y_UNIT_TEST(UserPutsStopBeforeOrange) {
        TFreshProjectionOosEnv env(TinyDiskSettings());
