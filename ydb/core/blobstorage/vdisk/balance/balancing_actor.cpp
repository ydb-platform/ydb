    ...
    struct TBatchManager {
        ...
        void Handle(NActors::TEvents::TEvCompleted::TPtr ev) {
            if (ev->Sender == SenderId) {
                ...
                YDBLOG_WARN("Unexpected actor id",
                    {"Marker", "BSVB05"},
                    {"Id", ev->Sender});
            }
        }
    ...
    private:
        ...
        void Handle(TEvBlobStorage::TEvVStatusResult::TPtr ev) {
            ...
            if (status != NKikimrProto::EReplyStatus::OK || !replicated || isReadOnly) {
                YDBLOG_INFO(VDISKP(Ctx->VCtx, "VDisk is not ready. Stop balancing"),
                    {"Marker", "BSVB02"},
                    {"VDiskId", vdiskId},
        ...
        void CollectKeys() {
            if (!ReadyToBalance()) {
                ...
                YDBLOG_INFO(VDISKP(Ctx->VCtx, "Not all vdisks are connected, balancing should work only for full groups"),
                    {"Marker", "BSVB11"},
            ...
            for (ui32 cnt = 0; It.Valid(); It.Next(), ++cnt) {
                if (cnt % 128 == 127 && TDuration::Seconds(timer.Passed()) > Ctx->Cfg.JobGranularity) {
                    ...
                    YDBLOG_DEBUG(VDISKP(Ctx->VCtx, "Collect keys"),
                        {"Marker", "BSVB04"},
                        {"collected", cnt},
                        {"passed", timer.Passed()});
                ...
                if (Ctx->Cfg.EnableDelete && TryDeleteParts.Size() < Ctx->Cfg.MaxToDeletePerEpoch) {
                    if (auto partsToDelete = merger.Ingress.LocalParts(top.GType) & delMask; !partsToDelete.Empty()) {
                        ...
                        for (ui8 partIdx = partsToDelete.FirstPosition(); partIdx < partsToDelete.GetSize(); partIdx = partsToDelete.NextPosition(partIdx)) {
                            ...
                            YDBLOG_DEBUG(VDISKP(Ctx->VCtx, "Delete"),
                                {"Marker", "BSVB10"},
            ...
            YDBLOG_DEBUG(VDISKP(Ctx->VCtx, "Keys collected"),
                {"Marker", "BSVB08"},
        ...
        void ContinueBalancing() {
            ...
            if (SendOnMainParts.Empty() && TryDeleteParts.Empty()) {
                ...
                YDBLOG_INFO(VDISKP(Ctx->VCtx, "Balancing completed"),
                    {"Marker", "BSVB03"});
            ...
            YDBLOG_INFO(VDISKP(Ctx->VCtx, "Ask repl token to continue balancing"),
                {"Marker", "BSVB01"},
                {"SelfId", SelfId()},
        ...
        void ScheduleJobQuant() {
            ...
            YDBLOG_INFO(VDISKP(Ctx->VCtx, "Schedule job quant"),
                {"Marker", "BSVB02"},
        ...
        void Handle(NActors::TEvents::TEvCompleted::TPtr ev) {
            YDBLOG_INFO(VDISKP(Ctx->VCtx, "TEvCompleted"),
                {"Marker", "BSVB04"},
                {"Type", ev->Type});
            ...
            if (StartTime + Ctx->Cfg.EpochTimeout < TlsActivationContext->Now()) {
                ...
                YDBLOG_INFO(VDISKP(Ctx->VCtx, "Epoch timeout"),
                    {"Marker", "BSVB04"});
        ...
        void Handle(TEvBalancingSendPartsOnMain::TPtr ev) {
            ...
            YDBLOG_INFO(VDISKP(Ctx->VCtx, "Received from deleter TEvBalancingSendPartsOnMain"),
                {"Marker", "BSVB05"},
        ...
        void Handle(NActors::TEvents::TEvUndelivered::TPtr ev) {
            if (ev.Get()->Type == TEvReplToken::EventType) {
                YDBLOG_ERROR(VDISKP(Ctx->VCtx, "Ask repl token msg not delivered"),
                    {"Marker", "BSVB06"},
                    {"SelfId", SelfId()},
                    {"PDiskId", Ctx->VDiskCfg->BaseInfo.PDiskId});
                ContinueBalancing();
        ...
        void Handle(TEvProxyQueueState::TPtr ev) {
            ...
            if (ev->Get()->IsConnected) {
                YDBLOG_DEBUG(VDISKP(Ctx->VCtx, "VDisk connected"),
                    {"Marker", "BSVB07"},
                    {"VDiskId", vdiskId.ToString()});
                ConnectedVDisks.insert(vdiskId);
            } else {
                YDBLOG_DEBUG(VDISKP(Ctx->VCtx, "VDisk disconnected"),
                    {"Marker", "BSVB09"},
                    {"VDiskId", vdiskId.ToString()});
        ...
        void Stop(TDuration timeoutBeforeNextLaunch) {
            YDBLOG_INFO(VDISKP(Ctx->VCtx, "Stop balancing"),
                {"Marker", "BSVB12"},
                {"SendOnMainParts", SendOnMainParts.Data.size()},
                {"TryDeleteParts", TryDeleteParts.Data.size()},
