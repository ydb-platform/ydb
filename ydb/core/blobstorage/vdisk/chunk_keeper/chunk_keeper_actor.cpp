...
public:
    ...
    void Bootstrap() {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Bootstrap TChunkKeeperActor"), {"Marker", "BSCK07"},
...
private:
    ...
    void Handle(const TEvChunkKeeperAllocate::TPtr& ev) {
        ...
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvChunkKeeperAllocate"), {"Marker", "BSCK02"},
            {"Subsystem", subsystem});
        { // validation
            if (ReadOnly) {
                TString errorReason = TStringBuilder() << "ChunkKeeper is in read-only mode, Subsystem# " << subsystem;
                YDBLOG_COMP_ERROR(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Bad allocation request"), {"Marker", "BSCK04"},
    ...
    void Handle(const TEvChunkKeeperFree::TPtr& ev) {
        ...
        ui32 subsystem = static_cast<ui32>(ev->Get()->Subsystem);
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvChunkKeeperFree"), {"Marker", "BSCK03"},
            {"ChunkIdx", chunkIdx},
            {"Subsystem", subsystem});
        ...
        { // validation
            if (ReadOnly) {
                ...
                YDBLOG_COMP_ERROR(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Bad deallocation request"), {"Marker", "BSCK04"},
                    {"ErrorReason", errorReason});
            ...
            if (it->second.Subsystem != subsystem) {
                TString errorReason = TStringBuilder() << "Chunk with idx# " << chunkIdx <<
                        ...
                        it->second.Subsystem;
                YDBLOG_COMP_ERROR(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Bad deallocation request"), {"Marker", "BSCK05"},
                    {"ErrorReason", errorReason});
                Send(ev->Sender, new TEvChunkKeeperFreeResult(chunkIdx, NKikimrProto::ERROR, errorReason));
                return;
            }
    ...
    void Handle(const TEvChunkKeeperDiscover::TPtr& ev) {
        ...
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvChunkKeeperDiscover"), {"Marker", "BSCK06"},
            {"Subsystem", subsystem},
            {"DiscoveredChunks", discoveredChunks});
    }

    void Handle(const NPDisk::TEvCutLog::TPtr& ev) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvCutLog"), {"Marker", "BSCK18"},
    ...
    void ProcessRequestAllocate(TRequestAllocate request) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Process allocation request"), {"Marker", "BSCK09"},
    ...
    void Handle(const NPDisk::TEvChunkReserveResult::TPtr& ev) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvChunkReserveResult"), {"Marker", "BSCK10"},
        ...
        default:
            YDBLOG_COMP_WARN(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Unsuccessful ChunkReserve request"), {"Marker", "BSCK21"},
    ...
    void HandleLogResultAllocate(const NPDisk::TEvLogResult::TPtr& ev) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvLogResult while processing"
                " allocation request"), {"Marker", "BSCK11"},
            {"Event", ev->Get()->ToString()});
        CHECK_PDISK_RESPONSE(Ctx.LogCtx->VCtx, ev, TActivationContext::AsActorContext());
        ...
        default:
            YDBLOG_COMP_WARN(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Unsuccessful Log request"), {"Marker", "BSCK22"},
                {"ErrorReason", ev->Get()->ErrorReason});
    ...
    void ProcessRequestFree(TRequestFree request) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Processing deallocation request"), {"Marker", "BSCK13"},
            {"Subsystem", request.Subsystem},
        ...
        if (it == Committed->Chunks.end() || it->second.Subsystem != request.Subsystem) {
            TString errorReason = TStringBuilder() << "Chunk doesn't belong to this subsystem, possible double-free, "
                    "ChunkIdx# " << request.ChunkIdx;
            YDBLOG_COMP_ERROR(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Race occurred"), {"Marker", "BSCK12"},
                {"ErrorReason", errorReason});
    ...
    void HandleLogResultFree(const NPDisk::TEvLogResult::TPtr& ev) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvLogResult while processing"
                " deallocation request"), {"Marker", "BSCK14"},
            {"Event", ev->Get()->ToString()});
        CHECK_PDISK_RESPONSE(Ctx.LogCtx->VCtx, ev, TActivationContext::AsActorContext());
        ...
        default:
            YDBLOG_COMP_WARN(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Unsuccessful Log request"), {"Marker", "BSCK23"},
                {"ErrorReason", ev->Get()->ErrorReason});
    ...
    void ProcessRequestCutLog(TRequestCutLog) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Processing cut log request"), {"Marker", "BSCK20"});
        Become(&TThis::StateProcessing);
        IssueCommit(NPDisk::TCommitRecord{});
    }

    void HandleLogResultCutLog(const NPDisk::TEvLogResult::TPtr& ev) {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Handle TEvLogResult while processing cut log request"), {"Marker", "BSCK17"},
            {"Event", ev->Get()->ToString()});
        CHECK_PDISK_RESPONSE(Ctx.LogCtx->VCtx, ev, TActivationContext::AsActorContext());
        ActiveRequest.reset();
        ...
        case NKikimrProto::OK:
            ...
            break;
        default:
            YDBLOG_COMP_WARN(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Unsuccessful Log request"), {"Marker", "BSCK24"},
                {"ErrorReason", ev->Get()->ErrorReason});
            break;
        }
        ProcessRequestQueue();
    ...
    void IssueCommit(NPDisk::TCommitRecord&& commitRecord) {
        ...
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Sending TEvLog"), {"Marker", "BSCK15"},
            {"Event", commitMsg->ToString()});
        Send(Ctx.LogCtx->LoggerId, commitMsg.release());
    }

    void SendCutLog() {
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Sending TEvVDiskCutLog"), {"Marker", "BSCK19"},
            {"CurEntryPointLsn", CurEntryPointLsn});
    ...
    TRcBuf SerializeEntryPoint() {
        ...
        YDBLOG_COMP_DEBUG(BS_CHUNK_KEEPER, VDISKP(Ctx.LogCtx->VCtx, "Printing entrypoint"), {"Marker", "BSCK25"},
