...
namespace NKikimr {
    ...
    struct TVMsgContext {
        ...
        TVMsgContext(ui32 recByteSize, NKikimrCapnProto::TMsgQoS::Reader msgQoS)
                : ClientId(NBackpressure::TQueueClientId(msgQoS))
                , RecByteSize(recByteSize)
                , MsgId(msgQoS.GetMsgId())
                , Cost(msgQoS.GetCost())
        ...
        , TEventWithRelevanceTracker
    {
        ...
        static void OutMsgQos(const TProtoMsgQoS &qos, TStringStream &str) {
            ...
            if (qos.HasMsgId()) {
                str << " MsgId# " << qos.GetMsgId().GetMsgId();
            }
            if (qos.HasCost()) {
            ...
            if (qos.HasWindow()) {
                ...
                if (window.HasExpectedMsgId()) {
                    str << " ExpectedMsgId# " << window.GetExpectedMsgId().GetMsgId();
                }
                if (window.HasFailedMsgId()) {
                    str << " FailedMsgId# " << window.GetFailedMsgId().GetMsgId();
                }
                str << "}";
            }
            str << "}";
    ...
    struct TEvBlobStorage::TEvVGet
        : TEventPB<TEvBlobStorage::TEvVGet, NKikimrCapnProto::TEvVGet::Builder, TEvBlobStorage::EvVGet>
        , TEventWithRelevanceTracker
    {
        TEvVGet() = default;
        ...
        bool Validate(TString& errorReason) {
            ...
            } else if (!Record.GetVDiskID().HasGroupID()) {
