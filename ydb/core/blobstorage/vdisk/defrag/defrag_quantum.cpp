...
namespace NKikimr {
    ...
    public:
        ...
        void Run() override {
            YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "defrag quantum start", {"Marker", "BSVDD00"},
                {"ActorId", SelfActorId});
            ...
            YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "defrag quantum end", {"Marker", "BSVDD06"},
                {"ActorId", SelfActorId});
        }
        ...
        void RunImpl() {
            ...
            if (!ChunksToDefrag) {
                YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "going to find chunks to defrag", {"Marker", "BSVDD07"},
                    {"ActorId", SelfActorId});
                ...
                while (findChunks.Scan(NDefrag::WorkQuantum)) {
                    if (GetCycleCountFast() >= endTime) {
                        YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix
                            << "timed out while finding chunks to defrag", {"Marker", "BSVDD08"},
                            {"ActorId", SelfActorId},
                            {"Stat", stat});
            ...
            if (*ChunksToDefrag || ChunksToDefrag->IsShred) {
                ...
                if (!isShred) {
                    YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix
                        << "commencing defragmentation", {"Marker", "BSVDD09"},
                        {"ActorId", SelfActorId},
                    ...
                    YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "locked chunks", {"Marker", "BSVDD11"},
                        {"ActorId", SelfActorId},
                    ...
                    YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix
                        << "commencing shredding", {"Marker", "BSVDD14"},
                        {"ActorId", SelfActorId},
                    ...
                    if (chunksToShred.empty()) {
                        YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "nothing to do", {"Marker", "BSVDD15"},
                            {"ActorId", SelfActorId},
                            {"Stat", stat});
                ...
                if (auto records = findRecords.GetRecordsToRewrite(); !records.empty()) {
                    ...
                    YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "rewriting records", {"Marker", "BSVDD12"},
                        {"ActorId", SelfActorId},
                ...
                if (DCtx->VCfg->GarbageThresholdToRunFullCompactionPerMille == 0) {
                    ...
                    if (auto records = findRecords.GetRecordsToRewrite(); !records.empty()) {
                        for (const auto& item : records) {
                            YDBLOG_COMP_WARN(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix
                                << "blob found again after rewriting", {"Marker", "BSVDD16"},
                                {"ActorId", SelfActorId},
                                {"Id", item.LogoBlobId},
                    ...
                    YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "compacting", {"Marker", "BSVDD13"},
                        {"ActorId", SelfActorId},
            ...
            YDBLOG_COMP_DEBUG(BS_VDISK_DEFRAG, DCtx->VCtx->VDiskLogPrefix << "quantum finished", {"Marker", "BSVDD15"},
                {"ActorId", SelfActorId},
                {"Stat", stat});
