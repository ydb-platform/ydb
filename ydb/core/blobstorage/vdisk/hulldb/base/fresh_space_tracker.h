...
namespace NKikimr {
    ...
    // What each Fresh segment still owes to a compaction. They are kept apart rather
    // than summed because a compaction takes one segment at a time and writes one sst
    // for it, so each one's leftovers round up to a chunk of their own.
    struct TFreshSpaceDebt {
        ui64 OldBytes = 0;
        ui64 DregBytes = 0;
        ui64 CurBytes = 0;
    };
    ...
    public:
        ...
        // of what the segments already hold and what is already in flight. Each segment
        // is compacted on its own and writes its own sst, so they are rounded up to
        // chunks separately: two segments of a handful of bytes each need two chunks,
        // not the one their byte total would suggest.
        ui64 GetProjectedChunks(ui64 admittedBytes, const TFreshSpaceDebt& debt) const {
            if (!Enabled) {
                return 0;
            }
            // Admitted writes that have not reached Fresh yet, and the candidate itself,
            // all land in the segment that is currently open for writing.
            const ui64 cur = SaturatingAdd(debt.CurBytes,
                SaturatingAdd(InFlightBytes.load(std::memory_order_relaxed), admittedBytes));
            return SaturatingAdd(CalculateChunks(debt.OldBytes),
                SaturatingAdd(CalculateChunks(debt.DregBytes), CalculateChunks(cur)));
        }
