...
namespace NKikimr {
    ...
    private:
        void Finish() {
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "finished replication job"), {"Marker", "BSVR01"},
                {"LastKey", LastKey},
                {"Eof", Eof});
            ...
            if (Phantoms.empty()) {
                ...
                YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "sending phantoms"), {"Marker", "BSVR06"},
        ...
        void Bootstrap() {
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "THullReplJobActor::Bootstrap"), {"Marker", "BSVR02"});
        ...
        void Handle(TEvReplPlanFinished::TPtr& ev) {
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "THullReplJobActor::Handle(TEvReplPlanFinished)"), {"Marker", "BSVR03"});
            ...
            if ((mon.ReplWorkUnitsRemaining() && ReplInfo->WorkUnitsTotal > (ui64)mon.ReplWorkUnitsRemaining()) ||
                ...
                YDBLOG_COMP_WARN(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "replication work added"), {"Marker", "BSVR36"},
        ...
        void HandleResume() {
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "THullReplJobActor::HandleResume"), {"Marker", "BSVR04"});
        ...
        bool MergeIteration() {
            ...
            if (!RecoveryMachineFinished) {
                ...
                YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "finished recovery machine"), {"Marker", "BSVR07"},
            ...
            if (!WriterFinished && Writer.GetState() != TReplSstStreamWriter::EState::STOPPED) {
                YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "finished writer"), {"Marker", "BSVR08"});
                Writer.Finish();
                WriterFinished = true;
                return true;
            }
            ...
            if (HugeBlobsInFlight != 0) {
                ...
                YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "huge blobs unwritten"), {"Marker", "BSVR09"},
        ...
        void Handle(TEvBlobStorage::TEvGetResult::TPtr ev) {
            YDBLOG_COMP_INFO(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "Received phantom validation reply"), {"Marker", "BSVR34"},
                {"Msg", ev->Get()->ToString()});
        ...
        void AddPhantomBlobRecord(const TRecoveryMachine::TPartSet& item, TIngress ingress, NMatrix::TVectorType partsToRecover) {
            YDBLOG_COMP_INFO(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "Sending phantom validation query"), {"Marker", "BSVR33"},
        ...
        void HandleYard(NPDisk::TEvChunkReserveResult::TPtr& ev) {
            CHECK_PDISK_RESPONSE(ReplCtx->VCtx, ev, ActorContext());
            YDBLOG_COMP_INFO(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "reserved chunks"), {"Marker", "BSVR10"},
        ...
        void Handle(TEvReplProxyNextResult::TPtr &ev) {
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix,
                "THullReplJobActor::Handle(TEvReplProxyNextResult)"), {"Marker", "BSVR11"});
            ...
            if (proxy->IsEof()) {
                YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix, "proxy finished"), {"Marker", "BSVR12"},
            ...
            YDBLOG_COMP_DEBUG(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix,
                "THullReplJobActor::Handle(TEvReplProxyNextResult)"), {"Marker", "BSVR13"},
        ...
        void Handle(TEvBlobStorage::TEvVPutResult::TPtr& ev) {
            ...
            switch (record.GetStatus()) {
                case NKikimrProto::OUT_OF_SPACE:
                    YDBLOG_COMP_ERROR(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix,
                            "PDisk out of space, delaying replication"), {"Marker", "BSVR43"},
                ...
                default: {  // TODO: Handle other reply statuses
                    if (flags.Check(PostponeReplicationThreshold)) {
                        YDBLOG_COMP_ERROR(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix,
                                "Available space is running low, delaying replication"), {"Marker", "BSVR41"},
                            {"StatusFlags", flags.ToString()});
                        SwitchToRetriableErrorState();
                    }
                    Merge();
        ...
        void CheckSpace(const TYardEventPtr& ev) {
            ...
            if (flags.Check(PostponeReplicationThreshold)) {
                YDBLOG_COMP_ERROR(BS_REPL, VDISKP(ReplCtx->VCtx->VDiskLogPrefix,
                        "Available space is running low, delaying replication"), {"Marker", "BSVR40"},
                    {"StatusFlags", flags.ToString()});
                SwitchToRetriableErrorState();
            }
        }
