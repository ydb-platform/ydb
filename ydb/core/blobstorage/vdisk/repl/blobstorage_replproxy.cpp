...
namespace NKikimr {
    ...
    namespace NRepl {
        ...
        void TVDiskProxy::HandleNext(TEvReplProxyNextResult::TPtr &ev) {
            ...
            RemoteNotReady = RemoteNotReady || ev->Get()->RemoteNotReady;
        }
        ...
        class TVDiskProxyActor : public TActorBootstrapped<TVDiskProxyActor> {
            ...
            void ProcessPendingRequests() {
                ...
                if (RequestFromVDiskProxyPending && Prefetch.Valid()) {
                    Send(Recipient, new TEvReplProxyNextResult(VDiskId, std::move(Prefetch), Stat, HasTransientErrors, RemoteNotReady));
                    Prefetch.Reset();
                    PrefetchDataSize = 0;
