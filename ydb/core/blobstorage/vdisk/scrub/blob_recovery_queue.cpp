...
namespace NKikimr {
    ...
    void TBlobRecoveryActor::Handle(TEvVGenerationChange::TPtr ev) {
        YDBLOG_COMP_INFO(BS_VDISK_SCRUB, VDISKP(LogPrefix, "received group generation change notification"), {"Marker", "VDS28"},
            {"SelfId", SelfId()},
            {"Msg", ev->Get()->ToString()});
        for (const auto& [vdiskId, queue] : Queues) {
    ...
    void TBlobRecoveryActor::Handle(TEvProxyQueueState::TPtr ev) {
        ...
        YDBLOG_COMP_INFO(BS_VDISK_SCRUB, VDISKP(LogPrefix, "BS_QUEUE state update"), {"Marker", "VDS29"},
            {"SelfId", SelfId()},
            {"VDiskId", it->first},
