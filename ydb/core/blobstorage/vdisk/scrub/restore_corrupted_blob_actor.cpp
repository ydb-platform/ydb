...
namespace NKikimr {
    ...
    public:
        ...
        void Bootstrap() {
            YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "bootstrapping TRestoreCorruptedBlobActor"), {"Marker", "VDS09"},
                {"SelfId", SelfId()});
        ...
        void Handle(TEvTakeHullSnapshotResult::TPtr ev) {
            ...
            if (ReadQ.empty()) { // nothing to read, issue restore queries
                ...
                for (auto& cmd : ReadQ) {
                    YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "sending read to PDisk"), {"Marker", "VDS18"},
                        {"SelfId", SelfId()},
        ...
        void Handle(NPDisk::TEvChunkReadResult::TPtr ev) {
            ...
            if (msg->Status == NKikimrProto::OK && msg->Data.IsReadable()) {
                ...
                YDBLOG_COMP_WARN(BS_VDISK_SCRUB, VDISKP(LogPrefix, "failed to read data from PDisk"), {"Marker", "VDS26"},
                    {"SelfId", SelfId()},
                    {"Location", cmd->Location},
                    {"BlobId", cmd->Item->BlobId},
                    {"Parts", cmd->Parts},
        ...
        void IssueQuery() {
            YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "IssueQuery"), {"Marker", "VDS00"},
                {"SelfId", SelfId()});
            ...
            for (size_t i = 0; i < Items.size(); ++i) {
                ...
                if (item.Status == NKikimrProto::UNKNOWN) {
                    ...
                    YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "IssueQuery item"), {"Marker", "VDS17"},
                        {"SelfId", SelfId()},
                        {"BlobId", item.BlobId},
        ...
        void Handle(TEvRecoverBlobResult::TPtr ev) {
            YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "Handle(TEvRecoverBlobResult)"), {"Marker", "VDS24"},
                {"SelfId", SelfId()});
            ...
            for (auto& item : ev->Get()->Items) {
                ...
                YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "Handle(TEvRecoverBlobResult) item"), {"Marker", "VDS43"},
                    {"SelfId", SelfId()},
                    {"BlobId", item.BlobId},
                    {"Status", item.Status},
                    {"PartsMask", item.PartsMask});
        ...
        void Handle(TEvBlobStorage::TEvVPutResult::TPtr ev) {
            YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "received TEvVPutResult"), {"Marker", "VDS37"},
                {"SelfId", SelfId()},
                {"Msg", ev->Get()->ToString()});
            ...
            switch (record.GetStatus()) {
                ...
                case NKikimrProto::VDISK_ERROR_STATE:
                    YDBLOG_COMP_ERROR(BS_VDISK_SCRUB, VDISKP(LogPrefix, "failed to restore blob"), {"Marker", "VDS10"},
                        {"SelfId", SelfId()},
                        {"Msg", ev->Get()->ToString()});
        ...
        void PassAway() override {
            YDBLOG_COMP_DEBUG(BS_VDISK_SCRUB, VDISKP(LogPrefix, "TRestoreCorruptedBlobActor terminating"), {"Marker", "VDS15"},
                {"SelfId", SelfId()});
