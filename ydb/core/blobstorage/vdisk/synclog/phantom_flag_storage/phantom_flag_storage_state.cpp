...
void TPhantomFlagStorageState::StartBuilding() {
    if (GType.BlobSubgroupSize() > MaxExpectedDisksInGroup) {
        YDBLOG_COMP_ERROR(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Attempted to start phantom flag storage building on unsupported configuration"), {"Marker", "BSPFS01"},
...
void TPhantomFlagStorageState::ProcessBlobRecordFromSyncLog(const TLogoBlobRec* blobRec, ui64 sizeLimit) {
    ...
    if (blobRec->Ingress.IsDoNotKeep(GType) &&
        ...
        YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Try to add DoNotKeepFlag flag to PhantomFlagStorage"), {"Marker", "BSPFS09"},
...
void TPhantomFlagStorageState::FinishInitialBuilding(TPhantomFlags&& flags, TPhantomFlagThresholds&& thresholds,
    ...
    if (IsPersistent) {
        ...
        YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Finish building"), {"Marker", "BSPFS06"},
...
void TPhantomFlagStorageState::Recover(TPhantomFlagStorageSnapshot&& snapshot) {
    YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Recovering PhantomFlagStorage"), {"Marker", "BSPFS10"});
    Building = false;
...
void TPhantomFlagStorageState::RequestSnapshot(TEvPhantomFlagStorageGetSnapshot::TPtr ev) const {
    if (IsPersistent) {
        ...
        YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Acquiring snapshot"), {"Marker", "BSPFS05"},
...
void TPhantomFlagStorageState::AdjustSize(ui64 sizeLimit) {
    ...
    if (newCapacity > MaxFlagsStoredCount) {
        ...
        YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Reserving additional space for PhantomFlagStorage"), {"Marker", "BSPFS03"},
        ...
        StoredFlags.reserve(newCapacity);
        YDBLOG_COMP_DEBUG(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Shrinking PhantomFlagStorage"), {"Marker", "BSPFS04"},
            {"OldCapacity", MaxFlagsStoredCount},
            {"NewCapacity", newCapacity},
            {"ActualCapacity", StoredFlags.capacity()},
...
bool TPhantomFlagStorageState::AddFlag(const TLogoBlobRec& blobRec) {
    if (StoredFlags.size() < StoredFlags.capacity()) {
        ...
        YDBLOG_COMP_INFO(BS_PHANTOM_FLAG_STORAGE, VDISKP(SlCtx->VCtx, "Cannot add flag to PhantomFlagStorage, memory limit reached"), {"Marker", "BSPFS02"},
