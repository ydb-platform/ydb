...
TLegacyCallResult CallChooseProxy(ui16 grpcPort, const TString& token = {}) {
...
TLegacyCallResult CallSchemeDescribe(ui16 grpcPort, const TString& token = {}) {
    auto stub = MakeStub(grpcPort);
...
TLegacyCallResult WaitChooseProxy(ui16 grpcPort, const TString& token = {}) {
    TLegacyCallResult result;
    const TInstant deadline = TInstant::Now() + CallTimeout;
    do {
        result = CallChooseProxy(grpcPort, token);
...
TLegacyCallResult WaitSchemeDescribe(ui16 grpcPort, const TString& token = {}) {
    TLegacyCallResult result;
    const TInstant deadline = TInstant::Now() + CallTimeout;
    do {
        result = CallSchemeDescribe(grpcPort, token);
        if (result.Status.error_code() != grpc::StatusCode::UNAVAILABLE) {
            return result;
        }
        Sleep(RetryDelay);
...
Y_UNIT_TEST_SUITE(TChooseProxyGrpc) {
    ...
    Y_UNIT_TEST(WithTokenWhenTokenRequired) {
        TChooseProxyServer env(/*enforceUserToken=*/true);
        AssertChooseProxyOk(WaitChooseProxy(env.GrpcPort, BuiltinToken));
    }
    ...
    Y_UNIT_TEST(SchemeDescribeWithoutTokenWhenTokenRequired) {
        TChooseProxyServer env(/*enforceUserToken=*/true);
        const auto result = WaitSchemeDescribe(env.GrpcPort);
        UNIT_ASSERT_VALUES_EQUAL(result.Status.error_code(), grpc::StatusCode::UNAUTHENTICATED);
    }
}
