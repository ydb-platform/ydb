...
public:
    ...
    bool CheckConfig(const NKikimrConsole::TConfigsConfig &config,
                     ...
                     TString &error);
    TValidateConfigResult ValidateYamlConfig(const TString& config, bool force = false, bool allowUnknownFields = false);

    void SendInReply(const TActorId& sender, const TActorId& icSession, std::unique_ptr<IEventBase> ev);
