...
bool TTenantsManager::DbLoadState(TTransactionContext &txc, const TActorContext &ctx)
{
    ...
    while (!registeredRowset.EndOfSet()) {
        ...
        if (tenant) {
            ...
            YDB_LOG_DEBUG_CTX(ctx, "Loaded registered unit",
                {"host", host},
                {"port", port},
                {"kind", kind},
                {"tenantPath", path});
        } else {
            YDB_LOG_CRIT_CTX(ctx, "Loaded registered unit for unknown tenant",
                {"host", host},
                {"port", port},
                {"kind", kind},
                {"tenantPath", path});
...
void TTenantsManager::DbRemoveComputationalUnits(TTenant::TPtr tenant,
                                                 ...
                                                 const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Remove computational units from database",
        {"tenantPath", tenant->Path},
        {"txId", tenant->TxId},
        {"issue", tenant->Issue});
...
void TTenantsManager::DbRemoveRegisteredUnit(TTenant::TPtr tenant,
                                             ...
                                             const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Remove registered computational unit from database",
        {"tenantPath", tenant->Path},
        {"host", host},
        {"port", port});
...
void TTenantsManager::DbUpdateComputationalUnit(TTenant::TPtr tenant,
                                                ...
                                                const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Remove computational unit from database",
        {"tenantPath", tenant->Path},
        {"kind", kind},
        {"zone", zone},
        {"count", count});
...
void TTenantsManager::DbUpdateRegisteredUnit(TTenant::TPtr tenant,
                                             ...
                                             const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Update registered computational unit in database",
        {"tenantPath", tenant->Path},
        {"host", host},
        {"port", port},
        {"kind", kind});
...
void TTenantsManager::DbUpdateSchemaOperationQuotas(TTenant::TPtr tenant,
                                                    ...
                                                    const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Update schema operation quotas",
        {"tenantPath", tenant->Path},
...
void TTenantsManager::DbUpdateDatabaseQuotas(TTenant::TPtr tenant,
                                             ...
                                             const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Update database quotas",
        {"tenantPath", tenant->Path},
        {"quotas", quotas.DebugString()});
...
void TTenantsManager::DbUpdateScaleRecommenderPolicies(TTenant::TPtr tenant,
                                                       ...
                                                       const TActorContext &ctx)
{
    YDB_LOG_TRACE_CTX(ctx, "Update scale recommender policies",
        {"tenantPath", tenant->Path},
...
void TTenantsManager::Handle(TEvPrivate::TEvPoolFailed::TPtr &ev, const TActorContext &ctx)
{
    ...
    if (pool->Worker != ev->Sender) {
        YDB_LOG_ERROR_CTX(ctx, "Ignoring TEvPrivate::TEvPoolFailed from outdated worker",
            {"poolName", pool->Config.GetName()},
            {"tenantPath", tenant->Path},
            {"issue", issue});
        return;
    ...
    if (tenant->State == TTenant::REMOVING_POOLS) {
        YDB_LOG_CRIT_CTX(ctx, "Couldn't delete storage pool",
            {"poolName", pool->Config.GetName()},
            {"tenantPath", tenant->Path},
            {"issue", issue});
        ...
        YDB_LOG_CRIT_CTX(ctx, "Couldn't update storage pool",
            {"poolName", pool->Config.GetName()},
            {"tenantPath", tenant->Path},
            {"issue", issue});
...
void TTenantsManager::Handle(TEvPrivate::TEvSubdomainFailed::TPtr &ev, const TActorContext &ctx)
{
    ...
    if (tenant->Worker != ev->Sender) {
        YDB_LOG_ERROR_CTX(ctx, "Ignoring TEvPrivate::TEvSubdomainFailed from outdated worker",
            {"tenantPath", tenant->Path},
            {"issue", issue});
        return;
    }
    ...
    if (tenant->IsRemoving()) {
        ...
        YDB_LOG_ERROR_CTX(ctx, "Cannot remove subdomain",
            {"tenantPath", tenant->Path},
            {"code", code},
            {"issue", issue});
    ...
    if (tenant->State == TTenant::CREATING_SUBDOMAIN) {
        YDB_LOG_ERROR_CTX(ctx, "Cannot create subdomain",
            {"tenantPath", tenant->Path},
            {"code", code},
            {"issue", issue});
        ...
        YDB_LOG_CRIT_CTX(ctx, "Cannot configure subdomain",
            {"tenantPath", tenant->Path},
            {"code", code},
            {"issue", issue});
...
void TTenantsManager::Handle(TEvPrivate::TEvSubdomainCreated::TPtr &ev, const TActorContext &ctx)
{
    ...
    if (tenant->Worker != ev->Sender) {
        YDB_LOG_DEBUG_CTX(ctx, "Ignoring created subdomain from outdated worker",
            {"tenantPath", tenant->Path},
            {"state", tenant->State});
        return;
    }
...
void TTenantsManager::Handle(TEvPrivate::TEvSubdomainKey::TPtr &ev, const TActorContext &ctx)
{
    ...
    if (tenant->Worker != ev->Sender) {
        YDB_LOG_DEBUG_CTX(ctx, "Ignoring subdomain key from outdated worker",
            {"tenantPath", tenant->Path},
            {"state", tenant->State});
        return;
    }
...
void TTenantsManager::Handle(TEvPrivate::TEvSubdomainReady::TPtr &ev, const TActorContext &ctx)
{
    ...
    if (tenant->IsRemoving()) {
        YDB_LOG_DEBUG_CTX(ctx, "Ignoring ready subdomain",
            {"tenantPath", tenant->Path},
            {"state", tenant->State});
        return;
    }
    ...
    if (tenant->Worker != ev->Sender) {
        YDB_LOG_DEBUG_CTX(ctx, "Ignoring ready subdomain from outdated worker",
            {"tenantPath", tenant->Path},
            {"state", tenant->State});
        return;
    }
