...
#include <ydb/library/disk_limiter/usage/config.h>
...
void AddExecutorPool(TCpuManagerConfig& cpuManager, const NKikimrConfig::TActorSystemConfig::TExecutor& poolConfig,
    const NKikimrConfig::TActorSystemConfig& systemConfig, ui32 poolId, const NKikimr::TAppData* appData) {
...
static TCpuManagerConfig CreateCpuManagerConfig(const NKikimrConfig::TActorSystemConfig& config, const NKikimr::TAppData* appData) {
...
static TInterconnectSettings GetInterconnectSettings(const NKikimrConfig::TInterconnectConfig& config, ui32 numNodes, ui32 numDataCenters) {
    ...
    if (config.HasMergePerPeerCounters() && !config.HasCounterMergeMode()) {
        mode = !config.GetMergePerPeerCounters() ? NKikimrConfig::TInterconnectConfig::NO_MERGE
               : numDataCenters > 1              ? NKikimrConfig::TInterconnectConfig::PER_DATA_CENTER
    ...
    auto readFile = [](std::optional<TString> value, std::optional<TString> path, const char* name) {
        if (value) {
            ...
            try {
                ...
                ythrow yexception() << "failed to read " << name << " file '" << *path << "': " << ex.what();
            }
        }
        return TString();
    };
    result.Certificate = readFile(config.HasCertificate() ? std::make_optional(config.GetCertificate()) : std::nullopt,
        config.HasPathToCertificateFile() ? std::make_optional(config.GetPathToCertificateFile()) : std::nullopt, "certificate");
    result.PrivateKey = readFile(config.HasPrivateKey() ? std::make_optional(config.GetPrivateKey()) : std::nullopt,
        config.HasPathToPrivateKeyFile() ? std::make_optional(config.GetPathToPrivateKeyFile()) : std::nullopt, "private key");
...
bool NeedToUseAutoConfig(const NKikimrConfig::TActorSystemConfig& config) {
    return config.GetUseAutoConfig() || config.HasNodeType() || config.HasCpuCount();
}

void TBasicServicesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.emplace_back(MakeIoDispatcherActorId(),
        TActorSetupCmd(CreateIoDispatcherActor(schedulerConfig.MonCounters->GetSubgroup("subsystem", "io_dispatcher")), TMailboxType::HTSwap,
            systemPoolId));
    ...
    if (Config.HasNameserviceConfig()) {
        ...
        setup->LocalServices.emplace_back(resolverId, TActorSetupCmd(resolver, TMailboxType::HTSwap, systemPoolId));
        ...
        setup->LocalServices.emplace_back(nameserviceId, TActorSetupCmd(nameservice, TMailboxType::HTSwap, systemPoolId));
        ...
        if (Config.HasInterconnectConfig() && Config.GetInterconnectConfig().GetStartTcp()) {
            ...
            for (const auto& channel : icConfig.GetChannel()) {
                ...
                ui32 weight = 0;
                Y_ABORT_UNLESS(
                    !(channel.HasQuota() && channel.HasWeight()), "Only one field should be set: Weight or Quota, Weight is preffered");
                ...
                Y_ABORT_UNLESS(index < 1U << IEventHandle::ChannelBits,
                    "Channel index is too large: got %" PRIu32 ", should be less than %" PRIu32, index, 1U << IEventHandle::ChannelBits);
                Y_ABORT_UNLESS(weight > 0U && weight <= std::numeric_limits<ui16>::max(),
                    "Channel weight is out of allowed range: got %" PRIu32 ", should be > 0 and < %" PRIu32, weight,
            ...
            setup->LocalServices.emplace_back(
                MakePollerActorId(), TActorSetupCmd(CreatePollerActor(), TMailboxType::ReadAsFilled, systemPoolId));
            ...
            if (icConfig.HasOutgoingHandshakeInflightLimit()) {
                ...
                setup->LocalServices.emplace_back(MakeHandshakeBrokerOutId(),
                    TActorSetupCmd(CreateHandshakeBroker(*icCommon->OutgoingHandshakeInflightLimit), TMailboxType::ReadAsFilled, systemPoolId));
            }
...
#define CHANNEL(NAME) { TInterconnectChannels::NAME, #NAME }
            ...
            if (const auto& whiteboardId = NNodeWhiteboard::MakeNodeWhiteboardServiceId(NodeId)) {
                ...
                icCommon->UpdateWhiteboard = [whiteboardId](const TWhiteboardSessionStatus& data) {
                    ...
                    record.SetConnectStatus(
                        static_cast<NKikimrWhiteboard::EFlag>(static_cast<int>(data.ConnectStatus) + 1 /*GREY = 0, GREEN = 1 ....*/));
            ...
            if (const auto& mon = appData->Mon) {
                icCommon->RegisterMonPage = [mon](
                                                const TString& path, const TString& title, TActorSystem* actorSystem, const TActorId& actorId) {
                    NMonitoring::TIndexMonPage* page =
                        mon->RegisterIndexPage("actors", "Actors")->RegisterIndexPage("interconnect", "Interconnect");
                    mon->RegisterActorPage(page, path, title, false, actorSystem, actorId, /*useAuth=*/true, /*sortPages=*/false);
                };
                setup->LocalServices.emplace_back(NInterconnect::MakeInterconnectMonActorId(NodeId),
                    TActorSetupCmd(NInterconnect::CreateInterconnectMonActor(icCommon), TMailboxType::ReadAsFilled, systemPoolId));
            }
            ...
            if (!nsConfig.GetSuppressVersionCheck() && !Config.GetFeatureFlags().GetSuppressCompatibilityCheck()) {
                ...
                bool success = CompatibilityInfo.MakeStored(NKikimrConfig::TCompatibilityRule::Interconnect)
                                   .SerializeToString(&*icCommon->CompatibilityInfo);
                ...
                icCommon->ValidateCompatibilityOldFormat = [&](const NActors::TInterconnectProxyCommon::TVersionInfo& peer,
                                                               TString& errorReason) {
            ...
            setup->LocalServices.emplace_back(
                GetDestructActorID(), TActorSetupCmd(new TDestructActor, TMailboxType::ReadAsFilled, interconnectPoolId));
            ...
            for (const auto& node : table->StaticNodeTable) {
                ...
                if (destId != NodeId) {
                    ...
                    setup->Interconnect.ProxyActors[destId] =
                        TActorSetupCmd(new TInterconnectProxyTCP(destId, icCommon), TMailboxType::ReadAsFilled, interconnectPoolId);
                } else {
                    ...
                    auto listener = new TInterconnectListenerTCP(address, node.second.second, icCommon);
                    if (int err = listener->Bind()) {
                        ythrow yexception() << "Failed to set up IC listener on port " << node.second.second << " errno# " << err << " ("
                                            << strerror(err) << ")";
                    }
                    setup->LocalServices.emplace_back(
                        MakeInterconnectListenerActorId(false), TActorSetupCmd(listener, TMailboxType::ReadAsFilled, interconnectPoolId));
                }
            }
            ...
            if (Config.GetDynamicNodeConfig().HasNodeInfo()) {
                ...
                if (int err = listener->Bind()) {
                    ythrow yexception() << "Failed to set up IC listener on port " << info.GetPort() << " errno# " << err << " ("
                                        << strerror(err) << ")";
                }
                setup->LocalServices.emplace_back(
                    MakeInterconnectListenerActorId(true), TActorSetupCmd(listener, TMailboxType::ReadAsFilled, interconnectPoolId));
            }

            if (!IsServiceInitialized(setup, MakeInterconnectListenerActorId(false)) &&
                !IsServiceInitialized(setup, MakeInterconnectListenerActorId(true))) {
                if (Config.HasFederatedQueryConfig() && Config.GetFederatedQueryConfig().GetEnabled()) {
                    ...
                    if (nodesManagerConfig.GetEnabled()) {
                        ...
                        if (int err = listener->Bind()) {
                            ythrow yexception() << "Failed to set up IC listener on port " << nodesManagerConfig.GetPort() << " errno# " << err
                                                << " (" << strerror(err) << ")";
                        }
                        setup->LocalServices.emplace_back(
                            MakeInterconnectListenerActorId(true), TActorSetupCmd(listener, TMailboxType::ReadAsFilled, interconnectPoolId));
...
#undef GET_FIELD_FROM_CONFIG
        ...
        if (wilsonUploader) {
            setup->LocalServices.emplace_back(
                NWilson::MakeWilsonUploaderId(), TActorSetupCmd(wilsonUploader.release(), TMailboxType::ReadAsFilled, appData->BatchPoolId));
        }
    }
}
...
void TImmediateControlBoardInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(MakeIcbId(NodeId),
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        TActorId(), TActorSetupCmd(NConsole::CreateImmediateControlsConfigurator(appData->Icb, Config.GetImmediateControlsConfig()),
...
void TBSNodeWardenInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    nodeWardenConfig->StartupStorageYaml = Config.HasStartupStorageYaml() ? std::make_optional(Config.GetStartupStorageYaml()) : std::nullopt;
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(MakeBlobStorageNodeWardenID(NodeId),
        TActorSetupCmd(CreateBSNodeWarden(nodeWardenConfig.Release()), TMailboxType::ReadAsFilled, appData->SystemPoolId)));

    setup->LocalServices.emplace_back(
        MakeUniversalSchedulerActorId(), TActorSetupCmd(CreateUniversalSchedulerActor(), TMailboxType::ReadAsFilled, appData->SystemPoolId));
}
        ...
        for (auto& ring : ringGroup.Rings) {
            for (TActorId replica : ring.Replicas) {
                if (replica.NodeId() == setup.NodeId) {
                    setup.LocalServices.emplace_back(replica, TActorSetupCmd(createFunc(info, index), TMailboxType::ReadAsFilled, poolId));
                }
                ++index;
            }
        }
...
void TStateStorageServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        MakeStateStorageProxyID(), TActorSetupCmd(proxyActor.release(), TMailboxType::ReadAsFilled, appData->SystemPoolId)));

    setup->LocalServices.emplace_back(
        TActorId(), TActorSetupCmd(CreateTenantNodeEnumerationPublisher(), TMailboxType::HTSwap, appData->SystemPoolId));
}
...
void TLocalServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    auto addToLocalConfig = [&localConfig, &tabletAvailabilities, tabletPool = appData->SystemPoolId](TTabletTypes::EType tabletType,
                                TTabletSetupInfo::TTabletCreationFunc op, NActors::TMailboxType::EType mailboxType, ui32 poolId) {
        auto availIt = tabletAvailabilities.find(tabletType);
        auto localIt = localConfig->TabletClassInfo
                           .emplace(tabletType, new TTabletSetupInfo(op, mailboxType, poolId, TMailboxType::ReadAsFilled, tabletPool))
                           .first;
    ...
    setup->LocalServices.push_back(
        std::make_pair(MakeTenantPoolRootID(), TActorSetupCmd(CreateTenantPool(tenantPoolConfig), TMailboxType::ReadAsFilled, 0)));

    setup->LocalServices.push_back(
        std::make_pair(TActorId(), TActorSetupCmd(CreateLabelsMaintainer(Config.GetMonitoringConfig()), TMailboxType::ReadAsFilled, 0)));

    setup->LocalServices.emplace_back(NTestShard::MakeStateServerInterfaceActorId(),
        TActorSetupCmd(NTestShard::CreateStateServerInterfaceActor(nullptr), TMailboxType::ReadAsFilled, 0));
...
void TSharedCacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.emplace_back(
        NSharedCache::MakeSharedPageCacheId(0), TActorSetupCmd(actor, TMailboxType::ReadAsFilled, appData->UserPoolId));
}
...
void TBlobCacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
...
TLoggerInitializer::TLoggerInitializer(
    const TKikimrRunConfig& runConfig, TIntrusivePtr<NActors::NLog::TSettings> logSettings, std::shared_ptr<TLogBackend> logBackend)
    : IKikimrServicesInitializer(runConfig)
...
void TLoggerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(configurator, TMailboxType::HTSwap, appData->UserPoolId));
}
...
void TSchedulerActorInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto& systemConfig = Config.GetActorSystemConfig();
...
void TProfilerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    const TIntrusivePtr<::NMonitoring::TDynamicCounters> utilsCounters = GetServiceCounters(appData->Counters, "utils");
...
void TResourceBrokerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    TActorSetupCmd actorSetup = { NResourceBroker::CreateResourceBrokerActor(config, counters), TMailboxType::ReadAsFilled,
        appData->UserPoolId };
...
void TRestartsCountPublisher::PublishRestartsCount(
    const ::NMonitoring::TDynamicCounters::TCounterPtr& counter, const TString& restartsCountFile) {
...
void TRestartsCountPublisher::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    Y_UNUSED(setup);
    const TIntrusivePtr<::NMonitoring::TDynamicCounters> utilsCounters = GetServiceCounters(appData->Counters, "utils");
...
void TTabletResolverInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    TIntrusivePtr<TTabletResolverConfig> tabletResolverConfig(new TTabletResolverConfig());
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        MakeTabletResolverID(), TActorSetupCmd(CreateTabletResolver(tabletResolverConfig), TMailboxType::ReadAsFilled, appData->SystemPoolId)));
}
...
void TTabletPipePerNodeCachesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto counters = GetServiceCounters(appData->Counters, "tablets");
    ...
    setup->LocalServices.emplace_back(MakePipePerNodeCacheID(false),
        TActorSetupCmd(CreatePipePerNodeCache(leaderPipeConfig), TMailboxType::ReadAsFilled, appData->UserPoolId));
    setup->LocalServices.emplace_back(MakePipePerNodeCacheID(true),
        TActorSetupCmd(CreatePipePerNodeCache(followerPipeConfig), TMailboxType::ReadAsFilled, appData->UserPoolId));
    setup->LocalServices.emplace_back(MakePipePerNodeCacheID(EPipePerNodeCache::Persistent),
...
void TTabletMonitoringProxyInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    TActorSetupCmd tabletMonitoringProxySetup(
        NTabletMonitoringProxy::CreateTabletMonitoringProxy(std::move(proxyConfig)), TMailboxType::ReadAsFilled, appData->UserPoolId);
    setup->LocalServices.push_back(
        std::pair<TActorId, TActorSetupCmd>(NTabletMonitoringProxy::MakeTabletMonitoringProxyID(), std::move(tabletMonitoringProxySetup)));
}
...
void TTabletCountersAggregatorInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    {
        ...
        setup->LocalServices.push_back(
            std::pair<TActorId, TActorSetupCmd>(MakeTabletCountersAggregatorID(NodeId, false), std::move(tabletCountersAggregatorSetup)));
    }
    {
        ...
        setup->LocalServices.push_back(
            std::pair<TActorId, TActorSetupCmd>(MakeTabletCountersAggregatorID(NodeId, true), std::move(tabletCountersAggregatorSetup)));
    }
}
...
void TGRpcProxyStatusInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
...
static TIntrusivePtr<TTabletSetupInfo> CreateTablet(const TString& typeName, const TIntrusivePtr<TTabletStorageInfo>& tabletInfo,
    const TAppData* appData, const TIntrusivePtr<ITabletFactory>& customTablets = nullptr) {
...
TBootstrapperInitializer::TBootstrapperInitializer(const TKikimrRunConfig& runConfig)
    : IKikimrServicesInitializer(runConfig) {
}

void TBootstrapperInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (Config.HasBootstrapConfig()) {
        for (const auto& boot : Config.GetBootstrapConfig().GetTablet()) {
            if (boot.GetAllowDynamicConfiguration()) {
                setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
                    TActorId(), TActorSetupCmd(CreateConfiguredTabletBootstrapper(boot), TMailboxType::HTSwap, appData->SystemPoolId)));
            } else {
                ...
                if (Find(boot.GetNode(), NodeId) != boot.GetNode().end()) {
                    ...
                    auto tabletSetupInfo = CreateTablet(TTabletTypes::TypeToStr(tabletType), info, appData);
                    ...
                    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(MakeBootstrapperID(info->TabletID, NodeId),
...
TTabletsInitializer::TTabletsInitializer(const TKikimrRunConfig& runConfig, TIntrusivePtr<ITabletFactory> customTablets)
    : IKikimrServicesInitializer(runConfig)
    , CustomTablets(customTablets) {
}

void TTabletsInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    ...
    for (const auto& tabletConfig : Config.GetTabletsConfig().GetTablet()) {
        for (ui32 bootstrapperNode : tabletConfig.GetNode()) {
            if (bootstrapperNode == setup->NodeId) {
                ...
                if (!tabletSetup) {
                    ythrow yexception() << "unknown tablet type: " << tabletConfig.GetType();
                }

                setup->LocalServices.push_back(std::make_pair(MakeBootstrapperID(tabletInfo->TabletID, bootstrapperNode),
                    TActorSetupCmd(CreateBootstrapper(tabletInfo.Get(), new TBootstrapperInfo(tabletSetup.Get()), tabletConfig.GetStandBy()),
...
void TMediatorTimeCastProxyInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
...
void TMiniKQLCompileServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        MakeMiniKQLCompileServiceID(), TActorSetupCmd(compileService, TMailboxType::ReadAsFilled, appData->UserPoolId)));
}

// TMessageBusServicesInitializer

TMessageBusServicesInitializer::TMessageBusServicesInitializer(const TKikimrRunConfig& runConfig, NMsgBusProxy::IMessageBusServer& busServer)
    : IKikimrServicesInitializer(runConfig)
    , BusServer(busServer) {
}

void TMessageBusServicesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (!IsServiceInitialized(setup, NMsgBusProxy::CreateMsgBusProxyId()) && Config.HasMessageBusConfig() &&
        Config.GetMessageBusConfig().GetStartBusProxy()) {
        if (IActor* proxy = BusServer.CreateProxy()) {
            setup->LocalServices.emplace_back(
                NMsgBusProxy::CreateMsgBusProxyId(), TActorSetupCmd(proxy, TMailboxType::ReadAsFilled, appData->UserPoolId));
            ...
            if (appData->PQConfig.GetEnabled()) {
                setup->LocalServices.emplace_back(NMsgBusProxy::CreatePersQueueMetaCacheV2Id(),
                    TActorSetupCmd(NMsgBusProxy::NPqMetaCacheV2::CreatePQMetaCache(appData->Counters, pqMetaRefresh), TMailboxType::ReadAsFilled,
                        appData->UserPoolId));
            }
        }
    }
...
void TSecurityServicesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (!IsServiceInitialized(setup, MakeLdapAuthProviderID()) && authConfig.HasLdapAuthentication()) {
        ...
        if (ldapAuthProvider) {
            setup->LocalServices.push_back(std::make_pair<TActorId, TActorSetupCmd>(
                MakeLdapAuthProviderID(), TActorSetupCmd(ldapAuthProvider, TMailboxType::HTSwap, appData->UserPoolId)));
        }
    }
    if (!IsServiceInitialized(setup, MakeTicketParserID())) {
        IActor* ticketParser = nullptr;
        auto grpcConfig = Config.GetGRpcConfig();
        TTicketParserSettings settings{ .AuthConfig = Config.GetAuthConfig(),
            .CertificateAuthValues = { .ClientCertificateAuthorization = Config.GetClientCertificateAuthorization(),
                .ServerCertificateFilePath =
                    grpcConfig.HasPathToCertificateFile() ? grpcConfig.GetPathToCertificateFile() : grpcConfig.GetCert(),
        ...
        if (ticketParser) {
            setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
                MakeTicketParserID(), TActorSetupCmd(ticketParser, TMailboxType::HTSwap, appData->UserPoolId)));
        }
    }
}
...
TGRpcServicesInitializer::TGRpcServicesInitializer(const TKikimrRunConfig& runConfig, std::shared_ptr<TModuleFactories> factories)
    : IKikimrServicesInitializer(runConfig)
    , Factories(factories) {
}

void TGRpcServicesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto& systemConfig = Config.GetActorSystemConfig();
    bool hasASCfg = Config.HasActorSystemConfig();
    ...
    if (!IsServiceInitialized(setup, NMsgBusProxy::CreateMsgBusProxyId()) && Config.HasGRpcConfig() &&
        ...
        if (appData->PQConfig.GetEnabled()) {
            ...
            IActor* cache = NMsgBusProxy::NPqMetaCacheV2::CreatePQMetaCache(appData->Counters, pqMetaRefresh);
            Y_ABORT_UNLESS(cache);
            setup->LocalServices.emplace_back(
                NMsgBusProxy::CreatePersQueueMetaCacheV2Id(), TActorSetupCmd(cache, TMailboxType::ReadAsFilled, appData->UserPoolId));
        }
    }
    ...
    if (!IsServiceInitialized(setup, NGRpcService::CreateGRpcRequestProxyId(0))) {
        ...
        for (size_t i = 0; i < proxyCount; ++i) {
            ...
            setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
                NGRpcService::CreateGRpcRequestProxyId(i), TActorSetupCmd(grpcReqProxy, TMailboxType::ReadAsFilled, appData->UserPoolId)));
        }
        setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
            TActorId(), TActorSetupCmd(NConsole::CreateJaegerTracingConfigurator(appData->TracingConfigurator, Config.GetTracingConfig()),
                            TMailboxType::ReadAsFilled, appData->UserPoolId)));
    }
    ...
    if (!IsServiceInitialized(setup, NKesus::MakeKesusProxyServiceId())) {
        if (IActor* proxy = NKesus::CreateKesusProxyService()) {
            setup->LocalServices.emplace_back(
                NKesus::MakeKesusProxyServiceId(), TActorSetupCmd(proxy, TMailboxType::ReadAsFilled, appData->UserPoolId));
        }
    }
    ...
    if (Config.HasGRpcConfig() && Config.GetGRpcConfig().GetStartGRpcProxy()) {
        ...
        setup->LocalServices.emplace_back(NGRpcService::CreateGrpcPublisherServiceActorId(),
...
void TStatsCollectorInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (appData->Mon) {
        IActor* statsCollector = CreateStatsCollector(1,   // seconds
            *setup, appData->Counters);
        setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(statsCollector, TMailboxType::HTSwap, appData->SystemPoolId));

        IActor* memStatsCollector = CreateMemStatsCollector(1,   // seconds
            appData->Counters);
        setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(memStatsCollector, TMailboxType::HTSwap, appData->SystemPoolId));

        IActor* procStatCollector = CreateProcStatCollector(5,   // seconds
            appData->Counters);
        setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(procStatCollector, TMailboxType::HTSwap, appData->SystemPoolId));
    }
}
#endif
...
void TSelfPingInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    for (size_t poolId = 0; poolId < setup->GetExecutorsCount(); ++poolId) {
        ...
        IActor* selfPingActor =
            CreateSelfPingActor(selfPingInterval, maxPingCounter, avgPingCounter, avgPingCounterWithSmallWindow, cpuTimeCounter);
        setup->LocalServices.push_back(std::make_pair(TActorId(), TActorSetupCmd(selfPingActor, TMailboxType::HTSwap, poolId)));
    }
}
...
void TWhiteBoardServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    IActor* tabletStateService = NNodeWhiteboard::CreateNodeWhiteboardService();
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NNodeWhiteboard::MakeNodeWhiteboardServiceId(NodeId), TActorSetupCmd(tabletStateService, TMailboxType::HTSwap, appData->SystemPoolId)));
}

// TTabletMonitorInitializer

TTabletMonitorInitializer::TTabletMonitorInitializer(const TKikimrRunConfig& runConfig,
...
void TTabletMonitorInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    IActor* nodeTabletMonitor = NNodeTabletMonitor::CreateNodeTabletMonitor(TabletStateClassifier, TabletListRenderer);
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NNodeTabletMonitor::MakeNodeTabletMonitorID(NodeId), TActorSetupCmd(nodeTabletMonitor, TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TViewerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(
        std::pair<TActorId, TActorSetupCmd>(MakeViewerID(NodeId), TActorSetupCmd(viewer, TMailboxType::HTSwap, appData->BatchPoolId)));
}
...
void TFailureInjectionInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.emplace_back(
        MakeBlobStorageFailureInjectionID(NodeId), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId));
    // FIXME: correct service id
}
...
void TPersQueueL2CacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(
        std::pair<TActorId, TActorSetupCmd>(NPQ::MakePersQueueL2CacheID(), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TNetClassifierInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NNetClassifier::MakeNetClassifierID(), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TPersQueueClusterTrackerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NPQ::NClusterTracker::MakeClusterTrackerID(), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TPersQueueDirectReadCacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NPQ::MakePQDReadCacheServiceActorId(), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
}

TMemProfMonitorInitializer::TMemProfMonitorInitializer(
    const TKikimrRunConfig& runConfig, TIntrusiveConstPtr<NMemory::IProcessMemoryInfoProvider> processMemoryInfoProvider)
    : IKikimrServicesInitializer(runConfig)
    , ProcessMemoryInfoProvider(std::move(processMemoryInfoProvider)) {
}

void TMemProfMonitorInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    IActor* monitorActor = CreateMemProfMonitor(TDuration::Seconds(1), ProcessMemoryInfoProvider, appData->Counters, filePathPrefix);

    setup->LocalServices.emplace_back(MakeMemProfMonitorID(NodeId), TActorSetupCmd(monitorActor, TMailboxType::HTSwap, appData->BatchPoolId));
}
...
void TMemoryTrackerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto* actor = NMemory::CreateMemoryTrackerActor(TDuration::MilliSeconds(20), appData->Counters);
    setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId));
}

TMemoryControllerInitializer::TMemoryControllerInitializer(
    const TKikimrRunConfig& runConfig, TIntrusiveConstPtr<NMemory::IProcessMemoryInfoProvider> processMemoryInfoProvider)
    : IKikimrServicesInitializer(runConfig)
    , ProcessMemoryInfoProvider(std::move(processMemoryInfoProvider)) {
}

void TMemoryControllerInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto* actor = NMemory::CreateMemoryController(TDuration::Seconds(1), ProcessMemoryInfoProvider, Config.GetMemoryControllerConfig(),
        NKikimrConfigHelpers::CreateMemoryControllerResourceBrokerConfig(Config), appData->Counters);
    setup->LocalServices.emplace_back(NMemory::MakeMemoryControllerId(0), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->BatchPoolId));
}
...
void TQuoterServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    Y_UNUSED(appData);
    setup->LocalServices.push_back(
        std::make_pair(MakeQuoterServiceID(), TActorSetupCmd(CreateQuoterService(), TMailboxType::HTSwap, appData->SystemPoolId)));
}

TKqpServiceInitializer::TKqpServiceInitializer(
    const TKikimrRunConfig& runConfig, std::shared_ptr<TModuleFactories> factories, IGlobalObjectStorage& globalObjects)
    : IKikimrServicesInitializer(runConfig)
    , Factories(std::move(factories))
...
void TKqpServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (enableKqp) {
        ...
        auto rm = NKqp::CreateKqpResourceManagerActor(
            Config.GetTableServiceConfig().GetResourceManager(), nullptr, {}, kqpProxySharedResources, NodeId);
        setup->LocalServices.push_back(
            std::make_pair(NKqp::MakeKqpRmServiceID(NodeId), TActorSetupCmd(rm, TMailboxType::HTSwap, appData->UserPoolId)));

        // We need to keep YqlLoggerScope alive as long as something may be trying to log
        GlobalObjects.AddGlobalObject(std::make_shared<NYql::NLog::YqlLoggerScope>(new NYql::NLog::TTlsLogBackend(new TNullLogBackend())));

        auto federatedQuerySetupFactory = NKqp::MakeKqpFederatedQuerySetupFactory(setup, appData, Config);

        auto s3ActorsFactory = NYql::NDq::CreateS3ActorsFactory();
        auto proxy = NKqp::CreateKqpProxyService(Config.GetLogConfig(), Config.GetTableServiceConfig(), Config.GetQueryServiceConfig(),
            std::move(settings), Factories->QueryReplayBackendFactory, std::move(kqpProxySharedResources), federatedQuerySetupFactory,
            s3ActorsFactory);
        setup->LocalServices.push_back(
            std::make_pair(NKqp::MakeKqpProxyID(NodeId), TActorSetupCmd(proxy, TMailboxType::HTSwap, appData->UserPoolId)));

        // Create finalize script service
        auto finalize = NKqp::CreateKqpFinalizeScriptService(Config.GetQueryServiceConfig(), federatedQuerySetupFactory, s3ActorsFactory);
        setup->LocalServices.push_back(
            std::make_pair(NKqp::MakeKqpFinalizeScriptServiceId(NodeId), TActorSetupCmd(finalize, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}
...
void TGroupedMemoryLimiterInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (serviceConfig.IsEnabled()) {
        ...
        setup->LocalServices.push_back(std::make_pair(NOlap::NGroupedMemoryManager::TScanMemoryLimiterOperator::MakeServiceId(NodeId),
...
void TCompDiskLimiterInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (serviceConfig.IsEnabled()) {
        ...
        setup->LocalServices.push_back(std::make_pair(
            NLimiter::TCompDiskOperator::MakeServiceId(NodeId), TActorSetupCmd(service, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}
...
void TCompPrioritiesInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (serviceConfig.IsEnabled()) {
        ...
        setup->LocalServices.push_back(std::make_pair(
            NPrioritiesQueue::TCompServiceOperator::MakeServiceId(NodeId), TActorSetupCmd(service, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}
...
void TExternalIndexInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (serviceConfig.IsEnabled()) {
        ...
        setup->LocalServices.push_back(
            std::make_pair(NCSIndex::MakeServiceId(NodeId), TActorSetupCmd(service, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}
...
void TMetadataProviderInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (serviceConfig.IsEnabled()) {
        ...
        setup->LocalServices.push_back(
            std::make_pair(NMetadata::NProvider::MakeServiceId(NodeId), TActorSetupCmd(service, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}

TMemoryLogInitializer::TMemoryLogInitializer(const TKikimrRunConfig& runConfig)
    : IKikimrServicesInitializer(runConfig) {
}

void TMemoryLogInitializer::InitializeServices(NActors::TActorSystemSetup*, const NKikimr::TAppData*) {
...
void TCmsServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    auto http = NCms::CreateCmsHttp();
    setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(http, TMailboxType::HTSwap, appData->UserPoolId));
}
...
void TTxProxyInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(MakeTxProxyID(),
...
void TLongTxServiceInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    TIntrusivePtr<::NMonitoring::TDynamicCounters> tabletGroup = GetServiceCounters(appData->Counters, "tablets");
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NLongTxService::MakeLongTxServiceID(NodeId), TActorSetupCmd(actor, TMailboxType::ReadAsFilled, appData->UserPoolId)));
}
...
void TSequenceProxyServiceInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    auto* actor = NSequenceProxy::CreateSequenceProxy();
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NSequenceProxy::MakeSequenceProxyServiceID(), TActorSetupCmd(actor, TMailboxType::ReadAsFilled, appData->UserPoolId)));
}
...
void TLeaseHolderInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    if (Config.GetDynamicNodeConfig().HasNodeInfo()) {
        ...
        setup->LocalServices.emplace_back(TActorId(), TActorSetupCmd(holder, TMailboxType::HTSwap, appData->UserPoolId));
    }
}
    ...
    if (Config.GetSqsConfig().GetEnableSqs()) {
        {
            ...
            setup->LocalServices.emplace_back(NSQS::MakeSqsServiceID(NodeId), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId));
        }

        {
            IActor* actor = NSQS::CreateSqsProxyService();
            setup->LocalServices.emplace_back(
                NSQS::MakeSqsProxyServiceID(NodeId), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId));
        }

        Factories->SqsAuthFactory->Initialize(setup->LocalServices, *appData, Config.GetSqsConfig());
    }
}
...
void THttpProxyServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    Factories->DataStreamsAuthFactory->Initialize(setup->LocalServices, *appData, Config.GetHttpProxyConfig(), Config.GetGRpcConfig());
}
...
void TConfigsDispatcherInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NConsole::MakeConfigsDispatcherID(NodeId), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));

    IActor* proxyActor = NConsole::CreateConfigsDispatcherProxy();
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NConsole::MakeConfigsDispatcherProxyID(NodeId), TActorSetupCmd(proxyActor, TMailboxType::HTSwap, appData->UserPoolId)));

    setup->LocalServices.emplace_back(
        MakeFeatureFlagsServiceID(), TActorSetupCmd(NConsole::CreateFeatureFlagsConfigurator(), TMailboxType::HTSwap, appData->UserPoolId));
}
...
void TConfigsCacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (PathToConfigCacheFile && appData->FeatureFlags.GetEnableConfigurationCache()) {
        ...
        setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
            NConsole::MakeConfigsCacheActorID(NodeId), TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
    }
}
...
void TTabletInfoInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
...
void TSysViewServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    for (ui32 i = 0; i < setup->GetExecutorsCount(); ++i) {
        config.Pools.push_back(NSysView::TExtCountersConfig::TPool{ setup->GetPoolName(i), setup->GetThreads(i) });
    }
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NSysView::MakeSysViewServiceID(NodeId), TActorSetupCmd(actor.Release(), TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TStatServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NStat::MakeStatServiceID(NodeId), TActorSetupCmd(actor.Release(), TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TMeteringWriterInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NMetering::MakeMeteringServiceID(), TActorSetupCmd(actor.Release(), TMailboxType::HTSwap, appData->IOPoolId)));
}
...
void TAuditWriterInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    ...
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NAudit::MakeAuditServiceID(), TActorSetupCmd(actor.Release(), TMailboxType::HTSwap, appData->IOPoolId)));
}
...
void TSchemeBoardMonitoringInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        MakeSchemeBoardMonitoringId(), TActorSetupCmd(CreateSchemeBoardMonitoring(), TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void TYqlLogsInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        MakeYqlLogsUpdaterId(), TActorSetupCmd(CreateYqlLogsUpdater(Config.GetLogConfig()), TMailboxType::HTSwap, appData->UserPoolId)));
}
...
void THealthCheckInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    setup->LocalServices.push_back(std::pair<TActorId, TActorSetupCmd>(
        NHealthCheck::MakeHealthCheckID(), TActorSetupCmd(NHealthCheck::CreateHealthCheckService(), TMailboxType::HTSwap, appData->UserPoolId)));
}

TFederatedQueryInitializer::TFederatedQueryInitializer(
    const TKikimrRunConfig& runConfig, std::shared_ptr<TModuleFactories> factories, NFq::IYqSharedResources::TPtr yqSharedResources)
    : IKikimrServicesInitializer(runConfig)
    , Factories(std::move(factories))
...
void TFederatedQueryInitializer::InitializeServices(TActorSystemSetup* setup, const TAppData* appData) {
    ...
    auto actorRegistrator = [&](NActors::TActorId serviceActorId, NActors::IActor* actor) {
        setup->LocalServices.push_back(
            std::pair<TActorId, TActorSetupCmd>(serviceActorId, TActorSetupCmd(actor, TMailboxType::HTSwap, appData->UserPoolId)));
    };
    ...
    NFq::Init(protoConfig, NodeId, actorRegistrator, appData, tenant, Factories->PqCmConnections, YqSharedResources,
        Factories->FolderServiceFactory, IcPort, Factories->AdditionalComputationNodeFactories);
}
...
void TReplicationServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.emplace_back(NReplication::MakeReplicationServiceId(NodeId),
...
void TLocalPgWireServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.emplace_back(NLocalPgWire::CreateLocalPgWireProxyId(),
    ...
    setup->LocalServices.emplace_back(
        TActorId(), TActorSetupCmd(NPG::CreatePGListener(MakePollerActorId(), NLocalPgWire::CreateLocalPgWireProxyId(), settings),
...
void TKafkaProxyServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (Config.GetKafkaProxyConfig().GetEnableKafkaProxy()) {
        ...
        setup->LocalServices.emplace_back(NKafka::MakeKafkaDiscoveryCacheID(),
            TActorSetupCmd(CreateDiscoveryCache(NGRpcService::KafkaEndpointId), TMailboxType::HTSwap, appData->UserPoolId));

        setup->LocalServices.emplace_back(NKafka::MakeTransactionsServiceID(NodeId),
            TActorSetupCmd(NKafka::CreateTransactionsCoordinator(), TMailboxType::HTSwap, appData->UserPoolId));
        setup->LocalServices.emplace_back(
            TActorId(), TActorSetupCmd(NKafka::CreateKafkaListener(MakePollerActorId(), settings, Config.GetKafkaProxyConfig()),
                            TMailboxType::HTSwap, appData->UserPoolId));

        IActor* metricsActor = CreateKafkaMetricsActor(NKafka::TKafkaMetricsSettings{ appData->Counters });
        setup->LocalServices.emplace_back(
            NKafka::MakeKafkaMetricsServiceID(), TActorSetupCmd(metricsActor, TMailboxType::HTSwap, appData->UserPoolId));
    }
}
...
void TIcNodeCacheServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    if (appData->FeatureFlags.GetEnableIcNodeCache()) {
        setup->LocalServices.emplace_back(NIcNodeCache::CreateICNodesInfoCacheServiceId(),
            TActorSetupCmd(NIcNodeCache::CreateICNodesInfoCacheService(appData->Counters), TMailboxType::HTSwap, appData->UserPoolId));
    }
}
...
void TDatabaseMetadataCacheInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.emplace_back(MakeDatabaseMetadataCacheId(NodeId),
...
void TGraphServiceInitializer::InitializeServices(NActors::TActorSystemSetup* setup, const NKikimr::TAppData* appData) {
    setup->LocalServices.emplace_back(NGraph::MakeGraphServiceId(),
