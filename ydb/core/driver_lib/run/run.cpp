...
class TBridePileIdInitializer : public IAppDataInitializer {
    const TKikimrRunConfig& Config;

public:
    TBridePileIdInitializer(const TKikimrRunConfig& runConfig)
        : Config(runConfig)
    {
    }
...
void TKikimrRunner::InitializeAppData(const TKikimrRunConfig& runConfig)
{
    ...
    // setup bridge pile id (note: currently only for dynamic nodes)
    appDataInitializers.AddAppDataInitializer(new TBridePileIdInitializer(runConfig));
