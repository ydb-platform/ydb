...
public:
    ...
    virtual void UpdateRow(const TTableId& tableId, const TArrayRef<const TCell>& row,
        const TArrayRef<const TUpdateCommand>& commands, const NACLib::TUserContext::TPtr userCtx) = 0;

    // Erases the single row.
    virtual void EraseRow(const TTableId& tableId, const TArrayRef<const TCell>& row, const NACLib::TUserContext::TPtr userCtx) = 0;
