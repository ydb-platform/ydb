...
IChunkedArray::TLocalChunkedArrayAddress TDeserializeChunkedArray::DoGetLocalChunkedArray(
    ...
    if (!!Data) {
        auto result = Loader->ApplyConclusion(Data, GetRecordsCount(), std::nullopt, DictionaryAccessor);
        ...
        auto result = Loader->ApplyConclusion(TString(DataBuffer.data(), DataBuffer.size()), GetRecordsCount(), std::nullopt, DictionaryAccessor);
        Y_ABORT_UNLESS(result.IsSuccess(), "Incorrect object for result request. internal path id: %s error: %s ", InternalPathId.data(),
            result.GetErrorMessage().data());
        return TLocalChunkedArrayAddress(result.DetachResult(), 0, 0);
    }
