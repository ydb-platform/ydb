...
TString SerializeBatch(const std::shared_ptr<arrow::RecordBatch>& batch, const arrow::ipc::IpcWriteOptions& options) {
    return NSerialization::TArrowSerializer(options).SerializePayload(batch);
}
...
{
    auto result = NSerialization::TArrowSerializer().Deserialize(blob, schema);
    if (result.ok()) {
        return *result;
    } else {
