...
Y_UNIT_TEST_SUITE(MergeBenchmarkEquivalence) {
    ...
    void AssertEqualRows(const std::shared_ptr<arrow::RecordBatch>& resOld, const std::shared_ptr<arrow20::Table>& resNew) {
        UNIT_ASSERT_VALUES_EQUAL(resOld->num_rows(), resNew->num_rows());
        UNIT_ASSERT_VALUES_EQUAL(resOld->num_columns(), 4);
        UNIT_ASSERT_VALUES_EQUAL(resNew->num_columns(), 4);
        ...
        for (int64_t i = 0; i < resOld->num_rows(); ++i) {
            auto tsNew = resNew->column(0)->GetScalar(i).ValueOrDie();
            auto aNew = resNew->column(1)->GetScalar(i).ValueOrDie();
            auto bNew = resNew->column(2)->GetScalar(i).ValueOrDie();
            auto vNew = resNew->column(3)->GetScalar(i).ValueOrDie();

            UNIT_ASSERT_C(tsNew->is_valid && aNew->is_valid && bNew->is_valid && vNew->is_valid, "row " << i << " has nulls");
            UNIT_ASSERT_VALUES_EQUAL_C(tsOld.Value(i), static_cast<const arrow20::TimestampScalar&>(*tsNew).value, "row " << i << " ts");
            UNIT_ASSERT_VALUES_EQUAL_C(std::string(aOld.GetView(i)), std::string(static_cast<const arrow20::StringScalar&>(*aNew).view()), "row " << i << " a");
            UNIT_ASSERT_VALUES_EQUAL_C(std::string(bOld.GetView(i)), std::string(static_cast<const arrow20::StringScalar&>(*bNew).view()), "row " << i << " b");
            UNIT_ASSERT_VALUES_EQUAL_C(vOld.Value(i), static_cast<const arrow20::Int64Scalar&>(*vNew).value, "row " << i << " ver");
        }
    }
    ...
    void CheckEquivalenceArrow5(int numSources) {
        ...
        auto resOld = MergeOnce(fx);
        auto resNew = MergeOnceArrow20(fx20);
        AssertEqualRows(resOld, resNew);
    }
