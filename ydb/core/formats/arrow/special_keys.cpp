...
bool TSpecialKeys::DeserializeFromString(const TString& data) {
    ...
    Data = NArrow::TStatusValidator::GetValid(NArrow::NSerialization::TArrowSerializer().Deserialize(data));
    return !!Data;
}
...
TString TSpecialKeys::SerializeToString() const {
    return NArrow::NSerialization::TArrowSerializer().SerializeFull(Data);
}
