...
Y_UNIT_TEST_SUITE(ArrowTest) {
    ...
    Y_UNIT_TEST(MergingSortedInputStream) {
        ...
        std::shared_ptr<arrow::RecordBatch> sorted;
        {
            ...
            auto merger = std::make_shared<NArrow::NMerger::TMergePartialStream>(batch->schema(), batch->schema(), false, vColumns, std::nullopt);
            for (auto&& i : batches) {
    ...
    Y_UNIT_TEST(MergingSortedInputStreamReplace) {
        ...
        std::shared_ptr<arrow::RecordBatch> sorted;
        {
            ...
            auto merger = std::make_shared<NArrow::NMerger::TMergePartialStream>(batch->schema(), batches[0]->schema(), false, vColumns, std::nullopt);
            for (auto&& i : batches) {
                merger->AddSource(i, nullptr, NArrow::NMerger::TIterationOrder::Forward(0));
            }
            merger->DrainAll(builder);
