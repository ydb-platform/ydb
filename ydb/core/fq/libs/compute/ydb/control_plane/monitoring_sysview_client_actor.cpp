...
public:
    ...
    void Handle(TEvYdbCompute::TEvCpuLoadRequest::TPtr& ev) {
        TableClient->RetryOperation(
            [actorSystem = TActivationContext::ActorSystem(), self = SelfId(), cookie = Cookie] (NYdb::NTable::TSession session) mutable {
                return session.ExecuteDataQuery(
                    "SELECT AVG(CpuUsage) as Usage, AVG(CpuIdle) as Idle, SUM(CpuThreads) as Threads FROM `.sys/nodes`",
                    NYdb::NTable::TTxControl::BeginTx(NYdb::NTable::TTxSettings::SerializableRW()).CommitTx()
                ).Apply([actorSystem = actorSystem, self = self, cookie = cookie](const NThreading::TFuture<NYdb::NTable::TDataQueryResult>& future) {
                        ...
                        if (result.IsSuccess()) {
                            ...
                            if (parser.TryNextRow()) {
                                ...
                                LOG_E("Load info is missed in nodes table");
