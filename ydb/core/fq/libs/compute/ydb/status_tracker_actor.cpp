...
public:
    ...
    void UpdatePlanAndStat(Fq::Private::PingTaskRequest& pingTaskRequest, bool final = false) {
        auto plan = QueryStats.query_plan();
        try {
            plan = Processor->ConvertPlan(plan);
        } catch(const NJson::TJsonException& ex) {
            LOG_E("Error plan conversion: " << ex.what());
        }
        PrepareAstAndPlan(pingTaskRequest, plan, QueryStats.query_ast());
        try {
            double cpuUsage = 0.0;
            ...
            ReportPublicCounters(Processor->GetPublicStat(stat));
        } catch(const NJson::TJsonException& ex) {
    ...
    void UpdateProgress() {
        ...
        UpdatePlanAndStat(pingTaskRequest, false);
    ...
    void Failed() {
        ...
        UpdatePlanAndStat(pingTaskRequest, true);
