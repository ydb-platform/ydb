    ...
    auto prepareParams = [meteringRecords, commonCounters=commonCounters, leaseLeftMs=Counters.LeaseLeftMs, config=Config, tablePathPrefix=YdbConnection->TablePathPrefix, finalStatus=finalStatus, response=response, counters=counters, actorSystem=NActors::TActivationContext::ActorSystem(), request=request](const std::vector<TResultSet>& resultSets) mutable {
        ...
        if (IsTerminalStatus(request.status())) {
            try {
                ...
                if (!isBillable) {
                    YDB_LOG_CTX_COMP_NOTICE(*actorSystem, ::NKikimrServices::YQ_CONTROL_PLANE_STORAGE, "Query is NOT billable,",
                ...
                YDB_LOG_CTX_COMP_ERROR(*actorSystem, ::NKikimrServices::YQ_CONTROL_PLANE_STORAGE, "Error on statistics",
    ...
    NKikimr::NKqp::TRetryLimiter& retryLimiter, TDuration& backoff, TInstant& expireAt)
{
    ...
    if (request.resign_query()) {
        ...
        YDB_LOG_CTX_COMP_DEBUG(*actorSystem, ::NKikimrServices::YQ_CONTROL_PLANE_STORAGE, "PingTaskRequest",
            {"(resign)", (!policyFound ? " DEFAULT POLICY" : "")},
...
void TYdbControlPlaneStorageActor::Handle(TEvControlPlaneStorage::TEvPingTaskRequest::TPtr& ev)
{
    ...
    YDB_LOG_COMP_TRACE(::NKikimrServices::YQ_CONTROL_PLANE_STORAGE, "PingTaskRequest",
        {"Request", request.DebugString()});

    if (const auto& issues = ValidateRequest(ev)) {
        YDB_LOG_COMP_WARN(::NKikimrServices::YQ_CONTROL_PLANE_STORAGE, "PingTaskRequest: validation",
            {"Request", request.DebugString()},
...
void TControlPlaneStorageBase::Handle(TEvControlPlaneStorage::TEvFinalStatusReport::TPtr& ev) {
    ...
    if (IsFailedStatus(event.Status)) {
        ...
        YDB_LOG_COMP_INFO(::NKikimrServices::YQ_AUDIT, "FinalFailedStatus: cloud id: ], scope: ], query id: ], job id: ], query type: [, status, transient issues",
    ...
    YDB_LOG_COMP_INFO(::NKikimrServices::YQ_AUDIT, "FinalStatus: cloud id: [ ], scope: [ ], query id: [ ], job id: [ ], query type: [ ],",
        {"cloudId", event.CloudId},
        {"scope", event.Scope},
        {"queryId", event.QueryId},
        {"jobId", event.JobId},
