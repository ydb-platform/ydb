...
class TKikimrStructuredTokenCredentialsFactory : public NYql::IStructuredTokenCredentialsFactory {
public:
    explicit TKikimrStructuredTokenCredentialsFactory(NYql::ISecuredServiceAccountCredentialsFactory::TPtr saFactory)
        : DefaultProvider_(NYql::CreateStructuredTokenCredentialsFactory(std::move(saFactory)))
    {}
        ...
        const TString& structuredTokenJson, bool addBearerToToken) override
    {
        if (NYql::IsStructuredTokenJson(structuredTokenJson)) {
            ...
            if (parser.HasIamAuth()) {
                return CreateForIamAuth(parser);
            }
        }
        return DefaultProvider_->Create(structuredTokenJson, addBearerToToken);
    }

private:
    std::shared_ptr<NYdb::ICredentialsProviderFactory> CreateForIamAuth(
        NYql::TStructuredTokenParser& parser)
    {
        auto* appData = NKikimr::AppData();
...
private:
    const NYql::IStructuredTokenCredentialsFactory::TPtr DefaultProvider_;
};
...
{
    return std::make_shared<TKikimrStructuredTokenCredentialsFactory>(std::move(saFactory));
}
