...
public:
    ...
    TOperationRequestExecuter(typename TRpc::TRequest&& proto, const TSettings& settings)
        ...
        , ClientLostStatus(std::make_shared<bool>(false))
...
private:
    static std::unique_ptr<TCtx> CreateCtx(typename TRpc::TRequest&& proto, const TSettings& settings, std::shared_ptr<bool> clientLostStatus) {
