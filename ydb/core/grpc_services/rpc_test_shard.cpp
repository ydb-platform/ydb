...
public:
    ...
    void SendProposeRequest(const TActorContext &ctx) {
        ...
        if (req->config().empty()) {
            Request_->RaiseIssue(NYql::TIssue("config must be specified"));
    ...
    void Handle(NSchemeShard::TEvSchemeShard::TEvDescribeSchemeResult::TPtr& ev, const TActorContext& ctx) {
        ...
        if (status != NKikimrScheme::StatusSuccess) {
            ...
            message << "Failed to describe created TestShardSet, status: " << NKikimrScheme::EStatus_Name(status);
            if (const auto& reason = record.GetReason(); !reason.empty()) {
                message << ", reason: " << reason;
            }
            Request_->RaiseIssue(NYql::TIssue(message));
            const auto ydbStatus = status == NKikimrScheme::StatusAccessDenied
                                   ? Ydb::StatusIds::UNAUTHORIZED
                                   : Ydb::StatusIds::GENERIC_ERROR;
            return this->Reply(ydbStatus, ctx);
        }

        const auto& pathDescription = record.GetPathDescription();
        if (pathDescription.GetSelf().GetPathType() != NKikimrSchemeOp::EPathTypeTestShardSet) {
            Request_->RaiseIssue(NYql::TIssue(TStringBuilder() << "Path '" << record.GetPath() << "' exists but is not a TestShardSet"));
...
public:
    ...
    void Bootstrap(const TActorContext& ctx) {
        TBase::Bootstrap(ctx);
        Become(&TDeleteTestShardSetRequest::StateWork);
        SendProposeRequest(ctx);
    }
