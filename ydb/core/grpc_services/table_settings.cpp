...
void ResolveTtlStoragePaths(Ydb::Table::TtlSettings& settings, const TMaybe<TString>& database) {
    ...
    for (auto& tier : *settings.mutable_tiered_ttl()->mutable_tiers()) {
        if (tier.has_evict_to_external_storage()) {
            ...
            eviction->set_storage(ResolvePathToDatabase(database, eviction->storage()));
        }
    }
}
