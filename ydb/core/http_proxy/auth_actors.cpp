...
namespace NKikimr::NHttpProxy {
    NActors::IActor* CreateAccessServiceActor(const NKikimrConfig::TServerlessProxyConfig& config, const TString& userAgentPrefix, bool enableV2Interface)
    {
        NCloud::TAccessServiceSettings asSettings(config.GetHttpConfig().GetAccessServiceEndpoint(), userAgentPrefix);
        if (config.GetCaCert()) {
    ...
    NActors::IActor* CreateIamTokenServiceActor(const NKikimrConfig::TServerlessProxyConfig& config, const TString& userAgentPrefix)
    {
        NCloud::TIamTokenServiceSettings tsSettings(config.GetHttpConfig().GetIamTokenServiceEndpoint(), userAgentPrefix);
        if (config.GetCaCert()) {
            TString certificate = TFileInput(config.GetCaCert()).ReadAll();
