...
/*
    Y_UNIT_TEST_F(TestGetRecordsThrottling, THttpProxyTestMock) {
        auto record = CreateCreateStreamRequest();
        auto res = SendHttpRequest("/Root", "kinesisApi.CreateStream", record,
                                  FormAuthorizationStr("ru-central1"));
        UNIT_ASSERT_VALUES_EQUAL(res.HttpCode, 200);
        ...
        record = CreateGetShardIteratorRequest();
        res = SendHttpRequest("/Root", "kinesisApi.GetShardIterator", record,
                                  FormAuthorizationStr("ru-central1"));
        UNIT_ASSERT_VALUES_EQUAL(res.HttpCode, 200);
        NJson::TJsonValue json;
        UNIT_ASSERT(NJson::ReadJsonTree(res.Body, &json));

        TString shardIterator = GetByPath<TString>(json, "ShardIterator");
        UNIT_ASSERT_VALUES_EQUAL(shardIterator.empty(), false);

        record = CreateGetRecordsRequest();
        record["ShardIterator"] = shardIterator;

        THttpResult res1, res2, res3, res4, res5;

        std::thread t1([&]() {
            for (int i = 0; i < 200; i++) {
                res1 = SendHttpRequest("/Root", "kinesisApi.GetRecords", record,
                                        FormAuthorizationStr("ru-central1"));
        ...
        std::thread t2([&]() {
            for (int i = 0; i < 200; i++) {
                res2 = SendHttpRequest("/Root", "kinesisApi.GetRecords", record,
                                        FormAuthorizationStr("ru-central1"));
        ...
        std::thread t3([&]() {
            for (int i = 0; i < 200; i++) {
                res3 = SendHttpRequest("/Root", "kinesisApi.GetRecords", record,
                                        FormAuthorizationStr("ru-central1"));
        ...
        std::thread t4([&]() {
            for (int i = 0; i < 200; i++) {
                res4 = SendHttpRequest("/Root", "kinesisApi.GetRecords", record,
                                        FormAuthorizationStr("ru-central1"));
        ...
        std::thread t5([&]() {
            for (int i = 0; i < 200; i++) {
                res5 = SendHttpRequest("/Root", "kinesisApi.GetRecords", record,
                                        FormAuthorizationStr("ru-central1"));
                if (res5.HttpCode != 200) {
                    break;
                }
            }
        });
        ...
        UNIT_ASSERT_VALUES_EQUAL(std::max(std::max(std::max(res1.HttpCode, res2.HttpCode), std::max(res3.HttpCode, res4.HttpCode)), res5.HttpCode), 400);
...
// Now theese tests are timing out
// Y_UNIT_TEST_SUITE(TestGRpcProxy) {
//     Y_UNIT_TEST_F(DiscoveryOk, THttpProxyTestMock) {
//         NYdb::TDriverConfig config;
//         config.SetDatabase("/Root");
//         config.SetEndpoint(TStringBuilder() << "127.0.0.1:" << GRpcServerPort);
//         NYdb::TDriver driver(config);
//         NYdb::NDiscovery::TDiscoveryClient client(driver);
//         auto res = client.ListEndpoints();
//         res.Wait();
//         Cerr << res.GetValue().IsSuccess() << " " << res.GetValue().GetIssues().ToString() << "\n";
//         for (auto& endpoint : res.GetValue().GetEndpointsInfo()) {
//             Cerr << endpoint.Address << ":" << endpoint.Port << "\n";
//         }
//         UNIT_ASSERT(res.GetValue().IsSuccess());
//     }


//     Y_UNIT_TEST_F(DiscoveryOkPath, THttpProxyTestMock) {
//         NYdb::TDriverConfig config;
//         config.SetDatabase("/Root/CM");
//         config.SetEndpoint(TStringBuilder() << "127.0.0.1:" << GRpcServerPort);
//         NYdb::TDriver driver(config);
//         NYdb::NDiscovery::TDiscoveryClient client(driver);
...
//         Cerr << res.GetValue().IsSuccess() << " " << res.GetValue().GetIssues().ToString() << "\n";
//         UNIT_ASSERT(res.GetValue().IsSuccess());
//         UNIT_ASSERT(res.GetValue().GetEndpointsInfo().size() == 1);
//     }

//     Y_UNIT_TEST_F(DiscoveryError, THttpProxyTestMock) {
//         NYdb::TDriverConfig config;
//         config.SetDatabase("/Root1/BadPath");
//         config.SetEndpoint(TStringBuilder() << "127.0.0.1:" << GRpcServerPort);
//         NYdb::TDriver driver(config);
//         NYdb::NDiscovery::TDiscoveryClient client(driver);
...
//     Y_UNIT_TEST_F(DiscoveryEmptyEndpoint, THttpProxyTestMock) {
//         NYdb::TDriverConfig config;
//         config.SetDatabase("/Root/SomePath");
//         config.SetEndpoint("");
//         NYdb::TDriver driver(config);
//         NYdb::NDiscovery::TDiscoveryClient client(driver);
...
//     Y_UNIT_TEST_F(DiscoveryGarbageEndpoint, THttpProxyTestMock) {
//         NYdb::TDriverConfig config;
//         config.SetDatabase("/Root/SomePath");
//         config.SetEndpoint("some_problematic_endpoint");
//         NYdb::TDriver driver(config);
//         NYdb::NDiscovery::TDiscoveryClient client(driver);
