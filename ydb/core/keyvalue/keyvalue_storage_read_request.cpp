...
public:
    ...
    void Bootstrap() {
        ...
        if (ReadItems.empty()) {
            ...
            YDBLOG_INFO("Inline read request",
                {"Marker", "KV320"},
                {"KeyValue", TabletInfo->TabletID},
                {"Status", status});
            ...
            if (isError) {
                ...
                YDBLOG_DEBUG("Expected OK or UNKNOWN and given " << NKikimrProto::EReplyStatus_Name(status)
                    << " readCount# " << readCount,
                    {"Marker", "KV322"});
    ...
    void Handle(TEvBlobStorage::TEvGetResult::TPtr &ev) {
        ...
        YDBLOG_INFO("Received GetResult",
            {"Marker", "KV20"},
            {"KeyValue", TabletInfo->TabletID},
