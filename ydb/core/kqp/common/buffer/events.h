...
struct TEvCommit : public TEventLocal<TEvCommit, TKqpBufferWriterEvents::EvCommit> {
    ...
    ui64 TxId;
    // Collect per-shard commit acks for the user-facing trace (full-detail sampling tier).
...
struct TEvResult : public TEventLocal<TEvResult, TKqpBufferWriterEvents::EvResult> {
    ...
    // Wall-clock windows of a distributed commit for the user-facing trace;
    // empty for immediate (single-shard) commit and flush/rollback replies.
