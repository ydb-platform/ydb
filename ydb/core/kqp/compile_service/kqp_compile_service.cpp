        ...
        if (request.Uid) {
            if (compileResult) {
                ...
                if (compileResult->Query->UserSid == userSid) {
                    ...
                    YDB_LOG_NOTICE_CTX(ctx, "Non-matching user sid for query expected actual",
                        {"sender", ev->Sender},
                        {"queryUid", *request.Uid},
        ...
        if (compileResult || request.Query) {
            ...
            if (compileResult) {
                ...
                if (query != *compileResult->Query) {
                    YDB_LOG_WARN_CTX(ctx, "QueryId in recompile request and queryId in cache are different queryId queryId",
                        {"inRequest", query.SerializeToString()},
                        {"inCache", compileResult->Query->SerializeToString()});
                }
            }
