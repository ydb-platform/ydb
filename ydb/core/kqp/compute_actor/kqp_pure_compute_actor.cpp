...
void TKqpComputeActor::DoBootstrap() {
    ...
    try {
        PrepareTaskRunner(TKqpTaskRunnerExecutionContext(TxId, RuntimeSettings.UseSpilling, ArrayBufferMinFillPercentage, std::move(wakeupCallback), std::move(errorCallback)));
