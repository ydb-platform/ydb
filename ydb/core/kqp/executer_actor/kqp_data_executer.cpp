...
public:
    ...
    bool CheckExecutionComplete() {
        ...
        for (const auto& x : ShardStates) {
            ...
            if (x.second.State != TShardState::EState::Finished) {
                notFinished++;
                STLOG_D(KQPDATA, "Datashard not finished yet",
    ...
    void Finalize() {
        ...
        if (!BufferActorId || (ReadOnlyTx && Request.LocksOp != ELocksOp::Rollback)) {
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            STLOG_D(KQPDATA, "Send Commit to BufferActor",
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            STLOG_D(KQPDATA, "Send Rollback to BufferActor",
                (buffer_actor_id, BufferActorId),
                (trace_id, TraceId()));
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            STLOG_D(KQPDATA, "Send Flush to BufferActor",
                (buffer_actor_id, BufferActorId),
                (trace_id, TraceId()));
    ...
    void HandleFinalize(TEvKqp::TEvAbortExecution::TPtr& ev) {
        if (IsCancelAfterAllowed(ev)) {
            ...
            STLOG_D(KQPDATA, "Got TEvAbortExecution, but cancellation is not allowed",
                (sender, ev->Sender),
                (trace_id, TraceId()));
        }
    }
    ...
    void HandleFinalize(TEvents::TEvUndelivered::TPtr& ev) {
        ...
        STLOG_W(KQPDATA, "Got Undelivered from BufferActor",
            (sender, ev->Sender),
            (trace_id, TraceId()));
    }
...
private:
    ...
    void HandlePrepare(TEvPersQueue::TEvProposeTransactionResult::TPtr& ev) {
        ...
        STLOG_D(KQPDATA, "Got propose result",
...
//        }
    ...
    void HandlePrepare(NKikimr::NEvents::TDataEvents::TEvWriteResult::TPtr& ev) {
        ...
        NDataIntegrity::LogIntegrityTrails("Prepare", Request.UserTraceId, ev, TlsActivationContext->AsActorContext());
        STLOG_D(KQPDATA, "Recv EvWriteResult (prepare)",
            (shard_id, shardId),
        ...
        switch (ev->Get()->GetStatus()) {
            ...
            case NKikimrDataEvents::TEvWriteResult::STATUS_LOCKS_BROKEN: {
                STLOG_D(KQPDATA, "Broken locks: " << res->Record.DebugString(),
                    (trace_id, TraceId()));
            ...
            case NKikimrDataEvents::TEvWriteResult::STATUS_OVERLOADED: {
                if (res->Record.HasOverloadSubscribed()) {
                    STLOG_D(KQPDATA, "Shard is overloaded, waiting",
                        (shard_id, shardId),
                        (trace_id, TraceId()));
                    return;
                }
    ...
    void HandlePrepare(TEvDataShard::TEvProposeTransactionAttachResult::TPtr& ev) {
        ...
        if (record.GetStatus() == NKikimrProto::OK) {
            ...
            STLOG_D(KQPDATA, "Reattached to shard",
                (shard_id, tabletId),
        ...
        STLOG_E(KQPDATA, "Shard transaction lost during reconnect",
            (shard_id, tabletId),
    ...
    void HandlePrepare(TEvPipeCache::TEvDeliveryProblem::TPtr& ev) {
        ...
        switch (shardState->State) {
            case TShardState::EState::Preparing: {
                ...
                STLOG_I(KQPDATA, "Shard propose error",
            ...
            case TShardState::EState::Prepared: {
                ...
                {
                    STLOG_N(KQPDATA, "Shard delivery problem (already prepared, reattaching in "
                ...
                STLOG_N(KQPDATA, "Shard delivery problem (already prepared)",
                    (shard_id, msg->TabletId),
                    (last_message_not_delivered, msg->NotDelivered),
                    (trace_id, TraceId()));
    ...
    void CancelProposal(ui64 exceptShardId) {
        for (auto& [shardId, state] : ShardStates) {
            ...
            {
                ui64 id = shardId;
                STLOG_D(KQPDATA, "Send CancelTransactionProposal to shard",
                    (shard_id, id),
                    (trace_id, TraceId()));
    ...
    bool ShardPreparedImpl(TShardState& state, const E& result) {
        ...
        if (!TxCoordinator || TxCoordinator != coordinator) {
            STLOG_E(KQPDATA, "Handle TEvProposeTransactionResult: unable to select coordinator. Tx canceled",
                (actor_id, SelfId()),
    ...
    void CheckPrepareCompleted() {
        ...
        for (const auto& [_, state] : ShardStates) {
            if (state.State != TShardState::EState::Prepared) {
                STLOG_D(KQPDATA, "Not all shards are prepared, waiting...",
                    (trace_id, TraceId()));
                return;
            }
        }
        ...
        STLOG_D(KQPDATA, "All shards prepared, become ExecuteState.",
            (trace_id, TraceId()));
    ...
    void ExecutePlanned() {
        ...
        STLOG_D(KQPDATA, "Execute planned transaction",
...
//                }
    ...
    void HandleExecute(NKikimr::NEvents::TDataEvents::TEvWriteResult::TPtr& ev) {
        ...
        STLOG_D(KQPDATA, "Recv EvWriteResult (execute)",
            (shard_id, shardId),
            (status, NKikimrDataEvents::TEvWriteResult::EStatus_Name(ev->Get()->GetStatus())),
            (tx_id, ev->Get()->Record.GetTxId()),
            (locks, [&]() {
    ...
    void HandleExecute(TEvDataShard::TEvProposeTransactionResult::TPtr& ev) {
        ...
        switch (res->GetStatus()) {
            case NKikimrTxDataShard::TEvProposeTransactionResult::COMPLETE: {
                ...
                for (auto& lock : res->Record.GetTxLocks()) {
                    STLOG_D(KQPDATA, "Shard completed, store lock",
                        (shard_id, shardId),
    ...
    void HandleExecute(TEvDataShard::TEvProposeTransactionRestart::TPtr& ev) {
        ...
        STLOG_D(KQPDATA, "Got transaction restart event",
            (tablet_id, shardId),
            (state, ToString(shardState->State)),
    ...
    void HandleExecute(TEvDataShard::TEvProposeTransactionAttachResult::TPtr& ev) {
        ...
        if (record.GetStatus() == NKikimrProto::OK) {
            // Transaction still exists at this shard
            STLOG_N(KQPDATA, "Reattached to shard",
                (shard_id, tabletId),
        ...
        STLOG_E(KQPDATA, "Transaction lost during reconnect",
            (ShardId, tabletId),
            (status, record.GetStatus()),
            (trace_id, TraceId()));
    ...
    void HandleExecute(TEvPrivate::TEvReattachToShard::TPtr& ev) {
        ...
        STLOG_I(KQPDATA, "Reattach to shard",
            (ShardId, tabletId),
            (trace_id, TraceId()));
    ...
    void HandleExecute(TEvTxProxy::TEvProposeTransactionStatus::TPtr &ev) {
        ...
        STLOG_D(KQPDATA, "Got transaction status",
    ...
    void HandleExecute(TEvPipeCache::TEvDeliveryProblem::TPtr& ev) {
        ...
        STLOG_D(KQPDATA, "DeliveryProblem to shard",
        ...
        if (msg->TabletId == TxCoordinator) {
            if (msg->NotDelivered) {
                STLOG_E(KQPDATA, "Not delivered to coordinator, abort execution",
            ...
            STLOG_E(KQPDATA, "Delivery problem to coordinator, abort execution",
                (tablet_id, msg->TabletId),
                (trace_id, TraceId()));
        ...
        switch (shardState->State) {
            case TShardState::EState::Prepared: // is it correct?
                STLOG_E(KQPDATA, "DeliveryProblem to shard",
                    (ShardId, msg->TabletId),
                    (last_message_not_delivered, msg->NotDelivered),
                    (tx_planned, TxPlanned),
                    (coordinator, TxCoordinator),
            ...
            case TShardState::EState::Executing: {
                if ((wasRestarting || shardState->ReattachState.ReattachInfo.Reattaching) &&
                     shardState->ReattachState.ShouldReattach(TlsActivationContext->Now()))
                {
                    STLOG_N(KQPDATA, "Lost pipe while waiting for reply (reattaching in "
                        << shardState->ReattachState.ReattachInfo.Delay << ")",
                        (tablet_id, msg->TabletId),
                        (trace_id, TraceId()));
                ...
                STLOG_N(KQPDATA, "Lost pipe while waiting for reply",
                    (ShardId, msg->TabletId),
                    (last_message_not_delivered, msg->NotDelivered),
                    (trace_id, TraceId()));
...
private:
    ...
    void ExecuteDatashardTransaction(ui64 shardId, NKikimrTxDataShard::TKqpTransaction& kqpTx)
    {
        ...
        STLOG_D(KQPDATA, "Executing KQP transaction on shard",
        ...
        STLOG_D(KQPDATA, "ExecuteDatashardTransaction",
    ...
    void ExecuteEvWriteTransaction(ui64 shardId, const NKikimrDataEvents::TEvWrite& evWrite) {
        ...
        STLOG_D(KQPDATA, "Executing EvWrite (PREPARE) on shard",
            (state, CurrentStateFuncName()),
            (shard_id, shardId),
            (tx_id, TxId),
        ...
        STLOG_D(KQPDATA, "Send EvWrite to shard",
            (shard_id, shardId),
        ...
        STLOG_D(KQPDATA, "ExecuteEvWriteTransaction",
            (trace_id_verbosity, std::to_string(traceId.GetVerbosity())),
            (trace_id, TraceId()));
    ...
    void HandleResolve(TEvPrivate::TEvResourcesSnapshot::TPtr& ev) {
        if (ev->Get()->Snapshot.empty()) {
            STLOG_E(KQPDATA, "Can not find default state storage group for database",
                (database, Database),
                (trace_id, TraceId()));
        }
    ...
    void Execute() {
        ...
        if (computeTasks.size() > Request.MaxComputeActors) {
            STLOG_N(KQPDATA, "Too many compute actors",
        ...
        if (shardsLimit > 0 && shards > shardsLimit) {
            STLOG_W(KQPDATA, "Too many affected shards",
    ...
    void HandleResolve(TEvKqpExecuter::TEvTableResolveStatus::TPtr& ev) {
        ...
        if (TasksGraph.GetMeta().StreamResult || IsEnabledReadsMerge() || TasksGraph.GetMeta().AllowOlapDataQuery) {
            ...
            if ((HasOlapTable || HasDatashardSourceScan) && shardIds) {
                STLOG_D(KQPDATA, "Start resolving tablets nodes...",
    ...
    void OnShardsResolve() {
        if (ForceAcquireSnapshot()) {
            ...
            STLOG_T(KQPDATA, "Create temporary mvcc snapshot, become WaitSnapshotState",
                (trace_id, TraceId()));
...
private:
    ...
    void Handle(NLongTxService::TEvLongTxService::TEvAcquireReadSnapshotResult::TPtr& ev) {
        ...
        STLOG_T(KQPDATA, "Read snapshot result",
            (status, record.GetStatus()),
    ...
    void ContinueExecute() {
        ...
        if (ImmediateTx) {
            STLOG_D(KQPDATA, "Immediate tx, become ExecuteState",
                (current_state, CurrentStateFuncName()),
                (immediate, true),
                (trace_id, TraceId()));
            Become(&TKqpDataExecuter::ExecuteState);
        } else {
            STLOG_D(KQPDATA, "Not immediate tx, become PrepareState",
                (current_state, CurrentStateFuncName()),
                (trace_id, TraceId()));
    ...
    void ExecuteTasks() {
        ...
        // error
        STLOG_T(KQPDATA, "Updating channels after the creation of compute actors",
            (trace_id, TraceId()));
        Y_ENSURE(Planner);
    ...
    void ExecuteTopicTabletTransactions(TTopicTabletTxs& topicTxs) {
        ...
        for (auto& [tabletId, t] : topicTxs) {
            ...
            auto traceId = ExecuterSpan.GetTraceId();
            STLOG_D(KQPDATA, "ExecuteTopicTabletTransaction",
                (trace_id_verbosity, std::to_string(traceId.GetVerbosity())),
                (trace_id, TraceId()));

            STLOG_D(KQPDATA, "Executing KQP transaction on topic tablet",
                (tablet_id, tabletId),
                (write_id, writeId),
                (trace_id, TraceId()));
    ...
    void Shutdown() override {
        if (Planner) {
            if (Planner->GetPendingComputeTasks().empty() && Planner->GetPendingComputeActors().empty()) {
                STLOG_I(KQPDATA, "Shutdown immediately - nothing to wait",
                    (trace_id, TraceId()));
                PassAway();
            } else {
                this->Become(&TThis::WaitShutdownState);
                STLOG_I(KQPDATA, "Waiting for shutdown",
    ...
    STATEFN(WaitShutdownState) {
        switch(ev->GetTypeRewrite()) {
            ...
            default:
                STLOG_E(KQPDATA, "Unexpected event while waiting for shutdown",
    ...
    void HandleShutdown(TEvInterconnect::TEvNodeDisconnected::TPtr& ev) {
        ...
        STLOG_N(KQPDATA, "Node has disconnected while shutdown",
            (node_id, nodeId),
            (trace_id, TraceId()));
    ...
    void HandleShutdown(TEvents::TEvPoison::TPtr& ev) {
        ...
        STLOG_I(KQPDATA, "Timed out on waiting for Compute Actors to finish - forcing shutdown",
            (Sender, ev->Sender),
            (trace_id, TraceId()));
    ...
    void HandleShutdown(TEvDq::TEvAbortExecution::TPtr& ev) {
        ...
        if (statusCode == Ydb::StatusIds::TIMEOUT) {
            FillLocksFromExtraData();
            STLOG_I(KQPDATA, "External timeout while waiting for Compute Actors to finish - forcing shutdown",
                (Sender, ev->Sender),
                (trace_id, TraceId()));
            PassAway();
        }
    ...
    void Handle(NFq::TEvCheckpointCoordinator::TEvZeroCheckpointDone::TPtr& ev) {
        STLOG_D(KQPDATA, "Coordinator saved zero checkpoint",
            (trace_id, TraceId()));
    ...
    void Handle(NFq::TEvCheckpointCoordinator::TEvRaiseTransientIssues::TPtr& ev) {
        STLOG_N(KQPDATA, "TEvRaiseTransientIssues from checkpoint coordinator",
    ...
    void StartCheckpointCoordinator() {
        ...
        STLOG_D(KQPDATA, "Created new CheckpointCoordinator",
