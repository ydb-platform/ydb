...
public:
    ...
    bool CheckExecutionComplete() {
        ...
        if (IsDebugLogEnabled()) {
            ...
            YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << sb,
                {"Marker", "KQPDATA"},
                {"trace_id", TraceId()});
        }
        return false;
    ...
    void Finalize() {
        ...
        if (!BufferActorId || (ReadOnlyTx && Request.LocksOp != ELocksOp::Rollback)) {
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Send Commit to BufferActor",
                {"Marker", "KQPDATA"},
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Send Rollback to BufferActor",
                {"Marker", "KQPDATA"},
                {"buffer_actor_id", BufferActorId},
                {"trace_id", TraceId()});
            ...
            Become(&TKqpDataExecuter::FinalizeState);
            YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Send Flush to BufferActor",
                {"Marker", "KQPDATA"},
                {"buffer_actor_id", BufferActorId},
                {"trace_id", TraceId()});
    ...
    void HandleFinalize(TEvKqp::TEvAbortExecution::TPtr& ev) {
        if (IsCancelAfterAllowed(ev)) {
            ...
            YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Got TEvAbortExecution, but cancellation is not allowed",
                {"Marker", "KQPDATA"},
                {"sender", ev->Sender},
                {"trace_id", TraceId()});
        }
    ...
    void HandleFinalize(TEvents::TEvUndelivered::TPtr& ev) {
        ...
        YDB_LOG_WARN("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Got Undelivered from BufferActor",
            {"Marker", "KQPDATA"},
            {"sender", ev->Sender},
            {"trace_id", TraceId()});
    }
...
private:
    ...
    void Execute() {
        ...
        if (shardsLimit > 0 && shards > shardsLimit) {
            YDB_LOG_WARN("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Too many affected shards",
                {"Marker", "KQPDATA"},
    ...
    void OnShardsResolve() {
        if (ForceAcquireSnapshot()) {
            ...
            YDB_LOG_TRACE("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Create temporary mvcc snapshot, become WaitSnapshotState",
                {"Marker", "KQPDATA"},
                {"trace_id", TraceId()});
...
private:
    ...
    void Handle(NLongTxService::TEvLongTxService::TEvAcquireReadSnapshotResult::TPtr& ev) {
        ...
        YDB_LOG_TRACE("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Read snapshot result",
            {"Marker", "KQPDATA"},
            {"status", msg->Status},
    ...
    void ContinueExecute() {
        ...
        YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "become ExecuteState",
            {"Marker", "KQPDATA"},
    ...
    void ExecuteTasks() {
        ...
        YDB_LOG_INFO("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Total tasks",
            {"Marker", "KQPDATA"},
        ...
        // error
        YDB_LOG_TRACE("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Updating channels after the creation of compute actors",
            {"Marker", "KQPDATA"},
            {"trace_id", TraceId()});
        Y_ENSURE(Planner);
    ...
    void Shutdown() override {
        if (Planner) {
            if (Planner->GetPendingComputeTasks().empty() && Planner->GetPendingComputeActors().empty()) {
                YDB_LOG_INFO("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Shutdown immediately - nothing to wait",
                    {"Marker", "KQPDATA"},
                    {"trace_id", TraceId()});
                PassAway();
            } else {
                this->Become(&TThis::WaitShutdownState);
                YDB_LOG_INFO("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Waiting for shutdown",
                    {"Marker", "KQPDATA"},
    ...
    STATEFN(WaitShutdownState) {
        switch(ev->GetTypeRewrite()) {
            ...
            default:
                YDB_LOG_ERROR("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Unexpected event while waiting for shutdown",
                    {"Marker", "KQPDATA"},
    ...
    void HandleShutdown(TEvInterconnect::TEvNodeDisconnected::TPtr& ev) {
        ...
        YDB_LOG_NOTICE("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Node has disconnected while shutdown",
            {"Marker", "KQPDATA"},
            {"node_id", nodeId},
            {"trace_id", TraceId()});
    ...
    void HandleShutdown(TEvents::TEvPoison::TPtr& ev) {
        ...
        YDB_LOG_INFO("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Timed out on waiting for Compute Actors to finish - forcing shutdown",
            {"Marker", "KQPDATA"},
            {"Sender", ev->Sender},
            {"trace_id", TraceId()});
    ...
    void HandleShutdown(TEvDq::TEvAbortExecution::TPtr& ev) {
        ...
        if (statusCode == Ydb::StatusIds::TIMEOUT) {
            FillLocksFromExtraData();
            YDB_LOG_INFO("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "External timeout while waiting for Compute Actors to finish - forcing shutdown",
                {"Marker", "KQPDATA"},
                {"Sender", ev->Sender},
                {"trace_id", TraceId()});
            PassAway();
    ...
    void Handle(NFq::TEvCheckpointCoordinator::TEvZeroCheckpointDone::TPtr& ev) {
        YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Coordinator saved zero checkpoint",
            {"Marker", "KQPDATA"},
            {"trace_id", TraceId()});
    ...
    void Handle(NFq::TEvCheckpointCoordinator::TEvRaiseTransientIssues::TPtr& ev) {
        YDB_LOG_NOTICE("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "TEvRaiseTransientIssues from checkpoint coordinator",
            {"Marker", "KQPDATA"},
    ...
    void StartCheckpointCoordinator() {
        ...
        YDB_LOG_DEBUG("ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Created new CheckpointCoordinator",
            {"Marker", "KQPDATA"},
