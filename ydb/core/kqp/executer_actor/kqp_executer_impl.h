...
public:
        ...
        , EnableWatermarks(executerConfig.TableServiceConfig.GetEnableWatermarks())
    {
        ...
        // Collection depth: the deeper of what the client asked and what the user trace needs.
        // Everything exported to the client is gated by Request.StatsMode — sampling must not
        // change the response.
...
protected:
    ...
    // max(Request.StatsMode, Request.UserFacingTraceCollectionMode); drives collection only —
    // client-facing exports use Request.StatsMode.
