...
namespace NKikimr {
namespace NKqp {
#define STLOG_T(MARKER, MESSAGE, ...) STLOG(PRI_TRACE, NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_D(MARKER, MESSAGE, ...) STLOG(PRI_DEBUG, NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_I(MARKER, MESSAGE, ...) STLOG(PRI_INFO,  NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_N(MARKER, MESSAGE, ...) STLOG(PRI_NOTICE, NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_W(MARKER, MESSAGE, ...) STLOG(PRI_WARN,  NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_E(MARKER, MESSAGE, ...) STLOG(PRI_ERROR, NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
#define STLOG_C(MARKER, MESSAGE, ...) STLOG(PRI_CRIT,  NKikimrServices::KQP_EXECUTER, MARKER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << MESSAGE, __VA_ARGS__)
...
public:
        ...
        , TasksGraph(Database, Request.Transactions, Request.TxAlloc, partitionPrunerConfig, AggregationSettings, Counters, BufferActorId)
    {
        ...
        STLOG_T(KQPEX, "Bootstrap done, become ReadyState",
            (trace_id, TraceId()));
    }
...
protected:
    ...
    bool HandleResolve(NShardResolver::TEvShardsResolveStatus::TPtr& ev) {
        ...
        if (reply.Status != Ydb::StatusIds::SUCCESS) {
            ...
            STLOG_W(KQPEX, "Shards nodes resolve failed",
        ...
        STLOG_D(KQPEX, "Shards nodes resolved",
        ...
        ui32 channelId, ui32 seqNo, bool finished)
    {
        ...
        STLOG_D(KQPEX, "Send TEvStreamData",
            (Recipient, Target),
    ...
    void HandleChannelData(NYql::NDq::TEvDqCompute::TEvChannelData::TPtr& ev) {
        ...
        if (TasksGraph.GetMeta().StreamResult && txResult.IsStream && txResult.QueryResultIndex.Defined()) {
            ...
            if (!trailingResults) {
                ...
                STLOG_D(KQPEX, "Staging TEvStreamData",
                    (Recipient, Target),
    ...
    bool HandleComputeStats(NYql::NDq::TEvDqCompute::TEvState::TPtr& ev) {
        ...
        STLOG_D(KQPEX, "Got execution state from compute actor",
    ...
    void HandleReady(TEvKqpExecuter::TEvTxRequest::TPtr ev) {
        ...
        if (IsDebugLogEnabled()) {
            for (auto& tx : Request.Transactions) {
                STLOG_D(KQPEX, "Executing physical tx",
        ...
        STLOG_T(KQPEX, "Got request, become WaitResolveState",
            (trace_id, TraceId()));
...
protected:
            ...
            case TEvKqpNode::TEvStartKqpTasksRequest::EventType: {
                switch (reason) {
                    case TEvents::TEvUndelivered::EReason::ReasonActorUnknown: {
                        STLOG_D(KQPEX, "Schedule a retry by ActorUnknown reason",
    ...
    void HandleDisconnected(TEvInterconnect::TEvNodeDisconnected::TPtr& ev) {
        ...
        STLOG_N(KQPEX, "Disconnected node", (node_id, nodeId),
            (trace_id, TraceId()));
            ...
            switch (reason) {
                ...
                case NKikimrKqp::TEvStartKqpTasksResponse::NODE_SHUTTING_DOWN: {
                    if (!AppData()->FeatureFlags.GetEnableShuttingDownNodeState()) {
                        STLOG_D(KQPEX, "Received NODE_SHUTTING_DOWN but feature flag EnableShuttingDownNodeState is disabled",
                            (trace_id, TraceId()));
                    ...
                    STLOG_D(KQPEX, "Received NODE_SHUTTING_DOWN, attempting run tasks locally",
                        (trace_id, TraceId()));
    ...
    void HandleAbortExecution(
        ...
        STLOG_D(KQPEX, "Got EvAbortExecution",
...
protected:
    void UpdateResourcesUsage(bool force) {
        ...
        if (Request.RlPath) {
            ...
            STLOG_D(KQPEX, "Resource usage for last stat interval",
            ...
            STLOG_D(KQPEX, "Resource usage for last stat interval, rate limiter was not found",
                (Consumption, consumption),
                (RequestUnits, ru),
                (ForceFlag, force),
                (trace_id, TraceId()));
    ...
    void TerminateComputeActors(Ydb::StatusIds::StatusCode code, const NYql::TIssues& issues) {
        for (const auto& task : this->TasksGraph.GetTasks()) {
            if (task.ComputeActorId && !task.Meta.Completed) {
                STLOG_I(KQPEX, "Aborting compute actor execution",
                    (Issues, issues.ToOneLineString()),
                ...
                STLOG_I(KQPEX, "Task does not have the CA id yet or is already complete",
                    (TaskId, task.Id),
                    (trace_id, TraceId()));
            }
        }
...
protected:
    ...
    void UnexpectedEvent(const TString& state, ui32 eventType) {
        if (eventType == TEvents::TEvPoison::EventType) {
            STLOG_D(KQPEX, "TKqpExecuter, TEvPoison event",
                (State, state),
                (SelfId, this->SelfId()),
                (trace_id, TraceId()));
            InternalError(TStringBuilder() << "TKqpExecuter got poisoned, state: " << state);
        } else {
            STLOG_E(KQPEX, "TKqpExecuter, unexpected event",
                (EventType, eventType),
                (State, state),
                (SelfId, this->SelfId()),
                (trace_id, TraceId()));
    ...
    void InternalError(const NYql::TIssues& issues) {
        STLOG_E(KQPEX, issues.ToOneLineString(),
            (trace_id, TraceId()));
    ...
    void ReplyUnavailable(const TString& message) {
        STLOG_E(KQPEX, "UNAVAILABLE: " << message,
            (trace_id, TraceId()));
    ...
    void TimeoutError(bool sessionSender, NYql::TIssues issues) {
        if (AlreadyReplied) {
            STLOG_E(KQPEX, "Timeout when we already replied - not good",
        ...
        STLOG_E(KQPEX, "Sending timeout response", (Recipient, Target),
            (trace_id, TraceId()));
    ...
    {
        if (AlreadyReplied) {
            STLOG_E(KQPEX, "Error when we already replied - not good",
                (Backtrace, TBackTrace().PrintToString()),
                (trace_id, TraceId()));
            return;
        }
        ...
        STLOG_T(KQPEX, "ReplyErrorAndDie",
    ...
    bool ValidateTaskSize(const TCollection& tasks) {
        for (const auto& task : tasks) {
            if (ui32 size = task->ByteSize(); size > MaxTaskSize) {
                STLOG_E(KQPEX, "Abort execution. Task size is too big",
                    (TaskId, task->GetId()),
                    (Size, size),
                    (MaxSize, MaxTaskSize),
                    (trace_id, TraceId()));
...
protected:
    ...
    void PassAway() override {
        ...
        // Fill response stats
        {
            ...
            if (Stats->CollectStatsByLongTasks) {
                ...
                if (!txPlansWithStats.empty()) {
                    STLOG_I(KQPEX, "Full stats: " << response.GetResult().GetStats(),
                        (trace_id, TraceId()));
                }
            }
        ...
        for (auto channelPair: ResultChannelProxies) {
            STLOG_D(KQPEX, "Terminate result channel",
        ...
        STLOG_D(KQPEX, "Terminate execution",
            (trace_id, TraceId()));
        ...
        STLOG_T(KQPEX, "Terminate, become ZombieState",
            (trace_id, TraceId()));
