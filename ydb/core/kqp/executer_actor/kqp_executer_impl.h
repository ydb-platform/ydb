...
public:
        ...
        , EnableWatermarks(executerConfig.TableServiceConfig.GetEnableWatermarks())
    {
        ...
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. Bootstrap done, become ReadyState",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
...
protected:
    ...
    ETableResolveStatus HandleResolve(TEvKqpExecuter::TEvTableResolveStatus::TPtr& ev) {
        ...
        if (shardIds.size() > 0) {
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Start resolving tablets nodes...",
                {"Marker", "KQPDATA"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
    ...
    bool HandleResolve(NShardResolver::TEvShardsResolveStatus::TPtr& ev) {
        ...
        if (reply.Status != Ydb::StatusIds::SUCCESS) {
            ...
            YDB_LOG_COMP_WARN(NKikimrServices::KQP_EXECUTER, ".. Shards nodes resolve failed",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Shards nodes resolved",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
        ...
        if (IsDebugLogEnabled()) {
            ...
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ". .",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
        ...
        ui32 channelId, ui32 seqNo, bool finished)
    {
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Send TEvStreamData",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void HandleChannelData(NYql::NDq::TEvDqCompute::TEvChannelData::TPtr& ev) {
        ...
        if (TasksGraph.GetMeta().StreamResult && txResult.IsStream && txResult.QueryResultIndex.Defined()) {
            ...
            if (!trailingResults) {
                ...
                ResponseEv->TakeResult(channel.DstInputIndex, std::move(batch));
                txResult.HasTrailingResult = true;
                YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Staging TEvStreamData",
                    {"Marker", "KQPEX"},
                    {"ActorId", SelfId()},
                    {"TxId", TxId},
                    {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. Got result",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
        ...
        ResponseEv->TakeResult(channel.DstInputIndex, std::move(batch));
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. Send ack",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void HandleStreamAck(TEvKqpExecuter::TEvStreamDataAck::TPtr& ev) {
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Send ack",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    bool HandleComputeStats(NYql::NDq::TEvDqCompute::TEvState::TPtr& ev) {
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Got execution state from compute actor",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void HandleReady(TEvKqpExecuter::TEvTxRequest::TPtr ev) {
        ...
        if (IsDebugLogEnabled()) {
            for (auto& tx : Request.Transactions) {
                YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Executing physical tx",
                    {"Marker", "KQPEX"},
                    {"ActorId", SelfId()},
                    {"TxId", TxId},
                    {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. Got request, become WaitResolveState",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
...
protected:
    ...
    void HandleUndelivered(TEvents::TEvUndelivered::TPtr& ev) {
        ...
        switch (eventType) {
            case TEvKqpNode::TEvStartKqpTasksRequest::EventType: {
                switch (reason) {
                    case TEvents::TEvUndelivered::EReason::ReasonActorUnknown: {
                        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Schedule a retry by ActorUnknown reason",
                            {"Marker", "KQPEX"},
                            {"ActorId", SelfId()},
                            {"TxId", TxId},
                            {"Ctx", *GetUserRequestContext()},
            ...
            default: {
                YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Event lost",
                    {"Marker", "KQPEX"},
                    {"ActorId", SelfId()},
                    {"TxId", TxId},
                    {"Ctx", *GetUserRequestContext()},
    ...
    void HandleDisconnected(TEvInterconnect::TEvNodeDisconnected::TPtr& ev) {
        ...
        YDB_LOG_COMP_NOTICE(NKikimrServices::KQP_EXECUTER, ".. Disconnected node",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void HandleStartKqpTasksResponse(TEvKqpNode::TEvStartKqpTasksResponse::TPtr& ev) {
        ...
        if (record.NotStartedTasksSize() != 0) {
            ...
            YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Stop executing",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
            ...
            switch (reason) {
                ...
                case NKikimrKqp::TEvStartKqpTasksResponse::NODE_SHUTTING_DOWN: {
                    if (!AppData()->FeatureFlags.GetEnableShuttingDownNodeState()) {
                        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Received NODE_SHUTTING_DOWN but feature flag EnableShuttingDownNodeState is disabled",
                            {"Marker", "KQPEX"},
                            {"ActorId", SelfId()},
                            {"TxId", TxId},
                            {"Ctx", *GetUserRequestContext()},
                    ...
                    YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Received NODE_SHUTTING_DOWN, attempting run tasks locally",
                        {"Marker", "KQPEX"},
                        {"ActorId", SelfId()},
                        {"TxId", TxId},
                        {"Ctx", *GetUserRequestContext()},
        ...
        for (auto& startedTask : record.GetStartedTasks()) {
            ...
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Executing task",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
    ...
    void HandleAbortExecution(
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Got EvAbortExecution",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
...
protected:
    void UpdateResourcesUsage(bool force) {
        ...
        if (Request.RlPath) {
            ...
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Resource usage for last stat interval",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
            ...
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Resource usage for last stat interval, rate limiter was not found",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
    ...
    void TerminateComputeActors(Ydb::StatusIds::StatusCode code, const NYql::TIssues& issues) {
        for (const auto& task : this->TasksGraph.GetTasks()) {
            if (task.ComputeActorId && !task.Meta.Completed) {
                YDB_LOG_COMP_INFO(NKikimrServices::KQP_EXECUTER, ".. Aborting compute actor execution",
                    {"Marker", "KQPEX"},
                    {"ActorId", SelfId()},
                    {"TxId", TxId},
                    {"Ctx", *GetUserRequestContext()},
                ...
                YDB_LOG_COMP_INFO(NKikimrServices::KQP_EXECUTER, ".. Task does not have the CA id yet or is already complete",
                    {"Marker", "KQPEX"},
                    {"ActorId", SelfId()},
                    {"TxId", TxId},
                    {"Ctx", *GetUserRequestContext()},
...
protected:
    ...
    void UnexpectedEvent(const TString& state, ui32 eventType) {
        if (eventType == TEvents::TEvPoison::EventType) {
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. TKqpExecuter, TEvPoison event",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
            ...
            YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. TKqpExecuter, unexpected event",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
    ...
    void InternalError(const NYql::TIssues& issues) {
        YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ". .",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void RuntimeError(Ydb::StatusIds::StatusCode code, const NYql::TIssues& issues) {
        YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Runtime error",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    void TimeoutError(bool isTargetSender, NYql::TIssues issues) {
        if (AlreadyReplied) {
            YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Timeout when we already replied - not good",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Abort execution",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Sending timeout response",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
    ...
    {
        if (AlreadyReplied) {
            YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, ".. Error when we already replied - not good",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. ReplyErrorAndDie",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
...
protected:
    ...
    void PassAway() override {
                ...
                if (CollectFullStats(Request.StatsMode)) {
                    ...
                    if (Stats->CollectStatsByLongTasks) {
                        ...
                        if (!txPlansWithStats.empty()) {
                            YDB_LOG_COMP_INFO(NKikimrServices::KQP_EXECUTER, ".. Full",
                                {"Marker", "KQPEX"},
                                {"ActorId", SelfId()},
                                {"TxId", TxId},
                                {"Ctx", *GetUserRequestContext()},
        ...
        for (auto channelPair: ResultChannelProxies) {
            YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Terminate result channel",
                {"Marker", "KQPEX"},
                {"ActorId", SelfId()},
                {"TxId", TxId},
                {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, ".. Terminate execution",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
        ...
        YDB_LOG_COMP_TRACE(NKikimrServices::KQP_EXECUTER, ".. Terminate, become ZombieState",
            {"Marker", "KQPEX"},
            {"ActorId", SelfId()},
            {"TxId", TxId},
            {"Ctx", *GetUserRequestContext()},
