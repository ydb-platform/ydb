...
void TJoinInputStats::Resize(ui32 taskCount) {
    BloomStartMs.resize(taskCount);
    BloomPositive.resize(taskCount);
    BloomNegative.resize(taskCount);
...
void TStageExecutionStats::UpdateJoinInputStats(ui32 index, TJoinInputStats& joinInputStats, const NYql::NDqProto::TDqJoinInputStats& joinInputStat) {
    SetNonZero(joinInputStats.BloomStartMs[index], joinInputStat.GetBloomStartMs());
    SetNonZero(joinInputStats.BloomPositive[index], joinInputStat.GetBloomPositive());
    SetNonZero(joinInputStats.BloomNegative[index], joinInputStat.GetBloomNegative());
...
void UpdateJoinInputAggr(NDqProto::TDqJoinInputStatsAggr& inputStats, const NDqProto::TDqJoinInputStats& inputStat) {
    UpdateAggr(inputStats.MutableBloomStartMs(), inputStat.GetBloomStartMs());
    UpdateAggr(inputStats.MutableBloomPositive(), inputStat.GetBloomPositive());
    UpdateAggr(inputStats.MutableBloomNegative(), inputStat.GetBloomNegative());
...
void TQueryExecutionStats::ExportExecStats(NYql::NDqProto::TDqExecutionStats& stats) {
    ...
    for (auto& p : StageStats) {
        ...
        for (auto& p2 : p.second.Operators) {
            ...
            ExportAggStats(p2.second.Left.BloomStartMs, *leftStat.MutableBloomStartMs());
            ExportAggStats(p2.second.Left.BloomPositive, *leftStat.MutableBloomPositive());
            ExportAggStats(p2.second.Left.BloomNegative, *leftStat.MutableBloomNegative());
            ExportAggStats(p2.second.Left.BloomSkipped, *leftStat.MutableBloomSkipped());
            auto& rightStat = *joinStat.MutableRight();
            ExportAggStats(p2.second.Right.BloomStartMs, *rightStat.MutableBloomStartMs());
            ExportAggStats(p2.second.Right.BloomPositive, *rightStat.MutableBloomPositive());
            ExportAggStats(p2.second.Right.BloomNegative, *rightStat.MutableBloomNegative());
...
void TQueryExecutionStats::AdjustBaseTime(NDqProto::TDqStageStats* stageStats) {
    ...
    for (auto& p : *stageStats->MutableOperator()) {
        if (p.second.HasJoin()) {
            auto& join = *p.second.MutableJoin();
            if (join.HasLeft()) {
                auto& left = *join.MutableLeft();
                if (left.HasBloomStartMs()) {
                    AdjustDqStatsAggr(*left.MutableBloomStartMs());
                }
            }
            if (join.HasRight()) {
                auto& right = *join.MutableRight();
                if (right.HasBloomStartMs()) {
                    AdjustDqStatsAggr(*right.MutableBloomStartMs());
                }
            }
        }
    }
