...
public:
        ...
        , UserRequestContext(userRequestContext)
    {
        ...
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Begin literal execution",
            {"Marker", "KQPLIT"},
    ...
    std::unique_ptr<TEvKqpExecuter::TEvTxResponse> ExecuteLiteral() {
        try {
            ...
            YDB_LOG_COMP_WARN(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "TKqpLiteralExecuter, memory limit exceeded.",
                {"Marker", "KQPLIT"},
                {"trace_id", TraceId()});
            ...
            YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "TKqpLiteralExecuter, TKqpEnsure failed.",
                {"Marker", "KQPLIT"},
                {"trace_id", TraceId()});
            CreateErrorResponse(Ydb::StatusIds::PRECONDITION_FAILED,
            ...
            YDB_LOG_COMP_CRIT(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "TKqpLiteralExecuter, unexpected exception caught: " << msg,
                {"Marker", "KQPLIT"},
                {"trace_id", TraceId()});
            CreateErrorResponse(Ydb::StatusIds::PRECONDITION_FAILED,
    ...
    void Finalize() {
        ...
        CleanupCtx();
        YDB_LOG_COMP_DEBUG(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Execution is complete",
            {"Marker", "KQPLIT"},
...
private:
    ...
    bool TerminateIfTimeout() {
        ...
        if (Deadline && *Deadline <= now) {
            YDB_LOG_COMP_INFO(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "Timeout exceeded.",
                {"Marker", "KQPLIT"},
                {"trace_id", TraceId()});
        ...
        if (CancelAt && *CancelAt <= now) {
            YDB_LOG_COMP_INFO(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << "CancelAt exceeded.",
                {"Marker", "KQPLIT"},
                {"trace_id", TraceId()});
...
private:
    ...
    void InternalError(const TString& message) {
        YDB_LOG_COMP_ERROR(NKikimrServices::KQP_EXECUTER, "ActorId: " << SelfId() << " TxId: " << TxId << ". " << "Ctx: " << *GetUserRequestContext() << ". " << message,
            {"Marker", "KQPLIT"},
            {"trace_id", TraceId()});
