...
public:
    ...
    void Bootstrap() {
        ...
        YDB_LOG_INFO(LogPrefix() << "Start resolving table partitions" << '.',
            {"Marker", "KQPPEA"});
    ...
    void HandlePrepare(TEvTxProxySchemeCache::TEvResolveKeySetResult::TPtr& ev) {
        ...
        YDB_LOG_TRACE(LogPrefix() << "Partitions were resolved" << '.',
            {"Marker", "KQPPEA"},
    ...
    void HandleAbort(TEvKqp::TEvAbortExecution::TPtr& ev) {
        ...
        YDB_LOG_ERROR(LogPrefix() << "Got abort execution" << '.',
            {"Marker", "KQPPEA"},
            {"Sender", ev->Sender},
    ...
    void HandleExecute(TEvKqpExecuter::TEvTxResponse::TPtr& ev) {
        ...
        if (it == ExecuterToPartition.end()) {
            YDB_LOG_WARN(LogPrefix() << "Got tx response from an unknown executer" << '.',
                {"Marker", "KQPPEA"},
                {"Sender", ev->Sender},
        ...
        YDB_LOG_TRACE(LogPrefix() << "Got tx response" << '.',
            {"Marker", "KQPPEA"},
            {"Sender", ev->Sender},
        ...
        switch (response->GetStatus()) {
            ...
            case Ydb::StatusIds::UNDETERMINED:
                YDB_LOG_DEBUG(LogPrefix() << "Executer retriable error, will retry" << '.',
                    {"Marker", "KQPPEA"},
                    {"PartitionIndex", partInfo->PartitionIndex},
                    {"Status", response->GetStatus()},
                    {"Issues", issues.ToOneLineString()});
        ...
        YDB_LOG_ERROR(LogPrefix() << "Executer unretriable error" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", partInfo->PartitionIndex},
            {"Status", response->GetStatus()},
            {"Issues", issues.ToOneLineString()});
    ...
    void HandleExecute(TEvKqpExecuter::TEvTxDelayedExecution::TPtr& ev) {
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Delayed execution timer fired" << '.',
            {"Marker", "KQPPEA"},
    ...
    void HandleExecute(TEvKqpBuffer::TEvError::TPtr& ev) {
        ...
        if (it == BufferToPartition.end()) {
            YDB_LOG_WARN(LogPrefix() << "Got error from an unknown buffer" << '.',
                {"Marker", "KQPPEA"},
                {"Sender", ev->Sender},
        ...
        YDB_LOG_TRACE(LogPrefix() << "Got buffer error" << '.',
            {"Marker", "KQPPEA"},
            {"Sender", ev->Sender},
            {"PartitionIndex", partInfo->PartitionIndex},
            {"Status", NYql::NDqProto::StatusIds_StatusCode_Name(msg.StatusCode)});
        ...
        switch (msg.StatusCode) {
            ...
            case NYql::NDqProto::StatusIds::UNDETERMINED:
                YDB_LOG_DEBUG(LogPrefix() << "Buffer retriable error, will retry" << '.',
                    {"Marker", "KQPPEA"},
                    {"PartitionIndex", partInfo->PartitionIndex},
                    {"Status", NYql::NDqProto::StatusIds_StatusCode_Name(msg.StatusCode)},
                    {"Issues", msg.Issues.ToOneLineString()});
        ...
        YDB_LOG_ERROR(LogPrefix() << "Buffer unretriable error" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", partInfo->PartitionIndex},
            {"Status", NYql::NDqProto::StatusIds_StatusCode_Name(msg.StatusCode)},
            {"Issues", msg.Issues.ToOneLineString()});
    ...
    STFUNC(AbortState) {
        try {
            ...
            default:
                YDB_LOG_WARN(LogPrefix() << "Got an unknown event" << '.',
                    {"Marker", "KQPPEA"},
                    {"Sender", ev->Sender},
                    {"EventType", ev->GetTypeRewrite()});
    ...
    void HandleAbort(TEvKqpBuffer::TEvError::TPtr& ev) {
        ...
        YDB_LOG_ERROR(LogPrefix() << "Got buffer error" << '.',
            {"Marker", "KQPPEA"},
            {"Sender", ev->Sender},
            {"PartitionIndex", partInfo->PartitionIndex},
            {"Status", NYql::NDqProto::StatusIds_StatusCode_Name(msg.StatusCode)},
...
private:
    ...
    void FillTableMetaInfo() {
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Filling table meta info" << '.',
            {"Marker", "KQPPEA"},
            {"TableId", TableId},
    ...
    void ResolvePartitioning() {
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Resolving table partitioning" << '.',
            {"Marker", "KQPPEA"},
            {"TableId", TableId},
    ...
    void CreateExecutersWithBuffers() {
        ...
        YDB_LOG_INFO(LogPrefix() << "Starting execution, creating executers with buffers" << '.',
            {"Marker", "KQPPEA"},
    ...
    TBatchPartitionInfo::TPtr CreatePartition(TPartitionIndex idx) {
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Created partition" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", idx},
    ...
    void CreateExecuterWithBuffer(TPartitionIndex partitionIndex, bool isRetry) {
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Created executer" << '.',
            {"Marker", "KQPPEA"},
    ...
    void Abort() {
        YDB_LOG_INFO(LogPrefix() << "Entering AbortState, trying to finish execution" << '.',
            {"Marker", "KQPPEA"},
    ...
    void OnSuccessResponse(TBatchPartitionInfo::TPtr& partInfo, TEvKqpExecuter::TEvTxResponse* ev) {
        ...
        if (minKey) {
            ...
            YDB_LOG_DEBUG(LogPrefix() << "Partition has more data, continue processing" << '.',
                {"Marker", "KQPPEA"},
                {"PartitionIndex", partInfo->PartitionIndex},
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Partition finished completely" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", partInfo->PartitionIndex});
    ...
    void RetryPartExecution(const TBatchPartitionInfo::TPtr& partInfo) {
        if (CurrentStateFunc() != &TKqpPartitionedExecuter::AbortState) {
            YDB_LOG_DEBUG(LogPrefix() << "Retrying partition" << '.',
                {"Marker", "KQPPEA"},
                {"PartitionIndex", partInfo->PartitionIndex},
                {"LimitSize", partInfo->LimitSize},
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Partition retry cancelled due to AbortState" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", partInfo->PartitionIndex});
    ...
    void ScheduleRetryWithNewLimit(TBatchPartitionInfo::TPtr& partInfo) {
        if (partInfo->RetryDelayMs == Settings.MaxRetryDelayMs) {
            YDB_LOG_ERROR(LogPrefix() << "Partition reached maximum retry delay" << '.',
                {"Marker", "KQPPEA"},
                {"PartitionIndex", partInfo->PartitionIndex},
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Scheduling retry for partition" << '.',
            {"Marker", "KQPPEA"},
            {"PartitionIndex", partInfo->PartitionIndex},
            {"OldDelay", oldDelay},
    ...
    bool IsColumnsNeedReorder(const TVector<ui32>& rowColumnIds) {
        ...
        for (size_t i = 0; i < KeyIds.size(); ++i) {
            ...
            if (it->second != i) {
                YDB_LOG_DEBUG(LogPrefix() << "Key columns need reorder to continue processing" << '.',
                    {"Marker", "KQPPEA"},
                    {"KeyColumnsCount", KeyIds.size()});
    ...
    void TryFinishExecution() {
        if (CheckExecutersAreFinished()) {
            YDB_LOG_INFO(LogPrefix() << "All partitions processed, finish execution" << '.',
                {"Marker", "KQPPEA"},
        ...
        YDB_LOG_DEBUG(LogPrefix() << "Not all partitions have been processed, cannot finish execution" << '.',
            {"Marker", "KQPPEA"},
    ...
    void AbortWithError(Ydb::StatusIds::StatusCode code, const NYql::TIssues& issues) {
        if (CurrentStateFunc() == &TKqpPartitionedExecuter::AbortState) {
            YDB_LOG_NOTICE(LogPrefix() << "Ignoring error because already in AbortState" << '.',
                {"Marker", "KQPPEA"},
        ...
        YDB_LOG_ERROR(LogPrefix() << "First error occurred" << '.',
            {"Marker", "KQPPEA"},
            {"Status", Ydb::StatusIds_StatusCode_Name(code)},
            {"Issues", issues.ToOneLineString()});
