...
void TKqpPlanner::ExecuteDataComputeTask(ui64 taskId, size_t computeTasksSize) {
    ...
    task.ComputeActorId = CaFactory_->CreateKqpComputeActor(
        ExecuterId, TxId, taskDesc, settings, NWilson::TTraceId(ExecuterSpan.GetTraceId()),
        TasksGraph.GetMeta().GetArenaIntrusivePtr(), SerializedGUCSettings, computeTasksSize,
        OutputChunkMaxSize, WithSpilling, GetDqStatsMode(StatsMode), Deadline,
        NRm::EKqpMemoryPool::DataQuery, (computeTasksSize <= 1),
        RlPath);
