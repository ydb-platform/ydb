...
public:
    ...
    TKqpScanExecuter(IKqpGateway::TExecPhysicalRequest&& request, const TString& database,
        ...
        TOutputFormat outputFormat, TKqpRequestCounters::TPtr counters,
        ...
        : TBase(std::move(request), std::move(asyncIoFactory), federatedQuerySetup, GUCSettings, database, userToken, std::move(outputFormat), counters, tableServiceConfig,
...
{
    return new TKqpScanExecuter(std::move(request), database, userToken, std::move(outputFormat), counters, tableServiceConfig, std::move(asyncIoFactory),
