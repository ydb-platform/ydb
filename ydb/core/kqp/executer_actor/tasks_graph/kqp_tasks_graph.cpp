...
TVector<TTaskMeta::TColumn> BuildKqpColumnsImpl(const Proto& op, TIntrusiveConstPtr<TTableConstInfo> tableInfo) {
...
// TODO: can we use TableKind from TableConstInfo from prepared query?
    ...
    const NKqpProto::TKqpPhyTableOperation& op, bool isPersistentScan)
{
    ...
    if (op.GetTypeCase() == NKqpProto::TKqpPhyTableOperation::kReadOlapRange) {
        [&meta, resultType = readSettings.ResultType, &readOlapRange = std::as_const(op.GetReadOlapRange())] {
            if (meta.Reads && !meta.Reads.GetRef().empty()) {
                // Validate parameters
                if (readOlapRange.GetOlapProgram().empty()) {
                    YQL_ENSURE(meta.ReadInfo.OlapProgram.Program.empty());
                    return;
                }

                YQL_ENSURE(meta.ReadInfo.OlapProgram.Program == readOlapRange.GetOlapProgram());
                return;
            }
            ...
            {
                Y_ABORT_UNLESS(meta.ReadInfo.GroupByColumnNames.empty());
                std::vector<std::string> groupByColumns;
                for (const auto& i : readOlapRange.GetGroupByColumnNames()) {
                    groupByColumns.emplace_back(i);
                }
                std::swap(meta.ReadInfo.GroupByColumnNames, groupByColumns);
            }
            ...
            meta.ReadInfo.OlapProgram.Program = readOlapRange.GetOlapProgram();
            for (auto& name: readOlapRange.GetOlapProgramParameterNames()) {
                meta.ReadInfo.OlapProgram.ParameterNames.insert(name);
            }
        }();
...
std::pair<ui32, TKqpTasksGraph::TTaskType::ECreateReason> TKqpTasksGraph::GetMaxTasksAggregation(const TStageInfo& stageInfo, const ui32 previousTasksCount, const ui32 nodesCount) const {
    ...
    for (const auto [reason, count] : tasksPerReason) {
        switch(reason) {
            ...
            case TTaskType::ECreateReason::FORCED:
                ...
                break;
            case TTaskType::ECreateReason::FORCED_NODE_QUOTA:
                introspections.push_back(ToString(count) + " tasks forced by node quota");
                break;
