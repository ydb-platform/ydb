...
namespace {
    ...
    TKqpFederatedQuerySetupFactoryDefault::TKqpFederatedQuerySetupFactoryDefault(
        ...
        if (appConfig.GetAuthConfig().HasTokenAccessorConfig()) {
            ...
            auto factory = NYql::CreateSecuredServiceAccountCredentialsOverTokenAccessorFactory(
                parsed.first,
                parsed.second,
                caContent,
                tokenAccessorConfig.GetConnectionPoolSize());
            CredentialsFactory = NFq::CreateKikimrStructuredTokenCredentialsFactory(factory);
        }
