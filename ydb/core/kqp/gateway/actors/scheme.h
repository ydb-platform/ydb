...
public:
    ...
    void HandleResponse(typename TResponse::TPtr &ev, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Received TEvProposeTransactionStatus for scheme request, scheme shard",
            {"TxId", response.GetTxId()},
            {"status", status},
            {"#_status", response.GetSchemeShardStatus()});
        ...
        switch (status) {
            case TEvTxUserProxy::TResultStatus::ExecInProgress: {
                ...
                YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Sent TEvNotifyTxCompletion request",
                    {"TxId", response.GetTxId()});
            ...
            case TEvTxUserProxy::TResultStatus::AccessDenied: {
                YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Access denied for scheme request",
                    {"TxId", response.GetTxId()});
            ...
            case TEvTxUserProxy::TResultStatus::ExecComplete: {
                ...
                {
                    YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Successful completion of scheme request",
                        {"TxId", response.GetTxId()});
            ...
            case TEvTxUserProxy::TResultStatus::ResolveError: {
                if (response.GetSchemeShardStatus() == NKikimrScheme::EStatus::StatusPathDoesNotExist
                    && SuccessOnNotExist) {
                    YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Successful completion of scheme request: path does not exist, SuccessOnNotExist: true,",
                        {"TxId", response.GetTxId()});
                    TResult result;
                    result.SetSuccess();
                    Promise.SetValue(std::move(result));
        ...
        YDB_LOG_CTX_COMP_ERROR(ctx, NKikimrServices::KQP_GATEWAY, "Unexpected error on scheme request",
            {"TxId", response.GetTxId()},
            {"ProxyStatus", status},
    ...
    void Handle(NSchemeShard::TEvSchemeShard::TEvNotifyTxCompletionResult::TPtr& ev, const TActorContext& ctx) {
        ...
        YDB_LOG_CTX_COMP_DEBUG(ctx, NKikimrServices::KQP_GATEWAY, "Received TEvNotifyTxCompletionResult for scheme request",
            {"TxId", response.GetTxId()});
