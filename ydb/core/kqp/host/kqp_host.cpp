        ...
        if (const auto requestContext = SessionCtx->GetUserRequestContext()) {
            if (requestContext->IsStreamingQuery) {
                ...
                if (Config->FeatureFlags.GetEnableExactlyOnceTopicsWriting()) {
                    state->ExactlyOnceTransactionPrefix = TStringBuilder() << "__ydb_streaming:" << requestContext->StreamingQueryPath << ":" << requestContext->CurrentExecutionId << ":" << requestContext->CurrentExecutionGeneration;
                }
            }
