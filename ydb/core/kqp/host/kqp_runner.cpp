...
public:
    TKqpRunner(TIntrusivePtr<IKqpGateway> gateway, const TString& cluster,
        ...
        TActorSystem* actorSystem, NKqpProto::EIsolationLevel isolationLevel = NKqpProto::ISOLATION_LEVEL_UNDEFINED)
        : Gateway(gateway)
        , Cluster(cluster)
        , TypesCtx(*typesCtx)
        ...
        , OptimizeCtx(MakeIntrusive<TKqpOptimizeContext>(cluster, Config, sessionCtx->QueryPtr(),
            sessionCtx->TablesPtr(), sessionCtx->GetUserRequestContext(), isolationLevel))
...
{
    return new TKqpRunner(gateway, cluster, typesCtx, sessionCtx, transformCtx, funcRegistry, actorSystem, isolationLevel);
}
