    ...
    const TKikimrTablesData& tablesData)
{
    ...
    if (!upsertSettings.IsUpdate) {
        for (auto& [name, meta] : table.second->Metadata->Columns) {
            if ((meta.NotNull || meta.IsCheckingNotNullInProgress) && !rowType->FindItem(name)) {
            ...
            if ((meta.NotNull || meta.IsCheckingNotNullInProgress) && rowType->FindItemType(name)->HasOptionalOrNull()) {
