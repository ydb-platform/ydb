...
public:
    ...
    void Bootstrap() {
        STLOG_I(KQPNS, "Starting KQP Node service",
...
private:
    ...
    STATEFN(ShuttingDownState) {
        switch(ev->GetTypeRewrite()) {
            ...
            default: {
                STLOG_W(KQPNS, "Ignoring unexpected event 0x%x (" << ev->GetTypeName() 
    ...
    void HandleWork(TEvKqpNode::TEvStartKqpTasksRequest::TPtr ev) {
        ...
        STLOG_D(KQPNS, "HandleStartKqpTasksRequest",
            (node_id, SelfId().NodeId()),
            (tx_id, txId),
            (requester, requester),
        ...
        if (State_->HasRequest(txId)) {
            ...
            if (isLocalRequest && State_->AddTasksToRequest(txId, executerId, requestTaskIds)) {
                STLOG_D(KQPNS, "Added tasks to existing local request",
                    (node_id, SelfId().NodeId()),
                    (tx_id, txId),
                ...
                STLOG_D(KQPNS, "Creating new request",
                    (node_id, SelfId().NodeId()),
                    (tx_id, txId),
                    (tasks_count, requestTaskIds.size()),
                    (executer, executerId),
        ...
        for (auto& dqTask: *msg.MutableTasks()) {
            ...
            if (State_->OnTaskStarted(txId, taskId, *actorId, executerId)) {
                ...
                STLOG_D(KQPNS, "Task finished in an instant",
                    (node_id, SelfId().NodeId()),
                    (tx_id, txId),
                    (task_id, taskId),
                    (compute_actor_id, *actorId),
    ...
    void HandleWork(TEvKqpNode::TEvCancelKqpTasksRequest::TPtr& ev) {
        ...
        STLOG_W(KQPNS, "Terminate transaction",
            (node_id, SelfId().NodeId()),
            (tx_id, txId),
            (reason, reason));
    ...
    void TerminateTx(ui64 txId, const TString& reason, NYql::NDqProto::StatusIds_StatusCode status = NYql::NDqProto::StatusIds::UNSPECIFIED) {
        ...
        if (auto tasksToAbort = State_->GetTasksByTxId(txId); !tasksToAbort.empty()) {
            STLOG_E(KQPNS, "Node service cancelled the task, because it " << reason,
                (node_id, SelfId().NodeId()),
                (tx_id, txId));
            for (const auto& [taskId, computeActorId]: tasksToAbort) {
    ...
    void HandleWork(TEvKqp::TEvInitiateShutdownRequest::TPtr& ev) {
        if (!AppData()->FeatureFlags.GetEnableShuttingDownNodeState()) {
            STLOG_I(KQPNS, "Feature flag EnableShuttingDownNodeState is disabled, ignoring shutdown request",
                (node_id, SelfId().NodeId()));
            return;
        }
        STLOG_I(KQPNS, "Prepare to shutdown: do not accept any messages from this time",
            (node_id, SelfId().NodeId()));
    ...
    void HandleShuttingDown(TEvKqpNode::TEvStartKqpTasksRequest::TPtr& ev) {
        ...
        if (ev->Sender.NodeId() == SelfId().NodeId()) {
            STLOG_D(KQPNS, "Accepting local StartRequest during shutdown",
                (node_id, SelfId().NodeId()),
                (tx_id, msg.GetTxId()));
            HandleWork(ev);
        } else if (msg.HasSupportShuttingDown() && msg.GetSupportShuttingDown()) {
            STLOG_D(KQPNS, "Rejecting remote StartRequest in ShuttingDown State",
                (node_id, SelfId().NodeId()),
                (tx_id, msg.GetTxId()));
...
private:
    static void HandleWork(NConsole::TEvConfigsDispatcher::TEvSetConfigSubscriptionResponse::TPtr&) {
        STLOG_D(KQPNS, "Subscribed for config changes");
    }
...
#undef FORCE_VALUE
            ...
            STLOG_I(KQPNS, "Updated table service config",
                (node_id, SelfId().NodeId()),
    ...
    void HandleWork(TEvents::TEvUndelivered::TPtr& ev) {
        switch (ev->Get()->SourceType) {
            ...
            case NConsole::TEvConfigsDispatcher::EvSetConfigSubscriptionRequest:
                STLOG_C(KQPNS, "Failed to deliver subscription request to config dispatcher",
                    (node_id, SelfId().NodeId()));
                break;

            case NConsole::TEvConsole::EvConfigNotificationResponse:
                STLOG_E(KQPNS, "Failed to deliver config notification response",
                    (node_id, SelfId().NodeId()));
                break;

            default:
                STLOG_E(KQPNS, "Undelivered event with unexpected source type",
                    (node_id, SelfId().NodeId()),
