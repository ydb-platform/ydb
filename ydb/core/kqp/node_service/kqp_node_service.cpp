...
private:
    ...
    void HandleWork(TEvKqp::TEvInitiateShutdownRequest::TPtr& ev) {
        if (!AppData()->FeatureFlags.GetEnableShuttingDownNodeState()) {
            LOG_I("[SHUTDOWN] Feature flag EnableShuttingDownNodeState is disabled, ignoring shutdown request");
            return;
        }
        
        LOG_I("[SHUTDOWN] Prepare to shutdown: do not acccept any messages from this time"
            << ", sender: " << ev->Sender
            << ", self: " << SelfId());
