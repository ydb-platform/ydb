...
/*
 * CBO Interesting Orderings framework — owned by ydb/core/kqp.
 *
 * Interesting ordering is an ordering which is produced by a query or tested in the query.
 * At this moment, we process shuffles and sortings.
 * For details of the algorithms and examples look at the white papers:
 * - "An efficient framework for order optimization" / "A Combined Framework for Grouping
 *   and Order Optimization" by T. Neumann, G. Moerkotte
 */
...
struct TOrdering {
    ...
    EType Type;
    /*
     * Definition was taken from 'Complex Ordering Requirements' section. Not natural orderings are
     * complex join predicates or grouping.
     * There can occur a problem when we have a natural ordering - shuffling (a, b) of the table and
     * we must aggregate by (b, a, c) - non natural ordering.
     * So for this case (b, a, c) suits for us as well and we must reorder (b, a, c) to (a, b, c).
     */
    bool IsNatural = false;
};
...
public:
    ...
    private:
        TDFSM* Dfsm_ = nullptr;
        /* we can have different args in hash shuffle function, so shuffles can be incompatible in this case */
...
public:
     ...
     * Public so that GENERATE_ENUM_SERIALIZATION can access nested enums.
     */
    class TNFSM {
    public:
        friend class TDFSM;
...
private:
    ...
    std::vector<i64> FdMapping_; // We need to remap FD idxes after the pruning
...
// Backward-compatible aliases in the NYql::NDq namespace.
namespace NYql::NDq {
...
namespace NYql {
using EJoinAlgoType = NKikimr::NKqp::EJoinAlgoType;
static constexpr auto AllJoinAlgos = NKikimr::NKqp::AllJoinAlgos;
} // namespace NYql
