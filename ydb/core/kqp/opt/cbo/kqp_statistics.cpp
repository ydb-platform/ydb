    ...
    const std::shared_ptr<NJson::TJsonValue>& stats)
{
    auto res = std::make_shared<TOptimizerStatistics>(
        s.Type, s.Nrows, s.Ncols, s.ByteSize, s.Cost,
        s.KeyColumns, s.ColumnStatistics, s.StorageType, s.Specific);
    ...
    if (auto keyCols = tableStats.find("key_columns"); keyCols != tableStats.end()) {
        ...
        res->KeyColumns = TIntrusivePtr<TOptimizerStatistics::TKeyColumns>(
            new TOptimizerStatistics::TKeyColumns(cols));
    }
    ...
    if (auto columns = tableStats.find("columns"); columns != tableStats.end()) {
        if (!res->ColumnStatistics) {
            res->ColumnStatistics = TIntrusivePtr<TOptimizerStatistics::TColumnStatMap>(
