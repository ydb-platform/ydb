 ...
 */
void InferStatisticsForMapJoin(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats, const NKikimr::NKqp::IProviderContext& ctx, TOptimizerHints hints) {
 ...
 */
void InferStatisticsForDqJoinBase(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats, const NKikimr::NKqp::IProviderContext& ctx, TOptimizerHints hints) {
...
void InferStatisticsForDqSource(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(dqSource.Settings().Raw());
    if (!inputStats) {
        return;
    }
...
void InferStatisticsForFlatMap(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(flatmapInput.Raw());
    ...
    if (IsPredicateFlatMap(flatmap.Lambda().Body().Ref())) {
        ...
        kqpStats->SetStats(input.Get(), kqpStats->GetStats(flatmap.Lambda().Body().Raw()));
    }
    else {
        kqpStats->SetStats(input.Get(), kqpStats->GetStats(flatmapInput.Raw()));
    }
}
...
void InferStatisticsForSkipNullMembers(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(skipNullMembersInput.Raw());
    if (!inputStats) {
        return;
    }
...
void InferStatisticsForExtendBase(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    for (const auto& input: input->Children()) {
        if (auto inputStats = kqpStats->GetStats(input.Get())) {
...
bool InferStatisticsForListParam(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    if (auto maybeListType = param.Type().Maybe<TCoListType>()) {
        ...
        if (auto maybeStructType = itemType.Maybe<TCoStructType>()) {
            ...
            kqpStats->SetStats(input.Get(), resStats);
        }
    }
    return true;
}
    ...
    for (size_t i=1; i<input->ChildrenSize(); i++) {
        ...
        if (callableInput->IsList()){
            for(size_t j=0; j<callableInput->ChildrenSize(); j++){
                ...
                if (inputStats){
                    kqpStats->SetStats( lambda.Args().Arg(j).Raw(), inputStats );
                }
            }

        }
        else {
            auto inputStats = kqpStats->GetStats(callableInput.Get());
            if (!inputStats) {
                return;
            }
            ...
            if (kqpStats->GetStats(input.Get())){
                inputStats = kqpStats->GetStats(input.Get());
            }

            kqpStats->SetStats( lambda.Args().Arg(0).Raw(), inputStats );
        }
    }
}
...
void InferStatisticsForStage(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto lambdaStats = kqpStats->GetStats( stage.Program().Body().Raw());
    if (lambdaStats){
        kqpStats->SetStats( stage.Raw(), lambdaStats );
    }
}
...
void InferStatisticsForDqMerge(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(merge.Output().Raw());
    if (!inputStats) {
        return;
    }
    ...
    kqpStats->SetStats(merge.Raw(), newStats);
}
...
void InferStatisticsForDqPhyCrossJoin(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(cross.LeftInput().Raw());
    if (!inputStats) {
        return;
    }

    auto outputStats = RemoveSorting(inputStats);
    kqpStats->SetStats(cross.Raw(), outputStats);
}
...
void InferStatisticsForAsStruct(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    for (const auto& field: input->Children()) {
        ...
        auto memberStats = kqpStats->GetStats(member.Struct().Raw());
...
void InferStatisticsForTopBase(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(topBase.Input().Raw());
    if (!inputStats) {
        return;
    }
...
void InferStatisticsForSortBase(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(sortBase.Input().Raw());
    if (!inputStats) {
        return;
    }
    auto topStats = std::make_shared<TOptimizerStatistics>(*inputStats);
...
void InferStatisticsForAggregationCallable(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    auto inputStats = kqpStats->GetStats(aggr.Input().Raw());
    if (!inputStats) {
        return;
    }
...
void InferStatisticsForEquiJoin(const TExprNode::TPtr& input, TKqpStatsStore* kqpStats) {
    ...
    for (size_t i = 0; i < equiJoin.ArgCount() - 2; ++i) {
        ...
        auto inputStats = kqpStats->GetStats(joinArg.Raw());
        if (inputStats == nullptr) {
            continue;
        }
    ...
    if (auto equiJoinStats = kqpStats->GetStats(equiJoin.Raw())) {
        ...
        equiJoinStats->TableAliases = MakeIntrusive<TTableAliasMap>(std::move(tableAliases));
        kqpStats->SetStats(equiJoin.Raw(), std::move(equiJoinStats));
    }
}
