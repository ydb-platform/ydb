        ...
        if (auto stageGuid = node.GetMapSafe().FindPtr("StageGuid")) {
            if (auto stat = stages.FindPtr(stageGuid->GetStringSafe())) {
                ...
                if (!(*stat)->GetOperator().empty()) {
                    ...
                    for (auto& [id, op] : (*stat)->GetOperator()) {
                        ...
                        switch (op.GetTypeCase()) {
                            case NYql::NDqProto::TDqOperatorStatsAggr::kJoin:
                                operatorInfo["Type"] = "Join";
                                break;
                            case NYql::NDqProto::TDqOperatorStatsAggr::kFilter:
                                operatorInfo["Type"] = "Filter";
                                break;
                            case NYql::NDqProto::TDqOperatorStatsAggr::kAggregation:
