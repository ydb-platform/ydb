    ...
    TExprContext& ctx)
{
    ...
    TVector<TExprBase> rowToAdd;
    rowToAdd.reserve(indexColumns.size());

    // rows to be updated in the index table in case if the given key has been found in the main table
    TVector<TExprBase> rowToUpdate;
    rowToUpdate.reserve(indexColumns.size());
    ...
    for (const auto& column : indexColumns) {
        ...
        if (inputColumns.contains(column)) {
            ...
            rowToAdd.emplace_back(tuple);
            rowToUpdate.emplace_back(tuple);
        } else {
