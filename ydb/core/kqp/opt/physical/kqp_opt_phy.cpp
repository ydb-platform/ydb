...
protected:
        ...
        IOptimizationContext& optCtx, const TGetParents& getParents)
    {
        ...
        TExprBase output = DqBuildJoin(node, ctx, optCtx, *getParents(), IsGlobal,
            pushLeftStage, KqpCtx.Config->GetHashJoinMode(), false, KqpCtx.Config->UseGraceJoinCoreForMap.Get().GetOrElse(false), true, KqpCtx.Config->OptShuffleElimination.Get().GetOrElse(KqpCtx.Config->DefaultEnableShuffleElimination), shuffleEliminationWithMap,
            rightCollectStage
        );
