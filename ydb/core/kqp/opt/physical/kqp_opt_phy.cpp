...
#define HNDL(name) "KqpPhysical-"#name, Hndl(&TKqpPhysicalOptTransformer::name)
        ...
        AddHandler(2, &TDqPhyStage::Match, HNDL(ReplaceUnionAllWithScatterForReadStages));
...
protected:
    ...
    // Replace DqCnParallelUnionAll with DqCnScatter when downstream is a heavy
    // connection (DqCnMerge or KqpCnStreamLookup) that benefits from adaptive routing.
    ...
    // Replace DqCnUnionAll → DqCnScatter when the upstream stage reads data (has DqSource)
    // and the current stage feeds into a heavy downstream (DqCnMerge, DqCnScatter,
    // DqCnParallelUnionAll, or KqpCnStreamLookup). This turns the N→1 fan-in into an
    // N→K scatter, preserving parallelism through intermediate compute stages.
    TMaybeNode<TExprBase> ReplaceUnionAllWithScatterForReadStages(TExprBase node, TExprContext& ctx,
        IOptimizationContext&, const TGetParents& getParents)
    {
        if (!KqpCtx.Config->GetEnableScatterConnection()) {
            return node;
        ...
        bool hasSourceUnionAll = false;
        for (const auto& input : stage.Inputs()) {
            if (auto maybeCn = input.Maybe<TDqCnUnionAll>()) {
                auto upstreamStage = maybeCn.Cast().Output().Stage().Cast<TDqPhyStage>();
                for (const auto& upInput : upstreamStage.Inputs()) {
                    if (upInput.Maybe<TDqSource>()) {
                        hasSourceUnionAll = true;
                        break;
                    }
                }
            }
            if (hasSourceUnionAll) break;
        }
        if (!hasSourceUnionAll) {
            return node;
        }
        ...
        if (stageIt != parents.end()) {
            for (const auto* outputNode : stageIt->second) {
                ...
                for (const auto* connNode : outputIt->second) {
                    if (TDqCnMerge::Match(connNode) || TDqCnScatter::Match(connNode)
                        || TDqCnParallelUnionAll::Match(connNode) || TKqpCnStreamLookup::Match(connNode)) {
                        hasHeavyDownstream = true;
                    }
                }
            }
        ...
        for (const auto& input : stage.Inputs()) {
            if (auto maybeCn = input.Maybe<TDqCnUnionAll>()) {
                auto cn = maybeCn.Cast();
                auto upstreamStage = cn.Output().Stage().Cast<TDqPhyStage>();
        ...
        DumpAppliedRule("ReplaceUnionAllWithScatterForReadStages", node.Ptr(), result.Ptr(), ctx);
        return result;
    }
