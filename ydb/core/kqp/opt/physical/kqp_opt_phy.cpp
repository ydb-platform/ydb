...
protected:
        ...
        IOptimizationContext& optCtx, const TGetParents& getParents)
    {
        ...
        TExprBase output = DqBuildJoin(node, ctx, optCtx, *getParents(), IsGlobal,
            pushLeftStage, KqpCtx.Config->GetHashJoinMode(), KqpCtx.Config->UseHashShuffleForMap.Get().GetOrElse(false), KqpCtx.Config->UseGraceJoinCoreForMap.Get().GetOrElse(false)
        );
