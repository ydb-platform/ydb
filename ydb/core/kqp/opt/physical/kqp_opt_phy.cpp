...
#define HNDL(name) "KqpPhysical-"#name, Hndl(&TKqpPhysicalOptTransformer::name)
        ...
        AddHandler(0, &TCoTop::Match, HNDL(BuildTopStageRemoveSort<false>));
        ...
        AddHandler(1, &TCoTop::Match, HNDL(BuildTopStageRemoveSort<true>));
...
protected:
    ...
    TMaybeNode<TExprBase> BuildStreamIdxLookupJoinStagesKeepSorted(TExprBase node, TExprContext& ctx) {
        TExprBase output = KqpBuildStreamIdxLookupJoinStagesKeepSorted(node, ctx, TypesCtx);
    ...
    TMaybeNode<TExprBase> BuildTopStageRemoveSort(TExprBase node, TExprContext& ctx,
        IOptimizationContext& optCtx, const TGetParents& getParents)
    {
        TExprBase output = DqBuildTopStageRemoveSort(node, ctx, optCtx, TypesCtx, *getParents(), IsGlobal);
