...
Y_UNIT_TEST_SUITE(TRBOBenchmarkCoverage) {
    Y_UNIT_TEST(PolicyFileMatchesFixedContract) {
        ...
        UNIT_ASSERT(
            ...
            std::set<ui32>({
                3, 8, 9, 15, 16, 19, 21, 28, 34, 38, 42, 48, 52, 55, 69, 73,
                87, 88, 90, 93, 94, 95, 96, 97, 99,
            }));
        UNIT_ASSERT_VALUES_EQUAL(
    ...
    Y_UNIT_TEST(PolicyEnforcesCuratedProofFloor) {
        const auto policy = LoadCoveragePolicy();
        const std::set<ui32> selected = {
            3, 8, 9, 15, 16, 19, 21, 28, 34, 38, 42, 48, 52, 55, 69, 73, 87,
            88, 90, 93, 94, 95, 96, 97, 99};
        const TMap<ui32, TString> statuses = {
            {3, "VERIFIED_BOUNDED"},
            {8, "VERIFIED_BOUNDED"},
            {9, "VERIFIED_BOUNDED"},
    ...
    Y_UNIT_TEST(PolicyReportsEveryProofFloorRegression) {
        ...
        const auto evaluation = EvaluateCoveragePolicy(
            policy,
            Tpcds,
            {3, 8, 9, 15, 16, 19, 21, 28, 34, 38, 42, 48, 52, 55, 69, 73, 87, 88, 90, 93, 94, 95, 96, 97, 99},
            statuses,
            {},
            {},
            policy.Suites.at(Tpcds.Name).RequiredPrepareSuccessQueries,
