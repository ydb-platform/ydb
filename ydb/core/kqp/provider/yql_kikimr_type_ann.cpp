    ...
    virtual TStatus HandleCreateTable(TKiCreateTable create, TExprContext& ctx) override {
        ...
        for (const auto& index : create.Indexes()) {
            ...
            if ((indexType == TIndexDescription::EType::LocalBloomFilter ||
                ...
                ctx.AddError(TIssue(ctx.GetPosition(index.Pos()), "Local Bloom indexes are supported only for column store tables"));
                return TStatus::Error;
            }
            ...
            switch (indexType) {
                ...
                case TIndexDescription::EType::LocalBloomFilter:
                    if (indexColums.size() != 1 || !dataColums.empty()) {
                        ctx.AddError(TIssue(ctx.GetPosition(index.Pos()), "Local bloom index requires exactly one index column and doesn't support data shared columns"));
                        return IGraphTransformer::TStatus::Error;
                    }
                ...
                case TIndexDescription::EType::LocalBloomNgramFilter:
                    if (indexColums.size() != 1 || !dataColums.empty()) {
                        ctx.AddError(TIssue(ctx.GetPosition(index.Pos()), "Local bloom ngram index requires exactly one index column and doesn't support data shared columns"));
                        return IGraphTransformer::TStatus::Error;
                    }
