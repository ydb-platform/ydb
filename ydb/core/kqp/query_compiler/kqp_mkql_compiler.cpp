...
TIntrusivePtr<IMkqlCallableCompiler> CreateKqlCompiler(const TKqlCompileContext& ctx, TTypeAnnotationContext& typesCtx) {
    ...
    compiler->AddCallable("BlockHashJoinCore",
        [&ctx](const TExprNode& node, TMkqlBuildContext& buildCtx) {
            ...
            auto leftRenamesVec = extractColumnIndices(node.Child(8));
            auto rightRenamesVec = extractColumnIndices(node.Child(9));
            ...
            return ctx.PgmBuilder().DqBlockHashJoin(leftInput, rightInput, joinKind,
                leftKeyColumns, rightKeyColumns, leftRenamesVec, rightRenamesVec, returnType, settings);
        });

    compiler->AddCallable("DqScalarHashJoin",
        [&ctx](const TExprNode& node, TMkqlBuildContext& buildCtx) {
            YQL_ENSURE(node.ChildrenSize() == 10, "DqScalarHashJoin should have 10 arguments");
            ...
            if (joinKindStr == "Inner") {
                ...
                YQL_ENSURE(false, "Unsupported join kind for DqScalarHashJoin: " << joinKindStr);
            }
            ...
            return ctx.PgmBuilder().DqScalarHashJoin(leftInput, rightInput, joinKind,
                leftKeyColumns, rightKeyColumns, leftRenamesVec, rightRenamesVec, returnType);
        });
