    ...
    void FillConnection(
        ...
        if (auto maybeStreamLookup = connection.Maybe<TKqpCnStreamLookup>()) {
            ...
            if (settings.IndexName) {
                ...
                tablesMap.emplace(indexMeta->Name, THashSet<TStringBuf>{});
                for (const auto& column : tableMeta->KeyColumnNames) {
                    tablesMap[indexMeta->Name].emplace(column);
                    streamLookupProto.AddIndexColumns(column);
                }
                FillTableId(*indexMeta, *streamLookupProto.MutableIndex());

                for (const auto& keyColumn : indexMeta->KeyColumnNames) {
                    streamLookupProto.AddIndexKeyColumns(keyColumn);
                }
            }
