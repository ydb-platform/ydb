...
public:
    ...
    TDocumentInfo(TOwnedCellVec&& keyCells, const TVector<NScheme::TTypeInfo>& keyColumnTypes, size_t numWords)
    ...
    NUdf::TUnboxedValue GetRow(const NKikimr::NMiniKQL::THolderFactory& holderFactory, i64& computeBytes) const {
        ...
        auto row = holderFactory.CreateDirectArrayHolder(
            KeyColumnTypes.size(), rowItems);
        for(size_t i = 0; i < KeyCells.size(); ++i) {
            rowItems[i] = NMiniKQL::GetCellValue(KeyCells[i], KeyColumnTypes[i]);
            computeBytes += NMiniKQL::GetUnboxedValueSize(rowItems[i], KeyColumnTypes[i]).AllocatedBytes;
        }
        return row;
    }
...
struct TDocumentIdComparator {
    ...
    TDocumentIdComparator(TConstArrayRef<NScheme::TTypeInfo> keyColumnTypes)
        : KeyColumnTypes(keyColumnTypes)
    {}
    ...
    bool operator()(const TDocumentId& a, const TDocumentId& b) const {
        ...
        int cmp = CompareTypedCellVectors(
            a.data(), b.data(), KeyColumnTypes.data(), KeyColumnTypes.size());
        return cmp > 0;
    }
};
...
public:
    ...
    bool operator<(const TDocumentIdPointer& other) const {
        ...
        int cmp = CompareTypedCellVectors(
            ...
            other.DocumentInfo->KeyColumnTypes.data(), other.DocumentInfo->KeyColumnTypes.size());
        return cmp < 0;
    }
};
...
private:
    ...
    void ExtractAndTokenizeExpression() {
        YQL_ENSURE(Settings->ExpressionSize() > 0, "Expected at least one column expression");

        // Get the first expression (assuming single expression for now)
        const auto& expr = Settings->GetExpression(0);
        YQL_ENSURE(expr.HasExpression(), "Expression string is required");

        TString expressionStr = expr.GetExpression();

        // Get analyzer settings from column metadata if available
        Ydb::Table::FulltextIndexSettings::Analyzers analyzers;
        if (expr.HasColumn() && expr.GetColumn().HasType()) {
            // Try to extract analyzer settings from column metadata
            // For now, use default standard tokenizer
        ...
        size_t wordIndex = 0;
        for(TString query: NFulltext::Analyze(expressionStr, analyzers)) {
    ...
    void StartReadRange(ui64 shardId, ui64 wordIndex, const TOwnedTableRange& range) {
        ...
        for (int i = 0; i < Settings->GetKeyColumns().size(); i++) {
            auto& column = Settings->GetKeyColumns(i);
            YQL_ENSURE(!IsSystemColumn(column.GetName()));
...
public:
        ...
        i64 freeSpace) override
    {
        ...
        finished = Reads.empty() && MergeQueue.empty();
        return computeBytes;
    }
    ...
    void HandleReadResult(TEvDataShard::TEvReadResult::TPtr& ev) {
        ...
        for(size_t i = 0; i < msg.GetRowsCount(); ++i) {
            ...
            auto docInfo = MakeIntrusive<TDocumentInfo>(TOwnedCellVec(row), KeyColumnTypes, Words.size());
