...
protected:
    ...
    const ui32 AllowNullKeysPrefixSize;
    const bool ShoulKeepRowsOrder;
    const NKqpProto::EStreamLookupStrategy LookupStrategy;

    const std::unordered_map<TString, TSysTables::TTableColumnInfo> KeyColumns;
    const std::vector<TSysTables::TTableColumnInfo*> LookupKeyColumns;
    const std::vector<TSysTables::TTableColumnInfo> Columns;
};
