...
//                IgnoreFunc(TEvTxProxySchemeCache::TEvInvalidateTableResult);
    ...
    void ContinueResolveClusters() {
        ...
        while (!LevelsFinished) {
            if (!LevelClusters.size()) {
                ...
                if (!LevelClusters.size()) {
                    // All clusters are invalid (0) or already leaf (no PostingParentFlag)
                    LevelsFinished = true;
                    break;
                }
                NextClusters.clear();
    ...
    void HandleRead(TEvNewAsyncInputDataArrived::TPtr) {
        ...
        ui64 freeSpace = 32*1024*1024; // FIXME where to take it from?
        ...
        bool finished = false;
        {
            ...
            rows.ForEachRow([&](NUdf::TUnboxedValue& value) {
                // FIXME: Check hypothetical value.size()
