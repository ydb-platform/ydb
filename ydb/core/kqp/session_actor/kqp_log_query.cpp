                     ...
                     const NYql::TIssues& issues) 
{
    ...
    if (NKikimr::ProtectQueryForLoggingIfSensitive(loggingRequestText, protectedRequestText)) {
        loggingRequestText = protectedRequestText;
    }

    const size_t total = loggingRequestText.empty() ? 1 :
        (loggingRequestText.size() + SQL_TEXT_MAX_SIZE - 1) / SQL_TEXT_MAX_SIZE;
    ...
    for (size_t i = 0; i < total; ++i) {
        ...
        if (!loggingRequestText.empty()) {
            json.WriteKey("data").WriteString(loggingRequestText.SubStr(i * SQL_TEXT_MAX_SIZE, SQL_TEXT_MAX_SIZE));
        }
