...
#define STLOG_C(MARKER, MESSAGE, ...) STLOG(PRI_CRIT, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_E(MARKER, MESSAGE, ...) STLOG(PRI_ERROR, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_W(MARKER, MESSAGE, ...) STLOG(PRI_WARN, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_N(MARKER, MESSAGE, ...) STLOG(PRI_NOTICE, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_I(MARKER, MESSAGE, ...) STLOG(PRI_INFO, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_D(MARKER, MESSAGE, ...) STLOG(PRI_DEBUG, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
#define STLOG_T(MARKER, MESSAGE, ...) STLOG(PRI_TRACE, NKikimrServices::KQP_SESSION, MARKER, LogPrefix() << MESSAGE, __VA_ARGS__)
...
public:
   ...
   TKqpSessionActor(const TActorId& owner,
        ...
        , GUCSettings(std::make_shared<TGUCSettings>())
    {
        ...
        STLOG_D(KQPS, "Create session actor", 
    ...
    void Bootstrap() {
        STLOG_D(KQPS, "Session actor bootstrapped",
            (trace_id, TraceId()));
    ...
    void PassRequestToResourcePool() {
        if (QueryState->UserRequestContext->PoolConfig) {
            STLOG_D(KQPS, "Request placed into pool from cache", 
    ...
    void HandleClientLost(NGRpcService::TEvClientLost::TPtr&) {
        STLOG_D(KQPS, "Got ClientLost event, send AbortExecution to executer", 
    ...
    void Handle(TEvKqp::TEvQueryRequest::TPtr& ev) {
        ...
        if (ev->Get()->HasYdbStatus() && ev->Get()->GetYdbStatus() != Ydb::StatusIds::SUCCESS) {
            ...
            STLOG_N(KQPS, "Got invalid query request, reply with error", 
                (status, status),
                (error_msg, errMsg),
                (trace_id, TraceId()));
        ...
        if (ShutdownState && ShutdownState->SoftTimeoutReached()) {
            ...
            STLOG_N(KQPS, "System shutdown requested: soft timeout reached, no queries can be accepted",
                (trace_id, TraceId()));
    ...
    void Handle(TEvents::TEvUndelivered::TPtr& ev) {
        if (ev->Get()->SourceType == TKqpWorkloadServiceEvents::EvPlaceRequestIntoPool) {
            STLOG_I(KQPS, "Failed to deliver request to workload service",
                (trace_id, TraceId()));
            CompileQuery();
        }
    }
    ...
    void Handle(NWorkload::TEvContinueRequest::TPtr& ev) {
        ...
        if (ev->Get()->Status == Ydb::StatusIds::UNSUPPORTED) {
            STLOG_T(KQPS, "Failed to place request in resource pool, feature flag is disabled",
                (trace_id, TraceId()));
    ...
    void CompileQuery() {
        ...
        STLOG_D(KQPS, "Sending CompileQuery request",
            (trace_id, TraceId()));
    ...
    void CompileSplittedQuery() {
        ...
        STLOG_D(KQPS, "Sending CompileSplittedQuery request",
            (trace_id, TraceId()));
    ...
    void CompileStatement() {
        ...
        STLOG_D(KQPS, "Sending CompileQuery request (statement)",
            (trace_id, TraceId()));
    ...
    void AcquirePersistentSnapshot() {
        STLOG_D(KQPS, "Acquire persistent snapshot",
            (trace_id, TraceId()));
    ...
    void AcquireMvccSnapshot() {
        ...
        STLOG_D(KQPS, "Acquire mvcc snapshot",
            (trace_id, TraceId()));
        auto timeout = QueryState->QueryDeadlines.TimeoutAt - TAppData::TimeProvider->Now();
    ...
    void Handle(TEvKqpSnapshot::TEvCreateSnapshotResponse::TPtr& ev) {
        ...
        STLOG_T(KQPS, "Read snapshot result", 
    ...
    void BeginTx(const Ydb::Table::TransactionSettings& settings) {
        ...
        alloc->Alloc->Ref().SetIncreaseMemoryLimitCallback([this, &alloc, mkqlMaxLimit](ui64 currentLimit, ui64 required) {
            if (required < mkqlMaxLimit) {
                STLOG_D(KQPS, "Increase memory limit", 
    ...
    void ExecutePartitioned(const TKqpPhyTxHolder::TConstPtr& tx) {
        ...
        for (const auto& effect : txCtx.DeferredEffects) {
            ...
            STLOG_D(KQPS, "TExecPhysicalRequest, add DeferredEffect to Transaction", 
            ...
            if (txCtx.EnableOltpSink.value_or(false)) {
                ...
                if (hasLocks || txCtx.TopicOperations.HasOperations()) {
                    if (!txCtx.GetSnapshot().IsValid() || txCtx.TxHasEffects() || txCtx.TopicOperations.HasOperations()) {
                        STLOG_D(KQPS, "TExecPhysicalRequest, tx has commit locks",
                            (trace_id, TraceId()));
                        request.LocksOp = ELocksOp::Commit;
                    } else {
                        STLOG_D(KQPS, "TExecPhysicalRequest, tx has rollback locks",
                            (trace_id, TraceId()));
                        request.LocksOp = ELocksOp::Rollback;
                    }
                } else if (txCtx.TxHasEffects()) {
                    STLOG_D(KQPS, "TExecPhysicalRequest, need commit locks",
                        (trace_id, TraceId()));
                    request.LocksOp = ELocksOp::Commit;
                }
            } else {
    ...
    void SendToExecuter(TKqpTransactionContext* txCtx, IKqpGateway::TExecPhysicalRequest&& request, bool isRollback = false) {
        ...
        STLOG_D(KQPS, "Sending to Executer", 
        ...
        STLOG_D(KQPS, "Created new KQP executer", 
            (executer_id, exId),
    ...
    void SendToPartitionedExecuter(TKqpTransactionContext* txCtx, IKqpGateway::TExecPhysicalRequest&& request)
    {
        ...
        ExecuterId = RegisterWithSameMailbox(executerActor);
        STLOG_D(KQPS, "Created new KQP partitioned executer", 
            (executer_id, ExecuterId),
            (trace_id, TraceId()));
    }
    ...
    void HandleExecute(TEvKqpExecuter::TEvExecuterProgress::TPtr& ev) {
        if (QueryState && QueryState->RequestActorId) {
            ...
            STLOG_D(KQPS, "Forwarded TEvExecuterProgress to " << QueryState->RequestActorId,
                (trace_id, TraceId()));
    ...
    void ProcessExecuterResult(TEvKqpExecuter::TEvTxResponse* ev) {
        ...
        if (response->GetStatus() != Ydb::StatusIds::SUCCESS) {
            ...
            STLOG_D(KQPS, "TEvTxResponse has non-success status", 
                (current_tx, QueryState->CurrentTx),
    ...
    void HandleExecute(TEvKqpExecuter::TEvStreamData::TPtr& ev) {
        ...
        STLOG_D(KQPS, "Forwarded TEvStreamData to " << QueryState->RequestActorId,
            (trace_id, TraceId()));
    ...
    void HandleExecute(TEvKqp::TEvAbortExecution::TPtr& ev) {
        ...
        STLOG_I(KQPS, "Got TEvAbortExecution, send it to Executer", 
    ...
    void Handle(TEvKqpBuffer::TEvError::TPtr& ev) {
        ...
        if (!QueryState || !QueryState->TxCtx || QueryState->TxCtx->BufferActorId != ev->Sender) {
            STLOG_E(KQPS, logMsg << ": Ignored error.",
                (trace_id, TraceId()));
            return;
        } else {
            STLOG_W(KQPS, logMsg,
    ...
    void HandleCleanup(TEvKqpBuffer::TEvError::TPtr& ev) {
        ...
        if (CleanupCtx->TransactionsToBeAborted.empty()) {
            STLOG_E(KQPS, logMsg <<  ": Ignored error. TransactionsToBeAborted is empty.",
                (trace_id, TraceId()));
        }
        ...
        if (txCtx->BufferActorId != ev->Sender) {
            STLOG_E(KQPS, logMsg <<  ": Ignored error. Current BufferActorId is not sender.",
                (trace_id, TraceId()));
            return;
        } else {
            STLOG_W(KQPS, logMsg,
    ...
    void AddTrailingInfo(TEvRecord& record) {
        if (ShutdownState) {
            STLOG_D(KQPS, "Session is closing, set trailing metadata to request session shutdown",
                (trace_id, TraceId()));
    ...
    void ReplySuccess() {
        ...
        if (QueryState->PreparedQuery) {
            ...
            STLOG_D(KQPS, "Create QueryResponse for action with SUCCESS status", 
    ...
    void ReplyQueryCompileError() {
        ...
        STLOG_W(KQPS, "ReplyQueryCompileError, remove tx",
            ...
            const TString& message)
    {
        ...
        STLOG_W(KQPS, "Reply query error, msg: " << message,
            (proxy_request_id, proxyRequestId),
            (trace_id, TraceId()));
    ...
    void HandleReady(TEvKqp::TEvCloseSessionRequest::TPtr&) {
        STLOG_I(KQPS, "Session closed due to explicit close event",
            (trace_id, TraceId()));
    ...
    void Handle(TEvKqp::TEvInitiateSessionShutdown::TPtr& ev) {
        if (!ShutdownState) {
            STLOG_N(KQPS, "Started session shutdown",
                (trace_id, TraceId()));
    ...
    void Handle(TEvKqp::TEvContinueShutdown::TPtr&) {
        ...
        if (ShutdownState->HardTimeoutReached()) {
            STLOG_N(KQPS, "Reached hard shutdown timeout",
                (trace_id, TraceId()));
            Send(SelfId(), new TEvKqp::TEvCloseSessionRequest());
        } else {
            ScheduleNextShutdownTick();
            STLOG_I(KQPS, "Schedule next shutdown tick",
                (trace_id, TraceId()));
        }
    }
    ...
    void HandleCleanup(TEvKqpExecuter::TEvTxResponse::TPtr& ev) {
        ...
        if (response.GetStatus() != Ydb::StatusIds::SUCCESS) {
            TIssues issues;
            IssuesFromMessage(response.GetIssues(), issues);
            STLOG_E(KQPS, "Failed to cleanup",
    ...
    void HandleCleanup(NWorkload::TEvCleanupResponse::TPtr& ev) {
        ...
        if (ev->Get()->Status != Ydb::StatusIds::SUCCESS && ev->Get()->Status != Ydb::StatusIds::NOT_FOUND) {
            STLOG_E(KQPS, "Failed to cleanup workload service", 
    ...
    void EndCleanup(bool isFinal) {
        ...
        if (isFinal) {
            ...
            STLOG_D(KQPS, "Cleanup temp tables", 
    ...
    {
        STLOG_W(KQPS, "Create QueryResponse for error on request, msg: " << message,
            (status, ydbStatus),
    ...
    void HandleFinalCleanup(TEvents::TEvGone::TPtr&) {
        ...
        STLOG_D(KQPS, "Session actor destroyed",
            (trace_id, TraceId()));
        PassAway();
    }
...
private:
    ...
    void InternalError(const TString& message) {
        STLOG_E(KQPS, "Internal error" << message,
            (trace_id, TraceId()));
        if (QueryState) {
