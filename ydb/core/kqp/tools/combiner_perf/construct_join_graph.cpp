...
TRenames MakeScalarMapJoinRenames(int leftSize, int rightDictValueSize){
    TRenames ret{};
    // ScalarMapJoin leftRenames rename list items, rightRenames rename dict value items(dict value items are all right items without join key  )  
...
THolder<IComputationGraph> ConstructInnerJoinGraphStream(ETestedJoinAlgo algo, TInnerJoinDescription descr){
    ...
    case ETestedJoinAlgo::kScalarGrace: {
        ...
        TRuntimeNode wideStream =dqPb.FromFlow(dqPb.GraceJoin(
            ToWideFlow(pb, args.Left),
           ToWideFlow(pb, args.Right),
    ...
    case NKikimr::NMiniKQL::ETestedJoinAlgo::kScalarMap: {
        ...
        TRuntimeNode rightDict = pb.ToSortedDict(args.Right, false, [&](TRuntimeNode tuple){
...
mapJoinSomething,
    ...
    case ETestedJoinAlgo::kBlockMap: {
        ...
        TRuntimeNode wideStream = BuildBlockJoin(pb, kInnerJoin,
            args.Left, descr.LeftSource.KeyColumnIndexes, kEmptyColumnDrops,
            args.Right, descr.RightSource.KeyColumnIndexes, kRightDroppedColumns,
            false
        );
        THolder<IComputationGraph> graph = descr.Setup->BuildGraph(wideStream, args.Entrypoints);
