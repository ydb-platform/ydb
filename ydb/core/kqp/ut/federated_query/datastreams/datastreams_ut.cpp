...
public:
    ...
    // Topic client SDK (external YD recipe)
    ...
    void ReadTopicMessages(const TString& topicName, const TVector<TString>& expectedMessage, TDuration disposition = TDuration::Seconds(100)) {
        ...
        WaitFor(TEST_OPERATION_TIMEOUT, "topic output messages", [&](TString& error) {
            if (!readSession->WaitEvent().HasValue()) {
                error = TStringBuilder() << "no event set, received #" << received.size() << " / " << expectedMessage.size() << " messages";
                return false;
            }
            ...
            error = TStringBuilder() << "got new event, received #" << received.size() << " / " << expectedMessage.size() << " messages";
            return false;
        });
