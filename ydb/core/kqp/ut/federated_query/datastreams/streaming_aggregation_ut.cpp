...
Y_UNIT_TEST_SUITE(KqpStreamingAggregation) {
    Y_UNIT_TEST_TWIN_F(FeatureFlagControlsRewrite, Enabled, TStreamingAggregationTestFixture) {
        ...
        for (const bool keyed : {false, true}) {
            ...
            )", "state_table"_a = Enabled ? "" : "/Root/nonexistent",
            ...
            UNIT_ASSERT_VALUES_EQUAL_C(ast.Contains("StreamingAggregation"), Enabled, ast);
            UNIT_ASSERT_VALUES_EQUAL(result.GetResultSets().size(), 1);
