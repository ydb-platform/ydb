...
Y_UNIT_TEST_QUAD(SelectWithFulltextMatchAndNgram, Edge, Covered) {
...

    {
        TString query = R"sql(
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "% renaissance%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "Werner Heisenberg", "Wildcard" as Mode)
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...

    {
        TString query = R"sql(
            ...
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "arena %", "Wildcard" as Mode)
            ORDER BY `Key`;

            -- {ber, ern} can be found separately in "Werner Heisenberg" but it's not correct result
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "bern %", "Wildcard" as Mode)
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
    ...
    { // N-gram sets are the same: {lus, use, sed, eda, dae, aed}. Wont work without postfilter
        TString query = R"sql(
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "lusedaedae", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "lusedaeda", "Wildcard" as Mode)
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
void DoSelectWithFulltextMatchAndNgramWildcard(bool relevance, bool edge, bool covered) {
...

    {
        TString query = R"sql(
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "aren%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "wer%ner %berg", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "edaeda%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "маш% обу%ние", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "мор%кот", "Wildcard" as Mode)
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
Y_UNIT_TEST(SelectWithFulltextMatchAndNgramWildcardUtf8Size) {
...

    {
        const TString query = R"sql(
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "%4️⃣" , "Wildcard" as Mode) -- that's grapheme cluster that consists of three utf8 runes
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
Y_UNIT_TEST(SelectWithFulltextMatchAndEdgeNgramWildcard) {
...

    {
        const TString query = R"sql(
            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "aaaaabbcd efg", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "aaaaab%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "aa%bc%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "ef", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "12%6+7%9=", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "123450+789=", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "123%", "Wildcard" as Mode)
            ORDER BY `Key`;

            SELECT `Key`, `Text` FROM `/Root/Texts` VIEW `fulltext_idx`
            WHERE FulltextMatch(`Text`, "1", "Wildcard" as Mode)
            ORDER BY `Key`;
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), querySettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
Y_UNIT_TEST(FulltextIndexWithInt64Key) {
    ...
    { // Query using FulltextMatch - verify correct keys returned
        TString query = R"sql(
...
Y_UNIT_TEST(FulltextIndexWithCompositeKeyTwoColumns) {
    ...
    { // Query with specific composite key result
        TString query = R"sql(
            ...
            WHERE FulltextMatch(Text, "birds fly")
            ORDER BY Category, Id
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
Y_UNIT_TEST(FulltextIndexWithCompositeKeyThreeColumns) {
    ...
    { // Query with specific composite key result
        TString query = R"sql(
            ...
            WHERE FulltextMatch(Text, "eagles soar")
            ORDER BY Tenant, Category, Id
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
...
Y_UNIT_TEST(FulltextIndexWithCompositeKeyMixedTypes) {
    ...
    { // Query using FulltextMatch - find machine learning projects
        TString query = R"sql(
            SELECT Year, Region, Name FROM `/Root/TextsCompositeKeyMixed` VIEW `fulltext_idx`
            WHERE FulltextMatch(Description, "machine learning")
    ...
    { // Query using FulltextMatch - find data related projects
        TString query = R"sql(
            SELECT Year, Region, Name FROM `/Root/TextsCompositeKeyMixed` VIEW `fulltext_idx`
            WHERE FulltextMatch(Description, "data")
            ORDER BY Year, Region, Name
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
    ...
    { // Query the newly inserted row
        TString query = R"sql(
            SELECT Year, Region, Name FROM `/Root/TextsCompositeKeyMixed` VIEW `fulltext_idx`
            WHERE FulltextMatch(Description, "quantum")
            ORDER BY Year, Region, Name
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
    ...
    { // Query should now find the updated row
        TString query = R"sql(
            SELECT Year, Region, Name FROM `/Root/TextsCompositeKeyMixed` VIEW `fulltext_idx`
            WHERE FulltextMatch(Description, "updated")
            ORDER BY Year, Region, Name
        )sql";
        auto result = db.ExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL_C(result.GetStatus(), EStatus::SUCCESS, result.GetIssues().ToString());
