...
Y_UNIT_TEST_SUITE(KqpBlockHashJoin) {
    ...
    Y_UNIT_TEST(JoinColumnPruning) {
        ...
        auto queryClient = kikimr.GetQueryClient();
        {
            auto status = queryClient.ExecuteQuery(
                R"(
                    CREATE TABLE `/Root/left_table` (
                        id Int64 NOT NULL,
                        join_key Int64 NOT NULL,
                        used_col String,
                        unused_left_1 String,
                        unused_left_2 String,
                        PRIMARY KEY (id)
                    )
                    WITH (STORE = COLUMN);
                    ...
                    CREATE TABLE `/Root/right_table` (
                        ...
                        join_key Int64 NOT NULL,
                        used_col String,
                        unused_right_1 String,
                        unused_right_2 String,
                        PRIMARY KEY (id)
                    )
                    WITH (STORE = COLUMN);
                )", NYdb::NQuery::TTxControl::NoTx()
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL_C(explainResult.GetStatus(), EStatus::SUCCESS,
            ...
            UNIT_ASSERT_C(ast.Contains("BlockHashJoin") || ast.Contains("BlockHashJoinCore"),
                TStringBuilder() << "Expected BlockHashJoin in AST: " << ast);

            UNIT_ASSERT_C(!ast.Contains("unused_left_1"),
                TStringBuilder() << "unused_left_1 should be pruned from AST: " << ast);
            UNIT_ASSERT_C(!ast.Contains("unused_left_2"),
                TStringBuilder() << "unused_left_2 should be pruned from AST: " << ast);
            UNIT_ASSERT_C(!ast.Contains("unused_right_1"),
                TStringBuilder() << "unused_right_1 should be pruned from AST: " << ast);
            UNIT_ASSERT_C(!ast.Contains("unused_right_2"),
                TStringBuilder() << "unused_right_2 should be pruned from AST: " << ast);
        }
    }
}
