...
Y_UNIT_TEST_SUITE(KqpJoin) {
    ...
    Y_UNIT_TEST(LeftJoinPushdownPredicate_1) {
        TKikimrRunner kikimr;
        auto db = kikimr.GetTableClient();
        auto session = db.CreateSession().GetValueSync().GetSession();
        ...
        auto result = session.ExecuteDataQuery(Q_(R"(
            PRAGMA RightSidePushdownForLeftJoin;
    ...
    Y_UNIT_TEST(LeftJoinPushdownPredicate_2) {
        TKikimrRunner kikimr;
        auto db = kikimr.GetTableClient();
        auto session = db.CreateSession().GetValueSync().GetSession();
        ...
        }
        {
            auto result = session.ExecuteDataQuery(Q_(R"(
                ...
                LEFT JOIN `/Root/Join1_2` AS t2
                ON t1.Fk1 = t2.Key
                WHERE COALESCE(t2.Value, 123) == 123
     ...
     Y_UNIT_TEST(LeftJoinPushdownPredicate_NestedJoin) {
        ...
        CreateLeftJoinSampleTables(session);
        {
            auto result = session.ExecuteDataQuery(Q_(R"(
                ...
                SELECT t1.Key1, t1.Key2, t1.Fk1, t1.Value, t2.Key, t2.Value 
                
                FROM `/Root/Join1_1` AS t1
                LEFT JOIN `/Root/Join1_2` AS t2
                ON t1.Fk1 = t2.Key
                CROSS JOIN `/Root/Join1_3`
    ...
    Y_UNIT_TEST(LeftJoinPushdownPredicate_NestedJoin_2) {
        TKikimrRunner kikimr;
        auto db = kikimr.GetTableClient();
        auto session = db.CreateSession().GetValueSync().GetSession();
