...
Y_UNIT_TEST_SUITE(KqpOlapIndexes) {
    ...
    Y_UNIT_TEST(CreateTableThenAddAndDropLocalMinMaxIndexWithSqlSyntax, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableCsMinMaxIndex(true);
        TKikimrRunner kikimr(settings);

        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/olapTableWithMinMaxIndex`
            (
                timestamp Timestamp NOT NULL,
                resource_id Utf8,
                uid Utf8 NOT NULL,
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/olapTableWithMinMaxIndex`
            ADD INDEX idx_minmax LOCAL USING min_max
                ON (resource_id);
        )");

        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/olapTableWithMinMaxIndex` DROP INDEX idx_minmax;
        )");
    }

    Y_UNIT_TEST(AddAndDropLocalMinMaxIndexWithSqlSyntax, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableCsMinMaxIndex(true);
        TKikimrRunner kikimr(settings);
        ...
        ExecQuery(kikimr, UseQueryService, "ALTER TABLE `/Root/olapTable` DROP INDEX idx_minmax;");
    }

    Y_UNIT_TEST(CreateTableWithLocalMinMaxIndexAndDropIsCorrect, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableCsMinMaxIndex(true);
        TKikimrRunner kikimr(settings);
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/olapTableCreateMinMax`
            (
                timestamp Timestamp NOT NULL,
                resource_id Utf8,
                uid Utf8 NOT NULL,
        ...
        ExecQuery(kikimr, UseQueryService, "ALTER TABLE `/Root/olapTableCreateMinMax` DROP INDEX idx_minmax;");
    }

    Y_UNIT_TEST(ShowCreateTableWithLocalMinMaxIndex, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/olapTableShowCreateMinMax`
            (
                timestamp Timestamp NOT NULL,
                resource_id Utf8,
                uid Utf8 NOT NULL,
        ...
        auto showResult = session.ExecuteQuery(
            "SHOW CREATE TABLE `/Root/olapTableShowCreateMinMax`;",
        ...
        UNIT_ASSERT_C(createText.Contains("ADD INDEX"),
            "SHOW CREATE should use ALTER TABLE ADD INDEX syntax, got: " << createText);
        UNIT_ASSERT_C(createText.Contains("LOCAL USING min_max"),
            "SHOW CREATE should contain LOCAL USING min_max, got: " << createText);
        UNIT_ASSERT_C(createText.Contains("idx_minmax"),
            "SHOW CREATE should contain index name idx_minmax, got: " << createText);
        UNIT_ASSERT_C(createText.Contains("resource_id"),
            "SHOW CREATE should contain indexed column resource_id, got: " << createText);
        UNIT_ASSERT_C(!createText.Contains("ACTION=UPSERT_INDEX"),
            "SHOW CREATE should not contain ALTER OBJECT syntax for min_max, got: " << createText);
    }

    Y_UNIT_TEST(RenameLocalMinMaxIndex, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true).SetEnableShowCreate(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableCsMinMaxIndex(true);
        TKikimrRunner kikimr(settings);
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/olapTableRenameMinMax`
            (
                timestamp Timestamp NOT NULL,
                resource_id Utf8,
                uid Utf8 NOT NULL,
        ...
        ExecQuery(kikimr, UseQueryService,
            "ALTER TABLE `/Root/olapTableRenameMinMax` RENAME INDEX idx_minmax TO idx_minmax_renamed;");

        {
            auto session = kikimr.GetQueryClient().GetSession().GetValueSync().GetSession();
            auto showResult = session.ExecuteQuery(
                "SHOW CREATE TABLE `/Root/olapTableRenameMinMax`;",
                NYdb::NQuery::TTxControl::NoTx()).ExtractValueSync();
            UNIT_ASSERT_C(showResult.IsSuccess(), showResult.GetIssues().ToString());
            UNIT_ASSERT(!showResult.GetResultSets().empty());
            NYdb::TResultSetParser parser(showResult.GetResultSet(0));
            UNIT_ASSERT_C(parser.TryNextRow(), "SHOW CREATE must return at least one row");
            TString createText = parser.ColumnParser(0).GetOptionalUtf8().value_or("");
            UNIT_ASSERT_C(createText.Contains("idx_minmax_renamed"),
                "SHOW CREATE should contain renamed index idx_minmax_renamed, got: " << createText);
            UNIT_ASSERT_C(!createText.Contains("idx_minmax LOCAL"),
                "SHOW CREATE should not contain old index name idx_minmax, got: " << createText);
        }

        ExecQuery(kikimr, UseQueryService, "ALTER TABLE `/Root/olapTableRenameMinMax` DROP INDEX idx_minmax_renamed;");
    }
