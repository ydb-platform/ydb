...
Y_UNIT_TEST_SUITE(KqpOlapIndexes) {
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(MinMaxIndexAppliedToDataAfterCompaction, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(AddAndDropLocalBloomIndexesWithSqlSyntax, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomFilterIndex(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomNgramFilterIndex(true);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(BloomNgramAlterObjectNgramAndFilterBytesThenSqlAlterFpp, EUseQueryService) {
        ...
        // ngramm_size + filter_size_bytes (+ records_count for explicit sizing). No false_positive_probability in FEATURES
        // (must not mix with filter_size_bytes in one UPSERT — BloomNgramAlterObjectUpsertRejectsMixingFppAndFilterSizeBytes).
        ExecQuery(kikimr, UseQueryService, R"(
        ...
        // Index was created with deprecated sizing (filter_size_bytes/records_count).
        // Switching to fpp-based mode via ALTER INDEX must be rejected.
        ...
        // Verify the index is unchanged after the rejected ALTER.
        {
            const auto [fpp, ngram, cs, filterBytes] = readNgram();
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(BloomNgramFppIndexRejectsDeprecatedSizingUpsert, EUseQueryService) {
        ...
        // Index was created with explicit fpp (new sizing mode).
        // Switching to deprecated sizing via ALTER OBJECT UPSERT_INDEX must be rejected.
        ExecQueryExpectErrorContains(kikimr, UseQueryService, R"(
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(BloomNgramAlterIndexBasic, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomFilterIndex(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomNgramFilterIndex(true);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(BloomNgramAlterIndexKeepsSameSchemeIndexId, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomFilterIndex(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomNgramFilterIndex(true);
        ...
        auto readBloomNgramIndexIdAndFpp = [&]() -> std::pair<ui32, double> {
            auto desc = client.Ls("/Root/olapAlterIdxSameSchemeId");
            UNIT_ASSERT_C(desc->Record.GetPathDescription().HasColumnTableDescription(), "expected column table path");
            const auto& schema = desc->Record.GetPathDescription().GetColumnTableDescription().GetSchema();
            for (const auto& idx : schema.GetIndexes()) {
                if (idx.GetName() == "idx_ngram" && idx.HasBloomNGrammFilter()) {
                    return {idx.GetId(), idx.GetBloomNGrammFilter().GetFalsePositiveProbability()};
                }
            }
            UNIT_ASSERT_C(false, "idx_ngram with BloomNGrammFilter not found");
            return {0, 0.0};
        ...
        UNIT_ASSERT_VALUES_EQUAL_C(idBefore, idAfter,
            "ALTER INDEX must update the same local OLAP index (stable scheme id), not drop+add a new index");
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(BloomNgramAlterIndexResetIndexSettingRejected, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomFilterIndex(true);
        settings.AppConfig.MutableFeatureFlags()->SetEnableLocalBloomNgramFilterIndex(true);
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/olapAlterIdxReset`
            (
                timestamp Timestamp NOT NULL,
                resource_id Utf8,
                uid Utf8 NOT NULL,
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/olapAlterIdxReset`
            ADD INDEX idx_ngram LOCAL USING bloom_ngram_filter
                ON (resource_id)
                WITH (ngram_size = 3, false_positive_probability = 0.01, case_sensitive = true);
        )");
        ...
        ExecQueryExpectErrorContains(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/olapAlterIdxReset` ALTER INDEX idx_ngram RESET (case_sensitive);
        )", "RESET is not supported for index settings");
    }

    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(RowTableAlterIndexTableSettingStillWorks, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        TKikimrRunner kikimr(TKikimrSettings().SetWithSampleTables(false));

        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            CREATE TABLE `/Root/rowAlterIdx`
            (
                key Uint64,
                value Utf8,
                PRIMARY KEY (key)
        ...
        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/rowAlterIdx`
            ADD INDEX idx_value GLOBAL SYNC ON (value);
        )");

        ExecQuery(kikimr, UseQueryService, R"(
            --!syntax_v1
            ALTER TABLE `/Root/rowAlterIdx`
            ALTER INDEX idx_value SET AUTO_PARTITIONING_MIN_PARTITIONS_COUNT 1;
        )");
    }
...
Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(RenameLocalBloomIndex, EUseQueryService) {
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(TablesInStore, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(IndexesActualization, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false).SetColumnShardAlterObjectEnabled(true);
        TKikimrRunner kikimr(settings);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(CountMinSketchIndex, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings()
            .SetColumnShardAlterObjectEnabled(true)
            .SetWithSampleTables(false);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(SchemeActualizationOnceOnStart, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        auto settings = TKikimrSettings().SetWithSampleTables(false);
        TKikimrRunner kikimr(settings);
...
//                    "skipped", csController->GetIndexesSkippingOnSelect().Val() - SkipStart);
    ...
    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(IndexesInBS, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        Y_UNUSED(UseQueryService);
        TTestIndexesScenario().SetStorageId("__DEFAULT").Initialize().ExecuteSkipIndexesScenario();
    }

    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(IndexesInLocalMetadata, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        Y_UNUSED(UseQueryService);
        TTestIndexesScenario().SetStorageId("__LOCAL_METADATA").Initialize().ExecuteSkipIndexesScenario();
    }

    Y_UNIT_TEST_ALL_ENUM_VALUES_VAR(CheckCompactionFailingOnIndexes, EUseQueryService) {
        const bool UseQueryService = (Arg<0>() == EUseQueryService::QueryService);
        Y_UNUSED(UseQueryService);
