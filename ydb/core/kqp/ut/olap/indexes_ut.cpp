...
Y_UNIT_TEST_SUITE(KqpOlapIndexes) {
    ...
    Y_UNIT_TEST(AnalyzeTable) {
...

        {
            ...
            shard = 0;
            for (auto&& i : csController->GetShardActualIds()) {
                auto request = std::make_unique<NStat::TEvStatistics::TEvAnalyzeTable>();
                request->Record.MutableTable()->MutablePathId()->SetLocalId(pathId);
            ...
            for (size_t shard = 0; shard < 3; ++shard) {
                auto event = runtime->GrabEdgeEvent<NStat::TEvStatistics::TEvAnalyzeTableResponse>(handle);
                UNIT_ASSERT(event);

                auto& response = event->Record;
                Cerr << response << Endl;
                UNIT_ASSERT_VALUES_EQUAL(response.GetStatus(), NKikimrStat::TEvAnalyzeResponse::STATUS_SUCCESS);
            }
        }
    }
