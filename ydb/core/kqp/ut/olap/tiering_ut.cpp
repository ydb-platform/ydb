...
public:
    ...
    void CheckAllDataInTier(const TString& tierName, const bool onlyActive = true) {
        auto check = [&](const TString& query) {
        ...
        if (tierName != NOlap::NBlobOperations::TGlobal::DefaultStorageId) {
            auto selectQuery = TStringBuilder();
            selectQuery << R"( SELECT TierName FROM `)" << TablePath << R"(/.sys/primary_index_stats`)";
            selectQuery << R"(WHERE EntityType == "COL")";
            if (onlyActive) {
                selectQuery << " AND Activity == 1";
            }
            selectQuery << " GROUP BY TierName";
            check(selectQuery);
        }
