...
Y_UNIT_TEST_SUITE(KqpKv) {
    ...
    Y_UNIT_TEST(ReadRows_Decimal) {
        ...
        // Base case: upsert Uin64 to Decimal column
        {
            ...
            UNIT_ASSERT_C(issues.Contains("Type mismatch for column Key35: expected Decimal(35,10), got Uint64"), issues);
        }
        ...
        // Base case: upsert Decimal to Uin64 column
        {
            ...
            UNIT_ASSERT(!upsertResult.IsSuccess());
            TString issues = upsertResult.GetIssues().ToString();
            UNIT_ASSERT_C(issues.Contains("Unexpected type for column ValueInt: expected Uint64"), issues);
        }
        ...
        // Base case: upsert Decimal(35,10) to Decimal(22,9) column
        {
            ...
            UNIT_ASSERT(!upsertResult.IsSuccess());
            TString issues = upsertResult.GetIssues().ToString();
            UNIT_ASSERT_C(issues.Contains("Type Decimal(35,10) doesn't match type Decimal(22,9) for column Key22"), issues);
        }
