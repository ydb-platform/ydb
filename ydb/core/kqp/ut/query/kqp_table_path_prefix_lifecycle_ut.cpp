...
Y_UNIT_TEST_SUITE(KqpTablePathPrefixRelativeLifecycle) {
    Y_UNIT_TEST_TWIN(InFlightCompilationDoesNotServeRequestsAfterFlagUpdate, AstCache) {
        ...
        const TString query = "PRAGMA TablePathPrefix = 'Root'; SELECT id FROM items;";
        ...
        CompareYson("[[2u]]", FormatResultSetYson(cachedResult.GetResultSet(0)));
