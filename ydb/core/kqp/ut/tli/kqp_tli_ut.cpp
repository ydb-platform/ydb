...
namespace {
    ...
    TTliLogPatterns MakeTliLogPatterns(bool useSink) {
        return {
            ...
            // Include "Read transaction" for InvisibleRowSkips victim detection
            useSink ? "(Read|Write) transaction was a victim of broken locks" : "(Read|KQP data) transaction was a victim of broken locks",
        };
    }
...
Y_UNIT_TEST_SUITE(KqpTli) {
    ...
    Y_UNIT_TEST_TWIN(InvisibleRowSkips, LogEnabled) {
        constexpr bool UseSink = true;
        TStringStream ss;
        TTliTestContext ctx(LogEnabled, UseSink, ss);
        ctx.CreateTable("/Root/Tenant1/TableSkips");
        ctx.SeedTable("/Root/Tenant1/TableSkips", {{1, "Initial"}});

        const TString victimRead1Text = "SELECT * FROM `/Root/Tenant1/TableSkips` WHERE Key = 1u";
        const TString breakerQueryText = "UPSERT INTO `/Root/Tenant1/TableSkips` (Key, Value) VALUES (1u, \"BreakerV2\")";
        const TString victimRead2Text = "SELECT * FROM `/Root/Tenant1/TableSkips` WHERE Key = 1u";
        ...
        // with QueryId = the original victim query that owned the locks
