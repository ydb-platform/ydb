...
Y_UNIT_TEST_SUITE(KqpTli) {
    ...
    Y_UNIT_TEST_QUAD(QueryMetricsLocksBroken, LogEnabled, UseSink) {
        ...
        if (LogEnabled) {
            kikimr.GetTestServer().GetRuntime()->SetLogPriority(NKikimrServices::TLI, NLog::PRI_TRACE);
        }
        ...
        TVector<std::pair<TString, ui64>> regexToMatchCount{
            ...
            {ConstructRegexToCheckLogs("DEBUG", "SessionActor", "Query had broken other locks"), LogEnabled ? 1 : 0},
        };
    ...
    Y_UNIT_TEST_QUAD(QueryMetricsLocksBrokenSeparateCommit, LogEnabled, UseSink) {
        ...
        TVector<std::pair<TString, ui64>> regexToMatchCount{
            // check session actor logs
            {ConstructRegexToCheckLogs("DEBUG", "SessionActor", "Commit had broken other locks"), LogEnabled ? 1 : 0},
        };
