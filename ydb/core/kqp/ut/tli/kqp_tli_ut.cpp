...
Y_UNIT_TEST_SUITE(KqpTli) {
    ...
    // Test: Multiple writes to the same shard with the real breaker in the middle of the transaction.
    // Demonstrates that per-batch QuerySpanId tracking correctly attributes lock breaks
    // even when many non-conflicting writes follow the breaker to the same shard.
    //
    // Breaker transaction (7 queries + standalone commit):
    //   Q1: UPSERT to OtherTableA Key=10  (different shard, no conflict)
    //   Q2: UPSERT to OtherTableB Key=20  (different shard, no conflict)
    //   Q3: UPSERT to BatchTable Key=1    (THE BREAKER -- first write to this shard, conflicts with victim)
    //   Q4: UPSERT to BatchTable Key=2    (same shard, no conflict)
    //   Q5: UPSERT to BatchTable Key=3    (same shard, no conflict)
    //   Q6: UPSERT to OtherTableC Key=30  (different shard, no conflict)
    //   Q7: UPSERT to BatchTable Key=4    (same shard, no conflict -- would be the wrongly attributed breaker without fix)
    //   breakerTx->Commit()
    //
    // Without per-batch QuerySpanId: EvWrite for BatchTable shard carries Q7's SpanId (last writer).
    // With per-batch QuerySpanId: EvWrite for BatchTable shard carries Q3's SpanId (first batch = actual breaker).
    Y_UNIT_TEST(SeparateCommitManyBatchesBreakerInMiddle) {
        TStringStream ss;
        TTliTestContext ctx(ss);
        ctx.CreateTable("/Root/Tenant1/BatchTable");
        ctx.CreateTable("/Root/Tenant1/OtherTableA");
        ctx.CreateTable("/Root/Tenant1/OtherTableB");
        ctx.CreateTable("/Root/Tenant1/OtherTableC");
        ctx.SeedTable("/Root/Tenant1/BatchTable", {{1, "Initial"}});

        const TString victimQueryText = "SELECT * FROM `/Root/Tenant1/BatchTable` WHERE Key = 1u";
        const TString victimCommitText = "UPSERT INTO `/Root/Tenant1/BatchTable` (Key, Value) VALUES (1u, \"VictimValue\")";

        // Non-conflicting writes to other tables (different shards)
        const TString breakerOtherA = "UPSERT INTO `/Root/Tenant1/OtherTableA` (Key, Value) VALUES (10u, \"OtherA\")";
        const TString breakerOtherB = "UPSERT INTO `/Root/Tenant1/OtherTableB` (Key, Value) VALUES (20u, \"OtherB\")";
        const TString breakerOtherC = "UPSERT INTO `/Root/Tenant1/OtherTableC` (Key, Value) VALUES (30u, \"OtherC\")";

        // The actual breaker: writes Key=1 to the same table the victim read from
        const TString breakerQueryText = "UPSERT INTO `/Root/Tenant1/BatchTable` (Key, Value) VALUES (1u, \"BreakerValue\")";

        // Non-conflicting writes to the SAME table (same shard as breaker, different keys)
        const TString breakerAfter1 = "UPSERT INTO `/Root/Tenant1/BatchTable` (Key, Value) VALUES (2u, \"After1\")";
        const TString breakerAfter2 = "UPSERT INTO `/Root/Tenant1/BatchTable` (Key, Value) VALUES (3u, \"After2\")";
        const TString breakerAfter3 = "UPSERT INTO `/Root/Tenant1/BatchTable` (Key, Value) VALUES (4u, \"After3\")";

        // Victim: read Key=1 (creates a lock)
        auto victimTx = BeginReadTx(ctx.VictimSession, victimQueryText);

        // Breaker: Q1-Q2 write to other tables, Q3 writes the conflicting key,
        // Q4-Q5 write non-conflicting keys to the same shard, Q6 writes to another table,
        // Q7 writes another non-conflicting key to the same shard, then standalone commit.
        auto breakerTx = BeginTx(ctx.Session, breakerOtherA);                    // Q1: OtherTableA
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerOtherB, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q2: OtherTableB
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerQueryText, TTxControl::Tx(*breakerTx)).GetValueSync());        // Q3: BatchTable Key=1 (BREAKER)
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter1, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q4: BatchTable Key=2
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter2, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q5: BatchTable Key=3
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerOtherC, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q6: OtherTableC
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter3, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q7: BatchTable Key=4
        NKqp::AssertSuccessResult(breakerTx->Commit().ExtractValueSync());        // Standalone commit
    ...
    Y_UNIT_TEST(SeparateCommitBreakerInMiddleOfSameShard) {
        ...
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerBefore2, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q2: Key=20
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerBefore3, TTxControl::Tx(*breakerTx)).GetValueSync());           // Q3: Key=30
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerQueryText, TTxControl::Tx(*breakerTx)).GetValueSync());         // Q4: Key=1 (BREAKER)
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter1, TTxControl::Tx(*breakerTx)).GetValueSync());            // Q5: Key=40
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter2, TTxControl::Tx(*breakerTx)).GetValueSync());            // Q6: Key=50
        NKqp::AssertSuccessResult(ctx.Session.ExecuteDataQuery(
            breakerAfter3, TTxControl::Tx(*breakerTx)).GetValueSync());            // Q7: Key=60
        NKqp::AssertSuccessResult(breakerTx->Commit().ExtractValueSync());         // Standalone commit
