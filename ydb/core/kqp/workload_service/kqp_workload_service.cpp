...
private:
    ...
    void Handle(TEvPrivate::TEvFetchPoolResponse::TPtr& ev) {
        ...
        if (ev->Get()->Status == Ydb::StatusIds::SUCCESS) {
            ...
            LOG_WARN_S(*TlsActivationContext, NKikimrServices::KQP_WORKLOAD_SERVICE, "[WorkloadService] " << LogPrefix() <<"Failed to fetch pool: " << poolId
    ...
    void Handle(TEvPrivate::TEvCleanupTablesFinished::TPtr& ev) {
        if (ev->Get()->Success) {
            ...
            LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::KQP_WORKLOAD_SERVICE, "[WorkloadService] " << LogPrefix() <<"Failed to cleanup tables, issues: " << ev->Get()->Issues.ToOneLineString());
        }
