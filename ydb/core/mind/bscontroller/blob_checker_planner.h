...
// TBlobCheckerPlanner gives timestamps when BlobCheckerOrchestrator is allowed to start scan routine
...
public:
    ...
    // returns whether there was planned scan for given group
    bool DequeueCheck(TGroupId groupId);

    // at first we obtain timestamp when to start the next scan
    TMonotonic GetNextAllowedCheckTimestamp(TMonotonic now);
    // and when it is time, we get groupId if there are groups available to scan by the moment
