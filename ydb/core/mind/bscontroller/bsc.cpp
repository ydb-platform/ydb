...
std::unique_ptr<TEvBlobStorage::TEvControllerConfigRequest> TBlobStorageController::BuildConfigRequestFromStorageConfig(
        const NKikimrBlobStorage::TStorageConfig& storageConfig, const THostRecordMap::element_type& hostRecords, bool validationMode) {
    ...
    if (validationMode) {
        auto *hostRecordSet = request->MutableEnforceHostRecords();
        for (const auto& [hostId, record] : hostRecords) {
            auto* rec = hostRecordSet->AddHostRecord();
            rec->MutableHostKey()->SetNodeId(record.NodeId);
            rec->MutableHostKey()->SetFqdn(std::get<0>(hostId));
            rec->MutableHostKey()->SetIcPort(std::get<1>(hostId));
            record.Location.Serialize(rec->MutableLocation(), false);
        }
    }
...
void TBlobStorageController::Handle(TEvBlobStorage::TEvControllerDistconfRequest::TPtr ev) {
    ...
    switch (record.GetOperation()) {
        ...
        case NKikimrBlobStorage::TEvControllerDistconfRequest::ValidateConfig: {
            ...
            const TString& validationYaml = storageYaml ? *storageYaml : *mainYaml;
            NKikimrBlobStorage::TStorageConfig storageConfig;

            try {
                NKikimrConfig::TAppConfig appConfig = NYaml::Parse(validationYaml);
                TString errorReason;
