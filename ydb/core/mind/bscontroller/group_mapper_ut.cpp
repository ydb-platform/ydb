...
class TTestContext {
    struct TPDiskRecord {
        ...
        TPDiskRecord(ui32 dataCenterId, ui32 roomId, ui32 rackId, ui32 bodyId, ui32 slotSizeInUnits)
...
public:
                ...
                for (ui32 c = 0; c < numRacks; ++c, ++rack) {
                    for (ui32 d = 0; d < numBodies; ++d, ++body, ++nodeId) {
                        for (ui32 e = 0; e < numDisks; ++e, ++pdiskId) {
                            TPDiskRecord pdisk(dataCenter, room, rack, body, 0);
    ...
    TTestContext(const std::vector<std::tuple<ui32, ui32, ui32, ui32, ui32>>& disks) {
        ...
        for (const auto& disk : disks) {
            ...
            for (ui32 pdiskId = 1; numDisks--; ++pdiskId) {
                TPDiskRecord pdisk(dataCenter, room, rack, body, 0u);
    ...
    ui32 AllocateGroup(TGroupMapper& mapper, TGroupMapper::TGroupDefinition& group,
        ui32 groupSizeInUnits = 0u, bool allowFailure = false
    ) {
        ui32 groupId = NextGroupId++;
        TString error;
        bool success = mapper.AllocateGroup(groupId, group, {}, {}, groupSizeInUnits, 0, false, error);
        ...
        if (!result.Disks.empty()) {
            ...
            for (auto vdisk : result.Disks) {
                auto target = mapper.TargetMisplacedVDisk(TGroupId::FromValue(groupId), group.Group, vdisk,
                    std::move(forbid), 0, requireOperational, group.GroupSizeInUnits, error);
                if (target) {
...
Y_UNIT_TEST_SUITE(TGroupMapperTest) {
    ...
    Y_UNIT_TEST(DifferentGroupSizeInUnits) {
        ...
        // TODO(ydynnikov): после изменения размера селфхил не откатывает изменения
        // TODO(ydynnikov): после изменения размера bsc не перенаселяет диски
    }
