...
Y_UNIT_TEST_SUITE(BsControllerConfig) {
    ...
    Y_UNIT_TEST(ReassignGroupDiskWithAttentionToReplication) {
        TEnvironmentSetup env(14, 1);
        auto test = [&] (const TString& dispatchName, std::function<void(TTestActorRuntime&)> setup, bool& outActiveZone) {
            TFinalizer finalizer(env);
            env.Prepare(dispatchName, setup, outActiveZone);

            TVector<TEnvironmentSetup::TNodeRecord> nodesSubset;
            for (const auto& node : env.GetNodes()) {
                if (nodesSubset.size() < 8) {
                    nodesSubset.push_back(node);
                    continue;
                }
                break;
            }
            ...
            TVector<TEnvironmentSetup::TPDiskDefinition> pdisks = {
                {"/dev/disk1", NKikimrBlobStorage::ROT, false, false, 0},
                {"/dev/disk2", NKikimrBlobStorage::ROT, false, false, 0},
                {"/dev/disk3", NKikimrBlobStorage::ROT, false, false, 0},
            };

            env.DefineBox(1, "box", pdisks, nodesSubset, request);
            env.DefineStoragePool(1, 1, "storage pool", 8, NKikimrBlobStorage::ROT, {}, request);
            invoke();

            env.DefineBox(1, "box", pdisks, env.GetNodes(), request, 1);
            invoke();

            std::vector<ui32> toNodes;

            for (ui32 groupId = 2147483648; groupId <= 2147483655; groupId++) {
                auto *cmd = request.AddCommand()->MutableReassignGroupDisk();
                cmd->SetGroupId(groupId);
                cmd->SetGroupGeneration(1);
                cmd->SetFailRealmIdx(0);
                cmd->SetFailDomainIdx(0);
                cmd->SetVDiskIdx(0);
                // cmd->SetWithAttentionToReplication(true);
                ...
                UNIT_ASSERT_EQUAL(1, status.ReassignedItemSize());
                const auto& reassigned = status.GetReassignedItem(0);
                TPDiskId pdiskTo(reassigned.GetTo().GetNodeId(), reassigned.GetTo().GetPDiskId());

                toNodes.push_back(pdiskTo.NodeId);
            }

            ui32 lastNodeInSubset = std::get<2>(*(nodesSubset.rbegin()));

            std::vector<ui32> expectedNodes = {
                lastNodeInSubset + 1,
                lastNodeInSubset + 2,
                lastNodeInSubset + 3,
                lastNodeInSubset + 4,
            ...
            UNIT_ASSERT_VALUES_EQUAL(toNodes, expectedNodes);
        };
