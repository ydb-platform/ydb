...
void THive::Handle(TEvHive::TEvStopTablet::TPtr& ev) {
    ...
    if (rec.HasTabletID()) {
        ...
        if (!(rec.HasTabletID())) { LOG_ERROR_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() << "Failed condition \"" << "rec.HasTabletID()" << "\" " << rec.ShortDebugString()); }
...
void THive::DeleteTabletWithoutStorage(TLeaderTabletInfo* tablet) {
    ...
    if (!(tablet->TabletStorageInfo->Channels.empty() || tablet->TabletStorageInfo->Channels[0].History.empty())) { LOG_ERROR_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() << "Failed condition \"" << "tablet->TabletStorageInfo->Channels.empty() || tablet->TabletStorageInfo->Channels[0].History.empty()" << "\" " << "tablet " << tablet->Id); }
    ...
    if (it != Tablets.end()) {
        ...
        for (auto nt = Nodes.begin(); nt != Nodes.end(); ++nt) {
            for (auto st = nt->second.Tablets.begin(); st != nt->second.Tablets.end(); ++st) {
                if (!(st->second.count(&tablet) == 0)) { LOG_ERROR_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() << "Failed condition \"" << "st->second.count(&tablet) == 0" << "\" " << " Deleting tablet found on node " << nt->first << " in state " << TTabletInfo::EVolatileStateName(st->first)); }
            }
            if (!(nt->second.LockedTablets.count(&tablet) == 0)) { LOG_ERROR_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() << "Failed condition \"" << "nt->second.LockedTablets.count(&tablet) == 0" << "\" " << " Deleting tablet found on node " << nt->first << " in locked set"); }
        }
...
static void AggregateDiff(TMetrics& aggregate, const TMetrics& before, const TMetrics& after, TTabletId tabletId, const TString& name) {
    ...
    if (!(newValue >= 0)) { LOG_ERROR_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() << "Failed condition \"" << "newValue >= 0" << "\" " << "tablet " << tabletId << " name=" << name << " oldValue=" << oldValue << " delta=" << delta << " newValue=" << newValue); }
...
bool THive::ReassignInactiveGroups(TStoragePoolInfo& pool) {
    ...
    if (operations.empty()) {
        ...
        LOG_INFO_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() <<"ShrinkPool - starting reassign for " << operations.size() << " tablets");
...
bool THive::CompactInactiveGroups(TStoragePoolInfo& pool) {
    ...
    if (tabletsToCompact.empty()) {
        ...
        LOG_INFO_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() <<"ShrinkPool - starting compact for " << tabletsToCompact.size() << " tablets");
...
void THive::CheckRemainingHistory(TStoragePoolInfo& pool) {
    if (!pool.RemainingHistory.empty() || pool.NeedShrinkFromTenant) {
        LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() <<"ShrinkPool - " << pool.RemainingHistory.size() << " history entries remaining");
        return;
    }
    LOG_DEBUG_S(*TlsActivationContext, NKikimrServices::HIVE, GetLogPrefix() <<"ShrinkPool - done");
