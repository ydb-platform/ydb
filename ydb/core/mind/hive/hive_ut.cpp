    ...
    */
    ...
    void TestHiveScatterThresholdSource(
        ...
        if (resource == NHive::EResourceToBalance::Counter) {
            // This branch has no DefaultTabletAllowedMetrics configuration.
            // Keep Dummy tablets free of compute metrics from creation, so
            // their all-time memory maximum does not suppress Counter.
            TResourceProfiles::TResourceProfile profile;
            profile.SetTabletType(TTabletTypes::Dummy);
            profile.SetName("default");
            profile.SetDefaultTabletMemoryUsage(0);
            auto& profiles = runtime.GetAppData().ResourceProfiles;
            if (!profiles) {
