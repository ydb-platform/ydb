...
Y_UNIT_TEST_SUITE(TDirectBlockGroupTest)
{
    ...
    Y_UNIT_TEST_F(ShouldIgnoreLateConnectionResponseWithOldSeqNo, TDBGFixture)
    {
        auto executor = MakeExecutor();
        ...
        // Test only one DDisk host (index 0) for simplicity.
        const auto ddiskId = MakeDDiskIds(100)[0];
        auto dbg = MakeDirectBlockGroup(executor, std::move(transport));

        TPartitionDirectServiceMock service(true);
        auto initialReady = dbg->Run(&service);

        // Step 1: all connections succeed immediately.
        initialReady.Wait(WaitTimeout);
        UNIT_ASSERT(initialReady.HasValue());
        DrainExecutor(executor);

        // After initial connect, host 0 has SessionSeqNo == 1.
        {
            auto seqNo = RunOnExecutor(
                             executor,
        ...
        // Step 2: deferred response.
        TStorageTransportMock::TConnectPromise connect2Promise;
        RunOnExecutor(
            executor,
            [&]()
            {
                connect2Promise = transportPtr->SetPendingConnect(
                    EConnectionType::DDisk,
                    ddiskId);
                dbg->ReEstablishDDiskConnection(0);
                return true;
            })
            .GetValue(WaitTimeout);
        DrainExecutor(executor);

        // Step 3: deferred response.
        TStorageTransportMock::TConnectPromise connect3Promise;
        RunOnExecutor(
            executor,
            [&]()
            {
                connect3Promise = transportPtr->SetPendingConnect(
                    EConnectionType::DDisk,
                    ddiskId);
                dbg->ReEstablishDDiskConnection(0);
                return true;
        ...
        // Resolve #3 first: SessionSeqNo should advance to 3.
        connect3Promise.SetValue(TStorageTransportMock::MakeConnectResult());
        DrainExecutor(executor);

        {
            auto seqNo = RunOnExecutor(
                             ...
                             [&]() { return dbg->GetDDiskSessionSeqNo(0); })
                             .GetValue(WaitTimeout);
            UNIT_ASSERT_VALUES_EQUAL(3, seqNo);
        }

        // Resolve #2 (late, old seqNo=2): SessionSeqNo must stay at 3.
        connect2Promise.SetValue(TStorageTransportMock::MakeConnectResult());
        DrainExecutor(executor);
