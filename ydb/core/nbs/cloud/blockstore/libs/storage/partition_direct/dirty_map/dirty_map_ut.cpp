...
Y_UNIT_TEST_SUITE(TDirtyMapTest)
{
    ...
    /*Y_UNIT_TEST(ShouldAdditionalEraseQueueNotAffectFlushQueue)
    {
        ...
        THostMask confirmed = MakeHostMask(true, true, true, false, false);
        ...
        confirmed = confirmed.Include(THostMask::MakeOne(3));
        dirtyMap.UpdateAdditionalEraseQueue(
            THostMask::MakeOne(3),
            123,
            TBlockRange64::WithLength(10, 10));

        UNIT_ASSERT_VALUES_EQUAL(1, dirtyMap.GetInflightCount());
        auto flushHint1 = dirtyMap.MakeFlushHint(1);
        UNIT_ASSERT_VALUES_EQUAL(true, flushHint1.DebugPrint().empty());
        ...
        dirtyMap.UpdateAdditionalEraseQueue(
            ...
            123,
            TBlockRange64::WithLength(10, 10));
        auto flushHint2 = dirtyMap.MakeFlushHint(1);
        UNIT_ASSERT_VALUES_EQUAL(true, flushHint2.DebugPrint().empty());
    }

    // @brief updating of ReadyToEraseHanging does not affect main erase queue
    Y_UNIT_TEST(ShouldFlushHintAfterFlushOnUpdateWriteFinished)
    {
        const auto vchunkConfig = MakeTestVChunkConfig();
        TBlocksDirtyMap dirtyMap(
            vchunkConfig,
...

        {
            dirtyMap.UpdateAdditionalEraseQueue(
                ...
                123,
                TBlockRange64::WithLength(10, 10));
            auto eraseHint1 = dirtyMap.MakeEraseHint(1);
            UNIT_ASSERT_VALUES_EQUAL(true, eraseHint1.DebugPrint().empty());
        }

        auto eraseHint1 = dirtyMap.MakeEraseHangingHint(1);
        UNIT_ASSERT_VALUES_EQUAL(
            "H3:123[10..19];"
            "H4:123[10..19];",
            eraseHint1.DebugPrint());

        // Finish erasing
        for (const auto& [host, hint]: eraseHint.GetAllHints()) {
            dirtyMap.EraseFinished(host, GetLsns(hint.Segments), {});
        }
        // for (const auto& [host, hint]: eraseHint1.GetAllHints()) {
        //     dirtyMap.EraseFinished(host, GetLsns(hint.Segments), {});
        // }
