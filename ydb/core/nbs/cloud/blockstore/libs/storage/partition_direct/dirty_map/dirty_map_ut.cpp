...
THostStatusList MakeStatuses(std::initializer_list<EHostStatus> statuses)
{
...
{
    return MakeStatuses({Primary, Primary, Primary, HandOff, HandOff});
}
...
TMasks MakeMasks(const THostStatusList& pbuffer, const THostStatusList& ddisk)
{
    return TMasks{
        .DDiskReadable = ddisk.GetActive(),
        .DDiskFlushTargets = ddisk.GetPrimary(),
        .PBufferReadable = pbuffer.GetActive(),
    };
}

TMasks DefaultMasks()
{
    return MakeMasks(MakeDefaultStatuses(), MakeDefaultStatuses());
}
...
Y_UNIT_TEST_SUITE(TDirtyMapTest)
{
    Y_UNIT_TEST(ShouldReadWithoutWrites)
    {
        ...
        // primary.
        const auto newStatuses =
            MakeStatuses({Disabled, Primary, Primary, Primary, Disabled});
        masks = MakeMasks(newStatuses, newStatuses);
    ...
    Y_UNIT_TEST(ShouldReadAfterWriteFinished)
    {
        ...
        const auto newStatuses =
            MakeStatuses({Disabled, Primary, Primary, Primary, HandOff});
        masks = MakeMasks(newStatuses, newStatuses);
    ...
    Y_UNIT_TEST(ShouldWriteAndFlushAndErase)
    {
        ...
        auto flushHint =
            dirtyMap.MakeFlushHint(1, masks.DDiskFlushTargets, ddiskStates);
        ...
        flushHint =
            dirtyMap.MakeFlushHint(2, masks.DDiskFlushTargets, ddiskStates);
        UNIT_ASSERT_EQUAL(true, flushHint.Empty());
        ...
        auto eraseHints = dirtyMap.MakeEraseHint(2, masks.PBufferReadable);
        ...
        // number of write operations.
        eraseHints = dirtyMap.MakeEraseHint(2, masks.PBufferReadable);
        UNIT_ASSERT_VALUES_EQUAL(
        ...
        {
            auto eraseHint = dirtyMap.MakeEraseHint(2, masks.PBufferReadable);
    ...
    Y_UNIT_TEST(ShouldWriteAndFlushAndEraseWhenAdditionalHandOffDesired)
    {
        ...
        // hand-off.
        const auto statuses =
            MakeStatuses({Primary, Primary, Primary, Primary, HandOff});
        const auto masks = MakeMasks(statuses, statuses);
        ...
        // Erase hints
        auto eraseHints = dirtyMap.MakeEraseHint(1, masks.PBufferReadable);
        UNIT_ASSERT_VALUES_EQUAL(
            "H1:123[10..19];"
            "H2:123[10..19];"
            "H3:123[10..19];",
    ...
    Y_UNIT_TEST(ShouldWriteAndFlushAndEraseWithTwoDisabled)
    {
        ...
        const auto statuses =
            MakeStatuses({Disabled, Disabled, Primary, Primary, Primary});
        const auto masks = MakeMasks(statuses, statuses);
    ...
    Y_UNIT_TEST(ShouldNotFlushOverWriteWatermark)
    {
        ...
        auto flushHint =
            dirtyMap.MakeFlushHint(3, masks.DDiskFlushTargets, ddiskStates);
        UNIT_ASSERT_VALUES_EQUAL(
    ...
    Y_UNIT_TEST(ShouldLockPBuffer)
    {
        ...
        eraseHints = dirtyMap.MakeEraseHint(1, masks.PBufferReadable);
    ...
    Y_UNIT_TEST(ShouldReadFromDDiskIfRangeIsNotCoveredByInflightRange)
    {
        ...
        auto readHint = dirtyMap.MakeReadHint(
            ...
            ddiskStates);
        UNIT_ASSERT_VALUES_EQUAL(
            "0{[H0,H1,H2,H3,H4][0..9][0..9]};"
            "124{[H0,H1,H2][10..19][10..19]};"
            "0{[H0,H1,H2,H3,H4][20..99][20..99]};",
            readHint.DebugPrint());
    }
