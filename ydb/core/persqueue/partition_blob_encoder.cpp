...
std::deque<TDataKey> TPartitionBlobEncoder::GetDataKeysBody() const
{
    return DataKeysBody;
}

void TPartitionBlobEncoder::RecalcBlobSizes(const TVector<TDataKey>& dataKeys) {
    auto iter = DataKeysBody.begin();
    ui64 currentCumulativeSize = 0;
    for (const auto& newDataKey : dataKeys) {
        Y_ABORT_UNLESS(iter != DataKeysBody.end());
        if (iter->Key == newDataKey.Key) {
            iter->Size = newDataKey.Size;
            iter->CumulativeSize = currentCumulativeSize;
            iter++;
        }
        currentCumulativeSize = iter->CumulativeSize + iter->Size;
    }
}

void TPartitionBlobEncoder::Dump() const
{
   #define DBGTRACE_LOG(message) {TStringBuilder msg; msg << message << Endl; Cerr << msg; }
