...
TPartitionCompaction::TReadState::TReadState(ui64 firstOffset, TPartition* partitionActor)
    ...
    , PartitionActor(partitionActor)
{
    ui64 firstHeadOffset = PartitionActor->BlobEncoder.EndOffset;
...
TPartitionCompaction::TCompactState::TCompactState(
    ...
    , DataKeysBody(partitionActor->BlobEncoder.DataKeysBody)
{
    if (!PartitionActor->BlobEncoder.HeadKeys.empty()) {
        FirstHeadOffset = PartitionActor->BlobEncoder.HeadKeys.front().Key.GetOffset();
        FirstHeadPartNo = PartitionActor->BlobEncoder.HeadKeys.front().Key.GetPartNo();
    } else {
...
void TPartitionCompaction::TCompactState::UpdateDataKeysBody() {
    ...
    std::deque<TDataKey> oldDataKeys = std::move(PartitionActor->BlobEncoder.DataKeysBody);
    auto iterExst = oldDataKeys.begin();
    PartitionActor->BlobEncoder.DataKeysBody.clear();
    ...
    auto addCurrentKey = [&]() {
        ...
        PartitionActor->BlobEncoder.DataKeysBody.emplace_back(std::move(*iterExst));
    };
    ...
    Y_ENSURE(PartitionActor->BlobEncoder.DataKeysBody.size() == oldDataKeys.size() - zeroedKeys);
    Y_ENSURE(currCumulSize == PartitionActor->BlobEncoder.BodySize - sizeDiff);
    PartitionActor->BlobEncoder.BodySize = currCumulSize;
    PartitionActor->BlobEncoder.StartOffset = Max(
                    PartitionActor->BlobEncoder.StartOffset,
                    PartitionActor->BlobEncoder.DataKeysBody.front().Key.GetOffset() + (ui32)(PartitionActor->BlobEncoder.DataKeysBody.front().Key.GetPartNo() > 0));
    UpdatedKeys.clear();
    DeletedKeys.clear();
}
