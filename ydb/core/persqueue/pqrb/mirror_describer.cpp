...
void TMirrorDescriber::HandleChangeConfig(TEvPQ::TEvChangePartitionConfig::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_("got new config, equal with previous: " << equalConfigs);
    if (!equalConfigs) {
...
const TString& TMirrorDescriber::GetLogPrefix() const {
    if (!LogPrefix) {
        LogPrefix = TStringBuilder() << "[MirrorDescriber][" << TopicName << "] ";
    }
    return *LogPrefix;
}
