...
void TConsumerActor::Bootstrap() {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Start MLP consumer " << Config.GetName());
...
void TConsumerActor::Queue(TEvPQ::TEvMLPReadRequest::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPReadRequest " << ev->Get()->Record.ShortDebugString());
    ReadRequestsQueue.push_back(std::move(ev));
}

void TConsumerActor::Queue(TEvPQ::TEvMLPCommitRequest::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPCommitRequest " << ev->Get()->Record.ShortDebugString());
    CommitRequestsQueue.push_back(std::move(ev));
}

void TConsumerActor::Queue(TEvPQ::TEvMLPUnlockRequest::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPUnlockRequest " << ev->Get()->Record.ShortDebugString());
    UnlockRequestsQueue.push_back(std::move(ev));
}

void TConsumerActor::Queue(TEvPQ::TEvMLPChangeMessageDeadlineRequest::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPChangeMessageDeadlineRequest " << ev->Get()->Record.ShortDebugString());
    ChangeMessageDeadlineRequestsQueue.push_back(std::move(ev));
}

void TConsumerActor::Queue(TEvPQ::TEvMLPPurgeRequest::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPPurgeRequest " << ev->Get()->Record.ShortDebugString());
...
void TConsumerActor::Queue(TEvPQ::TEvMLPUpdateExternalLockedMessageGroupsId::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Queue TEvPQ::TEvMLPUpdateExternalLockedMessageGroupsId " << ev->Get()->Record.ShortDebugString());
...
void TConsumerActor::HandleOnInit(TEvKeyValue::TEvResponse::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "HandleOnInit TEvKeyValue::TEvResponse");
            ...
            switch(readResult.GetStatus()) {
                case NKikimrProto::OK: {
                    ...
                    if (Config.GetGeneration() == snapshot.GetConfiguration().GetGeneration()) {
                        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Read snapshot");
                        HasSnapshot = true;
                        LastWALIndex = snapshot.GetWALIndex();
                        Storage->Initialize(snapshot);
                    } else {
                        LOG_WARN_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Received snapshot from old consumer generation: " << Config.GetGeneration() << " vs " << snapshot.GetConfiguration().GetGeneration());
                    }

                    break;
                }
                case NKikimrProto::NODATA: {
                    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Initializing new consumer");
                    break;
                }
                default:
                ...
                case NKikimrProto::OVERRUN: {
                    for (auto& w : walResult.GetPair()) {
                        ...
                        if (Config.GetGeneration() == wal.GetGeneration()) {
                            LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Read WAL " << w.key());
                            LastWALIndex = wal.GetWALIndex();
                            Storage->ApplyWAL(wal);
                        } else {
                            LOG_WARN_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Received WAL from old consumer generation: " << Config.GetGeneration() << " vs " << wal.GetGeneration() << " key: " << w.key());
                        }
                    }

                    if (walResult.GetStatus() == NKikimrProto::OVERRUN) {
                        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "WAL overrun");
                        auto request = std::make_unique<TEvKeyValue::TEvRequest>();
    ...
    if (!FetchMessagesIfNeeded()) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Initialized");
        NotifyPQRB(true);
...
void TConsumerActor::Handle(TEvKeyValue::TEvResponse::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "HandleOnWrite TEvKeyValue::TEvResponse " << ev->Get()->Record.ShortDebugString());
    ...
    if (record.GetCookie() == static_cast<ui64>(EKvCookie::BackgroundWrite)) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Background write finished");
        return;
    }

    AFL_ENSURE(CurrentStateFunc() == &TConsumerActor::StateWrite)("c", record.GetCookie());

    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "TX write finished");
    Become(&TConsumerActor::StateWork);
...
void TConsumerActor::CommitIfNeeded() {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Try commit offset: " << offset << " vs " << LastCommittedOffset);
...
void TConsumerActor::UpdateStorageConfig() {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Update config: RetentionPeriod: " << (RetentionPeriod.has_value() ? RetentionPeriod->ToString() : "infinity")
...
void TConsumerActor::HandleInit(TEvPQ::TEvEndOffsetChanged::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Handle TEvPQ::TEvEndOffsetChanged. Offset: " << ev->Get()->Offset);
...
STFUNC(TConsumerActor::StateWrite) {
    ...
    switch (ev->GetTypeRewrite()) {
        ...
        sFunc(TEvents::TEvPoison, PassAway);
        default:
            LOG_ERROR_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Unexpected " << EventStr("StateWrite", ev));
            AFL_VERIFY_DEBUG(false)("Unexpected", EventStr("StateWrite", ev));
    }
}

void TConsumerActor::Restart(TString&& error) {
    LOG_ERROR_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << error);
...
void TConsumerActor::ProcessEventQueue() {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "ProcessEventQueue");
    ...
    for (auto& ev : UpdateExternalLockedMessageGroupsIdRequestsQueue) {
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "UpdateExternalLockedMessageGroupsId: " << "Applied=" << updateResult.Applied << ", " << "Invalid=" << updateResult.Invalid << ", " << "ModeChanged=" << updateResult.ModeChanged << ", " << "SetChanged=" << updateResult.SetChanged << ", " << "VersionChanged=" << updateResult.VersionChanged << "; " << ShortDebugString(record.GetUpdate()));
    ...
    LOG_TRACE_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "AfterDeadlinesDump: " << Storage->DebugString());
    ...
    for (auto& ev : ReadRequestsQueue) {
        ...
        if (messages.empty() && ev->Get()->GetWaitDeadline() <= now) {
            ...
            LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Reply empty result: sender=" << ev->Sender.ToString() << " cookie=" << ev->Cookie);
...
void TConsumerActor::Persist() {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Persist");

    Storage->Compact();

    auto batch = Storage->ExtractBatch();
    if (batch.Empty()) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Batch is empty");
        MoveToDLQIfPossible();
        return;
    }
    ...
    LOG_TRACE_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Dump befor persist: " << Storage->DebugString());
    ...
    if (withWAL) {
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Write WAL Size: " << data.size() << " Key: " << key);
    ...
    if (!withWAL || LastWALIndex % MaxWALCount == 0) {
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Delete old WAL: " << from << " - " << to);
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Write Snapshot Count: " << Storage->GetMessageCount() << " Size: " << write->GetValue().size() << " cookie: " << cookie);
    }
}
...
bool TConsumerActor::FetchMessagesIfNeeded() {
    ...
    if (PartitionEndOffset <= Storage->GetLastOffset()) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Skip fetch: partition end offset is reached: " << PartitionEndOffset << " vs " << Storage->GetLastOffset());
        return false;
    }

    auto& metrics = Storage->GetMetrics();
    if (metrics.InflightMessageCount >= Storage->MaxMessages) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Skip fetch: infly limit exceeded");
        return false;
    }
    if (!Config.GetKeepMessageOrder()
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Skip fetch: there are enough messages. InflightMessageCount=" << metrics.InflightMessageCount
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Fetching " << maxMessages << " messages from offset " << Storage->GetLastOffset() << " from " << PartitionActorId);
...
void TConsumerActor::Handle(TEvPQ::TEvProxyResponse::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Handle TEvPQ::TEvProxyResponse");
...
void TConsumerActor::Handle(TEvPersQueue::TEvResponse::TPtr& ev) {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Fetched " << messageCount << " messages");
    if (allMessagesAdded) {
        FetchMessagesIfNeeded();
    }
...
void TConsumerActor::HandleOnWork(TEvents::TEvWakeup::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "HandleOnWork TEvents::TEvWakeup " << ev->Get()->Tag);
    switch (ev->Get()->Tag) {
...
void TConsumerActor::MoveToDLQIfPossible() {
    ...
    if (!messages.empty()) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Move to DLQ: " << JoinSeq(", ", messages));
...
void TConsumerActor::Handle(TEvPQ::TEvMLPDLQMoverResponse::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Handle TEvPQ::TEvMLPDLQMoverResponse");

    if (ev->Get()->Status != Ydb::StatusIds::SUCCESS) {
        LOG_WARN_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Error moving messages to the DLQ: " << ev->Get()->ErrorDescription);
    }

    auto& moved = ev->Get()->MovedMessages;
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Moved to the DLQ: " << JoinSeq(", ", moved | std::views::transform(AsTDLQMessage)));
...
void TConsumerActor::Handle(TEvents::TEvWakeup::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "Handle TEvents::TEvWakeup " << ev->Get()->Tag);
...
void TConsumerActor::UpdateLockedGroupsIdInChildPartitions(bool force) {
    ...
    if (!shouldSend) {
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "UpdateLockedGroupsIdInChildPartitions no send diff");
        return;
    }
    ...
    for (auto& [childPartitionId, state] : ChildPartitionsOrderManager.ChildrenPartitionWithKeepOrder) {
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, Service, NPQ_LOG_PREFIX << "UpdateLockedGroupsIdInChildPartitions: updating child partition " << childPartitionId << "; reason=" << state.SendFullStateReasonsAsString() << "; update=" << ShortDebugString(record));
