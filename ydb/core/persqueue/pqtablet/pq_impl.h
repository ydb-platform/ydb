...
private:
    ...
    // становится равен ему только в EndWriteTxs при успехе: на неуспехе таблетка травится и счётчик
    // не двигается. Запись помнит номер отправки в CreatedAtWriteTxsCycle и уходит, когда
    // CompletedWriteTxsCycle > CreatedAtWriteTxsCycle. Строгое «больше» - это «цикл, отправленный позже».
    //
    // Флага «запись уже проходила» не хватает. Пока цикл в полёте, приходит следующий шаг. Успех
    // покрывает только те записи, которые уже лежали в деке к моменту отправки. Запрос, ушедший
    // раньше шага, не доказывает лидерство на момент прихода шага: он мог примениться, после этого
    // поколение зафенсили, а шаг доехал по ещё живому пайпу. Флаг после успеха отпустил бы и его.
    //
    // Одного числа не хватает по той же причине. Если растить его только на успехе, шаг, пришедший
    // в полёте, и шаг, пришедший на простое, неразличимы: оба увидят одно и то же увеличение и оба
    // уйдут. Если растить только на отправке, неуспешный цикл выглядит как прогресс. Поэтому один
    // счётчик растёт на отправке, а второй копирует его значение на успехе. Снимок на записи нужен
    // отдельно: без него успех один на всю деку, и шаг, пришедший после отправки, уходит вместе
    // с теми, кто этого цикла ждал.
