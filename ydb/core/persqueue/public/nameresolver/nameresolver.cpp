...
// Port of TDiscoveryConverter + TTopicNameConverter (BuildFrom*, BuildInternals).
struct TNameBuilder {
    bool FstClass = false;
    bool Valid = true;
    TString Reason;
    ...
    void SetDatabaseFromConfig(const TString& database) {
        ...
        CHECK_SET_VALID(!FullModernName.empty(),
            TStringBuilder() << "Internal error: FullModernName empty when setting database for topic: " << OriginalTopic,
            return);
    ...
    void BuildInternals(const NKikimrPQ::TPQTabletConfig& config) {
        ...
        Database = db;
        if (FstClass) {
            CHECK_SET_VALID(!path.empty(),
                TStringBuilder() << "Empty topic path for first class topic: " << OriginalTopic,
                return);
            path.SkipPrefix(db);
            path.SkipPrefix("/");
            ClientsideName = path;
            ...
            CHECK_SET_VALID(!FullLegacyName.empty(),
                TStringBuilder() << "Empty full legacy name for topic: " << config.GetTopicPath(),
                return);
            ...
            CHECK_SET_VALID(!FullModernName.empty(),
                TStringBuilder() << "Empty full modern name for topic: " << config.GetTopicPath(),
                return);
        ...
        const TString& topicPath)
    {
        ...
        if (name.empty()) {
            CHECK_SET_VALID(!path.empty(), "Empty topic path and topic name", return);
            TStringBuf pathBuf(path), fst, snd;
            auto res = pathBuf.TryRSplit("/", fst, snd);
            CHECK_SET_VALID(res, TStringBuilder() << "Cannot derive topic name from path: " << path, return);
            name = snd;
        } else if (path.empty()) {
            path = name;
        }
...
TTopicNames NamesFromConfig(const NKikimrPQ::TPQTabletConfig& config, const TString& topicPath, bool firstClassCitizen) {
    return NamesFromConfig(config, topicPath, firstClassCitizen, {}, {});
}
