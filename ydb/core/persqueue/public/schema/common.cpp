...
TResult ProcessTopicAttributes(
    ...
    for (const auto& [attrName, attrValue] : attributes) {
        ...
        } else if (attrName == "_id") {
            // Topic id for SourceId->Partition mapping. Honored only for federation
            // (for FirstClass topics the id is the LocalPathId set by schemeshard at create)
            // and only under the feature flag; otherwise the attribute is silently ignored.
            // On alter an existing topic Id is never changed: schemeshard preserves the
            // stored Id and accepts this value only as a back-fill when the Id is empty.
            if (AppData()->FeatureFlags.GetEnableTopicSourceIdMappingById()
                    && !topicsAreFirstClassCitizen
                    && !attrValue.empty()) {
                ui64 idValue = 0;
                try {
                    idValue = FromString<ui64>(attrValue);
                } catch (...) {
                    return {Ydb::StatusIds::BAD_REQUEST,
                        TStringBuilder() << "Attribute " << attrName << " is " << attrValue
                            << ", which is not a valid positive integer"};
                }
                if (idValue == 0) {
                    return {Ydb::StatusIds::BAD_REQUEST,
                        TStringBuilder() << "Attribute " << attrName << " is " << attrValue
                ...
                tabletConfig->MutableId()->SetId(idValue);
            } else if (!attrValue.empty()) {
                AFL_WARN(NKikimrServices::PERSQUEUE)(
                    "msg", "_id attribute is set but will be ignored"
                )(
                    "reason", AppData()->FeatureFlags.GetEnableTopicSourceIdMappingById()
                        ? "FirstClassCitizen topic: id is generated by schemeshard"
                        : "feature flag EnableTopicSourceIdMappingById is off"
                );
            }
