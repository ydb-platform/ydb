...
void TUsersInfoStorage::ParseDeprecated(const TString& key, const TString& data, const TActorContext& ctx) {
    ...
    if (!userInfo) {
        Create(ctx, user, 0, false, session, 0, gen, step, static_cast<i64>(offset), 0, TInstant::Zero(), {}, false); //savnik check anyCommits
    } else {
...
void TUsersInfoStorage::Parse(const TString& key, const TString& data, const TActorContext& ctx) {
    ...
    if (!userInfo) {
        Create(
            ...
            userData.GetOffsetRewindSum(), TInstant::Zero(),  {}, userData.GetAnyCommits() // savnik
        );
    } else {
