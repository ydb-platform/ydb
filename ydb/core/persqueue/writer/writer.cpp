...
#define PW_ENSURE(condition) AFL_ENSURE(condition)("topic", Opts.TopicPath)("tablet_id", TabletId)
...
class TPartitionWriter : public TActorBootstrapped<TPartitionWriter>, public TPartitionWriterOpts::IGetter, private TRlHelpers {
    ...
    TWriteId MakeDeferredWriteIdFromOpts() const {
        PW_ENSURE(Opts.DeferredPublish);
    ...
    void StartUpsertDestination() {
        PW_ENSURE(IsDeferredPublishWriter());
        PW_ENSURE(!Opts.Database.empty());
    ...
    void SavePartitionIdToKqpTxn(const TActorContext& ctx) {
        PW_ENSURE(HasWriteId());
        PW_ENSURE(HasSupportivePartitionId());
    ...
    void HandleMaxSeqNo(TEvPersQueue::TEvResponse::TPtr& ev, const TActorContext& ctx) {
        ...
        PW_ENSURE(sourceIdInfo.GetSeqNo() >= 0)("source_id", SourceId)("seq_no", sourceIdInfo.GetSeqNo());
