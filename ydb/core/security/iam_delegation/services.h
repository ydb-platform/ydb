...
// a request. TEvSetupDelegation / TEvRevokeDelegation act with the authority of the node's system service
// account on behalf of the SubjectId they carry. A caller must therefore authorize the subject for the
// secret and the service account / cloud pair before asking; that is the job of the secret machinery these
// services are made for, and nothing else on the node should use these ids.
