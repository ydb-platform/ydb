...
public:
    static constexpr NKikimrServices::TActivity::EType ActorActivityType() {
        return NKikimrServices::TActivity::PASSWORD_HASHER;
    }
        ...
        , PasswordChecker(passwordComplexity)
    {
        ...
        for (const auto& record : HashesRegistry) {
            AvailableHashTypes.emplace(record.Type, record);
        }
    }
    ...
    void Bootstrap() {
        ...
        for (const auto& hashType : HashTypes) {
            if (AvailableHashTypes.contains(hashType)) {
                const auto& hashTypeDescription = AvailableHashTypes.at(hashType);
...
private:
    ...
    THashMap<EHashType, const THashTypeDescription&> AvailableHashTypes;
