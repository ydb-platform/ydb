...
private:
    ...
    [[nodiscard]] bool ComputeHash(const TActorContext &ctx,
        ...
        for (const auto& allowedHashType : ALLOWED_HASHES_TO_AUTH) {
            ...
            case EHashClass::Argon: {
                if (!IsBase64(itHashesInitParams->second)) {
                    YDB_LOG_CTX_ERROR(ctx, "Authentication failed: has broken Argon hash",
                        {"ActorName", ActorName},
                        {"ctx.SelfID", ctx.SelfID.ToString()},
                        {"AuthcId", AuthcId});
            ...
            case EHashClass::Scram: {
                ...
                if (!TryFromString(scramInitParams.IterationsCount, iterationsCount)
                    ...
                    YDB_LOG_CTX_ERROR(ctx, "Authentication failed: has broken Scram hash",
                        {"ActorName", ActorName},
                        {"ctx.SelfID", ctx.SelfID.ToString()},
                        {"AuthcId", AuthcId});
                    SendError(NKikimrIssues::TIssuesIds::UNEXPECTED, "");
        ...
        if (ChosenAuthHashType == EHashType::Unknown) {
            YDB_LOG_CTX_ERROR(ctx, "Authentication failed: has no hashes",
                {"ActorName", ActorName},
                {"ctx.SelfID", ctx.SelfID.ToString()},
                {"AuthcId", AuthcId});
            SendError(NKikimrIssues::TIssuesIds::UNEXPECTED, "");
