...
TSerializedCellVec SelectShortestMedianKeyPrefix(const NKikimr::NTable::THistogram& histogram, ui64 totalSize, const TKeyTypes& keyColumnTypes) {
    ...
    // Data size histogram assumed to be sorted and cumulative.
    // Keys are unique and sorted, with each weight including all previous weights.
        ...
        if (0 != CompareTypedCells(keyLo.GetCells()[i], keyMed.GetCells()[i], columnType)) {
            ...
            if (IsIntegerType(columnType) && !keyMed.GetCells()[i].IsNull()) {
                ...
                const auto it = UpperBound(
                            ...
                            keyMed,
                            fnCmpCurrentCell);
                // If no key strictly > med exists before idxHi, it == bucketsBegin+idxHi,
                // which points to keyHi — a valid fallback satisfying > med and <= hi.
    ...
    TSerializedCellVec lowestKey(histogram[0].EndKey);
    if (0 < CompareTypedCellVectors(lowestKey.GetCells().data(), splitKey.data(),
        keyColumnTypes.data(),
        lowestKey.GetCells().size(), splitKey.size())
    ) {
        return {};
    }
