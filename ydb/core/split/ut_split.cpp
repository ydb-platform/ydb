...
Y_UNIT_TEST_SUITE(TSplitMakeKeyAccessHistogram) {
...
Y_UNIT_TEST_SUITE(TSplitSelectShortestMedianKeyPrefixBySize) {
    ...
    Y_UNIT_TEST(BinarySearchNoIntermediateKey) {
        // When all buckets in [idxMed, idxHi) share the same i-th cell value as
        // keyMed, the binary search falls back to keyHi[i] as the split key.
        // This is correct: the contract is to find a value > med and <= hi,
        // and keyHi satisfies that when no strictly intermediate key exists.
        //
        // Histogram (cumulative, totalSize=100):
        //   bucket[0]: key="a", value=25  -> idxLo=0
        //   bucket[1]: key="a", value=50  -> idxMed=1 (diff=0, perfect median)
        //   bucket[2]: key="b", value=75  -> idxHi=2
        //   bucket[3]: key="b", value=100
        //
        // lo[0]="a", med[0]="a", hi[0]="b": enters binary search branch.
        // UpperBound on [idxMed, idxHi] finds no "a" < "a" in bucket[1],
        // falls through to bucket[2] (keyHi) -> splitKey[0] = "b".
        TVector<NScheme::TTypeInfo> keyTypes = {
            NScheme::TTypeInfo(NScheme::NTypeIds::Utf8)
        };
        ...
        auto result = SelectShortestMedianKeyPrefix(hist, 100, keyTypes);
        UNIT_ASSERT(!IsEmpty(result));
        UNIT_ASSERT_VALUES_EQUAL(PrintKey(result, keyTypes), "(Utf8 : b)");
    }
