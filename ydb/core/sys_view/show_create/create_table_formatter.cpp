...
void TCreateTableFormatter::FormatPrimitive(NYdb::TValueParser& parser) {
    switch (parser.GetPrimitiveType()) {
        ...
        case NYdb::EPrimitiveType::Date32: {
            Stream << "DATE32(";
            const NUdf::TUnboxedValue str = NMiniKQL::ValueToString(NUdf::EDataSlot::Date32, NUdf::TUnboxedValuePod(parser.GetDate32().count()));
            Y_ENSURE(str.HasValue());
            EscapeString(TString(str.AsStringRef()), Stream);
            Stream << ")";
            break;
        ...
        case NYdb::EPrimitiveType::Datetime64: {
            Stream << "DATETIME64(";
            const NUdf::TUnboxedValue str = NMiniKQL::ValueToString(NUdf::EDataSlot::Datetime64, NUdf::TUnboxedValuePod(static_cast<i64>(parser.GetDatetime64().count())));
            Y_ENSURE(str.HasValue());
            EscapeString(TString(str.AsStringRef()), Stream);
            Stream << ")";
            break;
        ...
        case NYdb::EPrimitiveType::Timestamp64: {
            Stream << "TIMESTAMP64(";
            const NUdf::TUnboxedValue str = NMiniKQL::ValueToString(NUdf::EDataSlot::Timestamp64, NUdf::TUnboxedValuePod(static_cast<i64>(parser.GetTimestamp64().count())));
            Y_ENSURE(str.HasValue());
            EscapeString(TString(str.AsStringRef()), Stream);
            Stream << ")";
            break;
