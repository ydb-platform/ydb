...
public:
    TStreamingQueriesScan(const TActorId& ownerId, ui32 scanId, const NKikimrSysView::TSysViewDescription& sysViewInfo,
...
private:
    ...
    void ContinueScan() {
        ...
        if (!DatabaseId) {
            ...
            const auto& databaseFetcher = Register(NKqp::NWorkload::CreateDatabaseFetcherActor(SelfId(), Database, MakeIntrusive<NACLib::TUserToken>(BUILTIN_ACL_METADATA, TVector<NACLib::TSID>{})));
        ...
        if (!ListedQueries.empty()) {
            ...
            const auto& describer = Register(new TStreamingQueryDescribeActor(Database, std::move(queriesPaths), UserToken));
...
private:
    ...
    TString LogPrefix() const {
        return TStringBuilder() << "[TStreamingQueriesScan] OwnerId: " << OwnerActorId << " ActorId: " << SelfId() << " ScanId: " << ScanId << " Database: " << Database << ". ";
    }
...
THolder<IActor> CreateStreamingQueriesScan(const TActorId& ownerId, ui32 scanId, const NKikimrSysView::TSysViewDescription& sysViewInfo,
    const TTableRange& tableRange, const TArrayRef<NMiniKQL::TKqpComputeContextBase::TColumn>& columns,
    TIntrusiveConstPtr<NACLib::TUserToken> userToken, const TString& database, bool reverse) {
    return MakeHolder<TStreamingQueriesScan>(ownerId, scanId, sysViewInfo, tableRange, columns, std::move(userToken), database, reverse);
}
