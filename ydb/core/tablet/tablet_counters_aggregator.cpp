...
public:
    ...
    void QueryLabeledCounters(const NKikimrLabeledCounters::TEvTabletLabeledCountersRequest& request, NKikimrLabeledCounters::TEvTabletLabeledCountersResponse& response, const TActorContext& ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "got request v",
        ...
        YDB_LOG_CTX_INFO(ctx, "request processed, groups processed",
            {"cc", cc});
    }
...
TTabletCountersAggregatorActor::HandleWork(TEvTabletCounters::TEvTabletAddLabeledCounters::TPtr &ev, const TActorContext &ctx) {
    ...
    YDB_LOG_CTX_DEBUG(ctx, "got labeledCounters",
        {"GetDatabasePath", msg->LabeledCounters.Get()->GetDatabasePath()},
        {"GetGroup", msg->LabeledCounters.Get()->GetGroup()},
        {"#_num_0", (msg->TabletType == TTabletTypes::PersQueue ? "PQ" : "different")});
    if (msg->LabeledCounters.Get()->GetDatabasePath()) {
        if (msg->TabletType == TTabletTypes::PersQueue) {
            YDB_LOG_CTX_DEBUG(ctx, "got labeledCounters",
                {"from_db", msg->LabeledCounters.Get()->GetDatabasePath()},
                {"tablet", msg->TabletID});
            TabletMon->ApplyLabeledDbCounters(msg->LabeledCounters.Get()->GetDatabasePath().GetRef(), msg->TabletID, msg->LabeledCounters.Get(), ctx);
        } else {
            YDB_LOG_CTX_ERROR(ctx, "got labeledCounters from unknown Tablet",
                {"Type", msg->TabletType},
...
public:
    ...
    void SendRequest(ui32 nodeId, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator actor request to node",
            {"nodeId", nodeId},
            {"SelfID", ctx.SelfID});
        ++NodesRequested;
    }
    ...
    void Bootstrap(const TActorContext& ctx) {
        ...
        if (NumWorkers > 0) {
            ...
            YDB_LOG_CTX_INFO(ctx, "aggregator new request V1 Initiator self worker",
                {"Initiator", Initiator},
                {"SelfID", ctx.SelfID},
                {"WorkerId", WorkerId});
        } else {
            YDB_LOG_CTX_INFO(ctx, "aggregator new request V1",
                {"SelfID", ctx.SelfID});
    ...
    void Undelivered(TEvents::TEvUndelivered::TPtr &ev, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator actor undelivered node",
            {"nodeId", nodeId},
            {"SelfID", ctx.SelfID});
    ...
    void Disconnected(TEvInterconnect::TEvNodeDisconnected::TPtr &ev, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator actor disconnected node",
            {"nodeId", nodeId},
            {"SelfID", ctx.SelfID});
        SessionSubscriptions->Handle(ev);
        if (PerNodeResponse.emplace(nodeId, nullptr).second) {
    ...
    void HandleResponse(TEvTabletCounters::TEvTabletLabeledCountersResponse::TPtr &ev, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator actor got response node",
            {"nodeId", nodeId},
            {"SelfID", ctx.SelfID});
        if (SessionSubscriptions->IsSubscribed(nodeId)) {
            SessionSubscriptions->Unsubscribe(nodeId);
    ...
    void HandleTimeout(const TActorContext &ctx) {
        YDB_LOG_CTX_DEBUG(ctx, "aggregator actor got TIMEOUT");
        ReplyAndDie(ctx);
    }
    ...
    void ReplyAndDie(const TActorContext& ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator all answers received - replying",
            {"SelfID", ctx.SelfID});
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator request processed - got groups and counters Initiator",
            {"numGroups", numGroups},
...
public:
    ...
    void Bootstrap(const TActorContext& ctx) {
        ...
        if (NumWorkers > 0) {
            ...
            TBase::Become(&TThis::StateRequestedBrowse);
            ctx.Schedule(TDuration::Seconds(AGGREGATOR_TIMEOUT_SECONDS), new TEvents::TEvWakeup());
            YDB_LOG_CTX_INFO(ctx, "aggregator new request V2 Initiator self worker",
                {"Initiator", Initiator},
                {"SelfID", ctx.SelfID},
                {"WorkerId", WorkerId});
        } else {
            YDB_LOG_CTX_INFO(ctx, "aggregator new request V2",
                {"SelfID", ctx.SelfID});
            for (ui32 i = 0; i < WorkerId; ++i) {
    ...
    void HandleResponse(TEvTabletCounters::TEvTabletLabeledCountersResponse::TPtr &ev, const TActorContext &ctx) {
        ...
        YDB_LOG_CTX_INFO(ctx, "aggregator actor merged response node",
            {"nodeId", nodeId},
            {"SelfID", ctx.SelfID});
    ...
    virtual void ReplyAndDie(const TActorContext& ctx) {
        YDB_LOG_CTX_INFO(ctx, "aggregator request processed Initiator",
            {"SelfID", ctx.SelfID},
            {"Initiator", Initiator});
        ui64 cookie = NumWorkers ? WorkerId : 0;
        ctx.Send(Initiator, Response.Release(), 0, cookie);
