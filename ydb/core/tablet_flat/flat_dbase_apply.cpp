...
bool TSchemeModifier::Apply(const TAlterRecord &delta)
{
    ...
    if (action == TAlterRecord::AddTable) {
        ...
        changes |= AddColumnWithTypeInfo(table, delta.GetColumnName(), delta.GetColumnId(),
            delta.GetColumnType(), typeInfoProto, delta.GetNotNull(), delta.GetSetNotNullInProgress(), delta.GetIsSensitive(), null);
...
bool TSchemeModifier::AddColumn(ui32 tid, const TString &name, ui32 id, ui32 type, bool notNull, bool setNotNullInProgress, bool isSensitive, TCell null)
{
    Y_ENSURE(!NScheme::NTypeIds::IsParametrizedType(type));
    return AddColumnWithTypeInfo(tid, name, id, type, {}, notNull, setNotNullInProgress, isSensitive, null);
}
        ...
        const std::optional<NKikimrProto::TTypeInfo>& typeInfoProto, bool notNull, bool setNotNullInProgress, bool isSensitive, TCell null)
{
    ...
    auto pr = table->Columns.emplace(id, TColumn(name, id, typeInfo, pgTypeMod, notNull, setNotNullInProgress, isSensitive));
    Y_ENSURE(pr.second);
    it = pr.first;
    table->ColumnNames.emplace(name, id);
