...
namespace NTabletFlatExecutor {
    ...
    /**
     * Builds a sorted Part directly inside the local database, bypassing the
    ...
    public:
         ...
         * part's column positions: key cells are in key order, value cells are in
         * valueTags order (each tag mapped to its column via the row scheme). Keys
         * must arrive in strictly ascending order; otherwise the writer fails and
         * returns false. version is baked into the part as the row's commit version.
         */
        bool AddRow(TArrayRef<const TCell> key,
                    TArrayRef<const TCell> values, TArrayRef<const ui32> valueTags,
                    TRowVersion version, const TActorContext& ctx)
        {
            ...
            NTable::TRowState row(Scheme->Cols.size());
            row.Touch(NTable::ERowOp::Upsert);
            for (const auto& col : Scheme->Cols) {
                if (col.IsKey()) {
                    row.Set(col.Pos, NTable::ECellOp::Set, key[col.Key]);
                }
            }
            Y_ENSURE(values.size() == valueTags.size(), "Value cell/tag count mismatch");
            for (size_t i = 0; i < valueTags.size(); ++i) {
                const auto *col = Scheme->ColInfo(valueTags[i]);
                Y_ENSURE(col, "Value column tag is missing from the row scheme");
                row.Set(col->Pos, NTable::ECellOp::Set, values[i]);
            }

            return AddRow(key, row, version, ctx);
        }

        /**
         * Append a single committed row already laid out as a TRowState (indexed by
         * TColInfo::Pos). Keys must arrive in strictly ascending order; otherwise the
         * writer fails and returns false. version is baked in as the commit version.
         */
