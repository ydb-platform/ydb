...
namespace NTest {
    ...
    public:
        ...
        const TSharedData* TryGetPage(const TPart* part, TPageLocation location, TGroupId groupId) override
        {
            auto pass = ShouldPass((const void*)part, static_cast<ui64>(static_cast<size_t>(location.Offset)),
            ...
            if (partGroupQueues.empty()) {
                ...
                for (ui32 room : xrange(partStore->Store->GetRoomCount())) {
                    ...
                    if (room < partStore->Store->GetGroupCount()) {
                        ...
                        partGroupQueues.push_back(Settle(partStore, room, MakeOuter(partStore), groupPageCollection, indexPageCollection));
                    } else if (room == partStore->Store->GetExternRoom()) {
                        partGroupQueues.push_back(Settle(partStore, room, MakeExtern(partStore), groupPageCollection, indexPageCollection));
                    } else {
