...
bool TCommonBlobsTracker::DoUseBlob(const TUnifiedBlobId& blobId) {
    ...
    if (it == BlobsUseCount.end()) {
        ...
        AFL_TRACE(NKikimrServices::TX_COLUMNSHARD)("method", "DoUseBlob")("blob_id", blobId)("count", 1);
        return true;
    } else {
        ++it->second;
        AFL_TRACE(NKikimrServices::TX_COLUMNSHARD)("method", "DoUseBlob")("blob_id", blobId)("count", it->second);
        return false;
    }
}
...
bool TCommonBlobsTracker::DoFreeBlob(const TUnifiedBlobId& blobId) {
    ...
    --useIt->second;
    AFL_TRACE(NKikimrServices::TX_COLUMNSHARD)("method", "DoFreeBlob")("blob_id", blobId)("count", useIt->second);
