...
bool TGCTask::DoOnCompleteTxBeforeCleaning(
    NColumnShard::TColumnShard& /*self*/,
    const std::shared_ptr<IBlobsGCAction>& /*taskAction*/
) {
...
void TGCTask::OnGCResult(TEvBlobStorage::TEvCollectGarbageResult::TPtr ev) {
    AFL_VERIFY(ev->Get()->Status == NKikimrProto::OK)
    ("status", ev->Get()->Status)("details", ev->Get()->ToString())("action_id", GetActionGuid());
...
std::unique_ptr<TEvBlobStorage::TEvCollectGarbage> TGCTask::BuildRequest(const TBlobAddress& address) const {
    ...
    ("event",
     "build_gc_request")("address", address.DebugString())("current_gen", CurrentGen)("gen", CollectGenStepInFlight)(
    ...
    auto result = std::make_unique<TEvBlobStorage::TEvCollectGarbage>(
        TabletId,
        CurrentGen,
        PerGenerationCounter.Val(),
        address.GetChannelId(),
        !!CollectGenStepInFlight,
        CollectGenStepInFlight ? CollectGenStepInFlight->Generation() : 0,
