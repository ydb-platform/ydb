...
protected:
    ...
    virtual bool DoOnCompleteTxBeforeCleaning(
        NColumnShard::TColumnShard& /*self*/, const std::shared_ptr<IBlobsGCAction>& /*taskAction*/) override {
        return true;
    }
...
public:
    TGCTask(const TString& storageId, std::deque<TUnifiedBlobId>&& draftBlobIds,
        const std::shared_ptr<TExternalStorageOperatorHolder>& externalStorageOperator, TBlobsCategories&& blobsToRemove,
