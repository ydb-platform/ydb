...
Y_UNIT_TEST_SUITE(TierObjectKey) {
    ...
    Y_UNIT_TEST(ParseRejectsForeignKeys) {
        ...
        // a tree blob written down as a flat key and vice versa: the key does not match the layout of its own id
    ...
    Y_UNIT_TEST(FanoutSpreadsBlobs) {
        ...
        // the whole point of the fan-out: one generation of one tablet does not sit under a single prefix
