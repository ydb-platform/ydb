...
std::unique_ptr<NColumnShard::TEvWriteCommitSyncTransactionOperator>
CreateTxOperator(const NKikimrTxColumnShard::ETransactionKind kind) const {
    if (IsPrimary()) {
...
void TColumnShard::Handle(NEvents::TDataEvents::TEvWrite::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!schema) {
        LWPROBE(EvWrite, TabletID(), source.ToString(), cookie, record.GetTxId(), writeTimeout.value_or(TDuration::Max()), 0, "", false,
            operation.GetIsBulk(), ToString(NKikimrDataEvents::TEvWriteResult::STATUS_BAD_REQUEST), "unknown schema version");
    ...
    if (!TablesManager.IsReadyForStartWrite(*internalPathId, false)) {
        LWPROBE(EvWrite, TabletID(), source.ToString(), cookie, record.GetTxId(), writeTimeout.value_or(TDuration::Max()), 0, "", false,
            operation.GetIsBulk(), ToString(NKikimrDataEvents::TEvWriteResult::STATUS_INTERNAL_ERROR), "unknown schema version");
    ...
    if (overloadStatus != EOverloadStatus::None) {
        ...
        LWPROBE(EvWrite, TabletID(), source.ToString(), cookie, record.GetTxId(), writeTimeout.value_or(TDuration::Max()), arrowData->GetSize(),
            ...
            "overload data error " + ToString(overloadStatus));
        auto result = NEvents::TDataEvents::TEvWriteResult::BuildError(
