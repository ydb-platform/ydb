    ...
    for (auto&& aggr : baseAggregations) {
        ...
        if (putResult.GetPutStatus() != NKikimrProto::OK) {
            ...
            if (writeMeta.HasLongTxId()) {
                auto result = std::make_unique<TEvColumnShard::TEvWriteResult>(TabletID(), writeMeta.GetPathId().GetLocalPathId(), writeMeta.GetDedupId(), writeMeta.GetWriteId(), errCode);
...
void TColumnShard::Handle(TEvColumnShard::TEvWrite::TPtr& ev, const TActorContext& ctx) {
    ...
    const auto localPathId = TLocalPathId::FromRawValue(record.GetTableId());
    ...
    auto writeMetaPtr = std::make_shared<NEvWrite::TWriteMeta>(writeId, TUnifiedPathId{*pathId, localPathId}, source, granuleShardingVersion,
    ...
    if (!TablesManager.IsReadyForStartWrite({*pathId, localPathId}, false)) {
...

    {
        auto status = TablesManager.GetPrimaryIndexAsVerified<NOlap::TColumnEngineForLogs>()
                          .GetGranuleVerified(writeMeta.GetPathId().GetInternalPathId())
                          .GetOptimizerPlanner()
                          .CheckWriteData();
        if (status.IsFail()) {
    ...
    if (overloadStatus != EOverloadStatus::None) {
        ...
        std::unique_ptr<NActors::IEventBase> result = std::make_unique<TEvColumnShard::TEvWriteResult>(
            TabletID(), writeMeta.GetPathId().GetLocalPathId(), writeMeta.GetDedupId(), writeMeta.GetWriteId(), NKikimrTxColumnShard::EResultStatus::OVERLOADED);
        ...
        if (ui64 writeId = (ui64)HasLongTxWrite(writeMeta.GetLongTxIdUnsafe(), writeMeta.GetWritePartId())) {
            ...
            auto result =
                std::make_unique<TEvColumnShard::TEvWriteResult>(TabletID(), writeMeta.GetPathId().GetLocalPathId(), writeMeta.GetDedupId(), writeId, NKikimrTxColumnShard::EResultStatus::SUCCESS);
            ctx.Send(writeMeta.GetSource(), result.release());
...
void TColumnShard::Handle(NEvents::TDataEvents::TEvWrite::TPtr& ev, const TActorContext& ctx) {
    ...
    const auto localPathId = TLocalPathId::FromRawValue(operation.GetTableId().GetTableId());
    const auto pathId = TablesManager.ResolveInternalPathId(localPathId);
    if (!pathId) {
    ...
    if (overloadStatus != EOverloadStatus::None) {
        ...
        OverloadWriteFail(overloadStatus,
            NEvWrite::TWriteMeta(0, {*pathId, localPathId}, source, {}, TGUID::CreateTimebased().AsGuidString(),
    ...
    WriteTasksQueue->Enqueue(TWriteTask(arrowData, schema, source, granuleShardingVersionId, {*pathId, localPathId}, cookie, lockId, *mType, behaviour));
