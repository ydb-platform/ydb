...
void TColumnShard::Handle(NOlap::NDataSharing::NEvents::TEvSendDataFromSource::TPtr& ev, const TActorContext& ctx) {
    ...
    THashMap<NColumnShard::TLocalPathId, NOlap::NDataSharing::NEvents::TPathIdData> dataByPathId;
    ...
    for (auto&& i : ev->Get()->Record.GetPathIdData()) {
        ...
        AFL_VERIFY(dataByPathId.emplace(TLocalPathId::FromProto(i.GetPathId()), data.DetachResult()).second);
    }
...
private:
    ...
    std::shared_ptr<NOlap::NDataAccessorControl::IAccessorCallback> FetchCallback;
    THashMap<TInternalPathId, std::vector<NOlap::TPortionInfo::TConstPtr>> PortionsByPath;
