...
void TTxApplyLinksModification::DoComplete(const TActorContext& /*ctx*/) {
    ...
    auto ev = std::make_unique<NOlap::NDataSharing::NEvents::TEvApplyLinksModificationFinished>(
        Task->GetTabletId(), SessionId, PackIdx
    );
    NActors::TActivationContext::AsActorContext().Send(
        MakePipePerNodeCacheID(false),
        new TEvPipeCache::TEvForward(ev.release(), (ui64)InitiatorTabletId, true),
