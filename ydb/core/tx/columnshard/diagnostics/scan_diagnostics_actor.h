...
class TScanDiagnosticsActor: public NActors::TActorBootstrapped<TScanDiagnosticsActor> {
    ...
    STRICT_STFUNC(StateMain,
        hFunc(NMon::TEvRemoteHttpInfo, Handle) hFunc(NColumnShard::TEvPrivate::TEvReportScanDiagnostics, Handle)
            hFunc(NColumnShard::TEvPrivate::TEvReportScanIteratorDiagnostics, Handle) cFunc(TEvents::TEvPoisonPill::EventType, PassAway))
