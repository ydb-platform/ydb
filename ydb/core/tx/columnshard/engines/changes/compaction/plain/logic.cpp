...
void TPlainMerger::DoStart(
    const std::vector<std::shared_ptr<NArrow::NAccessor::IChunkedArray>>& input,
    TMergingContext& /*mContext*/
) {
    for (auto&& p : input) {
        if (p) {
            Cursors.emplace_back(NCompaction::TPortionColumnCursor(p));
        } else {
            Cursors.emplace_back(NCompaction::TPortionColumnCursor(
                Context.GetLoader()->GetResultField()->type(), Context.GetLoader()->GetDefaultValue()
            ));
        }
    }
}

std::vector<NKikimr::NOlap::NCompaction::TColumnPortionResult>
TPlainMerger::DoExecute(const TChunkMergeContext& chunkContext, TMergingContext& mContext) {
