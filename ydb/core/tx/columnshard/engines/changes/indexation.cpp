...
TConclusionStatus TInsertColumnEngineChanges::DoConstructBlobs(TConstructionContext& context) noexcept {
    ...
    for (auto& [pathId, batches] : pathBatches) {
        ...
        for (auto&& b : result) {
            ...
            if (b->num_rows() < 100) {
                SaverContext.SetExternalSerializer(NArrow::NSerialization::TSerializerContainer(std::make_shared<NArrow::NSerialization::TArrowSerializer>(arrow::Compression::type::UNCOMPRESSED)));
            } else {
                SaverContext.SetExternalSerializer(NArrow::NSerialization::TSerializerContainer(std::make_shared<NArrow::NSerialization::TArrowSerializer>(arrow::Compression::type::LZ4_FRAME)));
            }
