...
TColumnEngineForLogs::TColumnEngineForLogs(const ui64 tabletId, const std::shared_ptr<TSchemaObjectsCache>& schemaCache,
    ...
    const std::shared_ptr<IStoragesManager>& storagesManager, const TSnapshot& snapshot, const TSchemaInitializationData& schema, const std::shared_ptr<NColumnShard::TPortionIndexStats>& counters)
...
void TColumnEngineForLogs::RegisterSchemaVersion(const TSnapshot& snapshot, const TSchemaInitializationData& schema) {
    ...
    if (schema.GetDiff()) {
        ...
        indexInfoOptional = NOlap::TIndexInfo::BuildFromProto(
            *schema.GetDiff(), VersionedIndex.GetLastSchema()->GetIndexInfo(), StoragesManager, SchemaObjectsCache, schema.IsStandalone());
    } else {
        indexInfoOptional = NOlap::TIndexInfo::BuildFromProto(schema.GetSchemaVerified(), StoragesManager, SchemaObjectsCache, schema.IsStandalone());
    }
    AFL_VERIFY(indexInfoOptional);
    RegisterSchemaVersion(snapshot, schema.GetPresetId(), std::move(*indexInfoOptional));
}
...
void TColumnEngineForLogs::RegisterOldSchemaVersion(const TSnapshot& snapshot, const TSchemaInitializationData& schema) {
    ...
    if (schema.GetDiff()) {
        ...
        indexInfoOptional =
            NOlap::TIndexInfo::BuildFromProto(*schema.GetDiff(), prevSchema->GetIndexInfo(), StoragesManager, SchemaObjectsCache, schema.IsStandalone());
    } else {
        indexInfoOptional = NOlap::TIndexInfo::BuildFromProto(schema.GetSchemaVerified(), StoragesManager, SchemaObjectsCache, schema.IsStandalone());
    }

    AFL_VERIFY(indexInfoOptional);
    VersionedIndex.AddIndex(snapshot, SchemaObjectsCache->UpsertIndexInfo(schema.GetPresetId(), std::move(*indexInfoOptional)));
}
