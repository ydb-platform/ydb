...
TPathInfo& TInsertionSummary::RegisterPathInfo(const TInternalPathId pathId) {
...
TPathInfo* TInsertionSummary::GetPathInfoOptional(const TInternalPathId pathId) {
    auto it = PathInfo.find(pathId);
    if (it == PathInfo.end()) {
        return nullptr;
    }
...
const TPathInfo* TInsertionSummary::GetPathInfoOptional(const TInternalPathId pathId) const {
    auto it = PathInfo.find(pathId);
    if (it == PathInfo.end()) {
        return nullptr;
    }
...
bool TInsertionSummary::IsOverloaded(const TInternalPathId pathId) const {
...
bool TInsertionSummary::EraseAborted(const TInsertWriteId writeId) {
    ...
    OnEraseAborted(it->second.GetSchemaVersion());
...
const TInsertedData* TInsertionSummary::AddAborted(TInsertedData&& data, const bool load /*= false*/) {
    ...
    OnNewAborted(data.GetSchemaVersion());
...
std::optional<TInsertedData> TInsertionSummary::ExtractInserted(const TInsertWriteId id) {
    ...
    if (result) {
        ...
        if (pathInfo) {
            OnEraseInserted(*pathInfo, result->BlobSize(), result->GetSchemaVersion());
        }
    }
    return result;
}
...
const TInsertedData* TInsertionSummary::AddInserted(TInsertedData&& data, const bool load /*= false*/) {
    ...
    OnNewInserted(GetPathInfoVerified(insertInfo->GetPathId()), insertInfo->BlobSize(), insertInfo->GetSchemaVersion(), load);
    return insertInfo;
}
