...
THashMap<TString, THashMap<TChunkAddress, std::shared_ptr<IPortionDataChunk>>> TPortionDataAccessor::RestoreEntityChunks(
    ...
    for (auto&& c : GetIndexesVerified()) {
        const TString& storageId = indexInfo.GetIndexStorageId(c.GetIndexId(), PortionInfo->GetTierNameDef(IStoragesManager::DefaultStorageId), PortionInfo->GetTieredIndexes());
...
THashMap<TChunkAddress, TString> TPortionDataAccessor::DecodeBlobAddressesImpl(
    ...
    for (auto&& record : GetIndexesVerified()) {
        ...
        std::optional<TString> blob =
            blobs.ExtractOptional(indexInfo.GetIndexStorageId(record.GetIndexId(), PortionInfo->GetTierNameDef(IStoragesManager::DefaultStorageId), PortionInfo->GetTieredIndexes()), RestoreBlobRange(record.GetBlobRangeVerified()));
        if (blob) {
            result.emplace(record.GetAddress(), std::move(*blob));
        }
    }
