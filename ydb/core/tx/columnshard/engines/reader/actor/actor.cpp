...
void TColumnShardScan::Bootstrap(const TActorContext& ctx) {
    ...
    std::shared_ptr<TReadContext> context =
        std::make_shared<TReadContext>(StoragesManager, DataAccessorsManager, ScanCountersPool, DuplicateFilteringCounters, ReadMetadataRange,
            SelfId(), ColumnShardActorId, ResourceSubscribeActorId, ReadCoordinatorActorId, ComputeShardingPolicy, ScanId, CPULimits);
