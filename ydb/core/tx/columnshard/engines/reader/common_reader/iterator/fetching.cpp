...
TConclusion<bool> TFetchingScriptCursor::Execute(const std::shared_ptr<IDataSource>& source) {
    ...
    while (!Script->IsFinished(CurrentStepIdx)) {
        ...
        const std::optional<TMemoryProfileGuard> mGuard = [&]() {
            if (IS_DEBUG_LOG_ENABLED(NKikimrServices::TX_COLUMNSHARD_SCAN_MEMORY)) {
                return TMemoryProfileGuard("SCAN_PROFILE::FETCHING::" + step->GetName() + "::" + Script->GetBranchName(),
                    IS_DEBUG_LOG_ENABLED(NKikimrServices::TX_COLUMNSHARD_SCAN_MEMORY));
            } else {
                return std::optional<TMemoryProfileGuard>();
            }
        }();
