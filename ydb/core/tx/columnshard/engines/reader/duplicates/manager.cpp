...
TDuplicateFilterConstructor::TSourceFilterConstructor::TSourceFilterConstructor(const std::shared_ptr<NSimple::TPortionDataSource>& source, const TSourceIntervals& intervals)
...
NArrow::NAccessor::IChunkedArray::TRowRange TDuplicateFilterConstructor::TSourceFilterConstructor::GetIntervalRange(const ui32 globalIntervalIdx) const {
...
TDuplicateFilterConstructor::TDuplicateFilterConstructor(
    const std::deque<NSimple::TSourceConstructor>& sources, const std::shared_ptr<NSimple::TSpecialReadContext>& context)
    ...
    for (const auto& source : sources) {
        WaitingSources.emplace(source.GetSourceId(), std::make_shared<TWaitingSourceInfo>(source.GetPortionIdx()));
    }
}
...
void TDuplicateFilterConstructor::StartAllocation(const ui64 sourceId, const std::shared_ptr<NSimple::IDataSource>& requester) {
    ...
    AFL_VERIFY(findWaiting)("source", sourceId);
    ...
    std::shared_ptr<TColumnFetchingContext> fetchingContext = std::make_shared<TColumnFetchingContext>(source->GetSourceId(), *findWaiting, SelfId(), source->GetContext());
...
void TDuplicateFilterConstructor::StartMergingColumns(const ui32 intervalIdx) const {
