...
TConclusionStatus TBaseMergeTask::PrepareResultBatch() {
    ...
    if (ResultBatch && ResultBatch->num_rows()) {
        ...
        YDB_LOG_DEBUG("",
            ...
            {"before_data", dataSizeBefore},
            {"before_memory", memorySizeBefore},
            {"after_memory", NArrow::GetTableMemorySize(ResultBatch)},
            {"after_data", NArrow::GetTableDataSize(ResultBatch)},
...
bool TBaseMergeTask::DoApply(IDataReader& indexedDataRead) {
    YDB_LOG_DEBUG("",
        {"event", "DoApply"},
        {"interval_idx", MergingContext->GetIntervalIdx()});
