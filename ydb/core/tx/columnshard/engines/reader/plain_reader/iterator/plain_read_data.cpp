...
TPlainReadData::TPlainReadData(const std::shared_ptr<TReadContext>& context)
    ...
    for (auto&& i : portions) {
        if (i->GetMeta().GetProduced() == NPortion::EProduced::COMPACTED ||
            i->GetMeta().GetProduced() == NPortion::EProduced::SPLIT_COMPACTED) {
    ...
    stats->SchemaColumns =
        (*SpecialReadContext->GetProgramInputColumns() - *SpecialReadContext->GetSpecColumns()).GetColumnsCount();
...
std::vector<std::shared_ptr<TPartialReadResult>>
TPlainReadData::DoExtractReadyResults(const int64_t /*maxRowsInBatch*/) {
...
//    auto result = TPartialReadResult::SplitResults(std::move(PartialResults), maxRowsInBatch);
    ...
    ("event", "DoExtractReadyResults")("result", result.size())("count", count)("finished", Scanner->IsFinished());
    return result;
}
