...
private:
    ...
    std::optional<NConveyorComposite::TProcessGuard> ConveyourProcessGuard;
...
public:
    ...
    void StartProcess(const ui64 processId) {
        AFL_VERIFY(!ConveyourProcessGuard);
        ConveyourProcessGuard.emplace(
            NConveyorComposite::TDeduplicationServiceOperator::StartProcess(processId, NConveyorComposite::TCPULimitsConfig()));
    }

    bool IsProcessStarted() const {
        return !!ConveyourProcessGuard;
    }

    ui64 GetConveyorProcessId() const {
        AFL_VERIFY(!!ConveyourProcessGuard);
        return ConveyourProcessGuard->GetInternalProcessId();
    }
