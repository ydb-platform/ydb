...
struct TInterval {
    ...
    TInterval(IntervalStorageType left_, IntervalStorageType right_)
        : left(left_)
        , right(right_) {
    }

    bool contains(IntervalStorageType point) const {
        return left <= point && point <= right;
    }

    bool intersects(TInterval interval) const {
        return left <= interval.right && right >= interval.left;
    }
};

template <typename IntervalStorageType>
auto operator<=>(const TInterval<IntervalStorageType> &lhs, const TInterval<IntervalStorageType> &rhs) {
    return std::tie(lhs.left, lhs.right) <=> std::tie(rhs.left, rhs.right);
}

template <typename IntervalStorageType>
bool operator==(const TInterval<IntervalStorageType> &lhs, const TInterval<IntervalStorageType> &rhs) {
    return std::tie(lhs.left, lhs.right) == std::tie(rhs.left, rhs.right);
}

struct TIntervalTreeVoidValue {};
...
public:
    ...
    static constexpr bool is_empty_value = std::is_same_v<Value, TIntervalTreeVoidValue>;

    TIntervalTree(const IntervalStorageType& defaultValue = {}) {
        nodes = {{0, 0, defaultValue}};
    }

    template <typename TValue = Value>
        requires std::is_same_v<Value, TIntervalTreeVoidValue>
    Y_FORCE_INLINE bool emplace(Interval interval) {
        Y_ABORT_UNLESS(!tree_is_built);
    ...
    template <typename TValue = Value, typename... Args>
        requires(!std::is_same_v<TValue, TIntervalTreeVoidValue>)
    Y_FORCE_INLINE bool emplace(Interval interval, Args &&...args) {
        Y_ABORT_UNLESS(!tree_is_built);
        if (Y_UNLIKELY(interval.left > interval.right)) {
            return false;
        }
    ...
    template <typename IntervalCallback>
    void find(Interval interval, IntervalCallback &&callback) const {
        if (Y_UNLIKELY(!tree_is_built)) {
            findIntervalsNonConstructedImpl(interval, callback);
            return;
        }
...
public:
    ...
    private:
        friend class TIntervalTree;

        Iterator(size_t node_index_, size_t current_interval_index_, const TIntervalTree *tree_)
...
private:
    ...
    template <typename IntervalCallback>
    void findIntervalsNonConstructedImpl(Interval interval, IntervalCallback &&callback) const {
        for (auto &interval_with_value : sorted_intervals) {
            auto &current_interval = getInterval(interval_with_value);

            if (interval.intersects(current_interval)) {
                callCallback(interval_with_value, callback);
            }
        }
    }
    ...
    size_t findFirstIteratorNodeIndex() const {
        ...
        if (Y_UNLIKELY(result_index == nodes_size)) {
            result_index = 0;
        }
...
template <typename IntervalType>
using IntervalSet = TIntervalTree<IntervalType, TIntervalTreeVoidValue>;

template <typename IntervalType, typename Value>
using IntervalMap = TIntervalTree<IntervalType, Value>;
