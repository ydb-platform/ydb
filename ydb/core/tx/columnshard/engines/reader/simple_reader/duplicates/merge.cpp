...
void TBuildDuplicateFilters::DoExecute(const std::shared_ptr<ITask>& /*taskPtr*/) {
    ...
    for (const auto& interval : Context.GetIntervals()) {
        for (auto&& [portionId, filter] : BuildFiltersOnInterval(interval, merger, columnData)) {
            AFL_VERIFY(filters.emplace(TDuplicateMapInfo(Context.GetGlobalContext().GetContext()->GetRequest()->Get()->GetMaxVersion(), TIntervalBordersView(interval.GetBegin().MakeView(), interval.GetEnd().MakeView()), portionId), std::move(filter)).second);
        }
    }

    TActivationContext::AsActorContext().Send(
        Context.GetGlobalContext().GetOwner(), new NPrivate::TEvFilterConstructionResult(std::move(filters)));
...
void TBuildDuplicateFilters::DoOnCannotExecute(const TString& reason) {
    TActivationContext::AsActorContext().Send(
        Context.GetGlobalContext().GetOwner(), new NPrivate::TEvFilterConstructionResult(TConclusionStatus::Fail(reason)));
}
...
THashMap<ui64, NArrow::TColumnFilter> TBuildDuplicateFilters::BuildFiltersOnInterval(const TIntervalInfo& interval,
    ...
    if (interval.GetIntersectingPortionsCount() == 1) {
        ...
        result.insert_or_assign(Context.GetGlobalContext().GetContext()->GetRequest()->Get()->GetSourceId(), std::move(filter));
