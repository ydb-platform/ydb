...
TConclusion<bool> TDetectInMem::DoExecuteInplace(const std::shared_ptr<IDataSource>& source, const TFetchingScriptCursor& /*step*/) const {
    ...
    const auto& context = *source->GetContext()->GetCommonContext();
    auto task = std::make_shared<TStepAction>(source, std::move(cursor), context.GetScanActorId(), false);
    NConveyor::TScanServiceOperator::SendTaskToExecute(task, context.GetResourcePoolKey());
    return false;
}
