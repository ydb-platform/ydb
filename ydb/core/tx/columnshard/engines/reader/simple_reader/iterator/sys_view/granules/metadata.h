...
private:
    ...
    EPathType GetPathType(const TString& tablePath) {
        if (tablePath.EndsWith("/.sys/primary_index_granule_stats")) {
            return EPathType::Table;
        } else if (tablePath.EndsWith("/.sys/store_primary_index_granule_stats")) {
