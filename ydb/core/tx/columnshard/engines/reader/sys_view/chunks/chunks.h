...
protected:
    ...
    BuildMetadata(const NColumnShard::TColumnShard* self, const TReadDescription& read) const override;

public:
    using TBase::TBase;
};

class TReadStatsMetadata
    : public NAbstract::TReadStatsMetadata
    , std::enable_shared_from_this<TReadStatsMetadata> {
private:
...
public:
    ...
    virtual std::unique_ptr<TScanIteratorBase> StartScan(const std::shared_ptr<TReadContext>& /*readContext*/)
        const override;
...
protected:
    ...
    DoCreateConstructor(const TSnapshot& snapshot, const ui64 itemsLimit, const bool reverse) const override {
