...
bool TStatsIterator::AppendStats(
    ...
    for (auto&& i : granule.GetOptimizerTasks()) {
        ...
        NArrow::Append<arrow::StringType>(
            *builders[5], arrow::util::string_view(i.GetStart().data(), i.GetStart().size())
        );
        NArrow::Append<arrow::StringType>(
            *builders[6], arrow::util::string_view(i.GetFinish().data(), i.GetFinish().size())
        );
        NArrow::Append<arrow::StringType>(
            *builders[7], arrow::util::string_view(i.GetDetails().data(), i.GetDetails().size())
        );
