...
void TTxScan::Complete(const TActorContext& ctx) {
    ...
    const bool useBatchPool = request.GetUseBatchPool();
    const ui32 scanPoolId = useBatchPool ? AppDataVerified().BatchPoolId : Max<ui32>();
    auto scanActorId = ctx.Register(
        new TColumnShardScan(Self->SelfId(), scanComputeActor, Self->ScanDiagnosticsActorId, Self->GetStoragesManager(),
            ...
            std::move(orbit), rawPathId, useBatchPool), TMailboxType::HTSwap, scanPoolId);
