...
bool TTxScan::GetDeduplicationEnabled(const TSnapshot& /*snapshot*/) const {
    return AppDataVerified().ColumnShardConfig.GetDeduplicationEnabled();
}
...
TConclusion<std::shared_ptr<ITableMetadataAccessor>> TTxScan::MakeTableAccessor(
    ...
    const auto& request = Ev->Get()->Record;
    Y_UNUSED(snapshot);
    return Self->TablesManager.BuildTableMetadataAccessor(request.GetTablePath() ? request.GetTablePath() : "undefined", ssPathId);
}
...
void TTxScan::StartScanActor(const TReadMetadataBase::TConstPtr& readMetadataRange, const ui64 rawPathId,
    ...
    auto scanActorId = ctx.Register(new TColumnShardScan(Self->SelfId(), Ev->Sender, Self->ScanDiagnosticsActorId, Self->GetStoragesManager(),
        Self->DataAccessorsManager.GetObjectPtrVerified(), Self->ColumnDataManager.GetObjectPtrVerified(), shardingPolicy, request.GetScanId(),
        request.GetTxId(), request.GetGeneration(), requestCookie, Self->TabletID(), TDuration::MilliSeconds(request.GetTimeoutMs()),
        readMetadataRange, request.GetDataFormat(), Self->Counters.GetScanCounters(), cpuLimits, std::move(orbit), rawPathId));
