...
public:
    TSourceData(const ui32 sourceIdx, const ui64 tabletId, std::vector<std::shared_ptr<const TGranuleMeta>>&& granules,
        std::vector<NColumnShard::TSchemeShardLocalPathId>&& externalPathIds, std::vector<ui32>&& portionsCount, NArrow::TTrivialRow&& start,
        NArrow::TTrivialRow&& finish, const std::shared_ptr<NReader::NCommon::TSpecialReadContext>& context)
