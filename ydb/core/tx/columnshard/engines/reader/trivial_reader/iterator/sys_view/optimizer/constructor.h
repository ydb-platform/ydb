...
#include <ydb/core/tx/columnshard/engines/reader/trivial_reader/iterator/sys_view/abstract/constructor.h>
...
public:
    TDataSourceConstructor(
        ...
        : TBase(tabletId, TSchemaAdapter::GetPKTrivialRow(externalPathId, tabletId, 0),
              TSchemaAdapter::GetPKTrivialRow(externalPathId, tabletId, Max<ui64>()))
...
public:
    TConstructor(const IPathIdTranslator& translator, const NColumnShard::TUnifiedOptionalPathId& unifiedPathId, const IColumnEngine& engine,
        const ui64 tabletId, const std::shared_ptr<NOlap::TPKRangesFilter>& pkFilter, const EPortionsSorting portionsSorting);
};
}   // namespace NKikimr::NOlap::NReader::NTrivial::NSysView::NOptimizer
