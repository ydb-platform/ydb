...
TSimpleColumnInfo::TSimpleColumnInfo(
    const ui32 columnId,
...
std::vector<std::shared_ptr<NKikimr::NOlap::IPortionDataChunk>> TSimpleColumnInfo::ActualizeColumnData(
    ...
    const auto checkNeedActualize = [&]() {
        if (!Serializer.IsEqualTo(sourceColumnFeatures.Serializer)) {
            ...
            ("event",
             "actualization")("reason", "serializer")("from", sourceColumnFeatures.Serializer.SerializeToProto().DebugString())(
        ...
        if (!!DictionaryEncoding != !!sourceColumnFeatures.DictionaryEncoding) {
            AFL_NOTICE(NKikimrServices::TX_COLUMNSHARD)
            ("event", "actualization")("reason", "dictionary")("from", !!sourceColumnFeatures.DictionaryEncoding)(
                "to", !!DictionaryEncoding
            );
            return true;
        }
    ...
    for (auto&& s : source) {
        ...
        if (!DataAccessorConstructor.IsEqualTo(sourceColumnFeatures.DataAccessorConstructor)) {
            ...
            data = DataAccessorConstructor.Construct(
                chunkedArray, Loader->BuildAccessorContext(s->GetRecordsCountVerified())
            );
        } else {
