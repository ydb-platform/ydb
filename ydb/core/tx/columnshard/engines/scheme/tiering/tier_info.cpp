...
std::optional<TInstant> TTierInfo::ScalarToInstant(const std::shared_ptr<arrow20::Scalar>& scalar) const {
    const ui64 unitsInSeconds = TtlUnitsInSecond ? TtlUnitsInSecond : 1;
    switch (scalar->type->id()) {
        case arrow20::Type::TIMESTAMP:
            return TInstant::MicroSeconds(std::static_pointer_cast<arrow20::TimestampScalar>(scalar)->value);
        case arrow20::Type::UINT16: // YQL Date
            return TInstant::Days(std::static_pointer_cast<arrow20::UInt16Scalar>(scalar)->value);
        case arrow20::Type::UINT32: // YQL Datetime or Uint32
            return TInstant::MicroSeconds(std::static_pointer_cast<arrow20::UInt32Scalar>(scalar)->value / (1.0 * unitsInSeconds / 1000000));
        case arrow20::Type::UINT64:
            return TInstant::MicroSeconds(std::static_pointer_cast<arrow20::UInt64Scalar>(scalar)->value / (1.0 * unitsInSeconds / 1000000));
        default:
            return {};
    }
}

TTiering::TTieringContext TTiering::GetTierToMove(const std::shared_ptr<arrow20::Scalar>& max, const TInstant now, const bool skipEviction) const {
