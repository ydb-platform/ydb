...
private:
    ...
    void RegisterRequestId(const IDataAccessorsRequest& request) {
        AFL_VERIFY(!Started);
        if (!request.IsFetched()) {
            AFL_VERIFY(RequestIds.emplace(requestId).second);
        }
    }

    friend class IDataAccessorsRequest;
...
public:
    ...
    virtual ~IDataAccessorRequestsSubscriber() {
        AFL_VERIFY(Started);
    }
};
...
public:
    void Merge(TPortionDataAccessor&& result) {
        for (auto&& i : result.ErrorsByPathId) {
            AFL_VERIFY(ErrorsByPathId.emplace(i.first, i.second));
        }
        for (auto&& i : result.AccessorsByPathId) {
            AFL_VERIFY(AccessorsByPathId.emplace(i.first, std::move(i.second)));
        }
        for (auto&& i : result.PortionsById) {
            AFL_VERIFY(PortionsById.emplace(i.first, std::move(i.second)));
        }
    ...
    void AddData(const ui64 pathId, TConclusion<std::vector<TPortionDataAccessor>>&& accessors) {
        if (accessors.IsFail()) {
            ...
            auto info = AccessorsByPathId.emplace(accessors.DetachResult());
            AFL_VERIFY(info.second);
            for (auto&& i : info.first->second) {
                AFL_VERIFY(PortionsById.emplace(i.GetPortionInfo(), i).second);
...
public:
    ...
    void AddPortion(const TPortionInfo::TConstPtr& portion) {
        AFL_VERIFY(portion);
        AFL_VERIFY(FetchStage <= 1);
        AFL_VERIFY(Portions.emplace(portion->GetPortionId(), portion));
        auto it = PathIdStatus.find(portion->GetPathId());
        if (it == PortionsByPathId.end()) {
            PreparingCount.Inc();
    ...
    void AddData(const ui64 pathId, TConclusion<std::vector<TPortionDataAccessor>>&& accessors) {
        AFL_VERIFY(FetchStage == 1);
        {
            auto itPortions = PortionsByPathId.find(pathId);
            AFL_VERIFY(itPortions != PortionsByPathId.end());
            ...
            if (accessors.IsFail()) {
                ...
                for (auto&& p : itPortions->second) {
                    AFL_VERIFY(Portions.erase(p->GetPortionId()));
                }
            } else {
                AFL_VERIFY(itPortions->size() == accessors->size());
                itStatus->second.OnFetched();
                for (auto&& acc : *accessors) {
                    AFL_VERIFY(Portions.erase(acc.GetPortionInfo().GetPortionId()));
                }
            }
            PortionsByPathId.erase(itPortions);
            PathIdStatus.erase(itStatus);
            AFL_VERIFY(FetchingCount.Dec() >= 0);
            ReadyCount.Inc();
            AccessorsByPathId.AddData(pathId, std::move(accessors));
        }
        AFL_VERIFY(PortionsByPathId.empty() == Portions.empty());
        if (PortionsByPathId.empty()) {
            AFL_VERIFY(!PreparingCount.Val());
            AFL_VERIFY(!FetchingCount.Val());
            FetchStage = 2;
            Subscriber->OnResult(AccessorsByPathId);
        }
    }
...
class TTrivialAccessorsFetcher: public IDataAccessorsRequest {
private:
    THashMap<ui64, TConclusion<std::vector<TPortionDataAccessor>>> Accessors;
    virtual void DoOnDataReceived(const THashMap<ui64, TConclusion<std::vector<TPortionDataAccessor>>>& accessors) override {
        Accessors = accessors;
    }

public:
    const THashMap<ui64, TConclusion<std::vector<TPortionDataAccessor>>>& GetAccessors() const {
        AFL_VERIFY(IsFetched())("stage", FetchStage);
        return Accessors;
    }
};
...
public:
    ...
    void AskData(const std::shared_ptr<IDataAccessorsRequest>& request) {
        AFL_VERIFY(request);
...
private:
    ...
    std::shared_ptr<IGranuleDataAccessor> DataAccessor = std::make_shared<TMemDataAccessor>();
