...
std::shared_ptr<arrow::Scalar> TIndexMeta::GetMaxScalarVerified(
    ...
    std::shared_ptr<arrow::Scalar> result;
    for (auto&& d : data) {
        std::shared_ptr<arrow::Scalar> current =
            NArrow::NScalar::TSerializer::DeserializeFromStringWithPayload(d, dataType).DetachResult();
...
NJson::TJsonValue TIndexMeta::DoSerializeDataToJson(const TString& data, const TIndexInfo& indexInfo) const {
    ...
    auto scalar =
        GetMaxScalarVerified({data}, indexInfo.GetColumnFeaturesVerified(*ColumnIds.begin()).GetArrowField()->type());
