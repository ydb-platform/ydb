...
inline NArrow::NAccessor::TMinMax Deserialize(TStringBuf data, const std::shared_ptr<arrow::DataType>& type) {
    ...
    auto readNext = [&] {
        ...
        AFL_VERIFY(offset <= data.size() && sizeof(TSizeType) <= data.size() - offset )("details",
                                            Sprintf("out of bounds read, data.size(): %i, current_offset: %i, read size: %i", data.size(), offset, sizeof(TSizeType)));
        memcpy(&size, data.data() + offset, sizeof(size));
        offset += sizeof(TSizeType);
        AFL_VERIFY(offset <= data.size() && size <= data.size() - offset)("details", Sprintf("out of bounds read, data.size(): %i, current_offset: %i, read size: %i", data.size(), offset, size));
        auto res =
    ...
    bool TIndexMeta::DoCheckValue(const TString& data, [[maybe_unused]] const std::optional<ui64> cat,
        ...
        NArrow::NAccessor::TMinMax chunkValue = NSerializePair::Deserialize(data, info.GetColumnFeaturesVerified(GetColumnId()).GetArrowField()->type());
        return !Skip(chunkValue, requestValue, op);
    }

    bool TIndexMeta::Skip(NArrow::NAccessor::TMinMax chunkValue, const std::shared_ptr<arrow::Scalar>& requestValue, const NArrow::NSSA::TIndexCheckOperation& op) const {
        switch (op.GetOperation()) {
            case NArrow::NSSA::TIndexCheckOperation::EOperation::Equals:
