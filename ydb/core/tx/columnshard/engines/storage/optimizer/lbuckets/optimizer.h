...
//        }
    ...
    std::vector<std::shared_ptr<TPortionInfo>> GetOptimizerTaskPortions(const ui64 sizeLimit, std::optional<NArrow::TReplaceKey>& separatePoint) const {
        ...
        ui64 currentSize = 0;
        for (auto&& i : sorted) {
            if (currentSize + i->GetRawBytes() > sizeLimit && result.size() > 1) {
                break;
            }
...
public:
    ...
    void RemoveOther(const std::shared_ptr<TPortionInfo>& portion) {
        auto gChartsThis = StartModificationGuard();
        AFL_VERIFY(Others.Remove(portion))("portion", portion->DebugString(true))("bucket_start", MainPortion ? MainPortion->DebugString(true) : "-inf")("bucket_finish", NextBorder ? NextBorder->DebugString() : "undef");
    }
