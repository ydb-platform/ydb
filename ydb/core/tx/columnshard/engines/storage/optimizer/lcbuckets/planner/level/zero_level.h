...
private:
    ...
    const bool SkipGoodPortionCompaction;

    std::set<TOrderedPortion> Portions;

    bool IsGoodPortionForSkipCompaction(const TPortionInfo::TConstPtr& portion) const;
...
public:
    TZeroLevelPortions(const ui32 levelIdx, const std::shared_ptr<IPortionsLevel>& nextLevel, const TLevelCounters& levelCounters,
        ...
        const ui64 highPriorityContribution = 0, bool compactAtLevel = true, bool skipGoodPortionCompaction = false);
};
