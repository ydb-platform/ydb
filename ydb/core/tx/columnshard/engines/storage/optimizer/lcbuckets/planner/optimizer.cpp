...
TOptimizerPlanner::TOptimizerPlanner(const TInternalPathId pathId, const std::shared_ptr<IStoragesManager>& storagesManager,
    ...
    std::vector<std::shared_ptr<IPortionsSelector>>&& selectors, const ui32 nodePortionsCountLimit, const ui32 weightKff)
    : TBase(pathId, nodePortionsCountLimit, weightKff)
    , Counters(counters)
