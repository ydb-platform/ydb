...
protected:
    ...
    virtual void DoModifyPortions(const THashMap<ui64, std::shared_ptr<TPortionInfo>>& add, const THashMap<ui64, std::shared_ptr<TPortionInfo>>& remove) override {    const TInstant currentInstant = TInstant::Now();
        for (auto&& [_, i] : remove) {
