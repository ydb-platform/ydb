...
void TWriteController::DoOnReadyResult(
...
TWriteController::TWriteController(
    ...
    for (auto&& i : blobsToWrite) {
        ...
        AFL_VERIFY(
            (((ui64)blobId.GetLogoBlobId().Step() >> 8) << 40) + ((ui64)blobId.GetLogoBlobId().Generation() << 8) +
                blobId.GetLogoBlobId().Channel() ==
            pathId
        );
