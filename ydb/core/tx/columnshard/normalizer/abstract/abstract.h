...
public:
    ...
    private:
        ...
        DoInit(const TNormalizationController& controller, NTabletFlatExecutor::TTransactionContext& txc) = 0;
...
public:
    ...
    TString DebugString() const {
        return TStringBuilder() << "normalizers_count=" << Normalizers.size() << ";current_normalizer="
                                << (Normalizers.size() ? Normalizers.front()->DebugString() : "NO_DATA");
    }
