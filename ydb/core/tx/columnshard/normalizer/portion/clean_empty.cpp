    ...
    virtual void Apply(NIceDb::TNiceDb& db) override {
        for (auto&& i : Chunks) {
            ...
            db.Table<NColumnShard::Schema::IndexColumnsV1>()
                .Key(PortionAddress.GetPathId().GetRawInternalPathIdValue(), PortionAddress.GetPortionId(), i.GetColumnId(), i.GetChunkIdx())
                .Delete();
        }
    }
...
private:
    ...
    virtual void Apply(NIceDb::TNiceDb& db) override {
        ...
        db.Table<NColumnShard::Schema::IndexColumnsV2>().Key(PortionAddress.GetPathId().GetRawInternalPathIdValue(), PortionAddress.GetPortionId()).Delete();
    }
...
private:
    ...
    virtual void Apply(NIceDb::TNiceDb& db) override {
        AFL_CRIT(NKikimrServices::TX_COLUMNSHARD)("event", "remove_portion")("path_id", PortionAddress.GetPathId())(
            "portion_id", PortionAddress.GetPortionId());
        db.Table<NColumnShard::Schema::IndexPortions>().Key(PortionAddress.GetPathId().GetRawInternalPathIdValue(), PortionAddress.GetPortionId()).Delete();
    }
...
bool GetColumnPortionAddresses(NTabletFlatExecutor::TTransactionContext& txc, std::map<TPortionAddress, std::shared_ptr<IDBModifier>>& resultV0,
    ...
    }
    {
        ...
        while (!rowset.EndOfSet()) {
            TPortionAddress address(TInternalPathId::FromRawInternalPathIdValue(rowset.GetValue<Schema::IndexColumnsV1::PathId>()), rowset.GetValue<Schema::IndexColumnsV1::PortionId>());
    ...
    }
    {
        ...
        while (!rowset.EndOfSet()) {
            TPortionAddress portionAddress(
                TInternalPathId::FromRawInternalPathIdValue(rowset.GetValue<Schema::IndexColumnsV2::PathId>()), rowset.GetValue<Schema::IndexColumnsV2::PortionId>());
