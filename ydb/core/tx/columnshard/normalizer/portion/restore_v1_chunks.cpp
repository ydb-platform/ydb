        ...
        for (auto&& i : Patches) {
            ...
            for (auto&& [_, c] : i.GetChunksInfo()) {
                db.Table<IndexColumnsV1>()
                    .Key(c.GetPathId().GetRawInternalPathIdValue(), c.GetPortionId(), c.GetAddress().GetColumnId(), c.GetAddress().GetChunkIdx())
...
public:
    ...
    virtual bool ApplyOnExecute(NTabletFlatExecutor::TTransactionContext& txc, const TNormalizationController&) const override {
        ...
        for (auto&& i : Patches) {
            db.Table<IndexColumnsV1>()
                .Key(i.GetChunkInfo().GetPathId().GetRawInternalPathIdValue(), i.GetChunkInfo().GetPortionId(), i.GetChunkInfo().GetAddress().GetEntityId(),
