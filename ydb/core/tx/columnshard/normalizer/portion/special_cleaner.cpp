    ...
    ApplyOnExecute(NTabletFlatExecutor::TTransactionContext& txc, const TNormalizationController& /*normController*/)
        ...
        for (const auto& k : Keys) {
            db.Table<Schema::IndexColumns>()
                .Key(k.Index, k.Granule, k.ColumnIdx, k.PlanStep, k.TxId, k.Portion, k.Chunk)
                .Delete();
        }
        AFL_NOTICE(NKikimrServices::TX_COLUMNSHARD)
        ("normalizer", "TDeleteTrash")("message", TStringBuilder() << GetSize() << " rows deleted");
        return true;
    }
...
TConclusion<std::vector<INormalizerTask::TPtr>> TDeleteTrashImpl::DoInit(
    ...
    ("normalizer", "TDeleteTrash")(
        "message",
        TStringBuilder() << "found " << removeCount << " rows to delete grouped in " << keysToDelete->size()
                         << " batches"
    );
...
std::optional<std::vector<TDeleteTrashImpl::TKeyBatch>>
TDeleteTrashImpl::KeysToDelete(NTabletFlatExecutor::TTransactionContext& txc, const size_t maxBatchSize) {
    NIceDb::TNiceDb db(txc.DB);
    using namespace NColumnShard;
    ...
    auto rowset = db.Table<Schema::IndexColumns>()
                      .Select<
