...
void TBuildBatchesTask::ReplyError(
    const TString& message,
    const NColumnShard::TEvPrivate::TEvWriteBlobsResult::EErrorClass errorClass
) {
    AFL_ERROR(NKikimrServices::TX_COLUMNSHARD)
    ("problem",
     "cannot build batch for insert")("reason", message)("data", WriteData.GetWriteMeta().GetLongTxIdOptional());
...
TConclusionStatus TBuildBatchesTask::DoExecute(const std::shared_ptr<ITask>& /*taskPtr*/) {
    ...
    if (batchConclusion.IsFail()) {
        ReplyError(
            "cannot extract incoming batch: " + batchConclusion.GetErrorMessage(),
    ...
    auto preparedConclusion = Context.GetActualSchema()->PrepareForModification(
        batchConclusion.DetachResult(), WriteData.GetWriteMeta().GetModificationType()
    );
    if (preparedConclusion.IsFail()) {
        ReplyError(
            "cannot prepare incoming batch: " + preparedConclusion.GetErrorMessage(),
    ...
    switch (WriteData.GetWriteMeta().GetModificationType()) {
        case NEvWrite::EModificationType::Upsert: {
            const std::vector<std::shared_ptr<arrow::Field>> defaultFields =
                Context.GetActualSchema()->GetAbsentFields(batch->schema());
            if (defaultFields.empty()) {
                ...
                auto conclusion = Context.GetActualSchema()->BuildDefaultBatch(
                    Context.GetActualSchema()->GetIndexInfo().ArrowSchema()->fields(), 1, true
                );
                ...
                merger = std::make_shared<TUpdateMerger>(
                    ...
                    insertionConclusion.IsSuccess() ? "" : insertionConclusion.GetErrorMessage(),
                    pos
                );
                break;
            }
    ...
    std::shared_ptr<NDataReader::IRestoreTask> task = std::make_shared<NOlap::TModificationRestoreTask>(
        BufferActorId, std::move(WriteData), merger, ActualSnapshot, batch, Context
    );
