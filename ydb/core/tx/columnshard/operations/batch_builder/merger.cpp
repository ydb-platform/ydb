...
NKikimr::NOlap::IMerger::TYdbConclusionStatus IMerger::AddExistsDataOrdered(const std::shared_ptr<arrow20::Table>& data) {
    AFL_VERIFY(data);
...
TUpdateMerger::TUpdateMerger(const NArrow::TContainerWithIndexes<arrow20::RecordBatch>& incoming,
    ...
    for (auto&& f : actualSchema->GetIndexInfo().ArrowSchema()) {
        ...
        if (fIdx == -1) {
            ...
            std::shared_ptr<arrow20::Array> flagsArray;
            if (fExistsIdx != -1) {
                AFL_VERIFY(IncomingData->column(fExistsIdx)->type_id() == arrow20::Type::BOOL);
                flagsArray = IncomingData->column(fExistsIdx);
            } else {
                flagsArray = NArrow::TThreadSimpleArraysCache::GetConst(arrow20::TypeTraits<arrow20::BooleanType>::type_singleton(),
                    std::make_shared<arrow20::BooleanScalar>(true), IncomingData->num_rows());
            }
            HasIncomingDataFlags.emplace_back(static_pointer_cast<arrow20::BooleanArray>(flagsArray));
            IncomingColumnRemap.emplace_back(fIdx);
        }
    }
}

NArrow::TContainerWithIndexes<arrow20::RecordBatch> TUpdateMerger::BuildResultBatch() {
    ...
    return NArrow::TContainerWithIndexes<arrow20::RecordBatch>(resultBatch);
}
