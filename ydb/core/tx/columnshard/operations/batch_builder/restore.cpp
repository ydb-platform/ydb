...
std::unique_ptr<NKikimr::TEvColumnShard::TEvInternalScan> TModificationRestoreTask::DoBuildRequestInitiator() const {
    auto request =
        std::make_unique<TEvColumnShard::TEvInternalScan>(LocalPathId, WriteData.GetWriteMeta().GetLockIdOptional());
    request->ReadToSnapshot = Snapshot;
    auto pkData =
        NArrow::TColumnOperator().VerifyIfAbsent().Extract(IncomingData, Context.GetActualSchema()->GetPKColumnNames());
...
NKikimr::TConclusionStatus TModificationRestoreTask::DoOnDataChunk(const std::shared_ptr<arrow::Table>& data) {
    ...
    if (result.IsFail()) {
        ...
        ("event", "merge_data_problems")("write_id", WriteData.GetWriteMeta().GetWriteId())("tablet_id", GetTabletId())(
...
void TModificationRestoreTask::DoOnError(const TString& errorMessage) {
    ...
    ("event", "restore_data_problems")("write_id", WriteData.GetWriteMeta().GetWriteId())("tablet_id", GetTabletId())(
...
void TModificationRestoreTask::SendErrorMessage(
    ...
    auto evResult = NColumnShard::TEvPrivate::TEvWriteBlobsResult::Error(
        NKikimrProto::EReplyStatus::CORRUPTED, std::move(buffer), errorMessage, errorClass
    );
