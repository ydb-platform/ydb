...
std::optional<std::vector<NKikimr::NArrow::TSerializedBatch>> TBuildSlicesTask::BuildSlices() {
    ...
    if (result.size() > 1) {
        for (auto&& i : result) {
            ...
            ("event", "strange_blobs_splitting")("blob", i.DebugString())("original_size", WriteData.GetSize());
        }
    }
    return result;
}

void TBuildSlicesTask::ReplyError(
...
TConclusionStatus TBuildSlicesTask::DoExecute(const std::shared_ptr<ITask>& /*taskPtr*/) {
    NActors::TLogContextGuard g(
        NActors::TLogContextBuilder::Build(NKikimrServices::TX_COLUMNSHARD)("tablet_id", TabletId)(
            "parent_id", Context.GetTabletActorId()
        )
    );
    if (!OriginalBatch) {
        AFL_INFO(NKikimrServices::TX_COLUMNSHARD)
        ("event", "ev_write_bad_data")("write_id", WriteData.GetWriteMeta().GetWriteId())(
    ...
    if (WriteData.GetWritePortions()) {
        if (OriginalBatch->num_rows() == 0) {
            ...
            std::vector<NColumnShard::TFailedWrite> fails = {
                NColumnShard::TFailedWrite(WriteData.GetWriteMeta(), WriteData.GetSize())
            };
            auto result = std::make_unique<NColumnShard::NPrivateEvents::NWrite::TEvWritePortionResult>(
            ...
            auto batches = NArrow::NMerger::TRWSortableBatchPosition::SplitByBordersInIntervalPositions(
                OriginalBatch,
                Context.GetActualSchema()->GetIndexInfo().GetPrimaryKey()->field_names(),
                WriteData.GetData()->GetSeparationPoints()
            );
            ...
            for (auto&& batch : batches) {
                ...
                auto portionConclusion = Context.GetActualSchema()->PrepareForWrite(
                    ...
                    WriteData.GetWriteMeta().GetTableId(),
                    batch,
                    WriteData.GetWriteMeta().GetModificationType(),
                    Context.GetStoragesManager(),
                    Context.GetSplitterCounters()
                );
                if (portionConclusion.IsFail()) {
                    ReplyError(
                        portionConclusion.GetErrorMessage(),
                ...
                std::shared_ptr<arrow::RecordBatch> pkBatch = NArrow::TColumnOperator().Extract(
                    batch, Context.GetActualSchema()->GetIndexInfo().GetPrimaryKey()->fields()
                );
            ...
            auto writeController = std::make_shared<NOlap::TPortionWriteController>(
                Context.GetTabletActorId(),
                WriteData.GetBlobsAction(),
                WriteData.GetWriteMeta(),
                std::move(portions),
                WriteData.GetSize()
            );
            if (WriteData.GetBlobsAction()->NeedDraftTransaction()) {
                TActorContext::AsActorContext().Send(
                    Context.GetTabletActorId(),
                    std::make_unique<NColumnShard::TEvPrivate::TEvWriteDraft>(writeController)
                );
            } else {
        ...
        auto subsetConclusion =
            NArrow::TColumnOperator().IgnoreOnDifferentFieldTypes().BuildSequentialSubset(OriginalBatch, indexSchema);
        if (subsetConclusion.IsFail()) {
            AFL_ERROR(NKikimrServices::TX_COLUMNSHARD)
            ("event",
             "unadaptable schemas")("index", indexSchema->ToString())("problem", subsetConclusion.GetErrorMessage());
            ReplyError(
        ...
        if (OriginalBatch->num_columns() != indexSchema->num_fields()) {
            AFL_VERIFY(OriginalBatch->num_columns() < indexSchema->num_fields())
            ("original", OriginalBatch->num_columns())("index", indexSchema->num_fields());
            if (HasAppData() && !AppDataVerified().FeatureFlags.GetEnableOptionalColumnsInColumnShard() &&
                ...
                const std::vector<ui32>& columnIdsVector =
                    Context.GetActualSchema()->GetIndexInfo().GetColumnIds(false);
                const std::set<ui32> columnIdsSet(columnIdsVector.begin(), columnIdsVector.end());
                auto normalized = Context.GetActualSchema()
                                      ->NormalizeBatch(
                                          *Context.GetActualSchema(),
                                          std::make_shared<NArrow::TGeneralContainer>(OriginalBatch),
                                          columnIdsSet
                                      )
                                      .DetachResult();
        ...
        if (batches) {
            ...
            std::shared_ptr<arrow::RecordBatch> pkBatch = NArrow::TColumnOperator().Extract(
                OriginalBatch, Context.GetActualSchema()->GetIndexInfo().GetPrimaryKey()->fields()
            );
            auto result = std::make_unique<NColumnShard::NWriting::TEvAddInsertedDataToBuffer>(
                writeDataPtr, std::move(*batches), pkBatch
            );
            TActorContext::AsActorContext().Send(BufferActorId, result.release());
        } else {
            ReplyError(
                "Cannot slice input to batches", NColumnShard::TEvPrivate::TEvWriteBlobsResult::EErrorClass::Internal
            );
