...
void TActor::DoReplyAllocated(const ui64 internalTaskId, const ui64 rbTaskId) {
    ...
    if (Aborted) {
        ...
        ("event", "result_resources_on_abort")("task_id", rbTaskId)("task", task->DebugString());
        ...
        AFL_DEBUG(NKikimrServices::TX_COLUMNSHARD)
        ("event", "result_resources")("task_id", rbTaskId)("task", task->DebugString());
...
void TActor::Handle(TEvStartTask::TPtr& ev) {
    ...
    if (!task->GetCPUAllocation() && !task->GetMemoryAllocation()) {
        ...
        Send(
            NKikimr::NResourceBroker::MakeResourceBrokerID(),
            new NKikimr::NResourceBroker::TEvResourceBroker::TEvSubmitTask(
