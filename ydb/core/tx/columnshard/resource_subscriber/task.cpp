...
void ITask::OnAllocationSuccess(const ui64 taskId, const NActors::TActorId& senderId) {
    ...
    ("event", "resource_allocated")("external_task_id", ExternalTaskId)("mem", MemoryAllocation)("cpu", CPUAllocation);
...
TResourcesGuard::~TResourcesGuard() {
    ...
    AFL_DEBUG(NKikimrServices::TX_COLUMNSHARD)
    ("event", "free_resources")("task_id", TaskId)("external_task_id", ExternalTaskId)("mem", Memory)("cpu", Cpu);
    if (TaskId) {
        auto ev = std::make_unique<IEventHandle>(
            NKikimr::NResourceBroker::MakeResourceBrokerID(),
            Sender,
            new NKikimr::NResourceBroker::TEvResourceBroker::TEvFinishTask(TaskId)
        );
...
TResourcesGuard::TResourcesGuard(
    ...
    AFL_DEBUG(NKikimrServices::TX_COLUMNSHARD)
    ("event", "allocate_resources")("external_task_id", ExternalTaskId)("task_id", TaskId)("mem", Memory)("cpu", Cpu);
}
...
void TResourcesGuard::Update(const ui64 memNew) {
    ...
    AFL_DEBUG(NKikimrServices::TX_COLUMNSHARD)
    ("event", "update_resources")("task_id", TaskId)("external_task_id", ExternalTaskId)("mem", memNew)("cpu", Cpu)(
        "mem_old", Memory
    );
    Memory = memNew;
    auto ev = std::make_unique<IEventHandle>(
        NKikimr::NResourceBroker::MakeResourceBrokerID(),
        Sender,
        new NKikimr::NResourceBroker::TEvResourceBroker::TEvUpdateTask(
            TaskId, {{Cpu, Memory}}, Context.GetTypeName(), Priority
        )
    );
    NActors::TActorContext::AsActorContext().Send(std::move(ev));
    Context.GetCounters()->GetBytesAllocated()->Add(Memory);
