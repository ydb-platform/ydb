...
Y_UNIT_TEST_SUITE(Splitter) {
    ...
    public:
        virtual std::optional<NKikimr::NArrow::NSplitter::TColumnSerializationStat>
        GetColumnSerializationStats(const ui32 /*columnId*/) const override {
            return {};
        }
        virtual std::optional<NKikimr::NArrow::NSplitter::TBatchSerializationStat>
        GetBatchSerializationStats(const std::shared_ptr<arrow::RecordBatch>& /*rb*/) const override {
            return {};
        }

        NKikimr::NArrow::NAccessor::TColumnLoader GetColumnLoader(const ui32 columnId) const {
            return NKikimr::NArrow::NAccessor::TColumnLoader(
                nullptr,
                NSerialization::TSerializerContainer::GetDefaultSerializer(),
                NKikimr::NArrow::NAccessor::TConstructorContainer::GetDefaultConstructor(),
                GetField(columnId),
                nullptr,
                columnId
            );
    ...
    public:
        void Execute(
            ...
            {
                auto slices =
                    TBatchSerializedSlice::BuildSimpleSlices(batch, settings, counters.SplitterCounters, Schema);
                for (auto&& i : slices) {
            ...
            for (auto&& i : packs) {
                ...
                AFL_VERIFY(portionSize >= settings.GetExpectedPortionSize() || packs.size() == 1)
                ("size", portionSize)("limit", settings.GetMaxPortionSize());
                ...
                AFL_VERIFY(pagesRestore == pagesOriginal || batch->num_columns() == 1)
                ("restore", pagesRestore)("original", pagesOriginal);
            ...
            AFL_VERIFY(blobsCount == ExpectBlobsCount.value_or(blobsCount))
            ("blobs_count", blobsCount)("expected", *ExpectBlobsCount);
            AFL_VERIFY(pagesSum == ExpectSlicesCount.value_or(pagesSum))
            ("sum", pagesSum)("expected", *ExpectSlicesCount);
            AFL_VERIFY(portionsCount == ExpectPortionsCount.value_or(portionsCount))
            ("portions_count", portionsCount)("expected", *ExpectPortionsCount);
            AFL_VERIFY(chunksCount == ExpectChunksCount.value_or(chunksCount))
            ("chunks_count", chunksCount)("expected", *ExpectChunksCount);
        }
    };

    Y_UNIT_TEST(Simple) {
        NConstruction::IArrayBuilder::TPtr column = std::make_shared<
            NKikimr::NArrow::NConstruction::TSimpleArrayConstructor<NKikimr::NArrow::NConstruction::TStringPoolFiller>>(
            "field", NKikimr::NArrow::NConstruction::TStringPoolFiller(8, 512)
        );
        std::shared_ptr<arrow::RecordBatch> batch =
            NKikimr::NArrow::NConstruction::TRecordBatchConstructor({column}).BuildBatch(80048);
        NKikimr::NColumnShard::TIndexationCounters counters("test");
    ...
    Y_UNIT_TEST(Minimal) {
        ...
        std::shared_ptr<arrow::RecordBatch> batch =
            NKikimr::NArrow::NConstruction::TRecordBatchConstructor({column}).BuildBatch(4048);
        NKikimr::NColumnShard::TIndexationCounters counters("test");
    ...
    Y_UNIT_TEST(Trivial) {
        ...
        std::shared_ptr<arrow::RecordBatch> batch =
            NKikimr::NArrow::NConstruction::TRecordBatchConstructor({column}).BuildBatch(10048);

        TSplitTester().SetExpectBlobsCount(1).SetExpectSlicesCount(1).Execute(batch);
    }

    Y_UNIT_TEST(BigAndSmall) {
        NConstruction::IArrayBuilder::TPtr columnBig = std::make_shared<
            NKikimr::NArrow::NConstruction::TSimpleArrayConstructor<NKikimr::NArrow::NConstruction::TStringPoolFiller>>(
            "field1", NKikimr::NArrow::NConstruction::TStringPoolFiller(8, 512)
        );
        NConstruction::IArrayBuilder::TPtr columnSmall = std::make_shared<
            NKikimr::NArrow::NConstruction::TSimpleArrayConstructor<NKikimr::NArrow::NConstruction::TStringPoolFiller>>(
    ...
    Y_UNIT_TEST(CritSmallPortions) {
        ...
        TSplitTester()
            .SetExpectBlobsCount(80)
            .SetExpectSlicesCount(80)
            .SetExpectedInternalSplitsCount(0)
            .SetExpectPortionsCount(40)
            .Execute(
    ...
    Y_UNIT_TEST(Crit) {
        ...
        TSplitTester()
            .SetExpectBlobsCount(80)
            .SetExpectSlicesCount(8)
            .SetExpectedInternalSplitsCount(8)
            .SetExpectPortionsCount(8)
            .Execute(batch);
    }
    ...
    Y_UNIT_TEST(CritSimple) {
        ...
        std::shared_ptr<arrow::RecordBatch> batch =
            NKikimr::NArrow::NConstruction::TRecordBatchConstructor({columnBig}).BuildBatch(80048);
        NKikimr::NColumnShard::TIndexationCounters counters("test");

        TSplitTester()
            .SetExpectBlobsCount(72)
            .SetExpectSlicesCount(8)
            .SetExpectedInternalSplitsCount(0)
            .SetExpectPortionsCount(8)
            .Execute(batch);
