...
bool WriteData(
    ...
    if (writeIds) {
        return WriteDataImpl(
            ...
            writeId,
            data,
            NArrow::MakeArrowSchema(ydbSchema),
            writeIds,
            mType
        );
    }
    ...
    return WriteDataImpl(
        ...
        data,
        NArrow::MakeArrowSchema(ydbSchema),
        waitResult ? &ids : nullptr,
        mType
    );
}

std::optional<ui64> WriteData(
    TTestBasicRuntime& runtime,
    TActorId& sender,
    const NLongTxService::TLongTxId& longTxId,
    ui64 tableId,
    const ui64 writePartId,
    const TString& data,
    const std::vector<NArrow::NTest::TTestColumn>& ydbSchema,
    const NEvWrite::EModificationType mType
) {
...
void ProposeCommit(
    ...
    ForwardToTablet(runtime, shardId, sender, new TEvColumnShard::TEvProposeTransaction(txKind, sender, txId, txBody));
    TAutoPtr<IEventHandle> handle;
...
TCell MakeTestCell(const TTypeInfo& typeInfo, ui32 value, std::vector<TString>& mem) {
    ...
    if (type == NTypeIds::Utf8 || type == NTypeIds::String || type == NTypeIds::String4k ||
        ...
        const TString& str = mem.back();
        return TCell(str.data(), str.size());
    } else if (type == NTypeIds::Timestamp || type == NTypeIds::Interval || type == NTypeIds::Timestamp64 ||
               type == NTypeIds::Interval64 || type == NTypeIds::Uint64 || type == NTypeIds::Int64) {
...
TSerializedTableRange MakeTestRange(
    ...
    return TSerializedTableRange(
        TConstArrayRef<TCell>(cellsFrom), inclusiveFrom, TConstArrayRef<TCell>(cellsTo), inclusiveTo
    );
}
        ...
        if (NOlap::NIndexes::NMax::TIndexMeta::IsAvailableType(columns[i].GetType())) {
            *schema->AddIndexes() = NOlap::NIndexes::TIndexMetaContainer(
                                        std::make_shared<NOlap::NIndexes::NMax::TIndexMeta>(
                                            1000 + i, "MAX::INDEX::" + columns[i].GetName(), "__LOCAL_METADATA", i + 1
                                        )
            )
                                        .SerializeToProto();
        }
...
NOlap::TIndexInfo BuildTableInfo(
    const std::vector<NArrow::NTest::TTestColumn>& ydbSchema,
