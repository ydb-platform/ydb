...
class TLongTxTransactionOperator
    : public IProposeTxOperator
    , public TMonitoringObjectsCounter<TLongTxTransactionOperator> {
    using TBase = IProposeTxOperator;
    using TProposeResult = TTxController::TProposeResult;
    static inline auto Registrator =
        TFactory::TRegistrator<TLongTxTransactionOperator>(NKikimrTxColumnShard::TX_KIND_COMMIT);
...
public:
    ...
    virtual void DoOnTabletInit(TColumnShard& owner) override {
        for (auto&& writeId : WriteIds) {
            AFL_VERIFY(owner.LongTxWrites.contains(writeId))
            ("problem", "ltx_not_exists_for_write_id")("txId", GetTxId())("writeId", (ui64)writeId);
            owner.AddLongTxWrite(writeId, GetTxId());
        }
    }

    bool ProgressOnExecute(
        TColumnShard& owner,
        ...
        auto counters =
            owner.InsertTable->Commit(dbTable, version.GetPlanStep(), version.GetTxId(), WriteIds, pathExists);
