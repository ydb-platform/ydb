    ...
    if (owner.CurrentSchemeShardId) {
        ...
        ctx.Send(
            ...
            new TEvPipeCache::TEvForward(
                BuildProposeResultEvent(owner).release(), (ui64)owner.CurrentSchemeShardId, true
            )
        );
    } else {
        AFL_WARN(NKikimrServices::TX_COLUMNSHARD)
        ("event", "scheme_shard_tablet_not_initialized")("source", GetTxInfo().Source);
...
std::unique_ptr<NKikimr::TEvColumnShard::TEvProposeTransactionResult> IProposeTxOperator::BuildProposeResultEvent(
    const TColumnShard& owner
) const {
    const auto& txInfo = GetTxInfo();
    std::unique_ptr<TEvColumnShard::TEvProposeTransactionResult> evResult =
        std::make_unique<TEvColumnShard::TEvProposeTransactionResult>(
            owner.TabletID(),
            txInfo.TxKind,
            txInfo.TxId,
            GetProposeStartInfoVerified().GetStatus(),
    ...
    if (IsFail()) {
        ...
        ("message",
         GetProposeStartInfoVerified().GetStatusMessage())("tablet_id", owner.TabletID())("tx_id", txInfo.TxId);
    } else {
