...
TTxController::TProposeResult TSchemaTransactionOperator::DoStartProposeOnExecute(TColumnShard& owner, NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    switch (SchemaTxBody.TxBody_case()) {
        ...
        {
            const TLocalPathId srcLocalPathId = TLocalPathId::FromRawValue(SchemaTxBody.GetMoveTable().GetSrcPathId());
            const TLocalPathId dstLocalPathId = TLocalPathId::FromRawValue(SchemaTxBody.GetMoveTable().GetDstPathId());
            AFL_INFO(NKikimrServices::TX_COLUMNSHARD)("propose_execute", "move_table")("src", srcLocalPathId)("dst", dstLocalPathId);
            if (!owner.TablesManager.ResolveInternalPathId(srcLocalPathId)) {
                return TProposeResult(NKikimrTxColumnShard::EResultStatus::SCHEMA_ERROR, "No such table");
            }
            if (owner.TablesManager.ResolveInternalPathId(dstLocalPathId)) {
            ...
            owner.TablesManager.MoveTableProposeOnExecute(srcLocalPathId);
            break;
        }
...
void TSchemaTransactionOperator::DoOnTabletInit(TColumnShard& owner) {
   AFL_VERIFY(!WaitOnPropose);
    switch (SchemaTxBody.TxBody_case()) {
        ...
        case NKikimrTxColumnShard::TSchemaTxBody::kMoveTable:
        {
            const auto srcPathId = TLocalPathId::FromRawValue(SchemaTxBody.GetMoveTable().GetSrcPathId());
            const auto dstPathId = TLocalPathId::FromRawValue(SchemaTxBody.GetMoveTable().GetDstPathId());

            AFL_VERIFY(owner.TablesManager.ResolveInternalPathId(srcPathId));
            AFL_VERIFY(!owner.TablesManager.ResolveInternalPathId(dstPathId));
