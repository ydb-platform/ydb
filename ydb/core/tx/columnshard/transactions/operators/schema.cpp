...
TTxController::TProposeResult TSchemaTransactionOperator::DoStartProposeOnExecute(TColumnShard& owner, NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    switch (SchemaTxBody.TxBody_case()) {
        ...
        {
            const TLocalPathId srcLocalPathId = TLocalPathId::FromLocalPathIdValue(SchemaTxBody.GetMoveTable().GetSrcPathId());
            const TLocalPathId dstLocalPathId = TLocalPathId::FromLocalPathIdValue(SchemaTxBody.GetMoveTable().GetDstPathId());
