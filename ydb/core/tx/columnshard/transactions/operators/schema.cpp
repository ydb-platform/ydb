...
public:
    TWaitEraseTablesTxSubscriber(const ui64 txId, THashSet<ui64>&& waitTables)
...
class TWaitTransactions: public TWaitOnProposeTxSubscriberBase {
    THashSet<ui64> TxIdsToWait;
public:
    TWaitTransactions(const ui64 txId, const THashSet<ui64>&& txIdsToWait)
        : TWaitOnProposeTxSubscriberBase(txId)
    ...
    switch (SchemaTxBody.TxBody_case()) {
        ...
        case NKikimrTxColumnShard::TSchemaTxBody::kMoveTable:
        {
            ...
            if (!txIdsToWait.empty()) {
                //TODO txIdsToWait and recover on table init
                AFL_VERIFY(!txIdsToWait.contains(GetTxId()))("tx_id", GetTxId())("tx_ids", JoinSeq(",", txIdsToWait));
                WaitOnPropose = std::make_shared<TWaitTransactions>(GetTxId(), std::move(txIdsToWait));
            }
...
void TSchemaTransactionOperator::DoStartProposeOnComplete(TColumnShard& shard, const TActorContext& /*ctx*/) {
    if (SchemaTxBody.TxBody_case() == NKikimrTxColumnShard::TSchemaTxBody::kMoveTable) {
        const auto srcPathId = SchemaTxBody.GetMoveTable().GetSrcPathId();
        const auto dstPathId = SchemaTxBody.GetMoveTable().GetDstPathId();
        using namespace NOlap::NDataLocks;
        LockGuard = shard.GetDataLocksManager()->RegisterLock<TListTablesLock>(
            TStringBuilder() << "lock_tables:" << srcPathId << "," << dstPathId,
            THashSet<ui64>{srcPathId, dstPathId},
            ELockCategory::Any //TODO consider to relax
        );
