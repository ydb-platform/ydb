...
bool TTxController::Load(NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    YDB_LOG_INFO("",
        ...
        {"noDl", countNoDeadline},
        {"dl", countWithDeadline},
        {"operators", Operators.size()},
        {"plan", PlanQueue.size()},
        {"dlQueue", DeadlineQueue.size()});
    return true;
}
    ...
    if (!!txInfoPtr) {
        if (!txOperator->CheckAllowUpdate(*txInfoPtr)) {
            ...
            YDB_LOG_WARN("",
                ...
                {"deprecatedTx", txInfoPtr->DebugString()});
