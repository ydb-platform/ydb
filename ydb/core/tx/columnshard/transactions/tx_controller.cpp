...
bool TTxController::Load(NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    YDB_LOG_INFO("",
        {"override", countOverrideDeadline},
        {"no_dl", countNoDeadline},
        {"dl", countWithDeadline},
        {"operators", Operators.size()},
        {"plan", PlanQueue.size()},
        {"dl_queue", DeadlineQueue.size()});
    return true;
}
...
TTxController::EPlanResult TTxController::PlanTx(const ui64 planStep, const ui64 txId, NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    if (it == Operators.end()) {
        ...
        YDB_LOG_TRACE("",
            {"event", "plan_tx"},
            {"tx_id", txId},
            {"plan_step", it->second->MutableTxInfo().PlanStep});
    }
    ...
    if (txInfo.PlanStep == 0) {
        ...
        YDB_LOG_INFO("",
            {"event", "skip_plan_tx_plan_step_is_not_zero"},
            {"tx_id", txId},
            {"plan_step", txInfo.PlanStep},
    ...
    if (!!txInfoPtr) {
        if (!txOperator->CheckAllowUpdate(*txInfoPtr)) {
            YDB_LOG_WARN("",
                ...
                {"actual_tx", txInfoPtr->DebugString()});
            ...
            YDB_LOG_WARN("",
                {"error", "update duplication data"},
                {"deprecated_tx", txInfoPtr->DebugString()});
        ...
        if (txOperator->StartProposeOnExecute(Owner, txc)) {
            ...
            YDB_LOG_DEBUG("",
                {"event", "registered"},
                {"tx_operator", txOperator->GetOpType()});
        } else {
            YDB_LOG_ERROR("",
                {"error", "problem on start"},
...
void TTxController::FinishProposeOnExecute(const ui64 txId, NTabletFlatExecutor::TTransactionContext& txc) {
    ...
    if (auto txOperator = GetTxOperatorOptional(txId)) {
        ...
        YDB_LOG_WARN("",
            {"error", "cannot found txOperator in propose transaction base"},
            {"tx_id", txId});
    }
}
...
void TTxController::FinishProposeOnComplete(ITransactionOperator& txOperator, const TActorContext& ctx) {
    ...
    YDB_LOG_DEBUG("",
        {"event", "start"},
        {"tx_info", txOperator.GetTxInfo().DebugString()});
