...
// A cut follows only a MoveData session: it is what persists the row the cutter reads back.
void RunMoveDataForOldGroup(TTestBasicRuntime& runtime) {
    runtime.SendToPipe(TTestTxConfig::TxTablet0, runtime.AllocateEdgeActor(), new TEvTablet::TEvMoveData(std::vector<ui32>{ 0u }), 0,
...
void ConfigureCutter(TTestBasicRuntime& runtime) {
    ...
    // Handle(TEvMoveData) returns early without this, so no row would ever be written.
...
Y_UNIT_TEST_SUITE(TCutHistoryTablet) {
    ...
    Y_UNIT_TEST(EmptyOldEntryIsCutAfterFirstGcRound) {
        ...
        RebootWithOldEntry(runtime, tabletActorId, written.Generation, launcher);
        RunMoveDataForOldGroup(runtime);
