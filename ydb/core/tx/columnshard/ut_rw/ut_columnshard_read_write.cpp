...
void TestWriteOverload(const TestTableDescription& table) {
    ...
    TString testBlob = MakeTestBlob({0, 100 * 1000}, ydbSchema);
    UNIT_ASSERT(testBlob.size() > NColumnShard::TLimits::MAX_BLOB_SIZE / 2);
    UNIT_ASSERT(testBlob.size() < NColumnShard::TLimits::MAX_BLOB_SIZE);
    ...
    auto captureEvents = [&](TTestActorRuntimeBase&, TAutoPtr<IEventHandle> &ev) {
        if (auto* msg = TryGetPrivateEvent<TEvColumnShard::TEvWrite>(ev)) {
            Cerr << "CATCH TEvWrite, status " << msg->PutStatus << Endl;
            if (toCatch && msg->PutStatus != NKikimrProto::UNKNOWN) {
