...
//    const TDuration exportTimeout = TDuration::Seconds(40);
    ...
    for (ui32 i = 0; i < specs.size(); ++i) {
        ...
        // Read crossed with eviction (start)
        {
            ...
            if (!misconfig) {
                reader = std::make_unique<TShardReader>(runtime, TTestTxConfig::TxTablet0, tableId, NOlap::TSnapshot(planStep, Max<ui64>()));
...
void TestDrop(bool reboots) {
    ...
    // Drop table
    minPlanStep = SetupSchema(runtime, sender, TTestSchema::DropTableTxBody(tableId, 2), ++txId);
    UNIT_ASSERT(minPlanStep);
    planStep = *minPlanStep;
