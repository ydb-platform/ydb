...
struct TTxCoordinator::TTxSubscribeReadStep: public TTransactionBase<TTxCoordinator> {
    ...
    TTxType GetTxType() const override {
        return TXTYPE_SUBSCRIBE_READ_STEP;
    }
    ...
    void Complete(const TActorContext& ctx) override {
        ...
        ctx.Send(
            Self->EnsureReadStepSubscriptionManager(ctx),
            new TEvPrivate::TEvReadStepSubscribed(PipeServer, Ev, lastStep, nextStep)
        );
    }
};
...
struct TTxCoordinator::TTxUnsubscribeReadStep: public TTransactionBase<TTxCoordinator> {
    ...
    TTxType GetTxType() const override {
        return TXTYPE_UNSUBSCRIBE_READ_STEP;
    }
