...
Y_UNIT_TEST_SUITE(Coordinator) {
    Y_UNIT_TEST(ReadStepSubscribe) {
        ...
        serverSettings.SetDomainName("Root").SetNodeCount(2).SetUseRealThreads(false);
        ...
        waitFor(
            [&] {
                return lastMediatorStep != 0;
            },
            "the first mediator step"
        );
        ...
        waitFor(
            [&] {
                return lastMediatorStep > firstMediatorStep;
            },
            "the next mediator step"
        );
        ...
        {
            ForwardToTablet(
                runtime, coordinatorId, sender, new TEvTxProxy::TEvSubscribeReadStep(coordinatorId, /* seqno */ 1)
            );
            auto ev = runtime.GrabEdgeEvent<TEvTxProxy::TEvSubscribeReadStepResult>(sender);
            auto* msg = ev->Get();
            Cerr << "... read step subscribe result: [" << msg->Record.GetLastAcquireStep() << ", "
                 << msg->Record.GetNextAcquireStep() << "]" << Endl;
        ...
        {
            ForwardToTablet(
                runtime, coordinatorId, sender3, new TEvTxProxy::TEvSubscribeReadStep(coordinatorId, /* seqno */ 51), 1
            );
    ...
    public:
        ...
        static typename TEvent::TPtr
        Wait(TTestActorRuntimeBase& runtime, std::deque<std::unique_ptr<IEventHandle>>& events, int attempts = 1) {
            ...
            UNIT_ASSERT_C(
                events.front()->GetTypeRewrite() == TEvent::EventType,
                "Expected event " << TypeName<TEvent>() << " but received " << events.front()->ToString()
            );
    ...
    Y_UNIT_TEST(RestoreDomainConfiguration) {
        ...
        TPortManager pm;
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetNodeCount(1).SetUseRealThreads(false);
        ...
        UNIT_ASSERT_C(hooks.PersistConfig_.size() > 0, "Coordinators didn't even try to persist configs");
    ...
    using TEvCreateDatabaseRequest = NKikimr::NGRpcService::
        TGrpcRequestOperationCall<Ydb::Cms::CreateDatabaseRequest, Ydb::Cms::CreateDatabaseResponse>;
    ...
    Y_UNIT_TEST(RestoreTenantConfiguration) {
        ...
        UNIT_ASSERT_C(
            hooks.PersistConfig_.size() == coordinators.size(), "Expected all coordinators to persist restored config"
        );
        for (auto& pr : hooks.PersistConfig_) {
            UNIT_ASSERT_C(
                pr.second.GetVersion() == 2,
