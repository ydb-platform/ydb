...
Y_UNIT_TEST_SUITE(CoordinatorVolatile) {
    Y_UNIT_TEST(PlanResentOnReboots) {
        ...
        waitFor(
            ...
            "plan step resent on tablet reboot"
        );
        ...
        waitFor(
            ...
            "plan step resent on mediator reboot"
        );
        UNIT_ASSERT_VALUES_EQUAL(observedSteps.at(0), step1);
        observedSteps.clear();
        ...
        ui64 txId2 = txId + 1;
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, txId2, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            tx->SetFlags(TEvTxProxy::TEvProposeTransaction::FlagVolatile);
            auto* affected = tx->AddAffectedSet();
        ...
        UNIT_ASSERT_C(step2 > step1 && step2 - step1 < 10, "unexpected step " << step2 << " after step " << step1);
        observedSteps.clear();
        ...
        ui64 txId3 = txId2 + 1;
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, txId3, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            tx->SetFlags(TEvTxProxy::TEvProposeTransaction::FlagVolatile);
            auto* affected = tx->AddAffectedSet();
        ...
        UNIT_ASSERT_C(step3 > step2 && step3 - step2 > 100, "unexpected step " << step3 << " after step " << step2);
        observedSteps.clear();
        ...
        runtime.SimulateSleep(TDuration::MilliSeconds(1));
        UNIT_ASSERT_C(
            observedSteps.empty(), "unexpected resend of acknowledged step after coordinator+mediator reboot"
        );

        ui64 txId4 = txId2 + 1;
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, txId4, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            tx->SetFlags(TEvTxProxy::TEvProposeTransaction::FlagVolatile);
            auto* affected = tx->AddAffectedSet();
        ...
        UNIT_ASSERT_C(step4 > step3 && step4 - step3 < 10, "unexpected step " << step4 << " after step " << step3);
        observedSteps.clear();
    }
    ...
    Y_UNIT_TEST(MediatorReconnectPlanRace) {
        ...
        waitFor(
            [&] {
                return mediatorQueueSteps.size() > 0;
            },
            "TEvMediatorQueueStep"
        );
        ...
        auto injectMediatorQueueStep =
            runtime.AddObserver<TEvTabletPipe::TEvClientDestroyed>([&](TEvTabletPipe::TEvClientDestroyed::TPtr& ev) {
        ...
        waitFor(
            [&] {
                return mediatorQueueSteps.empty();
            },
            "injected mediator steps"
        );
    ...
    Y_UNIT_TEST(CoordinatorMigrateUncommittedVolatileTx) {
        struct TCoordinatorHooks: public ICoordinatorHooks {
            ...
            void BeginPlanStep(ui64 tabletId, ui64 generation, ui64 planStep) override {
                Cerr << "... coordinator " << tabletId << " gen " << generation << " is planning step " << planStep
                     << Endl;
        ...
        TPortManager pm;
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetNodeCount(1).SetUseRealThreads(false).SetDomainPlanResolution(50);
        ...
        waitFor(
            [&] {
                return hooks.PlannedSteps.size() >= 2;
            },
            "the first two planned steps"
        );
        ...
        auto blockPutResponses =
            runtime.AddObserver<TEvBlobStorage::TEvPutResult>([&](TEvBlobStorage::TEvPutResult::TPtr& ev) {
                auto* msg = ev->Get();
        ...
        auto blockPlanStepAccepted =
            runtime.AddObserver<TEvTxProcessing::TEvPlanStepAccepted>([&](TEvTxProcessing::TEvPlanStepAccepted::TPtr& ev
                                                                      ) {
                auto* msg = ev->Get();
        ...
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, persistentTxId, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            auto* affected = tx->AddAffectedSet();
            affected->SetTabletId(tabletId);
        ...
        auto blockPutRequests = runtime.AddObserver<TEvBlobStorage::TEvPut>([&](TEvBlobStorage::TEvPut::TPtr& ev) {
            auto* msg = ev->Get();
        ...
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, volatileTxId1, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            tx->SetFlags(TEvTxProxy::TEvProposeTransaction::FlagVolatile);
            auto* affected = tx->AddAffectedSet();
        ...
        waitFor(
            [&] {
                return hooks.PlannedSteps.size() >= 1;
            },
            "planned volatile tx"
        );
        ...
        UNIT_ASSERT_C(
            volatilePlanStep > persistentPlanStep,
        ...
        auto blockOldTabletDead =
            runtime.AddObserver<TEvTablet::TEvTabletDead>([&](TEvTablet::TEvTabletDead::TPtr& ev) {
        ...
        auto blockStateResponses = runtime.AddObserver<TEvTxCoordinator::TEvCoordinatorStateResponse>(
            [&](TEvTxCoordinator::TEvCoordinatorStateResponse::TPtr& ev) {
                Cerr << "... blocking state response from " << ev->Sender << " to " << ev->GetRecipientRewrite()
                     << Endl;
        ...
        StartTestTablet(
            runtime, CreateTestTabletInfo(coordinatorId, TTabletTypes::Coordinator), &CreateFlatTxCoordinator
        );

        // Wait until new coordinator almost receives the in-memory state
        waitFor(
            [&] {
                return blockedStateResponses.size() >= 1;
            },
            "migrated state"
        );
        ...
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, volatileTxId2, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            tx->SetFlags(TEvTxProxy::TEvProposeTransaction::FlagVolatile);
            auto* affected = tx->AddAffectedSet();
        ...
        UNIT_ASSERT_C(
            volatilePlanStep2 > volatilePlanStep,
    ...
    Y_UNIT_TEST(CoordinatorRestartWithEnqueuedVolatileStep) {
        ...
        auto observeSteps =
            runtime.AddObserver<TEvTxCoordinator::TEvCoordinatorStep>([&](TEvTxCoordinator::TEvCoordinatorStep::TPtr& ev
                                                                      ) {
                auto* msg = ev->Get();
                ...
                if (it == observedSteps.end()) {
                    ...
                    UNIT_ASSERT_C(
                        it->second == txIds,
                        "Step " << step << " changed transactions list " << dumpTxIds(it->second) << " -> "
                                << dumpTxIds(txIds)
                    );
                }
            });
        ...
        runtime.SendToPipe(
            coordinatorId, sender, new TEvTxProxy::TEvRequirePlanSteps(coordinatorId, persistentPlanStep + 50)
        );
        ...
        waitFor(
            ...
            "planning for the required step"
        );
        ...
        if (auto propose = std::make_unique<TEvTxProxy::TEvProposeTransaction>(
                coordinatorId, persistentTxId2, 0, Min<ui64>(), Max<ui64>()
            )) {
            auto* tx = propose->Record.MutableTransaction();
            auto* affected = tx->AddAffectedSet();
            affected->SetTabletId(tabletId);
        ...
        waitFor(
            ...
            "planned another persistent tx"
        );

        // Wait until mediator observes it
        waitFor(
            [&] {
                return observedSteps.size() > oldObservedSteps;
            },
            "new step reaches mediator"
        );

        runtime.SimulateSleep(TDuration::MilliSeconds(50));
        UNIT_ASSERT_C(
            observedTabletTxs.contains(persistentTxId2), "Tablet did not observe a persistent tx " << persistentTxId2
        );
