...
TWritersController::TWritersController(
...
void TShardWriter::SendWriteRequest() {
    if (ImmediateWrite) {
        ...
        auto ev = MakeHolder<TEvColumnShard::TEvWrite>(
            SelfId(), ExternalController->GetLongTxId(), TableId, DedupId, "", WritePartIdx, ModificationType
        );
...
void TShardWriter::Handle(NEvents::TDataEvents::TEvWriteResult::TPtr& ev) {
    ...
    if (ydbStatus != NKikimrDataEvents::TEvWriteResult::STATUS_COMPLETED) {
        ...
        ExternalController->OnFail(
            statusInfo.GetYdbStatusCode(),
            TStringBuilder() << "Cannot write data into shard(" << statusInfo.GetIssueGeneralText() << ") " << ShardId
...
void TShardWriter::Handle(TEvPipeCache::TEvDeliveryProblem::TPtr& ev) {
    ...
    const TString errMsg = TStringBuilder()
                           << "Shard " << ShardId << " is not available after " << NumRetries << " retries";
    if (msg->NotDelivered) {
...
void TShardWriter::Handle(NActors::TEvents::TEvWakeup::TPtr& ev) {
    if (ev->Get()->Tag) {
        ...
        ExternalController->OnFail(
            ...
            TStringBuilder() << "Cannot write data (TIMEOUT) into shard " << ShardId << " in longTx "
                             << ExternalController->GetLongTxId().ToString()
        );
