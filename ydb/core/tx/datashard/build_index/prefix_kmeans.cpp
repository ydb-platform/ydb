...
public:
    ...
    TPrefixKMeansScanBase(ui64 tabletId, const TUserTable& table,
        ...
        , DataColumnCount{(ui32)request.GetDataColumns().size()}
    {
        ...
        }
        {
            auto outputTypes = MakeOutputTypes(table, UploadState, embedding,
                {data.begin(), data.begin()+DataColumnCount}, request.GetSourcePrimaryKeyColumns());
...
private:
    ...
    {
        if (auto pos = Clusters.FindCluster(row, EmbeddingPos); pos) {
            AddRowBuildToBuild(*OutputBuf, Child + *pos, row.Slice(DataColumnCount), row.Slice(0, DataColumnCount), key);
        }
    }
    ...
    {
        if (auto pos = Clusters.FindCluster(row, EmbeddingPos); pos) {
            AddRowBuildToPosting(*OutputBuf, Child + *pos, row.Slice(DataColumnCount), row.Slice(0, DataColumnCount), key);
        }
    }
...
void TDataShard::HandleSafe(TEvDataShard::TEvPrefixKMeansRequest::TPtr& ev, const TActorContext& ctx)
{
    ...
    try {
        ...
        if (request.GetSourcePrimaryKeyColumns().size() == 0) {
            badRequest("Request should include original primary key columns");
        }
