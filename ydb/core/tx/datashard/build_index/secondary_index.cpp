...
private:
    ...
    void HandleNextVal(NSequenceProxy::TEvSequenceProxy::TEvNextValResult::TPtr& ev) {
        ...
        if (seqCol.BitReverse) {
            // The bit-reverse flag is set only for __ydb_row_id; apply the shared row-id layout
            // (spread bucket in the high bits, dense seq in the low bits) so backfilled values match
            // those produced by the online-insert sequencer (kqp_sequencer_actor.cpp).
