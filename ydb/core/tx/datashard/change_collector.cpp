...
public:
    ...
    void AddChange(const TTableId& tableId, const TPathId& pathId, TChangeRecord::EKind kind, const TDataChange& body)
        override {
        NIceDb::TNiceDb db(Db);
        ...
        if (!WriteTxId) {
            ...
            builder.WithOrder(Self->AllocateChangeRecordOrder(db))
                .WithGroup(group)
                .WithStep(WriteVersion.Step)
                .WithTxId(WriteVersion.TxId);
        } else {
            ui64 lockOffset = Self->GetNextChangeRecordLockOffset(WriteTxId);
            builder.WithLockId(WriteTxId).WithLockOffset(lockOffset);
        }

        auto recordPtr = builder.WithPathId(pathId)
...
Y_DECLARE_OUT_SPEC(, NKikimr::NDataShard::IDataShardChangeCollector::TChange, o, x) {
    o << "{"
      << " Order: " << x.Order << " PathId: " << x.PathId << " BodySize: " << x.BodySize << " TableId: " << x.TableId
