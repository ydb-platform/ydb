...
class TCdcStreamChangeCollector: public TBaseChangeCollector {
    TMaybe<NTable::TRowState> GetState(
    ...
    static NTable::TRowState PatchState(
        const NTable::TRowState& oldState,
        NTable::ERowOp rop,
        const THashMap<NTable::TTag, NTable::TPos>& tagToPos,
        const THashMap<NTable::TTag, NTable::TUpdateOp>& updates
    );
    ...
    void Persist(
        ...
        TArrayRef<const NTable::TTag> keyTags,
        const NTable::TRowState* oldState,
        const NTable::TRowState* newState,
        TArrayRef<const NTable::TTag> valueTags
    );
