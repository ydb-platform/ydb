...
void TChangeRecordBodySerializer::SerializeCells(
    TSerializedCells& out,
    TArrayRef<const TRawTypeValue> in,
...
void TChangeRecordBodySerializer::SerializeCells(TSerializedCells& out, TArrayRef<const TUpdateOp> in) {
    if (!in) {
        return;
    }
...
void TChangeRecordBodySerializer::Serialize(
    TDataChange& out,
    ERowOp rop,
