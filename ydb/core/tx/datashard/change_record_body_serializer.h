...
class TChangeRecordBodySerializer {
    ...
    static void
    SerializeCells(TSerializedCells& out, TArrayRef<const TRawTypeValue> in, TArrayRef<const NTable::TTag> tags);
    static void SerializeCells(TSerializedCells& out, TArrayRef<const NTable::TUpdateOp> in);
    static void
    SerializeCells(TSerializedCells& out, const NTable::TRowState& state, TArrayRef<const NTable::TTag> tags);
