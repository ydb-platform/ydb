                                             ...
                                             const TActorContext &ctx)
{
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Propose scheme transaction at tablet txId ssId seqNo",
        {"TabletID", DataShard.TabletID()},
        {"GetTxId", op->GetTxId()},
        {"GetSchemeShardId", activeTx->GetSchemeShardId()},
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Prepared scheme transaction txId at tablet",
        {"GetTxId", op->GetTxId()},
        {"TabletID", DataShard.TabletID()});
...
{
    YDB_LOG_CTX_INFO(TActivationContext::AsActorContext(), "Check scheme tx, proposed scheme expected",
        {"version", proposedSchemaVersion},
        {"current_version", currentSchemaVersion},
        {"#_version", expectedSchemaVersion},
    ...
    if (proposedSchemaVersion && currentSchemaVersion && expectedSchemaVersion != proposedSchemaVersion) {
        ...
        YDB_LOG_CTX_CRIT(TActivationContext::AsActorContext(), "",
            {"err", err});
        return false;
    }
...
bool TCheckSchemeTxUnit::HasDuplicate(TActiveTransaction *activeTx, const TStringBuf kind, TPipelineHasSmthFunc checker) {
    ...
    YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "Ignoring duplicate at tablet txId currentTxId",
        {"kind", kind},
        {"TabletID", DataShard.TabletID()},
        {"GetTxId", activeTx->GetTxId()},
...
bool TCheckSchemeTxUnit::HasConflicts(TActiveTransaction *activeTx, const TStringBuf kind, const THashMap<TString, TPipelineHasSmthFunc>& checkers) {
    for (const auto& kv : checkers) {
        ...
        YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "Ignoring during ongoing at tablet txId currentTxId",
            {"kind", kind},
...
bool TCheckSchemeTxUnit::HasPathId(TActiveTransaction *activeTx, const T &op, const TStringBuf kind) {
    ...
    YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "description has no PathId at tablet txId currentTxId",
        {"kind", kind},
        {"TabletID", DataShard.TabletID()},
        {"GetTxId", activeTx->GetTxId()},
        {"CurrentSchemaTxId", Pipeline.CurrentSchemaTxId()});
...
bool TCheckSchemeTxUnit::CheckSchemeTx(TActiveTransaction *activeTx)
{
    ...
    case TSchemaOperation::ETypeTruncate:
        ...
        break;
    default:
        YDB_LOG_CTX_ERROR(TActivationContext::AsActorContext(), "Unknown scheme tx type detected at tablet txId txBody",
            {"TabletID", DataShard.TabletID()},
            {"GetTxId", activeTx->GetTxId()},
            {"ShortDebugString", tx.ShortDebugString()});
        BuildResult(activeTx, NKikimrTxDataShard::TEvProposeTransactionResult::ERROR);
    }
...
bool TCheckSchemeTxUnit::CheckAlter(TActiveTransaction *activeTx)
{
    ...
    if (alter.HasPartitionConfig() && alter.GetPartitionConfig().HasFreezeState()) {
        if (alter.ColumnsSize() || alter.DropColumnsSize()) {
            YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "Ignoring alter, combine freeze with other actions is forbiden, tablet txId currentTxId",
                {"TabletID", DataShard.TabletID()},
                {"GetTxId", activeTx->GetTxId()},
                {"CurrentSchemaTxId", Pipeline.CurrentSchemaTxId()});
        ...
        if (DataShard.IsFollower()) {
            YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "Ignoring alter, attempt to freeze follower, tablet txId currentTxId",
                {"TabletID", DataShard.TabletID()},
                {"GetTxId", activeTx->GetTxId()},
                {"CurrentSchemaTxId", Pipeline.CurrentSchemaTxId()});
            BuildResult(activeTx, NKikimrTxDataShard::TEvProposeTransactionResult::BAD_REQUEST);
        ...
        if (err) {
            ...
            YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "",
                {"errText", errText});
...
bool TCheckSchemeTxUnit::CheckFinalizeBuildIndex(TActiveTransaction *activeTx) {
    ...
    if (DataShard.GetSnapshotManager().FindAvailable(snapshotKey) == nullptr) {
        YDB_LOG_CTX_DEBUG(TActivationContext::AsActorContext(), "FinalizeBuildIndex description has unexisting snapshot key. Tablet txId pathId snapshotKey",
            {"TabletID", DataShard.TabletID()},
            {"GetTxId", activeTx->GetTxId()},
            {"pathId", pathId},
