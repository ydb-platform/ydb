...
TCheckWriteUnit::TCheckWriteUnit(TDataShard& dataShard, TPipeline& pipeline)
...
EExecutionStatus TCheckWriteUnit::Execute(TOperation::TPtr op, TTransactionContext&, const TActorContext& ctx) {
    ...
    // or system table.
    if (DataShard.IsAnyChannelYellowStop() && (writeTx->HasWrites() || !op->IsImmediate())) {
        ...
        LOG_LOG_S_THROTTLE(
            DataShard.GetLogThrottler(TDataShard::ELogThrottlerType::CheckWriteUnit_Execute),
            ctx,
    ...
    if (!op->IsImmediate()) {
        ...
        writeOp->SetWriteResult(NEvents::TDataEvents::TEvWriteResult::BuildPrepared(
            ...
            {op->GetMinStep(),
             op->GetMaxStep(),
             DataShard.GetProcessingParams() ? DataShard.GetProcessingParams()->GetCoordinators()
                                             : google::protobuf::RepeatedField<ui64>{}}
        ));
