...
void TDataShard::OnActivateExecutor(const TActorContext& ctx) {
    YDB_LOG_CTX_INFO(ctx, "TDataShard::OnActivateExecutor: tablet actor",
        {"TabletID", TabletID()},
        {"SelfID", ctx.SelfID});
    ...
    if (!IsFollower()) {
        ...
        YDB_LOG_CTX_INFO(ctx, "Follower switched to work",
            {"state", TabletID()});
    }
}
...
void TDataShard::SwitchToWork(const TActorContext &ctx) {
    ...
    YDB_LOG_CTX_INFO(ctx, "Switched to work state tabletId",
...
void TDataShard::SendRegistrationRequestTimeCast(const TActorContext &ctx) {
    ...
    if (!ProcessingParams) {
        YDB_LOG_CTX_DEBUG(ctx, "not sending time cast registration request in state : missing processing params",
            {"TabletID", TabletID()},
            {"#_DatashardStateName(State)", DatashardStateName(State)});
        return;
    }
        ...
        State == TShardState::SplitDstReceivingSnapshot)
    {
        ...
        YDB_LOG_CTX_DEBUG(ctx, "not sending time cast registration request in state",
            {"TabletID", TabletID()},
            {"#_DatashardStateName(State)", DatashardStateName(State)});
        return;
    }

    YDB_LOG_CTX_INFO(ctx, "Send registration request to time cast tabletId mediators count is coordinators count is buckets per mediator",
        {"#_DatashardStateName(State)", DatashardStateName(State)},
        {"TabletID", TabletID()},
        {"MediatorsSize", ProcessingParams->MediatorsSize()},
        {"CoordinatorsSize", ProcessingParams->CoordinatorsSize()},
        {"GetTimeCastBucketsPerMediator", ProcessingParams->GetTimeCastBucketsPerMediator()});
...
public:
    ...
    void OnCommit(ui64) override {
        ...
        if (error) {
            ...
            YDB_LOG_DEBUG("Complete [ ] from at tablet send result to client, exec ms, propose ms",
                {"Step", Step},
                {"TxId", TxId},
                {"TabletID", Self->TabletID()},
                {"#_TabletID", Self->TabletID()},
                {"Target", Target},
                {"latency", Result->Record.GetExecLatency()},
                {"#_latency", Result->Record.GetProposeLatency()});
        }
                                   ...
                                   NWilson::TTraceId traceId)
{
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Complete [ ] from at tablet send result to client, exec ms, propose ms",
        {"step", step},
        {"txId", txId},
        {"TabletID", TabletID()},
        {"#_TabletID", TabletID()},
        ...
        {"#_latency", res->Record.GetProposeLatency()});
        ...
        NWilson::TTraceId traceId)
{
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Complete write [ ] from at tablet send result to client",
        {"step", step},
        {"txId", txId},
        {"TabletID", TabletID()},
        {"#_TabletID", TabletID()},
...
void TDataShard::CreateChangeSender(const TActorContext& ctx) {
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Change sender created",
        {"at_tablet", TabletID()},
        {"actorId", OutChangeSender});
}

void TDataShard::MaybeActivateChangeSender(const TActorContext& ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "Trying to activate change sender",
        {"at_tablet", TabletID()});

    OutChangeSenderSuspended = false;

    if (ReceiveActivationsFrom) {
        YDB_LOG_CTX_NOTICE(ctx, "Cannot activate change sender, wait to activation",
            {"at_tablet", TabletID()},
    ...
    case TShardState::Offline:
        YDB_LOG_CTX_INFO(ctx, "Cannot activate change sender",
            {"at_tablet", TabletID()},
    ...
    case TShardState::PreOffline:
        if (!ChangesQueue) {
            YDB_LOG_CTX_INFO(ctx, "Cannot activate change sender, queue",
                {"at_tablet", TabletID()},
                {"state", DatashardStateName(State)},
    ...
    YDB_LOG_CTX_INFO(ctx, "Change sender activated",
        {"at_tablet", TabletID()});
}
...
void TDataShard::KillChangeSender(const TActorContext& ctx) {
    if (OutChangeSender) {
        ...
        YDB_LOG_CTX_INFO(ctx, "Change sender killed",
            {"at_tablet", TabletID()});
    }
}
...
void TDataShard::NotifySchemeshard(const TActorContext& ctx, ui64 txId) {
    ...
    YDB_LOG_CTX_INFO(ctx, "Sending notify to schemeshard txId state TxInFly",
        {"TabletID", TabletID()},
...
{
    YDB_LOG_CTX_DEBUG(ctx, "Add schema snapshot",
        {"pathId", pathId},
...
void TDataShard::SnapshotComplete(TIntrusivePtr<NTabletFlatExecutor::TTableSnapshotContext> snapContext, const TActorContext &ctx) {
    if (auto txSnapContext = dynamic_cast<TTxTableSnapshotContext*>(snapContext.Get())) {
        ...
        if (!op) {
            YDB_LOG_CTX_CRIT(ctx, "Got snapshot for missing operation at",
                {"stepOrder", stepOrder},
                {"TabletID", TabletID()});
            return;
        }
...
void TDataShard::EnableKeyAccessSampling(const TActorContext &ctx, TInstant until) {
    if (CurrentKeySampler == DisabledKeySampler) {
        ...
        YDB_LOG_CTX_NOTICE(ctx, "Started key access sampling",
            {"at_datashard", TabletID()});
    } else {
        YDB_LOG_CTX_NOTICE(ctx, "Extended key access sampling",
            {"at_datashard", TabletID()});
    }
...
TRowVersion TDataShard::GetMvccTxVersion(EMvccTxMode mode, TOperation* op) const {
    ...
    YDB_LOG_TRACE("GetMvccTxVersion at",
        {"TabletID", TabletID()},
        ...
        const TRowVersion& version, EPromotePostExecuteEdges mode, TTransactionContext& txc)
{
    ...
    switch (mode) {
        ...
        case EPromotePostExecuteEdges::RepeatableRead: {
            YDB_LOG_TRACE("PromoteImmediatePostExecuteEdges at promoting UnprotectedReadEdge to",
                {"TabletID", TabletID()},
                {"version", version});
...
void TDataShard::SendAfterMediatorStepActivate(ui64 mediatorStep, const TActorContext& ctx) {
    for (auto it = MediatorDelayedReplies.begin(); it != MediatorDelayedReplies.end();) {
        ...
        if (MediatorTimeCastEntry && (MediatorTimeCastWaitingSteps.empty() || step < *MediatorTimeCastWaitingSteps.begin())) {
            MediatorTimeCastWaitingSteps.insert(step);
            Send(MakeMediatorTimecastProxyID(), new TEvMediatorTimecast::TEvWaitPlanStep(TabletID(), step));
            YDB_LOG_CTX_DEBUG(ctx, "Waiting for from mediator time cast",
                {"PlanStep", step});
        }
        break;
    }
...
void TDataShard::CheckMediatorStateRestored() {
    ...
    const ui64 observedStep = GetMaxObservedStep();
    YDB_LOG_DEBUG("CheckMediatorStateRestored at",
        {"TabletID", TabletID()},
        {"waitStep", waitStep},
        {"readStep", readStep},
        {"observedStep", observedStep});
...
void TDataShard::FinishMediatorStateRestore(TTransactionContext& txc, ui64 readStep, ui64 observedStep) {
    ...
    YDB_LOG_TRACE("CheckMediatorStateRestored at promoting UnprotectedReadEdge to",
        {"TabletID", TabletID()},
        {"edge", edge});
...
void TDataShard::Handle(TEvDataShard::TEvSchemaChangedResult::TPtr& ev, const TActorContext& ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "Handle TEvSchemaChangedResult datashard state",
        {"GetTxId", ev->Get()->Record.GetTxId()},
        {"TabletID", TabletID()},
        {"#_DatashardStateName(State)", DatashardStateName(State)});
...
void TDataShard::Handle(TEvDataShard::TEvStateChangedResult::TPtr& ev, const TActorContext& ctx) {
    Y_UNUSED(ev);
    YDB_LOG_CTX_DEBUG(ctx, "Handle TEvStateChangedResult datashard state",
        {"TabletID", TabletID()},
        {"#_DatashardStateName(State)", DatashardStateName(State)});
    // TODO: implement
    NTabletPipe::CloseAndForgetClient(SelfId(), StateReportPipe);
...
void TDataShard::Handle(TEvTxProcessing::TEvPlanStep::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!CheckMediatorAuthorisation(srcMediatorId)) {
        YDB_LOG_CTX_CRIT(ctx, "tablet receive PlanStep from unauthorized mediator",
            {"TabletID", TabletID()},
            {"GetStep", ev->Get()->Record.GetStep()},
...
void TDataShard::Handle(TEvPrivate::TEvRegisterScanActor::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!op) {
        YDB_LOG_CTX_INFO(ctx, "Cannot find op to register scan actor",
            {"txId", txId});
        return;
    }

    if (!op->IsReadTable()) {
        YDB_LOG_CTX_INFO(ctx, "Cannot register scan actor for op of kind",
            {"txId", txId},
            {"GetKind", op->GetKind()});
        return;
    }
...
void TDataShard::Handle(TEvTabletPipe::TEvClientConnected::TPtr &ev, const TActorContext &ctx) {
    ...
    if (LoanReturnTracker.Has(ev->Get()->TabletId, ev->Get()->ClientId)) {
        if (ev->Get()->Status != NKikimrProto::OK) {
            if (!ev->Get()->Dead) {
                YDB_LOG_CTX_DEBUG(ctx, "Resending loan returns from to",
                    {"TabletID", TabletID()},
                    {"#_ev->Get()->TabletId", ev->Get()->TabletId});
                LoanReturnTracker.ResendLoans(ev->Get()->TabletId, ctx);
            } else {
                YDB_LOG_CTX_DEBUG(ctx, "Auto-Acking loan returns to dead from",
                    {"#_ev->Get()->TabletId", ev->Get()->TabletId},
                    {"TabletID", TabletID()});
...
void TDataShard::Handle(TEvPipeCache::TEvDeliveryProblem::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!msg->Connected) {
        YDB_LOG_CTX_NOTICE(ctx, "Client pipe to tablet from failed to connect",
...
void TDataShard::Handle(TEvTabletPipe::TEvServerConnected::TPtr &ev, const TActorContext &ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "Server connected at",
...
void TDataShard::Handle(TEvTabletPipe::TEvServerDisconnected::TPtr &ev, const TActorContext &ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "Server disconnected at",
        {"#_num_0", (IsFollower() ? Sprintf("follower %u ", FollowerId()) : "leader ")},
        {"tablet", ev->Get()->TabletId},
        {"clientId", ev->Get()->ClientId},
        {"serverId", ev->Get()->ServerId},
...
void TDataShard::Handle(TEvMediatorTimecast::TEvRegisterTabletResult::TPtr& ev, const TActorContext& ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "Got TEvMediatorTimecast::TEvRegisterTabletResult at time",
        {"TabletID", TabletID()},
...
void TDataShard::Handle(TEvMediatorTimecast::TEvSubscribeReadStepResult::TPtr& ev, const TActorContext& ctx) {
    auto* msg = ev->Get();
    YDB_LOG_CTX_DEBUG(ctx, "Got TEvMediatorTimecast::TEvSubscribeReadStepResult at coordinator last step next step",
        {"TabletID", TabletID()},
...
void TDataShard::Handle(TEvMediatorTimecast::TEvNotifyPlanStep::TPtr& ev, const TActorContext& ctx) {
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Notified by mediator time cast with at tablet",
        {"PlanStep", step},
        {"TabletID", TabletID()});
...
void TDataShard::DoPeriodicTasks(const TActorContext &ctx) {
    ...
    if (CurrentKeySampler == EnabledKeySampler && ctx.Now() > StopKeyAccessSamplingAt) {
        CurrentKeySampler = DisabledKeySampler;
        YDB_LOG_CTX_NOTICE(ctx, "Stoped key access sampling",
            {"at_datashard", TabletID()});
    }
...
void TDataShard::UpdateLagCounters(const TActorContext &ctx) {
    ...
    if (txCompleteLag > TDuration::Minutes(5)) {
        YDB_LOG_CTX_WARN(ctx, "Tx completion lag ( ) is > 5 min on tablet",
    ...
    if (scanTxCompleteLag > TDuration::Hours(1)) {
        YDB_LOG_CTX_WARN(ctx, "Scan completion lag ( ) is > 1 hour on tablet",
        ...
        THolder<TEvTxProcessing::TEvReadSet>&& rs)
{
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Send RS at from to txId",
        {"seqno", seqno},
        {"TabletID", TabletID()},
        {"source", source},
        {"target", target},
...
void TDataShard::ResendReadSet(const TActorContext& ctx, ui64 step, ui64 txId, ui64 source, ui64 target,
                                      const TString& body, ui64 seqNo)
{
    YDB_LOG_CTX_INFO(ctx, "Resend RS at from to txId",
        {"TabletID", TabletID()},
        {"source", source},
        {"target", target},
        {"txId", txId});
...
void TDataShard::ResolveTablePath(const TActorContext &ctx)
{
    ...
    for (auto& [pathId, info] : TableInfos) {
        ...
        YDB_LOG_CTX_DEBUG(ctx, "Resolve path at",
            {"TabletID", TabletID()},
            {"reason", reason});
...
void TDataShard::Handle(TEvSchemeShard::TEvDescribeSchemeResult::TPtr ev, const TActorContext &ctx) {
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Got scheme resolve result at",
        {"TabletID", TabletID()},
        {"ShortDebugString", rec.ShortDebugString()});

    ui64 pathId = rec.GetPathId();
    if (!TableInfos.contains(pathId)) {
        YDB_LOG_CTX_ERROR(ctx, "Shard got describe result for unknown table",
            {"TabletID", TabletID()},
            {"pathId", pathId});
        return;
    }
    ...
    if (!rec.GetPath()) {
        YDB_LOG_CTX_CRIT(ctx, "Shard couldn't get path for table with status",
            {"TabletID", TabletID()},
            {"pathId", pathId},
            {"GetStatus", rec.GetStatus()});
        return;
    }
                               ...
                               const TActorContext &ctx)
{
    ...
    YDB_LOG_CTX_INFO(ctx, "Shard disconnected from node",
        {"TabletID", TabletID()},
        {"nodeId", nodeId});
                                     ...
                                     const TActorContext &ctx)
{
    if (auto* noTxScan = dynamic_cast<INoTxScan*>(prod.Get())) {
        YDB_LOG_CTX_DEBUG(ctx, "Non-transactinal scan complete at",
            {"TabletID", TabletID()});

        noTxScan->OnFinished(this);
        prod.Destroy();
    } else if (cookie != 0 && cookie != Max<ui64>()) {
        YDB_LOG_CTX_DEBUG(ctx, "FullScan complete at",
            {"TabletID", TabletID()});

        auto op = Pipeline.FindOp(cookie);
        if (op) {
            YDB_LOG_CTX_DEBUG(ctx, "Found op",
                {"cookie", cookie},
                {"at", TabletID()});
            ...
            if (InFlightCondErase && InFlightCondErase.TxId == cookie) {
                YDB_LOG_CTX_DEBUG(ctx, "Conditional erase complete",
                    {"cookie", cookie},
                    {"at", TabletID()});

                InFlightCondErase.Clear();
            } else if (CdcStreamScanManager.Has(cookie)) {
                YDB_LOG_CTX_DEBUG(ctx, "Cdc stream scan complete",
                    {"cookie", cookie},
                    {"at", TabletID()});

                CdcStreamScanManager.Complete(cookie);
            } else if (!Pipeline.FinishStreamingTx(cookie)) {
                YDB_LOG_CTX_ERROR(ctx, "Scan complete at for unknown tx",
                    {"TabletID", TabletID()},
                    {"cookie", cookie});
            }
        }
...
void TDataShard::Handle(TEvDataShard::TEvAsyncJobComplete::TPtr &ev, const TActorContext &ctx) {
    YDB_LOG_CTX_DEBUG(ctx, "AsyncJob complete at",
        {"TabletID", TabletID()});

    auto op = Pipeline.FindOp(ev->Cookie);
    if (op) {
        YDB_LOG_CTX_DEBUG(ctx, "Found op at cookie",
            {"TabletID", TabletID()},
            {"Cookie", ev->Cookie});
        ...
        YDB_LOG_CTX_ERROR(ctx, "AsyncJob complete at for unknown tx",
            {"TabletID", TabletID()},
            {"Cookie", ev->Cookie});
    }
...
void TDataShard::Handle(TEvPrivate::TEvRestartOperation::TPtr &ev, const TActorContext &ctx) {
    ...
    if (auto op = Pipeline.FindOp(txId)) {
        YDB_LOG_CTX_DEBUG(ctx, "Restart at",
            {"op", txId},
            {"TabletID", TabletID()});
