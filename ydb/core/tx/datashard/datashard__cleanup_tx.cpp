...
public:
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        if (!Self->IsStateActive()) {
            YDB_LOG_INFO_CTX(ctx, "Cleanup tx at non-ready tablet",
        ...
        switch (cleanupStatus) {
            ...
            case ECleanupStatus::Success:
                ...
                YDB_LOG_INFO_CTX(ctx, "Cleaned up old txs",
                    {"tabletId", Self->TabletID()},
        ...
        if (Self->Pipeline.CanRunAnotherOp()) {
            YDB_LOG_DEBUG_CTX(ctx, "Can run another op at tablet, scheduling plan queue progress",
                {"tabletId", Self->TabletID()});
...
public:
    ...
    bool Execute(TTransactionContext&, const TActorContext& ctx) override {
        if (!Self->IsStateActive()) {
            YDB_LOG_INFO_CTX(ctx, "Cleanup volatile tx at non-ready tablet",
                {"tabletId", Self->TabletID()},
                {"state", Self->State});
            return true;
        }
        ...
        if (Self->Pipeline.CleanupVolatile(TxId, ctx, Replies)) {
            YDB_LOG_INFO_CTX(ctx, "Cleaned up volatile tx",
                {"txId", TxId},
                {"tabletId", Self->TabletID()},
                {"txInFly", Self->TxInFly()});
            Self->IncCounter(COUNTER_TX_PROGRESS_CLEANUP);
