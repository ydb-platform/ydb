...
static bool MaybeReject(TDataShard* self, TEvRequest& ev, const TActorContext& ctx, const TString& txDesc, bool isWrite, TDataShard::ELogThrottlerType logThrottlerType) {
    ...
    if (self->CheckDataTxReject(txDesc, ctx, rejectStatus, rejectReason)) {
        Reject<TEvResponse, TEvRequest>(self, ev, txDesc, rejectReason, &WrongShardState, ctx, logThrottlerType);
        return true;
    }

    if (self->CheckChangesQueueOverflow()) {
        rejectReason = TStringBuilder() << "Change queue overflow at tablet " << self->TabletID();
        Reject<TEvResponse, TEvRequest>(self, ev, txDesc, rejectReason, &Overloaded, ctx, logThrottlerType);
        return true;
    }
    ...
    if (isWrite) {
        if (self->IsAnyChannelYellowStop()) {
            ...
            Reject<TEvResponse, TEvRequest>(self, ev, txDesc, rejectReason, &OutOfSpace, ctx, logThrottlerType);
            return true;
...
void TDataShard::Handle(TEvDataShard::TEvUploadRowsRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    if (IsReplicated()) {
        return Reject<TEvDataShard::TEvUploadRowsResponse>(this, ev, "bulk upsert",
            "Can't execute bulk upsert at replicated table", &ReadOnly, ctx, TDataShard::ELogThrottlerType::UploadRows_Reject);
    }
...
void TDataShard::Handle(TEvDataShard::TEvEraseRowsRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    if (IsReplicated()) {
        return Reject<TEvDataShard::TEvEraseRowsResponse>(this, ev, "erase",
            "Can't execute erase at replicated table", &ExecError, ctx, TDataShard::ELogThrottlerType::EraseRows_Reject);
    }
