...
TDataShard::TTxProposeTransactionBase::TTxProposeTransactionBase(
    TDataShard* self,
...
bool TDataShard::TTxProposeTransactionBase::Execute(
    ...
    try {
        ...
        if (Self->IsFollower()) {
            ...
            if (status != NKikimrTxDataShard::TError::OK) {
                LOG_LOG_S_THROTTLE(
                    ...
                    "Errors while proposing transaction txid " << TxId << " at tablet " << Self->TabletID()
                ...
                auto result = MakeHolder<TEvDataShard::TEvProposeTransactionResult>(
                    kind, Self->TabletID(), TxId, NKikimrTxDataShard::TEvProposeTransactionResult::ERROR
                );
        ...
        if (Ev) {
            ...
            TOperation::TPtr op = Self->Pipeline.BuildOperation(
                Ev, ReceivedAt, TieBreakerIndex, txc, ctx, std::move(DatashardTransactionSpan)
            );
