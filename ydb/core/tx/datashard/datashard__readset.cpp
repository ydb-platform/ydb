...
THolder<IEventHandle> TDataShard::TTxReadSet::MakeAck(const TActorContext& ctx) {
    return THolder(
        new IEventHandle(Ev->Sender, ctx.SelfID, new TEvTxProcessing::TEvReadSetAck(*Ev->Get(), Self->TabletID()))
    );
}
...
THolder<IEventHandle> TDataShard::TTxReadSet::MakeNoDataReply(const TActorContext& ctx) {
    ...
    auto event = MakeHolder<TEvTxProcessing::TEvReadSet>(
        record.GetStep(), record.GetTxId(), record.GetTabletDest(), record.GetTabletSource(), Self->TabletID()
    );
...
void TDataShard::TTxReadSet::Complete(const TActorContext& ctx) {
    ...
    if (Ack || NoDataReply) {
        ...
        struct TSendState: public TThrRefBase {
            ...
            TSendState(TDataShard* self, THolder<IEventHandle>&& ack, THolder<IEventHandle>&& noDataReply)
                : Self(self)
                , Ack(std::move(ack))
