...
/// Get
TDataShard::TTxGetS3Upload::TTxGetS3Upload(TDataShard* ds, TEvDataShard::TEvGetS3Upload::TPtr ev)
    : TBase(ds)
    , Ev(std::move(ev)) {}
...
/// Store id
TDataShard::TTxStoreS3UploadId::TTxStoreS3UploadId(TDataShard* ds, TEvDataShard::TEvStoreS3UploadId::TPtr ev)
    : TBase(ds)
    , Ev(std::move(ev)) {}
...
bool TDataShard::TTxChangeS3UploadStatus::Execute(TTransactionContext& txc, const TActorContext&) {
    ...
    auto& msg = *Ev->Get();
    const auto& upload =
        Self->S3Uploads.ChangeStatus(txc.DB, msg.TxId, msg.Status, std::move(msg.Error), std::move(msg.Parts));
    Reply.Reset(new TEvDataShard::TEvS3Upload(upload));
