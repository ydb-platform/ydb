...
public:
    ...
    const TSharedData* TryGetPage(const TPart* part, TPageId pageId, TGroupId groupId) override {
        ...
        if (msg->Status != NKikimrProto::OK) {
            LOG_ERROR_S(GetActorContext(), NKikimrServices::TX_DATASHARD, "[BuildStats] Failed to build at datashard "
...
void TDataShard::Handle(TEvPrivate::TEvAsyncTableStats::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!tableInfo.StatsUpdateInProgress) { // how can this happen?
        LOG_ERROR_S(ctx, NKikimrServices::TX_DATASHARD, "[BuildStats] Unexpected async stats update at datashard " << TabletID());
    }
...
void TDataShard::UpdateTableStats(const TActorContext &ctx) {
    ...
    LOG_DEBUG_S(ctx, NKikimrServices::TX_DATASHARD, "[BuildStats] UpdateTableStats at datashard " << TabletID());
...
void TDataShard::CollectCpuUsage(const TActorContext &ctx) {
    ...
    if (cpuPercent > CpuUsageReportThresholdPercent) {
        ...
        LOG_ERROR_S(ctx, NKikimrServices::TX_DATASHARD, "[BuildStats] CPU usage " << cpuPercent
