...
public:
    ...
    private:
        void Apply(NUdf::IApplyContext& applyContext) const override {
            ...
            Y_ENSURE(dsApplyCtx.ShardTableStats);
            Y_ENSURE(dsApplyCtx.TaskTableStats);
...
public:
    TKqpUpsertRowsWrapper(TComputationMutables& mutables, const TTableId& tableId, IComputationNode* rowsNode,
            ...
            TVector<ui32>&& keyIndices, TVector<TUpsertColumn>&& upsertColumns, NACLib::TUserContext::TPtr userCtx)
        : TBase(mutables)
        , TableId(tableId)
        , RowsNode(rowsNode)
...
IComputationNode* WrapKqpUpsertRows(TCallable& callable, const TComputationNodeFactoryContext& ctx,
    TKqpDatashardComputeContext& computeCtx, NACLib::TUserContext::TPtr userCtx)
{
