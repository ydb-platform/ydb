...
namespace NKikimr::NDataShard {
    ...
    TReplicationSourceState::TOffsetState* TReplicationSourceState::AddSplitKey(IReplicationSourceOffsetsDb& db, TSerializedCellVec key) {
        ...
        Y_VERIFY(res.second, "AddSplitKey used with a key that already exists");
        Y_VERIFY(res.first != Offsets.begin(), "AddSplitKey somehow added split key before the first key");
        auto& prev = **--res.first;
