...
void TReplicationSourceOffsetsDb::PersistSplitKey(
    const TPathId& pathId,
    ui64 sourceId,
    ui64 splitKeyId,
    const TString& splitKey,
    i64 offset
) {
    using Schema = TDataShard::Schema;
...
void TReplicationSourceOffsetsDb::PersistSplitKeyOffset(
    const TPathId& pathId,
    ui64 sourceId,
    ui64 splitKeyId,
    i64 offset
    ...
    db.Table<Schema::ReplicationSourceOffsets>()
        .Key(pathId.OwnerId, pathId.LocalPathId, sourceId, splitKeyId)
        .Update(NIceDb::TUpdate<Schema::ReplicationSourceOffsets::MaxOffset>(offset));
}
...
void TReplicationSourceOffsetsDb::PersistSplitKeyDelete(const TPathId& pathId, ui64 sourceId, ui64 splitKeyId) {
    ...
    db.Table<Schema::ReplicationSourceOffsets>().Key(pathId.OwnerId, pathId.LocalPathId, sourceId, splitKeyId).Delete();
}

bool TReplicationSourceState::TLessByKeyPrefix::operator()(TConstArrayRef<TCell> a, TConstArrayRef<TCell> b) const {
    return ComparePrefixBorders(*KeyColumnTypes, a, PrefixModeLeftBorderInclusive, b, PrefixModeLeftBorderInclusive) <
           0;
}
...
ui64 TReplicationSourceState::CalcStatBytes(const TOffsetState& state) const {
    return (
        0 // PathId
        + 8 // Id
        + 4 + Name.size() + 8 // SplitKeyId
        + 4 + state.SplitKey.GetBuffer().size() + 8 // MaxOffset
    );
}
...
TReplicationSourceState::TOffsetState*
TReplicationSourceState::AddSplitKey(IReplicationSourceOffsetsDb& db, TSerializedCellVec key) {
...
TReplicationSourceState::TOffsetState*
TReplicationSourceState::AddSplitKey(IReplicationSourceOffsetsDb& db, TSerializedCellVec key, i64 offset) {
    const ui64 splitKeyId = NextSplitKeyId++;
    auto& state = OffsetBySplitKeyId[splitKeyId];
    state.SplitKeyId = splitKeyId;
    state.SplitKey = std::move(key);
...
bool TReplicationSourceState::EnsureSplitKey(
    IReplicationSourceOffsetsDb& db,
    const TSerializedCellVec& key,
    i64 offset
) {
...
TReplicatedTableState::TReplicatedTableState(const TPathId& pathId, const TVector<NScheme::TTypeInfo>& keyColumnTypes)
    : PathId(pathId)
...
TReplicatedTableState::EnsureSource(IReplicationSourceOffsetsDb& db, const TString& sourceName) {
