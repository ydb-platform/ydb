...
public:
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        YDB_LOG_CTX_DEBUG(ctx, "received split OpId at state",
    ...
    void Complete(const TActorContext &ctx) override {
        ...
        if (SplitAlreadyFinished) {
            ...
            for (const TActorId& ackTo : Self->SrcAckSplitTo) {
                YDB_LOG_CTX_DEBUG(ctx, "ack split to schemeshard",
                    {"TabletID", Self->TabletID()},
...
public:
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        YDB_LOG_CTX_DEBUG(ctx, "starting snapshot for split OpId",
            {"TabletID", Self->TabletID()},
            {"opId", opId});
...
public:
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ui64 opId = Self->SrcSplitOpId;
        YDB_LOG_CTX_DEBUG(ctx, "snapshot complete for split OpId",
            {"TabletID", Self->TabletID()},
            {"opId", opId});
        ...
        for (ui32 i = 0; i < Self->SrcSplitDescription->DestinationRangesSize(); ++i) {
            ...
            for (ui32 localTableId : SnapContext->TablesToSnapshot()) {
                ...
                if (snapBody.empty()) {
                    YDB_LOG_CTX_DEBUG(ctx, "BorrowSnapshot needs to load pages for table for split OpId",
                        {"TabletID", Self->TabletID()},
                    ...
                    YDB_LOG_CTX_DEBUG(ctx, "BorrowSnapshot: table snapshot size is total snapshot size is for split OpId",
                        {"TabletID", Self->TabletID()},
                        {"localTableId", localTableId},
                        {"size", snapBody.size()},
        ...
        if (needToReadPages) {
            YDB_LOG_CTX_DEBUG(ctx, "BorrowSnapshot is restarting for split OpId",
                {"TabletID", Self->TabletID()},
                {"opId", opId});
            return false;
        } else {
    ...
    void Complete(const TActorContext &ctx) override {
        YDB_LOG_CTX_DEBUG(ctx, "Sending snapshots from src for split OpId",
            {"TabletID", Self->TabletID()},
            {"SrcSplitOpId", Self->SrcSplitOpId});
...
public:
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        YDB_LOG_CTX_DEBUG(ctx, "Received snapshot Ack from dst for split OpId",
            {"TabletID", Self->TabletID()},
    ...
    void Complete(const TActorContext &ctx) override {
        for (const auto& [ackTo, opIds] : Waiters) {
            for (const ui64 opId : opIds) {
                YDB_LOG_CTX_DEBUG(ctx, "ack split partitioning changed to schemeshard",
                    {"TabletID", Self->TabletID()},
                    {"opId", opId});
...
void TDataShard::Handle(TEvDataShard::TEvSplitPartitioningChanged::TPtr& ev, const TActorContext& ctx) {
    ...
    YDB_LOG_CTX_DEBUG(ctx, "Got TEvSplitPartitioningChanged",
        {"opId", opId},
    ...
    if (ChangesQueue || !ChangeSenderActivator.AllAcked()) {
        YDB_LOG_CTX_NOTICE(ctx, "delay partitioning changed ack, ChangesQueue, siblings to be",
            {"TabletID", TabletID()},
