    ...
    const TArrayRef<const NIceDb::TUpdateOp> ops) 
{
    ...
    if (LockTxId) {
        ...
        NDataIntegrity::LogIntegrityTrailsLocks(*TlsActivationContext, Self.TabletID(), GlobalTxId, brokenLocks);
    }
...
void TDataShardUserDb::BreakWriteConflict(ui64 txId) {
    if (VolatileCommitTxIds.contains(txId)) {
        ...
        NDataIntegrity::LogIntegrityTrailsLocks(*TlsActivationContext, Self.TabletID(), GlobalTxId, {txId});
    }
}
