...
bool HasIssue(
    ...
    ui32 code,
    TStringBuf message,
    std::function<bool(const TIssue& issue)> predicate = {}
) {
    bool hasIssue = false;
...
Y_UNIT_TEST_SUITE(KqpErrors) {
    ...
    Y_UNIT_TEST(ProposeError) {
        ...
        auto test = [&](auto proposeStatus, auto ydbStatus, auto issue, auto issueMessage, TMod mod = {}) {
            ...
            SendRequest(
                *fixture.Runtime, client, MakeSQLRequest(Q_("upsert into `/Root/table-1` (key, value) values (5, 5);"))
            );
            ...
            IssuesFromMessage(record.GetResponse().GetQueryIssues(), issues);
            UNIT_ASSERT_C(
                HasIssue(issues, issue, issueMessage),
                "issue not found, issue: " << (int)issue << ", message: " << issueMessage
                                           << ", response: " << record.GetResponse().DebugString()
            );
        };

        test(
            TEvProposeTransactionResult::OVERLOADED, // propose error
            Ydb::StatusIds::OVERLOADED, // ydb status
            NYql::TIssuesIds::KIKIMR_OVERLOADED, // issue status
            "Kikimr cluster or one of its subsystems is overloaded."
        ); // main issue message (more detailed info can be in subissues)

        test(
            TEvProposeTransactionResult::ABORTED,
        ...
        test(
            TEvProposeTransactionResult::TRY_LATER,
        ...
        test(
            TEvProposeTransactionResult::RESULT_UNAVAILABLE,
        ...
        test(
            TEvProposeTransactionResult::CANCELLED,
        ...
        test(
            TEvProposeTransactionResult::BAD_REQUEST,
        ...
        test(
            TEvProposeTransactionResult::ERROR,
            Ydb::StatusIds::UNAVAILABLE,
            NYql::TIssuesIds::KIKIMR_TEMPORARILY_UNAVAILABLE,
            "Kikimr cluster or one of its subsystems was unavailable."
        );
        ...
        test(
            TEvProposeTransactionResult::EXEC_ERROR,
        ...
        test(
            TEvProposeTransactionResult::RESPONSE_DATA,
            Ydb::StatusIds::GENERIC_ERROR,
            NYql::TIssuesIds::DEFAULT_ERROR,
        ...
        TestProposeResultLost(
            *fixture.Runtime,
            ...
            [](const NKikimrKqp::TEvQueryResponse& record) {
                ...
                UNIT_ASSERT_C(
                    HasIssue(
                        issues, NYql::TIssuesIds::KIKIMR_OPERATION_STATE_UNKNOWN, "State of operation is unknown."
                    ),
                ...
                UNIT_ASSERT_C(
                    HasIssue(
                        issues,
                        NKikimrIssues::TIssuesIds::TX_STATE_UNKNOWN,
                        "",
                        [](const TIssue& issue) {
