...
public:
    ...
    static ui64 TabletID() {
        return 0;
    }
    static ui32 Generation() {
        return 0;
    }
    TRowVersion LastCompleteTxVersion() const {
    ...
    template <typename T>
    void SetCounter(T, ui64) const {}
    template <typename T>
    void IncCounter(T, ui64 num = 1) const {
        Y_UNUSED(num);
    }
    template <typename T>
    void IncCounter(T, const TDuration&) const {}

private:
    NTable::TScheme Schema;

    static ui32 NumSysTables() {
        return 10;
    }
...
public:
    ...
    struct TRangeKey {
        ...
        TTableRange GetRowsRange() const {
            return TTableRange(
                TArrayRef<const TCell>(&Cells[0], 1),
                Flags & EIncFrom,
                TArrayRef<const TCell>(&Cells[0], 1),
                Flags & EIncTo
            );
...
private:
    ...
    static TTableId LocksTableId() {
        return TTableId(TSysTables::SysSchemeShard, TSysTables::SysTableLocks);
    }
};

template <typename T>
static void EmulateTx(
    TLockTester& tester,
    ui64 lockTxId,
    const TVector<T>& selects,
    const TVector<T>& updates,
    const TVector<std::pair<T, T>>& rangeSelects,
    ui32 rangeFlags = 0
) {
...
Y_UNIT_TEST_SUITE(TDataShardLocksTest) {
    Y_UNIT_TEST(MvccTestOooTxDoesntBreakPrecedingReadersLocks) {
        ...
        // lock tx
            ...
            for (auto lock : tester.ApplyTxLocks()) UNIT_ASSERT(!lock.IsError());
        }
    ...
    Y_UNIT_TEST(Points_OneTx) {
        ...
        std::function<void(ui32&)> fInc = [](ui32& val) {
            ++val;
        };
    ...
    // Create table with two shards.
        ui64 txId;
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                reply->Record.GetStatus(), TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecInProgress
            );
        ...
        {
            auto request =
                MakeSQLRequest("UPSERT INTO `/Root/table-1` (key, value) VALUES (1,0x80000002),(0x80000001,2)");
...

        {
            auto request = MakeSQLRequest(
                "UPSERT INTO `/Root/table-1` (key, value) SELECT value as key, value FROM `/Root/table-1`"
            );
            runtime.Send(new IEventHandle(NKqp::MakeKqpProxyID(runtime.GetNodeId()), sender, request.Release()));
        ...
        // Get shard IDs.
            ui64 shards[2];
            {
                ...
                for (auto i = 0; i < 2; ++i)
                    shards[i] = reply->GetRecord().GetPathDescription().GetTablePartitions(i).GetDatashardId();
            }
...

        {
            ...
            // So either 2 or 4 records are allowed.
                UNIT_ASSERT(resp.GetYdbResults(0).rows_size() == 4 || resp.GetYdbResults(0).rows_size() == 2);
            }
        }
    }
