...
//
    ...
    Y_UNIT_TEST(ReadSpecialColumns) {
...

        {
            ...
            UNIT_ASSERT(
                row1["__tablet"].HaveValue() && row1["__updateNo"].HaveValue() && row1["__updateEpoch"].HaveValue()
            );
            UNIT_ASSERT(
                row2["__tablet"].HaveValue() && row2["__updateNo"].HaveValue() && row2["__updateEpoch"].HaveValue()
            );
            UNIT_ASSERT(
                row3["__tablet"].HaveValue() && row3["__updateNo"].HaveValue() && row3["__updateEpoch"].HaveValue()
            );

            Cout << "row1 tablet: " << (ui64)row1["__tablet"] << " update: " << (ui64)row1["__updateEpoch"] << '.'
                 << (ui64)row1["__updateNo"] << Endl;
            Cout << "row2 tablet: " << (ui64)row2["__tablet"] << " update: " << (ui64)row1["__updateEpoch"] << '.'
                 << (ui64)row2["__updateNo"] << Endl;
            Cout << "row3 tablet: " << (ui64)row3["__tablet"] << " update: " << (ui64)row1["__updateEpoch"] << '.'
                 << (ui64)row3["__updateNo"] << Endl;
...
//
    ...
    void GetTableStats(
        TTestActorRuntime & runtime, ui64 tabletId, ui64 tableId, NKikimrTableStats::TTableStats & stats
    ) {
        ...
        TEvDataShard::TEvGetTableStatsResult* response =
            runtime.GrabEdgeEventRethrow<TEvDataShard::TEvGetTableStatsResult>(handle);
    ...
    Y_UNIT_TEST(TableStats) {
        ...
        UNIT_ASSERT_C(
            statsAfterUpdate.GetIndexSize() > 0 || statsAfterUpdate.GetInMemSize() > 0, "Table shoud not be empty"
        );
        ...
        UNIT_ASSERT_VALUES_EQUAL_C(
            statsAfterUpdate.GetLastAccessTime(),
            statsAfterUpdate.GetLastUpdateTime(),
    ...
    // Non-existing table
        ...
        UNIT_ASSERT_VALUES_EQUAL_C(
            statsAfterRead.GetLastUpdateTime(),
            statsAfterUpdate.GetLastUpdateTime(),
            "LastUpdateTime shoud not change after read"
        );
        UNIT_ASSERT_C(
            statsAfterRead.GetLastAccessTime() > statsAfterUpdate.GetLastAccessTime(),
...
//
    ...
    void CrossShard_2_SwapAndCopy_Impl(
        const TString& dispatchName, std::function<void(TTestActorRuntime&)> setup, bool& activeZone
    ) {
        TTester t(TTester::ESchema_MultiShardKV, dispatchName, setup, activeZone);
        TFakeMiniKQLProxy proxy(t);
...
//
    ...
    void CrossShard_3_AllToOne_Impl(
        const TString& dispatchName, std::function<void(TTestActorRuntime&)> setup, bool& activeZone
    ) {
        TTester t(TTester::ESchema_MultiShardKV, dispatchName, setup, activeZone);
        TFakeMiniKQLProxy proxy(t);
...
//
    ...
    void CrossShard_4_OneToAll_Impl(
        const TString& dispatchName, std::function<void(TTestActorRuntime&)> setup, bool& activeZone
    ) {
        TTester t(TTester::ESchema_MultiShardKV, dispatchName, setup, activeZone);
        TFakeMiniKQLProxy proxy(t);
...
//
    ...
    void CrossShard_5_AllToAll_Impl(
        const TString& dispatchName, std::function<void(TTestActorRuntime&)> setup, bool& activeZone
    ) {
        TTester t(TTester::ESchema_MultiShardKV, dispatchName, setup, activeZone);
        TFakeMiniKQLProxy proxy(t);
...
//
    ...
    Y_UNIT_TEST(WriteKeyTooLarge) {
        ...
        UNIT_ASSERT_EQUAL(
            proxy.Execute(Sprintf(progUpsert, 123u, key2.c_str(), "foo"), /* wait */ false), IEngineFlat::EStatus::Error
        );
    }
    ...
    Y_UNIT_TEST(WriteValueTooLarge) {
        ...
        UNIT_ASSERT_EQUAL(
            proxy.Execute(Sprintf(progUpsert, 123u, "foo", value.c_str()), /* wait */ false),
            IEngineFlat::EStatus::Error
        );
    }
    ...
    // Would write an external blob
        ...
        UNIT_ASSERT_EQUAL(
            proxy.Execute(Sprintf(progUpsert, 123u, "foo", value.c_str()), res1), IEngineFlat::EStatus::Complete
        );
    ...
    // Would write an inline value (too large for an external blob)
        ...
        UNIT_ASSERT_EQUAL(
            proxy.Execute(Sprintf(progUpsert, 234u, "bar", value.c_str()), res2), IEngineFlat::EStatus::Complete
        );
    }
    ...
    void CheckCounters(
        const std::tuple<ui64, ui64, ui64>& newCnt,
        const std::tuple<ui64, ui64, ui64>& oldCnt,
        const TCounter1& d1,
        const TCounter2& d2,
        const TCounter3& d3
    ) {
