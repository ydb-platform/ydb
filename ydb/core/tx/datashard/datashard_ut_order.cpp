    ...
    // strict ordered txs: {0->1001, 1000->1} {1->1002, 1001->2}
        ...
        // relaxed order: 20->1031, 1020->31
            for (ui32 shift = 0; shift < limit - 10; shift += 10) {
                for (ui32 i = 0; i < 10; ++i) {
                    TString programText =
                        Sprintf(zigzag, shift + i, 1000 + shift + i, shift + i + 11, 1000 + shift + i + 11);
                //UNIT_ASSERT_EQUAL(proxy.Execute(programText), IEngineFlat::EStatus::Complete);
...
//
    ...
    static void EvictShardCache(TFakeMiniKQLProxy & proxy, ui32 count = 500) {
        ...
        for (ui32 i = 0; i < count; ++i) {
            UNIT_ASSERT_EQUAL(
                proxy.Execute(Sprintf(programWrite, (3 * i) % 1000, text)), IEngineFlat::EStatus::Complete
            );
        }
    ...
    // Does not conflict on the first shard and will be executed out of order
        ...
        proxy.Enqueue(
            Sprintf(programReadKeys3, 1, 1000, 3),
            [&](TFakeProxyTx& tx) -> bool {
    ...
    // Read the final state of important keys
        ...
        proxy.Enqueue(
            Sprintf(programReadKeys4, 0, 1, 2, 1000),
            [&](TFakeProxyTx& tx) -> bool {
...
//
    ...
    static TString MkRandomTx(
        ui64 txId, ui32 points, ui32 rw, ui32 keysCount, TVector<ui32> & expected, bool range = false
    ) {
        UNIT_ASSERT(keysCount <= 32);
        Cout << "tx " << txId << ' ' << Bin(points) << ' ' << Bin(points & rw) << " (" << points << '/' << rw << ')'
             << Endl;
    ...
    static void CompareShots(
        const TVector<ui32>& finalShot, const TVector<ui32>& intermShot, std::pair<ui32, ui32> range
    ) {
    ...
    static void RandomTxDeps(
        const TTester::TOptions& opts,
        ui32 numTxs,
        ui32 maxKeys,
        bool lessWrites,
    ...
    // must be online
        ...
        if (sendImmediates) {
            for (const auto& s : shots) CompareShots(expected, intermediate, s);
        }
    }

//

    static constexpr ui32 NumRun() {
        return 2;
    }
    ...
    Y_UNIT_TEST(RandomPoints_ReproducerDelayRS1) {
        ...
        RandomTxDeps(
            opts, 8, 8, true, false, {2, 7, 7, 7, 6, 3, 2, 4}, {2, 66, 30, 104, 25, 4, 0, 1}, {0, 40, 2, 33, 8, 4, 1, 3}
        );
    }
...
///
    ...
    public:
        ...
        const TVector<ui32>& GetWrites() const {
            return Writes;
        }
        const TVector<ui32>& GetReads() const {
            return Reads;
        }
        const TVector<TSimpleRange>& GetRanges() const {
            return Ranges;
        }
        const TMap<ui32, ui32>& GetResults() const {
            return Results;
        }
        ...
        void SetResults(const TVector<ui32>& kv) {
            ...
            for (const auto& r : Ranges) {
                for (ui32 i = r.From; i <= r.To; ++i) points.insert(i);
            }
...
///
    ...
    void Print(const TMap<ui32, ui32>& m) {
        for (auto& pair : m) Cerr << pair.first << ':' << pair.second << ' ';
        Cerr << Endl;
    }
    ...
    void RandomPointsAndRanges(TFakeMiniKQLProxy & proxy, ui32 numTxs, ui32 maxWrites, ui32 maxReads, ui32 maxRanges) {
        ...
        for (auto tx : txs) {
            tx->Generate(
                RandomNumber<ui32>(maxWrites - 1) + 1,
    ...
    static void RandomPointsAndRanges(
        const TTester::TOptions& opts, ui32 numTxs, ui32 maxWrites, ui32 maxReads, ui32 maxRanges
    ) {
        TTester t(TTester::ESchema_MultiShardKV, opts);
        TFakeMiniKQLProxy proxy(t);
...
Y_UNIT_TEST_SUITE(DataShardScan) {
    Y_UNIT_TEST(ScanFollowedByUpdate) {
        ...
        proxy.EnqueueScan(
            ...
            [ref1, checkScanResult](TFakeProxyTx& tx) {
        ...
        proxy.EnqueueScan(
            dataTransaction.SerializeAsString(),
            [ref2, checkScanResult](TFakeProxyTx& tx) {
...
Y_UNIT_TEST_SUITE(DataShardOutOfOrder) {
    ...
    // However the above commit must succeed (readsets are already being exchanged)
        auto f3 = SendRequest(
            runtime,
            MakeSimpleRequestRPC(Q_("UPSERT INTO `/Root/table-1` (key, value) VALUES (5, 3)"), sender3Session, "", true)
        );
    ...
    // Select keys 3 and 4 from both tables, either both or none should be inserted
        {
            ...
            if (committed) {
                expected =
                    "{ items { uint32_value: 3 } items { uint32_value: 2 } }, { items { uint32_value: 4 } items { "
                    "uint32_value: 2 } }";
            } else {
                expected = "";
            }
    ...
    Y_UNIT_TEST_QUAD(TestOutOfOrderNonConflictingWrites, StreamLookup, EvWrite) {
        ...
        app.MutableTableServiceConfig()->SetEnableKqpDataQueryStreamLookup(StreamLookup);
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetAppConfig(app).SetUseRealThreads(false);
    ...
    // to 3 and 5 already happened, but physically write to 3 is still waiting.
        {
            ...
            auto req = MakeSimpleRequestRPC(
                Q_("SELECT key, value FROM `/Root/table-1` WHERE key = 3;"), tmpSessionId, "", true
            );
    ...
    // Wait until we captured both readsets again
        ...
        UNIT_ASSERT_C(
            readSets.size() >= expectedReadSets, "expected " << readSets.size() << " >= " << expectedReadSets
        );
    ...
    // Note: volatile upserts are not blocked however, so it will succeed
        {
            ...
            auto req =
                MakeSimpleRequestRPC(Q_("SELECT key, value FROM `/Root/table-1` WHERE key = 7;"), sender4, "", true);
            req.mutable_operation_params()->mutable_operation_timeout()->set_seconds(1);
            auto response = AwaitResponse(runtime, SendRequest(runtime, std::move(req)));
            UNIT_ASSERT_VALUES_EQUAL(
                response.operation().status(), usesVolatileTxs ? Ydb::StatusIds::SUCCESS : Ydb::StatusIds::TIMEOUT
            );
        }
    ...
    // Note: volatile upserts are not blocked however, so it will succeed
        {
            ...
            auto req =
                MakeSimpleRequestRPC(Q_("SELECT key, value FROM `/Root/table-1` WHERE key = 7;"), sender5, "", true);
            req.mutable_operation_params()->mutable_operation_timeout()->set_seconds(1);
            auto response = AwaitResponse(runtime, SendRequest(runtime, std::move(req)));
            UNIT_ASSERT_VALUES_EQUAL(
                response.operation().status(), usesVolatileTxs ? Ydb::StatusIds::SUCCESS : Ydb::StatusIds::TIMEOUT
    ...
    void AsyncReadTable(Tests::TServer::TPtr server, TActorId sender, const TString& path) {
        auto& runtime = *server->GetRuntime();
 ...
 */
    ...
    // Send a commit request, it would block on readset exchange
        auto qCommit = SendRequest(
            runtime,
            MakeSimpleRequestRPC(
                Q_("UPSERT INTO `/Root/table-1` (key, value) VALUES (3, 2); "
                   "UPSERT INTO `/Root/table-2` (key, value) VALUES (4, 2); "),
                sessionId,
                txId,
                true
    ...
    // because it arrived after the read table and they block each other
        auto fWriteImm = SendRequest(
            runtime,
            MakeSimpleRequestRPC(Q_("UPSERT INTO `/Root/table-1` (key, value) VALUES (5, 3)"), senderWriteImm, "", true)
        );

    // Start a planned write to both tables, wait for its plan step too
        auto fWriteDist = SendRequest(
            runtime,
            MakeSimpleRequestRPC(
                Q_("UPSERT INTO `/Root/table-1` (key, value) VALUES (7, 4); "
 ...
 */
    ...
    // Send a lot of write requests in parallel (so there's a large propose queue)
        for (int i = 0; i < totalWrites; ++i) {
            ...
            SendSQL(
                server, writeSender, Q_(Sprintf("UPSERT INTO `/Root/table-1` (key, value) VALUES (%d, %d);", i, i))
            );
    ...
    // Split shouldn't take too much time to complete
        ...
        UNIT_ASSERT_C(
            elapsed < TDuration::Seconds(NValgrind::PlainOrUnderValgrind(5, 25)),
    ...
    // We expect all transactions to fail
        UNIT_ASSERT_C(
            successes + failures == totalWrites,
            "Unexpected " << successes << " successes and " << failures << " failures"
        );
    }
    ...
    void TestLateKqpQueryAfterColumnDrop(bool dataQuery, const TString& query) {
        ...
        CreateShardedTable(
            ...
            TShardedTableOptions().Columns(
                {{"key", "Uint32", true, false}, {"value1", "Uint32", false, false}, {"value2", "Uint32", false, false}}
            )
        );
    ...
    Y_UNIT_TEST(TestSecondaryClearanceAfterShardRestartRace) {
        ...
        auto state =
            StartReadShardedTable(server, "/Root/table-1", TRowVersion::Max(), /* pause */ true, /* ordered */ false);

        waitFor(
            [&] {
                return seenStreamClearanceResponses >= 2;
            },
            "observed TEvStreamClearanceResponse"
        );
        ...
        waitFor(
            [&] {
                return capturedDeliveryProblem.size() >= 1;
            },
            "intercepted TEvDeliveryProblem"
        );
        waitFor(
            [&] {
                return seenStreamClearanceRequests >= 1;
            },
            "observed TEvStreamClearanceRequest"
        );
    ...
    Y_UNIT_TEST_TWIN(TestShardRestartNoUndeterminedImmediate, StreamLookup) {
        ...
        {
            auto result =
                KqpSimpleExec(runtime, Q_(R"(SELECT key, value FROM `/Root/table-1` WHERE key = 1 ORDER BY key)"));
            UNIT_ASSERT_VALUES_EQUAL(result, "{ items { uint32_value: 1 } items { uint32_value: 1 } }");
        }
    }
    ...
    Y_UNIT_TEST_QUAD(TestShardRestartPlannedCommitShouldSucceed, StreamLookup, EvWrite) {
        ...
        // Select key 3 and verify its value was updated
        {
            ...
            auto result =
                KqpSimpleExec(runtime, Q_(R"(SELECT key, value FROM `/Root/table-1` WHERE key = 3 ORDER BY key)"));
            UNIT_ASSERT_VALUES_EQUAL(result, "{ items { uint32_value: 3 } items { uint32_value: 2 } }");
        }
    }
    ...
    Y_UNIT_TEST(TestShardSnapshotReadNoEarlyReply) {
        ...
        waitFor(
            [&] {
                return blockedCommits.size() >= 2;
            },
            "at least 2 blocked commits"
        );
        ...
        SendRequest(
            runtime,
            MakeSimpleRequestRPC(Q_("UPSERT INTO `/Root/table-1` (key, value) VALUES (3, 3)"), sq1, "", true),
            "/Root"
        );
        SendRequest(
            runtime,
            MakeSimpleRequestRPC(Q_("UPSERT INTO `/Root/table-2` (key, value) VALUES (4, 4)"), sq2, "", true),
            "/Root"
        );
        waitFor(
            [&] {
    ...
    Y_UNIT_TEST(TestSnapshotReadPriority) {
        ...
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetControls(controls).SetUseRealThreads(false);
        ...
        // Spam schedules in the runtime to prevent mediator time jumping prematurely
        {
            ...
            for (int i = 1; i <= 10; ++i) {
                runtime.Schedule(
                    new IEventHandle(senderWakeupSpam, senderWakeupSpam, new TEvents::TEvWakeup()),
                    TDuration::MicroSeconds(i * 250)
                );
            }
        }

        // Send an immediate write transaction, but don't wait for result
        auto fImmWrite = SendRequest(
            runtime,
            MakeSimpleRequestRPC(
                Q_(R"(
        UPSERT INTO `/Root/table-1` (key, value) VALUES (5, 5)
        )"),
                senderImmediateWrite,
                "",
                true
            )
        );
        ...
        auto fImmWrite2 = SendRequest(
            runtime,
            MakeSimpleRequestRPC(
                Q_(R"(
        UPSERT INTO `/Root/table-1` (key, value) VALUES (9, 9)
        )"),
                senderImmediateWrite2,
                "",
                true
            )
        );
    ...
    Y_UNIT_TEST(TestUnprotectedReadsThenWriteVisibility) {
        ...
        TServerSettings::TControls controls;
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetNodeCount(2).SetControls(controls).SetUseRealThreads(false);
        ...
        auto captureTimecast = [&](TAutoPtr<IEventHandle>& ev) -> auto {
            ...
            switch (ev->GetTypeRewrite()) {
                case TEvMediatorTimecast::TEvUpdate::EventType: {
                    ...
                    Cerr << "... node " << nodeId << " observed update from " << mediatorId << " for bucket " << bucket
                         << " to step " << step << " (allowed " << state.AllowedStep << ")" << Endl;
                ...
                case TEvMediatorTimecast::TEvWaitPlanStep::EventType: {
                    ...
                    Cerr << "... node " << nodeId << " observed wait by " << tabletId << " for step " << step << Endl;
                    auto& state = mediatorState[nodeId];
                        ...
                        for (auto& kv1 : state.Steps) {
                            ...
                            for (auto& kv2 : kv1.second) {
                                ...
                                if (passedStep < step && passedStep < observedStep) {
                                    ...
                                    runtime.Send(
                                        new IEventHandle(ev->GetRecipientRewrite(), ev->GetRecipientRewrite(), update),
                                        nodeIndex,
                                        /* viaActorSystem */ true
                                    );
                                }
    ...
    Y_UNIT_TEST(UncommittedReadSetAck) {
        ...
        waitFor(
            [&] {
                return capturedPlanTxIds.size() >= 2;
            },
        ...
        waitFor(
            [&] {
                return capturedReadSets.size() >= 2;
            },
            "initial readsets"
        );
        ...
        waitFor(
            [&] {
                return capturedCommits.size() > 0 && capturedReadSets.size() > 0;
            },
            "tx complete"
        );
        SimulateSleep(runtime, TDuration::MilliSeconds(5));
        ...
        waitFor(
            [&] {
                return capturedCommits.size() > prevCommitsBlocked && observedReadSets >= 1;
            },
            "second tx complete"
        );
        SimulateSleep(runtime, TDuration::MilliSeconds(5));
        ...
        for (auto& ev : capturedCommits) {
            ...
            auto res = ev->Get<TEvBlobStorage::TEvPut>()->MakeErrorResponse(
