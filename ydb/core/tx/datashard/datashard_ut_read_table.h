...
public:
    ...
    STRICT_STFUNC(
        ...
        HFunc(TEvTxUserProxy::TEvProposeTransactionStatus, Handle) HFunc(TEvTxProcessing::TEvStreamQuotaRequest, Handle)
            HFunc(TEvTxProcessing::TEvStreamQuotaRelease, Handle) HFunc(TEvNext, Handle)
    )
...
inline NTxProxy::TReadTableSettings
MakeReadTableSettings(const TString& path, bool ordered = false, ui64 maxRows = Max<ui64>()) {
