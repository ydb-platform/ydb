...
Y_UNIT_TEST_SUITE(DataShardReplication) {
    Y_UNIT_TEST(SimpleApplyChanges) {
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(result, "key = 1, value = 11\n");
        }
...

        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(result, "key = 4, value = 44\n");
        }
    }
    ...
    Y_UNIT_TEST(ApplyChangesToReplicatedTable) {
        ...
        CreateShardedTable(
            ...
            "/Root",
            "table-1",
            TShardedTableOptions().Replicated(true).ReplicationConsistency(EReplicationConsistency::Weak)
        );
