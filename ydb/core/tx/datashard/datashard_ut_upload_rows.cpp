...
static void
DoWaitUploadTestRows(const Tests::TServer::TPtr& server, const TActorId& sender, Ydb::StatusIds::StatusCode expected) {
    auto& runtime = *server->GetRuntime();
...
static TActorId DoStartUploadRows(
    ...
    auto actor = NTxProxy::CreateUploadRowsInternal(sender, tableName, std::move(types), std::move(rows), mode);
    runtime.Register(actor);
...
Y_UNIT_TEST_SUITE(TTxDataShardUploadRows) {
    ...
    Y_UNIT_TEST(TestUploadRowsDropColumnRace) {
        ...
        waitFor(
            [&] {
                return uploadRequests.size() >= 3;
            },
            "TEvUploadRowsRequest"
        );
    ...
    Y_UNIT_TEST_TWIN(TestUploadRowsLocks, StreamLookup) {
        ...
        TPortManager pm;
        TServerSettings serverSettings(pm.GetPort(2134));
        serverSettings.SetDomainName("Root").SetAppConfig(appConfig).SetUseRealThreads(false);
        ...
        TString txId;
        {
            auto result = KqpSimpleBegin(runtime, sessionId, txId, "SELECT value FROM `/Root/table-1` WHERE key = 3");
        ...
        {
            auto result =
                KqpSimpleCommit(runtime, sessionId, txId, "UPSERT INTO `/Root/table-1` (key, value) VALUES (6, 600);");
    ...
    Y_UNIT_TEST(TestUploadShadowRows) {
        ...
        // Write shadow data: keys from 1 to 9 historically had value=key*10
        {
            ...
            auto actor = NTxProxy::CreateUploadRowsInternal(
                sender, "/Root/table-1", std::move(types), rows, NTxProxy::EUploadRowsMode::WriteToTableShadow
            );
            runtime.Register(actor);
    ...
    Y_UNIT_TEST(TestUploadShadowRowsShadowData) {
        ...
        WaitTxNotification(server, AsyncAlterAndDisableShadow(server, "/Root", "table-1", policy.Get()));
    ...
    Y_UNIT_TEST(TestUploadShadowRowsShadowDataSplitThenPublish) {
        ...
        // Writes to shadow data should not be visible yet
        UNIT_ASSERT_VALUES_EQUAL(
            ReadShardedTable(server, "/Root/table-1"),
            "key = 1, value = 100\n"
            "key = 3, value = 300\n"
            "key = 6, value = (empty maybe)\n"
            "key = 7, value = (empty maybe)\n"
    ...
    Y_UNIT_TEST(TestUploadShadowRowsShadowDataAlterSplitThenPublish) {
        ...
        WaitTxNotification(server, AsyncAlterAddExtraColumn(server, "/Root", "table-1"));
    ...
    void DoShouldRejectOnChangeQueueOverflow(bool overloadSubscribe) {
        ...
        auto observerRequestHandler =
            runtime.AddObserver<TEvDataShard::TEvUploadRowsRequest>([&overloadSubscribe](auto& ev) {
                if (!overloadSubscribe) {
                    ev->Get()->Record.ClearOverloadSubscribe();
                }
            });

        auto observerResponseHandler =
            runtime.AddObserver<TEvDataShard::TEvUploadRowsResponse>([&observedUploadStatus](auto& ev) {
    ...
    Y_UNIT_TEST(BulkUpsertDuringAddIndexRaceCorruption) {
        ...
        auto captureBulkUpserts =
            runtime.AddObserver<TEvDataShard::TEvUploadRowsRequest>([&](TEvDataShard::TEvUploadRowsRequest::TPtr& ev) {
        ...
        WaitFor(
            runtime,
            [&] {
                return bulkUpserts.size() > 0;
            },
            "captured bulk upsert"
        );
