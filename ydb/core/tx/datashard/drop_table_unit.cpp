...
public:
    TDropTableUnit(TDataShard& dataShard, TPipeline& pipeline);
...
TDropTableUnit::TDropTableUnit(TDataShard& dataShard, TPipeline& pipeline)
...
EExecutionStatus TDropTableUnit::Execute(TOperation::TPtr op, TTransactionContext& txc, const TActorContext& ctx) {
    TActiveTransaction* tx = dynamic_cast<TActiveTransaction*>(op.Get());
    Y_VERIFY_S(tx, "cannot cast operation of kind " << op->GetKind());
...
void TDropTableUnit::Complete(TOperation::TPtr, const TActorContext& ctx) {
