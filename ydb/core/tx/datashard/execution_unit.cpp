...
bool TExecutionUnit::CheckRejectDataTx(TOperation::TPtr op, const TActorContext& ctx) {
    ...
    if (DataShard.GetState() == TShardState::SplitSrcWaitForNoTxInFlight ||
        ...
        TString err = TStringBuilder() << "Wrong shard state: " << (TShardState)DataShard.GetState()
    ...
    if (DataShard.GetState() != TShardState::Ready && DataShard.GetState() != TShardState::Readonly &&
    ...
    if (DataShard.IsStopping()) {
        TString err = TStringBuilder() << "Tablet " << DataShard.TabletID() << " is restarting";
    ...
    if (!op->IsReadOnly() && DataShard.CheckChangesQueueOverflow()) {
        TString err = TStringBuilder() << "Can't execute at blocked shard: " << " tablet id: " << DataShard.TabletID();

        if (writeOp) {
            writeOp->SetError(NKikimrDataEvents::TEvWriteResult::STATUS_OVERLOADED, err);

            DataShard.SetOverloadSubscribed(
                writeOp->GetWriteTx()->GetOverloadSubscribe(),
                writeOp->GetRecipient(),
                op->GetTarget(),
                ERejectReasons::ChangesQueueOverflow,
                writeOp->GetWriteResult()->Record
            );
        } else {
            BuildResult(op, NKikimrTxDataShard::TEvProposeTransactionResult::OVERLOADED)
    ...
    if (DataShard.IsReplicated() && !(op->IsReadOnly() || op->IsCommitWritesTx())) {
        TString err = TStringBuilder() << "Can't execute write tx at replicated table:"
                                       << " tablet id: " << DataShard.TabletID();
...
TOutputOpData::TResultPtr&
TExecutionUnit::BuildResult(TOperation::TPtr op, NKikimrTxDataShard::TEvProposeTransactionResult::EStatus status) {
    auto kind = static_cast<NKikimrTxDataShard::ETransactionKind>(op->GetKind());
    op->Result().Reset(new TEvDataShard::TEvProposeTransactionResult(kind, DataShard.TabletID(), op->GetTxId(), status)
    );
