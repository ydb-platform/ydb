...
public:
    TFinishProposeUnit(TDataShard& dataShard, TPipeline& pipeline);
...
private:
    ...
    void UpdateCounters(TOperation::TPtr op, const TActorContext& ctx);
};

TFinishProposeUnit::TFinishProposeUnit(TDataShard& dataShard, TPipeline& pipeline)
...
TFinishProposeUnit::PromoteImmediatePostExecuteEdges(TOperation* op, TTransactionContext& txc) {
...
EExecutionStatus TFinishProposeUnit::Execute(TOperation::TPtr op, TTransactionContext& txc, const TActorContext& ctx) {
    if (op->Result())
...
void TFinishProposeUnit::Complete(TOperation::TPtr op, const TActorContext& ctx) {
...
void TFinishProposeUnit::CompleteRequest(TOperation::TPtr op, const TActorContext& ctx) {
    ...
    if (errors.size()) {
        LOG_LOG_S_THROTTLE(
            ...
            "Errors while proposing transaction txid " << op->GetTxId() << " at tablet " << DataShard.TabletID()
                                                       << " status: " << res->GetStatus() << " errors: " << errors
        );
    }
...
void TFinishProposeUnit::UpdateCounters(TOperation::TPtr op, const TActorContext& ctx) {
    auto& res = op->Result();
    Y_ABORT_UNLESS(res);
    ...
    if (res->IsPrepared()) {
        ...
        if (res->IsError()) {
            ...
            LOG_LOG_S_THROTTLE(
                ...
                NActors::NLog::PRI_ERROR,
                NKikimrServices::TX_DATASHARD,
                "Prepare transaction failed. txid " << op->GetTxId() << " at tablet " << DataShard.TabletID()
                                                    << " errors: " << res->GetError()
            );
        } else {
