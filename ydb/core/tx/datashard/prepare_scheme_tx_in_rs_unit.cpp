...
std::unique_ptr<TExecutionUnit> CreatePrepareSchemeTxInRSUnit(TDataShard &dataShard,
                                                      TPipeline &pipeline)
{
    return std::unique_ptr<TPrepareSchemeTxInRSUnit>(new TPrepareSchemeTxInRSUnit(dataShard, pipeline));
}
