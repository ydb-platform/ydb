...
public:
    ...
    ui64 GetMessageSize() const {
        return ResultString.size();
    }
    ui64 GetMessageRows() const {
        return CurrentMessageRows;
    }
    ...
    void Flush(NKikimrTxDataShard::TEvProposeTransactionResult& res) {
        *res.MutableTxResult() = ResultString;
        ...
        for (auto& offset : RowOffsets) res.AddRowOffsets(offset);
        StartNewMessage();
    }
...
private:
    ...
    ui32 GetResultVersion() const override {
        return 0;
    }
...
private:
    ...
    ui32 GetResultVersion() const override {
        return NKikimrTxUserProxy::TReadTableTransaction::YDB_V1;
    }
    ...
    void BuildResultCommonPart(const NKikimrTxDataShard::TReadTableTransaction& request) {
        ...
        for (auto& col : request.GetColumns()) {
            ...
            if (col.GetTypeId() == NScheme::NTypeIds::Pg) {
                ...
                auto id = static_cast<NYql::NProto::TypeIds>(col.GetTypeId());
                auto xType =
                    notNullResp ? meta->mutable_type() : meta->mutable_type()->mutable_optional_type()->mutable_item();
                if (id == NYql::NProto::Decimal) {
                    NScheme::ProtoFromDecimalType(
                        typeInfoMod.TypeInfo.GetDecimalType(), *xType->mutable_decimal_type()
                    );
                } else {
...
class TReadTableScan
    : public TActor<TReadTableScan>
    , public NTable::IScan {
public:
    ...
    TReadTableScan(
        ...
        for (auto& col : tx.GetColumns()) Tags.push_back(col.GetId());
    }
...
private:
    void Die(const TActorContext& ctx) override {
        ctx.Send(TActivationContext::InterconnectProxy(Sink.NodeId()), new TEvents::TEvUnsubscribe());

        TActor<TReadTableScan>::Die(ctx);
    }

    void Undelivered(TEvents::TEvUndelivered::TPtr&, const TActorContext& ctx) {
        LOG_ERROR(ctx, NKikimrServices::TX_DATASHARD, "TReadTableScan: undelivered event TxId: %" PRIu64, TxId);
    ...
    void Disconnected(TEvInterconnect::TEvNodeDisconnected::TPtr&, const TActorContext& ctx) {
        LOG_ERROR(ctx, NKikimrServices::TX_DATASHARD, "TReadTableScan: disconnect TxId: %" PRIu64, TxId);
    ...
    void Handle(TEvTxProcessing::TEvStreamIsDead::TPtr& ev, const TActorContext& ctx) {
        LOG_INFO(ctx, NKikimrServices::TX_DATASHARD, "TReadTableScan: stream disconnect TxId: %" PRIu64, TxId);
    ...
    void CheckQuota(const TActorContext& ctx) {
        ...
        TAutoPtr<TEvTxProcessing::TEvStreamQuotaRequest> request = new TEvTxProcessing::TEvStreamQuotaRequest;
        request->Record.SetTxId(TxId);
        request->Record.SetShardId(ShardId);
        ctx.Send(Sink, request.Release(), IEventHandle::FlagTrackDelivery | IEventHandle::FlagSubscribeOnSession);
    }
    ...
    TInitialState Prepare(IDriver* driver, TIntrusiveConstPtr<TScheme> scheme) noexcept override {
        ...
        for (const auto& columnRecord : Tx.GetColumns()) {
            if (!scheme->ColInfo(columnRecord.GetId())) {
                Error = TStringBuilder() << "ReadTable cannot find column " << columnRecord.GetName() << " ("
                                         << columnRecord.GetId() << ")";
                SchemaChanged = true;
                return {EScan::Final, {}};
            }
    ...
    EScan MaybeSendResponseMessage(bool last, const TArrayRef<const TCell>& lastKey = {}) {
        ...
        if (Writer->GetMessageSize() < MessageSizeLimit && (!MessageRowsLimit || MessageRowsLimit > rows) &&
            (!RowLimit || RowLimit > rows) && !last)
            return EScan::Feed;
        ...
        ctx.Send(Sink, result, IEventHandle::FlagTrackDelivery | IEventHandle::FlagSubscribeOnSession);
    ...
    bool DebugCheckKeyInRange(TArrayRef<const TCell> key) {
        auto cmp = CompareBorders<true, true>(
            key, ScanRange.To.GetCells(), true, ScanRange.ToInclusive, TableInfo->KeyColumnTypes
        );
    ...
    TAutoPtr<IDestructable> Finish(EAbort abort) noexcept override {
        ...
        if (!SchemaChanged) {
            ...
            TAutoPtr<TEvTxProcessing::TEvStreamQuotaRelease> request = new TEvTxProcessing::TEvStreamQuotaRelease;
            request->Record.SetTxId(TxId);
            request->Record.SetShardId(ShardId);
...
TAutoPtr<NTable::IScan> CreateReadTableScan(
    ui64 txId,
    ui64 shardId,
    TUserTable::TCPtr tableInfo,
    const NKikimrTxDataShard::TReadTableTransaction& tx,
