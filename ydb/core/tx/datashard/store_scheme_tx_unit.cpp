...
public:
    TStoreSchemeTxUnit(TDataShard& dataShard, TPipeline& pipeline);
...
TStoreSchemeTxUnit::TStoreSchemeTxUnit(TDataShard& dataShard, TPipeline& pipeline)
...
EExecutionStatus TStoreSchemeTxUnit::Execute(TOperation::TPtr op, TTransactionContext& txc, const TActorContext& ctx) {
    ...
    if (DataShard.GetPathOwnerId() == INVALID_TABLET_ID) {
        if (tx->GetSchemeTx().HasCreateTable() &&
            tx->GetSchemeTx().GetCreateTable().HasPathId()) { // message from new SS
...
void TStoreSchemeTxUnit::Complete(TOperation::TPtr op, const TActorContext& ctx) {
