...
public:
    ...
    TTxType GetTxType() const override {
        return TXTYPE_VOLATILE_TX_COMMIT;
    }
...
public:
    ...
    TTxType GetTxType() const override {
        return TXTYPE_VOLATILE_TX_ABORT;
    }
...
bool TVolatileTxManager::Load(NIceDb::TNiceDb& db) {
    ...
    if (db.HaveTable<Schema::TxVolatileDetails>() && db.HaveTable<Schema::TxVolatileParticipants>()) {
        if (!LoadTxDetails(db)) {
            return false;
        }
            ...
            case EVolatileTxState::Waiting:
                for (ui64 target : pr.second->Participants) {
                    if (Self->AddExpectation(target, pr.second->Version.Step, pr.second->TxId)) {
                        Self->SendReadSetExpectation(
                            ctx, pr.second->Version.Step, pr.second->TxId, Self->TabletID(), target
                        );
                    }
                }
                break;
...
bool TVolatileTxManager::LoadTxDetails(NIceDb::TNiceDb& db) {
    ...
    while (!rowset.EndOfSet()) {
        ...
        auto res = VolatileTxs.insert(std::make_pair(txId, std::make_unique<TVolatileTxInfo>()));
...
void TVolatileTxManager::PersistAddVolatileTx(
    ...
    db.Table<Schema::TxVolatileDetails>()
        .Key(info->TxId)
        .Update(
...
void TVolatileTxManager::AttachWaitingSnapshotEvent(
...
void TVolatileTxManager::RunCommitCallbacks(TVolatileTxInfo* info) {
    if (info->IsArbiterOnHold && !info->ArbiterReadSets.empty()) {
        Self->OutReadSets.ReleaseOnHoldReadSets(
            info->ArbiterReadSets, TActivationContext::ActorContextFor(Self->SelfId())
        );
        info->ArbiterReadSets.clear();
    }
