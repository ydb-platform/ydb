...
struct TVolatileTxInfo: public TIntrusiveListItem<TVolatileTxInfo, TVolatileTxInfoCommitOrderListTag> {
    ui64 CommitOrder;
    ui64 TxId;
...
public:
    ...
    size_t GetTxInFlight() const {
        return VolatileTxs.size();
    }

    const TVolatileTxByVersion& GetVolatileTxByVersion() const {
        return VolatileTxByVersion;
    }
    ...
    bool AttachVolatileTxCallback(ui64 txId, IVolatileTxCallback::TPtr callback);

    bool AttachBlockedOperation(ui64 txId, ui64 dependentTxId);

    bool AttachWaitingRemovalOperation(ui64 txId, ui64 dependentTxId);

    void AttachWaitingSnapshotEvent(const TRowVersion& snapshot, std::unique_ptr<IEventHandle>&& event);
    ...
    void AbortWaitingTransaction(TVolatileTxInfo* info);
         ...
         */
    bool ProcessReadSet(const TEvTxProcessing::TEvReadSet& rs, THolder<IEventHandle>&& ack, TTransactionContext& txc);

    void ProcessReadSetMissing(ui64 source, ui64 txId, TTransactionContext& txc);
