        ...
        TABLE_CLASS("table table-sortable") {
            ...
            TABLEBODY() {
                ...
                for (ui64 txId : txIds) {
                    ...
                    TABLER() {
                        TABLED() {
                            html << "<a href=\"app?TabletID=" << TabletID() << "&page=volatile-txs&TxId=" << txId
                                 << "\">" << txId << "</a>";
                        }
                        TABLED() {
                            html << info->State;
                        ...
                        TABLED() {
                            html << info->CommitOrder;
                        }
                        TABLED() {
                            html << info->Participants.size();
                        }
                        TABLED() {
                            html << info->Dependencies.size();
                        }
                        TABLED() {
                            bool empty = true;
...
void TDataShard::HandleMonVolatileTxs(NMon::TEvRemoteHttpInfo::TPtr& ev, ui64 txId) {
    ...
    auto linkToTxId = [&](ui64 txId) -> TString {
        return TStringBuilder() << "<a href=\"app?TabletID=" << TabletID() << "&page=volatile-txs&TxId=" << txId
                                << "\">" << txId << "</a>";
    };
    ...
    HTML(html) {
        ...
        PRE() {
            html << "TxId:                     " << info->TxId << (info->AddCommitted ? "" : " (not persisted yet)")
                 << "\n";
            html << "State:                    " << info->State << "\n";
            html << "Version:                  " << info->Version << "\n";
            html << "CommitOrder:              " << info->CommitOrder << (info->CommitOrdered ? " (ordered)" : "")
                 << "\n";
