...
public:
    TWaitForStreamClearanceUnit(TDataShard& dataShard, TPipeline& pipeline);
...
private:
    ...
    void Handle(TDataShard::TEvPrivate::TEvNodeDisconnected::TPtr& ev, TOperation::TPtr op, const TActorContext& ctx);
    void Handle(TEvTxProcessing::TEvStreamClearanceResponse::TPtr& ev, TOperation::TPtr op, const TActorContext& ctx);
    void Handle(TEvTxProcessing::TEvInterruptTransaction::TPtr& ev, TOperation::TPtr op, const TActorContext& ctx);
    void Handle(TEvents::TEvUndelivered::TPtr& ev, TOperation::TPtr op, const TActorContext& ctx);
    void Abort(const TString& err, TOperation::TPtr op, const TActorContext& ctx);
};

TWaitForStreamClearanceUnit::TWaitForStreamClearanceUnit(TDataShard& dataShard, TPipeline& pipeline)
...
EExecutionStatus
TWaitForStreamClearanceUnit::Execute(TOperation::TPtr op, TTransactionContext&, const TActorContext& ctx) {
    // Pass aborted operations
    ...
    if (!op->IsWaitingForStreamClearance()) {
        ...
        TAutoPtr<TEvTxProcessing::TEvStreamClearanceRequest> request = new TEvTxProcessing::TEvStreamClearanceRequest;
...
void TWaitForStreamClearanceUnit::Handle(
    TDataShard::TEvPrivate::TEvNodeDisconnected::TPtr& ev,
    TOperation::TPtr op,
    ...
    if (op->IsWaitingForStreamClearance()) {
        ...
        if (ev->Get()->NodeId == tx->GetStreamSink().NodeId()) {
            Abort(
                TStringBuilder() << "Disconnected from stream sink (node " << ev->Get()->NodeId
...
void TWaitForStreamClearanceUnit::Handle(
    TEvTxProcessing::TEvStreamClearanceResponse::TPtr& ev,
    TOperation::TPtr op,
    const TActorContext& ctx
) {
    if (op->IsWaitingForStreamClearance()) {
        if (ev->Get()->Record.GetCleared()) {
            ...
            Abort(
                TStringBuilder() << "Got stream clearance reject for " << *op << " at " << DataShard.TabletID(), op, ctx
            );
        }
    }
}
    ...
    if (op->IsWaitingForStreamClearance()) {
        Abort(
            TStringBuilder() << "Interrupted operation " << *op << " at " << DataShard.TabletID()
                             << " while waiting for stream clearance",
            op,
            ctx
        );
    }
...
void TWaitForStreamClearanceUnit::Handle(
    TEvents::TEvUndelivered::TPtr&,
    TOperation::TPtr op,
    const TActorContext& ctx
) {
    if (op->IsWaitingForStreamClearance()) {
        Abort(
            TStringBuilder() << "Couldn't deliver stream clearance request for " << *op << " at "
                             << DataShard.TabletID(),
            op,
            ctx
        );
...
void TWaitForStreamClearanceUnit::Abort(const TString& err, TOperation::TPtr op, const TActorContext& ctx) {
    TActiveTransaction* tx = dynamic_cast<TActiveTransaction*>(op.Get());
    Y_VERIFY_S(tx, "cannot cast operation of kind " << op->GetKind());
...
void TWaitForStreamClearanceUnit::Complete(TOperation::TPtr, const TActorContext&) {}
