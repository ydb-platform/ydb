...
public:
    ...
    static void ModifyObjects(THashMap<TAddress, TObject>&& add, THashSet<TAddress>&& remove) {
        AFL_VERIFY(NActors::TlsActivationContext);
        auto& context = NActors::TActorContext::AsActorContext();
        context.Send(
            GetCurrentNodeServiceId(), new NSource::TEvents<TPolicy>::TEvAdditionalObjectsInfo(std::move(add), std::move(remove)));
    }
