...
public:
    ...
    bool Load(TVector<TLockRow>& rows) override {
        ...
        THashMap<ui64, size_t> lockIndex;
        {
            ...
            while (!rowset.EndOfSet()) {
                ...
                lock.VictimQuerySpanId = rowset.template GetValueOrDefault<typename Schema::Locks::VictimQuerySpanId>(0);
    ...
    void PersistAddLock(ui64 lockId, ui32 lockNodeId, ui32 generation, ui64 counter, ui64 createTs, ui64 flags = 0, ui64 victimQuerySpanId = 0) override {
        using Schema = TSchemaDescription;
        NIceDb::TNiceDb db(DB);
        db.Table<typename Schema::Locks>().Key(lockId).Update(
            ...
            NIceDb::TUpdate<typename Schema::Locks::VictimQuerySpanId>(victimQuerySpanId));
        HasChanges_ = true;
    }
