    ...
    void Handle(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev) {
        ...
        switch (entry.Status) {
            ...
            case NSchemeCache::TSchemeCacheNavigate::EStatus::RootUnknown:
                return ReplyError(
                    ...
                    TStringBuilder() << "The specified database '" << DatabaseName << "' does not exist"
                );
        ...
        if (coordinators.empty()) {
            return ReplyError(
                ...
                TStringBuilder() << "The specified database '" << DatabaseName << "' has no coordinators"
            );
        }
...
private:
    void SendToTablet(ui64 tabletId, THolder<IEventBase> event, bool subscribe = true) {
        Send(
            LeaderPipeCache,
            new TEvPipeCache::TEvForward(event.Release(), tabletId, subscribe),
            IEventHandle::FlagTrackDelivery
        );
    }
