...
private:
    ...
    STFUNC(StateAllocateTxId) {
        switch (ev->GetTypeRewrite()) { hFunc(TEvTxUserProxy::TEvAllocateTxIdResult, Handle); }
    }
...
private:
    ...
    bool SendPrepareTransaction(ui64 tabletId, bool delayed = false) {
        ...
        if (delayed) {
            ...
            ++data.NumRetries;
            if (ToRetry.empty()) {
                TimeoutTimerActorId = CreateLongTimer(
                    TDuration::MilliSeconds(TRetryData::RetryDelayMs),
        ...
        SendToTablet(
            tabletId,
            MakeHolder<TEvColumnShard::TEvProposeTransaction>(
                NKikimrTxColumnShard::TX_KIND_COMMIT, SelfId(), TxId, data.TxBody
            )
        );
        return true;
    }
    ...
    bool UpdateCoordinators(ui64 tabletId, const google::protobuf::RepeatedField<ui64>& domainCoordinators) {
        ui64 privateCoordinator =
            TCoordinators(TVector<ui64>(domainCoordinators.begin(), domainCoordinators.end())).Select(TxId);
...
private:
    void PlanTransaction() {
        ...
        auto req = MakeHolder<TEvTxProxy::TEvProposeTransaction>(SelectedCoordinator, TxId, 0, MinStep, MaxStep);
    ...
    bool SendCheckPlannedTransaction(ui64 tabletId, bool delayed = false) {
        ...
        SendToTablet(tabletId, MakeHolder<TEvColumnShard::TEvCheckPlannedTransaction>(SelfId(), PlanStep, TxId));
        return true;
    }
