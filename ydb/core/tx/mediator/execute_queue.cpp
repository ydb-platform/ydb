...
class TTxMediatorExecQueue: public TActor<TTxMediatorExecQueue> {
    ...
    void SendStepToBucket(TTabletId tablet, TStepId step, TVector<TTx>& tx, const TActorContext& ctx) {
        ...
        LOG_DEBUG(ctx, NKikimrServices::TX_MEDIATOR_PRIVATE, [&]() {
            TStringBuilder ss;
            ss << "Mediator exec queue [" << MediatorId << "], step# " << step << " for tablet [" << tablet
               << "]. TxIds:";
            for (const auto& x : tx) ss << " txid# " << x.TxId;
            ss << " marker# M2";
            return (TString)ss;
        }());
    ...
    void Die(const TActorContext& ctx) override {
        for (const TBucket& bucket : Buckets) ctx.Send(bucket.ActiveActor, new TEvents::TEvPoisonPill());
        Buckets.clear();
    ...
    void Handle(TEvTxMediator::TEvCommitStep::TPtr& ev, const TActorContext& ctx) {
        ...
        for (ui32 i = 0; i != totalCoordinators; ++i) {
            ...
            Sort(
                coord.TabletsToTransaction.begin(),
                coord.TabletsToTransaction.end(),
                TCoordinatorStep::TabletToTransactionCmp()
            );
        }
    ...
    void Handle(TEvTxMediator::TEvRequestLostAcks::TPtr& ev, const TActorContext& ctx) {
        ...
        Sort(
            step->TabletsToTransaction.begin(),
            step->TabletsToTransaction.end(),
            TCoordinatorStep::TabletToTransactionCmp()
        );
        ...
        for (TVector<std::pair<TTabletId, std::size_t>>::const_iterator it = step->TabletsToTransaction.begin(),
                                                                        end = step->TabletsToTransaction.end();
             it != end;
             ++it) {
    ...
    void Bootstrap(const TActorContext& ctx) {
        ...
        for (ui32 bucketIdx = 0; bucketIdx < Buckets.size(); ++bucketIdx)
            Buckets[bucketIdx].ActiveActor = ctx.ExecutorThread.RegisterActor(
                CreateTxMediatorTabletQueue(ctx.SelfID, MediatorId, 1, bucketIdx), TMailboxType::ReadAsFilled
            );
    }
