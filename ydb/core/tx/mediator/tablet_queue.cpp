...
class TTxMediatorTabletQueue : public TActor<TTxMediatorTabletQueue> {
    ...
    void SendToTablet(TTabletEntry::TStep* tabletStep, ui64 tabletId, const TActorContext& ctx) {
        ...
        for (const TTx& tx : tabletStep->Transactions) {
            ...
            YDB_LOG_DEBUG_CTX_COMP(ctx, NKikimrServices::TX_MEDIATOR_PRIVATE, "Send from to tablet marker M5lu",
                {"mediator", Mediator},
                {"tabletId", tabletId},
                {"step", tabletStep->StepRef->Step},
                {"txid", tx.TxId});
        }
        YDB_LOG_DEBUG_CTX_COMP(ctx, NKikimrServices::TX_MEDIATOR_TABLETQUEUE, "SEND",
            {"actor", ctx.SelfID},
            {"mediator", Mediator},
            {"to", tabletId},
            {"evx", evx->ToString()});
    ...
    void Handle(TEvTxMediator::TEvCommitTabletStep::TPtr& ev, const TActorContext& ctx) {
        ...
        YDB_LOG_DEBUG_CTX_COMP(ctx, NKikimrServices::TX_MEDIATOR_TABLETQUEUE, "HANDLE",
            {"actor", ctx.SelfID},
            {"mediator", Mediator},
            {"msg", msg->ToString()},
            {"marker", "M4"});
    ...
    void Handle(TEvTabletPipe::TEvClientConnected::TPtr &ev, const TActorContext &ctx) {
        ...
        if (!Pipes->OnConnect(ev)) {
            if (msg->Dead) {
                YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::TX_MEDIATOR_TABLETQUEUE, "HANDLE TEvClientConnected(Dead=true)",
                    {"actor", ctx.SelfID},
                    {"mediator", Mediator});
    ...
    void Handle(TEvents::TEvUndelivered::TPtr& ev, const TActorContext& ctx) {
        YDB_LOG_DEBUG_CTX_COMP(ctx, NKikimrServices::TX_MEDIATOR_TABLETQUEUE, "HANDLE TEvUndelivered",
            {"actor", ctx.SelfID},
            {"mediator", Mediator});
