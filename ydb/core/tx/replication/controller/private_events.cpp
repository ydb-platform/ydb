...
TEvPrivate::TEvDiscoveryTargetsResult::TEvDiscoveryTargetsResult(
    ui64 rid,
...
TString TEvPrivate::TEvDiscoveryTargetsResult::ToString() const {
    return TStringBuilder() << ToStringHeader() << " {"
                            << " ReplicationId: " << ReplicationId << " ToAdd [" << JoinSeq(",", ToAdd) << "]"
...
TEvPrivate::TEvCreateDstResult::TEvCreateDstResult(
    ui64 rid,
    ui64 tid,
...
TString TEvPrivate::TEvCreateDstResult::ToString() const {
    return TStringBuilder() << ToStringHeader() << " {" << ToStringBody() << " DstPathId: " << DstPathId << " }";
}
...
TString TEvPrivate::TEvResolveTenantResult::ToString() const {
    return TStringBuilder() << ToStringHeader() << " {"
                            << " ReplicationId: " << ReplicationId << " Tenant: " << Tenant << " Sucess: " << Success
                            << " }";
}
...
TString TEvPrivate::TEvResolveSecretResult::ToString() const {
    return TStringBuilder() << ToStringHeader() << " {"
                            << " ReplicationId: " << ReplicationId << " Success: " << Success << " Error: " << Error
                            << " }";
}
...
TEvPrivate::TEvAlterDstResult::TEvAlterDstResult(
    ui64 rid,
    ui64 tid,
    NKikimrScheme::EStatus status,
    const TString& error
...
Y_DECLARE_OUT_SPEC(
    ,
    NKikimr::NReplication::NController::TEvPrivate::TEvDiscoveryTargetsResult::TAddEntry,
    stream,
    value
) {
...
Y_DECLARE_OUT_SPEC(
    ,
    NKikimr::NReplication::NController::TEvPrivate::TEvDiscoveryTargetsResult::TFailedEntry,
    stream,
    value
) {
