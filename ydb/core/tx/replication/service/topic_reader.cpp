    ...
    TStringBuf GetLogPrefix() const {
        if (!LogPrefix) {
            LogPrefix = TStringBuilder()
                << "[RemoteTopicReader]"
                << "[" << Settings.ToBase().Topics_[0].Path_ << "]"
                << "[" << Settings.ToBase().Topics_[0].PartitionIds_[0] << "]"
                << SelfId() << " ";
        }
...
public:
    ...
    explicit TRemoteTopicReader(const TActorId& ydbProxy, const TEvYdbProxy::TTopicReaderSettings& opts)
        ...
        , Settings(opts)
    {
        Y_ABORT_UNLESS(Settings.ToBase().Topics_.size() == 1);
        Y_ABORT_UNLESS(Settings.ToBase().Topics_.at(0).PartitionIds_.size() == 1);
    }
