...
bool WriteTopic(const Env& env, const TString& topicPath, const TString& data) {
    NYdb::NTopic::TTopicClient client(
        env.GetDriver(),
        NYdb::NTopic::TTopicClientSettings().DiscoveryEndpoint(env.GetEndpoint()).Database(env.GetDatabase())
    );

    auto session = client.CreateSimpleBlockingWriteSession(
        NYdb::NTopic::TWriteSessionSettings().Path(topicPath).ProducerId("producer").MessageGroupId("producer")
    );
