...
TString TSchemeCacheNavigate::TEntry::ToString() const {
    return TStringBuilder() << "{"
                            << " Path: " << JoinPath(Path) << " TableId: " << TableId << " RequestType: " << RequestType
                            << " Operation: " << Operation
                            << " RedirectRequired: " << (RedirectRequired ? "true" : "false")
                            << " ShowPrivatePath: " << (ShowPrivatePath ? "true" : "false")
                            << " SyncVersion: " << (SyncVersion ? "true" : "false") << " Status: " << Status
                            << " Kind: " << Kind << " DomainInfo " << (DomainInfo ? DomainInfo->ToString() : "<null>")
                            << " }";
}
...
TString TSchemeCacheNavigate::ToString(const NScheme::TTypeRegistry& typeRegistry) const {
    return TStringBuilder() << "{"
                            ...
                            << " DomainOwnerId: " << DomainOwnerId << " Instant: " << Instant << " ResultSet ["
                            << ResultSetToString(ResultSet, typeRegistry) << "]"
                            << " }";
}
...
TString TSchemeCacheRequest::TEntry::ToString() const {
    return TStringBuilder() << "{"
                            ...
                            << " Access: " << Access << " SyncVersion: " << (SyncVersion ? "true" : "false")
                            << " Status: " << Status << " Kind: " << Kind << " PartitionsCount: "
                            << (KeyDescription ? ::ToString(KeyDescription->GetPartitions().size()) : "<moved>")
...
TString TSchemeCacheRequest::TEntry::ToString(const NScheme::TTypeRegistry& typeRegistry) const {
    ...
    out << "{"
        << " TableId: " << (KeyDescription ? ::ToString(KeyDescription->TableId.PathId) : "<moved>")
        << " Access: " << Access << " SyncVersion: " << (SyncVersion ? "true" : "false") << " Status: " << Status
        << " Kind: " << Kind
    ...
    if (KeyDescription) {
        TDbTupleRef from(
            KeyDescription->KeyColumnTypes.data(), KeyDescription->Range.From.data(), KeyDescription->Range.From.size()
        );
        TDbTupleRef to(
            KeyDescription->KeyColumnTypes.data(), KeyDescription->Range.To.data(), KeyDescription->Range.To.size()
        );

        if (KeyDescription->Range.Point) {
            out << " Point: " << DbgPrintTuple(from, typeRegistry);
        } else {
            out << " From: " << DbgPrintTuple(from, typeRegistry) << " IncFrom: " << KeyDescription->Range.InclusiveFrom
                << " To: " << DbgPrintTuple(to, typeRegistry) << " IncTo: " << KeyDescription->Range.InclusiveTo;
        }
    }
...
TString TSchemeCacheRequest::ToString(const NScheme::TTypeRegistry& typeRegistry) const {
    return TStringBuilder() << "{"
                            << " ErrorCount: " << ErrorCount << " DatabaseName: " << DatabaseName
                            << " DomainOwnerId: " << DomainOwnerId << " ResultSet ["
                            << ResultSetToString(ResultSet, typeRegistry) << "]"
                            << " }";
}
