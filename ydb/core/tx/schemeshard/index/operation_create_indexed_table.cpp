...
TVector<ISubOperation::TPtr> CreateIndexedTable(TOperationId nextId, const TTxTransaction& tx, TOperationContext& context) {
    ...
    YDBLOG_CTX_DEBUG(context.Ctx, "TCreateTableIndex construct operation  table path:  domain path id:  domain path:  shardsToCreate:  shardsPerPath:  GetShardsInside:  MaxShards: ",
        {"path", baseTablePath.PathString()},
        {"id", baseTablePath.GetPathIdForDomain()},
        {"path", TPath::Init(baseTablePath.GetPathIdForDomain(), context.SS).PathString()},
