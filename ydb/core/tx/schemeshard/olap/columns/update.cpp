...
bool TOlapColumnAdd::ParseFromRequest(
    const NKikimrSchemeOp::TOlapColumnDescription& columnSchema,
    ...
    if (!GetTypeInfo(
            AppData()->TypeRegistry->GetType(TypeName), columnSchema.GetTypeInfo(), TypeName, Name, Type, errStr
        )) {
    ...
    if (Type.GetTypeId() == NScheme::NTypeIds::Pg) {
        if (!TOlapColumnAdd::IsAllowedPgType(NPg::PgTypeIdFromTypeDesc(Type.GetPgTypeDesc()))) {
            errors.AddError(
                TStringBuilder() << "Type '" << TypeName << "' specified for column '" << Name << "' is not supported"
            );
            return false;
        }
    } else {
    ...
    if (columnSchema.HasDefaultValue()) {
        ...
        if (!DefaultValue.IsCompatibleType(*arrowTypeResult)) {
            errors.AddError(
                "incompatible types for default write: def" + DefaultValue.DebugString() +
                ", col:" + (*arrowTypeResult)->ToString()
            );
            return false;
        }
    }
    ...
    for (auto& columnSchema : alterRequest.GetAddColumns()) {
        ...
        if (addColumnNames.contains(column.GetName())) {
            errors.AddError(
                ...
                TStringBuilder() << "column '" << column.GetName() << "' duplication for add"
            );
            return false;
        }
    ...
    for (auto& columnSchemaDiff : alterRequest.GetAlterColumns()) {
        ...
        if (addColumnNames.contains(columnDiff.GetName())) {
            errors.AddError(
                ...
                TStringBuilder() << "column '" << columnDiff.GetName() << "' have to be either add or update"
            );
            return false;
        }
        if (alterColumnNames.contains(columnDiff.GetName())) {
            errors.AddError(
                NKikimrScheme::StatusSchemeError,
                TStringBuilder() << "column '" << columnDiff.GetName() << "' duplication for update"
            );
            return false;
        }
...
bool TOlapColumnsUpdate::Parse(
    ...
    for (auto&& pkKey : tableSchema.GetKeyColumnNames()) {
        if (!keyColumnNames.emplace(pkKey, keyColumnNames.size()).second) {
            errors.AddError(
                NKikimrScheme::StatusSchemeError, TStringBuilder() << "Duplicate key column '" << pkKey << "'"
            );
            return false;
        }
    }
    ...
    for (auto& columnSchema : tableSchema.GetColumns()) {
        ...
        if (column.IsKeyColumn()) {
            if (!TOlapColumnAdd::IsAllowedPkType(column.GetType().GetTypeId())) {
                errors.AddError(
                    NKikimrScheme::StatusSchemeError,
                    TStringBuilder() << "Type '" << column.GetTypeName() << "' specified for column '"
                                     << column.GetName() << "' is not supported as primary key"
                );
                return false;
            }
        }
        if (columnNames.contains(column.GetName())) {
            errors.AddError(
                ...
                TStringBuilder() << "Duplicate column '" << column.GetName() << "'"
            );
            return false;
        }
        if (!allowNullKeys) {
            if (keyColumnNames.contains(column.GetName()) && !column.IsNotNull()) {
                errors.AddError(
                    NKikimrScheme::StatusSchemeError,
                    TStringBuilder() << "Nullable key column '" << column.GetName() << "'"
                );
                return false;
            }
        }
