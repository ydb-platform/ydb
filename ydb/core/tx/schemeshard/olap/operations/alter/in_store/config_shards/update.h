...
private:
    ...
    virtual TString DoGetShardTxBodyString(const ui64 tabletId, const TMessageSeqNo& seqNo) const override {
        ...
        if (NewShardIds.contains(tabletId)) {
            ...
            NColumnShard::TLocalPathId::FromRawValue(TargetInStoreTable->GetPathId().LocalPathId).ToProto(create);
        }
        if (DeleteShardIds.contains(tabletId)) {
            NColumnShard::TLocalPathId::FromRawValue(TargetInStoreTable->GetPathId().LocalPathId).ToProto(*result.MutableDropTable());
        } else {
