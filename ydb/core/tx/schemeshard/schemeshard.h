...
static_assert(
    EvEnd < EventSpaceEnd(TKikimrEvents::ES_FLAT_TX_SCHEMESHARD),
    "expect EvEnd < EventSpaceEnd(TKikimrEvents::ES_FLAT_TX_SCHEMESHARD)"
);

struct TEvProcessingResponse
    : public TEventPB<TEvProcessingResponse, NKikimrScheme::TEvProcessingResponse, EvProcessingResponse> {
private:
    using TBase = TEventPB<TEvProcessingResponse, NKikimrScheme::TEvProcessingResponse, EvProcessingResponse>;
...
struct TEvProcessingRequest
    : public TEventPB<TEvProcessingRequest, NKikimrScheme::TEvProcessingRequest, EvProcessingRequest> {
private:
    using TBase = TEventPB<TEvProcessingRequest, NKikimrScheme::TEvProcessingRequest, EvProcessingRequest>;
...
struct TEvModifySchemeTransactionResult
    : public TEventPB<
    ...
    TEvModifySchemeTransactionResult(
        ...
        ui64 txid,
        ui64 schemeshardId,
        const TStringBuf& reason = TStringBuf()
    )
        : TEvModifySchemeTransactionResult(TTxId(txid), TTabletId(schemeshardId)) {
    ...
    void SetPathCreateTxId(ui64 txId) {
        Record.SetPathCreateTxId(txId);
    }
    void SetPathDropTxId(ui64 txId) {
        Record.SetPathDropTxId(txId);
    }
    void SetPathId(ui64 pathId) {
...
struct TEvInitRootShardResult
    : public TEventPB<TEvInitRootShardResult, NKikimrTxScheme::TEvInitRootShardResult, EvInitRootShardResult> {
    enum EStatus {
        StatusUnknown,
        StatusSuccess,
...
struct TEvDescribeScheme: public TEventPB<TEvDescribeScheme, NKikimrSchemeOp::TDescribePath, EvDescribeScheme> {
    TEvDescribeScheme() {}
...
struct TEvDescribeSchemeResult
    ...
    TString ToString() const override {
        TStringStream str;
        str << ToStringHeader() << " PreSerializedData size# " << PreSerializedData.size() << " Record# "
...
struct TEvNotifyTxCompletion
    : public TEventPB<TEvNotifyTxCompletion, NKikimrScheme::TEvNotifyTxCompletion, EvNotifyTxCompletion> {
    explicit TEvNotifyTxCompletion(ui64 txId = 0) {
        Record.SetTxId(txId);
    }
};

struct TEvNotifyTxCompletionRegistered
    : public TEventPB<
          TEvNotifyTxCompletionRegistered,
...
struct TEvWakeupToMeasureSelfResponseTime
    : public TEventLocal<TEvWakeupToMeasureSelfResponseTime, EvWakeupToMeasureSelfResponseTime> {};

struct TEvInitTenantSchemeShard
    : public TEventPB<TEvInitTenantSchemeShard, NKikimrScheme::TEvInitTenantSchemeShard, EvInitTenantSchemeShard> {
    TEvInitTenantSchemeShard() = default;

    TEvInitTenantSchemeShard(
        ui64 selfTabletId,
        ui64 pathId,
        TString tenantRootPath,
        TString owner,
        TString effectiveRootACL,
        ui64 effectiveRootACLVersion,
        const NKikimrSubDomains::TProcessingParams& processingParams,
        const TStoragePools& storagePools,
        const TMap<TString, TString> userAttrData,
        ui64 UserAttrsVersion,
        const TSchemeLimits& limits,
        ui64 sharedHive,
        const TPathId& resourcesDomainId = TPathId()
    );
};

struct TEvInitTenantSchemeShardResult
    : public TEventPB<
          TEvInitTenantSchemeShardResult,
...
struct TEvMigrateSchemeShard: public TEventPB<TEvMigrateSchemeShard, NKikimrScheme::TEvMigrate, EvMigrateSchemeShard> {
...
struct TEvMigrateSchemeShardResult
    : public TEventPB<TEvMigrateSchemeShardResult, NKikimrScheme::TEvMigrateResult, EvMigrateSchemeShardResult> {
...
struct TEvPublishTenantAsReadOnly
    : public TEventPB<
          TEvPublishTenantAsReadOnly,
...
struct TEvPublishTenantAsReadOnlyResult
    : public TEventPB<
          TEvPublishTenantAsReadOnlyResult,
...
struct TEvRewriteOwner: public TEventPB<TEvRewriteOwner, NKikimrScheme::TEvRewriteOwner, EvRewriteOwner> {
    TEvRewriteOwner() = default;
};

struct TEvRewriteOwnerResult
    : public TEventPB<TEvRewriteOwnerResult, NKikimrScheme::TEvRewriteOwnerResult, EvRewriteOwnerResult> {
    TEvRewriteOwnerResult() = default;
};

struct TEvPublishTenant: public TEventPB<TEvPublishTenant, NKikimrScheme::TEvPublishTenant, EvPublishTenant> {
...
struct TEvPublishTenantResult
    : public TEventPB<TEvPublishTenantResult, NKikimrScheme::TEvPublishTenantResult, EvPublishTenantResult> {
...
struct TEvSyncTenantSchemeShard
    : public TEventPB<TEvSyncTenantSchemeShard, NKikimrScheme::TEvSyncTenantSchemeShard, EvSyncTenantSchemeShard> {
...
struct TEvUpdateTenantSchemeShard
    : public TEventPB<
          TEvUpdateTenantSchemeShard,
...
struct TEvFindTabletSubDomainPathIdResult
    : public TEventPB<
          TEvFindTabletSubDomainPathIdResult,
...
IActor* CreateFindSubDomainPathIdActor(
    const TActorId& parent,
    ui64 tabletId,
    ui64 schemeShardId,
    bool delayFirstRequest,
    TDuration maxFindSubDomainPathIdDelay = TDuration::Minutes(10)
);
} // namespace NKikimr
