...
struct TSchemeShard::TTxCleanTables : public TTransactionBase<TSchemeShard> {
    ...
    void Complete(const TActorContext &ctx) override {
        if (RemovedCount) {
            YDB_LOG_WARN_CTX(ctx, "TTxCleanTables Complete: done PersistRemoveTable for tables",
...
struct TSchemeShard::TTxCleanBlockStoreVolumes : public TTransactionBase<TSchemeShard> {
    ...
    void Complete(const TActorContext& ctx) override {
        if (RemovedCount) {
            YDB_LOG_WARN_CTX(ctx, "TTxCleanBlockStoreVolumes Complete: done PersistRemoveBlockStoreVolume for volumes",
                {"removedCount", RemovedCount},
...
struct TSchemeShard::TTxCleanDroppedPaths : public TTransactionBase<TSchemeShard> {
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDB_LOG_DEBUG_CTX(ctx, "TTxCleanDroppedPaths Execute",
            {"candidatesCount", Self->CleanDroppedPathsCandidates.size()},
            {"schemeshard", Self->TabletID()},
        );
    ...
    void Complete(const TActorContext& ctx) override {
        ...
        if (RemovedCount || SkippedCount) {
            YDB_LOG_NOTICE_CTX(ctx, "TTxCleanDroppedPaths Complete",
                {"removedCount", RemovedCount},
                {"skippedCount", SkippedCount},
                {"left", Self->CleanDroppedPathsCandidates.size()},
                {"schemeshard", Self->TabletID()},
            );
        }
...
struct TSchemeShard::TTxCleanDroppedSubDomains : public TTransactionBase<TSchemeShard> {
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDB_LOG_DEBUG_CTX(ctx, "TTxCleanDroppedSubDomains Execute",
            {"candidatesCount", Self->CleanDroppedSubDomainsCandidates.size()},
            {"schemeshard", Self->TabletID()},
        );
        ...
        while (RemovedCount < BucketSize && !Self->CleanDroppedSubDomainsCandidates.empty()) {
            ...
            {
                YDB_LOG_DEBUG_CTX(ctx, "TTxCleanDroppedSubDomains: PersistRemoveSubDomain",
                    {"pathId", pathId},
                    {"schemeshard", Self->TabletID()},
                );
    ...
    void Complete(const TActorContext& ctx) override {
        ...
        if (RemovedCount || SkippedCount) {
            YDB_LOG_NOTICE_CTX(ctx, "TTxCleanDroppedSubDomains Complete",
                {"removedCount", RemovedCount},
                {"skippedCount", SkippedCount},
                {"left", Self->CleanDroppedSubDomainsCandidates.size()},
                {"schemeshard", Self->TabletID()},
            );
