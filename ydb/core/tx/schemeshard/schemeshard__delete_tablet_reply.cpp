...
struct TSchemeShard::TTxDeleteTabletReply: public TSchemeShard::TRwTxBase {
    ...
    TTxDeleteTabletReply(TSelf* self, TEvHive::TEvDeleteTabletReply::TPtr& ev)
        : TRwTxBase(self)
        , Ev(ev)
        , ShardIdx(self->MakeLocalId(TLocalShardIdx(Ev->Get()->Record.GetTxId_Deprecated()))
          ) // We use TxId field as a cookie where we store shardIdx
        ...
        if (Ev->Get()->Record.HasShardOwnerId()) {
            ...
            ShardIdx = TShardIdx(Ev->Get()->Record.GetShardOwnerId(), Ev->Get()->Record.GetShardLocalIdx(0));
        }
    ...
    TTxType GetTxType() const override {
        return TXTYPE_FREE_TABLET_RESULT;
    }
