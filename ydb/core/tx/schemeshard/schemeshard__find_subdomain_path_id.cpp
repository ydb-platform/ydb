...
public:
    ...
    TTxType GetTxType() const override {
        return TXTYPE_FIND_TABLET_SUBDOMAIN_PATH_ID;
    }
...
private:
    ...
    void WakeUp() {
        ...
        NTabletPipe::SendData(
            SelfId(), SchemeShardPipe, new NSchemeShard::TEvSchemeShard::TEvFindTabletSubDomainPathId(TabletId)
        );
...
private:
    ...
    void Handle(NSchemeShard::TEvSchemeShard::TEvFindTabletSubDomainPathIdResult::TPtr& ev) {
        ...
        Send(
            Parent,
            new NSchemeShard::TEvSchemeShard::TEvSubDomainPathIdFound(
                msg->Record.GetSchemeShardId(), msg->Record.GetSubDomainPathId()
            )
        );
        PassAway();
    }
...
IActor* CreateFindSubDomainPathIdActor(
    ...
    return new NSchemeShard::TFindSubDomainPathIdActor(
        parent, tabletId, schemeShardId, delayFirstRequest, maxFindSubDomainPathIdDelay
    );
}
