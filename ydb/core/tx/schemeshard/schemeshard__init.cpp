...
#undef RETURN_IF_NO_PRECHARGED
        ...
        for (auto& item : Self->Operations) {
            ...
            YDB_LOG_NOTICE_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "TTxInit for TxInFlight execute ProgressState for all parts",
                {"txId", operation->TxId},
                {"parts", operation->Parts.size()},
                {"awaitNum", operation->WaitOperations.size()},
                {"dependentNum", operation->DependentOperations.size()},
                {"schemeshard", Self->TabletID()});
        ...
        // Read forced compactions
        {
            ...
            while (!compactionsRowset.EndOfSet()) {
                ...
                if (!info->TablesToCompact.empty()) {
                    ...
                    YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Empty tables to compact for",
                        {"compaction", info->Id},
                        {"schemeshard", Self->TabletID()});
                }
            ...
            while (!shardsRowset.EndOfSet()) {
                ...
                if (auto* info = Self->ForcedCompactions.FindPtr(compactionId)) {
                    if (const auto* shardInfo = Self->ShardInfos.FindPtr(shardIdx); shardInfo && (*info)->TablesToCompact.contains(shardInfo->PathId)) {
                        ...
                        YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Unknown shardIdx for",
                            {"shardIdx", shardIdx},
                    ...
                    YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Unknown forced compaction id for",
