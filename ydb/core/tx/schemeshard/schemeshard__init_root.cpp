...
struct TSchemeShard::TTxInitRoot: public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        const NKikimrConfig::TDomainsConfig::TSecurityConfig& securityConfig =
            Self->GetDomainsConfig().GetSecurityConfig();
        ...
        for (const auto& defaultGroup : securityConfig.GetDefaultGroups()) {
            auto response =
                Self->LoginProvider.CreateGroup({.Group = defaultGroup.GetName(), .Options = {.CheckName = false}});
            if (response.Error) {
                LOG_ERROR_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD,
                     "TTxInitRoot DoExecute"
                         << ", path: " << rootName
        ...
        TPathElement::TPtr newPath =
            new TPathElement(Self->RootPathId(), Self->RootPathId(), Self->RootPathId(), joinedRootPath, owner);
...
struct TSchemeShard::TTxInitRootCompatibility: public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        auto reply = MakeHolder<TEvSchemeShard::TEvInitRootShardResult>(
            Self->TabletID(), TEvSchemeShard::TEvInitRootShardResult::StatusAlreadyInitialized
        );
...
NTabletFlatExecutor::ITransaction* TSchemeShard::CreateTxInitRootCompatibility(
    TEvSchemeShard::TEvInitRootShard::TPtr& ev
) {
...
struct TSchemeShard::TTxInitTenantSchemeShard: public TSchemeShard::TRwTxBase {
    ...
    void RegisterShard(
        NIceDb::TNiceDb& db,
        TSubDomainInfo::TPtr& subdomain,
        const TContainer& tabletIds,
        TTabletTypes::EType type
    ) {
        for (ui32 i = 0; i < (ui64)tabletIds.size(); ++i) {
            const auto id = TTabletId(tabletIds[i]);
            const auto shardIdx =
                Self->RegisterShardInfo(TShardInfo(InvalidTxId, Self->RootPathId(), type).WithTabletID(id));
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        TSubDomainInfo::TPtr subdomain = new TSubDomainInfo(
        ...
        if (processingParams.HasSysViewProcessor()) {
            RegisterShard(
                db, subdomain, TVector<ui64>{processingParams.GetSysViewProcessor()}, TTabletTypes::SysViewProcessor
            );
        }
        if (processingParams.HasStatisticsAggregator()) {
            RegisterShard(
                db,
                subdomain,
                TVector<ui64>{processingParams.GetStatisticsAggregator()},
        ...
        if (processingParams.HasBackupController()) {
            RegisterShard(
                db, subdomain, TVector<ui64>{processingParams.GetBackupController()}, TTabletTypes::BackupController
            );
        }
...
NTabletFlatExecutor::ITransaction* TSchemeShard::CreateTxInitTenantSchemeShard(
    TEvSchemeShard::TEvInitTenantSchemeShard::TPtr& ev
) {
...
struct TSchemeShard::TTxPublishTenantAsReadOnly: public TSchemeShard::TRwTxBase {
    ...
    TTxType GetTxType() const override {
        return TXTYPE_SUBDOMAIN_MIGRATE;
    }
...
NTabletFlatExecutor::ITransaction* TSchemeShard::CreateTxPublishTenantAsReadOnly(
    TEvSchemeShard::TEvPublishTenantAsReadOnly::TPtr& ev
) {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        switch (Self->InitState) {
            case TTenantInitState::ReadOnlyPreview:
                ...
                db.Table<Schema::SysParams>()
                    .Key(Schema::SysParam_IsReadOnlyMode)
                    .Update(NIceDb::TUpdate<Schema::SysParams::Value>("0"));
                break;
            case TTenantInitState::Done:
                break;
            case TTenantInitState::InvalidState:
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        db.Table<Schema::MigratedPaths>()
            ...
            .Update(
                ...
                NIceDb::TUpdate<Schema::MigratedPaths::DirAlterVersion>(
                    pathDescr.GetDirAlterVersion() + 1
                ), // we need to bump version on 1 to override data at SB
        ...
        for (auto& userAttr : pathDescr.GetUserAttributes()) {
            db.Table<Schema::MigratedUserAttributes>()
                .Key(pathId.OwnerId, pathId.LocalPathId, userAttr.GetKey())
                .Update(NIceDb::TUpdate<Schema::MigratedUserAttributes::AttrValue>(userAttr.GetValue()));
        }
        ...
        for (const auto& shard : record.GetShards()) {
            auto shardIdx =
                TShardIdx(TOwnerId(shard.GetShardIdx().GetOwnerId()), TLocalShardIdx(shard.GetShardIdx().GetLocalId()));
        ...
        if ((TPathElement::EPathType)pathDescr.GetPathType() == TPathElement::EPathType::EPathTypeTable) {
            ...
            for (const NKikimrScheme::TMigrateColumn& colDescr : tableDescr.GetColumns()) {
                db.Table<Schema::MigratedColumns>()
                    .Key(pathId.OwnerId, pathId.LocalPathId, colDescr.GetId())
                    .Update(
                        ...
                        NIceDb::TUpdate<Schema::MigratedColumns::DefaultKind>(
                            ETableColumnDefaultKind(colDescr.GetDefaultKind())
                        ),
            ...
            for (const NKikimrScheme::TMigratePartition& partDescr : tableDescr.GetPartitions()) {
                const auto shardIdx =
                    TShardIdx(partDescr.GetShardIdx().GetOwnerId(), partDescr.GetShardIdx().GetLocalId());
                ...
                if (partDescr.HasPartitionConfig()) {
                    db.Table<Schema::MigratedTableShardPartitionConfigs>()
                        .Key(shardIdx.GetOwnerId(), shardIdx.GetLocalId())
                        .Update(NIceDb::TUpdate<Schema::MigratedTableShardPartitionConfigs::PartitionConfig>(
                            partDescr.GetPartitionConfig()
                        ));
                }
            }
        }
        ...
        if ((TPathElement::EPathType)pathDescr.GetPathType() == TPathElement::EPathType::EPathTypeTableIndex) {
            ...
            db.Table<Schema::MigratedTableIndex>()
                .Key(pathId.OwnerId, pathId.LocalPathId)
                .Update(
                    NIceDb::TUpdate<Schema::MigratedTableIndex::AlterVersion>(tableIndexDescr.GetAlterVersion()),
                    NIceDb::TUpdate<Schema::MigratedTableIndex::IndexType>(
                        NKikimrSchemeOp::EIndexType(tableIndexDescr.GetType())
                    ),
                    NIceDb::TUpdate<Schema::MigratedTableIndex::State>(
                        NKikimrSchemeOp::EIndexState(tableIndexDescr.GetState())
                    )
                );

            for (ui32 keyId = 0; keyId < tableIndexDescr.KeysSize(); ++keyId) {
                db.Table<Schema::MigratedTableIndexKeys>()
                    .Key(pathId.OwnerId, pathId.LocalPathId, keyId)
                    .Update(NIceDb::TUpdate<Schema::MigratedTableIndexKeys::KeyName>(tableIndexDescr.GetKeys(keyId)));
            }
        }
