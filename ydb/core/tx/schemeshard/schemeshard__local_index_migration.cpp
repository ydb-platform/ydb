    ...
    void ProcessItems(const TActorContext& ctx) {
        ...
        if (pathParts.empty()) {
            ...
            if (item.Backoff.HasMore()) {
                ...
                YDB_LOG_ERROR_CTX(ctx, "LocalIndexMigrator failed to split path will retry after delay",
        ...
        if (item.IsColumnTable) {
            ...
            if (!NOlap::ConvertCreationConfigToRequested(item.IndexConfig, upsertIndex)) {
                ...
                if (item.Backoff.HasMore()) {
                    auto delay = item.Backoff.Next();
                    YDB_LOG_ERROR_CTX(ctx, "LocalIndexMigrator failed to convert index config for will retry after delay",
                        {"atSchemeshard", SelfTabletId},
                        {"#_item.WorkingDir", item.WorkingDir},
                        {"delay", delay});
                    ctx.Schedule(delay, new TEvents::TEvWakeup());
                } else {
                    YDB_LOG_CRIT_CTX(ctx, "LocalIndexMigrator failed to convert index config for after max retries, dying",
                        {"atSchemeshard", SelfTabletId},
                        {"#_item.WorkingDir", item.WorkingDir});
                    PassAway();
                }
    ...
    void Handle(TEvSchemeShard::TEvModifySchemeTransactionResult::TPtr& ev, const TActorContext& ctx) {
        ...
        default:
            if (AwaitingRequests.at(txId).Backoff.HasMore()) {
                ...
                YDB_LOG_ERROR_CTX(ctx, "LocalIndexMigrator failed to will retry after delay",
                    {"atSchemeshard", SelfTabletId},
                ...
                YDB_LOG_CRIT_CTX(ctx, "LocalIndexMigrator failed to after max retries, dying",
                    {"atSchemeshard", SelfTabletId},
                    {"#_AwaitingRequests.at(txId).DebugString", AwaitingRequests.at(txId).DebugString()},
                    {"reason", record.GetReason()});
                PassAway();
