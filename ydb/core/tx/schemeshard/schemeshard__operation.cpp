    ...
    for (auto& part : parts) {
        ...
        if (response->IsDone()) {
            ...
            if (!operation->Parts.empty()) {
                YDB_LOG_NOTICE_CTX(context.Ctx, "Abort operation: IgniteOperation fail to propose a part already accepted propose result with tx",
        ...
        if (prevProposeUndoSafe && !context.IsUndoChangesSafe()) {
            ...
            YDB_LOG_WARN_CTX(context.Ctx, "Operation part proposed ok, but propose itself is undo unsafe suboperation first GetDB called",
...
void AbortOperation(TOperationContext& context, const TTxId txId, const TString& reason) {
    YDB_LOG_ERROR_CTX(context.Ctx, "TTxOperationPropose Execute operation is rejected and all changes reverted",
        {"txId", txId},
        {"reason", reason},
        {"atSchemeshard", context.SS->SelfTabletId()});
...
void OutOfScopeEventHandler<TEvDataShard::TEvSchemaChanged>(const TEvDataShard::TEvSchemaChanged::TPtr& ev, TOperationContext& context) {
    ...
    YDB_LOG_DEBUG_CTX(context.Ctx, "TTxOperationReply< > execute send out-of-scope reply, for txId",
    ...
    bool Execute(NTabletFlatExecutor::TTransactionContext& txc, const TActorContext& ctx) override {
...

        {
            ...
            if (part) {
                ...
                YDB_LOG_WARN_CTX(ctx, "TTxOperationReply< > execute unknown operation or suboperation is already done, event is out-of-scope",
                    {"#_EvReply->GetTypeName", EvReply->GetTypeName()},
                    {"operationId", OperationId},
                    {"atSchemeshard", Self->TabletID()});
