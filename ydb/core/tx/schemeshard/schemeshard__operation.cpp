...
struct TSchemeShard::TTxOperationProposeCancelTx: public NTabletFlatExecutor::TTransactionBase<TSchemeShard> {
    ...
    TTxType GetTxType() const override {
        return TXTYPE_CANCEL_BACKUP_IMPL;
    }
...
THolder<TProposeResponse> TSchemeShard::IgniteOperation(TProposeRequest& request, TOperationContext& context) {
    ...
    if (Operations.contains(txId)) {
        ...
        response->SetError(
            ...
            "There is operation with the same txId has been found in flight."
...
bool IsCommitRedoSizeOverLimit(TString* reason, TOperationContext& context) {
    ...
    if (commitRedoBytes >= limitBytes) {
        *reason =
            TStringBuilder() << "local tx commit redo size generated by IgniteOperation() is more than allowed limit: "
                             << "commit redo size " << commitRedoBytes << ", limit " << limitBytes << ", excess "
                             << (commitRedoBytes - limitBytes);
        return true;
    }
    return false;
}
...
struct TSchemeShard::TTxOperationPropose: public NTabletFlatExecutor::TTransactionBase<TSchemeShard> {
    ...
    bool Execute(NTabletFlatExecutor::TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        if (tokenParseError) {
            Response = MakeHolder<TProposeResponse>(
                NKikimrScheme::StatusInvalidParameter, ui64(txId), ui64(selfId), "Failed to parse user token"
            );
            return true;
        }
        if (userToken) {
        ...
        if (Self->Operations.contains(txId)) {
            ...
            if (IsCommitRedoSizeOverLimit(&reason, context)) {
                Response =
                    MakeHolder<TProposeResponse>(NKikimrScheme::StatusSchemeError, ui64(txId), ui64(selfId), reason);
...
//
template <>
void OutOfScopeEventHandler<TEvDataShard::TEvSchemaChanged>(
    ...
    bool Execute(NTabletFlatExecutor::TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        for (size_t i = 0; i < txCount; ++i) {
            ...
            for (ui32 partIdx = 0; partIdx < operation->Parts.size(); ++partIdx) {
                ...
                TOperationContext context{Self, txc, ctx, OnComplete, MemChanges, DbChanges};
                THolder<TEvPrivate::TEvOperationPlan> msg =
                    MakeHolder<TEvPrivate::TEvOperationPlan>(ui64(step), ui64(txId));
                TEvPrivate::TEvOperationPlan::TPtr personalEv =
                    (TEventHandle<TEvPrivate::TEvOperationPlan>*)new IEventHandle(
...
NTabletFlatExecutor::ITransaction* TSchemeShard::CreateTxOperationPropose(
...
TString JoinPath(const TString& workingDir, const TString& name) {
    ...
    return TStringBuilder() << workingDir << (workingDir.EndsWith('/') ? "" : "/") << name;
}
...
TOperation::TSplitTransactionsResult
TOperation::SplitIntoTransactions(const TTxTransaction& tx, const TOperationContext& context) {
    ...
    while (path != parentPath) {
        TPath::TChecker checks = path.Check();
        checks.NotUnderDomainUpgrade().IsAtLocalSchemeShard();
        ...
        if (path.IsResolved()) {
            ...
            if (path.IsDeleted()) {
                ...
                checks.NotDeleted().NotUnderDeleting().IsCommonSensePath().IsLikeDirectory();
...
ISubOperation::TPtr
TOperation::RestorePart(TTxState::ETxType txType, TTxState::ETxState txState, TOperationContext& context) const {
    switch (txType) {
...
void TOperation::DoNotify(TSchemeShard*, TSideEffects& sideEffects, const TActorContext& ctx) {
    ...
    for (auto& subscriber : Subscribers) {
        THolder<TEvSchemeShard::TEvNotifyTxCompletionResult> msg =
            MakeHolder<TEvSchemeShard::TEvNotifyTxCompletionResult>(ui64(TxId));
        LOG_DEBUG_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD,
                    "TOperation DoNotify"
...
void TOperation::DoPropose(TSchemeShard* ss, TSideEffects& sideEffects, const TActorContext& ctx) const {
...

    {
        ...
        THolder<TEvTxProxy::TEvProposeTransaction> message(new TEvTxProxy::TEvProposeTransaction(
            ui64(coordinatorId), ui64(TxId), execLevel, ui64(effectiveMinStep), ui64(maxStep)
        ));
        ...
        sideEffects.BindMsgToPipe(
            TOperationId(TxId, InvalidSubTxId), coordinatorId, TPipeMessageId(0, TxId), message.Release()
        );
    }
}
...
TSet<TOperationId> TOperation::ActivatePartsWaitPublication(TPathId pathId, ui64 pathVersion) {
    ...
    while (it != WaitingPublicationsByPath.end() && it->first.first == pathId && it->first.second <= pathVersion) {
