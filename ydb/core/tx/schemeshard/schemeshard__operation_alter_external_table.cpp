...
private:
    ...
    TString DebugHint() const override {
        return TStringBuilder() << "TAlterExternalTable TPropose"
    ...
    void ClearDescribePathCaches(
        ...
        const TPathElement::TPtr& oldDataSourcePathPtr
    ) const {
...
public:
    explicit TPropose(TOperationId id, bool isSameDataSource, const TPathId oldSourcePathId)
...
private:
    ...
    static void AddPathInSchemeShard(const THolder<TProposeResponse>& result, TPath& dstPath) {
        result->SetPathId(dstPath.Base()->PathId.LocalPathId);
    }
    ...
    void CreateTransaction(
        ...
        TTxState& txState = context.SS->CreateTx(
            OperationId, TTxState::TxAlterExternalTable, externalTablePathId, externalDataSourcePathId
        );
    ...
    static void LinkExternalDataSourceWithExternalTable(
        ...
        if (!isSameDataSource) {
            ...
            EraseIf(
                *oldDataSource->ExternalTableReferences.MutableReferences(),
                [pathId =
                     externalTable->PathId](const NKikimrSchemeOp::TExternalTableReferences::TReference& reference) {
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString& owner, TOperationContext& context) override {
        ...
        const auto oldExternalTableInfo = context.SS->ExternalTables.Value(dstPath->PathId, nullptr);
        ...
        LinkExternalDataSourceWithExternalTable(
            externalDataSource, externalTable, dstPath, oldDataSource, IsSameDataSource
        );
