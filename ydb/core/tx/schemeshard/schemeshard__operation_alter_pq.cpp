...
public:
            ...
            TOperationContext& context)
    {
        ...
        YDB_LOG_DEBUG_CTX(context.Ctx, "AlterPQGroup Parts -> Groups -> -> adding deleting",
            ...
            {"#_count", pqGroup->TotalGroupCount},
            ...
            {"#_partitions", pqGroup->AlterData->PartitionsToDelete.size()});
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        YDB_LOG_NOTICE_CTX(context.Ctx, "TAlterPQ Propose /",
            {"path", parentPathStr},
            {"name", name},
            {"pathId", pathId},
            {"opId", OperationId},
                ...
                for (const size_t childIndex : {0, 1}) {
                    ...
                    if (prescribedChildPartitionId && ShouldCreateSiblingAtRootLevel(split, topic)) [[unlikely]] {
                        if (topic->Partitions.contains(*prescribedChildPartitionId)) {
                            YDB_LOG_TRACE_CTX(context.Ctx, "Dump #_num_0",
