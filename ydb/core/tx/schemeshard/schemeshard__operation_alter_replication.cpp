...
class TConfigureParts: public TSubOperationState {
    TString DebugHint() const override {
        return TStringBuilder() << "TAlterReplication TConfigureParts"
                                << " opId# " << OperationId << " ";
    }
...
public:
    ...
    bool ProgressState(TOperationContext& context) override {
        ...
        for (const auto& shard : txState->Shards) {
            ...
            if (tabletId == InvalidTabletId) {
                ...
                if (alterData->Description.GetState().GetStateCase() !=
                    context.SS->Replications.at(pathId)->Description.GetState().GetStateCase()) {
    ...
    bool HandleReply(NReplication::TEvController::TEvAlterReplicationResult::TPtr& ev, TOperationContext& context)
        override {
...
class TPropose: public TSubOperationState {
    TString DebugHint() const override {
        return TStringBuilder() << "TAlterReplication TPropose"
                                << " opId# " << OperationId << " ";
    }
...
class TAlterReplication: public TSubOperation {
    ...
    bool ValidateAlterState(
        TProposeResponse& result,
        const NKikimrSchemeOp::TReplicationDescription& desc,
        const NKikimrReplication::TReplicationState& newState
    ) {
    ...
    bool ValidateAlterConfig(
        TProposeResponse& result,
        const NKikimrSchemeOp::TReplicationDescription& desc,
        const NKikimrReplication::TReplicationConfig& newConf
    ) {
        switch (desc.GetState().GetStateCase()) {
            using TState = NKikimrReplication::TReplicationState;
            case TState::kStandBy:
                result.SetError(
                    ...
                    "Please ensure the replication is not in StandBy state before attempting to modify its settings. "
                    "Modifications are not allowed in StandBy state"
                );
                return false;
            case TState::kPaused:
            case TState::kDone:
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        const auto pathId = op.HasPathId() ? PathIdFromPathId(op.GetPathId()) : InvalidPathId;
        ...
        const auto path = pathId ? TPath::Init(pathId, context.SS) : TPath::Resolve(workingDir, context.SS).Dive(name);
        {
        ...
        if (op.HasConfig() && op.HasState()) {
            result->SetError(
                NKikimrScheme::StatusInvalidParameter,
                "It is not allowed to change both settings and the state of the replication in the same query. Please "
                "submit separate queries for each action"
            );
            return result;
        }
            ...
            if (newConf.HasSrcConnectionParams()) {
                ...
                if (newParam.HasStaticCredentials()) {
                    ...
                    if (newParam.GetStaticCredentials().HasPasswordSecretName()) {
                        oldParam.MutableStaticCredentials()->SetPasswordSecretName(
                            newParam.GetStaticCredentials().GetPasswordSecretName()
                        );
        ...
        txState.Shards.emplace_back(
            replication->AlterData->ControllerShardIdx, ETabletType::ReplicationController, TTxState::ConfigureParts
        );
