...
public:
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        if (0 != settings.GetPlanResolution()) {
            if (subDomainInfo->GetPlanResolution() != 0 &&
                subDomainInfo->GetPlanResolution() != settings.GetPlanResolution()) {
                result->SetError(
            ...
            if (subDomain->IsRoot()) {
                result->SetError(
                    NKikimrScheme::StatusInvalidParameter,
                    "Malformed subdomain request: unable to change PlanResolution at root, only addition storage pools "
                    "is allowed"
                );
                return result;
            }
        ...
        if (0 != settings.GetTimeCastBucketsPerMediator()) {
            ...
            if (subDomain->IsRoot()) {
                result->SetError(
                    NKikimrScheme::StatusInvalidParameter,
                    "Malformed subdomain request: unable to change TimeCastBucketsPerMediator at root, only addition "
                    "storage pools is allowed"
                );
                return result;
            }
        }
        ...
        auto requestedPools =
            TVector<TStoragePool>(settings.GetStoragePools().begin(), settings.GetStoragePools().end());
        ...
        if (uniqEnd != requestedPools.end()) {
            result->SetError(
                NKikimrScheme::StatusInvalidParameter,
                "Malformed subdomain request: requested storage pools is not unique, for example, the pool '" +
...

        {
            ...
            if (omittedPools && requestedPools) {
                result->SetError(
                    NKikimrScheme::StatusInvalidParameter,
                    "Malformed subdomain request: deleting storage pool is not allowed, for example, requested "
                    "deletion '" +
                        omittedPools.begin()->GetName() + "'"
                );
                return result;
            }
        }
        ...
        TSubDomainInfo::TPtr alterData = new TSubDomainInfo(
            *subDomainInfo, settings.GetPlanResolution(), settings.GetTimeCastBucketsPerMediator(), addedPools
        );
