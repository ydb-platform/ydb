...
public:
    ...
    THolder<TProposeResponse> Propose(const TString& owner, TOperationContext& context) override {
        ...
        switch (Action) {
            case EAction::Create:
                return ProposeCreate(owner, context);
            case EAction::Alter:
                return ProposeAlter(owner, context);
            case EAction::Drop:
                return ProposeDrop(owner, context);
        }
        ...
        TPath dstPath = parentPath.Child(description.GetName());
        {
            ...
            if (dstPath.IsResolved()) {
                checks.IsResolved().NotUnderDeleting().FailOnExist(
                    TPathElement::EPathType::EPathTypeBlobDepot, !Transaction.GetFailOnExist()
                );
            } else {
