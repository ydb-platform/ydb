...
bool TCreateParts::HandleReply(TEvHive::TEvAdoptTabletReply::TPtr& ev, TOperationContext& context) {
    YDB_LOG_CTX_INFO(context.Ctx, "HandleReply TEvAdoptTablet",
        {"DebugHint", DebugHint()},
        {"at_tabletId", context.SS->SelfTabletId()});
    YDB_LOG_CTX_DEBUG(context.Ctx, "HandleReply TEvAdoptTablet, message%",
        {"DebugHint", DebugHint()},
    ...
    if (!context.SS->AdoptedShards.contains(shardIdx)) {
        YDB_LOG_CTX_INFO(context.Ctx, "HandleReply TEvAdoptTablet Got TTxAdoptTabletReply for shard but it is not present in AdoptedShards",
            {"DebugHint", DebugHint()},
            {"shardIdx", shardIdx},
            {"tabletId", tabletId});
        return false;
...
bool TCreateParts::HandleReply(TEvHive::TEvCreateTabletReply::TPtr& ev, TOperationContext& context) {
    YDB_LOG_CTX_INFO(context.Ctx, "HandleReply TEvCreateTabletReply",
        {"DebugHint", DebugHint()},
        {"at_tabletId", context.SS->SelfTabletId()});
    YDB_LOG_CTX_DEBUG(context.Ctx, "HandleReply TEvCreateTabletReply",
        {"DebugHint", DebugHint()},
    ...
    if (status ==  NKikimrProto::BLOCKED) {
        ...
        YDB_LOG_CTX_NOTICE(context.Ctx, "CreateRequest BLOCKED",
            {"DebugHint", DebugHint()},
            {"at_Hive", hive},
            {"msg", DebugReply(ev)});
    ...
    if (status ==  NKikimrProto::INVALID_OWNER) {
        ...
        YDB_LOG_CTX_DEBUG(context.Ctx, "CreateRequest Redirect",
            {"DebugHint", DebugHint()},
            {"from_Hive", hive},
            {"to_Hive", redirectTo},
...
THolder<TEvHive::TEvAdoptTablet> TCreateParts::AdoptRequest(TShardIdx shardIdx, TOperationContext& context) {
    ...
    YDB_LOG_CTX_DEBUG(context.Ctx, "AdoptRequest Event",
        {"DebugHint", DebugHint()},
        {"to_Hive", ev->Record.DebugString().c_str()});
...
bool TCreateParts::ProgressState(TOperationContext& context) {
    ...
    if (txState->TxType == TTxState::TxDropTable
        ...
        if (NTableState::SourceTablePartitioningChangedForCopyTable(*txState, context)) {
            YDB_LOG_CTX_INFO(context.Ctx, "ProgressState SourceTablePartitioningChangedForCopyTable, tx",
                {"DebugHint", DebugHint()},
                {"type", TTxState::TypeName(txState->TxType)});
    ...
    for (const auto& shard : txState->Shards) {
        ...
        if (context.SS->AdoptedShards.contains(shard.Idx)) {
            ...
            YDB_LOG_CTX_DEBUG(context.Ctx, "CreateRequest Event",
                {"DebugHint", DebugHint()},
                {"to_Hive", hiveToRequest},
    ...
    if (nothingToDo) {
        YDB_LOG_CTX_DEBUG(context.Ctx, "ProgressState no shards to create, do next state",
            {"DebugHint", DebugHint()});
        ...
        TFuncToString toString)
{
    ...
    YDB_LOG_CTX_INFO(context.Ctx, "TEvProposeTransactionResult",
        {"at_tablet", ssId});
    ...
    if (!checkPrepared(status)) {
        YDB_LOG_CTX_ERROR(context.Ctx, "Ignore TEvProposeTransactionResult as not prepared, result",
            {"shard", tabletId},
            {"operationId", operationId},
    ...
    if (!txState.ShardsInProgress.contains(shardIdx)) {
        YDB_LOG_CTX_DEBUG(context.Ctx, "Ignore TEvProposeTransactionResult as duplicate",
            {"shard", tabletId},
            {"shardIdx", shardIdx},
            {"operationId", operationId},
            {"at_schemeshard", ssId});
    ...
    YDB_LOG_CTX_DEBUG(context.Ctx, "CollectProposeTransactionResults accept TEvProposeTransactionResult, left",
        {"shard", tabletId},
        {"shardIdx", shardIdx},
        {"operationId", operationId},
        ...
        TOperationContext& context)
{
    ...
    if (pTablet && generation && (pTablet->second >= *generation)) {
        YDB_LOG_CTX_DEBUG(context.Ctx, "CollectSchemaChanged Ignore as outdated, shard, event, known",
            {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()},
            {"operationId", operationId},
            {"shardIdx", shardIdx},
            {"shardId", shardId},
            {"generation", generation},
            {"#_generation", pTablet->second},
            {"at_schemeshard", ssId});
        return false;
    }
    ...
    YDB_LOG_CTX_DEBUG(context.Ctx, "CollectSchemaChanged accept, left",
        {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()},
        {"operationId", operationId},
        {"shardIdx", shardIdx},
        {"shard", shardId},
...
void AckAllSchemaChanges(const TOperationId &operationId, TTxState &txState, TOperationContext &context) {
    ...
    YDB_LOG_CTX_INFO(context.Ctx, "all shard schema changes has been received",
        {"operationId", operationId},
        {"at_schemeshard", ssId});
    ...
    for (const auto& items : txState.SchemeChangeNotificationReceived) {
        ...
        YDB_LOG_CTX_DEBUG(context.Ctx, "send schema changes ack message",
...
bool TProposedWaitParts::HandleReplyImpl(const TEvent& ev, TOperationContext& context) {
    ...
    YDB_LOG_CTX_INFO(context.Ctx, "HandleReply",
        ...
        {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()},
        {"at_tablet", ssId});
    YDB_LOG_CTX_DEBUG(context.Ctx, "HandleReply",
        {"DebugHint", DebugHint()},
        {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()},
        {"at_tablet", ssId},
    ...
    if (!CollectSchemaChanged(OperationId, ev, context)) {
        YDB_LOG_CTX_DEBUG(context.Ctx, "HandleReply CollectSchemaChanged: false",
            {"DebugHint", DebugHint()},
            {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()});
        return false;
    }
    ...
    if (!txState.ReadyForNotifications) {
        YDB_LOG_CTX_DEBUG(context.Ctx, "HandleReply ReadyForNotifications: false",
            {"DebugHint", DebugHint()},
            {"#_TEvSchemaChangedTraits<TEvent>::GetName()", TEvSchemaChangedTraits<TEvent>::GetName()});
        return false;
    }
...
void CollectShards(const THashSet<TPathId>& paths, TOperationId operationId, TTxState *txState, TOperationContext &context) {
    ...
    for (auto shardIdx: shards) {
        ...
        auto& shardInfo = context.SS->ShardInfos.at(shardIdx);
        YDB_LOG_CTX_DEBUG(context.Ctx, "Collect shard, shard, path",
            {"idx", shardIdx},
...
void AbortUnsafeDropOperation(const TOperationId& opId, const TTxId& txId, TOperationContext& context) {
    ...
    YDB_LOG_CTX_NOTICE(context.Ctx, "AbortUnsafe",
