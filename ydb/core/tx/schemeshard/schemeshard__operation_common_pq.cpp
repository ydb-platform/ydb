    ...
    )
{
    ...
    YDBLOG_CTX_DEBUG(context.Ctx, "Propose configure PersQueue, message: ",
                            ...
                            TOperationContext& context)
{
    ...
    if (evRecord.GetStatus() == NKikimrPQ::TEvProposeTransactionResult::COMPLETE) {
        ...
        YDBLOG_CTX_DEBUG(context.Ctx, "CollectPQConfigChanged accept TEvPersQueue::TEvProposeTransactionResult, operationId: , shardIdx: , shard: , left await: , txState.State: , txState.ReadyForNotifications: , at schemeshard: ",
                            ...
                            TOperationContext& context)
{
    ...
    YDBLOG_CTX_DEBUG(context.Ctx, "CollectPQConfigChanged accept TEvDataShard::TEvProposeTransactionAttachResult, operationId: , shardIdx: , shard: , left await: , txState.State: , txState.ReadyForNotifications: , at schemeshard: ",
        {"operationId", operationId},
        {"shardIdx", shardIdx},
        {"shard", shardId},
        {"await", txState.ShardsInProgress.size()},
...
bool TConfigureParts::HandleReply(TEvPersQueue::TEvProposeTransactionResult::TPtr& ev, TOperationContext& context)
{
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply TEvProposeTransactionResult, at schemeshard: ",
...
bool TConfigureParts::HandleReply(TEvPersQueue::TEvUpdateConfigResponse::TPtr& ev, TOperationContext& context) {
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply TEvUpdateConfigResponse at tablet",
        {"#_DebugHint()", DebugHint()},
        {"#_ssId", ssId});
    YDBLOG_CTX_DEBUG(context.Ctx, " HandleReply TEvUpdateConfigResponse message:  at tablet",
        {"#_DebugHint()", DebugHint()},
    ...
    if (status == NKikimrPQ::ERROR_UPDATE_IN_PROGRESS) {
        YDBLOG_CTX_ERROR(context.Ctx, "PQ reconfiguration is in progress. We'll try to finish it later. Tx  tablet  at schemeshard: ",
...
bool TConfigureParts::ProgressState(TOperationContext& context) {
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply ProgressState, at schemeshard: ",
        {"#_DebugHint()", DebugHint()},
        {"schemeshard", ssId});
    ...
    for (auto shard : txState->Shards) {
        ...
        if (shard.TabletType == ETabletType::PersQueue) {
            ...
            YDBLOG_CTX_DEBUG(context.Ctx, "Propose configure PersQueue, opId: , tabletId: , Partitions size: , at schemeshard: ",
                {"opId", OperationId},
                {"tabletId", tabletId},
            ...
            YDBLOG_CTX_DEBUG(context.Ctx, "Propose configure PersQueue, opId: , tabletId: , at schemeshard: ",
                {"opId", OperationId},
                {"tabletId", tabletId},
                {"schemeshard", ssId});
            ...
            YDBLOG_CTX_DEBUG(context.Ctx, "Propose configure PersQueueReadBalancer, opId: , tabletId: , at schemeshard: ",
                {"opId", OperationId},
                {"tabletId", tabletId},
                {"schemeshard", ssId});
            ...
            YDBLOG_CTX_DEBUG(context.Ctx, "Propose configure PersQueueReadBalancer, opId: , tabletId: , message: , at schemeshard: ",
                {"opId", OperationId},
                {"tabletId", tabletId},
                {"message", event->Record.ShortUtf8DebugString()},
                {"schemeshard", ssId});
...
bool TPropose::HandleReply(TEvPersQueue::TEvProposeTransactionResult::TPtr& ev, TOperationContext& context)
{
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply TEvProposeTransactionResult triggers early, at schemeshard:  message# ",
        {"#_DebugHint()", DebugHint()},
        {"schemeshard", ssId},
        {"message", evRecord.ShortDebugString()});

    const bool collected = CollectPQConfigChanged(OperationId, ev, context);
    YDBLOG_CTX_DEBUG(context.Ctx, " HandleReply TEvProposeTransactionResult CollectPQConfigChanged: ",
        {"#_DebugHint()", DebugHint()},
...
bool TPropose::HandleReply(TEvDataShard::TEvProposeTransactionAttachResult::TPtr& ev, TOperationContext& context)
{
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply TEvProposeTransactionAttachResult triggers early, at schemeshard:  message# ",
        {"#_DebugHint()", DebugHint()},
        {"schemeshard", ssId},
        {"message", evRecord.ShortDebugString()});

    const bool collected = CollectPQConfigChanged(OperationId, ev, context);
    YDBLOG_CTX_DEBUG(context.Ctx, " HandleReply TEvProposeTransactionAttachResult CollectPQConfigChanged: ",
        {"#_DebugHint()", DebugHint()},
        {"CollectPQConfigChanged", (collected ? "true" : "false")});
...
bool TPropose::HandleReply(TEvPrivate::TEvOperationPlan::TPtr& ev, TOperationContext& context)
{
    ...
    YDBLOG_CTX_INFO(context.Ctx, " HandleReply TEvOperationPlan, step: , at tablet: ",
        {"#_DebugHint()", DebugHint()},
        {"step", step},
        {"tablet", ssId});
...
bool TPropose::ProgressState(TOperationContext& context)
{
    ...
    YDBLOG_CTX_INFO(context.Ctx, "NPQState::TPropose ProgressState, operationId: , at schemeshard: ",
        {"operationId", OperationId},
        {"schemeshard", ssId});
...
{
    if (!txState.ShardsInProgress.empty()) {
        YDBLOG_CTX_DEBUG(context.Ctx, " can't persist state: ShardsInProgress is not empty, remain: ",
            {"#_DebugHint()", DebugHint()},
