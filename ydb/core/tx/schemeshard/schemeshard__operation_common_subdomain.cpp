...
bool TConfigureParts::HandleReply(TEvSchemeShard::TEvInitTenantSchemeShardResult::TPtr& ev, TOperationContext& context) {
    ...
    if (status != NKikimrScheme::EStatus::StatusSuccess && status != NKikimrScheme::EStatus::StatusAlreadyExists) {
        YDB_LOG_CRIT_CTX(context.Ctx, "Got error status on SubDomain Configure from tenant schemeshard",
            {"debugHint", DebugHint()},
            {"tablet", tabletId},
            {"shard", shardIdx},
...
bool TConfigureParts::HandleReply(TEvSubDomain::TEvConfigureStatus::TPtr& ev, TOperationContext& context) {
    ...
    if (status == NKikimrTx::TEvSubDomainConfigurationAck::REJECT) {
        YDB_LOG_CRIT_CTX(context.Ctx, "Got REJECT on SubDomain Configure",
            {"debugHint", DebugHint()},
            {"fromTablet", tabletId},
            {"shard", shardIdx},
            {"opId", OperationId},
    ...
    for (auto& shard : txState->Shards) {
        ...
        case ETabletType::Mediator: {
            YDB_LOG_DEBUG_CTX(context.Ctx, "Send configure request",
                {"#_to_coordinator/mediator", tabletID},
                {"opId", OperationId},
                {"schemeshard", ssId});
