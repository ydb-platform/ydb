    ...
    TVector<ISubOperation::TPtr>& result)
{
    ...
    for (const auto& descr: op.GetCopyTableDescriptions()) {
        ...
        YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Processing table",
        ...
        if (context.SS->Tables.contains(srcPath.Base()->PathId)) {
            ...
            YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Table info",
            ...
            for (size_t i = 0; i < static_cast<size_t>(tableDesc.TableIndexesSize()); ++i) {
                ...
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Table has index in description",
        ...
        if (context.SS->ColumnTables.contains(srcPath.Base()->PathId)) {
            ...
            YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Column Table info",
        ...
        for (const auto& child: srcPath.Base()->GetChildren()) {
            ...
            YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Child found",
                {"name", name},
                {"pathId", pathId},
        ...
        for (const auto& child: srcPath.Base()->GetChildren()) {
            ...
            if (srcIndexPath.IsDeleted()) {
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Skipping deleted",
                    {"child", name});
                continue;
            }

            if (srcIndexPath.IsSequence()) {
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Skipping sequence",
                    {"child", name});
                continue;
            }

            if (descr.GetOmitIndexes()) {
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Skipping due",
                    {"to_OmitIndexes", name});
                continue;
            }

            if (!srcIndexPath.IsTableIndex()) {
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Skipping non-index",
                    {"child", name});
                continue;
            }

            YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Creating index copy operation",
                {"for", name});
            ...
            for (const auto& [srcImplTableName, srcImplTablePathId] : srcIndexPath.Base()->GetChildren()) {
                ...
                YDBLOG_CTX_TRACE(context.Ctx, "CreateConsistentCopyTables: Creating index impl table copy",
