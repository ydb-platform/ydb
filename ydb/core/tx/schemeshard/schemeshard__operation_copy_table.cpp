...
public:
    explicit TCopyColumnTable(const TOperationId& id, TTxState::ETxState txState, TTxState* state)
        : TSubOperation(id, txState)
    {
        Y_ENSURE(state);
        TargetState = state->TargetPathTargetState;
    ...
    explicit TCopyColumnTable(const TOperationId& id, const TTxTransaction& tx, TMaybe<TPathElement::EPathState> targetState)
        : TSubOperation(id, tx)
        , TargetState(targetState)
    {
    }
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        const TString& parentPath = Transaction.GetWorkingDir();
        const TString& name = Transaction.GetCreateColumnTable().GetName();

        LOG_NOTICE_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD,
                     "TCopyColumnTable Propose"
                         << ", path: " << parentPath << "/" << name
                         << ", opId: " << OperationId
                         << ", at schemeshard: " << ssId);
        ...
        TPath srcPath = TPath::Resolve(name, context.SS);
        ...
        TColumnTableInfo::TPtr srcTableInfo = context.SS->ColumnTables.at(srcPath.Base()->PathId).GetPtr();

        context.MemChanges.GrabNewTxState(context.SS, OperationId);
        context.DbChanges.PersistTxState(OperationId);
        TTxState& txState = context.SS->CreateTx(OperationId, TTxState::TxCopyTable, srcPath.Base()->PathId, srcPath.Base()->PathId);
...
{
    return tx.HasCreateColumnTable() ? MakeSubOperation<TCopyColumnTable>(id, tx, targetState) : MakeSubOperation<TCopyTable>(id, tx, localSequences, targetState);
}
