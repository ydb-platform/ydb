...
public:
    ...
    bool ProgressState(TOperationContext& context) override {
        ...
        for (const auto& shard : txState->Shards) {
            ...
            if (tabletId == InvalidTabletId) {
                ...
                LOG_DEBUG_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[" << context.SS->TabletID() << "] " <<DebugHint() << "Send TEvCreateReplication to controller"
                    << ": tabletId# " << tabletId
                    << ", ev# " << ev->ToString());
    ...
    bool HandleReply(NReplication::TEvController::TEvCreateReplicationResult::TPtr& ev, TOperationContext& context) override {
        ...
        case NKikimrReplication::TEvCreateReplicationResult::ALREADY_EXISTS:
            break;
        default:
            LOG_WARN_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[" << context.SS->TabletID() << "] " <<DebugHint() << "Ignoring unexpected TEvCreateReplicationResult"
                << " tabletId# " << tabletId
        ...
        if (!txState->ShardsInProgress.erase(shardIdx)) {
            LOG_WARN_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[" << context.SS->TabletID() << "] " <<DebugHint() << "Ignoring duplicate TEvCreateReplicationResult");
            return false;
        }
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString& owner, TOperationContext& context) override {
        ...
        LOG_NOTICE_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[" << context.SS->TabletID() << "] " <<"TCreateReplication Propose"
            << ": opId# " << OperationId
    ...
    void AbortUnsafe(TTxId txId, TOperationContext& context) override {
        LOG_NOTICE_S(context.Ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[" << context.SS->TabletID() << "] " <<"TCreateReplication AbortUnsafe"
            << ": opId# " << OperationId
            << ", txId# " << txId);
