...
using TAffectedESchemeOpCreateSequence = TAffectedPathsTraits<NKikimrSchemeOp::EOperationType::ESchemeOpCreateSequence>;

namespace NOperation {

template <>
std::optional<TAffectedPaths> GetAffectedPaths<TAffectedESchemeOpCreateSequence>(
    TAffectedESchemeOpCreateSequence,
    const TTxTransaction& tx,
    const TOperationContext& context)
{
    Y_UNUSED(context);
    return DeclareChildOfWorkingDir(tx.GetWorkingDir(), tx.GetSequence().GetName());
}
