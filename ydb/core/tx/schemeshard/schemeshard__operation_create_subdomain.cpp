...
void DeclareShards(
    ...
    for (ui64 i = 0; i < count; ++i) {
        auto shardId = ss->RegisterShardInfo(TShardInfo(txId, pathId, type).WithBindedChannels(channelsBindings));
...
void PersistShards(NIceDb::TNiceDb& db, TTxState& txState, TSchemeShard* ss) {
    for (const auto& shard : txState.Shards) {
        ...
        ss->PersistShardMapping(
            db, shard.Idx, InvalidTabletId, shardInfo.PathId, shardInfo.CurrentTxId, shardInfo.TabletType
        );
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString& owner, TOperationContext& context) override {
        ...
        if (!parentPathStr) {
            result->SetError(NKikimrScheme::StatusInvalidParameter, "Malformed subdomain request: no working dir");
            return result;
        }

        if (!name) {
            result->SetError(NKikimrScheme::StatusInvalidParameter, "Malformed subdomain request: no name");
            return result;
        }
        ...
        NSchemeShard::TPath dstPath = parentPath.Child(name);
        {
            ...
            if (dstPath.IsResolved()) {
                checks.IsResolved().NotUnderDeleting().FailOnExist(
                    TPathElement::EPathType::EPathTypeSubDomain, acceptExisted
                );
            } else {
        ...
        if (0 == settings.GetTimeCastBucketsPerMediator() && !onlyDeclaration) {
            result->SetError(
                NKikimrScheme::StatusInvalidParameter, "Malformed subdomain request: TimeCastBucketsPerMediator is 0"
            );
            return result;
        }
        ...
        if (0 == settings.GetPlanResolution() && !onlyDeclaration) {
            result->SetError(
                NKikimrScheme::StatusInvalidParameter, "Malformed subdomain request: plan resolution is 0"
            );
            return result;
        }
        ...
        if (settings.HasResourcesDomainKey()) {
            result->SetError(
                NKikimrScheme::StatusInvalidParameter, "Resources domain key unsupported for non-external subdomains"
            );
            return result;
        }
        ...
        auto requestedStoragePools =
            TStoragePools(settings.GetStoragePools().begin(), settings.GetStoragePools().end());
        ...
        if (uniqEnd != requestedStoragePools.end()) {
            if (!context.SS->ChannelProfiles || context.SS->ChannelProfiles->Profiles.size() == 0) {
                result->SetError(
                    NKikimrScheme::StatusInvalidParameter,
                    "Requested not uniq storage pools, for example, '" + uniqEnd->GetName() + "'"
                );
                return result;
            }
        }
        ...
        TSubDomainInfo::TPtr alter = new TSubDomainInfo(
            1, settings.GetPlanResolution(), settings.GetTimeCastBucketsPerMediator(), newNode->PathId
        );
        ...
        if (settings.HasDatabaseQuotas()) {
            if (!requestedStoragePools.empty() &&
                !CheckStoragePoolsInQuotas(
                    settings.GetDatabaseQuotas(), requestedStoragePools, dstPath.PathString(), errStr
                )) {
                result->SetError(NKikimrScheme::StatusInvalidParameter, errStr);
                return result;
            }
