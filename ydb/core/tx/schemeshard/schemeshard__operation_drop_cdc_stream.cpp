...
class TPropose: public TSubOperationState {
    TString DebugHint() const override {
        return TStringBuilder() << "DropCdcStream TPropose"
                                << " opId# " << OperationId << " ";
    }
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        const auto streamPath = op.HasId() ? TPath::Init(context.SS->MakeLocalId(op.GetId()), context.SS)
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        if (DropSnapshot && !context.SS->TablesWithSnapshots.contains(tablePath.Base()->PathId)) {
            result->SetError(
                ...
                TStringBuilder() << "Table has no snapshots"
        ...
        const auto txType =
            DropSnapshot ? TTxState::TxDropCdcStreamAtTableDropSnapshot : TTxState::TxDropCdcStreamAtTable;
...
void DoDropStream(
...
) {
    {
        auto outTx = TransactionTemplate(
            workingDirPath.PathString(), NKikimrSchemeOp::EOperationType::ESchemeOpDropCdcStreamAtTable
        );
    ...
    if (lockTxId != InvalidTxId) {
        auto outTx =
            TransactionTemplate(workingDirPath.PathString(), NKikimrSchemeOp::EOperationType::ESchemeOpDropLock);
    ...
    {
        auto outTx =
            TransactionTemplate(tablePath.PathString(), NKikimrSchemeOp::EOperationType::ESchemeOpDropCdcStreamImpl);
    ...
    for (const auto& [name, pathId] : streamPath.Base()->GetChildren()) {
        ...
        auto outTx =
            TransactionTemplate(streamPath.PathString(), NKikimrSchemeOp::EOperationType::ESchemeOpDropPersQueueGroup);
...
CreateDropCdcStream(TOperationId opId, const TTxTransaction& tx, TOperationContext& context) {
    ...
    const auto lockTxId =
        stream->State == TCdcStreamInfo::EState::ECdcStreamStateScan ? streamPath.Base()->CreateTxId : InvalidTxId;
