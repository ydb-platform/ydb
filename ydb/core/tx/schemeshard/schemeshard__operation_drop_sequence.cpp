...
public:
    ...
    bool HandleReply(NSequenceShard::TEvSequenceShard::TEvDropSequenceResult::TPtr& ev, TOperationContext& context) override {
        ...
        YDB_LOG_DEBUG_CTX(context.Ctx, "TDropSequence TDropParts HandleReply TEvDropSequenceResult at tablet",
            {"shardId", tabletId},
            {"status", status},
            {"operationId", OperationId},
            {"ssId", ssId});
        ...
        switch (status) {
            ...
            default:
                ...
                YDB_LOG_WARN_CTX(context.Ctx, "TDropSequence TDropParts HandleReply ignoring unexpected TEvDropSequenceResult at tablet",
                    {"shardId", tabletId},
                    {"status", status},
                    {"operationId", OperationId},
                    {"ssId", ssId});
        ...
        if (!txState->ShardsInProgress.erase(shardIdx)) {
            YDB_LOG_WARN_CTX(context.Ctx, "TDropSequence TDropParts HandleReply ignoring duplicate TEvDropSequenceResult at tablet",
                {"shardId", tabletId},
                {"status", status},
                {"operationId", OperationId},
                {"ssId", ssId});
