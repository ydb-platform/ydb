...
void TSideEffects::ApplyOnExecute(TSchemeShard* ss, NTabletFlatExecutor::TTransactionContext& txc, const TActorContext& ctx) {
    YDBLOG_CTX_TRACE(ctx, "TSideEffects ApplyOnExecute at tablet# ",
        {"tablet", ss->TabletID()});
...
void TSideEffects::ApplyOnComplete(TSchemeShard* ss, const TActorContext& ctx) {
    YDBLOG_CTX_TRACE(ctx, "TSideEffects ApplyOnComplete at tablet# ",
        {"tablet", ss->TabletID()});
...
void TSideEffects::DoActivateOps(TSchemeShard* ss, const TActorContext& ctx) {
    for (auto txId: ActivationOps) {
        if (!ss->Operations.contains(txId)) {
            YDBLOG_CTX_INFO(ctx, "Unable to activate ",
                {"#_txId", txId});
            continue;
        }
        ...
        if (operation->WaitOperations.size()) {
            YDBLOG_CTX_INFO(ctx, "Delay activating, operation: , there is await operations num ",
                {"operation", txId},
                {"#_num_0", operation->WaitOperations.size()});
            continue;
        }

        for (ui32 partIdx = 0; partIdx < operation->Parts.size(); ++partIdx) {
            YDBLOG_CTX_TRACE(ctx, "Activate send for ",
    ...
    for (auto& opPart: ActivationParts) {
        ...
        if (operation->WaitOperations.size()) {
            YDBLOG_CTX_INFO(ctx, "Delay activating, operation part: , there is await operations num ",
                {"part", opPart},
                {"#_num_0", operation->WaitOperations.size()});
            continue;
        }
...
void TSideEffects::DoMediatorsAck(TSchemeShard* ss, const TActorContext& ctx) {
    for (auto& rec: MediatorAcks) {
        ...
        YDBLOG_CTX_TRACE(ctx, "Ack mediator stepId#",
            {"stepId", step});
...
void TSideEffects::DoCoordinatorAck(TSchemeShard* ss, const TActorContext& ctx) {
    ...
    for (auto& byCoordinator: toCoordinatorAck) {
        ...
        for (auto& byStep: byCoordinator.second) {
            ...
            YDBLOG_CTX_TRACE(ctx, "Ack coordinator stepId# first txId# countTxs#",
                {"stepId", step},
                {"txId", *txIds.begin()},
                {"countTxs", txIds.size()});
...
void TSideEffects::DoUpdateTenant(TSchemeShard* ss, NTabletFlatExecutor::TTransactionContext &txc, const TActorContext& ctx) {
    for (const TPathId pathId : TenantsToUpdate) {
        ...
        if (!ss->PathsById.at(pathId)->IsExternalSubDomainRoot()) {
            YDBLOG_CTX_DEBUG(ctx, "DoUpdateTenant no IsExternalSubDomainRoot, pathId: : , at schemeshard: ",
                {"#_pathId", pathId},
        ...
        if (!ss->SubDomainsLinks.IsActive(pathId)) {
            YDBLOG_CTX_INFO(ctx, "DoUpdateTenant no IsActiveChild, pathId: : , at schemeshard: ",
                {"#_pathId", pathId},
                {"schemeshard", ss->TabletID()});
            continue;
        }
        ...
        if (!hasChanges) {
            YDBLOG_CTX_DEBUG(ctx, "DoUpdateTenant no hasChanges, pathId: , tenantLink: , subDomain->GetVersion(): , actualEffectiveACLVersion: , actualUserAttrsVersion: , tenantHive: , tenantSysViewProcessor: , at schemeshard: ",
                {"pathId", pathId},
        ...
        YDBLOG_CTX_INFO(ctx, "Send TEvUpdateTenantSchemeShard, to actor: , msg: , at schemeshard: ",
...
void TSideEffects::DoPersistPublishPaths(TSchemeShard* ss, NTabletFlatExecutor::TTransactionContext& txc, const TActorContext& ctx) {
    ...
    for (const auto& kv : PublishPaths) {
        ...
        if (!ss->Operations.contains(txId)) {
            YDBLOG_CTX_DEBUG(ctx, "Cannot publish paths for unknown operation id#",
                {"id", txId});
            continue;
        }
...
void TSideEffects::DoSend(TSchemeShard* ss, const TActorContext& ctx) {
    for (auto& rec: Messages) {
        ...
        YDBLOG_CTX_TRACE(ctx, "Send  to actor:  msg type:  msg:  at schemeshard: ",
            {"actor", actor},
            {"type", message->Type()},
...
void TSideEffects::DoBindMsg(TSchemeShard *ss, const TActorContext &ctx) {
    for (auto& rec: BindedMessages) {
        ...
        YDBLOG_CTX_DEBUG(ctx, "Send tablet strongly msg operationId:  from tablet:  to tablet:  cookie:  msg type: ",
            {"operationId", opId},
            {"tablet", ss->TabletID()},
            {"tablet", tablet},
            {"cookie", cookie},
        ...
        if (!ss->Operations.contains(opId.GetTxId())) {
            YDBLOG_CTX_DEBUG(ctx, "Send tablet strongly msg , operation already done, operationId:  from tablet:  to tablet:  cookie:  msg type: ",
                {"operationId", opId},
                {"tablet", ss->TabletID()},
                {"tablet", tablet},
                {"cookie", cookie},
...
void TSideEffects::DoBindMsgAcks(TSchemeShard *ss, const TActorContext &ctx) {
    for (auto& ack: BindedMessageAcks) {
        ...
        YDBLOG_CTX_TRACE(ctx, "Ack tablet strongly msg opId:  from tablet:  to tablet:  cookie: ",
            {"opId", opId},
            {"tablet", ss->TabletID()},
            {"tablet", tablet},
            {"cookie", cookie});
...
void TSideEffects::DoDoneParts(TSchemeShard *ss, const TActorContext &ctx) {
    for (auto& opId: DoneOperations) {
        ...
        if (!ss->Operations.contains(txId)) {
            YDBLOG_CTX_DEBUG(ctx, "Part operation has been done before id#",
                {"id", opId});
            continue;
        }
        ...
        YDBLOG_CTX_INFO(ctx, "Part operation is done id# progress is /",
            {"id", opId},
...
void TSideEffects::DoDoneTransactions(TSchemeShard *ss, NTabletFlatExecutor::TTransactionContext &txc, const TActorContext &ctx) {
    for (auto& txId: DoneTransactions) {
        ...
        for (auto& dependent: operation->DependentOperations) {
            YDBLOG_CTX_DEBUG(ctx, "Remove dependency, parent tx: , dependent tx: ",
                {"tx", txId},
                {"tx", dependent});
...
void TSideEffects::DoSetBarriers(TSchemeShard *ss, const TActorContext &ctx) {
    for (auto& entry : Barriers) {
        ...
        if (it != ss->Operations.end()) {
            ...
            YDBLOG_CTX_DEBUG(ctx, "Set barrier, OperationId: , name: , done: , blocked: , parts count: ",
                {"OperationId", opId},
                {"name", name},
...
void TSideEffects::DoCheckBarriers(TSchemeShard *ss, NTabletFlatExecutor::TTransactionContext &txc, const TActorContext &ctx) {
    ...
    for (auto& txId : touchedOperations) {
        ...
        YDBLOG_CTX_NOTICE(ctx, "All parts have reached barrier, tx: , done: , blocked: ",
            {"tx", txId},
            {"done", operation->DoneParts.size()},
