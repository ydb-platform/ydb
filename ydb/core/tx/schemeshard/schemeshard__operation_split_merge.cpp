...
public:
    ...
    bool HandleReply(TEvDataShard::TEvInitSplitMergeDestinationAck::TPtr& ev, TOperationContext& context) override {
        ...
        YDB_LOG_INFO_CTX(context.Ctx, "HandleReply TEvInitSplitMergeDestinationAck",
            {"debugHint", DebugHint()},
            {"operationId", OperationId},
            {"atSchemeshard", ssId},
    ...
    bool ProgressState(TOperationContext& context) override {
        ...
        for (const auto& shard: txState->Shards) {
            ...
            YDB_LOG_DEBUG_CTX(context.Ctx, "Initializing scheme on dst",
                {"datashard", datashardId},
                {"splitOp", OperationId},
                {"alterVersion", alterVersion},
                {"atTablet", context.SS->TabletID()});
...
public:
    ...
    bool HandleReply(TEvDataShard::TEvSplitAck::TPtr& ev, TOperationContext& context) override {
        ...
        YDB_LOG_INFO_CTX(context.Ctx, "HandleReply TEvSplitAck",
            {"debugHint", DebugHint()},
            {"atSchemeshard", ssId},
    ...
    bool ProgressState(TOperationContext& context) override {
        ...
        for (const auto& shard : txState->Shards) {
            ...
            YDB_LOG_DEBUG_CTX(context.Ctx, "Starting split on src datashard at tablet",
                {"debugHint", DebugHint()},
...
public:
    ...
    THolder<TProposeResponse> Propose(const TString&, TOperationContext& context) override {
        ...
        YDB_LOG_NOTICE_CTX(context.Ctx, "TSplitMerge Propose",
        ...
        auto setResultError = [&](NKikimrScheme::EStatus status, const TString& error) {
            ...
            YDB_LOG_WARN_CTX(context.Ctx, "TSplitMerge Propose failed",
                {"status", status},
                {"error", error},
                {"tableStr", info.GetTablePath()},
                {"tableId", pathId},
