...
struct TSchemeShard::TTxPublishToSchemeBoard: public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext&, const TActorContext& ctx) override {
        ...
        while (!Paths.empty() && size++ < Self->PublishChunkSize) {
            ...
            YDB_LOG_DEBUG_CTX(ctx, "TTxPublishToSchemeBoard DescribePath",
                {"schemeshard", Self->TabletID()},
                {"txId", txId},
                {"pathId", paths.front()},
            );
    ...
    void DoComplete(const TActorContext& ctx) override {
        ...
        for (auto& kv : Descriptions) {
            ...
            for (auto& desc : descriptions) {
                YDB_LOG_DEBUG_CTX(ctx, "TTxPublishToSchemeBoard Send",
                    {"populator", Self->SchemeBoardPopulator},
                    {"schemeshard", Self->TabletID()},
                    {"txId", txId},
                    {"pathId", desc->Record.GetPathId()},
                );
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        if (Self->Operations.contains(txId)) {
            ...
            for (auto opId: toActivateWaitPublication) {
                YDB_LOG_NOTICE_CTX(ctx, "TTxAckPublishToSchemeBoard: operation is ready to ack that some awaited paths are published",
                    {"opId", opId},
                    {"leftAwaitPublications", operation->CountWaitPublication(opId)},
                    {"schemeshard", Self->TabletID()},
                    {"txId", txId},
                );
