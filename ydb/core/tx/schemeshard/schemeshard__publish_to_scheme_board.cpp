...
struct TSchemeShard::TTxPublishToSchemeBoard: public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext&, const TActorContext& ctx) override {
        YDBLOG_CTX_INFO(ctx, "TTxPublishToSchemeBoard DoExecute, at schemeshard: ",
        ...
        while (!Paths.empty() && size++ < Self->PublishChunkSize) {
            ...
            YDBLOG_CTX_DEBUG(ctx, "TTxPublishToSchemeBoard DescribePath, at schemeshard: , txId: , path id: ",
                {"schemeshard", Self->TabletID()},
                {"txId", txId},
                {"id", paths.front()});
    ...
    void DoComplete(const TActorContext& ctx) override {
        YDBLOG_CTX_INFO(ctx, "TTxPublishToSchemeBoard DoComplete, at schemeshard: ",
            {"schemeshard", Self->TabletID()});
        ...
        for (auto& kv : Descriptions) {
            ...
            for (auto& desc : descriptions) {
                YDBLOG_CTX_DEBUG(ctx, "TTxPublishToSchemeBoard Send, to populator: , at schemeshard: , txId: , path id: ",
...
struct TSchemeShard::TTxAckPublishToSchemeBoard: public TTransactionBase<TSchemeShard> {
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        YDBLOG_CTX_DEBUG(ctx, "TTxAckPublishToSchemeBoard Execute, at schemeshard: , msg: , cookie: ",
            {"schemeshard", Self->TabletID()},
        ...
        if (Self->Operations.contains(txId)) {
            YDBLOG_CTX_INFO(ctx, "Operation in-flight, at schemeshard: , txId: ",
                {"schemeshard", Self->TabletID()},
                {"txId", txId});
            ...
            if (AckPublish(db, txId, pathId, version, operation->Publications, ctx)
                ...
                YDBLOG_CTX_NOTICE(ctx, "TTxAckPublishToSchemeBoard, operation is ready to notify, at schemeshard: , txId: ",
                    {"schemeshard", Self->TabletID()},
                    {"txId", txId});
            ...
            for (auto opId: toActivateWaitPublication) {
                YDBLOG_CTX_NOTICE(ctx, "TTxAckPublishToSchemeBoard, operation is ready to ack that some awaited paths are published, opId: , left await publications: , at schemeshard: , txId: ",
                    {"opId", opId},
            ...
            YDBLOG_CTX_INFO(ctx, "Publication in-flight, count: , at schemeshard: , txId: ",
            ...
            if (AckPublish(db, txId, pathId, version, publication.Paths, ctx)) {
                YDBLOG_CTX_NOTICE(ctx, "Publication complete, notify & remove, at schemeshard: , txId: , subscribers: ",
                    {"schemeshard", Self->TabletID()},
                    {"txId", txId},
            ...
            YDBLOG_CTX_WARN(ctx, "Unknown operation & publication, at schemeshard: , txId: ",
                {"schemeshard", Self->TabletID()},
                {"txId", txId});
        }
    ...
    void Complete(const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxAckPublishToSchemeBoard Complete, at schemeshard: , cookie: ",
            {"schemeshard", Self->TabletID()},
            {"cookie", Ev->Cookie});
...
private:
    bool AckPublish(
        ...
        while (it != paths.end() && it->first == pathId && it->second <= version) {
            YDBLOG_CTX_INFO(ctx, "AckPublish, at schemeshard: , txId: , pathId: , version: ",
                {"schemeshard", Self->TabletID()},
                {"txId", txId},
                {"pathId", pathId},
                {"version", it->second});
    ...
    void Notify(TTxId txId, const THashSet<TActorId>& subscribers, const TActorContext& ctx) {
        for (const auto& subscriber : subscribers) {
            YDBLOG_CTX_DEBUG(ctx, "TTxAckPublishToSchemeBoard Notify send TEvNotifyTxCompletionResult, at schemeshard: , to actorId: ",
                {"schemeshard", Self->TabletID()},
                {"actorId", subscriber});
