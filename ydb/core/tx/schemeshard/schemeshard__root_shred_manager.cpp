    ...
    , IsManualStartup((ShredInterval.Seconds() == 0 ? true : false))
{
    ...
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Created: InflightLimit# , ShredInterval# , ShredBSCInterval# , CurrentWakeupInterval# , IsManualStartup# ",
...
void TRootShredManager::UpdateConfig(const NKikimrConfig::TDataErasureConfig& config) {
    ...
    const auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Config updated: InflightLimit# , ShredInterval# , ShredBSCInterval# , CurrentWakeupInterval# , IsManualStartup# ",
...
void TRootShredManager::Start() {
    const auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Start: Generation# , Status# ",
...
void TRootShredManager::Stop() {
    const auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Stop");
    Queue->Stop();
}
...
void TRootShredManager::StartShred(NIceDb::TNiceDb& db) {
    ...
    for (auto& [pathId, subdomain] : SchemeShard->SubDomains) {
        ...
        if (Queue->Enqueue(pathId)) {
            ...
            YDBLOG_CTX_TRACE(ctx, "[RootShredManager] [Enqueue] Enqueued pathId#  at schemeshard ",
                {"pathId", pathId},
                {"#_SchemeShard->TabletID()", SchemeShard->TabletID()});
        } else {
            YDBLOG_CTX_TRACE(ctx, "[RootShredManager] [Enqueue] Skipped or already exists pathId#  at schemeshard ",
                {"pathId", pathId},
                {"#_SchemeShard->TabletID()", SchemeShard->TabletID()});
        }
    }
    ...
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Run: Generation# , WaitingShredTenants.size# , Status# ",
        {"#_Generation", Generation},
...
void TRootShredManager::Continue() {
    ...
    const auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Continue: Generation# , Status# ",
        {"#_Generation", Generation},
        {"Status", Status});
}
...
void TRootShredManager::ScheduleShredWakeup() {
    ...
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] ScheduleShredWakeup: Next shred iteration will run at ",
        {"#_num_0", AppData(ctx)->TimeProvider->Now() + CurrentWakeupInterval});
}
...
void TRootShredManager::WakeupToRunShred(TEvSchemeShard::TEvWakeupToRunShred::TPtr& ev, const NActors::TActorContext& ctx) {
    ...
    YDBLOG_CTX_DEBUG(ctx, "[RootShredManager] WakeupToRunShred: Timestamp# ",
        {"Timestamp", AppData(ctx)->TimeProvider->Now()});
...
NOperationQueue::EStartStatus TRootShredManager::StartShredOperation(const TPathId& pathId) {
    ...
    if (it == SchemeShard->SubDomains.end()) {
        YDBLOG_CTX_WARN(ctx, "[RootShredManager] [Start] Failed to resolve subdomain info for pathId#  at schemeshard# ",
            {"pathId", pathId},
            {"schemeshard", SchemeShard->TabletID()});
        return NOperationQueue::EStartStatus::EOperationRemove;
    }
    const auto& tenantSchemeShardId = it->second->GetTenantSchemeShardID();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] [Start] Shred for pathId# , tenant schemeshard#  at schemeshard ",
        {"pathId", pathId},
...
void TRootShredManager::HandleDisconnect(TTabletId tabletId, const TActorId& clientId, const TActorContext& ctx) {
    if (tabletId == BSC) {
        YDBLOG_CTX_DEBUG(ctx, "[RootShredManager] HandleDisconnect resend request to BSC at schemeshard ",
            {"#_SchemeShard->TabletID()", SchemeShard->TabletID()});
        SendRequestToBSC();
        return;
    }
    ...
    YDBLOG_CTX_INFO(ctx, "[RootShredManager] [Disconnect] Shred disconnect to tablet: , at schemeshard: ",
        {"tablet", tabletId},
        {"schemeshard", SchemeShard->TabletID()});
...
void TRootShredManager::FinishShred(NIceDb::TNiceDb& db, const TPathId& pathId) {
    ...
    if (!SchemeShard->SubDomains.contains(pathId)) {
        YDBLOG_CTX_WARN(ctx, "[RootShredManager] [Finished] Failed to resolve subdomain info for pathId#  in#  ms at schemeshard ",
            {"pathId", pathId},
            {"in", duration.MilliSeconds()},
            {"#_SchemeShard->TabletID()", SchemeShard->TabletID()});
    } else {
        YDBLOG_CTX_INFO(ctx, "[RootShredManager] [Finished] Shred completed for pathId#  in#  ms at schemeshard ",
            {"pathId", pathId},
            {"in", duration.MilliSeconds()},
            {"#_SchemeShard->TabletID()", SchemeShard->TabletID()});
    }
    ...
    if (WaitingShredTenants.empty()) {
        YDBLOG_CTX_INFO(ctx, "[RootShredManager] Shred in tenants is completed. Send request to BS controller");
        Queue->Clear();
        ActivePipes.clear();
        Status = EShredStatus::IN_PROGRESS_BSC;
...
void TRootShredManager::ScheduleRequestToBSC() {
    ...
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] ScheduleRequestToBSC: Interval# ",
        {"Interval", ShredBSCInterval});
}
...
void TRootShredManager::SendRequestToBSC() {
    auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] SendRequestToBSC: Generation# , BscGeneration# ",
        {"#_Generation", Generation},
...
void TRootShredManager::Complete() {
    ...
    YDBLOG_CTX_NOTICE(ctx, "[RootShredManager] Complete: Generation# , duration#  s",
        {"Generation", Generation},
        {"duration", shredDuration.Seconds()});
}
...
bool TRootShredManager::Restore(NIceDb::TNiceDb& db) {
    ...
    auto ctx = SchemeShard->ActorContext();
    YDBLOG_CTX_INFO(ctx, "[RootShredManager] Restore: Generation# , Status# , WakeupInterval#  s, WaitingShredTenants# ",
        {"Generation", Generation},
        {"Status", Status},
        {"WakeupInterval", CurrentWakeupInterval.Seconds()},
...
struct TSchemeShard::TTxShredManagerInit : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxShredManagerInit Execute at schemeshard: ",
    ...
    void DoComplete(const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxShredManagerInit Complete at schemeshard: ",
            {"schemeshard", Self->TabletID()});
    }
};
...
struct TSchemeShard::TTxRunShred : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxRunShred Execute at schemeshard: ",
            {"schemeshard", Self->TabletID()});
    ...
    void DoComplete(const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxRunShred Complete at schemeshard: , NeedSendRequestToBSC# ",
            {"schemeshard", Self->TabletID()},
...
struct TSchemeShard::TTxCompleteShredTenant : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredTenant Execute at schemeshard: ",
            {"schemeshard", Self->TabletID()});
        ...
        if (completedGeneration != shredManager->GetGeneration()) {
            YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredTenant Unknown generation#, Expected gen#  at schemeshard: ",
        ...
        if (shredManager->GetStatus() == EShredStatus::UNSPECIFIED ||
            ...
            YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredTenant Generation# marked as ",
                {"Generation", completedGeneration},
                {"#_shredManager->GetStatus()", shredManager->GetStatus()});
            return;
        }
    ...
    void DoComplete(const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredTenant Complete at schemeshard: , NeedSendRequestToBSC# ",
            {"schemeshard", Self->TabletID()},
            {"NeedSendRequestToBSC", (NeedSendRequestToBSC ? "true" : "false")});
        if (NeedSendRequestToBSC) {
            Self->RootShredManager->SendRequestToBSC();
...
struct TSchemeShard::TTxCompleteShredBSC : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredBSC Execute at schemeshard: ",
            {"schemeshard", Self->TabletID()});
        ...
        if (ui64 currentBscGeneration = record.GetCurrentGeneration(); currentBscGeneration > shredManager->GetBscGeneration()) {
            YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredBSC Unknown generation#, Expected BscGen#  at schemeshard: ",
                {"generation", currentBscGeneration},
                {"BscGen", shredManager->GetBscGeneration()},
                {"schemeshard", Self->TabletID()});
        ...
        if (record.GetCompleted()) {
            YDBLOG_CTX_NOTICE(ctx, "TTxCompleteShredBSC: Data shred in BSC is completed");
            ...
            YDBLOG_CTX_NOTICE(ctx, "TTxCompleteShredBSC: Progress data shred in BSC %",
                {"#_num_0", static_cast<double>(record.GetProgress10k()) / 100});
            NeedScheduleRequestToBSC = true;
        }
    }

    void DoComplete(const TActorContext& ctx) override {
        YDBLOG_CTX_DEBUG(ctx, "TTxCompleteShredBSC Complete at schemeshard: , NeedScheduleRequestToBSC# ",
            {"schemeshard", Self->TabletID()},
