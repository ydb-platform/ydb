...
struct TSchemeShard::TTxServerlessStorageBilling : public TTransactionBase<TSchemeShard> {
    ...
    bool Execute(TTransactionContext &txc, const TActorContext &ctx) override {
        ...
        if (!Self->ServerlessStorageLastBillTime) {
            ...
            YDB_LOG_NOTICE_CTX(ctx, "TTxServerlessStorageBilling: initiate at first time set next retry",
                {"schemeshardId", Self->SelfTabletId()},
                {"domainId", Self->ParentDomainId},
                {"now", now},
        ...
        if (now < last) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: unable do anything from the past next retry",
                {"schemeshardId", Self->SelfTabletId()},
                {"domainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
        ...
        if (now <= lastBilled.End || toBill.Start <= lastBilled.End) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: too soon call, wait until current period ends -- -- next retry",
                {"schemeshardId", Self->SelfTabletId()},
                {"domainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
        ...
        if (now > TimeGrid.GetNext(TimeGrid.GetNext(lastBilled)).End) {
            ...
            YDB_LOG_INFO_CTX(ctx, "TTxServerlessStorageBilling: too late call, there are could be gaps in the metric -- -- next retry",
                {"schemeshardId", Self->SelfTabletId()},
                {"domainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
        ...
        YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: make a bill, record: -- -- next retry",
