...
struct TSchemeShard::TTxServerlessStorageBilling : public TTransactionBase<TSchemeShard> {
    ...
    bool Execute(TTransactionContext &txc, const TActorContext &ctx) override {
        YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling.Execute"
        );
        ...
        if (!Self->AllowServerlessStorageBilling) {
            YDB_LOG_INFO_CTX(ctx, "TTxServerlessStorageBilling: unable to make a bill, AllowServerlessStorageBilling is false",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
        ...
        if (!cloud_id || !folder_id || !database_id) {
            YDB_LOG_INFO_CTX(ctx, "TTxServerlessStorageBilling: unable to make a bill, neither cloud_id nor folder_id nor database_id have found in user attributes at the domain",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"nextRetryAt", TimeToNextBill},
            );
        ...
        if (!spaceUsage.Tables.TotalSize) {
            YDB_LOG_INFO_CTX(ctx, "TTxServerlessStorageBilling: nothing to bill",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"nextRetryAt", TimeToNextBill},
            );
        ...
        if (!Self->ServerlessStorageLastBillTime) {
            ...
            YDB_LOG_NOTICE_CTX(ctx, "TTxServerlessStorageBilling: initiate at first time",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"now", now},
        ...
        if (now < last) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: unable do anything from the past",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
        ...
        if (now <= lastBilled.End || toBill.Start <= lastBilled.End) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: too soon call, wait until current period ends",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
                {"lastBilledStart", lastBilled.Start},
                {"lastBilledEnd", lastBilled.End},
                {"toBillStart", toBill.Start},
        ...
        if (now > TimeGrid.GetNext(TimeGrid.GetNext(lastBilled)).End) {
            ...
            YDB_LOG_INFO_CTX(ctx, "TTxServerlessStorageBilling: too late call, there could be gaps in the metric",
                {"schemeshard", Self->TabletID()},
                {"rootSubdomainId", Self->ParentDomainId},
                {"now", now},
                {"lastBillTime", last},
        ...
        YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling: make a bill",
            {"record", billRecord},
            {"schemeshard", Self->TabletID()},
            {"rootSubdomainId", Self->ParentDomainId},
            {"now", now},
    ...
    void Complete(const TActorContext &ctx) override {
        YDB_LOG_DEBUG_CTX(ctx, "TTxServerlessStorageBilling.Complete"
        );
