...
NOperationQueue::EStartStatus TTenantShredManager::StartShredOperation(const TShardIdx& shardIdx) {
    ...
    if (it == SchemeShard->ShardInfos.end()) {
        YDB_LOG_WARN_CTX(ctx, "[TenantShredManager] [Start] Failed to resolve shard info for shred",
            {"shardIdx", shardIdx},
    ...
    YDB_LOG_INFO_CTX(ctx, "[TenantShredManager] [Start] Shred",
        {"pathId", pathId},
        {"tabletId", tabletId},
...
void TTenantShredManager::Start() {
    ...
    if (Status == EShredStatus::COMPLETED) {
        ...
        YDB_LOG_DEBUG_CTX(ctx, "[Start] Send to root schemeshard",
            {"generation", SchemeShard->TenantShredManager->GetGeneration()},
        );
...
void TTenantShredManager::Stop() {
    const auto ctx = SchemeShard->ActorContext();
    YDB_LOG_NOTICE_CTX(ctx, "[TenantShredManager] Stop"
    );
    Queue->Stop();
}
    ...
    for (const auto& [shardIdx, shardInfo] : SchemeShard->ShardInfos) {
        ...
        case NKikimr::NSchemeShard::ETabletType::PersQueue: {
            if (Queue->Enqueue(shardIdx)) {
                ...
                YDB_LOG_TRACE_CTX(ctx, "[TenantShredManager] [Enqueue] Enqueued shard",
                    {"shardIdx", shardIdx},
                    {"schemeshard", SchemeShard->TabletID()},
                );
            } else {
                YDB_LOG_TRACE_CTX(ctx, "[TenantShredManager] [Enqueue] Skipped or already exists shard",
                    {"shardIdx", shardIdx},
                    {"schemeshard", SchemeShard->TabletID()},
                );
            }
...
void TTenantShredManager::FinishShred(NIceDb::TNiceDb& db, const TTabletId& tabletId) {
    ...
    if (shardIdx == InvalidShardIdx) {
        YDB_LOG_WARN_CTX(ctx, "[TenantShredManager] [Finished] Failed to resolve shard info",
            {"tabletId", tabletId},
            {"durationMs", duration.MilliSeconds()},
            {"schemeshard", SchemeShard->TabletID()},
        );
    } else {
        YDB_LOG_INFO_CTX(ctx, "[TenantShredManager] [Finished] Shred is completed",
            {"tabletId", tabletId},
            {"shardIdx", shardIdx},
            {"durationMs", duration.MilliSeconds()},
            {"schemeshard", SchemeShard->TabletID()},
...
bool TTenantShredManager::Restore(NIceDb::TNiceDb& db) {
    ...
    auto ctx = SchemeShard->ActorContext();
    YDB_LOG_INFO_CTX(ctx, "[TenantShredManager] Restore",
        {"generation", Generation},
        {"status", Status},
        {"waitingShredShards", WaitingShredShards.size()},
...
bool TTenantShredManager::StopWaitingShred(const TShardIdx& shardIdx) {
    ...
    if (it != WaitingShredShards.end()) {
        ...
        if (WaitingShredShards.empty()) {
            ...
            CompletedGeneration = Generation;
            auto ctx = SchemeShard->ActorContext();
            YDB_LOG_INFO_CTX(ctx, "[TenantShredManager] [StopWaitingShred] Shred in shards is completed. Send response to root schemeshard",
                {"completedGeneration", CompletedGeneration},
            );
        }
        return true;
...
void TTenantShredManager::CleanupOldGenerationsOnRestore(NIceDb::TNiceDb& db, const TVector<ui64>& generationsToCleanup) {
    ...
    for (ui64 generation : generationsToCleanup) {
        if (generation >= Generation) {
            ...
            YDB_LOG_WARN_CTX(ctx, "[TenantShredManager] Restore: Invalid element in collection >= current shredding generation. Element will be skipped. This should never occur.",
                {"generation", generation},
...
struct TSchemeShard::TTxRunTenantShred : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        if (record.GetGeneration() <= shredManager->GetCompletedGeneration()) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxRunTenantShred: Already complete",
                {"requestedGeneration", record.GetGeneration()},
                {"schemeshard", Self->TabletID()},
                {"requester", Ev->Sender},
            );
...
struct TSchemeShard::TTxCompleteShredShard : public TSchemeShard::TRwTxBase {
    ...
    void DoExecute(TTransactionContext& txc, const TActorContext& ctx) override {
        ...
        if (completedGeneration != shredManager->GetGeneration()) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxCompleteShredShard: Unknown generation",
                {"completedGeneration", completedGeneration},
                {"expectedGeneration", shredManager->GetGeneration()},
                {"schemeshard", Self->TabletID()},
            );
            return;
        }
    ...
    void DoComplete(const TActorContext& ctx) override {
        if (Response) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxCompleteShredShard Complete. Send to root schemeshard",
                {"generation", Self->TenantShredManager->GetGeneration()},
            );
...
private:
    ...
    void HandleBadStatus(TEvDataShard::TEvVacuumResult::TPtr& ev, const TActorContext& ctx) const {
        const auto& record = ev->Get()->Record;
        YDB_LOG_DEBUG_CTX(ctx, "TTxCompleteShredShard: shred failed at DataShard",
    ...
    void HandleBadStatus(TEvKeyValue::TEvVacuumResponse::TPtr& ev, const TActorContext& ctx) const {
        const auto& record = ev->Get()->Record;
        YDB_LOG_DEBUG_CTX(ctx, "TTxCompleteShredShard: shred failed at KeyValue",
            {"dataShard", record.tablet_id()},
...
struct TSchemeShard::TTxAddNewShardToShred : public TSchemeShard::TRwTxBase {
    ...
    void DoComplete(const TActorContext& ctx) override {
        if (Response) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxAddEntryToShred Complete. Send to root schemeshard",
                {"schemeshard", Self->TabletID()},
                {"generation", Self->TenantShredManager->GetGeneration()},
            );
            Self->PipeClientCache->Send(ctx, ui64(Self->ParentDomainId.OwnerId), Response.release());
...
struct TSchemeShard::TTxCancelShredShards : public TSchemeShard::TRwTxBase {
    ...
    void DoComplete(const TActorContext& ctx) override {
        ...
        if (Response) {
            YDB_LOG_DEBUG_CTX(ctx, "TTxCancelShredShards Complete. Send to root schemeshard",
                {"schemeshard", Self->TabletID()},
                {"generation", Self->TenantShredManager->GetGeneration()},
            );
            Self->PipeClientCache->Send(ctx, ui64(Self->ParentDomainId.OwnerId), Response.release());
