...
void TSchemeShard::PersistCreateBuildIndex(NIceDb::TNiceDb& db, const TIndexBuildInfo& info) {
    ...
    if (info.ImplTableDescriptions || !std::holds_alternative<std::monostate>(info.SpecializedIndexDescription)) {
        ...
        std::visit([&serializableRepresentation](auto&& specializedDescription) {
            using T = std::decay_t<decltype(specializedDescription)>;
