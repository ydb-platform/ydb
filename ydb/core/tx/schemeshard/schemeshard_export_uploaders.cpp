...
public:
    ...
    static const TDuration& GetMaxDelay() {
        return MaxDelay;
    }
...
class TKesusResourcesUploader : public TSchemeWithPipeUploader<TKesusResourcesUploader<TSettings>, TSettings> {
    ...
    void UploadBatch() {
        ...
        EncryptedIdx = CurrentBatchEncryptedIdx;
    ...
    void RetryBatch() {
        ...
        // Удаляем метаданные, добавленные в неудавшемся батче
        i32 batchSize = std::min(BatchSize, (i32)Resources.size() - CurrentBatchOffset);
        if ((i32)ResourcesMetadata.size() >= batchSize) {
            ResourcesMetadata.resize(ResourcesMetadata.size() - batchSize);
        }

        // Планируем повтор через задержку
        BatchDelay = Min(BatchDelay * BatchAttempt, MaxBatchDelay);
        const TDuration random = TDuration::FromValue(
            TAppData::RandomProvider->GenRand64() % BatchDelay.MicroSeconds()
        );
        this->Schedule(BatchDelay + random, new TEvents::TEvWakeup());
    }

    void OnFilesUploaded(bool success, const TString& error) override {
        if (!success) {
            if (++BatchAttempt <= MaxBatchAttempts) {
                LOG_W("Batch upload failed, will retry"
                    << ", self: " << this->SelfId()
                    << ", attempt: " << BatchAttempt
                    << ", error: " << error);
                return RetryBatch();
        ...
        BatchAttempt = 0;
        CurrentBatchEncryptedIdx = EncryptedIdx;
    ...
    void Finish(bool success = true, const TString& error = TString()) override {
        ...
        this->Send(this->ReplyTo, new TEvPrivate::TEvExportUploadKesusResourcesResult(
            ExportId, ItemIdx, success, error, ResourcesMetadata));
        this->PassAway();
    }
...
private:
    ...
    i32 CurrentBatchOffset = 0;
    ui32 CurrentBatchEncryptedIdx = 0;
    ui32 BatchAttempt = 0;
    TDuration BatchDelay = TDuration::Seconds(10);
    static constexpr TDuration MaxBatchDelay = TDuration::Minutes(10);
    static constexpr ui32 MaxBatchAttempts = 10;
...
class TSchemeUploader: public TExportFilesUploader<TSchemeUploader<TSettings>, TSettings> {
    ...
    void RetryResourcesUploadOrFail(const TString& error) {
        ...
        KesusResourcesUploadDelay = Min(KesusResourcesUploadDelay * KesusResourcesUploadAttempts, TBase::GetMaxDelay());
