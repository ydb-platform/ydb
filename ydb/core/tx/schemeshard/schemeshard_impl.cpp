...
void TSchemeShard::OnActivateExecutor(const TActorContext &ctx) {
    ...
    EnableShred = appData->FeatureFlags.GetEnableShred() || appData->FeatureFlags.GetEnableDataErasure();
...
void TSchemeShard::SetPartitioning(TPathId pathId, TTableInfo::TPtr tableInfo, TVector<TTableShardInfo>&& newPartitioning) {
    ...
    if (!tableInfo->IsBackup) {
        ...
        std::vector<TShardIdx> newShredShards;
        for (const auto& p: newPartitioning) {
            ...
            if (it != partitionStats.end()) {
                ...
                newShredShards.push_back(p.ShardIdx);
            }
        }
...
void TSchemeShard::ApplyConsoleConfigs(const NKikimrConfig::TAppConfig& appConfig, const TActorContext& ctx) {
    ...
    if (appConfig.HasShredConfig()) {
        const auto& shredConfig = appConfig.GetShredConfig();
        ConfigureShredManager(shredConfig);
    } else if (appConfig.HasDataErasureConfig()) {
        const auto& shredConfig = ConvertToShredConfig(appConfig.GetDataErasureConfig());
        ConfigureShredManager(shredConfig);
    }
...
void TSchemeShard::ApplyConsoleConfigs(const NKikimrConfig::TFeatureFlags& featureFlags, const TActorContext& ctx) {
    ...
    EnableShred = featureFlags.GetEnableShred() || featureFlags.GetEnableDataErasure();
...
THolder<TShredManager> TSchemeShard::CreateShredManager(const NKikimrConfig::TShredConfig& config) {
    if (IsDomainSchemeShard) {
...
void TSchemeShard::ConfigureShredManager(const NKikimrConfig::TShredConfig& config) {
    if (ShredManager) {
