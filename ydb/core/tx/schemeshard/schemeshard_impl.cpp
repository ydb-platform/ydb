...
void TSchemeShard::InitializeTabletMigrations() {
    ...
    for (auto& [pathId, subdomain] : SubDomains) {
        ...
        YDBLOG_COMP_INFO(NKikimrServices::FLAT_TX_SCHEMESHARD, "TabletMigrator - creating tablets, working dir: , db name: , create SVP: , create SA: , create BCT: , at schemeshard: ",
            {"dir", workingDir},
            {"name", dbName},
            {"SVP", createSVP},
            {"SA", createSA},
...
EAttachChildResult TSchemeShard::AttachChild(TPathElement::TPtr child) {
    ...
    if (!parent->GetChildren().contains(child->Name)) {
        ...
        YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "AttachChild: child attached as only one child to the parent, parent id: , parent name: , child name: , child id: , at schemeshard: ",
            {"id", parent->PathId},
            {"name", parent->Name},
            {"name", child->Name},
            {"id", child->PathId},
    ...
    YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "AttachChild: decision is: , parent id: , parent name: , child name: , prev child id: , child id: , prev order: , child order: , at schemeshard: ",
        {"is", decision},
        {"id", parent->PathId},
        {"name", parent->Name},
        {"name", child->Name},
        {"id", prevChild->PathId},
        {"id", child->PathId},
        {"order", prevChildOrder.ToString()},
        {"order", childOrder.ToString()},
        {"schemeshard", TabletID()});
...
void TSchemeShard::IncrementPathDbRefCount(const TPathId& pathId, const TStringBuf& debug) {
    ...
    if (it != PathsById.end()) {
        YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "IncrementPathDbRefCount reason  for pathId  was ",
            {"#_debug", debug},
            {"#_pathId", pathId},
            {"#_it->second->DbRefCount", it->second->DbRefCount});
...
void TSchemeShard::DecrementPathDbRefCount(const TPathId& pathId, const TStringBuf& debug) {
    ...
    if (it != PathsById.end()) {
        ...
        YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "DecrementPathDbRefCount reason  for pathId  was ",
            {"#_debug", debug},
            {"#_pathId", pathId},
            {"#_it->second->DbRefCount", it->second->DbRefCount});
...
void TSchemeShard::ChangeTxState(NIceDb::TNiceDb& db, const TOperationId opId, TTxState::ETxState newState) {
    ...
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Change state for txid   -> ",
        {"#_opId", opId},
        {"#_num_0", NKikimr::NSchemeShard::TxStateName(TxInFlight[opId].State)},
        {"#_num_1", NKikimr::NSchemeShard::TxStateName(newState)});
...
TTxState &TSchemeShard::CreateTx(TOperationId opId, TTxState::ETxType txType, TPathId targetPath, TPathId sourcePath) {
    ...
    YDBLOG_CTX_COMP_DEBUG(TActivationContext::AsActorContext(), NKikimrServices::FLAT_TX_SCHEMESHARD, "CreateTx for txid  type:  target path:  source path: ",
        {"#_opId", opId},
...
void TSchemeShard::RemoveTx(const TActorContext &ctx, NIceDb::TNiceDb &db, TOperationId opId, TTxState *txState) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "RemoveTx for txid ",
        {"#_opId", opId});
...
void TSchemeShard::ExamineTreeVFS(TPathId nodeId, std::function<void (TPathElement::TPtr)> func, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ExamineTreeVFS visit path id  name:  type:  state:  stepDropped:  droppedTxId:  parent: ",
        {"#_nodeId", nodeId},
        {"name", node->Name},
    ...
    if (node->Dropped() && !Operations.contains(node->DropTxId)) {
        YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ExamineTreeVFS skip path id ",
            {"#_nodeId", nodeId});
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ExamineTreeVFS run path id: ",
        {"id", nodeId});
...
void TSchemeShard::MarkAsMigrated(TPathElement::TPtr node, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Mark as Migrated path id ",
        {"#_node->PathId", node->PathId});
...
void TSchemeShard::MarkAsDropping(TPathElement::TPtr node, TTxId txId, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Mark as Dropping path id  by tx: ",
        {"#_node->PathId", node->PathId},
        {"tx", txId});
    if (!node->Dropped()) {
...
void TSchemeShard::Handle(TEvDataShard::TEvCompactTableResult::TPtr &ev, const TActorContext &ctx) {
    ...
    case ECompactionType::Forced:
        ...
        break;
    default:
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvCompactTableResult with unknown cookie# , tabletId# , at schemeshard# ",
            {"cookie", ev->Cookie},
...
void TSchemeShard::Handle(TEvDataShard::TEvSchemaChanged::TPtr& ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSchemaChanged, tabletId: , at schemeshard: , message: ",
        {"tabletId", TabletID()},
        {"schemeshard", TabletID()},
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvSchemaChanged for unknown txId  message# ",
            {"#_txId", txId},
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvSchemaChanged for unknown part in txId:  message# ",
            {"txId", txId},
            {"message", ev->Get()->Record.DebugString()});
...
void TSchemeShard::Handle(TEvDataShard::TEvStateChanged::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvStateChanged, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(TEvDataShard::TEvInitSplitMergeDestinationAck::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got InitSplitMergeDestinationAck for unknown txId  datashard ",
            {"#_txId", txId},
            {"#_tabletId", tabletId});
        return;
    }
...
void TSchemeShard::Handle(TEvDataShard::TEvSplitAck::TPtr& ev, const TActorContext& ctx) {
    ...
    const auto tabletId = TTabletId(ev->Get()->Record.GetTabletId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got SplitAck for unknown txId  datashard ",
            {"#_txId", txId},
            {"#_tabletId", tabletId});
        return;
    }
...
void TSchemeShard::Handle(TEvDataShard::TEvSplitPartitioningChangedAck::TPtr& ev, const TActorContext& ctx) {
    ...
    const auto tabletId = TTabletId(ev->Get()->Record.GetTabletId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSplitPartitioningChangedAck for unknown txId  datashard ",
            {"#_txId", txId},
            {"#_tabletId", tabletId});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvModifySchemeTransaction::TPtr &ev, const TActorContext &ctx) {
    if (IsReadOnlyMode) {
        ...
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Schema modification rejected because of ReadOnly mode, at tablet:  txid: ",
            {"tablet", selfId},
            {"txid", txId});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvProcessingRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "TSchemeShard::Handle, at schemeshard: , processor: ",
        {"schemeshard", TabletID()},
...
void TSchemeShard::Handle(TEvPrivate::TEvProgressOperation::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvPrivate::TEvProgressOperation for unknown txId ",
            {"#_txId", txId});
        return;
    }
...
void TSchemeShard::Handle(TEvDataShard::TEvProposeTransactionAttachResult::TPtr& ev, const TActorContext& ctx)
{
    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvProposeTransactionAttachResult for unknown txId:  message: ",
            {"txId", txId},
            {"message", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvProposeTransactionAttachResult but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvTabletPipe::TEvClientConnected::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_TRACE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvClientConnected, tabletId: , status: , at schemeshard: ",
        {"tabletId", tabletId},
    ...
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Failed to connect, to tablet: , at schemeshard: ",
        {"tablet", tabletId},
        {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvTabletPipe::TEvServerConnected::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_TRACE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Pipe server connected, at tablet: ",
...
void TSchemeShard::Handle(TEvTabletPipe::TEvClientDestroyed::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_TRACE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Client pipe, to tablet: , from: is reset",
        {"tablet", tabletId},
        {"from", TabletID()});
...
void TSchemeShard::Handle(TEvTabletPipe::TEvServerDisconnected::TPtr &, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_TRACE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Server pipe is reset, at schemeshard: ",
        {"schemeshard", TabletID()});
}

void TSchemeShard::Handle(TEvSchemeShard::TEvSyncTenantSchemeShard::TPtr& ev, const TActorContext& ctx) {
    const auto& record = ev->Get()->Record;
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, at schemeshard: , msg: ",
        {"schemeshard", TabletID()},
    ...
    if (!SubDomains.contains(pathId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, at schemeshard: , ignore spurious message from dropped subdomain's schemeshard (partial cleanup)",
            {"schemeshard", TabletID()},
            {"#_pathId", pathId});
        return;
    }
    ...
    if (!PathsById.contains(pathId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, at schemeshard: , ignore spurious message from dropped subdomain's schemeshard (full cleanup)",
            {"schemeshard", TabletID()},
            {"#_pathId", pathId});
        return;
    }
    ...
    if (PathsById.at(pathId)->Dropped()) {
        ...
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, at schemeshard: , ignore spurious message from dropped subdomain's schemeshard (pre cleanup)",
            {"schemeshard", TabletID()},
            {"#_pathId", pathId});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvUpdateTenantSchemeShard::TPtr& ev, const TActorContext& ctx) {
    const auto& record = ev->Get()->Record;
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvUpdateTenantSchemeShard, at schemeshard: , msg: ",
        {"schemeshard", TabletID()},
        {"msg", record.ShortDebugString()});
    Y_VERIFY_S(!IsDomainSchemeShard, "unexpected message: schemeshard: " << TabletID() << " mgs: " << record.DebugString());
...
void TSchemeShard::Handle(NSchemeBoard::NSchemeshardEvents::TEvUpdateAck::TPtr& ev, const TActorContext& ctx) {
    const auto& record = ev->Get()->Record;
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvUpdateAck, at schemeshard: , msg: , cookie: ",
        {"schemeshard", TabletID()},
        {"msg", record.ShortDebugString()},
        {"cookie", ev->Cookie});
    ...
    if (!Operations.contains(txId) && !Publications.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvUpdateAck for unknown txId , at schemeshard: ",
            {"#_txId", txId},
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvHive::TEvCreateTabletReply::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvCreateTabletReply at schemeshard:  message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
    ...
    if (!ShardInfos.contains(shardIdx)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvCreateTabletReply for unknown shard idx  tabletId ",
            {"#_shardIdx", shardIdx},
            {"#_ev->Get()->Record.GetTabletID()", ev->Get()->Record.GetTabletID()});
        return;
    }
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvCreateTabletReply for unknown txId: , shardIdx: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"shardIdx", shardIdx},
            {"tabletId", ev->Get()->Record.GetTabletID()},
            {"schemeshard", TabletID()});
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvCreateTabletReply but partId in unknown, for txId: , shardIdx: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"shardIdx", shardIdx},
            {"tabletId", ev->Get()->Record.GetTabletID()},
            {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvHive::TEvAdoptTabletReply::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!ShardInfos.contains(shardIdx)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvAdoptTabletReply for unknown shard idx  tabletId ",
            {"#_shardIdx", shardIdx},
            {"#_ev->Get()->Record.GetTabletID()", ev->Get()->Record.GetTabletID()});
        return;
    }
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvAdoptTabletReply for unknown txId  shardIdx  tabletId ",
            {"#_txId", txId},
            {"#_shardIdx", shardIdx},
            {"#_ev->Get()->Record.GetTabletID()", ev->Get()->Record.GetTabletID()});
...
void TSchemeShard::Handle(TEvHive::TEvDeleteTabletReply::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Free tablet reply, message: , at schemeshard: ",
        {"message", ev->Get()->Record.ShortDebugString()},
        {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvHive::TEvDeleteOwnerTabletsReply::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Free owner tablets reply, message: , at schemeshard: ",
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDeleteOwnerTabletsReply for unknown txId  ownerID  form hive  at schemeshard ",
            {"#_txId", txId},
            {"#_record.GetOwner()", record.GetOwner()},
            {"#_record.GetOrigin()", record.GetOrigin()},
            {"#_TabletID()", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvHive::TEvUpdateTabletsObjectReply::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Update tablets object reply, message: , at schemeshard: ",
        {"message", record.ShortDebugString()},
        {"schemeshard", TabletID()});
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvUpdateTabletsObjectReply for unknown txId  at schemeshard ",
            {"#_txId", txId},
            {"#_TabletID()", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvHive::TEvUpdateDomainReply::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Update domain reply, message: , at schemeshard: ",
        {"message", record.ShortDebugString()},
        {"schemeshard", TabletID()});

    const auto txId = TTxId(record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvUpdateDomainReply for unknown txId  at schemeshard ",
            {"#_txId", txId},
            {"#_TabletID()", TabletID()});
        return;
    }
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvHive::TEvUpdateDomainReply but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvPersQueue::TEvDropTabletReply::TPtr &ev, const TActorContext &ctx) {
    ...
    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvPersQueue::TEvDropTabletReply for unknown txId , message: ",
            {"#_txId", txId},
            {"message", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    TSubTxId partId = Operations.at(txId)->FindRelatedPartByTabletId(tabletId, ctx);
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvPersQueue::TEvDropTabletReply but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvPersQueue::TEvUpdateConfigResponse::TPtr& ev, const TActorContext& ctx)
{
    ...
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvPersQueue::TEvUpdateConfigResponse for unknown txId  message ",
            {"#_txId", txId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvUpdateConfigResponse but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvPersQueue::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx)
{
    const TTxId txId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvPersQueue::TEvProposeTransactionResult for unknown txId  message ",
            {"#_txId", txId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    const TSubTxId partId = Operations.at(txId)->FindRelatedPartByTabletId(tabletId, ctx);
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvProposeTransactionResult but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvBlobDepot::TEvApplyConfigResult::TPtr& ev, const TActorContext& ctx) {
    ...
    if (const auto it = Operations.find(txId); it == Operations.end()) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvBlobDepot::TEvApplyConfigResult for unknown txId  message ",
            {"#_txId", txId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
    } else if (const TSubTxId partId = it->second->FindRelatedPartByTabletId(tabletId, ctx); partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvBlobDepot::TEvApplyConfigResult but partId is unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
    } else {
...
void TSchemeShard::Handle(TEvColumnShard::TEvProposeTransactionResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvProposeTransactionResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});

    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvColumnShard::TEvProposeTransactionResult for unknown txId, ignore it, txId: , message: , at schemeshard: ",
            {"txId", txId},
            {"message", ev->Get()->Record.ShortDebugString()},
            {"schemeshard", TabletID()});
        return;
    ...
    TSubTxId partId = Operations.at(txId)->FindRelatedPartByTabletId(tabletId, ctx);
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvProposeTransactionResult but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvColumnShard::TEvNotifyTxCompletionResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvNotifyTxCompletionResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});

    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvColumnShard::TEvNotifyTxCompletionResult for unknown txId, ignore it, txId: , message: , at schemeshard: ",
            {"txId", txId},
            {"message", ev->Get()->Record.ShortDebugString()},
            {"schemeshard", TabletID()});
        return;
    ...
    TSubTxId partId = Operations.at(txId)->FindRelatedPartByTabletId(tabletId, ctx);
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvNotifyTxCompletionResult but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvCreateSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvCreateSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvDropSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvDropSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvUpdateSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvUpdateSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvFreezeSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvFreezeSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvRestoreSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvRestoreSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvRedirectSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvRedirectSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NSequenceShard::TEvSequenceShard::TEvGetSequenceResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvGetSequenceResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NReplication::TEvController::TEvCreateReplicationResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvCreateReplicationResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NReplication::TEvController::TEvAlterReplicationResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvAlterReplicationResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(NReplication::TEvController::TEvDropReplicationResult::TPtr &ev, const TActorContext &ctx) {
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvDropReplicationResult, at schemeshard: , message: ",
        {"schemeshard", TabletID()},
        {"message", ev->Get()->Record.ShortDebugString()});
...
void TSchemeShard::Handle(TEvDataShard::TEvProposeTransactionResult::TPtr &ev, const TActorContext &ctx) {
    ...
    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvDataShard::TEvProposeTransactionResult for unknown txId, ignore it, txId: , message: , at schemeshard: ",
            {"txId", txId},
            {"message", ev->Get()->Record.ShortDebugString()},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvSubDomain::TEvConfigureStatus::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSubDomain::TEvConfigureStatus, no route has found by tabletId  message ",
            {"#_tabletId", tabletId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (opId.GetSubTxId() == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSubDomain::TEvConfigureStatus but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", opId.GetTxId()},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvBlockStore::TEvUpdateVolumeConfigResponse::TPtr& ev, const TActorContext& ctx) {
    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvBlockStore::TEvUpdateVolumeConfigResponse for unknown txId  tabletId ",
            {"#_txId", txId},
            {"#_ev->Get()->Record.GetOrigin()", ev->Get()->Record.GetOrigin()});
        return;
    }
    ...
    if (partId == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvUpdateVolumeConfigResponse but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", txId},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvFileStore::TEvUpdateConfigResponse::TPtr& ev, const TActorContext& ctx) {
    const auto txId = TTxId(ev->Get()->Record.GetTxId());
    if (!Operations.contains(txId)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvFileStore::TEvUpdateConfigResponse for unknown txId  tabletId ",
            {"#_txId", txId},
            {"#_ev->Get()->Record.GetOrigin()", ev->Get()->Record.GetOrigin()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvInitTenantSchemeShardResult::TPtr& ev, const TActorContext& ctx) {
    ...
    TOperationId opId = RouteIncoming(tabletId, ctx);
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSchemeShard::TEvInitTenantSchemeShardResult no route has found by tabletId  message ",
            {"#_tabletId", tabletId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvPublishTenantAsReadOnlyResult::TPtr& ev, const TActorContext& ctx) {
    ...
    TOperationId opId = RouteIncoming(tabletId, ctx);
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSchemeShard::TEvPublishTenantAsReadOnlyResult no route has found by tabletId  message ",
            {"#_tabletId", tabletId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (opId.GetSubTxId() == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSchemeShard::TEvPublishTenantAsReadOnlyResult but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", opId.GetTxId()},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(TEvSchemeShard::TEvPublishTenantResult::TPtr& ev, const TActorContext& ctx) {
    ...
    TOperationId opId = RouteIncoming(tabletId, ctx);
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSchemeShard::TEvPublishTenantResult no route has found by tabletId  message ",
            {"#_tabletId", tabletId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (opId.GetSubTxId() == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got TEvSchemeShard::TEvPublishTenantResult but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", opId.GetTxId()},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::Handle(NKesus::TEvKesus::TEvSetConfigResult::TPtr& ev, const TActorContext& ctx) {
    ...
    TOperationId opId = RouteIncoming(tabletId, ctx);
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got NKesus::TEvKesus::TEvSetConfigResult no route has found by tabletId  message ",
            {"#_tabletId", tabletId},
            {"#_num_0", ev->Get()->Record.ShortDebugString()});
        return;
    }
    ...
    if (opId.GetSubTxId() == InvalidSubTxId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got NKesus::TEvKesus::TEvSetConfigResult but partId in unknown, for txId: , tabletId: , at schemeshard: ",
            {"txId", opId.GetTxId()},
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
...
void TSchemeShard::RestartPipeTx(TTabletId tabletId, const TActorContext& ctx) {
    for (auto item : PipeTracker.FindTx(ui64(tabletId))) {
        ...
        YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Transaction  reset current state at schemeshard  because pipe to tablet  disconnected",
            {"#_txId", txId},
            {"#_TabletID()", TabletID()},
            {"#_tabletId", tabletId});
        ...
        if (!operation->PipeBindedMessages.contains(tabletId)) {
            for (ui64 pipeTrackerCookie : PipeTracker.FindCookies(ui64(txId), ui64(tabletId))) {
                YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Pipe attached message is not found, ignore event, opId:, tableId: , at schemeshardId: ",
        ...
        for (auto& item: operation->PipeBindedMessages.at(tabletId)) {
            ...
            YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Pipe attached message is found and resent into the new pipe, opId:, dst tableId: , msg type: , msg cookie: , at schemeshardId: ",
                {"opId", msg.OpId},
                {"tableId", tabletId},
                {"type", msg.Type},
                {"cookie", msgCookie},
...
void TSchemeShard::Handle(TEvSchemeShard::TEvCancelTx::TPtr& ev, const TActorContext& ctx) {
    if (IsReadOnlyMode) {
        YDBLOG_CTX_COMP_ERROR(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Ignoring message TEvSchemeShard::TEvCancelTx reason# schemeshard in readonly schemeshard# ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvMigrateSchemeShard::TPtr &ev, const TActorContext &ctx) {
    if (InitState != TTenantInitState::Inprogress) {
        YDBLOG_CTX_COMP_ERROR(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Ignoring message TEvSchemeShard::TEvMigrateSchemeShard: reason# schemeshard not in TTenantInitState::Inprogress state state is  schemeshard# ",
            {"#_(ui64) InitState", (ui64) InitState},
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvMigrateSchemeShardResult::TPtr &ev, const TActorContext &ctx) {
    ...
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "unable to resolve operation by tabletID:  ignore TEvSubDomain::TEvMigrateSchemeShardResult , at schemeshard: ",
            {"tabletID", tabletId},
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvDataShard::TEvMigrateSchemeShardResponse::TPtr &ev, const TActorContext &ctx) {
    ...
    auto opId = RouteIncoming(tabletId, ctx);
    if (!opId) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "unable to resolve operation by tabletID:  ignore TEvDataShard::TEvMigrateSchemeShardResponse , at schemeshard: ",
            {"tabletID", tabletId},
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvPrivate::TEvRunConditionalErase::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvRunConditionalErase, at schemeshard: ",
        {"schemeshard", TabletID()});
...
bool TSchemeShard::ProcessPendingConditionalEraseResponseBatch(const TInstant& now, const TActorContext& ctx) {
    ...
    if (PendingCondEraseResponses.size() > 0 && (completeBySize || completeByTime)) {
        YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Conditional erase flush pending response batch (by ), batch size /, batch age /, at schemeshard: ",
            {"#_num_0", (completeBySize ? "size" : "time")},
            {"#_num_1", PendingCondEraseResponses.size()},
            {"#_CondEraseResponseBatchSize", CondEraseResponseBatchSize},
            {"#_batchAge", batchAge},
            {"#_CondEraseResponseBatchMaxTime", CondEraseResponseBatchMaxTime},
            {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvPrivate::TEvFlushConditionalEraseBatch::TPtr& ev, const TActorContext& ctx) {
    ...
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvFlushConditionalEraseBatch, at schemeshard: ",
        {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvDataShard::TEvConditionalEraseRowsResponse::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!ShardInfos.contains(shardIdx)) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Unable to resolve shard info: tabletId: , at schemeshard: ",
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
    }
    ...
    if (record.GetStatus() == NKikimrTxDataShard::TEvConditionalEraseRowsResponse::ACCEPTED) {
        YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Conditional erase accepted: tabletId: , at schemeshard: ",
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
    }
    if (record.GetStatus() == NKikimrTxDataShard::TEvConditionalEraseRowsResponse::PARTIAL) {
        YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Conditional erase already running: tabletId: , at schemeshard: ",
            {"tabletId", tabletId},
            {"schemeshard", TabletID()});
        return;
    }
    ...
    if (batchingEnabled) {
        if (!ProcessPendingConditionalEraseResponseBatch(now, ctx)) {
            YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Conditional erase finished, tabletId: , status: , batch size /, batch age /, enqueued, at schemeshard: ",
                {"tabletId", tabletId},
                {"status", record.GetStatus()},
                {"#_num_0", PendingCondEraseResponses.size()},
                {"#_CondEraseResponseBatchSize", CondEraseResponseBatchSize},
                {"#_num_1", (now - PendingCondEraseResponsesStartTime)},
                {"#_CondEraseResponseBatchMaxTime", CondEraseResponseBatchMaxTime},
                {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvTxAllocatorClient::TEvAllocateResult::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvAllocateResult: Cookie# , at schemeshard: ",
        {"Cookie", ev->Cookie},
        {"schemeshard", TabletID()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvAllocateResult:  Cookie: , at schemeshard: ",
        {"Cookie", id},
        {"schemeshard", TabletID()});
}

void TSchemeShard::Handle(TEvSchemeShard::TEvModifySchemeTransactionResult::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvModifySchemeTransactionResult: txId# , status# ",
        {"txId", ev->Get()->Record.GetTxId()},
        {"status", ev->Get()->Record.GetStatus()});
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Message:\n",
        {"#_num_0", ev->Get()->Record.ShortDebugString()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvModifySchemeTransactionResult:  txId: , at schemeshard: ",
        {"txId", txId},
        {"schemeshard", TabletID()});
}

void TSchemeShard::Handle(TEvIndexBuilder::TEvCreateResponse::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvIndexBuilder::TEvCreateResponse: txId# , status# ",
        {"txId", ev->Get()->Record.GetTxId()},
        {"status", ev->Get()->Record.GetStatus()});
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Message:\n",
        {"#_num_0", ev->Get()->Record.ShortDebugString()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvIndexBuilder::TEvCreateResponse:  txId: , at schemeshard: ",
        {"txId", txId},
        {"schemeshard", TabletID()});
}
...
void TSchemeShard::Handle(TEvSchemeShard::TEvNotifyTxCompletionResult::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvNotifyTxCompletionResult: txId# ",
        {"txId", ev->Get()->Record.GetTxId()});
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Message:\n",
        {"#_num_0", ev->Get()->Record.ShortDebugString()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvNotifyTxCompletionResult:  txId: , at schemeshard: ",
        {"txId", txId},
        {"schemeshard", TabletID()});
}

void TSchemeShard::Handle(TEvSchemeShard::TEvCancelTxResult::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvCancelTxResult: Cookie: , at schemeshard: ",
        {"Cookie", ev->Cookie},
        {"schemeshard", TabletID()});
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Message:\n",
        {"#_num_0", ev->Get()->Record.ShortDebugString()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvCancelTxResult: Cookie: , at schemeshard: ",
        {"Cookie", id},
        {"schemeshard", TabletID()});
}

void TSchemeShard::Handle(TEvIndexBuilder::TEvCancelResponse::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle: TEvIndexBuilder::TEvCancelResponse: Cookie: , at schemeshard: ",
        {"Cookie", ev->Cookie},
        {"schemeshard", TabletID()});
    YDBLOG_CTX_COMP_DEBUG(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Message:\n",
        {"#_num_0", ev->Get()->Record.ShortDebugString()});
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "no able to determine destination for message TEvIndexBuilder::TEvCancelResponse: Cookie: , at schemeshard: ",
        {"Cookie", id},
        {"schemeshard", TabletID()});
}
...
void TSchemeShard::Handle(TEvPrivate::TEvConsoleConfigsTimeout::TPtr&, const TActorContext& ctx) {
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot get console configs");
...
void TSchemeShard::Handle(TEvents::TEvUndelivered::TPtr& ev, const TActorContext& ctx) {
    ...
    YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot subscribe to console configs");
    LoadTableProfiles(nullptr, ctx);
}
...
void TSchemeShard::ApplyConsoleConfigs(const NKikimrConfig::TFeatureFlags& featureFlags, const TActorContext& ctx) {
    if (featureFlags.GetAllowServerlessStorageBillingForSchemeShard() != (bool)AllowServerlessStorageBilling) {
        YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ApplyConsoleConfigs: AllowServerlessStorageBillingForSchemeShard has been changed, schemeshardId: , old: , new: ",
...
void TSchemeShard::ConfigureStatsBatching(const NKikimrConfig::TSchemeShardConfig& config, const TActorContext& ctx) {
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "StatsBatching config: StatsBatchTimeout# , StatsMaxBatchSize# , StatsMaxExecuteTime# ",
        {"StatsBatchTimeout", StatsBatchTimeout},
...
void TSchemeShard::ConfigureStatsOperations(const NKikimrConfig::TSchemeShardConfig& config, const TActorContext& ctx) {
    ...
    if (InFlightLimits.empty()) {
        ...
        YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "OperationsProcessing config: using default configuration");
    }

    for (auto it = InFlightLimits.begin(); it != InFlightLimits.end(); ++it) {
        YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "OperationsProcessing config: type , limit ",
            {"#_TTxState::TypeName(it->first)", TTxState::TypeName(it->first)},
    ...
    const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "BackgroundCompactionQueue configured: Timeout# , compact single parted# , Rate# , WakeupInterval# , RoundInterval# , InflightLimit# , MinCompactionRepeatDelaySeconds# , MaxRate# ",
        {"Timeout", compactionConfig.Timeout},
        {"parted", (queueConfig.CompactSinglePartedShards ? "yes" : "no")},
    ...
    const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "BorrowedCompactionQueue configured: Timeout# , Rate# , WakeupInterval# , InflightLimit# ",
        {"Timeout", compactionConfig.Timeout},
    ...
    const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ForcedCompactionQueue configured: Timeout# , Rate# , WakeupInterval# , InflightLimit# , ForcedCompactionPersistBatchSize# , ForcedCompactionPersistBatchMaxTime# ",
        {"Timeout", compactionConfig.Timeout},
    ...
    const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "BackgroundCleaningQueue configured: Timeout# , Rate# , WakeupInterval# , InflightLimit# ",
        {"Timeout", cleaningConfig.Timeout},
        ...
        const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "PasswordComplexity for LoginProvider configured: MinLength# , MinLowerCaseCount# , MinUpperCaseCount# , MinNumbersCount# , MinSpecialCharsCount# , SpecialChars# , CanContainUsername# ",
        ...
        const TActorContext &ctx)
{
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "AccountLockout configured: AttemptThreshold# , AttemptResetDuration# ",
        {"AttemptThreshold", accountLockoutInitializer.AttemptThreshold},
...
void TSchemeShard::ConfigureExternalSources(
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ExternalSources configured: HostnamePatterns# , AvailableExternalDataSources# ",
        {"HostnamePatterns", Join(", ", hostnamePatterns)},
...
void TSchemeShard::ConfigureCondErase(const NKikimrConfig::TSchemeShardConfig& config, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "ConditionalErase configured: ShardsInFlight (for table) , BatchSize , BatchMaxTime ",
        {"#_MaxTTLShardsInFlight", MaxTTLShardsInFlight},
        {"#_CondEraseResponseBatchSize", CondEraseResponseBatchSize},
        {"#_CondEraseResponseBatchMaxTime", CondEraseResponseBatchMaxTime});
}
...
void TSchemeShard::Handle(NConsole::TEvConfigsDispatcher::TEvSetConfigSubscriptionResponse::TPtr &, const TActorContext &ctx) {
     YDBLOG_CTX_COMP_NOTICE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Subscription to Console has been set up, schemeshardId: ",
         {"schemeshardId", SelfTabletId()});
}
...
void TSchemeShard::Handle(NConsole::TEvConsole::TEvConfigNotificationRequest::TPtr &ev, const TActorContext &ctx) {
    ...
    YDBLOG_CTX_COMP_INFO(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Got new config: ",
    ...
    YDBLOG_CTX_COMP_TRACE(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Send TEvConfigNotificationResponse: ",
...
void TSchemeShard::Handle(TEvSchemeShard::TEvWakeupToRunShred::TPtr &ev, const TActorContext &ctx) {
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvWakeupToRunShred in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvShredInfoRequest::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvShredInfoRequest, at schemeshard: ",
        {"schemeshard", TabletID()});
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvShredInfoRequest in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvShredManualStartupRequest::TPtr&, const TActorContext& ctx) {
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvShredManualStartupRequest in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
    RunRootShred();
...
void TSchemeShard::Handle(TEvSchemeShard::TEvTenantShredRequest::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvTenantShredRequest, at schemeshard: ",
        {"schemeshard", TabletID()});
...
void TSchemeShard::Handle(TEvSchemeShard::TEvTenantShredResponse::TPtr& ev, const TActorContext& ctx) {
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvTenantShredResponse in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvBlobStorage::TEvControllerShredResponse::TPtr& ev, const TActorContext& ctx) {
    YDBLOG_COMP_DEBUG(NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvControllerShredResponse, at schemeshard: ",
        {"schemeshard", TabletID()});
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvControllerShredResponse in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvSchemeShard::TEvWakeupToRunShredBSC::TPtr&, const TActorContext& ctx) {
    if (!IsDomainSchemeShard) {
        YDBLOG_CTX_COMP_WARN(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Cannot handle EvWakeupToRunShredBSC in tenant schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
...
void TSchemeShard::Handle(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev, const TActorContext&) {
    YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "Handle TEvTxProxySchemeCache::TEvNavigateKeySetResult, at schemeshard: ",
        {"schemeshard", TabletID()});
    ...
    if (entry.DomainInfo->Params.HasStatisticsAggregator()) {
        ...
        YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "Handle TEvTxProxySchemeCache::TEvNavigateKeySetResult, StatisticsAggregatorId=, at schemeshard: ",
...
void TSchemeShard::Handle(TEvPrivate::TEvSendBaseStatsToSA::TPtr&, const TActorContext& ctx) {
    ...
    YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "Schedule next SendBaseStatsToSA in , at schemeshard: ",
        {"#_delta", delta},
        {"schemeshard", TabletID()});
...
void TSchemeShard::ResolveSA() {
    ...
    if (IsServerlessDomain(subDomainInfo)) {
        ...
        YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "ResolveSA(), StatisticsAggregatorId=, at schemeshard: ",
            {"StatisticsAggregatorId", StatisticsAggregatorId},
            {"schemeshard", TabletID()});
        ConnectToSA();
    }
...
void TSchemeShard::ConnectToSA() {
    ...
    if (!StatisticsAggregatorId) {
        YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "ConnectToSA(), no StatisticsAggregatorId, at schemeshard: ",
            {"schemeshard", TabletID()});
        return;
    }
    ...
    YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "ConnectToSA(), pipe client id: , at schemeshard: , StatisticsAggregatorId: , at schemeshard: ",
        {"id", SAPipeClientId},
        {"schemeshard", TabletID()},
        {"StatisticsAggregatorId", StatisticsAggregatorId},
        {"schemeshard", TabletID()});
...
TDuration TSchemeShard::SendBaseStatsToSA() {
    ...
    if (!SAPipeClientId) {
        ResolveSA();
        if (!StatisticsAggregatorId) {
            YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "SendBaseStatsToSA(), no StatisticsAggregatorId, at schemeshard: ",
                {"schemeshard", TabletID()});
            return TDuration::Seconds(30);
        } else {
            YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "SendBaseStatsToSA(), StatisticsAggregatorId=, at schemeshard: ",
                {"StatisticsAggregatorId", StatisticsAggregatorId},
                {"schemeshard", TabletID()});
        }
    }
    ...
    if (!count) {
        YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "SendBaseStatsToSA() No tables to send, at schemeshard: ",
            {"schemeshard", TabletID()});
        return TDuration::Seconds(30);
    }
    ...
    YDBLOG_COMP_DEBUG(NKikimrServices::STATISTICS, "SendBaseStatsToSA(), path count: , paths with incomplete stats: , at schemeshard: ",
        {"count", count},
