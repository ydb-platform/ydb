...
EAttachChildResult TSchemeShard::AttachChild(TPathElement::TPtr child) {
    ...
    if (!parent->GetChildren().contains(child->Name)) {
        ...
        YDB_LOG_DEBUG_COMP(NKikimrServices::FLAT_TX_SCHEMESHARD, "AttachChild: child attached as only one child to the parent parent parent child child",
            {"id", parent->PathId},
            {"name", parent->Name},
            {"#_name", child->Name},
            {"#_id", child->PathId},
    ...
    YDB_LOG_DEBUG_COMP(NKikimrServices::FLAT_TX_SCHEMESHARD, "AttachChild: decision parent parent child prev child child child",
        {"is", decision},
        {"id", parent->PathId},
        {"name", parent->Name},
        {"#_name", child->Name},
        {"#_id", prevChild->PathId},
        {"#_id", child->PathId},
...
void TSchemeShard::Handle(TEvTabletPipe::TEvClientDestroyed::TPtr &ev, const TActorContext &ctx) {
    ...
    YDB_LOG_TRACE_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Client pipe is reset",
        {"toTablet", tabletId},
        {"from", TabletID()});
...
void TSchemeShard::Handle(TEvSchemeShard::TEvSyncTenantSchemeShard::TPtr& ev, const TActorContext& ctx) {
    ...
    if (!SubDomains.contains(pathId)) {
        YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, ignore spurious message from dropped subdomain's schemeshard (partial cleanup)",
            {"atSchemeshard", TabletID()},
            {"pathId", pathId});
        return;
    }
    ...
    if (!PathsById.contains(pathId)) {
        YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, ignore spurious message from dropped subdomain's schemeshard (full cleanup)",
            {"atSchemeshard", TabletID()},
            {"pathId", pathId});
        return;
    }
    ...
    if (PathsById.at(pathId)->Dropped()) {
        ...
        YDB_LOG_WARN_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Handle TEvSyncTenantSchemeShard, ignore spurious message from dropped subdomain's schemeshard (pre cleanup)",
            {"atSchemeshard", TabletID()},
            {"pathId", pathId});
        return;
    }
...
bool TSchemeShard::ProcessPendingConditionalEraseResponseBatch(const TInstant& now, const TActorContext& ctx) {
    ...
    if (PendingCondEraseResponses.size() > 0 && (completeBySize || completeByTime)) {
        YDB_LOG_DEBUG_CTX_COMP(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "Conditional erase flush pending response batch (by batch size / batch age /",
            {"completeBy", (completeBySize ? "size" : "time")},
