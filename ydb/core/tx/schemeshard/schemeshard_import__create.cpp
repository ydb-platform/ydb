...
private:
    ...
    void ExecutePreparedQuery(TImportInfo::TPtr importInfo, ui32 itemIdx, TTxId txId) {
        Y_ABORT_UNLESS(itemIdx < importInfo->Items.size());
    ...
    void OnSchemeResult(TTransactionContext& txc, const TActorContext& ctx) {
        ...
        if (!IsCreatedByQuery(item)) {
            ...
            const TString source = TStringBuilder()
                << importInfo->Settings.items(msg.ItemIdx).source_prefix() << NYdb::NDump::CREATE_VIEW_FILE_NAME;
    ...
    void OnAllocateResult(const TActorContext&) {
        ...
        for (ui32 i : xrange(importInfo->Items.size())) {
            ...
            case EState::CreateSchemeObject:
                if (item.PreparedCreationQuery) {
                    ExecutePreparedQuery(importInfo, i, txId);
                    itemIdx = i;
                    break;
                }
                if (IsCreatedByQuery(item)) {
