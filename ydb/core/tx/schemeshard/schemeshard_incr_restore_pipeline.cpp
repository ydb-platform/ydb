...
#define LOG_D(stream) LOG_DEBUG_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[IncrRestorePipeline] " << stream)
#define LOG_N(stream) LOG_NOTICE_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[IncrRestorePipeline] " << stream)
#define LOG_W(stream) LOG_WARN_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[IncrRestorePipeline] " << stream)
#define LOG_E(stream) LOG_ERROR_S(ctx, NKikimrServices::FLAT_TX_SCHEMESHARD, "[IncrRestorePipeline] " << stream)

namespace NKikimr::NSchemeShard {

using namespace NTabletFlatExecutor;

class TIncrRestorePipelineFinalizer: public TActorBootstrapped<TIncrRestorePipelineFinalizer> {
public:
    static constexpr NKikimrServices::TActivity::EType ActorActivityType() {
        return NKikimrServices::TActivity::SCHEMESHARD_CDC_STREAM_SCAN_FINALIZER;
    }

    explicit TIncrRestorePipelineFinalizer(const TActorId& ssActorId, THolder<TEvSchemeShard::TEvModifySchemeTransaction>&& req)
        : SSActorId(ssActorId)
    ...
    void Bootstrap() {
        AllocateTxId();
        Become(&TIncrRestorePipelineFinalizer::StateWork);
    }
...
}; // TIncrRestorePipelineFinalizer

struct TSchemeShard::TIncrRestorePipeline::TTxProgress: public TTransactionBase<TSchemeShard> {
    // params
    TEvPrivate::TEvRunIncrRestorePipeline::TPtr RunIncrRestorePipeline = nullptr;
    TEvDataShard::TEvIncrRestorePipelineResponse::TPtr IncrRestorePipelineResponse = nullptr;
    struct {
...
public:
    explicit TTxProgress(TSelf* self, TEvPrivate::TEvRunIncrRestorePipeline::TPtr& ev)
        : TBase(self)
        , RunIncrRestorePipeline(ev)
    {
    }

    explicit TTxProgress(TSelf* self, TEvDataShard::TEvIncrRestorePipelineResponse::TPtr& ev)
        : TBase(self)
        , IncrRestorePipelineResponse(ev)
    {
    }
    ...
    bool Execute(TTransactionContext& txc, const TActorContext& ctx) override {
        if (RunIncrRestorePipeline) {
            return OnRunIncrRestorePipeline(txc, ctx);
        } else if (IncrRestorePipelineResponse) {
            return OnIncrRestorePipelineResponse(txc, ctx);
        } else if (PipeRetry) {
    ...
    void Complete(const TActorContext& ctx) override {
        for (auto& [streamPathId, tabletId, ev] : ScanRequests) {
            Self->IncrRestorePipelinePipes.Create(streamPathId, tabletId, std::move(ev), ctx);
        }

        if (StreamToProgress) {
            ctx.Send(ctx.SelfID, new TEvPrivate::TEvRunIncrRestorePipeline(StreamToProgress));
        }
        ...
        if (Finalize) {
            ctx.Register(new TIncrRestorePipelineFinalizer(ctx.SelfID, std::move(Finalize)));
        }
    }

private:
    bool OnRunIncrRestorePipeline(TTransactionContext& txc, const TActorContext& ctx) {
        const auto& streamPathId = RunIncrRestorePipeline->Get()->StreamPathId;
        ...
        if (streamInfo->ScanShards.empty()) {
            ...
            for (const auto& shard : table->GetPartitions()) {
                const auto status = TIncrRestorePipelineInfo::TShardStatus(NKikimrTxDataShard::TEvIncrRestorePipelineResponse::PENDING);
                // FIXME
                // streamInfo->ScanShards.emplace(shard.ShardIdx, status);
                streamInfo->PendingShards.insert(shard.ShardIdx);
                Self->PersistIncrRestorePipelineShardStatus(db, streamPathId, shard.ShardIdx, status);
            }
        }
        ...
        while (!streamInfo->PendingShards.empty()) {
            ...
            auto ev = MakeHolder<TEvDataShard::TEvIncrRestorePipelineRequest>();
    ...
    bool OnIncrRestorePipelineResponse(TTransactionContext& txc, const TActorContext& ctx) {
        ...
        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::ACCEPTED:
        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::IN_PROGRESS:
        //     break;

        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::DONE:
        //     status.Status = record.GetStatus();
        //     streamInfo->DoneShards.insert(shardIdx);
        //     streamInfo->InProgressShards.erase(shardIdx);
        //     Self->IncrRestorePipelinePipes.Close(streamPathId, tabletId, ctx);
        //     StreamToProgress = streamPathId;
        //     Bill(streamPathId, shardIdx, TRUCalculator::ReadTable(record.GetStats().GetBytesProcessed()), ctx);
        //     break;

        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::OVERLOADED:
        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::ABORTED:
        ...
        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::BAD_REQUEST:
        // case NKikimrTxDataShard::TEvIncrRestorePipelineResponse::SCHEME_ERROR:
        //     Y_ABORT("unreachable");
        ...
        // NIceDb::TNiceDb db(txc.DB);
        // Self->PersistIncrRestorePipelineShardStatus(db, streamPathId, shardIdx, status);
...
ITransaction* TSchemeShard::CreateTxProgressIncrRestorePipeline(TEvPrivate::TEvRunIncrRestorePipeline::TPtr& ev) {
    return new TIncrRestorePipeline::TTxProgress(this, ev);
}

ITransaction* TSchemeShard::CreateTxProgressIncrRestorePipeline(TEvDataShard::TEvIncrRestorePipelineResponse::TPtr& ev) {
    return new TIncrRestorePipeline::TTxProgress(this, ev);
}

void TSchemeShard::Handle(TEvPrivate::TEvRunIncrRestorePipeline::TPtr& ev, const TActorContext& ctx) {
    Execute(CreateTxProgressIncrRestorePipeline(ev), ctx);
}

void TSchemeShard::Handle(TEvDataShard::TEvIncrRestorePipelineResponse::TPtr& ev, const TActorContext& ctx) {
    Execute(CreateTxProgressIncrRestorePipeline(ev), ctx);
}

void TSchemeShard::ResumeIncrRestorePipelines(const TVector<TPathId>& ids, const TActorContext& ctx) {
    for (const auto& id : ids) {
        Send(ctx.SelfID, new TEvPrivate::TEvRunIncrRestorePipeline(id));
    }
}

void TSchemeShard::PersistIncrRestorePipelineShardStatus(NIceDb::TNiceDb& db, const TPathId& streamPathId,
        const TShardIdx& shardIdx, const TIncrRestorePipelineInfo::TShardStatus& status)
{
    db.Table<Schema::IncrRestorePipelineShardStatus>()
        .Key(streamPathId.OwnerId, streamPathId.LocalPathId, shardIdx.GetOwnerId(), shardIdx.GetLocalId())
        .Update(
            NIceDb::TUpdate<Schema::IncrRestorePipelineShardStatus::Status>(status.Status)
        );
}

void TSchemeShard::RemoveIncrRestorePipelineShardStatus(NIceDb::TNiceDb& db, const TPathId& streamPathId, const TShardIdx& shardIdx) {
    db.Table<Schema::IncrRestorePipelineShardStatus>()
        .Key(streamPathId.OwnerId, streamPathId.LocalPathId, shardIdx.GetOwnerId(), shardIdx.GetLocalId())
        .Delete();
}
