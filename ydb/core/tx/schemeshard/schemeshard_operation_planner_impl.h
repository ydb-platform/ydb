...
// every operation type uses; each type adds its own members in its own file and registers
// them in the table at the top of schemeshard_operation_planner.cpp.
...
public:
    ...
    // The database root a plan is relative to, from the path a request anchors on.
    ...
    // --- per operation type ----------------------------------------------------------------
    // A type contributes a planner for its request and the path its request anchors on, the
    // one whose nearest existing ancestor names the database.

    // schemeshard_operation_planner_create_table.cpp
    bool PlanCreateTable(ui32 requestIdx, const TTxTransaction& tx);
    static TPath AnchorCreateTable(TSchemeShard* ss, const TTxTransaction& tx);

    // schemeshard_operation_planner_drop_table.cpp
    bool PlanDropTable(ui32 requestIdx, const TTxTransaction& tx);
    static TPath AnchorDropTable(TSchemeShard* ss, const TTxTransaction& tx);
    ...
    bool PlanSinglePart(ui32 requestIdx, EPlannedPartKind kind, const TTxTransaction& tx);
    static TPath AnchorSinglePart(TSchemeShard* ss, EPlannedPartKind kind, const TTxTransaction& tx);
