...
bool isSysDirCreateAllowed = false;
...
const TPath::TChecker& TPath::TChecker::IsSysViewDirectory(EStatus status) const {
    ...
    return Fail(status, TStringBuilder() << "path is not a sys view directory"
        << " (" << BasicPathInfo(Path.Base()) << ")");
}
...
const TPath::TChecker& TPath::TChecker::IsSysView(EStatus status) const {
    ...
    return Fail(status, TStringBuilder() << "path is not a sys view"
        << " (" << BasicPathInfo(Path.Base()) << ")"
    );
}
