...
public:
    ...
    void Bootstrap() {
        ...
        for (auto& [txId, requestInfo] : SysViewUpdates) {
            ...
            YDB_LOG_DEBUG_CTX(ctx, "SysViewsRosterUpdate: Send TEvModifySchemeTransaction",
    ...
    STFUNC(StateUpdateSysViewFolder) {
        switch(ev->GetTypeRewrite()) {
            ...
            default:
                YDB_LOG_CRIT("TSysViewsRosterUpdate StateUpdateSysViewFolder unexpected event",
...
private:
    ...
    void SubscribeToCompletion(TTxId txId) const {
        const auto& ctx = TlsActivationContext->AsActorContext();
        YDB_LOG_DEBUG_CTX(ctx, "SysViewsRosterUpdate: Send TEvNotifyTxCompletion",
            {"actor", ctx.SelfID.ToString()},
            {"schemeshard", SelfTabletId},
            {"txId", txId},
        );
    ...
    void Handle(TEvSchemeShard::TEvModifySchemeTransactionResult::TPtr& ev, const TActorContext& ctx) {
        ...
        YDB_LOG_DEBUG_CTX(ctx, "SysViewsRosterUpdate: Handle TEvModifySchemeTransactionResult",
            {"actor", ctx.SelfID.ToString()},
            {"schemeshard", SelfTabletId},
            {"operation", modifyInfo.DebugString()},
            {"status", status},
        );
        ...
        case NKikimrScheme::StatusAccepted:
            ...
            break;
        default:
            YDB_LOG_ERROR_CTX(ctx, "SysViewsRosterUpdate: failed to execute operation",
                {"actor", ctx.SelfID.ToString()},
                {"schemeshard", SelfTabletId},
                {"operation", modifyInfo.DebugString()},
        ...
        if (AwaitingModifySchemeRequests.empty()) {
            YDB_LOG_DEBUG_CTX(ctx, "SysViewsRosterUpdate: Send TEvRosterUpdateFinished",
                {"actor", ctx.SelfID.ToString()},
                {"schemeshard", SelfTabletId},
            );
    ...
    void Handle(TEvSchemeShard::TEvNotifyTxCompletionResult::TPtr& ev, const TActorContext& ctx) {
        ...
        YDB_LOG_DEBUG_CTX(ctx, "SysViewsRosterUpdate: Handle TEvNotifyTxCompletionResult",
            {"actor", ctx.SelfID.ToString()},
            {"schemeshard", SelfTabletId},
            {"operation", modifyInfo.DebugString()},
        );
