...
bool TUserAttributes::ApplyPatch(
    EUserAttributesOp op,
...
bool TUserAttributes::CheckAttribute(EUserAttributesOp op, const TString& name, const TString& value, TString& errStr) {
    ...
    if (name.size() > TUserAttributesLimits::MaxNameLen) {
        errStr = Sprintf("UserAttributes: name too long, name# '%s' value# '%s'", name.c_str(), value.c_str());
        return false;
    }

    if (value.size() > TUserAttributesLimits::MaxValueLen) {
        errStr = Sprintf("UserAttributes: value too long, name# '%s' value# '%s'", name.c_str(), value.c_str());
        return false;
    }
    ...
    switch (ParseName(name)) {
        ...
        case EAttribute::ASYNC_REPLICATION:
            if (op != EUserAttributesOp::CreateChangefeed) {
                errStr =
                    Sprintf("UserAttributes: attribute '%s' can only be set during CreateChangefeed", name.c_str());
                return false;
            }
...
bool TUserAttributes::CheckValueStringWeak(const TString& name, const TString& value, TString& errStr) {
    if (!IsValidPathName_WeakCheck(value)) {
        errStr = Sprintf(
            "UserAttributes: attribute '%s' has invalid value '%s', forbidden symbols are found",
            name.c_str(),
            value.c_str()
        );
        return false;
...
bool TUserAttributes::CheckValueUint64(
    const TString& name,
    const TString& value,
    TString& errStr,
    ui64 minValue,
    ...
    if (!TryFromString(value, parsed)) {
        errStr = Sprintf("UserAttributes: attribute '%s' has invalid value '%s'", name.c_str(), value.c_str());
        return false;
    }
    if (parsed < minValue) {
        errStr = Sprintf(
            "UserAttributes: attribute '%s' has invalid value '%s' < %" PRIu64, name.c_str(), value.c_str(), minValue
        );
        return false;
    }
    if (parsed > maxValue) {
        errStr = Sprintf(
            "UserAttributes: attribute '%s' has invalid value '%s' > %" PRIu64, name.c_str(), value.c_str(), maxValue
        );
        return false;
    }
    return true;
