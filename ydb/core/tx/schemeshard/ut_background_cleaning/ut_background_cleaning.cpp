...
void TestMkDir(
    ...
    AsyncMkDir(
        runtime, TTestTxConfig::SchemeShard, txId, workingDir, scheme, ownerActorId, ownerNodeIdx, allowCreateInTempDir
    );
...
THashSet<TString> GetTables(TTestActorRuntime& runtime, ui64 tabletId) {
...
void CheckTable(
    ...
    for (auto& part : description.GetPathDescription().GetTablePartitions()) shards.push_back(part.GetDatashardId());
...
){Y_UNIT_TEST(SchemeshardBackgroundCleaningTestSimpleCreateClean){TTestBasicRuntime runtime(3);
...
Y_UNIT_TEST(SchemeshardBackgroundCleaningTestSimpleDropIndex) {
    ...
    TestLs(
        runtime, "/MyRoot/tmp/TempTable/ValueIndex", TDescribeOptionsBuilder().SetShowPrivateTable(true), NLs::PathExist
    );
    ...
    TestLs(
        runtime,
        "/MyRoot/tmp/TempTable/ValueIndex",
...
Y_UNIT_TEST(TempInTemp) {
    ...
    TestMkTempDir(
        runtime, txId, "/MyRoot/test", "tmp", ownerActorId, {NKikimrScheme::StatusPreconditionFailed}, 1, true
    );
    env.TestWaitNotification(runtime, txId);
    ++txId;
    ...
    TestMkTempDir(
        runtime, txId, "/MyRoot/test/tmp", "tmp2", ownerActorId, {NKikimrScheme::StatusPreconditionFailed}, 1, false
    );
    env.TestWaitNotification(runtime, txId);
    ++txId;
    TestMkTempDir(
        runtime, txId, "/MyRoot/test/tmp", "tmp2", ownerActorId, {NKikimrScheme::StatusPreconditionFailed}, 1, true
    );
    env.TestWaitNotification(runtime, txId);
    ++txId;
    ...
    TestMkTempDir(
        runtime, txId, "/MyRoot/test/tmp/a/b", "tmp2", ownerActorId, {NKikimrScheme::StatusPreconditionFailed}, 1, false
    );
    env.TestWaitNotification(runtime, txId);
    ++txId;
    TestMkTempDir(
        runtime, txId, "/MyRoot/test/tmp/a/b", "tmp2", ownerActorId, {NKikimrScheme::StatusPreconditionFailed}, 1, true
    );
    env.TestWaitNotification(runtime, txId);
    ++txId;
}
