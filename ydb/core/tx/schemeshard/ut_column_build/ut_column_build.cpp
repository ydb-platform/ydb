...
*/
    ...
    Y_UNIT_TEST(DisableAndEnableFlag) {
        TTestBasicRuntime runtime;
        TTestEnv env(runtime, TTestEnvOptions().EnableAddColumsWithDefaults(true));
        ...
        TBlockEvents<TEvDataShard::TEvBuildIndexCreateRequest> blocked(runtime, [&](auto&) {
            return true;
        });
        ...
        UNIT_ASSERT_VALUES_EQUAL(blocked.size(), 2);
    ...
    Y_UNIT_TEST(LockingFailed) {
        ...
        TBlockEvents<TEvSchemeShard::TEvModifySchemeTransaction> blocked(runtime, [&](const auto& ev) {
