...
std::pair<TTableInfoMap, ui64> GetTables(TTestActorRuntime& runtime, ui64 tabletId) {
...
TPathInfo
GetPathInfo(TTestActorRuntime& runtime, const char* fullPath, ui64 schemeshardId = TTestTxConfig::SchemeShard) {
    TPathInfo info;
...
void DisableBackgroundCompactionViaRestart(TTestActorRuntime& runtime, TTestEnv&, ui64 schemeShard) {
...
TCompactionStats GetCompactionStats(
    ...
    for (auto shard : shards) {
        stats.Update(GetCompactionStats(runtime, userTable, shard, ownerId));
    }
...
TCompactionStats
GetCompactionStats(TTestActorRuntime& runtime, const TString& path, ui64 schemeshardId = TTestTxConfig::SchemeShard) {
    auto info = GetPathInfo(runtime, path.c_str(), schemeshardId);
    UNIT_ASSERT(!info.Shards.empty());

    return GetCompactionStats(runtime, info.UserTable, info.Shards, info.OwnerId);
}
...
void CheckShardBorrowedCompacted(
    ...
    auto count = GetCompactionStats(runtime, userTable, tabletId, ownerId).CompactBorrowedCount;
...
void CheckShardBackgroundCompacted(
    ...
    auto count = GetCompactionStats(runtime, userTable, tabletId, ownerId).BackgroundRequestCount;
...
void CheckNoBackgroundCompactionsInPeriod(
    ...
    auto count1 = GetCompactionStats(runtime, info.UserTable, info.Shards, info.OwnerId).BackgroundRequestCount;

    env.SimulateSleep(runtime, TDuration::Seconds(30));

    auto count2 = GetCompactionStats(runtime, info.UserTable, info.Shards, info.OwnerId).BackgroundRequestCount;

    UNIT_ASSERT_VALUES_EQUAL(count1, count2);
}

template <typename F>
void TestBackgroundCompaction(TTestActorRuntime& runtime, TTestEnv& env, F&& enableBackgroundCompactionFunc) {
    ui64 txId = 1000;
...
ui64 TestServerless(TTestActorRuntime& runtime, TTestEnv& env, bool enableServerless) {
    ui64 txId = 100;
    ui64 schemeshardId = TTestTxConfig::SchemeShard;
...
){Y_UNIT_TEST(SchemeshardShouldRequestCompactionsSchemeshardRestart){        // enabled via schemeshard restart
...
Y_UNIT_TEST(SchemeshardShouldHandleCompactionTimeouts) {
    ...
    while (compactionResultCount < 3UL) env.SimulateSleep(runtime, DefaultTimeout + RetryDelay + TDuration::Seconds(1));
}
}
;

Y_UNIT_TEST_SUITE(TSchemeshardBorrowedCompactionTest){Y_UNIT_TEST(SchemeshardShouldCompactBorrowedBeforeSplit
...
Y_UNIT_TEST(SchemeshardShouldHandleBorrowCompactionTimeouts) {
    ...
    while (borrowedRequests < 3) env.SimulateSleep(runtime, TDuration::Seconds(10));
}
...
Y_UNIT_TEST_SUITE(TSchemeshardCompactionQueueTest) {
    ...
    Y_UNIT_TEST(RemoveLastShardFromSubQueues) {
        ...
        while (!queue.Empty()) queue.PopFront();
    ...
    Y_UNIT_TEST(ShouldPopWhenOnlyLastCompactionQueue) {
        ...
        std::vector<TShardCompactionInfo> shardInfos = {            //                id,   ts
