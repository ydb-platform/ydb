...
Y_UNIT_TEST_SUITE(TExternalDataSourceTest) {
    ...
    void DropTwice(const TString& path, TRuntimeTxFn createFn, TRuntimeTxFn dropFn) {
        ...
        TestDescribeResult(DescribePath(runtime, path), {NLs::PathNotExist});
    }
    ...
    Y_UNIT_TEST(ParallelCreateExternalDataSource) {
        ...
        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirA/MyExternalDataSource1"), {NLs::PathVersionEqual(2)});
        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirA/MyExternalDataSource2"), {NLs::PathVersionEqual(2)});
    }
    ...
    Y_UNIT_TEST(ParallelCreateSameExternalDataSource) {
        ...
        for (ui32 i = 0; i < 3; ++i) {
            if (sts[i] == ESts::StatusAlreadyExists) {
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/NilNoviSubLuna"), {NLs::Finished, NLs::IsExternalDataSource}
                );
            }
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/NilNoviSubLuna"),
            {NLs::Finished, NLs::IsExternalDataSource, NLs::PathVersionEqual(2)}
        );
    ...
    Y_UNIT_TEST(ReadOnlyMode) {
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/MyExternalDataSource"), {NLs::Finished, NLs::IsExternalDataSource}
        );
    ...
    Y_UNIT_TEST(SchemeErrors) {
        ...
        TestCreateExternalDataSource(
            ...
            Sprintf(
                ...
                TString{1001, 'a'}.c_str()
            ),
            {{NKikimrScheme::StatusSchemeError,
              "Maximum length of location must be less or equal equal to 1000 but got 1001"}}
        );
        TestCreateExternalDataSource(
            runtime,
            ++txId,
            ...
            {{NKikimrScheme::StatusSchemeError,
              "Maximum length of installation must be less or equal equal to 1000 but got 1001"}}
        );
        TestCreateExternalDataSource(
            runtime,
            ++txId,
    ...
    Y_UNIT_TEST(PreventDeletionOfDependentDataSources) {
        ...
        TestDropExternalDataSource(
            ...
            "ExternalDataSource",
            {{NKikimrScheme::StatusSchemeError,
              "Other entities depend on this data source, please remove them at the beginning: /MyRoot/ExternalTable"}}
        );
        env.TestWaitNotification(runtime, txId);
    ...
    Y_UNIT_TEST(RemovingReferencesFromDataSources) {
        ...
        TestDropExternalTable(runtime, ++txId, "/MyRoot", "ExternalTable", {NKikimrScheme::StatusAccepted});

        TestLs(runtime, "/MyRoot/ExternalTable", false, NLs::PathNotExist);

        TestDropExternalDataSource(runtime, ++txId, "/MyRoot", "ExternalDataSource", {NKikimrScheme::StatusAccepted});
        env.TestWaitNotification(runtime, txId);
