...
Y_UNIT_TEST_SUITE(TSchemeShardTestExtSubdomainReboots) {
    ...
    Y_UNIT_TEST_FLAG(CreateExternalSubdomain, AlterDatabaseCreateHiveFirst) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            TestCreateExtSubDomain(runtime, ++t.TxId, "/MyRoot", R"(Name: "USER_0")");
...

            {
                ...
                TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::ChildrenCount(2)});
...

            {
                ...
                ui64 subdomainHiveTablets = TTestTxConfig::FakeHiveTablets +
                                            (AlterDatabaseCreateHiveFirst ? TFakeHiveState::TABLETS_PER_CHILD_HIVE : 1);
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/USER_0"),
                    {
                        ...
                        NLs::DomainCoordinators(
                            {subdomainHiveTablets + 1, subdomainHiveTablets + 2, subdomainHiveTablets + 3}
                        ),
                ...
                TestDescribeResult(
                    DescribePath(runtime, subdomainSchemeshard, "/MyRoot/USER_0"),
                    {
                        NLs::PathExist,
    ...
    Y_UNIT_TEST_FLAG(CreateExternalSubdomainWithoutHive, AlterDatabaseCreateHiveFirst) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                    ...
                    DescribePath(runtime, "/MyRoot/USER_0"),
                    {
                        ...
                        NLs::DomainCoordinators(
                            {TTestTxConfig::FakeHiveTablets + 1,
                             ...
                             TTestTxConfig::FakeHiveTablets + 3}
                        ),
                ...
                // check scheme from extsubdomain schemeshard
                TestDescribeResult(
                    DescribePath(runtime, TTestTxConfig::FakeHiveTablets, "/MyRoot/USER_0"),
                    {
                        NLs::PathExist,
                        NLs::IsSubDomain("MyRoot/USER_0"),
                        NLs::DomainKey(3, TTestTxConfig::SchemeShard),
    ...
    Y_UNIT_TEST_FLAG(CreateForceDrop, AlterDatabaseCreateHiveFirst) {
        ...
        t.GetTestEnvOptions().EnableAlterDatabaseCreateHiveFirst(AlterDatabaseCreateHiveFirst);
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            AsyncCreateExtSubDomain(runtime, ++t.TxId, "/MyRoot", R"(Name: "USER_0")");
            t.TestEnv->ReliablePropose(
                runtime, ForceDropExtSubDomainRequest(++t.TxId, "/MyRoot", "USER_0"), {NKikimrScheme::StatusAccepted}
            );
    ...
    Y_UNIT_TEST_FLAG(SchemeLimits, AlterDatabaseCreateHiveFirst) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot"),
                    {NLs::ChildrenCount(2), NLs::DomainLimitsIs(limits.MaxPaths, limits.MaxShards)}
                );
