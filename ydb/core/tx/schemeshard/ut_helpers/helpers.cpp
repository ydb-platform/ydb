...
static ui32 ReliableProposeImpl(
    ...
    const TActorId& proposer,
    TEvRequest* evRequest,
    const TVector<TStatus>& expectedStatuses
) {
...
ui32 NSchemeShardUT_Private::TTestEnv::ReliablePropose(
    ...
    return ReliableProposeImpl<TEvSchemeShard::TEvCancelTxResult>(runtime, TxReliablePropose, evTx, expectedResults);
}
...
ui32 NSchemeShardUT_Private::TTestEnv::ReliablePropose(
    ...
    return ReliableProposeImpl<TEvExport::TEvCancelExportResponse>(runtime, TxReliablePropose, ev, expectedStatuses);
}
...
ui32 NSchemeShardUT_Private::TTestEnv::ReliablePropose(
    ...
    return ReliableProposeImpl<TEvExport::TEvForgetExportResponse>(runtime, TxReliablePropose, ev, expectedStatuses);
}
...
ui32 NSchemeShardUT_Private::TTestEnv::ReliablePropose(
    ...
    return ReliableProposeImpl<TEvImport::TEvCancelImportResponse>(runtime, TxReliablePropose, ev, expectedStatuses);
}

NKikimrSchemeOp::TAlterUserAttributes
AlterUserAttrs(const TVector<std::pair<TString, TString>>& add, const TVector<TString>& drop) {
...
void CheckExpectedResult(
    const TVector<TExpectedResult>& expected,
    TEvSchemeShard::EStatus actualStatus,
    const TString& actualReason
) {
    for (auto i : expected) {
...
void CheckExpectedStatus(
    const TVector<NKikimrScheme::EStatus>& expected,
    TEvSchemeShard::EStatus actualStatus,
    const TString& actualReason
) {
...
void SkipModificationReply(TTestActorRuntime& runtime, ui32 num) {
    TAutoPtr<IEventHandle> handle;
    for (ui32 i = 0; i < num; ++i) runtime.GrabEdgeEvent<TEvSchemeShard::TEvModifySchemeTransactionResult>(handle);
}
...
TEvSchemeShard::TEvModifySchemeTransaction* CreateModifyACLRequest(
    ...
    TString parentPath,
    TString name,
    const TString& diffAcl,
    const TString& newOwner
) {
...
NKikimrScheme::TEvDescribeSchemeResult DescribePath(
    TTestActorRuntime& runtime,
    ui64 schemeShard,
    const TString& path,
...
NKikimrScheme::TEvDescribeSchemeResult DescribePathId(
    TTestActorRuntime& runtime,
    ui64 schemeShard,
    ui64 pathId,
    const NKikimrSchemeOp::TDescribeOptions& opts = {}
...
DescribePath(TTestActorRuntime& runtime, const TString& path, const NKikimrSchemeOp::TDescribeOptions& opts) {
    return DescribePath(runtime, TTestTxConfig::SchemeShard, path, opts);
}

NKikimrScheme::TEvDescribeSchemeResult
DescribePathId(TTestActorRuntime& runtime, ui64 pathId, const NKikimrSchemeOp::TDescribeOptions& opts = {}) {
    return DescribePathId(runtime, TTestTxConfig::SchemeShard, pathId, opts);
}

NKikimrScheme::TEvDescribeSchemeResult DescribePrivatePath(
    TTestActorRuntime& runtime,
    ui64 schemeShard,
    const TString& path,
    bool returnPartitioning,
    bool returnBoundaries
) {
...
NKikimrScheme::TEvDescribeSchemeResult
DescribePrivatePath(TTestActorRuntime& runtime, const TString& path, bool returnPartitioning, bool returnBoundaries) {
...
NKikimrScheme::TEvDescribeSchemeResult DescribePath(
    ...
    return DescribePath(
        runtime, TTestTxConfig::SchemeShard, path, returnPartitioning, returnBoundaries, showPrivate, returnBackups
    );
}

TPathVersion ExtractPathVersion(const NKikimrScheme::TEvDescribeSchemeResult& describe) {
    TPathVersion result;
    result.PathId = TPathId(
        describe.GetPathDescription().GetSelf().GetSchemeshardId(), describe.GetPathDescription().GetSelf().GetPathId()
    );
...
TPathVersion
TestDescribeResult(const NKikimrScheme::TEvDescribeSchemeResult& describe, TVector<NLs::TCheckFunc> checks) {
...
TString TestLs(TTestActorRuntime& runtime, const TString& path, bool returnPartitioningInfo, NLs::TCheckFunc check) {
...
Navigate(TTestActorRuntime& runtime, const TString& path, NSchemeCache::TSchemeCacheNavigate::EOp op) {
...
TEvSchemeShard::TEvModifySchemeTransaction* CopyTableRequest(
    ui64 txId,
    const TString& dstPath,
    const TString& dstName,
    const TString& srcFullName,
    TApplyIf applyIf
) {
...
TEvSchemeShard::TEvModifySchemeTransaction*
MoveTableRequest(ui64 txId, const TString& srcPath, const TString& dstPath, ui64 schemeShard, const TApplyIf& applyIf) {
    THolder<TEvSchemeShard::TEvModifySchemeTransaction> evTx =
        MakeHolder<TEvSchemeShard::TEvModifySchemeTransaction>(txId, schemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
TEvSchemeShard::TEvModifySchemeTransaction* MoveIndexRequest(
    ui64 txId,
...
void AsyncMoveIndex(
    ...
    TActorId sender = runtime.AllocateEdgeActor();
    ForwardToTablet(
        runtime, schemeShard, sender, MoveIndexRequest(txId, tablePath, srcPath, dstPath, allowOverwrite, schemeShard)
    );
}
...
TEvSchemeShard::TEvModifySchemeTransaction* LockRequest(ui64 txId, const TString& parentPath, const TString& name) {
    THolder<TEvSchemeShard::TEvModifySchemeTransaction> evTx =
        MakeHolder<TEvSchemeShard::TEvModifySchemeTransaction>(txId, TTestTxConfig::SchemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
TEvSchemeShard::TEvModifySchemeTransaction*
UnlockRequest(ui64 txId, ui64 lockId, const TString& parentPath, const TString& name) {
    THolder<TEvSchemeShard::TEvModifySchemeTransaction> evTx =
        MakeHolder<TEvSchemeShard::TEvModifySchemeTransaction>(txId, TTestTxConfig::SchemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
auto CreateTransaction(
    ...
    const TApplyIf& applyIf,
    NKikimrSchemeOp::EOperationType type,
    TModifySchemeFunc<NKikimrSchemeOp::TSplitMergeTablePartitions> func
) {
    NKikimrSchemeOp::TModifyScheme tx;
...
auto CreateTransaction(
    ...
    const TApplyIf& applyIf,
    NKikimrSchemeOp::EOperationType type,
    TModifySchemeFunc<NKikimrSchemeOp::TDrop> func
) {
    NKikimrSchemeOp::TModifyScheme tx;
...
auto CreateTransaction(
    ...
    const TApplyIf& applyIf,
    NKikimrSchemeOp::EOperationType type,
    TModifySchemeFunc<NKikimrSchemeOp::TBackupTask> func
) {
    NKikimrSchemeOp::TModifyScheme tx;
...
auto CreateTransaction(
    ...
    const TApplyIf& applyIf,
    NKikimrSchemeOp::EOperationType type,
    TModifySchemeFunc<NKikimrSchemeOp::TMkDir> func
) {
    NKikimrSchemeOp::TModifyScheme tx;
...
auto CreateTransaction(
    ...
    const TApplyIf& applyIf,
    NKikimrSchemeOp::EOperationType type,
    TModifySchemeFunc<NKikimrSchemeOp::TAlterUserAttributes> func
) {
    NKikimrSchemeOp::TModifyScheme tx;
...
#define DROP_BY_PATH_ID_HELPERS(name, op) \
    ...
    // subdomain
GENERIC_WITH_ATTRS_HELPERS(
    CreateSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableSubDomain
)
GENERIC_HELPERS(
    AlterSubDomain,
...
GENERIC_HELPERS(
    DropSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
GENERIC_HELPERS(
    ForceDropSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpForceDropSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_WITH_ATTRS_HELPERS(
    CreateExtSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateExtSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableSubDomain
)
GENERIC_WITH_ATTRS_HELPERS(
    AlterExtSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterExtSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableSubDomain
)
GENERIC_HELPERS(
    ForceDropExtSubDomain,
    NKikimrSchemeOp::EOperationType::ESchemeOpForceDropExtSubDomain,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_WITH_ATTRS_HELPERS(
    ...
    &NKikimrSchemeOp::TModifyScheme::MutableMkDir
)
...
GENERIC_WITH_ATTRS_HELPERS(
    UserAttrs,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterUserAttributes,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterUserAttributes
)
...
GENERIC_WITH_ATTRS_HELPERS(
    ...
    &NKikimrSchemeOp::TModifyScheme::MutableCreateTable
)
GENERIC_HELPERS(
    CreateIndexedTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateIndexedTable,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateIndexedTable
)
GENERIC_HELPERS(
    ConsistentCopyTables,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateConsistentCopyTables,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateConsistentCopyTables
)
GENERIC_HELPERS(
    AlterTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterTable,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterTable
)
GENERIC_HELPERS(
    SplitTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpSplitMergeTablePartitions,
    &NKikimrSchemeOp::TModifyScheme::MutableSplitMergeTablePartitions
)
GENERIC_HELPERS(
    DropTable,
...
GENERIC_HELPERS(
    ...
    &NKikimrSchemeOp::TModifyScheme::MutableDropIndex
)

    // backup & restore
GENERIC_HELPERS(
    Backup,
    NKikimrSchemeOp::EOperationType::ESchemeOpBackup,
    &NKikimrSchemeOp::TModifyScheme::MutableBackup
)
GENERIC_HELPERS(
    BackupToYt,
    NKikimrSchemeOp::EOperationType::ESchemeOpBackup,
...
GENERIC_HELPERS(
    Restore,
    NKikimrSchemeOp::EOperationType::ESchemeOpRestore,
    &NKikimrSchemeOp::TModifyScheme::MutableRestore
)

    // cdc stream
GENERIC_HELPERS(
    CreateCdcStream,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateCdcStream,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateCdcStream
)
GENERIC_HELPERS(
    AlterCdcStream,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterCdcStream,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterCdcStream
)
GENERIC_HELPERS(
    DropCdcStream,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropCdcStream,
    &NKikimrSchemeOp::TModifyScheme::MutableDropCdcStream
)

    // continuous backup
GENERIC_HELPERS(
    CreateContinuousBackup,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateContinuousBackup,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateContinuousBackup
)
GENERIC_HELPERS(
    AlterContinuousBackup,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterContinuousBackup,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterContinuousBackup
)
GENERIC_HELPERS(
    DropContinuousBackup,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropContinuousBackup,
    &NKikimrSchemeOp::TModifyScheme::MutableDropContinuousBackup
)

    // olap store
GENERIC_HELPERS(
    CreateOlapStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateColumnStore,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateColumnStore
)
GENERIC_HELPERS(
    AlterOlapStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterColumnStore,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterColumnStore
)
GENERIC_HELPERS(
    DropOlapStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropColumnStore,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreateColumnTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateColumnTable,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateColumnTable
)
GENERIC_HELPERS(
    AlterColumnTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterColumnTable,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterColumnTable
)
GENERIC_HELPERS(
    DropColumnTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropColumnTable,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    ...
    &NKikimrSchemeOp::TModifyScheme::MutableSequence
)
GENERIC_HELPERS(
    DropSequence,
...
GENERIC_HELPERS(
    AlterSequence,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterSequence,
    &NKikimrSchemeOp::TModifyScheme::MutableSequence
)
...
GENERIC_HELPERS(
    CreateReplication,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateReplication,
    &NKikimrSchemeOp::TModifyScheme::MutableReplication
)
GENERIC_HELPERS(
    AlterReplication,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterReplication,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterReplication
)
GENERIC_HELPERS(
    DropReplication,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropReplication,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
DROP_BY_PATH_ID_HELPERS(DropReplication, NKikimrSchemeOp::EOperationType::ESchemeOpDropReplication)
GENERIC_HELPERS(
    DropReplicationCascade,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropReplicationCascade,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreatePQGroup,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreatePersQueueGroup,
    &NKikimrSchemeOp::TModifyScheme::MutableCreatePersQueueGroup
)
GENERIC_HELPERS(
    AlterPQGroup,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterPersQueueGroup,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterPersQueueGroup
)
GENERIC_HELPERS(
    DropPQGroup,
...
GENERIC_HELPERS(
    CreateRtmrVolume,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateRtmrVolume,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateRtmrVolume
)

    // solomon
GENERIC_HELPERS(
    CreateSolomon,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateSolomonVolume,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateSolomonVolume
)
GENERIC_HELPERS(
    AlterSolomon,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterSolomonVolume,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterSolomonVolume
)
GENERIC_HELPERS(
    DropSolomon,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropSolomonVolume,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreateKesus,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateKesus,
    &NKikimrSchemeOp::TModifyScheme::MutableKesus
)
GENERIC_HELPERS(
    AlterKesus,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterKesus,
    &NKikimrSchemeOp::TModifyScheme::MutableKesus
)
GENERIC_HELPERS(
    DropKesus,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropKesus,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreateFileStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateFileStore,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateFileStore
)
GENERIC_HELPERS(
    AlterFileStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterFileStore,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterFileStore
)
GENERIC_HELPERS(
    DropFileStore,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropFileStore,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    ...
    &NKikimrSchemeOp::TModifyScheme::MutableCreateBlockStoreVolume
)
GENERIC_HELPERS(
    AlterBlockStoreVolume,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterBlockStoreVolume,
    &NKikimrSchemeOp::TModifyScheme::MutableAlterBlockStoreVolume
)

    // external table
GENERIC_HELPERS(
    CreateExternalTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateExternalTable,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateExternalTable
)
GENERIC_HELPERS(
    DropExternalTable,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropExternalTable,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    ...
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateExternalDataSource,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateExternalDataSource
)
GENERIC_HELPERS(
    DropExternalDataSource,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropExternalDataSource,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreateView,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateView,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateView
)
GENERIC_HELPERS(
    DropView,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropView,
    &NKikimrSchemeOp::TModifyScheme::MutableDrop
)
...
GENERIC_HELPERS(
    CreateResourcePool,
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateResourcePool,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateResourcePool
)
GENERIC_HELPERS(
    AlterResourcePool,
    NKikimrSchemeOp::EOperationType::ESchemeOpAlterResourcePool,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateResourcePool
)
GENERIC_HELPERS(
    DropResourcePool,
...
GENERIC_HELPERS(
    ...
    NKikimrSchemeOp::EOperationType::ESchemeOpCreateBackupCollection,
    &NKikimrSchemeOp::TModifyScheme::MutableCreateBackupCollection
)
GENERIC_HELPERS(
    DropBackupCollection,
    NKikimrSchemeOp::EOperationType::ESchemeOpDropBackupCollection,
    &NKikimrSchemeOp::TModifyScheme::MutableDropBackupCollection
)
...
void AsyncDropBlockStoreVolume(
    TTestActorRuntime& runtime,
    ui64 txId,
    const TString& parentPath,
    const TString& name,
...
void AsyncAssignBlockStoreVolume(
    TTestActorRuntime& runtime,
    ui64 txId,
    const TString& parentPath,
    const TString& name,
...
void TestAssignBlockStoreVolume(
    TTestActorRuntime& runtime,
    ui64 txId,
    const TString& parentPath,
    const TString& name,
...
void TestCancelTxTable(
    ...
    do {
        ...
        UNIT_ASSERT(event);
        Cerr << "TEvCancelTxResult for TargetTxId: " << event->Record.GetTargetTxId()
             << ", wait until TargetTxId: " << targetTxId << "\n";
...
NKikimrExport::TEvGetExportResponse TestGetExport(
    TTestActorRuntime& runtime,
    ui64 schemeshardId,
    ui64 id,
    const TString& dbName,
    ...
    ForwardToTablet(
        runtime, schemeshardId, runtime.AllocateEdgeActor(), new TEvExport::TEvGetExportRequest(dbName, id)
    );
...
TestGetExport(TTestActorRuntime& runtime, ui64 id, const TString& dbName, Ydb::StatusIds::StatusCode expectedStatus) {
...
NKikimrExport::TEvCancelExportResponse TestCancelExport(
    TTestActorRuntime& runtime,
    ui64 schemeshardId,
    ui64 txId,
    const TString& dbName,
...
NKikimrImport::TEvGetImportResponse TestGetImport(
    TTestActorRuntime& runtime,
    ui64 schemeshardId,
    ui64 id,
    const TString& dbName,
    ...
    ForwardToTablet(
        runtime, schemeshardId, runtime.AllocateEdgeActor(), new TEvImport::TEvGetImportRequest(dbName, id)
    );
    ...
    if (!found) {
        UNIT_ASSERT_C(
            found,
            "Unexpected status: " << Ydb::StatusIds::StatusCode_Name(result)
                                  << " issues: " << ev->Record.GetResponse().GetEntry().GetIssues()
        );
    }
...
TestGetImport(TTestActorRuntime& runtime, ui64 id, const TString& dbName, Ydb::StatusIds::StatusCode expectedStatus) {
...
NKikimrImport::TEvCancelImportResponse TestCancelImport(
    TTestActorRuntime& runtime,
    ui64 schemeshardId,
    ui64 txId,
    const TString& dbName,
...
NKikimrProto::EReplyStatus LocalMiniKQL(
    TTestActorRuntime& runtime,
    ui64 tabletId,
    const TString& query,
...
bool GetByKeyFilterEnabled(TTestActorRuntime& runtime, ui64 tabletId, ui32 table) {
    ...
    for (ui32 i = 0; i < scheme.DeltaSize(); ++i) {
        const auto& d = scheme.GetDelta(i);
        if (d.GetDeltaType() == NTabletFlatScheme::TAlterRecord::SetTable && d.GetTableId() == table &&
            d.HasByKeyFilter()) {
...
NKikimr::NLocalDb::TCompactionPolicyPtr
GetCompactionPolicy(TTestActorRuntime& runtime, ui64 tabletId, ui32 localTableId) {
    NTabletFlatScheme::TSchemeChanges scheme;
    TString err;
    NKikimrProto::EReplyStatus status = LocalSchemeTx(runtime, tabletId, "", true, scheme, err);
    UNIT_ASSERT_VALUES_EQUAL(status, NKikimrProto::EReplyStatus::OK);
    ...
    for (ui32 i = 0; i < scheme.DeltaSize(); ++i) {
        const auto& d = scheme.GetDelta(i);
        if (d.GetDeltaType() == NTabletFlatScheme::TAlterRecord::SetCompactionPolicy &&
            d.GetTableId() == localTableId) {
...
void SetSchemeshardSchemaLimits(TTestActorRuntime& runtime, TSchemeLimits limits, ui64 schemeShard) {
    ...
    TString prog = Sprintf(
        ...
        limits.MaxDepth,
        limits.MaxPaths,
        limits.MaxChildrenInDir,
        limits.MaxAclBytesSize,
        limits.MaxTableColumns,
        limits.MaxColumnTableColumns,
        limits.MaxTableColumnNameLength,
        limits.MaxTableKeyColumns,
        limits.MaxTableIndices,
        limits.MaxTableCdcStreams,
        limits.MaxShards,
        limits.MaxShardsInPath,
        limits.MaxConsistentCopyTargets,
        limits.MaxPathElementLength,
        escapedStr.c_str(),
        limits.MaxPQPartitions,
        limits.MaxExports,
        limits.MaxImports
    );
    Cdbg << prog << "\n";
...
void SetSchemeshardDatabaseQuotas(
    TTestActorRuntime& runtime,
...
TEvSchemeShard::TEvModifySchemeTransaction*
UpgradeSubDomainRequest(ui64 txId, const TString& parentPath, const TString& name) {
    auto evTx = new TEvSchemeShard::TEvModifySchemeTransaction(txId, TTestTxConfig::SchemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
TEvSchemeShard::TEvModifySchemeTransaction* UpgradeSubDomainDecisionRequest(
    ui64 txId,
    const TString& parentPath,
    const TString& name,
    NKikimrSchemeOp::TUpgradeSubDomain::EDecision decision
) {
    auto evTx = new TEvSchemeShard::TEvModifySchemeTransaction(txId, TTestTxConfig::SchemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
void AsyncUpgradeSubDomainDecision(
    TTestActorRuntime& runtime,
    ui64 txId,
    const TString& parentPath,
    const TString& name,
...
void TestUpgradeSubDomainDecision(
    TTestActorRuntime& runtime,
    ui64 txId,
    const TString& parentPath,
    const TString& name,
...
TRowVersion CreateVolatileSnapshot(TTestActorRuntime& runtime, const TVector<TString>& tables, TDuration timeout) {
    TActorId sender = runtime.AllocateEdgeActor();
...
TEvIndexBuilder::TEvCreateRequest*
CreateBuildIndexRequest(ui64 id, const TString& dbName, const TString& src, const TBuildIndexConfig& cfg) {
...
std::unique_ptr<TEvIndexBuilder::TEvCreateRequest> CreateBuildColumnRequest(
    ui64 id,
    const TString& dbName,
    const TString& src,
    const TString& columnName,
    const Ydb::TypedValue& literal
) {
    NKikimrIndexBuilder::TIndexBuildSettings settings;
    settings.set_source_path(src);
    settings.set_max_batch_rows(2);
...
void AsyncBuildIndex(
    TTestActorRuntime& runtime,
    ui64 id,
    ui64 schemeShard,
    const TString& dbName,
    const TString& src,
    const TBuildIndexConfig& cfg
) {
    auto sender = runtime.AllocateEdgeActor();
...
void AsyncBuildIndex(
    ...
    AsyncBuildIndex(
        ...
        dbName,
        src,
        TBuildIndexConfig{name, NKikimrSchemeOp::EIndexTypeGlobal, columns, dataColumns}
    );
}

void TestBuildColumn(
    TTestActorRuntime& runtime,
    ui64 id,
    ui64 schemeShard,
    const TString& dbName,
    ...
    UNIT_ASSERT_EQUAL_C(
        ...
        expectedStatus,
        "status mismatch" << " got " << Ydb::StatusIds::StatusCode_Name(event->Record.GetStatus()) << " expected "
                          << Ydb::StatusIds::StatusCode_Name(expectedStatus) << " issues was "
...
void TestBuildIndex(
    ...
    TestBuildIndex(
        ...
        dbName,
        src,
        TBuildIndexConfig{name, NKikimrSchemeOp::EIndexTypeGlobal, columns, {}},
        expectedStatus
    );
}

void TestBuildVectorIndex(
    TTestActorRuntime& runtime,
    ui64 id,
    ui64 schemeShard,
    const TString& dbName,
    ...
    TestBuildIndex(
        ...
        dbName,
        src,
        TBuildIndexConfig{name, NKikimrSchemeOp::EIndexTypeGlobalVectorKmeansTree, {column}, {}},
        expectedStatus
    );
}

TEvIndexBuilder::TEvCancelRequest*
CreateCancelBuildIndexRequest(const ui64 id, const TString& dbName, const ui64 buildIndexId) {
    return new TEvIndexBuilder::TEvCancelRequest(id, dbName, buildIndexId);
}

void CheckExpectedStatusCode(
    const TVector<Ydb::StatusIds::StatusCode>& expected,
    Ydb::StatusIds_StatusCode result,
    ...
    UNIT_ASSERT_C(
        isExpectedStatus, "Unexpected status code: " << Ydb::StatusIds::StatusCode_Name(result) << ": " << reason
    );
}

NKikimrIndexBuilder::TEvCancelResponse TestCancelBuildIndex(
    TTestActorRuntime& runtime,
    const ui64 id,
...
NKikimrIndexBuilder::TEvListResponse
TestListBuildIndex(TTestActorRuntime& runtime, ui64 schemeShard, const TString& dbName) {
    auto sender = runtime.AllocateEdgeActor();
...
NKikimrIndexBuilder::TEvGetResponse
TestGetBuildIndex(TTestActorRuntime& runtime, ui64 schemeShard, const TString& dbName, ui64 id) {
    auto sender = runtime.AllocateEdgeActor();
...
TEvIndexBuilder::TEvForgetRequest*
ForgetBuildIndexRequest(const ui64 id, const TString& dbName, const ui64 buildIndexId) {
...
TPathId TestFindTabletSubDomainPathId(
    ...
    UNIT_ASSERT_EQUAL_C(
        record.GetStatus(),
        expected,
        "Unexpected status " << NKikimrScheme::TEvFindTabletSubDomainPathIdResult::EStatus_Name(record.GetStatus())
                             << " (expected status "
...
TEvSchemeShard::TEvModifySchemeTransaction*
CreateAlterLoginCreateUser(ui64 txId, const TString& user, const TString& password) {
    auto evTx = new TEvSchemeShard::TEvModifySchemeTransaction(txId, TTestTxConfig::SchemeShard);
    auto transaction = evTx->Record.AddTransaction();
...
NMiniKQL::IEngineFlat::EStatus TFakeDataReq::Propose(bool immediate, bool& activeZone, ui32 txFlags) {
    ...
    NMiniKQL::TEngineFlatSettings settings(
        NMiniKQL::IEngineFlat::EProtocol::V1,
    ...
    for (ui32 i = 0; i < shardsCount; ++i) {
        ...
        for (;;) {
            ...
            TAutoPtr<IEventHandle> handle;
            auto event = Runtime.GrabEdgeEventIf<TEvDataShard::TEvProposeTransactionResult>(
                handle,
            ...
            if (event->GetStatus() == NKikimrTxDataShard::TEvProposeTransactionResult::ERROR ||
                ...
                for (auto err : event->Record.GetError()) {
                    Cerr << "DataShard error: " << shardData.ShardId
...
NMiniKQL::TRuntimeNode TFakeDataReq::ProgramText2Bin(const TString& query) {
    ...
    auto resFuture = NYql::ConvertToMiniKQL(expr, Runtime.GetAppData().FunctionRegistry, &Env, &dbSchemeResolver);
    ...
    auto fnFIllInfo = [this, &dbSchemeResolver](const NKikimrScheme::TEvDescribeSchemeResult& record) {
        ...
        for (size_t i = 0; i < tdesc.ColumnsSize(); i++) {
            ...
            table.Columns.insert(std::make_pair(
                c.GetName(),
                TColumn{c.GetId(), keyIdx, NScheme::TTypeInfo(c.GetTypeId()), 0, EColumnTypeConstraint::Nullable}
            ));
        }
...
std::shared_ptr<const TVector<TKeyDesc::TPartitionInfo>> TFakeDataReq::TTablePartitioningInfo::ResolveKey(
    ...
    TVector<TBorder>::const_iterator low =
        LowerBound(Partitioning.begin(), Partitioning.end(), true, [&](const TBorder& left, bool) {
            const int compares = CompareBorders<true, false>(
                left.KeyTuple.GetCells(),
                range.From,
                left.Inclusive || left.Point,
    ...
    do {
        ...
        int prevComp = CompareBorders<true, true>(
            low->KeyTuple.GetCells(), range.To, low->Point || low->Inclusive, range.InclusiveTo, KeyColumnTypes
        );
        if (prevComp >= 0)
            return partitions;
...
TEvSchemeShard::TEvModifySchemeTransaction* CombineSchemeTransactions(
    const TVector<TEvSchemeShard::TEvModifySchemeTransaction*>& transactions
) {
    ui64 txId = 0;
    ui64 tabletId = 0;
    if (transactions) {
    ...
    TEvSchemeShard::TEvModifySchemeTransaction* combination =
        new TEvSchemeShard::TEvModifySchemeTransaction(txId, tabletId);
...
void AsyncSend(TTestActorRuntime& runtime, ui64 targetTabletId, IEventBase* ev, ui32 nodeIndex, TActorId sender) {
    if (sender == TActorId()) {
...
TTestActorRuntimeBase::TEventObserver
SetSuppressObserver(TTestActorRuntime& runtime, TVector<THolder<IEventHandle>>& suppressed, ui32 type) {
...
void SendTEvPeriodicTopicStats(
    TTestActorRuntime& runtime,
    ui64 topicId,
    ui64 generation,
    ui64 round,
...
void WriteToTopic(TTestActorRuntime& runtime, const TString& path, ui32& msgSeqNo, const TString& message) {
    ...
    NKikimr::NPQ::CmdWrite(
        &runtime, tabletId, edge, partitionId, "sourceid0", msgSeqNo, data, false, {}, true, cookie, 0
    );
}
...
void WriteRow(
    ...
    TTableId tableId(
        pathDesc.GetSelf().GetSchemeshardId(),
        pathDesc.GetSelf().GetPathId(),
        pathDesc.GetTable().GetTableSchemaVersion()
    );
    ...
    auto evWrite =
        std::make_unique<NKikimr::NEvents::TDataEvents::TEvWrite>(txId, NKikimrDataEvents::TEvWrite::MODE_IMMEDIATE);
    ui64 payloadIndex =
        NKikimr::NEvWrite::TPayloadWriter<NKikimr::NEvents::TDataEvents::TEvWrite>(*evWrite).AddDataToPayload(
            std::move(matrix.ReleaseBuffer())
        );
    ...
    UNIT_ASSERT_C(
        successIsExpected == (status == NKikimrDataEvents::TEvWriteResult::STATUS_COMPLETED),
...
i64 WaitNextValResult(TTestActorRuntime& runtime, const TActorId& sender, Ydb::StatusIds::StatusCode expectedStatus) {
