...
template <typename TestFunc, typename CurrentTest>
struct TTestFlagsN {
    static constexpr size_t N = TestFunc::N_FLAGS;
    ...
    TTestFlagsN() {
        ...
        auto basename = GetFunctionName<TestFunc::Ptr>();
...
// Helper to count function arguments at compile time
...
template<typename... Args>
struct TCountArgs<void(*)(NUnitTest::TTestContext&, Args...)> {
    static constexpr size_t value = sizeof...(Args);
};
...
#define Y_UNIT_TEST_FLAGS_N(N, ...)                                            \
    ...
    struct test##N##_wrapper {                                                 \
        static constexpr auto Ptr = &N;                                        \
        static constexpr size_t N_FLAGS = TCountArgs<decltype(&N)>::value;     \
    };                                                                         \
    static TTestFlagsN<test##N##_wrapper, TCurrentTest> test##N##_instance;    \
    void N(NUnitTest::TTestContext&, __VA_ARGS__)
