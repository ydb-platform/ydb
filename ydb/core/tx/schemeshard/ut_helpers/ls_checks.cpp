...
TCheckFunc HasColumnTableTtlSettingsTier(const TString& columnName, const TDuration& evictAfter, const TString& storageName) {
    return [=] (const NKikimrScheme::TEvDescribeSchemeResult& record) {
        const auto& table = record.GetPathDescription().GetColumnTableDescription();
        UNIT_ASSERT(table.HasTtlSettings());
        const auto& ttl = table.GetTtlSettings();
        ...
        UNIT_ASSERT_VALUES_EQUAL(tier.GetEvictAfterSeconds(), evictAfter.Seconds());
        UNIT_ASSERT_VALUES_EQUAL(tier.GetStorageName(), storageName);
    };
}
