...
TCheckFunc FollowerGroups(
    const TVector<NKikimrHive::TFollowerGroup>& followerGroup = TVector<NKikimrHive::TFollowerGroup>{}
);
...
inline void HasNotOffloadConfig(const NKikimrScheme::TEvDescribeSchemeResult& record) {
    return HasOffloadConfigBase(record, nullptr, {.Value = true});
}
