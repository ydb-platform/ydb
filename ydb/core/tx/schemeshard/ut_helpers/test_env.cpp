...
private:
    void Handle(TEvFileStore::TEvUpdateConfig::TPtr& ev, const TActorContext& ctx) {
        const auto& request = ev->Get()->Record;
        TAutoPtr<TEvFileStore::TEvUpdateConfigResponse> response = new TEvFileStore::TEvUpdateConfigResponse();
        response->Record.SetTxId(request.GetTxId());
        response->Record.SetOrigin(TabletID());
...
private:
    ...
    void SendToSchemeshard(ui64 txId, const TActorContext& ctx) {
        ...
        auto pipe =
            ctx.Register(NTabletPipe::CreateClient(ctx.SelfID, SchemeshardTabletId, GetPipeConfigWithRetries()));
...
private:
    ...
    void
    HandleWriteMeteringJson(const NMetering::TEvMetering::TEvWriteMeteringJson::TPtr& ev, const TActorContext& ctx) {
        Y_UNUSED(ctx);
    ...
    void HandleUnexpectedEvent(STFUNC_SIG) {
        ALOG_DEBUG(
            ...
            "TFakeMetering:" << " unhandled event type: " << ev->GetTypeRewrite() << " event: " << ev->ToString()
        );
    }
...
NSchemeShardUT_Private::TTestEnv::TTestEnv(
    ...
    TAppPrepare app(
        dsExportFactory ? dsExportFactory
                        : static_cast<std::shared_ptr<NKikimr::NDataShard::IExportFactory>>(
                              std::make_shared<TDataShardExportFactory>()
                          )
    );
    ...
    if (opts.RunFakeConfigDispatcher_) {
        for (ui32 node = 0; node < runtime.GetNodeCount(); ++node) {
            runtime.RegisterService(
                NConsole::MakeConfigsDispatcherID(runtime.GetNodeId(node)),
                runtime.Register(new TFakeConfigDispatcher(), node),
                node
            );
        }
    }
...
NSchemeShardUT_Private::TTestEnv::TTestEnv(
    ...
    : TTestEnv(
          ...
          TTestEnvOptions()
              ...
              .EnableSystemViews(enableSystemViews),
          ssFactory
      ) {}
...
void NSchemeShardUT_Private::TTestEnv::AddDomain(
    TTestActorRuntime& runtime,
    TAppPrepare& app,
    ui32 domainUid,
    ui64 hive,
...
TAutoPtr<ITabletScheduledEventsGuard> NSchemeShardUT_Private::TTestEnv::EnableSchemeshardPipeRetries(
    TTestActorRuntime& runtime
) {
    TActorId sender = runtime.AllocateEdgeActor();
...
NActors::TActorId
NSchemeShardUT_Private::CreateNotificationSubscriber(NActors::TTestActorRuntime& runtime, ui64 schemeshardId) {
...
void NSchemeShardUT_Private::TestWaitNotification(
...
void NSchemeShardUT_Private::TTestEnv::TestWaitNotification(
    NActors::TTestActorRuntime& runtime,
    TSet<ui64> txIds,
    ui64 schemeshardId
) {
...
void NSchemeShardUT_Private::TTestEnv::TestWaitTabletDeletion(
    NActors::TTestActorRuntime& runtime,
    TSet<ui64> tabletIds,
    ui64 hive
) {
...
void NSchemeShardUT_Private::TTestEnv::TestWaitShardDeletion(
    NActors::TTestActorRuntime& runtime,
    ui64 schemeShard,
...
std::function<NActors::IActor*(const NActors::TActorId&, NKikimr::TTabletStorageInfo*)>
NSchemeShardUT_Private::TTestEnv::GetTabletCreationFunc(ui32 type) {
    switch (type) {
...
void NSchemeShardUT_Private::TTestEnv::TestServerlessComputeResourcesModeInHive(
    TTestActorRuntime& runtime,
    const TString& path,
...
TEvSchemeShard::TEvInitRootShardResult::EStatus NSchemeShardUT_Private::TTestEnv::InitRoot(
    NActors::TTestActorRuntime& runtime,
    ui64 schemeRoot,
...
void NSchemeShardUT_Private::TTestEnv::InitRootStoragePools(
    NActors::TTestActorRuntime& runtime,
    ui64 schemeRoot,
    const NActors::TActorId& sender,
    ui64 domainUid
...
NSchemeShardUT_Private::TTestWithReboots::TTestWithReboots(
    bool killOnCommit,
    NSchemeShardUT_Private::TTestEnv::TSchemeShardFactory ssFactory
)
...
void NSchemeShardUT_Private::TTestWithReboots::Run(
    std::function<void(TTestActorRuntime&, bool&)> testScenario,
    bool allowLogBatching
) {
...
void NSchemeShardUT_Private::TTestWithReboots::RunWithTabletReboots(
    std::function<void(TTestActorRuntime&, bool&)> testScenario
) {
    RunTestWithReboots(
        TabletIds,
...
void NSchemeShardUT_Private::TTestWithReboots::RunWithPipeResets(
    std::function<void(TTestActorRuntime&, bool&)> testScenario
) {
...
void NSchemeShardUT_Private::TTestWithReboots::Prepare(
...
bool NSchemeShardUT_Private::TTestWithReboots::PassUserRequests(
    TTestActorRuntimeBase& runtime,
    TAutoPtr<IEventHandle>& event
) {
    Y_UNUSED(runtime);
    return event->Type == TEvSchemeShard::EvModifySchemeTransaction ||
           event->Type == TEvSchemeShard::EvDescribeScheme || event->Type == TEvSchemeShard::EvNotifyTxCompletion ||
           event->Type == TEvSchemeShard::EvMeasureSelfResponseTime ||
           event->Type == TEvSchemeShard::EvWakeupToMeasureSelfResponseTime || event->Type == TEvTablet::EvLocalMKQL ||
           event->Type == TEvFakeHive::EvSubscribeToTabletDeletion || event->Type == TEvSchemeShard::EvCancelTx ||
           event->Type == TEvExport::EvCreateExportRequest || event->Type == TEvIndexBuilder::EvCreateRequest ||
           event->Type == TEvIndexBuilder::EvGetRequest || event->Type == TEvIndexBuilder::EvCancelRequest ||
