...
Y_UNIT_TEST_SUITE(IndexBuildTest) {
    ...
    Y_UNIT_TEST(BaseCase) {
        ...
        const TString meteringData =
            R"({"usage":{"start":0,"quantity":179,"finish":0,"unit":"request_unit","type":"delta"},"tags":{},"id":"106-72075186233409549-2-101-1818-101-1818","cloud_id":"CLOUD_ID_VAL","source_wt":0,"source_id":"sless-docapi-ydb-ss","resource_id":"DATABASE_ID_VAL","schema":"ydb.serverless.requests.v1","folder_id":"FOLDER_ID_VAL","version":"1.0.0"})";
        ...
        TestBuildIndex(
            runtime, ++txId, tenantSchemeShard, "/MyRoot/CommonDB", "/MyRoot/CommonDB/Table", "index1", {"index"}
        );
        buildIndexId = txId;
    ...
    Y_UNIT_TEST(CancellationNotEnoughRetries) {
        ...
        auto upgradeEvent = [&](TAutoPtr<IEventHandle>& ev) -> auto {
            if (ev->Type == TEvSchemeShard::EvModifySchemeTransaction) {
                ...
                if (msg->Record.GetTransaction(0).GetOperationType() ==
                    NKikimrSchemeOp::EOperationType::ESchemeOpCreateIndexBuild) {
                    ...
                    NKikimrSchemeOp::TTableDescription& indexTableDescr =
                        indexConfig.MutableIndexImplTableDescriptions()->at(0);
...

        {
            ...
            *index.mutable_global_index() = Ydb::Table::GlobalIndex();
            ...
            TEvIndexBuilder::TEvCreateResponse* event =
                runtime.GrabEdgeEvent<TEvIndexBuilder::TEvCreateResponse>(handle);
            UNIT_ASSERT(event);
            ...
            UNIT_ASSERT_EQUAL_C(
                ...
                "status mismatch" << " got " << Ydb::StatusIds::StatusCode_Name(event->Record.GetStatus())
        ...
        TestDescribeResult(DescribePath(runtime, "/MyRoot/Table/index1", true, true, true), {NLs::PathNotExist});
    ...
    Y_UNIT_TEST(CancellationNoTable) {
        ...
        NKikimrIndexBuilder::TEvListResponse listing =
            TestListBuildIndex(runtime, TTestTxConfig::SchemeShard, "/MyRoot");
        UNIT_ASSERT_VALUES_EQUAL(listing.EntriesSize(), 0);
    }
    ...
    Y_UNIT_TEST(WithFollowers) {
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/WithFollowers"),
            {NLs::PathExist, NLs::IndexesCount(1), NLs::PathVersionEqual(6)}
        );

        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/WithFollowers/UserDefinedIndexByValue0", true, true, true),
            {NLs::PathExist, NLs::IndexState(NKikimrSchemeOp::EIndexState::EIndexStateReady)}
        );
    ...
    Y_UNIT_TEST(Lock) {
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/Table/nameOK", true, true, true),
            {NLs::PathExist, NLs::IndexState(NKikimrSchemeOp::EIndexState::EIndexStateReady)}
        );

        NKikimrIndexBuilder::TEvGetResponse descr =
            TestGetBuildIndex(runtime, TTestTxConfig::SchemeShard, "/MyRoot", buildIndexId);
        UNIT_ASSERT_VALUES_EQUAL(descr.GetIndexBuild().GetState(), Ydb::Table::IndexBuildState::STATE_DONE);
...
//        KIKIMR-9945
    ...
    Y_UNIT_TEST(MergeIndexTableShardsOnlyWhenReady) {
        ...
        TWaitForFirstEvent<TEvSchemeShard::TEvModifySchemeTransaction> indexInitializationWaiter(
            runtime,
        ...
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/ByValue"),
            {NLs::PathExist, NLs::IndexState(NKikimrSchemeOp::EIndexStateWriteOnly)}
        );
        ...
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/ByValue/indexImplTable", true),
            {NLs::PathExist, NLs::PartitionCount(4), shardCollector}
        );
        ...
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/ByValue"), {NLs::IndexState(NKikimrSchemeOp::EIndexStateReady)}
        );
    ...
    Y_UNIT_TEST(DropIndex) {
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/Table"), {NLs::Finished, NLs::PathVersionEqual(3), NLs::IndexesCount(2)}
        );
        TestDescribeResult(
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/Table"), {NLs::Finished, NLs::PathVersionEqual(5), NLs::IndexesCount(1)}
        );
        TestDescribeResult(DescribePrivatePath(runtime, "/MyRoot/Table/UserDefinedIndexByValue0"), {NLs::PathNotExist});
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/UserDefinedIndexByValue0/indexImplTable"), {NLs::PathNotExist}
        );
        ...
        TestDescribeResult(DescribePrivatePath(runtime, "/MyRoot/Table/UserDefinedIndexByValue1"), {NLs::PathExist});
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/UserDefinedIndexByValue1/indexImplTable"), {NLs::PathExist}
        );
    ...
    Y_UNIT_TEST(RejectsDropIndex) {
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/Table"), {NLs::Finished, NLs::PathVersionEqual(3), NLs::IndexesCount(1)}
        );
        TestDescribeResult(
            DescribePrivatePath(runtime, "/MyRoot/Table/UserDefinedIndexByValue0"),
            {NLs::Finished,
    ...
    Y_UNIT_TEST(RejectsCancel) {
        ...
        //
        TestCancelBuildIndex(
            runtime,
            ++txId,
            TTestTxConfig::SchemeShard,
            "/MyRoot",
            buildIndexId + 1,
            TVector<Ydb::StatusIds::StatusCode>{Ydb::StatusIds::NOT_FOUND}
        );
        TestCancelBuildIndex(
            runtime,
            ++txId,
        ...
        TestCancelBuildIndex(
            ...
            "/MyRoot",
            buildIndexId,
            TVector<Ydb::StatusIds::StatusCode>{Ydb::StatusIds::PRECONDITION_FAILED}
        );
        ...
        TestDescribeResult(DescribePath(runtime, "/MyRoot/Table/index1", true, true, true), {NLs::PathExist});
    }
}
