...
Y_UNIT_TEST_SUITE(TSchemeShardLoginTest) {
    ...
    Y_UNIT_TEST_FLAG(AddAccess_NonExisting, StrictAclCheck) {
        ...
        if (StrictAclCheck) {
            AsyncModifyACL(runtime, ++txId, "/MyRoot", "Dir1", diffACL.SerializeAsString(), "");
            TestModificationResults(runtime, txId, {{NKikimrScheme::StatusPreconditionFailed, "SID user1 not found in database with path `/MyRoot`"}});

            AsyncModifyACL(runtime, ++txId, "/MyRoot", "Dir1", NACLib::TDiffACL{}.SerializeAsString(), "user1");
            TestModificationResults(runtime, txId, {{NKikimrScheme::StatusPreconditionFailed, "Owner SID user1 not found in database with path `/MyRoot`"}});
        }
