        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                UNIT_ASSERT_VALUES_EQUAL(
                    resourcePoolDescription.GetProperties().DebugString(), properties.DebugString()
                );
            }
        });
    }
    ...
    Y_UNIT_TEST(ParallelCreateDrop) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                TInactiveZone inactive(activeZone);
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/.metadata/workload_manager/pools/DropMe"), {NLs::PathNotExist}
                );
            }
        });
    }
    ...
    Y_UNIT_TEST(SimpleDropResourcePoolWithReboots) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                TInactiveZone inactive(activeZone);
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/.metadata/workload_manager/pools/ResourcePool"), {NLs::PathNotExist}
                );
            }
        });
    }
