...
Y_UNIT_TEST_SUITE(TSchemeShardServerLess) {
    ...
    Y_UNIT_TEST(BaseCase) {
        ...
        TestCreateExtSubDomain(runtime, ++txId, "/MyRoot", "Name: \"SharedDB\"");
        ...
        TestDescribeResult(
            ...
            {NLs::PathExist, NLs::IsExternalSubDomain("SharedDB"), NLs::ExtractDomainHive(&sharedHive)}
        );
        UNIT_ASSERT(sharedHive != 0 && sharedHive != (ui64)-1 && sharedHive != TTestTxConfig::Hive);

        TString createData = TStringBuilder() << "ResourcesDomainKey { SchemeShard: " << TTestTxConfig::SchemeShard
                                              << " PathId: " << 2 << " } "
                                              << "Name: \"ServerLess0\"";
        ...
        TestDescribeResult(
            DescribePath(runtime, tenantSchemeShard, "/MyRoot/ServerLess0/dir/table0"), {NLs::PathExist, NLs::Finished}
        );

        TestForceDropExtSubDomain(runtime, ++txId, "/MyRoot", "ServerLess0");
        env.TestWaitNotification(runtime, txId);

        TestDescribeResult(DescribePath(runtime, "/MyRoot/ServerLess0/dir/table0"), {NLs::PathNotExist});

        TestDescribeResult(DescribePath(runtime, "/MyRoot/ServerLess0"), {NLs::PathNotExist});
    ...
    Y_UNIT_TEST(StorageBilling) {
        ...
        TestDescribeResult(
            DescribePath(runtime, tenantSchemeShard, "/MyRoot/ServerLessDB"),
            {NLs::UserAttrsHas(
                {{"cloud_id", "CLOUD_ID_VAL"}, {"folder_id", "FOLDER_ID_VAL"}, {"database_id", "DATABASE_ID_VAL"}}
            )}
        );
        ...
        auto waitMeteringMessage = [&]() {
            ...
            options.FinalEvents.push_back(
                TDispatchOptions::TFinalEventCondition(NMetering::TEvMetering::TEvWriteMeteringJson::EventType)
            );
