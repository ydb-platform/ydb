...
Y_UNIT_TEST_SUITE(TSchemeShardServerLessReboots) {
    ...
    void TestTenantSchemeShardSync(
        const TTestWithReboots& t,
    ...
    Y_UNIT_TEST(TestServerlessComputeResourcesModeWithReboots) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            ...
            TestCreateExtSubDomain(runtime, ++t.TxId, "/MyRoot", R"(Name: "SharedDB")");
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/SharedDB"),
                    {NLs::PathExist,
                ...
                UNIT_ASSERT(sharedHive != 0 && sharedHive != (ui64)-1 && sharedHive != TTestTxConfig::Hive);

                UNIT_ASSERT(
                    sharedDbSchemeShard != 0 && sharedDbSchemeShard != (ui64)-1 &&
                    sharedDbSchemeShard != TTestTxConfig::SchemeShard
                );

                TestDescribeResult(
                    DescribePath(runtime, sharedDbSchemeShard, "/MyRoot/SharedDB"),
                    {NLs::PathExist, NLs::ServerlessComputeResourcesMode(EServerlessComputeResourcesModeUnspecified)}
                );
            }
...

            {
                TInactiveZone inactive(activeZone);
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/ServerLess0"),
                    {NLs::PathExist,
                ...
                UNIT_ASSERT(
                    tenantSchemeShard != 0 && tenantSchemeShard != (ui64)-1 &&
                    tenantSchemeShard != TTestTxConfig::SchemeShard
                );

                TestDescribeResult(
                    DescribePath(runtime, tenantSchemeShard, "/MyRoot/ServerLess0"),
                    {NLs::PathExist, NLs::ServerlessComputeResourcesMode(EServerlessComputeResourcesModeShared)}
                );
            }
...

            {
                ...
                t.TestEnv->TestServerlessComputeResourcesModeInHive(
                    runtime, "/MyRoot/ServerLess0", EServerlessComputeResourcesModeExclusive, sharedHive
                );
                TestTenantSchemeShardSync(
                    t,
                    runtime,
                    tenantSchemeShard,
                    "/MyRoot/ServerLess0",
...

            {
                ...
                t.TestEnv->TestServerlessComputeResourcesModeInHive(
                    runtime, "/MyRoot/ServerLess0", EServerlessComputeResourcesModeShared, sharedHive
                );
                TestTenantSchemeShardSync(
                    t,
                    runtime,
