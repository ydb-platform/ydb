...
void WaitStat(TTestActorRuntime& runtime, TTestEnv& env, ui64 rowsExpected, ui64& storageStat) {
    while (true) {
...
void SetStatsObserver(
    TTestActorRuntime& runtime,
    const std::function<TTestActorRuntime::EEventAction()>& statsObserver
) {
...
TVector<ui64> GetTableShards(TTestActorRuntime& runtime, const TString& path) {
    TVector<ui64> shards;
...
Y_UNIT_TEST_SUITE(TSchemeshardStatsBatchingTest){Y_UNIT_TEST(ShouldNotBatchWhenDisabled){TTestBasicRuntime runtime;
...
Y_UNIT_TEST(TopicPeriodicStatMeteringModeReserved) {
    ...
    const auto Assert = [&](ui64 expectedAccountSize, ui64 expectedUsedReserveSize) {
        TestDescribeResult(
            DescribePath(runtime, topicPath),
            {NLs::Finished,
             NLs::TopicAccountSize(expectedAccountSize),
             NLs::TopicUsedReserveSize(expectedUsedReserveSize)}
        );
    ...
    Assert(
        3 * 2678400 * 17, 16975298
    ); // 16975298 - it is unstable value. it can change if internal message store change
}
...
Y_UNIT_TEST(TopicPeriodicStatMeteringModeRequest) {
    ...
    ui64 balancerId =
        DescribePath(runtime, "/MyRoot/Topic1").GetPathDescription().GetPersQueueGroup().GetBalancerTabletID();
    ...
    UNIT_ASSERT_EQUAL_C(
        16975298, stats->Record.GetDataSize(), "DataSize from ReadBalancer " << stats->Record.GetDataSize()
    );
    UNIT_ASSERT_EQUAL_C(
        0,
        stats->Record.GetUsedReserveSize(),
        "UsedReserveSize from ReadBalancer " << stats->Record.GetUsedReserveSize()
    );
...
Y_UNIT_TEST_SUITE(TStoragePoolsStatsPersistence) {
    Y_UNIT_TEST(SameAggregatedStatsAfterRestart) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(
            NKikimr::CompactTable(runtime, datashard, ResolveTableId(runtime, "/MyRoot/SomeTable")).GetStatus(),
        ...
        const auto expectedDatabaseStats =
            DescribePath(runtime, "/MyRoot").GetPathDescription().GetDomainDescription().GetDiskSpaceUsage();
