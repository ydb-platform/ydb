...
Y_UNIT_TEST_SUITE(SubDomainWithReboots) {
    ...
    Y_UNIT_TEST(DeclareAndDefine) { //+
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            ...
            {
                TInactiveZone inactive(activeZone);
                TestCreateSubDomain(runtime, ++t.TxId, "/MyRoot/DirA", "Name: \"USER_0\"", AlterUserAttrs(userAttrs));
                t.TestEnv->TestWaitNotification(runtime, t.TxId);
            }
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(2), NLs::ShardsInsideDomain(0)}
                );
            }
        });
    }
    ...
    Y_UNIT_TEST(RootWithStoragePoolsAndTable) { //+
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/table_0"), {NLs::PathExist, NLs::PathVersionEqual(3)}
                );
            }
        });
    }
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA/USER_0"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(1), NLs::ShardsInsideDomain(3)}
                );

                TestSplitTable(
                    runtime,
                    ++t.TxId,
                    "/MyRoot/DirA/USER_0/table_0",
                    R"(
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA/USER_0"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(1), NLs::ShardsInsideDomain(5)}
                );
            }
    ...
    Y_UNIT_TEST(SplitTabletInsideWithStoragePools) { //+
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA/USER_0"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(1), NLs::ShardsInsideDomainOneOf({5, 6})}
                );
                t.TestEnv->TestWaitTabletDeletion(runtime, TTestTxConfig::FakeHiveTablets + 2); //delete src
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA/USER_0"),
    ...
    Y_UNIT_TEST(ForceDeleteCreateTableInFlyWithRebootAtCommit) { //+
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA"),
                    {NLs::PathVersionOneOf({5, 6}), NLs::PathsInsideDomain(1), NLs::ShardsInsideDomain(0)}
                );
    ...
    Y_UNIT_TEST(ForceDeleteCreateTableInFly) { //+
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA"),
                    {NLs::PathVersionOneOf({6, 7}), NLs::PathsInsideDomain(1), NLs::ShardsInsideDomain(0)}
                );
                t.TestEnv->TestWaitShardDeletion(runtime, {1, 2, 3});
                UNIT_ASSERT(!CheckLocalRowExists(runtime, TTestTxConfig::SchemeShard, "SubDomains", "PathId", 3));
                UNIT_ASSERT(!CheckLocalRowExists(runtime, TTestTxConfig::SchemeShard, "Paths", "Id", 3));
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            {
                ...
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA/USER_0"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(2), NLs::ShardsInsideDomain(3)}
                );
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/DirA"),
                    {NLs::PathVersionEqual(5), NLs::PathsInsideDomain(2), NLs::ShardsInsideDomain(0)}
    ...
    Y_UNIT_TEST(DoNotLostDeletedTablets) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
            TPathVersion pathVersion;
            {
                ...
                pathVersion = TestDescribeResult(DescribePath(runtime, "/MyRoot/USER_0"), {NLs::PathExist});
                ...
                opts.FinalEvents.push_back(
                    TDispatchOptions::TFinalEventCondition(TEvDataShard::EvSchemaChangedResult, 2)
                );
    ...
    Y_UNIT_TEST(PathsAndShardsCountersSimultaneousAlterSubDomain) {
        ...
        t.Run([&](TTestActorRuntime& runtime, bool& activeZone) {
...

            {
                TInactiveZone inactive(activeZone);
                TestDescribeResult(
                    DescribePath(runtime, "/MyRoot/USER_0"), {NLs::PathsInsideDomain(3), NLs::ShardsInsideDomain(4)}
                );
            }
        });
    }
