...
#include <ydb/core/tx/schemeshard/schemeshard__op_traits.h>  // for IsCreateOperation
...
// Methods to create schema entities of all types.
//
//
// For any type, create method should return TEvModifySchemeTransaction,
...
//
// *Request() -- are the methods defined by the GENERIC_HELPERS macros
// in ydb/core/tx/schemeshard/ut_helpers/helpers.{h,cpp}.
//
...
static const std::vector<TCreatePathOp> CreatePathOperations({
    {
...
Y_UNIT_TEST_SUITE(TSchemeShardSysNames) {
    ...
    void SystemNamesProtect(
        ...
        {
            auto& expected = expectedResultForName;
            for (const auto& name : GetReservedNames()) {
                Cerr << "TEST: SystemNames, test " << typeName << ", whole name " << name
                    << ", should be " << (expectedNameResult == EExpectedResult::ALLOWED ? "ALLOWED" : "FORBIDDEN")
                    << Endl;
        ...
        // Protected cases, ordinary create, prefix
        {
            ...
            for (const auto& name : names) {
                Cerr << "TEST: SystemNames, test type " << typeName << ", name with prefix " << name
                    << ", should be " << (expectedPrefixResult == EExpectedResult::ALLOWED ? "FORBIDDEN" : "ALLOWED")
                    << Endl;
