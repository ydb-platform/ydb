...
Y_UNIT_TEST_SUITE(TSchemeShardUserAttrsTest) {
    ...
    Y_UNIT_TEST(MkDir) {
        ...
        TestMkDir(
            runtime, txId++, "/MyRoot/DirA", "SubDirA", {NKikimrScheme::StatusAccepted}, AlterUserAttrs(subdirAAttrs)
        );
        ...
        TestMkDir(
            ...
            AlterUserAttrs(subdirBAttrs)
        );

        env.TestWaitNotification(runtime, {100, 101, 102, 103});

        TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
        TestDescribeResult(
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA/SubDirA"),
            {NLs::Finished, NLs::UserAttrsEqual(subdirAAttrs), NLs::PathVersionEqual(5)}
        );
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA/SubDirA/DirB"),
            {NLs::Finished,
    ...
    Y_UNIT_TEST(SetAttrs) {
        ...
        TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::UserAttrsEqual({{"AttrRoot", "ValRoot"}})});
    }
    ...
    Y_UNIT_TEST(UserConditionsAtAlter) {
        ...
        TestMkDir(
            runtime, txId++, "/MyRoot", "DirA", {NKikimrScheme::StatusAccepted}, AlterUserAttrs({{"AttrA1", "ValA1"}})
        );
        env.TestWaitNotification(runtime, {txId - 2, txId - 1});

        auto dirVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"),
        ...
        TestUserAttrs(
            ...
            "DirA",
            {NKikimrScheme::StatusAccepted},
            AlterUserAttrs({}, {"AttrA1"}),
            {dirVer}
        );
        env.TestWaitNotification(runtime, txId - 1);

        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirA"), {NLs::UserAttrsEqual({}), NLs::PathVersionEqual(4)});
        ...
        dirVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"), {NLs::UserAttrsEqual({}), NLs::PathVersionEqual(4)}
        );
    ...
    Y_UNIT_TEST(UserConditionsAtCreateDropOps) {
        ...
        auto dirAVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"),
            {NLs::UserAttrsEqual({{"AttrA1", "ValA1"}}), NLs::PathVersionEqual(3)}
        );
        auto rootVer = TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});

        TestMkDir(runtime, txId++, "/MyRoot", "DirB", {NKikimrScheme::StatusAccepted}, {}, {dirAVer, rootVer});

        auto dirBVer =
            TestDescribeResult(DescribePath(runtime, "/MyRoot/DirB"), {NLs::Finished, NLs::PathVersionEqual(3)});
        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirA"), {NLs::Finished, NLs::PathVersionEqual(3)});
        TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
        ...
        TestMkDir(
            ...
            {NKikimrScheme::StatusPreconditionFailed},
            {},
            {dirBVer, dirAVer, rootVer}
        );
        rootVer = TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
        TestMkDir(runtime, txId++, "/MyRoot", "DirC", {NKikimrScheme::StatusAccepted}, {}, {dirBVer, dirAVer, rootVer});
        env.TestWaitNotification(runtime, txId - 1);

        TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
        auto dirCVer =
            TestDescribeResult(DescribePath(runtime, "/MyRoot/DirC"), {NLs::Finished, NLs::PathVersionEqual(3)});
        ...
        TestRmDir(
            ...
            "DirC",
            {NKikimrScheme::StatusPreconditionFailed},
            {dirCVer, dirBVer, dirAVer, rootVer}
        );
        rootVer = TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
        TestRmDir(
            runtime, txId++, "/MyRoot", "DirC", {NKikimrScheme::StatusAccepted}, {dirCVer, dirBVer, dirAVer, rootVer}
        );
        env.TestWaitNotification(runtime, txId - 1);
        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirC"), {NLs::PathNotExist});
        TestDescribeResult(DescribePath(runtime, "/MyRoot"), {NLs::Finished});
    }
    ...
    Y_UNIT_TEST(VariousUse) {
        ...
        env.TestWaitNotification(runtime, txId - 1);
        auto dirAVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"), {NLs::UserAttrsEqual({}), NLs::PathVersionEqual(6)}
        );
        ...
        TestUserAttrs(
            ...
            "DirA",
            {NKikimrScheme::StatusPreconditionFailed},
            AlterUserAttrs({}, {"AttrA6"}),
            {dirAVer}
        );

        dirAVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"),
            {NLs::Finished, NLs::UserAttrsEqual({{"AttrA6", "ValA6"}}), NLs::PathVersionEqual(7)}
        );
        ...
        TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirA"), {NLs::Finished, NLs::UserAttrsEqual({}), NLs::PathVersionEqual(8)}
        );
        ...
        TestMkDir(
            ...
            "DirB",
            {NKikimrScheme::StatusPreconditionFailed},
            AlterUserAttrs(dirAAttrs),
            {dirAVer}
        );

        ++dirAVer.Version;
        TestMkDir(
            runtime, txId++, "/MyRoot", "DirB", {NKikimrScheme::StatusAccepted}, AlterUserAttrs(dirAAttrs), {dirAVer}
        );
        env.TestWaitNotification(runtime, txId - 1);
        auto dirBVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirB"),
        ...
        TestDescribeResult(DescribePath(runtime, "/MyRoot/DirB"), {NLs::Finished, NLs::PathVersionEqual(5)});
        auto dirBBVer =
            TestDescribeResult(DescribePath(runtime, "/MyRoot/DirB/DirBB"), {NLs::Finished, NLs::PathVersionEqual(3)});

        TestRmDir(
            runtime, txId++, "/MyRoot/DirB", "DirBB", {NKikimrScheme::StatusPreconditionFailed}, {dirAVer, dirBVer}
        );
        dirBVer = TestDescribeResult(DescribePath(runtime, "/MyRoot/DirB"), {NLs::Finished, NLs::PathVersionEqual(5)});
        TestRmDir(runtime, txId++, "/MyRoot/DirB", "DirBB", {NKikimrScheme::StatusAccepted}, {dirAVer, dirBVer});
        env.TestWaitNotification(runtime, txId - 1);
        dirBVer = TestDescribeResult(
            DescribePath(runtime, "/MyRoot/DirB"), {NLs::Finished, NLs::NoChildren, NLs::PathVersionEqual(7)}
        );

        TestRmDir(
            runtime, txId++, "/MyRoot", "DirB", {NKikimrScheme::StatusPreconditionFailed}, {dirAVer, dirBVer, dirBBVer}
        );
    }
    ...
    Y_UNIT_TEST(SpecialAttributes) {
        ...
        TestMkDir(
            runtime, txId++, "/MyRoot", "DirA", {NKikimrScheme::StatusInvalidParameter}, AlterUserAttrs(dirAAttrs)
        );

        TUserAttrs dirBAttrs{{"__volume_space_limit", "not a number"}};
        TestMkDir(
            runtime, txId++, "/MyRoot", "DirB", {NKikimrScheme::StatusInvalidParameter}, AlterUserAttrs(dirBAttrs)
        );
        ...
        TestMkDir(
            runtime, txId++, "/MyRoot", "DirD", {NKikimrScheme::StatusInvalidParameter}, AlterUserAttrs(dirDAttrs)
        );
    }
}
