...
public:
    ...
    void Bootstrap() {
        Send(
            PipeCache,
            new TEvPipeCache::TEvForward(new TEvSequenceShard::TEvAllocateSequence(PathId, Cache), TabletId, true),
    ...
    void
    ReplyAndDie(Ydb::StatusIds::StatusCode status, NKikimrIssues::TIssuesIds::EIssueCode code, const TString& message) {
...
private:
        ...
        switch (msg->Record.GetStatus()) {
            ...
            case NKikimrTxSequenceShard::TEvAllocateSequenceResult::SEQUENCE_FROZEN:
                return ReplyAndDie(
                    ...
                    TStringBuilder() << "sequence shard " << TabletId << " has sequence " << PathId
                                     << " temporarily frozen"
                );
            ...
            case NKikimrTxSequenceShard::TEvAllocateSequenceResult::SEQUENCE_OVERFLOW:
                return ReplyAndDie(
                    ...
                    TStringBuilder() << "sequence " << PathId << " doesn't have any more values available"
                );
            ...
            default:
                return ReplyAndDie(
                    ...
                    TStringBuilder() << "Unexpected error from sequence shard " << TabletId
                );
        }
    ...
    void Handle(TEvPipeCache::TEvDeliveryProblem::TPtr& ev) {
        ...
        if (tabletId == TabletId) {
            const TString error = TStringBuilder() << "sequence shard " << tabletId << " is unavailable";
