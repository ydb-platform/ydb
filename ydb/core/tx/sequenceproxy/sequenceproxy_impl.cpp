...
TSequenceProxyCounters::TSequenceProxyCounters() {
    ...
    SequenceShardAllocateCount = group->GetHistogram(
        "SequenceProxy/SequenceShard/AllocateCountPerRequest", NMonitoring::ExponentialHistogram(20, 2, 1)
    );
    ...
    NextValLatency = group->GetHistogram("SequenceProxy/Latency", NMonitoring::ExponentialHistogram(20, 2, 1));
};
...
void TSequenceProxy::Reply(
    const TNextValRequestInfo& request,
...
void TSequenceProxy::OnResolveError(
    const TString& database,
    const TString& path,
    Ydb::StatusIds::StatusCode status,
    const NYql::TIssues& issues
...
void TSequenceProxy::OnResolved(
    const TString& database,
    const TPathId& pathId,
...
bool TSequenceProxy::DoMaybeReplyUnauthorized(
    const TNextValRequestInfo& request,
    const TPathId& pathId,
    TSequenceByPathId& info
) {
    if (request.UserToken && info.SecurityObject) {
        ...
        if (!info.SecurityObject->CheckAccess(access, *request.UserToken)) {
            const TString error = TStringBuilder() << "Access denied for " << request.UserToken->GetUserSID()
                                                   << " to sequence " << pathId;
...
bool TSequenceProxy::DoReplyFromCache(
    const TNextValRequestInfo& request,
    const TPathId& pathId,
    TSequenceByPathId& info
) {
