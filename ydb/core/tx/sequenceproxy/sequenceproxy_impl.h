...
private:
    ...
    void
    DoNextVal(TNextValRequestInfo&& request, const TString& database, const TPathId& pathId, bool needRefresh = true);
    void OnResolveError(
    ...
    void OnResolved(
        const TString& database,
        const TPathId& pathId,
        TSequenceByPathId& info,
        TList<TNextValRequestInfo>& resolved
    );
