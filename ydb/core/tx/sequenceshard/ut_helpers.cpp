...
void TTestContext::Setup(ui32 nodeCount, bool useRealThreads) {
    ...
    TActorId bootstrapper = CreateTestBootstrapper(
        *Runtime, CreateTestTabletInfo(TabletId, TabletType, TErasureType::ErasureNone), &CreateSequenceShard
    );
...
void TTestContext::SendFromEdge(const TActorId& edge, IEventBase* payload, ui64 cookie) {
    ...
    Runtime->SendToPipe(TabletId, edge, payload, nodeIndex, GetPipeConfigWithRetries(), ClientId, cookie);
}
...
THolder<TEvSequenceShard::TEvCreateSequenceResult>
TTestContext::NextCreateSequenceResult(ui64 cookie, const TActorId& edge) {
...
void TTestContext::SendAllocateSequence(ui64 cookie, const TActorId& edge, const TPathId& pathId, ui64 cache) {
    SendFromEdge(edge, new TEvSequenceShard::TEvAllocateSequence(pathId, cache), cookie);
}

THolder<TEvSequenceShard::TEvAllocateSequenceResult>
TTestContext::NextAllocateSequenceResult(ui64 cookie, const TActorId& edge) {
    auto result = ExpectEdgeEvent<TEvSequenceShard::TEvAllocateSequenceResult>(edge, cookie);
    UNIT_ASSERT_VALUES_EQUAL(result->Record.GetOrigin(), TabletId);
    return result;
}

THolder<TEvSequenceShard::TEvAllocateSequenceResult> TTestContext::AllocateSequence(const TPathId& pathId, ui64 cache) {
    ui64 cookie = RandomNumber<ui64>();
    auto edge = Runtime->AllocateEdgeActor();
...
void TTestContext::SendDropSequence(ui64 cookie, const TActorId& edge, const TPathId& pathId) {
    SendFromEdge(edge, new TEvSequenceShard::TEvDropSequence(pathId), cookie);
}

THolder<TEvSequenceShard::TEvDropSequenceResult>
TTestContext::NextDropSequenceResult(ui64 cookie, const TActorId& edge) {
    auto result = ExpectEdgeEvent<TEvSequenceShard::TEvDropSequenceResult>(edge, cookie);
    UNIT_ASSERT_VALUES_EQUAL(result->Record.GetOrigin(), TabletId);
    return result;
}

THolder<TEvSequenceShard::TEvDropSequenceResult> TTestContext::DropSequence(const TPathId& pathId) {
    ui64 cookie = RandomNumber<ui64>();
    auto edge = Runtime->AllocateEdgeActor();
...
THolder<TEvSequenceShard::TEvUpdateSequenceResult>
TTestContext::NextUpdateSequenceResult(ui64 cookie, const TActorId& edge) {
...
void TTestContext::SendFreezeSequence(ui64 cookie, const TActorId& edge, const TPathId& pathId) {
    SendFromEdge(edge, new TEvSequenceShard::TEvFreezeSequence(pathId), cookie);
}

THolder<TEvSequenceShard::TEvFreezeSequenceResult>
TTestContext::NextFreezeSequenceResult(ui64 cookie, const TActorId& edge) {
    auto result = ExpectEdgeEvent<TEvSequenceShard::TEvFreezeSequenceResult>(edge, cookie);
    UNIT_ASSERT_VALUES_EQUAL(result->Record.GetOrigin(), TabletId);
    return result;
}

THolder<TEvSequenceShard::TEvFreezeSequenceResult> TTestContext::FreezeSequence(const TPathId& pathId) {
    ui64 cookie = RandomNumber<ui64>();
    auto edge = Runtime->AllocateEdgeActor();
...
THolder<TEvSequenceShard::TEvRestoreSequenceResult>
TTestContext::NextRestoreSequenceResult(ui64 cookie, const TActorId& edge) {
...
void TTestContext::SendRedirectSequence(ui64 cookie, const TActorId& edge, const TPathId& pathId, ui64 redirectTo) {
    SendFromEdge(edge, new TEvSequenceShard::TEvRedirectSequence(pathId, redirectTo), cookie);
}

THolder<TEvSequenceShard::TEvRedirectSequenceResult>
TTestContext::NextRedirectSequenceResult(ui64 cookie, const TActorId& edge) {
...
THolder<TEvSequenceShard::TEvRedirectSequenceResult>
TTestContext::RedirectSequence(const TPathId& pathId, ui64 redirectTo) {
    ui64 cookie = RandomNumber<ui64>();
    auto edge = Runtime->AllocateEdgeActor();
...
void TTestContext::SendGetSequence(ui64 cookie, const TActorId& edge, const TPathId& pathId) {
    SendFromEdge(edge, new TEvSequenceShard::TEvGetSequence(pathId), cookie);
}

THolder<TEvSequenceShard::TEvGetSequenceResult> TTestContext::NextGetSequenceResult(ui64 cookie, const TActorId& edge) {
    auto result = ExpectEdgeEvent<TEvSequenceShard::TEvGetSequenceResult>(edge, cookie);
    UNIT_ASSERT_VALUES_EQUAL(result->Record.GetOrigin(), TabletId);
    return result;
}

THolder<TEvSequenceShard::TEvGetSequenceResult> TTestContext::GetSequence(const TPathId& pathId) {
    ui64 cookie = RandomNumber<ui64>();
    auto edge = Runtime->AllocateEdgeActor();
