...
Y_UNIT_TEST_SUITE(SequenceShardTests) {
    Y_UNIT_TEST(Basics) {
        ...
        {
            auto createResult = ctx.CreateSequence(MakeHolder<TEvSequenceShard::TEvCreateSequence>(TPathId(123, 42)));
            UNIT_ASSERT_VALUES_EQUAL(
                createResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvCreateSequenceResult::SUCCESS
            );
        }
            ...
            // creating a different path id should succeed
        {
            auto createResult = ctx.CreateSequence(
                TEvSequenceShard::TEvCreateSequence::Build(TPathId(123, 51)).SetStartValue(100001).SetCache(10).Done()
            );
            UNIT_ASSERT_VALUES_EQUAL(
                createResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvCreateSequenceResult::SUCCESS
            );
            ...
            // allocate from the first path, default granularity
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                allocateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvAllocateSequenceResult::SUCCESS
            );
            ...
            // allocate from non-existing path
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                allocateResult->Record.GetStatus(),
            ...
            // try to allocate one more value from the first path
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                allocateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvAllocateSequenceResult::SEQUENCE_OVERFLOW
            );
        }
            ...
            // drop the first path
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                dropResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvDropSequenceResult::SUCCESS
            );
        }
            ...
            // dropping an already dropped path should fail
        {
            auto dropResult = ctx.DropSequence(TPathId(123, 42));
            UNIT_ASSERT_VALUES_EQUAL(
                dropResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvDropSequenceResult::SEQUENCE_NOT_FOUND
            );
        }
        ...
        {
            auto updateResult = ctx.UpdateSequence(TEvSequenceShard::TEvUpdateSequence::Build(TPathId(123, 51))
                                                       .SetNextValue(200000)
                                                       .SetNextUsed(true)
                                                       .Done());
            UNIT_ASSERT_VALUES_EQUAL(
                updateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvUpdateSequenceResult::SUCCESS
            );
        }
        ...
        {
            auto updateResult =
                ctx.UpdateSequence(TEvSequenceShard::TEvUpdateSequence::Build(TPathId(123, 51)).SetCache(5u).Done());
            UNIT_ASSERT_VALUES_EQUAL(
                updateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvUpdateSequenceResult::SUCCESS
            );
        }
            ...
            // get sequence
        {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                getResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvGetSequenceResult::SUCCESS
            );
    ...
    Y_UNIT_TEST(MarkedPipeRetries) {
            ...
            // retry creating a sequence
        {
            auto createResult = ctx.CreateSequence(MakeHolder<TEvSequenceShard::TEvCreateSequence>(TPathId(123, 42)));
            UNIT_ASSERT_VALUES_EQUAL(
                createResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvCreateSequenceResult::PIPE_OUTDATED
            );
        }
            ...
            // retry dropping a sequence
        {
            auto dropResult = ctx.DropSequence(TPathId(123, 42));
            UNIT_ASSERT_VALUES_EQUAL(
                dropResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvDropSequenceResult::PIPE_OUTDATED
            );
        }
    ...
    Y_UNIT_TEST(FreezeRestoreRedirect) {
        ...
        ctx.Setup();
            ...
            // create a sequence
        {
            auto createResult = ctx.CreateSequence(TEvSequenceShard::TEvCreateSequence::Build(TPathId(123, 42))
                                                       .SetMinValue(1)
                                                       .SetStartValue(1)
                                                       .SetCache(100)
                                                       .Done());
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                freezeResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvFreezeSequenceResult::SUCCESS
            );
            ...
            // allocate some values
        {
            auto allocateResult = ctx.AllocateSequence(TPathId(123, 42), 10);
            UNIT_ASSERT_VALUES_EQUAL(
                allocateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvAllocateSequenceResult::SEQUENCE_FROZEN
            );
        }
        ...
        {
            auto restoreResult =
                ctx.RestoreSequence(MakeHolder<TEvSequenceShard::TEvRestoreSequence>(TPathId(123, 43), freezeRecord));
            UNIT_ASSERT_VALUES_EQUAL(
                restoreResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvRestoreSequenceResult::SUCCESS
            );
        }
            ...
            // restoring again must fail, since sequence was active and may have changed its values
        {
            auto restoreResult =
                ctx.RestoreSequence(MakeHolder<TEvSequenceShard::TEvRestoreSequence>(TPathId(123, 43), freezeRecord));
            UNIT_ASSERT_VALUES_EQUAL(
                restoreResult->Record.GetStatus(),
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            UNIT_ASSERT_VALUES_EQUAL(
                redirectResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvRedirectSequenceResult::SUCCESS
            );
        }
            ...
            // allocate some values from redirected sequence
        {
            auto allocateResult = ctx.AllocateSequence(TPathId(123, 42));
            UNIT_ASSERT_VALUES_EQUAL(
                allocateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvAllocateSequenceResult::SEQUENCE_MOVED
            );
        ...
        {
            auto restoreResult =
                ctx.RestoreSequence(MakeHolder<TEvSequenceShard::TEvRestoreSequence>(TPathId(123, 42), freezeRecord));
            UNIT_ASSERT_VALUES_EQUAL(
                restoreResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvRestoreSequenceResult::SUCCESS
            );
        }
            ...
            // check we cannot freeze a redirected sequence
        {
            auto freezeResult = ctx.FreezeSequence(TPathId(123, 43));
            UNIT_ASSERT_VALUES_EQUAL(
                freezeResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvFreezeSequenceResult::SEQUENCE_MOVED
            );
    ...
    Y_UNIT_TEST(NegativeIncrement) {
        ...
        {
            auto createResult = ctx.CreateSequence(
                TEvSequenceShard::TEvCreateSequence::Build(TPathId(123, 42)).SetIncrement(-1).SetCache(10).Done()
            );
            UNIT_ASSERT_VALUES_EQUAL(
                createResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvCreateSequenceResult::SUCCESS
            );
        ...
        {
            auto updateResult =
                ctx.UpdateSequence(TEvSequenceShard::TEvUpdateSequence::Build(TPathId(123, 42)).SetCycle(true).Done());
            UNIT_ASSERT_VALUES_EQUAL(
                updateResult->Record.GetStatus(), NKikimrTxSequenceShard::TEvUpdateSequenceResult::SUCCESS
            );
        }
