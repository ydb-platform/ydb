        ...
        if (fConfig) {
            ...
            if (!::google::protobuf::TextFormat::ParseFromString(*fConfig, &proto)) {
                ...
                if (proto.GetObjectStorage().HasSecretableAccessKey()) {
                    auto accessKey = NMetadata::NSecret::TSecretIdOrValue::DeserializeFromProto(
                        proto.GetObjectStorage().GetSecretableAccessKey(), defaultUserId
                    );
                    if (!accessKey) {
                    ...
                    *proto.MutableObjectStorage()->MutableSecretableAccessKey() = accessKey->SerializeToProto();
                    ...
                    auto accessKey = NMetadata::NSecret::TSecretIdOrValue::DeserializeFromString(
                        proto.GetObjectStorage().GetAccessKey(), defaultUserId
                    );
                    if (!accessKey) {
                        return TConclusionStatus::Fail(
                            "AccessKey is incorrect: " + proto.GetObjectStorage().GetAccessKey() +
                            " for userId: " + defaultUserId
                        );
                    }
                ...
                if (proto.GetObjectStorage().HasSecretableSecretKey()) {
                    auto secretKey = NMetadata::NSecret::TSecretIdOrValue::DeserializeFromProto(
                        proto.GetObjectStorage().GetSecretableSecretKey(), defaultUserId
                    );
                    if (!secretKey) {
                    ...
                    *proto.MutableObjectStorage()->MutableSecretableSecretKey() = secretKey->SerializeToProto();
                    ...
                    auto secretKey = NMetadata::NSecret::TSecretIdOrValue::DeserializeFromString(
                        proto.GetObjectStorage().GetSecretKey(), defaultUserId
                    );
                    if (!secretKey) {
            ...
            result.SetColumn(
                TTierConfig::TDecoder::TierConfig, NMetadata::NInternal::TYDBValue::Utf8(proto.DebugString())
            );
        }
    }
    return result;
...
void TTiersManager::DoPrepareObjectsBeforeModification(
