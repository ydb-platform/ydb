...
Y_UNIT_TEST_SUITE(ColumnShardTiers) {
    ...
    Y_UNIT_TEST(DSConfigsStub) {
        ...
        TLocalHelper lHelper(*server);
        {
            ...
            lHelper.StartSchemaRequest(R"(ALTER TABLE `/Root/olapStore/olapTable` SET TTL Interval("P10D") TO tier1, Interval("P20D") TO tier2 ON timestamp)");
...
*/
        lHelper.StartSchemaRequest(R"(ALTER TABLE `/Root/olapStore/olapTable` SET TTL Interval("P10000D") TO tier1, Interval("P20000D") TO tier2 ON timestamp)");
        {
            const TInstant start = Now();
            bool check = false;
