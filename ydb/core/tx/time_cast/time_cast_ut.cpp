...
Y_UNIT_TEST_SUITE(MediatorTimeCast) {
    ...
    void SendSubscribeRequest(
        TTestActorRuntime & runtime, const TActorId& sender, ui64 coordinatorId, ui64 cookie = 0
    ) {
    ...
    TEvMediatorTimecast::TEvSubscribeReadStepResult::TPtr WaitSubscribeResult(
    ...
    void SendWaitRequest(
        TTestActorRuntime & runtime, const TActorId& sender, ui64 coordinatorId, ui64 readStep, ui64 cookie = 0
    ) {
    ...
    TActorId BootTarget(TTestActorRuntime & runtime, ui64 tabletId) {
        auto boot = CreateTestBootstrapper(
            runtime,
    ...
    ui64 PlanTx(const TServer::TPtr& server, ui64 txId, std::vector<ui64> tablets) {
        ...
        auto req =
            std::make_unique<TEvTxProxy::TEvProposeTransaction>(coordinatorId, txId, 0, Min<ui64>(), Max<ui64>());
    ...
    Y_UNIT_TEST(GranularTimecast) {
        ...
        TBlockEvents<TEvTxProcessing::TEvPlanStep> blockPlan1(runtime, [&](const auto& ev) {
            return ev->Get()->Record.GetTabletID() == tablet1;
        });
        TBlockEvents<TEvTxProcessing::TEvPlanStep> blockPlan2(runtime, [&](const auto& ev) {
        ...
        runtime.Send(new IEventHandle(
            MakeMediatorTimecastProxyID(), sender, new TEvMediatorTimecast::TEvWaitPlanStep(tablet2, stepTx2 + 1)
        ));
        runtime.WaitFor("step notify", [&] {
            return blockNotify.size() > 0;
        });
        ...
        UNIT_ASSERT_C(
            notifiedStep >= stepTx2 + 1, "notified at " << notifiedStep << " expected at least " << (stepTx2 + 1)
        );
        ...
        runtime.Send(new IEventHandle(
            MakeMediatorTimecastProxyID(), sender, new TEvMediatorTimecast::TEvWaitPlanStep(tablet2, stepTx3)
        ));
        runtime.WaitFor("granular update", [&] {
            return blockUpdate.size() > 0 && blockUpdate.back()->Get()->Record.GetLatestStep() >= stepTx3;
        });
