...
private:
    ...
    void Reply(
        const ui64 rangeBegin,
        const ui64 rangeEnd,
        const TEvTxAllocator::TEvAllocate::TPtr& ev,
...
public:
    struct Schema: NIceDb::Schema {
        ...
        struct config: Table<1> {
            ...
            struct dummyKey: Column<1, NScheme::NTypeIds::Bool> {
                using Type = EKeyType;
            }; // PK
...
public:
    ...
    void Enqueue(STFUNC_SIG) override {
        ALOG_ERROR(
            NKikimrServices::TX_ALLOCATOR,
            "tablet# " << TabletID() << " IGNORING message type# " << ev->GetTypeRewrite() << " from Sender# "
                       << ev->Sender.ToString() << " at StateInit"
        );
    }
        ...
        switch (ev->GetTypeRewrite()) {
            ...
            default:
                if (!HandleDefaultEvents(ev, SelfId())) {
                    ALOG_ERROR(
                        NKikimrServices::TX_ALLOCATOR,
                        "tablet# " << TabletID() << " IGNORING message type# " << ev->GetTypeRewrite()
                                   << " from Sender# " << ev->Sender.ToString() << " at StateWork"
                    );
                }
        }
    }
