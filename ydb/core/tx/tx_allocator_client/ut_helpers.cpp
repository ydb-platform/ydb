...
TAnswerWithCookie GrabAnswer(NActors::TTestActorRuntime& runtime) {
    ...
    TEvTxAllocatorClient::TEvAllocateResult* event =
        runtime.GrabEdgeEvent<TEvTxAllocatorClient::TEvAllocateResult>(handle);
    UNIT_ASSERT(event);
...
void TTestEnv::Boot() {
    CreateTestBootstrapper(
        Runtime, CreateTestTabletInfo(TxAllocatorTablet, TTabletTypes::TxAllocator), &CreateTxAllocator
    );
...
void TTestEnv::Setup() {
    ...
    auto domain = TDomainsInfo::TDomain::ConstructDomainWithExplicitTabletIds(
        "dc-1", domainId, 0, 100500, TVector<ui64>{}, TVector<ui64>{}, TVector<ui64>{}, DefaultPoolKinds(2)
    );
