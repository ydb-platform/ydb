...
private:
    ...
    void HandleResolveTimeout(const TActorContext& ctx) {
        LOG_ERROR_S_SAMPLED_BY(
            ...
            NKikimrServices::TX_PROXY,
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " HANDLE ResolveTimeout TCommitWritesReq"
        );
    ...
    void HandleResolve(TEvResolveTablesResponse::TPtr& ev, const TActorContext& ctx) {
        ...
        for (const auto& entry : msg->Tables) {
            ...
            if (entry.KeyDescription->TableId.IsSystemView() ||
                ...
                const TString explanation = TStringBuilder() << "Cannot commit writes to system tableId# "
        ...
        for (auto& kv : PerShardStates) {
            ...
            LOG_DEBUG_S_SAMPLED_BY(
                ...
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " SEND TEvProposeTransaction to datashard "
                          << shardId << " with commit writes request"
            ...
            Send(
                ...
                new TEvPipeCache::TEvForward(
                    new TEvDataShard::TEvProposeTransaction(
                        NKikimrTxDataShard::TX_KIND_COMMIT_WRITES,
                        ctx.SelfID,
                        TxId,
                        txBody,
...
private:
    ...
    void HandleExecTimeout(const TActorContext& ctx) {
        LOG_ERROR_S_SAMPLED_BY(
            ...
            NKikimrServices::TX_PROXY,
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " HANDLE ExecTimeout TCommitWritesReq"
        );
        ...
        switch (msg->GetStatus()) {
            ...
            // nop
                LOG_DEBUG_S_SAMPLED_BY(
                    ...
                    TxId,
                    "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                              << " HANDLE TEvProposeTransactionStatus TCommitWritesReq marker# P6 Status# "
                              << msg->GetStatus()
                );
                break;
