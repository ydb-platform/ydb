...
public:
    ...
    const TVector<NScheme::TTypeInfo>& GetKeyTypes() {
        return KeyTypes;
    }
    ...
    const TQueue<ui64>& GetShardsQueue() const {
        return ShardsQueue;
    }
    bool IsShardsQueueEmpty() const {
        return ShardsQueue.empty();
    }
    ui64 ShardsQueueFront() const {
        return ShardsQueue.front();
    }
    void ShardsQueuePop() {
        ShardsQueue.pop();
    }
    ...
    const TSerializedTableRange& GetSpace() const {
        return SpaceRange;
    }
...
struct TReadTableRequest: public TThrRefBase {
    ...
    TReadTableRequest(const NKikimrTxUserProxy::TReadTableTransaction& tx)
        ...
        , QuotaRequestId(1)
        , RequestVersion(
              tx.HasApiVersion() ? tx.GetApiVersion() : (ui32)NKikimrTxUserProxy::TReadTableTransaction::UNSPECIFIED
          ) {
...
private:
    struct TEvPrivate {
        ...
        struct TEvProxyDataReqOngoingTransactionsWatchdog
            : public TEventLocal<
                  TEvProxyDataReqOngoingTransactionsWatchdog,
                  EvProxyDataReqOngoingTransactionsWatchdog> {};
    ...
    TAutoPtr<TEvTxProxySchemeCache::TEvResolveKeySet>
    PrepareFlatMKQLRequest(TStringBuf miniKQLProgram, TStringBuf miniKQLParams, const TActorContext& ctx);
    ...
    void
    MarkShardError(ui64 tabletId, TDataReq::TPerTablet& perTablet, bool invalidateDistCache, const TActorContext& ctx);
    ...
    void ProcessStreamResponseData(TEvDataShard::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx);
    ...
    const TDomainsInfo::TDomain&
    SelectDomain(NSchemeCache::TSchemeCacheRequest& cacheRequest, const TActorContext& ctx);
...
public:
    ...
    STFUNC(StateWaitInit) {
        ...
        switch (ev->GetTypeRewrite()) { HFuncTraced(TEvTxProxyReq::TEvMakeRequest, Handle); }
    }
    ...
    STFUNC(StateWaitResolve) {
        ...
        switch (ev->GetTypeRewrite()) {
            ...
            CFunc(
                TEvents::TSystem::Wakeup, HandleExecTimeoutResolve
            ); // we must wait for resolve completion to keep key description
...
void TKeySpace::Initialize(bool ordered, TConstArrayRef<NScheme::TTypeInfo> keyTypes, const TTableRange& range) {
...
void TKeySpace::AddRange(const NKikimrTx::TKeyRange& range, ui64 shard) {
    ...
    for (;; ++it) {
        if (it == Ranges.end() || IsGreater(it->From.GetCells(), fromCells)) {
    ...
    if (OrderedQueue) {
        auto point = it;
        while (point != Ranges.end() && !IsGreater(point->From.GetCells(), QueuePoint.GetCells()) &&
               IsGreater(point->To.GetCells(), QueuePoint.GetCells())) {
            for (auto s : point->Shards) ShardsQueue.push(s);
            QueuePoint = point->To;
            ++point;
        }
    } else {
...
bool TKeySpace::IsFull() const {
    ...
    if (IsGreater(SpaceRange.To.GetCells(), range.To.GetCells()))
        return false;
    else if (SpaceRange.To.GetBuffer() == range.To.GetBuffer() && SpaceRange.ToInclusive)
        return false;
...
void TKeySpace::FlushShardsToQueue() {
    ...
    while (it != Ranges.end()) {
        for (auto s : it->Shards) ShardsQueue.push(s);
        ++it;
    }
}
...
void TDataReq::ReportStatus(
    ...
    if (const ui32 cs = ComplainingDatashards.size()) {
        ...
        for (auto ds : ComplainingDatashards) x->Record.AddComplainingDataShards(ds);
    }
    ...
    TDuration executeTime =
        WallClockPrepared.GetValue() ? wallClockEnd - WallClockPrepared : wallClockEnd - WallClockAccepted;
    ...
    switch (status) {
        ...
        case TEvTxUserProxy::TResultStatus::ExecAlready:
            LOG_LOG_S_SAMPLED_BY(
                ...
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " RESPONSE Status# "
                          << TEvTxUserProxy::TResultStatus::Str(status) << "  prepare time: " << prepareTime.ToString()
                          << "  execute time: " << executeTime.ToString() << "  total time: " << totalTime.ToString()
                          << "  marker# P13"
            );
            ...
            if (PerTablet.size() > 1) {
                if (WallClockFirstPrepareReply.GetValue()) {
                    TxProxyMon->TxPrepareSpreadHgram->Collect(
                        (WallClockLastPrepareReply - WallClockFirstPrepareReply).MilliSeconds()
                    );
                    TxProxyMon->TxPrepareArriveSpreadHgram->Collect(
                        (WallClockMaxPrepareArrive - WallClockMinPrepareArrive).MilliSeconds()
                    );
                    TxProxyMon->TxPrepareCompleteSpreadHgram->Collect(
                        (WallClockMaxPrepareComplete - WallClockMinPrepareComplete).MilliSeconds()
                    );
                }
                if (WallClockFirstExecReply.GetValue()) {
                    TxProxyMon->TxExecSpreadHgram->Collect(
                        (WallClockLastExecReply - WallClockFirstExecReply).MilliSeconds()
                    );
                }
            }
            break;
        ...
        case TEvTxUserProxy::TResultStatus::ProxyShardOverloaded:
            LOG_LOG_S_SAMPLED_BY(
                ...
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " RESPONSE Status# "
                          << TEvTxUserProxy::TResultStatus::Str(status) << " shard: " << ComplainingDatashards.front()
                          << " table: " << fnGetTableIdByShard(ComplainingDatashards.front()) << "  marker# P13a"
            );
        ...
        case TEvTxUserProxy::TResultStatus::ProxyShardUnknown:
            LOG_LOG_S_SAMPLED_BY(
                ...
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " RESPONSE Status# "
                          << TEvTxUserProxy::TResultStatus::Str(status) << " shard: "
                          << (ComplainingDatashards ? ComplainingDatashards.front() : 0) << "  marker# P13b"
            );
            TxProxyMon->ReportStatusNotOK->Inc();
            break;
        case TEvTxUserProxy::TResultStatus::ExecResponseData:
            LOG_LOG_S_SAMPLED_BY(
                ...
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " RESPONSE Status# "
                          << TEvTxUserProxy::TResultStatus::Str(status) << "  marker# P13d"
            );
        ...
        default:
            LOG_LOG_S_SAMPLED_BY(
                ...
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " RESPONSE Status# "
                          << TEvTxUserProxy::TResultStatus::Str(status) << "  marker# P13c"
            );
            TxProxyMon->ReportStatusNotOK->Inc();
            break;
    }
...
void TDataReq::ProcessFlatMKQLResolve(NSchemeCache::TSchemeCacheRequest* cacheRequest, const TActorContext& ctx) {
    ...
    const bool forceSnapshot =
        (FlatMKQLRequest->ReadOnlyProgram && !FlatMKQLRequest->Snapshot && rsCount == 0 &&
         engine.GetAffectedShardCount() > 1 && ((TxFlags & NTxDataShard::TTxFlags::ForceOnline) == 0) &&
         !DatabaseName.empty());

    if (forceSnapshot) {
        Send(
            NLongTxService::MakeLongTxServiceID(ctx.SelfID.NodeId()),
...
void TDataReq::ContinueFlatMKQLResolve(const TActorContext& ctx) {
    ...
    TInstant shardCancelDeadline =
        WallClockAccepted + shardCancelAfter + TDuration::Seconds(ShardCancelDeadlineShiftSec);
    ...
    for (ui32 shx = 0, affectedShards = engine.GetAffectedShardCount(); shx != affectedShards; ++shx) {
        ...
        dataTransaction.SetImmediate(
            shardData.Immediate || FlatMKQLRequest->Snapshot && FlatMKQLRequest->ReadOnlyProgram
        );
        ...
        if (transactionBuffer.size() > MaxDatashardProgramSize) {
            TString error = TStringBuilder() << "Datashard program size limit exceeded (" << transactionBuffer.size()
                                             << " > " << MaxDatashardProgramSize << ")";
        ...
        LOG_DEBUG_S_SAMPLED_BY(
            ...
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " SEND TEvProposeTransaction to datashard "
                      << shardData.ShardId << " with " << shardData.Program.size() << " bytes program"
                      << " affected shards " << engine.GetAffectedShardCount() << " followers "
                      << (CanUseFollower ? "allowed" : "disallowed") << " marker# P4"
        );
...
void TDataReq::ProcessReadTableResolve(NSchemeCache::TSchemeCacheRequest* cacheRequest, const TActorContext& ctx) {
    ...
    for (auto& partition : ReadTableRequest->KeyDesc->GetPartitions()) {
        ...
        LOG_DEBUG_S_SAMPLED_BY(
            ...
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " SEND TEvProposeTransaction to datashard "
                      << partition.ShardId << " with read table request"
                      << " affected shards " << ReadTableRequest->KeyDesc->GetPartitions().size() << " followers "
                      << (CanUseFollower ? "allowed" : "disallowed") << " marker# P4b"
        );
...
TAutoPtr<TEvTxProxySchemeCache::TEvResolveKeySet>
TDataReq::PrepareFlatMKQLRequest(TStringBuf miniKQLProgram, TStringBuf miniKQLParams, const TActorContext& ctx) {
    Y_UNUSED(ctx);
    ...
    *TxProxyMon->MiniKQLProgramSize += miniKQLProgram.size();
    ...
    for (auto& keyd : keyDescriptions) {
        if (keyd->RowOperation != TKeyDesc::ERowOperation::Read ||
            keyd->ReadTarget.GetMode() != TReadTarget::EMode::Follower) {
            CanUseFollower = false;
            LOG_DEBUG_S_SAMPLED_BY(
                ctx,
                NKikimrServices::TX_PROXY,
                TxId,
                "Actor " << ctx.SelfID.ToString() << " txid " << TxId << " disallow followers cause of operation "
                         << (ui32)keyd->RowOperation << " read target mode " << (ui32)keyd->ReadTarget.GetMode()
            );
        }
...
void TDataReq::MarkShardError(
    ui64 shardId,
    TDataReq::TPerTablet& perTablet,
    bool invalidateDistCache,
    const TActorContext& ctx
) {
    ...
    if (++TabletErrors == TabletsLeft) {
        LOG_ERROR_S_SAMPLED_BY(
            ...
            NKikimrServices::TX_PROXY,
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " invalidateDistCache: " << invalidateDistCache
...
void TDataReq::Handle(TEvTxProxyReq::TEvMakeRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    if (CancelAfter.Hours() > 8) {
        LOG_WARN_S_SAMPLED_BY(
            ...
            NKikimrServices::TX_PROXY,
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " huge CancelAfter duration "
                      << CancelAfter.ToString() << ", disabling CancelAfter"
        );
        CancelAfter = {};
    }
    ...
    ctx.Schedule(
        TDuration::MilliSeconds(KIKIMR_DATAREQ_WATCHDOG_PERIOD),
        new TEvPrivate::TEvProxyDataReqOngoingTransactionsWatchdog()
    );
    ...
    LOG_DEBUG_S_SAMPLED_BY(
        ...
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " Cookie# " << (ui64)ev->Cookie << " txid# " << TxId
                  << " HANDLE TDataReq marker# P1"
    );
    ...
    if (txbody.HasMiniKQLTransaction()) {
        ...
        if (mkqlTxBody.GetFlatMKQL()) {
            ...
            if (FlatMKQLRequest->LlvmRuntime) {
                LOG_DEBUG_S_SAMPLED_BY(
                    ctx, NKikimrServices::TX_PROXY, TxId, "Using LLVM runtime to execute transaction: " << TxId
                );
            ...
            if (ctx.LoggerSettings()->Satisfies(NLog::PRI_DEBUG, NKikimrServices::MINIKQL_ENGINE, TxId)) {
                ...
                auto txId = TxId;
                settings.BacktraceWriter =
                    [txId, actorSystem](const char* operation, ui32 line, const TBackTrace* backtrace) {
                        LOG_DEBUG_SAMPLED_BY(
                            *actorSystem,
    ...
    if (!resolveReq || !resolveReq->Request) {
        ...
        if (FlatMKQLRequest) {
            FlatMKQLRequest->EngineResultStatusCode =
                FlatMKQLRequest->Engine->PrepareShardPrograms(NMiniKQL::IEngineFlat::TShardLimits(0, 0));
            if (FlatMKQLRequest->EngineResultStatusCode != NMiniKQL::IEngineFlat::EResult::Ok) {
                IssueManager.RaiseIssue(
                    MakeIssue(NKikimrIssues::TIssuesIds::ENGINE_ERROR, "could not prepare shard programs")
                );
                ReportStatus(
                    TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::WrongRequest,
                    NKikimrIssues::TStatusIds::QUERY_ERROR,
    ...
    LOG_DEBUG_S_SAMPLED_BY(
        ...
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " SEND to# " << Services.SchemeCache.ToString()
                  << " TSchemeCache with " << resolveReq->Request->ResultSet.size()
                  << " scheme entries. DataReq marker# P2"
    );
...
void TDataReq::Handle(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ctx,
        (resp->ErrorCount == 0 ? NActors::NLog::PRI_DEBUG : NActors::NLog::PRI_ERROR),
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                  << " HANDLE EvNavigateKeySetResult TDataReq marker# P3b ErrorCount# " << resp->ErrorCount
    );
    ...
    if (!ParseRangeKey(ReadTableRequest->Range.GetFrom(), keyTypes, ReadTableRequest->FromValues, fromExpand) ||
        !ParseRangeKey(ReadTableRequest->Range.GetTo(), keyTypes, ReadTableRequest->ToValues, toExpand)) {
    ...
    TTableRange range(
        ReadTableRequest->FromValues.GetCells(), fromInclusive, ReadTableRequest->ToValues.GetCells(), toInclusive
    );
    ...
    ReadTableRequest->KeyDesc.Reset(new TKeyDesc(res.TableId, range, TKeyDesc::ERowOperation::Read, keyTypes, columns));
...
void TDataReq::Handle(TEvTxProxySchemeCache::TEvResolveKeySetResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ctx,
        (request->ErrorCount == 0 ? NActors::NLog::PRI_DEBUG : NActors::NLog::PRI_ERROR),
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
        ...
        if (FlatMKQLRequest && entry.Kind == NSchemeCache::TSchemeCacheRequest::KindAsyncIndexTable) {
            ...
            if (entry.KeyDescription->RowOperation != TKeyDesc::ERowOperation::Read) {
                ...
                error = TStringBuilder() << "Read operation can be performed on async index table"
                                         << ": " << entry.KeyDescription->TableId
                                         << " only with StaleRO isolation level";
            }
            ...
            if (error) {
                ...
                ReportStatus(
                    TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecError,
                    NKikimrIssues::TStatusIds::NOTSUPPORTED,
                    true,
                    ctx
                );
                return Die(ctx);
...
void TDataReq::Handle(TEvPrivate::TEvReattachToShard::TPtr& ev, const TActorContext& ctx) {
    ...
    Send(
        pipeCache,
        new TEvPipeCache::TEvForward(new TEvDataShard::TEvProposeTransactionAttach(tabletId, TxId), tabletId, true),
        0,
        ++perTablet->ReattachState.Cookie
    );
}
...
void TDataReq::HandlePrepare(TEvPipeCache::TEvDeliveryProblem::TPtr& ev, const TActorContext& ctx) {
    ...
    if (perTablet->TabletStatus == TPerTablet::ETabletStatus::StatusPrepared) {
        if (!ReadTableRequest && (wasRestarting || perTablet->ReattachState.Reattaching) &&
        ...
        LOG_LOG_S_SAMPLED_BY(
            ...
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " shard " << msg->TabletId
                      << " delivery problem (already prepared)"
        ...
        explanation << "tx state unknown for shard " << msg->TabletId << " with txid# " << TxId;
        IssueManager.RaiseIssue(MakeIssue(NKikimrIssues::TIssuesIds::TX_STATE_UNKNOWN, explanation.Str()));
        auto status = IsReadOnlyRequest() ? TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ProxyShardNotAvailable
                                          : TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ProxyShardUnknown;
        ReportStatus(status, NKikimrIssues::TStatusIds::TIMEOUT, true, ctx);
...
void TDataReq::HandlePrepareErrors(TEvPipeCache::TEvDeliveryProblem::TPtr& ev, const TActorContext& ctx) {
    ...
    if (perTablet->TabletStatus == TPerTablet::ETabletStatus::StatusWait) {
        LOG_LOG_S_SAMPLED_BY(
            ...
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " shard " << msg->TabletId
                      << " delivery problem (gathering prepare errors)"
        );
...
void TDataReq::HandlePrepare(TEvDataShard::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ...
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                  ...
                  << " GetStatus# " << msg->GetStatus() << " shard id " << tabletId << " read size "
                  << record.GetReadSize() << " out readset size " << record.OutgoingReadSetInfoSize() << " marker# P6"
    );
    ...
    WallClockMinPrepareArrive =
        WallClockMinPrepareArrive.GetValue() ? Min(WallClockMinPrepareArrive, reportedArriveTime) : reportedArriveTime;
    WallClockMaxPrepareArrive = Max(WallClockMaxPrepareArrive, reportedArriveTime);
    WallClockMinPrepareComplete =
        WallClockMaxPrepareComplete.GetValue() ? Min(WallClockMaxPrepareComplete, completionDelta) : completionDelta;
    ...
    switch (msg->GetStatus()) {
        case NKikimrTxDataShard::TEvProposeTransactionResult::PREPARED: {
            ...
            ui64 privateCoordinator =
                TCoordinators(
                    TVector<ui64>(record.GetDomainCoordinators().begin(), record.GetDomainCoordinators().end())
                )
                    .Select(TxId);
            ...
            if (!SelectedCoordinator || SelectedCoordinator != privateCoordinator) {
                ...
                explanation << "tx state canceled, unable to choose coordinator neither by resolved keys nor by "
                               "TEvProposeTransactionResult from datashard txid#"
                            << TxId;
                IssueManager.RaiseIssue(
    ...
    for (const auto& x : PerTablet)
        if (x.first != exceptTablet) {
            Send(
                ...
                new TEvPipeCache::TEvForward(new TEvDataShard::TEvCancelTransactionProposal(TxId), x.first, false)
            );
        }
}
...
void TDataReq::HandlePrepareErrors(TEvDataShard::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ...
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                  << " HANDLE PrepareErrors TEvProposeTransactionResult TDataReq TabletStatus# "
                  << perTablet->TabletStatus << " shard id " << tabletId
    );
...
void TDataReq::HandlePlan(TEvDataShard::TEvProposeTransactionAttachResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ...
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " shard " << tabletId
                  << " transaction lost during reconnect"
    );
...
void TDataReq::HandlePlan(TEvDataShard::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ...
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                  << " HANDLE Plan TEvProposeTransactionResult TDataReq GetStatus# " << msg->GetStatus() << " shard id "
                  << tabletId << " marker# P12"
    );
...
void TDataReq::Handle(TEvDataShard::TEvGetReadTableSinkStateRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    for (auto& pr : ReadTableRequest->ClearanceSenders) rec.AddClearanceRequests()->SetId(pr.first);
    for (auto& pr : ReadTableRequest->QuotaRequests) rec.AddQuotaRequests()->SetId(pr.second.ShardId);
    for (auto& pr : ReadTableRequest->StreamingShards) rec.AddStreamingShards()->SetId(pr.first);
...
void TDataReq::Handle(TEvTxProcessing::TEvStreamClearanceRequest::TPtr& ev, const TActorContext& ctx) {
    ...
    if (ReadTableRequest->ClearanceSenders.contains(shard) || PerTablet[shard].StreamCleared) {
        ...
        ReportStatus(
            TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecError, NKikimrIssues::TStatusIds::ERROR, true, ctx
        );
        Die(ctx);
        return;
    }
...
void TDataReq::HandleExecTimeoutResolve(const TActorContext& ctx) {
    LOG_ERROR_S_SAMPLED_BY(
        ...
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " HANDLE ExecTimeout TDataReq"
    );
    ReportStatus(
        TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecTimeout, NKikimrIssues::TStatusIds::TIMEOUT, true, ctx
    );
    ...
    switch (FlatMKQLRequest->EngineResponseStatus) {
        ...
        case NMiniKQL::IEngineFlat::EStatus::Error:
            LOG_ERROR_S_SAMPLED_BY(
                ...
                NKikimrServices::TX_PROXY,
                TxId,
                "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " MergeResult ExecError TDataReq marker# P16"
            );
            CpuTime += timer.GetTime();
            ReportStatus(
                TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecError,
...
void TDataReq::ProcessStreamResponseData(
...
void TDataReq::FinishShardStream(TEvDataShard::TEvProposeTransactionResult::TPtr& ev, const TActorContext& ctx) {
    ...
    if (ReadTableRequest->KeySpace.IsFull() && ReadTableRequest->StreamingShards.empty() &&
...
const TDomainsInfo::TDomain&
TDataReq::SelectDomain(NSchemeCache::TSchemeCacheRequest& /*cacheRequest*/, const TActorContext& ctx) {
...
void TDataReq::FailProposedRequest(
    TEvTxUserProxy::TEvProposeTransactionStatus::EStatus status,
    TString errMsg,
    const TActorContext& ctx
) {
    LOG_ERROR_S_SAMPLED_BY(
        ...
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " FailProposedRequest: " << errMsg << " Status# "
                  << status
    );
...
void TDataReq::RegisterPlan(const TActorContext& ctx) {
    ...
    if (FlatMKQLRequest && FlatMKQLRequest->Limits.GetTotalReadSizeLimitBytes()) {
        sizeLimit = sizeLimit ? std::min(sizeLimit, FlatMKQLRequest->Limits.GetTotalReadSizeLimitBytes())
    ...
    for (const auto& xp : PerTablet) {
        ...
        if (rsSize > sizeLimit) {
            ...
            FailProposedRequest(
                TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecError,
                Sprintf(
                    "Transaction incoming read set size %" PRIu64 " for tablet %" PRIu64 " exceeded limit %" PRIu64,
                    rsSize,
                    targetTabletId,
                    sizeLimit
                ),
    ...
    TAutoPtr<TEvTxProxy::TEvProposeTransaction> req(
        new TEvTxProxy::TEvProposeTransaction(SelectedCoordinator, TxId, 0, AggrMinStep, AggrMaxStep)
    );
    ...
    LOG_DEBUG_S_SAMPLED_BY(
        ...
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " SEND EvProposeTransaction to# "
                  << SelectedCoordinator << " Coordinator marker# P7 "
    );
...
void TDataReq::HandleWatchdog(const TActorContext& ctx) {
    ...
    LOG_LOG_S_SAMPLED_BY(
        ...
        NKikimrServices::TX_PROXY,
        TxId,
        "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId << " Transactions still running for " << fromStart
    );
    ctx.Schedule(
        TDuration::MilliSeconds(KIKIMR_DATAREQ_WATCHDOG_PERIOD),
        new TEvPrivate::TEvProxyDataReqOngoingTransactionsWatchdog()
...
void TDataReq::SendStreamClearanceResponse(ui64 shard, bool cleared, const TActorContext& ctx) {
    TAutoPtr<TEvTxProcessing::TEvStreamClearanceResponse> response = new TEvTxProcessing::TEvStreamClearanceResponse;
    response->Record.SetTxId(TxId);
...
void TDataReq::ProcessStreamClearance(bool cleared, const TActorContext& ctx) {
    ...
    if (cleared) {
        ...
        while (!ReadTableRequest->KeySpace.IsShardsQueueEmpty() && limit > ReadTableRequest->StreamingShards.size()) {
        ...
        while (!ReadTableRequest->KeySpace.IsShardsQueueEmpty()) ProcessNextStreamClearance(cleared, ctx);
    }
}

bool TDataReq::ParseRangeKey(
    const NKikimrMiniKQL::TParams& proto,
    TConstArrayRef<NScheme::TTypeInfo> keyType,
    TSerializedCellVec& buf,
    EParseRangeKeyExp exp
...
IActor* CreateTxProxyDataReq(
    const TTxProxyServices& services,
    const ui64 txid,
    const TIntrusivePtr<NKikimr::NTxProxy::TTxProxyMon>& mon,
    const TRequestControls& requestControls
) {
...
template <>
inline void Out<NKikimr::NTxProxy::TDataReq::TPerTablet::ETabletStatus>(
    IOutputStream& o,
