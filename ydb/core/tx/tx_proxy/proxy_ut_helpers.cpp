...
TString
HasUserAttributes(const TAutoPtr<NMsgBusProxy::TBusResponse>& resp, TVector<std::pair<TString, TString>> attrs) {
    UNIT_ASSERT(resp.Get());
    auto& record = resp->Record;
    UNIT_ASSERT_VALUES_EQUAL(record.GetStatus(), NMsgBusProxy::MSTATUS_OK);
    UNIT_ASSERT_VALUES_EQUAL(record.GetSchemeStatus(), NKikimrScheme::StatusSuccess);
    ...
    std::set_difference(attrs.begin(), attrs.end(), present.begin(), present.end(), std::back_inserter(diff));
    UNIT_ASSERT_C(
        diff.size() == 0,
        diff.size() << " attrs not present or has different value, for example name: " << diff.front().first
...
TString IsExtSubdomain(const TAutoPtr<NMsgBusProxy::TBusResponse>& resp) {
    ...
    UNIT_ASSERT_VALUES_EQUAL_C(
        ...
        "status was: " << NMsgBusProxy::EResponseStatus(record.GetStatus()) << " expected: " << NMsgBusProxy::MSTATUS_OK
                       << " explain: " << record.GetErrorReason() << " message: " << record.ShortDebugString()
    );
    UNIT_ASSERT_VALUES_EQUAL(record.GetSchemeStatus(), NKikimrScheme::StatusSuccess);
    auto& descr = record.GetPathDescription();
    auto actualType = descr.GetSelf().GetPathType();
...
ui64 CreateSubDomainAndTabletInside(
    ...
    const auto desc = NTestLs::ExtractDomainDescription(env.GetClient().Ls(subdomain_path));
    const auto key = TSubDomainKey(desc.GetDomainKey());

    TAutoPtr<NMsgBusProxy::TBusResponse> resp = env.GetClient().HiveCreateTablet(
        env.GetSettings().Domain, owner, shard_index, TTabletTypes::Dummy, {}, {key}, channelBindings
    );
    ...
    UNIT_ASSERT(env.GetClient().TabletExistsInHive(&env.GetRuntime(), tablet_id));

    return tablet_id;
}

void CheckTableIsOfline(TBaseTestEnv& env, ui64 tablet_id) {
    UNIT_ASSERT_EQUAL(Max<ui32>(), env.GetClient().GetLeaderNode(&env.GetRuntime(), tablet_id));
}

void CheckTableBecomeAlive(TBaseTestEnv& env, ui64 tablet_id) {
    auto prev = env.GetRuntime().SetDispatchTimeout(WaitTimeOut);
    UNIT_ASSERT(env.GetClient().WaitForTabletAlive(&env.GetRuntime(), tablet_id, true, WaitTimeOut));
...
void CheckTableBecomeOfline(TBaseTestEnv& env, ui64 tablet_id) {
    {
        auto prev = env.GetRuntime().SetDispatchTimeout(WaitTimeOut);
        UNIT_ASSERT(env.GetClient().WaitForTabletDown(&env.GetRuntime(), tablet_id, true, WaitTimeOut));
        env.GetRuntime().SetDispatchTimeout(prev);
    }
    ...
    TDuration negativeTimeout = TDuration::Seconds(1);
    {
        ...
        UNIT_ASSERT(!env.GetClient().WaitForTabletAlive(&env.GetRuntime(), tablet_id, true, negativeTimeout));
        env.GetRuntime().SetDispatchTimeout(prev);
    }
}

void CheckTableRunOnProperTenantNode(TBaseTestEnv& env, const TString& tenant, ui64 tablet_id) {
    UNIT_ASSERT(env.GetTenants().IsActive(tenant, env.GetClient().GetLeaderNode(&env.GetRuntime(), tablet_id)));
}
