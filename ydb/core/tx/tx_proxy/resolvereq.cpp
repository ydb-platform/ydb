...
bool TEvResolveTablesResponse::CheckDomainLocality() const {
    ...
    for (const auto& entry : Tables) {
        if (entry.KeyDescription->TableId.IsSystemView() || TSysTables::IsSystemTable(entry.KeyDescription->TableId)) {
            continue;
        }
...
private:
    ...
    void HandleResolve(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev, const TActorContext& ctx) {
        ...
        for (size_t index = 0; index < Tables.size(); ++index) {
            ...
            const EParseRangeKeyExp fromExpand =
                (table.KeyRange.HasFrom() ? (fromInclusive ? EParseRangeKeyExp::TO_NULL : EParseRangeKeyExp::NONE)
                                          : EParseRangeKeyExp::TO_NULL);
            const EParseRangeKeyExp toExpand =
                (table.KeyRange.HasTo() ? (toInclusive ? EParseRangeKeyExp::NONE : EParseRangeKeyExp::TO_NULL)
                                        : EParseRangeKeyExp::NONE);

            if (!ParseRangeKey(table.KeyRange.GetFrom(), keyTypes, table.FromValues, fromExpand, UnresolvedKeys) ||
                !ParseRangeKey(table.KeyRange.GetTo(), keyTypes, table.ToValues, toExpand, UnresolvedKeys)) {
                IssueManager.RaiseIssue(
                    MakeIssue(NKikimrIssues::TIssuesIds::KEY_PARSE_ERROR, "could not parse key string")
                );
                return ReplyAndDie(
                    TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ResolveError,
            ...
            TTableRange range(table.FromValues.GetCells(), fromInclusive, table.ToValues.GetCells(), toInclusive);
            ...
            table.KeyDescription =
                MakeHolder<TKeyDesc>(entry.TableId, range, TKeyDesc::ERowOperation::Read, keyTypes, columns);
    ...
    void HandleResolve(TEvTxProxySchemeCache::TEvResolveKeySetResult::TPtr& ev, const TActorContext& ctx) {
        ...
        LOG_LOG_S_SAMPLED_BY(
            ...
            TxId,
            "Actor# " << ctx.SelfID.ToString() << " txid# " << TxId
                      << " HANDLE EvResolveKeySetResult TResolveTablesActor marker# P2 ErrorCount# "
                      << request->ErrorCount
        );
        ...
        return ReplyAndDie(
            TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ProxyResolved, NKikimrIssues::TStatusIds::SUCCESS, ctx
        );
    }
