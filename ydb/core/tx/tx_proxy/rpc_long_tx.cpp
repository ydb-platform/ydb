    ...
    void ProceedWithSchema(const NSchemeCache::TSchemeCacheNavigate& resp) {
        ...
        for (auto& [shard, infos] : splittedData.GetShardsInfo()) {
            for (auto&& shardInfo : infos) {
                ...
                this->Register(new NEvWrite::TShardWriter(shard, shardsSplitter->GetTableId(), DedupId, shardInfo, ActorSpan, InternalController, ++writeIdx, NEvWrite::EModificationType::Upsert));
            }
        }
