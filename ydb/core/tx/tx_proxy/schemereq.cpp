...
struct TBaseSchemeReq: public TActorBootstrapped<TDerived> {
    ...
    auto& GetRequestEv() {
        return *SchemeRequest->Ev->Get();
    }

    auto& GetRequestProto() {
        return GetRequestEv().Record;
    }

    auto& GetModifyScheme() {
        return *GetRequestProto().MutableTransaction()->MutableModifyScheme();
    }

    auto& GetModifications() {
        return *GetRequestProto().MutableTransaction()->MutableTransactionalModification();
    }
    ...
    bool
    CheckTablePrereqs(const NKikimrSchemeOp::TTableDescription& desc, const TString& path, const TActorContext& ctx) {
    ...
    bool ExtractResolveForACL(NKikimrSchemeOp::TModifyScheme& pbModifyScheme) {
        ui32 accessToUserAttrs = pbModifyScheme.HasAlterUserAttributes() ? NACLib::EAccessRights::WriteUserAttributes
        ...
        switch (pbModifyScheme.GetOperationType()) {
            ...
            case NKikimrSchemeOp::ESchemeOpAlterExtSubDomain: {
                ...
                toResolve.RequiredAccess =
                    NACLib::EAccessRights::CreateDatabase | NACLib::EAccessRights::AlterSchema | accessToUserAttrs;
    ...
    ui64
    GetShardToRequest(NSchemeCache::TSchemeCacheNavigate::TEntry& resolveResult, const TPathToResolve& resolveTask) {
        ...
        for (const auto& entry : navigate->ResultSet) {
            switch (entry.Status) {
                ...
                case NSchemeCache::TSchemeCacheNavigate::EStatus::AccessDenied: {
                    ...
                    ReportStatus(
                        TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::AccessDenied, nullptr, &issue, ctx
                    );
                    break;
                }
    ...
    bool CheckACL(const NSchemeCache::TSchemeCacheNavigate::TResultSet& resolveSet, const TActorContext& ctx) {
        ...
        while (resolveIt != resolveSet.end() && requestIt != ResolveForACL.end()) {
            ...
            if (!entry.SecurityObject->CheckAccess(access, *UserToken)) {
                ...
                const TString errString = TStringBuilder() << "Access denied for " << UserToken->GetUserSID()
                                                           << " to path " << JoinPath(entry.Path);
                auto issue = MakeIssue(NKikimrIssues::TIssuesIds::ACCESS_DENIED, errString);
        ...
        for (const auto& entry : resolveSet) {
            if (entry.Attributes.contains(NDocApi::VersionAttribute)) {
                auto issue = MakeIssue(
                    ...
                    TStringBuilder() << "Document API table cannot be modified"
    ...
    void Handle(NSchemeShard::TEvSchemeShard::TEvModifySchemeTransactionResult::TPtr& ev, const TActorContext& ctx) {
        ...
        switch (record.GetStatus()) {
            ...
            case NKikimrScheme::EStatus::StatusAccepted:
                ReportStatus(
                    TEvTxUserProxy::TEvProposeTransactionStatus::EStatus::ExecInProgress, &record, nullptr, ctx
                );
                break;
            default:
    ...
    void Handle(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev, const TActorContext& ctx) {
        ...
        LOG_LOG_S_SAMPLED_BY(
            ctx,
            (navigate->ErrorCount == 0 ? NActors::NLog::PRI_DEBUG : NActors::NLog::PRI_NOTICE),
...
struct TFlatSchemeReq: public TBaseSchemeReq<TFlatSchemeReq> {
    ...
    STFUNC(StateWaitResolveWorkingDir) {
        switch (ev->GetTypeRewrite()) { HFunc(TEvTxProxySchemeCache::TEvNavigateKeySetResult, HandleWorkingDir); }
    }

    STFUNC(StateWaitResolve) {
        switch (ev->GetTypeRewrite()) { HFunc(TEvTxProxySchemeCache::TEvNavigateKeySetResult, Handle); }
    }
...
void TFlatSchemeReq::HandleWorkingDir(
    ...
    auto parts =
        Merge(SplitPath(GetModifyScheme().GetWorkingDir()), SplitPath(GetPathNameForScheme(GetModifyScheme())));

    if (!workingDir || workingDir->size() >= parts.size()) {
        const TString errText = TStringBuilder() << "Cannot resolve working dir"
