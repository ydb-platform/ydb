...
TUploadCounters::TUploadCounters()
    ...
    PackageSizeRecordsByRecords =
        TBase::GetHistogram("ByRecords/PackageSize/Records", NMonitoring::ExponentialHistogram(15, 2, 10));
    PackageSizeCountByRecords =
        TBase::GetHistogram("ByRecords/PackageSize/Count", NMonitoring::ExponentialHistogram(15, 2, 10));
    ...
    for (ui32 i = 0; i < (ui32)descriptor->value_count(); ++i) {
        ...
        CodesCount.emplace(
            vDescription->name(), CreateSubGroup("reply_code", vDescription->name()).GetDeriviative("Replies/Count")
        );
    }
}
