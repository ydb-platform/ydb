...
private:
    ...
    void ProcessResponses() {
        ...
        if (NodeWardenStorageConfigResponse && NodeWardenStorageConfigResponse->IsOk()) {
            if (NodeWardenStorageConfigResponse->Get()->BridgeInfo) {
                ...
                for (size_t i = 0; i < pbBridgeInfo.PilesSize(); ++i) {
                    pbBridgeInfo.MutablePiles(i)->SetNodes(pileNodes[i]);
                }
            } else {
