...
Y_UNIT_TEST_SUITE(ViewerColumnTableLocalIndex) {
    Y_UNIT_TEST(ColumnTableLocalIndexInDescribeEndpoint) {
        TPortManager tp;
        ui16 port = tp.GetPort(2134);
        ui16 grpcPort = tp.GetPort(2135);
        ui16 monPort = tp.GetPort(8765);
        ...
        auto status = client.CreateColumnTable("/Root", R"(
            Name: "TestColumnTable"
            ColumnShardCount: 1
            Schema {
        ...
        UNIT_ASSERT_EQUAL(status, NMsgBusProxy::MSTATUS_OK);

        TKeepAliveHttpClient httpClient("localhost", monPort);
        NViewerTests::WaitForHttpReady(httpClient);

        {
            TStringStream responseStream;
            TKeepAliveHttpClient::THeaders headers;
            headers["Accept"] = "application/json";
            const auto statusCode = httpClient.DoGet(
                "/viewer/describe?path=/Root/TestColumnTable&subs=1", &responseStream, headers);
            TString describeResponse = responseStream.ReadAll();
            UNIT_ASSERT_EQUAL_C(statusCode, HTTP_OK, statusCode << ": " << describeResponse);

            NJson::TJsonValue json;
            NJson::ReadJsonTree(describeResponse, &json, true);

            const auto& pathDescription = json["PathDescription"];
            const auto& self = pathDescription["Self"];
            UNIT_ASSERT_C(self["ChildrenExist"].GetBoolean(),
                "Column table with local index must have ChildrenExist=true");
...

        {
            ...
            headers["Accept"] = "application/json";
            const auto statusCode = httpClient.DoGet(
                "/viewer/describe?path=/Root&subs=1", &responseStream, headers);
            TString describeResponse = responseStream.ReadAll();
            UNIT_ASSERT_EQUAL_C(statusCode, HTTP_OK, statusCode << ": " << describeResponse);
            ...
            const auto& children = json["PathDescription"]["Children"];
            bool foundSys = false;
            for (const auto& child : children.GetArray()) {
            ...
            UNIT_ASSERT_C(foundSys,
                ".sys must be visible at root level. Response: " + describeResponse);
        }
    }
}
