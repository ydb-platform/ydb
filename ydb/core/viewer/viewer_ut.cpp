...
Y_UNIT_TEST_SUITE(Viewer) {
    ...
    struct TPathAliasViewerFixture {
        TPortManager Ports;
        ui16 GrpcPort = Ports.GetPort(2135);
        ui16 MonPort = Ports.GetPort(8765);
        TServerSettings Settings{Ports.GetPort(2134)};
        THolder<TServer> Server;
        THolder<TClient> Client;
        THolder<TKeepAliveHttpClient> Http;

        explicit TPathAliasViewerFixture(ui32 mode) {
            ...
            auto& security = *Settings.AppConfig->MutableDomainsConfig()->MutableSecurityConfig();
            security.SetEnforceUserTokenCheckRequirement(true);
            security.AddAdministrationAllowedSIDs(ROOT_TOKEN);
            security.AddViewerAllowedSIDs("username");
            security.AddRegisterDynamicNodeAllowedSIDs(ROOT_TOKEN);
            ...
            if (mode == 1) {
                addRule("^/Never(/|$)", "/Unused\\1");
            } else if (mode == 2) {
                addRule("^/alias/Relative$", "/Root/Visible");
                addRule("^/object$", "/Root/Visible");
                addRule("^/shortcut$", "/Root/Second");
            ...
            GrantRead(*Client);
            Client->TestMkDir("/Root", "Visible");
            Client->TestMkDir("/Root", "Second");
            Client->TestMkDir("/Root", "Private");
            NACLib::TDiffACL denied;
            denied.AddAccess(NACLib::EAccessType::Deny, NACLib::EAccessRights::GenericRead, "username");
            Client->TestModifyACL("/Root", "Private", denied.SerializeAsString());
            Http = MakeHolder<TKeepAliveHttpClient>("localhost", MonPort);
            WaitForHttpReady(*Http);
        }
        ...
        TResponse Get(const TString& endpoint, const TString& database, const TString& path,
                      const TString& token = VALID_TOKEN)
        {
            TKeepAliveHttpClient::THeaders headers;
            headers["Accept"] = "application/json";
            headers["Authorization"] = token;
            TStringStream body;
            const auto code = Http->DoGet(TStringBuilder() << endpoint << "?database=" << database
                << "&path=" << path << "&direct=1&ask_scheme_cache=never&timeout=5000", &body, headers);
            return {code, body.ReadAll()};
        }
    };

    Y_UNIT_TEST(PathAliasingViewerHttpRoutingAndTargetAcl) {
        TPathAliasViewerFixture fixture(2);
        const auto canonical = fixture.Get("/viewer/describe", "/Root", "/Root/Visible");
        UNIT_ASSERT_VALUES_EQUAL_C(canonical.Code, HTTP_OK, canonical.Body);
        UNIT_ASSERT_STRING_CONTAINS(canonical.Body, "Visible");
        const auto aliased = fixture.Get("/viewer/describe", "/alias", "/object");
        UNIT_ASSERT_VALUES_EQUAL_C(aliased.Code, HTTP_OK, aliased.Body);
        UNIT_ASSERT_VALUES_EQUAL(aliased.Body, canonical.Body);

        // Authentication must use the resolved database, and authorization must
        // still inspect the target object's ACL rather than the alias spelling.
        const auto deniedCanonical = fixture.Get("/viewer/describe", "/Root", "/Root/Private");
        const auto deniedAlias = fixture.Get("/viewer/describe", "/alias", "/alias/Private");
        UNIT_ASSERT_VALUES_EQUAL(deniedCanonical.Code, HTTP_FORBIDDEN);
        UNIT_ASSERT_VALUES_EQUAL(deniedAlias.Code, deniedCanonical.Code);
        const auto admin = fixture.Get("/viewer/describe", "/alias", "/alias/Private", ROOT_TOKEN);
        UNIT_ASSERT_VALUES_EQUAL_C(admin.Code, HTTP_OK, admin.Body);
        UNIT_ASSERT_STRING_CONTAINS(admin.Body, "Private");
    }

    Y_UNIT_TEST(PathAliasingViewerRelativeDirectoryUsesLogicalDatabase) {
        TPathAliasViewerFixture fixture(2);
        const auto canonical = fixture.Get("/scheme/directory", "/Root", "/Root/Visible");
        UNIT_ASSERT_VALUES_EQUAL_C(canonical.Code, HTTP_OK, canonical.Body);
        const auto relative = fixture.Get("/scheme/directory", "/alias", "Relative");
        UNIT_ASSERT_VALUES_EQUAL_C(relative.Code, HTTP_OK, relative.Body);
        UNIT_ASSERT_VALUES_EQUAL(relative.Body, canonical.Body);
    }

    Y_UNIT_TEST(PathAliasingViewerStorageStatsResolvesEveryLogicalOperand) {
        TPathAliasViewerFixture fixture(2);
        const auto response = fixture.Get("/viewer/storage_stats", "/alias", "Relative,/object,/shortcut", ROOT_TOKEN);
        UNIT_ASSERT_VALUES_EQUAL_C(response.Code, HTTP_OK, response.Body);
        NJson::TJsonValue json;
        NJson::TJsonReaderConfig config;
        NJson::ReadJsonTree(response.Body, &config, &json, true);
        const auto& paths = json["Paths"].GetArray();
        UNIT_ASSERT_VALUES_EQUAL_C(paths.size(), 3, response.Body);
        UNIT_ASSERT_VALUES_EQUAL(paths[0]["FullPath"].GetString(), "/Root/Visible");
        UNIT_ASSERT_VALUES_EQUAL(paths[1]["FullPath"].GetString(), "/Root/Visible");
        UNIT_ASSERT_VALUES_EQUAL(paths[2]["FullPath"].GetString(), "/Root/Second");
    }

    Y_UNIT_TEST(PathAliasingViewerAutocompleteUsesLogicalDirectoryLookup) {
        TPathAliasViewerFixture fixture(2);
        fixture.Client->TestMkDir("/Root/Visible", "Nested");
        auto autocomplete = [&](const TString& database, const TString& prefix, const TString& table) {
            TKeepAliveHttpClient::THeaders headers;
            headers["Accept"] = "application/json";
            headers["Authorization"] = VALID_TOKEN;
            TStringStream body;
            const auto code = fixture.Http->DoGet(TStringBuilder() << "/viewer/autocomplete?database="
                << database << "&prefix=" << prefix << "&table=" << table << "&direct=1&timeout=5000", &body, headers);
            const TString response = body.ReadAll();
            UNIT_ASSERT_VALUES_EQUAL_C(code, HTTP_OK, response);
            NJson::TJsonValue json;
            NJson::TJsonReaderConfig config;
            NJson::ReadJsonTree(response, &config, &json, true);
            UNIT_ASSERT_C(json["Success"].GetBoolean(), response);
            const auto& entities = json["Result"]["Entities"].GetArray();
            UNIT_ASSERT_VALUES_EQUAL_C(entities.size(), 1, response);
            UNIT_ASSERT_VALUES_EQUAL(entities[0]["Name"].GetString(), "Nested");
            UNIT_ASSERT_VALUES_EQUAL(entities[0]["Parent"].GetString(), "Visible");
        };
        autocomplete("/Root", "Visible/Ne", "");
        autocomplete("/alias", "Relative/Ne", "");
        autocomplete("/alias", "Ne", "Relative");
    }

    Y_UNIT_TEST_TWIN(PathAliasingViewerDisabledAndUnrelatedRulesPreserveStatus, Unrelated) {
        TPathAliasViewerFixture fixture(Unrelated ? 1 : 0);
        const auto canonical = fixture.Get("/scheme/directory", "/Root", "/Root/Visible");
        UNIT_ASSERT_VALUES_EQUAL_C(canonical.Code, HTTP_OK, canonical.Body);
        const auto missingAlias = fixture.Get("/scheme/directory", "/Root", "/alias/Visible");
        const auto missing = fixture.Get("/scheme/directory", "/Root", "/Unconfigured/Visible");
        UNIT_ASSERT_VALUES_EQUAL(missing.Code, HTTP_BAD_REQUEST);
        UNIT_ASSERT_VALUES_EQUAL(missingAlias.Code, missing.Code);
    }
