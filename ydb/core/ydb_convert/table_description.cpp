    ...
    Ydb::StatusIds::StatusCode& code, TString& error)
{
    ...
    for (const auto& index : req->add_indexes()) {
        ...
        if (asSchemeObject) {
            ...
            auto* indexConfig = modifyScheme->MutableAlterTable()->AddAddLocalIndexes();
            indexConfig->SetName(index.name());
            indexConfig->SetType(NKikimrSchemeOp::EIndexTypeLocalBloomFilter);
            indexConfig->SetState(NKikimrSchemeOp::EIndexState::EIndexStateReady);
