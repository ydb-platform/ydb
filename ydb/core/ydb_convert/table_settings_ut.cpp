...
Y_UNIT_TEST_SUITE(ConvertTableMetricsSettings) {
    ...
    Y_UNIT_TEST(CreateMetricsLevels) {
        for (const auto& level : Levels) {
            ...
            UNIT_ASSERT_C(FillCreateTableSettingsDesc(table, request, code, error, warnings, false), error);
            UNIT_ASSERT(table.GetDetailedMetricsSettings().HasConfigured());
            UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(table.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(level.Internal));
            ...
            NKikimrSchemeOp::TTableDescription restored;
            UNIT_ASSERT_C(FillCreateTableSettingsDesc(restored, exported, code, error, warnings, false), error);
            UNIT_ASSERT(restored.GetDetailedMetricsSettings().HasConfigured());
            UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(restored.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(level.Internal));
        }
    }
    ...
    Y_UNIT_TEST(AlterMetricsLevels) {
        for (const auto& level : Levels) {
            ...
            auto code = Ydb::StatusIds::SUCCESS;
            TString error;
            UNIT_ASSERT_C(FillAlterTableSettingsDesc(table, request, code, error, false), error);
            UNIT_ASSERT(table.GetDetailedMetricsSettings().HasConfigured());
            UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(table.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(level.Internal));
        }
    }
    ...
    Y_UNIT_TEST(OmittedEmptyAndResetMetricsSettings) {
        ...
        NKikimrSchemeOp::TTableDescription table;
        UNIT_ASSERT_C(FillCreateTableSettingsDesc(table, create, code, error, warnings, false), error);
        UNIT_ASSERT(!table.HasDetailedMetricsSettings());

        create.mutable_metrics_settings();
        UNIT_ASSERT_C(FillCreateTableSettingsDesc(table, create, code, error, warnings, false), error);
        UNIT_ASSERT(table.GetDetailedMetricsSettings().HasConfigured());
        UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(table.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(TInternal::MetricsLevelDisabled));
        ...
        UNIT_ASSERT_C(FillAlterTableSettingsDesc(table, alter, code, error, false), error);
        UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(table.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(TInternal::MetricsLevelTable));
    ...
    Y_UNIT_TEST(RejectInvalidMetricsLevelsWithoutClearingOverride) {
        for (const int level : {int(TApi::METRICS_LEVEL_DISABLED), 99, -1}) {
            ...
            TString error;
            TList<TString> warnings;
            UNIT_ASSERT(!FillCreateTableSettingsDesc(table, create, code, error, warnings, false));
            ...
            error.clear();
            UNIT_ASSERT(!FillAlterTableSettingsDesc(table, alter, code, error, false));
            UNIT_ASSERT_VALUES_EQUAL(code, Ydb::StatusIds::BAD_REQUEST);
            UNIT_ASSERT(!error.empty());
            UNIT_ASSERT(table.GetDetailedMetricsSettings().HasConfigured());
            UNIT_ASSERT_VALUES_EQUAL(static_cast<int>(table.GetDetailedMetricsSettings().GetConfigured().GetMetricsLevel()), static_cast<int>(TInternal::MetricsLevelPartition));
        }
    }
