...
**Правило:** (а) Каждая статья должна начинаться с вводного блока, описывающего её содержимое. (б) Примеры кода должны быть согласованы по стилю: если используются шаблоны (плейсхолдеры), то во всём блоке кода должны быть шаблоны; если конкретные значения — то везде конкретные значения. При появлении хотя бы одного шаблона весь блок должен быть переведён в формат шаблонов, а в примечании после примера каждый шаблон должен быть раскрыт с описанием и примером конкретного значения.

**Проверка:**
1. Прочитать начало статьи (до первого H2-заголовка). Проверить: есть ли текст, описывающий, о чём эта статья и что в ней содержится?
2. Для каждого блока кода проверить: используются ли в нём одновременно шаблоны (например, `<endpoint>`) и конкретные значения (например, `https://storage.yandexcloud.net/my_bucket/`)? Если да — это нарушение.
3. Если в блоке кода есть шаблоны, проверить: есть ли после блока кода примечание или блок «Где:» с описанием каждого шаблона и примером конкретного значения? Если нет — это нарушение.
...
Шаблон `<endpoint>` и конкретное значение `my-real-bucket` в одном блоке — несогласованность.
...
Все значения в блоке кода — шаблоны. После блока идёт «Где:» с описанием каждого шаблона и примером конкретного значения.
...
| Высокий | 1, 2, 6, 9, 14 | Структурные проблемы, из-за которых читатель не понимает текст |
