...
If no consumer is specified, reading from the topic is performed without one. Specifying a consumer allows tracking the read position and lag from the topic side, for example via the [CLI](../reference/ydb-cli/topic-read.md).
