...
* Added the ability for the `{{ ydb-cli }} workload tpch` and `{{ ydb-cli }} workload tpcds` [commands](./reference/ydb-cli/commands/workload/index.md) to use the `--scale` option as a fraction, specifying the scale factor as a percentage of full benchmark's data size and workload.
