# List of changes to the Yandex Corporate DBMS server
...
The version includes all the improvements contained in build {{ ydb-short-name }} 26.1.1.22, see [description of changes](./changelog-server.md#26-1-1-22). In addition, the version includes the following [improvements specific to the Corporate DBMS](#26-1-1-ent-3-extras).

#### Improvements specific to the Corporate DBMS {#26-1-1-ent-3-extras}

The changes listed below are available in Yandex Corporate DBMS in addition to the corresponding build {{ ydb-short-name }}:
...
The version includes all the improvements contained in build {{ ydb-short-name }} 25.4.1.15, see [description of changes](./changelog-server.md#25-4-1-15). In addition, all [additional fixes](#25-2-1-ent-13-extras) listed below for version 25.2.1.ent.13 are included.
...
The version includes all the improvements contained in build {{ ydb-short-name }} 25.3.1.27, see [description of changes](./changelog-server.md#25-3-1-27). In addition, all [additional fixes](#25-2-1-ent-13-extras) listed below for version 25.2.1.ent.13 are included.
...
The version includes all the improvements contained in build {{ ydb-short-name }} 25.2.1.26, see [description of changes](./changelog-server.md#25-2-1-26). In addition, the version includes a number of additional improvements ported from the current version 26.1.

#### Additional fixes {#25-2-1-ent-13-extras}

The changes listed below have been transferred from version 26.1 to the supported stable versions of the Corporate DBMS Yandex:
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/24265) a bug in [Workload Manager](./dev/resource-consumption-management.md) that could cause CPU usage by columnar tables to exceed the set limits.
...
* A number of bugs related to [changefeed](./concepts/glossary.md#changefeed) have been fixed:
  ...
  * [Fixed](https://github.com/ydb-platform/ydb/pull/25453) a [bug](https://github.com/ydb-platform/ydb/issues/25454) that could cause table import without change streams to fail due to incorrect changefeed file search.
* [Fixed](https://github.com/ydb-platform/ydb/pull/26069) a [bug](https://github.com/ydb-platform/ydb/issues/25869) that could lead to failures during UPSERT operations in columnar tables.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/26879) a [bug](https://github.com/ydb-platform/ydb/issues/26565) of checksum mismatch during restoration of compressed backups from S3.
...
* [Implemented](https://github.com/ydb-platform/ydb/pull/19504) a [vector index](./dev/vector-indexes.md?version=v25.1) for approximate vector search. Recipes for [YDB CLI and YQL](./recipes/vector-search?version=v25.1) and examples of work [in C++ and Python](./recipes/ydb-sdk/vector-search?version=v25.1) have been published for vector search.
...
* Added a [V2 configuration mechanism](./devops/configuration-management/configuration-v2/config-overview?version=v25.1) to simplify the deployment of new clusters {{ ydb-short-name }} and further work with them. [Comparison](./devops/configuration-management/compare-configs?version=v25.1) of V1 and V2 configuration mechanisms.
...
* Added support for [auto-partitioning of topics](./concepts/cdc.md?version=v25.1#topic-partitions) in CDC for row-oriented tables. Enabled by setting the `enable_topic_autopartitioning_for_cdc` flag in the cluster configuration.
...
* [Supported](https://github.com/ydb-platform/ydb/pull/7052) the [format DEBEZIUM_JSON](./concepts/cdc.md?version=v25.1#debezium-json-record-structure) for change streams (changefeed).
...
* [Added](https://github.com/ydb-platform/ydb/pull/12909) automatic integrity checking of backups during import, preventing restoration from damaged backups and protecting against data loss.
...
* Added system views with information about [access rights settings](./dev/system-views?version=v25.1#auth), [history of overloaded partitions](./dev/system-views?version=v25.1#top-overload-partitions) (enabled by setting the `enable_followers_stats` flag in the cluster configuration), [history of row-oriented table partitions with broken locks (TLI)](./dev/system-views?version=v25.1#top-tli-partitions).
* Added new parameters to the [CREATE USER](./yql/reference/syntax/create-user.md?version=v25.1) and [ALTER USER](./yql/reference/syntax/alter-user.md?version=v25.1) statements:
  ...
  * `LOGIN` and `NOLOGIN` — user unlocking and locking.
...
* [Implemented](https://github.com/ydb-platform/ydb/issues/9748) the ability to switch functional flags while the server {{ ydb-short-name }} is running. Flags for which the [proto file](https://github.com/ydb-platform/ydb/blob/main/ydb/core/protos/feature_flags.proto#L60) does not specify the `(RequireRestart) = true` parameter will be applied without restarting the cluster.
* Now the oldest (not the newest) locks [are changed to full-shard](https://github.com/ydb-platform/ydb/pull/11329) when the number of locks on shards is exceeded.
* [Implemented](https://github.com/ydb-platform/ydb/pull/12567) saving optimistic locks in memory during a smooth restart of data shards, which should reduce the number of ABORTED errors due to loss of locks when balancing tables between nodes.
* [Implemented](https://github.com/ydb-platform/ydb/pull/12689) cancellation of volatile transactions with ABORTED status during a smooth restart of data shards.
* [Added](https://github.com/ydb-platform/ydb/pull/6342) the ability to remove `NOT NULL`constraints on a column in a table using a `ALTER TABLE ... ALTER COLUMN ... DROP NOT NULL` query.
...
* **_(Experimentally)_** [Added](https://github.com/ydb-platform/ydb/pull/14075) an experimental mode of operation with more stringent access rights checks. It is enabled by setting the following flags:
  ...
  * `enable_database_admin` — adds the role of a database administrator.
* [Added](https://github.com/ydb-platform/ydb/pull/21119) the ability to use familiar tools for stream data processing — Kafka Connect, Confluent Schema Registry, Kafka Streams, Apache Flink, AKH via [Kafka API](./reference/kafka-api/index.md) when working with YDB Topics. Now YDB Topics Kafka API supports:
  * client balancing of readers — enabled by setting the `enable_kafka_native_balancing` flag in the [cluster configuration](./reference/configuration/feature_flags.md). [How reader balancing works in Apache Kafka](https://www.confluent.io/blog/cooperative-rebalancing-in-kafka-streams-consumer-ksqldb). Now reader balancing in the Kafka API of YDB Topics will work the same way;
...
* [Added](https://github.com/ydb-platform/ydb/pull/20982) [a new protocol](https://github.com/ydb-platform/ydb/issues/11064) in [Node Broker](./concepts/glossary.md#node-broker), eliminating spikes in network traffic on large clusters (more than 1000 servers) associated with broadcasting information about nodes.

#### Changes with loss of backward compatibility
...
* The "Operations" tab has been added to the database page, which allows you to view a list of operations and cancel them.
* The cluster monitoring panel has been updated, and the option to collapse it has been added.
* Support for case-sensitive search has been implemented in the hierarchical JSON display tool.
...
* Extra confirmation requests when closing the browser page in the query editor have been removed — confirmation is requested only when necessary.
* [Fixed](https://github.com/ydb-platform/ydb/pull/17839) [error](https://github.com/ydb-platform/ydb/issues/15230) due to which not all tablets were displayed in the "Tablets" tab in the diagnostics section.
...
* [Added](https://github.com/ydb-platform/ydb/issues/6512) a new granular timestamps protocol, which will reduce the execution time of distributed transactions (slowing down one shard will not slow down all).
...
* [Implemented](https://github.com/ydb-platform/ydb/pull/15255) pipelined processing of internal transactions in [Node Broker](./concepts/glossary?version=v25.1#node-broker), which has sped up the launch of dynamic nodes in the cluster {{ ydb-short-name }}.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/10460) the error of inconsistent reading from a topic when trying to explicitly acknowledge a message read within a transaction. Now, when trying to acknowledge a message, the user will receive an error.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/16269) the problem of the «Drop Tablet» operation freezing in PQ tablet, especially during Interconnect delays.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/18378) the issue where, after updating the cluster version, Hive subscribers were started in datacenters without running database nodes.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/18938) an error where `ydb scheme describe` returned a list of columns not in the order they were specified when creating the table.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/22203) an error where tablets could freeze due to insufficient memory on nodes. Now, tablets will automatically start as soon as sufficient resources become available on any node.
...
* Backup and restore operations [support](https://github.com/ydb-platform/ydb/pull/25675) views (VIEW). To use this functionality, you need to set the `enable_view_export` flag in the `feature_flags` section of the [database](./devops/configuration-management/configuration-v1/dynamic-config.md) or [cluster](./devops/configuration-management/configuration-v1/static-config.md) configuration.
...
* The columns by which the results of the query execution are sorted are taken into account by the optimizer when automatically selecting a secondary index. The functionality works only for queries to a single table, without joining other tables.

#### Error corrections

* When receiving error `OperationAborted` in the response from S3, the export operation does not fail, but retries the write to S3.
...
* [Added](https://github.com/ydb-platform/ydb/pull/11775) priorities for checking authentication tickets [using a third-party IAM provider](./security/authentication.md#iam), with the highest priority given to requests from new users. Tickets in the cache update their information with lower priority.
...
* [Optimized](https://github.com/ydb-platform/ydb/issues/18473) CPU consumption by the Scheme shard tablet, which reduced the latency of responses to requests. The limit on the number of Scheme shard operations is now checked before performing split and merge operations on tablets.
* Automatic selection of a secondary index [is enabled by default](./dev/secondary-indexes.md#avtomaticheskoe-ispolzovanie-indeksov-pri-vyborke) when executing a query.

#### Error corrections

* [Fixed](https://github.com/ydb-platform/ydb/pull/12221) an error that caused CPU load to significantly increase when reading small messages from a topic in small portions. This could lead to delays in reading/writing to this topic.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/12804) an error that could cause reading on tablet subscribers to fail during automatic table splitting.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/17123) a rare error where client applications would freeze during transaction commit when a partition was deleted before the write quota to the topic was updated.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15077) a rare error that caused a failure when tablet subscribers connected to a leader with an inconsistent command log state.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15933) a rare error that allowed a user to write to a topic bypassing the account quota restrictions.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15862) an issue that caused an error when inserting data into row-oriented tables with default values using `UPSERT`.
* [Fixed](https://github.com/ydb-platform/ydb/pull/15334) an error that caused a failure when executing queries to tables with secondary indexes that return lists of results using the expression `RETURNING *`.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15182) an issue with long caching of negative responses to authentication requests from an LDAP-compatible user and group directory.
...
* Added [query tracing](./reference/observability/tracing/setup), a tool that allows you to see in detail the path a query takes through a distributed system.
...
* For the Docker container with YDB, a startup parameter `FQ_CONNECTOR_ENDPOINT` has been added to specify the address of the connector to external data sources. TLS encryption of the connection to the connector has been added. The ability to output the connector service port, running locally on the same host as the dynamic YDB node, has been added.
* Added [auto-partitioning](./concepts/datamodel/topic#autopartitioning) mode for topics, in which topics can split partitions based on load while maintaining guarantees of message reading order and exactly once writing. The mode can be enabled by the cluster administrator using the `enable_topic_split_merge` and `enable_pqconfig_transactions_at_scheme_shard` settings in the [dynamic configuration](./devops/configuration-management/configuration-v1/dynamic-config#obnovlenie-dinamicheskoj-konfiguracii).
* Added [transactions](./concepts/transactions#topic-table-transactions) involving [topics](https://ydb.tech/docs/ru/concepts/datamodel/topic) and row-oriented tables. This allows you to transactionally move data from tables to topics and vice versa, as well as between topics, so that data is not lost or duplicated. Transactions can be enabled by the cluster administrator using the `enable_topic_service_tx` and `enable_pqconfig_transactions_at_scheme_shard` settings in the [dynamic configuration](./devops/configuration-management/configuration-v1/dynamic-config#obnovlenie-dinamicheskoj-konfiguracii).
...
* Added logging of [audit log](./security/audit-log) events for user logins in YDB, user session termination events in the user interface, as well as backup and restore requests.
* Added a system view that allows you to get information about sessions established with the database using a query.
* Added support for default constant values for columns in row-oriented tables.
...
* [Added](https://github.com/ydb-platform/ydb/pull/8708) start and end times and the author to the metadata of backup and restore operations from an S3-compatible storage.
* Added support for backup and restore of ACL for tables from an S3-compatible storage.
...
* [Added](https://github.com/ydb-platform/ydb-embedded-ui/issues/929) indication of [auto-increment columns](./yql/reference/types/serial).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/6850) an error that rendered a database with a large number of tables or partitions inoperable when restoring from a backup. Now, if database size limits are exceeded, the restore operation will fail, and the database will continue to operate normally.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/9509) the initial CDC scan hang when the table is frequently updated.
...
* [Eliminated](https://github.com/ydb-platform/ydb/pull/11061) read delay occurring during and after partition splitting.
* Fixed errors when reading data from S3.
* [Fixed](https://github.com/ydb-platform/ydb/pull/4793) the way aws signature is calculated when accessing S3.
* Fixed false positives of the HealthCheck system during database backup with a large number of shards.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/12018) a potential breakdown of ["frozen" locks](./contributor/datashard-locks-and-change-visibility#vzaimodejstvie-s-raspredelyonnymi-tranzakciyami), which could be caused by mass operations (for example, deletion by TTL).
...
* Added retrieval of nested groups from an [LDAP server](./security/authentication.md#ldap), improved host parsing in [LDAP configuration](reference/configuration/auth_config.md#ldap-auth-config) and added an option to disable built-in login and password authentication.
...
* [Added](https://github.com/ydb-platform/ydb-embedded-ui/pull/1028) sorting by `CPUTime` to the table listing queries.
...
* Enabled query editor autocomplete by default.
* [Added](https://github.com/ydb-platform/ydb-embedded-ui/pull/834) View support.

### Bug fixes

* Added a check on the size of a local transaction before committing it to fix [errors](https://github.com/ydb-platform/ydb/issues/6677) in schema operations when exporting/backing up large databases.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/5993) a rare error that caused deletion by [TTL](./concepts/ttl) to crash the process.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/6008) errors that could cause tablet draining from a node to hang.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/7714) an error executing queries that performed multiple left joins to the same row-oriented table.
...
* Added a resource consumption diagnostics dashboard, which is located on the database information tab and allows you to determine the current state of resource consumption: CPU cores, RAM, and network distributed storage space.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/2624) a rarely occurring error when, with enabled and used volatile transactions, a successful transaction confirmation result was returned before the transaction was successfully committed. Volatile transactions are disabled by default and are under development.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3638) the issue of increased resource consumption by the topic actor `PERSQUEUE_PARTITION_ACTOR`.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/2169) an error of possible incomplete commit of accumulated changes when using distributed transactions. This error occurs with an extremely rare combination of events, including restarting tablets serving the transaction-involved table partitions.
* [Eliminated](https://github.com/ydb-platform/ydb/pull/3165) a race between table merge and garbage collection processes, which could cause garbage collection to fail with an invariant violation and, as a result, cause the server process `ydbd` to crash.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3002) an error in Blob Storage that prevented data storage nodes from starting with a correct configuration. The error occurred in systems with the experimental "blob depot" feature explicitly enabled (this feature is disabled by default).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3587) a metric display error for the number of partitions in the topic; previously, it displayed an incorrect value.
...
* Support for [Kafka API for topics](reference/kafka-api/index.md) has been added. Now YDB topics can be worked with via Kafka-compatible API, designed for migrating existing applications. Support for the Kafka protocol version 3.4.0 is provided.
...
* **_(Experimentally)_** Support for PostgreSQL compatibility has been added. The new mechanism allows executing SQL queries in PostgreSQL dialect on YDB infrastructure using the PostgreSQL network protocol. You can use familiar PostgreSQL tools, such as psql and drivers (pq for Golang and psycopg2 for Python), and develop queries using familiar PostgreSQL syntax with YDB's horizontal scalability and fault tolerance.
* **_(Experimentally)_** Support for [federated queries](concepts/query_execution/federated_query/index.md) has been added. It allows obtaining information from various data sources without moving them to YDB. Interaction with ClickHouse, PostgreSQL, and S3 via YQL queries is supported without duplicating data between systems.

### Built-in UI

* A new option `PostgreSQL` has been added to the request type selector settings, which is available when the `Enable additional query modes` parameter is enabled. The query history now takes into account the syntax used when executing the query.
...
* Iterator reads have been implemented. The new functionality allows separating reads and computations. Iterator reads enable DataShards to increase the throughput of read queries.
* The performance of writing to YDB topics has been optimized.
* Tablet balancing when nodes are overloaded has been improved.

### Bug fixes

* Fixed an error that could block snapshots with reading iterators that coordinators were not aware of.
...
* Implemented logging of `DDL` operation records in the general log.
* Implemented a prohibition for the `ydb table attribute add/drop` command to work with any objects other than tables.
...
* Fixed an error where incorrect values could be stored in Pdisks `ExpectedSerial`.
...
* Fixed the overestimation of the result `SELECT COUNT(*)` immediately after import.
* Fixed an error where `TEvScan` could return an empty dataset in the case of a DataShard split.
...
* Merged the logic of schedulers from `data executer` and `scan executer` into a single class.
...
* Fixed an error where `DELETE` returned data but did not delete them.
...
* Use status `UNAVAILABLE` instead of `GENERIC_ERROR` when shard identification fails.
* Added support for `rope payload` in `TEvVGet`.
* Added ignoring of obsolete events.
