...
The version includes all the improvements contained in build {{ ydb-short-name }} 26.1.1.22, see [change description](./changelog-server.md#26-1-1-22). In addition, the version includes the following [improvements specific to the Corporate DBMS](#26-1-1-ent-3-extras).
...
* A set of fixes has been implemented in index access (StreamIndexLookup) that eliminates the possibility of rare query «freezing» situations and reduces RAM consumption during query execution.
...
The changes listed below were transferred from version 26.1 to the supported stable versions of the Corporate DBMS:

* A bug has been fixed that violated the sorting order specified in the query when accessing system tables.
* A bug in the logic for checking the integrity of the internal state has been fixed, which in rare cases could lead to a single (not massive) restart of storage nodes.
* An optimization has been added that allows filtering rows by columns from the index before querying the main table, which reduces the number of requests to the main table when executing certain types of queries.
* A set of fixes has been implemented in index access (StreamIndexLookup) that eliminates the possibility of rare query «freezing» situations and reduces RAM consumption during query execution.
...
* TLI (Transaction Locks Invalidated) error messages are provided with an identifier or path of the affected table in all cases.
...
* Fixed a number of issues related to cluster initialization:
  ...
  * [Fixed](https://github.com/ydb-platform/ydb/pull/28886) a [problem](https://github.com/ydb-platform/ydb/issues/27228) that made it impossible to create new databases immediately after deploying a cluster for several minutes.
...
* Added support for [auto-partitioning of topics](./concepts/cdc.md?version=v25.1#topic-partitions) in CDC for string tables. It is enabled by setting the flag `enable_topic_autopartitioning_for_cdc` in the cluster configuration.
* [Added](https://github.com/ydb-platform/ydb/pull/8264) the ability to [change the data retention time](./concepts/cdc.md?version=v25.1#topic-options) in the CDC topic using the expression `ALTER TOPIC`.
...
* In backup and restore operations [supported](https://github.com/ydb-platform/ydb/issues/7054) change streams. To use the functionality, you need to set the flags `enable_changefeeds_export` and `enable_changefeeds_export` in the `feature_flags` section of the [database](./devops/configuration-management/configuration-v1/dynamic-config.md) or [cluster](./devops/configuration-management/configuration-v1/static-config.md) configuration.
...
* Added system views with information about [access rights settings](./dev/system-views?version=v25.1#auth), [history of overloaded partitions](./dev/system-views?version=v25.1#top-overload-partitions) — enabled by setting the flag `enable_followers_stats` in the cluster configuration, [history of string table partitions with broken locks (TLI)](./dev/system-views?version=v25.1#top-tli-partitions).
...
* [Implemented](https://github.com/ydb-platform/ydb/issues/9748) the ability to switch functional flags while the {{ ydb-short-name }} server is running. Flags for which the [proto file](https://github.com/ydb-platform/ydb/blob/main/ydb/core/protos/feature_flags.proto#L60) does not specify the `(RequireRestart) = true` parameter will be applied without restarting the cluster.
* Now the oldest (rather than new) locks [are changed to shard-wide](https://github.com/ydb-platform/ydb/pull/11329) when the number of locks on shards is exceeded.
* [Implemented](https://github.com/ydb-platform/ydb/pull/12567) the saving of optimistic locks in memory during a smooth restart of datashards, which should reduce the number of ABORTED errors due to loss of locks when balancing tables between nodes.
...
* Diagnostics and error introspection related to memory have been improved ([#10419](https://github.com/ydb-platform/ydb/pull/10419), [#11968](https://github.com/ydb-platform/ydb/pull/11968)).
...
* [Added](https://github.com/ydb-platform/ydb/pull/20982) a [new protocol](https://github.com/ydb-platform/ydb/issues/11064) in [Node Broker](./concepts/glossary.md#node-broker), which eliminates spikes in network traffic on large clusters (more than 1000 servers) associated with broadcasting information about nodes.
...
* Fixed [serialization error](https://github.com/ydb-platform/ydb-embedded-ui/issues/2164), which could lead to crashes when opening query execution statistics.
* The logic of nodes transitioning to a critical state has been changed — a CPU pool filled to 75-99% now triggers a warning rather than a critical state.
...
* [Improved](https://github.com/ydb-platform/ydb/pull/15607) the resilience of Node Broker to high load from cluster nodes.
...
* [Optimized](https://github.com/ydb-platform/ydb/issues/18473) CPU consumption by the Scheme shard tablet, which reduced the latency of responses to queries. Now the limit on the number of Scheme shard operations is checked before performing split and merge operations on tablets.
* The [automatic selection of a secondary index](./dev/secondary-indexes.md#avtomaticheskoe-ispolzovanie-indeksov-pri-vyborke) when executing a query is enabled by default.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/17123) a rare error where client applications would hang during transaction commit when a partition was deleted before the write quota for the topic was updated.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/17519) [an error](https://github.com/ydb-platform/ydb/issues/17499) where a commit without confirmation of writing to the topic led to blocking the current and subsequent transactions with topics.
* Fixed transaction hangs when working with topics during [reboot](https://github.com/ydb-platform/ydb/issues/17843) or [deletion](https://github.com/ydb-platform/ydb/issues/17915) of a tablet.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/14811) an error that significantly reduced the read speed from [tablet subscribers](./concepts/glossary.md#tablet-follower).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15160) an issue where a transaction would hang when a user was simultaneously managing a topic and moving a PQ tablet to another node.
* [Fixed](https://github.com/ydb-platform/ydb/pull/15233) an issue with a leaked counter value for userInfo, which could lead to a reading error `too big in flight`.
...
* For the YDB Docker container, added a startup parameter `FQ_CONNECTOR_ENDPOINT` to specify the address of the connector to external data sources. Added the option to encrypt the connection with the connector using TLS. Added the option to output the port of the connector service running locally on the same host as the YDB dynamic node.
...
* Added [transactions](./concepts/transactions#topic-table-transactions) involving [topics](https://ydb.tech/docs/en/concepts/datamodel/topic) and string tables. This allows you to transactionally move data from tables to topics and vice versa, as well as between topics, so that data is not lost or duplicated. Transactions can be enabled by the cluster administrator using the `enable_topic_service_tx` and `enable_pqconfig_transactions_at_scheme_shard` settings in the [dynamic configuration](./devops/configuration-management/configuration-v1/dynamic-config#obnovlenie-dinamicheskoj-konfiguracii).
...
* [Added](https://github.com/ydb-platform/ydb-embedded-ui/pull/1127) search on the pages with query history and saved queries.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/8925) a rarely occurring error that led to transaction execution order violations.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/12018) a potential failure of [“frozen” locks](./contributor/datashard-locks-and-change-visibility#vzaimodejstvie-s-raspredelyonnymi-tranzakciyami), which could be caused by mass operations (e.g., deletion by TTL).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/6008) errors that could cause the soft drain of tablets from a node to hang.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3638) the issue of increased consumption of computational resources by the topic actor `PERSQUEUE_PARTITION_ACTOR`.
...
* The ability to [write to a topic without deduplication](concepts/datamodel/topic.md#no-dedup) has been added. This type of writing is suitable for cases where the order of message processing is not critical. Writing without deduplication is faster and consumes fewer server resources, but messages are not ordered or deduplicated on the server.
...
* **_(Experimentally)_** Support for compatibility with PostgreSQL has been added. The new mechanism allows executing SQL queries in the PostgreSQL dialect on the YDB infrastructure using the PostgreSQL network protocol. You can use familiar PostgreSQL tools, such as psql and drivers (pq for Golang and psycopg2 for Python), and develop queries using the familiar PostgreSQL syntax with YDB's horizontal scalability and fault tolerance.
...
* Added a `usage` filter in `/storage` in `viewer backend`.
...
* Fixed an error where during a rolling restart, when going through the balancer, `healthcheck` alerts about storage flickered.
...
* Fixed error `SIGSEGV` in dynodes when importing `CSV` via `YDB CLI`.
