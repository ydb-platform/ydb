...
* [Added](https://github.com/ydb-platform/ydb/issues/15586) system views containing information about [access control entities](../dev/system-views#auth).
* Added system views containing [follower statistics](../dev/system-views#partitions) for row-oriented tables.
...
* [Added](https://github.com/ydb-platform/ydb/issues/7054) support for backup/restore of changefeed from/to S3-compatible storage. Feature flags `EnableChangefeedsExport` and `EnableChangefeedsImport`.
* ?25-1-2[Added]((https://github.com/ydb-platform/ydb/pull/12909)) automatic integrity verification of backups during import, which prevents restoration from corrupted backups and protects against data loss.
* [Change](https://github.com/ydb-platform/ydb/pull/11329) oldest (not newest) locks into full shard locks when shards have too many locks.
* [Implemented](https://github.com/ydb-platform/ydb/pull/12567) that in-memory locks are now preserved when datashards are gracefully restarted, which should reduce ABORTED errors due to lost locks while rebalancing tablets between nodes, and might improve shard startup time due to less waiting for mediator state restore.
* [Implemented](https://github.com/ydb-platform/ydb/pull/12689) abort volatile transactions with an ABORTED status when datashards are gracefully restarted.
* [Allow](https://github.com/ydb-platform/ydb/pull/8264) to [alter CDC topic's retention period](../concepts/cdc?version=main#topic-options) using ALTER TOPIC.
* ?25-1-2 [Supported](https://github.com/ydb-platform/ydb/pull/7052) the `DEBEZIUM_JSON` format for changefeeds.
* [Supported](https://github.com/ydb-platform/ydb/pull/7052) [constant folding](https://en.wikipedia.org/wiki/Constant_folding) in the query optimizer by default, optimizing query performance by evaluating constant expressions at compile time.
...
* https://github.com/ydb-platform/ydb/pull/11483 A datashard could catch VERIFY during restart with scheme transaction inflight
* #15721 Fixed a bug in YDB UUID column handling in ReadTable RPC method #15697
* #15850 Fixed a bug in optional columns handling in ReadRows RPC method #15701
* #10460 You cannot simultaneously commit offsets in a transaction and out of a transaction. - Fixed simultaneous commit handling
* #11920 A race between actors. The actor of the KP session is transferred to the FinalCleanupState. In this composition, he does not respond to the TEvQueryRequest message from the TPartitionWriter actor. - Fixed race between actors
* #12060 MinStep value - transactions are ordered by (Step, TxId) - the name of the state instead of the number. - Improved transaction ordering
* #12220 When a transaction is committed, the value of the AvgWriteBytes metric changes. This, in turn, can lead to a split of the partition. - Fixed issue with AvgWriteBytes metric changes
* #12905 Fixed the errors: 1. The program did not take into account that it can receive a TEvReadSet more times than the number of senders in TEvProposeTransaction. Deleted the check from Y_ABORT_UNLESS'. 2. Replaced the Y_ABORT_UNLESSchecks with theForceFalseflag. Instead of an abnormal termination, the program returnsABORTED`. - Fixed TEvReadSet handling
* #13863 Two transactions. In the head of the first one is PartNo != 0. During the commit, the partition gathers them together to write them in one operation. The final key does not take into account the PartNo from the head of the first transaction. - Fixed PartNo handling in transaction commits
* #9122 NodeBroker now sends deltas when new nodes are registered in the cluster. / NodeBroker теперь отправляет дельты при регистрации нод в кластере.
* #9168 Limit inflight coordination service session requests to 100k. / Кол-во одновременных запросов на создание сессий в сервисе координации теперь ограничено в 100 тысяч.
* #9848 Fixed handling of accidental duplicate nodes in system tablet config. / Исправлена работа в случае если для системных таблеток одна нода указана несколько раз.
* #11059 Fixed excessive read latency during and after some shard splits. / Исправлено большое время отклика на чтениях во время и после перешардирования таблиц.
* #13910 Fixed Key is out of range error during import from S3-compatible storage.
* #9723 Fixed crash when reading from asynchronous replicas.
* #9507 Fixed CDC initial scan freezing.
* #13741 Fixed incorrect metadata end detection in dynamic configs.

  
17122:Fixed a rare issue that caused client applications to hang during commit operations. The problem occurred because the `TEvDeletePartition` message could arrive before the TEvApproveWriteQuota message. The batch did not send TEvConsumed and this blocked the queue of write quota requests. #17122 (Alek5andr-Kotov)
16649:Fixed an issue where distconf would issue a VERIFY error and crash when nodes are removed through the legacy Console configuration management interface. #16649 (Alexander Rutkovsky)
16635:Fixed an error that caused the RETURNING clause to work incorrectly with INSERT/UPSERT operations. #16635 (Vitalii Gridnev)
16420:Added Ydb::StatusIds::TIMEOUT to the list of retryable errors, which improves the ability to build large secondary indexes. #16420 (azevaykin)
16269:Fixed the issue of a hanging Drop Tablet operation in the PQ tablet, caused by the tablet receiving duplicate TEvPersQueue::TEvProposeTransaction messages from SS, especially during delays in IC operation. #16269 (Alek5andr-Kotov)
16194:Fixed a verification failure that occurred during VDisk compaction. #16194 (Alexander Rutkovsky)
15570:Allow creation of views that use UDFs in their queries. #15570 (Daniil Demin)
15515:Fixed an issue where long-running read sessions incorrectly failed with "too big inflight" errors. #15233 (FloatingCrowbar)
15515:Fixed a topic reading hang that occurred when at least one partition had no incoming data but was being read by multiple consumers. #15515 (FloatingCrowbar)
