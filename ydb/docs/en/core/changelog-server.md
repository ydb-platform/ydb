...
* [Added configurable throttling of synchronous VDisk writes](https://github.com/ydb-platform/ydb/pull/43895) based on the current amount of fresh data. This limits memory growth under intensive write workloads.
* [Literal `DEFAULT` values can be added to write query plans at compile time](https://github.com/ydb-platform/ydb/pull/45049), avoiding their runtime materialization. The behavior is controlled by `TableServiceConfig.EnableCompileTimeDefaults`.
* [Added JSON indexes](https://github.com/ydb-platform/ydb/pull/36478) for accelerating queries with `JSON_EXISTS` and `JSON_VALUE`. Index creation and automatic selection are controlled by the `EnableJsonIndex` and `EnableJsonIndexAutoSelect` feature flags.
...
* [Added strict serializable isolation for read-write transactions](https://github.com/ydb-platform/ydb/pull/43962), including real-time transaction ordering and commit timestamps. The mode is controlled by the `EnableStrictSerializableIsolation` feature flag.
...
* [Backup restoration can write SST parts directly to DataShard](https://github.com/ydb-platform/ydb/pull/45009), reducing CPU and disk usage compared to row-by-row restoration. The behavior is controlled by `EnableDataShardDirectPartImport`.
* [Added a compact full-text index format](https://github.com/ydb-platform/ydb/pull/43404) that reduces storage consumption. The format is controlled by the `EnableCompactFulltextIndex` feature flag.
* Full-text indexes support [filter columns](./dev/fulltext-indexes.md?version=v26.3#filtered), allowing search within a logical partition of a table.
...
* Workload Manager resource pool classifiers support [`ACTION="reject"`](https://github.com/ydb-platform/ydb/pull/46051), which rejects matching queries before they are routed to a resource pool.
* Full-text indexes can be created for tables with [arbitrary primary key types](./dev/fulltext-indexes.md?version=v26.3#primary-key). The capability is controlled by the `EnableFulltextIndexRowId` feature flag and is disabled by default in this release.
* Added [hybrid search](./dev/hybrid-search.md?version=v26.3), which combines full-text relevance and vector similarity into a single ranked result. The capability is controlled by `TableServiceConfig.EnableHybridSearch` and is disabled by default in this release.
...
* [`ANALYZE` is represented as a background operation](./yql/reference/syntax/analyze.md?version=v26.3) that can be monitored, cancelled, and forgotten through the standard operation APIs and CLI commands.
* [Local indexes are represented as schema objects](https://github.com/ydb-platform/ydb/pull/43144), including prefix Bloom filters for row-oriented tables and local indexes for column-oriented tables. The behavior is controlled by `EnableLocalIndexAsSchemeObject`.
...
* Administrators can [require non-administrative requests to static nodes to specify a database](https://github.com/ydb-platform/ydb/pull/40964) by enabling `ForbidRequestsToStaticNodesWithoutDatabase`. The restriction is disabled by default in this release.
