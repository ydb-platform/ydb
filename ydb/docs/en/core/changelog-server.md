# Change log {{ ydb-short-name }} Server
...
* [Backup and restore of columnar tables, including S3-compatible storage, are available](./recipes/backup/backup-collections/exporting-to-external-storage.md?version=main).
* Columnar table columns support [dictionary encoding](./yql/reference/syntax/create_table/index.md?version=v26.3#encoding). Use `ENCODING(DICT)` for values with low cardinality.
...
* [Decommissioning of storage groups using virtual groups has been added](./maintenance/manual/virtual_storage_groups_decommit.md?version=v26.3). Data is moved to virtual groups in the background, and applications continue to read and write.
...
* Columnar table engine optimization: columnar tables use an updated compaction strategy that organizes data more efficiently and a new data merging strategy when reading, which speeds up queries on constantly changing data.
* Authentication/authorization subsystem optimization: batch authorization requests to AccessService are enabled by default, reducing overhead.
* System virtual attributes such as `__ydb_create_time`, `__ydb_write_time`, etc., and user attributes `__ydb_user_attributes` are now available for streaming YQL queries. [Link to functionality](./concepts/query_execution/topics.md?version=v26.3#system-metadata).
* Distributed Storage subsystem optimization: full VDisk synchronization is faster due to the removal of processed SyncLog data.
...
* The [column compression](./yql/reference/syntax/create_table/index.md?version=v26.2) setting in columnar tables is available by default.
...
* For string tables, you can now force [compaction](./yql/reference/syntax/alter_table/compact.md?version=v26.2) using `ALTER TABLE ... COMPACT`.
* Automatic storage balancing between groups and background verification of disk placement correctness have been added.
...
* Support for [incremental backups](./concepts/datamodel/backup-collection.md?version=v26.2) has been added, which allows you to save only changes relative to the previous backup in the collection.
...
* Added snapshot retention for long analytical queries to columnar tables so that snapshot data is not deleted until the query is completed.
* QueryService can notify the SDK about the completion of a node or session so that the client stops sending new requests there.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/49929) topic reading freeze after the read balancer restart.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/50897) crashes and [freezes](https://github.com/ydb-platform/ydb/pull/50621) of streaming queries when creating checkpoints.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/40912) loss of `ReadSet` acknowledgments, which could prevent topic transactions from completing.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/46033) leak of script execution records created by streaming queries.
...
* The TLI (Transaction Lock Invalidation) [diagnostics](./reference/configuration/tli_config.md?version=v26.1) has been expanded: the configuration `tli_config`, [logging](./troubleshooting/performance/queries/tli-logging.md?version=v26.1) and [system views](./dev/system-views.md?version=v26.1#top-tli-partitions).
* When [auto-partitioning by load](./concepts/datamodel/table.md?version=v26.1#auto_partitioning_by_load) is used, the CPU load on the partition leader and all its replicas is taken into account.
* [Stream queries](./dev/streaming-query/index.md?version=v26.1) support [writing results to local tables](./dev/streaming-query/table-writing.md?version=v26.1).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/35793) the hanging of async checkpointing with a full entry and an empty checkpoint.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/36220) freezes in the PQ read session.
* [Fixed](https://github.com/ydb-platform/ydb/pull/36292) the hanging of the scan executor on `SELECT … LIMIT` for empty tables.
* [Fixed](https://github.com/ydb-platform/ydb/pull/36692) the delayed reset of TLI `LOCKS_BROKEN`.
* [Fixed](https://github.com/ydb-platform/ydb/pull/37130) the timeout overflow of the stream query.
* [Fixed](https://github.com/ydb-platform/ydb/pull/37145) thread safety and parsing of the token expiration date in the IAM credentials provider.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/38033) CDC collection for `IsBuildInProgress` columns is disabled.
* [Fixed](https://github.com/ydb-platform/ydb/pull/38490) segfault during the update.
* [Fixed](https://github.com/ydb-platform/ydb/pull/38544) Shuffle Elimination with the pragma `HashJoinMode`.
* [Fixed](https://github.com/ydb-platform/ydb/pull/39337) duplication of rows in scan query when there are delivery issues.
* [Fixed](https://github.com/ydb-platform/ydb/pull/39687) access to HTTP endpoints of the viewer: only viewer/admin SID; database scope for database-only tokens.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/43068) local reading of CDC from YQL.
* [Fixed](https://github.com/ydb-platform/ydb/pull/44340) fast remote cancellation of requests in the query service.
...
* YQL instructions [`BATCH UPDATE`](./yql/reference/syntax/batch-update.md?version=v25.4) and [`BATCH DELETE FROM`](./yql/reference/syntax/batch-delete.md?version=v25.4) are available for bulk updating and deleting data in tables.
* The mechanism for executing write operations has changed significantly — now writing is done in streaming mode without fully materializing data on the Query Processor side before sending it to datashards, which improves performance for large write operations. The change applies to some scenarios; in certain cases (for example, tables with secondary indexes), the previous approach is still used. For general information about the execution pipeline, see the section [Query execution](./concepts/query_execution/index.md?version=v25.4).
* Execution of Lookup Join has been optimized: streaming mode is used without materializing one of the sides of the join, which reduces peak memory consumption, speeds up execution on large datasets, and removes previous limitations on the size of the join sides. See the description [Index lookup Join](./faq/yql.md?version=v25.4#index-lookup-join) and the syntax [of the operator `JOIN`](./yql/reference/syntax/select/join.md?version=v25.4).
* The ability to set access rights for [system views](./devops/observability/system-views.md?version=v25.4) of the cluster and databases has been added.
* For string tables, it is possible to configure [caching modes](./concepts/datamodel/table.md?version=v25.4#cache-modes) and a new mode `in_memory` that allows preloading table data into RAM, provided there is sufficient RAM available.
* For topic readers, a parameter [`availability-period`](./reference/ydb-cli/topic-consumer-add.md?version=v25.4) has been added, which allows extending the storage period for unacknowledged messages beyond the retention period.
* Partition-level metrics for topics and export to user shard quotas [are available](./reference/observability/metrics/index.md?version=v25.4#topics_partitions) for monitoring and observability.
* Queries with `LIMIT` in columnar tables are faster due to early limiting of the sample on storage nodes (for queries without sorting or with sorting by the primary key). The general syntax [`LIMIT` and `OFFSET`](./yql/reference/syntax/select/limit_offset.md?version=v25.4) in YQL.
* Columnar tables support the `Bool` type in schemas and queries — see [primitive YQL types](./yql/reference/types/primitive.md?version=v25.4#numeric).
* [The filterable vector index](./dev/vector-indexes.md?version=v25.4#filtered) correctly finds rows with new filter column values that were inserted into the table after the index was created.
* Streaming processing and data delivery are more tightly integrated into the core: [topic-to-table transfer](./concepts/transfer.md?version=v25.4), [streaming queries](./dev/streaming-query/index.md?version=v25.4) are available to users when 'EnableStreamingQueries' is enabled.
...
* Full vector search without an ANN index has been sped up through pushdown (vector search, KNN UDF) — see [vector search](./concepts/query_execution/vector_search.md?version=v25.4) and [KNN module](./yql/reference/udf/list/knn.md?version=v25.4).
...
* [Fixed the behavior](https://github.com/ydb-platform/ydb/pull/18121) that caused `ALTER TABLE` to fail for tables with a vector index.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/20519) an error that caused VDisk to hang in local recovery after an error `ChunkRead`.
* [Eliminated](https://github.com/ydb-platform/ydb/pull/18924) the appearance of phantom VDisks due to races between create and delete group operations.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/25145) [an error](https://github.com/ydb-platform/ydb/issues/20866) that caused an error when changing the follower of a table
* A number of errors related to [changefeed](./concepts/glossary.md#changefeed) have been fixed:
  * [Fixed](https://github.com/ydb-platform/ydb/pull/25689) [error](https://github.com/ydb-platform/ydb/issues/25524) that could cause the import of a table with a Utf8 key and enabled changefeed to fail
  * [Fixed](https://github.com/ydb-platform/ydb/pull/25453) [error](https://github.com/ydb-platform/ydb/issues/25454) when importing a table without change streams could fail due to incorrect changefeed file search
* [Fixed](https://github.com/ydb-platform/ydb/pull/26069) [error](https://github.com/ydb-platform/ydb/issues/25869) that could lead to failures during UPSERT operations in columnar tables
...
* A number of issues related to cluster initialization have been fixed:
  ...
  * [Fixed](https://github.com/ydb-platform/ydb/pull/28886) [issue](https://github.com/ydb-platform/ydb/issues/27228) that made it impossible to create new databases immediately after cluster deployment for several minutes
* [Fixed](https://github.com/ydb-platform/ydb/pull/28655) [error](https://github.com/ydb-platform/ydb/issues/28510) that could cause a race condition and clients to receive error `Could not find correct token validator` if recently issued tokens were used before the `LoginProvider` state was updated
* [Fixed](https://github.com/ydb-platform/ydb/pull/29940) [error](https://github.com/ydb-platform/ydb/issues/29903) where a named expression containing another named expression led to an incorrect backup of `VIEW`
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/24265) an error in [Workload Manager](./dev/resource-consumption-management.md) that could cause CPU consumption by columnar tables to exceed the set limits.
...
* [Added](https://github.com/ydb-platform/ydb/pull/20982) a [new protocol](https://github.com/ydb-platform/ydb/issues/11064) in [Node Broker](./concepts/glossary.md#node-broker), eliminating network traffic spikes in large clusters (more than 1000 servers) associated with broadcasting node information.

#### YDB UI

* [Fixed](https://github.com/ydb-platform/ydb/pull/17839) a [bug](https://github.com/ydb-platform/ydb/issues/15230) that caused not all tablets to be displayed in the Tablets tab in the diagnostics section.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/38425) a vulnerability in [LDAP authentication](./security/authentication.md): knowing the username and password of any LDAP user (including those not in the group with access to {{ ydb-short-name }}), it was possible to bypass the group membership check and gain access to the cluster (injection into the LDAP search filter; special characters are now escaped according to RFC 2254).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/22203) a bug that caused tablets to get stuck when there was a lack of memory on the nodes. Now tablets will automatically start as soon as sufficient resources become available on any of the nodes.
...
* In backup and restore operations, the range of supported objects has been expanded. Enabled by setting the flags specified in parentheses:
...
* [Implemented](https://github.com/ydb-platform/ydb/issues/9748) the ability to switch functional flags during server operation {{ ydb-short-name }}. Flags that do not have the `(RequireRestart) = true` parameter specified in the [proto file](https://github.com/ydb-platform/ydb/blob/main/ydb/core/protos/feature_flags.proto#L60) will be applied without restarting the cluster.
...
* [Implemented](https://github.com/ydb-platform/ydb/pull/12567) the preservation of optimistic locks in memory during a smooth restart of data shards, which should reduce the number of ABORTED errors due to lock loss during table balancing between nodes.
...
* The query editor [has been updated with](https://github.com/ydb-platform/ydb-embedded-ui/pull/1974) support for partial loading of results — display starts immediately upon receiving the first fragment from the server without waiting for the query to complete fully. This allows for faster retrieval of results.
...
* A "Operations" tab has been added to the database page, allowing you to view a list of operations and cancel them.
...
* [Enabled](https://github.com/ydb-platform/ydb/pull/19440) loadable B-Tree indexes by default instead of non-loadable SST indexes, which reduces memory consumption when storing "cold" data.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/11059) an error causing long (seconds) data reading during frequent table resharding operations.
* [Fixed](https://github.com/ydb-platform/ydb/pull/9723) an error reading from asynchronous replicas that caused a failure.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/10460) an error of inconsistent reading from a topic when trying to explicitly acknowledge a message read within a transaction. Now, the user will receive an error when trying to acknowledge the message.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/16635) an error executing the expression `RETURNING` in `INSERT INTO` and `UPSERT INTO` queries.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/18378) an issue where Hive subscribers were started in data centers without running database nodes after the cluster version was updated.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/18905) an error that caused a segmentation fault when executing a healthcheck request and disabling a cluster node simultaneously.
* [Fixed](https://github.com/ydb-platform/ydb/pull/18899) a failure in [string table partitioning](./concepts/datamodel/table.md?version=v25.1#partitioning_row_table) when selecting a partitioned key from access samples containing mixed operations with the full key and key prefix (e.g., exact read or range read).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/17123) a rare freeze of client applications during transaction commit execution when partition deletion occurred before quota update for writing to the topic.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/15160) an issue that caused a transaction to hang when a user was simultaneously managing a topic and moving a PQ tablet to another node.
...
* Added the ability to register a [database node](./concepts/glossary.md#database-node) using a certificate. A flag for `AuthorizeByCertificate` using a certificate during registration has been added to [Node Broker](./concepts/glossary.md#node-broker).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/13918) an error in restoring from a backup copy that was created during automatic table splitting.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/12448) [an error](https://github.com/ydb-platform/ydb/issues/12443) that caused [Health Check](./reference/ydb-sdk/health-check-api) not to report time synchronization issues.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/13948) issues with consistency related to caching remote ranges.
...
* Added support for [views (VIEW)](https://ydb.tech/docs/en/concepts/datamodel/view), which can be enabled by the cluster administrator using the `enable_views` setting in [dynamic configuration](./devops/configuration-management/configuration-v1/dynamic-config#obnovlenie-dinamicheskoj-konfiguracii).
...
* [Added](https://github.com/ydb-platform/ydb-embedded-ui/pull/1291) a row limit setting for the query editor and display if the query results exceed the limit.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/8847) a [error](https://github.com/ydb-platform/ydb/issues/6985) in the display of the time at which a set of transactions is scheduled to be executed (planned step).
* [Fixed](https://github.com/ydb-platform/ydb/pull/9161) a [problem](https://github.com/ydb-platform/ydb/issues/8942) with interrupting blue-green deployment in large clusters due to frequent updates of the node list.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/9841) a [error](https://github.com/ydb-platform/ydb/issues/9797) in the EvWrite API that led to incorrect memory release.
* [Fixed](https://github.com/ydb-platform/ydb/pull/10698) a [problem](https://github.com/ydb-platform/ydb/issues/10674) with volatile transactions hanging after restart.
...
* Added retrieval of nested groups from the [LDAP server](./security/authentication.md#ldap), improved host parsing in the [LDAP configuration](reference/configuration/auth_config.md#ldap-auth-config), and added an option to disable built-in login and password authentication.
...
* Added graphs for monitoring the main indicators of cluster operation {{ ydb-short-name }}.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3917) the error of possible queue overflow, [Change Data Capture](./dev/cdc.md) reserves the capacity of the change queue during the initial scan.
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/3074) a rare error that could lead to a violation of data integrity guarantees during concurrent writing and reading of data by a certain key.
* [Fixed](https://github.com/ydb-platform/ydb/pull/4343) the problem that caused read replicas to stop processing queries.
* [Fixed](https://github.com/ydb-platform/ydb/pull/4979) a rare error that could lead to an abnormal termination of database processes when there were unconfirmed transactions on a table at the time of its renaming.
...
* [Optimized](https://github.com/ydb-platform/ydb/pull/2083) the use of resources by SchemeBoard replicas. The greatest effect is noticeable when modifying the metadata of tables with a large number of partitions.
...
* [Resolved](https://github.com/ydb-platform/ydb/pull/3165) a race condition between table merge processes and garbage collection, which could cause garbage collection to fail due to invariant violations and, as a result, an abnormal termination of the server process `ydbd`.
* [Fixed](https://github.com/ydb-platform/ydb/pull/2696) an error in Blob Storage, which could cause information about changes in the storage group composition to not be delivered in a timely manner to certain cluster nodes. As a result, in rare cases, read and write operations on data stored in the affected group could be blocked, requiring manual intervention by an administrator.
* [Fixed](https://github.com/ydb-platform/ydb/pull/3002) an error in Blob Storage that could prevent data storage nodes from starting even with a correct configuration. The error occurred in systems with the experimental "blob depot" feature explicitly enabled (this feature is disabled by default).
...
* [Fixed](https://github.com/ydb-platform/ydb/pull/2651) an issue causing the `ydbd` process to crash due to an erroneous write session state in the topic.
* [Fixed](https://github.com/ydb-platform/ydb/pull/3587) an error in the metric displaying the number of partitions in a topic, which previously showed an incorrect value.
* [Resolved](https://github.com/ydb-platform/ydb/pull/2126) memory leaks that occurred when copying topic data between clusters {{ ydb-short-name }}. They could lead to server processes `ydbd` terminating due to exhaustion of available RAM.
...
* Tablet balancing during node overload has been improved.

### Bug fixes

* Fixed an error that could cause snapshots unknown to coordinators to be blocked by reading iterators.
...
* The check that was triggering `VERIFY failed: SendResult(): requirement ChunksLimiter.Take(sendBytes) failed` has been fixed.
* `ALTER TABLE` has been fixed according to `TTL` for columnar tables.
* `FeatureFlag` has been implemented, which allows enabling/disabling work with `CS` and `DS`.
* The discrepancy in coordinator time between 23-2 and 23-3 has been fixed (50 ms).
* A bug has been fixed where the handle `storage` returned extra groups when the parameter `node_id` was in `viewer backend`.
* The `usage` filter has been added to `/storage` in `viewer backend`.
* A bug in Storage v2 has been fixed, where an incorrect number was returned in `Degraded`.
* The issue with subscription cancellation from sessions in iterator reads upon tablet restart has been fixed.
* A bug has been fixed where, during a rolling restart, when going through the balancer, `healthcheck` was flashing with storage alerts.
* The `cpu usage` metrics in ydb have been updated.
* The ignoring of `NULL` when specifying `NOT NULL` in the table schema has been fixed.
* The output of records about `DDL` operations to the general log has been implemented.
* A prohibition has been implemented for the `ydb table attribute add/drop` command to work with any objects other than tables.
* `CloseOnIdle` has been disabled for `interconnect`.
* The doubling of reading speed in the UI has been fixed.
* A bug has been fixed where data could be lost on `block-4-2`.
* A check for the topic name has been added.
* A possible `deadlock` in the actor system has been fixed.
* Test `KqpScanArrowInChanels::AllTypesColumns` has been fixed.
* Test `KqpScan::SqlInParameter` has been fixed.
* Concurrency issues for OLAP queries have been fixed.
* The insertion of `ClickBench parquet` has been fixed.
* The missing call of `CheckChangesQueueOverflow` in the general `CheckDataTxReject` has been added.
* A bug with returning an empty status when calling `ReadRows API` has been fixed.
* The incorrect retry of export in the final stage has been fixed.
* The issue with an infinite quota on the number of records in the CDC topic has been fixed.
* A bug with importing columns `string` and `parquet` into the `string` OLAP column has been fixed.
* The crash of `KqpOlapTypes.Timestamp` under tsan has been fixed.
...
* A bug has been fixed where an incorrect value of `ExpectedSerial` could be stored in Pdisks.
* A bug has been fixed where database nodes were crashing due to `segfault` in the S3 actor.
...
* A separate issue/error code has been added in case of exhausting available space.
* Bug `GRPC_LIBRARY Assertion failed` has been fixed.
* A bug has been fixed where scanning queries with a secondary index read returned an empty result.
...
* A scheme for seamless tablet updates for `QueryService` has been implemented.
* A bug has been fixed where `DELETE` returned data but did not delete it.
* A bug in the operation of `DELETE ON` in `query service` has been fixed.
* The unexpected disabling of batching in the default schema settings has been fixed.
* The triggering check `VERIFY failed: MoveUserTable(): requirement move.ReMapIndexesSize() == newTableInfo->Indexes.size()` has been fixed.
* The default timeout for grpc-streaming has been increased.
* Unused messages and methods from `QueryService` have been excluded.
* Sorting by `Rack` in `/nodes` in `viewer backend` has been added.
...
* The status `UNAVAILABLE` should be used instead of `GENERIC_ERROR` in case of shard identification error.
* Support for `rope payload` in `TEvVGet` has been added.
* Ignoring of outdated events has been added.
...
* **_ (Experimentally) _** Iterator reads have been implemented. This functionality allows separating reads and computations from each other. Iterator reads allow date shards to increase the throughput of read queries. To enable this functionality, add `enable_kqp_data_query_source_read: true` to the `table_service_config` section in the [configuration file](reference/configuration/index.md).
...
* A bug that leads to a full scan when searching by the primary key prefix via `LIKE` has been fixed.
...
* A bug causing operations to hang from non-existent iterators has been fixed.
* Bugs in reading a range on the `NOT NULL` column have been fixed.
* A bug causing VDisk replication to hang has been fixed.
...
* [Initial table scanning](concepts/cdc.md#initial-scan) when creating a CDC change stream has been added. Now it is possible to export all data that exists at the time of stream creation.
...
* Data transfer formats between query execution stages have been improved, which has accelerated SELECT by 10% on queries with parameters and up to 30% on write operations.
* [Automatic configuration](reference/configuration/index.md) of actor system pools depending on their load has been added. This improves performance by more efficient sharing of CPU resources.
* The logic of applying predicates has been optimized — applying restrictions using OR and IN with parameters is automatically moved to the DataShard side.
* (Experimentally) For scanning queries, the ability to efficiently search for individual rows using a primary key or secondary indexes has been implemented, which can significantly improve performance in many cases. As in regular queries, to use a secondary index, you must explicitly specify its name in the query text using the `VIEW` keyword.
...
* Fixed statistics calculation when MVCC is enabled.
* Fixed errors with backups.
* Fixed a race condition during table split and deletion with CDC.
...
* Query Processing:
  ...
  * Query Processor 2.0 has been added — a new subsystem for executing OLTP queries with significant improvements over the previous version.
