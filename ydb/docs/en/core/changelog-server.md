...
* Implemented [BATCH UPDATE](../core/yql/reference/syntax/batch-update.md) and [BATCH DELETE](../core/yql/reference/syntax/batch-delete.md) statements, allowing the application of changes to large row-oriented tables outside of transactional constraints. This mode is enabled by setting the `enable_batch_updates` flag in the cluster configuration.
