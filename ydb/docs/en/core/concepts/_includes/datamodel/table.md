...
{{ ydb-short-name }} allows grouping columns in a table to optimize their storage and usage. The column group mechanism improves performance for partial row reads by separating table columns into multiple storage groups. The most common use case involves storing rarely used attributes in a separate column group (potentially with compression and on slower storage devices).
...
A column group may contain any number of columns from its table, including zero. Each table column belongs to one and only one column group (column groups don't overlap). Column assignments to groups are established during table creation but can be modified later.
...
Column group attributes are set during table creation (including explicit definitions for the primary group) and can be modified later. Storage attribute changes aren't immediately applied to existing data; instead, they take effect during subsequent background [LSM compaction](../../glossary.md#compaction).
