...
* For eviction to private S3-compatible storage, configure the external data source with `AUTH_METHOD="AWS"` and reference secret schema objects via `AWS_ACCESS_KEY_ID_SECRET_NAME` and `AWS_SECRET_ACCESS_KEY_SECRET_NAME`.
