...
* For eviction to private S3-compatible storage, configure an [external data source](../datamodel/external_data_source.md) with `AUTH_METHOD="AWS"` and use [secrets](../datamodel/secrets.md) in `AWS_ACCESS_KEY_ID_SECRET_NAME` and `AWS_SECRET_ACCESS_KEY_SECRET_NAME`.
