...
The simplest way is to create a copy of a table (or multiple tables) within the same cluster using the [`{{ ydb-cli }} tools copy`](../reference/ydb-cli/tools-copy.md) command. The copy is created atomically from a consistent snapshot and uses a copy-on-write mechanism, so the operation is fast.
...
To restore, the entire chain is needed: first apply the full copy, then all increments sequentially. Restoration is performed to the state at the time of the last increment in the chain.
...
This mechanism is used to restore cluster metadata when [restoring from database backups](#full-backup) is technically possible but not suitable in terms of time or scope of work. A typical scenario is when the total volume of databases in the cluster is large due to their number, the size of individual databases, or a combination of both factors; a full `import/restore` of all data to a new cluster in such a case leads to prolonged downtime. In this scenario, you can restore system tablets and bring the cluster back to an operational state without performing a bulk restore of user data on the new cluster.

If the database volume allows for a standard restore, use [export/import](#s3) or [dump/restore](#dump) first. Backup of system tablets should be used as a special mechanism for situations where you need to restore cluster metadata and reduce the scope of restore operations.

{% note info %}

For practical instructions on enabling and restoring, see [recipes for backing up system tablets](../recipes/system-tablet-backup/index.md).

{% endnote %}

{% note warning %}

Backups of different system tablets are created independently of each other and are not consistent with each other. After restoration, the state of tablets may become inconsistent, which may negatively affect cluster operation.

{% endnote %}

### Operating principle {#system-tablet-backup-how-it-works}

Backup consists of two components:

- **State snapshot (snapshot)** — at each startup, the tablet scans all its tables and writes its full state to the backup, including the data schema. Scanning is performed based on a state snapshot and does not block tablet operation.
- **Change log (changelog)** — with each change to data or schema, the tablet asynchronously writes the change to the log in parallel with writing to the distributed storage. When the log size exceeds the state snapshot size, the tablet automatically takes a new snapshot.
