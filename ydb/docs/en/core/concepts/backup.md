...
{{ ydb-short-name }} ensures data safety during hardware failures through [replication and fault tolerance](topology.md#cluster-config). However, replication does not protect against **logical errors**: an accidental `DROP TABLE`, an erroneous mass `UPDATE`, or `DELETE` will be reproduced on all replicas. To protect against such scenarios, **backup** is required — a separate copy of data that can be restored from.
...
If the volume of databases allows standard restoration, use [export/import](#s3) or [dump/restore](#dump) first. System tablet backup should be used as a special mechanism for situations where you need to restore exactly the cluster metadata and reduce the volume of recovery operations.
...
Backups of different system tablets are created independently of each other and are not consistent with each other. After restoration, the state of the tablets may be inconsistent, which can negatively affect cluster operation.
...
- **State snapshot** — on each startup, the tablet scans all its tables and writes its full state to a backup, including the data schema. The scan is performed based on a state snapshot and does not block the tablet's operation.
- **Change log** — on each change of data or schema, the tablet asynchronously writes the change to the log in parallel with writing to the distributed storage. When the log size exceeds the state snapshot size, the tablet automatically takes a new snapshot.
...
|| [Backup of system tablets](#system-tablet-backup) | Local file system of cluster hosts | Yes | Recovery of cluster metadata in emergency situations ||
|#
...
- [Recipes for backing up system tablets](../recipes/system-tablet-backup/index.md) — enabling and recovery
- [Backup and recovery](../devops/backup-and-recovery/index.md) — practical guide
