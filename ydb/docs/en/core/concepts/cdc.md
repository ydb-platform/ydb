...
* `userIdentity`: Includes user information. Contains `type` (`"User"` or `"Service"`) and `principalId` (user's [SID](glossary.md#sid-access-sid)). If record is deleted by the TTL process, `type` is `"Service"` and `principalId` is `"dynamodb.amazonaws.com"`.
