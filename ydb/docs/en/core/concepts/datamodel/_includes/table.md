...
When a query is executed based on a column (or columns) for which an index is created, the DBMS can use that index to quickly find the corresponding rows, avoiding a full scan of all data. For example, if you have an index on the "author" column and you are looking for all articles written by "Gray", the DBMS uses the index to quickly find all rows with that surname.
...
Automatic partitioning by load. If a partition consumes more than 50% of the CPU for a few tens of seconds, it is enqueued for splitting. If the total load on two or more adjacent partitions is less than 35% of a single CPU core within an hour, they are enqueued for merging.
...
* To handle read queries from followers without affecting modifying queries running on a partition. This can be useful both for isolating different scenarios and for increasing the partition bandwidth.
