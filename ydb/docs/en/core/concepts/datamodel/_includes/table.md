...
[An index](%E2%9F%A6U1%E2%9F%A7) is a data structure that improves the speed of data retrieval operations based on one or several columns. It's analogous to an index in a book: instead of scanning every page of the book to find a specific chapter, you can refer to the index at the back of the book and quickly navigate to the desired page.

When a query is executed based on a column (or columns) on which an index is created, the DBMS can use that index to quickly find the corresponding rows, avoiding a full scan of all data. For instance, if you have an index on the "author" column and you're looking for articles written by "Gray," the DBMS leverages this index to quickly identify all rows associated with that surname.
...
Automatic partitioning by load. If a partition consumes more than 50% of the CPU for a few dozens of seconds, it is enqueued for splitting. If the total load on two or more adjacent partitions uses less than 35% of a single CPU core within an hour, they are enqueued for merging.
...
You can enable running read replicas for each partition of the table in the table data schema. The read replicas are typically accessed without leaving the data center network, which ensures response delays in milliseconds.
...
The internal state of each replica is restored exactly from the leader state and is fully consistent.
