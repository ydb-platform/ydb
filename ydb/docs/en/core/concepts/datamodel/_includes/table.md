...
When making a decision to split a partition based on load or to merge several partitions based on load, YDB considers the CPU load both on the leader of the given partition and all its followers (read-only replicas). For splitting, even if only one of the followers (or the leader) for the given partition is experiencing a high CPU load above the splitting threshold, the entire partition will be split into two. The median key for splitting the partition will be calculated based on the current load on this specific follower (or the leader), which is experiencing a high CPU load. For merging, the CPU load on the follower (or the leader), which is experiencing the highest load, is considered the effective CPU load for the given partition. If the combined effective CPU load for several adjacent partitions is below the merging threshold, these partitions are merged into one.
