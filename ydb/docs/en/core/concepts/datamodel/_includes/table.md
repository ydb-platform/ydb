...
Automatic partitioning by load. If a partition consumes more than 50% of the CPU for a few tens of seconds, it is enqueued for splitting. If the total load on two or more adjacent partitions uses less than 35% of a single CPU core within an hour, they are enqueued for merging.
