...
A backup collection organizes full and incremental backups of selected [row tables](table.md#row-oriented-tables) into manageable chains. It allows you to **restore data to the state of the latest backup in the chain**, providing protection against accidental data loss — erroneous deletions or modifications.
...
For **disaster recovery** and protection against failures of the entire cluster, regularly export backup collections to external storage (S3-compatible storage or file system) using [export/import operations](../../reference/ydb-cli/export-import/index.md).
...
1. **Import from external storage** (if needed): If backups were exported, import the full backup and all incremental backups up to the desired restore point.
2. **Performing restore**: Run `RESTORE collection_name` to restore all tables from the backup collection. The system applies the full backup and all incremental backups sequentially to reach the latest backup point.

{% note warning %}

The restore operation will fail if any of the tables being restored already exist at the same path. Rename or delete conflicting tables before restoring.
