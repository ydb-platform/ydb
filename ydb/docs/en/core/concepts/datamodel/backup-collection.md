...
A backup collection organizes full and incremental backups of selected [row tables](table.md#row-oriented-tables) into managed chains. It allows you to **restore data to the state of the latest backup in the chain**, providing protection against accidental data loss — erroneous deletions or modifications.

{% note info %}

For practical instructions on creating and managing backup collections, see the [Backup collections](../../devops/backup-and-recovery/index.md#backup-collections) section of the Backup and Restore documentation.

{% endnote %}

## Overview

Backup collections address typical backup tasks for production workloads:
...
- **Consistent recovery**: All tables in the collection are created from a single global snapshot, ensuring referential integrity between tables during recovery.
- **Chain recovery**: Restore to the state of the **latest** backup in the current chain — see [`RESTORE`](../../yql/reference/syntax/restore-backup-collection.md).
...
- **[Incremental backup](../glossary.md#backup)**: Captures only changes (inserts, updates, deletes) since the previous backup. Requires the entire backup chain for recovery.
...
- **Row tables only**: [Columnar tables](table.md#column-oriented-tables) are not supported.
...
- **No partial recovery**: You cannot restore individual tables from a collection; the entire collection is restored as a whole.
...
Backup collections use a copy-on-write mechanism combined with [change data capture](../cdc.md) for efficient incremental backups. This section explains how the components work together.
...
- Creates **change data capture** for each table to track subsequent modifications.
...
- Uses a **distributed transaction** to read change data capture from all tables at a consistent point, ensuring referential integrity of the entire collection.
- Reads accumulated changes from change streams created during a full backup and accumulating changes **since the previous backup (full or incremental)**.
...
Schema changes (ALTER TABLE) to tables in a backup collection are not tracked by incremental backups. If you need to change the schema of a table with a backup, create a new full backup after the schema change so that the backup chain reflects the new structure.
...
- [`ydb tools dump`](../../reference/ydb-cli/export-import/tools-dump.md) for file system on a local computer.
...
Restoration returns data to the state of the latest backup in the chain that is in the cluster. To restore to an earlier state, import only the required chain prefix from external storage — see [Import and restore](../../recipes/backup-collections/importing-and-restoring.md).

### Restoration process

1. **Import from external storage** (if necessary): If backups were exported, import the full backup and all incremental backups up to the desired recovery point.
2. **Perform restoration**: Run `RESTORE collection_name` to restore all tables from the backup collection. The system applies the full backup and all incremental backups sequentially to reach the latest backup point.
...
During the restore operation, target tables are unavailable for modifications. Partially restored data may be visible to read loads. Schedule the restore for a maintenance window or disable application access to the affected tables until the operation completes.
