...
- **One collection per table**: A table can only belong to one backup collection at a time. To include a table in a different collection, run [`DROP BACKUP COLLECTION`](../../yql/reference/syntax/drop-backup-collection.md) for the current collection and create a new one with the desired set of tables.
...
- [`ydb tools dump`](../../reference/ydb-cli/export-import/tools-dump.md) for filesystem storage.
...
All backup and restore operations run asynchronously, allowing normal database operations to continue. Monitor progress using [`ydb operation list incbackup`](../../reference/ydb-cli/operation-list.md).
