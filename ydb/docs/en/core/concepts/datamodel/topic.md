...
The recommended maximum number of `producer_id` is up to 100,000 per partition over the last 14 days. As a guideline, this is approximately 5 new `producer_id` per minute around the clock.
...
In {{ ydb-short-name }}, you can write data in such a way that messages from one source arrive at the same application instance. To achieve this, messages from each source are written with their unique source identifiers (`producer_id`), and a message sequence number from the source (`seqno`) is used to protect against duplicates. In {{ ydb-short-name }}, messages with the same `producer_id` go to the same partition. When reading from a topic, each reader instance serves its own subset of partitions, eliminating the need for synchronization between instances. For example, this approach can ensure that messages about transactions for the same account always go to the same partition and are processed by the application instance associated with that partition.
...
This parameter sets the upper limit of in-flight data — the amount of data for a single partition that has already been sent by the server in read responses but not yet acknowledged by a commit of the offset.
...
A reader can have the "important" attribute. The presence of this attribute means that messages in the topic will not be deleted until the reader reads and acknowledges them. This attribute can be set for the most critical readers that must process all data even during long idle periods.

{% note warning %}

Since a long idle period of an important reader may lead to all available storage space being used by unread messages, it is necessary to monitor the read lag of important readers.
...
This option allows you to extend the retention period of messages in the topic from the [retention period](#retention-time) up to the specified availability time, if the reader does not acknowledge processing. Unlike the ["important"](#important-consumer) reader attribute, this parameter limits the maximum age of messages that will be stored in the topic. If the option is not set or all readers acknowledge processing without significant delay (less than the [retention period](#retention-time) of the topic), then the data will be deleted according to the usual rules.
...
There is also limited support for the Kafka API protocol version 3.4.0 (see [Working with Kafka API](../../reference/kafka-api/index.md)).
...
Nevertheless, it is recommended to choose the operating mode with transactions, taking into account the specifics of transactional work with topics: data is published at the moment of transaction commit. That is, if the transaction is long-running, the data will become visible only after a significant amount of time.
