...
* At-least-once guarantees when subscribers read messages.
...
Messages can contain user attributes in the "key-value" format. They are returned together with the message body when read. User attributes allow the reader to decide whether to process the message without unpacking the message body. Message attributes are set when initializing a write session. This means that all messages written within a single write session will have the same attributes when read.
...
Autopartitioning is disabled. In this case, the number of partitions remains unchanged, and automatic scaling does not occur.
...
Autopartitioning is enabled on the topic in the 'up' direction, meaning that when the write speed increases, the number of partitions increases. When the write speed decreases, the number of partitions remains unchanged.
...
Autopartitioning on the topic is paused. Automatic increase in the number of partitions does not occur. If necessary, you can re-enable the partition increase mode.

Examples of YQL queries for switching the topic to different autopartitioning modes can be found [here](../../yql/reference/syntax/alter-topic.md#autopartitioning).

### Limitations {#autopartitioning_constraints}

The following limitations apply when using autopartitioning:
...
3. Autopartitioning cannot be enabled on a topic with the local storage mode.
...
Consider a financial application whose task is to calculate the user's account balance and allow or deny debiting of funds.
...
For correct balance calculation, the order of message processing is important. If a user first deposits and then makes a purchase, the messages with information about these operations must be processed by the application in the same sequence. Otherwise, a business logic error may occur, and, for example, the application will reject the purchase due to insufficient funds. Message queues have mechanisms for guaranteed delivery order, but they cannot ensure the order of messages within a single queue for arbitrary data volumes.
...
For some tasks, the order of message processing is not critical. For example, sometimes it is important just to deliver data, and the storage system will handle ordering.
...
A read session is a client connection to a topic for receiving messages on behalf of a reader. Read sessions work through the Topic API. A single reader can establish multiple read sessions: in this case, topic partitions are distributed among these sessions. To work with read sessions, it is recommended to use the {{ ydb-short-name }} SDK (see [Working with topics](../../reference/ydb-sdk/topic.md)).
...
A shared (common) reader in {{ ydb-short-name }} is a reading model where one or more messages from a topic are assigned to a reader, rather than an entire topic partition. This allows a single partition to be processed simultaneously by a large number of consumers.
...
A message is locked for a consumer for a certain period of time. If during this time the consumer has not reported completion of message processing and has not extended its processing time, it means that the consumer failed to process the message, and the message becomes available for reading and will be read again. The processing time is configured in the reader parameters, but can also be set for each request to read messages.
...
Depending on the configuration of the shared (common) reader, reading from a topic can be performed either with message ordering preserved or without it. Message order is preserved among messages with the same message-group-id, which can be set for each message when it is written. If the shared (common) reader is configured to read with message ordering preserved, then at any given time, such a reader delivers no more than one message from a single message-group-id to consumers for processing.

When placing a message in the queue, you can specify a delay before the message is first delivered for processing. The order of messages is determined by the time the message was added to the topic — if reading is performed with message ordering preserved, a delayed message will suspend the delivery for processing of all subsequent messages with the same message-group-id.
...
### Limiting reading from a single partition {#partition-max-in-flight-bytes}

The `partition_max_in_flight_bytes` parameter can be set when creating a reading session.

This parameter sets the upper limit of in-flight — the amount of data for a single partition that has already been delivered by the server in read responses but not yet confirmed by a commit of the offset.

- `0` — no separate in-flight limit per partition is applied.
- positive value — when the limit is reached, the server temporarily stops delivering new portions for this partition until the client commits and reduces in-flight.

A value that is too small, with slow processing or infrequent commits, may cause reading pauses for the partition. In such a case, increase the limit or perform commits more often.
...
Since a long idle period of an important consumer can lead to using all available storage space for unread messages, it is necessary to monitor the read lag of important consumers.

{% endnote %}

### Message availability time for the consumer {#availability-period-consumer}

A time availability period can be set for the consumer, during which messages from the topic that have not yet been processed will be available to them.
...
If the option is not set or all consumers acknowledge processing without significant delay (less than the topic's [retention time](#retention-time)), then the data will be deleted according to the usual rules.
...
Additionally, the Kafka API version 3.4.0 (see [{#T}](../../reference/kafka-api/index.md)) and Amazon SQS API (see [{#T}](../../reference/sqs-api/index.md)) protocols are partially supported.
...
When writing transactionally to a topic, data is stored outside the partition until commit, and then published (becomes visible) at the moment of transaction commit. In this case, the data will be appended to the end of the partition at sequential offsets. Visibility of own changes in topics within transactions involving topics is not supported.
