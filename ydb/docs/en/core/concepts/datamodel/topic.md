...
- **Inactive.** You can only read from them. Inactive partitions appear after a partition split when [auto-partitioning](#autopartitioning) is enabled. Inactive partitions are automatically deleted when all messages from such a partition have been deleted after the retention period expires.
...
On first use, the source identifier `producer_id` is bound to a topic [partition](#partitioning) using a round-robin algorithm, and all messages with this `producer_id` go to the same partition. The binding is removed if no new messages using this source identifier appear within 14 days.
...
All messages from a single source have a sequence number, [`sequence number`](#seqno), used for deduplication. The message sequence number must increase monotonically within a pair `topic`, `source`. When the server receives a message with a sequence number less than or equal to the maximum recorded for the pair `topic`, `source`, the message will be skipped as a duplicate. Gaps in the sequence of message numbers are allowed. Message sequence numbers must be unique only within a pair `topic`, `source`.
...
This parameter sets the upper limit of in-flight data — the volume of data for one partition that has already been sent by the server in read responses but has not yet been confirmed by a commit of the offset.

- `0` — no separate in-flight limit per partition is applied.
- a positive value — when the limit is reached, the server temporarily stops delivering new portions for this partition until the client commits and reduces in-flight.
...
Since a long downtime of an important reader can lead to the use of all available storage space by unread messages, it is necessary to monitor the read lag of important readers.

{% endnote %}

### Message availability time for a reader {#availability-period-consumer}

A reader can be assigned an availability time period during which messages from the topic that it has not yet processed will be available to it.
...
If the option is not set or all readers acknowledge processing without a significant delay (less than the topic's [retention time](#retention-time)), then the data will be deleted according to the usual rules.
