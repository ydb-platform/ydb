...
* Guarantees of sequential message processing ( [FIFO](https://en.wikipedia.org/wiki/Message_queue)) for messages published with the same [source ID](#producer-id).
...
3. Thus, the guarantees of exactly-once write and read order continue to hold for a specific [source ID (producer-id)](#producer-id).
...
A read session is a client connection to a topic for receiving messages on behalf of a reader. Read sessions work via the Topic API. One reader can establish multiple read sessions: in this case, topic partitions are distributed among these sessions. To work with read sessions, it is recommended to use the {{ ydb-short-name }} SDK (see [Working with topics](../../reference/ydb-sdk/topic.md)).
...
Since a long idle period of an important consumer may lead to using all available data storage space with unread messages, you need to monitor the read lag of important consumers.
...
Kafka API version 3.4.0 (see [{#T}](../../reference/kafka-api/index.md)) and Amazon SQS API (see [{#T}](../../reference/sqs-api/index.md)) are also partially supported.
