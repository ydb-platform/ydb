...
A {{ ydb-short-name }} topic has the following properties:
...
The topic has autopartitioning set to 'up', meaning that when the write speed increases, the number of partitions increases. When the write speed decreases, the number of partitions remains unchanged.
...
When multiple application instances read messages from the stream, one instance may receive the deposit message and another the withdrawal message. In this case, there is no instance that is guaranteed to have the correct balance information. To solve this problem, you can store data in a DBMS, exchange information between application instances, build a distributed cache, and so on.
...
A read session is a client connection to a topic for receiving messages on behalf of a reader. Read sessions work via the Topic API. One reader can establish multiple read sessions: in this case, the topic partitions are distributed among these sessions. To work with read sessions, it is recommended to use the {{ ydb-short-name }} SDK (see [Working with topics](../../reference/ydb-sdk/topic.md)).

In {{ ydb-short-name }}, readers can be of the following types:

- [**streaming**](#streaming-consumer) — a reader type where a partition is exclusively assigned to one consumer, and all messages from this partition are processed by that consumer.
- [**shared**](#shared-consumer) — a reader type that allows multiple consumers to jointly process messages from a single topic without exclusive assignment of partitions to each of them.

## Streaming reader {#streaming-consumer}

A streaming reader is designed for streaming data processing from a topic. In this mode, multiple consumer application instances read messages in parallel and process them as they arrive. Typical scenarios include collecting and analyzing application logs, building real-time aggregates, and replicating events to external systems.
...
A message is locked to a consumer for a certain period. If during this time the consumer has not reported completion of message processing and has not extended its processing time, it means the consumer could not process the message, and the message becomes available for reading and will be read again. The processing time is configured in the reader parameters, but can also be set for each message read request.
...
By default, the Dead letter policy is disabled (none). The number of message processing attempts before moving it to a DLQ (move) or deleting it (delete) is configured on the reader. If the message could not be moved to the DLQ, it is returned to the queue (as when the Dead letter policy is disabled), and on the next failed attempt to process the message, another attempt will be made to move the message to the DLQ. For example, if you delete the topic specified as the DLQ, messages after unsuccessful processing will be returned to the queue until the topic specified as the DLQ is created or another existing topic is specified.
...
### Per-partition read limit {#partition-max-in-flight-bytes}
...
A consumer can be assigned a time availability period during which messages from the topic that it has not yet processed will be available to it.
...
If the option is not set or all consumers acknowledge processing without significant delay (less than the topic's [retention time](#retention-time)), the data will be deleted according to the usual rules.
