...
* Guarantees at-least-once delivery when subscribers read messages.
* Guarantees exactly-once delivery when publishing messages (message deduplication).
* Guarantees sequential processing of messages ( [FIFO](https://en.wikipedia.org/wiki/Message_queue)) published with the same [source identifier](#producer-id).
...
- **Inactive.** Only reading is possible from them. Inactive partitions appear after a partition is split when [auto-partitioning](#autopartitioning) is enabled. Inactive partitions are automatically deleted when all messages from such a partition are deleted after the retention period expires.
...
During transmission, the writer application indicates that the message can be compressed using one of the supported codecs. The codec name is passed during writing, stored with the message, and returned on reading. Messages are compressed individually; batch compression is not supported. Data compression and decompression operations are performed on the reader and writer application side.
...
This parameter sets the upper limit of in-flight data — the volume of data for a single partition that has already been delivered by the server in read responses but not yet confirmed by an offset commit.
...
A value that is too small with slow processing or infrequent commits can cause reading pauses on a partition. In this case, increase the limit or perform commits more often.

### Important reader {#important-consumer}

A reader can have the "important" attribute. The presence of this attribute means that messages in the topic will not be deleted until the reader reads and confirms them. This attribute can be set for the most critical readers that must process all data even during a long downtime.

{% note warning %}

Since a long idle period of an important reader can lead to using all available data storage space with unread messages, it is necessary to monitor the read lag of important readers.

{% endnote %}

### Message availability time for the reader {#availability-period-consumer}

A time availability period can be set for the reader, during which messages from the topic that they have not yet processed will be available.
