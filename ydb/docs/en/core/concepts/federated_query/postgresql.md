...
| `json` | `Optional<Json>` ||
| `numeric(p,s)` | `Optional<Decimal(p,s)>` | `p` - total number of digits in the number, `s` - number of digits after the decimal point. Unconstrained numbers (`numeric` without parameters) are mapped into `<Optional<Decimal(35,0)>>`. `numeric` types with `p > 35` and `s < 0` are not supported. |
