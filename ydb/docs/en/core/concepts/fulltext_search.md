...
The core idea of a fulltext index is an **inverted index**: for each term, the index stores the list of documents (primary keys) that contain that term. This allows queries like “documents containing terms A and B” to avoid full table scans.
...
* fulltext matching via `FulltextMatch()` (query modes `Keywords` / `Query` / `Wildcard`, default operator `And` / `Or`)
* relevance ranking ([BM25](https://en.wikipedia.org/wiki/Okapi_BM25)) via `FulltextScore()` when using `fulltext_relevance`
