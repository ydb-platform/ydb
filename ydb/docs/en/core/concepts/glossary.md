...
Each local database table is stored using the [LSM](#lsm) data structure.

#### LSM {#lsm}

An **LSM**, or [Log-Structured Merge Tree](https://en.wikipedia.org/wiki/Log-structured_merge-tree), is a data structure designed to optimize write and read performance in storage systems and used in {{ ydb-short-name }} to store tablets data.
...
A **shared cache** is a memory buffer that stores data pages recently accessed and read from Blob Storage. By caching these pages, it reduces disk I/O operations and accelerates data retrieval, enhancing overall system performance.
