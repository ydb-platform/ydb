...
A local min_max skip index is a special case of a [local index](#local-index): a range filter that stores the minimum and maximum value of a single column for each data fragment and skips fragments whose range cannot satisfy the query predicate. For more information, see [min_max skip index](../dev/min_max-skip-index.md) and [local indexes](query_execution/local_indexes.md).
