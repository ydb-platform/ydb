...
**User token** is the internal result of validating an [authentication token](#auth-token). It contains the [SID](#access-sid) of the user and groups and is used during [authorization](../security/authorization.md). The user token is used and stored only on the [node](#node) that created it and is not passed to the client. For more details, see the article [{#T}](../security/caching-authentication-results.md).
