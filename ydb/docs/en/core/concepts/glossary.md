...
PDisk divides block device resources (bandwidth and free space) into equal shares called slots or [VSlots](#slot). These slots belong to VDisks from different [storage groups](#storage-group) that share the PDisk. In a cluster with non-uniform drive sizes one VDisk can occupy more than one slot.

PDisk partitions the block device into 128 MB chunks. Some chunks are reserved for system needs, particularly for the recovery log shared between PDisk service records and all VDisks. The remaining chunks can be allocated by VDisks. A VDisk receives a chunk quota proportional to the number of slots it occupies, but can exceed this quota if there are unused chunks available.

The PDisk scheduler ensures shared use of disk bandwidth. It aims to provide minimum latency for VDisks that comply with their allocated bandwidth. At the same time, a VDisk can receive more bandwidth if free resources are available.

#### VDisk {#vdisk}

**VDisk** or **Virtual disk** is a component that implements the persistence of [distributed storage](#distributed-storage) [LogoBlobs](#logoblob) on [PDisks](#pdisk). VDisk stores all its data on PDisks. One VDisk corresponds to one PDisk, but usually, several VDisks are linked to one PDisk. Unlike PDisk, which hides chunks and logs behind it, VDisk provides an interface at the LogoBlob and [LogoBlobID](#logoblobid) level, like writing LogoBlob, reading LogoBlobID data, and deleting a set of LogoBlob using a special command. VDisk is a member of a [storage group](#storage-group). VDisk itself is local, but the set of VDisks in a group provides reliable data storage. All VDisks in a group synchronize data with each other and replicate the data in case of loss. A set of VDisks in a storage group forms a distributed RAID.
