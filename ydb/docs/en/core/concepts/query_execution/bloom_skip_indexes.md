...
### How they differ from global secondary, vector, and fulltext indexes

* A global [secondary index](../glossary.md#secondary-index) is a separate index structure with its own key for data access.
* A [vector index](../../dev/vector-indexes.md) is a separate index structure for vector similarity search.
