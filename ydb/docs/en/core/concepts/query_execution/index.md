...
{{ ydb-short-name }} provides a unified interface for executing queries in the [YQL](../../yql/reference/index.md) language and a unified distributed engine for their execution. The same syntax and the same mechanisms allow access to data of various kinds: from rows in database tables to unbounded event streams in topics and data in external systems.
...
- [topics](#streaming) — queries to unbounded data streams: streaming queries and endless queries to topics.
...
[Federated queries](federated_query/index.md) allow accessing data located in external systems without moving that data into {{ ydb-full-name }}. Using YQL queries, you can read data from external DBMSs and object stores (S3), and also combine it with data in {{ ydb-short-name }} tables.
