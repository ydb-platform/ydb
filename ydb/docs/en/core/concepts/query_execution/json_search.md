...
* Without an index: scanning the table and applying the `JSON_EXISTS` or `JSON_VALUE` functions to each row. The approach is simple but does not scale well: the amount of scanning work grows with the table size.
...
* if the path leads to a scalar value (string, number, boolean, or `null`), a token «path + value» is additionally created.
...
Conceptually, the following tokens are indexed (regardless of the specifics of the internal representation):
...
* find rows by value in a path using [JSON_VALUE](../../yql/reference/builtins/json.md#json_value) or a filter predicate inside `JSON_EXISTS`.

When executing a query, a JSON index can be automatically used by the [optimizer](../glossary.md#optimizer). It is enough to write a regular condition `WHERE` with `JSON_EXISTS` / `JSON_VALUE` expressions on an indexed JSON column — the optimizer will recognize such a predicate and apply a JSON index read instead of scanning the records of the source table. Additionally, a JSON index, like any other [secondary index](../glossary.md#secondary-index), can be forced to be used by specifying its name in the `VIEW IndexName` section.
...
* [JSON indexes](../../dev/json-indexes.md): an overview of features, supported predicates, and constraints.
* [JSON functions](../../yql/reference/builtins/json.md) — reference on `JSON_EXISTS`, `JSON_VALUE`, and the JsonPath language.
* [VIEW (JSON index)](../../yql/reference/syntax/select/json_index.md) — syntax of queries with a JSON index.
