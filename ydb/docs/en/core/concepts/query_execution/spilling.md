...
At the Resource Manager level, the threshold for switching to spilling is set by [`resource_manager.spilling_percent`](../../reference/configuration/table_service_config.md#spilling-percent): when the `query_memory_limit` pool on a node is filled to the specified percentage, compute operations start preferring disk offload over further RAM growth.
