...
After creating a source, for example named `ext_source`, accessing the `input_topic` topic in the external database is written as follows:
...
### Moving data from a topic to a table via UPSERT {#upsert-from-topic}
...
### System fields {#system-metadata}

When reading, you can request system fields:
...
Filters on service fields are evaluated before reading data from the topic and significantly reduce the volume of messages read. Supported are comparison operators (`=`, `<>`, `<`, `<=`, `>`, `>=`, `IN`), logical conditions (`AND`, `OR`), and the fields `partition_id`, `write_time`, `offset`. Predicates on other service fields do not limit the read volume.
