...
**Local topics** are topics created in the **same database** {{ ydb-short-name }} as the query being executed.
...
Reading from a topic can be performed in [table](#table-read) and [streaming](#streaming-read) modes (not to be confused with [streaming queries](../streaming-query/streaming-query.md)).
