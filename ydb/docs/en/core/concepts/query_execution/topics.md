...
Reading from a topic can be performed in [tabular](#table-read) and [streaming](#streaming-read) modes (not to be confused with [streaming queries](../streaming-query/streaming-query.md)).

### Tabular reading {#table-read}

In tabular mode, reading is performed from the first to the last offset stored in the topic at the time the query is started. If data continues to be written to the topic, the query will stop after reaching the last offset known at the time of start. Specifying filters on [service fields](#system-metadata) speeds up reading, as reading occurs only over the specified ranges.
...
The same result can be obtained without the `WITH` block — see [tabular reading](#table-read).
...
Data from a topic can be moved to a table via [UPSERT INTO](../../yql/reference/syntax/upsert_into.md):
...
Filters on service fields are evaluated before reading data from the topic and significantly reduce the volume of messages read. Comparison operators (`=`, `<>`, `<`, `<=`, `>`, `>=`, `IN`), logical conditions (`AND`, `OR`), and the `partition_id`, `write_time`, `offset` fields are supported. Predicates on other service fields do not limit the read volume.
...
Transactional write via YQL/`INSERT INTO` is not supported — partial query results may appear in the topic.
