...
{{ ydb-short-name }} implements stream processing as part of a unified data platform. In addition to typical streaming query use cases, integration into the {{ydb-short-name}} unified platform allows you to get data from [topics](../datamodel/topic.md) {{ydb-short-name}}, process change streams from [row tables](../datamodel/table.md#row-oriented-tables) via [CDC](../cdc.md), and write processing results to output topics or directly to tables. This enables building data processing pipelines within {{ydb-short-name}} with minimal latency.
...
Streaming queries read data from [topics](../datamodel/topic.md) {{ ydb-short-name }} and write results to [topics](../datamodel/topic.md) or [tables](../datamodel/table.md) {{ ydb-short-name }}. Data can be written to a topic from external systems (for example, an application sends events via the SDK), but the streaming query itself only works with {{ ydb-short-name }} entities. Direct reading from external systems, such as Apache Kafka, or writing to tables in other databases, such as PostgreSQL, is not supported.
...
Under normal operation, streaming queries provide [at-least-once](https://en.wikipedia.org/wiki/Reliable_messaging#At-least-once_delivery) data delivery guarantee — on failures, the query automatically recovers from a [checkpoint](../../dev/streaming-query/checkpoints.md) and resumes reading from the saved offsets.
...
Detailed description of guarantees, anomalies and ways to minimize them — in the section [{#T}](../../dev/streaming-query/guarantees.md).

{% note info %}

We continuously work on improving streaming processing mechanisms. In future releases, the provided guarantees will be enhanced.
...
- The query must contain at least one read from a topic, because streaming processing requires a continuous input data stream.
...
Streaming queries are created, altered, and dropped using YQL commands:

- [CREATE STREAMING QUERY](../../yql/reference/syntax/create-streaming-query.md) — creation
- [ALTER STREAMING QUERY](../../yql/reference/syntax/alter-streaming-query.md) — alteration and state management
...
- [{#T}](../../dev/streaming-query/guarantees.md) — guarantees, anomalies in windowed aggregation and recommendations
