...
The blob interface stores *blobs* (binary fragments ranging from 1 byte to 10 mebibytes in size) with a unique identifier. The subsystem also supplies the DDisk and PersistentBuffer primitives used by direct block storage. These paths share device management and placement infrastructure, but expose different data and replication contracts.
