...
Before starting work on the documentation for a new command:
...
The heading should briefly describe the action of the command. Use a wording that starts with a noun or a verbal noun.
...
If the command has important features, limitations, or warnings, add them immediately after the description using the `{% note %}` block.
...
- Specify the value type if it is important (for example, `VAL`, `STR`, `INT`, `BOOL`).
...
For commands with a large number of parameters, you can split them into groups:
...
- Show the results of executing the command if they are important for understanding.
