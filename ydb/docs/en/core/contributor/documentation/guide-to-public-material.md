...
The first step when publishing video materials is choosing a title. Typically, this is a second-level heading, and the name is exactly the same as the video title on the platform from which the original link came.
...
The next step is a short description in one paragraph reflecting only the key points of the video. It is not necessary to watch the entire video yourself; you can use the video summarization feature with [AI](https://300.ya.ru/), but remember that AI can make mistakes and it is better to double-check the summary yourself.

### Target audience

The next paragraph must be about the target audience of the published video. Consider their specific role ( [application developers](../../dev/index.md), [cluster administrators](../../devops/index.md), [security engineers](../../security/index.md), etc.), and if you cannot figure it out yourself, ask the talk author. If the author is unavailable, you can again ask AI for advice. It will most likely not be wrong.
...
With Rutube, after getting the video link in the format `https://rutube.ru/video/6840af8411a8be4e7da9f82cb4a25103/?r=plwd`, it is enough to keep only the part with the video ID, as shown in the example above.

With Yandex, it is enough to copy the video link, and specifying the hosting name in square brackets is optional.
...
If a video is available on multiple resources, links from these resources must be formatted using tabs `{% list tabs %}`
