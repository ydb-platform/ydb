...
The first step when publishing video materials is choosing a title. Typically, this is a second-level heading, and the title is exactly the same as the video title from the platform where the original link came from.
...
The next step is a brief description in one paragraph, reflecting only the most essential points of the video. It is not necessary to watch the entire video yourself; you can use the video summarization feature with [AI](https://300.ya.ru/), however, remember that AI can make mistakes and it is better to double-check the summary yourself.
...
To get a link from YouTube, simply click **Copy video URL** and use the obtained link.

For Rutube, after getting the video link in the format `https://rutube.ru/video/6840af8411a8be4e7da9f82cb4a25103/?r=plwd`, you only need to keep the part with the video ID, as shown in the example above.

For Yandex, simply copy the video link, and specifying the hosting name in square brackets is optional.
...
If a video is available on multiple platforms, links from these platforms should be formatted using tabs `{% list tabs %}`

{% endnote %}

### Adding slides

Some video materials (e.g., conference talks) require attaching presentation slides, placed after the video link in the **Slides** tab.
...
This will give a link `2025/ru/infra_source_craft/presentation.pdf`.

- Prepend `https://presentations.ydb.tech/` to this link.
