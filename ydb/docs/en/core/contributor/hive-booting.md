...
2. When processing the queue, a [suitable node is selected](#findbestnode) for it.
3. A command to [start the tablet](#booting) is sent to the node.
...
* Local does not report that it has stopped working.
