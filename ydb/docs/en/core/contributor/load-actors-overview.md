...
* Cluster {{ ydb-ui-name }}: Allows you to create, based on a configuration, and start a load actor either on the current node or all tenant nodes at once.
...
You can view the test results using the {{ ydb-ui-name }}. For a description of output parameters, see the documentation of the respective actor.
