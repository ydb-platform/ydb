...
This section contains a step-by-step scenario that will help you perform the necessary setup steps and learn how to make changes to the {{ ydb-short-name }} project. You don't have to strictly follow this scenario; you can develop your own approach based on the provided information.
...
Using GitHub CLI allows you to create pull requests and manage the repository from the command line. You can also use the GitHub UI for such actions.
...
| Upload your SSH public key to your GitHub account? | Select the file with the public key ( extension) from those created in the ["Create an SSH key pair"](#ssh_key_pair) step, for example **/home/user/.ssh/id_ed25519`.pub`** |
...
After the last response, you will be prompted to enter a token that you can generate in the GitHub user interface:
...
Forking a repository is an instant action, but cloning to a local computer takes some time to transfer about 650 MB of repository data over the network.
...
Edit files locally, use standard Git commands to add files, check status, make commits, and push changes to your fork repository:
...
Subsequent pushes do not require an upstream or a branch name:
...
- GitHub CLI
  ...
  After you answer some questions, the Pull Request will be created, and you will receive a link to its page on GitHub.com.

{% endlist %}

### Precommit checks {#create_pr_desc}

When creating a Pull Request, the description will be filled with the text from the template, which you need to edit:

1. **Changelog Entry.** In this block, add a description of the change for end users of the system (see [requirements](#changelog_entry_req)). The content of this block will be published in the [changelog](../changelog-server.md) if the PR is merged.
2. **Description for reviewers.** In this block, you can add a link to the task and any additional information that will be useful for reviewing your change. The content of this block will not be included in the list of changes.
...
- Describe what has changed in the system's operation for the end user.

### Preliminary checks {#precommit_checks}

Before merging changes, pre-commit checks of the Pull Request are run.

For changes to the {{ ydb-short-name }} code, pre-commit checks collect artifacts and run the tests described in the `ya.make` files. The build and tests are run on a special merge commit that merges your changes with the current `main` branch.

You can see the status of checks on the Pull Request page. Also, key information about the progress of the {{ ydb-short-name }} build/tests and the current status is published in comments to the PR.
...
A Pull Request can be merged after receiving approval from a {{ ydb-short-name }} team member. Comments are used for communication. After confirmation, a {{ ydb-short-name }} team member clicks the "Merge" button.

### Update changes {#update}

If a Pull Request is open in your repository for a development branch, it will be updated every time you push to that branch, with checks being restarted.
...
When you need to move a patch to a stable branch, branch off the stable branch:
