...
* use query parameter values passed by the application when processing the predicates being checked.

A JSON index is a [global synchronous](../concepts/glossary.md#secondary-index) index — its data is always consistent with the main table.
...
Ready-made usage scenarios are collected in the [JSON document search recipes](../recipes/json-search/index.md) section.

## Updating JSON indexes {#update}

JSON indexes are automatically maintained when data is modified and are updated synchronously with the base table. Tables with JSON indexes support:
...
Batch operations (`BATCH UPDATE` and `BATCH DELETE`) are not supported for tables with JSON indexes. When attempting to execute such a query on a table for which a JSON index has been created, this query will be rejected, a corresponding error will be returned to the application, and the data will remain unchanged.

Additionally, for tables with JSON indexes, the following is not supported:

* Bulk data loading via the `BulkUpsert` call — the requested operation will be rejected with a corresponding error message.
* automatic row deletion by [TTL](../concepts/ttl.md) — errors will be returned when trying to create a table simultaneously with a TTL policy and a JSON index, as well as when trying to obtain such a combination of properties through the `ALTER TABLE` commands.

## Supported predicates {#predicates}

For execution via JSON indexes, only expressions based on functions `JSON_EXISTS` and `JSON_VALUE` in the `WHERE` block, combined by operators `AND` / `OR` according to the rules below, are supported.
...
**Prohibited** (error when using the `VIEW` operator or automatic index selection failure):
...
The `JSON_EXISTS` function returns `true` for any non-empty JsonPath result. The `$.key == 10` predicate specified at the top level would yield 'path existence' even when the comparison is false, which does not match the expected semantics. Comparisons should be moved into `JSON_VALUE` calls or into a filter of the form `? (...)`.

{% endnote %}

### JSON_VALUE {#json-value}

Extracting a scalar value with a mandatory `RETURNING <type>`.

To compare a value using `JSON_VALUE`, you must always specify `RETURNING` with the required type. By default, `JSON_VALUE` returns type `Utf8`, which during query execution leads to incorrect comparison — values of different types are compared as strings:
...
To check 'value equals `false`' or 'value equals `null`', use a JsonPath filter inside `JSON_EXISTS`, for example `JSON_EXISTS(doc, '$.k ? (@ == false)')` or `JSON_EXISTS(doc, '$.k ? (@ == null)')`, rather than `JSON_VALUE(...) IS NULL`.

{% endnote %}

## Limitations {#limitations}

* JSON indexes are supported only for [row-based tables](../concepts/datamodel/table.md#row-oriented-tables).
...
* The type of the read query parameter from the index cannot be wrapped in `Optional<T>` — optional parameters are not supported.
* Equality comparison with an integer literal whose absolute value exceeds 2⁵³ is not accelerated by the value index (such numbers do not fit into the numeric type used in `Json` and `JsonDocument`) and reduces to a path existence check.
