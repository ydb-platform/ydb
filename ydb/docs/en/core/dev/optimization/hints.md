...
- BroadcastJoin — this is a type of join where one of the data sets is small enough to be copied (broadcast) to all necessary nodes in the cluster. This allows each node to perform the join locally without transmitting data over the network.

{% note info %}

If the order of joins is not fixed by a separate hint, the optimizer will build both versions of the plans: where the left and right inputs of the join are sent. If the order of joins is fixed by a hint, the right side of the join will be sent.
...
- The nested structure of brackets defines the order of joins
...
You can use several types of hints simultaneously within the same pragma:
