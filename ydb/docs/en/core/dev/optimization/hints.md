...
#### The parameters are similar to Rows, but are applied to the data size in bytes
...
Allows you to fix a certain subtree of joins in the general join tree.
...
The optimizer will only consider those plans that include the specified partial or complete join order.
...
We will get the following plan:
...
Here you can see that the order of joins has changed to the one specified in the hint.
