...
[Resource pools](../concepts/glossary.md#resource-pool) allow you to isolate database resources between running queries or configure resource allocation strategies in case of oversubscription (requesting more resources than are available in the system). All resource pools are equal, without any hierarchy, and only affect each other in case of a general resource shortage.

For example, one of the typical scenarios for resource isolation is to separate two classes of consumers:
...
The example below shows the syntax for creating a separate resource pool named "olap" for running analytical queries:
...
Let's consider the example above and see what these parameters actually mean and how they will affect resource allocation. Suppose that {{ ydb-short-name }} database has 10 nodes with 10 vCPU each. In total, such a database has 100 vCPU. Then, for each node, the resource pool named `olap` will be allocated:
...
For a single query in this resource pool, the following will be allocated:
...
Suppose that 9 queries are already running in the `olap` resource pool. When a new query arrives, it will immediately start running in parallel with the other 9 queries. Now there will be 10 queries running in the pool. If the 11th query arrives in the pool, it will not start running but will be placed in the waiting queue. When at least one of the 10 running queries completes, the 11th query will be taken out of the queue and will start running.

If there are already $QUEUE\_SIZE = 1000$ queries in the queue, then when the 1001st query is sent, the client will immediately receive an error in response, and this query will not be executed. Example of an error:
...
The number of concurrently running queries is affected not only by `CONCURRENT_QUERY_LIMIT` but also by `DATABASE_LOAD_CPU_THRESHOLD`.

### How DATABASE_LOAD_CPU_THRESHOLD Works {#database_load_cpu_threshold}

When a query arrives at a resource pool with `DATABASE_LOAD_CPU_THRESHOLD` set, 10% of the available CPU on the node is immediately reserved, assuming that the query will require at least this amount of resources. Then, every 10 seconds, the consumed resources across the entire database are recalculated, which allows refining the initial 10% estimate. This means that if more than 10 queries arrive at a cluster node simultaneously, no more than 10 queries will be started, and the rest will wait for the actual CPU consumption to be clarified.

As with `CONCURRENT_QUERY_LIMIT`, if the specified load threshold is exceeded, queries are sent to the waiting queue.

### Resource Allocation According to RESOURCES_WEIGHT {#resources_weight}

![resource_pools](../_assets/resources_weight.png)

The `RESOURCES_WEIGHT` parameter only starts working in case of oversubscription and when there is more than one resource pool in the system. In the current implementation, `RESOURCES_WEIGHT` only affects the distribution of vCPU resources. When queries appear in a resource pool, it begins to participate in resource allocation. For this, the pools recalculate limits according to the [Max-min fairness](https://en.wikipedia.org/wiki/Max-min_fairness) algorithm. The resource redistribution itself is performed on each compute node individually, as shown in the figure above.

Suppose we have a node in the system with 10 vCPU available. The following restrictions are set:
...
Even if no resource pools have been created, the system always has a `default` resource pool, which cannot be deleted. Every request running in the system always belongs to some pool — there are no situations where a request is not associated with any resource pool. By default, the settings of the `default` resource pool look like this:
...
This means that no restrictions are applied in the `default` resource pool: it operates independently of other pools and has no limits on resource consumption. In the `default` resource pool, you can change parameters using the [{#T}](../yql/reference/syntax/alter-resource-pool.md) query, except for the `CONCURRENT_QUERY_LIMIT`, `DATABASE_LOAD_CPU_THRESHOLD`, and `QUEUE_SIZE` parameters. This limitation is intentional to minimize the risks associated with incorrect configuration of the default resource pool.
## Resource Pool ACL Management

### Permissions to Create, Modify, and Delete a Resource Pool

To create, modify, or delete a resource pool, you need to grant access permissions in accordance with the permissions described in the reference [{#T}](../yql/reference/syntax/create-resource-pool.md). For example, to create resource pools, you need to have `CREATE TABLE` permission on the `.metadata/workload_manager/pools` directory, which can be granted with a query of the following form:

```yql
GRANT CREATE TABLE ON `.metadata/workload_manager/pools` TO user1;
```

### Permissions to Run a Query in a Resource Pool {#run-access}

To run a query in a pool, a user must have [access permission](../yql/reference/syntax/grant.md#permissions-list) `SELECT` on that pool. An example of granting permissions:
...
- `RESOURCE_POOL` — the name of the resource pool to which requests that meet the criteria specified in the resource pool classifier will be sent.
- `MEMBER_NAME` — the user group or user whose requests will be sent to the specified resource pool.
## Resource Pool Classifier ACL Management

Resource pool classifiers are global for the entire database and apply to all users. To create, delete, or modify a resource pool classifier, you need to have [access permission](../yql/reference/syntax/grant.md#permissions-list) `USE` for the entire database, which can be granted with a query like:
...
To use a classifier, a user must have [access to the resource pool](#run-access) that this classifier refers to. If there is no such access, the classifier is skipped and the next one is checked. This behavior may change in future versions, so do not use the lack of pool permissions as a mechanism to control which classifier is applied.
...
Suppose there are two resource pool classifiers with conflicting conditions, and the user `user1@domain` meets the criteria for both resource pools: `olap1` and `olap2`. If no classifiers existed in the system before, then `RANK=1000` is set for `olap1`, and `RANK=2000` for `olap2`. Resource pool classifiers with a lower `RANK` value have higher priority. In this example, since `olap1` has a higher-priority `RANK` than `olap2`, `olap1` will be selected.

You can also set `RANK` for resource pool classifiers yourself when creating them using the syntax construct [{#T}](../en/docs/yql/reference/syntax/create-resource-pool-classifier.md), or change `RANK` for existing resource pool classifiers using [{#T}](../en/docs/yql/reference/syntax/alter-resource-pool-classifier.md).
...
The current version of the **YDB Python SDK** does not allow specifying the resource pool in which the query should be executed.
...
Detailed information about query plans can be found on the page [query plan structure](../yql/query_plans.md). To get information about the resource pool used, you need to run a command that generates statistics in `json-unicode` format. Example command:

```bash
ydb -p <profile_name> sql -s 'select 1' --stats full --format json-unicode
```

In the body of the query plan obtained using the command above, you can find useful attributes for diagnosing resource pool usage. Example of such information:
...
- `QueuedTimeUs` — total time the query spent in the queue.

### Status of an Executing Query

Information about how a query is processed in Workload Manager can be obtained from the system view [`.sys/query_sessions`](system-views.md#query-sessions). This view includes the following fields:

- `WmPoolId` `(Utf8)` — ID of the pool where the query is being executed.
- `WmState` `(Utf8)` — status of the query in WM.
- `WmEnterTime` `(Timestamp)` — time when the query entered the PENDING or DELAYED status.
- `WmExitTime` `(Timestamp)` — time when the query was sent for execution.

Possible values for the WmState field:
...
The following query displays information about all active queries in the system:
...
select
    ...
    WmEnterTime,    -- Time when the query entered the PENDING or DELAYED status
    WmExitTime      -- Time when the query was sent for execution
