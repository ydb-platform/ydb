...
This mechanism is experimental and is disabled by default for now. Enable it using the [`index_auto_choose_mode` setting in `table_service_config`](https://github.com/ydb-platform/ydb/blob/d5caec3e4939a9fd3dc738161da3c9f7f205e267/ydb/core/protos/table_service_config.proto#L276). The setting also affects query service behavior.
...
The [`UPDATE`](../yql/reference/syntax/update.md), [`UPSERT`](../yql/reference/syntax/upsert_into.md), and [`REPLACE`](../yql/reference/syntax/replace_into.md) YQL record modification statements don't permit specifying a secondary index to perform a search for data, so an attempt to make an `UPDATE ... WHERE indexed_field = $value` will result in a full scan of the table. To avoid this, you can first run `SELECT` by index to get the primary key value and then run `UPDATE` by the primary key. You can also use `UPDATE ON`.
