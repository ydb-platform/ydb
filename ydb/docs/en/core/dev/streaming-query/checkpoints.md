...
A **checkpoint** is persisted state of a running [streaming query](../../concepts/streaming-query/streaming-query.md), used to recover processing after failures. {{ ydb-short-name }} periodically checkpoints all running streaming queries.
...
Events that arrived between the last checkpoint and the failure are processed again. That provides [at-least-once](../../dev/streaming-query/guarantees.md#at-least-once) delivery — each event is processed at least once.
...
- [{#T}](../../concepts/streaming-query/streaming-query.md) — streaming queries overview.
