...
On recovery, the query rolls back to the last checkpoint: it resumes reading from the saved offsets and restores the aggregation states. Events that arrived between the checkpoint and the failure will be reprocessed. For more details on the checkpoint mechanism, see the [{#T}](checkpoints.md) section.
...
This is expected behavior on the first start — all subsequent windows will receive data for the full interval, which is important to consider when recreating the query.

### Event loss when recreating a query {#incomplete-windows-restart}

To change the query text, use the combination of [DROP](../../yql/reference/syntax/drop-streaming-query.md) and [CREATE](../../yql/reference/syntax/create-streaming-query.md) commands. When `DROP` is executed, the checkpoint is deleted along with the query, because {{ ydb-short-name }} uses internal storage for read offsets from the source, so these offsets are deleted along with the query. The new query does not have a saved position and starts reading from the end of the topic. All events that arrived in the topic between the deletion of the old query and the start of the new one will not be read.
...
- [{#T}](checkpoints.md) — checkpoint mechanism ensuring recovery after failures.
