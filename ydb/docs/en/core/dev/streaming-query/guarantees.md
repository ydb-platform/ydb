...
When writing results to a table via [UPSERT](../../yql/reference/syntax/upsert_into.md), reprocessing does not cause duplication: UPSERT updates the existing row by primary key. Data is not lost, and duplicates do not accumulate.

When writing results to an output topic, reprocessing leads to duplicates: the same events will be written to the topic more than once. The consumer of the output topic must account for this and perform deduplication on its own if necessary.
...
A similar situation occurs if the data pointed to by the offset in the checkpoint has already been deleted from the topic due to [TTL](../../concepts/datamodel/topic.md#retention-time).
