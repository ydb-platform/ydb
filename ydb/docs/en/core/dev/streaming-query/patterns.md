...
- `ext_source` is a pre-created `external data source`.
- `input_topic` is the topic from which data is read.
- `output_topic` is the topic to which results are written.
- `output_table` is the {{ ydb-short-name }} table to which results are written.
