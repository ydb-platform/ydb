...
Reading structured messages is performed using `SELECT ... FROM ... WITH (FORMAT, SCHEMA)`. The `WITH` block specifies the input data format and schema — which fields are expected in each message and their types.
...
-- Writing to table (only UPSERT, INSERT is not supported)
