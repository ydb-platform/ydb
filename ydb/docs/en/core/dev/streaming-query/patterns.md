...
Structured messages are read using `SELECT ... FROM ... WITH (FORMAT, SCHEMA)`. The `WITH` block specifies the input data format and schema — which fields are expected in each message and their types.
...
SELECT
    -- Constructing JSON from individual fields
...
Writing to tables in streaming queries is supported **only in UPSERT mode**. The `INSERT INTO` operation is not supported because, during event reprocessing (the at-least-once guarantee), it would lead to duplicate rows. With `UPSERT`, if a row with the same primary key already exists, it will be updated; otherwise, a new row will be inserted, while `INSERT INTO` will fail with an error.
...
-- Writing to a table (only UPSERT, INSERT is not supported)
