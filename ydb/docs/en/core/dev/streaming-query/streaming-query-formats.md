...
Built-in formats in {{ ydb-short-name }}:
...
- A valid JSON object on each separate line;
- Valid JSON objects combined into a list.

`json_as_string` does not split the input JSON document into fields; it represents each message as a single JSON object (or one line). This format is useful when the set of fields varies between rows.
...
In this format, the schema of the read data must consist of a single column with one of the allowed data types; see [below](#schema) for details.
...
This format allows reading message payloads in [Apache Parquet](https://parquet.apache.org) format.
...
This format allows reading message payloads as-is, in raw form. Data read this way can be processed with [YQL](../../yql/reference/udf/list/string). Default schema: `SCHEMA(Data String)`.
