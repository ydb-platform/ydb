# Topic read/write data formats

This section describes the data formats supported for [streaming queries](../../concepts/glossary.md#streaming-query) when reading from topics in {{ ydb-full-name }}, and the [YQL types](../../yql/reference/types/index.md) allowed in the schema for each format.

## Supported data formats {#formats}

{{ ydb-short-name }} provides built-in default formats and user-defined formats for specialized tasks.
...
In the examples below, `ydb_source` is a pre-created [external data source](../../concepts/datamodel/external_data_source.md), `input_topic` and `output_topic` are topics available through it, and `output_table` is a {{ ydb-short-name }} table.
...
Based on [CSV](https://en.wikipedia.org/wiki/Comma-separated_values). Columns are comma-separated; the first line contains column names.

Example data (one message):
...
Based on [tab-separated values](https://en.wikipedia.org/wiki/Tab-separated_values). Columns are separated by tab (`0x9`); the first line contains column names.
...
Valid example (list of JSON objects):
...
Invalid example (separate objects without an array wrapper):
...
Based on [JSON](https://en.wikipedia.org/wiki/JSON). Each message must be a single JSON **object**. Common for Kafka-style streaming and [{{ ydb-short-name }} topics](../../concepts/datamodel/topic.md).
Multiple JSON objects in one message are not supported; a bare JSON array is not supported.
...
- or multiple JSON objects concatenated into a list.

`json_as_string` does not split a JSON document into columns; each message is treated as one JSON object (or one line). Useful when the field set varies between rows.
...
The readable schema must contain a single column of an allowed type; see [below](#schema).
...
Reads message payloads as opaque bytes. Process further with [YQL string functions](../../yql/reference/udf/list/string). Default schema: `SCHEMA(Data String)`.
...
Types supported in the query schema:
