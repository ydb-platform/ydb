...
- An object in valid JSON representation in each separate line of the file
- Objects in valid JSON representation combined into a list.

The `json_as_string` format does not split the input JSON document into fields, but represents each message as a single JSON object (or a single line). This format is convenient if the list of fields is not the same across all rows but can vary.
