...
- [INSERT INTO](../../yql/reference/syntax/insert_into) — use UPSERT INTO instead. INSERT INTO is not used because under at-least-once delivery it would duplicate rows on retries.
...
The query reads events from a topic and writes them to `output_table`. `Ts` is cast from string to `Timestamp` using `CAST`, and [Unwrap](../../yql/reference/builtins/basic#unwrap) removes optionality.
