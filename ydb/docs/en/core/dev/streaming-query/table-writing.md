...
- [INSERT INTO](../../yql/reference/syntax/insert_into.md) — use UPSERT INTO instead. INSERT INTO is not used because it would duplicate rows during reprocessing under at-least-once delivery.
...
The query reads events from a topic and writes them to `output_table`. `Ts` is cast from string to `Timestamp` using ⟦C4⟧, and [Unwrap](../../yql/reference/builtins/basic#unwrap) removes optionality.
