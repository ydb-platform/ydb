...
Watermark — a monotonically increasing lower bound of event times in the stream (more about the concept: [{#T}](../../concepts/streaming-query/watermarks.md)). This section describes the configuration of watermarks in {{ ydb-short-name }} streaming queries.

## Event time {#event-time}

In stream processing, each event has a timestamp by which the system tracks the progress of time in the stream. In the current implementation, the event time source can only be the event write time to a [topic](../../concepts/datamodel/topic.md), available through the system column `__ydb_write_time`.

{% note info %}

Support for arbitrary expressions for extracting time from event data (for example, the `event.created_at` field) is planned in future versions.
...
When the system receives an event, it updates the watermark — **advances** it forward along the time axis. The watermark is calculated as `maximum observed event time − delay`, where `delay` is the lag value specified in the [`WATERMARK`](#configuration) expression (for example, `Interval("PT5S")` in `WATERMARK = __ydb_write_time - Interval("PT5S")`).
...
If the input topic contains multiple [partitions](../../concepts/datamodel/topic.md#partitioning), each of them advances its watermark independently. The overall query watermark does not outpace the slowest partition: windows are not closed until at least one partition has reached the corresponding point in time.

If one of the partitions stops receiving data, its watermark stops moving forward. Such a partition is called idle. While an idle partition is taken into account when calculating the overall watermark, it also stops moving forward, and results are not output despite data arriving from other partitions.

To avoid this blocking, an idle partition is excluded from the common watermark computation after a configurable timeout period (the `WATERMARK_IDLE_TIMEOUT` parameter, see [Configuration](#configuration) for details).
...
Below is an example of a streaming query with a watermark and window aggregation. The query reads events from a topic, filters them by the `pass` field, and aggregates `payload` values in windows of 10 seconds with a 5-second step. The watermark is configured with a 5-second lag.
...
- `__ydb_write_time` is a system column containing the event write time in the [topic](../../concepts/datamodel/topic.md).
- `FORMAT = json_each_row`: [data format](streaming-query-formats.md) in the topic, each line contains a separate JSON object.
- `WATERMARK = __ydb_write_time - Interval("PT5S")` — watermark with a 5-second lag. `Interval("PT5S")` sets the interval in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) format.
...
1. The first event (`"a"`, record time 40s) passes the filter (`pass > 0`) and falls into windows `[35; 45)` and `[40; 50)`. The watermark advances to 35s and does not close any window.
