...
Watermark — monotonically increasing lower bound on event times in the stream (for more details on the concept: [{#T}](../../concepts/streaming-query/watermarks.md)). This section describes the configuration of watermarks in {{ ydb-short-name }} streaming queries.

## Event time {#event-time}

In stream processing, each event has a timestamp by which the system tracks the progress of time in the stream. In the current implementation, the event time source can only be the time the event was written to a [topic](../../concepts/datamodel/topic.md), available through the system column `__ydb_write_time`.
...
When the system receives an event, it updates the watermark — **advancing** it forward along the time axis. The watermark is computed as `maximum observed event time − delay`, where `delay` is the lag value specified in the [`WATERMARK`](#configuration) expression (for example, `Interval("PT5S")` in `WATERMARK = __ydb_write_time - Interval("PT5S")`).
...
The `delay` parameter sets the allowed time tolerance for late-arriving events. For example, with `delay` set to 5 seconds, an event with timestamp 00:00:48 will be accepted even if events with timestamp 00:00:50 have already arrived: the watermark has not yet reached 00:00:48. If the same event arrives later, when the watermark has already advanced past 00:00:48, it will be considered late and discarded.
...
If one of the partitions stops receiving data, its watermark stops advancing. Such a partition is called idle. While an idle partition is taken into account when computing the overall watermark, the watermark also stops advancing, and results are not output, despite data arriving from other partitions.
...
- `__ydb_write_time` is a system column containing the time of writing an event to a [topic](../../concepts/datamodel/topic.md).
- `FORMAT = json_each_row`: [data format](streaming-query-formats.md) in the topic, each line contains a separate JSON object.
- `WATERMARK = __ydb_write_time - Interval("PT5S")` — watermark with a lag of 5 seconds. `Interval("PT5S")` sets the interval in the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) format.
...
- [`HoppingWindow(ts, "PT5S", "PT10S")`](../../yql/reference/syntax/select/group-by.md#group-by-hopping_window) — window function with step 5 seconds and window size 10 seconds.
...
1. The first event (`"a"`, record time 40s) passes the filter (`pass > 0`) and enters windows `[35; 45)` and `[40; 50)`. The watermark advances to 35s and does not close any window.
