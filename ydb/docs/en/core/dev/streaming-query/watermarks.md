...
Watermark is a monotonically increasing lower bound on event times in a stream (for more on the concept: [{#T}](../../concepts/streaming-query/watermarks.md)). This section describes configuring watermarks in {{ ydb-short-name }} streaming queries.
...
If one of the partitions stops receiving data, its watermark stops moving forward. Such a partition is called idle. While the idle partition is taken into account when calculating the overall watermark, it also stops moving forward, and results are not output despite data arriving from other partitions.

To avoid this blocking, an idle partition is excluded from the overall watermark computation after a configurable timeout period (parameter `WATERMARK_IDLE_TIMEOUT`, see [Configuration](#configuration) for details).
...
- `__ydb_write_time` is a system column that contains the event write time in the [topic](../../concepts/datamodel/topic.md).
...
4. The fourth event (`"d"`, record time 40s) does not advance the watermark: it is already at 45s. The event passes the filter but is discarded as late — its record time (40s) is less than the current watermark (45s). Although window `[40; 50)` is still open, the watermark has already promised that no events with time < 45s will arrive, so `d` is not counted in any of its windows.
