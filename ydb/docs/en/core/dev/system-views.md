...
* `partition_stats` — contains information about instantaneous metrics and cumulative counters of operations. The first type includes, for example, CPU load or the number of executing [transactions](../concepts/transactions.md). The second type includes the total number of read rows.
...
| --- | --- | --- | --- |
| `OwnerId` | SchemeShard table identifier.<br/>Key: `0`. | `Uint64` | Instantaneous |
...
| `FollowerId` | Identifier of the partition tablet [replica](../concepts/glossary.md#tablet-follower). Value 0 indicates the leader.<br/>Key: `3`. | `Uint32` | Instantaneous |
...
| `CPUCores` | Instantaneous load value on the partition (share of CPU core time spent by the partition actor). | `Double` | Instantaneous |
...
| `TxRejectedByOutOfStorage` | Number of transactions canceled due to insufficient storage space. | `Uint64` | Cumulative |
| `TxCompleteLag` | Transaction execution latency (how far transactions lag behind the scheduled time). | `Interval` | Instantaneous |
| `LastTtlRunTime` | Last start time of TTL partition cleanup | `Timestamp` | Instant |
| `LastTtlRowsProcessed` | Number of partition rows checked during the last TTL cleanup | `Uint64` | Instant |
| `LastTtlRowsErased` | Number of deleted rows of the partition during the last TTL cleanup | `Uint64` | Instantaneous |
| `LocksAcquired` | Number of [locks](../contributor/datashard-locks-and-change-visibility.md). | `Uint64` | Cumulative |
...
* `top_queries_by_duration_one_minute` — data is split into minute intervals and contains data for the last 6 hours.
...
| `Rank` | Rank of the query in the top.<br/>Type: `Uint32`.<br/>Key: `1`. |
| `QueryText` | Query text.<br/>Type: `Utf8`. |
| `Duration` | Total query execution time.<br/>Type: `Interval`. |
| `EndTime` | End time of query execution.<br/>Type: `Timestamp`. |
...
## Detailed information about queries {#query-metrics}
...
| `Rank` | Rank of the query within the interval (by the SumCPUTime field).<br/>Type: `Uint32`.<br/>Key: `1`. |
| `QueryText` | Query text.<br/>Type: `Utf8`. |
...
| `NodeId` | Node ID where the session is running.<br/>Type: `Uint32`. |
...
The system view `compile_cache_queries` allows you to get data on the cache state on all or selected cluster nodes. When you query this view, requests are sent to each node, unless a restriction on `NodeId` is specified in the `WHERE` condition. The response returns up-to-date information about the cache contents for each node, and the resulting table combines the received data.
...
| `AccessCount` | Number of hits for the query text in the cache.<br/>Type: `Uint64`. |
...
The following system views contain the history of moments of high load on individual partitions of database tables:
...
* `Rank` — rank of the partition by peak load `CPUCores` in this interval.
...
| `Rank` | Rank of the partition within the interval (by `CPUCores`).<br/>Type: `Uint32`.<br/>Key: `1`. |
| `TabletId` | Tablet ID serving the partition.<br/>Type: `Uint64`. |
| `FollowerId` | Identifier of the [replica](../concepts/glossary.md#tablet-follower) of the partition tablet. Value 0 means the leader.<br/>Type: `Uint32` |
...
| `NodeId` | Node ID where the partition was located at the time of the peak.<br/>Type: `Uint32`. |
| `DataSize` | Approximate partition size in bytes at the time of the peak.<br/>Type: `Uint64`. |
| `RowCount` | Approximate row count at the time of the peak.<br/>Type: `Uint64`. |
| `IndexSize` | Partition index size in the tablet at the time of the peak.<br/>Type: `Uint64`. |
| `InFlightTxCount` | Number of transactions in the process of execution at the time of the peak.<br/>Type: `Uint32`. |

### Query examples {#top-overload-partitions-examples}

The following query outputs partitions with CPU consumption over 70% in the specified time interval, with tablet IDs and their sizes at the time of exceeding. The query is executed against the `.sys/top_partitions_one_minute` view, which contains data for the last 6 hours broken down by minute intervals:
...
The following query outputs partitions with CPU consumption over 90% in the specified time interval, with tablet IDs and their sizes at the time of exceeding. The query is made to the `.sys/top_partitions_one_hour` view, which contains data for the last 2 weeks broken down by hourly intervals:
...
The following system views contain a history of moments with a non-zero number of broken [locks](../contributor/datashard-locks-and-change-visibility.md) `LocksBroken` in individual partitions of database tables:

* `top_partitions_by_tli_one_minute` — data is split into minute intervals and contains history for the last 6 hours.
* `top_partitions_by_tli_one_hour` — data is divided into hourly intervals, containing history for the last 2 weeks.

Views output the top 10 partitions with a non-zero number of broken locks `LocksBroken`. Within a single interval, partitions are ranked by the number of broken locks `LocksBroken`.

The view keys are:

* `IntervalEnd` - interval end time
* `Rank` - partition rank by number of broken locks `LocksBroken` in this interval.

For example, `top_partitions_by_tli_one_hour` for an hourly interval `"20.12.2024 10:00-11:00"` will return 10 rows sorted in descending order by `LocksBroken`. They will have `Rank` from 1 to 10 and the same `IntervalEnd` `"20.12.2024 11:00"`.
...
| `Rank` | Rank of the partition within the interval (by `LocksBroken`).<br/>Type: `Uint32`.<br/>Key: `1`. |
| `TabletId` | Identifier of the tablet serving the partition.<br/>Type: `Uint64`. |
| `FollowerId` | Identifier of the [replica](../concepts/glossary.md#tablet-follower) of the partition tablet. A value of 0 indicates the leader.<br/>Type: `Uint32` |
| `Path` | Full path to the table.<br/>Type: `Utf8`. |
| `LocksAcquired` | Number of locks set on "key range" in this interval.<br/>Type: `Uint64`. |
...
| `DataSize` | Approximate partition size in bytes at peak.<br/>Type: `Uint64`. |
| `RowCount` | Approximate number of rows at peak.<br/>Type: `Uint64`. |
| `IndexSize` | Size of the partition index in the tablet at peak.<br/>Type: `Uint64`. |
...
System view `resource_pools` contains information about [settings](../yql/reference/syntax/create-resource-pool.md#parameters) of [resource pools](../concepts/glossary.md#resource-pool).
...
System view `resource_pools_classifiers` contains information about [settings](../yql/reference/syntax/create-resource-pool-classifier.md#parameters) of [resource pool classifiers](../concepts/glossary.md#resource-pool-classifier).
...
In this view, a user sees only those [access objects](../concepts/glossary.md#access-object) for which they have the `ydb.granular.describe_schema` right.
...
Getting explicitly granted rights on the access object - table `my_table`:
...
Getting effective rights on the access object - table `my_table`:
...
In this view, a user sees only those [access objects](../concepts/glossary.md#access-object) for which they have been granted the `ydb.granular.describe_schema` right.
...
In this view, a user sees only those [streaming queries](../concepts/streaming-query/streaming-query.md) for which they have been granted the `ydb.granular.describe_schema` right.
...
| `ResourcePool` | Name of the resource pool to which the query was bound ( [see example](../yql/reference/syntax/create-streaming-query.md#examples))<br />Type: `Utf8` |
