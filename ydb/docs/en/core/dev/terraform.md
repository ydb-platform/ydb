...
* The connection string `connection_string` is an expression of the form `grpc(s)://HOST:PORT/?database=/database/path`, where `grpc(s)://HOST:PORT/` endpoint, and `/database/path` is the path of the database.
  For example, `grpcs://example.com:{{ ydb-ports.grpcs }}?database=/Root/testdb0`.
...
resource "ydb_topic" "ydb_topic" {
  database_endpoint = "grpcs://example.com:{{ ydb-ports.grpcs }}/?database=/Root/testdb0" #database connection example
  name              = "test/test1"
  supported_codecs  = ["zstd"]
...
* `database_endpoint` - (required) is the full path to the database, for example: `"grpcs://example.com:{{ ydb-ports.grpcs }}/?database=/Root/testdb0"`; analogous to `connection_string` for tables.
