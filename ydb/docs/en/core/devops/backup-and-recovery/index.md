...
- `{{ ydb-cli }} admin database dump` — for backing up the database
...
- `{{ ydb-cli }} admin database restore` — for restoring the database from a backup
...
For backup to an S3-compatible storage (for example, [AWS S3](https://docs.aws.amazon.com/AmazonS3/latest/dev/Introduction.html)), the command `{{ ydb-cli }} export s3` is used. Follow [the link](../../reference/ydb-cli/export-import/export-s3.md) to the {{ ydb-short-name }} CLI reference for information about this command.

For recovery from a backup created in an S3-compatible storage, the command `{{ ydb-cli }} import s3` is used. Follow [the link](../../reference/ydb-cli/export-import/import-s3.md) to the {{ ydb-short-name }} CLI reference for information about this command.
...
When performing an export, a directory named `export_*` is created in the root directory of the database, where `*` is the numeric part of the export identifier. This directory contains tables that hold a consistent snapshot of the exported data at the start of the export. After the backup operation completes successfully, the directory `export_*` along with its contents is deleted.
