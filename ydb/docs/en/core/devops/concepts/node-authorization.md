...
Node authentication in a {{ ydb-short-name }} cluster verifies the identity of database nodes when they make service calls to other nodes over [gRPC](https://grpc.io/). Node authorization verifies and grants the required permissions when processing service calls, including operations for registering started nodes in the cluster and accessing [configuration management](../configuration-management/index.md). Using node authentication and authorization is recommended for all {{ ydb-short-name }} clusters, as it helps avoid unauthorized data access through the inclusion of attacker-controlled nodes in the cluster.

Before a [dynamic node](../../concepts/glossary.md#dynamic) of a database joins the cluster and starts exchanging data with other nodes over the internal [Interconnect](../../concepts/glossary.md#actor-system-interconnect) protocol, it must register with the already running [storage nodes](../../concepts/glossary.md#storage-node). To do this, the started node establishes a gRPC connection to one of the storage nodes specified in the `--node-broker` option and uses this connection to make service calls, including a registration request. These gRPC calls are exactly what node authentication and authorization protect: without successful verification, a node is not allowed to register and cannot join the cluster.
...
**Purpose of authentication.** To make sure that a new database node is an authorized node. [Certificates](../../reference/configuration/client_certificate_authorization.md) are used for this.

## How node verification works

1. The started database node opens a gRPC connection to one of the storage nodes specified in the `--node-broker` option. The connection is established over TLS, and the client certificate used is the certificate of the started node, specified in the `--grpc-cert` option.
2. The storage node and the database node perform [mTLS](../../concepts/glossary.md#mtls): each node verifies the other's certificate. The trust chain is checked (the certificate is signed by a known certificate authority) and the host name must match the value of the "Subject Name" field of the certificate.
...
To enable node authentication and authorization, you must prepare the certificate infrastructure and configure node startup using TLS certificates before changing the cluster configuration.

1. Make sure that {{ ydb-short-name }} [has gRPC traffic encryption configured](../../reference/configuration/tls.md#grpc) using TLS in the deployed cluster.
...
6. Perform a phased restart of cluster database nodes using ydbops or the Ansible playbook.
