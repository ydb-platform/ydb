...
Node authentication in a {{ ydb-short-name }} cluster verifies the identity of database nodes when they make service calls to other nodes over [gRPC](https://grpc.io/). Node authorization verifies and grants the required permissions when processing service calls, including operations for registering launched nodes in the cluster and accessing [configuration management](../configuration-management/index.md). Using node authentication and authorization is recommended for all {{ ydb-short-name }} clusters, as it helps avoid unauthorized data access through the inclusion of attacker-controlled nodes in the cluster.
...
**Purpose of authentication.** To ensure that the new database node is an authorized node. [Certificates](../../reference/configuration/client_certificate_authorization.md) are used for this purpose.
...
2. The storage node and the database node perform [mTLS](../../concepts/glossary.md#mtls): each node verifies the other's certificate. The trust chain is checked (the certificate is signed by a known certificate authority) and the hostname is checked against the value of the "Subject Name" field of the certificate.
...
2. When issuing certificates for nodes, set uniform rules for filling in the "Subject" field so that cluster node certificates can be distinguished. For details, see the [certificate verification rules documentation](../../reference/configuration/client_certificate_authorization.md).
   ...
   An example [script for generating self-signed certificates](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/tls_cert_gen/) for {{ ydb-short-name }} nodes fills the "Subject" field with `O=YDB` for all nodes. The examples in this article assume exactly this way of filling in the "Subject" field.
