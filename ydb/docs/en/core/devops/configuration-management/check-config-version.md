...
1. [{{ ydb-ui-name }}](#embedded-ui)
2. [Cluster metrics](#metrics)

## With {{ ydb-ui-name }} {#embedded-ui}

This method can be used if metrics collection from the {{ ydb-short-name }} cluster to the monitoring system is not configured. You can check the configuration version for a specific node or switch between nodes in the built-in web interface [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md):
