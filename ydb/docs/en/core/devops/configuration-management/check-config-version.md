...
1. [{{ ydb-ui-name }}](#ydb-ui)
2. [Cluster metrics](#metrics)

## With {{ ydb-ui-name }} {#ydb-ui}

This method can be used if metrics collection from the {{ ydb-short-name }} cluster to the monitoring system is not configured. You can check the configuration version for a specific node or switch between nodes in [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md):
