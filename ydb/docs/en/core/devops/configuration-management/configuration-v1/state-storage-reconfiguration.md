# Configuring State Storage, Board, and Scheme Board metadata distribution subsystems

Used if you need to change the [configuration of metadata distribution subsystems](../../../reference/configuration/domains_config.md) consisting of [State Storage](../../../concepts/glossary.md#state-storage), [Board](../../../concepts/glossary.md#board), and [Scheme Board](../../../concepts/glossary.md#scheme-board) on the {{ ydb-short-name }} cluster.

{% include [warning-configuration-error](../configuration-v1/_includes/warning-configuration-error.md) %}

To change the configuration of metadata distribution subsystems in the {{ ydb-short-name }} cluster, follow these steps.
...
1. Make the required changes in the `domains_config` sections of the `config.yaml` configuration file on each node of the {{ ydb-short-name }} cluster:
   For rules on changing the `domains_config` section, see the [Metadata distribution subsystem configuration rules](#metadata-subsystems-reconfig-rules) section.
...
Configuration changes are made in several steps. First, a new ring group consisting of correctly selected nodes (according to the failure model) is added, and then the old ring group is removed.
...
4. If different ring groups use the same cluster nodes, add the `ring_group_actor_id_offset` parameter to the ring group with a unique value (for example, `1`, `2`, …). The value must be unique among ring groups.
   ...
   This parameter will make the replica identifiers in this ring group unique; they will not coincide with identifiers from other groups, and this will allow placing multiple replicas of the same type on one cluster node.
...
**Step 4**
Apply the target configuration on the cluster:
...
You can verify that the changes have been applied in the `CMS` section of the cluster's [Embedded UI](../../../reference/embedded-ui/index.md) (available on port 8765): go to the `Tablets` tab and check by the replicas of metadata subsystem tablets that the configuration has been picked up.
