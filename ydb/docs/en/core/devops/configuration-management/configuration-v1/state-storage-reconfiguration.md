...
Use it if you need to change the [configuration of metadata distribution subsystems](../../../reference/configuration/domains_config.md) consisting of [State Storage](../../../concepts/glossary.md#state-storage), [Board](../../../concepts/glossary.md#board), [Scheme Board](../../../concepts/glossary.md#scheme-board) on the {{ ydb-short-name }} cluster.
...
1. Make the required changes in the `domains_config` section of the `config.yaml` configuration file on each node of the {{ ydb-short-name }} cluster:
...
2. Using the [rolling-restart](../../../maintenance/manual/node_restarting.md) procedure, sequentially restart all nodes of the {{ ydb-short-name }} cluster — [static](../../../concepts/glossary.md#static-node) and [dynamic](../../../concepts/glossary.md#dynamic): metadata subsystem replicas are placed on static nodes, and tablets on dynamic nodes access them. For more details about nodes, see the [Cluster topology](../../../concepts/topology.md) section.
   Before restarting the next host, a delay is required so that the restart on the previous host completes and the node participates in the cluster operation again.
