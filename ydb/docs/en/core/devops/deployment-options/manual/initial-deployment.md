# Manual deployment of {{ ydb-short-name }} cluster

<!-- markdownlint-disable blanks-around-fences -->

{% note warning %}

This guide is intended only for deploying clusters with [V1 configuration](../../configuration-management/configuration-v1/index.md). Deployment of clusters with [V2 configuration](../../configuration-management/configuration-v2/index.md) is currently under development.
...
When placing multiple dynamic nodes on the same server, separate ports are required for gRPC, Interconnect, HTTP interface, and Kafka API of each dynamic node on the server.
...
* Use the `mirror-3-dc` fault tolerance scheme to deploy the cluster in three availability zones (AZ), using at least 9 servers. This scheme can withstand the failure of 1 AZ and 1 server in another AZ. The number of servers used in each AZ must be equal.

{% note info %}

Run each static node (storage node) on a separate server. It is possible to co-locate static and dynamic nodes on the same server, as well as place multiple dynamic nodes on one server if sufficient computing resources are available.
...
For the {{ ydb-short-name }} service to have access to block disks for operation, add the user under which {{ ydb-short-name }} processes will be started to the `disk` group:
...
Where `ydb` is the name of the user under which `ydbd` is started.
...
To speed up and simplify the initial deployment {{ ydb-short-name }}, the configuration file already contains most settings for installing the cluster. Just replace the default FQDN hosts with the actual ones in the `hosts` and `blob_storage_config` sections.
...
After starting the static nodes, check their operability via the built-in web interface {{ ydb-short-name }} (Embedded UI):
...
To execute the administrative command for creating a database, you will need the certificate authority file `ca.crt`, similar to the procedure described above for cluster initialization.
...
- Manually
  ...
  In the command example above, `<ydb-static-node1>`, `<ydb-static-node2>`, `<ydb-static-node3>` are the FQDNs of any three servers on which static cluster nodes are running.
- Using systemd
  ...
  In the example command above, `<ydb-static-node1>`, `<ydb-static-node2>`, `<ydb-static-node3>`  - FQDN of any three servers on which static cluster nodes are running.
...
1. Set a password for the `root` account using the previously obtained token:
   ...
   Instead of the `passw0rd` value, substitute the required password. Save the password to a separate file. Subsequent commands on behalf of user `root` will be executed using the password passed via the `--password-file <path_to_user_password>` key. You can also save the password in the connection profile, as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).
...
In the command examples listed above, `<node.ydb.tech>` is the FQDN of the server on which any dynamic node serving the `/Root/testdb` database is running. When connecting via SSH to a dynamic node {{ ydb-short-name }}, it is convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.
...
To check access to the built-in web interface {{ ydb-short-name }}, just open the page at `https://<node.ydb.tech>:8765` in a web browser, where `<node.ydb.tech>` is the FQDN of the server on which any static node {{ ydb-short-name }} is running.

In the web browser, trust must be configured for the certificate authority that issued the certificates for the {{ ydb-short-name }} cluster; otherwise, a warning about using an untrusted certificate will be displayed.
...
Typically, to provide access to the built-in web interface {{ ydb-short-name }}, a fault-tolerant HTTP load balancer based on software `haproxy`, `nginx`, or similar is configured. Details of configuring the HTTP load balancer are beyond the scope of the standard installation instructions for {{ ydb-short-name }}.
...
Unsecured mode of {{ ydb-short-name }} is designed for solving test tasks, mainly related to software development and testing {{ ydb-short-name }}. In unsecured mode:
...
2. From the configuration files of cluster nodes, the sections `security_config`, `interconnect_config`, and `grpc_config` are excluded.
3. A simplified version of the commands for starting static and dynamic cluster nodes is used: options with certificate and key file names are excluded, the `grpc` protocol is used instead of `grpcs` when specifying connection endpoints.
