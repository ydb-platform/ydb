...
This instruction is only for deploying clusters with [configuration V1](../../configuration-management/configuration-v1/index.md). Deploying clusters with [configuration V2](../../configuration-management/configuration-v2/index.md) is currently under development.
...
If the Linux type used on the cluster servers uses `syslogd` for logging, configure log rotation using `logrotate` or its equivalents. {{ ydb-short-name }} services can generate a significant amount of system logs, especially when increasing the logging level for diagnostic purposes, so it is important to enable system log rotation to prevent `/var` filesystem overflow.
...
The required parameters for certificate generation are determined by the organization's policy. Typically, {{ ydb-short-name }} certificates and keys are generated with the following parameters:

* RSA keys with a length of 2048 or 4096 bits;
* certificate signing algorithm SHA-256 with RSA encryption;
* cluster node certificates must be valid for at least 1 year;
* the CA certificate must be valid for at least 3 years.
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The certificates must include the following uses: "Digital Signature, Key Encipherment" and the extended uses "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that the node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
For more information about spill configuration and its relation to file descriptors, see the [Spill Configuration](../../../reference/configuration/table_service_config.md#file-system-requirements) section.
...
Device names depend on the operating system settings, either set by the base image or configured manually. Typically, device names consist of three parts:
...
Save the YDB configuration file as `/opt/ydb/cfg/config.yaml` on each cluster server.[{#T}](../../../reference/configuration/index.md)
...
## Start storage nodes {#start-storage}
...
- Using systemd[скачать из репозитория](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service)

  Create a systemd configuration file `/etc/systemd/system/ydbd-storage.service` on each server that will host a static cluster node, using the template below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
...
After starting the static nodes, check their functionality using the {{ ydb-short-name }} embedded UI (Embedded UI):
...
3. Make sure that all 3 static nodes are displayed in the list.[Ручная установка, запущенные статические узлы](../_assets/manual_installation_1.png)
...
To execute the administrative command to create a database, you will need the registration authority certificate file `ca.crt`, following the same procedure as described above for cluster initialization.

When you create a database, the initial number of storage groups is set, which determines the available I/O throughput and maximum storage capacity. The number of storage groups can be increased after the database is created if necessary.
...
When the database is created successfully, the command completion code displayed on the screen should be zero.
...
- Using systemd[скачать из репозитория](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-testdb.service)
...
In the command examples listed above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node serving the `/Root/testdb` database. When connecting via SSH to a dynamic {{ ydb-short-name }} node, it's convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.[документации {{ ydb-short-name }} CLI](../../../reference/ydb-cli/profile/index.md)
...
## Test the operation of the created database {#try-first-db}[документации](../../../reference/ydb-cli/install.md) {#try-first-db}
...
Your web browser must trust the certificate authority that issued the certificates for the {{ ydb-short-name }} cluster, otherwise you will see a warning about an untrusted certificate.[{#T}](../../../reference/embedded-ui/index.md)
...
## Features of installing {{ ydb-short-name }} in an unsecured mode
...
The installation procedure described above involves deploying {{ ydb-short-name }} in the standard secure mode.
...
2. The `security_config`, `interconnect_config`, and `grpc_config` sections are excluded from the cluster node configuration files.
3. A simplified version of the commands for starting static and dynamic cluster nodes is used: options with certificate and key file names are excluded, and the `grpc` protocol is used instead of `grpcs` when specifying connection points.
4. The step of obtaining an authentication token before initializing the cluster and creating a database, which is unnecessary in unsecured mode, is skipped.
