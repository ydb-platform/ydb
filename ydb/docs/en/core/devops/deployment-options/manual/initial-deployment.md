...
When hosting multiple dynamic nodes on a single server, separate ports for gRPC, Interconnect, the HTTP interface, and Kafka API are required for each dynamic node on the server.
...
If the Linux type used on the cluster servers employs `syslogd` for logging, configure log rotation using `logrotate` or its equivalents. {{ ydb-short-name }} services can generate a significant amount of system logs, especially when increasing the logging level for diagnostic purposes, so it is important to enable system log rotation to prevent `/var` filesystem overflow.

Select the servers and disks that will be used for data storage:

* Use the `block-4-2` fault tolerance scheme to deploy a cluster in a single availability zone (AZ), using at least 8 servers. This scheme allows surviving the failure of 2 servers.
* Use the `mirror-3-dc` fault tolerance scheme to deploy a cluster across three availability zones (AZs), using at least 9 servers. This scheme allows surviving the failure of 1 AZ and 1 server in another AZ. The number of servers used in each AZ must be the same.

{% note info %}

Run each static node (data storage node) on a separate server. It is possible to combine static and dynamic nodes on the same server, as well as host multiple dynamic nodes on a single server if sufficient computing resources are available.
...
Traffic protection and server node authentication in {{ ydb-short-name }} are handled using the TLS protocol. Before installing the cluster, you must plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
* `ca.crt` — the certification authority (CA) certificate that signs the other TLS certificates (same files on all cluster nodes);
...
The required parameters for certificate generation are determined by organizational policy. Typically, {{ ydb-short-name }} certificates and keys are generated with the following parameters:

* RSA keys with a length of 2048 or 4096 bits;
* SHA-256 signature algorithm with RSA encryption for certificates;
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The following usages must be enabled for the certificates: "Digital Signature, Key Encipherment", and the extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
## Create a System User and Group for {{ ydb-short-name }} to Run Under {#create-user}

On each server where {{ ydb-short-name }} will be running, execute the following commands:
...
After modifying the file, you need to reboot the system or log in again for the new limits to take effect.
...
1. Download and extract the archive with the `ydbd` executable and the libraries required for {{ ydb-short-name }} to operate:
...
### Example of a Full Command for Partitioning Three Disks
...
To verify that the disks have been partitioned correctly, run the following command on each cluster server:
...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for installing the cluster. It is sufficient to replace the standard FQDN hosts with the actual ones in the `hosts` and `blob_storage_config` sections.
...
The prepared TLS keys and certificates must be copied to a secure directory on each node of the {{ ydb-short-name }} cluster. Below are example commands for creating a secure directory and copying the key and certificate files.
...
- Using systemd
  ...
  Create a systemd configuration file `/etc/systemd/system/ydbd-storage.service` on each server that will host a static cluster node, using the template below. You can also [download the template from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
...
After starting the static nodes, check their functionality via the {{ ydb-short-name }} Embedded UI:
...
To initialize the cluster, you need the registration authority certificate file `ca.crt`, the path to which must be specified when running the corresponding commands. Before running the commands, copy the `ca.crt` file to the server where the commands will be executed.
...
To execute the administrative command to create a database, you need the registration authority certificate file `ca.crt`, following the same procedure as described above for cluster initialization.
...
Start additional dynamic nodes on other servers to scale and ensure the database's fault tolerance.

## Setting Up User Accounts {#security-setup}
...
In the command examples above, `<node.ydb.tech>` is the FQDN of a server running any dynamic node serving the `/Root/testdb` database. When connecting via SSH to a dynamic {{ ydb-short-name }} node, it's convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.

When executing commands to create user accounts and assign groups, the {{ ydb-short-name }} CLI client will prompt for the `root` user's password. To avoid entering the password multiple times, you can create a connection profile, as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).
...
1. Create a test row-based (`test_row_table`) or column-based table (`test_column_table`):

{% list tabs %}

- Creating a row-based table
...
If authentication is enabled in the cluster, your web browser will prompt you to enter a username and password. After entering the correct authentication data, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).
...
The installation procedure described above deploys {{ ydb-short-name }} in the standard secured mode.
...
Installing {{ ydb-short-name }} to operate in unsecured mode follows the procedure described above, with the following exceptions:
...
1. When accessing the database from the {{ ydb-short-name }} CLI and applications, the `grpc` protocol is used instead of `grpcs`, and authentication is not used.
