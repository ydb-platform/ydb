...
{% endnote %}[конфигурацией V1](../../configuration-management/configuration-v1/index.md)[конфигурацией V2](../../configuration-management/configuration-v2/index.md)
...
You must have SSH access to all servers. This is required to install the artifacts and run the {{ ydb-short-name }} executable file.[системными требованиями](../../../devops/concepts/system-requirements.md)[топологией кластера](../../../concepts/topology.md)
...
### Preparing TLS keys and certificates {#tls-certificates}[{#T}](../../../devops/concepts/system-requirements.md) {#tls-certificates}
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. Node certificates must have the following uses enabled: "Digital Signature, Key Encipherment" and extended uses "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
## Create a system user and group on behalf of which {{ ydb-short-name }} will run {#create-user}[примером скрипта](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/tls_cert_gen/) {#create-user}
...
To allow the {{ ydb-short-name }} service access to block disks, add the user under which {{ ydb-short-name }} processes will run to the `disk` group:
...
To change the file descriptor limit, add the following lines to the `/etc/security/limits.conf` file:[спиллинга](../../../concepts/query_execution/spilling.md)
...
{% endnote %}[requirements](../../../reference/configuration/table_service_config.md#file-system-requirements)

## Install {{ ydb-short-name }} software on each server {#install-binaries}

1. Download and extract the archive with the `ydbd` executable file and the libraries required for {{ ydb-short-name }} to work:
...
1. Copy the executable file and libraries to the appropriate directories:[загрузок](../../../downloads/index.md)
...
You can get a list of block devices on the server using the `lsblk` command. Example output:[_includes/storage-device-requirements.md](../../../_includes/storage-device-requirements.md)
...
2. Clean the disk using the command built into the `ydbd` executable file:
    ...
    Data on the disk will be erased after running the command.
...
## Copy TLS keys and certificates to each server {#tls-copy-cert}[{#T}](../../../reference/configuration/index.md) {#tls-copy-cert}
...
Create a systemd configuration file `/etc/systemd/system/ydbd-storage.service` on each server where a static cluster node will be located, using the example below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
  ...
  After=network-online.target rc-local.service[скачать из репозитория](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service)
...
To initialize the cluster, you need the `ca.crt` registration authority certificate file, the path to which must be specified when running the corresponding commands. Before running the commands, copy the `ca.crt` file to the server where the commands will be executed.[Ручная установка, запущенные статические узлы](../_assets/manual_installation_1.png)

On one of the storage servers in the cluster, run the following commands:[Ручная установка, запущенные статические узлы](../_assets/manual_installation_1.png)
...
To execute the administrative command to create a database, you need the `ca.crt` registration authority certificate file, following the procedure described above for cluster initialization.
...
Create a systemd config file `/etc/systemd/system/ydbd-testdb.service` using the example below. You can also [download the sample file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-testdb.service).
  ...
  [Service]
  Restart=always[скачать из репозитория](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-testdb.service)
  RestartSec=1
  User=ydb
  PermissionsStartOnly=true
  StandardOutput=syslog
...
Launch additional dynamic nodes on other servers to scale and ensure the database's fault tolerance.
...
In the command examples listed above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node serving the `/Root/testdb` database. When connecting to a {{ ydb-short-name }} dynamic node via SSH, it's convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.

When executing commands to create accounts and assign groups, the {{ ydb-short-name }} CLI client will prompt for the `root` user's password. You can avoid entering the password multiple times by creating a connection profile as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).
...
- Creating a columnar table
    ```bash
    ydb --ca-file ca.crt -e grpcs://<node.ydb.tech>:2136 -d /Root/testdb --user root \
        yql -s 'CREATE TABLE `testdir/test_column_table` (id Uint64 NOT NULL, title Utf8, PRIMARY KEY (id)) WITH (STORE = COLUMN);'[документации](../../../reference/ydb-cli/install.md)
    ```
{% endlist %}
...
The installation procedure described above deploys {{ ydb-short-name }} in the standard secure mode.[{#T}](../../../reference/embedded-ui/index.md)

The insecure mode of {{ ydb-short-name }} is intended for solving test tasks primarily related to the development and testing of {{ ydb-short-name }} software. In insecure mode:[{#T}](../../../reference/embedded-ui/index.md)
