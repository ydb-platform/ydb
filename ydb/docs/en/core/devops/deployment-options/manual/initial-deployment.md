...
This instruction is intended only for deploying clusters with [Configuration V1](../../configuration-management/configuration-v1/index.md). Deployment of clusters with [Configuration V2](../../configuration-management/configuration-v2/index.md) is currently under development.
...
* Use the `mirror-3-dc` fault tolerance scheme to deploy a cluster across three availability zones (AZs), using at least 9 servers. This scheme allows surviving the failure of 1 AZ and 1 server in another AZ. The number of servers involved in each AZ must be the same.
...
Traffic protection and server node authentication in {{ ydb-short-name }} are implemented using the TLS protocol. Before installing the cluster, you must plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
The required parameters for certificate generation are determined by the organization's policy. Typically, certificates and keys for {{ ydb-short-name }} are generated with the following parameters:
...
The CA certificate must be marked accordingly: the CA flag must be set, and the following usages must be included: "Digital Signature, Non Repudiation, Key Encipherment, Certificate Sign".

For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The certificates must include the usages "Digital Signature, Key Encipherment" and the extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (option `extendedKeyUsage = serverAuth,clientAuth` in OpenSSL settings).
...
## Create a system user and group to run {{ ydb-short-name }} {#create-user}
...
1. Create a row-based (`test_row_table`) or column-based table (`test_column_table`):
...
If authentication is enabled in the cluster, the web browser will display a login and password prompt. After entering the correct authentication credentials, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).
