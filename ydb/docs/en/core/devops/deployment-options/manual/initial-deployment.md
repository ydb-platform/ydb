...
Run each static node (data storage node) on a separate server. It is possible to combine static and dynamic nodes on the same server, as well as to place multiple dynamic nodes on one server if there are sufficient computing resources.
...
Traffic protection and server node authentication in {{ ydb-short-name }} is implemented using the TLS protocol. Before installing the cluster, you must plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The following usages must be enabled for the certificates: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
2. Clean the disk using the command built into the `ydbd` executable file:
    ...
    Running this command will erase the data on the disk.
...
{% endlist %}After starting the static nodes, check their functionality through the {{ ydb-short-name }} built-in web interface (Embedded UI):
...
{% endlist %}Launch additional dynamic nodes on other servers to scale and ensure the database's fault tolerance.
...
1. Create a test row (`test_row_table`) or column (`test_column_table`) table:{% list tabs %}
...
{% endlist %}Where `<node.ydb.tech>` is the FQDN of the server running the dynamic node that serves the `/Root/testdb` database.

## Checking access to the embedded web interface

To check access to the {{ ydb-short-name }} embedded web interface, open the page with the address `https://<node.ydb.tech>:8765` in your web browser, where `<node.ydb.tech>` is the FQDN of the server running any static {{ ydb-short-name }} node.

Your web browser must trust the certificate authority that issued the certificates for the {{ ydb-short-name }} cluster; otherwise, you'll see a warning about an untrusted certificate.

If authentication is enabled in the cluster, your web browser will prompt for a username and password. After entering the correct authentication credentials, the initial page of the embedded web interface should appear. For a description of the available functions and user interface, see [the section](../../../reference/embedded-ui/index.md).

{% note info %}

Typically, to provide access to the {{ ydb-short-name }} embedded web interface, a highly available HTTP load balancer based on software like `haproxy`, `nginx`, or similar is configured. The details of configuring an HTTP load balancer are beyond the scope of the standard {{ ydb-short-name }} installation instructions.
...
1. When preparing for installation, you don't need to generate TLS certificates and keys, and you don't copy certificates and keys to the cluster nodes.
