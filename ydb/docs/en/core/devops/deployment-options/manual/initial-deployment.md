...
Make sure that the system clocks on all cluster servers are synchronized using `ntpd` or `chrony` tools. It is advisable to use a single time source for all cluster servers to ensure consistent handling of leap seconds.

If the Linux type used on the cluster servers uses `syslogd` for logging, you must configure log rotation using `logrotate` or its analogs. {{ ydb-short-name }} services can generate a significant amount of system logs, especially when increasing the logging level for diagnostic purposes, so it is important to enable system log rotation to prevent `/var` filesystem overflow.
...
* Use the `mirror-3-dc` fault tolerance scheme to deploy a cluster in three availability zones (AZ), using at least 9 servers. This scheme allows you to withstand the failure of 1 AZ and 1 server in another AZ. The number of servers in each AZ must be the same.
...
For more details on hardware requirements, see the [System Requirements](../../../devops/concepts/system-requirements.md) section.

### Preparing TLS keys and certificates {#tls-certificates}

{{ ydb-short-name }} traffic protection and server node authentication are implemented using the TLS protocol. Before installing the cluster, you need to plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
* `node.key` — TLS private keys for each cluster node (a separate key for each server in the cluster);
* `node.crt` — TLS certificates for each cluster node (certificate corresponding to the key);
* `web.pem` — concatenation of the node's private key, node certificate, and CA certificate for the HTTP monitoring interface (a separate file for each server in the cluster).

The required certificate generation parameters are determined by organizational policy. Typically, {{ ydb-short-name }} certificates and keys are generated with the following parameters:

* RSA keys with a length of 2048 or 4096 bits;
* SHA-256 signature algorithm with RSA encryption;
* node certificates with a validity period of at least 1 year;
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The certificates must include the following usages: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". The node certificates must support both server and client authentication (option `extendedKeyUsage = serverAuth,clientAuth` in OpenSSL settings).

To generate or update {{ ydb-short-name }} cluster certificates in bulk using OpenSSL, you can use the [script example](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/tls_cert_gen/) from the {{ ydb-short-name }} GitHub repository. The script allows you to automatically generate the necessary key and certificate files for the entire set of cluster nodes in a single operation, simplifying preparation for installation.
...
After modifying the file, you need to reboot the system or relogin to apply the new limits.
...
1. Download and extract the archive with the `ydbd` executable and the libraries required for {{ ydb-short-name }} to work:
...
1. Create partitions on the selected disks:
    ...
    Run the command `ls -l /dev/disk/by-partlabel/` to verify that the system has a disk with the label `/dev/disk/by-partlabel/ydb_disk_ssd_01`.
...
2. Clean the disk using the command built into the `ydbd` executable:
    ...
    Perform this operation on each disk that will be used for {{ ydb-short-name }} data storage.
...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for cluster installation. You just need to replace the standard FQDN hosts with actual ones in the `hosts` and `blob_storage_config` sections.
...
For more detailed information on creating the configuration file, see the [Configuration](../../../reference/configuration/index.md) section.

## Copy TLS keys and certificates to each server {#tls-copy-cert}

The prepared TLS keys and certificates must be copied to a secure directory on each cluster node. Below is an example of commands to create a secure directory and copy the key and certificate files.
...
- Manually
  ...
  Start the {{ ydb-short-name }} data storage service on each static cluster node:
...
- Using systemd
  ...
  Create a systemd configuration file `/etc/systemd/system/ydbd-storage.service` on each server hosting a static cluster node, using the example below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
...
After starting the static nodes, check their functionality through the {{ ydb-short-name }} Embedded UI:
...
To initialize the cluster, you need the CA certificate file `ca.crt`, the path to which must be specified when executing the corresponding commands. Before running the commands, copy the `ca.crt` file to the server where the commands will be executed.

On one of the storage servers in the cluster, run the commands:

First, get an authorization token for request registration by running the command below.
...
To work with string or columnar tables, you need to create at least one database and start the processes that serve this database (dynamic nodes).

To execute the administrative command to create a database, you need the CA certificate file `ca.crt`, following the same procedure as described for cluster initialization.

When creating a database, the initial number of storage groups is set, which determines the available I/O throughput and maximum storage capacity. The number of storage groups can be increased after database creation if needed.
...
If the database creation is successful, the command completion code displayed on the screen should be zero.

The commands above use the following parameters:
...
* `ssd:8` — specifies the storage pool for the database and the number of groups in it. The pool name (`ssd`) must match the disk type specified in the cluster configuration (for example, in `default_disk_type`) and is case-insensitive. The number after the colon is the number of storage groups to allocate.
...
## Set up user accounts {#security-setup}
...
1. Set a password for the `root` account using the previously obtained token:
    ...
    Replace `passw0rd` with the desired password. Save the password in a separate file. Subsequent commands as the `root` user will use the password passed via the `--password-file <path_to_user_password>` key. You can also save the password in a connection profile as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).
...
1. Set user account permissions by adding them to built-in groups:
...
In the command examples above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node serving the `/Root/testdb` database. When connecting to a {{ ydb-short-name }} dynamic node via SSH, it is convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.

When executing commands to create user accounts and assign groups, the {{ ydb-short-name }} CLI will prompt for the `root` user password. To avoid repeatedly entering the password, you can create a connection profile as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).

## Test database functionality {#try-first-db}

1. Install {{ ydb-short-name }} CLI as described in the [documentation](../../../reference/ydb-cli/install.md).

1. Create a test string (`test_row_table`) or columnar table (`test_column_table`):
{% list tabs %}
...
To check access to the built-in {{ ydb-short-name }} web interface, simply open the page with the address `https://<node.ydb.tech>:8765` in your web browser, where `<node.ydb.tech>` is the FQDN of the server running any static {{ ydb-short-name }} node.
...
* user authentication is not used (enabling authentication without encrypting traffic makes no sense, as usernames and passwords would be transmitted over the network in plain text in such a configuration).
