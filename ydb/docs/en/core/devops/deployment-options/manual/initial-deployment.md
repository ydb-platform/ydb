...
* Use the `mirror-3-dc` fault tolerance scheme to deploy the cluster in three availability zones (AZs), using at least 9 servers. This scheme allows you to withstand the failure of 1 AZ and 1 server in another AZ. The number of servers involved in each AZ must be the same.
...
For more details on hardware requirements, see the [{#T}](../../../devops/concepts/system-requirements.md).
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The certificates must include the following usages: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
For more information about spill configuration and its relation to file descriptors, see the section [Spill Configuration](../../../reference/configuration/table_service_config.md#file-system-requirements).
...
- Using systemd[скачать из репозитория](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service)
  ...
  Create a systemd configuration file `/etc/systemd/system/ydbd-storage.service` on each server where a static cluster node will be hosted, using the example below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
...
To initialize the cluster, you need the `ca.crt` registration authority certificate file, the path to which must be specified when running the corresponding commands. Before running the corresponding commands, copy the `ca.crt` file to the server where these commands will be executed.

On one of the storage servers in the cluster, run the following commands:

First, get the authorization token for registering requests. To do this, run the command below.
...
When you create a database, you set the initial number of storage groups, which determines the available I/O throughput and maximum storage capacity. The number of storage groups can be increased after the database is created if needed.
...
When a database is successfully created, the command completion code displayed on the screen should be zero.
...
- Manually
  ...
  Run a dynamic {{ ydb-short-name }} node for the `/Root/testdb` database:
...
1. Set the permissions for user accounts by adding them to the built-in groups:
...
In the command examples listed above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node that serves the `/Root/testdb` database. When connecting via SSH to a dynamic {{ ydb-short-name }} node, it is convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.[документации {{ ydb-short-name }} CLI](../../../reference/ydb-cli/profile/index.md)
...
## Test working with the created database {#try-first-db}[документации](../../../reference/ydb-cli/install.md) {#try-first-db}
...
To check access to the embedded {{ ydb-short-name }} web interface, simply open the page with the address `https://<node.ydb.tech>:8765` in your web browser, where `<node.ydb.tech>` is the FQDN of the server running any static {{ ydb-short-name }} node.
...
If authentication is enabled in the cluster, your web browser will prompt you to enter a username and password. After entering the correct authentication credentials, the initial page of the embedded web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).

{% note info %}

Typically, to provide access to the embedded {{ ydb-short-name }} web interface, a fault-tolerant HTTP load balancer based on software like `haproxy`, `nginx`, or similar is configured. The details of configuring an HTTP load balancer are beyond the scope of the standard {{ ydb-short-name }} installation instructions.
...
1. When preparing for installation, you do not need to generate TLS certificates and keys, and you do not copy certificates and keys to the cluster nodes.
