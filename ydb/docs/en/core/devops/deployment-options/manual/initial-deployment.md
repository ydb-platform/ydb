...
- Authentication enabled
  ...
  If the command above is executed successfully, the authentication token will be written to `auth_token`. Copy the token file to one of the storage servers in the cluster, then run the following commands on the server:
...
- Using systemd
  ...
  ExecStart=/opt/ydb/bin/ydbd server \
      ...
      --mon-port 8766 --mon-cert /opt/ydb/certs/web.pem \
      --kafka-port 9093 \
      --config-dir /opt/ydb/cfg --tenant /Root/testdb \
      --node-broker grpcs://<ydb1>:2135 \
      --node-broker grpcs://<ydb2>:2135 \
      --node-broker grpcs://<ydb3>:2135
  LimitNOFILE=65536
...
1. Set the password for the `root` account:
  ...
  Replace the value `passw0rd` with the required password. Save the password in a separate file. Subsequent commands as the root user will be executed using the password passed with the `--password-file <path_to_root_pass_file>` option. Additionally, the password can be saved in the connection profile, as described in the [documentation for {{ ydb-short-name }} CLI](../../../reference/ydb-cli/profile/index.md).
