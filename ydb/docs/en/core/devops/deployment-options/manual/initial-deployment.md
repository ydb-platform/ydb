...
When placing multiple dynamic nodes on a single server, separate ports are required for gRPC, Interconnect, HTTP interface, and Kafka API for each dynamic node on the server.
...
Run each static node (storage node) on a separate server. It is possible to co-locate static and dynamic nodes on the same server, as well as place multiple dynamic nodes on a single server if sufficient computing resources are available.
...
The certificate of the registration authority must be marked accordingly: the CA flag must be set, and the key usages "Digital Signature, Non Repudiation, Key Encipherment, Certificate Sign" must be enabled.
...
For the {{ ydb-short-name }} service to have access to block disks for operation, you need to add the user under which the {{ ydb-short-name }} processes will be run to the `disk` group:
...
The names of block devices depend on the operating system settings, either default or manually configured. Typically, device names consist of three parts:
...
More detailed information on creating the configuration file is provided in the [{#T}](../../../reference/configuration/index.md) section.
...
After starting the static nodes, verify their operability via the {{ ydb-short-name }} built-in web interface (Embedded UI):
...
In the web browser, trust must be configured in the certificate authority that issued certificates for the {{ ydb-short-name }} cluster; otherwise, a warning about using an untrusted certificate will be displayed.
...
The unsecured mode of {{ ydb-short-name }} is designed for test tasks, primarily related to software development and testing {{ ydb-short-name }}. In unsecured mode:
...
1. When preparing for installation, you do not need to generate TLS certificates and keys, and copying certificates and keys to cluster nodes is not performed.
...
3. A simplified version of the startup commands for static and dynamic cluster nodes is used: options with certificate and key file names are excluded, the `grpc` protocol is used instead of `grpcs` when specifying connection endpoints.
4. The unnecessary step of obtaining an authentication token before performing cluster initialization and database creation is skipped in insecure mode.
