...
{{ ydb-short-name }} server node traffic protection and authentication is implemented using the TLS protocol. Before installing the cluster, you must plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The following usages must be enabled for the certificates: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (option `extendedKeyUsage = serverAuth,clientAuth` in OpenSSL settings).
...
1. Create partitions on the selected disks:
    ...
    Run the `ls -l /dev/disk/by-partlabel/` command to make sure that a disk with the `ydb_disk_ssd_01` label has appeared in the system.
...
The prepared TLS keys and certificates must be copied to a secure directory on each {{ ydb-short-name }} cluster node. Below is an example of commands to create a secure directory and copy key and certificate files.
...
If authentication is enabled in the cluster, your web browser will prompt you for a username and password. After entering the correct authentication credentials, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).
