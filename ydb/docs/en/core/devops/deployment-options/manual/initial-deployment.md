...
When placing multiple dynamic nodes on one server, separate ports for gRPC, Interconnect, the HTTP interface, and Kafka API are required for each dynamic node within the server.
...
* `node.crt` — TLS certificates for each of the cluster nodes (certificate corresponding to the key);
...
* SHA-256 signing algorithm for certificates with RSA encryption;
...
1. Download and extract the archive with the `ydbd` executable and the libraries required for {{ ydb-short-name }} to run:
...
To initialize the cluster, you need the registration authority certificate file `ca.crt`, the path to which must be specified when executing the corresponding commands. Before running the commands, copy the `ca.crt` file to the server where the commands will be executed.

On one of the storage servers in the cluster, run the following commands:

First, get an authorization token for request registration. Run the command below for this.
...
When executing commands to create user accounts and assign groups, the {{ ydb-short-name }} CLI will prompt for the `root` user's password. To avoid entering the password multiple times, you can create a connection profile, as described in the [{{ ydb-short-name }} CLI documentation](../../../reference/ydb-cli/profile/index.md).
...
If authentication is enabled in the cluster, your web browser will prompt you to enter a username and password. After entering the correct authentication credentials, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).
