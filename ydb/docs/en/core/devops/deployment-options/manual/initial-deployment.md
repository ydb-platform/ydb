...
Read about [system requirements](../../../devops/concepts/system-requirements.md) and [cluster topology](../../../concepts/topology.md).

You must have SSH access to all servers. This is necessary to install artifacts and run the {{ ydb-short-name }} executable file.
...
If you place several dynamic nodes on one server, you will need separate ports for gRPC, Interconnect, HTTP interface, and Kafka API for each dynamic node within the server.
...
If the Linux type used on the cluster servers uses `syslogd` for logging, it is necessary to set up log file rotation using the `logrotate` tool or its analogs. {{ ydb-short-name }} services can generate a significant amount of system logs, especially when increasing the logging level for diagnostic purposes, so it is important to enable system log file rotation to prevent /var filesystem overflow.
...
The necessary parameters for generating certificates are determined by the organization's policy. Typically, certificates and keys for {{ ydb-short-name }} are generated with the following parameters:

* RSA keys with a length of 2048 or 4096 bits;
* certificate signing algorithm SHA-256 with RSA encryption;
* the validity period of node certificates is at least 1 year;
...
On each server where {{ ydb-short-name }} will be launched, execute:
```bash
sudo groupadd ydb
...
2. Clean the disk with the command built into the `ydbd` executable file:
{% note warning %}
  ...
  client_certificate_definitions:
    - member_groups: ["registerNode@cert"]
      ...
      - short_name: "O"
        values: ["YDB"]
```
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for cluster installation. It is enough to replace the standard FQDN hosts with the actual ones in the `hosts` and `blob_storage_config` sections.
...
- Using systemd
  ...
  Create a systemd config file `/etc/systemd/system/ydbd-storage.service` on each server where a static cluster node will be located, using the template below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-storage.service).
  ```ini
  [Unit]
...
To initialize the cluster, you need a registration authority certificate file `ca.crt`, the path to which must be specified when executing the corresponding commands. Before running the corresponding commands, copy the `ca.crt` file to the server on which these commands will be executed.
...
To work with string or columnar tables, you need to create at least one database and start the process or processes that service this database (dynamic nodes).
...
Create a systemd config file `/etc/systemd/system/ydbd-testdb.service` using the template below. You can also [download the file from the repository](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/systemd_services/ydbd-testdb.service).
  ```ini
  [Unit]
...
The web browser must trust the registration authority that issued the certificates for the {{ ydb-short-name }} cluster, otherwise a warning about the use of an untrusted certificate will be displayed.

If authentication is enabled in the cluster, the web browser should display a login and password request. After entering the correct authentication data, the initial page of the built-in web interface should be displayed. A description of the available functions and user interface is provided in the section [{#T}](../../../reference/embedded-ui/index.md).
{% note info %}
...
The insecure mode of {{ ydb-short-name }} is intended for solving test tasks mainly related to the development and testing of {{ ydb-short-name }} software. In insecure mode:
...
Installing {{ ydb-short-name }} for operation in insecure mode is carried out in the manner described above, with the following exceptions:
