...
The prepared TLS keys and certificates must be copied to a secure directory on each {{ ydb-short-name }} cluster node. Below is an example of commands for creating a secure directory and copying key and certificate files.
...
To initialize the cluster, you need the registration authority certificate file `ca.crt`, the path to which must be specified when running the corresponding commands. Before running the corresponding commands, copy the `ca.crt` file to the server where these commands will be executed.
...
If authentication is enabled in the cluster, your web browser will prompt you to enter your login and password. After entering the correct authentication data, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../../reference/embedded-ui/index.md).
...
- User authentication is not used (enabling authentication without encrypting traffic makes no sense, as the login and password would be transmitted over the network in plain text in such a configuration).
