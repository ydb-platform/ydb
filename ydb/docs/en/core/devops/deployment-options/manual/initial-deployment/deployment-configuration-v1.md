...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for installing the cluster. It is enough to replace the standard FQDN hosts with actual ones in the `hosts` and `blob_storage_config` sections.
