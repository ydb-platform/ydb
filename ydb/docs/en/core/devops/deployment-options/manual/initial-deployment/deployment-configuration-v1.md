...
Prepare the configuration file {{ ydb-short-name }} depending on the topology you have chosen (see [topology selection](../../../deployment-options/ansible/initial-deployment/deployment-preparation.md#topology-select)). Examples for each supported topology are provided below in the tabs — select and use the one suitable for your case.
Also, if you need to enable Kafka API support for topics, add the kafka_proxy_config section to the configuration file (see [configuring the Kafka API](../../../reference/configuration/kafka_proxy_config)).
...
Save the configuration file {{ydb-short-name}} under the name `/opt/ydb/cfg/config.yaml` on each server of the cluster.
...
4. Set account permissions by adding them to built-in groups:
...
Where `<node.ydb.tech>` is the FQDN of the server running the dynamic node serving the database `/Root/testdb`.
