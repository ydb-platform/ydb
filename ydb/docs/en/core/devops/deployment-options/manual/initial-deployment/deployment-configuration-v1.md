...
Before deploying the system, make sure to complete the preparatory steps. Refer to the document [{#T}](deployment-preparation.md).

## Prepare configuration files {#config}

Prepare the {{ ydb-short-name }} configuration file based on the topology you have chosen (see [selecting a topology](../../../deployment-options/ansible/initial-deployment/deployment-preparation.md#topology-select)). Examples for each supported topology are provided in the tabs below — select and use the one that fits your case.

Also, if you need to enable Kafka API with topics, add the kafka_proxy_config section to the configuration file (see [configuring Kafka API](../../../reference/configuration/kafka_proxy_config)).{% list tabs %}
...
{% endlist %}To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already includes most of the settings for cluster installation. You just need to replace the standard FQDN hosts with the actual ones in the `hosts` and `blob_storage_config` sections.
...
For more detailed information on creating a configuration file, see [the documentation](../../../../reference/configuration/index.md).
...
## Start static nodes {#start-storage}{% list tabs group=manual-systemd %} {#start-storage}
...
{% endlist %}After starting the static nodes, check their functionality via the {{ ydb-short-name }} embedded web interface (Embedded UI):
...
To execute the administrative command to create a database, you need a registration authority certificate file `ca.crt`, following the same procedure as described above for cluster initialization.
...
## Start dynamic nodes {#start-dynnode}{% list tabs group=manual-systemd %} {#start-dynnode}
...
In the command examples listed above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node serving the `/Root/testdb` database. When connecting to a dynamic {{ ydb-short-name }} node via SSH, it's convenient to use the `grpcs://$(hostname -f):2136` construct to obtain the FQDN.
...
1. When preparing for installation, you do not need to generate TLS certificates and keys, nor copy them to the cluster nodes.
