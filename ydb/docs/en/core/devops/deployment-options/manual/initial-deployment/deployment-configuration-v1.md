...
Save the {{ydb-short-name}} configuration file as `/opt/ydb/cfg/config.yaml` on each server in the cluster.
...
- user authentication is not used (enabling authentication without traffic encryption would be meaningless, as the login and password would be transmitted over the network in plain text in such a configuration).
