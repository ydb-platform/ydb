...
echo $?
```
Upon successful cluster initialization, the command completion code displayed on the screen should be zero.
...
To check access to the built-in web interface of {{ ydb-short-name }}, simply open the page with the address `https://<node.ydb.tech>:8765` in a web browser, where `<node.ydb.tech>` is the FQDN of the server running any static node of {{ ydb-short-name }}.
...
If authentication is enabled in the cluster, the web browser will prompt for a username and password. After entering the correct authentication credentials, the initial page of the built-in web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../../reference/embedded-ui/index.md).
{% note info %}
...
To install {{ ydb-short-name }} in insecure mode, follow the procedure described above, with the following exceptions:
