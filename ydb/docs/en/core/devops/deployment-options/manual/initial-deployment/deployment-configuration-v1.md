...
To check access to the built-in web interface {{ ydb-short-name }}, simply open the page with the address `https://<node.ydb.tech>:8765` in a web browser, where `<node.ydb.tech>` is the FQDN of the server on which any static node {{ ydb-short-name }} is running.
