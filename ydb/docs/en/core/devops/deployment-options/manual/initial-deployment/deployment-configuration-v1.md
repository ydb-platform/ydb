...
Save the {{ydb-short-name}} configuration file under the name `/opt/ydb/cfg/config.yaml` on each server of the cluster.
...
1. Install the {{ ydb-short-name }} CLI as described in the [documentation](../../../../reference/ydb-cli/install.md).
2. Create a test row table (`test_row_table`) or columnar table (`test_column_table`):
...
To verify access to the built-in web interface {{ ydb-short-name }}, simply open the page with the address `https://<node.ydb.tech>:8765` in a web browser, where `<node.ydb.tech>` is the FQDN of the server running any static node {{ ydb-short-name }}.
