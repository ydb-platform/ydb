...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for cluster installation. You just need to replace the default FQDN hosts in the `hosts` section and the disk paths in the `hosts_configs` section with the actual ones.
...
To check access to the {{ ydb-short-name }} embedded web interface, simply open the page `https://<node.ydb.tech>:8765` in your web browser, where `<node.ydb.tech>` is the FQDN of the server running any static {{ ydb-short-name }} node.
...
If authentication is enabled in the cluster, your web browser will prompt you for a username and password. After entering the correct authentication credentials, the initial page of the embedded web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../../reference/embedded-ui/index.md).
...
* user authentication is not used (enabling authentication without traffic encryption would be meaningless, as usernames and passwords would be transmitted over the network in plain text in such a configuration).
