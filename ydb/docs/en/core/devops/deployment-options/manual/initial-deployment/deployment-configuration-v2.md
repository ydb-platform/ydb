...
If you need to enable Kafka API with topics, add the kafka_proxy_config section to the configuration file (see [configuring Kafka API](../../../reference/configuration/kafka_proxy_config)).
...
The original file `/tmp/config.yaml` is no longer used after running this command; you can delete it.
...
To initialize the cluster, you need the `ca.crt` registration authority certificate file, the path to which must be specified when executing the corresponding commands. Before running the commands, copy the `ca.crt` file to the server where the commands will be executed.
...
When you create a database, the initial number of storage groups is set, which determines the available I/O throughput and maximum storage capacity. The number of storage groups can be increased after the database is created if needed.
...
If authentication is enabled in the cluster, your web browser will prompt you to enter your login and password. After entering the correct authentication data, the initial page of the embedded web interface should appear. A description of the available functions and user interface is provided in the section [{#T}](../../../../reference/embedded-ui/index.md).
