...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most of the settings for installing the cluster. You just need to replace the default FQDN hosts in the `hosts` section and the disk paths with the actual ones in the `hosts_configs` section.
...
4. The unnecessary step of obtaining an authentication token before performing cluster initialization and database creation is skipped in unsecured mode.
