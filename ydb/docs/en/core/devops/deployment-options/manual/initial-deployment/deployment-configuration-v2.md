...
The prepared TLS keys and certificates must be copied to a secure directory on each {{ ydb-short-name }} cluster node. Below are example commands for creating a secure directory and copying key and certificate files.
...
To check access to the built-in {{ ydb-short-name }} web interface, simply open the page `https://<node.ydb.tech>:8765` in your web browser, where `<node.ydb.tech>` is the FQDN of the server running any static {{ ydb-short-name }} node.
