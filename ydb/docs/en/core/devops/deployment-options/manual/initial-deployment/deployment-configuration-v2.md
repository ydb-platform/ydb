...
To speed up and simplify the initial deployment of {{ ydb-short-name }}, the configuration file already contains most settings for setting up the cluster. You just need to replace the default FQDN hosts in the `hosts` section and the disk paths with the actual ones in the `hosts_configs` section.
...
After running this command, the original file `/tmp/config.yaml` is no longer used and can be deleted.
...
After initializing the cluster, you must obtain an authentication token before running any further administrative commands.
