...
* Use the `mirror-3-dc` fault tolerance scheme to deploy a cluster in three availability zones (AZs), using at least 9 servers. This scheme allows you to withstand the failure of 1 AZ and 1 server in another AZ. The number of servers involved in each AZ must be the same.
...
* `web.pem` — a concatenation of the node's secret key, the node certificate, and the CA certificate for the monitoring HTTP interface (a separate file for each cluster server).
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The certificates must include the following usages: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary for node certificates to support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
To ensure that the {{ ydb-short-name }} service has access to block disks, you must add the user under which the {{ ydb-short-name }} processes will run to the `disk` group:
...
1. Create partitions on the selected disks:
    ...
    Run the `ls -l /dev/disk/by-partlabel/` command to verify that a disk with the `ydb_disk_ssd_01` label has appeared in the system.
