...
### Prepare TLS keys and certificates {#tls-certificates}
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The following usages must be included in the certificates: "Digital Signature, Key Encipherment", and the following extended usages: "TLS Web Server Authentication, TLS Web Client Authentication". It is necessary that node certificates support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
### Download and extract the archive with the `ydbd` executable file and the libraries required for {{ ydb-short-name }} to operate{% list tabs %}
...
{% endlist %}where `binaries_url` is a link to the archive of the version you need from the [downloads page](../../../../downloads/index.md).
...
To verify that the disks have been partitioned correctly, run the command on each cluster server:
...
After completing the preparatory steps, you can proceed with system deployment. Choose the instruction that matches your configuration:
