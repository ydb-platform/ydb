...
* `ca.crt` — Certification Authority (CA) certificate that signs the other TLS certificates (the same files on all cluster nodes);
...
To check that the disks have been partitioned correctly, run the command on each cluster server:
