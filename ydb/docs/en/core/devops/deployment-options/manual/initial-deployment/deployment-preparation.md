...
To change the file descriptor limit, add the following lines to the `/etc/security/limits.conf`file:```bash
...
### Download and unpack the archive with the executable file and the {{ ydb-short-name }} libraries required for operation

{% list tabs %}[requirements](../../../../reference/configuration/table_service_config.md#file-system-requirements)

- OSS [requirements](../../../../reference/configuration/table_service_config.md#file-system-requirements)
...
You can get a list of block devices on the server using the `lsblk`command. Example output:[_includes/storage-device-requirements.md](../../../../_includes/storage-device-requirements.md)```txt
NAME   MAJ:MIN RM   SIZE RO TYPE MOUNTPOINTS
loop0    7:0    0  63.3M  1 loop /snap/core20/1822 [загрузок](../../../../downloads/index.md)
...
...
1. Create partitions on the selected disks:
  ...
  ```bash
    DISK=/dev/vdb
    sudo parted ${DISK} mklabel gpt -s [_includes/storage-device-requirements.md](../../../../_includes/storage-device-requirements.md)
