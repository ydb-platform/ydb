...
If the Linux type used on the cluster servers uses `syslogd` for logging, you must configure log file rotation using `logrotate` or its analogs. {{ ydb-short-name }} services can generate a significant amount of system logs, especially when the logging level is increased for diagnostic purposes, so it is important to enable log file rotation to prevent `/var` filesystem overflow.
...
For node certificates, it is important that the actual hostname (or hostnames) matches the values specified in the "Subject Alternative Name" field. The following usages must be included in the certificates: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". Node certificates must support both server and client authentication (the `extendedKeyUsage = serverAuth,clientAuth` option in OpenSSL settings).
...
To ensure proper operation of {{ ydb-short-name }}, especially when using [spilling](../../../../concepts/query_execution/spilling.md) in multi-node clusters, it is recommended to increase the limit on the number of simultaneously open file descriptors.
...
### Download and extract the archive with the `ydbd` executable file and the libraries required for {{ ydb-short-name }} to work
...
Run the `ls -l /dev/disk/by-partlabel/` command to make sure that the system has a disk with the label `/dev/disk/by-partlabel/ydb_disk_ssd_01`.
...
Perform this operation on each disk that will be used to store {{ ydb-short-name }} data.
...
To check that the disks are partitioned correctly, run the command on each cluster server:
...
After completing the preparatory steps, you can proceed with system deployment. Choose an instruction according to your configuration:

* [Deploy a cluster using configuration V1](deployment-configuration-v1.md)
* [Deploy a cluster using configuration V2](deployment-configuration-v2.md)
