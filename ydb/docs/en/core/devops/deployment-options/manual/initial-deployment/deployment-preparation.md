...
* Use the `mirror-3-dc` fault tolerance scheme to deploy the cluster in three availability zones (AZ), using at least 9 servers. This scheme can withstand the failure of 1 AZ and 1 server in another AZ. The number of servers involved in each AZ must be the same.
...
For node certificates, it is important that the actual hostname(s) match the values specified in the "Subject Alternative Name" field. The certificates must include the following usages: "Digital Signature, Key Encipherment" and extended usages "TLS Web Server Authentication, TLS Web Client Authentication". Node certificates must support both server and client authentication (option `extendedKeyUsage = serverAuth,clientAuth` in OpenSSL settings).
...
On each server where {{ ydb-short-name }} will be running, do the following:
...
### Copy the executable file and libraries to the appropriate directories
  ...
  ```bash
  sudo cp -iR ydbd-stable-linux-amd64/bin /opt/ydb/[_includes/storage-device-requirements.md](../../../../_includes/storage-device-requirements.md)
  sudo cp -iR ydbd-stable-linux-amd64/lib /opt/ydb/
  ```

### Set the owner of files and directories[_includes/storage-device-requirements.md](../../../../_includes/storage-device-requirements.md)
