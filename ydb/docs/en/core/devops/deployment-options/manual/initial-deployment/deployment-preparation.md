...
Traffic protection and server node authentication in {{ ydb-short-name }} is implemented using the TLS protocol. Before installing the cluster, you need to plan the server composition, decide on the node naming scheme and specific names, and prepare TLS keys and certificates.
...
* SHA-256 signature algorithm for certificates with RSA encryption;
...
### Download and extract the archive with the `ydbd` executable file and libraries required for {{ ydb-short-name }}
