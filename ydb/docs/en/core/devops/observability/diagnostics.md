# Diagnosing an isolated cluster

If a cluster is hidden behind network barriers, direct monitoring of its state is impossible. In such cases, you can use a mechanism for saving its state to a file to detect issues in the cluster's internal environment.
...
* `--no-sanitize` — enables a full report by disabling the removal of user table, column, and query names (which are stripped by default).

The command writes the internal state of the cluster nodes to the specified file over a specified period. Every `period` seconds, metrics are collected from each cluster node and written to the file.
