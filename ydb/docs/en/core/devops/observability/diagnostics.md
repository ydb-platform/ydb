...
If a cluster is hidden behind network barriers, direct monitoring of its state is impossible. In such a case, to identify problems in the cluster's internal environment, you can use a mechanism to save its state to a file.
...
* `--no-sanitize` — enables a full report by disabling the sanitization of user table names, column names, and queries (which are stripped by default).

The command writes the internal state of cluster nodes to the specified file over the specified time. Every `period` seconds, metrics are collected from each cluster node and written to the file.
