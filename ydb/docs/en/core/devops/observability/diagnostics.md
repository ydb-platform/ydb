...
* `--no-sanitize` — enables a full report by disabling sanitization of user table names, columns, and queries (which are stripped by default).
...
* Health Check report.
* Queries to the tables of cluster partition `/Root/.sys` and their responses.
