...
- Ansible (with TLS)
  ...
  Check the contents of the generated files `ydbd-storage.yml` and `ydbd-database.yml`. The list of nodes and ports must match the actual cluster topology, including the nodes displayed in the [YDB UI](../../reference/ydb-ui/ydb-monitoring.md).
