...
- Hierarchical navigator for objects: tables, topics, external data sources, external tables, and views.
...
- Visualization of query execution plans (`EXPLAIN` / `EXPLAIN ANALYZE`).
...
- Management of [streaming queries](../../concepts/glossary.md#streaming-query): viewing, modifying, starting, and stopping.
...
1. In the search field, enter `YDB`. Select **YDB** from the list and click **Next**.
...
After connecting, the {{ ydb-short-name }} object hierarchy is displayed in the **Database Navigator** panel. The root node is the connection, and inside it is the path to the database, which contains the following folders:
...
Right-click on the connection and select **Manage Sessions**, or use the menu item **Database → Manage Sessions**. The opened view displays all active sessions with the current query, state, and duration (data from the system view [`.sys/query_sessions`](../../dev/system-views.md#query-sessions)). The **Hide Idle** checkbox hides sessions without an active query.
...
For more information on how the converter works, supported dialects, and limitations, see the article [SQL Dialect Converter to YQL](../sql-dialect-converter.md).
...
DBeaver uses the Eclipse P2 mechanism to detect and install updates. When you install the plugin, DBeaver remembers the source — the URL of the repository. When a new version is published, DBeaver compares the installed version with the version in the repository and offers an update in one of two ways:
