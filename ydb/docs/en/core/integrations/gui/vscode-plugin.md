...
[YDB for VS Code](https://github.com/ydb-platform/ydb-vscode-plugin) is a VS Code extension with native support for {{ ydb-short-name }}. The plugin provides a specialized interface for working with {{ ydb-short-name }} objects: hierarchical navigator for tables, topics, views, external data sources, support for all authentication methods, [YQL](../../concepts/glossary.md#yql) editor with syntax highlighting, visualization of execution plans, session and cluster monitoring, access control management (ACL), built-in [MCP server](https://modelcontextprotocol.io/) for AI assistants, and other features.

## Key plugin features {#features}

- Connection to {{ ydb-name }} with all [authentication](../../security/authentication.md) methods: anonymous, static, token-based, service account, metadata.
...
3. The connection creation form will open. Fill in the fields:
   ...
   | **Monitoring URL** | URL of the [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md), used for the dashboard (auto-filled from the host, can be overridden) | `http://ydb.example.com:8765` |
...
5. Select an authentication method from the **Auth type** dropdown (see [Authentication methods](#auth-methods)).
6. Click **Test Connection** to verify the settings. A success message will appear if the connection is successful.
...
After connecting, click on the connection in the **Connections** panel — the **Navigator** panel opens with the {{ ydb-short-name }} object hierarchy. The Navigator contains the following sections:
...
Execution results are displayed in the **Results** panel as a table, JSON, or diagram (switch between tabs).
...
The dashboard is only available when working with self-hosted {{ ydb-short-name }} installations that have access to the [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md). In Yandex Cloud Managed Service for {{ ydb-short-name }}, the Embedded UI is not published, so the dashboard data is unavailable — for monitoring, use [cloud platform tools](https://yandex.cloud/en/docs/ydb/operations/monitoring).
...
1. In the **Source Dialect** drop-down list, select the source SQL dialect.
...
For more details about how the converter works, supported dialects, and limitations, see the article [SQL dialect converter to YQL](../sql-dialect-converter.md).
...
### Connect Claude Code {#mcp-claude}

1. Make sure the YDB extension is running in VS Code and at least one connection has been added in the **Connections** panel (see [Create a connection](#connection)).
...
### Enable RAG {#rag-enable}

1. When creating or editing a connection, set the **Use RAG** flag.
2. The plugin will automatically detect the {{ ydb-short-name }} version and download the corresponding index from the cloud on first connection.

### Search modes {#rag-modes}

- **Keyword search** — works without additional dependencies, used by default.
- **Semantic search (vector)** — requires a locally running [Ollama](https://ollama.com) with the [`nomic-embed-text`](https://ollama.com/library/nomic-embed-text) embedding model. See [Install Ollama and embedding models](#install-ollama).

The RAG status (Running / Not running) and Ollama status are displayed directly in the connection form.

### Install Ollama and embedding models {#install-ollama}
