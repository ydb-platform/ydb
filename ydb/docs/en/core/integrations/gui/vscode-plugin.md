...
- YQL [editor](../../concepts/glossary.md#yql) with syntax highlighting and auto-completion for tables and columns.
- Executing queries and visualizing results: table, JSON, chart.
- Visualizing the [query execution plan](../../dev/query-execution-optimization/query-plans-optimization.md) (`EXPLAIN`).
...
After installation using any of the methods, restart VS Code. The **YDB** icon will appear in the Activity Bar panel.
...
Authentication using username and password. Specify the username in the **Username** field and the password in the **Password** field. Used if username and password authentication is enabled on the {{ ydb-short-name }} server ([see the documentation](../../security/authentication.md#static-credentials)).
...
| `ydb_yql_help` | `query`, `connection?` | Search the YQL documentation (requires [RAG](#rag) to be enabled) |
...
- **Keyword search (keyword)** — works without additional dependencies and is used by default.
- **Semantic search (vector)** — requires Ollama (locally running) with the `nomic-embed-text` embedding model. See [Installing Ollama and the Embedding Model](#install-ollama).
...
You cannot use a different embedding model: the indexes published by the plugin in the cloud are based on `nomic-embed-text`, and changing the model will make the query and documentation vectors incompatible.
