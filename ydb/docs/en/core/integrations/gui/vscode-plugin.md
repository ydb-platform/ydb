...
Authentication using username and password. Specify the username in the **Username** field and the password in the **Password** field. Used if username and password authentication is enabled on the {{ ydb-short-name }} server ([authentication methods](../../security/authentication.md#static-credentials)).
...
- **Semantic search (vector)** — requires Ollama (locally running) with the embedding model [`nomic-embed-text`](https://ollama.com/library/nomic-embed-text). See [Installing Ollama and the Embedding Model](#install-ollama).
