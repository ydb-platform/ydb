...
- Built-in [MCP server](#mcp) — direct access to databases from AI assistants (Claude Code, etc.).
...
You cannot use another embedding model: the indexes published by the plugin in the cloud are built specifically on `nomic-embed-text`, and if you change the model, the vectors of the query and documentation will become incompatible.
