...
- Embedded [MCP server](#mcp) for direct database access from AI assistants (Claude Code and others).
...
Authentication using username and password. Enter the username in the **Username** field and the password in the **Password** field. Used if {{ ydb-short-name }} server has [username and password authentication](../../security/authentication.md#static-credentials) enabled.
...
Authentication via the [Yandex Cloud metadata service](https://yandex.cloud/en/docs/compute/operations/vm-metadata/get-vm-metadata). The plugin receives an IAM token from the virtual machine metadata service. Used only when VS Code is running on a Yandex Cloud virtual machine.
...
For more information on the converter's operation, supported dialects, and limitations, see the [SQL dialect converter to YQL](../sql-dialect-converter.md) article.
...
The plugin launches an embedded [MCP server](https://modelcontextprotocol.io/) (Model Context Protocol), which allows AI assistants (Claude Code and others) to directly execute queries to {{ ydb-short-name }} databases configured in the plugin.
...
The plugin supports semantic and keyword search of YQL documentation using an embedded RAG index (Retrieval-Augmented Generation). This allows AI assistants (via the `ydb_yql_help` tool) to find relevant fragments of the syntax reference when writing queries.
...
1. Install it over the old version in the same way as during the first installation — VS Code will automatically replace the previous version.
