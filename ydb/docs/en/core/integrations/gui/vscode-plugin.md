...
Authentication using a username and password. Enter the username in the **Username** field and the password in the **Password** field. Used when {{ ydb-short-name }} server has [username and password authentication enabled](../../security/authentication.md#static-credentials).
{% note info %}
...
Open the query workspace using `Ctrl+Shift+Q` (`Cmd+Shift+Q` on macOS) or click **Open Query Workspace** in the context menu of the connection. In the workspace, you can write and execute YQL queries, view history and results.
...
| `ydb_yql_help` | `query`, `connection?` | Search YQL documentation (requires RAG enabled [RAG](#rag)) |
...
You cannot use a different embedding model: the indexes published by the plugin in the cloud are based on `nomic-embed-text`, and if you change the model, the vectors of the query and documentation will become incompatible.
