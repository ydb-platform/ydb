...
- Connecting to {{ ydb-name }} with all [authentication](../../security/authentication.md) methods: anonymous, static, token, service account, metadata.
...
Authentication by the key of a [service account](https://yandex.cloud/en/docs/iam/concepts/users/service-accounts) in Yandex Cloud. Specify the path to the JSON file with the key in the **Service Account Key File** field (use the **Browse** button to select the file). For details on how to create an authorized key, see the [Yandex Cloud documentation](https://yandex.cloud/en/docs/iam/operations/authentication/manage-authorized-keys).
...
After connecting, click on the connection in the **Connections** panel — the **Navigator** panel with the {{ ydb-short-name }} object hierarchy will open. The navigator contains the following sections:
...
The **Database Load** panel (Activity Bar → YDB) displays real-time cluster load (updates every 10 seconds):
...
The dashboard is only available when working with self-hosted {{ ydb-short-name }} installations that have access to the [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md). In Yandex Cloud Managed Service for {{ ydb-short-name }}, the Embedded UI is not published, so dashboard data is unavailable — use [cloud platform tools](https://yandex.cloud/en/docs/ydb/operations/monitoring) for monitoring.
...
The plugin launches a built-in [MCP server](https://modelcontextprotocol.io/) (Model Context Protocol) that allows AI assistants (Claude Code and others) to directly execute queries to {{ ydb-short-name }} databases configured in the plugin.
...
The `connection` parameter — the connection name as it appears in the **Connections** panel.
...
1. When creating or editing a connection, enable the **Use RAG** flag.
...
- **Semantic search (vector)** — requires a locally running [Ollama](https://ollama.com) with the embedding model [`nomic-embed-text`](https://ollama.com/library/nomic-embed-text). See [Installing Ollama and embedding models](#install-ollama).

RAG status (Running / Not running) and Ollama status are displayed directly in the connection form.

### Installing Ollama and embedding models {#install-ollama}

[Ollama](https://ollama.com) — a local server for running language models and embedding models. The plugin accesses it via HTTP to convert documentation texts and search queries into vectors for semantic search.
...
2. Install it over the old version the same way as during the initial installation — VS Code will automatically replace the previous version.
