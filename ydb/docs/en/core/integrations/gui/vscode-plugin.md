...
[YDB for VS Code](https://github.com/ydb-platform/ydb-vscode-plugin) is a VS Code extension with native support for {{ ydb-short-name }}. The plugin provides a specialized interface for working with {{ ydb-short-name }} objects: a hierarchical navigator for tables, topics, views, external data sources, support for all authentication methods, a [YQL][(][(][(](../../concepts/glossary.md#yql)#column-oriented-table)#row-oriented-table)#yql) editor with syntax highlighting, visualization of execution plans, monitoring of sessions and the cluster, access control (ACL) management, a built-in [MCP server](https://modelcontextprotocol.io/) for AI assistants, and other features.
...
- Hierarchical object navigator: tables (row-oriented and column-oriented), topics, views, external data sources, external tables, transfers, streaming queries.
- System objects: system views (`.sys`), resource pools.
- YQL editor with syntax highlighting and auto-completion for tables and columns.
...
1. In the search bar, enter `YDB for VS Code` and select the extension from the `ydb-tech` publisher ([direct link](https://marketplace.visualstudio.com/items?itemName=ydb-tech.ydb-vscode-plugin)).
1. Click **Install**.
...
1. Click **Test Connection** to check the settings. A success message will appear if the connection is successful.
...
Authentication using a username and password. Enter the username in the **Username** field and the password in the **Password** field. This is used when {{ ydb-short-name }} server has [username and password authentication enabled](../../security/authentication.md#static-credentials).
{% note info %}

In managed installations of {{ ydb-name }}, authentication via username and password is disabled: managed services use the centralized access management system of the cloud platform ([IAM](https://yandex.cloud/en/docs/iam/)).
...
The IAM token has a [lifespan — no more than 12 hours](https://yandex.cloud/en/docs/iam/concepts/authorization/iam-token#lifetime) — after which you need to obtain a new one and update it in the connection settings. For long-lived connections, use authentication via a [service account](#auth-service-account) or [metadata service](#auth-metadata).

{% endnote %}
### Service Account Key File {#auth-service-account}

Authentication using the key of a [service account](https://yandex.cloud/en/docs/iam/concepts/users/service-accounts) in Yandex Cloud. Specify the path to the JSON file with the key in the **Service Account Key File** field (use the **Browse** button to select the file). For more information on how to create an authorized key, see the [Yandex Cloud documentation](https://yandex.cloud/en/docs/iam/operations/authentication/manage-authorized-keys).
...
After connecting, click on the connection in the **Connections** panel — the **Navigator** panel will open, showing the hierarchy of {{ ydb-short-name }} objects. The navigator includes the following sections:

- **Tables** — tables (string and columnar), organized by subdirectories according to the path in {{ ydb-short-name }}.
...
Select **Explain YQL Query** in the editor context menu or in the command palette (`Ctrl+Shift+P`) to get the [query execution plan](../../dev/query-execution-optimization/query-plans-optimization.md). The plugin displays the plan's operation tree in text format.
...
- Start and stop the query (**Start** / **Stop**).
...
Right-click on an object in the navigator and select **View Permissions** to view the [access control list (ACL)](../../security/authorization.md#right) assigned to this object.
### DDL generation {#ddl}
...
| `ydb_yql_help` | `query`, `connection?` | Search YQL documentation (requires RAG to be enabled [RAG](#rag)) |
...
- **Semantic search (vector)** — requires Ollama running locally with the [`nomic-embed-text` embedding model](https://ollama.com/library/nomic-embed-text). See [Installing Ollama and the embedding model](#install-ollama).
...
1. Open the connection form in the plugin and make sure that the Ollama status is displayed as **Running**. If the status is **Not running**, check that the Ollama service is running and accessible at `http://localhost:11434`.
{% note info %}
...
You cannot use another embedding model: the indices published by the plugin in the cloud are based on `nomic-embed-text`, and if you change the model, the vectors of the query and documentation will become incompatible.
...
1. Download the new `.vsix` file from the [GitHub Releases page](https://github.com/ydb-platform/ydb-vscode-plugin/releases).
