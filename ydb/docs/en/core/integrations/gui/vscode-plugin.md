...
- Access control (ACL) management: viewing permissions on database objects.
...
- [SQL dialect converter](../sql-dialect-converter.md) from other dialects (PostgreSQL, MySQL, ClickHouse, and others) to YQL.
- Built-in [MCP server](#mcp) for direct database access from AI assistants (Claude Code and others).
- [Semantic search for YQL documentation](#rag) (RAG) to assist AI in writing queries.
...
- Auto-completion of table and column names (based on the connected database schema).
...
Query results are displayed in the **Results** panel as a table, JSON, or chart (switch between tabs).
...
The dashboard is only available when working with self-hosted {{ ydb-short-name }} installations where access to [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md) is available. In Yandex Cloud Managed Service for {{ ydb-short-name }}, Embedded UI is not published, so dashboard data is unavailable — use [cloud platform tools](https://yandex.cloud/ru/docs/ydb/operations/monitoring) for monitoring.
...
For more information on the converter's principles, supported dialects, and limitations, see the [SQL Dialect Converter to YQL](../sql-dialect-converter.md) article.
