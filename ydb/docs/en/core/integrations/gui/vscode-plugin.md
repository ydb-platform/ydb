...
- Cluster dashboard based on [{{ ydb-short-name }} YDB UI](../../reference/ydb-ui/index.md): CPU load, memory usage, network traffic (updates every 10 seconds).
...
3. The connection creation form will open. Fill in the fields:
   ...
   | **Monitoring URL** | URL of [{{ ydb-short-name }} YDB UI](../../reference/ydb-ui/index.md), used for the dashboard (filled in automatically based on the host, can be overridden) | `http://ydb.example.com:8765` |
...
5. Select the authentication method from the **Auth type** drop-down list (see [Authentication methods](#auth-methods)).
...
The dashboard is only available when working with self-hosted {{ ydb-short-name }} installations that have access to [{{ ydb-short-name }} YDB UI](../../reference/ydb-ui/index.md). In Yandex Cloud Managed Service for {{ ydb-short-name }}, the Embedded UI is not published, so dashboard data is unavailable — use [cloud platform tools](https://yandex.cloud/en/docs/ydb/operations/monitoring) for monitoring.
