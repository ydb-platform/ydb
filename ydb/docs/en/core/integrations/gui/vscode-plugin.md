...
- Query execution and visualization of results: table, JSON, chart.
...
- Management of [access rights (ACL)](../../security/authorization.md#right): viewing permissions for database objects.
...
After installation using any of the methods, restart VS Code. The **YDB** icon will appear in the Activity Bar.
...
1. If necessary, specify the path to the custom CA certificate (PEM) in the **CA Certificate File** field — for connections with non-standard TLS. If the field is not filled in, the built-in Yandex Cloud certificate is used.
...
Authentication using username and password. Enter the username in the **Username** field and the password in the **Password** field. Used if username and password authentication is enabled on the {{ ydb-short-name }} server ([username and password authentication](../../security/authentication.md#static-credentials)).
...
- Autocompletion of table and column names (based on the schema of the connected database).
...
### EXPLAIN and Query Plan {#explain}
...
1. The plugin will automatically detect the {{ ydb-short-name }} version and load the appropriate index from the cloud on the first connection.

### Search Modes {#rag-modes}

- **Keyword search** — works without additional dependencies and is used by default.
- **Semantic search (vector)** — requires Ollama running locally ([Ollama](https://ollama.com)) with the `nomic-embed-text` embedding model ([model page](https://ollama.com/library/nomic-embed-text)). See [Installing Ollama and the Embedding Model](#install-ollama).
...
You cannot use a different embedding model: the indexes published by the plugin in the cloud are built specifically on `nomic-embed-text`, and changing the model will make the request and documentation vectors incompatible.
