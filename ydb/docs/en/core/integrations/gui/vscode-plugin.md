...
[Visual Studio Code](https://code.visualstudio.com) is a free, open-source, cross-platform code editor that supports a broad ecosystem of extensions for working with databases, language servers, and development tools.
...
Authentication by login and password. Specify the username in the **Username** field and the password in the **Password** field. Used if [login and password authentication](%E2%9F%A6V1%E2%9F%A7) is enabled on the [](../../security/authentication.md#static-credentials) server.
...
The dashboard is only available when working with self-hosted installations of {{ ydb-short-name }}, where there is access to [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md). In Yandex Cloud Managed Service for {{ ydb-short-name }}, the Embedded UI is not published, so the dashboard data is unavailable — for monitoring, use [cloud platform tools](https://yandex.cloud/en/docs/ydb/operations/monitoring).
...
1. Install Ollama following the instructions on the [official download page](https://ollama.com/download) (supports macOS, Windows, and Linux).
...
3. Download the embedding model `nomic-embed-text` (~270 MB):
...
5. Open the connection form in the plugin and make sure the Ollama status is displayed as **Running**. If the status is **Not running**, make sure the Ollama service is running and available at `http://localhost:11434`.
...
You cannot use a different embedding model: the indices published by the plugin in the cloud are built specifically on `nomic-embed-text`, and changing the model will make the query and documentation vectors incompatible.
