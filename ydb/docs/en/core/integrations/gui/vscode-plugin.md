...
- Connecting to {{ ydb-name }} with all authentication methods [available](../../security/authentication.md): anonymous, static, token-based, service account, metadata-based.
...
- Access control (ACL) management [](../../security/authorization.md#right): viewing permissions for database objects.[правами доступа (ACL)](../../security/authorization.md#right)
- Generation of DDL scripts ([`CREATE`](../../yql/reference/syntax/create_table/index.md)) for any database object.
- Streaming query management [](../../concepts/glossary.md#streaming-query): viewing, starting, stopping.[потоковыми запросами](../../concepts/glossary.md#streaming-query)
...
| **Host** | Host of the {{ ydb-name }} cluster endpoint [endpoint](../../concepts/connect.md#endpoint) | `ydb.example.com` |
...
| **Use RAG** | Enable [YQL documentation search](#rag)
...
2. Select the authentication method from the **Auth type** dropdown list (see [Authentication methods](#auth-methods)).
...
Authentication using username and password. Enter the username in the **Username** field and the password in the **Password** field. Used if the server {{ ydb-short-name }} has [username and password authentication](../../security/authentication.md#static-credentials) enabled.
...
### Service Account Key File {#auth-service-account} {#auth-service-account} {#auth-service-account}[срок жизни — не более 12 часов](https://yandex.cloud/en/docs/iam/concepts/authorization/iam-token#lifetime)[сервисному аккаунту](#auth-service-account)[сервису метаданных](#auth-metadata) {#auth-service-account}
...
The execution results are displayed in the **Results** panel as a table, JSON, or chart (switchable via tabs).

### EXPLAIN and execution plan {#explain}

Select **Explain YQL Query** from the editor's context menu or the command palette (`Ctrl+Shift+P`) to get the [query execution plan](../../dev/query-execution-optimization/query-plans-optimization.md). The plugin displays the plan's operation tree in text format.

### Session manager {#session-manager}

The **Sessions** panel (Activity Bar → YDB) shows all active sessions with the current query, status, and duration (data from the system view [`.sys/query_sessions`](../../dev/system-views.md#query-sessions)). The **Toggle Hide Idle** button hides sessions without an active query.

### Cluster dashboard {#cluster-dashboard}

The **Database Load** panel (Activity Bar → YDB) shows real-time cluster load (updated every 10 seconds):
...
The dashboard is only available when working with self-hosted {{ ydb-short-name }} installations that have access to [{{ ydb-short-name }} Embedded UI](../../reference/embedded-ui/index.md). In Yandex Cloud Managed Service for {{ ydb-short-name }}, Embedded UI is not published, so dashboard data is unavailable — use [cloud platform monitoring tools](https://yandex.cloud/en/docs/ydb/operations/monitoring) for monitoring.
...
The plugin allows you to convert SQL queries written in other dialects (PostgreSQL, MySQL, ClickHouse, etc.) to YQL. The converter is available in the **Convert Dialect** panel on the Activity Bar.
...
For more information about the converter's operation principles, supported dialects, and limitations, see the [SQL Dialect Converter to YQL](../sql-dialect-converter.md) article.
...
2. The plugin will automatically detect the {{ ydb-short-name }} version and load the appropriate index from the cloud on the first connection.

### Search modes {#rag-modes}

- **Keyword search** — works without additional dependencies and is used by default.
- **Semantic search (vector)** — requires [Ollama](https://ollama.com) [running locally](https://ollama.com) with the [`nomic-embed-text`](https://ollama.com/library/nomic-embed-text) embedding model. See [Installing Ollama and the embedding model](#install-ollama).
...
1. Open the connection form in the plugin and make sure the Ollama status is displayed as **Running**. If the status is **Not running**, check that the Ollama service is running and accessible at `http://localhost:11434`.
