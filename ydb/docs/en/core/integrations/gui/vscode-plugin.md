...
Authentication using username and password. Enter the username in the **Username** field and the password in the **Password** field. Used if username and password authentication is enabled on the {{ ydb-short-name }} server ([authentication methods](../../security/authentication.md#static-credentials)).
...
- **Semantic search (vector)** — requires Ollama running locally (https://ollama.com) with the `nomic-embed-text` embedding model (https://ollama.com/library/nomic-embed-text). See [Installing Ollama and the Embedding Model](#install-ollama).
