...
- Access control (ACL) management: view permissions on database objects.
...
1. If necessary, specify the path to the user CA certificate (PEM) in the **CA Certificate File** field for connections with non-standard TLS. If the field is not filled in, the Yandex Cloud built-in certificate is used.
...
Query results are displayed in the **Results** panel as a table, JSON, or chart (switch using tabs).

### EXPLAIN and execution plan {#explain}

Select **Explain YQL Query** in the editor context menu or in the command palette (`Ctrl+Shift+P`) to get the [query execution plan](../../dev/query-execution-optimization/query-plans-optimization.md). The plugin displays the plan operation tree in text form.
...
To convert, the plugin sends the source query to an external HTTPS service. The converter does not work without internet access. Do not use the converter for queries containing confidential data.
...
You cannot use another embedding model: the indexes published by the plugin in the cloud are built specifically on `nomic-embed-text`, and if you change the model, the vectors of the query and documentation will be incompatible.
