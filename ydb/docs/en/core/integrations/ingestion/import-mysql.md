...
Transferring schemas and data from **MySQL** to {{ ydb-short-name }} — including **large tables** whose size exceeds available RAM — is performed using the [mysql2ydb utility](https://github.com/ydb-platform/mysql-ydb-importer).

**The utility's goal is to get an exact copy of the source database.** Table and column names are preserved as in MySQL; data types are mapped to the nearest {{ ydb-short-name }} equivalents (`INT` → `Int32`, `VARCHAR` → `Text`, `AUTO_INCREMENT` → `BigSerial`, etc.). Since names are transferred unchanged, they must be valid identifiers in {{ ydb-short-name }} (characters, length, reserved words). The result should be a {{ ydb-short-name }} database that replicates the original structure and allows queries to be executed with minimal "translation" from MySQL.

Each data fragment is read from MySQL by a single bounded query `SELECT`, written to {{ ydb-short-name }} in a batch (`BulkUpsert` or transactional `UPSERT`), and then freed from memory. The process repeats until the entire table is processed — no more than one batch is stored in memory at a time (no more than two during pipeline reading).

The import is performed in the following order:

1. The utility connects to MySQL and, based on the `information_schema` metadata, determines the composition of tables and their structure.
...
4. After each batch is successfully written, the cursor position is saved to a service state table — in case of failure, the migration can be resumed from the last successful fragment.
...
| `DATE` | `Date` |
| `DATETIME`, `TIMESTAMP` | `Timestamp` — the value is stored as UTC. For `DATETIME` (a type without time zone), the interpretation depends on the MySQL `time_zone` during reading; take this into account when migrating historical data. |
| `DECIMAL(p, s)`, `NUMERIC(p, s)` | `Decimal(22, 9)` for all — the original `p`/`s` are not preserved; values that do not fit into 22 digits will be lost |
| `YEAR` | `Uint16` |
...
| Table without `PRIMARY KEY` | Not supported during automatic schema creation — see [Tables without a primary key](#tables-without-pk) |
| Partitioning | Only `AUTO_PARTITIONING_BY_LOAD` |

### Secondary indexes {#secondary-indexes}

Non-unique MySQL keys become `INDEX … GLOBAL ASYNC`, unique ones (except primary) become `INDEX … GLOBAL UNIQUE SYNC`. Tables with synchronous unique indexes automatically switch to transactional `UPSERT` during data loading, because `BulkUpsert` only supports asynchronous indexes.
...
2. **Read → write pipeline** — one goroutine reads the next batch from MySQL while another writes the previous one to {{ ydb-short-name }}. No more than two batches are in flight for a table at the same time.
...
Building requires [Go](https://go.dev/) version not lower than specified in the `go.mod` repository:
...
Connection parameters to MySQL by default are read from the file **~/.my.cnf** (as in the client `mysql`), section `[client]`. The flag `-mysql` overrides the configuration file.

{% note warning "Tables without primary key" %}

Tables without `PRIMARY KEY` in MySQL **are not supported** when automatically creating a schema. Before running, add a primary key in MySQL or prepare the schema manually and use `-data-only` (see the [Tables without a primary key](#tables-without-pk) section).
...
Minimum run when `~/.my.cnf` is present (only `-ydb` is required):
...
| `-ydb-database` | Path to the {{ ydb-short-name }} database (default `local`) |
| `-schema-only` | Only create schema, no data transfer |
...
| `-max-chunk-rows` | Hard upper limit of rows per batch (default 25,000) |
| `-parallel-tables` | Number of tables for parallel transfer (default 1) |
| `-tables` | Comma-separated list of tables (default: all) |
| `-force` | Transfer all tables again, ignoring the saved state |
...
## Fragment-by-fragment reading {#chunked-reading}

- For tables with a **primary key** in MySQL, cursor-based pagination is used: `WHERE (pk > ?) ORDER BY pk LIMIT batch_size`. No more than one portion is kept in memory at a time.
- If there is no `PRIMARY KEY` in MySQL, but the schema in {{ ydb-short-name }} has already been created manually (`-data-only` mode), `LIMIT batch_size OFFSET offset` is applied — see the [Tables without a primary key](#tables-without-pk) section.
...
Queries to {{ ydb-short-name }} are marked as idempotent, so the SDK automatically retries them on network failures. The absence of duplicates when restarting the migration is ensured by overwriting rows by key and saving progress in a service table (see the [Features](#features) section, item 'Resumption').

## Comparison with YDB Importer {#ydb-importer-comparison}

There are also other tools for loading data into {{ ydb-short-name }}. [YDB Importer](import-jdbc.md) — a universal JDBC importer for PostgreSQL, Oracle, SQL Server, and other sources: XML configuration, parallel import, partition configuration.

**mysql2ydb** solves a narrower task — transferring a MySQL database one-to-one with a single binary file, without JVM. For migration only from MySQL with large tables, `~/.my.cnf` and the `-ydb` flag are sufficient; progress is saved in the service table {{ ydb-short-name }}.

|  | [YDB Importer](import-jdbc.md) | **mysql2ydb** |
| --- | --- | --- |
| Runtime | Java + JDBC drivers + XML configuration | A single Go binary file, native MySQL protocol |
...
| Large tables | Parallel ranges, partition buffers | Cursor-based paginated reading, batch size considering RAM |
| Resume after failure | Re-running the import | Checkpoint for each batch in the service table {{ ydb-short-name }} |
...
| --- | --- | --- |
| **Table names** | As in MySQL (`users`, `orders`, …) | Renaming by pattern, e.g. `mysql1/${schema}/${table}` → `mysql1/mydb/users` ( [configuration example](https://github.com/ydb-platform/ydb-importer/blob/main/scripts/sample-mysql.xml)) |
| **Column names** | As in MySQL | Sanitization: spaces, `.`, `/`, ````` → `_` |
| `AUTO_INCREMENT` | `BigSerial` + `ALTER SEQUENCE … START WITH` from `TABLES.AUTO_INCREMENT` | Regular `Int32`/`Int64`; without `BigSerial` and sequence reset |
...
| Secondary `KEY` / `UNIQUE KEY` | `INDEX … GLOBAL ASYNC` / `GLOBAL UNIQUE SYNC` in `CREATE TABLE` | In DDL only `PRIMARY KEY`; secondary indexes are not created |
| `ENUM`, `SET`, `JSON` | `Text` | `Text` (similarly) |
| `BLOB` / `BINARY` | Same column, value inline in `String` (bytes) | Column becomes `Int64` (blob identifier); data is moved to a **separate table** `${schema}/${table}_${field}` with rows `(id, pos, val)` in 64 KB blocks |
| `TEXT` / `CLOB` (large text) | Inline `Text` in the main table | Optionally a separate CLOB table (32 KB character blocks) or inline `Text` depending on settings |
...
| Partitioning | Only `AUTO_PARTITIONING_BY_LOAD` | `PARTITION_AT_KEYS`, partitioning by source partitions, `HASH` for column store, etc. |
