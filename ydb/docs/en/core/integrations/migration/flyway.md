...
The result of executing the `flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration info` will provide detailed information about the status of the migrations:
...
After applying the `flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration validate` command with current migrations, the logs will show that the latest migration was not applied to our database:
