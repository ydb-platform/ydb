...
- example of a custom configuration for Spring Boot 3 applications.
...
- [`tech.ydb.jdbc:ydb-jdbc-driver`](https://central.sonatype.com/artifact/tech.ydb.jdbc/ydb-jdbc-driver) — same place.

If the application uses DAO, JSON DSL, or other Exposed modules, connect them in the standard way along with the main dialect.

## Connection and configuration {#configuration-dialect}

First, register the dialect and driver, then open `Database` through the usual `Database.connect(...)` and pass the recommended `DatabaseConfig` values:
...
The recommended Exposed configuration for {{ ydb-short-name }} includes:

- `useNestedTransactions = false` is a setting on the Exposed side; it prevents Exposed from emulating nested transactions. {{ ydb-short-name }} does not support nested transactions, so such emulation would lead to errors.
- registration of `YdbDialect` via `registerYdbDialect(...)` before opening `Database`.

## DDL features in {{ ydb-short-name }} {#ddl}

Basic usage of Exposed — defining tables, DSL/DAO queries, transactions, etc. — is described in the [official JetBrains Exposed documentation](https://www.jetbrains.com/help/exposed/home.html). This section covers only {{ ydb-short-name }}-specific features of DDL generation by the dialect.
...
Creating a schema is done using standard Exposed tools:
...
`createYdbStatement()` returns `List<String>`, so arbitrary YQL statements can be added to the generated DDL — for example, `CREATE TABLE` of a column-oriented table, `ALTER TABLE ADD INDEX ... LOCAL` or any other DDL that the dialect does not cover first-class.
...
This is a convenient way to describe tables for which the Exposed DSL does not cover the required {{ ydb-short-name }} syntax. For overall schema management, it is recommended to use [Flyway](../migration/flyway.md) or [Liquibase](../migration/liquibase.md).
...
Mapping table of standard Exposed types to [{{ ydb-short-name }} types](../../yql/reference/types/primitive.md):
...
- `jsonb` corresponds to `JsonDocument` — this is a binary representation of JSON, optimized for efficient retrieval via `JSON_VALUE` / `JSON_EXISTS` / `JSON_QUERY`. `JsonDocument` itself cannot be used as a B-tree index key (in {{ ydb-short-name }} JSON is not a comparable type);
...
For literal `Decimal` values in update expressions, `ydbDecimalLiteral(...)` is available.
...
`ydbUint64(...)` stores the value in Kotlin `Long`, so only the `0..Long.MAX_VALUE` range is supported. For values above `Long.MAX_VALUE`, use `ydbUlong(...)` — it is represented by the Kotlin type `ULong` and covers the entire YQL `Uint64` range.
...
Standard properties of the official Exposed starter remain available, including:
...
In this variant, the property name `spring.exposed.ydb.enable-signed-datetimes` is chosen by the application itself by convention with the rest of the configuration. It can be replaced with any other if more convenient. If signed mode is enabled, the value in this property must match the JDBC URL, i.e., use `forceSignedDatetimes=true`.
...
`spring.exposed.generate-ddl=true` works the same as in the official Exposed starter: when the application starts, Exposed creates a schema based on the discovered `Table` classes.

For {{ ydb-short-name }}, there is an important nuance: tables that should be created via DDL must be declared using a regular `Table` with an override:
...
`@Transactional` provides the standard Spring Exposed integration but does not add {{ ydb-short-name }} retry policy for optimistic concurrency conflicts.
...
Typically, for one code section, one of two paths is chosen:
...
3. is verified against the Exposed model on the application side.
...
In Exposed 1.3.0, the full path `MigrationUtils.statementsRequiredForDatabaseMigration(...)` unconditionally reads metadata about `CHECK` constraints from `INFORMATION_SCHEMA.CHECK_CONSTRAINTS`. The current {{ ydb-short-name }} JDBC driver does not provide this part of the metadata, so the full generic diff of Exposed for {{ ydb-short-name }} does not work completely.

A practical {{ ydb-short-name }}-compatible path:

- apply the schema via external migrations.
...
The dialect repository has integration coverage for the scenario where the schema is created by raw SQL and then verified against the Exposed model without failing on unsupported metadata.

{% note info %}

If the schema was just changed with raw SQL, perform validation in a new transaction so that Exposed does not work with stale metadata cache.

{% endnote %}

### Signed temporal types in DDL {#signed-temporal-types}

`enableSignedDatetimes` refers exclusively to schema generation via Exposed DDL: it changes DDL names for standard Exposed types `date()`, `datetime()`, and `timestamp()`. It does not change anything for existing tables or runtime binding of individual columns by itself.
...
By default (`enableSignedDatetimes = false`), standard Exposed temporal types appear in the generated `CREATE TABLE` as:
...
In this mode, standard Exposed types `date`, `datetime`, and `timestamp` will appear in the generated `CREATE TABLE` as `Date32`, `Datetime64`, and `Timestamp64`.

If explicit type control of an individual column is needed, use the extensions `ydbDate(...)`, `ydbDate32(...)`, `ydbDatetime(...)`, `ydbDatetime64(...)`, `ydbTimestamp(...)`, and `ydbTimestamp64(...)`. They always generate DDL and bind the parameter via JDBC vendor code of exactly the type specified in the function name, regardless of `enableSignedDatetimes`.

If there are already other parameters in the JDBC URL, the flag must be added via `&forceSignedDatetimes=true`.

## Limitations and features {#limitations}

- Exposed 1.3.0 does not provide a dialect hook for rendering single-column `PRIMARY KEY` inside `CREATE TABLE`.
...
- `SchemaUtils.createMissingTablesAndColumns(...)` and `spring.exposed.generate-ddl=true` require the same override of `createStatement()`.
