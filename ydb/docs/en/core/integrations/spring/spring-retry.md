...
Retry applies only to the outer transaction boundary: a method that joins an already open transaction (`PROPAGATION_REQUIRED` when a transaction is active) is not retried individually — the entire transaction is retried at the top level.
...
| `maxAttempts` | `0` | Maximum number of attempts, including the first. For example, `maxAttempts = 5` means up to five total method executions: the first attempt and at most four retries, while `maxAttempts = 1` means exactly one execution with no retries. `0` means take the value from [`ydb.transaction.retry.max-attempts`](#configuration). Negative values are not allowed. |
...
- With `idempotent = true`, non-deterministic status codes are additionally retried (for example, when the commit result is unknown). Enable this mode only for operations whose repeated execution is safe.
...
The module extracts the status code {{ ydb-short-name }} from the exception chain and decides whether to retry. First, the code is checked against the retry policy (retry always or only for idempotent operations), then a backoff level is selected for the retriable code:
...
- **Zero** — retry without delay, as it only involves recreating the session.

Other status codes (for example, `TIMEOUT`, `PRECONDITION_FAILED`, `NOT_FOUND`) are not retried.

## Configuration {#configuration}

Global retry behavior is configured in `application.properties` (default values shown):
