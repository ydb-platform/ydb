...
- Automatic retry of `@Transactional` methods for retriable {{ ydb-short-name }} status codes.
- `@YdbTransactional` annotation with per-method retry settings (maximum number of attempts, [backoff](../../concepts/glossary.md#exponential-backoff), [idempotence](../../concepts/glossary.md#idempotency)).
- Two-level delay strategy (fast/slow) with [jitter](../../concepts/glossary.md#jitter), tuned to {{ ydb-short-name }} error semantics.
...
The module is configured automatically via the Spring Boot auto-configuration mechanism. Once the dependency is on the classpath, the module replaces the standard Spring [transaction interceptor](../../concepts/glossary.md#transaction-interceptor) (the `transactionInterceptor` bean) with its own implementation and automatically wraps all methods annotated with `@Transactional` and `@YdbTransactional` with retry logic. No additional annotations or explicit configuration are required — just configure the {{ ydb-short-name }} data source:
...
`@YdbTransactional` is needed when a specific method requires its own retry settings. For regular retry, `@Transactional` is sufficient.
...
| --- | --- | --- |
| `enabled` | `true` | Enables or disables retry for the method. Retry can only be disabled locally — it cannot be enabled on a method if it is disabled globally. |
| `maxAttempts` | `0` | Maximum number of attempts, including the first. For example, `maxAttempts = 5` means up to five method executions total: the first attempt and no more than four retries, while `maxAttempts = 1` means exactly one run with no retries. `0` — take the value from [`ydb.transaction.retry.max-attempts`](#configuration). Negative values are not allowed. |
...
| `slowCapBackoffMs` | `0` | Slow backoff ceiling, ms. `0` — from global configuration. |
| `fastCapBackoffMs` | `0` | Fast backoff ceiling, ms. `0` — from global configuration. |

### Idempotence {#idempotency}

[Transaction retry](../../concepts/glossary.md#transaction-retry) means re-executing the entire transaction. This is safe only for idempotent operations — those whose repeated execution leads to the same result (for example, `UPSERT` with a deterministic key or read operations).

- Without `idempotent = true`, only status codes that are known to be temporary and for which the transaction is guaranteed not to have been applied are retried.
- With `idempotent = true`, non-deterministic status codes (for example, when the commit result is unknown) are also retried. Enable this mode only for operations whose repeated execution is safe.
...
The module extracts the status code {{ ydb-short-name }} from the exception chain and decides whether to retry. First, the code is checked against the retry policy (always retry or only for idempotent operations), then a backoff level is selected for the retryable code:

| Status code | When to retry | Backoff level |
| --- | --- | --- |
| `ABORTED` | always | fast |
...
- **Fast** — exponential delay with base `fastBackoffBaseMs` and ceiling `fastCapBackoffMs`.
- **Slow** — exponential delay with base `slowBackoffBaseMs` and ceiling `slowCapBackoffMs`.
- **Zero** — retry without delay, as it is only related to session recreation.

Other status codes (e.g., `TIMEOUT`, `PRECONDITION_FAILED`, `NOT_FOUND`) are not retried.
...
| --- | --- | --- |
| `ydb.transaction.retry.enabled` | `true` | Global enable/disable of retry. |
| `ydb.transaction.retry.max-attempts` | `10` | Maximum number of attempts, including the first. For example, a value of `10` means up to ten method executions in total: the initial attempt and at most nine retries. The allowed minimum is `0`. |
| `ydb.transaction.retry.slow-backoff-base-ms` | `50` | Base delay of slow backoff, ms. |
| `ydb.transaction.retry.slow-cap-backoff-ms` | `5000` | Ceiling of slow backoff, ms. |
| `ydb.transaction.retry.fast-backoff-base-ms` | `5` | Base delay of fast backoff, ms. |
| `ydb.transaction.retry.fast-cap-backoff-ms` | `500` | Ceiling of fast backoff, ms. |

Parameters `@YdbTransactional` override the globals for a specific method. A value of `0` in the annotation means “take from the global configuration”; negative values are not allowed.
