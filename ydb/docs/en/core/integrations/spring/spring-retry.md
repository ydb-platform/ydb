...
Retry is applied only to the outer transaction boundary: a method that joins an already open transaction (`PROPAGATION_REQUIRED` when a transaction is active) is not retried individually — the entire transaction is retried at the top level.
...
| `maxAttempts` | `0` | Maximum number of attempts, including the first. For example, `maxAttempts = 5` means up to five executions of the method total: the first attempt and at most four retries, while `maxAttempts = 1` means exactly one execution with no retries. `0` means take the value from [`ydb.transaction.retry.max-attempts`](#configuration). Negative values are not allowed. |
...
The module extracts the {{ ydb-short-name }} status code from the exception chain and decides on a retry. First, the code is checked against the retry policy (retry always or only for idempotent operations), then a backoff level is selected for the retriable code:
