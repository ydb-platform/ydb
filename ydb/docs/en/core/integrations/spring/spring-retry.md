...
The module extracts the {{ ydb-short-name }} status code from the exception chain and makes a retry decision. First, the code is checked against the retry policy (always retry or only for idempotent operations), then a backoff level is selected for the retriable code:
...
| `UNDETERMINED` | only for `idempotent = true` | fast |
| `TRANSPORT_UNAVAILABLE` (transport error) | only for `idempotent = true` | fast |
| `CLIENT_GRPC_ERROR` | only for `idempotent = true` | fast |
| `SESSION_EXPIRED` | only for `idempotent = true` | zero |
