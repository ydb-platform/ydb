...
The SQL Dialect Converter is a service that transforms SQL queries written in other dialects (PostgreSQL, MySQL, ClickHouse, and others) into [YQL](../yql/reference/index.md) syntax. The service is designed to simplify the migration of existing queries to {{ ydb-name }} and lower the entry barrier for developers already familiar with other DBMS.

The converter is used in the [DBeaver plugin for {{ ydb-short-name }}](gui/dbeaver-plugin.md#convert-dialect) on the **Convert Dialect** tab. The user inserts a query in the source dialect, selects the dialect from the list, and receives a version of the same query in YQL.

## Operating Principle {#principles}

The conversion is performed in three stages:

1. **Parsing the source SQL.** The source query is parsed by a dialect-specific parser into an abstract syntax tree (AST) — a structure that describes the meaning of the query regardless of syntax.
2. **AST transformation.** Constructions specific to the source dialect are replaced with their equivalents in YQL. For example, type casting functions, date and string operations, `LIMIT`/`OFFSET` operators, and `WITH` constructions are adapted to YQL rules.
3. **YQL generation.** A text representation of the query in YQL is generated from the modified AST.

The stages are performed using the [SQLGlot](https://github.com/tobymao/sqlglot) library, which includes a separate dialect module for {{ ydb-short-name }}. SQLGlot is an open-source SQL parser and transpiler that supports more than twenty dialects. It acts as both the parser for the source query and the generator for YQL.
...
- **Dialect-specific functions.** Functions that do not have a direct equivalent in YQL (for example, PostgreSQL arrays, MySQL `JSON_EXTRACT` with specific path syntax) are translated into approximate equivalents or left for manual refinement.
- **Stored procedures and triggers.** Not supported, as {{ ydb-short-name }} uses a different code execution model in the database.
- **Specific DDL.** `CREATE TABLE` for complex types may require manual adjustments considering [columnar and row tables](../concepts/datamodel/table.md) in {{ ydb-short-name }}.
