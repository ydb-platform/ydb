...
2. Select the source SQL dialect in the dropdown list.
...
4. Click **Convert** — the result in YQL will appear in the **Conversion result (YDB)** field.

## How it works {#principles}

The conversion is performed in three stages:

1. **Parsing the source SQL.** The source query is parsed by the parser of the specific dialect into an abstract syntax tree (AST) — a structure that describes the query's meaning independently of syntax.
2. **AST transformation.** Constructs specific to the source dialect are replaced with equivalent ones in YQL. For example, type casting functions, date and string handling, `LIMIT`/`OFFSET` operators, and `WITH` constructs are adapted to YQL rules.
3. **YQL generation.** From the modified AST, a textual representation of the query in YQL is generated.

The stages are based on the [SQLGlot](https://github.com/tobymao/sqlglot) library and the [{{ ydb-short-name }} plugin for it](sqlglot.md), which adds a separate {{ ydb-short-name }} dialect to SQLGlot. SQLGlot is an open-source SQL parser and transpiler supporting more than twenty dialects. It serves as both the parser of the source query and the YQL generator.

If you need not a one-time conversion in the GUI but embedding the transformation into your own code or data processing pipeline, use the plugin directly — see the article [SQLGlot](sqlglot.md).

{% note warning %}

The source query is sent to an external HTTPS service. Do not send queries containing confidential data (personal data, trade secrets, identifiers of real production objects) to the converter.
...
- **Stored procedures and triggers.** Not supported, since {{ ydb-short-name }} uses a different model of code execution in the database.
- **Dialect-specific DDL.** `CREATE TABLE` for complex types may require manual adjustment considering [columnar and row-based tables](../../concepts/datamodel/table.md) in {{ ydb-short-name }}.
