...
If you need not a one-time conversion in the GUI but embedding the transformation into your own code or data processing pipeline, use the plugin directly — see the [SQLGlot](sqlglot.md) article.
...
- **Dialect-specific functions.** Functions that have no direct analog in YQL (for example, PostgreSQL arrays, `JSON_EXTRACT` MySQL with special path syntax) are translated to approximate equivalents or left for manual refinement.
...
- **Dialect-specific DDL.** `CREATE TABLE` for complex types may require manual adjustment taking into account [columnar and row tables](../../concepts/datamodel/table.md) in {{ ydb-short-name }}.
