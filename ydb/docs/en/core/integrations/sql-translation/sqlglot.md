...
[SQLGlot](https://github.com/tobymao/sqlglot) is a pure Python SQL parser, transpiler, optimizer, and formatter that supports over twenty dialects (PostgreSQL, MySQL, ClickHouse, BigQuery, Snowflake, Spark SQL, and others). SQLGlot parses a query into an abstract syntax tree (AST), which can be analyzed and transformed programmatically, and then generated back into SQL in any of the supported dialects.

The [ydb-sqlglot-plugin plugin](https://github.com/ydb-platform/ydb-sqlglot-plugin) adds the {{ ydb-short-name }} dialect to SQLGlot. After installing it, SQLGlot can both parse queries in [YQL](../../yql/reference/index.md) and generate YQL from queries written in other dialects. The conversion is bidirectional: any supported dialect ↔ YQL.
...
- Column lineage analysis — tracking which tables and columns each resulting column originates from.
...
The full list of supported features and current limitations is provided in the [plugin repository README](https://github.com/ydb-platform/ydb-sqlglot-plugin).
...
After installation, the {{ ydb-short-name }} dialect is available in SQLGlot automatically — additional imports are not needed. It has two equivalent names, `ydb` and `yql`; any of them can be specified in the `read` and `write` arguments.
...
Reverse conversion — from YQL to PostgreSQL. The YQL named expression (`$t = (...)`) becomes a CTE (`WITH ... AS`):
...
The `sqlglot.transpile()` function returns a list of strings — one for each statement in the source text. If the query has a single statement, take the first element (`[0]`).
...
When migrating to {{ ydb-short-name }}, the plugin automatically converts the constructs of the original dialect to YQL rules. For example, names with a schema reference (`schema.table`) are transformed into a [{{ ydb-short-name }} path](../../concepts/datamodel/dir.md) with backticks:
...
Correlated subqueries not directly supported in {{ ydb-short-name }} are rewritten as `JOIN` where possible, and `WITH` constructs are rewritten as YQL named expressions.
...
Since YQL is parsed into standard SQLGlot AST, the built-in SQLGlot column lineage analysis works for YQL queries. This allows you to trace which source tables and columns each resulting column comes from:
...
## Function and type mapping {#mappings}
