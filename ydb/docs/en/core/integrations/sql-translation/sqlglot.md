...
- **Programmatic access to AST.** In addition to transpilation, query parsing, column lineage analysis, optimization, and formatting are available — everything that SQLGlot can do.
...
Reverse translation — from YQL to PostgreSQL. A named YQL expression (`$t = (...)`) is converted into a CTE (`WITH ... AS`):
...
When migrating to {{ ydb-short-name }}, the plugin automatically converts source dialect constructs to YQL rules. For example, schema-qualified names (`schema.table`) are converted to [path {{ ydb-short-name }}](../../concepts/datamodel/dir.md) with backticks:
...
Since YQL is parsed into the standard SQLGlot AST, for YQL queries the column lineage analysis built into SQLGlot works. This allows you to trace which source tables and columns each resulting column is derived from:
...
The plugin contains mapping tables between standard SQL constructs and their {{ ydb-short-name }} equivalents.

Data types are mapped to {{ ydb-short-name }} equivalents:

| Type in source dialect | {{ ydb-short-name }} |
| --- | --- |
| `TINYINT` | `Int8` |
| `INT` | `Int32` |
| `BIGINT` | `Int64` |
