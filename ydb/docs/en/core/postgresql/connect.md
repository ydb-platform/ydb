...
- `<ydb_address>` - the address of the {{ ydb-short-name }} cluster you are connecting to.
...
You can execute queries in the PostgreSQL dialect using any {{ydb-name}} tools by explicitly specifying the dialect indicator at the beginning of the query body with a special comment: `--!syntax_pg`.
