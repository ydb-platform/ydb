...
3. After processing the queries, the results are collected and sent back to the requesting program via the PostgreSQL network protocol. During query processing, the query can be parallelized and executed on any number of {{ ydb-name }} nodes.
...
3. Read this data using PostgreSQL syntax
   ...
   - `<ydb_address>` is the address of the {{ ydb-short-name }} cluster to which the connection is made.
   - `<database_name>` is the name of the database in the cluster. It can be a complex name, for example, `mycluster/tenant1/database`.
   - `<user_name>` is the user login.

## Data type mapping {#supported_types}

The data type systems of {{ydb-name}} and PostgreSQL are similar but not identical.

### Using data from tables created with YQL syntax in PostgreSQL syntax {#topg}

Data types from {{ydb-name}} are automatically converted to the corresponding PostgreSQL types. The conversion is performed implicitly using the [`ToPg`](../yql/reference/udf/list/postgres.md#topg) command.
...
3. Read this data using PostgreSQL syntax. The YQL data type `INT` was automatically converted to the PostgreSQL type `int4`, on which the increment operation was performed.
...
The rules for converting PostgreSQL types to YQL types are given in the table:
...
4. For the `Ip::FromString` function to work, you must first convert data types using the `PgFrom` function:
...
Additionally, when working with PostgreSQL data types, [you can use PostgreSQL's own functions](../yql/reference/udf/list/postgres.md#callpgfunction) directly from YQL syntax:
