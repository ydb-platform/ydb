...
3. After query processing, the results are collected and sent back to the requesting program via the PostgreSQL network protocol. During query processing, the query can be parallelized and executed on any number of {{ ydb-name }} nodes.
...
3. Read this data using PostgreSQL syntax
   ...
   - `<ydb_address>` - the address of the {{ ydb-short-name }} cluster being connected to.
...
YQL built-in functions are designed to work with native data types, for example, `Ip::FromString` takes data types `Utf8` or `String` as input. Therefore, YQL built-in functions cannot work with PostgreSQL data types. To solve the type conversion problem, there is a function [`FromPg`](../yql/reference/udf/list/postgres.md#frompg) that converts data from PostgreSQL types to YQL types.
...
4. For the `Ip::FromString` function to work, you must first convert the data types using the `PgFrom` function:
