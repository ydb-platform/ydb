...
By default, the PostgreSQL wire protocol listener (pgwire) is **disabled** on `ydbd` nodes. To enable it on a cluster node, set `enable_local_pg_wire: true` in the [`local_pg_wire_config`](../reference/configuration/local_pg_wire_config.md) section of the configuration file. The [local {{ ydb-short-name }} Docker image](../reference/docker/start.md) enables pgwire by default for local development.
