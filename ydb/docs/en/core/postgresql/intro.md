...
By default, the PostgreSQL wire protocol listener (pgwire) is **disabled** on `ydbd` nodes and in the local {{ ydb-short-name }} Docker image. To enable pgwire and PostgreSQL compatibility features on a cluster node, set `enable_local_pg_wire: true` in the [`local_pg_wire_config`](../reference/configuration/local_pg_wire_config.md) section of the configuration file. In Docker, set `YDB_EXPERIMENTAL_PG=1` — see [{#T}](docker-connect.md).
