...
## Implementing an MCP Server for DBMS in Python — YDB’s Experience  {#2025-conf-fosdem}

{% include notitle [database_internals_tag](../_includes/tags.md#database_internals) %}

Discover how YDB leverages the Model Context Protocol (MCP) to bridge AI models with database systems. [{{ team.blinkov.name }}]({{ team.blinkov.profile }}) ({{ team.blinkov.position }}) shares the story of the development of a Python-based MCP server that enables seamless interactions between large language models and YDB’s open-source Distributed DBMS. Learn about the design decisions, challenges faced, and the practical benefits of integrating MCP into data architecture. Ideal for developers and AI practitioners aiming to enhance AI capabilities with direct database access.

@[YouTube](https://www.youtube.com/watch?v=mBBbAoD0Rxg)

The presentation will be of interest to developers who want to integrate their products with AI via the MCP protocol.

[Slides](https://presentations.ydb.tech/2025/en/pycon/mcp/presentation.pdf)
...
## Yandex Go Feature Store

{% include notitle [pracrice_tag](../_includes/tags.md#practice)%}

In his talk, Pavel Popov discusses the "Avalon" system in Yandex Go. "Avalon" is a feature store in the Yandex Go service with a data volume of 3 TB, 6.5 billion keys, and 2048 tokens, with {{ ydb-short-name }} at its core. The talk covers the concept, architecture, and specifics of working with data in the "Avalon" system.

The talk will be of interest to backend developers, system architects, and data analysts.

@[](https://runtime.video.cloud.yandex.net/player/video/vplvdk7lekxk6tx35kab?lang=en)

[Slides](https://presentations.ydb.tech/2025/ru/infra_avallon/presentation.pdf)

## {{ ydb-short-name }} Transactions Between Topics and Tables at a Stream Rate of About 3 GB/s

{% include notitle [database_internals_tag](../_includes/tags.md#database_internals) %}

The talk is dedicated to using {{ ydb-short-name }} transactions between topics and tables at a stream rate of about 3 GB/s. It covers approaches to solving the task without this functionality, as well as the factors affecting the performance of the schema when using it. In particular, the talk examines the performance issues encountered by AppMetrica during migration and possible ways to resolve them.

The talk will be of interest to developers of high-load systems, stream processing systems, and database administrators.

@[](https://runtime.video.cloud.yandex.net/player/video/vplv56rcfrzardc6t4uz?lang=en)
