...
- Minikube
   ...
   10. To continue, get access to port {{ ydb-ports.https }} from outside Kubernetes using `kubectl port-forward database-minikube-sample-0 {{ ydb-ports.https }}`.
...
- Kind
   ...
   11. To continue, get access to port {{ ydb-ports.https }} from outside Kubernetes using `kubectl port-forward database-kind-sample-0 {{ ydb-ports.https }}`.
...
The simplest way to launch your first {{ ydb-short-name }} query is via the built-in web interface. It is launched by default on port {{ ydb-ports.https }} of the {{ ydb-short-name }} server. If you have launched it locally, open [localhost:{{ ydb-ports.https }}](http://localhost:{{ ydb-ports.https }}) in your web browser. If not, replace `localhost` with your server's hostname in this URL or use `ssh -L {{ ydb-ports.https }}:localhost:{{ ydb-ports.https }} my-server-hostname-or-ip.example.com` to set up port forwarding and still open [localhost:{{ ydb-ports.https }}](http://localhost:{{ ydb-ports.https }}). You'll see a page like this:
