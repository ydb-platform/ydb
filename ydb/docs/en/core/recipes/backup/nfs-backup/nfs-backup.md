...
The `export nfs` / `import nfs` operations are performed by **server processes** {{ ydb-short-name }}, not by the CLI client. This means the file system must be accessible from each cluster node:
...
File operations are performed by server processes {{ ydb-short-name }}, not by the CLI client. Therefore, permissions to the NFS directory must be granted to the OS user under which the `ydbd` process runs.
...
| --- | --- | --- |
| `r` | `read-data` — read content | `list-directory` — view content |
...
Below are instructions for configuring the NFS server and {{ ydb-short-name }} client nodes for two permission management options: standard POSIX permissions and extended NFSv4 ACL.
...
3. Add the directory to `/etc/exports`:
   ...
   - `sync` — synchronous write (data is written to disk before acknowledging the client).
...
4. To enable automatic mounting at boot, add a line to `/etc/fstab`:
...
3. Mount the NFS directory using the NFSv4 protocol:
   ...
   - `-o rw` — mount with read and write permissions.
4. To enable automatic mounting, add a line to `/etc/fstab`:
...
5. Determine the UID of user `ydb`:
   ...
   Example output: `1871`
6. Configure NFSv4 ACLs on the directory:
   ...
   - `who` — who the permissions are granted to:
   ...
   - Inherited permissions for files and subdirectories (`fi`, `di`), which will apply only to descendants.
...
Two mechanisms affect the performance of export and import operations via NFS: the **resource broker** (manages the parallelism of scan and data load tasks) and the **actor system IO pool** (handles file write and read operations).
...
- Monitor the pool load via the [{{ ydb-ui-name }}](../../../reference/ydb-ui/ydb-monitoring.md#node_list_page).

{% endnote %}

## Logging {#nfs-logging}

The progress of export and import operations is reflected in the {{ ydb-short-name }} server logs. The main logging components are:
...
- Search the server logs for entries with components `EXPORT` or `IMPORT` and the operation ID.
- For file write or read errors, search the logs for entries with component `FS_WRAPPER` — they contain the file path and error description, which helps determine on which host and with which file the problem occurred.
