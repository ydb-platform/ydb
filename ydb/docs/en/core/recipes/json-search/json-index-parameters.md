...
The index receives a token of 'path + value' (`$.owner_id = $owner_id`) — the parameter is treated as a regular value. This allows performing a lookup with the same selectivity as when comparing with a literal.
...
For index lookup, the path token `$.content.y` is used, and the `@.y > $threshold` condition is checked by a post-filter.
...
* [{#T}](json-index-quickstart.md) — basic use case of a JSON index.
