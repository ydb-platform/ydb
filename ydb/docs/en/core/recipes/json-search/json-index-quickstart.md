...
The table primary key must consist of a single column of integer type (`Uint64`, `Uint32`, `Int64`, or `Int32`) — this is a [current limitation](../../dev/json-indexes.md#limitations) of the JSON index implementation.
...
## Filter by path presence in document
...
For index search, the `$.user.id` path token is used. The index returns the result without scanning the main table.
...
When checking equality, the token «path + value» (`$.user.name = "Alice"`) is indexed, which ensures the highest selectivity.

## Combination of conditions

Multiple `JSON_EXISTS` / `JSON_VALUE` calls on a single indexed JSON column can be combined using the `AND` and `OR` operators:
...
* [JSON indexes](../../dev/json-indexes.md) — a comprehensive overview of features and limitations.
* [VIEW (JSON index)](../../yql/reference/syntax/select/json_index.md) — query syntax using `VIEW`.
