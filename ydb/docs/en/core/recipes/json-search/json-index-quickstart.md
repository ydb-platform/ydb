...
The `JsonDocument` column type stores JSON in a compact binary format and is preferred for the indexed column. Alternatively, the `Json` type (text representation) can be used.
...
## Filter by path existence in a document
...
When checking equality, the index contains the token «path + value» (`$.user.name = "Alice"`), which ensures the highest selectivity.
