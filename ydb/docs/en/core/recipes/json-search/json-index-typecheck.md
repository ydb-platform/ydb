...
The path token `$.data` goes into the index — the `.type()` method completes the path token construction. The exact check of the string value `"array"` is performed by a post-filter.
...
The path `$.archived` (or `$.value`) and the values `false` or `null`, respectively, go into the index search operation.
