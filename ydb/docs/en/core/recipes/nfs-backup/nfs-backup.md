...
This recipe describes preparing NFS for the server commands **`export nfs`** and **`import nfs`**: cluster requirements, access permissions, step-by-step configuration of the NFS server and clients, feature flags, performance, and diagnostics. For command syntax, see the reference: [export to NFS](../../reference/ydb-cli/export-import/export-nfs.md), [import from NFS](../../reference/ydb-cli/export-import/import-nfs.md).

## Prerequisites {#nfs-prerequisites}

The `export nfs` / `import nfs` operations are performed by **server processes** {{ ydb-short-name }}, not by the CLI client. This means that the file system must be accessible from each cluster node:
...
## Access permissions {#nfs-permissions}

File operations are performed by server processes {{ ydb-short-name }}, not by the CLI client. Therefore, permissions to the NFS directory must be granted to the OS user under which the `ydbd` process is running.
...
Since files are created by the `ydbd` process during export, their permissions cannot be set in advance. In NFSv4, **ACE inheritance** is used for this — the `f` (file-inherit) and `d` (directory-inherit) flags on the parent directory's ACE. When a new file or subdirectory is created, the ACE with these flags is automatically copied to the new object.
...
The performance of export and import operations via NFS is affected by two mechanisms: the **resource broker** (manages the parallelism of scan and data upload tasks) and the **actor system IO pool** (handles file write and read operations).
...
- Keep in mind that the IO pool handles not only NFS operations but also other blocking tasks (e.g., log writing).
...
- **`EXPORT`** — logging of the export process on SchemeShard: creating directories, copying tables, starting backup tasks, loading schema.
- **`IMPORT`** — logging of the import process on SchemeShard: reading metadata, creating tables, restoring data.
- **`FS_WRAPPER`** — logging of file operations on each node. This component logs I/O errors (file unavailability, insufficient space, locking errors), which helps identify issues with NFS mounts on specific nodes.
...
- For file write or read errors, search logs for entries with component `FS_WRAPPER` — they contain the file path and error description, allowing you to determine which host and file caused the problem.
