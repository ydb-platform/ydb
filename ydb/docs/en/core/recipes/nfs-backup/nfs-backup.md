...
The recipe describes preparing NFS for server commands **`export nfs`** and **`import nfs`**: cluster requirements, access rights, step‑by‑step configuration of the NFS server and clients, feature flags, performance, and diagnostics. See the command syntax in the reference: [export to NFS](../../reference/ydb-cli/export-import/export-nfs.md), [import from NFS](../../reference/ydb-cli/export-import/import-nfs.md).

## Prerequisites {#nfs-prerequisites}

Operations `export nfs` / `import nfs` are performed by **server processes** {{ ydb-short-name }}, not by the CLI client. This means the file system must be accessible from each **node** of the **cluster** {{ ydb-short-name }}:

- The NFS **directory** must be mounted on **all nodes** of the **cluster** {{ ydb-short-name }} at the same absolute path.
- The process {{ ydb-short-name }} must have **write** permission for export and **read** permission for import to this directory.
- The path `--fs-path` must be absolute. Relative paths, paths containing `..` or `.`, as well as paths with zero bytes will be rejected by the server.
...
File operations are performed by server processes {{ ydb-short-name }}, not by the CLI client. Therefore, access rights to the NFS directory must be granted to the OS user under which the process `ydbd` runs.
...
NFSv4 uses an extended permission model instead of POSIX `rwx`. The same letter in an NFSv4 ACE has a different meaning for files and directories:
...
|| `D` | — | `delete-child` — delete nested objects ||
...
Since files are created by the `ydbd` process during export, their permissions cannot be set in advance. In NFSv4 this is achieved using **ACE inheritance** — flags `f` (file-inherit) and `d` (directory-inherit) on the parent directory’s ACE. When a new file or subdirectory is created, the ACE with these flags is automatically copied to the new object.

{% cut "Step-by-step setup NFS-server and clients" %}

Below are instructions for configuring the NFS server and client {{ ydb-short-name }} nodes for two permission management options: standard POSIX permissions and extended NFSv4 ACL.
...
2. Create a directory for export and assign an owner:
   ...
   - `7` (rwx) — the owner (`ydb`) can read, write, and traverse the directory.
   - `5` (r-x) — the group and others can read and traverse, but not write.
...
4. Apply the changes and start the NFS server:
   ...
   - `-r` — re-export all directories (synchronizing with `/etc/exports`).
   - `-v` — verbose output.
...
3. Mount the NFS directory:
   ...
   In this example the server host does not belong to the {{ ydb-short-name }} cluster, yet backup and restore will work correctly if one of the cluster hosts is designated as the NFS server.
...
NFSv4 ACLs allow more granular access permissions and also configure permission inheritance for newly created files and directories.
...
Unlike POSIX permissions, which are set on the NFS server, NFSv4 ACLs are configured on each client after mounting the directory. If the NFS server is also a {{ ydb-short-name }} cluster node, you must perform all the steps below on it.
...
3. Mount the NFS directory using the NFSv4 protocol:
   ...
   - `-t nfs4` — explicitly specify the NFSv4 protocol (required for proper ACL operation).
   - `-o rw` — mount with read and write permissions.
...
6. Configure NFSv4 ACL on the directory:
   ...
   - `who` — to whom the permissions are granted:

     - `1871` — UID of user `ydb`.
     - `OWNER@` — object owner.
   - Permissions:

     - `r` — read-data or list-directory.
...
To use export and import via NFS, you must enable the feature flag `enable_fs_backups` in the cluster [configuration](../../reference/configuration/feature_flags.md). By default this flag is **disabled**.

## Performance configuration {#nfs-performance}

Two mechanisms affect the performance of export and import operations via NFS: **resource broker** (manages parallelism of scanning and data loading tasks) and **actor system IO pool** (handles file write and read operations).

### Resource broker

Export and import via NFS use the same [resource broker](../../reference/configuration/resource_broker_config.md) queues as S3: `queue_backup` for export and `queue_restore` for import. Increasing the limits speeds up operations but raises CPU load. Default values and configuration method are described in the [{#T}](../../reference/configuration/resource_broker_config.md) section.
...
- Keep in mind that the IO pool serves not only NFS operations but also other blocking tasks (e.g., log writing).
- Monitor pool load via the [Embedded UI](../../reference/embedded-ui/ydb-monitoring.md#node_list_page).

{% endnote %}

## Logging {#nfs-logging}

The progress of export and import operations is reflected in the server logs {{ ydb-short-name }}. Main logging components:

- **`EXPORT`** — logging of the export process on SchemeShard: creating directories, copying tables, launching backup tasks, loading the schema.
- **`IMPORT`** — logging of the import process on SchemeShard: reading metadata, creating tables, restoring data.
- **`FS_WRAPPER`** — logging of file operations on each node. This component records I/O errors (file unavailability, insufficient space, lock errors), allowing you to identify NFS mounting issues on specific nodes.

When diagnosing issues, it is recommended to:

- Check the operation status via `{{ ydb-cli }} operation get`.
- Search the server logs for entries with components `EXPORT` or `IMPORT` and the operation identifier.
- When file write or read errors occur, look in the logs for entries with component `FS_WRAPPER` — they contain the file path and error description, allowing you to determine on which host and with which file the problem occurred.
- Verify that the NFS directory is accessible and has sufficient free space on **all** cluster nodes.
...
- Import always creates objects anew — you cannot import into existing tables.
- The `--list` parameter for listing export objects in the current version is available only for import from S3.
