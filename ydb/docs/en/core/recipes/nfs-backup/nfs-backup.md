...
File operations are performed by the {{ ydb-short-name }} server processes, not by the CLI client. Therefore, access rights to the NFS directory must be granted to the OS user under which the `ydbd` process is running.
...
|| `r` | `read-data` — read content | `list-directory` — list content ||
...
Below are instructions for configuring the NFS server and {{ ydb-short-name }} client nodes for two access control options: standard POSIX permissions and extended NFSv4 ACLs.
...
4. Apply the changes and start the NFS server:
   ...
   - `-r` — re-export all directories (synchronize with `/etc/exports`).
   - `-v` — verbose output.
...
3. Mount the NFS directory:
   ...
   In this example, the host server does not belong to the {{ ydb-short-name }} cluster, but backup and restore will work correctly if one of the cluster hosts is assigned as the NFS server.
4. To automatically mount at boot, add a line to `/etc/fstab`:
...
3. Mount the NFS directory using the NFSv4 protocol:
   ...
   - `-o rw` — mount with read and write permissions.
4. To automatically mount, add a line to `/etc/fstab`:
...
To use export and import via NFS, you need to enable the feature flag `enable_fs_backups` in the cluster [configuration](../../reference/configuration/feature_flags.md). By default, this flag is **disabled**.
...
Export and import via NFS use the same [resource broker](../../reference/configuration/resource_broker_config.md) queues as S3: `queue_backup` for export and `queue_restore` for import. Increasing the limits speeds up operations but increases CPU load. The default values and configuration method are described in the [{#T}](../../reference/configuration/resource_broker_config.md) section.
...
- **`FS_WRAPPER`** — logging of file operations on each node. This component records I/O errors (file unavailability, insufficient space, locking errors), which helps identify problems with NFS mounting on specific nodes.
...
- Check that the NFS directory is accessible and has enough free space on **all** cluster nodes.
