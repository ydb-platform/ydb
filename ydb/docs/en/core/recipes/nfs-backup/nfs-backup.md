...
The `export nfs` / `import nfs` operations are performed by **server processes** {{ ydb-short-name }}, not by the CLI client. This means the file system must be accessible from every cluster node:
...
Since files are created by the `ydbd` process during export, permissions cannot be set in advance. In NFSv4, **ACE inheritance** is used for this — the `f` (file-inherit) and `d` (directory-inherit) flags on the parent directory's ACE. When a new file or subdirectory is created, the ACE with these flags is automatically copied to the new object.
...
3. Mount the NFS directory:
   ...
   In this example, the host server does not belong to the {{ ydb-short-name }} cluster, but backup and restore will work correctly if one of the cluster hosts is designated as an NFS server.
...
The performance of export and import operations via NFS is affected by two mechanisms: the **resource broker** (manages the parallelism of scan and data load tasks) and the **actor system IO pool** (handles file write and read operations).
...
- Keep in mind that the IO pool serves not only NFS operations but also other blocking tasks (for example, writing logs).
...
- For file write or read errors, search the logs for entries with component `FS_WRAPPER` — they contain the file path and error description, allowing you to determine on which host and with which file the problem occurred.
- Check that the NFS directory is available and has enough free space on **all** cluster nodes.

## Limitations {#nfs-limitations}

- Export and import via NFS support the same object types as export to S3. A detailed list of supported objects is provided in the documentation of the [export command](../../reference/ydb-cli/export-import/export-nfs.md).
