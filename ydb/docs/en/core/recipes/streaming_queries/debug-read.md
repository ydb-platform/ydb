...
When developing [streaming queries](../../concepts/streaming-query/streaming-query.md), it is useful to quickly see what data is coming into the [topic](../../concepts/datamodel/topic.md) without creating a full streaming query. To do this, you can run a regular `SELECT` with the `STREAMING = TRUE` parameter.

{% note warning %}

This method is intended only for debugging and checking data in the topic. For production use, create streaming queries via [CREATE STREAMING QUERY](../../yql/reference/syntax/create-streaming-query.md).
...
The `LIMIT` parameter is required — without it, the query will not complete because it will wait for new messages indefinitely.
