# Quickstart: read and write topics
...
Events arrive as JSON with timestamp, log level, and host fields.

You will:

* [create topics](#step1);
* [create an external data source](#step2);
* [create a streaming query](#step3);
* [inspect query state](#step4);
* [write test data to the input topic](#step5);
* [read the output topic](#step6);
* [drop the streaming query](#step7).
...
* feature flags `enable_external_data_sources` and `enable_streaming_queries`.
...
## Step 3. Create a streaming query {#step3}
...
## Step 4. Inspect query state {#step4}

Check status in the [streaming_queries](../../dev/system-views.md#streaming_queries) system view:
...
Confirm `Status` is `RUNNING`. Otherwise inspect `Issues`.

If the query is `SUSPENDED` or `Issues` is non-empty, see troubleshooting documentation.

## Step 5. Write test data to the input topic {#step5}

Write messages with [{{ ydb-short-name }} CLI](../../reference/ydb-cli/index.md):
...
Output appears after the 10-minute aggregation window closes.
...
## Step 7. Drop the query {#step7}

Drop the query with [DROP STREAMING QUERY](../../yql/reference/syntax/drop-streaming-query.md):
...
- Supported [data formats](../../dev/streaming-query/streaming-query-formats.md).
