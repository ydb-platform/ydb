...
Below are code examples of token-based authentication in different {{ ydb-short-name }} SDKs.
...
- Go
  ...
  - database/sql
    ...
    {% cut "If using connector to create connection to {{ ydb-short-name }}" %}
    ...
    {% cut "If using string connection" %}
...
- Java
  ...
  - JDBC
    ...
    public void work() throws SQLException {
        // Connecting with specifying the authentication token value
        ...
        // Connecting with reading the authentication token from the specified file
    ...
    In Spring Boot, ORM, and other third-party frameworks built around JDBC, use the same JDBC connection string and the same authentication parameters as above (for example, `spring.datasource.url` with query parameters, or `spring.datasource.*` for the token and token file).
