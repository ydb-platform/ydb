...
Access token authentication passes a pre‑obtained IAM token to each request to {{ ydb-short-name }}. This method is suitable when the token is already present in the application—for example, obtained via OAuth or the `yc` utility. Typical steps: obtain the token, create an authentication provider, open a transport, and execute a request. Details about authentication modes are in the [Authentication](../../reference/ydb-sdk/auth.md) section; basic connection is covered in the [driver initialization](./init.md) recipe. Alternatives: [anonymous](./auth-anonymous.md), [via environment variables](./auth-env.md), [metadata](./auth-metadata.md), [service account](./auth-service-account.md), [login and password](./auth-static.md).

Below are examples of authentication code using a token in different {{ ydb-short-name }} SDKs.
...
- Go
  ...
  - database/sql
    ...
    {% cut "If is used connector for creating connection to {{ ydb-short-name }}" %}
  ...
  - JDBC
    ...
    public class AccessTokenJdbcExample {
        public static void main(String[] args) throws SQLException {
            ...
            // Option 2: token is read from file — tokenFile property
