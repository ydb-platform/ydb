...
Authentication with an access token sends a pre-obtained IAM token with each request to {{ ydb-short-name }}. This method is suitable when the token is already available in the application — for example, obtained via OAuth or the `yc` utility. Typical steps: obtain the token, create an authentication provider, open a transport, and execute the request. Details about authentication modes are in the [Authentication](../../reference/ydb-sdk/auth.md) section; basic connection is covered in the [driver initialization](./init.md) recipe. Alternatives: [anonymous](./auth-anonymous.md), [via environment variables](./auth-env.md), [metadata](./auth-metadata.md), [service account](./auth-service-account.md), [login and password](./auth-static.md).

Below are authentication code examples using a token in different {{ ydb-short-name }} SDKs.
...
- Go
  ...
  - database/sql
    ...
    {% cut "If a connector is used to create {{ ydb-short-name }}" %}
  ...
  - JDBC
    ...
    public class AccessTokenJdbcExample {
        public static void main(String[] args) throws SQLException {
            ...
            // Option 1: token is set via the token property
            ...
            // Option 2: token is read from a file — tokenFile property
    ...
    In Spring Boot, ORM, and other third‑party frameworks built on JDBC, use the same JDBC connection string and the same authentication parameters as above (for example, `spring.datasource.url` with query parameters or `spring.datasource.*` for a token and a token file).
...
- C#
  ...
  It is not supported for Entity Framework and linq2db.
