...
Anonymous authentication does not transmit credentials — {{ ydb-short-name }} SDK connects to the database without a token, login, or password. This is the default mode for local development and testing against a Docker container with {{ ydb-short-name }}. Typical steps: specify a connection string, explicitly or implicitly select an anonymous provider, and execute a query. For details, see the [Authentication](../../reference/ydb-sdk/auth.md) section; for basic connection, see the [driver initialization](./init.md) recipe. Other methods: [token](./auth-access-token.md), [environment variables](./auth-env.md), [metadata](./auth-metadata.md), [service account](./auth-service-account.md), [login and password](./auth-static.md).

Below are code examples of anonymous authentication in different {{ ydb-short-name }} SDK.
...
- C++
  ...
  - userver
    ...
    If in the static config you do not set `credentials-provider`, do not specify `databases.*.credentials`, and do not put `token`, `iam_jwt_params`, and the pair `user`/`password` in secdist for this database, the driver will use anonymous mode by default.
...
- Java
  ...
  - JDBC
    ...
    In Spring Boot, ORM, and other third-party frameworks around JDBC, the connection is set by the same JDBC connection string as above (for example, `spring.datasource.url`).
