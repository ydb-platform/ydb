...
Authentication via environment variables allows you to avoid hardcoding credentials in the code: the SDK or JDBC driver itself determines the mode based on the `YDB_*` variables in the process environment. This method is convenient for containers, CI/CD, and cloud deployment, where secrets are passed through the environment. Typical steps: set the required environment variables, create an authentication provider (or connect via JDBC without explicit properties), and execute a query. For details on the mode selection order, see the [Authentication](../../reference/ydb-sdk/auth.md#env) section; for basic connection, see the [driver initialization](./init.md) recipe. Other methods: [token](./auth-access-token.md), [anonymous](./auth-anonymous.md), [metadata](./auth-metadata.md), [service account](./auth-service-account.md), [login and password](./auth-static.md).
...
- Java
  ...
  - Native SDK
    ...
    public class EnvironAuthExample {
        public static void main(String[] args) throws Exception {
            // Connection string from environment variable or default local {{ ydb-short-name }}
  ...
  - JDBC
    ...
    The JDBC driver reads the `YDB_*` environment variables in the order described in the [Authentication](../../reference/ydb-sdk/auth.md#env) section. There is no need to explicitly pass credentials — an empty `Properties` object is sufficient.
