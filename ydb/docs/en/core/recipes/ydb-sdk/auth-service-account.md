...
Authentication with a service account file uses a JSON key to generate a JWT and obtain an IAM token. This method is suitable for server applications outside the cloud, CI/CD, and local development with access to {{ ydb-short-name }} in Yandex Cloud. Typical steps: save the service account key to a file, pass the path to it to the authentication provider, open a transport, and execute a query. For details, see the [Authentication](../../reference/ydb-sdk/auth.md) section; basic connection is described in the [driver initialization](./init.md) recipe. Other methods: [token](./auth-access-token.md), [anonymous](./auth-anonymous.md), [environment variables](./auth-env.md), [metadata](./auth-metadata.md), [login and password](./auth-static.md).

Below are code examples for authentication using a service account file in different {{ ydb-short-name }} SDKs.
