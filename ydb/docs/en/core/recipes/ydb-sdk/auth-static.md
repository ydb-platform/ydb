...
Static authentication (login and password) transmits a pair `user`/`password` when connecting to {{ ydb-short-name }}. This method is used on dedicated installations {{ ydb-short-name }} where login and password authentication is enabled. Typical steps: set the login and password, create a static credentials provider, open the transport, and execute the query. Details are in the [Authentication](../../reference/ydb-sdk/auth.md) section; basic connection is in the [driver initialization](./init.md) recipe. Other methods: [token](./auth-access-token.md), [anonymous](./auth-anonymous.md), [environment variables](./auth-env.md), [metadata](./auth-metadata.md), [service account](./auth-service-account.md).
...
- Go
  ...
  - database/sql
    ...
    You can also pass the login and password explicitly when initializing the driver through the connector using the special `ydb.WithStaticCredentials` option:
...
- Java
  ...
  - JDBC
    ...
    In Spring Boot, ORM, and other third‑party frameworks around JDBC, set the same JDBC URL, login, and password as in the example above (for example, `spring.datasource.url`, `spring.datasource.username`, `spring.datasource.password` or the equivalent in the pool configuration).
