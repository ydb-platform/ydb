...
- Go
  ...
  - database/sql
    ...
    Client-side balancing in the `database/sql` driver for {{ ydb-short-name }} is performed only when a new connection is established (in terms of `database/sql`), which is a session {{ ydb-short-name }} on a specific node. After the session is created, all queries on this session are directed to the node where the session was created. Balancing of queries on the same session {{ ydb-short-name }} between different nodes {{ ydb-short-name }} does not occur.
