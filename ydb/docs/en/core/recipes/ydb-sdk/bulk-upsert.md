...
{{ ydb-short-name }} supports bulk insertion of a large number of rows without atomicity guarantees. Data writes are split into multiple independent transactions, each affecting a single partition, executed in parallel. This makes the approach more efficient than `YQL`. On success, the `BulkUpsert` method guarantees insertion of all data passed in the request.
  ...
  - Native SDK
    ...
    public static void main(String[] args) {
      ...
      try (GrpcTransport transport = GrpcTransport.forConnectionString(connectionString)
          ...
          // For bulk upsert, you need to use the full path to the table
