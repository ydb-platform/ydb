...
- Python
  ...
  {{ ydb-short-name }} Python SDK writes logs via the standard module `logging` (loggers named `ydb.*`). Use the OpenTelemetry built-in `LoggingHandler` to redirect these records to `LoggerProvider` and export via OTLP:
...
- Java
  ...
  {{ ydb-short-name }} Java SDK writes logs via `slf4j`. If logback is used as the `slf4j` implementation, connect the ready-made appender `opentelemetry-logback-appender-1.0`, which forwards each event to the OpenTelemetry Logs SDK:
...
- C++
  ...
  {{ ydb-short-name }} C++ SDK writes logs via `TLogBackend` from `util`. Implement a backend that converts each record into an OTel log record, and pass it to the driver via `TDriverConfig::SetLog`. See [Getting Started](https://opentelemetry.io/docs/languages/cpp/getting-started/) for building the OpenTelemetry C++ SDK.
