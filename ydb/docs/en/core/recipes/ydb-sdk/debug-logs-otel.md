...
1. Create an OpenTelemetry `LoggerProvider` with an OTLP log exporter and the resource attribute `service.name`.
2. Redirect the SDK logger to this provider via an adapter (log appender / bridge) that converts each SDK log record into an OTel log record. For the list of adapters and log support status in different languages, see the [OpenTelemetry logging documentation](https://opentelemetry.io/docs/concepts/signals/logs/).
