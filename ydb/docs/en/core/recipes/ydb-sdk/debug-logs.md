...
- Go
  ...
  - Native SDK
    ...
    {% cut "Implement Getting information about server errors `IterateByIssues`" %}

    When working with {{ ydb-short-name }} through the Go SDK, you can not only enable logging of requests and responses, but also programmatically obtain detailed information about server errors (issues) — additional messages that YDB returns as part of the response when operation execution errors occur. To iterate over the list of issues contained in the server response, use the [IterateByIssues](https://pkg.go.dev/github.com/ydb-platform/ydb-go-sdk/v3#IterateByIssues) method.
...
- Java
  ...
  - Native SDK
    ...
    * The `info` level is recommended for default use
  ...
  - JDBC
    ...
    The same debug logs are available in all other frameworks around JDBC (Spring Boot, ORM, connection pools, etc.): they access {{ ydb-short-name }} through this driver; just include the same `slf4j` / log4j2 / logback configuration in the application.
...
- Rust
  ...
  Inside the `ydb` crate, messages go through the [`tracing`](https://docs.rs/tracing) library, which is standard in the Rust ecosystem (this is the crate name; it also includes ordinary text logs at debug/trace level, not just 'distributed tracing'). To see output in the console, before creating the client, connect a subscriber, for example [`⟦C3⟧_subscriber::fmt`](https://docs.rs/tracing-subscriber) with the required level (`TRACE` for maximum detail). Example: [`basic-logs.rs`](https://github.com/ydb-platform/ydb-rs-sdk/blob/master/ydb/examples/basic-logs.rs).
...
- PHP
  ...
  * `NullLogger` - the default, which outputs nothing
