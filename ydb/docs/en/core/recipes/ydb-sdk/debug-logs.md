...
- Java
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} Java SDK, the slf4j library is used for logging, which allows using different logging levels (`error`, `warn`, `info`, `debug`, `trace`) for one or more loggers. The following loggers are available in the current implementation:
    ...
    * The `tech.ydb.table.SessionRetryContext` logger at the `debug` level will inform you about the number of retries, results of executed queries, execution time of individual retries, and the total execution time of the entire operation.
  ...
  - JDBC
    ...
    The same debug logs are available in all other frameworks around JDBC (Spring Boot, ORM, connection pools, etc.): they access {{ ydb-short-name }} through this driver, so you just need to add the same `slf4j` / log4j2 / logback configuration to your application.
