...
- Java
  ...
  - Native SDK
    ...
    The {{ ydb-short-name }} Java SDK uses the slf4j library for logging, which allows using various logging levels (`error`, `warn`, `info`, `debug`, `trace`) for one or more loggers. The following loggers are available in the current implementation:
    ...
    * The `tech.ydb.table.impl` logger at the `debug` level allows tracking the internal state of the ydb driver, in particular the operation of the session pool.
  ...
  - JDBC
    ...
    The same debug logs are available in all other frameworks around JDBC (Spring Boot, ORM, connection pools, etc.): they access {{ ydb-short-name }} through this driver, so it is enough to connect the same `slf4j` / log4j2 / logback configuration in the application.
...
- Rust
  ...
  Inside the `ydb` crate, messages are routed through the standard Rust ecosystem library [`tracing`](https://docs.rs/tracing) (this is the crate name; it also covers ordinary text logs at debug/trace level, not just “distributed tracing”). To see console output, attach a subscriber before creating the client, for example [`tracing_subscriber::fmt`](https://docs.rs/tracing-subscriber) with the required level (`TRACE` for maximum detail). Example: [`basic-logs.rs`](https://github.com/ydb-platform/ydb-rs-sdk/blob/master/ydb/examples/basic-logs.rs).
...
- PHP
  ...
  * `NullLogger` - the default one, which outputs nothing
