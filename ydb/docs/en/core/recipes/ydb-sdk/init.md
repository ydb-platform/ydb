...
To connect to {{ ydb-short-name }}, you need to specify the required parameters (see [Connecting to the {{ ydb-short-name }} server](../../concepts/connect.md) for details) and additional ones that determine the driver's behavior during operation.
...
- Go
  ...
  - database/sql
    ...
    The `database/sql` driver is registered when the specific driver package is imported using the underscore symbol:
