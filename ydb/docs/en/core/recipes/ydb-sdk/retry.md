...
Maintenance work may be performed on the server side, server racks or entire data centers may be temporarily disabled.
...
{{ ydb-short-name }} SDK provides built-in retry mechanisms to ensure high availability, which take into account error types and define the reaction to them.
...
- C++
  ...
  - Native SDK
    ...
    To execute queries asynchronously with automatic retries, the `RetryQuery` method is used.
    The method returns `NThreading::TFuture`, allowing operations to be performed asynchronously.
    ...
    To execute streaming queries with automatic retries, the `StreamExecuteQuery` method is used.
  ...
  - userver
    ...
    In `ydb::TableClient`, retry with proper error handling is implemented in all methods. userver automatically analyzes errors and performs retries according to their type.
    ...
    * `get_session_settings`, `commit_settings`, and `rollback_settings` (relevant only for `ydb::RetryTxSettings`) - settings for session acquisition, commit, or transaction rollback queries
    ...
    The [`ydb.operation-settings`](https://github.com/userver-framework/userver/blob/develop/ydb/src/ydb/component.yaml) section in the static config sets default values: if a field is not set when called in code (`std::nullopt` or zero where that means "not set"), the value from the config is used, otherwise from the code.
...
- Go
  ...
  - Native SDK
    ...
    The details of retry execution are hidden as much as possible.
    The user can influence the logic of the `retry.Retry` function in two ways:

    * Via context (you can set deadline and cancel).
    * Via the idempotency flag of the operation `retry.WithIdempotent()`. By default, the operation is considered non-idempotent.
    ...
    If the user function returns an error, the {{ ydb-short-name }} Go SDK tries to identify this error and performs retries depending on it.
    ...
    For retry error handling at the session level of the {{ ydb-short-name }} table service, there is a function `db.Table().Do(ctx, op)` that provides a prepared session for executing queries. The `db.Table().Do(ctx, op)` function uses the `retry` package and also monitors the lifetime of {{ ydb-short-name }} sessions. The user operation `op`, according to its signature, must return an error or `nil` so that the driver can 'understand' from the error type what to do: retry the operation or not, with or without a delay, on the same session or a new one. The user can influence the retry logic through the context and the idempotency flag, and {{ ydb-short-name }} Go SDK interprets the errors returned from `op`.
    ...
    For retry error handling at the level of an interactive transaction of the {{ ydb-short-name }} table service, there is a `db.Table().DoTx(ctx, txOp)` function that provides a prepared {{ ydb-short-name }} transaction on a session for executing queries. The `db.Table().DoTx(ctx, txOp)` function uses the `retry` package and also monitors the {{ ydb-short-name }} session lifetime. From the user operation `txOp`, according to its signature, it is required to return an error or `nil`, so that the driver can 'understand' from the error type what to do: retry the operation or not, with a delay or not, on the same transaction or a new one. The user can influence the logic of retry query execution through the context and the idempotency flag, and {{ ydb-short-name }} Go SDK interprets the errors returned from `op`.
  ...
  - database/sql
    ...
    For example, in the [code](https://github.com/golang/go/tree/master/src/database/sql) of the `database/sql` package, you can often find a retry policy of three attempts:
    ...
    For retry error handling on the `*sql.Conn` connection object, there is a helper function `retry.Do(ctx, db, op)` that provides a prepared connection `*sql.Conn` for executing queries. The `retry.Do` function requires passing a context, a database object, and the user operation to be executed. From client code, you can influence the retry logic through the context and the idempotency flag, and {{ ydb-short-name }} Go SDK in turn interprets the errors returned from `op`.

    The user operation `op` must return an error or `nil`:

    - if the user function returns `nil`, retries stop;
    - if the user function returns an error, {{ ydb-short-name }} Go SDK tries to identify this error and makes retries depending on it.
    ...
    For error retry handling on the interactive transaction object `*sql.Tx`, there is a helper function `retry.DoTx(ctx, db, op)` that provides a prepared transaction `*sql.Tx` for executing queries.
    The function `retry.DoTx` requires passing a context, a database object, and the user operation to be performed.
    The function receives a prepared transaction `*sql.Tx`, on which queries to {{ ydb-short-name }} should be executed.
    From the client code, you can influence the retry logic through the context and the operation idempotency flag, while the {{ ydb-short-name }} Go SDK in turn interprets the errors returned from `op`.

    The user operation `op` must return an error or `nil`:

    - if the user function returns `nil`, then retries are stopped;
    - if the user function returns an error, the {{ ydb-short-name }} Go SDK tries to identify this error and, depending on it, performs retries.

    The `retry.DoTx` function uses the default read-write transaction isolation mode `sql.LevelDefault`, which can be changed via the `retry.WithTxOptions` option.
...
- Java
  ...
  - Native SDK
    ...
    * `idempotent(boolean)` — idempotence of the operation; expands the list of retryable errors (default `false`)
    ...
    * `supplyResult` — operation returning data (`executeDataQuery`, `QueryReader.readFrom`, etc.)
    ...
    A retry is performed if the lambda returned a [retryable](../../reference/ydb-sdk/error_handling.md) status or threw `UnexpectedResultException` with such a status.
  ...
  - JDBC
    ...
    `SessionRetryContext` refers to the native API (`TableClient` / `QueryClient`). When working through JDBC, the driver performs limited built-in retries (e.g., on `BAD_SESSION` outside a transaction); for other temporary failures, implement a retry loop at the application level. Classes `YdbRetryableException` and `YdbConditionallyRetryableException` mark errors that are worth retrying — see [Error handling](../../reference/ydb-sdk/error_handling.md#handling-retryable-errors). Connection — in [Driver initialization](./init.md).
...
- Python
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} Python SDK, retries are implemented in `QuerySessionPool` using the `RetrySettings` class to configure retry parameters. The `RetrySettings` class supports the following options:

    * `max_retries` - maximum number of retries (default 10)
    * `idempotent` - idempotency flag of the operation. Idempotent operations are retried for a wider list of errors (default False)
...
- C#
  ...
  - OpenRetryableConnectionAsync
    ...
    The `OpenRetryableConnectionAsync` method creates a connection with automatic retries on transient errors. A connection obtained this way does not support interactive transactions — use `ExecuteInTransactionAsync` to work with transactions.
...
- Rust
  ...
  Retries for queries via the Query Service are handled by `QueryClient`: helper methods for executing a single transactional SQL query (`query_row`, `exec`, etc.) are retried automatically; for multiple operations in a single transaction — `retry_transaction`.
...
- PHP
  ...
  In the {{ ydb-short-name }} PHP SDK, retries for queries to the Table API are set via `Table::retryTransaction()` (transaction + commit + retries on supported errors) or `Table::retrySession()` (a single session without the 'whole transaction' wrapper). The second argument `retryTransaction` is the idempotency flag (`true` expands the set of errors on which a retry is performed).
