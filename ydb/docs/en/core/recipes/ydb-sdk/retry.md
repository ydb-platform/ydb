...
- C++
  ...
  {% cut "Synchronous execution of retry attempts" %}
  ...
  {% cut "Asynchronous execution of retry attempts" %}
  ...
  {% cut "Execution of retry attempts of retry attempts when working with streaming" %}
  ...
  {% cut "queries Configuring of retry attempts" %}
...
- Go
  ...
  - Native SDK
    ...
    {% cut "retry parameters General-purpose retry" %}
    ...
    * Via the operation's idempotency flag `retry.WithIdempotent()`. By default, the operation is considered non-idempotent.
    ...
    {% cut "Execution of retry attempts of retry function errors on the {{ ydb-short-name }}" %}
    ...
    {% cut "Execution of retry attempts of retry function errors on session object {{ ydb-short-name }}" %}
    ...
    {% cut "interactive transaction Queries to {{ ydb-short-name }}" %}
  ...
  - database/sql
    ...
    {% cut "Execution of retry attempts of retry function errors on other `*sql.Conn`:" %}
    ...
    {% cut "Execution of retry attempts of retry function errors on session object `*sql.Tx`:" %}
...
- Java
  ...
  - Native SDK
    ...
    * The lambda function invoked an `UnexpectedResultException` with a [retryable](../../reference/ydb-sdk/error_handling.md) error code
      ...
      {% cut "services connection, Example SessionRetryContext.supplyStatus:" %}
      ...
      {% cut "services connection, Example SessionRetryContext.supplyResult:" %}
  ...
  - JDBC
    ...
    Retries at the `SessionRetryContext` level apply to the native API (`TableClient` / `QueryClient`). When using JDBC, implement retries at the application level or use native transport and clients as in [Driver initialization](./init.md).
...
- Rust
  ...
  Retries for Table API queries are performed by `TableClient::retry_transaction`: the callback receives a transaction, inside which `query` is called and, if necessary, `commit`.
...
- PHP
  ...
  In {{ ydb-short-name }} PHP SDK, retries for Table API queries are configured via `Table::retryTransaction()` (transaction + commit + retries on supported errors) or `Table::retrySession()` (a single session without the "whole transaction" wrapper). The second argument `retryTransaction` is the idempotency flag (`true` expands the set of errors for which a retry is performed).
