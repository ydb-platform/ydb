...
{{ ydb-short-name }} is a distributed DBMS with automatic scaling under load. On the server side, maintenance may be performed, server racks or entire data centers may be temporarily disconnected. Therefore, some errors are allowed when working with {{ ydb-short-name }}. Depending on the error type, you should react differently to them. {{ ydb-short-name }} SDK provides built-in retry mechanisms for high availability, which take into account error types and have a fixed reaction to them.
...
- C++
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} C++ SDK, retrying with correct error handling is implemented in several APIs:

    {% cut "Synchronous execution retry attempts" %}

    To execute queries with automatic retries, the `RetryQuerySync` method is used. The method takes a lambda function that receives a session object and returns the query result. The {{ ydb-short-name }} C++ SDK automatically analyzes errors and performs retries according to their type.
    ...
    void ExecuteQueryWithRetry(NYdb::NQuery::TQueryClient client) {
        ...
        if (!result.IsSuccess()) {
            // Processing the error after all attempts
    ...
    For asynchronous query execution with automatic retries, the `RetryQuery` method is used. The method returns `NThreading::TFuture`, which allows operations to be performed asynchronously.
    ...
    To execute streaming queries with automatic retries, the `StreamExecuteQuery` method is used. Streaming queries allow processing large volumes of data by receiving results in parts.
    ...
    {% cut "Setting up parameters retry attempts" %}
    ...
    * `MaxTimeout(TDuration)` - maximum time for all attempts
  ...
  - userver
    ...
    In `ydb::TableClient`, retrying with correct error handling is implemented in all methods. userver automatically analyzes errors and performs retries according to their type.
    ...
    * `client_timeout_ms` or `timeout_ms` respectively - maximum time for all attempts
    ...
    * `get_session_settings`, `commit_settings`, and `rollback_settings` (relevant only for `ydb::RetryTxSettings`) - settings for session acquisition, commit, or transaction rollback queries
    ...
    `ydb::RetryTxSettings` is used only for the `ydb::TableClient::RetryTx` method, which performs an interactive transaction with retries on errors for the entire transaction.

    The [`ydb.operation-settings`](https://github.com/userver-framework/userver/blob/develop/ydb/src/ydb/component.yaml) section in the static config sets default values: if a field is not set in the code call (`std::nullopt` or zero where that means "not set"), the value from the config is used, otherwise from the code.
...
- Go
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} Go SDK, correct error handling is implemented in several APIs:

    {% cut "Function retries general purpose" %}

    The main error handling logic is implemented by the helper function `retry.Retry`
    The details of retrying are hidden as much as possible.
    ...
    * via the operation idempotency flag `retry.WithIdempotent()`. By default, the operation is considered non-idempotent.
    ...
    If `nil` is returned from the user function, retries are stopped.
    ...
    For retrying errors at the session level of the table service {{ ydb-short-name }}, there is a function `db.Table().Do(ctx, op)` that provides a prepared session for executing queries.
    The `db.Table().Do(ctx, op)` function uses the `retry` package and also monitors the session lifetime {{ ydb-short-name }}.
    From the user operation `op`, according to its signature, you need to return an error or `nil` so that the driver can "understand" by the error type what to do: retry the operation or not, with a delay or not, on the same session or a new one.
    ...
    For retry error handling at the interactive transaction level of the {{ ydb-short-name }} table service, there is the `db.Table().DoTx(ctx, txOp)` function, which provides a prepared {{ ydb-short-name }} transaction on a session for executing queries.
    The `db.Table().DoTx(ctx, txOp)` function uses the `retry` package and also monitors the lifetime of {{ ydb-short-name }} sessions.
    From the `txOp` user operation, according to its signature, it is required to return an error or `nil` so that the driver can "understand" by the error type what to do: repeat the operation or not, with or without delay, on the same transaction or a new one.
    ...
    (`db.Scripting()`, `db.Scheme()`, `db.Coordination()`, `db.Ratelimiter()`, `db.Discovery()`) also use the `retry.Retry` function internally for retries and do not require external helper functions for retries.

    {% endcut %}

  - database/sql

    The standard package `database/sql` uses internal retry logic based on the errors returned by the specific driver implementation.
    For example, in the [code](https://github.com/golang/go/tree/master/src/database/sql) of the `database/sql` package, you can find a retry policy of three attempts in many places:

    - 2 attempts to execute on an existing connection or a new one (if the `database/sql` connection pool is empty)
    - 1 attempt to execute on a new connection.

    In most cases, such a retry policy is enough to survive temporary unavailability of {{ ydb-short-name }} nodes or {{ ydb-short-name }} session issues.
    ...
    For retry error handling on the `*sql.Conn` connection object, there is a helper function `retry.Do(ctx, db, op)` that provides a prepared connection `*sql.Conn` for executing queries.
    You need to pass a context, a database object, and the user operation to be executed to the `retry.Do` function.
    From the client code, you can influence the retry logic through the context and the idempotency flag, and {{ ydb-short-name }} Go SDK in turn interprets the errors returned from `op`.

    The user operation `op` must return an error or `nil`:

    - if `nil` is returned from the user function, then retries stop.
    - if an error is returned from the user function, {{ ydb-short-name }} Go SDK tries to identify this error and makes retry attempts depending on it.
    ...
    For error retry handling on the interactive transaction object `*sql.Tx`, there is a helper function `retry.DoTx(ctx, db, op)` that provides a prepared transaction `*sql.Tx` for executing queries. The function `retry.DoTx` requires passing a context, a database object, and the user operation to be performed. The function receives a prepared transaction `*sql.Tx`, on which queries to {{ ydb-short-name }} should be executed. From the client code, you can influence the retry logic through the context and the operation idempotency flag, while {{ ydb-short-name }} Go SDK in turn interprets errors returned from `op`.

    The user operation `op` must return an error or `nil`:

    - if `nil` is returned from the user function, retries are stopped;
    - if an error is returned from the user function, {{ ydb-short-name }} Go SDK tries to identify this error and, depending on it, makes retries.
...
- Java
  ...
  - JDBC
    ...
    Retries at the `SessionRetryContext` level belong to the native API (`TableClient` / `QueryClient`). When working via JDBC, use retries at the application level or connect the native transport and client, as in the [Driver initialization](./init.md) section.
...
- Python
  ...
  - Native SDK
    ...
    * `fast_backoff_settings`, `slow_backoff_settings` — settings for fast and slow retries
    ...
    To execute queries with retries, `QuerySessionPool` provides the `retry_operation_sync` and `execute_with_retries` methods. The `execute_with_retries` method is intended for one-shot queries with implicit transaction mode. For other cases (explicit transactions, multiple operations in one transaction), use `retry_operation_sync`.
  ...
  - SQLAlchemy
    ...
    When using {{ ydb-short-name }} via SQLAlchemy, retries happen under the hood and are not configurable from outside.
...
- C#
  ...
  - OpenRetryableConnectionAsync
    ...
    The `OpenRetryableConnectionAsync` method creates a connection with automatic retries on transient errors. A connection obtained this way does not support interactive transactions — for working with transactions, use `ExecuteInTransactionAsync`.
  ...
  - ExecuteInTransactionAsync
    ...
    The `ExecuteInTransactionAsync` method executes several operations within a single transaction with automatic retry on conflicts:
...
- Rust
  ...
  Retries for queries to the Table API are performed by `TableClient::retry_transaction`: the callback receives a transaction, inside which `query` and, if necessary, `commit` are called.
...
- PHP
  ...
  In {{ ydb-short-name }} PHP SDK, retries for queries to the Table API are set via `Table::retryTransaction()` (transaction + commit + retries on supported errors) or `Table::retrySession()` (a single session without the "whole transaction" wrapper). The second argument `retryTransaction` is the idempotency flag (`true` expands the set of errors on which retry is performed).
