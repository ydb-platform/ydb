...
- Python
  ...
  - Native SDK
    ...
    * `fast_backoff_settings`, `slow_backoff_settings` — fast and slow retry tuning
    ...
    Example of code using execute_with_retries:
    ...
    Example of code using retry_operation_sync:
...
- PHP
  ...
  In {{ ydb-short-name }} PHP SDK, retries for Table API queries are configured via `Table::retryTransaction()` (transaction + commit + retries on supported errors) or `Table::retrySession()` (a single session without the "whole transaction" wrapper). The second argument `retryTransaction` is an idempotency flag (`true` expands the set of errors for which a retry is performed).
