...
This may result in errors when working with {{ ydb-short-name }}.
Depending on the error type, you should respond differently.
{{ ydb-short-name }} SDKs provide built-in tools for retries to ensure high availability,
...
- Go
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} Go SDK, proper error handling is provided in several program interfaces:
  ...
  - database/sql
    ...
    The standard package `database/sql` uses internal retry logic based on the errors returned by a specific driver implementation.
    For example, in the [code](https://github.com/golang/go/tree/master/src/database/sql) of the `database/sql` package, you can find a three-attempt retry policy in many places:
...
- Java
  ...
  - Native SDK
    ...
    In the {{ ydb-short-name }} Java SDK, the retry mechanism is implemented as a helper class `SessionRetryContext`. This class is constructed using the `SessionRetryContext.create` method, which requires passing an implementation of the `SessionSupplier` interface — usually an instance of the `TableClient` or `QueryClient` class.

    Additionally, you can set some other options:

    * `maxRetries(int maxRetries)` — the maximum number of operation retries, not including the first execution. Default value is `10`
    * `retryNotFound(boolean retryNotFound)` — an option to retry operations that returned the `NOT_FOUND` status. Enabled by default.
    * `idempotent(boolean idempotent)` — an idempotency flag for operations. Idempotent operations will be retried for a wider list of errors. Disabled by default.

    To run operations with retries, the `SessionRetryContext` class provides two methods:

    * `CompletableFuture<Status> supplyStatus` — executes an operation that returns a status. It takes a lambda `Function<Session, CompletableFuture<Status>> fn` as an argument.
    * `CompletableFuture<Result<T>> supplyResult` — executes an operation that returns data. It takes a lambda `Function<Session, CompletableFuture<Result<T>>> fn` as an argument.
    ...
    * During the lambda execution, `UnexpectedResultException` was called with a [retryable](../../reference/ydb-sdk/error_handling.md) error code.
  ...
  - JDBC
    ...
    Retries at the `SessionRetryContext` level belong to the native API (`TableClient` / `QueryClient`). When working through JDBC, use retries at the application level or connect the native transport and client, as described in the [Driver initialization](./init.md) section.
...
- Python
  ...
  - Native SDK
    ...
    * `fast_backoff_settings`, `slow_backoff_settings` — fast and slow retry tuning
    ...
    Example code using execute_with_retries:
    ...
    Example code using retry_operation_sync:
  ...
  - SQLAlchemy
    ...
    When using {{ ydb-short-name }} through SQLAlchemy, retries happen under the hood and cannot be controlled externally.
...
- Rust
  ...
  Retries for Table API queries are performed by `TableClient::retry_transaction`: the callback receives a transaction, inside which `query` and, if necessary, `commit` are called.
...
- PHP
  ...
  In the {{ ydb-short-name }} PHP SDK, retries for Table API queries are set via `Table::retryTransaction()` (transaction + commit + retries on supported errors) or `Table::retrySession()` (a single session without the “whole transaction” wrapper). The second argument `retryTransaction` is an idempotency flag (`true` expands the set of errors for which a retry is performed).

  The retrier is available in the separate `retryTransaction` package.
