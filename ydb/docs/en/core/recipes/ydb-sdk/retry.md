...
- Go
  ...
  - Native SDK
    ...
    You can influence the logic of the `retry.Retry` function in two ways:
...
- Java
  ...
  - Native SDK
    ...
    * `retryNotFound(boolean retryNotFound)` — an option to retry operations that returned status `NOT_FOUND`. Enabled by default.
    * `idempotent(boolean idempotent)` — a flag indicating operation idempotency. Idempotent operations will be retried for a wider list of errors. Disabled by default.
    ...
    * `CompletableFuture<Result<T>> supplyResult` — executes an operation that returns data. Takes a lambda `Function<Session, CompletableFuture<Result<T>>> fn` as an argument.
    ...
    When using the `SessionRetryContext` class, note that the operation will be retried in the following cases:
  ...
  - JDBC
    ...
    Retries at the `SessionRetryContext` level belong to the native API (`TableClient` / `QueryClient`). When working through JDBC, use retries at the application level or connect the native transport and client, as in the [Driver initialization](./init.md) section.
