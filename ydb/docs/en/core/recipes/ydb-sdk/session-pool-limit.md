...
{{ ydb-short-name }} creates an [actor](../../concepts/glossary.md#actor) for each session. As a result, the size of the session pool on the client affects resource consumption (memory, CPU) on the server side of {{ ydb-short-name }}.

For example, if 1000 clients of one database open 1000 sessions each, then 1,000,000 actors are created on the server side. Such a number of actors consumes significant amounts of memory and CPU resources. Without a limit on the number of sessions on the client, this can lead to slow cluster operation and its semi-emergency state.
...
It is recommended to set the limit on the number of sessions on the client to the minimum necessary for normal operation of the client application. Note that a session is single-threaded both on the server and client side. Accordingly, if the application needs to execute 1000 concurrent requests (inflight) to {{ ydb-short-name }} for the estimated load, the limit should be set to 1000 sessions.
...
- Go
  ...
  - database/sql
    ...
    The `database/sql` library has its own connection pool. Each connection in `database/sql` corresponds to a specific {{ ydb-short-name }} session. Connection pool management in `database/sql` is done using the `sql.DB.SetMaxOpenConns` and `sql.DB.SetMaxIdleConns` functions. More details are available in the [documentation](https://pkg.go.dev/database/sql#DB.SetMaxOpenConns) `database/sql`.
...
- Java
  ...
  - JDBC
    ...
    Example of configuring a HikariCP pool in Spring configuration:
...
- C#
  ...
  * `MaxPoolSize`: maximum session pool size (default is 100)
