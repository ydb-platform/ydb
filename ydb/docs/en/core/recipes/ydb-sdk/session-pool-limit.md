...
Set the client session limit to the minimum needed for normal application operation. Remember that a session is single-threaded on both server and client. If the application must run 1000 concurrent in-flight requests to {{ ydb-short-name }}, set the limit to 1000 sessions.
