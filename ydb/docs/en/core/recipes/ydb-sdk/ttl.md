...
To enable eviction, you need an [external data source](../../concepts/datamodel/external_data_source.md) object that describes the connection to the external storage. You can create an external data source object via [YQL](../../yql/reference/recipes/ttl.md#enable-tiering-on-existing-tables) and the {{ ydb-short-name }} CLI.
...
For a newly created table, you can pass TTL settings along with its description:
...
- Java
  ...
  TTL for the table is set in `TableDescription` during creation. You can check the settings via `describeTable`.
