...
- Java
  ...
  - JDBC
    ...
    The JDBC driver does not allow explicitly specifying the `ImplicitTx` mode for transaction execution. It uses this mode on its own for executing queries that require it:
    ...
    public static void main(String[] args) {
        ...
        try (Connection connection = DriverManager.getConnection(connectionUrl)) {
            try (Statement statement = connection.createStatement()) {
                // Query will automatically be executed in ImplicitTx mode
...
- C#
  ...
  - Entity Framework
    ...
    // Auto-commit mode of Entity Framework (without explicit transaction)
  ...
  - linq2db
    ...
    // Auto-commit mode of linq2db (without explicit transaction)
...
- Rust
  ...
  // ImplicitTx — default mode for helper methods of QueryClient that execute one transactional SQL query:
  // server selects isolation based on SQL type (SELECT → snapshot RO, DML → serializable RW).
...
- Java
  ...
  - JDBC
    ...
    The JDBC standard does not support transaction levels other than the [standard ones](https://docs.oracle.com/javase/8/docs/api/constant-values.html#java.sql.Connection.TRANSACTION_NONE). However, the JDBC driver allows setting this mode by specifying the [non-standard constant 32](https://github.com/ydb-platform/ydb-jdbc-driver/blob/v2.3.25/jdbc/src/main/java/tech/ydb/jdbc/YdbConst.java#L142).
...
- Rust
  ...
  let mut qc = client.query_client();
  // Stale Read-Only mode is supported only for such calls on the query client (not for retry_transaction).
  let mut row = qc
      .query_row("SELECT 1 AS one")
