...
Below are code examples of using the built-in {{ ydb-short-name }} SDK tools for performing inserts:
...
- Java
  ...
  - Native SDK
    ...
    Use `SessionRetryContext` and `TableSession.executeDataQuery` with the `$seriesData` parameter of type `List<Struct<...>>`. Values for `AS_TABLE($seriesData)` are collected in the same way as row structures in the [batch insert](./bulk-upsert.md) example.
  ...
  - JDBC
    ...
    try (Connection conn = DriverManager.getConnection("jdbc:ydb:grpc://localhost:2136/local");
        ...
        // The $seriesData parameter is set according to the query type (see JDBC driver documentation)
    }
    ```


    In Spring Boot, Hibernate, JOOQ, and other frameworks on top of JDBC, the driver also seeks to optimize **large** sequences of inserts and updates: when necessary, **UPSERT**, `INSERT`, `UPDATE`, `DELETE` are automatically **grouped into batches** on the driver side (including for large batches from ORM).
...
- Python
  ...
  - SQLAlchemy
    ...
    When using {{ ydb-short-name }} through SQLAlchemy, the `ydb_sqlalchemy.upsert` function is used to insert data, which generates a `UPSERT INTO` query based on the table and the passed values. You can insert either a single row or multiple rows in a single call:
