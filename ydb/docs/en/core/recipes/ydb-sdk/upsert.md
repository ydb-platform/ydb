...
Below are code examples of using the built-in insertion tools in the {{ ydb-short-name }} SDK:
...
- Java
  ...
  - Native SDK
    ...
    Use `SessionRetryContext` and `TableSession.executeDataQuery` with the `$seriesData` parameter of type `List<Struct<...>>`. The values for `AS_TABLE($seriesData)` are collected in the same way as row structures in the [batch insert](./bulk-upsert.md) example.
...
- Python
  ...
  - Native SDK
    ...
    To insert data, `QuerySessionPool` and the `execute_with_retries` method are used with a parameterized YQL query. The query operates on the container type `List<Struct<...>>`, which allows passing multiple rows in a single call.
