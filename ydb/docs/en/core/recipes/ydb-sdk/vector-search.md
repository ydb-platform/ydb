...
- [Vector search](#%D0%B2%D0%B5%D0%BA%D1%82%D0%BE%D1%80%D0%BD%D1%8B%D0%B9-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA)
...
This section describes the minimum required steps to execute queries in {{ ydb-short-name }}.
For more information about connecting to {{ ydb-short-name }}, see [{#T}](./init.md).
...
- Java
  ...
  To execute YQL queries, use `QueryClient`, the YQL Query Service client. Retries on temporary errors are handled by `SessionRetryContext`, which is created based on `QueryClient`. For more details on configuring transport and clients, see [Driver initialization](./init.md).

  Below is a minimal runnable example: connecting, creating `SessionRetryContext`, and checking the connection with a `SELECT 1` query. The `retryCtx` object will be needed in the following steps of the recipe.
...
- Java
  ...
  Create a table for documents and their vector embeddings. Use `SessionRetryContext` from the [Connecting to {{ ydb-short-name }}](%7B#connect-ydb%7D) section.
...
- C++
  ...
  - Recommended approach
    ...
    The `ConvertVectorToBytes` function assumes that the client uses a processor with [little-endian byte order](https://en.wikipedia.org/wiki/Endianness), for example x86_64. If a different byte order is used, the `ConvertVectorToBytes` function must be adapted.
...
- Java
  ...
  - Alternative approach
    ...
    The option of passing vector components as `List<Float>` and converting on the YQL side via `Knn::ToBinaryStringFloat` follows the same scheme as the alternative examples for Python and C++ below:
...
- Python
  ...
  - Recommended approach
    ...
    To use the structure in the example below, a `items_struct_type = ydb.StructType()` is created, in which the types of all fields are specified. To pass a list of such structures, it must be wrapped in `ydb.ListType`: `ydb.ListType(items_struct_type)`.
...
Using a vector index allows you to efficiently solve the problem of approximate nearest neighbor search. More details about the benefits and usage specifics are described in the [vector index](../../dev/vector-indexes.md) documentation.
...
This approach allows you to create an index both when it is first created and when rebuilding (if the index already exists).
...
Each strategy defines a function that will be used for subsequent search. More details about the functions are described in the [distance and similarity functions](../../yql/reference/udf/list/knn.md#fuctions-distance) documentation.
...
- Java
  ...
  Create a vector index for the table from the previous steps. The YQL query `ALTER TABLE ... ADD INDEX` is executed via `queryRetry`; renaming the temporary index is done via `TableClient` and `tableRetry` (see [Connection](#connect-ydb)). The parameters `dimension`, `levels`, and `clusters` define the geometry of the `vector_kmeans_tree` index — as in the [final Python example](#full-example).
  ...
  static void addVectorIndex(
      ...
      // Step 2: rename the temporary index to a permanent one via Table API.
...
- JavaScript
  ...
  Example of adding a vector index and the full program for JavaScript SDK are not yet available.
...
A special YQL query is used to search for documents by vector, in which you need to define a similarity or distance function.
Available values:
...
For more details, see the documentation on [distance and similarity functions](../../yql/reference/udf/list/knn.md#fuctions-distance).

The method allows you to specify the index name. If specified, the expression `VIEW index_name` will be added to the query, enabling the use of the vector index during search.

The method returns a list of dictionaries with fields `id`, `document`, and `score` — a number reflecting the degree of similarity (or distance) to the query vector.
...
- C++
  ...
  - Alternative method
    ...
    Passing the vector as `List<Float>` with conversion on the YQL side via `Knn::ToBinaryStringFloat` gives worse performance than encoding the byte array on the client.
...
- Java
  ...
  Find the nearest documents to the query vector in the table populated in the [Inserting vectors](#insert-vectors) step. Pass the index name in `indexName` to use `VIEW` (after the [Adding an index](#add-vector-index) step). The `topClusters` parameter sets `PRAGMA ydb.KMeansTreeSearchTopSize` — like `top_clusters` in the Python example.
...
- Go
  ...
  Vector search functionality is fully supported in the Go SDK. A complete example combining all the above operations (table creation, data insertion, index creation, search) is built from the code snippets above. See the working example in the [ydb-go-sdk](https://github.com/ydb-platform/ydb-go-sdk/tree/master/examples) repository.
- Java
...
- Python
  ...
  The result shows that the table was created, 9 documents were added, and vector proximity search was successfully performed — both before and after adding the vector index.
