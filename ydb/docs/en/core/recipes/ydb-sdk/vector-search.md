...
| `id` | text identifier |
| `document` | Text |
| `embedding` | Text vector representation |
...
The vector is stored using type `String`. See the documentation on [exact vector search](../../yql/reference/udf/list/knn.md#data-types) for more details.
...
Using a vector index allows you to efficiently solve the problem of approximate nearest neighbor search. More details about the benefits and usage specifics are described in the documentation on the [vector index](../../dev/vector-indexes.md).
...
6. Search for nearest vectors using an index
