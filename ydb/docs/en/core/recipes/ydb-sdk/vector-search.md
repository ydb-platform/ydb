...
This section contains code recipes in different programming languages for solving [vector search](../../concepts/query_execution/vector_search.md) tasks using the {{ ydb-short-name }} SDK.
...
This recipe will create a text storage with the following structure:
...
This section describes the minimum actions required to run queries in {{ ydb-short-name }}.
For more information about connecting to {{ ydb-short-name }}, see the article [{#T}](./init.md).
...
- Java
  ...
  For queries, use `QueryClient` and `SessionRetryContext` (see [driver initialization](./init.md)). Below is the minimum connection and client creation for the YQL Query Service:
...
The query operates on the container data type `List<Struct<...>>` (a list of structures), which allows passing an arbitrary number of objects at once through parameters.

In {{ ydb-short-name }} tables, vectors are stored as a serialized byte sequence. It is **recommended to perform the conversion to this representation on the client**. An alternative is to delegate the conversion to the server using the [Knn UDF](../../yql/reference/udf/list/knn.md#functions-convert) conversion function. Examples demonstrating both approaches are provided below.
...
- Python
  ...
  The method takes an array of dictionaries `items`, where each dictionary contains the fields `id` - identifier, `document` - text, `embedding` - vector representation of the text, pre-serialized into a byte sequence.

  To use the structure in the example below, `items_struct_type = ydb.StructType()` is created, in which the types of all fields are set. To pass a list of such structures, it must be wrapped in `ydb.ListType`: `ydb.ListType(items_struct_type)`.
...
- C++
  ...
  The function `ConvertVectorToBytes` assumes that the client uses a processor with [little-endian byte order](https://en.wikipedia.org/wiki/Endianness), for example x86_64. If a different byte order is used, the function `ConvertVectorToBytes` must be adapted.
...
- Java
  ...
  The variant with passing vector components as `List<Float>` and converting them on the YQL side via `Knn::ToBinaryStringFloat` — following the same scheme as for the alternative Python and C++ examples below:
...
- Python (alternative)
  ...
  The method takes an array of dictionaries `items`, where each dictionary contains the fields `id` - identifier, `document` - text, `embedding` - vector representation of the text.
...
The method allows you to specify the index name. If it is set, the expression `VIEW index_name` is added to the query, which allows using the vector index during search.
...
Combine all the methods described above into one example that includes the following steps:
...
- Go
  ...
  Vector search functionality is fully supported in the Go SDK. A complete example combining all the operations described above (creating a table, inserting data, creating an index, searching) is built from the code fragments above. See a working example in the [ydb-go-sdk](https://github.com/ydb-platform/ydb-go-sdk/tree/master/examples) repository.
...
- Python
  ...
  As a result, you can see that the table was created, 9 documents were added, and vector proximity search was successfully performed both before and after adding the vector index.
