...
For more detailed information about connecting to {{ ydb-short-name }}, refer to the article [{#T}](./init.md).
...
- Java
  ...
  For queries, use `QueryClient` and `SessionRetryContext` (see [driver initialization](./init.md)). Below is the minimal connection and client creation for YQL Query Service:
...
- C++
  ...
  - Alternative approach
    ...
    Passing the vector as `List<Float>` with conversion on the YQL side via `Knn::ToBinaryStringFloat` gives worse performance than encoding a byte array on the client.
...
- Go
  ...
  The function converts the float32 vector to a binary representation and executes a parameterized query:
...
- Java
  ...
  - Alternative approach
    ...
    The variant with passing vector components as `List<Float>` and converting on the YQL side via `Knn::ToBinaryStringFloat` follows the same scheme as the alternative examples for Python and C++ below:
...
- Python
  ...
  - Recommended approach
    ...
    The method accepts an array of dictionaries `items`, where each dictionary contains fields `id` - identifier, `document` - text, `embedding` - vector representation of the text, pre-serialized into a byte sequence.

    To use the structure, the example below creates a `items_struct_type = ydb.StructType()` that specifies the types of all fields. To pass a list of such structures, wrap it in `ydb.ListType`: `ydb.ListType(items_struct_type)`.
  ...
  - Alternative method
    ...
    The method accepts an array of dictionaries `items`, where each dictionary contains fields `id` - identifier, `document` - text, `embedding` - vector representation of the text.
...
Using a vector index allows you to efficiently solve the problem of approximate nearest neighbor search. More details about the benefits and usage specifics are described in the documentation on [vector index](../../dev/vector-indexes.md).
...
- JavaScript
  ...
  Example of adding a vector index and a complete program for JavaScript SDK are not yet available.
...
The method allows you to specify an index name. If specified, the expression `VIEW index_name` will be added to the query, which will allow using the vector index during search.
...
- PHP
  ...
  Examples of vector search for the PHP SDK are not yet available.
...
- JavaScript
  ...
  An example of adding a vector index and the full program for JavaScript SDK are not yet available.
