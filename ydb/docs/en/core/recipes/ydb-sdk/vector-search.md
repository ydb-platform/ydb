...
The operations will be examined in detail:
...
- [Vector search](#%D0%B2%D0%B5%D0%BA%D1%82%D0%BE%D1%80%D0%BD%D1%8B%D0%B9-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA)
  ...
  - [Final example](#full-example)

This recipe creates a text store with the following schema:
...
This section describes the minimal required steps to execute queries in {{ ydb-short-name }}.
For more detailed information about connecting to {{ ydb-short-name }}, see the article [{#T}](./init.md).
...
- Java
  ...
  To execute YQL queries, use `QueryClient` — the YQL Query Service client. Retries on transient errors are provided by `SessionRetryContext`, which is built on top of `QueryClient`. For more details on configuring transport and clients, see the article [Driver initialization](./init.md).
...
| `embedding` | `String` | Document vector representation |

{% note warning %}

The `String` type is used to store vectors. See the documentation on [exact vector search](../../yql/reference/udf/list/knn.md#data-types) for more details.
...
To insert vectors, you need to prepare and execute a proper YQL query. It is parameterized to unify insertion of different data.

The query operates on the container data type `List<Struct<...>>` (a list of structs), allowing you to pass an arbitrary number of objects in a single request via parameters.

In {{ ydb-short-name }} tables, vectors are stored as a serialized byte sequence. **It is recommended to perform the conversion on the client side**. An alternative approach is to delegate the conversion to the server using the [Knn UDF](../../yql/reference/udf/list/knn.md#functions-convert) transformation function. Below are examples demonstrating both approaches.
...
- Java
  ...
  - Alternative approach
    ...
    The variant that passes vector components as `List<Float>` and performs conversion on the YQL side via `Knn::ToBinaryStringFloat` follows the same pattern as the alternative examples for Python and C++ below:
...
- Python
  ...
  - Recommended approach
    ...
    To use the structure in the example below, `items_struct_type = ydb.StructType()` is created, specifying the types of all fields. To pass a list of such structures, it must be wrapped in `ydb.ListType`: `ydb.ListType(items_struct_type)`.
...
Using a vector index allows you to efficiently solve the problem of approximate nearest neighbor search. More details on the advantages and usage specifics are described in the [vector index](../../dev/vector-indexes.md) documentation.
...
Each strategy defines the function that will be used for subsequent search. The functions are described in more detail in the [distance and similarity functions](../../yql/reference/udf/list/knn.md#fuctions-distance) documentation.
...
To search for documents by vector, a special YQL query is used, which must define a similarity or distance function.
Available values:
...
- Java
  ...
  Find the nearest documents to the query vector in the table populated in the [Insert vectors](#insert-vectors) step. Pass the index name in `indexName` to use `VIEW` (after the [Add index](#add-vector-index) step). The `topClusters` parameter sets `PRAGMA ydb.KMeansTreeSearchTopSize` — as `top_clusters` in the Python example.
...
We combine all the methods described above into a single example that includes the following steps:

1. Delete the existing table
2. Create a new table
3. Insert objects
4. Search for nearest vectors without using an index
...
- Go
  ...
  Vector search functionality is fully supported in the Go SDK. A complete example that combines all the operations described above (table creation, data insertion, index creation, search) is built from the code snippets above. See a working example in the [ydb-go-sdk](https://github.com/ydb-platform/ydb-go-sdk/tree/master/examples) repository.
- Java
...
- Python
  ...
  The result shows that the table was created, 9 documents were added, and a vector proximity search was successfully performed — both before and after adding the vector index.
