...
{{ ydb-short-name }} allows using various methods of user authentication in the system. Authentication settings and authentication providers are specified in the `auth_config` section of the {{ ydb-short-name }} configuration file.
...
|| `use_login_provider`
| The flag allows authentication of local users by auth tokens obtained by logging in with a login and password. The login procedure in {{ ydb-short-name }} is the exchange of login and password for an authentication token.

Possible values:

- `true` — allows authentication of local users by authentication tokens.
- `false` — prohibits authentication of local users by authentication tokens.
...
| The flag allows creating local users and obtaining an authentication token for them in exchange for a login and password.
...
||
|| `domain_login_only`
| The flag defines the boundaries of access rights of local users in the {{ ydb-short-name }} cluster.
...
{{ ydb-short-name }} allows you to prevent a user from authenticating if they have made several unsuccessful password attempts. To configure user lockout conditions, fill in the `account_lockout` section.
...
|| Parameter | Description ||
|| `attempt_threshold`
| Number of incorrect password attempts after which the user account is temporarily blocked. If the user enters the wrong password the specified number of times in a row, they are prohibited from authenticating for the time specified in the parameter `attempt_reset_duration`.
...
| Period of time during which the user is considered blocked. During this period, the user will not be able to authenticate in the system even if they enter the correct username and password. The blocking period starts from the moment of the last incorrect password attempt.

If a zero value is specified (`"0s"` — an entry equivalent to 0 seconds), the user will be blocked for an unlimited time. In this case, the block can be removed using the [ALTER USER ...  LOGIN](../../yql/reference/syntax/alter-user.md) command.
...
Combining units of measurement in a single string is not allowed. For example, the following entry is incorrect: `1d12h`. Such an entry must be replaced with an equivalent one, for example `36h`.
...
| The flag determines whether the username may be included in the password.
...
Any changes to the password policy do not affect existing user passwords, so there is no need to change them; they will be accepted as they are.

{% endnote %}

## LDAP authentication configuration {#ldap-auth-config}

One way to authenticate users in {{ ydb-short-name }} is to use an [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) directory. More details about this type of authentication are described in the section about [using an LDAP directory](../../security/authentication.md#ldap). To configure LDAP authentication, you need to describe the `ldap_authentication` section.
...
||
|| `search_filter`
| Filter for searching the user record in the LDAP directory. The filter string may contain the character sequence *$username*, which will be replaced with the username requested for authentication in the database
||
|| `use_tls`
| Settings for configuring the TLS connection between {{ ydb-short-name }} and the LDAP server
||
|| `enable`
| Determines whether an attempt will be made to establish a TLS connection with [ using the `StartTls` request ](../../security/authentication.md#starttls). When setting this parameter to `true`, you must disable the `ldaps` connection scheme by assigning the value `ldap` to the parameter `ldap_authentication.scheme`
||
|| `ca_cert_file`
...
- `TRY` - {{ ydb-short-name }} requires the LDAP server to provide a certificate. If the provided certificate cannot be trusted, the TLS connection establishment is terminated.
...
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection with the LDAP server over TLS from the very first request. To successfully establish a connection using the `ldaps` scheme, you need to disable the use of [query `StartTls`](../../security/authentication.md#starttls) in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
- If any other value is used, the default value `ldap` will be used.

Default value: `ldap`
||
|| `requested_group_attribute`
| Reverse group membership attribute. By default `memberOf`
||
|| `extended_settings.enable_nested_groups_search`
| The flag determines whether a query is executed to retrieve the entire tree of groups that include the user's immediate groups.
...
| The flag determines whether [service account authentication](../../security/authentication.md#ldap-service-account-auth) is performed using the SASL protocol with the EXTERNAL mechanism.

Possible values:

- `true` - For service account authentication, the SASL protocol with the EXTERNAL mechanism is used (authentication via a client TLS certificate within mTLS). The client certificate specified in the `use_tls.cert_file` and `use_tls.key_file` parameters is used as authentication information. In this case, the `bind_dn` and `bind_password` parameters are not set.
- `false` - For service account authentication, the simple bind method will be used. You must specify the `bind_dn` and `bind_password` parameters.
...
| A username suffix that distinguishes users from the LDAP directory from users authenticated by other providers.
...
| A username suffix that distinguishes users authenticated by client certificate from users authenticated by other methods.

Default value: `cert` (that is, the default SID suffix is `@cert`)
||
|#

## Configuring authentication using an external IdP {#external-idp-auth-config}
...
| Allowed clock skew when checking time-related JWT fields: `exp` sets the token expiration time, `nbf` the token start time, and `iat` the issue time.
...
| Maximum age of the JWKS cache. If JWKS cannot be refreshed, the keys are removed after this period expires.
...
Third-party IAM provider and external IdP using the OIDC protocol use tokens of type `Bearer`. If the `use_access_service` parameter is enabled, the IAM provider takes priority and intercepts all such tokens. Therefore, simultaneous use of authentication through an IAM provider and an external IdP using the OIDC protocol is not supported.
...
||
|| `path_to_root_ca`
| Path to the certificate file of the certificate authority used for interaction with AccessService.

Default value: `/etc/ssl/certs/YandexInternalRootCA.pem`
||
|| `access_service_grpc_keep_alive_time_ms`
| The period of time, in milliseconds, after which {{ ydb-short-name }} sends a keepalive ping to the IAM server to keep the connection alive.

Default value: `10000`
||
|| `access_service_grpc_keep_alive_timeout_ms`
| The period of time to wait for a response from the IAM server to a keepalive ping, in milliseconds. If no response is received from the IAM server within the timeout, {{ ydb-short-name }} closes the connection.
...
To reduce the number of [authentication token checks](../../security/authentication.md#token-validation), each {{ ydb-short-name }} node caches the results of checking [user tokens](../../concepts/glossary.md#user-token). For more information, see the article [{#T}](../../security/caching-authentication-results.md).
...
#|
|| `refresh_period`
| Determines how often the {{ ydb-short-name }} node scans user tokens in the cache for reaching the time limits specified in the `refresh_time`, `life_time`, and `expire_time` parameters, after which a token refresh or deletion is required. The shorter the specified user token check interval, the higher the CPU load.
...
||
|| `life_time`
| The period of storing a user token in the cache of the {{ ydb-short-name }} node since its last use. If no requests from the user for whom the token was created have arrived at the {{ ydb-short-name }} node during the specified period, the node removes this user token from its cache.

Default value: `1h`
||
|| `expire_time`
| The validity period of a successful check result for most types of authentication tokens. After a successful refresh, the countdown restarts. When the period expires, the entry is removed from the cache regardless of `life_time`.
...
| The initial interval between repeated checks after a retryable error in user token refresh.
...
| Limits the increase of the interval between retry checks after retryable errors of user token update. Does not limit the total duration of retry checks.
...
After creating a user token, the next scheduled check is performed after an interval randomly selected in the range from `30m` to `1h` (`refresh_time`). This condition is checked once every `1s` (`refresh_period`). After a successful check, the interval is selected again. If a retryable error occurs, the first retry check is performed immediately. If it also ends with a retryable error, the next check is performed after an interval randomly selected in the range from `500ms` to `1s` (`min_error_refresh_time`). Then the current interval is doubled, but does not exceed `59s` (`max_error_refresh_time - min_error_refresh_time`). The actual delay is randomly selected each time in the range from half to the full current interval.

For an authentication token obtained by login and password, the `expire_time` parameter is not applied: the validity period of such a token is set by `login_token_expire_time`, so it becomes invalid after `12h`. The cached entry stops being used at the nearest check after this condition, but may be deleted earlier due to the absence of requests for `2h` (`life_time`) or a persistent error.
...
## Configuration of the node registration token {#node-registration-token}
...
| Defines the authentication type of database nodes when they are registered in the {{ ydb-short-name }} cluster.
...
- "root@builtin" — authentication mode via a special debug token. This mode is planned to be removed in future releases and is not recommended for use: to ensure cluster security, it is recommended to use the node authentication mode via TLS certificates by setting an empty parameter value.

  ||
  |#

Example of the `auth_config` section with node registration configuration by certificate:
