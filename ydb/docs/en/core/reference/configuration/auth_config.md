...
{{ ydb-short-name }} allows using various user authentication methods in the system. Authentication settings and authentication providers are specified in the `auth_config` section of the {{ ydb-short-name }} configuration file.
...
|| `use_login_provider`
| The flag enables authentication of local users by auth tokens obtained as a result of login and password authentication. The {{ ydb-short-name }} login procedure is the exchange of a login and password for an authentication token.
...
- `false` — forbids creating local users and obtaining an authentication token for them.

Default value: `true`
||
|| `domain_login_only`
| The flag defines the access boundaries for local users in the {{ ydb-short-name }} cluster.

Possible values:

- `true` — {{ ydb-short-name }} local users exist at the cluster level and can be granted access rights to a set of [databases](../../concepts/glossary.md#database).
...
| The lifetime of an authentication token created in exchange for a local user's login and password.
...
Default value: empty string (allows all valid special characters)
||
...
Any password policy changes do not affect existing user passwords, so there is no need to change them; they will be accepted as is.

{% endnote %}

## LDAP authentication configuration {#ldap-auth-config}

One way to authenticate users in {{ ydb-short-name }} is to use an [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) directory. More details about this type of authentication are described in the section on [using an LDAP directory](../../security/authentication.md#ldap). To configure LDAP authentication, specify the `ldap_authentication` section.
...
||
|| `search_filter`
| Filter for searching a user record in the LDAP directory. The filter string may contain the character sequence *$username*, which will be replaced with the user name requested for authentication in the database
||
|| `use_tls`
| Settings for configuring the TLS connection between {{ ydb-short-name }} and the LDAP server
||
|| `enable`
| Determines whether an attempt will be made to establish a TLS connection [using query `StartTls`](../../security/authentication.md#starttls). When setting this parameter to `true`, you must disable the `ldaps` connection scheme by setting the `ldap_authentication.scheme` parameter to `ldap`
||
|| `ca_cert_file`
...
- `TRY` - {{ ydb-short-name }} requires the LDAP server to provide a certificate. If the provided certificate cannot be trusted, the TLS connection establishment is aborted.
...
| Flag determines whether a query is executed to retrieve the entire tree of groups that include the user's immediate groups.
...
{{ ydb-short-name }} supports [authentication using JWT tokens of an external identity provider with OpenID Connect](../../security/authentication.md#external-idp). To enable authentication, add the `external_idp_config` section to `auth_config`.
...
| Expected value of the `iss` field (issuer), which identifies the issuer of the JWT token, and the base URL for OIDC Discovery. Required parameter; must start with `https://` and must not end with the character `/`. The values of `issuer` in the Discovery document and `iss` in the JWT must exactly match the specified value.

Default value: empty string
||
|| `external_idp_config.audience`
| Expected value of the `aud` field (audience), which identifies the recipient of the JWT token. If the parameter is not set, the token recipient is not verified.

Default value: empty string
||
|| `external_idp_config.allowed_clock_skew`
| Allowed clock skew when checking JWT time-related fields: `exp` sets the token expiration time, `nbf` the token start time, and `iat` the issue time.
...
| Maximum age of the JWKS cache. If JWKS refresh fails, the keys are removed after this period expires.
...
Third-party IAM provider and external IdP over OIDC use tokens of type `Bearer`. If the `use_access_service` parameter is enabled, the IAM provider takes priority and intercepts all such tokens. Therefore, simultaneous use of authentication through an IAM provider and an external IdP over OIDC is not supported.
...
The lifetime and other aspects of user token operation are configured using the following parameters. Time parameter values are specified as a number with a unit-of-measurement suffix: `ms` — milliseconds, `s` — seconds, `m` — minutes, `h` — hours, `d` — days. For example, `300ms`, `30s`, `10m`, `1h`, or `2d`.
...
||
|| `life_time`
| Period of storing the user token in the {{ ydb-short-name }} node cache since its last use. If no requests from the user for whom the token was created have arrived at the {{ ydb-short-name }} node during the specified period, the node deletes this user token from its cache.

Default value: `1h`
||
|| `expire_time`
| Validity period of the successful check result for most types of authentication tokens. After a successful refresh, the countdown starts over. When the period expires, the record is deleted from the cache regardless of `life_time`.

For login and password tokens and tokens of an external identity provider, the validity period of the authentication token itself is used. For requests signed with an access key, a separate parameter `as_signature_expire_time` is used.

Default value: `24h`
||
|| `as_signature_expire_time`
| Validity period of the check result for a request authenticated using an access key signature.

Default value: `1m`
||
|| `min_error_refresh_time`
| Initial interval between repeated checks after a retryable user token refresh error.
...
| Limits the increase of the interval between retries after retryable errors of user token refresh. Does not limit the total duration of retries.
...
After a user token is created, the next scheduled check is performed after an interval randomly selected in the range from `30m` to `1h` (`refresh_time`). This condition is checked once every `1s` (`refresh_period`). After a successful check, the interval is selected again. If a retryable error occurs, the first retry check is performed immediately. If it also ends with a retryable error, the next check is performed after an interval randomly selected in the range from `500ms` to `1s` (`min_error_refresh_time`). The current interval is then doubled, but does not exceed `59s` (`max_error_refresh_time - min_error_refresh_time`). The actual delay is randomly selected each time in the range from half to the full current interval.

For an authentication token obtained by login and password, the `expire_time` parameter does not apply: the validity period of such a token is set by `login_token_expire_time`, so it becomes invalid after `12h`. The cached entry stops being used at the nearest check after this condition, but may be deleted earlier due to the absence of requests for `2h` (`life_time`) or a permanent error.
...
- Empty string (`""`): the authentication mode of nodes via TLS certificates is used. In this case, nodes must use certificates for authentication when registering in the cluster. For more information about configuring node authentication via certificates, see the section [Authentication and authorization of database nodes](../../devops/configuration-management/configuration-v1/node-authorization.md).
- "root@builtin" — the authentication mode via a special debug token. This mode is planned to be removed in future releases and is not recommended for use: to ensure cluster security, it is recommended to use the node authentication mode via TLS certificates by setting an empty parameter value.
