...
For more on authentication of [local users](../../concepts/glossary.md#access-user), see the section on [login and password authentication](../../security/authentication.md#static-credentials). To configure authentication of local users by login and password, specify the following parameters in the `auth_config` section:

#|
|| Parameter | Description ||
|| `use_login_provider`
| A flag that enables authentication of local users by auth tokens obtained as a result of logging in with a login and password. The login procedure in {{ ydb-short-name }} is the exchange of a login and password for an authentication token.
...
| A flag that enables creation of local users and obtaining an authentication token for them in exchange for a login and password.
...
||
|| `domain_login_only`
| A flag that defines the boundaries of access rights of local users in the {{ ydb-short-name }} cluster.
...
|| Parameter | Description ||
|| `attempt_threshold`
| Number of incorrect password attempts after which the user account is temporarily blocked. If the user enters an incorrect password the specified number of times in a row, they are prohibited from authenticating for the time specified in the `attempt_reset_duration` parameter.

If the parameter is set to `0`, the number of incorrect password attempts is unlimited. After successful authentication (entering the correct username and password), the counter of failed attempts resets to 0.
...
Any changes to the password policy do not affect existing user passwords, so you do not need to change existing passwords; they will be accepted as they are.
...
||
|| `base_dn`
| Root of the subtree in the LDAP directory from which the user entry search will start
||
|| `bind_dn`
| Distinguished Name (DN) of the service account on whose behalf the user entry search is performed
||
|| `bind_password`
| Password of the service account on whose behalf the user entry search is performed. Not set when `extended_settings.enable_sasl_external_bind: true`
||
|| `search_filter`
| Filter for searching for a user entry in the LDAP directory. The filter string may contain the character sequence *$username*, which will be replaced with the user name requested for authentication in the database
||
|| `use_tls`
...
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection to the LDAP server over TLS from the very first request. To successfully establish a connection using the `ldaps` scheme, you need to disable the use of the [query `StartTls`](../../security/authentication.md#starttls) in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
...
| Flag that determines whether a query is executed to retrieve the entire tree of groups that include the user's direct groups.
...
| Allowed clock skew when checking JWT time-related fields: `exp` sets the token expiration time, `nbf` the token start time, and `iat` the issuance time.
...
|| Parameter | Description ||
|| `use_access_service`
| The flag enables user authentication in Yandex Cloud through IAM using AccessService.
...
## Caching settings for authentication results {#caching-auth-results}
