...
|| `use_login_provider`
| The flag enables authentication of local users using auth tokens obtained as a result of login and password entry. The login procedure in {{ ydb-short-name }} is the exchange of login and password for an authentication token.

Possible values:

- `true` — enables authentication of local users using authentication tokens.
- `false` — prohibits authentication of local users using authentication tokens.

Default value: `true`

{% note info %}

To enable creation and authentication of local users, the `use_login_provider` and `enable_login_authentication` parameters must be set to `true`. Otherwise, local users will not be able to authenticate to {{ ydb-short-name }}.
...
- `true` — local users of {{ ydb-short-name }} exist at the cluster level and can be granted access rights to a set of [databases](../../concepts/glossary.md#database).
...
### Configuration of user lockout on incorrect password {#account-lockout}

{{ ydb-short-name }} allows you to prevent a user from authenticating if they have made several unsuccessful password entry attempts. To configure user lockout conditions, you need to fill in the `account_lockout` section.
...
|| Parameter | Description ||
|| `attempt_threshold`
| Number of incorrect password attempts after which the user account is temporarily blocked. If the user enters the wrong password the specified number of times in a row, they are prohibited from authenticating for the time specified in the `attempt_reset_duration` parameter.
...
| Period of time during which the user is considered blocked. During this period, the user will not be able to authenticate to the system even if they enter the correct username and password. The blocking period starts from the moment of the last incorrect password attempt.
...
### Configuration of password complexity requirements {#password-complexity}
...
||
|| `scheme`
| Connection scheme to the LDAP server.
...
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection to the LDAP server over TLS from the very first query. To successfully establish a connection using the `ldaps` scheme, you need to disable the use of the [`StartTls` query](../../security/authentication.md#starttls) in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
...
| Flag that determines whether a query is executed to retrieve the entire tree of groups that include the user's immediate groups.
...
- `true` - The SASL protocol with the EXTERNAL mechanism will be used for service account authentication (authentication via a client TLS certificate within mTLS). The client certificate specified in the `use_tls.cert_file` and `use_tls.key_file` parameters is used as authentication information. In this case, the `bind_dn` and `bind_password` parameters are not set.
...
||
|| `host`
| Hostname of the LDAP server. This is a deprecated parameter; the `hosts` parameter should be used instead.
||
...
| User name suffix that distinguishes users authenticated by client certificate from users authenticated by other methods.

Default value: `cert` (that is, the default SID suffix is `@cert`)
||
|#

## Configuring authentication with an external IdP {#external-idp-auth-config}
...
| Timeout for requesting JWKS.

Default value: `15s`
||
|| `external_idp_config.jwks_cache_settings.timeout`
| Maximum age of the JWKS cache. If JWKS cannot be updated, the keys are removed after this period expires.

Default value: `2h`
||
|| `external_idp_authentication_domain`
| User name suffix that distinguishes users of an external IdP from users authenticated by other providers. The same suffix is added to group names obtained from the JWT token.
...
| The "user source" suffix in [SID](../../concepts/glossary.md#access-sid) for users coming to {{ ydb-short-name }} from Yandex Cloud IAM.

Default value: `as` ("access service")
||
|| `path_to_root_ca`
| Path to the certificate authority certificate file used to interact with AccessService.
...
| The time period to wait for a response from the IAM server to a keepalive ping, in milliseconds. If no response is received from the IAM server within the timeout, {{ ydb-short-name }} closes the connection.
...
||
|| `life_time`
| The period of storing the user token in the {{ ydb-short-name }} node cache since its last use. If no requests from the user for whom the token was created have arrived at the {{ ydb-short-name }} node during the specified period, the node removes this user token from its cache.

Default value: `1h`
||
|| `expire_time`
| The validity period of a successful check result for most types of authentication tokens. After a successful refresh, the countdown starts over. After the period expires, the record is removed from the cache regardless of `life_time`.
...
| The initial interval between repeated checks after a retryable error in updating the user token.
...
Setting the parameter value to `0` is not recommended, as immediate retries create excessive load.
...
| Limits the increase of the interval between retries after retryable errors of user token update. Does not limit the total duration of retries.
...
After a user token is created, the next scheduled check is performed after an interval randomly selected in the range from `30m` to `1h` (`refresh_time`). This condition is checked once every `1s` (`refresh_period`). After a successful check, the interval is selected again. If a retryable error occurs, the first retry check is performed immediately. If it also ends with a retryable error, the next check is performed after an interval randomly selected in the range from `500ms` to `1s` (`min_error_refresh_time`). Then the current interval doubles but does not exceed `59s` (`max_error_refresh_time - min_error_refresh_time`). The actual delay is chosen randomly each time in the range from half to the full current interval.
