...
{{ ydb-short-name }} allows you to use various methods of user authentication in the system. Authentication settings and authentication providers are specified in the `auth_config` section of the {{ ydb-short-name }} configuration file.

## Authentication configuration for local users {{ ydb-short-name }} {#local-auth-config}

For more information about authentication of [local users](../../concepts/glossary.md#access-user), see the section on [authentication by login and password](../../security/authentication.md#static-credentials). To configure authentication of local users by login and password, specify the following parameters in the `auth_config` section:

#|
|| Parameter | Description ||
|| `use_login_provider`
| The flag enables authentication of local users using auth tokens obtained as a result of logging in with a login and password. The login procedure in {{ ydb-short-name }} is the exchange of a login and password for an authentication token.
...
To enable creation and authentication of local users, the `use_login_provider` and `enable_login_authentication` parameters must be set to `true`. Otherwise, local users will not be able to authenticate in {{ ydb-short-name }}.
...
| The flag enables creation of local users and obtaining an authentication token for them in exchange for a login and password.

Possible values:

- `true` — allows creating local users and obtaining an authentication token for them
- `false` — prohibits creating local users and obtaining an authentication token for them.

Default value: `true`
||
|| `domain_login_only`
| The flag defines the access boundaries of local users in the {{ ydb-short-name }} cluster.

Possible values:

- `true` — local users of {{ ydb-short-name }} exist at the cluster level and can be granted access to a set of [databases](../../concepts/glossary.md#database).
- `false` — local users can exist both at the cluster level and at the level of each individual database. The access boundaries of local users created at the database level are limited to the database in which they were created.

Default value: `true`
||
|| `login_token_expire_time`
| Lifetime of the authentication token created in exchange for the local user's login and password.
...
{{ ydb-short-name }} allows you to prevent a user from authenticating if they have made several unsuccessful password attempts. To configure user lockout conditions, you need to fill in the `account_lockout` section.
...
|| Parameter | Description ||
|| `attempt_threshold`
| The number of incorrect password attempts after which the user account is temporarily blocked. If the user enters the wrong password the specified number of times in a row, they are prohibited from authenticating for the time specified in the `attempt_reset_duration` parameter.
...
| The period of time during which the user is considered blocked. During this period, the user will not be able to authenticate to the system even if they enter the correct username and password. The blocking period starts from the moment of the last incorrect password attempt.

If a zero value is specified (`"0s"` — an entry equivalent to 0 seconds), the user will be blocked indefinitely. In this case, the block can be lifted using the [ALTER USER ...  LOGIN](../../yql/reference/syntax/alter-user.md) command.
...
Combining units of measurement in a single line is not allowed. For example, the following entry is incorrect: `1d12h`. Such an entry must be replaced with an equivalent one, for example `36h`.
...
Any changes to the password policy do not affect existing user passwords, so there is no need to change existing passwords; they will be accepted as they are.

{% endnote %}

## LDAP authentication configuration {#ldap-auth-config}

One way to authenticate users in {{ ydb-short-name }} is to use an [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) directory. More details about this type of authentication are provided in the section on [using an LDAP directory](../../security/authentication.md#ldap). To configure LDAP authentication, you need to describe the `ldap_authentication` section.
...
||
|| `base_dn`
| Root of the subtree in the LDAP directory from which the user entry search will be performed
||
|| `bind_dn`
| Distinguished Name (DN) of the service account on whose behalf the user entry search is performed
||
|| `bind_password`
| Password of the service account used to search for the user entry. Not set when `extended_settings.enable_sasl_external_bind: true`
||
|| `search_filter`
| Filter for searching the user entry in the LDAP directory. The filter string may contain the character sequence *$username*, which will be replaced with the username requested for authentication in the database
||
|| `use_tls`
| Settings for configuring the TLS connection between {{ ydb-short-name }} and the LDAP server
||
|| `enable`
| Determines whether an attempt will be made to establish a TLS connection with [using the `StartTls` request](../../security/authentication.md#starttls). When setting this parameter to `true`, you must disable the `ldaps` connection scheme by setting the `ldap_authentication.scheme` parameter to `ldap`
||
|| `ca_cert_file`
...
- `TRY` - {{ ydb-short-name }} requires the LDAP server to provide a certificate. If the provided certificate cannot be trusted, TLS connection establishment is terminated.
...
- `ldap` — {{ ydb-short-name }} will establish a connection to the LDAP server without any encryption. Passwords will be sent to the LDAP server in plain text.
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection with the LDAP server using the TLS protocol from the very first query. To successfully establish a connection using the `ldaps` scheme, you must disable the use of the [`StartTls`](../../security/authentication.md#starttls) query in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
- If any other value is used, the default value `ldap` will be used.
...
- `true` - For service account authentication, the SASL protocol with the EXTERNAL mechanism will be used (authentication via client TLS certificate within mTLS). The client certificate specified in the `use_tls.cert_file` and `use_tls.key_file` parameters is used as authentication information. The `bind_dn` and `bind_password` parameters are not set in this case.
- `false` - For service account authentication, the simple bind method will be used. The `bind_dn` and `bind_password` parameters must be specified.
...
| User name suffix that allows distinguishing users from the LDAP directory from users authenticated by other providers.
...
| Allowed clock skew when checking JWT time-related fields: `exp` sets the token expiration time, `nbf` — the token start time, `iat` — the issue time.

Default value: `30s`
||
|| `external_idp_config.subject_claim_name`
| Name of the JWT string field from which the user SID is formed. If the field is missing or has a different type, the `sub` field is used.
...
| Discovery document refresh period after a successful request.
...
| JWKS refresh period after a successful request.
...
| Maximum age of the JWKS cache. If JWKS refresh fails, the keys are deleted after this period expires.

Default value: `2h`
||
|| `external_idp_authentication_domain`
| User name suffix that distinguishes external IdP users from users authenticated by other providers. The same suffix is added to group names obtained from the JWT token.
...
|| Parameter | Description ||
|| `use_access_service`
| Flag that enables user authentication in Yandex Cloud via IAM using AccessService.
...
| Flag that enables the use of TLS connections between {{ ydb-short-name }} and AccessService.
...
||
|| `path_to_root_ca`
| Path to the certificate authority certificate file used for interaction with AccessService.
...
| Flag that enables the use of IAM API keys. An API key is a secret key issued in Yandex Cloud IAM for simplified authorization of service accounts in the Yandex Cloud API. It is used when it is not possible to automatically request an IAM token.
...
To reduce the number of [authentication token checks](../../security/authentication.md#token-validation), each {{ ydb-short-name }} node caches the check results in [user tokens](../../concepts/glossary.md#user-token). For more details, see the article [{#T}](../../security/caching-authentication-results.md).

The token lifetime and other aspects of its operation are configured using the following parameters. Time parameter values are specified as a number with a unit suffix: `ms` — milliseconds, `s` — seconds, `m` — minutes, `h` — hours, `d` — days. For example, `300ms`, `30s`, `10m`, `1h`, or `2d`.
...
||
|| `life_time`
| The period of storing the user token in the cache of the {{ ydb-short-name }} node since its last use. If no requests from the user for whom the token was created have arrived at the {{ ydb-short-name }} node during the specified period, the node removes this user token from its cache.

Default value: `1h`
||
|| `expire_time`
| The validity period of the successful check result for most types of authentication tokens. After a successful refresh, the countdown restarts. After the period expires, the entry is removed from the cache regardless of `life_time`.
...
| Limits the increase of the interval between retry checks after retryable user token update errors. Does not limit the total duration of retry checks.

Default value: `1m`
||
|#

Example of login and password authentication:
...
After a user token is created, the next scheduled check is performed after an interval randomly selected in the range from `30m` to `1h` (`refresh_time`). This condition is checked every `1s` (`refresh_period`). After a successful check, the interval is selected again. If a retryable error occurs, the first retry check is performed immediately. If it also ends with a retryable error, the next check is performed after an interval randomly selected in the range from `500ms` to `1s` (`min_error_refresh_time`). Then the current interval is doubled, but does not exceed `59s` (`max_error_refresh_time - min_error_refresh_time`). The actual delay is each time randomly selected in the range from half to the full current interval.

For an authentication token obtained by login and password, the `expire_time` parameter does not apply: the validity period of such a token is set by `login_token_expire_time`, so it becomes invalid after `12h`. The cached entry stops being used at the nearest check after this condition, but may be deleted earlier due to lack of requests for `2h` (`life_time`) or a persistent error.
...
{{ ydb-short-name }} allows you to configure the authentication type of database nodes when they are registered in the cluster. This type is configured via the `node_registration_token` parameter of the `auth_config` section.

#|
|| Parameter | Description ||
|| `node_registration_token`
| Defines the authentication type of database nodes when they are registered in the cluster {{ ydb-short-name }}.
...
- "root@builtin" — an authentication mode via a special debug token. This mode is planned to be removed in future releases and is not recommended for use: to ensure cluster security, it is recommended to use the node authentication mode via TLS certificates by setting an empty value for the parameter.

  ||
  |#

Example of the `auth_config` section with certificate-based node registration:
