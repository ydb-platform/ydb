...
|| `use_login_provider`
| The flag enables authentication of local users by auth tokens obtained by logging in with a login and password. The login procedure in {{ ydb-short-name }} is the exchange of a login and password for an authentication token.
...
||
|| `bind_dn`
| Distinguished Name (DN) of the service account used to search for the user record
||
|| `bind_password`
| Password of the service account used to search for the user record. Not set when `extended_settings.enable_sasl_external_bind: true`
||
|| `search_filter`
| Filter for searching the user record in the LDAP directory. The filter string may contain the sequence *$username*, which will be replaced with the username requested for authentication in the database
||
|| `use_tls`
| Settings for configuring the TLS connection between {{ ydb-short-name }} and the LDAP server
||
|| `enable`
| Determines whether an attempt will be made to establish a TLS connection with [ using the `StartTls` request ](../../security/authentication.md#starttls). When this parameter is set to `true`, you must disable the use of the `ldaps` connection scheme by setting the `ldap_authentication.scheme` parameter to `ldap`
||
|| `ca_cert_file`
...
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection to the LDAP server over TLS from the very first request. To successfully establish a connection using the `ldaps` schema, you need to disable the use of [query `StartTls`](../../security/authentication.md#starttls) in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
...
| The flag determines whether a query will be executed to retrieve the entire tree of groups that include the user's immediate groups.
...
| The username suffix that distinguishes LDAP directory users from users authenticated by other providers.
...
| Allowed clock skew when checking time-related JWT fields: `exp` sets the token expiration time, `nbf` sets the token start time, `iat` sets the issue time.
...
| Refresh period for the Discovery document after a successful request.
...
| Refresh period for JWKS after a successful request.
...
The parameter applies to refreshable authentication tokens, for example, login and password entry tokens and tokens of an external identity provider.
...
||
|| `expire_time`
| The validity period of a successful check result for most types of authentication tokens. After a successful refresh, the countdown starts over. When the period expires, the record is removed from the cache regardless of `life_time`.

For login and password entry tokens and tokens of an external identity provider, the validity period of the authentication token itself is used. For requests signed with an access key, a separate parameter `as_signature_expire_time` is used.

Default value: `24h`
||
|| `as_signature_expire_time`
| The validity period of the check result of a request authenticated with an access key signature.
...
| Limits the increase of the interval between retries after retryable user token update errors. Does not limit the total duration of retries.
...
{{ ydb-short-name }} allows you to configure the authentication type of database nodes when they are registered in the cluster. This type is configured through the `node_registration_token` parameter of the `auth_config` section.
...
Example of the `auth_config` section with node registration configuration using a certificate:
