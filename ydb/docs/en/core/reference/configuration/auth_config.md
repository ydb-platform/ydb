...
|| `use_login_provider`
| The flag enables authentication of local users by auth tokens obtained as a result of login with login and password. The login procedure in {{ ydb-short-name }} is the exchange of login and password for an authentication token.
...
- `false` — local users can exist both at the cluster level and at the level of each individual database. The access rights boundaries of local users created at the database level are limited to the database in which they are created.
...
Any changes to the password policy do not affect existing user passwords, so there is no need to change them; they will be accepted as is.

{% endnote %}

## LDAP authentication configuration {#ldap-auth-config}

One of the ways to authenticate users in {{ ydb-short-name }} is to use an [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) directory. More about this type of authentication is described in the section about [using an LDAP directory](../../security/authentication.md#ldap). To configure LDAP authentication, you need to describe the `ldap_authentication` section.
...
- `ldaps` — {{ ydb-short-name }} will establish an encrypted connection to the LDAP server over the TLS protocol from the very first request. To successfully establish a connection using the `ldaps` scheme, you need to disable the use of [query `StartTls`](../../security/authentication.md#starttls) in the `ldap_authentication.use_tls.enable: false` section and fill in the certificate information `ldap_authentication.use_tls.ca_cert_file` and the certificate requirement level `ldap_authentication.use_tls.cert_require`.
...
| The flag determines whether a query will be executed to retrieve the entire tree of groups that include the user's direct groups.
...
||
|| `host`
| The hostname on which the LDAP server runs. This is a deprecated parameter; the `hosts` parameter should be used instead.
||
|| `ldap_authentication_domain`
| The username suffix that distinguishes users from the LDAP directory from users authenticated using other providers.
...
The lifetime and other aspects of the user token's operation are configured using the following parameters. Time parameter values are specified as a number with a unit suffix: `ms` — milliseconds, `s` — seconds, `m` — minutes, `h` — hours, `d` — days. For example, `300ms`, `30s`, `10m`, `1h`, or `2d`.
...
The parameter applies to refreshable authentication tokens, for example, login and password tokens and tokens of an external identity provider.

Default value: `1h`
||
|| `life_time`
| The period of storing a user token in the {{ ydb-short-name }} node cache since its last use. If no requests from the user for whom the token was created have arrived at the {{ ydb-short-name }} node during the specified period, the node removes this user token from its cache.
...
| The validity period of the check result for a request authenticated with an access key signature.

Default value: `1m`
||
|| `min_error_refresh_time`
| Initial interval between repeated checks after a retryable error in user token refresh.
...
It is not recommended to set the parameter value to `0`, because immediate retries create excessive load.
