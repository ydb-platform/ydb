...
{{ ydb-short-name }} lets you use various ways to authenticate users in the system. Authentication and authentication provider settings are specified in the `auth_config` section of the {{ ydb-short-name }} configuration file.

## Configuring authentication of {{ ydb-short-name }} local users {#local-auth-config}

For more about authentication of [local users](../../concepts/glossary.md#access-user), see the section on [login and password authentication](../../security/authentication.md#static-credentials). To configure authentication of local users by login and password, specify the following parameters in the `auth_config` section:

#|
|| Parameter | Description ||
|| `use_login_provider`
| The flag enables authentication of local users by auth tokens obtained as a result of logging in with a login and password. The login procedure in {{ ydb-short-name }} is the exchange of a login and password for an authentication token.
...
For local users to be created and authenticated, the `use_login_provider` and `enable_login_authentication` parameters must be set to `true`. Otherwise, local users will not be able to authenticate in {{ ydb-short-name }}.
...
| The flag enables creating local users and obtaining an authentication token for them in exchange for a login and password.

Possible values:

- `true` — enables creating local users and obtaining an authentication token for them
- `false` — disables creating local users and obtaining an authentication token for them
...
- `true` — {{ ydb-short-name }} local users exist at the cluster level and can be granted access rights to multiple [databases](../../concepts/glossary.md#database).
...
### Configuring user lockout on incorrect password entry {#account-lockout}

{{ ydb-short-name }} lets you prevent a user from authenticating if they have made several failed password entry attempts. To configure user lockout conditions, fill in the `account_lockout` section.
...
|| Parameter | Description ||
|| `attempt_threshold`
| The number of incorrect password attempts after which the user account is temporarily blocked. If the user enters an incorrect password the specified number of times in a row, they are prohibited from authenticating for the time specified in the `attempt_reset_duration` parameter.
...
| The period of time during which the user is considered blocked. During this period, the user will not be able to authenticate in the system even if they enter the correct username and password. The blocking period starts from the moment of the last incorrect password attempt.

If a zero value is specified (`"0s"`, a record equivalent to 0 seconds), the user will be blocked for an unlimited time. In this case, the block can be removed using the [ALTER USER ...  LOGIN](../../yql/reference/syntax/alter-user.md) command.
...
Combining units of measurement in a single string is not allowed. For example, the following record is incorrect: `1d12h`. Such a record must be replaced with an equivalent one, for example `36h`.
...
{{ ydb-short-name }} allows authenticating users by login and password. For more details, see the [login and password authentication](../../security/authentication.md#static-credentials) section. To improve security, {{ ydb-short-name }} provides the ability to configure the complexity of passwords used by [local users](../../concepts/glossary.md#access-user). To configure password requirements, you need to define the `password_complexity` section.
...
|| Parameter | Description ||
|| `hosts`
| List of hostnames on which the LDAP server runs
||
|| `port`
...
||
|| `search_filter`
| Filter for searching user records in the LDAP directory. The string filter may contain the sequence of characters *$username*, which will be replaced with the username requested for authentication in the database
||
|| `use_tls`
| Settings for configuring the TLS connection between {{ ydb-short-name }} and the LDAP server
||
|| `enable`
| Determines whether an attempt will be made to establish a TLS connection with [ using the `StartTls`](../../security/authentication.md#starttls) request. When setting this parameter to `true`, you must disable the use of the `ldaps` connection scheme by setting the `ldap_authentication.scheme` parameter to `ldap`
||
|| `ca_cert_file`
| Path to the certificate file of the certificate authority
||
|| `cert_require`
...
| Flag that determines whether a query will be executed to retrieve the entire tree of groups that include the user's immediate groups.
...
- `true` - The SASL protocol with the EXTERNAL mechanism will be used for service account authentication (authentication using a client TLS certificate within mTLS). The client certificate specified in the `use_tls.cert_file` and `use_tls.key_file` parameters is used as authentication information. The `bind_dn` and `bind_password` parameters are not set in this case.
...
{{ ydb-short-name }} supports [authentication using JWT tokens from an external identity provider with OpenID Connect](../../security/authentication.md#external-idp). To enable authentication, add the `external_idp_config` section to `auth_config`.
...
| Expected value of the `iss` field (issuer) that identifies the JWT token issuer, and the base URL for OIDC Discovery. Required parameter; must start with `https://` and must not end with the `/` character. The `issuer` values in the Discovery document and `iss` in the JWT must exactly match the specified value.

Default value: empty string
||
|| `external_idp_config.audience`
| Expected value of the `aud` field (audience) that identifies the JWT token recipient. If the parameter is not set, the token recipient is not checked.
...
| Period for updating the Discovery document after a successful request.
...
| Period for updating JWKS after a successful request.
...
| Timeout for the JWKS request.

Default value: `15s`
||
|| `external_idp_config.jwks_cache_settings.timeout`
| Maximum age of the JWKS cache. If JWKS update fails, the keys are removed after this period expires.
...
|| Parameter | Description ||
|| `use_access_service`
| The flag enables user authentication in Yandex Cloud via IAM using AccessService.
...
| Time period to wait for a response from the IAM server to a keepalive ping, in milliseconds. If no response from the IAM server arrives within the timeout, {{ ydb-short-name }} closes the connection.
...
To reduce the number of [authentication token checks](../../security/authentication.md#token-validation), each {{ ydb-short-name }} node caches the verification results in [user tokens](../../concepts/glossary.md#user-token). For more details, see the article [{#T}](../../security/caching-authentication-results.md).
...
||
|| `refresh_time`
| The maximum interval between a successful user token refresh and the next refresh attempt. The specific refresh time is chosen in the range from `refresh_time/2` to `refresh_time`.
...
||
|| `expire_time`
| The validity period of a successful check result for most types of authentication tokens. After a successful refresh, the countdown restarts. After the period expires, the entry is removed from the cache regardless of `life_time`.
...
| The initial interval between repeated checks after a retryable error in refreshing a user token.
...
Setting the parameter value to `0` is not recommended, since immediate retries create excessive load.
