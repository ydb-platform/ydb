...
| `require_same_issuer` | Require the "Issuer" field (certification authority name) to match for the client and server certificates.<br/>Valid values:<br/><ul><li>`true` — matching is required (applied by default if the parameter is not set);</li><li>`false` — matching is not required (the client and server certificates may be issued by different certification authorities).</li></ul> |
