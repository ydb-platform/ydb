...
|| `node_select_strategy` | NodeSelectStrategy | Enumeration | Strategy for selecting a node to launch a tablet. Possible values:
...
|| `boot_strategy` | — | Enumeration | Controls behavior when launching a large number of tablets. Possible values:
...
|| `default_tablet_limit` | — | Nested section | Restrictions on launching tablets of different types on a single node. Specified as a list, where each element has fields `type` and `max_count`. | Empty section ||
|| `default_tablet_preference` | —  | Nested section | Priorities for selecting datacenters for launching tablets of different types. For each tablet type, you can specify multiple groups of datacenters. Datacenters within the same group will have the same priority, and an earlier group will have priority over later ones. Example format:
...
| Empty section ||
|| `system_category_id` | — | Integer | If any non-zero number is specified, all coordinators and mediators are launched in the same datacenter if possible. | 1 ||
|# {wide-content}
...
In the subsections `default_tablet_limit` and `default_tablet_preference`, you need to specify tablet types. The exact names of tablet types are listed in the [glossary](../../concepts/glossary.md#tablet-types).
...
|| `min_counter_scatter_to_balance` | MinCounterScatterToBalance | Real number | Threshold of the Scatter metric for the dummy [Counter](../../contributor/hive.md#counter) resource. | 0.02 ||
|| `min_node_usage_to_balance` | MinNodeUsageToBalance | Real number | Resource consumption on a node below this value is treated as this value. Used to avoid balancing tablets between underloaded nodes. | 0.1 ||
...
|| `tablet_kick_cooldown_period` | TabletKickCooldownPeriod | Integer number of seconds | Minimum time period between movements of a single tablet. | 600 ||
...
|| `tablet_balance_strategy` | TabletBalanceStrategy | Enumeration | Strategy for selecting a tablet for relocation during autobalancing. Possible options:
...
|| `object_imbalance_to_balance` | ObjectImbalanceToBalance | Real number | Threshold of the [tablet imbalance metric for one object](../../contributor/hive.md#imbalance). | 0.02 ||
...
|| `balancer_ignore_tablet_types` | BalancerIgnoreTabletTypes | List of tablet types. When set via Hive UI — semicolon-separated. | Tablet types that are not subject to auto-balancing. | Empty list ||
|# {wide-content}
...
Hive collects [compute resource consumption metrics](../../contributor/hive.md#resources) from each node — CPU time, RAM, network — both overall per node and broken down by tablet. These settings allow you to control the collection of these metrics, their normalization, and aggregation.
...
|| `resource_overcommitment` | ResourceOvercommitment | Real number | Overcommitment ratio for node resources. | 3.0 ||
|| `pools_to_monitor_for_usage` | — | Pool names separated by commas | Actor system pools whose consumption is taken into account when calculating node resource consumption. | System,User,IC ||
|# {wide-content}

## Channel distribution options across storage groups {#storage}
...
This table contains advanced settings that typically do not need to be changed.
...
|| `default_unit_throughput` | DefaultUnitThroughput | Integer bytes/second | Default throughput consumption for one channel. | 1000 ||
|| `default_unit_size` | DefaultUnitSize | Integer bytes | Default disk space consumption for one channel. | 100000000 ||
|| `storage_overcommit` | StorageOvercommit | Real number | Overcommitment ratio for storage group resources. | 1.0 ||
...
- `HIVE_STORAGE_BALANCE_STRATEGY_SIZE` — Only occupied space is considered.
- `HIVE_STORAGE_BALANCE_STRATEGY_AUTO` — The parameter among the above whose consumption is maximum is considered.

| `HIVE_STORAGE_BALANCE_STRATEGY_SIZE` ||
|| `storage_safe_mode` | StorageSafeMode | true/false | Check for exceeding the maximum consumption of storage group resources. | true ||
...
- `HIVE_STORAGE_SELECT_STRATEGY_RANDOM` — Random selection of a group.
- `HIVE_STORAGE_SELECT_STRATEGY_ROUND_ROBIN` — selection of a group within a storage pool using the [Round-robin](https://en.wikipedia.org/wiki/Round-robin_scheduling) principle.
  ...
  || `min_period_between_reassign` | MinPeriodBetweenReassign | Integer seconds | Minimum period of time between reassignments of storage groups for channels of one tablet. | 300 ||
  ...
  || `space_usage_penalty_threshold` | SpaceUsagePenaltyThreshold | Real number | Minimum ratio of free space in the target group to free space in the source group, at which the target group will be penalized by applying a multiplicative penalty to the weight when moving a channel. | 1.1 ||
  || `space_usage_penalty` | SpaceUsagePenalty | Real number | Penalty coefficient for the penalization described above. | 0.2 ||
  || `channel_balance_strategy` | ChannelBalanceStrategy | Enumeration | Channel selection strategy for reassignment during channel balancing. Possible options:
...
- `HIVE_CHANNEL_BALANCE_STRATEGY_RANDOM` — Random channel selection.
...
|| `min_group_usage_to_balance` | MinGroupUsageToBalance | Real number | Storage group resource consumption threshold below which balancing is not triggered. | 0.1 ||
...
|| `tablet_restarts_period` | — | Integer milliseconds | Window size for counting tablet restarts to pessimize the launch of problematic tablets. | 1000 ||
|| `tablet_restarts_max_count` | — | Integer | Number of restarts in the `tablet_restarts_period` window after which pessimization is applied. | 2 ||
|| `postopone_start_period` | — | Integer milliseconds | Frequency of attempts to launch problematic tablets. | 1000 ||
|# {wide-content}
...
|| `node_restarts_for_penalty` | NodeRestartsForPenalty | Integer | Number of restarts in the `node_restart_watch_period` window after which nodes receive a priority reduction. | 3 ||
|# {wide-content}
...
#|
|| Configuration parameter name | Hive web-viewer parameter name | Format | Description | Default value ||
|| `drain_inflight` | DrainInflight | Integer | Number of tablets simultaneously restarted during the smooth migration of all tablets from one node (drain). | 10 ||
...
|| `warm_up_enabled` | WarmUpEnabled | true/false | When enabled, the Hive database waits for all nodes to connect before starting tablets. When disabled, all tablets can be started on the first node that connects. | true ||
