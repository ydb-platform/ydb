...
| `listening_address` | string | `[::]` | The network address on which Kafka Proxy listens for incoming connections. Use `[::]` to listen on all interfaces (dual-stack), `127.0.0.1` or `[::1]` to restrict access to localhost. |
