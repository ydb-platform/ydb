...
#|
|| Parameter | Type | Value by default | Description ||
...
|| `transaction_timeout_ms` | uint32 | `300000` (5 minutes) | Maximum timeout for Kafka transactions, after which the transaction will be cancelled. ||
...
|| `ssl_cerificate` | string | - | Path to the SSL certificate file that includes both the certificate file and the key file. When this parameter is specified, Kafka Proxy automatically starts processing requests using the specified SSL certificate. ||
|| `cert` | string | - | Path to the SSL certificate file. When this parameter is specified, Kafka Proxy automatically starts processing requests using the specified SSL certificate. ||
