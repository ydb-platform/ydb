...
| `json_text_message_without_values` | bool | false | For `format: "json"`: if `false`, `message` includes structured values as `key=value` (e.g. `Text (a=1 b=2)`); if `true`, `message` contains only the text and structured values are emitted as separate JSON fields (reserved names are prefixed with `_`). |
