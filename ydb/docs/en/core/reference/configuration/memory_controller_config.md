...
There are many components inside {{ ydb-short-name }} [nodes](../../concepts/glossary.md#database-node) that utilize memory. Most of them need a fixed amount, but some are flexible and can use varying amounts of memory, thereby improving the performance of the entire system.
