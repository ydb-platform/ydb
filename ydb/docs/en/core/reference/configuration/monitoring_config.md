...
The `monitoring_config` section of the {{ ydb-short-name }} configuration file defines the [YDB Monitoring](../ydb-ui/ydb-monitoring.md) parameters.
...
- `true`: access to `/counters` and `/counters/hosts` only with an [authentication token](../../concepts/glossary.md#auth-token); requests undergo authentication and [permissions](../../concepts/glossary.md#access-right) checks.
...
- `true`: any response from `/healthcheck`, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (parameter `format=prometheus`), is returned only when the request includes an authentication token; requests undergo authentication and permission checks.
...
Example of setting parameters with authentication enabled on individual built-in monitoring pages.
...
{{ ydb-short-name }} opens a separate HTTP port for the [built-in interface](../ydb-ui/index.md), displaying [metrics](../../devops/observability/monitoring.md), and other auxiliary commands.
...
- `false` — a client certificate is not required; the behavior when a certificate is requested is determined by the `monitoring_ca_file` parameter.
