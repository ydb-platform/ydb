...
- `true` — any response from `/healthcheck`, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (parameter `format=prometheus`), is returned only when the request has an authentication token; requests undergo authentication and permission checks.
...
Path to the root (CA) certificate file. Enables the client certificate request during the TLS handshake for monitoring.
