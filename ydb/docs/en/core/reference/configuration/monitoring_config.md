...
The `monitoring_config` section of the {{ ydb-short-name }} configuration file sets the parameters for [YDB Monitoring](../embedded-ui/ydb-monitoring.md).
...
- `true` — any response `/healthcheck`, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (parameter `format=prometheus`), is returned only when requested with an authentication token; requests undergo authentication and permission checks.
...
Path to the file with the root (CA) certificate. Enables requesting a client certificate during TLS handshake for monitoring.
...
- Empty string — the server does not request a client certificate during TLS handshake; device authentication and client certificate authentication for monitoring are unavailable with this setting.
...
Requirement for a client certificate during TLS handshake for monitoring.
