...
The `monitoring_config` section of the {{ ydb-short-name }} configuration file defines the parameters of [YDB Monitoring](../embedded-ui/ydb-monitoring.md).
...
- `true` — any `/healthcheck` response, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (the `format=prometheus` parameter), is returned only when the request includes an authentication token; requests undergo authentication and permission checks.
...
- `false` — when mandatory authentication is enabled in the cluster, requests to `/healthcheck` without a token are still allowed if the output is requested in the Prometheus format (`format=prometheus`). For other response formats, `/healthcheck` general rules apply (see the note below).
...
{{ ydb-short-name }} opens a separate HTTP port for the [built-in interface](../../reference/embedded-ui/index.md), displaying [metrics](../../devops/observability/monitoring.md), and other auxiliary commands.

You can enable [TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security) on the HTTP port, so the port will accept only HTTPS connections. Regular HTTP requests will then be rejected at the TLS handshake level without any response and without a redirect to HTTPS. Simultaneous operation of HTTP and HTTPS on different ports is not supported: monitoring uses a single port that works either as HTTP or as HTTPS only.
...
Client certificate requirement during the TLS handshake for monitoring.
