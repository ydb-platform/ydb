...
The `monitoring_config` section of the {{ ydb-short-name }} configuration file sets the parameters for [YDB Monitoring](../ydb-ui/ydb-monitoring.md).
...
- `false` — with mandatory authentication in the cluster, requests to `/healthcheck` without a token are still allowed if the requested output is in the Prometheus format (`format=prometheus`). For other response formats, `/healthcheck` follows the general rules (see the note below).
