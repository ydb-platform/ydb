...
- `true` — any response from `/healthcheck`, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (parameter `format=prometheus`), is returned only for requests with an authentication token; requests undergo authentication and permission checks.
...
If mandatory authentication is enabled in security_config, a token is required for `/healthcheck` responses in any format except Prometheus, regardless of the `require_healthcheck_authentication` value.
...
Parameter for passing the contents of the SSL certificate and private SSL key directly in [PEM format](https://en.wikipedia.org/wiki/Privacy-Enhanced_Mail) without using separate files. Requirements for the parameter content:
...
Example of enabling mTLS for monitoring: requesting a client certificate without requiring its presentation.
