...
The `monitoring_config` section of the {{ ydb-short-name }} configuration file sets the parameters of YDB Monitoring.
...
- `true` — access to `/counters` and `/counters/hosts` only with an [authentication token](../../concepts/glossary.md#auth-token); requests undergo authentication and [permission](../../concepts/glossary.md#access-right) checks.
...
- `true` — any `/healthcheck` response, including the [Prometheus format](https://prometheus.io/docs/instrumenting/exposition_formats/) (the `format=prometheus` parameter), is only issued upon a request with an authentication token; requests undergo authentication and permission checks.
...
- `false` — with mandatory authentication in the cluster, requests to `/healthcheck` without a token are still allowed if the response is requested in the Prometheus format (`format=prometheus`). For other response formats, `/healthcheck` general rules apply (see the note below).
...
[TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security) can be enabled on the HTTP port, so the port will only accept HTTPS connections. Regular HTTP requests will then be rejected at the TLS handshake level without any response and without a redirect to HTTPS. Simultaneous HTTP and HTTPS operation on different ports is not supported: monitoring uses a single port that works either as HTTP or as HTTPS only.
...
- Then, sequentially specify intermediate certificates, if any;
...
Path to the file with the root (CA) certificate. Enables the client certificate request during the TLS handshake for monitoring.
...
- `true` — the server requires a client certificate: a connection without a certificate or with an untrusted certificate is not established. `true` can only be specified together with the specified `monitoring_ca_file`.
