...
⟦V1⟧ supports [LDAP](⟦U1⟧) for user authentication. The LDAP protocol has two options for enabling TLS.
...
Example of enabling TLS for LDAP via the `ldaps` protocol extension:
...
Example of enabling TLS for LDAP via ⟦C1⟧:

### Federated Queries

[Federated queries](../../concepts/query_execution/federated_query/index.md) allow {{ ydb-short-name }} to query various external data sources. Whether these queries occur over TLS-encrypted connections is controlled by the `USE_TLS` setting of ⟦C2⟧ queries. No changes to the server-side configuration are required.
