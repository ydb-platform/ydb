...
* `ydb:{{ third-party-ports.kafka }}` — host name and port.
...
```java
String HOST = "ydb:{{ third-party-ports.kafka }}";
...
output {
  kafka {
    ...
    bootstrap_servers => "ydb:{{ third-party-ports.kafka }}"
...
[OUTPUT]
  name                          kafka
  match                         *
  Brokers                       ydb:{{ third-party-ports.kafka }}
...
1. Create the executor process settings file `/etc/kafka-connect-worker/worker.properties'
    ...
    ```ini
    # Main properties
    bootstrap.servers=ydb:{{ third-party-ports.kafka }}
