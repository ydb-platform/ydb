...
* **Access token** is used when the [`token`](properties.md#token) or [`tokenFile`](properties.md#tokenFile) property is configured. This authentication method requires a {{ ydb-short-name }} authentication token, which can be obtained by executing the following {{ ydb-short-name }} CLI command: `ydb auth get-token`.
...
* **Service Account Key** is used when the [`saFile`](properties.md#saKeyFile) property is configured. This method extracts the service account key from a file and uses it for authentication.