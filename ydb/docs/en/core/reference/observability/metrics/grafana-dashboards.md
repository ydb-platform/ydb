...
| Elapsed Time vs CPU Time | Ratio of operation elapsed time (`ElapsedMicrosec`) to CPU time (`CpuMicrosec`) by node. A sustained value above 100% means sessions are spending time waiting rather than actively working: typically this is I/O wait or CPU overcommit on the hypervisor side. |
...
This section contains graphs showing the number of errors that occur.
...
| Dropped requests | Number of requests per second whose processing was terminated at the transport level due to an error (by gRPC method type) |
| Dropped responses | Number of responses per second whose sending was terminated at the transport level due to an error (by gRPC method type) |
...
Transaction information from the `DataShard transaction proxy` level.

| Name | Description |
| --- | --- |
| Transactions | Data shard transaction metrics |
| Latencies | Histograms of execution times of various stages of data shard transactions |
...
| Name | Description |
| --- | --- |
| Operations | Statistics of operations with the data shard for different operation types |
| Transactions | Information about data shard tablet transactions (by transaction types) |
...
| Write latency | Write latency: time from message creation to its write to the topic. Percentage of messages for which the write latency fell within intervals <100 ms, <200 ms, etc. |
| Partition throttling | Write throttling duration - waiting for available write quota. Percentage of messages whose write throttling duration fell within intervals <1 ms, <5 ms, etc. |
...
The dashboard displays charts for metrics of a single topic and its associated consumer. The topic is selected in the `topic` filter, the consumer in the `consumer` filter. The panels and metric descriptions are listed below.
...
| End-to-end latency | End-to-end latency: time from message creation to its read. Percentage of messages for which the end-to-end latency fell within intervals <100 ms, <200 ms, etc. |
| Read latency max | Maximum (across all partitions) difference between the current time and the write time of the last message to the topic, ms |
...
| Committed read lag max | Maximum (across all partitions) difference between the current time and the write time of the last committed message to the topic, ms |
