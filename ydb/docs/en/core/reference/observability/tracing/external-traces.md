...
{{ ydb-short-name }} supports passing external trace-ids to build a complete operation trace. The trace-id is passed according to the [trace context specification](https://w3c.github.io/trace-context/#traceparent-header) — the `traceparent` header of the gRPC request must contain a string of the form `00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01`. It consists of 4 parts separated by the `-` character:

1. Version – at the time of writing, the specification defines only version 00.
2. Trace id – the trace identifier that the request is part of.
3. Parent id – the parent span identifier.
...
Some SDKs support trace-id passing; you can find their list and usage examples on the [{#T}](../../../recipes/ydb-sdk/debug-otel-tracing.md) page.
