...
## Constructing an endpoint for connection
...
- `database` — full path of the database containing the topics.
...
- `my_db.balancer.example.com` — DNS name of the load balancer through which the SQS protocol is accessible;
- `8443` — network port;
...
For read and write operations, the AWS CLI uses the `--queue-url` parameter. It can be obtained via `get-queue-url`.
