...
- `my_db.balancer.example.com` — DNS name of the load balancer through which the Amazon SQS protocol is available.
- `8443` — network port.
...
- `{queue_name}` — value of the `--queue-name` parameter, the Amazon SQS queue name in the `{topic_name}@{consumer_name}` format. If the [shared (common) reader](../../concepts/datamodel/topic.md#shared-consumer) has the name `ydb-sqs-consumer`, it is sufficient to specify only `{topic_name}`.
