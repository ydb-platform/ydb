...
To allow the reader to determine the compression algorithm, an attribute `BodyEncoding` is provided with each message. If the attribute `BodyEncoding` is missing, no decompression is required. Possible attribute values: `gzip`, `lzop`, `zstd`, or `base64`.
