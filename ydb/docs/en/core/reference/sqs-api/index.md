...
To allow the reader to determine the compression algorithm, the `BodyEncoding` attribute is provided with each message. If the `BodyEncoding` attribute is missing, no decompression is required. Possible attribute values: `gzip`, `lzop`, `zstd`, or `base64`.
