...
See the description of the command for getting a token:
...
| `--timeout` | Client response timeout in milliseconds. After this time expires, there is no point in waiting for a result. |
...
By default, the command requests confirmation before outputting the token, since the token will be output to the console:
