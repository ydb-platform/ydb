...
| `--include-index-data BOOL` | Export index table data. By default, only index metadata is exported and indexes are built during import — this saves space and reduces export time, but can potentially increase import time. When this option is enabled, index table data is exported in full, allowing it to be copied rather than built during import.<br/>Default value: `false`. |
