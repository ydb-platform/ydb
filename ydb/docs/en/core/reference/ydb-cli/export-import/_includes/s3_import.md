...
If you need to load additional data into existing tables from S3, you can copy the S3 contents to the file system (for example, using [S3cmd](https://s3tools.org/s3cmd)) and use the [`tools restore`](../tools-restore.md) command.

{% note warning %}

The `tools restore` command loads data only into [row tables](../../../../concepts/datamodel/table.md#row-oriented-tables). To append data to existing [column tables](../../../../concepts/datamodel/table.md#column-oriented-tables), use [external data sources](../../../../concepts/datamodel/external_data_source.md). For more information, see [{#T}](../../../../concepts/query_execution/federated_query/import_and_export.md#import).
