...
- `--upload-batch-bytes <VAL>`: Limits the batch size of uploaded data, the default value is 512 KiB. `<VAL>` specifies the data amount with a unit, for example, 1MiB. Maximum value is 16 MiB.
