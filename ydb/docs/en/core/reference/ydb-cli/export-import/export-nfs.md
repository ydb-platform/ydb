...
The NFS export command requires specifying a mounted directory (or subdirectory) common to all objects participating in the export. Since the export is performed asynchronously on all {{ ydb-short-name }} hosts, the specified directory must exist on each {{ ydb-short-name }} host and be mounted in NFS.
