...
* **Улучшенная производительность:** Параметризованные запросы значительно повышают производительность при массовом выполнении однотипных операций, которые различаются только входными значениями. Это происходит благодаря кешированию плана запроса. После первой компиляции запроса его план сохраняется в [LRU-кеше](https://en.wikipedia.org/wiki/Cache_replacement_policies) [узла {{ ydb-short-name }}](../../concepts/glossary.md#node). Все последующие вызовы того же запроса с новыми параметрами избегают стадии компиляции и начинают выполняться сразу, что экономит время и ресурсы.
...
|Name|Description|
| ---|--- |
| `-p, --param ` |The value of a query parameter in the format `$name=value ` or ` name=value `, where ` name ` is the parameter name and ` value` is its value (a valid[JSON value](https://www.json.org/json-ru.html)). |
| `--input-file` |The name of the file in the format[JSON](https://en.wikipedia.org/wiki/JSON)in the encoding[UTF-8](https://en.wikipedia.org/wiki/UTF-8), which specifies the parameter values mapped to query parameters by key names. A maximum of one parameter file can be used.|
| `--input-format ` |The format for representing parameter values. Applies to all methods of passing them (via the command parameter, file, or ` stdin `).<br/>Possible values:<ul><li>` json `(default): The format[JSON](https://en.wikipedia.org/wiki/JSON).</li><li>` csv ` is the format[CSV](https://en.wikipedia.org/wiki/CSV). By default, parameter names must be in the CSV file header. For a single query execution, only one line in the file is allowed, excluding the header.</li><li>` tsv ` is the format[TSV](https://en.wikipedia.org/wiki/TSV).</li><li>` raw ` Input stream from ` stdin ` or `--input-file ` contains only the parameter value as binary data. The parameter name must be specified by the option `--input-param-name`.</li></ul> |
| `--input-binary-strings ` |The encoding format for parameter values of the «binary string» type (` DECLARE $par AS String `). Indicates how binary strings from the input stream should be interpreted.<br/>Possible values:<ul><li>` unicode `: Each byte in the binary string that is not a printable ASCII character (codes 32-126) must be encoded in[UTF-8](https://en.wikipedia.org/wiki/UTF-8).</li><li>` base64`: Binary strings are represented in the[Base64](https://en.wikipedia.org/wiki/Base64)encoding. This allows you to pass binary data, which will be decoded from Base64 by{{ ydb-short-name }} CLI.</li></ul> |

If values are specified for all parameters that do not allow `NULL ` (that is, with the NOT NULL type) [as part of the ` DECLARE`](../../yql/reference/syntax/declare.md) operator, the query will be executed on the server. If there is no value for at least one such parameter, the command will fail with the message "No value specified for the parameter."
...
| `--input-columns` |A string with column names that replace the header of a CSV/TSV document read from the input stream. When this option is specified, it is assumed that there is no header. The option is valid only with CSV and TSV formats.|
...
The query is executed as many times as there are sets of parameter values received. Each set received via the input stream (�## Iterative stream processing `stdin`

`--input-file ` CLI supports the ability to execute a query multiple times with different sets of parameter values by passing them via `--param` **or** an input file (not simultaneously). In this case, the connection to the database is established once, and the query execution plan is cached, which significantly improves the performance of this approach compared to separate CLI calls.

To use this feature, you need to pass different sets of values for the same parameters to the command input one after another, telling `--input--framing` CLI the rule that will allow you to separate these sets from each other.

The query is executed as many times as there are sets of parameter values received. Each set received via the input stream (
...
1. �The result obtained in this format can be used as input for the next query execution command.
...
Run the query by passing the contents of the file to `stdin`, with the output formatted as CSV:
...
The result obtained in this format can be used as input for the next query execution command by setting the header with the data in TSV format using the `--input-columns` option.
...
2. Limiting the maximum data processing delay.[выключено](#streaming-iterate)
...
| `--input-batch-max-delay ` |The maximum delay for sending a received set of parameters to processing in adaptive batching mode. It is specified as a number with a time dimension —` s `(seconds),` ms `(milliseconds),` m ` The default value is ` 1s `(1 second).<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first set of parameters for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on ` stdin`. |
...
{{ ydb-cli }} -p quickstart sql \
  ...
  --input-framing newline-delimited \[{#T}](topic-pipeline.md#example-read-to-yql-param)
...
Examples of processing messages read from a topic are provided in the article {{ ydb-short-name }}.

## See also ](../ydb-sdk/parameterized_queries.md)
