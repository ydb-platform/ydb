...
| `--input-batch-max-delay` |The maximum delay for sending a received parameter set to processing in adaptive batching mode. Specified as a number with a time dimension —`s`(seconds),`ms`(milliseconds),`m`(minutes), etc. The default value is`1s`(1 second).<br/><br/>{{ ydb-short-name }}CLI will count time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |
...
| `--input-batch-max-delay` |The maximum delay for sending a received set of parameters to processing in adaptive batching mode. It is specified as a number with a time dimension — `s` (seconds), `ms` (milliseconds), `m` (minutes), etc. The default value is `1s` (1 second).<br/><br/>{{ ydb-short-name }}The CLI will count the time from the moment the first set of parameters for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on `stdin`. |
...
## See also {#see-also}[{#T}](topic-pipeline.md#example-read-to-yql-param) {#see-also}
