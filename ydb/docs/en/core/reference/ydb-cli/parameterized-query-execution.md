...
* **Improved performance:** Parameterized queries significantly improve performance when executing large numbers of similar operations that differ only in input values. This is due to query plan caching. After the first compilation of a query, its plan is saved in the [LRU cache](https://en.wikipedia.org/wiki/Cache_replacement_policies) of the [{{ ydb-short-name }} node](../../concepts/glossary.md#node). All subsequent calls to the same query with new parameters skip the compilation stage and start executing immediately, saving time and resources.
...
This command supports passing parameters via command-line options, a file, and `stdin`. When passing parameters via `stdin` or a file, you can execute the query multiple times in a stream with different parameter values and support batching. The [{{ ydb-cli }} sql](sql.md) command provides the following parameters for this:

| Name | Description |
| --- | --- |
| `-p, --param` | The value of a single query parameter in the `$name=value` or `name=value` format, where `name` is the parameter name and `value` is its value (a valid [JSON value](https://www.json.org/json-ru.html)). |
| `--input-file` | The name of a file in [JSON](https://en.wikipedia.org/wiki/JSON) format and [UTF-8](https://en.wikipedia.org/wiki/UTF-8) encoding that contains parameter values matched against the query parameters by key names. Only one input file can be used. |
| `--input-format` | The format of parameter values applied to all sources of parameters (command line, file, or `stdin`).<br/>Available options:<ul><li>`json` (default): JSON format.</li><li>`csv`: [CSV](https://en.wikipedia.org/wiki/JSON) format.</li><li>`tsv`: [TSV](https://en.wikipedia.org/wiki/CSV) format.</li><li>`raw`: The input stream from `stdin` or `--input-file` contains only the parameter value as binary data. The parameter name must be specified with the `--input-param-name` option.</li></ul> |[TSV](https://en.wikipedia.org/wiki/TSV)
| `--input-binary-strings` | The encoding format for parameter values of the "binary string" type (`DECLARE $par AS String`). Defines how binary strings from the input stream should be interpreted.<br/>Available options:<ul><li>`unicode`: Every byte in the binary string that is not a printable ASCII character (codes 32-126) must be encoded in [UTF-8](https://en.wikipedia.org/wiki/UTF-8).</li><li>`base64`: Binary strings are represented in [Base64](https://en.wikipedia.org/wiki/Base64) encoding. This allows passing binary data that {{ ydb-short-name }} CLI will decode from Base64.</li></ul> |

If values are specified for all parameters that do not allow `NULL` (that is, with the NOT NULL type) [in the `DECLARE` clause](../../yql/reference/syntax/declare.md), the query will be executed on the server. If there is no value for at least one such parameter, the command will fail with the message "No value specified for the parameter."
...
| Name | Description |
| --- | --- |
| `--input-framing` | Sets the framing for the input stream (`stdin` or `--input-file`). Defines how the input stream will be split into separate parameter sets.<br/>Available options:<br/><ul><li>`no-framing` (default): The input stream is expected to contain a single parameter set, and the query is executed once.</li><li>`newline-delimited`: A newline character marks the end of a parameter set in the input stream, separating it from the next one. A parameter set is considered collected each time a newline character is received in the input stream.</li></ul> |
| `--input-param-name` | The name of the parameter whose value is passed to the input stream. Specified without the `$` symbol. Required when using the `raw` format in `--input-format`.<br/><br/>When used with the JSON format, the input stream is interpreted not as a JSON document, but as a JSON value, passing the value to the parameter with the specified name. |
| `--input-columns` | A string with column names that replaces the CSV/TSV document header read from the input stream. When specified, it is assumed that the header is absent. This option is only valid for CSV and TSV formats. |
| `--input-skip-rows` | The number of rows from the beginning of the data read from stdin to skip, not including the header row if present. This option is only valid for stdin CSV and TSV formats. |
| `--input-batch` | The mode for batching parameter sets obtained from the input stream (`stdin` or `--input-file`).<br/>Available options:<br/><ul><li>`iterative` (default): Batching is [disabled](#streaming-iterate)<br/>Available options:<br/><ul><li>`iterative` (default): Batching is [disabled](#streaming-iterate)<br/>Available options:<br/><ul><li>`iterative` (default): Batching is [disabled](#streaming-iterate). The query is executed for each parameter set (exactly once if `no-framing` is specified for `--input-framing`).</li><li>`full`: Full batch. The query will run once after the input stream is fully read, and all received parameter sets are wrapped in `List<...>`, with the parameter name set via the `--input-param-name` option.</li><li>`adaptive`: Adaptive batching. The query is executed each time a limit on the number of parameter sets in one request (`--input-batch-max-rows`) or a processing delay limit (`--input-batch-max-delay`) is triggered. All parameter sets received up to that point are wrapped in `List<...>`, with the parameter name set via the `--input-param-name` option.</li></ul> |
| `--input-batch-max-rows` | The maximum number of parameter sets per batch in the `adaptive` batching mode. The next batch will be sent for execution with the query if the number of data sets in it reaches the specified value. Setting to `0` removes the limit.<br/><br/>Default value: `1000`.<br/><br/>Parameters are passed to the query without streaming, and the total size of one gRPC request that includes parameter values has an upper limit of about 5 MB. |
| `--input-batch-max-delay` | The maximum delay for sending a received parameter set for processing in the `adaptive` batching mode. Specified as a number with a time unit: `s` (seconds), `ms` (milliseconds), `m` (minutes), etc. Default value: `1s` (1 second).<br/><br/>{{ ydb-short-name }} CLI will start counting time from the moment it receives the first parameter set for the batch and will send the accumulated batch for execution as soon as the time exceeds the specified value. This parameter allows for efficient batching in case of an unpredictable rate of new parameter sets appearing on `stdin`. |
...
The result obtained in this format can be used as input for the next query execution command by specifying the data in CSV format with the option `--input-columns`.
...
{{ ydb-short-name }} CLI supports automatic conversion of parameter sets into `List<...>`, allowing you to process multiple parameter sets in a single transaction with one request to the server, which can significantly improve performance compared to executing requests one by one.
...
This mode allows you to efficiently handle a wide range of input loads with unpredictable or infinite amounts of data, as well as unpredictable or rapidly changing rates of data arrival. In particular, this profile is typical when the output of another command is fed into `stdin` via the `|` operator.
