...
* **Protection against SQL injections:** Another important advantage of using parameterized queries is protection against ](https://en.wikipedia.org/wiki/SQL_injection)SQL injections. This security feature ensures proper handling of input data, reducing the risk of executing malicious code.

## Single query execution This command supports passing parameters via command-line options, a file, and`stdin`. When passing parameters via sql {#one-request}

|Name|Description|[{{ ydb-cli }} sql](sql.md)
| ---|--- |
...
| `--input-format` |The format for representing parameter values. Applies to all methods of passing them (via the command parameter, file, or`stdin`).<br/>Possible values:<ul><li>`json`(default): The format [JSON](https://www.json.org/json-ru.html).</li><li>`csv`is the format 
| `--input-binary-strings` |The encoding format for parameter values of the «binary string» type (`DECLARE $par AS String`). Indicates how binary strings from the input stream should be interpreted.<br/>Possible values:<ul><li>`unicode`: Each byte in the binary string that is not a printable ASCII character (codes 32-126) must be encoded in the [UTF-8](https://en.wikipedia.org/wiki/JSON).</li><li>`base64`: Binary strings are represented in the [Base64](https://en.wikipedia.org/wiki/UTF-8)encoding. This allows you to pass binary data, which will be decoded from Base64 by{{ ydb-short-name }} CLI.</li></ul> |

If values are specified for all parameters that do not allow `NULL`(that is, with the NOT NULL type) [as part of the`DECLARE`](https://en.wikipedia.org/wiki/UTF-8) operator, the query will be executed on the server. If there is no value for at least one such parameter, the command will fail with the message "No value specified for the parameter."[Base64](https://en.wikipedia.org/wiki/Base64)
...
| `--input-batch` |The mode for batching parameter sets obtained from the input stream (`stdin`or`--input-file`).<br/>Possible values:<br/><ul><li>`iterative`(default): Batching is [disabled](#streaming-iterate)<br/>|
...
| `--input-batch-max-delay`|The maximum delay for sending a received parameter set to processing in adaptive batching mode. Specified as a number with a time dimension —`s`(seconds),`ms`(milliseconds),`m`(minutes), etc. The default value is`1s`(1 second).<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |[disabled](#streaming-iterate)<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |[disabled](#streaming-iterate)<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |[disabled](#streaming-iterate)<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |[disabled](#streaming-iterate)<br/><br/>{{ ydb-short-name }} The CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on`stdin`. |[disabled](#streaming-iterate)
...
|Name|Description|
| ---|--- |
| `--input-framing` |Sets the framing for the input stream (file or`stdin`). <br/>Possible values:<ul><li>`no-framing`(default) — the input expects a single set of parameters, and the query is executed once after the stream is fully read.</li><li>`newline-delimited`— a newline character marks the end of one set of parameters in the input, separating it from the next. The query is executed each time a newline character is received.</li></ul> |

{% note warning %}

When using a line break as a delimiter for parameter sets, you must ensure that it does not appear within the parameter sets. Enclosing text in quotes does not make it acceptable to include a line break within such text. Multiline JSON documents are not allowed.
...
{{ ydb-short-name }} CLI supports automatic conversion of parameter sets into `List<...>`, allowing you to process multiple parameter sets in a single transaction with one request to the server. This can significantly improve performance compared to executing queries one by one.
...
{{ ydb-cli }} -p quickstart import file csv -p test_delete_1
  ```
{{ ydb-cli }} -p quickstart sql \
  -s 'SELECT t.id FROM test_delete_1 AS t WHERE t.id > 10' \[disabled](#streaming-iterate)
  --format json-unicode | \
{{ ydb-cli }} -p quickstart sql \
