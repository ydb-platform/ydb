...
## Single query execution `stdin` {#one-request}

This command supports passing parameters via command-line options, a file, and `stdin`. When passing parameters via [{{ ydb-cli }} or a file, you can execute the query multiple times in a stream with different parameter values and support batching. The ](sql.md) sql
...
|Name|Description|
| ---|--- |
| `--input-framing ` |Sets the framing for the input stream (` stdin ` or `--input-file `). Determines how the input stream will be divided into separate sets of parameters.<br/>Possible values:<br/><ul><li>` no-framing `(default): The input stream is expected to contain one set of parameters, and the query is executed once.</li><li>` newline-delimited`: A newline character marks the end of one set of parameters in the input stream, separating it from the next. A set of parameters is considered complete each time a newline character is received in the input stream.</li></ul> |
...
| `--input-skip-rows` |The number of lines from the start of the data read from stdin to skip, not including the header line if it is present. The option is valid only with CSV and TSV formats from stdin.|
| `--input-batch ` |The mode for batching parameter sets obtained from the input stream (` stdin ` or `--input-file `).<br/>Possible values:<br/><ul><li>` iterative `(default): Batching is [disabled](#streaming-iterate)<br/>Possible values:<br/><ul><li>` iterative `(default): Batching is [disabled](#streaming-iterate)<br/>Possible values:<br/><ul><li>` iterative `(default): Batching is [disabled](#streaming-iterate)<br/>Possible values:<br/><ul><li>` iterative `(default): Batching is [disabled](#streaming-iterate). The query is executed for each set of parameters (exactly once if the option `--input-framing ` uses ` no-framing `)</li><li>` full `: Full batch. The query will run once after the input stream is fully read, and all received parameter sets will be wrapped in ` List<...>`, the parameter name is set by the option `--input-param-name `</li><li>` adaptive `: Adaptive batching. The query is executed each time the limit on the number of parameter sets in one query (`--input-batch-max-rows `) or the processing delay (`--input-batch-max-delay `) is reached. All parameter sets received by that point are wrapped in ` List<...>`, the parameter name is set by the option `--input-param-name`.</li></ul> |
...
| `--input-batch-max-delay ` |The maximum delay for sending a received set of parameters to processing in adaptive batching mode. Specified as a number with a time dimension — ` s ` (seconds), ` ms ` (milliseconds), ` m ` (minutes), etc. The default value is ` 1s ` (1 second).<br/><br/>{{ ydb-short-name }} CLI will count the time from the moment the first set of parameters for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases where the rate of new parameter sets appearing is unpredictable.` stdin`. |
...
Through `stdin ` by passing only the parameter value and specifying the parameter name using the `--input-param-name` option:
...
|Name|Description|
| ---|--- |
| `--input-framing ` |Sets the framing for the input stream (file or ` stdin `). <br/>Possible values:<ul><li>` no-framing `(default) — the input is expected to contain a single set of parameters, and the query is executed once after the stream is fully read.</li><li>` newline-delimited`— a newline character marks the end of one set of parameters in the input, separating it from the next set. The query is executed each time a newline character is received.</li></ul> |
...
1. `a ` = 10, ` b` = 20
2. `a ` = 15, ` b` = 25
3. `a` = 35, �The result obtained in this format can be used as input for the next query execution command.

- TSV

Suppose we need to execute the query three times with the following sets of parameter values `b` and 
...
The result obtained in this format can be used as input for the next query execution command by setting the header with the TSV data using the `--input-columns` option.
...
{{ ydb-short-name }} CLI supports automatic conversion of parameter sets to `List<...>`, allowing you to process multiple parameter sets in a single transaction with a single request to the server, which can significantly improve performance compared to executing requests one by one.
...
|Name|Description|[выключено](#streaming-iterate)
| ---|--- |
...
| `--input-batch-max-delay ` |The maximum delay for sending the received parameter set to processing in adaptive batching mode. It is specified as a number with a time dimension —` s `(seconds),` ms `(milliseconds),` m `(minutes), etc. The default value is ` 1s `(1 second).<br/><br/>{{ ydb-short-name }} CLI will count the time from the moment the first parameter set for the batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows for efficient batching in cases of unpredictable rates of new parameter sets appearing on ` stdin`. |
