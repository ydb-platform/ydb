...
| `--input-param-name` | The name of the parameter whose value is passed in the input stream. Specified without the `$` character. Required when using the `raw` format in `--input-format`.<br/><br/>When used with the JSON format, the input stream is interpreted not as a JSON document but as a JSON value, passing the value to the parameter with the specified name. |
...
| `--input-skip-rows` | The number of lines from the beginning of the data read from stdin to skip, not including the header line if present. The option is only valid with stdin CSV and TSV formats. |
| `--input-batch` | Batching mode for parameter set values received from the input stream (`stdin` or `--input-file`).<br/>Possible values:<br/><ul><li>`iterative` (default): Batching [disabled](#streaming-iterate). The query is executed for each parameter set (exactly once if the `--input-framing` option uses `no-framing`)</li><li>`full`: Full batch. The query is executed once after the input stream has been fully read, all received parameter sets are wrapped in `List<...>`, the parameter name is set by the `--input-param-name` option</li><li>`adaptive`: Adaptive batching. The query is executed each time a limit is triggered on the number of parameter sets in a single query (`--input-batch-max-rows`) or on the processing delay (`--input-batch-max-delay`). All parameter sets received by that time are wrapped in `List<...>`, the parameter name is set by the `--input-param-name` option.</li></ul> |
...
| `--input-batch-max-delay` | Maximum delay for sending a received parameter set for processing in adaptive batching mode. Specified as a number with a time dimension - `s` (seconds), `ms` (milliseconds), `m` (minutes), etc. Default value — `1s` (1 second).<br/><br/>{{ ydb-short-name }} CLI will start counting time from the moment the first parameter set for a batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. The parameter allows efficient batching in case of an unpredictable rate of new parameter sets arriving at `stdin`. |
...
Command output (the exact number of bytes may differ):
...
{{ ydb-short-name }} CLI supports the ability to execute a query multiple times with different sets of parameter values, when they are passed via `stdin` **or** an input file (not simultaneously). In this case, the connection to the database is established once, and the query execution plan is cached, which significantly improves the performance of this approach compared to separate CLI invocations.

To use this feature, you need to pass different sets of values of the same parameters one after another to the command input, telling {{ ydb-short-name }} CLI the rule by which these sets can be separated from each other.

The query is executed as many times as the number of parameter value sets received. Each set received via the input stream (`stdin` or `--input-file`) is combined with the parameter values defined via the `--param` options. Command execution will be completed after the input stream ends. Each query is executed in its own transaction.
...
| Name | Description |
| --- | --- |
| `--input-framing` | Specifies framing for the input stream (file or `stdin`). <br/>Possible values:<ul><li>`no-framing` (default) — one parameter set is expected at the input, the query is executed once after the stream reading is complete.</li><li>`newline-delimited` — a newline character marks the end of one parameter set at the input, separating it from the next. The query is executed each time a newline character is received.</li></ul> |

{% note warning %}

When using a newline character as a separator for parameter sets, you must ensure that it is not present inside the parameter sets. Quoting the text does not make a newline inside such text valid. Multiline JSON documents are not allowed.
...
- CSV
  ...
  Let's execute the query, passing the contents of this file to `stdin`, with output formatted as CSV:
  ...
  Or simply passing the file name to the `--input-file` option:
...
- TSV
  ...
  Let's execute the query, passing the contents of this file to `stdin`, with output formatted as TSV:
...
{{ ydb-short-name }} CLI supports automatic conversion of parameter sets into `List<...>`, allowing a single server request to process multiple parameter sets in one transaction, which can further significantly improve performance compared to executing queries "one by one".
...
Full (`full`) mode is a simplified batching option where the query is executed once, wrapping all parameter sets received from the input stream into `List<...>`. If the query size turns out to be too large, an error will be returned.
...
When operating in adaptive (`adaptive`) mode, the input stream processing is split into multiple transactions, with automatic batch size selection for each of them.

This mode allows efficient processing of a wide range of input loads, with unpredictable or infinite amounts of data, as well as unpredictable or rapidly changing input arrival rates. In particular, this profile is typical when feeding the output of another command to `stdin` via the `|` operator.

Adaptive mode solves two main problems of dynamic stream processing:

1. Limiting the maximum batch size.
2. Limiting the maximum data processing latency.
...
| Name | Description |
| --- | --- |
| `--input-batch-max-rows` | Maximum number of parameter sets in a batch for adaptive batching mode. The next batch will be sent for execution together with the query if the number of parameter sets in it reaches the specified value. Setting to `0` removes the limit.<br/><br/>Default value — `1000`.<br/><br/>Parameters are passed to the query without streaming, and the total size of a single gRPC request that includes the parameter values has an upper limit of about 5 MB. |
| `--input-batch-max-delay` | Maximum send delay for processing the received parameter set for adaptive batching mode. Specified as a number with a time unit - `s` (seconds), `ms` (milliseconds), `m` (minutes), etc. Default value — `1s` (1 second).<br/><br/>{{ ydb-short-name }} CLI will start counting time from the moment the first parameter set for a batch is received and will send the accumulated batch for execution as soon as the time exceeds the specified value. This parameter allows efficient batching in case of an unpredictable rate of new parameter sets arriving at `stdin`. |
...
The first batch includes all rows accumulated at the input during the database connection opening time, so it is larger than subsequent ones.
...
#### Limit by number of records {#example-adaptive-limit}

To demonstrate adaptive batching with the trigger by the number of parameter sets, the first line of the command below generates 200 rows. The command will display parameter batches in each subsequent query call, taking into account the specified limit `--input-batch-max-rows` equal to 20 (default 1000).
...
#### Deleting many records from a {{ ydb-short-name }} row table by primary keys {#example-adaptive-delete-pk}
...
Insert 100,000 records into it:
...
Delete all records with `id` values greater than 10:
