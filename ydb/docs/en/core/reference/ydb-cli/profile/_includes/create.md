...
```bash
ydb config profile create quickstart --endpoint grpc://localhost:{{ ydb-ports.grpc }} --database <path_database>
```
...
```bash
{{ ydb-cli }} config profile replace local --endpoint grpc://localhost:{{ ydb-ports.grpc }} --database /Root/local
```
