...
| `--run-interval` | The interval for running the operation to delete rows with expired TTL. Specified in seconds. The default database settings do not allow an interval of less than 15 minutes (900 seconds). The default value is `3600`.<br/><br/>When choosing `--run-interval`, balance the delay before expired data is physically removed against the extra load on the cluster. Too small an interval triggers deletion background tasks frequently and can noticeably increase CPU and I/O load, especially with large data volumes. Too large an interval, conversely, delays physical deletion of rows: expired data occupies disk space longer and may continue to participate in scans.<br/><br/>Choose `--run-interval` based on the acceptable delay before data is actually deleted in your scenario. For most cases, starting with 600–900 seconds is reasonable, then tuning based on cluster load and how quickly expired rows are removed. For rarely updated tables and small data volumes, you can use a less frequent schedule; for heavily loaded tables with many expiring rows, use a more frequent schedule, but avoid an unnecessarily small interval.                                                                                                                                                                                                                                                     |
