...
| `--auto-partitioning-strategy` | [Auto-partitioning modes](../../concepts/datamodel/topic.md#autopartitioning_strategies). Possible values:<ul><li>`up` — the number of partitions can increase under high load, but cannot decrease;</li><li>`paused` — automatic scaling of the number of partitions is paused.</li></ul> |
