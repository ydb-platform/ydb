...
| `--retention-storage-mb`       | The maximum storage size, specified in MB. When the limit is reached, the oldest data will be deleted. Currently, this functionality does not work correctly when autopartitioning is enabled. The reason is that the amount of consumed space is controlled separately for each topic partition rather than for the topic as a whole. In other words, the size specified in the option is divided by the number of partitions and then enforced per partition. If the number of partitions changes automatically, the per-partition limits remain unchanged. This behavior will be fixed in future versions.<br/>The default value is `0` (no limit).                                                                                                                                                                                                                                 |
