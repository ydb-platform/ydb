...
| `--content-based-deduplication` | Enables content-based message deduplication for the topic. When enabled, messages with identical content are treated as duplicates and only the first message is stored. This is useful for FIFO topics where exact-once semantics are required based on message content rather than explicit deduplication IDs. |
