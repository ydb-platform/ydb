...
| `--retention-period`           | Data retention time in a topic. A positive number followed by a unit of time.<br/>The following units are supported:<ul><li>`s` – seconds;</li><li>`m` – minutes;</li><li>`h` – hours;</li><li>`d` – days.</li></ul>The default value is `24h`.                                                                                                                                 |
| `--partition-write-speed-kbps` | The maximum write speed to a [partition](../../concepts/datamodel/topic.md#partitioning), specified in KiB/s.<br/>The default value is `1024`.                                                                                                                                                                                                                                             |
| `--retention-storage-mb`       | The maximum storage size, specified in MiB. When the limit is reached, the oldest data will be deleted. The consumed space may exceed the set value when autopartitioning is enabled.<br/>The default value is `0` (no limit).                                                                                                                                                                                                                                 |
| `--auto-partitioning-strategy` | [Auto-partitioning modes](../../concepts/datamodel/topic.md#autopartitioning_modes). Possible values:<ul><li>`up` — the number of partitions can increase under high load, but cannot decrease;</li><li>`paused` — automatic scaling of the number of partitions is paused;</li><li>`disabled` — automatic scaling of the number of partitions is disabled.</li></ul>The default value is `disabled`.<br/> |
