...
| `--float-mode <value>`     | Float mode. Defines the type to be used for floating-point values. Available options: `double`, `decimal`. For more information about working with floating-point values, see [here](../../yql/reference/types/float.md). | `double` |
