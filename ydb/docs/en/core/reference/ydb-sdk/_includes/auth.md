...
1. Methods with explicit parameter passing, each of which implements one of the [authentication modes](../../../security/authentication.md).
...
You can click the methods below to go to the source code of the relevant example in the repository. You can also check out the [Authentication code recipes](../../../recipes/ydb-sdk/auth.md).
...
## Order of determining the authentication mode and parameters from the environment {#env}

The following algorithm is executed, which is the same for all SDKs:
...
The presence of **Metadata** as the last item in the mode selection algorithm allows you to deploy a working application on virtual machines and in Cloud Functions {{ yandex-cloud }} without setting any environment variables.

## Format of the settings file for the OAuth 2.0 token exchange authentication mode {#oauth2-key-file-format}

Description of the fields of the JSON settings file for the **OAuth 2.0 token exchange** authentication method. The set of fields depends on the type of the source token, `JWT` or `FIXED`.

In the table below, `creds_json` denotes JSON with parameters for the source token exchanged for an access token.
...
| `requested-token-type` | string | Type of the token to be received | `urn:ietf:params:oauth:token-type:access_token` |
| `subject-credentials` | creds_json | Subject credentials | optional |
| `actor-credentials` | creds_json | Actor credentials | optional |
| `token-endpoint` | string | Token endpoint. For {{ ydb-short-name }} CLI, it is overridden by the `--iam-endpoint` option. | optional |
...
| `private-key` | string | (Private) key in PEM format (for the `ES*`, `PS*`, and `RS*` algorithms) or Base64 (for the `HS*` algorithms) for signing |  |
...
## Features of {{ ydb-short-name }} Python SDK v2 (legacy version)

{% note warning %}

The behavior of {{ ydb-short-name }} Python SDK v2 (legacy version) differs from the one described above.

{% endnote %}

* How the `construct_credentials_from_environ()` function of {{ ydb-short-name }} Python SDK v2 works:
