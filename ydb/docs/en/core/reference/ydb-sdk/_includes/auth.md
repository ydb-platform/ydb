...
## File format that is used for OAuth 2.0 token exchange authentication mode parameters {#oauth2-key-file-format}
...
|token-endpoint|string|Token endpoint. In case of {{ ydb-short-name }} CLI it is overriden with `--iam-endpoint` option|optional|
