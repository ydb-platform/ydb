...
This article describes how to use {{ ydb-short-name }} SDK to coordinate the work of multiple instances of a client application by using [coordination nodes](../../concepts/datamodel/coordination-node.md) and the semaphores they contain.
...
- C++
  ...
  - `ReadConsistencyMode` – by default `RELAXED`, which allows reading a not‑most‑recent value in case of a leader change. Optionally you can enable `STRICT` read mode, where all reads go through the consensus algorithm and guarantee returning the most recent value, but become significantly more expensive.
  - `AttachConsistencyMode` – by default `STRICT`, which means the consensus algorithm is required for session recovery. Optionally you can enable `RELAXED` session recovery mode in case of failures, which disables this requirement. A relaxed mode may be needed with a very large number of clients, allowing session recovery without going through consensus, which does not affect overall correctness but may worsen reading a not‑most‑recent value during a leader change, as well as cause session expiration in case of issues.
  - `SelfCheckPeriod` (default 1 second) – the frequency at which the service performs its own liveness checks. Changing it is not recommended except in special cases.
    ...
    - The smaller the specified value, the higher the load on the server and the greater the responsiveness in detecting problems, but false positives may be generated when the service mistakenly detects issues.
  - `SessionGracePeriod` (default 10 seconds) – the period during which the new leader does not close open sessions, extending them.

    - The smaller the value, the smaller the window during which sessions from non‑existent clients, which did not have time to report their disappearance during a leader change, will hold semaphores and block other clients.
    - The smaller the value, the higher the likelihood of false positives, when a live leader may shut down as a precaution because it cannot be sure that this period has not ended on the new leader.
...
- C++
  ...
  - `Timeout` – the maximum timeout within which the session can be restored after losing connection to the service.
...
- C++
  ...
  In the C++ SDK, a background‑established session also supports and automatically restores the connection to the {{ ydb-short-name }} cluster.

- Go

  In the Go SDK, a session context `session.Context()` is used to track such situations; it ends together with the session. The SDK independently handles transport‑level errors and restores the connection to the service, attempting to re‑establish the session when possible. Thus, you only need to monitor the session context to react promptly to its loss.
...
- JavaScript
  ...
  In the JavaScript SDK, a signal `session.signal` is used to track such situations; it is aborted together with the session. The SDK independently handles transport‑level errors and restores the connection to the service, attempting to re‑establish the session when possible. Thus, you only need to monitor the session signal to avoid performing actions when the session has been closed or expired.
...
- Java
  ...
  Below is a complete example of a semaphore's lifecycle [semaphore](../../concepts/datamodel/coordination-node.md#semaphore): creating a node and a session, creating the semaphore, acquiring it, updating and reading data, and releasing it. A persistent semaphore must be created explicitly (`createSemaphore`); ephemeral semaphores are created on the first acquire (see the “Acquire semaphore” section).
...
- C++
  ...
  - `Count` - the amount by which the semaphore is increased when acquiring.
  ...
  - `OnAccepted` - called when the operation is queued (for example, if the semaphore could not be acquired immediately).
    ...
    - It is important to note that the call may occur in parallel with the result `TFuture`.
  - `Timeout` - the maximum time the operation may remain in the server queue.

    - The operation returns `false` if the semaphore could not be acquired within `Timeout` after being added to the queue.
    - When `Timeout` is set to 0, the operation effectively works like `TryAcquire`, i.e., the semaphore will be either acquired atomically and the operation returns `true`, or the operation returns `false` without using queues.
  ...
  - `Exclusive()` - alias for setting `Count = max`, acquiring the semaphore in exclusive mode (for semaphores created with a limit `Max<ui64>()`).
...
You can decrease (but not increase) the acquired semaphore's value by calling its `AcquireSemaphore` method again with a smaller value.

### Updating semaphore data {#update-semaphore}

You can update (replace) the semaphore data bound at its creation using the `UpdateSemaphore` method.
...
- Java
  ...
  Data update — the `updateSemaphore` method (step 4 in the “Creating a semaphore” example). The call does not require acquiring the semaphore and does not affect it.
...
- C++
  ...
  - `OnChanged` - called once after data changes on the server. With parameter `bool`, if `true` — the call occurred due to some changes, if `false` — it is a spurious call and you need to repeat `DescribeSemaphore` to restore the subscription.
  ...
  - `SessionId` - session identifier that performed this `AcquireSemaphore`.
...
- Java
  ...
  To subscribe to changes, use `watchSemaphore` with the same description mode and [WatchSemaphoreMode](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/main/java/tech/ydb/coordination/settings/WatchSemaphoreMode.java) (data, owners, or both). The [SemaphoreWatcher](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/main/java/tech/ydb/coordination/description/SemaphoreWatcher.java) object contains a snapshot of `SemaphoreDescription` and `getChangedFuture()` — `CompletableFuture<Result<SemaphoreChangedEvent>>` (see [SemaphoreChangedEvent](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/main/java/tech/ydb/coordination/description/SemaphoreChangedEvent.java), fields `isDataChanged`, `isOwnersChanged`). The Future completes on the next event; after notification, to continue watching, call `watchSemaphore` again (see [tests](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/test/java/tech/ydb/coordination/CoordinationServiceTest.java)).
...
- Rust
  ...
  Call [`Lease::release`](https://docs.rs/ydb/latest/ydb/struct.Lease.html#method.release) or simply drop ownership of `Lease` — when the value is destroyed, a release is also sent to the server.
...
Operations `AcquireSemaphore` and `ReleaseSemaphore` are idempotent. If `AcquireSemaphore` was invoked on the semaphore, repeated calls to `AcquireSemaphore` only change the acquisition parameters. For example, calling `AcquireSemaphore` with `count=10` can add an operation to the queue. Before or after a successful acquisition you can call `AcquireSemaphore` again with `count=9`, decreasing the number of acquired units; the new operation will replace the old one (which will finish with code `ABORTED` if it has not yet completed successfully). The position in the queue does not change despite replacing one operation `AcquireSemaphore` with another.

The `AcquireSemaphore` and `ReleaseSemaphore` operations return a `bool` indicating whether the operation changed the semaphore state. For example, `AcquireSemaphore` returns `false` if acquiring the semaphore failed within `Timeout` because it was held by another. The `ReleaseSemaphore` operation may return `false` if the semaphore is not held in the current session.

You can complete the queued operation `AcquireSemaphore` early by invoking `ReleaseSemaphore`. Regardless of the number of calls `AcquireSemaphore` for a specific semaphore within a single session, the release occurs with a single call `ReleaseSemaphore`, meaning that operations `AcquireSemaphore` and `ReleaseSemaphore` cannot be used as analogs of `Acquire` or `Release` on a recursive mutex.

The `DescribeSemaphore` operation with flags `WatchData` or `WatchOwners` creates a subscription to semaphore changes. Any older subscription to the same semaphore in the session is cancelled, triggering `OnChanged(false)`. You should ignore `OnChanged` from previous `DescribeSemaphore` calls if a new overriding call is made, for example by remembering the current call id.

The `OnChanged(false)` call can occur not only because of cancellation by a new `DescribeSemaphore`, but also for other reasons, such as a temporary connection break between the gRPC client and server, a temporary break between the gRPC server and the current service leader, or a change of the service leader—that is, at the slightest suspicion that a notification may have been lost. To restore the subscription, client code must issue a new `DescribeSemaphore` call, correctly handling the possibility that the result of the new call may differ (for example, if the notification was indeed lost).
