...
- Java
  ...
  If necessary, configure the node via [NodeConfig](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/main/java/tech/ydb/coordination/description/NodeConfig.java) using the `NodeConfig.create().with…` chain. Available parameters: `SelfCheckPeriod` and `SessionGracePeriod` periods, read and session connection consistency modes (`readConsistencyMode`, `attachConsistencyMode`), and rate limiter counter mode (`rateLimiterCountersMode`). Default values match the C++ description (see above). The resulting `NodeConfig` is passed to `CoordinationNodeSettings`.
...
- Java
  ...
  Typical scenario: after a successful `connect()`, you perform semaphore operations for your task (locking, leadership, and so on), then close the session. It is convenient to use try-with-resources to call `close()` upon completion and properly stop the thread with the node.
...
- Rust
  ...
  The session is created by [`CoordinationClient::create_session`](https://docs.rs/ydb/latest/ydb/struct.CoordinationClient.html#method.create_session) with a path to the node and [`SessionOptions`](https://docs.rs/ydb/latest/ydb/struct.SessionOptions.html) ([`SessionOptionsBuilder`](https://docs.rs/ydb/latest/ydb/struct.SessionOptionsBuilder.html): timeout, description, etc.). The stream with the node is started inside the session constructor; there is no separate `connect` call, as in Java.
...
- JavaScript
  ...
  In the JS SDK, the `session.signal` signal is used to track such situations; it is interrupted together with the session. The SDK independently handles transport-level errors and restores the connection to the service, attempting to restore the session if possible. Thus, the client only needs to monitor the session signal to avoid performing actions when the session has been closed or expired.

  The JavaScript SDK also provides a method for obtaining a new session when the old one is lost, and this approach is recommended for long-term use of `for await (session of client.openSession()) { session.signal }`.

- Java

  End the session (`close()`) when your scenario has finished: this explicitly releases the connection to the node. While the session is open, the SDK automatically reconnects on network failures according to `CoordinationSessionSettings`. Hold the semaphore only for the duration of the user task and release it via `SemaphoreLease.release()` when the resource is no longer needed.

- Rust

  In [`CoordinationSession`](https://docs.rs/ydb/latest/ydb/struct.CoordinationSession.html), call [`alive`](https://docs.rs/ydb/latest/ydb/struct.CoordinationSession.html#method.alive): it returns [`CancellationToken`](https://docs.rs/tokio-util/latest/tokio_util/sync/struct.CancellationToken.html), which is canceled when the session ends (similar to context tracking in Go). When [`Lease`](https://docs.rs/ydb/latest/ydb/struct.Lease.html) is released or on `Drop` session, the semaphore release is sent to the server in the background.
...
- Rust
  ...
  // or with custom data stored at the semaphore:
  session
...
- Java
  ...
  The `describeSemaphore` method takes a semaphore name and a [DescribeSemaphoreMode](https://github.com/ydb-platform/ydb-java-sdk/blob/master/coordination/src/main/java/tech/ydb/coordination/settings/DescribeSemaphoreMode.java) mode: data only, with a list of owners, with a list of waiters, or both lists.
  ...
  The elements of the owner and waiter lists (`getOwnersList`, `getWaitersList`) provide access to the session ID, timeout, requested `count`, operation data, and `orderId` (see the nested `SemaphoreDescription.Session` type in the source code).
...
- JavaScript
  ...
  To release a semaphore acquired in a session, call the `Release` method on the `Lease` object. If the semaphore was acquired using a `using` construct, it is released automatically when the scope is exited.
...
- Rust
  ...
  Call [`Lease::release`](https://docs.rs/ydb/latest/ydb/struct.Lease.html#method.release) or simply drop the `Lease` — releasing the value also sends a release to the server.
