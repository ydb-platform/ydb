...
| :--- | :--- | :--- |
| `ReturnVerboseStatus` | `bool` | If set, the response will also contain a summary of the overall database health in the `database_status` field ( [Example](#example-verbose)). Default is `false`. |
...
| `database_status` | If the `verbose` parameter is set in the settings, the `database_status` field will be populated. It provides a summary of the overall database health and is used for a quick assessment of the database state and identifying serious issues at a high level. [Example](#example-verbose). You can view the full response structure in the [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/public/api/protos/ydb_monitoring.proto) file in the {{ ydb-short-name }} Git repository. |
...
**How it works:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate group status based on them.
**Actions to take:** In the YDB Embedded UI, go to the database page, select the `Storage` tab, set the `Groups` and `Degraded` filters, and check the availability of nodes and disks on the nodes using the known group `id`.

#### Group has no redundancy

**Description:** The storage group has lost redundancy. One more disk failure may result in the loss of the group.
**How it works:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate group status based on them.
**Actions to take:** In the YDB Embedded UI, go to the database page, select the `Storage` tab, set the `Groups` and `Degraded` filters, and check the availability of nodes and disks on the nodes using the known group `id`.
...
**Actions to take:** In the Embedded UI, go to the database page, select the `Storage` tab, set the `Nodes` and `Degraded` filters. Using the known node `id` and pDisk, check the availability of nodes and disks on the nodes.
...
**Actions to take:** In the Embedded UI, go to the database page, select the `Storage` tab, set the `Nodes` and `Out of Space` filters, and view the available space using the known node `id` and PDisk.

#### PDisk is not available

**Description:** The physical disk is missing.
**Actions to take:** In the Embedded UI, go to the database page, select the `Storage` tab, set the `Nodes` and `Degraded` filters, and check the availability of nodes and disks on the nodes using the known node `id` and PDisk.

### STORAGE_NODE

#### Storage node is not available

**Description:** A storage node is missing. This information helps diagnose the upper layer of `PDISK`.
...
The shortest service response looks as follows. It is returned when the database is healthy:
...
A response in case of problems may look like this:
