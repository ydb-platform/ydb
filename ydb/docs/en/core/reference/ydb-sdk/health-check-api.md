...
**Actions:** Use the [YDB Embedded UI](../embedded-ui/ydb-monitoring.md) to check the availability of nodes and disks on the nodes from the list in the check. For example, through the [YDB Embedded UI](../embedded-ui/ydb-monitoring.md). If necessary, move vdisks from unavailable disks/nodes using `dstool`.
...
**Logic of work:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate status for the group accordingly.
**Actions:** Check the availability of nodes and disks on the nodes from the list in the check.
