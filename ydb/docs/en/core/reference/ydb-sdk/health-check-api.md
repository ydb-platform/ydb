...
- JavaScript
  ...
  You can create a monitoring client and call the health check methods yourself:
...
| `database_status` | If the `verbose` parameter is set in the settings, the `database_status` field will be filled. It provides a summary of the overall database state and is used for quickly assessing the database state and identifying serious issues at a high level. [Example](#example-verbose). The full response structure can be viewed in the [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/public/api/protos/ydb_monitoring.proto) file in the {{ ydb-short-name }} Git repository. |
...
**Description:** The storage group has lost integrity and is inoperative. Data is unavailable.
**How it works:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate group status based on them.
**Actions on trigger:** In [YDB Embedded UI](../embedded-ui/ydb-monitoring.md), go to the database page, select the `Storage` tab, set the `Groups` and `Degraded` filters, and check the availability of nodes and disks on the nodes using the known `id` of the group.
...
The shortest service response looks as follows. It is returned if the database is in order:
