...
**Logic:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate group status based on them.
...
**Description:** The storage group has lost redundancy. One more disk failure could lead to loss of the group.
**Logic:** `HealthCheck` checks various parameters (fault tolerance mode, number of failed disks, disk status, etc.) and sets the appropriate group status based on them.
**Actions on trigger:** In [YDB Embedded UI](../embedded-ui/ydb-monitoring.md), go to the database page, select the `Storage` tab, set the `Groups` and `Degraded` filters, and check the availability of nodes and disks on the nodes using the known `id` of the group.
...
**Description:** Reports the status of a physical disk.
...
**Actions on trigger:** In [Embedded UI](../embedded-ui/ydb-monitoring.md), go to the database page, select the `Storage` tab, set the `Nodes` and `Degraded` filters, and using the known `id` of the node and PDisk, check the availability of nodes and disks on the nodes.
