...
- Rust
  ...
  Inside the crate `ydb`, messages go through the standard library for the Rust ecosystem [`tracing`](https://docs.rs/tracing) (this is the crate name; it also includes regular text logs at debug or trace level, not only “distributed tracing”). To see output on the console, attach a subscriber before creating the client, for example [`⟦C3⟧_subscriber::fmt`](https://docs.rs/tracing-subscriber) with the desired level (`TRACE` for maximum detail). Example: [`basic-logs.rs`](https://github.com/ydb-platform/ydb-rs-sdk/blob/master/ydb/examples/basic-logs.rs).
...
- C#
  ...
  In {{ ydb-short-name }} C# SDK logging is connected via the standard `ILoggerFactory` interface from `Microsoft.Extensions.Logging`. You can pass any implementation — a console logger, Serilog, NLog, and others:
  ...
  Since `LoggerFactory` accepts the standard `ILoggerFactory`, you can plug in Serilog or NLog without additional adapters:
...
- PHP
  ...
  The YDB-PHP-SDK includes loggers in the `YdbPlatform\Ydb\Logger` namespace:

  * `NullLogger` – the default, which does not output anything
  * `SimpleStdLogger($level)` – a logger that writes logs to stderr.
