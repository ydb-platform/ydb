...
1. Create an OpenTelemetry `LoggerProvider` with an OTLP log exporter and the `service.name` resource attribute.
2. Redirect the SDK logger to this provider through an adapter (log appender / bridge) that converts each SDK log record into an OTel log record. For the list of adapters and the status of log support in different languages, see the [OpenTelemetry logging documentation](https://opentelemetry.io/docs/concepts/signals/logs/).
...
The method of log collection depends on the language and infrastructure — there is no single format:
...
* **Collector agents.** The application writes logs to a file or `stdout`, and a separate agent (e.g., the [filelog receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/filelogreceiver) in the [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/)) reads, parses, and forwards them to the backend — without changing the application code.
...
- C++
  ...
  {{ ydb-short-name }} The {{ ydb-short-name }} C++ SDK writes logs through `TLogBackend` from `util`. Implement a backend that converts each record into an OTel log record, and pass it to the driver via `TDriverConfig::SetLog`. For building the OpenTelemetry C++ SDK, see [Getting Started](https://opentelemetry.io/docs/languages/cpp/getting-started/).
  ...
  int main() {
    ...
    // 2. Passing the bridge to the YDB driver
...
- Go
  ...
  For the {{ ydb-short-name }} Go SDK, there is a ready-made adapter [ydb-go-sdk-otel](https://github.com/ydb-platform/ydb-go-sdk-otel) that converts SDK events into OpenTelemetry signals: traces (`WithTracer`), metrics (`WithMetrics`), and logs (`WithLogger`). The adapter does not configure exporters itself — you create a `LoggerProvider` with an OTLP log exporter, obtain a logger from it, and pass it to the `ydbOtel.WithLogger` option when calling `ydb.Open`. Each internal SDK log record (driver initialization, session pool, query execution, retries, etc.) is then sent to the collector as an OTLP log record.
...
- Java
  ...
  {{ ydb-short-name }} Java SDK writes logs via `slf4j`. If logback is used as the `slf4j` implementation, connect the ready-made `opentelemetry-logback-appender-1.0` appender, which forwards each event to the OpenTelemetry Logs SDK:
  ...
  Build an OpenTelemetry SDK instance with an OTLP log exporter and set it in the appender by calling `OpenTelemetryAppender.install(...)` before starting the workload:
  ...
  // Passing the OpenTelemetry SDK to the appender declared in logback.xml.
...
- Python
  ...
  {{ ydb-short-name }} Python SDK writes logs via the standard `logging` module (loggers named `ydb.*`). Use the built-in OpenTelemetry `LoggingHandler` to redirect these records to `LoggerProvider` and export via OTLP:
  ...
  # Bridge stdlib logging -> OpenTelemetry. Binding the handler to the root logger
  # intercepts everything that the SDK writes via logging.getLogger("ydb...").
...
- C#
  ...
  {{ ydb-short-name }} C# SDK writes logs via the `ILoggerFactory` passed to it. Create a factory based on the OpenTelemetry logging provider with an OTLP exporter and pass it to the data source:
  ...
  // Passing the factory to the SDK: each internal log (driver initialization, session pool,
