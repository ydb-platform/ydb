...
- Java
  ...
  When changing a topic, in the parameters of the `alterTopic` method, specify the topic path and the parameters to be changed.
...
- C++
  ...
  A connection to a topic for writing is represented by a write session object with the `IWriteSession` or `ISimpleBlockingWriteSession` interface (a variant for simple single-message write without acknowledgment, blocking when the number of in-flight records or the SDK buffer size is exceeded). Write session settings are represented by the `TWriteSessionSettings` structure; for the `ISimpleBlockingWriteSession` variant, some settings are not supported.
...
- C++
  ...
  Working with the `IWriteSession` object is generally structured as handling an event loop with three event types: `TReadyToAcceptEvent`, `TAcksEvent`, and `TSessionClosedEvent`.
  ...
  To write each message, the user must "spend" a move-only `TContinuationToken` object that the SDK provides with the `TReadyToAcceptEvent` event. When writing a message, you can set custom seqNo and creation timestamp, but by default the SDK sets them automatically.

  By default, `Write` is executed asynchronously: data is read from messages and saved to an internal buffer, and sending occurs in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session itself reconnects to {{ ydb-short-name }} when connections are broken and retries sending messages as long as possible, according to the setting `RetryPolicy`. When an error is received after which it is impossible to continue, the reading session sends `TSessionClosedEvent` with diagnostic information to the user.

  This is what writing multiple messages in an event loop without handlers might look like:
...
- Python
  ...
  - Native SDK
    ...
    # Convenient to start, convenient to use while only the message content is important.
    writer = driver.topic_client.writer(topic_path)
    ...
    writer.write(["mess-1", "mess-2"])  # Здесь за один вызов отправляется несколько сообщений —
                                         ...
                                         # makes sense for a large stream of messages.
...
- Java
  ...
  - Synchronous API
    ...
    The message entering this queue means that the writer will do everything possible to deliver the message.
    ...
    However, the message entering the send queue does not guarantee that the message will eventually be written.
    For example, errors may occur that cause the writer to stop before the messages from the queue are sent.
  ...
  - Asynchronous API
    ...
    You can also increase the size of the client buffer (`setMaxSendBufferMemorySize`) to handle a larger volume of messages before it becomes full.
...
- C++
  ...
  In such a write session, `TAcksEvent` events will not arrive to the user in `GetEvent` / `GetEvents`; instead, when receiving acknowledgments from the server, the SDK will call the passed handler. Similarly, you can configure handlers for other event types.
- Go

  When connecting, you can specify the synchronous write option - topicoptions.WithSyncWrite(true). Then Write will return only after receiving acknowledgment from the server that all messages passed in the call have been saved. At the same time, the SDK will, as usual, reconnect and retry sending messages if necessary. In this mode, the context only controls the timeout for the response from the SDK, i.e., even after context cancellation, the SDK will continue to attempt to send messages.
...
- C#
  ...
  Asynchronous write of a message to a topic. If the internal buffer overflows, it will wait until the buffer is free for retransmission.
...
- JavaScript
  ...
  All messages are written to the internal buffer. There are three mechanisms for sending to the server: two automatic and one manual. The manual one: calling the `writer.flush` method, which returns the last seqno written on the server. Automatic sending occurs under the following conditions:
...
- C++
  ...
  You can use the metadata writing function with the `Write()` method that accepts a `TWriteMessage` object:
...
- Java
  ...
  When constructing a message for writing using a Builder, you can pass it objects of type `MetadataItem` with a key-value pair where the key is of type `String` and the value is of type `byte[]`.
...
- Java
  ...
  - Asynchronous API
    ...
    For an asynchronous reader, in addition to the general read settings `ReaderSettings`, you will need the event handler settings `ReadEventHandlersSettings`, in which you must pass an instance of a subclass of `ReadEventHandler`.
...
You can also use the advanced connection creation option to specify multiple topics and set read parameters. The following code will create a connection to topics `my-topic` and `my-specific-topic` through the reader `my-consumer`:
...
- C++
  ...
  If a handler for a certain event is not set, you need to get and process it in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` signature.
- Go
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data in an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, the reader must be closed explicitly using the `Reader.Close()` call.
- Python

  `commit` is a fast call: it saves data in an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, the reader must be closed explicitly.
...
- Java
  ...
  To confirm message processing, it is enough to call the `commit` method on the message.
  ...
  This method returns `CompletableFuture<Void>`, whose successful completion means that the server has confirmed processing.
  In case of a commit error, do not attempt to retry it.
  Most likely, the error is caused by session closure.
...
- C++
  ...
  When processing events `TStartPartitionSessionEvent`, you can set the position from which to start reading in the response to the server. To do this, you should pass to the `Confirm` method the parameter `readOffset`. Additionally, you can pass the `commitOffset` parameter, which will specify the position up to which messages should be considered [committed](#commit).
  ...
  Also, the `ReadFromTimestamp` setting is supported in `TReadSessionSettings` for reading events with write timestamps not less than this. This setting is intended not for precise start positioning, but for skipping a volume of data over a large time interval. The first few received messages may have write timestamps less than the specified one.
- Go

  {% note tip %}

  In the default reader mode, offsets up to the position specified via `res.StartFrom` are committed on the server. After that, re-reading the same messages by moving the position back becomes impossible. To disable auto-commit, use the no-commit mode when creating a reader.
...
Usually, topic reading progress is saved on the server in each `Consumer`. But you can choose not to store such progress on the server and explicitly specify when creating a reader that reading will occur without `Consumer`.
...
- C++
  ...
  Upon reconnection, the read progress is not saved on the server. To avoid starting from the beginning, each time you start a partition read session, pass the offset in `TStartPartitionSessionEvent::Confirm` — see [storing the position on the client](#client-commit).
- Go

  You need to pass an empty string as the consumer name and the `topicoptions.WithReaderWithoutConsumer(false)` option (**experimental** mode, see [VERSIONING](https://github.com/ydb-platform/ydb-go-sdk/blob/master/VERSIONING.md) in the SDK repository). In the read selector, specify the topic path and the list of partitions. Message commits are not available in this mode (`CommitModeNone`); on reconnections, you need to restore progress on the client side — see [storing position on the client](#client-commit).
...
- Java
  ...
  In this case, keep in mind that when the connection is re-established, the progress on the server will be reset. Therefore, to avoid starting reading from the beginning, the SDK should pass the offset of the start of reading each time a partition reading session starts:
...
- C++
  ...
  Confirm event `TStopPartitionSessionEvent` processing after calling `Commit`.
...
- Rust
  ...
  The Rust SDK handles stop and partition session close events internally; there is no public API for configuring soft or hard read interrupts yet.
...
- Python
  ...
  In this example, message processing in the batch will stop if the partition is taken away during operation. This optimization requires additional code on the client. In simple cases, when processing taken-away partitions is not a problem, it can be omitted.
...
- C++
  ...
  In compatibility mode, there is no explicit signal about the completion of reading from a partition, and the server will try to heuristically determine that the client has processed the partition to the end. This may lead to a delay between the completion of reading from the source partition and the start of reading from its child partitions.

  If the client acknowledges message processing (commit), then the signal for completion of message processing from a partition will be the acknowledgment of processing of the last message of that partition. If the client does not acknowledge message processing, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to check the correctness of handling a soft read interrupt: the client must process the received messages, acknowledge their processing (commit) or save the read position in its database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go

  Enabling topic auto-scaling during its creation is done using the option `topicoptions.CreateWithAutoPartitioningSettings`:
  ...
  Enabling auto-scaling for an existing topic is done using the option `topicoptions.AlterWithAutoPartitioningStrategy` of `.Topic().Alter`:
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set by the option `topicoptions.WithReaderSupportSplitMergePartitions` when creating the reader. By default, full support mode (`true`) is used.
...
- Python
  ...
  Enabling topic auto-scaling during its creation is done using the argument `auto_partitioning_settings` of `create_topic`:
  ...
  Modifying an existing topic is done using the argument `alter_auto_partitioning_settings` of `alter_topic`:
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set by the argument `auto_partitioning_support` when creating the reader. By default, full support mode is used.
  ...
  From a practical standpoint, the modes are no different for the end user. Full support mode differs from compatibility mode in who guarantees the read order — the client or the server. Compatibility mode is achieved by server-side processing and, as a rule, works slower.
- JavaScript
...
Most often, it is convenient to perform commit within the reader that receives messages. However, there are scenarios where commit must be performed by a process different from the reading process. In that case, a commit method located outside the reader is needed.

{% list tabs group=lang %}

- C++

  Commit outside the read session is performed using the method `NYdb::NTopic::TTopicClient::CommitOffset`:
  ...
  If there is an active read session at the time of commit (e.g., via `CreateReadSession`), it is recommended to pass its identifier using the option `ReadSessionId` in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:
...
- Go
  ...
  Commit outside the reader is performed using the method `db.Topic().CommitOffset`:
  ...
  If there is an active read session at the time of commit (via `StartReader` or `StartListener`), it is recommended to pass its identifier using the option `WithCommitOffsetReadSessionID`. This allows the server not to interrupt the current read session:
...
- Python
  ...
  Commit outside the reader is performed using the method `topic_client.commit_offset`:
