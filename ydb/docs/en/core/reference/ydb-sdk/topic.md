...
- Go
  ...
  To work with topics, an instance of the {{ ydb-short-name }} driver created using `ydb.Open` is used. The topic client is available through the `db.Topic()` method.
...
- Python
  ...
  To work with topics, an instance of the {{ ydb-short-name }} driver is created. The topic client is available through the `topic_client` attribute and is used for management operations on topics, as well as creating writers and readers.
...
- C++
  ...
  An example of adding an [important consumer](../../concepts/datamodel/topic#important-consumer) to a topic and setting the [message retention time](../../concepts/datamodel/topic#retention-time) for the topic to two days:
...
- Go
  ...
  When modifying a topic, in the parameters you need to specify the topic path and the parameters to be changed.
...
- C++
  ...
  - IProducer
    ...
    - `PartitioningKeyHasher` — key transformation function before matching against ranges; used only for the `Bound` strategy. You can set your own, for example, so that the original key without hashing is used in the comparison.
...
- C++
  ...
  - IWriteSession
    ...
    Working with the `IWriteSession` object is structured as processing an event loop with three event types: `TReadyToAcceptEvent`, `TAcksEvent`, and `TSessionClosedEvent`.
  ...
  - ISimpleBlockingWriteSession
    ...
    As with `IWriteSession`, sending to the server is performed in the background. To wait for all writes to complete and close the session, call `Close()`.
  ...
  - IProducer
    ...
    `TProducerSettings` inherits `TWriteSessionSettings`, so buffering, sending, and reconnection work the same as in `IWriteSession`: `Write` puts the message into an internal buffer, sending to the server happens in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The producer reconnects to {{ ydb-short-name }} on connection breaks and retries sending as long as possible, according to `RetryPolicy`. On a fatal error, the producer closes; the status and reason can be obtained from the result of `Write` or `Flush`.

    `Flush` waits for delivery of accumulated data to the server; `Close` waits for sending remaining messages in the buffer and shuts down the producer.
...
- Go
  ...
  To send a message, it is enough to store a Reader in the Data field, from which data can be read. You can rely on the fact that the data of each message is read once (or until the first error); by the time Write returns, the data will have been read and stored in the internal buffer.

  SeqNo and message creation date are set automatically by default.

  By default, Write is performed asynchronously - data from messages is read and stored in the internal buffer, sending happens in the background. The Writer reconnects to {{ ydb-short-name }} on connection breaks and retries sending messages as long as possible. Upon receiving an error after which it is impossible to continue, the Writer stops and subsequent Write calls will fail with an error.
  ...
  For key-based writing to multiple partitions, use `WithWriteToManyPartitions(...)` when creating a writer and fill in the `Key` field in `topicwriter.Message`.

  Routing strategies (specified in `WithWriterPartitionByKey(...)` or `WithWriterPartitionByPartitionID()`):

  - `BoundPartitionChooser` — the key is matched against topic partition ranges (`FromBound`/`ToBound`). By default, before matching, the key passes through MurmurHash64. Recommended for topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning): the SDK updates boundaries when partitions are split.
  - `KafkaHashPartitionChooser` — by analogy with Kafka: MurmurHash of the key modulo the number of partitions. Convenient when migrating from Kafka. Not supported when auto-partitioning is enabled.
...
- Python
  ...
  To send messages, you can pass either the message content (bytes, str) or manually set some properties. Objects can be passed one at a time or in an array (list). The `write` method is executed asynchronously. The method returns as soon as the messages are placed into the client's internal buffer; this usually happens quickly. A wait may occur if the internal buffer is already full and you need to wait until some data is sent to the server.
  ...
  - Native SDK
    ...
    ```python
    # Simple message sending, without explicit metadata.
    # Convenient to start, convenient to use while only message content matters.
    writer = driver.topic_client.writer(topic_path)
    ...
    writer.write(["mess-1", "mess-2"])  # Здесь за один вызов отправляется несколько сообщений —
                                         ...
                                         # makes sense with a large message flow.

    # Full form, used when, besides message content, you need to manually set its properties.
...
- Java
  ...
  - Synchronous API
    ...
    The `send` method blocks execution until the message is placed into the send queue.
    The message entering this queue means that the writer will do its best to deliver the message.
    ...
    But entering the send queue does not guarantee that the message will eventually be written.
    For example, errors may occur that cause the writer to shut down before messages from the queue are sent.
...
- C#
  ...
  Asynchronous message writing to a topic.
...
### Writing messages with acknowledgment of persistence on the server
...
- C++
  ...
  - IWriteSession
    ...
    In such a write session, `TAcksEvent` events will not be delivered to the user via `GetEvent` / `GetEvents`; instead, when the SDK receives confirmations from the server, it will call the passed handler. Similarly, you can configure handlers for other event types.
...
- Go
  ...
  When connecting, you can specify the synchronous write option - topicoptions.WithSyncWrite(true). Then Write will return only after receiving confirmation from the server that all messages passed in the call have been saved. At the same time, the SDK will, as usual, reconnect and retry sending messages if necessary. In this mode, the context only controls the timeout for waiting for a response from the SDK, i.e., even after the context is canceled, the SDK will continue trying to send messages.
...
- Python
  ...
  - Native SDK
    ...
    ```python
    # Put several messages into the internal buffer, then wait until
    # all of them are delivered to the server.
    for mess in messages:
        writer.write(mess)

    writer.flush()

    # You can send several messages and wait for confirmation for the whole group.
    writer.write_with_ack(["mess-1", "mess-2"])

    # Waiting when sending each message — this method will return result only after receiving
...
- C#
  ...
  Asynchronous write of a message to a topic. If the internal buffer overflows, it will wait for the buffer to become free for retransmission.
  ...
  If the server is unavailable, messages may accumulate in the queue waiting to be sent. To control the wait time, you can use a cancellation token (`CancellationToken`). However, with this approach, there is a risk that the user may cancel the sending of an already confirmed message.
...
- C++
  ...
  - IProducer
    ...
    For `IProducer`, the transaction is set not by the `Write` argument, but in `TWriteMessage` via `Tx()`. After that, the producer writes the message in the same way as a regular message with a partitioning key:
...
You can also use the extended connection creation option to specify multiple topics and set read parameters. The following code will create a connection to topics `my-topic` and `my-specific-topic` via the `my-consumer` reader:
...
- C++
  ...
  If a handler for a certain event is not set, you must get and process it in the `GetEvent` / `GetEvents` methods. For non-blocking wait for the next event, there is the `WaitEvent` method with the `TFuture<void>()` signature.
- Go
...
- C++
  ...
  In this example, after creating the session, the main thread waits for the session to be completed by the server in the `GetEvent` method; other event types will not arrive.
- Go
...
- Java
  ...
  To acknowledge message processing, it is enough to call the `commit` method on the message.
  ...
  This method returns `CompletableFuture<Void>`, whose successful completion means acknowledgment of processing by the server.
...
Instead of committing messages to the server, you can store the read progress yourself. In this case, you need to pass a handler to the SDK that will be called when starting to read each partition. In this handler, you will need to
...
- C++
  ...
  When processing events `TStartPartitionSessionEvent`, you can set the position from which to start reading in the response to the server.
  To do this, pass to method `Confirm` the parameter `readOffset`.
...
- Java
  ...
  In the `StartPartitionSessionEvent` event handler, you can set the position from which reading should start when responding to the server.
...
- C++
  ...
  Upon reconnection, read progress is not saved on the server. To avoid starting from the beginning, at each start of a partition read session, pass the offset in `TStartPartitionSessionEvent::Confirm` — see [storing position on the client](#client-commit).
- Go

  You need to pass an empty string as the consumer name and the `topicoptions.WithReaderWithoutConsumer(false)` option (**experimental** mode, see [VERSIONING](https://github.com/ydb-platform/ydb-go-sdk/blob/master/VERSIONING.md) in the SDK repository). In the reader selector, specify the topic path and the list of partitions. Message commits are not available in this mode (`CommitModeNone`); upon reconnections, progress must be restored on the client side — see [storing position on the client](#client-commit).
...
- Java
  ...
  To read without a consumer, explicitly specify this in the reader settings `ReaderSettings` by calling `withoutConsumer()`:
  ...
  In this case, note that upon reconnection, progress on the server will be reset. Therefore, to avoid starting reading from the beginning, you should pass the starting read offset in the SDK at each start of a partition read session:
...
- Go
  ...
  for {
    err := db.Query().DoTx(ctx, func(ctx context.Context, tx query.TxActor) error {
      batch, err := reader.PopMessagesBatchTx(ctx, tx) // the batch will be committed upon the common commit of the transaction
      if err != nil {
        return err
      }
...
- Java
  ...
  - Synchronous API
    ...
    The `receive` method will associate the message offsets with the transaction on the server by calling `sendUpdateOffsetsInTransaction` and return control when it receives a response to it.
...
In {{ ydb-short-name }}, server-side partition balancing is used between clients. This means that the server can interrupt reading messages from arbitrary partitions.
...
- C++
  ...
  A soft interrupt arrives as an event `TStopPartitionSessionEvent` with the `Confirm` method. The client can finish processing messages and send an acknowledgment to the server.
...
- Go
  ...
  The client code immediately receives all messages available in the buffer (on the SDK side), even if there are not enough of them to form a packet during batch processing.
...
- C++
  ...
  A hard interrupt comes as an `TPartitionSessionClosedEvent` event either in response to a soft interrupt acknowledgment or when the connection to a partition is lost. You can find out the reason by calling the `GetReason` method.
...
- Python
  ...
  In this example, message processing in a batch will stop if the partition is revoked during operation. This optimization requires additional code on the client side. In simple cases where processing of revoked partitions is not a problem, you can skip it.
...
- Java
  ...
  - Synchronous API
    ...
    Not relevant, because the synchronous reader does not provide a way to configure handling of such events.
...
### Support for topic autoscaling {#autoscaling}
...
- C++
  ...
  In compatibility mode, there is no explicit signal of reading completion from a partition, and the server will try to heuristically determine that the client has processed the partition to the end. This may cause a delay between finishing reading from the original partition and starting reading from its child partitions.

  If the client commits messages, the signal of completion of message processing from a partition will be the commit of the last message of that partition. If the client does not commit messages, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to check the correctness of handling a soft read interruption: the client must process the received messages, commit them, or save the read position in its database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go
  ...
  Enabling autoscaling on an existing topic is done using the `topicoptions.AlterWithAutoPartitioningStrategy` option of `.Topic().Alter`:
  ...
  // full support mode (autoscaling handling in SDK, default)
  reader, err := db.Topic().StartReader(
    "consumer",
...
Most often, it is convenient to perform commits within the reader that receives messages. However, there are scenarios where commits must be performed by a process different from the reading process. In such a case, a commit method located outside the reader is needed.
...
- C++
  ...
  If at the time of commit there is an active read session (for example, via `CreateReadSession`), it is recommended to pass its identifier using the `ReadSessionId` option in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:
...
- Go
  ...
  If at the time of commit there is an active read session (via `StartReader` or `StartListener`), it is recommended to pass its identifier using the `WithCommitOffsetReadSessionID` option. This allows the server not to interrupt the current read session:
...
- Java
  ...
  // For AsyncReader, the session identifier can be obtained when processing the SessionStartedEvent event
