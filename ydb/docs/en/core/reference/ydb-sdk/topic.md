...
- C++
  ...
  The {{ ydb-short-name }} driver handles the interaction between the application and {{ ydb-short-name }} at the transport level. The driver must exist throughout the entire lifecycle of working with topics and must be initialized before creating the client.

  The topic service client ( [source code](https://github.com/ydb-platform/ydb/blob/d2d07d368cd8ffd9458cc2e33798ee4ac86c733c/ydb/public/sdk/cpp/client/ydb_topic/topic.h#L1589)) runs on top of the {{ ydb-short-name }} driver and is responsible for topic management operations as well as creating read and write sessions.
...
- Java
  ...
  The {{ ydb-short-name }} transport handles the interaction between the application and {{ ydb-short-name }} at the transport level. It must exist throughout the entire lifecycle of working with topics and must be initialized before creating the client.
  ...
  The topic service client ( [source code](https://github.com/ydb-platform/ydb-java-sdk/blob/master/topic/src/main/java/tech/ydb/topic/TopicClient.java#L34)) runs on top of the {{ ydb-short-name }} transport and is responsible for both topic management operations and creating writers and readers.
  ...
  This automatically closes the client and transport when exiting the block, as both are descendants of `AutoCloseable`.
- C#
...
- Python
  ...
  To work with topics, an instance of the {{ ydb-short-name }} driver is created. The topic client is available via the `topic_client` attribute and is used for topic management operations as well as creating writers and readers.
...
The only required parameter for creating a topic is its path; all other parameters are optional.
...
- C++
  ...
  Example of adding an [important consumer](../../concepts/datamodel/topic#important-consumer) to a topic and setting the [message retention period](../../concepts/datamodel/topic#retention-time) to two days:
...
- C++
  ...
  There is a separate method for getting information about a consumer - `DescribeConsumer`.
- Go
...
To delete a topic, simply specify its path.
...
- C++
  ...
  - IProducer
    ...
    `IProducer` is a high-level API over write sessions: a single object hides the management of multiple sessions and automatically selects a partition based on the message key. Producer settings are represented by the `TProducerSettings` structure, which inherits from `TWriteSessionSettings`, so the common write settings are the same as `IWriteSession`.

    Settings are specified via `TProducerSettings`:

    - `ProducerIdPrefix` — producer id prefix for write sub-sessions;
    - `PartitionChooserStrategy` — partition selection strategy based on message key:
      ...
      - `KafkaHash` — analogous to Kafka: MurmurHash is computed from the key, and the partition index is the remainder of dividing the hash by the number of partitions. Convenient when migrating from Kafka. Not supported when auto-partitioning is enabled.
    - `PartitioningKeyHasher` — key transformation function before matching against ranges; used only for the `Bound` strategy. You can set your own, for example, so that the original key is used in comparison without hashing.
...
- C++
  ...
  - IWriteSession
    ...
    Working with the `IWriteSession` object is structured as event loop handling with three event types: `TReadyToAcceptEvent`, `TAcksEvent`, and `TSessionClosedEvent`.
    ...
    If a handler for a particular event is not set, you need to retrieve and process it in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` interface.

    To write each message, the user must "consume" a move-only object `TContinuationToken` that the SDK provides with the `TReadyToAcceptEvent` event. When writing a message, you can set custom seqNo and creation timestamp, but by default the SDK sets them automatically.

    By default, `Write` is executed asynchronously — data from messages is read and stored in an internal buffer, and sending occurs in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session itself reconnects to {{ ydb-short-name }} on connection breaks and retries sending messages as long as possible, according to the setting `RetryPolicy`. When an error is received after which it is impossible to continue, the write session sends `TSessionClosedEvent` with diagnostic information to the user.
  ...
  - IProducer
    ...
    `TProducerSettings` inherits from `TWriteSessionSettings`, so buffering, sending, and reconnection work the same as in `IWriteSession`: `Write` puts the message into an internal buffer, sending to the server happens in the background according to the `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, and `BatchFlushSizeBytes` settings. The producer reconnects to {{ ydb-short-name }} when the connection is broken and retries sending as long as possible, according to `RetryPolicy`. On an unrecoverable error, the producer closes; the status and reason can be obtained from the result of `Write` or `Flush`.
...
- Go
  ...
  To send a message, simply store a Reader in the Data field from which the data can be read. You can rely on the fact that the data of each message is read once (or until the first error); by the time Write returns, the data will have been read and stored in the internal buffer.

  SeqNo and message creation date are set automatically by default.

  By default, Write is performed asynchronously — data from messages is read and stored in the internal buffer, and sending occurs in the background. The Writer automatically reconnects to {{ ydb-short-name }} on connection breaks and retries sending messages as long as possible. Upon receiving an error after which it is impossible to continue, the Writer stops and subsequent Write calls will fail with an error.
  ...
  - `BoundPartitionChooser` — the key is matched against the topic partition ranges (`FromBound`/`ToBound`). By default, the key is passed through MurmurHash64 before matching. Recommended for topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning): the SDK updates the boundaries when partitions split.
  - `KafkaHashPartitionChooser` — similar to Kafka: MurmurHash of the key modulo the number of partitions. Convenient when migrating from Kafka. Not supported when auto-partitioning is enabled.
  - `WithWriterPartitionByPartitionID` — write to the partition specified in the `PartitionID` field of the message. Does not combine with key-based routing; when a partition splits, the writer must be recreated manually.
  ...
  See a detailed example with key-based routing, alternative strategies (`KafkaHash` and `PartitionID`), and a transactional variant in the [ydb-go-sdk](https://github.com/ydb-platform/ydb-go-sdk/blob/master/examples/topic/topicwriter/topicwriter_to_many_partitions.go) repository.
- Python

  To send messages, you can pass either just the message content (bytes, str) or manually set some properties. Objects can be passed one at a time or in an array (list). The `write` method is asynchronous. The method returns as soon as the messages are placed into the client's internal buffer, which usually happens quickly. Waiting may occur if the internal buffer is already full and you need to wait until some data is sent to the server.
...
- Java
  ...
  - Synchronous API
    ...
    The `send` method blocks control until the message is placed into the send queue.
    A message entering this queue means that the writer will do everything possible to deliver the message.
    ...
    However, a message entering the send queue does not guarantee that the message will eventually be written.
    For example, errors may occur that cause the writer to terminate before the messages in the queue are sent.
  ...
  - Asynchronous API
    ...
    This is a way to signal the user that the write stream should be slowed down.
    In such a case, you should either skip messages or retry writes with exponential backoff.
...
### Writing messages with confirmation of server-side persistence
...
- C++
  ...
  - IProducer
    ...
    Confirmations from the server come the same way as for `IWriteSession`: through the `AcksHandler` handler in `TProducerSettings::EventHandlers`. To wait for the accumulated buffer to be delivered to the server, call `Flush()`.
...
- Go
  ...
  When connecting, you can specify the synchronous message write option - topicoptions.WithSyncWrite(true). Then Write will return only after receiving confirmation from the server that all messages passed in the call have been saved. At the same time, the SDK, as usual, will reconnect and retry sending messages if necessary. In this mode, the context only controls the timeout for waiting for a response from the SDK, i.e., even after the context is canceled, the SDK will continue trying to send messages.
...
- Python
  ...
  - `write_with_ack(...)` — sends a message and waits for confirmation of its delivery from the server. When sending several messages in a row, this method works slowly.
  ...
  - Native SDK
    ...
    # Waiting when sending each message — this method returns result only after receiving
...
- Java
  ...
  The `send` method returns `CompletableFuture<WriteAck>`. Its successful completion means confirmation of the write by the server.
...
- C++
  ...
  - IWriteSession
    ...
    If you need to send a message compressed with a different codec within a write session, you can use the `WriteEncoded` method specifying the codec and the size of the uncompressed message. For a successful write using this method, the used codec must be allowed in the topic settings.
...
- Go
  ...
  If necessary, you can set a fixed codec in the connection options. Then it will be used and no measurements will be taken.
...
For more details about writing without deduplication, see the [corresponding section of concepts](../../concepts/datamodel/topic#no-dedup).
...
- Python
  ...
  In the example below, there is no explicit call to `tx.commit()` — it happens implicitly when the lambda `callee` completes successfully.
...
- Java
  ...
  - Synchronous API
    ...
    In the settings of `SendSettings` of the `send` method, you can specify a transaction.
...
Reading messages from a topic can be performed with or without specifying a Consumer associated with this topic. If no Consumer is specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is provided in the [corresponding section](#no-consumer).

You can create a Consumer when [creating](#create-topic) or [altering](#alter-topic) a topic.
...
- Java
  ...
  - Synchronous API
    ...
    - `init()`: non-blocking, starts the initialization process in the background and does not wait for its completion.
    ...
    - `initAndWait()`: blocking, starts the initialization process and waits for its completion. If an error occurs during initialization, an exception is thrown.
...
You can also use the extended connection creation option to specify multiple topics and set read parameters. The following code will create a connection to topics `my-topic` and `my-specific-topic` via reader `my-consumer`:
...
Messages can also be read [without processing acknowledgment](#no-commit). In this case, on a new connection, all unacknowledged messages will be read, including those already processed.

Information about which messages have already been processed can be [stored on the client side](#client-commit) by passing the starting read position to the server when creating the connection. In this case, the message read position on the server does not change.

You can use [transactions](#read-tx). In this case, the read position changes when the transaction is committed. On a new connection, all unacknowledged messages will be read.

{% list tabs group=lang %}

- C++

  User's work with the `IReadSession` object is generally arranged as processing an event loop with the following event types: `TDataReceivedEvent`, `TCommitOffsetAcknowledgementEvent`, `TStartPartitionSessionEvent`, `TEndPartitionSessionEvent`, `TStopPartitionSessionEvent`, `TPartitionSessionStatusEvent`, `TPartitionSessionClosedEvent`, and `TSessionClosedEvent`.
  ...
  If a handler for a certain event is not set, it must be obtained and processed in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is a method `WaitEvent` with the signature `TFuture<void>()`.
- Go
...
- Rust
  ...
  // processing batch.messages and commit transaction
  ```
...
### Reading without message commit {#no-commit}
...
- Java
  ...
  - Synchronous API
    ...
    To read messages one by one without commit, use the following code:
  ...
  - Asynchronous API
    ...
    In the asynchronous client, there is no way to read messages one by one.
...
- C++
  ...
  When setting up a read session with the `SimpleDataHandlers` setting, it is enough to pass a handler for data messages. The SDK will call this handler for each batch of messages received from the server. Read commits will not be sent by default.
  ...
  In this example, after creating the session, the main thread waits for the session to be terminated by the server in the `GetEvent` method; other event types will not arrive.
- Go
...
- Java
  ...
  - Synchronous API
    ...
    In the synchronous client, there is no way to read a batch of messages at once.

  - Asynchronous API

    To read a batch of messages without commit, use the following code:
...
### Reading with message commit {#commit}

Message commit (commit) informs the server that the message from the topic has been processed by the receiver and no longer needs to be sent. When using reading with commit, you must commit all received messages without skipping. Message commit on the server occurs after committing the next interval of messages 'without holes'; the commits themselves can be sent in any order.

For example, messages 1, 2, 3 arrive from the server. The program processes them in parallel and sends commits in this order: 1, 3, 2. In this case, message 1 will be committed first, and messages 2 and 3 will be committed only after the server receives the commit for message 2.

In case of an error on message commit, you can log the error and continue working. The state of the message at this point is unknown. The message might have been committed, and then a network error occurred and the client did not receive the acknowledgment. If the message was not committed, it will be read again and will be processed again (possibly by another reader). There is no point in retrying the commit itself, because the read session for this message is already lost.

#### Reading messages one by one with commit
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, the reader must be closed explicitly by calling `Reader.Close()`.
- Python
...
- Java
  ...
  To confirm message processing, it is enough to call the `commit` method on the message. This applies to both synchronous and asynchronous readers. In an asynchronous reader, when processing a batch of messages, you can call `commit` either on the entire batch at once or on each message individually. This method returns `CompletableFuture<Void>`, the successful execution of which means acknowledgment of processing by the server. In case of a commit error, you should not try to retry it. Most likely, the error is caused by session closure. The reader (not necessarily the same one) will create a new session for this partition and the message will be read again.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the latest commits before exiting the program, you need to explicitly close the reader.
- Python
  ...
  `commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, the reader must be explicitly closed.
- Java
  ...
  - Synchronous API
    ...
    Not relevant, because the synchronous reader does not have the ability to read messages in batches.
...
Instead of committing messages to the server, you can store read progress yourself. In this case, you need to pass a handler to the SDK that will be called when reading of each partition starts. In this handler, you need to specify the position from which to start reading this partition.

{% list tabs group=lang %}

- C++

  When processing `TStartPartitionSessionEvent` events, you can set the position from which to start reading in the response to the server. To do this, pass to the `Confirm` method the `readOffset` parameter. Additionally, you can pass the `commitOffset` parameter, which will specify the position up to which messages should be considered [committed](#commit).
  ...
  `GetOffsetToReadFrom` is part of the example, not the SDK. Use your own method to determine the required starting read position for a partition with the given partition id.

  Also, `TReadSessionSettings` supports the `ReadFromTimestamp` setting for reading events with write timestamps not less than this. This setting is intended not for precise start positioning, but to skip a volume of data over a large time interval. The first few received messages may have write timestamps less than the specified one.
- Go

  {% note tip %}

  In the default reader mode, offsets up to the position specified via `res.StartFrom` are acknowledged on the server. After that, re-reading the same messages by shifting the position backward becomes impossible. To disable automatic acknowledgment, use the no-commit mode when creating the reader.
...
- Java
  ...
  Reading from a given offset in Java is only possible in the async reader.
  In the `StartPartitionSessionEvent` event handler, you can set the position to start reading from when responding to the server.
  ...
  You can also configure the `setReadFrom` reader to read events with write timestamps not less than a given value.
- JavaScript
...
Typically, the topic read progress is stored on the server in each `Consumer`. However, you can choose not to store such progress on the server and explicitly specify when creating the reader that reading will occur without `Consumer`.
...
- C++
  ...
  On reconnection, the read progress is not saved on the server. To avoid starting from the beginning, pass the offset in `TStartPartitionSessionEvent::Confirm` at each start of a partition read session — see [storing position on the client side](#client-commit).
- Go

  You need to pass an empty string as the consumer name and the `topicoptions.WithReaderWithoutConsumer(false)` option (**experimental** mode, see [VERSIONING](https://github.com/ydb-platform/ydb-go-sdk/blob/master/VERSIONING.md) in the SDK repository). In the reader selector, specify the topic path and the list of partitions. Message commits are not available in this mode (`CommitModeNone`); on reconnections, progress must be restored on the client side — see [storing position on the client side](#client-commit).
...
- Python
  ...
  - `event_handler` - a subclass of `ydb.TopicReaderEvents.EventHandler` that implements the function `on_partition_get_start_offset`. This function is responsible for returning the initial offset for reading messages when the reader starts, as well as during reconnections. The client application must specify this offset in the `ydb.TopicReaderEvents.OnPartitionGetStartOffsetResponse.start_offset` parameter. The function can also be implemented as asynchronous.
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to the transaction object into the settings for receiving events from the session.
  ...
  Acknowledgment of processing event `TStopPartitionSessionEvent` must be done after calling `Commit`.
...
- Go
  ...
  for {
    err := db.Query().DoTx(ctx, func(ctx context.Context, tx query.TxActor) error {
      batch, err := reader.PopMessagesBatchTx(ctx, tx) // batch will be committed upon the overall transaction commit
      if err != nil {
        return err
      }
...
- Java
  ...
  - Synchronous API
    ...
    In the settings of the `ReceiveSettings` method `receive`, you can specify a transaction:
    ...
    The `receive` method will link the message offsets with the transaction on the server by calling `sendUpdateOffsetsInTransaction` and will return control when it receives a response.
  ...
  - Asynchronous API
    ...
    After receiving a message in the handler `onMessages`, you can link one or more messages with a transaction.
...
With a *soft interrupt*, the client receives a notification that the server has already finished sending messages from the partition and no more messages will be read. The client can finish processing the messages and send an acknowledgment to the server.
...
- Go
  ...
  Client code immediately receives all messages available in the buffer (on the SDK side), even if they are not enough to form a packet during batch processing.
...
- Java
  ...
  - Synchronous API
    ...
    Not relevant, because in the synchronous reader there is no way to configure handling of such events.
    The client immediately responds to the server with an acknowledgment of stopping.

  - Asynchronous API

    To be able to react to such an event, override the `onStopPartitionSession(StopPartitionSessionEvent event)` method in the `ReadEventHandler` descendant object (see [Connecting to a topic for reading messages](#start-reader)).
...
- Rust
  ...
  The Rust SDK handles partition session stop and close events internally; there is no public API for configuring soft or hard read interruption yet.
...
- C++
  ...
  A hard interrupt comes as the `TPartitionSessionClosedEvent` event either in response to acknowledgment of a soft interrupt or when the connection to a partition is lost. You can find out the reason by calling the `GetReason` method.
...
- Python
  ...
  In this example, processing messages in a batch will stop if the partition is revoked during operation. This optimization requires additional code on the client side. In simple cases, when processing revoked partitions is not an issue, you can skip it.
...
- Java
  ...
  - Synchronous API
    ...
    Not relevant, because the synchronous reader does not have the ability to configure handling of such events.
...
- Rust
  ...
  The Rust SDK handles stop and close events of a partition session internally; there is no public API for configuring soft or hard interruption of reading yet.
...
- C++
  ...
  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always immediately confirm their processing (commit). This will balance the reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  If the client commits messages, the signal that message processing from a partition is complete will be the commit of the last message of that partition. If the client does not commit messages, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.
...
- Go
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set using the `topicoptions.WithReaderSupportSplitMergePartitions` option when creating the reader. By default, full support mode (`true`) is used.
...
- Python
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set using the `auto_partitioning_support` argument when creating the reader. By default, full support mode is used.
  ...
  From a practical standpoint, the modes do not differ for the end user. Full support mode differs from compatibility mode in who guarantees the read order — the client or the server. Compatibility mode is achieved through server-side processing and is generally slower.
- JavaScript
...
- Rust
  ...
  Track progress or vote for support in the Rust SDK: [ydb-rs-sdk#311](https://github.com/ydb-platform/ydb-rs-sdk/issues/311)
- PHP
...
- C++
  ...
  Commit outside the read session is done using the `NYdb::NTopic::TTopicClient::CommitOffset` method:
  ...
  If an active read session exists at the time of commit (for example, via `CreateReadSession`), it is recommended to pass its identifier using the `ReadSessionId` option in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:
...
- Go
  ...
  Commit processing outside the reader is performed using the `db.Topic().CommitOffset` method:
  ...
  If an active read session exists at the time of commit (via `StartReader` or `StartListener`), it is recommended to pass its identifier using the `WithCommitOffsetReadSessionID` option. This allows the server not to interrupt the current read session:
...
- Python
  ...
  Commit processing outside the reader is performed using the `topic_client.commit_offset` method:
