...
- Java
  ...
  This allows automatically closing the client and transport when exiting the block, since both are descendants of `AutoCloseable`.
- C#
...
- C++
  ...
  When modifying a topic, you need to specify the topic path and the parameters to be changed in the `AlterTopic` method parameters. The modifiable parameters are represented by the `TAlterTopicSettings` structure.
...
- C++
  ...
  To get topic information, use the `DescribeTopic` method.
  ...
  There is a separate method for getting consumer information - `DescribeConsumer`.
- Go
...
- C++
  ...
  - `IWriteSession` is a low-level write session to a single partition with a full set of features: event loop (`TReadyToAcceptEvent`, `TAcksEvent`, `TSessionClosedEvent`), explicit back-pressure via `TContinuationToken`, per-message acknowledgments, sending pre-compressed data via `WriteEncoded`. The `TWriteSessionSettings` structure is defined here — the other two options reuse it. Suitable when you need the status of each message, custom asynchronous logic, or manual compression control.
  - `ISimpleBlockingWriteSession` is a synchronous fire-and-forget API for writing to a single topic partition, the simplest option. The `Write(message, blockTimeout)` method puts the message into an internal buffer; sending to the server happens in the background. In normal mode, the call returns immediately and blocks only when the buffer is full (by `MaxMemoryUsage` / `MaxInflightCount`), for no longer than `blockTimeout`. A return of `false` means the message **did not enter the buffer** and is lost. There are no per-message acknowledgments; you can only verify that the entire buffer has been delivered to the server by calling `Close()` — it waits for an ack from the server. Suitable when an "all or nothing" guarantee by session close is sufficient and you need simple synchronous code.
  - `IProducer` is a high-level API on top of multiple write sessions: it transparently shards messages across topic partitions by key. Inspired by the Apache Kafka Producer interface, but takes into account the specifics of {{ ydb-short-name }} and, when working with topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning), provides full ordering and exactly-once guarantees. Server acknowledgments are available via the `AcksHandler` handler; to wait for delivery of the accumulated buffer — `Flush()`, to shut down correctly — `Close()`. Suitable when you need to write to a **multi-partition** topic with key-based routing.

  {% list tabs %}

  - IWriteSession

    `IWriteSession` is the base write session from which other write options inherit settings. The write session settings are represented by the `TWriteSessionSettings` structure; for the `ISimpleBlockingWriteSession` option, some settings are not supported.
  ...
  - IProducer
    ...
    - `PartitioningKeyHasher` — key transformation function before matching with ranges; used only for the `Bound` strategy. You can set your own, for example, so that the original key without hashing is used in the comparison.
...
- C++
  ...
  - IWriteSession
    ...
    By default, `Write` is executed asynchronously — data from messages is read and saved to an internal buffer, and sending occurs in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session itself reconnects to {{ ydb-short-name }} on connection breaks and retries sending messages as long as possible, in accordance with the `RetryPolicy` setting. Upon receiving an error after which it is impossible to continue operation, the write session sends `TSessionClosedEvent` with diagnostic information to the user.
  ...
  - IProducer
    ...
    `TProducerSettings` inherits `TWriteSessionSettings`, so buffering, sending, and reconnection work the same as in `IWriteSession`: `Write` puts a message into an internal buffer, sending to the server happens in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The producer reconnects to {{ ydb-short-name }} upon connection breaks and retries sending as long as possible, according to `RetryPolicy`. On a fatal error, the producer closes; the status and reason can be obtained from the result of `Write` or `Flush`.

    `Flush` waits for delivery of accumulated data to the server; `Close` waits for sending remaining messages in the buffer and terminates the producer.
...
- Go
  ...
  - `WithWriterPartitionByPartitionID` — write to the partition specified in the `PartitionID` field of the message. Does not combine with key routing; when a partition is split, the writer must be recreated manually.
...
- Python
  ...
  - Native SDK
    ...
    # Convenient to start, convenient to use as long as only the message content matters.
    writer = driver.topic_client.writer(topic_path)
    ...
    writer.write(["mess-1", "mess-2"])  # Здесь за один вызов отправляется несколько сообщений —
                                         ...
                                         # makes sense for a large message stream.
    ...
    # This makes sense for a large stream of sent messages — to reduce
...
- JavaScript
  ...
  // For immediate sending, call flush
  await writer.flush();
...
- C++
  ...
  - IWriteSession
    ...
    In such a write session, `TAcksEvent` events will not be delivered to the user in `GetEvent` / `GetEvents`; instead, when the SDK receives acknowledgments from the server, it will call the passed handler. Similarly, you can configure handlers for other event types.

  - ISimpleBlockingWriteSession

    Unlike `IWriteSession`, `ISimpleBlockingWriteSession` does not return acknowledgments for individual messages: `TAcksEvent` events and their handlers are not available. To wait until all messages from the buffer are written to the server, call `Close()` — the method waits for acknowledgment from the server and closes the session.

  - IProducer

    Acknowledgments from the server come in the same way as for `IWriteSession`: through the `AcksHandler` handler in `TProducerSettings::EventHandlers`. To wait for the accumulated buffer to be delivered to the server, call `Flush()`.
...
- Go
  ...
  When connecting, you can specify the synchronous message write option - topicoptions.WithSyncWrite(true). Then Write will return only after receiving acknowledgment from the server that all messages passed in the call have been saved. At the same time, the SDK will, as usual, reconnect and retry sending messages if necessary. In this mode, the context only controls the wait time for a response from the SDK, i.e., even after the context is canceled, the SDK will continue trying to send messages.
...
- Python
  ...
  There are two ways to get acknowledgment of message writing on the server:

  - `flush()` — waits for acknowledgment for all messages previously written to the internal buffer.
  - `write_with_ack(...)` — sends a message and waits for acknowledgment of its delivery from the server. When sending multiple messages in a row, this method works slowly.
  ...
  - Native SDK
    ...
    # Waiting when sending each message — this method returns a result only after receiving
...
- Java
  ...
  The `send` method returns `CompletableFuture<WriteAck>`. Its successful completion means the server has acknowledged the write.
...
- C#
  ...
  Asynchronous write of a message to the topic. In case of internal buffer overflow, it will wait for the buffer to free up for resending.
  ...
  If the server is unavailable, messages may accumulate in the queue waiting to be sent. To control the wait time, you can use a cancellation token (`CancellationToken`). However, with this approach, there is a risk that the user may cancel the sending of an already acknowledged message.
...
- JavaScript
  ...
  All messages are written to the internal buffer. There are 3 mechanisms for sending to the server: two automatic and one manual. Manual is calling the `writer.flush` method which returns the last seqno written on the server. Automatic sending occurs under the following conditions:

  - Exceeding the internal buffer size `maxBufferBytes` (default value = 256MiB).
  - By the periodic send interval tick `flushIntervalMs` (default value = 10ms).
...
For more details about writing without deduplication, see the [corresponding concepts section](../../concepts/datamodel/topic#no-dedup).
...
- C++
  ...
  - IProducer
    ...
    In `IProducer`, the transaction is specified not by the `Write` argument, but in `TWriteMessage` via `Tx()`. After that, the producer writes the message in the same way as a regular message with a partitioning key:

  {% endlist %}
- Go

  To write to a topic in a transaction, you need to create a transactional writer by calling [TopicClient.StartTransactionalWriter](https://pkg.go.dev/github.com/ydb-platform/ydb-go-sdk/v3/topic#Client.StartTransactionalWriter). After that, you can send messages as usual. There is no need to close the transactional writer — it happens automatically upon transaction completion.
...
- Python
  ...
  To write to a topic in a transaction, you need to create a transactional writer by calling `topic_client.tx_writer`. After that, you can send messages as usual. There is no need to close the transactional writer — it happens automatically upon transaction completion.

  In the example below, there is no explicit call to `tx.commit()` — it happens implicitly upon successful completion of the lambda `callee`.
...
- Java
  ...
  - Synchronous API
    ...
    In the settings of `SendSettings` method `send`, you can specify a transaction.
...
Reading messages from a topic can be performed with a Consumer associated with this topic, as well as without a Consumer. If a Consumer is not specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is provided in the [corresponding section](#no-consumer).

You can create a Consumer when [creating](#create-topic) or [changing](#alter-topic) a topic.
...
- C++
  ...
  To create a connection to an existing topic `my-topic` via a previously added reader `my-consumer`, use the following code:
...
- Java
  ...
  - Synchronous API
    ...
    - `initAndWait()`: blocking, starts the initialization process and waits for its completion. If an error occurs during initialization, an exception will be thrown.
  ...
  - Asynchronous API
    ...
    For an asynchronous reader, in addition to the common read settings `ReaderSettings`, you will need the event handler settings `ReadEventHandlersSettings`, in which you must pass an instance of a subclass of `ReadEventHandler`.
    It will describe how the processing of various events occurring during reading should happen.
    ...
    To implement an event handler, you can inherit from `AbstractReadEventHandler` and override method `onMessages`.
    Method `onMessages` is called each time the SDK receives the next batch of messages from the server. Within a single call, one or more messages arrive, which can be acknowledged (`commit`) either individually or after processing the entire batch. Example implementation:
...
You can also use an extended version of creating a connection to specify multiple topics and set read parameters. The following code will create a connection to topics `my-topic` and `my-specific-topic` via reader `my-consumer`:
...
- Go
  ...
  The example above also sets the time from which to start reading messages.
- Python
...
The server stores the [message read position](../../concepts/datamodel/topic.md#consumer-offset). After reading the next message, the client can [send a processing confirmation to the server](#commit). The read position will change, and on a new connection, only unacknowledged messages will be read.

Messages can also be read [without processing confirmation](#no-commit). In this case, on a new connection, all unacknowledged messages will be read, including those already processed.
...
- C++
  ...
  If a handler for a certain event is not set, you must obtain and process it in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` signature.
- Go
...
- C++
  ...
  When setting up a read session with the `SimpleDataHandlers` setting, you only need to pass a handler for data messages. The SDK will call this handler for each batch of messages received from the server. Read acknowledgments will not be sent by default.
...
Message processing acknowledgment (commit) tells the server that a message from the topic has been processed by the receiver and no longer needs to be sent. When using reading with acknowledgment, you must acknowledge all received messages without skipping. Message commit on the server occurs after acknowledging the next interval of messages 'without gaps'; the acknowledgments themselves can be sent in any order.

For example, messages 1, 2, 3 arrive from the server. The program processes them in parallel and sends acknowledgments in this order: 1, 3, 2. In this case, message 1 will be committed first, and messages 2 and 3 will be committed only after the server receives acknowledgment that message 2 has been processed.

If an error occurs on a message commit, you can log the error and continue working. The state of the message at this point is unknown. The message might have been committed, and then a network error occurred and the client did not receive the acknowledgment. If the message was not committed, it will be read again and will be processed again (possibly by a different reader). There is no point in retrying the commit itself, because the read session for this message is already lost.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, you must explicitly close the reader using the `Reader.Close()` call.
- Python

  `commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, you must explicitly close the reader.
...
- Java
  ...
  This method returns `CompletableFuture<Void>`; its successful completion means the server has confirmed processing.
...
- Python
  ...
  `commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending occurs later. Therefore, to avoid losing the latest commits before exiting the program, the reader must be explicitly closed.
- Java
  ...
  - Synchronous API
    ...
    Not relevant, because the synchronous reader cannot read messages in batches.
...
Instead of committing messages to the server, you can store the read progress yourself. In this case, you need to pass a handler to the SDK that will be called when reading each partition starts. In this handler, you need to specify the position from which to start reading this partition.

{% list tabs group=lang %}

- C++

  When processing events `TStartPartitionSessionEvent`, you can set the position from which to start reading when responding to the server. To do this, the `Confirm` method should receive the `readOffset` parameter. Additionally, you can pass the `commitOffset` parameter, which will specify the position up to which messages should be considered [committed](#commit).
...
- Go
  ...
  In the default reader mode, offsets up to the position specified via `res.StartFrom` are committed on the server. After that, re-reading the same messages by moving the position backward becomes impossible. To disable auto-commit, use the no-commit mode when creating the reader.
...
- Java
  ...
  Reading from a specified offset in Java is only possible in the asynchronous reader.
  ...
  You can also configure the `setReadFrom` reader to read events with write timestamps no less than the specified one.
- JavaScript
...
Usually, the reading progress of a topic is saved on the server in each `Consumer`. However, you can choose not to store such progress on the server and explicitly specify when creating a reader that reading will occur without `Consumer`.
...
- C++
  ...
  When reconnecting, the read progress is not saved on the server. To avoid starting from the beginning, pass the offset in `TStartPartitionSessionEvent::Confirm` each time you start a partition read session — see [storing the position on the client](#client-commit).
- Go
...
- Java
  ...
  In this case, you need to consider that when the connection is re-established, the progress on the server will be reset. Therefore, to avoid starting reading from the beginning, you should pass the read start offset in the SDK each time a partition read session starts:
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to the transaction object in the settings for receiving events from the session.
  ...
  NYdb::NStatusHelpers::ThrowOnError(queryClient.RetryQuerySync([&readSession](NYdb::NQuery::TSession session) -> NYdb::TStatus {
      ...
      for (auto& event : events) {
          // process the event and write the results to a table
      }
  ...
  You must confirm processing of event `TStopPartitionSessionEvent` after calling `Commit`.
...
- Java
  ...
  - Synchronous API
    ...
    Then the received message will be committed together with the transaction. It does not need to be committed separately.
  ...
  - Asynchronous API
    ...
    After receiving a message in the `onMessages` handler, you can associate one or more messages with a transaction. To do this, call a separate method `reader.updateOffsetsInTransaction` and wait for its execution on the server. This method takes a list of offsets as a parameter. For convenience, `Message` and `DataReceivedEvent` have a method `getPartitionOffsets()` that returns such a list.
...
In the case of a *hard interrupt*, the client receives a notification that it can no longer work with the partition's messages. The client must stop processing the read messages. Unacknowledged messages will be passed to another reader.
...
- C++
  ...
  A soft interrupt comes as an event `TStopPartitionSessionEvent` with the method `Confirm`. The client can finish processing the messages and send an acknowledgment to the server.
...
- Go
  ...
  The client code immediately receives all messages available in the buffer (on the SDK side), even if there are not enough to form a batch for group processing.
...
- Java
  ...
  - Synchronous API
    ...
    Not applicable, because the synchronous reader does not have the ability to configure handling of such events.
    The client will immediately respond to the server with a stop acknowledgment.

  - Asynchronous API

    To be able to react to such an event, override the method `onStopPartitionSession(StopPartitionSessionEvent event)` in the inheritor object `ReadEventHandler` (see [Connecting to a topic for reading messages](#start-reader)).
    `event.confirm()` must be called, because the server expects this response to continue the stop.
...
- C++
  ...
  A hard interrupt comes as an event `TPartitionSessionClosedEvent` either in response to a soft interrupt acknowledgment, or when the connection to the partition is lost. You can find out the reason by calling the method `GetReason`.
...
- Python
  ...
  In this example, message processing in a batch will stop if the partition is taken away during operation. This optimization requires additional code on the client. In simple cases, when processing taken-away partitions is not a problem, it can be omitted.
...
- C++
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The reading mode is set in the parameters of creating a read session. By default, compatibility mode is used.
  ...
  In full support mode, when all messages from a partition have been read, an event `TEndPartitionSessionEvent` arrives. After receiving this event, no new messages will appear in the partition for reading. To continue reading from child partitions, you must call `Confirm()`, thereby confirming that the application is ready to receive messages from child partitions. If messages from all partitions are processed in a single thread, then `Confirm()` can be called immediately after receiving `TEndPartitionSessionEvent`. If message processing from different partitions is done in different threads, then you should finish processing the messages, for example, execute the accumulated batch, acknowledge their processing (commit), or save the read position in your own database, and only then call `Confirm()`.

  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always immediately acknowledge their processing (commit). This will balance the reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  In compatibility mode, there is no explicit signal that reading from a partition is complete, and the server will try to heuristically determine that the client has processed the partition to the end. This can lead to a delay between finishing reading from the original partition and starting reading from its child partitions.

  If the client acknowledges message processing (commits), the signal to complete processing messages from a partition will be the acknowledgment of the last message of that partition. If the client does not acknowledge message processing, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to verify the correct handling of a soft read interrupt: the client must process the received messages, acknowledge their processing (commit) or save the read position in its database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set by the `topicoptions.WithReaderSupportSplitMergePartitions` option when creating the reader. By default, full support mode (`true`) is used.
  ...
  // compatibility mode (auto-scaling handling on server)
  reader, err := db.Topic().StartReader(
    "consumer",
    topicoptions.ReadTopic("topic"),
...
- Python
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set by the `auto_partitioning_support` argument when creating the reader. By default, full support mode is used.
...
Most often, it is convenient to acknowledge processing within the reader that receives messages. However, there are scenarios where acknowledgment must be performed by a process different from the reading process. In such cases, an acknowledgment method outside the reader is needed.

{% list tabs group=lang %}

- C++

  Acknowledging processing outside the read session is done using the `NYdb::NTopic::TTopicClient::CommitOffset` method:
  ...
  If there is an active read session at the time of acknowledgment (for example, via `CreateReadSession`), it is recommended to pass its identifier using the `ReadSessionId` option in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:
...
- Go
  ...
  Acknowledging processing outside the reader is done using the `db.Topic().CommitOffset` method:
  ...
  If there is an active read session at the time of acknowledgment (via `StartReader` or `StartListener`), it is recommended to pass its identifier using the `WithCommitOffsetReadSessionID` option. This allows the server not to interrupt the current read session:
...
- Python
  ...
  Acknowledging processing outside the reader is done using the `topic_client.commit_offset` method:
