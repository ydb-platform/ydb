...
- Go
  ...
  To work with topics, use an instance of the {{ ydb-short-name }} driver created using `ydb.Open`. The topic client is available via the `db.Topic()` method.
...
- C++
  ...
  When changing a topic, in the parameters of the `AlterTopic` method you need to specify the topic path and the parameters that will be changed. The parameters to be changed are represented by the `TAlterTopicSettings` structure.
  ...
  An example of adding an [important consumer](../../concepts/datamodel/topic#important-consumer) to a topic and setting the [message retention period](../../concepts/datamodel/topic#retention-time) for the topic to two days:
...
- Go
  ...
  When changing a topic, in the parameters you need to specify the topic path and the parameters that will be changed.
  ...
  An example of adding a consumer to a topic
...
- Java
  ...
  When changing a topic, in the parameters of the `alterTopic` method you need to specify the topic path and the parameters that will be changed.
...
- C++
  ...
  - IProducer
    ...
    `IProducer` is a high-level API over write sessions: a single object hides the management of multiple sessions and automatically selects a partition based on the message key. The producer settings are represented by the `TProducerSettings` structure, which inherits `TWriteSessionSettings`, so the common write settings match `IWriteSession`.
    ...
    - `PartitioningKeyHasher` — key conversion function before matching against ranges; used only for the `Bound` strategy. You can set your own, for example, to use the original key without hashing in the comparison.
...
- C++
  ...
  - IProducer
    ...
    `TProducerSettings` inherits from `TWriteSessionSettings`, so buffering, sending, and reconnection work the same as in `IWriteSession`: `Write` puts a message into an internal buffer, sending to the server happens in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The producer reconnects to {{ ydb-short-name }} on connection breaks and retries sending as long as possible, in accordance with `RetryPolicy`. On an unrecoverable error, the producer closes; the status and reason can be obtained from the result of `Write` or `Flush`.

    `Flush` waits for the accumulated data to be delivered to the server; `Close` waits for the remaining buffered messages to be sent and terminates the producer.
...
- Go
  ...
  To send a message, it is enough to store a Reader in the Data field from which the data can be read. You can expect that the data of each message is read once (or until the first error); by the time Write returns, the data will have been read and stored in the internal buffer.
  ...
  See a detailed example with key routing, alternative strategies (`KafkaHash` and `PartitionID`), and the transactional variant in the [ydb-go-sdk repository](https://github.com/ydb-platform/ydb-go-sdk/blob/master/examples/topic/topicwriter/topicwriter_to_many_partitions.go).
- Python
  ...
  - Native SDK
    ...
    # Convenient to start with, convenient to use as long as only the message content matters.
    writer = driver.topic_client.writer(topic_path)
    ...
    writer.write(["mess-1", "mess-2"])  # Здесь за один вызов отправляется несколько сообщений —
                                         ...
                                         # makes sense for a large flow of messages.
...
### Writing messages with confirmation of server storage
...
- C++
  ...
  - IWriteSession
    ...
    In such a write session, `TAcksEvent` events will not be delivered to the user via `GetEvent` / `GetEvents`; instead, when receiving confirmations from the server, the SDK will call the passed handler. Similarly, you can configure handlers for other event types.
...
- Python
  ...
  There are two ways to get confirmation of message writes on the server:
...
- Go
  ...
  In **ydb-go-sdk**, when creating a writer, if `topicoptions.WithWriterProducerID` is not passed, the SDK still substitutes the producer ID (generates it automatically). The write mode without deduplication, equivalent to the absence of `ProducerId` in the C++ example above, is not available in the current SDK version.
- Java
...
- C++
  ...
  - IProducer
    ...
    As with write sessions, metadata is set in `TWriteMessage` via `MessageMeta()`. The difference with `IProducer` is that the message also contains a partitioning key, by which the producer selects a partition:
...
- Java
  ...
  When constructing a message for writing using the Builder, you can pass it objects of type `MetadataItem` with a key of type `String` and a value of type `byte[]`.
...
- Java
  ...
  - Synchronous API
    ...
    In the settings `SendSettings` of the `send` method, you can specify a transaction.
...
Reading messages from a topic can be performed with a Consumer associated with this topic, as well as without a Consumer. If a Consumer is not specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is discussed in the [corresponding section](#no-consumer).
...
- Java
  ...
  - Asynchronous API
    ...
    For an asynchronous reader, in addition to the general read settings `ReaderSettings`, you will need the event handler settings `ReadEventHandlersSettings`, in which you need to pass an instance of a `ReadEventHandler` descendant.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data in an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, you need to explicitly close the reader by calling `Reader.Close()`.
- Python
...
- Java
  ...
  This method returns `CompletableFuture<Void>`, whose successful execution means that the server has acknowledged the processing.
  If a commit error occurs, do not attempt to retry it.
  Most likely, the error is caused by the session being closed.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing recent commits before exiting the program, the reader must be closed explicitly.
- Python
...
- C++
  ...
  To do this, you should pass the parameter `readOffset` to the method `Confirm`.
  ...
  Also, `TReadSessionSettings` supports the `ReadFromTimestamp` setting for reading events with write timestamps not less than a given value. This setting is intended not for precise start positioning, but for skipping the amount of data over a large time interval. The first few received messages may have write timestamps less than the specified one.
- Go

  {% note tip %}

  In the default reader mode, offsets up to the position specified via `res.StartFrom` are acknowledged on the server. After that, re-reading the same messages by moving the position backward becomes impossible. To disable automatic acknowledgment, use the no-commit mode when creating a reader.
...
- Java
  ...
  Reading from a given offset in Java is only possible in the asynchronous reader. In the `StartPartitionSessionEvent` event handler, you can set the position to start reading from when responding to the server. To do this, pass the `StartPartitionSessionSettings` settings with the specified offset via `setReadOffset` to the `confirm` method. Also, by calling `setCommitOffset`, you can specify the offset that should be considered committed.
...
Typically, the topic read progress is saved on the server in each `Consumer`. However, you can choose not to store such progress on the server and explicitly specify when creating a reader that reading will occur without `Consumer`.
...
- C++
  ...
  On reconnection, the read progress is not saved on the server. To avoid starting from the beginning, pass the offset in `TStartPartitionSessionEvent::Confirm` at each start of a partition read session — see [storing the position on the client](#client-commit).
- Go

  You need to pass an empty string as the consumer name and the `topicoptions.WithReaderWithoutConsumer(false)` option (**experimental** mode, see [VERSIONING](https://github.com/ydb-platform/ydb-go-sdk/blob/master/VERSIONING.md) in the SDK repository). In the read selector, specify the topic path and the list of partitions. Message commits are not available in this mode (`CommitModeNone`); on reconnections, the progress must be restored on the client side — see [storing the position on the client](#client-commit).
...
- Java
  ...
  In this case, note that when the connection is re-established, the progress on the server will be reset. Therefore, to avoid starting reading from the beginning, you should pass the reading start offset in the SDK at each start of a partition read session:
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to the transaction object to the session event receiving settings.
  ...
  When processing `events` events, you do not need to explicitly commit processing for events of type `TDataReceivedEvent`.

  {% endnote %}

  Commit processing of the `TStopPartitionSessionEvent` event must be done after calling `Commit`.
...
- Java
  ...
  - Asynchronous API
    ...
    After receiving a message in the `onMessages` handler, you can link one or more messages with a transaction.
...
In case of a *hard interruption*, the client receives a notification that it can no longer work with partition messages. The client must stop processing the read messages. Unacknowledged messages will be transferred to another reader.
...
- C++
  ...
  A soft interrupt comes as an `TStopPartitionSessionEvent` event with an `Confirm` method. The client can finish processing messages and send an acknowledgment to the server.
...
- C++
  ...
  A hard interrupt comes as an `TPartitionSessionClosedEvent` event either in response to acknowledgment of a soft interrupt or when the connection to a partition is lost. You can find out the reason by calling the `GetReason` method.
...
- Python
  ...
  In this example, batch message processing will stop if a partition is revoked during operation. This optimization requires additional code on the client side. In simple cases where processing revoked partitions is not an issue, it can be omitted.
...
### Topic auto-scaling support {#autoscaling}
...
- C++
  ...
  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always immediately commit their processing. This will balance reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  In compatibility mode, there is no explicit signal about the end of reading from a partition, and the server will try to heuristically determine that the client has processed the partition to the end. This may lead to a delay between finishing reading from the original partition and starting reading from its child partitions.

  If the client commits message processing, then the signal for completion of message processing from a partition will be the commit of the last message of that partition. If the client does not commit message processing, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to check the correctness of handling a soft read interruption: the client must process the received messages, commit them or save the read position in its own database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set by the `topicoptions.WithReaderSupportSplitMergePartitions` option when creating a reader. By default, full support mode (`true`) is used.
...
- Python
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set by the `auto_partitioning_support` argument when creating a reader. By default, full support mode is used.
  ...
  From a practical point of view, the modes do not differ for the end user. Full support mode differs from compatibility mode in who guarantees the read order — the client or the server. Compatibility mode is achieved by server-side processing and, as a rule, works slower.
- JavaScript
...
Most often, it is convenient to perform commit processing within the reader that receives messages. However, there are scenarios where commit processing must be performed by a process different from the reading process. In such a case, a commit method outside the reader is needed.

{% list tabs group=lang %}

- C++

  Commit processing outside the read session is performed using the `NYdb::NTopic::TTopicClient::CommitOffset` method:
  ...
  If there is an active read session at the time of confirmation (for example, via `CreateReadSession`), it is recommended to pass its identifier using the `ReadSessionId` option in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:


  ```cpp
  // Getting the read session ID
...
- Go
  ...
  Confirmation of processing outside the reader is performed using the `db.Topic().CommitOffset` method:


  ```go
  // Basic method — offset commit without an active read session
...
- Python
  ...
  Confirmation of processing outside the reader is performed using the `topic_client.commit_offset` method:
...
- Java
  ...
  // For AsyncReader, the session ID can be obtained when processing the SessionStartedEvent event
