...
- C++
  ...
  When changing a topic, specify the topic path and the parameters to be changed in the `AlterTopic` method parameters. The parameters to be changed are represented by the `TAlterTopicSettings` structure.
...
- Go
  ...
  When changing a topic, specify the topic path and the parameters to be changed in the parameters.
...
- Java
  ...
  When changing a topic, specify the topic path and the parameters to be changed in the `alterTopic` method parameters.
...
- C++
  ...
  - IWriteSession
    ...
    See the [header file](https://github.com/ydb-platform/ydb/blob/main/ydb/public/sdk/cpp/include/ydb-cpp-sdk/client/topic/write_session.h#L56) for the full list of settings.
  ...
  - IProducer
    ...
    `IProducer` is a high-level API over write sessions: a single object hides the management of multiple sessions and automatically selects a partition by message key. Producer settings are represented by the `TProducerSettings` structure, which inherits `TWriteSessionSettings`, so the common write settings are the same as `IWriteSession`.
    ...
    - `PartitionChooserStrategy` — partition selection strategy by message key:
      ...
      - `Bound` — the key is matched against the topic partition ranges (`FromBound`/`ToBound` from the topic description). By default, the key passes through MurmurHash64 before matching. Recommended for topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning): when a partition splits, the SDK updates the boundaries and continues routing messages with the same key to the correct range.
      - `KafkaHash` — by analogy with Kafka: MurmurHash is computed from the key, the partition index is the remainder of dividing the hash by the number of partitions. Convenient when migrating from Kafka. Not supported when auto-partitioning is enabled.
    - `PartitioningKeyHasher` — key transformation function before matching with ranges; used only for the `Bound` strategy. You can set your own, for example, so that the original key without hashing participates in the comparison.

    See the [header file](https://github.com/ydb-platform/ydb/blob/main/ydb/public/sdk/cpp/include/ydb-cpp-sdk/client/topic/producer.h#L10) for the full list of settings.
...
- C++
  ...
  - IWriteSession
    ...
    By default, `Write` is executed asynchronously: data from messages is read and stored in an internal buffer, and sending occurs in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session itself reconnects to {{ ydb-short-name }} when connections are broken and retries sending messages as long as possible, according to the setting `RetryPolicy`. When an error is received after which it is impossible to continue operation, the write session sends the user `TSessionClosedEvent` with diagnostic information.
...
- Go
  ...
  To send a message, it is enough to store a Reader in the Data field from which data can be read. You can rely on the fact that the data of each message is read once (or until the first error); by the time Write returns, the data will have been read and saved to the internal buffer.
  ...
  - `BoundPartitionChooser` — the key is matched against the topic partition ranges (`FromBound`/`ToBound`). By default, before matching, the key goes through MurmurHash64. Recommended for topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning): the SDK updates the boundaries when partitions split.
...
- Python
  ...
  - Native SDK
    ...
    writer.write(["mess-1", "mess-2"])  # Здесь за один вызов отправляется несколько сообщений —
                                         ...
                                         # makes sense with a high message throughput.
    ...
    # This makes sense with a high outgoing message throughput — to reduce
...
- C++
  ...
  - IWriteSession
    ...
    In such a write session, `TAcksEvent` events will not be delivered to the user via `GetEvent` or `GetEvents`; instead, when receiving confirmations from the server, the SDK will call the passed handler. Similarly, handlers for other event types can be configured.
  ...
  - IProducer
    ...
    Confirmations from the server arrive in the same way as for `IWriteSession`: via the `AcksHandler` handler in `TProducerSettings::EventHandlers`. To wait for the accumulated buffer to be delivered to the server, call `Flush()`.
...
- Python
  ...
  There are two ways to get confirmation of message writing on the server:
  ...
  - Native SDK
    ...
    # Wait on each message send — this method returns only after receiving
...
- C#
  ...
  Asynchronous writing of a message to a topic. In case of internal buffer overflow, it will wait until the buffer is freed for retransmission.
  ...
  If the server is unavailable, messages may accumulate in a queue waiting to be sent. To control the waiting time, you can use a cancellation token (`CancellationToken`). However, with this approach, there is a risk that the user may cancel the sending of an already confirmed message.
...
- C++
  ...
  - IWriteSession
    ...
    If you need to send a message compressed with a different codec within a write session, you can use the `WriteEncoded` method specifying the codec and the size of the uncompressed message. For successful writing using this method, the used codec must be allowed in the topic settings.
...
- Go
  ...
  By default, the SDK selects a codec automatically (taking into account topic settings). In automatic mode, the SDK first sends one message group with each of the allowed codecs, then will occasionally try to compress messages with all available codecs and select the codec that gives the smallest message size. If the list of allowed codecs for the topic is empty, auto-selection is performed between Raw and Gzip codecs.

  If necessary, you can set a fixed codec in the connection options. Then that one will be used and measurements will not be performed.
...
- Java
  ...
  When constructing a message for writing using a Builder, you can pass it objects of type `MetadataItem` with a pair of key type `String` + value type `byte[]`.

  You can pass multiple `List` such objects:
  ...
  When reading, these message metadata can be obtained by calling its `getMetadataItems()` method:
...
- Python
  ...
  To use the metadata passing function, create an `TopicWriterMessage` object with the `metadata_items` argument, as shown below:
...
- Rust
  ...
  Track progress or vote for support in Rust SDK: [ydb-rs-sdk#341](https://github.com/ydb-platform/ydb-rs-sdk/issues/341)
- PHP
...
Reading messages from a topic can be performed with or without specifying a Consumer associated with this topic. If a Consumer is not specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is provided in the [corresponding section](#no-consumer).
...
You can also use the advanced connection creation option to specify multiple topics and set read parameters. The following code will create a connection to topics `my-topic` and `my-specific-topic` via reader `my-consumer`:
...
- Rust
  ...
  // processing batch.messages and transaction commit
  ```
...
Message processing acknowledgment (commit) informs the server that the message from the topic has been processed by the receiver and no longer needs to be sent. When using reading with acknowledgment, you must acknowledge all received messages without skipping. Message commit on the server occurs after acknowledging the next interval of messages 'without gaps'; the acknowledgments themselves can be sent in any order.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the last commits before exiting the program, the reader must be explicitly closed using the `Reader.Close()` call.
- Python
...
- Python
  ...
  `commit` is a fast call: saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the latest commits before exiting the program, the reader must be explicitly closed.
- Java
  ...
  - Synchronous API
    ...
    Irrelevant, because the synchronous reader does not support reading messages in batches.
...
Instead of committing messages to the server, you can store the read progress yourself. In this case, you need to pass a handler to the SDK that will be called when reading each partition starts. In this handler, you will need to specify the position from which to start reading this partition.
...
- Go
  ...
  In the default reader mode, offsets up to the position specified via `res.StartFrom` are committed on the server. After that, re-reading the same messages by moving the position back becomes impossible. To disable automatic commit, use the no-commit mode when creating a reader.
...
- Java
  ...
  Reading from a given offset in Java is only possible in an asynchronous reader.
  In the event handler `StartPartitionSessionEvent`, you can specify the position from which to start reading when responding to the server.
  To do this, pass settings `StartPartitionSessionSettings` with the specified offset via `setReadOffset` to the method `confirm`.
  ...
  Also, configuration of the `setReadFrom` reader is supported for reading events with write timestamps not less than the specified one.
- JavaScript
...
Typically, topic read progress is stored on the server in each `Consumer`. However, you can choose not to store such progress on the server and explicitly specify when creating a reader that reading will occur without `Consumer`.
...
- Java
  ...
  In this case, note that upon reconnection, the progress on the server will be reset. Therefore, to avoid starting reading from the beginning, you should pass the starting read offset in the SDK at each start of a partition read session:
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to a transaction object into the session event receiving settings.
...
During a *soft interrupt*, the client receives a notification that the server has finished sending messages from the partition and no more messages will be read. The client can finish processing the messages and send an acknowledgment to the server.
...
- C++
  ...
  A soft interrupt arrives as an event `TStopPartitionSessionEvent` with a method `Confirm`. The client can finish processing messages and send an acknowledgment to the server.
...
- Go
  ...
  Client code immediately receives all messages available in the buffer (on the SDK side), even if there are not enough to form a batch during batch processing.
...
- Java
  ...
  - Asynchronous API
    ...
    To be able to respond to such an event, you should override the `onStopPartitionSession(StopPartitionSessionEvent event)` method in the `ReadEventHandler` descendant object (see [Connecting to a topic to read messages](#start-reader)).
...
- Rust
  ...
  The Rust SDK handles stop and close partition session events internally; a public API for configuring soft or hard read interruption is not yet available.
...
- C++
  ...
  A hard interrupt arrives as an `TPartitionSessionClosedEvent` event, either in response to a soft interrupt acknowledgment or when the connection to a partition is lost. You can find out the reason by calling the `GetReason` method.
...
- Go
  ...
  If reading is interrupted, the context of the message or message batch will be canceled.
...
- Python
  ...
  In this example, batch message processing will stop if the partition is reassigned during operation. This optimization requires additional code on the client side. In simple cases where processing reassigned partitions is not a problem, you can omit it.
...
### Support for auto-scaling of topics {#autoscaling}

{% list tabs group=lang %}

- C++

  SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set in the parameters of creating a read session. By default, compatibility mode is used.
  ...
  In full support mode, when all messages from a partition have been read, the `TEndPartitionSessionEvent` event will arrive. After receiving this event, no new messages will appear in the partition for reading. To continue reading from child partitions, you must call `Confirm()`, thereby confirming that the application is ready to accept messages from child partitions. If messages from all partitions are processed in a single thread, then `Confirm()` can be called immediately after receiving `TEndPartitionSessionEvent`. If messages from different partitions are processed in different threads, you should finish processing the messages, for example, execute the accumulated batch, commit their processing, or save the read position in your own database, and only then call `Confirm()`.

  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always immediately commit their processing. This will balance the reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  If the client commits message processing, the signal that processing messages from a partition is complete will be the commit of the last message of that partition. If the client does not commit message processing, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to verify the correct handling of a soft read interruption: the client must process the received messages, commit them, or save the read position in its own database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set by the `topicoptions.WithReaderSupportSplitMergePartitions` option when creating the reader. By default, full support mode (`true`) is used.
...
- Python
  ...
  The SDK supports two modes for reading topics with autoscaling enabled: full support mode and compatibility mode. The read mode is set by the `auto_partitioning_support` argument when creating the reader. By default, full support mode is used.
...
### Commit processing outside the reader {#commit-outside-the-reader}

Most often, it is convenient to commit processing within the reader that receives messages. However, there are scenarios where commit processing must be performed by a process other than the reading process. In such a case, a commit method outside the reader is needed.

{% list tabs group=lang %}

- C++

  Commit processing outside the read session is done using the `NYdb::NTopic::TTopicClient::CommitOffset` method:
  ...
  If there is an active read session at the time of acknowledgment (for example, via `CreateReadSession`), it is recommended to pass its ID using the `ReadSessionId` option in `NYdb::NTopic::TCommitOffsetSettings`. This allows the server not to interrupt the current read session:
...
- Go
  ...
  If there is an active read session at the time of acknowledgment (via `StartReader` or `StartListener`), it is recommended to pass its ID using the `WithCommitOffsetReadSessionID` option. This allows the server not to interrupt the current read session:
