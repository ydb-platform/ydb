...
- Go
  ...
  To work with topics, use an instance of the {{ ydb-short-name }} driver created with `ydb.Open`. The topics client is available via the `db.Topic()` method.
...
- Java
  ...
  Both code examples above use the ( [try-with-resources](https://docs.oracle.com/javase/tutorial/essential/exceptions/tryResourceClose.html)) block.
...
- Python
  ...
  To work with topics, create an instance of the {{ ydb-short-name }} driver. The topics client is available via the `topic_client` attribute and is used for management operations with topics, as well as creating writers and readers.
...
- Go
  ...
  When modifying a topic, in the parameters you need to specify the topic path and the parameters that will be changed.
...
- Java
  ...
  When modifying a topic, in the parameters of the `alterTopic` method you need to specify the topic path and the parameters to be changed.
...
Currently, only connections with matching identifiers of the [source and message group](../../concepts/datamodel/topic#producer-id) (`producer_id` and `message_group_id`) are supported; this restriction will be lifted in the future.

{% list tabs group=lang %}

- C++

  A write connection to a topic is represented by a write session object with the `IWriteSession` or `ISimpleBlockingWriteSession` interface (a variant for simple one-message writes without acknowledgment, which blocks when the number of inflight writes or the SDK buffer size is exceeded). Write session settings are represented by the `TWriteSessionSettings` structure; for the `ISimpleBlockingWriteSession` variant, some settings are not supported.
...
- C++
  ...
  The user's interaction with the `IWriteSession` object is generally structured as event loop processing with three event types: `TReadyToAcceptEvent`, `TAcksEvent`, and `TSessionClosedEvent`.
  ...
  If a handler for a certain event is not set, you need to retrieve and process it in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` interface.
  ...
  By default, `Write` is executed asynchronously: data from messages is read and stored in an internal buffer, and sending occurs in the background according to settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session itself reconnects to {{ ydb-short-name }} when the connection is lost and retries sending messages as long as possible, according to the `RetryPolicy` setting. When an error that prevents further operation is received, the reading session sends `TSessionClosedEvent` with diagnostic information to the user.

  This is what writing multiple messages in an event loop without handlers may look like:
...
- Go
  ...
  To send a message, it is enough to store a Reader in the Data field, from which the data can be read. You can rely on the fact that the data of each message is read once (or until the first error); by the time Write returns, the data will already be read and saved to the internal buffer.
...
- Python
  ...
  - Native SDK
    ...
    writer = driver.topic_client.writer(topic_path)
    writer.write("mess")  # Strings will be passed in utf-8 encoding, so it's convenient to send
                          # text messages.
    ...
    writer.write(["mess-1", "mess-2"])  # Here, multiple messages are sent in one call —
                                         ...
                                         # makes sense with a large flow of messages.
    ...
    # This makes sense with a large flow of sent messages — to reduce
...
- Java
  ...
  - Synchronous API
    ...
    If you need confirmation of successful write for each message, use the asynchronous writer and check the status returned by the `send` method.
...
- C#
  ...
  Asynchronous write of a message to a topic.
...
### Writing messages with confirmation of saving on the server

{% list tabs group=lang %}

- C++

  You can receive confirmations from the server via the `IWriteSession` interface.

  Responses about writing messages on the server come to the SDK client as `TAcksEvent` events. One event may contain responses about several previously sent messages. Response options: write confirmed (`EES_WRITTEN`), write discarded as a duplicate of a previously written message (`EES_ALREADY_WRITTEN`), or write discarded due to a failure (`EES_DISCARDED`).
...
- Go
  ...
  When connecting, you can specify the synchronous message write option: topicoptions.WithSyncWrite(true). Then Write will return only after receiving confirmation from the server that all messages passed in the call have been saved. In this case, the SDK, as usual, will reconnect and retry sending messages if necessary. In this mode, the context controls only the waiting time for a response from the SDK, i.e., even after the context is canceled, the SDK will continue trying to send messages.
...
- Python
  ...
  There are two ways to get confirmation that messages have been written on the server:

  - `flush()` — waits for acknowledgment for all messages previously written to the internal buffer.
  - `write_with_ack(...)` — sends a message and waits for the server to confirm its delivery. When sending multiple messages in a row, this method works slowly.
  ...
  - Native SDK
    ...
    # This is the slowest way to send messages, use it only if this mode
    # is really needed.
...
- Java
  ...
  The `WriteAck` structure contains information about the seqNo, offset, and write status:
...
- With#
  ...
  Asynchronously writes a message to the topic. If the internal buffer overflows, it will wait until the buffer frees up for resending.
...
- C++
  ...
  The compression used when sending messages with the `Write` method is set when [creating a write session](#start-writer) using the `Codec` and `CompressionLevel` settings. By default, the GZIP codec is selected.
...
- Go
  ...
  If necessary, you can set a fixed codec in the connection options. Then that codec will be used and no measurements will be taken.
...
- C++
  ...
  You can use the metadata writing function with the `Write()` method, which takes a `TWriteMessage` object:
...
- Python
  ...
  To use the metadata transfer function, create an object `TopicWriterMessage` with the argument `metadata_items`, as shown below:
  ...
  While reading, metadata can be obtained from the `metadata_items` field of the `PublicMessage` object:
...
- Go
  ...
  The example above also specifies the time from which messages should be read.
...
- C++
  ...
  The user's work with the `IReadSession` object is generally structured as event loop processing with the following event types: `TDataReceivedEvent`, `TCommitOffsetAcknowledgementEvent`, `TStartPartitionSessionEvent`, `TEndPartitionSessionEvent`, `TStopPartitionSessionEvent`, `TPartitionSessionStatusEvent`, `TPartitionSessionClosedEvent`, and `TSessionClosedEvent`.

  For each event type, you can set an event handler, and you can also set a common handler. Handlers are set in the write session settings before it is created.

  If a handler for an event is not set, you need to get and process it in the `GetEvent` or `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` signature.
...
#### Reading messages one at a time

{% list tabs group=lang %}

- C++

  Reading messages one at a time is not supported in the C++ SDK. The `TDataReceivedEvent` event contains a batch of messages.
...
- Java
  ...
  - Synchronous API
    ...
    To read messages one at a time without acknowledging processing, use the following code:
  ...
  - Asynchronous API
    ...
    The asynchronous client does not support reading messages one at a time.
...
If an error occurs when committing a message, you can log the error and continue working. The state of the message at this point is unknown. The message might have been committed, and then a network error occurred and the client did not receive the acknowledgment. If the message was not committed, it will be read again and processed again (possibly by another reader). There is no point in retrying the commit itself, because the read session for this message is already lost.

#### Reading messages one at a time with acknowledgment
...
- Java
  ...
  In the asynchronous reader, when processing a batch of messages, you can call `commit` either on the entire batch at once or on each message individually.
  This method returns `CompletableFuture<Void>`, whose successful completion means the server has acknowledged processing.
  If a commit error occurs, you should not try to retry it. Most likely, the error is caused by the session being closed.
...
- Go
  ...
  By default, `Commit` is a fast call: it saves data to an internal buffer and immediately returns control, while the actual sending happens later. Therefore, to avoid losing the latest commits before exiting the program, you need to close the reader explicitly.
...
- Go
  ...
  In the default reader mode, offsets up to the position specified via `res.StartFrom` are committed on the server. After that, re-reading the same messages by moving the position back becomes impossible. To disable automatic commit, use the no-commit mode when creating the reader.
...
- Java
  ...
  Reading from a specified offset in Java is only possible in an asynchronous reader.
  ...
  To do this, pass `StartPartitionSessionSettings` settings with the specified offset via `setReadOffset` to the `confirm` method.
  You can also use the `setCommitOffset` call to specify the offset to be considered committed.
  ...
  You can also configure the reader `setReadFrom` to read events with write timestamps not earlier than the specified one.
...
Usually, the topic reading progress is stored on the server in each `Consumer`. However, you can choose not to store such progress on the server and explicitly specify when creating a reader that reading will occur without `Consumer`.

{% list tabs group=lang %}

- Go

  You need to pass an empty string as the consumer name and the `topicoptions.WithReaderWithoutConsumer(false)` option (the mode is **experimental**, see [VERSIONING](https://github.com/ydb-platform/ydb-go-sdk/blob/master/VERSIONING.md) in the SDK repository). In the read selector, specify the topic path and the list of partitions. Message commits are not available in this mode (`CommitModeNone`); on reconnects, progress must be restored on the client side — see [client-side position storage](#client-commit).
...
- Java
  ...
  In this case, note that when the connection is re-established, the progress on the server will be reset. Therefore, to avoid starting the read from the beginning, you should pass the offset of the start of reading in the SDK each time a partition read session starts:
...
- Python
  ...
  - `topic` is an `ydb.TopicReaderSelector` object with the specified `path` and a list of `partitions`.
  - `consumer` must be `None`
  - `event_handler` is a subclass of `ydb.TopicReaderEvents.EventHandler`, which implements the function `on_partition_get_start_offset`. This function is responsible for returning the initial offset for reading messages when the reader starts and during reconnections. The client application must specify this offset in the `ydb.TopicReaderEvents.OnPartitionGetStartOffsetResponse.start_offset` parameter. The function can also be implemented asynchronously.
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to a transaction object to the settings for receiving events from the session.
  ...
  When processing `events` events, you don't need to explicitly acknowledge processing for events of type `TDataReceivedEvent`.

  {% endnote %}

  You should acknowledge the processing of event `TStopPartitionSessionEvent` after calling `Commit`.
...
- Java
  ...
  - Synchronous API
    ...
    Then the received message will be committed together with the transaction. There is no need to commit it separately.
  ...
  - Asynchronous API
    ...
    To do this, call the `reader.updateOffsetsInTransaction` method and wait for it to complete on the server.
...
In the case of a *hard interruption*, the client receives a notification that it can no longer work with the partition's messages. The client must stop processing the messages it has read. Unacknowledged messages will be passed to another reader.
...
- Go
  ...
  Client code immediately receives all messages available in the buffer (on the SDK side), even if there are not enough of them to form a batch during batch processing.
...
- Java
  ...
  - Synchronous API
    ...
    Outdated, because the synchronous reader does not allow configuring handling of such events.
    The client will immediately respond to the server with a stop confirmation.

  - Asynchronous API

    To be able to react to such an event, you should override the `onStopPartitionSession(StopPartitionSessionEvent event)` method in the `ReadEventHandler` inheritor object (see [Connecting to a topic for reading messages](#start-reader)).
...
- C++
  ...
  A fragment of the event loop may look like this:
...
- Python
  ...
  In this example, processing messages in a batch will stop if the partition is revoked during operation. This optimization requires additional client-side code. In simple cases, when processing revoked partitions is not a problem, you can skip it.
...
- C++
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set in the read session creation parameters. Compatibility mode is used by default.
  ...
  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always immediately acknowledge their processing (commit). This will balance reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  If the client acknowledges message processing (commit), the signal that processing messages from a partition is complete will be the acknowledgment of the last message of that partition. If the client does not acknowledge message processing, the server will periodically interrupt reading from the partition and switch to reading in another session (if there are other sessions ready to process the partition). This will continue until reading [starts](#client-commit) from the end of the partition.

  It is recommended to check the correctness of handling a soft read interrupt: the client must process the received messages, acknowledge their processing (commit), or save the read position in its own database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
...
- Go
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set with the `topicoptions.WithReaderSupportSplitMergePartitions` option when creating the reader. Full support mode (`true`) is used by default.
...
- Python
  ...
  The SDK supports two modes for reading topics with auto-scaling enabled: full support mode and compatibility mode. The read mode is set with the `auto_partitioning_support` argument when creating the reader. Full support mode is used by default.
...
### Acknowledging processing outside the reader {#commit-outside-the-reader}

Most often, it is convenient to acknowledge processing within the reader that receives messages. However, there are scenarios where acknowledgment must be performed by a process other than the reading process. In such cases, an acknowledgment mechanism outside the reader is required.
...
- Go
  ...
  ```go
  // Basic method — offset acknowledgement without an active read session
