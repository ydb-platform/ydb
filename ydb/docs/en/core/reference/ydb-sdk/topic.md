...
- Python
  ...
  To work with topics, create an instance of the {{ ydb-short-name }} driver. The topic client is available via the `topic_client` attribute and is used for management operations on topics, as well as creating writers and readers.
...
- C++
  ...
  You can view the full list of settings [in the header file](https://github.com/ydb-platform/ydb/blob/d2d07d368cd8ffd9458cc2e33798ee4ac86c733c/ydb/public/sdk/cpp/client/ydb_topic/topic.h#L394).
...
- Go
  ...
  You can view the full list of supported parameters in the [SDK documentation](https://pkg.go.dev/github.com/ydb-platform/ydb-go-sdk/v3/topic/topicoptions#CreateOption).
...
- Java
  ...
  You can view the full list of settings [in the SDK code](https://github.com/ydb-platform/ydb-java-sdk/blob/master/topic/src/main/java/tech/ydb/topic/settings/CreateTopicSettings.java#L97).
...
- C++
  ...
  In the C++ SDK, three API options are available for writing to a topic. The basic settings (buffering, codecs, retries) are the same for all three and are set by the `TWriteSessionSettings` structure, so the tips below focus on the differences and usage scenarios.
  ...
  - ISimpleBlockingWriteSession
    ...
    `ISimpleBlockingWriteSession` is a simple synchronous variant of the write session `IWriteSession` for writing one message at a time without acknowledgment per message. The `Write` method blocks when the number of inflight records or the SDK buffer size is exceeded. Write session settings are represented by the `TWriteSessionSettings` structure, as in the case of `IWriteSession`, but some settings are not supported.
  ...
  - IProducer
    ...
    - `ProducerIdPrefix` — prefix of the producer ID for write subsessions;
    - `PartitionChooserStrategy` — strategy for selecting a partition based on the message key:

      - `Bound` — the key is matched against the topic partition ranges (`FromBound`/`ToBound` from the topic description). By default, the key is passed through MurmurHash64 before matching. Recommended for topics with [auto-partitioning](../../concepts/datamodel/topic.md#autopartitioning): when a partition splits, the SDK updates the boundaries and continues to route messages with the same key to the correct range.
    ...
    - `PartitioningKeyHasher` — key transformation function before matching against ranges; used only for the `Bound` strategy. You can set your own, for example, to use the original key without hashing in the comparison.
...
- C++
  ...
  - IWriteSession
    ...
    If a handler for a certain event is not set, it must be retrieved and processed in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the `TFuture<void>()` interface.
    ...
    By default, `Write` is executed asynchronously — messages are read and saved to an internal buffer, and sending occurs in the background in accordance with the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The session automatically reconnects to {{ ydb-short-name }} upon connection breaks and retries sending messages as long as possible, in accordance with the setting `RetryPolicy`. Upon receiving an error after which it cannot continue operation, the write session sends the user `TSessionClosedEvent` with diagnostic information.

    Here is an example of writing multiple messages in an event loop without using handlers:
  ...
  - IProducer
    ...
    `TProducerSettings` inherits from `TWriteSessionSettings`, so buffering, sending, and reconnection work the same as in `IWriteSession`: `Write` puts a message into an internal buffer, sending to the server happens in the background according to the settings `MaxMemoryUsage`, `MaxInflightCount`, `BatchFlushInterval`, `BatchFlushSizeBytes`. The producer reconnects to {{ ydb-short-name }} on connection breaks and retries sending as long as possible, according to `RetryPolicy`. On an unrecoverable error, the producer closes; the status and reason can be obtained from the result of `Write` or `Flush`.
...
- Go
  ...
  - `WithWriterPartitionByPartitionID` — write to the partition specified in the `PartitionID` field of the message. Not combined with key-based routing; when a partition splits, the writer must be recreated manually.
...
### Writing messages with confirmation of server-side saving
...
- Java
  ...
  When constructing a message for writing using a Builder, you can pass objects of type `MetadataItem` with a key of type `String` and a value of type `byte[]`.
  ...
  When reading, these message metadata can be obtained by calling the `getMetadataItems()` method on it:
...
- Go
  ...
  To write to a topic in a transaction, you need to create a transactional writer by calling [TopicClient.StartTransactionalWriter](https://pkg.go.dev/github.com/ydb-platform/ydb-go-sdk/v3/topic#Client.StartTransactionalWriter). After that, you can send messages as usual. Closing the transactional writer is not required — it happens automatically when the transaction completes.
...
- Python
  ...
  To write to a topic in a transaction, you need to create a transactional writer by calling `topic_client.tx_writer`. After that, you can send messages as usual. Closing the transactional writer is not required — it happens automatically when the transaction completes.
...
Reading messages from a topic can be performed with a Consumer associated with this topic, or without a Consumer. If a Consumer is not specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is discussed in the [corresponding section](#no-consumer).

You can create a Consumer when [creating](#create-topic) or [changing](#alter-topic) a topic.
A topic can have multiple Consumers, and for each of them the server stores its own read progress.

{% list tabs group=lang %}

- C++

  A connection for reading from one or more topics is represented by a read session object with the interface `IReadSession`. The read session settings are represented by the structure `TReadSessionSettings`.
...
- Java
  ...
  - Asynchronous API
    ...
    For an asynchronous reader, in addition to the general read settings `ReaderSettings`, you will need event handler settings `ReadEventHandlersSettings`, in which you must pass an instance of a subclass `ReadEventHandler`.
...
You can also use an extended connection creation option to specify multiple topics and set read parameters. The following code creates a connection to topics `my-topic` and `my-specific-topic` via the reader `my-consumer`:
...
You can use [transactions](#read-tx). In this case, the read position changes upon transaction commit. On a new connection, all unacknowledged messages will be read.

{% list tabs group=lang %}

- C++

  The user's work with the `IReadSession` object is generally structured as processing an event loop with the following event types: `TDataReceivedEvent`, `TCommitOffsetAcknowledgementEvent`, `TStartPartitionSessionEvent`, `TEndPartitionSessionEvent`, `TStopPartitionSessionEvent`, `TPartitionSessionStatusEvent`, `TPartitionSessionClosedEvent`, and `TSessionClosedEvent`.

  For each event type, you can set a handler for that event, and you can also set a common handler. Handlers are set in the write session settings before creating it.

  If a handler for an event is not set, you need to get and process it in the `GetEvent` / `GetEvents` methods. For non-blocking waiting for the next event, there is the `WaitEvent` method with the signature `TFuture<void>()`.
- Go
...
- Java
  ...
  To confirm message processing, it is sufficient to call the `commit` method on the message.
...
#### Reading messages in batch with acknowledgment
...
- C++
  ...
  To do this, you should pass to the method `Confirm` the parameter `readOffset`.
...
- C++
  ...
  On reconnection, the read progress is not saved on the server. To avoid starting from the beginning, pass the offset in `TStartPartitionSessionEvent::Confirm` at each start of a partition read session — see [storing position on the client](#client-commit).
- Go
...
- C++
  ...
  Before reading from a topic, the client code must pass a reference to the transaction object in the session event receiving settings.
...
- Java
  ...
  - Synchronous API
    ...
    The `receive` method will associate the message offsets with the transaction on the server by calling `sendUpdateOffsetsInTransaction` and return control when it receives a response.
  ...
  - Asynchronous API
    ...
    This method takes a list of offsets as a parameter. For convenience, `Message` and `DataReceivedEvent` have a `getPartitionOffsets()` method that returns such a list.
...
In {{ ydb-short-name }}, server-side partition balancing between clients is used. This means that the server can interrupt reading messages from arbitrary partitions.
...
In case of *hard interruption*, the client receives a notification that it can no longer work with partition messages. The client must stop processing read messages. Unacknowledged messages will be transferred to another reader.
...
- Java
  ...
  - Asynchronous API
    ...
    To be able to react to such an event, override the `onStopPartitionSession(StopPartitionSessionEvent event)` method in the inheritor object `ReadEventHandler` (see [Connecting to a topic for reading messages](#start-reader)).
...
- C++
  ...
  A hard interrupt comes as a `TPartitionSessionClosedEvent` event, either in response to a soft interrupt acknowledgment or when the connection to a partition is lost. You can find out the reason by calling the `GetReason` method.
...
- Go
  ...
  When reading is interrupted, the context of the message or message packet will be canceled.
...
- Python
  ...
  In this example, batch message processing will stop if the partition is reassigned during operation. This optimization requires additional client code. In simple cases, when processing reassigned partitions is not a problem, it can be omitted.
...
- Java
  ...
  - Synchronous API
    ...
    Irrelevant, because the synchronous reader does not allow configuring handling of such events.
...
- Rust
  ...
  The Rust SDK handles partition session stop and close events internally; there is no public API for configuring soft or hard reading interruption yet.
...
- C++
  ...
  After receiving `TEndPartitionSessionEvent` and processing all messages, it is recommended to always commit them immediately. This will balance the reading of child partitions among different read sessions, leading to an even distribution of load across all readers.
  ...
  It is recommended to verify the correct handling of a soft read interruption: the client must process the received messages, commit them or save the read position in its database, and only then call `Confirm()` for the `TStopPartitionSessionEvent` event.
- Go
...
Most often, it is convenient to commit within the reader that receives messages. However, there are scenarios where commit must be performed by a process different from the reading process. In such a case, a commit method outside the reader is needed.

{% list tabs group=lang %}

- C++

  Commit outside the read session is performed using the `NYdb::NTopic::TTopicClient::CommitOffset` method:
...
- Go
  ...
  Acknowledgment of processing outside the reader is done using the `db.Topic().CommitOffset` method:
...
- Python
  ...
  Acknowledgment of processing outside the reader is done using the `topic_client.commit_offset` method:
