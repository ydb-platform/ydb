...
This article provides examples of how to use {{ ydb-short-name }} SDK to work with [topics](../../concepts/topic.md).
...
You can also use the advanced connection creation option to specify multiple topics and set reading parameters. The following code will create a connection to the `my-topic` and `my-specific-topic` topics via the `my-consumer` consumer and also set the time to start reading messages from:
...
The server stores the [consumer offset](../../concepts/topic.md#consumer-offset). After reading a message, the client can [send a processing confirmation to the server](#commit). The consumer offset will change and only unconfirmed messages will be read in case of a new connection.

You can read messages without a [processing confirmation](#no-commit) as well. In this case, all unconfirmed messages, including those processed, will be read if there is a new connection.
...
### Reading without a message processing confirmation {#no-commit}
...
### Reading with a message processing confirmation {#commit}

To confirm the processing of messages one by one, use the following code:
...
To confirm the processing of message batches, use the following code:
...
In case of a _soft interruption_, the client receives a notification that the server has finished sending messages from the partition and messages will no longer be read. The client can finish message processing and send a confirmation to the server.

In case of a _hard interruption_, the client receives a notification that it is no longer possible to work with partitions. The client must stop processing the read messages. Unconfirmed messages will be transferred to another consumer.
