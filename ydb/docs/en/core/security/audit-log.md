...
|| `program_text`             | `ExecuteTabletMiniKQL` payload of the TabletService. ||
|| `schema_changes`           | `ChangeTabletSchema` payload of the TabletService. ||
|| `table`                    | Full table path. ||
|| `row_count`                | Number of rows affected by `BulkUpsert`. ||
