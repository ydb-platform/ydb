...
Different [criteria](#password-complexity) for password complexity may be established.
...
{{ ydb-short-name }} provides protection against password brute-forcing. A user is considered blocked if they exceed the number of incorrect password attempts. After the specified time expires, they can authenticate again.
...
{{ ydb-short-name }} integrates with [LDAP directory](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol). The LDAP directory is external to the {{ ydb-short-name }} service and is used for authenticating and authorizing database users. Before using this authentication and authorization method, you must have a deployed LDAP service and configured network access between it and the {{ ydb-short-name }} servers.
...
Currently, {{ ydb-short-name }} supports only one method of LDAP user authentication — *search+bind*: after receiving the login and password, a service *bind* is performed on behalf of the [service account](#ldap-service-account-auth) (login and password in `bind_dn` or `bind_password`, or a certificate and SASL EXTERNAL — see [configuration](../reference/configuration/auth_config.md#ldap-auth-config)), then a search for the user record and a second *bind* on behalf of the user.
...
2. After a successful connection, a search is performed for the user attempting to authenticate. The search runs across the entire subtree specified in the `base_dn` configuration parameter and by the filter specified in the `search_filter` parameter.
...
The frequency of updating information about a user and their groups is set by the [`auth_config.refresh_time`](../reference/configuration/auth_config.md#caching-auth-results) parameter. For more details, see the article on [caching of authentication results](./caching-authentication-results.md#refreshing-user-tokens).

{% endnote %}

{% note warning %}

It should be noted that currently {{ ydb-short-name }} cannot track group renames made on the LDAP server side. Thus, a group with a new name will not have the same rights that the group with the previous name had.
...
Below are examples of `user1` user authentication using [{{ ydb-short-name }} CLI](../reference/ydb-cli/index.md):

* User authentication from an LDAP directory: `ydb --user user1@ldap -p ydb_profile scheme ls`
* User authentication by the internal {{ ydb-short-name }} mechanism: `ydb --user user1 -p ydb_profile scheme ls`
...
When using an unencrypted connection, all data transmitted in requests to the LDAP server will be transmitted in plaintext, including passwords. This connection method is easier to start using and is more suitable for experiments or testing.
...
4. From the `jwks_uri` field of the Discovery document, the URL is extracted, from which {{ ydb-short-name }} periodically obtains a set of public keys [JWKS](https://www.rfc-editor.org/rfc/rfc7517).
...
6. The user identifier and the list of groups are extracted from the token fields. The configured authentication domain suffix is added to them, after which the resulting SIDs are used for [authorization](./authorization.md).

The Discovery URL, `issuer`, and `jwks_uri` must use the `https://` scheme. If the Discovery document or JWKS is temporarily unavailable, {{ ydb-short-name }} retries requests with an increasing interval. The obtained keys are cached and periodically refreshed to support key rotation on the IdP side. After the cache lifetime expires, stale keys are removed; until JWKS is successfully updated, new token verifications fail with a temporary error.
...
Symmetric algorithms of the `HS*` family are not supported. The public key in JWKS must have matching `kty` (key type) and `kid` and contain `x5c` (X.509 certificate chain). The public key is extracted from the first certificate `x5c`; JWK containing only RSA or EC parameters without `x5c` are skipped.
...
{{ ydb-short-name }} uses the group list from the token without additional calls to the IdP and without expanding nested groups. Users and groups of the external IdP cannot be managed using the `CREATE USER`, `ALTER USER`, `CREATE GROUP`, and `ALTER GROUP` commands. Rights are assigned to the formed SIDs using the methods described in the [{#T}](./authorization.md) section.
...
Maximum security and performance are achieved when using the **Metadata** mode, as it eliminates the need to work with secrets when deploying an application, and also allows you to contact IAM and cache the token in advance, before the application starts.
