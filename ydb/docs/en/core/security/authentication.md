...
By default, {{ ydb-short-name }} allows executing queries without specifying authentication data, such as a username or a [token](../concepts/glossary.md#auth-token). Access rights checking ([authorization](authorization.md)) is also not performed in this case.
...
{{ ydb-short-name }} allows you to configure password complexity requirements. If the password provided via the `CREATE USER` or `ALTER USER` commands does not meet the complexity criteria, the command will fail with an error.
...
{{ ydb-short-name }} provides protection against password brute force. A user will be considered blocked if they exceed the number of incorrect password attempts. After the specified time has elapsed, they will be able to authenticate again.
...
If necessary, a cluster or database administrator can [unlock](../yql/reference/syntax/alter-user.md) the user ahead of schedule.
...
{{ ydb-short-name }} integrates interaction with the [LDAP directory](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol). The LDAP directory is a service external to {{ ydb-short-name }} and is used for authenticating and authorizing database users. Before using this authentication and authorization method, you must have a deployed LDAP service and configured network access between it and the {{ ydb-short-name }} servers.
...
2. After a successful connection, a search is performed for the user attempting to authenticate. The search runs across the entire subtree specified in the `base_dn` configuration parameter and by the filter set in the `search_filter` parameter.
...
The frequency of updating information about a user and their groups is set by the parameter [`auth_config.refresh_time`](../reference/configuration/auth_config.md#caching-auth-results). For more details, see the article on caching authentication results.
...
For {{ ydb-short-name }} to automatically establish an encrypted connection with the LDAP server, set the value `ldaps` in the [configuration parameter](../reference/configuration/auth_config.md#ldap-auth-config) **scheme**. The TLS handshake will be initiated on the port specified in the configuration. If no port is specified, the default port 636 will be used for the `ldaps` scheme. The LDAP server must be configured to accept TLS connections on the specified ports.
...
5. Based on the `alg` (algorithm, signature algorithm) and `kid` (key ID, key identifier) fields of the JWT header, the public key is selected. {{ ydb-short-name }} verifies the token signature, issuer, audience, and expiration times.
...
Other fields to be checked are optional:
...
{{ ydb-short-name }} uses the list of groups from the token without additional access to the IdP and without revealing nested groups. You cannot manage users and groups of an external IdP using the `CREATE USER`, `ALTER USER`, `CREATE GROUP`, and `ALTER GROUP` commands. Permissions are assigned to the formed SIDs using the tools described in the section [{#T}](./authorization.md).
...
{{ ydb-short-name }} can authenticate a client using the client certificate data obtained during TLS connection establishment. The verification is performed at the application protocol level (gRPC and others), when the server already accepts requests over an open connection.
...
3. The server uses the certificate for authentication and verifies it according to the rules of the [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md) section.
...
Certificate-based authentication is applied only to requests without an [authentication token](../concepts/glossary.md#auth-token). If the client passes an authentication token — for example, in the `Authorization` header for HTTP or through SDK/CLI mechanisms for IAM, login, and password — the token takes precedence. In this case, the certificate is passed at the TLS level but is not used for authentication.
...
Successful certificate authentication creates a user SID with the suffix `@<domain>`, where `<domain>` is the [parameter value](../reference/configuration/auth_config.md#certificate-auth-config) `certificate_authentication_domain` in the `auth_config` section (default: `cert`). The name is formed from all attributes of the certificate's Subject field in `Имя=Значение,...@<domain>` notation. The attribute order corresponds to the field order in the certificate. Example:
...
Certificate verification and group assignment rules are set in the [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md) section of the cluster static configuration. To enable client certificate requests during TLS handshake over gRPC, set the `request_client_certificate: true` parameter.
...
- **gRPC** and **YDB Monitoring** — you can enable client certificate request for device authentication, and also separately enable its mandatory verification (an untrusted certificate is always rejected). The gRPC configuration is described in sections [grpc_config](../reference/configuration/tls.md#grpc) and [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md), and client connection — in section [TLS connection parameters](../reference/ydb-cli/connect.md#tls); the YDB Monitoring configuration is described in section [monitoring_config](../reference/configuration/monitoring_config.md#tls).

## Authentication using a third-party IAM provider {#iam}

* **Access Token** — a fixed token is specified as a parameter for the client (SDK or CLI) and is passed in requests.
...
* **Refresh Token** can be used for one-off manual operations under a personal account, for example, those related to database data maintenance, ad-hoc CLI operations, or launching applications from a workstation. You can obtain such a token manually in IAM once for a long period and store it in an environment variable on your personal workstation so that it is applied automatically when you run the CLI without additional authentication parameters.
...
The token to be specified in the parameters can be obtained from the IAM system associated with a particular {{ ydb-short-name }} installation. In particular, for the {{ ydb-short-name }} service in {{ yandex-cloud }}, Yandex.Passport OAuth and {{ yandex-cloud }} service accounts are used. When using {{ ydb-short-name }} in corporate contexts, standard centralized authentication systems for the organization may be used.
