...
The authentication client is a user who undergoes the authentication procedure when accessing {{ ydb-short-name }}. Examples of clients include applications using [SDK](../reference/ydb-sdk/index.md) or [CLI](../reference/ydb-cli/index.md).
...
By default, {{ ydb-short-name }} allows requests to be executed without specifying authentication data, such as a username or [token](../concepts/glossary.md#auth-token). Access rights checking ([authorization](authorization.md)) is also not performed.
...
Depending on the [access level settings](../reference/configuration/security_config.md#security-access-levels), anonymous requests may also perform actions in the system that require administrative access.
...
{{ ydb-short-name }} provides protection against password brute-forcing by the user. A user is considered blocked if they exceed the number of incorrect password attempts. After the specified time expires, they will be able to authenticate again.
...
You can obtain information about the user's lock status and the number of incorrect password entry attempts from the [system view](../dev/system-views.md#users) of the user.

## Authentication using an LDAP directory {#ldap}

{{ ydb-short-name }} integrates with the [LDAP directory](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol). The LDAP directory is external to the {{ ydb-short-name }} service and is used for authenticating and authorizing database users. Before using this authentication and authorization method, you need to have a deployed LDAP service and configured network access between it and the {{ ydb-short-name }} servers.
...
The suffix `@<auth-domain>` is added to the user identifier and each group from the JWT. The `<auth-domain>` value is set by the `external_idp_authentication_domain` parameter. By default, the value `sso` is used.
...
Authentication via an external IdP is enabled in the [authentication configuration](../reference/configuration/auth_config.md#external-idp-auth-config) if the `external_idp_config` section is present.

## Client certificate authentication {#client-certificate}

{{ ydb-short-name }} can authenticate a client using the client certificate data obtained when establishing a TLS connection. The check is performed at the application protocol level (gRPC, etc.), when the server already accepts requests over an open connection.
...
Device authentication is verification of the [client certificate](../concepts/glossary.md#client-certificate) when establishing a TLS connection; a [SID](../concepts/glossary.md#access-sid) is not generated in this case. If a certificate is presented, the trust chain to the CA is checked; an untrusted certificate causes the connection to be rejected before application requests are processed. Whether a certificate must be presented depends on the interface (see the section [Use in {{ ydb-short-name }}](#device-auth-interfaces)).
...
* **Refresh Token** can be used for one-off manual operations under a personal account, for example, those related to data maintenance in the database, running ad-hoc operations in the CLI, or launching applications from a workstation. You can obtain such a token manually in IAM once for a long period and store it in an environment variable on your personal workstation so that it is applied automatically when you run the CLI without additional authentication parameters.
