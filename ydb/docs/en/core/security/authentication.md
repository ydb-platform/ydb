...
By default, {{ ydb-short-name }} allows executing requests without providing authentication data, such as a username or [token](../concepts/glossary.md#auth-token). Access rights checking ([authorization](authorization.md)) is also not performed in this case.
...
{{ ydb-short-name }} provides protection against password brute force. A user will be considered blocked if they exceed the number of incorrect password attempts. After the specified time, they will be able to authenticate again.
...
By default, the user has 4 attempts to enter the correct password. Otherwise, authentication for the user will be blocked for one hour. You can configure the user blocking criteria in the [configuration](../reference/configuration/auth_config.md#account-lockout).
...
{{ ydb-short-name }} integrates interaction with an [LDAP directory](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol). The LDAP directory is an external service relative to the {{ ydb-short-name }} service and is used for authenticating and authorizing database users. Before using this authentication and authorization method, you must have a deployed LDAP service and configured network access between it and the {{ ydb-short-name }} servers.
...
3. If the user is found, {{ ydb-short-name }} performs a repeated bind operation — now on behalf of the found user, using their password.
...
To distinguish that the entered login must be a user login from the LDAP directory rather than a local user login of {{ ydb-short-name }}, you need to add the suffix `@ldap` to it.
...
* Authenticating a user via the internal mechanism of {{ ydb-short-name }}: `ydb --user user1 -p ydb_profile scheme ls`
...
{{ ydb-short-name }} uses the list of groups from the token without additional calls to the IdP and without expanding nested groups. Users and groups of an external IdP cannot be managed using the `CREATE USER`, `ALTER USER`, `CREATE GROUP`, and `ALTER GROUP` commands. Rights are assigned to the generated SIDs using the tools described in the section [{#T}](./authorization.md).
...
{{ ydb-short-name }} can authenticate a client using the client certificate data obtained during TLS connection establishment. The check is performed at the application protocol level (gRPC and others) when the server already accepts requests over an open connection.
...
Certificate-based authentication is applied only to requests without an [authentication token](../concepts/glossary.md#auth-token). If the client passes an authentication token — for example, in the `Authorization` header for HTTP or via SDK/CLI mechanisms for IAM, login, and password — the token takes priority. In this case, the certificate is passed at the TLS level but is not used for authentication.
...
Device authentication is verification of the [client certificate](../concepts/glossary.md#client-certificate) when establishing a TLS connection; [SID](../concepts/glossary.md#access-sid) is not generated in this case. If a certificate is presented, the trust chain to the CA is checked; an untrusted certificate causes the connection to be rejected before application requests are processed. Whether a certificate must be presented depends on the interface (see the section [Usage in {{ ydb-short-name }}](#device-auth-interfaces)).
