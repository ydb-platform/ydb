...
**gRPC** and **YDB Monitoring** — you can enable client certificate request for device authentication, and also separately enable its mandatory verification (an untrusted certificate is always rejected). The gRPC configuration is described in sections ⟦L1⟧grpc_config⟦L1⟧ and ⟦L2⟧client_certificate_authorization⟦L2⟧, and client connection — in section ⟦L3⟧TLS connection parameters⟦L3⟧; the YDB Monitoring configuration is described in section ⟦L4⟧monitoring_config⟦L4⟧.
