...
### Forced user blocking or unblocking
...
{{ ydb-short-name }} provides protection against password brute-forcing by a user. A user is considered blocked if they exceed the number of incorrect password attempts. After the specified time expires, they will be able to authenticate again.
...
By default, the user is given 4 attempts to enter the correct password. Otherwise, authentication will be denied for them for one hour. You can configure user blocking criteria in the [configuration](../reference/configuration/auth_config.md#account-lockout).
...
2. After a successful connection, a search is performed for the user attempting to authenticate. The search covers the entire subtree specified in the `base_dn` configuration parameter and uses the filter specified in the `search_filter` parameter.
...
4. The final result — successful or unsuccessful authentication — is determined by the outcome of the second bind (performed on behalf of the user).

Thus, {{ ydb-short-name }} does not store user passwords and fully relies on the LDAP authentication mechanism.

Upon successful verification of the user's login and password in the LDAP directory, {{ ydb-short-name }} returns an [authentication token](../concepts/glossary.md#auth-token). This token is then used instead of the login and password. Using a token speeds up the authentication process and improves security.
...
In the current implementation, the group names that {{ ydb-short-name }} will operate with match the values written in the `memberOf` attribute. They can be long and difficult to read.
...
The frequency of updating information about a user and their groups is set by the parameter [`auth_config.refresh_time`](../reference/configuration/auth_config.md#caching-auth-results). For more details, see the article on [caching of authentication results](./caching-authentication-results.md#refreshing-user-tokens).
...
Below are examples of authenticating user `user1` using [{{ ydb-short-name }} CLI](../reference/ydb-cli/index.md):
...
When using an unencrypted connection, all data transmitted in requests to the LDAP server will be sent in plaintext, including passwords. This connection method is easier to get started with and is more suitable for experiments or testing.
...
5. The public key is selected by the `alg` (algorithm, signature algorithm) and `kid` (key ID, key identifier) fields of the JWT header. {{ ydb-short-name }} verifies the token signature, issuer, audience, and expiration times.
6. The user identifier and group list are extracted from the token fields. The configured authentication domain suffix is added to them, and the resulting SIDs are used for [authorization](./authorization.md).

The Discovery URL, `issuer`, and `jwks_uri` must use the `https://` scheme. If the Discovery document or JWKS is temporarily unavailable, {{ ydb-short-name }} retries requests with an increasing interval. The obtained keys are cached and periodically refreshed to support key rotation on the IdP side. After the cache lifetime expires, outdated keys are removed; until JWKS is successfully updated, new token verifications fail with a temporary error.
...
The other checked fields are optional:
...
{{ ydb-short-name }} uses the group list from the token without additional calls to the IdP and without expanding nested groups. Users and groups of an external IdP cannot be managed using the `CREATE USER`, `ALTER USER`, `CREATE GROUP`, and `ALTER GROUP` commands. Rights are assigned to the formed SIDs using the methods described in the [{#T}](./authorization.md) section.
...
{{ ydb-short-name }} can authenticate a client using the client certificate data obtained during the TLS connection establishment. The verification is performed at the application protocol level (gRPC and others) when the server already accepts requests over an open connection.
...
Certificate verification and group assignment rules are set in the [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md) section of the static cluster configuration. To enable client certificate requests during TLS handshake over gRPCs, set the `request_client_certificate: true` parameter.

### Client configuration

For more information about configuring [{{ ydb-short-name }} CLI](../reference/ydb-cli/index.md), see the [TLS connection parameters](../reference/ydb-cli/connect.md#tls) section.

## Device authentication by certificate {#device-auth}

Device authentication is the verification of [the client certificate](../concepts/glossary.md#client-certificate) when establishing a TLS connection; [SID](../concepts/glossary.md#access-sid) is not generated in this case. If a certificate is presented, the chain of trust to the CA is verified; an untrusted certificate causes the connection to be rejected before application requests are processed. Whether a certificate must be presented depends on the interface (see the [Usage in {{ ydb-short-name }}](#device-auth-interfaces) section).
...
3. Making application-level attacks harder — a process on a foreign host without a suitable certificate does not get access to the cluster API, even if a network route to the port exists.
...
2. If a certificate is presented, the server verifies it at the TLS level: the chain of trust to the configured certificate authority (CA), validity period, and so on. Additional rules for matching certificate fields (for example, `require_same_issuer`, Subject, and SAN) are applied during [client certificate authentication](./authentication.md#client-certificate).
...
- **gRPC** and **YDB Monitoring** — you can enable client certificate request for device authentication, and also separately enable its mandatory verification (an untrusted certificate is always rejected). gRPC configuration is described in sections [grpc_config](../reference/configuration/tls.md#grpc) and [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md), and client connection — in the section [TLS connection parameters](../reference/ydb-cli/connect.md#tls); YDB Monitoring configuration is described in section [monitoring_config](../reference/configuration/monitoring_config.md#tls).

## Authentication using a third-party IAM provider {#iam}

* **Access Token** — a fixed token is specified as a parameter for the client (SDK or CLI) and passed in requests.
* **Refresh Token** — an [OAuth token](https://auth0.com/blog/refresh-tokens-what-are-they-and-when-to-use-them/) of a personal account is specified as a parameter for the client (SDK or CLI), based on which the client periodically accesses the IAM API in the background to rotate (obtain the next) token passed in requests.
* **Service Account Key** — the attributes of a service account and a signing key are specified as parameters for the client (SDK or CLI), based on which the client periodically accesses the IAM API in the background to rotate (obtain the next) token passed in requests.
...
Maximum security and performance are achieved when using the **Metadata** mode, as it eliminates the need to work with secrets when deploying the application and also allows you to contact IAM and cache the token in advance, before the application starts.
...
* **Access Token** is used when other modes are not supported on the server side or for configuration and debugging purposes. It does not require client interactions with IAM. However, if IAM supports an API for token rotation, the fixed tokens issued by such IAM typically have a short lifetime, which forces you to manually renew them in IAM on a regular basis.
* **Refresh Token** can be used for one-off manual operations under a personal account, for example, those related to data maintenance in the database, running ad-hoc operations in the CLI, or launching applications from a workstation. Such a token can be obtained manually in IAM once for a long period and stored in an environment variable on your personal workstation so that it is automatically applied when you run the CLI without additional authentication parameters.
...
A token to be specified in parameters can be obtained from the IAM system associated with a particular {{ ydb-short-name }} installation. In particular, for the {{ ydb-short-name }} service in {{ yandex-cloud }}, Yandex.Passport OAuth and {{ yandex-cloud }} service accounts are used. When using {{ ydb-short-name }} in corporate contexts, standard centralized authentication systems for the organization may be used.

When using modes that involve the {{ ydb-short-name }} client accessing IAM, you can additionally specify the IAM URL that provides the token issuance API. By default, existing SDKs and CLIs attempt to access the {{ yandex-cloud }} IAM API hosted on `iam.api.cloud.yandex.net:443`.
