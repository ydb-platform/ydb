...
Regardless of the [authentication](https://en.wikipedia.org/wiki/Authentication) method, [authorization](https://en.wikipedia.org/wiki/Authorization) is always performed on the server side of {{ ydb-short-name }} based on the information about objects and access rights stored there. Access rights define the set of operations that can be performed.

Authorization is performed for every user action: their rights are not cached, as they may be revoked or granted at any time.

## User {#user}

Users in {{ ydb-short-name }} can be created from different sources:
...
A user `root` with maximum rights is distinguished separately. It is created during the initial cluster deployment, during which a password must be set immediately. Further use of this account is not recommended; you should create users with limited rights.
...
The suffix `@<auth-domain>` identifies the “user source”, within which the uniqueness of all user logins or identifiers is guaranteed. For example, in the case of [LDAP authentication](authentication.md#ldap-auth-provider), user SIDs will be `user1@ldap` and `user2@ldap`.<br/>
...
Any [user](../concepts/glossary.md#access-user) can be added to or removed from an [access group](../concepts/glossary.md#access-group). As soon as a user is added to a group, they receive all rights to [database objects](../concepts/glossary.md#access-object) that were granted to the access group.
...
An access group can be empty, meaning it contains no users.
...
[Access rights](../concepts/glossary.md#access-right) in {{ ydb-short-name }} are tied not to the [subject](../concepts/glossary.md#access-subject), but to the [access object](../concepts/glossary.md#access-object).
...
By default, rights are inherited from parents to descendants along the access object tree.
...
Each access object has an [owner](../concepts/glossary.md#access-owner). By default, the owner is the [access subject](../concepts/glossary.md#access-subject) that created the [access object](../concepts/glossary.md#access-object).
...
The cluster as a whole and each database also have an object owner.
