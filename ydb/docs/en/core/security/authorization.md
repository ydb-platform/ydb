...
Access level lists form a hierarchy (used by the [YDB UI](../reference/ydb-ui/ydb-monitoring.md), viewer, and many other cluster-wide actions; ordered from lower to higher privileges):
...
- **Viewer** (presence in `viewer_allowed_sids`) — read-only access to cluster-wide state: subjects can open [YDB UI](../reference/ydb-ui/ydb-monitoring.md) pages and diagnostics, but cannot run actions that change system state.
