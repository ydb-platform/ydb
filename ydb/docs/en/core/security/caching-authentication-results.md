...
During authentication, a user session receives [an authentication token](../concepts/glossary.md#auth-token), which is sent with each request to the {{ ydb-short-name }} cluster. Since {{ ydb-short-name }} is a distributed system, requests are processed on different [nodes](../concepts/glossary.md#node). Upon receiving a request, each node independently [checks the authentication token](./authentication.md#token-validation) and, upon successful verification, creates [a user token](../concepts/glossary.md#user-token).

The authentication token and the user token are different entities. The authentication token is sent by the client. The user token is created inside a node based on the verification results, contains the [SID](../concepts/glossary.md#access-sid) of the user and groups, and is used for [authorization](./authorization.md). It is stored only on a specific node and is not sent to the client.
