...
An authentication token and a user token are different entities. The authentication token is sent by the client. The user token is created inside a node based on the verification result, contains the user's and groups' [SID](../concepts/glossary.md#access-sid), and is used for [authorization](./authorization.md). It is stored only on a specific node and is not sent to the client.
...
- The record has not been used for `auth_config.life_time`. Only a request that matches the cache key (authentication token and verification context) updates the record's last use time.
- The record has expired. For login and password login tokens and external identity provider tokens, the record lifetime is determined by the authentication token's expiration. For requests signed with an access key, `auth_config.as_signature_expire_time` is used; for other types, `auth_config.expire_time` is used.

An error is considered retryable if verification can be repeated without changing the request. The corresponding authentication subsystem determines the error type.
