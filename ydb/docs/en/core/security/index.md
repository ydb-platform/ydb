...
- **Access subjects**:
  ...
  - **Users**. {{ ydb-short-name }} allows you to work with both local [users](./authorization.md#user) and users from external sources (LDAP directories, IAM systems, etc.).
  - **Groups**. {{ ydb-short-name }} allows you to combine users into named sets. The set of users in a group can be changed later. A group can also be empty — containing no users.
...
- **[Authentication](./authentication.md) and [authorization](./authorization.md)**. The access control system in {{ ydb-short-name }} provides a data protection mechanism in the {{ ydb-short-name }} cluster. Only authenticated [access subjects](../concepts/glossary.md#access-subject) (users and groups) can work with data, and access to data can be restricted.
  ...
  - **Client authentication**. When accessing the {{ ydb-short-name }} cluster, [users](../concepts/glossary.md#access-user) undergo [authentication](./authentication.md) — a verification process that confirms the user's identity. {{ ydb-short-name }} supports various authentication mechanisms; their detailed description can be found in the corresponding [authentication](./authentication.md) section.
    ...
    - The authentication token is used by {{ ydb-short-name }} nodes to authorize user access before processing their requests.
      ...
      The obtained authentication token can be reused by the user in their work when executing queries to the {{ ydb-short-name }} cluster. For more information about the authentication token, for example about caching parameters, etc., see the section [{#T}](../reference/configuration/auth_config.md).
  - **Authorization**. Based on authentication data, [authorization](./authorization.md) is performed — a process of checking for specific [access rights](../concepts/glossary.md#access-right) and [access levels](../concepts/glossary.md#access-level) for the user to perform the requested operation.
...
- **Encryption**. {{ ydb-short-name }} is a distributed system that typically runs on clusters located in multiple data centers. To protect user data, {{ ydb-short-name }} provides the following mechanisms:
  ...
  - [data encryption in transit](./encryption/data-in-transit.md) to ensure the security of data transmitted between the client and {{ ydb-short-name }}, between the nodes of the cluster itself {{ ydb-short-name }}
