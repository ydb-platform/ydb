...
## Key elements and concepts of {{ ydb-short-name }} security

![Overview diagram](./_assets/security-overview.png)

The {{ ydb-short-name }} security system uses the following concepts:

- **Access subjects**:

  - **Users**. {{ ydb-short-name }} allows working with both local [users](./authorization.md#user) and users from external sources (LDAP directories, IAM systems, and so on).
...
- **Access objects**. In {{ ydb-short-name }}, access objects are schema objects (tables, system views, and so on) for which access rights are configured.
- **Access rights**. **Access rights** in {{ ydb-short-name }} define the list of permitted operations on access objects for a specific user or group.

  Access rights are the ability to perform certain actions (create, delete, read, update, and so on) on an access object.
  ...
  For more about access rights, see the section [{#T}](./authorization.md#right).
- **Access levels**. **Access levels** in {{ ydb-short-name }} define access to additional cluster management capabilities for a specific user or group. {{ ydb-short-name }} uses hierarchical access levels: database, viewer, monitoring, and administration. Higher levels automatically include all lower ones. Additionally, there are two special non-hierarchical lists: bootstrap (for initial cluster initialization) and register node (for registering dynamic nodes).
  ...
  For more about access levels, see the section [{#T}](./authorization.md#access-level-lists).
- **[Authentication](./authentication.md) and [authorization](./authorization.md)**. The access control system in {{ ydb-short-name }} provides a data protection mechanism in the {{ ydb-short-name }} cluster. Only authenticated [access subjects](../concepts/glossary.md#access-subject) (users and groups) can work with data, and data access can be restricted.
  ...
  - **Client authentication**. When accessing a {{ ydb-short-name }} cluster, [users](../concepts/glossary.md#access-user) undergo [authentication](./authentication.md) — the process of verifying the user's identity. {{ ydb-short-name }} supports various authentication mechanisms; their detailed description can be found in the corresponding [authentication](./authentication.md) section.
    ...
    - The authentication token is used by {{ ydb-short-name }} nodes to authorize user access before processing their requests.
      ...
      The user can reuse the obtained authentication token multiple times when executing queries to the {{ ydb-short-name }} cluster. For more detailed information about the authentication token, such as caching parameters, etc., see the section [{#T}](../reference/configuration/auth_config.md).
  - **Authorization**. Based on authentication data, [authorization](./authorization.md) is performed — the process of checking for specific [access rights](../concepts/glossary.md#access-right) and [access levels](../concepts/glossary.md#access-level) for the user to perform the requested operation.
- **Audit logs**. Actions aimed at changing security settings are additionally logged in a separate journal called the [audit log](./audit-log.md). This journal will primarily be of interest to those responsible for information security. The audit log includes actions such as creating or deleting access objects, creating or deleting users, changing passwords, granting or revoking access rights, and so on.
- **Encryption**. {{ ydb-short-name }} is a distributed system that typically runs on clusters located in multiple data centers. To protect user data, {{ ydb-short-name }} provides the following mechanisms:
  ...
  - [Data encryption in transit](./encryption/data-in-transit.md) to ensure the security of data transmitted between the client and {{ ydb-short-name }}, and between nodes of the {{ ydb-short-name }} cluster itself.
