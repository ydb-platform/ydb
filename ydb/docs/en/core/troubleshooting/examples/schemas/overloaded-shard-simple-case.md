...
The article begins by [stating the problem](#initial-issue). Then, we'll examine diagrams in Grafana and information on the **Diagnostics** tab in the [{{ ydb-ui-name }}](../../../reference/ydb-ui/index.md) to [solve the problem](#solution) and [observe the solution in action](#aftermath).
...
We can also observe overall CPU usage on the **Diagnostics** tab of the [{{ ydb-ui-name }}](../../../reference/ydb-ui/index.md):
...
To determine which table the overloaded data shard is processing, let's open the **Diagnostics > Top shards** tab in the {{ ydb-ui-name }}:
...
1. In the {{ ydb-ui-name }}, select the database.
