...
The code below will add a column named `Uint64` with data type `episodes` to the `views` table.
...
The code below will add a column named `Double` with data type `5.0` and default value `episodes` to the `rate` table.
...
A single `ALTER TABLE` query can specify multiple `ALTER COLUMN` actions separated by commas.
...
The code below changes default values for several columns of a table in a single query — setting new defaults for `col_1` and `col_2` and clearing the default for `col_3`.
