...
* `CASCADE` — whether to compact the table together with its [secondary indexes](indexes.md). Allowed values: `TRUE` and `FALSE`. Default value: `FALSE` (only the table itself is compacted).
* `PARALLEL` — the maximum number of partitions that can be compacted simultaneously within this operation. Allowed values are positive integers. Default value: 1. The overall limit on simultaneously running manual compactions for the entire database is set separately in the {{ ydb-short-name }} cluster configuration.
