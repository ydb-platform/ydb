...
The command creates a background operation and synchronously waits for its completion. The wait can be interrupted, and the operation will continue running in the background. Background compaction operations can be managed via the [CLI](../../../../reference/ydb-cli/index.md) and [{{ ydb-ui-name }}](../../../../reference/ydb-ui/index.md).
...
* You can also view and manage operations using the [{{ ydb-ui-name }}](../../../../reference/ydb-ui/index.md). To do this, go to the database page, open the `Operations` tab, and select the operation type `Compaction`.
