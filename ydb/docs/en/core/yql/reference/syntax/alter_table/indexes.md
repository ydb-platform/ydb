# Adding, dropping, and renaming an index

## Adding an index {#add-index}

`ADD INDEX` — adds an index with the specified name and type for the given set of columns in {% if backend_name == "YDB" and oss == true %}row tables.{% else %}tables.{% endif %} Syntax:
...
* - maximum number of `parallel` workers based on [partitions](../../../../concepts/glossary.md#partition) involved in building the index (integer between `1` and `MaxBuildIndexShardsInFlight` from `SchemeShardConfig`).

- If the parameter is not specified, the current default value `32` or `MaxBuildIndexShardsInFlight` is used if that value is smaller. `MaxBuildIndexShardsInFlight` defaults to `1000`. In future versions, the default parallelism selection logic may change.
...
The `ADD INDEX` operation for creating global secondary (`GLOBAL`, `UNIQUE`, etc.) and vector indexes is supported only for row tables. For [column tables](../../../../concepts/datamodel/table.md#column-oriented-tables) via `ADD INDEX` [only local bloom indexes are supported](#local-bloom).

Features of local bloom indexes:

{% include [bloom_skip_index_features.md](../_includes/bloom_skip_index_features.md) %}

{% note info "Constraints" %}
...
* `<table_name>` - name of the table whose index needs to be altered.
* `<index_name>` - name of the index to be altered.
...
* `<value>` - new parameter value. Possible values include:
  ...
  * for `FALSE_POSITIVE_PROBABILITY` — a floating-point number in the range `(0, 1)`; a lower value usually reduces false positives but increases the index size
  * for `NGRAM_SIZE` — an integer in the range `3` to `8` (it is usually recommended to start with `3`)
  * for `CASE_SENSITIVE` — `true` or `false`

### Example

The code in the following example enables load-based automatic partitioning for the index named `title_index` in table `series`, sets the minimum number of partitions to 5, and launches one replica in each availability zone (AZ) for each partition:
...
`DROP INDEX` — drops the index with the specified name. The code below drops the index named `title_index`.
...
You can also drop an index using the [table index](../../../../reference/ydb-cli/commands/secondary_index.md#drop) {{ ydb-short-name }} CLI command.

{% endif %}

## Renaming a secondary index {#rename-secondary-index}

`RENAME INDEX` — renames the index with the specified name. If an index with the new name already exists, an error is returned.
...
This applies to global secondary indexes (hidden index table and mode `--replace`). Local bloom indexes are not applicable to such atomic replacement under load.
