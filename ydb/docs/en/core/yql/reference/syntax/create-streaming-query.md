...
* `AS DO BEGIN ... END DO` — the full text of the new streaming query, including all necessary SQL statements. Restrictions on the query text are given in [{#T}](../../../concepts/streaming-query.md#limitations), see [below](#examples) for query text examples.
...
Writing to tables in streaming queries is supported **only in UPSERT mode**. The `INSERT INTO` operation is not supported because, during event reprocessing (the [at-least-once](../../../concepts/streaming-query/streaming-query.md#guarantees) guarantee), it would lead to duplicate rows. With `UPSERT`, if a row with the same primary key already exists, it will be updated; otherwise, a new row will be inserted, while `INSERT INTO` will fail with an error.
