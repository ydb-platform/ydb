...
Function `AsStruct` creates a structure from the specified fields, `Yson::From` converts it to Yson, `Yson::SerializeJson` serializes it to a JSON string, and `ToBytes` converts it to type `String`, which is required for writing to a topic.
...
DO BEGIN
    ...
    SELECT
        -- Generate JSON from individual fields
...
Writing to tables in streaming queries is supported **only in UPSERT mode**. The `INSERT INTO` operation is not supported, because when events are reprocessed (at-least-once guarantee [at-least-once](../../../concepts/streaming-query/streaming-query.md#guarantees)), it would lead to duplicate rows. With `UPSERT`, if a row with the same primary key already exists, it will be updated; otherwise, a new row will be inserted, while `INSERT INTO` will fail with an error.
...
DO BEGIN
    ...
    -- Write to table (UPSERT only, INSERT not supported)
...
CREATE STREAMING QUERY my_streaming_query WITH (
    RUN = FALSE,                      -- Do not run automatically
