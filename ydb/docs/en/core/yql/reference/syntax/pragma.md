...
When the `EnableSequentialTablePathPrefix` feature flag is enabled, each `TablePathPrefix` pragma takes effect from its position in the query text. A later `TablePathPrefix` changes the prefix for subsequent table references without changing paths in preceding statements.
