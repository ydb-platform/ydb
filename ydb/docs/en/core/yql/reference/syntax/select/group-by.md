...
Groups the results `SELECT` by values of specified columns or expressions. Together with `GROUP BY`, [aggregate functions](../../builtins/aggregation.md) (`COUNT`, `MAX`, `MIN`, `SUM`, `AVG`) are often used to perform calculations in each group.
...
2. Aggregate functions (see the next section). Columns that are **not** grouped can only be included as arguments of an aggregate function.
...
The ability to specify a name for a column or expression in `GROUP BY .. AS foo` is a YQL extension. Such a name becomes visible in `WHERE` despite the fact that filtering by `WHERE` is performed [earlier](../index.md#selectexec) than grouping. In particular, if the table `T` has two columns `foo` and `bar`, then in the query `SELECT foo FROM T WHERE foo > 0 GROUP BY bar AS foo` filtering will actually occur on the column `bar` from the source table.

{% endnote %}

## GROUP BY ... SessionWindow() {#session-window}

YQL supports grouping by sessions. You can add a special function `SessionWindow` to regular expressions in `GROUP BY`:
...
3. The resulting sessions are the final partitions on which aggregate functions are computed.

The key column of SessionWindow() (in the example, `session_start`) has the value "minimum `time_expr` in the session". In addition, when SessionWindow() is present in `GROUP BY`, a special aggregate function [SessionStart](../../builtins/aggregation.md#session-start) can be used.
...
Using the extended version of SessionWindow, you can solve a problem like this: split a partition into sessions as in the two-argument version of SessionWindow, but with a limit on the maximum session length by some constant:
...
* `ROLLUP`: groups column values in the order they are listed in the arguments (strictly left to right), forming subtotals for each group and a grand total.
...
`ROLLUP`, `CUBE`, and `GROUPING SETS` can be combined using commas.

### GROUPING {#grouping}

In the intermediate total, column values that do not participate in computations are replaced with `NULL`. In the grand total, values of all columns are replaced with `NULL`. `GROUPING` is a function that allows distinguishing the original `NULL` values from `NULL` values that were added when forming the grand and intermediate totals.

`GROUPING` returns a bitmask:

* `0` — `NULL` for the original empty value.
* `1` — `NULL`, added for subtotal or grand total.
...
The presence of the [SQL hint](../lexer.md#sql-hints) `COMPACT` immediately after the `GROUP` keyword allows more efficient aggregation in cases where the query author knows in advance that none of the aggregation keys will have a large amount of data (on the order of a gigabyte or a million rows). If this assumption proves incorrect, the operation may fail with an Out of Memory error or start running significantly slower compared to a regular GROUP BY.

Unlike regular GROUP BY, the Map-side combiner stage and additional Reduce for each field with [DISTINCT](#distinct) aggregation are disabled.
...
`HOP` groups data by overlapping time windows ( [hopping windows](https://en.wikipedia.org/wiki/Window_function_(SQL)#Hopping_window)). Supported both in [analytical queries on tables](#hop-table) and in [streaming queries on topics](#hop-topic).
...
- `hop` — step between the starts of adjacent windows in the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) format, for example `"PT10S"` (10 seconds).
...
Also available are aggregate functions `HOP_START()` and `HOP_END()`, which return the timestamp of the start and end of the current window of type `Timestamp` respectively.
...
- The window is considered complete upon receiving an event with a timestamp not less than the end of that window + `delay` (20 seconds). For example, window `[10; 40)` closes upon receiving an event with a timestamp of 60 or more.
...
Also available are the `HOP_START()` and `HOP_END()` functions, which return the timestamps of the start and end of the current window.
...
When working with [topics](../../../../concepts/datamodel/topic.md), `HoppingWindow` uses [watermarks](../../../../dev/streaming-query/watermarks.md) to determine the moment a window closes. A window closes when the watermark value is not less than the end of the window. This provides more accurate aggregation results compared to `HOP`, where the window closes at a fixed `delay`.
...
3. Events are processed in an order close to ascending `time_extractor`. Small reorderings of the input stream order are allowed.
...
Below is streaming reading from a topic: in `SELECT` it is convenient to output the end of the window via `HOP_END()`. For **tables** in an analytical query, `HOP_START()` or `HOP_END()` is more often used, depending on which window boundary needs to be shown in the result; the meaning of the windows is the same, only the selected label differs.
