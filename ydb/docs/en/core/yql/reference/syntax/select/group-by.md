...
Groups the `SELECT` results by the values of specified columns or expressions. Together with `GROUP BY`, [aggregate functions](../../builtins/aggregation.md) (`COUNT`, `MAX`, `MIN`, `SUM`, `AVG`) are often used to perform calculations in each group.
...
A query of the form `SELECT * FROM table GROUP BY k1, k2, ...` returns all columns listed in GROUP BY, i.e., it is equivalent to the query `SELECT DISTINCT k1, k2, ... FROM table`.
...
YQL also provides an aggregate function factory mechanism implemented using the functions [`AGGREGATION_FACTORY`](../../builtins/basic.md#aggregationfactory) and [`AGGREGATE_BY`](../../builtins/aggregation.md#aggregateby).
...
2. Each partition is divided into non-overlapping subsets of rows (sessions). To do this, the partition is sorted in ascending order of the value of expression `time_expr`. Session boundaries are drawn between adjacent elements of the partition whose difference in values of `time_expr` exceeds `timeout_expr`.
3. The sessions obtained in this way are the final partitions on which aggregate functions are computed.

The key column of SessionWindow() (in the example, `session_start`) has the value "minimum `time_expr` in the session". Additionally, when SessionWindow() is present in `GROUP BY`, a special aggregate function [SessionStart](../../builtins/aggregation.md#session-start) can be used.
...
Using the extended version of SessionWindow, you can solve, for example, the following task: split a partition into sessions as in the SessionWindow variant with two arguments, but with a limit on the maximum session length by some constant:
...
* `CUBE` — groups values for all possible column combinations, generates subtotals for each group, and a grand total.
...
GROUP BY
    ...
    GROUPING SETS(
        (column2, column3),
        (column3)
        -- if you also add (column2) here, then in total
...
The presence of the [SQL hint](../lexer.md#sql-hints) `COMPACT` immediately after the `GROUP` keyword allows for more efficient aggregation in cases where the query author knows in advance that no aggregation key will have a large amount of data (on the order of a gigabyte or a million rows). If this assumption is not met, the operation may fail with an Out of Memory error or start working significantly slower compared to a regular GROUP BY.

Unlike a regular GROUP BY, the Map-side combiner stage and additional Reduce stages are disabled for each field with a [DISTINCT](#distinct) aggregation.
...
- `time_extractor` is an SQL expression of type `Timestamp` that defines the event time. A timestamp is computed from each input row, and this timestamp determines the window assignment.
...
- The window is considered complete when an event with a timestamp not less than the end of this window + `delay` (20 seconds) is received. For example, window `[10; 40)` closes when an event with a timestamp of 60 or more is received.
...
In streaming queries, events may arrive not in strict chronological order. The `delay` parameter specifies the wait time after the formal window completion: the window does not close immediately, but after `delay` seconds, to allow delayed events time to arrive. Events that arrive after the window closes are ignored.
...
2. Each partition is sorted by `time_extractor` in ascending order.
...
For `HoppingWindow` to work correctly in streaming mode, you need to configure the watermark in the [WITH](with.md) section of the source. For more information, see [{#T}](../../../../dev/streaming-query/watermarks.md#configuration).
