...
If the `VIEW` expression is not used, the query will perform a full table scan with pairwise comparison of vectors. It is recommended to check the optimality of the written query using [query statistics](../../../../dev/query-plans-optimization.md). In particular, ensure there is no full scan of the main table.
