...
* `DIRECT_READ` — suppresses the work of some optimizers and forces using the table contents as is. The behavior is similar to setting the debug [DirectRead pragma](../pragma.md#debug), but for a specific data source.
...
* `FORMAT` — the format of stored data in file storages in [federated queries](../../../../concepts/query_execution/federated_query/s3/formats.md). Allowed values: `csv_with_names`, `tsv_with_names`, `json_list`, `json_each_row`, `json_as_string`, `parquet`, `raw`.
