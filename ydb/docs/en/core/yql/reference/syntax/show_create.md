...
- **CreateQuery** — The complete set of DDL statements needed to create the object:
    - For tables: the main [CREATE TABLE](create_table/index.md) statement (with the path relative to the database root), plus any additional statements describing the current configuration, such as:
        - [ALTER TABLE ... ALTER INDEX](alter_table/indexes.md#alter-index): for index partitioning settings.
        - [ALTER TABLE ... ADD CHANGEFEED](alter_table/changefeed.md) — for adding a changefeed.
        - [ALTER SEQUENCE](alter-sequence.md): for restoring the sequence state for [serial columns](../types/serial.md).
    - For views: the definition via [CREATE VIEW](create-view.md), and, if necessary, the statements the view has captured from the creation context, for example, [PRAGMA TablePathPrefix](pragma.md#table-path-prefix).
