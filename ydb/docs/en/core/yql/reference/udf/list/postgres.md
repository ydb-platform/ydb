...
If the original type is an array type (in PostgreSQL such types start with an underscore: `_int4` is an array of 32-bit integers), then the type name in YQL also starts with an underscore – `_PgInt4`.
...
In particular, YQL escaping rules must be followed, not PostgreSQL's.
...
SELECT
    ...
    PgInt4(1234),   -- literal constants can be used as an argument
...
```yql
SELECT
    FromPg("тест"pt), -- Just(Utf8("test")) - pg types are always nullable
...
Mapping table of YQL and PostgreSQL data types when using the `ToPg` function:

{% include [topg](../../_includes/topg.md) %}

### Type mapping table `FromPg` {#frompg}

Mapping table of PostgreSQL and YQL data types when using the `FromPg` function:
