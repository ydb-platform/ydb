...
[YQL](../../index.md) provides access to PostgreSQL [functions](https://www.postgresql.org/docs/16/functions.html) and [data types](https://www.postgresql.org/docs/16/datatype.html).
...
Values of string/numeric literals (i.e., what comes before the suffix) must be a valid string/number from the YQL perspective.
In particular, YQL escaping rules must be followed, not PostgreSQL ones.
...
To build an array literal, use the `PgArray` function:
...
### Literal constructor for arbitrary types {#literals_constructor}
...
To cast a value of one Pg type to another, use the built-in function `PgCast(<source value>, <desired type>)`:
...
To call a PostgreSQL function, add the `Pg::` prefix to its name:
...
## Calling PostgreSQL aggregate functions {#pgaggrfunction}

To call a PostgreSQL aggregate function, add the `Pg::` prefix to its name:
...
You can also use a PostgreSQL aggregate function to build an aggregate function factory for subsequent use in `AGGREGATE_BY`:
...
The `DISTINCT` mode over arguments when calling PostgreSQL aggregate functions is not supported, nor is the use of `MULTI_AGGREGATE_BY`.

{% endnote %}

## Logical operations

To perform logical operations, the functions `PgAnd`, `PgOr`, `PgNot` are used:
