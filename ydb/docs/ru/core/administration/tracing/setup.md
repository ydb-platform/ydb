...
Каждое правило включает в себя опциональное поле `scope` набором селекторов, которые определяют, к каким запросам применяется данное правило. На данный момент поддерживаются селекторы:
