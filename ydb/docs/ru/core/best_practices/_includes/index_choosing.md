...
Ранее YDB требовал явного указания вторичного индекса для использования в запросе.
```
SELECT * FROM `Table` VIEW Index
```

Однако популярные базы данных как правило предоставляют возможность автоматического использования индексов в запросах исходя из определенной некоторым образом оптимальности результирующего плана выполнения.
Поэтому, в YDB реализована возможность автоматического принятия решения о использовании индекса:
запрос 
```
SELECT * FROM `Table` WHERE Column = $param1
```
при существовании индекса по Column может выполниться с его использованием.

На данный момент механизм является экспериментальным и доступен только после включения в конфигурации базы.
...
<!-- При этом накладываются дополнительные ограничения на способы требующие джойна с основной таблицей -- рассматриваются только точечные чтения. -->
