...
## Отслеживание изменений {#read}

Для отслеживания изменений данных можно воспользоваться [SDK](../reference/ydb-sdk) или [{{ ydb-short-name }} CLI](../reference/ydb-cli). Как и к любому другому объекту схемы данных, к потоку изменений можно обратиться по его пути, который формируется следующим образом:
