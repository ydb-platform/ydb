...
* Улучшения планов и статистики запросов:
  * Добавлены выражения и атрибуты к различным свойствам оператора (например, GroupBy)
  * Статистика по каждому оператору (строки)
  * Статистика по сегментам столбцов (строки и байты)
* Исправлена ошибка, из-за которой команда `ydb workload * run` могла привести к сбою в режиме `--dry-run`.
* Добавлена поддержка просмотров в локальных резервных копиях: `ydb tools dump` и `ydb tools restore`. Представления сохраняются в виде запросов "CREATE VIEW", сохраненных в файлах "create_view.sql", которые могут быть выполнены для воссоздания исходных представлений.
* Заменен параметр "--query-settings" на "--query-prefix" в "ydb workload <workload> run`.
* Добавлены новые опции в "ydb workload topic": --tx-commit-interval и --tx-commit-messages, позволяющие указывать интервал между коммитами offset'ов либо в миллисекундах, либо в количестве записанных сообщений.
Также теперь вы можете загружать тесты YDB topics, используя широкие транзакции, которые охватывают все разделы в разделе. Это работает как при записи, так и при сквозных сценариях рабочей нагрузки.
* Команда `ydb import file csv` теперь сохраняет прогресс выполнения импорта. Повторный запуск команды импорта продолжится с той строки, на которой она была прервана
* QueryService установлен по умолчанию (`--executer generic`) в командах "ydb workload kv" и "ydb workload stock"
* В тестах "ydb workload" теперь используйтся формат parquet вместо CSV для заполнения таблиц.
* Флажок "--consumer" в команде "ydb topic read" стал необязательным. Теперь, если этот флажок не указан, чтение выполняется в режиме без пользовател. В этом режиме идентификаторы разделов должны указываться с помощью параметра `--partition-ids`.
* Исправлена ошибка в `ydb import file csv`, из-за которой несколько столбцов с экранированными кавычками в одной строке неправильно обрабатывались
* Усечены результаты запросов в тестах
* Добавлена команда "ydb admin cluster bootstrap" для автоматической первоначальной настройки кластера
