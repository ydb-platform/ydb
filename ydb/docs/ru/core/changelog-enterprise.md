...
* В системные представления [`.sys/top_queries_*`](./dev/system-views.md) и `.sys/query_sessions` добавлена колонка `TraceId` с идентификатором трассировки запроса.
...
Версия включает в себя все доработки, содержащиеся в сборке {{ ydb-short-name }} 25.4.1.15, см. [описание изменений](./changelog-server.md#25-4-1-15). Кроме того, включены все [дополнительные исправления](#25-2-1-ent-13-extras), перечисленные ниже для версии 25.2.1.ent.13.
...
Версия включает в себя все доработки, содержащиеся в сборке {{ ydb-short-name }} 25.3.1.27, см. [описание изменений](./changelog-server.md#25-3-1-27). Кроме того, включены все [дополнительные исправления](#25-2-1-ent-13-extras), перечисленные ниже для версии 25.2.1.ent.13.
...
Версия включает в себя все доработки, содержащиеся в сборке {{ ydb-short-name }} 25.2.1.26, см. [описание изменений](./changelog-server.md#25-2-1-26). Кроме того, версия включает в себя ряд дополнительных доработок, портированных из актуальной версии 26.1.
