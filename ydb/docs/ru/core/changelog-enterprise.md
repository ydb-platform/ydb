...
Версия включает в себя все доработки, содержащиеся в сборке {{ ydb-short-name }} 25.4.1.15, см. [описание изменений](./changelog-server.md#25-4-1-15).

Кроме того, включены все [дополнительные исправления](#25-2-1-ent-13-extras), перечисленные ниже для версии 25.2.1.ent.13.
...
* Реализован набор исправлений в индексном доступе (StreamIndexLookup), устраняющий возможность проявления редких ситуаций "зависания" выполняемых запросов, и сокращающих потребление оперативной памяти при выполнении запросов.
...
* Сообщения об ошибках TLI (Transaction Logs Invalidated) во всех случаях снабжаются идентификатором либо путём затронутой таблицы.
