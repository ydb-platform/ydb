...
* [Добавлен](https://github.com/ydb-platform/ydb/issues/6512) новый протокол гранулярного таймкаста, который позволит сократить времени выполнения распределенных транзакций (теперь замедление одного шарда не будет приводить к замедлению всех).
* [Реализована](https://github.com/ydb-platform/ydb/issues/11561) функциональность сохранения состояния даташардов в памяти при перезапусках. Это обеспечивает сохранение блокировок и в результате сокращает общую продолжительность выполнения длительных операций чтения/записи.
* [Реализована](https://github.com/ydb-platform/ydb/issues/9748) возможность переключения функциональных флагов в рантайме. Теперь большинство флагов будут применяться без необходимости рестарта.
...
* [Добавлена](https://github.com/ydb-platform/ydb/issues/7054) поддержка потока изменений (changefeed) при выполнении операций резервного копирования и восстановления, которая может быть включена с помощью настроек `enable_changefeeds_export` и `enable_changefeeds_import` в динамической конфигурации кластера.
