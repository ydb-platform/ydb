...
* [Добавлен настраиваемый троттлинг синхронной записи на VDisk](https://github.com/ydb-platform/ydb/pull/43895) в зависимости от текущего объема свежих данных. Это ограничивает рост потребления памяти при интенсивной записи. Поведение по умолчанию отключено и включается с помощью immediate control `VDiskControls.EnableFreshSyncDataThrottling`.
* [Литеральные значения `DEFAULT` можно добавлять в план запроса на запись при компиляции](https://github.com/ydb-platform/ydb/pull/45049), без материализации во время выполнения. Поведение по умолчанию отключено и включается настройкой `table_service_config.enable_compile_time_defaults: true`.
* [Добавлены JSON-индексы](https://github.com/ydb-platform/ydb/pull/36478) для ускорения запросов с `JSON_EXISTS` и `JSON_VALUE`. Создание и автоматический выбор индексов по умолчанию отключены и включаются [флагами](./reference/configuration/feature_flags.md?version=v26.3) `enable_json_index` и `enable_json_index_auto_select`.
* [Добавлена аутентификация через внешних поставщиков удостоверений OpenID Connect (OIDC)](https://github.com/ydb-platform/ydb/pull/39558). {{ ydb-short-name }} может проверять JWT-токены по набору ключей JSON Web Key Set (JWKS) поставщика и периодически обновлять данные аутентификации.
* [Добавлен строгий сериализуемый уровень изоляции для read-write-транзакций](https://github.com/ydb-platform/ydb/pull/43962), включая упорядочивание транзакций в реальном времени и временные метки коммита. Режим по умолчанию отключен и включается настройкой `table_service_config.enable_strict_serializable_isolation: true`.
* Добавлена [декомиссия групп хранения](./maintenance/manual/virtual_storage_groups_decommit.md?version=v26.3). Данные перемещаются в виртуальные группы в фоновом режиме, при этом приложения могут продолжать чтение и запись.
* [При восстановлении бэкапа SST-части можно записывать напрямую в DataShard](https://github.com/ydb-platform/ydb/pull/45009), снижая нагрузку на CPU и диски по сравнению с построчным восстановлением. Поведение по умолчанию отключено и включается настройкой `feature_flags.enable_data_shard_direct_part_import: true`.
* [Добавлен компактный формат полнотекстового индекса](https://github.com/ydb-platform/ydb/pull/43404), который сокращает потребление дискового пространства. Формат по умолчанию отключен и включается настройкой `feature_flags.enable_compact_fulltext_index: true`.
* Полнотекстовые индексы поддерживают [колонки фильтрации](./dev/fulltext-indexes.md?version=v26.3#filtered), что позволяет выполнять поиск внутри логического раздела таблицы. Возможность по умолчанию отключена и включается настройкой `feature_flags.enable_fulltext_index_prefix: true`.
* [Потоковая запись включена для вторичных индексов, `RETURNING` и `DEFAULT`](https://github.com/ydb-platform/ydb/pull/42752). Потоковый путь выполнения снижает потребление памяти и задержку больших операций записи.
* Классификаторы пулов ресурсов Workload Manager поддерживают [`ACTION="reject"`](https://github.com/ydb-platform/ydb/pull/46051), что позволяет отклонять подходящие запросы до их направления в пул ресурсов. Действие по умолчанию отключено и включается настройкой `feature_flags.enable_reject_action_in_resource_pool_classifiers: true`.
* Полнотекстовые индексы можно создавать для таблиц с [произвольными типами первичного ключа](./dev/fulltext-indexes.md?version=v26.3#primary-key). Возможность по умолчанию отключена и включается настройкой `feature_flags.enable_fulltext_index_row_id: true`.
* Добавлен [гибридный поиск](./dev/hybrid-search.md?version=v26.3), объединяющий полнотекстовую релевантность и векторную близость в единый ранжированный результат. Возможность по умолчанию отключена и включается настройкой `table_service_config.enable_hybrid_search: true`.
* [Добавлены метрики трансферов и статистика в `DescribeTransfer`](https://github.com/ydb-platform/ydb/pull/30345), упрощающие мониторинг и диагностику трансферов.
* Kafka API поддерживает [взаимную TLS-аутентификацию](./reference/kafka-api/auth.md?version=v26.3). Клиентский сертификат сопоставляется с идентификатором безопасности, а SASL-аутентификация для соединения не требуется.
* [Добавлен настраиваемый предел числа хранимых операций принудительной компакции](https://github.com/ydb-platform/ydb/pull/43766). После достижения предела завершенные и отмененные операции могут удаляться автоматически.
* [Записи Change Data Capture могут содержать идентификатор трейса OpenTelemetry](https://github.com/ydb-platform/ydb/pull/34489) из запроса, создавшего изменение.
* Оператор [`ANALYZE`](./yql/reference/syntax/analyze.md?version=v26.3) по-прежнему выполняется синхронно и создаёт запись о фоновой операции для наблюдаемости. Просмотр, получение, отмена и удаление этой записи по умолчанию отключены и включаются настройкой `feature_flags.enable_analyze_long_running_operation: true`.
* [Локальные индексы представляются схемными объектами](https://github.com/ydb-platform/ydb/pull/43144), включая префиксные Bloom-фильтры для строковых таблиц и локальные индексы для колоночных таблиц. Поведение по умолчанию отключено и включается настройкой `feature_flags.enable_local_index_as_scheme_object: true`.
* [Добавлена стратегия компакции Tiling++](https://github.com/ydb-platform/ydb/pull/36711) для колоночных таблиц. Ее можно выбрать с помощью параметра `default_compaction_preset`.
* Администраторы могут [потребовать у неадминистративных запросов к статическим узлам указывать базу данных](https://github.com/ydb-platform/ydb/pull/40964). Ограничение по умолчанию отключено и включается настройкой `feature_flags.forbid_requests_to_static_nodes_without_database: true`.
...
* [Для колоночных таблиц включены min-max индексы](https://github.com/ydb-platform/ydb/pull/38585). С их помощью ColumnShard пропускает порции данных, не входящие в диапазоны запроса, что сокращает чтение с диска и время выполнения запросов.
* [Для колоночных таблиц применяются квоты на небольшие блобы на уровне базы данных](https://github.com/ydb-platform/ydb/pull/43393). После исчерпания квоты, рассчитанной на основе `data_size_hard_quota`, новые записи отклоняются.
* Для колоночных таблиц [`DISTINCT` и `DISTINCT LIMIT` выполняются на стороне ColumnShard](https://github.com/ydb-platform/ydb/pull/38461), что сокращает объем промежуточных данных и время выполнения запросов.
* [Для сканирования колоночных таблиц включен Trivial Reader](https://github.com/ydb-platform/ydb/pull/38377), который эффективнее выполняет подходящие запросы чтения.
* Колонки колоночных таблиц поддерживают [словарное кодирование](./yql/reference/syntax/create_table/index.md?version=v26.3). Используйте `ENCODING(DICT)`, чтобы сократить объем хранимых данных с низкой кардинальностью.
* [По умолчанию включены пакетные запросы авторизации в AccessService](https://github.com/ydb-platform/ydb/pull/44266), что снижает накладные расходы на авторизацию.
* Потоковые YQL-запросы могут [читать пользовательские атрибуты сообщений из топиков](./concepts/query_execution/topics.md?version=v26.3#system-metadata) через служебное поле `__ydb_user_attributes`.
* [Для длительных аналитических запросов включено удержание снапшотов](https://github.com/ydb-platform/ydb/pull/36668), чтобы колоночные шарды не удаляли данные, необходимые активным операциям чтения.
* [При коммите транзакций, работающих только с топиками, используется BufferActor](https://github.com/ydb-platform/ydb/pull/37432), что оптимизирует обработку коммита без участия таблиц.
* [При чтении топика с указанной временной метки отфильтровываются сообщения с более ранним временем записи](https://github.com/ydb-platform/ydb/pull/40064), в том числе сообщения из одного блоба с более новыми сообщениями.

### Надёжность

* [Динамические узлы сохраняют длительные аренды](https://github.com/ydb-platform/ydb/pull/45855), благодаря чему кластер остаётся доступным при временной недоступности NodeBroker. Поведение по умолчанию отключено и включается настройкой `feature_flags.enable_node_broker_long_lease: true`.
