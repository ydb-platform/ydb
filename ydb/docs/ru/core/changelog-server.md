...
* Добавлен [экспорт и импорт объектов базы данных](./reference/ydb-cli/export-import/export-nfs.md?version=main) через локальную файловую систему, в том числе через файловые системы, подключённые по NFS. Возможность по умолчанию отключена и может быть включена администратором кластера.
...
* [Проверки прав IAM можно отправлять в Access Service пакетами](https://github.com/ydb-platform/ydb/pull/911). Возможность по умолчанию отключена и может быть включена администратором кластера.
