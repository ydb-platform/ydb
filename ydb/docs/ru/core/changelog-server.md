...
### Версия 25.4.x.y {#25-4-x-y}

Дата выхода: уточняется.

#### Функциональность

* Доступны инструкции YQL [`BATCH UPDATE`](./yql/reference/syntax/batch-update.md?version=v25.4) и [`BATCH DELETE FROM`](./yql/reference/syntax/batch-delete.md?version=v25.4) для массового обновления и удаления данных в больших таблицах вне транзакционных ограничений (пакетная обработка по партициям).
* Улучшено выполнение записи данных: запись выполняется в потоковом режиме без полной материализации перед отправкой в даташарды, что повышает производительность при крупных операциях записи. Изменение распространяется на часть сценариев; в отдельных случаях (например, таблицы со вторичными индексами) по-прежнему используется прежний подход. Общие сведения о конвейере выполнения см. в разделе [Выполнение запросов](./concepts/query_execution/index.md?version=v25.4).
* Улучшено выполнение Lookup Join: используется потоковый режим без материализации одной из сторон соединения, что снижает пиковое потребление памяти, ускоряет выполнение на больших наборах данных и снимает прежние ограничения на размер сторон соединения. См. описание [Index lookup Join](./faq/yql.md?version=v25.4#index-lookup-join) и синтаксис [оператора `JOIN`](./yql/reference/syntax/select/join.md?version=v25.4).
...
* Для строковых таблиц доступна настройка [режимов кэширования](./concepts/datamodel/table.md?version=v25.4#cache-modes) и новый режим `in_memory`.
* [Согласовано удаление строк](./concepts/ttl.md?version=v25.4) при неподходящих условиях TTL.
...
* Потоковая обработка и поставка данных теснее интегрированы в ядро: [трансфер «топик → таблица»](./concepts/transfer.md?version=v25.4), [потоковые запросы](./dev/streaming-query/index.md?version=v25.4).
...
* Изменён синтаксис работы с секретами: создание, изменение, удаление и использование — см. [Секреты](./concepts/datamodel/secrets.md?version=v25.4).
* Для `UNION ALL` используются параллельные соединения к шардам (Extend), что ускоряет аналитические запросы — см. [структуру планов запросов](./yql/query_plans.md?version=v25.4) и [использование планов при оптимизации запросов](./dev/query-plans-optimization.md?version=v25.4).
...
>>>>>>> 8f0b0bfb05b (return removed part back)
