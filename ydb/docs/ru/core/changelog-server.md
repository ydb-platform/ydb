...
* [Добавлен настраиваемый троттлинг синхронной записи на VDisk](https://github.com/ydb-platform/ydb/pull/43895) в зависимости от текущего объема свежих данных. Это ограничивает рост потребления памяти при интенсивной записи.
* [Литеральные значения `DEFAULT` можно добавлять в план запроса на запись при компиляции](https://github.com/ydb-platform/ydb/pull/45049), без материализации во время выполнения. Поведение контролируется параметром `TableServiceConfig.EnableCompileTimeDefaults`.
* [Добавлены JSON-индексы](https://github.com/ydb-platform/ydb/pull/36478) для ускорения запросов с `JSON_EXISTS` и `JSON_VALUE`. Создание и автоматический выбор индексов контролируются флагами `EnableJsonIndex` и `EnableJsonIndexAutoSelect`.
...
* [Добавлен строгий сериализуемый уровень изоляции для read-write-транзакций](https://github.com/ydb-platform/ydb/pull/43962), включая упорядочивание транзакций в реальном времени и временные метки коммита. Режим контролируется флагом `EnableStrictSerializableIsolation`.
...
* [При восстановлении бэкапа SST-части можно записывать напрямую в DataShard](https://github.com/ydb-platform/ydb/pull/45009), снижая нагрузку на CPU и диски по сравнению с построчным восстановлением. Поведение контролируется флагом `EnableDataShardDirectPartImport`.
* [Добавлен компактный формат полнотекстового индекса](https://github.com/ydb-platform/ydb/pull/43404), который сокращает потребление дискового пространства. Формат контролируется флагом `EnableCompactFulltextIndex`.
* Полнотекстовые индексы поддерживают [колонки фильтрации](./dev/fulltext-indexes.md?version=v26.3#filtered), что позволяет выполнять поиск внутри логического раздела таблицы.
...
* Классификаторы пулов ресурсов Workload Manager поддерживают [`ACTION="reject"`](https://github.com/ydb-platform/ydb/pull/46051), что позволяет отклонять подходящие запросы до их направления в пул ресурсов.
* Полнотекстовые индексы можно создавать для таблиц с [произвольными типами первичного ключа](./dev/fulltext-indexes.md?version=v26.3#primary-key). Возможность контролируется флагом `EnableFulltextIndexRowId` и по умолчанию отключена в этом релизе.
* Добавлен [гибридный поиск](./dev/hybrid-search.md?version=v26.3), объединяющий полнотекстовую релевантность и векторную близость в единый ранжированный результат. Возможность контролируется параметром `TableServiceConfig.EnableHybridSearch` и по умолчанию отключена в этом релизе.
...
* [`ANALYZE` представляется фоновой операцией](./yql/reference/syntax/analyze.md?version=v26.3), которую можно отслеживать, отменять и удалять через стандартные API операций и команды CLI.
* [Локальные индексы представляются схемными объектами](https://github.com/ydb-platform/ydb/pull/43144), включая префиксные Bloom-фильтры для строковых таблиц и локальные индексы для колоночных таблиц. Поведение контролируется флагом `EnableLocalIndexAsSchemeObject`.
...
* Администраторы могут [потребовать у неадминистративных запросов к статическим узлам указывать базу данных](https://github.com/ydb-platform/ydb/pull/40964), включив `ForbidRequestsToStaticNodesWithoutDatabase`. В этом релизе ограничение по умолчанию отключено.
...
* [Добавлена поддержка интерфейса AccessService V2](https://github.com/ydb-platform/ydb/pull/43466), включая пакетные запросы авторизации. Интерфейс контролируется флагом `EnableAccessServiceV2Interface`.
