...
## Версия 23.3.17 {#23-3-17}

Дата выхода: 5 января 2024.

**Исправления ошибок:**

* Исправлена ошибка с `InFlightTxCount`, который становился отрицательным во время `split/merge`.
* Исправлено ошибочное разрешение совместимости между нестабильными версиями различных приложений.
* Улучшена стабильность работы `segfault`, балансировщика и событий очереди загрузки в  `StateInit`.
* Убрана гонка данных при срабатывании `issue` счётчиков.
* Устранена утечка памяти в `TRopeArena`. 
* Устранено чрезмерное потребление CPU при большом количестве слушателей топика.
* Исправлена ошибка импорта ...
* ...
* Исправлена проверка ошибок во время `read_table_scan` и экспорта.
* Устранена перегрузка лога сообщениями от даташарда. 

**Функциональность:**
* Добавлены ANSI-настройки для общего запроса.
* Добавлено DML-логирование.
* ...
* Добавлено Grpc KV api.
* ...
* Добавлена поддержка `Pqv0` на стороне сервера.
* ...
* Добавлена возможность перезагрузки недоступного узла.
* Добавлены «ленивые гистограммы» для grpc запросов.
