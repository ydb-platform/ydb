...
* [Исправлена](https://github.com/ydb-platform/ydb/pull/36926) взаимоблокировка на даташарде при одновременном чтении и удалении таблицы: операция чтения (`TTxReadViaPipeline`), попадавшая в pipeline через low-priority очередь, и `DROP TABLE` могли взаимно объявить зависимости друг на друга и зависнуть. Добавлена проверка отсутствия операции удаления таблицы перед выполнением чтения.
