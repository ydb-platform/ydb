...
* Для колоночных таблиц доступны экспорт и импорт резервных копий, включая S3-совместимое хранилище.
* Колонки колоночных таблиц поддерживают [словарное кодирование](./yql/reference/syntax/create_table/index.md?version=v26.3). Используйте `ENCODING(DICT)` для значений с низкой кардинальностью.
* Для колоночных таблиц включены min-max индексы. ColumnShard пропускает порции вне диапазонов запроса, сокращая чтение с диска и время выполнения.
* Добавлена [декомиссия групп хранения](./maintenance/manual/virtual_storage_groups_decommit.md?version=v26.3). Данные перемещаются в виртуальные группы в фоновом режиме, приложения продолжают чтение и запись.
* Потоковая запись доступна для вторичных индексов, `RETURNING` и `DEFAULT`, сокращая потребление памяти и задержку больших операций записи.
* Добавлена аутентификация через внешних поставщиков удостоверений OpenID Connect (OIDC). {{ ydb-short-name }} проверяет JWT-токены по JSON Web Key Set (JWKS) поставщика и периодически обновляет данные аутентификации.
...
* Из `ydbd` удалены экспериментальный PostgreSQL wire protocol и синтаксис PostgreSQL SQL. PostgreSQL-совместимые типы, функции `Pg::` и федеративные запросы к внешним PostgreSQL не затронуты.
* Добавлена стратегия компакции Tiling++ для колоночных таблиц.
* При сканировании колоночных таблиц доступен Trivial Reader.
* Для колоночных таблиц можно запускать принудительную компакцию с помощью `ALTER TABLE ... COMPACT`.
* Колоночные таблицы поддерживают типы данных `Interval`, `Uuid` и `DyNumber`.
* Для колоночных таблиц `DISTINCT` и `DISTINCT LIMIT` выполняются на стороне ColumnShard, сокращая объём промежуточных данных и время запроса.
* Для длительных аналитических запросов включено удержание snapshot, чтобы колоночные шарды не удаляли данные, необходимые активному чтению.
* Для колоночных таблиц применяются квоты на небольшие блобы на уровне базы данных. После исчерпания квоты из `data_size_hard_quota` новые записи отклоняются.
* Добавлен кеш уровней KQP для векторных индексов. Максимальный размер задаётся параметром `resource_manager.kqp_level_cache_max_size_bytes`.
* По умолчанию включены пакетные запросы авторизации в AccessService, что снижает накладные расходы.
* Потоковые YQL-запросы могут [читать пользовательские атрибуты сообщений из топиков](./concepts/query_execution/topics.md?version=v26.3#system-metadata) через `__ydb_user_attributes`.
* По умолчанию включено локальное удаление обработанных данных SyncLog, что улучшает полную синхронизацию VDisk.
* В `DescribeTransfer` добавлены метрики и статистика для мониторинга и диагностики трансферов.
* Добавлен настраиваемый предел числа хранимых операций принудительной компакции. Завершённые и отменённые операции могут удаляться автоматически после достижения предела.
* Записи Change Data Capture могут содержать идентификатор трейса OpenTelemetry запроса, создавшего изменение.
* При коммите транзакций, работающих только с топиками, используется BufferActor, оптимизируя обработку без таблиц.
* При чтении топика с временной метки отфильтровываются сообщения с более ранним временем записи, в том числе из одного блоба с более новыми сообщениями.
...
* Добавлен строгий сериализуемый уровень изоляции для read-write-транзакций, включая упорядочивание в реальном времени и временные метки коммита.
* Добавлены [JSON-индексы](./reference/configuration/feature_flags.md?version=v26.3) для ускорения запросов с `JSON_EXISTS` и `JSON_VALUE`.
...
* Добавлен компактный формат полнотекстового индекса, сокращающий потребление дискового пространства.
* При восстановлении бэкапа SST-части можно записывать напрямую в DataShard, снижая нагрузку на CPU и диски по сравнению с построчным восстановлением.
* Локальные индексы представляются схемными объектами, включая префиксные Bloom-фильтры для строковых и локальные индексы для колоночных таблиц.
* Добавлен настраиваемый троттлинг синхронной записи на VDisk в зависимости от объёма свежих данных, ограничивающий рост памяти при интенсивной записи.
* Классификаторы пулов ресурсов Workload Manager поддерживают `ACTION="reject"`, отклоняя подходящие запросы до направления в пул.
* Добавлена поддержка AccessService V2, включая пакетные запросы авторизации. Изменение конфигурации требует перезапуска сервера.
* `ANALYZE` создаёт запись о фоновой операции для наблюдаемости; её можно просматривать, получать, отменять и удалять.
* Литеральные значения `DEFAULT` можно добавлять в планы запросов записи при компиляции, избегая материализации во время выполнения.
* Администраторы могут потребовать у неадминистративных запросов к статическим узлам указывать базу данных.
* Структурированные значения в JSON-журнале можно выводить отдельными полями вместо добавления в `message`.

### Надёжность

* Динамические узлы сохраняют длительные аренды, благодаря чему кластер остаётся доступным при временной недоступности NodeBroker.
