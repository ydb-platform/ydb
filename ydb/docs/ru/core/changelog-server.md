...
* Добавлена возможность перекладывать данные из топика в таблицу. [Трансфер данных](./concepts/transfer.md) позволяет прокачивать как маленькие так и гигантские (gb/s) объемы данных из топика в таблицу за секунды. Создать трансфер можно с помощью новой команды [CREATE TRANSFER](./yql/reference/syntax/create-transfer.md) в YQL. Подготовлена [инструкция с примером](./recipes/transfer/quickstart.md) для быстрого старта.
* Добавлен [спиллинг](./concepts/spilling.md), механизм управления памятью, при котором промежуточные данные, возникающие в результате выполнения запросов и превышающие доступный объём оперативной памяти узла, временно выгружаются во внешнее хранилище. В YDB для спиллинга в настоящее время используется диск. Спиллинг обеспечивает выполнение пользовательских запросов, которые требуют обработки больших объёмов данных, превышающих доступную память узла.
* Включены по умолчанию:

  * [векторный индекс](./dev/vector-indexes.md?version=v25.1) для приближённого векторного поиска,
  ...
  * поддержка [автопартиционирования топиков](./concepts/cdc.md?version=v25.1#topic-partitions) в CDC для строковых таблиц,
  * поддержка параметризованного [типа Decimal](./yql/reference/types/primitive.md?version=v25.1#numeric),
  ...
  * возможность [указания числа реплик](./yql/reference/syntax/alter_table/indexes.md?version=v25.1) для вторичного индекса,
  * системные представления с [историей перегруженных партиций](./dev/system-views?version=v25.1#top-overload-partitions).

* Увеличено максимальное время на выполнение одного запроса с 30 минут до 2 часов. 
