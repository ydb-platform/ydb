...
* Поддерживаются [экспорт и импорт колоночных таблиц](./recipes/import-export-column-tables.md?version=v26.2) через S3-совместимые хранилища. Функциональность отключена флагом.
...
* Добавлено удержание snapshot для длительных аналитических запросов к колоночным таблицам, чтобы данные snapshot не удалялись до завершения запроса. Функциональность отключена флагом.
* QueryService может уведомлять SDK о завершении работы узла или сессии, чтобы клиент перестал направлять туда новые запросы. Функциональность отключена флагом.
* Для колоночных таблиц добавлены лимиты на количество и объём малых блобов на уровне базы данных. При превышении жёсткого лимита новые записи отклоняются. Функциональность отключена флагом.
...
* Потоковые запросы могут читать пользовательские атрибуты сообщений топика. Функциональность отключена флагом.
...
* Добавлено онлайн-построение уникальных вторичных индексов. Функциональность отключена флагом.
* Для транзакций между топиками и таблицами добавлена оптимизированная проверка конфликтов. Функциональность отключена флагом.
