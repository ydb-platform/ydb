...
* [Добавлена](https://github.com/ydb-platform/ydb/issues/11454) поддержка консистентной асинхронной репликации, которая гарантирует целостность и согласованность данных на реплике. Решает проблему восстановления после сбоев, если база содержит критически важные данные и важна скорость восстановления.
...
* [Добавлена]((https://github.com/ydb-platform/ydb/pull/11764)) возможность включить парсер запросов ANTLRv4 вместо парсера ANTLRv3 с помощью флага `enable_antlr4_parser`.
* [Добавлена]((https://github.com/ydb-platform/ydb/issues/7054)) поддержка потока изменений(changefeed) при выполнении операций резервного копирования и восстановления. Под флагами `EnableChangefeedsExport` and `EnableChangefeedsImport`.
...
* [Реализовано](https://github.com/ydb-platform/ydb/pull/12567) сохранение оптимистичные блокировки в памяти в случае плавного перезапуска даташардов, что должно уменьшить количество ошибок ABORTED из-за потери блокировок при балансировке таблиц между узлами и может сократить время запуска сегментов за счёт сокращения времени ожидания восстановления состояния посредника.
...
* ?25-1-2[Поддержан](https://github.com/ydb-platform/ydb/pull/7052) формат `DEBEZIUM_JSON` для потоков изменений (changefeed).
* ?[Добавлены](https://github.com/ydb-platform/ydb/pull/14173) динамические конфигурации для баз данных.
* [Добавлена](https://github.com/ydb-platform/ydb/issues/12724) поддержка представлений (VIEW) при выполнении операций резервного копирования и восстановления. Под флагом `EnableViewExport`.
