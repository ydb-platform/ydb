...
* Добавлен [стоимостный оптимизатор](./concepts/optimizer#stoimostnoj-optimizator-zaprosov) для сложных запросов к колоночным таблицам, который рассматривает большое количество альтернативных планов выполнения и выбирает из них лучший на основе оценки стоимости каждого варианта. На текущий момент оптимизатор работает только с планами, где есть операции [JOIN](./yql/reference/syntax/join).
...
* Поддержана [асинхронная репликация](./concepts/async-replication), которая позволяет синхронизировать данные между базами YDB почти в реальном времени. Также она может быть использована для миграции данных между базами данных с минимальным простоем работающих с ними приложений.
...
* Разработана документация по разворачиванию YDB с функциональностью [федеративных запросов](./concepts/federated_query/) (в ручном режиме).
...
* Поддержан [автоинкремент](./yql/reference/types/serial) для колонок, включенных в первичный ключ таблицы.
* [Поддержан](https://github.com/ydb-platform/ydb/pull/7150) [CDC](./concepts/cdc) для синхронных вторичных индексов.
* Реализована поставка [Count-min sketch](https://en.wikipedia.org/wiki/Count%E2%80%93min_sketch) статистики по [колоночным таблицам](./concepts/datamodel/table#column-oriented-tables) в анализатор запросов.
...
* Реализована начальная версия [менеджера рабочей нагрузки](./dev/resource-consumption-management), который позволяет создавать пулы ресурсов с ограничениями по процессору, памяти и количеству активных запросов. Реализованы классификаторы ресурсов для отнесения запросов к определенному пулу ресурсов.
* Добавлен просмотр сессий, установленных с базой данных.
* Поддержаны константные значения для колонок по умолчанию.
...
* Поддержано резервное копирование/восстановление из S3-совместимого хранилища ACL для таблиц.
...
* Добавлена возможность изменить период хранения в CDC топиках.
...
* Добавлены метрики на время старта таблетки после команды Hive и выведены на дашборды базы данных.
* Улучшена диагностика проблем хранилища в HealthCheck.
* **_(Экспериментально)_** Добавлена поддержка [представлений (VIEW)](https://ydb.tech/docs/ru/concepts/datamodel/view).
* **_(Экспериментально)_** Добавлен режим автопартиционирования топиков, в котором топики могут разбивать партиции в зависимости от нагрузки с сохранением гарантий порядка чтения сообщений и exactly once записи.  Для управления режимом воспользуйтесь инструкцией.
* **_(Экспериментально)_** Добавлены транзакции с участием [топиков](https://ydb.tech/docs/ru/concepts/topic) и строковых таблиц. Таким образом, можно транзакционно перекладывать данные из таблиц в топики и в обратном направлении, а также между топиками, чтобы данные не терялись и не дублировались.
* **_(Экспериментально)_** Реализован [автоматический выбор индекса](https://ydb.tech/docs/ru/dev/secondary-indexes#avtomaticheskoe-ispolzovanie-indeksov-pri-vyborke) при выполнении запроса, который может быть включен с помощью настройки index_auto_choose_mode в table_service_config.
...
* [Добавлена](https://github.com/ydb-platform/ydb-embedded-ui/pull/1218) настройка для добавления трассировки ко всем запросам и отображение результатов трассировки запроса.
* На страницу PDisk добавлены атрибуты, информация о потреблении дискового пространства, а также кнопка, которая запускает [декомиссию диска](./devops/manual/decommissioning).
* Добавлена информация о выполняющихся в данный момент запросах.
...
* Добавлено отображение топа запросов по потреблению CPU за последний час.
...
* Добавлено отображение количества полученных строк для всех запросов.
* [Разделено](https://github.com/ydb-platform/ydb-embedded-ui/pull/1422) отображение дисков от дисков-доноров.
* Поддержаны InterruptInheritance ACL и улучшено отображение действующих ACL.
* [Добавлено]( https://github.com/ydb-platform/ydb-embedded-ui/pull/889 ) отображение текущей версии пользовательского интерфейса.
* Добавлена возможность включать/выключать экспериментальную функциональность базы данных.

**Производительность:**

* Улучшена производительность CDC-топиков, содержащих тысячи партиций.
* Улучшен алгоритм балансировки таблеток Hive.
* Ускорено восстановление из бэкапа таблиц со вторичными индексами.
...
* Реализован механизм, принудительно запускающий фоновый [компакшн](./concepts/glossary#compaction) при обнаружении несоответствий между схемой данных и данными, хранящимися в [DataShard](./concepts/glossary#data-shard). Это решает редко возникающую проблему задержки в изменении схемы данных.
...
* [Исправлена](https://github.com/ydb-platform/ydb/pull/9377) ошибка нарушения инварианта при первоначальном сканировании CDC.
...
* [Исправлена](https://github.com/ydb-platform/ydb/pull/11525) потеря схемного снепшота в CDC при параллельном выполнении схемной операции и RW-транзакций.
* Исправлена [ошибка](https://github.com/ydb-platform/ydb/issues/6985) в отображении времени, на которое запланировано выполнение набора транзакций (планируемый шаг).
* [Исправлена](https://github.com/ydb-platform/ydb/pull/9161) [проблема](https://github.com/ydb-platform/ydb/issues/8942) прерывания [BGD](https://en.wikipedia.org/wiki/Blue%E2%80%93green_deployment) в больших кластерах, возникающая из-за частого обновления списка узлов.
