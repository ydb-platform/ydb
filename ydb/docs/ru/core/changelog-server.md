...
* Добавлен [стоимостной оптимизатор](./concepts/optimizer#stoimostnoj-optimizator-zaprosov) для сложных запросов к колоночным таблицам, который рассматривает большое количество альтернативных планов выполнения и выбирает из них лучший на основе оценки стоимости каждого варианта. Для строковых таблиц оптимизатор может быть включен с помощью [настройки](https://ydb.tech/docs/ru/yql/reference/syntax/pragma#costbasedoptimizationlevel) в `table_service_config`. На текущий момент оптимизатор работает только с планами, где есть операции [JOIN](./yql/reference/syntax/join).
...
* Добавлен просмотр сессий, установленных с базой данных, с помощью запроса.
...
* [Добавлена](https://github.com/ydb-platform/ydb-embedded-ui/pull/1229) вкладка с информацией об экспериментальной функциональности базы данных.
