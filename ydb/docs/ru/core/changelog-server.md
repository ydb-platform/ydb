...
* Добавлена поддержка [инкрементальных резервных копий](./concepts/datamodel/backup-collection.md?version=v26.2), которые позволяют сохранять только изменения относительно предыдущей резервной копии в коллекции. Функциональность отключена флагом.
* Поддерживаются [экспорт и импорт колоночных таблиц](https://github.com/ydb-platform/ydb/issues/26498) через S3-совместимые хранилища. Таблицы с большим количеством колонок не поддерживаются. Функциональность отключена флагом.
* Добавлены [экспорт и импорт строковых таблиц](./reference/ydb-cli/export-import/export-nfs.md?version=main) через локальную файловую систему, в том числе через файловые системы, подключённые по NFS. Функциональность отключена флагом.
* Добавлено [удержание snapshot](https://github.com/ydb-platform/ydb/pull/36668) для длительных аналитических запросов к колоночным таблицам, чтобы данные snapshot не удалялись до завершения запроса. Функциональность отключена флагом.
* QueryService может [уведомлять SDK о завершении работы узла или сессии](https://github.com/ydb-platform/ydb/pull/39274), чтобы клиент перестал направлять туда новые запросы. Функциональность отключена флагом.
* Для колоночных таблиц добавлены [лимиты на количество и объём малых блобов](https://github.com/ydb-platform/ydb/pull/43393) на уровне базы данных. При превышении жёсткого лимита новые записи отклоняются. Функциональность отключена флагом.
...
* Выражения для [watermarks](./dev/streaming-query/watermarks.md?version=v26.2) могут вычисляться вне контекста отдельного сообщения. Функциональность отключена флагом.
* Потоковые запросы могут читать [пользовательские атрибуты сообщений топика](https://github.com/ydb-platform/ydb/pull/40378). Функциональность отключена флагом.
...
* Добавлены [min-max-индексы](https://github.com/ydb-platform/ydb/pull/38585) для колоночных таблиц. Функциональность отключена флагом.
* Добавлено [словарное кодирование](./yql/reference/syntax/create_table/index.md?version=v26.2#encoding) колонок в колоночных таблицах. Функциональность отключена флагом.
* Добавлено [построение уникальных вторичных индексов без остановки таблицы](https://github.com/ydb-platform/ydb/pull/35981). Функциональность отключена флагом.
...
* [Для транзакций между топиками и таблицами добавлена оптимизированная проверка конфликтов](https://github.com/ydb-platform/ydb/pull/36706). Функциональность отключена флагом.
