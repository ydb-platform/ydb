...
* Операции модификации данных в шардах переведены на новое сообщение `EvWrite`. Снижаются время выполнения и потребление CPU для части пишущих запросов. См. также раздел [Фаза подготовки в таблетке DataShard](./contributor/datashard-distributed-txs.md?version=v25.4#faza-podgotovki-v-tabletke-datashard) в статье о распределённых транзакциях DataShard.
