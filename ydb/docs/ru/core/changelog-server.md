...
* Исправлена ошибка, возникавшая в некоторых ситуациях записи в топик с пустым `producer_id` при выключенной дедупликации, и приводившая к аварийному завершению серверного процесса.
* [Устранена](https://github.com/ydb-platform/ydb/pull/3165) гонка между процессами слияния таблиц и сжатия данных, из-за которой сборка мусора могла завершиться ошибкой нарушения инвариантов и аварийным завершением серверного процесса.
* [Исправлена](https://github.com/ydb-platform/ydb/pull/2169) ошибка возможной неполной фиксации накопленных изменений при использовании распределенных транзакций.
* Исправлена ошибка отображения метрики количества партиций в топике, количество партиций отображалось некорректно.
* Исправлена проблема, приводящая к падению процесса `ydbd` из-за ошибочного состояния сессии записи в топик.
* Исправлена ошибка в Blob Storage, из-за которой при корректной конфигурации могли не запускаться узлы хранения данных.
* Исправлена ошибка в Blob Storage, из-за которой информация о смене состава группы хранения могла не поступать своевременно на узлы кластера.
* Исправлена ошибка  INTERNAL_ERROR (has already committed) в [стриминговых запросах](https://ydb.tech/docs/ru/reference/ydb-cli/yql).
* Исправлены ошибки, которые в том числе приводили утечкам памяти и рестартам узлов ydb topics.
