...
* Добавлена поддержка режима "In-Memory" для строчных таблиц: реализован [режим кэширования в памяти (`CACHE_MODE`) для семейств столбцов таблиц](https://github.com/ydb-platform/ydb/issues/18691), включая необходимую поддержку в синтаксисе YQL.
* В корпоративной сборке реализована [поддержка режима работы "Bridge"](https://ydb.tech/docs/ru/concepts/bridge?version=v25.3), обеспечивающего синхронную запись данных при размещении системы в двух и более дата-центрах.
* Улучшения в [федеративных запросах](./concepts/query_execution/federated_query/):
   * Добавлена поддержка новых внешних источников данных: [Prometheus](https://github.com/ydb-platform/ydb/pull/17148), [Apache Iceberg](https://github.com/ydb-platform/ydb/pull/17007), [OpenSearch](https://github.com/ydb-platform/ydb/pull/18444) и [Redis](https://github.com/ydb-platform/ydb/pull/16957).
   * Реализована поддержка pushdown для предикатов [`REGEXP`](https://github.com/ydb-platform/ydb/pull/19227) и [`LIKE`](https://github.com/ydb-platform/ydb/pull/17695) по колонкам `Utf8`, уменьшая объём читаемых внешних данных.
* Улучшения в языке запросов:
   * Добавлена поддержка SQL-операторов [`SHOW CREATE TABLE`](https://github.com/ydb-platform/ydb/issues/33) и [`SHOW CREATE VIEW`](https://github.com/ydb-platform/ydb/issues/16283).
   * Лимиты схемных объектов, такие как `MAX_SHARDS` и `MAX_PATHS`, теперь можно [изменять с помощью оператора `ALTER DATABASE`](https://github.com/ydb-platform/ydb/pull/19580) (см. также [#16742](https://github.com/ydb-platform/ydb/issues/16742)).
   * [Улучшена](https://github.com/ydb-platform/ydb/pull/20670) диагностическая информация, выводимая для DDL-операций над внешними таблицами, а также для оператора `ALTER TABLE ... RENAME ...`.
   * Реализован набор SQL-операторов для управления секретами `CREATE / ALTER / DROP SECRET`.
* Доработки в системе безопасности:
   * Переработано управление секретами: проведён [редизайн внутреннего хранилища секретов](https://github.com/ydb-platform/ydb/issues/23194). Старые секреты (`CREATE OBJECT TYPE SECRET`) по-прежнему доступны, миграция на новую систему управления секретами будет требоваться в будущих релизах.
   * Расширен аудит: YDB теперь пишет в журнал аудита [операции `ALTER/MODIFY USER`](https://github.com/ydb-platform/ydb/pull/16989), а также [события управления очередями YMQ](https://github.com/ydb-platform/ydb/pull/18333).
   * Добавлена поддержка [установки полномочий для системных таблиц](https://github.com/ydb-platform/ydb/issues/17039), что упрощает управление правами доступа.
   * Реализована [базовая валидация имён групп](https://github.com/ydb-platform/ydb/pull/19216) пользователей при работе с внутренним хранилищем пользователей и групп.
* Улучшения в работе топиков:
   * API топиков расширен: в выдачу [DescribeConsumer добавлены новые параметры](https://github.com/ydb-platform/ydb/issues/27766), а [попартиционные метрики топиков можно отгружать в пользовательские квоты](https://github.com/ydb-platform/ydb/issues/27769).
   * В Kafka API [появилась возможность создавать компактифицированные топики](https://github.com/ydb-platform/ydb/pull/18683), при этом YDB [автоматически создаёт и удаляет служебного consumer'а для компактификации топиков](https://github.com/ydb-platform/ydb/pull/20176).
   ...
   * Топики теперь можно [использовать как внешний источник данных](https://github.com/ydb-platform/ydb/pull/18955) в SQL-сценариях.
* Расширены возможности резервного копирования и восстановления данных:
   * Реализованы [резервное копирование](https://github.com/ydb-platform/ydb/pull/18138) и [восстановление](https://github.com/ydb-platform/ydb/pull/18214) конфигурации топика из S3 (см. также [#18195](https://github.com/ydb-platform/ydb/issues/18195)).
   * Реализована начальная поддержка [инкрементального резервного копирования](https://github.com/ydb-platform/ydb/issues/22043).
   * Обеспечено [сохранение ACL в резервных копиях](https://github.com/ydb-platform/ydb/issues/11452).
   * Добавлена поддержка [резервного копирование и восстановления для представлений](https://github.com/ydb-platform/ydb/issues/12724).
   * Реализовано [контроль целостности резервных копий на основе контрольных сумм](https://github.com/ydb-platform/ydb/issues/11459).
* Улучшена работа со скриптами: добавлена [поддержка выдачи промежуточных результатов](https://github.com/ydb-platform/ydb/pull/17421) во время выполнения скрипта и отдельный [статус `running`](https://github.com/ydb-platform/ydb/pull/19134) в API выполнения скриптов.
* Операции построения индексов [теперь сохраняют](https://github.com/ydb-platform/ydb/pull/17751) время начала, время завершения и SID пользователя, инициировавшего операцию.
* Команда `ydb tools dump` больше [не пытается сохранять](https://github.com/ydb-platform/ydb/pull/18401) таблицы-получатели и потоки изменений, задействованные в асинхронной репликации. Также добавлена поддержка [обработки колонок типа `UUID`](https://github.com/ydb-platform/ydb/pull/17198).
* Улучшения в распределённом хранилище:
   * Улучшена надёжность BlobStorage в сценариях дефицита места: [добавлена логика для безопасной обработки ситуаций](https://github.com/ydb-platform/ydb/issues/12510) при исчерпании места для хранения данных, переполнении области лога и переполнении области хранения метаданных мелкими blob-ами.
   * Реализована команда `ydb-dstool pdisk move` для [переключения диска с одного узла на другой](https://github.com/ydb-platform/ydb/pull/19684) при наличии общего доступа к диску.
   * Добавлены новые параметры управления хранением данных: [`GroupSizeInUnits` для групп хранения и `SlotSizeInUnits` для PDisks](https://github.com/ydb-platform/ydb/pull/19337).
   * Добавлена поддержка [малых дисков (8GB+)](https://github.com/ydb-platform/ydb/issues/27432).
* Улучшения в инструментах мониторинга:
   * добавлены [счётчики ожидания операций спиллинга](https://github.com/ydb-platform/ydb/pull/17394),
   * добавлены [счётчики узлов, VDisk и PDisk](https://github.com/ydb-platform/ydb/pull/18731),
   * включены [дополнительные атрибуты в span'ах DSProxy и VDisk](https://github.com/ydb-platform/ydb/pull/21010), упрощающие диагностику.
* Скорректировано поведение проверок состояния компонентов кластера:
   * [понижен уровень серьёзности состояния `FAULTY` для PDisks](https://github.com/ydb-platform/ydb/pull/17095),
   * добавлено [определение состояния bootstrap кластера в Healthcheck API](https://github.com/ydb-platform/ydb/pull/19600) для окружений при использовании конфигурации V2,

#### Производительность

* Улучшен оптимизатор запросов: реализован и включен по умолчанию оператор [StreamLookupJoin](https://github.com/ydb-platform/ydb/issues/27425) для соединения таблиц без предварительного вычисления ключей левой части, а также добавлено [проталкивание фильтров в обе стороны оператора `LEFT JOIN`](https://github.com/ydb-platform/ydb/issues/27558).
* В YDB [по умолчанию включён режим multi-broadcast](https://github.com/ydb-platform/ydb/pull/17794), что уменьшает накладные расходы для распределённых запросов, где широковещательная рассылка является оптимальной стратегией.
* Производительность на пути записи данных улучшена: добавлена [новая схема приоритетов PDisk, разделяющая realtime- и compaction-записи](https://github.com/ydb-platform/ydb/pull/21705).
...
* Накладные расходы на аутентификацию снижены за счёт [выноса проверки пароля в отдельный актор](https://github.com/ydb-platform/ydb/pull/19687) вместо выполнения этой логики внутри локальных транзакций компонента SchemeShard.
* Переработана логика планирования выделения [процессорных ресурсов при обработке запросов](https://github.com/ydb-platform/ydb/pull/19618) на основе подхода [HDRF](https://people.eecs.berkeley.edu/~alig/papers/h-drf.pdf) для повышения эффективности использования доступных вычислительных ресурсов.
* Для больших кластеров продолжено внедрение [Node Broker Delta Protocol](https://github.com/ydb-platform/ydb/issues/11064), снижающего нагрузку на сеть при обмене конфигурационной информацией.
* Улучшено управление нагрузками в крупных инсталляциях: [добавлена эвристика](https://github.com/ydb-platform/ydb/pull/18376), предотвращающая бесконечное перемещение по узлам таблеток, которые способны перегрузить узел в одиночку.

#### Исправления ошибок

* Исправлен ряд проблем корректности и стабильности в колоночных таблицах, включая:
  * [исправление](https://github.com/ydb-platform/ydb/pull/16061) обработки предикатов в scan-запросах,
  * [исправление](https://github.com/ydb-platform/ydb/pull/20238) гонки в CPU limiter, которые могли приводить к неконсистентным результатам.
* [Устранены](https://github.com/ydb-platform/ydb/pull/18121) сбои и ошибки при изменении таблиц с векторными индексами, включая операцию [`RENAME INDEX`](https://github.com/ydb-platform/ydb/pull/17982).
* Исправлены проблемы консистентности транзакций при работе со строчными таблицами:
   * [добавлены](https://github.com/ydb-platform/ydb/pull/18088) защитные механизмы от некорректных результатов в некоторых read-write транзакциях,
   * [устранена](https://github.com/ydb-platform/ydb/pull/18234) ошибка, из-за которой конфликтующие read-write транзакции могли нарушать сериализуемость после рестартов шардов.
* Исправления ошибок в топиках:
   * [Исправлена](https://github.com/ydb-platform/ydb/pull/16016) ошибка, приводящая к редким падениям узлов при балансировке сессий чтения из топиков.
   * [Исправлена](https://github.com/ydb-platform/ydb/pull/20560) ошибка управления памятью при изменении позиции в топиках со включённым автоматическим партиционированием.
   * [Устранены](https://github.com/ydb-platform/ydb/pull/20084) неожиданные ошибки `PathErrorUnknown` при подтверждении offset'ов.
* [Устранена](https://github.com/ydb-platform/ydb/pull/22298) проблема утечки объектов attach streams на стороне сервера после фактического завершения сессии.
* Исправлен ряд проблем стабильности в распределённом хранилище:
   ...
   * [исключено](https://github.com/ydb-platform/ydb/pull/18924) появление фантомных VDisk-ов из-за гонок между операциями создания и удаления группы,
   * [исправлена](https://github.com/ydb-platform/ydb/pull/19781) ошибка, приводившая к аварийному завершению серверного процесса при повторной попытке захвата PDisk без VDisk в CMS,
   * выбор состояния PDisk переключён на [менее двусмысленный источник данных](https://github.com/ydb-platform/ydb/pull/17687).
* Сервис координации [теперь корректно возвращает код `SCHEME_ERROR`](https://github.com/ydb-platform/ydb/pull/16901) для несуществующих ресурсов, вместо использовавшегося ошибочно кода `INTERNAL_ERROR`.
...
* Обновлена обработка аутентификации для [обработчиков `whoami` и `capabilities`](https://github.com/ydb-platform/ydb/pull/17942).
* Исправлена проблема, из-за которой запросы информации о PDisk [могли завершаться таймаутом](https://github.com/ydb-platform/ydb/pull/20432), если целевой узел был отключён или недоступен.
