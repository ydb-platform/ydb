...
* Полная синхронизация VDisk стала быстрее: по умолчанию локально удаляются обработанные данные SyncLog.
...
* Для колоночных и строчных таблиц обеспечен паритет по набору [типов YQL](./yql/reference/types/primitive.md?version=main), включая `Interval`, `Uuid` и `DyNumber`.
