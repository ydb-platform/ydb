...
* Добавлена поддержка [инкрементальных резервных копий](./concepts/datamodel/backup-collection.md?version=v26.2), которые позволяют сохранять только изменения относительно предыдущей резервной копии в коллекции. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Поддерживаются [экспорт колоночных таблиц](https://github.com/ydb-platform/ydb/issues/26498) в S3-совместимые хранилища и YT, а также импорт из S3-совместимых хранилищ. Таблицы с большим количеством колонок не поддерживаются. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Добавлены [экспорт и импорт строковых таблиц](./reference/ydb-cli/export-import/export-nfs.md?version=main) через локальную файловую систему, в том числе через файловые системы, подключённые по NFS. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Добавлено [удержание снапшотов](https://github.com/ydb-platform/ydb/pull/36668) для длительных аналитических запросов к колоночным таблицам, чтобы данные снапшота не удалялись до завершения запроса. Возможность по умолчанию отключена и может быть включена администратором кластера.
* QueryService может [уведомлять SDK о завершении работы узла или сессии](https://github.com/ydb-platform/ydb/pull/39274), чтобы клиент перестал направлять туда новые запросы. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Для колоночных таблиц добавлены [лимиты на количество и объём малых блобов](https://github.com/ydb-platform/ydb/pull/43393) на уровне базы данных. При превышении жёсткого лимита новые записи отклоняются. Возможность по умолчанию отключена и может быть включена администратором кластера.
...
* Выражения для [watermarks](./dev/streaming-query/watermarks.md?version=v26.2) могут вычисляться вне контекста отдельного сообщения. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Потоковые запросы могут читать [пользовательские атрибуты сообщений топика](https://github.com/ydb-platform/ydb/pull/40378). Возможность по умолчанию отключена и может быть включена администратором кластера.
...
* Добавлены [min-max-индексы](https://github.com/ydb-platform/ydb/pull/38585) для колоночных таблиц. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Добавлено [словарное кодирование](./yql/reference/syntax/create_table/index.md?version=v26.2#encoding) колонок в колоночных таблицах. Возможность по умолчанию отключена и может быть включена администратором кластера.
* Добавлено [построение уникальных вторичных индексов без остановки таблицы](https://github.com/ydb-platform/ydb/pull/35981). Возможность по умолчанию отключена и может быть включена администратором кластера.
...
* [Ускорены](https://github.com/ydb-platform/ydb/pull/38803) операции разделения и слияния таблиц с большим числом партиций: SchemeShard обновляет только затронутые партиции вместо полного пересчёта списка.
* [Для транзакций между топиками и таблицами добавлена оптимизированная проверка конфликтов](https://github.com/ydb-platform/ydb/pull/36706). Возможность по умолчанию отключена и может быть включена администратором кластера.
