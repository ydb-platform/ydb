...
Если сервер имеет боллее 32 ядер CPU, для повышение производительности YDB следует запускать динноды в раздельных taskset/cpuset, размером от 10 до 32 ядер. Например, в случае 128 ядер лучше всего запустить 4 динноды: каждую в своем taskset, размером 32 ядра.
