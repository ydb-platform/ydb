...
* **Дисковая подсистема**
  ...
  При планировании места стоит учитывать, что {{ ydb-short-name }} использует часть дискового пространства для своих внутренних нужд. Так, например, на среднего размера кластере из 8 нод можно ожидать потребления около 100gb на весь кластер. На самом большом кластере с >1500 нод — около 200gb. Так же есть логи размером 25.6 гб на каждом Pdisk и системная область на каждом Pdisk. Её размер зависит от размера Pdisk, но не менее 0.2 gb.
