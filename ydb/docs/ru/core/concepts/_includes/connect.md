...
Имя базы данных (`database`) - строка, задающая базу данных, к которой адресован запрос. Имеет структуру [пути к файлу](https://ru.wikipedia.org/wiki/Путь_к_файлу), с использованием символа `/` в качестве разделителя. 

На кластере YDB может быть развернуто множество баз данных, и значения параметра `database` определяются конфигурацией кластера. Как и `endpoint`, для облачных баз данных имя `database` показывается в консоли управления на странице требуемой БД, а также обычно может быть получено через CLI облачного поставщика. В корпоративных окружениях имена YDB `database` предоставляются командой администраторов, или также могут быть получены в консоли управления внутренним облаком.
