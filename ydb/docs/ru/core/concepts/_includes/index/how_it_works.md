...
### Управление ресурсами базы данных {#db-resources}

В базе данных выделяются ресурсы (CPU, RAM, максимальная параллельность выполнения запросов) используемые всеми запросами на равных условиях без каких либо ограничений. Если же необходимо разделять ресурсы в одной базе данных, то для этого в {{ ydb-short-name }} существуют пулы ресурсов и классификаторы пулов ресурсов. Один из сценариев применения такой функциональности, это разделение контуров онлайн вычислений и фоновой аналитики. Этот функционал доступен по умолчанию в {{ ydb-short-name }} 
