...
{{ ydb-short-name }} поддерживает реляционную [модель данных](../../../concepts/datamodel/table.md) и оперирует таблицами с предопределённой схемой. Для удобства организации таблиц поддерживается создание директорий по аналогии с файловой системой.
...
{{ ydb-short-name }} используется в сервисах Яндекса в качестве высокопроизводительной [OLTP](https://en.wikipedia.org/wiki/Online_transaction_processing) СУБД. В частности, сервисы {{ yandex-cloud }} {{ objstorage-full-name}} использует {{ ydb-short-name }} для хранения данных и базируется на её компонентах.
