...
Администраторы баз данных могут настраивать ограничения объектов схемы через файл `config.yaml` в секции `scheme_shard_config.scheme_limits`. Если ограничения не указаны, используются значения по умолчанию, приведённые в таблице ниже.
