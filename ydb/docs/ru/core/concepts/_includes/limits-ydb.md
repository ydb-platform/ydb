...
| Объект | Ограничение | Значение | Пояснение | Внутреннее<br/>название | Тип<br/>ошибки | Текст ошибки
| :--- | :--- | :--- | :--- | :---: | :---: | :---
...
| База данных | Максимальное количество путей (объектов в схеме) | 10 000 | Максимальное количество элементов пути (директорий, таблиц) в базе данных. | MaxPaths | PRECONDITION_FAILED | Paths count limit exceeded
| База данных | Максимальное количество таблеток | 200 000 | Максимальное количество таблеток (шардов таблиц и системных таблеток), которое может работать в базе данных. На запрос создания, копирования или изменения таблицы, в результате выполнения которого данное ограничение может быть превышено, будет возвращена ошибка. При достижении максимального количества таблеток в базе автоматическое шардирование таблиц не происходит. | MaxShards | PRECONDITION_FAILED | Shards count limit exceeded (in subdomain)
...
| База данных | Максимальный размер ACL | 10 КиБ | Максимальный суммарный размер всех правил контроля доступа, которые можно сохранить для одного объекта схемы. | MaxAclBytesSize | BAD_REQUEST | ACL size limit exceeded
| Директория | Максимальное количество объектов | 100 000 | Максимальное количество таблиц, директорий, созданных в директории. | MaxChildrenInDir | PRECONDITION_FAILED | Children count limit exceeded
| Таблица | Максимальное количество шардов таблицы  | 35 000 | Максимальное количество шардов таблицы. | MaxShardsInPath | PRECONDITION_FAILED | Shards count limit exceeded (in dir)
| Таблица | Максимальное количество столбцов | 200 | Ограничивает общее количество столбцов в таблице. | MaxTableColumns | SCHEME_ERROR при создании / BAD_REQUEST при изменении | Too many columns
...
| Максимальный размер значения строкового столбца  | 16 МиБ | GENERIC_ERROR
...
| Максимальная длина текста запроса | 10 КиБ | Ограничение на длину текста YQL-запроса. | `BAD_REQUEST`
| Максимальный размер значений параметров | 50 МБ | Ограничение на суммарный размер параметров, передаваемых при выполнении параметризованного запроса. | `BAD_REQUEST`
