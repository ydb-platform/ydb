...
## Настройка ограничений объектов схемы {#configure-limits}

Администраторы баз данных могут настраивать ограничения объектов схемы через файл `config.yaml` в секции `domains_config.domain.scheme_limits`. Если ограничения не указаны, используются значения по умолчанию, приведённые в таблице ниже.
...
domains_config:
  domain:
    - name: Root
      scheme_limits:
        max_paths: 10000              # Максимальное количество объектов схемы (таблиц, директорий). По умолчанию: 10000
        max_table_columns: 200        # Максимум столбцов в таблице. По умолчанию: 200
        max_table_indices: 20         # Максимум индексов в таблице. По умолчанию: 20
        max_shards: 200000            # Максимум шардов в базе. По умолчанию: 200000
      storage_pool_types:
        - kind: ssd
          # ... остальная конфигурация
```

Все поля в `scheme_limits` опциональны. Доступные параметры соответствуют внутренним названиям, указанным в таблице ниже.
...
| :--- | :--- | :--- | :--- | :---: | :---: |
| База данных | Максимальная глубина пути | 32 | Максимальное количество вложенных элементов пути (директорий, таблиц). Можно настроить через `scheme_limits.max_depth` в config.yaml. | MaxDepth | SCHEME_ERROR |
| База данных | Максимальное количество путей (объектов в схеме) | 10 000 | Максимальное количество элементов пути (директорий, таблиц) в базе данных. Можно настроить через `scheme_limits.max_paths` в config.yaml. | MaxPaths | GENERIC_ERROR |
| База данных | Максимальное количество таблеток | 200 000 | Максимальное количество таблеток (шардов таблиц и системных таблеток), которое может работать в базе данных. На запрос создания, копирования или изменения таблицы, в результате выполнения которого данное ограничение может быть превышено, будет возвращена ошибка. При достижении максимального количества таблеток в базе автоматическое шардирование таблиц не происходит. Можно настроить через `scheme_limits.max_shards` в config.yaml. | MaxShards | GENERIC_ERROR |
...
| Директория | Максимальное количество объектов | 100 000 | Максимальное количество таблиц, директорий, созданных в директории. Можно настроить через `scheme_limits.max_children_in_dir` в config.yaml. | MaxChildrenInDir | SCHEME_ERROR |
| Таблица | Максимальное количество шардов таблицы  | 35 000 | Максимальное количество шардов таблицы. Можно настроить через `scheme_limits.max_shards_in_path` в config.yaml. | MaxShardsInPath | GENERIC_ERROR |
| Таблица | Максимальное количество столбцов | 200 | Ограничивает общее количество столбцов в таблице. Можно настроить через `scheme_limits.max_table_columns` в config.yaml. | MaxTableColumns | GENERIC_ERROR |
...
| Таблица | Максимальное количество столбцов в первичном ключе | 20 | Каждая таблица должна иметь первичный ключ. Количество столбцов, входящих в состав первичного ключа, не может превышать это ограничение. Можно настроить через `scheme_limits.max_table_key_columns` в config.yaml. | MaxTableKeyColumns | GENERIC_ERROR |
| Таблица | Максимальное количество индексов | 20 | Максимальное количество индексов помимо индекса первичного ключа, которые могут быть созданы на таблице. Можно настроить через `scheme_limits.max_table_indices` в config.yaml. | MaxTableIndices | GENERIC_ERROR |
| Таблица | Максимальное количество реплик | 3 | Максимальное количество read-only реплик, которое можно указать при создании таблицы с репликами. | MaxFollowersCount | GENERIC_ERROR |
...
| :--- | :--- | :--- | :---: |
| Длительность запроса | 2 часа | Максимальное время на выполнение одного запроса. | `TIMEOUT` |
