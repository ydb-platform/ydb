...
[Блум индекс](../../dev/bloom-skip-indexes.md) — особый вид локального индекса-фильтра.

В отличие от глобальных вторичных и полнотекстовых индексов, он не задаётся через `VIEW` в запросе: оптимизатор использует его для пропуска фрагментов данных без искомых значений. См. также: [Блум индексы и фильтрация](../query_execution/bloom_skip_indexes.md).
