...
Этот тип индекса реализует семантику уникального значения в колонке или наборе колонок, а также, как и другие индексы, позволяет эффективно выполнять точечные чтения по набору индексируемых колонок. {{ ydb-short-name }} использует его для выполнения дополнительных проверок, чтобы гарантировать, что каждое уникальное значение индексируемых колонок присутствует в таблице не более одного раза. Если модифицирующий запрос нарушает это ограничение, он отменяется со статусом `PRECONDITION_FAILED`. Таким образом, пользовательский код должен быть готов к обработке этого статуса.

Уникальный вторичный индекс является синхронным индексом, поэтому с точки зрения транзакционности процесс его обновления такой же, как у [синхронного вторичного индекса](#sync), описанного выше.

## Векторный индекс {#vector}
>>>>>>> c2575aeb6aa (feat: json index docs prototype (#41271))
...
[JSON-индекс](../../dev/json-indexes.md) является особым видом вторичного индекса, как и полнотекстовый — оба построены поверх [инвертированного индекса](https://ru.wikipedia.org/wiki/Инвертированный_индекс), но используют разные токенизаторы.
...
[Локальные индексы](../query_execution/local_indexes.md) — вспомогательные структуры, хранящиеся вместе с данными таблицы и применяемые при чтении на стороне хранилища. Они не материализуют отдельную индексную таблицу. Сейчас реализованы [Блум-индексы](../../dev/bloom-skip-indexes.md), в будущем планируются и другие виды.
