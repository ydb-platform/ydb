...
Отдельно существуют [локальные индексы](../glossary.md#local-index): вспомогательные структуры, которые хранятся вместе с данными таблицы и применяются при чтении на стороне хранилища, не материализуя отдельную индексную таблицу (см. раздел [Блум-индекс](#bloom-skip-index) ниже).
