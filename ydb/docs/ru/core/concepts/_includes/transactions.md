...
В текущей реализации, при чтении из [колоночных таблиц](../datamodel/table.md#column-tables) в режимах транзакции *Online Read-Only* и *Stale Read-Only* данные считываются из снапшота базы на момент начала транзакции. Таким образом, для колоночных таблиц, эти режимы фактически эквивалентны режиму Snapshot Read-Only.
