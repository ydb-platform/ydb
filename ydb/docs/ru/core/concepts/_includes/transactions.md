...
Чтения производятся из согласованного снимка данных, зафиксированного в момент выполнения первого запроса. Транзакция успешно зафиксируется только при условии, что от момента взятия снимка данных до момента коммита транзакции модифицированные ей строки не были модифицированы другими транзакциями.
...
* Все чтения в транзакции видят одно и то же состояние данных на снимке, полученном в момент выполнения первого запроса, плюс собственные изменения, сделанные ранее в этой же транзакции (read-your-own-writes);
