...
{{ ydb-short-name }} поддерживает несколько режимов выполнения транзакций. По умолчанию транзакции выполняются в режиме *Serializable*, который предоставляет самый строгий [уровень изоляции](https://en.wikipedia.org/wiki/Isolation_(database_systems)#Serializable) для пользовательских транзакций. Режим выполнения транзакции задаётся в настройках при её создании. Примеры для {{ ydb-short-name }} SDK см. в [{#T}](../../recipes/ydb-sdk/tx-control.md). Во [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md) режим транзакции также можно выбрать в настройках.
...
**Суть.** Сериализуемое исполнение пользовательских транзакций (Serializable [уровень изоляции](https://en.wikipedia.org/wiki/Isolation_(database_systems)#Serializable)).

**Особенности.** Используется механизм [Optimistic Concurrency Control](https://en.wikipedia.org/wiki/Optimistic_concurrency_control). На прочитанные в ходе выполнения транзакции строки ставятся оптимистичные блокировки. При завершении транзакции проверяется, что блокировки не были инвалидированы. Оптимистичность блокировок выливается в важное для пользователя свойство — в случае конфликта выигрывает транзакция, которая завершается первой. Конкурирующие транзакции завершатся с ошибкой `Transaction locks invalidated`.

**Гарантии.**

* Результат успешно выполненных параллельных транзакций эквивалентен некоторому последовательному порядку их выполнения;
* Для успешных транзакций нет [аномалий чтений](https://en.wikipedia.org/wiki/Isolation_(database_systems)#Read_phenomena);
* Транзакция видит все изменения, которые были зафиксированы (закоммичены) до её первого чтения (времени взятия снапшота), плюс собственные изменения, сделанные ранее в этой же транзакции (read-your-own-writes);
* Гарантируется линеаризуемость **по ключу**: если транзакция T1, затрагивающая некоторый ключ, завершилась до начала транзакции T2, затрагивающей **тот же ключ**, то в порядке выполнения T1 будет находиться перед T2. Для транзакций, работающих с разными ключами, такой порядок не гарантируется — наблюдаемый порядок их фиксации может отличаться от реального порядка их завершения во времени.

### Snapshot Read-Write {#snapshot-read-write}

**Суть.** Представляет собой изоляцию [Snapshot Isolation](https://en.wikipedia.org/wiki/Snapshot_isolation) (аналог Repeatable Read в PostgreSQL).
Чтения производятся из согласованного снимка данных, зафиксированного до момента выполнения первого чтения. Транзакция успешно зафиксируется только при условии, что от момента взятия снимка данных до момента коммита транзакции модифицированные ей строки не были модифицированы другими транзакциями.

**Особенности.** Используется механизм [Optimistic Concurrency Control](https://en.wikipedia.org/wiki/Optimistic_concurrency_control). В отличие от Serializable, транзакции не берут блокировки на читаемые строки — это позволяет им успешно зафиксироваться, даже если прочитанные ими данные были модифицированы другими транзакциями. На модифицируемые же строки берутся блокировки: при параллельных транзакциях, изменяющих те же ключи, только транзакция, которая завершается первой, завершится успешно. Конкурирующие транзакции будут отклонены с write-write конфликтом на этапе фиксации с ошибкой `Transaction locks invalidated`.

**Гарантии.**

* Все чтения в транзакции видят одно и то же состояние данных на снимке, полученном до момента выполнения первого чтения, плюс собственные изменения, сделанные ранее в этой же транзакции (read-your-own-writes);
* При наличии write-write конфликта транзакция не сможет зафиксироваться;
* Может наблюдаться аномалия [write skew](https://en.wikipedia.org/wiki/Snapshot_isolation).

### Snapshot Read-Only {#snapshot-read-only}

**Суть.** Транзакция работает с целостным снимком базы, зафиксированным до момента выполнения первого чтения. Гарантии на чтение данных такие же, как Snapshot Read-Write. Запись в данном режиме запрещена, за счет этого режим эффективнее, чем Snapshot Read-Write, когда необходимо только чтение данных.

**Особенности.** Обеспечивает максимальную актуальность данных на момент начала транзакции, но может иметь более высокую задержку ответа из‑за необходимости формирования снимка.

**Гарантии.** Все чтения в транзакции видят одно и то же состояние данных на снимке; коммиты после момента взятия снапшота не видны.

### Stale Read-Only {#stale-read-only}

**Суть.** Чтения выполняются на [репликах таблетки (шарда)](../glossary.md#tablet-follower) с возможным отставанием от [лидера таблетки (шарда)](../glossary.md#tablet-leader) (обычно доли секунды). Режим хорошо подходит для сценария чтения по ключу, когда нужны минимальные задержки. Пользователь читает закоммиченные, но возможно устаревшие данные.

**Особенности.** Низкая задержка и высокая пропускная способность за счёт чтения с реплик. Реплика для чтения выбирается обычно в той же зоне доступности, но точное расположение не гарантируется.

**Гарантии.** Согласованность данных на уровне ключа внутри **одного** `SELECT` выражения; между **разными** `SELECT` выражениями в одной транзакции согласованность **не** гарантируется.

**Ограничения.** Нет единого снимка на всю транзакцию; возможна задержка относительно данных на лидере (данные могут быть не самыми свежими). Использование реплик для чтений возможно только в случае, если все читаемые строки находятся в одном шарде. Если же чтение затрагивает несколько шардов, то чтение будет произведено из лидеров со взятием снимка аналогично Snapshot Read-Only. Чтение из [колоночных таблиц](../datamodel/table.md#column-oriented-tables) в этом режиме не поддерживается (см. предупреждение ниже). Не поддерживаются интерактивные транзакции. Дополнительные ограничения и нюансы по типам запросов см. в документации SDK для выбранного языка.
...
* Snapshot Read-Only — режим чтения из согласованного снимка.
...
* [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md) — вкладка **Query** на странице базы данных ([форма выполнения запроса](../../reference/ydb-ui/ydb-monitoring.md#tenant_scheme)), при запуске без выбора режима транзакции в настройках.
...
* [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md) — выберите режим транзакции в настройках выполнения на вкладке **Query**.
...
Ниже перечислены возможности и ограничения поддержки YQL в {{ ydb-short-name }}, на которые стоит обратить внимание:

* Допускаются multistatement transactions, то есть транзакции, состоящие из последовательности выражений YQL. При выполнении транзакции допускается взаимодействие с клиентской программой, иначе говоря, взаимодействие клиента с базой может выглядеть следующим образом: `начать транзакцию и выполнить SELECT; проанализировать результаты SELECT на клиенте; ...; выполнить UPDATE и коммит транзакции`. Каждый из запросов внутри транзакции может также содержать несколько выражений YQL. Стоит отметить, что если тело транзакции полностью сформировано до обращения к базе данных, то транзакция может обрабатываться эффективнее;
...
* Любые ошибки инвалидируют всю транзакцию целиком, а не отдельный запрос, поэтому после завершения транзакции с ошибкой, сообщающей о [временном сбое](../../reference/ydb-sdk/error_handling.md), выполнение транзакции необходимо повторять с самого начала;
* Чтения в транзакции видят все изменения данных, которые были выполнены в той же транзакции ранее;
* Все изменения, производимые в рамках транзакции, накапливаются в памяти сервера базы данных. Они не видны другим транзакциям до момента успешного завершения текущей и применяются атомарно при коммите. Описанная схема накладывает ограничение: объём изменений в рамках одной транзакции должен умещаться в оперативную память. **Деталь реализации:** если транзакция читает данные из таблицы, которую она ранее модифицировала, накопленные изменения записываются в шарды досрочно — это влияет на эффективность (см. рекомендацию ниже). Досрочно записанные данные не видны другим транзакциям и откатываются при отмене транзакции;
* Для эффективности транзакций избегайте чтения из таблиц, ранее модифицированных в этой же транзакции (read-after-write), так как это приводит к преждевременной записи данных в шарды. Для каждой таблицы выполняйте все чтения раньше модификаций.
