...
* Транзакция видит все изменения, которые были зафиксированы (закоммичены) до начала её выполнения, плюс собственные изменения, сделанные ранее в этой же транзакции (read-your-own-writes);
...
**Гарантии.** Все чтения в транзакции видят одно и то же состояние данных на снимке; коммиты после момента взятия снапшота.
...
## Особенности транзакций YDB {#yql-transactions}
...
* Для эффективности транзакций избегайте чтения собственных изменений внутри одной транзакции (read-after-write), так как это приводит к преждевременной записи данных в шарды. Для каждой таблицы выполняйте все чтения раньше модификаций.
