...
- **Data Definition Language (DDL) операторы**
  DDL-запросы (такие как `CREATE TABLE`, `DROP TABLE` и т.д.) всегда выполняются вне транзакционного контекста. Запросы, состоящие из нескольких операторов, поддерживаются только в том случае, если все они являются DDL. При возникновении ошибки изменения, внесенные предыдущими операторами в пакете, не отменяются (не откатываются).

- **Data Manipulation Language (DML) инструкции**
  DML-запросы (такие как `INSERT`, `UPDATE`, `DELETE` и т.д.) автоматически выполняются внутри транзакции с уровнем изоляции Snapshot (для запросов только на чтение) или Serializable (для запросов на чтение-запись). Такая транзакция создается и управляется сервером. Поддерживаются запросы из нескольких инструкций, если все они являются DML. При успешном выполнении изменения коммитятся (фиксируются); в случае ошибки все изменения откатываются.
