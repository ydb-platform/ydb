...
Ниже описано поведение **сервера** при выполнении **одного** YQL-скрипта, когда клиент **не задаёт явно** один из [режимов транзакции](#modes) в настройках запроса и не использует сценарий с явным управлением транзакцией (см. [установку режима в {{ ydb-short-name }} SDK](../../recipes/ydb-sdk/tx-control.md)). Типичные точки входа, где запрос уходит на сервер «как есть»:

* [Встроенный UI](../../reference/embedded-ui/index.md) — вкладка **Query** на странице базы данных ([форма выполнения запроса](../../reference/embedded-ui/ydb-monitoring.md#tenant_scheme)); режим транзакции можно выбрать в настройках выполнения; если выполняется обычный сценарий без отдельной явной транзакции, применяется логика неявной транзакции.
* [{{ ydb-short-name }} CLI](../../reference/ydb-cli/index.md) — разовая отправка скрипта командой [`ydb sql`](../../reference/ydb-cli/sql.md). У подкоманды [`table query execute`](../../reference/ydb-cli/table-query-execute.md) для запросов типа `data` режим задаётся параметром [`--tx-mode`](../../reference/ydb-cli/table-query-execute.md#options): по умолчанию используется значение `serializable-rw`, то есть режим *Serializable* в терминах [режимов транзакции](#modes). Это явная настройка на стороне CLI, её не следует путать с описанной ниже серверной классификацией DDL/DML для неявной транзакции.
* Приложения на [{{ ydb-short-name }} SDK](../../reference/ydb-sdk/index.md) — режим [ImplicitTx](../../recipes/ydb-sdk/tx-control.md#implicittx).

Если для запроса в таком сценарии не задан [режим транзакции](../transactions.md#modes) на стороне клиента, {{ ydb-short-name }} автоматически управляет его поведением. Такой режим называется **неявной транзакцией**.
