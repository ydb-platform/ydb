...
Ниже описано поведение **сервера** при выполнении **одного** YQL-скрипта, когда клиент **не задаёт явно** один из [режимов транзакции](#modes) в настройках запроса и не использует сценарий с явным управлением транзакцией (см. [установку режима в {{ ydb-short-name }} SDK](../../recipes/ydb-sdk/tx-control.md)). Типичные точки входа, где запрос отправляется на сервер как единый скрипт без явного управления транзакцией:
