...
**Суть.** Чтение и запись поверх одного консистентного снимка данных; по смыслу совпадает с [Snapshot Isolation](https://en.wikipedia.org/wiki/Snapshot_isolation) (Repeatable Read в PostgreSQL). В режиме Snapshot Read-Write транзакции не берут блокировки на чтениях, что позволяет им успешно закоммититься в случае, если прочитанные ими данные были модифицированы другими транзакциями.

**Гарантии.**

* Все чтения в транзакции видят одно и то же состояние данных на снимке, полученном в момент старта транзакции;
* При наличии write-write конфликта транзакция не сможет закоммититься;
...
**Суть.** Чтения выполняются на локальных [репликах таблетки](../glossary.md#tablet-follower) с возможным отставанием от [лидера таблетки](../glossary.md#tablet-leader) (обычно доли секунды). Режим хорошо подходит для сценария чтения по ключу, когда нужны минимальные задержки. Пользователь читает закоммиченные, но возможно устаревшие данные.
...
* Чтения в транзакции видят все изменения данных, которые были выполненны в той же транзакции ранее.
* Все изменения, производимые в рамках транзакции, накапливаются в памяти сервера базы данных и записываются в таблетки либо в момент завершения транзакции, либо в случае чтения транзакцией таблиц, которые были изменены в рамках транзакции ранее (изменения не видны другим транзакциям до момента успешного завершения транзакции). Описанная схема накладывает некоторые ограничения: объем изменений, осуществляемых в рамках одной транзакции, должен умещаться в оперативную память.
* Для наиболее эффективного выполнения транзакций следует формировать их таким образом, чтобы избегать чтения собственных изменений транзакции, приводящего к преждевременной записи данных в таблетки. То есть для каждой таблицы стоит выполнять чтения до модификаций. 
