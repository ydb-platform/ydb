...
* Гарантируется линеаризуемость по ключу: если транзакция T1, затрагивающая некоторый ключ, завершилась до начала транзакции T2, которая затрагивает тот же ключ, то в порядке выполнения транзакция T1 будет находиться перед T2.
...
**Поведение.** Транзакции не берут блокировки на чтениях — это позволяет им успешно зафиксироваться даже если прочитанные ими данные были модифицированы другими транзакциями.

**Гарантии.**

* Все чтения в транзакции видят одно и то же состояние данных на снимке, полученном в момент старта транзакции;
...
**Суть.** Чтения выполняются на локальных (расположенных на узле-обработчике запроса) [репликах таблетки](../glossary.md#tablet-follower) с возможным отставанием от [лидера таблетки](../glossary.md#tablet-leader) (обычно доли секунды). Режим хорошо подходит для сценария чтения по ключу, когда нужны минимальные задержки. Пользователь читает закоммиченные, но возможно устаревшие данные.
