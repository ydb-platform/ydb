...
YDB поддерживает транзакции с участием как таблиц, так и топиков. В рамках такой транзакции можно производить операции чтения и записи как с таблицами, так и с топиками. При этом соблюдаются ACID-принипы работы транзакций. В одной транзакции могут участвовать несколько таблиц и несколько топиков. Все топики и таблицы, с которыми требуется работать в рамках одной транзакции, должны быть расположены внутри одной БД.

Если в транзакции участвует топик, то такой транзакции автоматически присваивается уровень *Serializable*.

Подробнее о транзакционных операциях при работе с топиками см. [Транзакции с учаcтием топиков] (../topic.md/topic-transactions).

Транзакции с участием топиков поддержаны в C++ SDK и Java SDK (см. [Работа с топиками в SDK] (../reference/ydb-sdk/topic.md)).
