...
Особенность работы с колоночными таблицами: при чтении из колоночных таблиц в режимах Online Read-Only и Stale Read-Only данные всё равно считываются из снапшота, сделанного в момент старта транзакции — как и в режиме Snapshot Read-Only.

Это означает, что:

* Для колоночных таблиц все три режима Read-Only ведут себя одинаково — все чтения консистентны и видят состояние базы на момент начала транзакции.
* Реальный режим транзакции (например, Online Read-Only) игнорируется, если транзакция работает только с колоночными таблицами
* Для транзакций читающих одновременно из строчных и колоночных таблиц, чтение из строчных таблиц будет проводиться согласно выбранному режиму, а из колоночных - всегда из снепшота.

Такое поведение обусловлено особенностями текущей реализации колоночных таблиц и будет изменено в будущих версиях.
