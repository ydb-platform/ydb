...
{{ ydb-short-name }} поддерживает транзакции с одновременным участием таблиц и топиков. В рамках такой транзакции можно производить операции чтения и записи как с таблицами, так и с топиками. При этом соблюдаются ACID-принципы работы транзакций. В одной транзакции могут участвовать несколько таблиц и несколько топиков. Операции с топиками в транзакции всегда выполняются с уровнем *Serializable*. Все топики и таблицы, с которыми требуется работать в рамках одной транзакции, должны быть расположены внутри одной БД.
