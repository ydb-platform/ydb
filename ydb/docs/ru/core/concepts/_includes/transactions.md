...
По умолчанию транзакции в {{ ydb-short-name }} выполняются в режиме *Serializable*. Режим выполнения транзакции задаётся в настройках при её создании. Примеры для {{ ydb-short-name }} SDK см. в [{#T}](../../recipes/ydb-sdk/tx-control.md).

Поддерживаются **режимы чтения-записи**: *Serializable* (по умолчанию) и *Snapshot Read-Write*; **режимы только для чтения**: *Snapshot Read-Only* и *Stale Read-Only*. Для режимов только чтения характерен компромисс между **консистентностью** (целостностью данных в рамках транзакции), **свежестью** чтений и **задержкой** ответа. Режим *Online Read-Only* оставлен для совместимости со старым кодом (**legacy**); в новых сценариях вместо него следует использовать *Snapshot Read-Only*.
...
**Типичные сценарии.** Любые транзакции с записью и случаи, где нужны самые строгие гарантии.
...
**Типичные сценарии.** Нужны и изменение данных, и согласованное чтение на всю транзакцию.

**Режимы только для чтения**

### Snapshot Read-Only {#snapshot-read-only}

**Суть.** Транзакция работает с целостным снимком базы на момент её старта.

**Гарантии.** Полная консистентность на всю транзакцию: все `SELECT` видят одно и то же состояние; коммиты после старта транзакции не видны.

**Типичные сценарии.** Отчёты, ETL, бэкапы, аналитика — везде, где нужна целостная картина на фиксированный момент времени без записи.

### Stale Read-Only {#stale-read-only}

**Суть.** Чтения выполняются из реплик с возможным отставанием от лидера (обычно доли секунды).

**Гарантии.** Низкая задержка и высокая пропускная способность за счёт чтения с реплик. Внутри **одного** запроса данные согласованы; между **разными** запросами в одной транзакции согласованность **не** гарантируется.

**Ограничения.** Нет единого снимка на всю транзакцию; согласованность между несколькими `SELECT` не гарантируется; возможна задержка относительно данных на лидере. Чтение из [колоночных таблиц](../datamodel/table.md#column-oriented-tables) в этом режиме не поддерживается (см. предупреждение ниже). Дополнительные ограничения и нюансы по типам запросов см. в документации SDK для выбранного языка.

**Типичные сценарии.** Высоконагруженное чтение, где допустима репликационная задержка: ленты, счётчики, топы и т.п.
