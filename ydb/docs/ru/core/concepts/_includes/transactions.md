...
{{ ydb-short-name }} поддерживает несколько режимов выполнения транзакций. По умолчанию транзакции выполняются в режиме *Serializable*. Режим выполнения транзакции задаётся в настройках при её создании. Примеры для {{ ydb-short-name }} SDK см. в [{#T}](../../recipes/ydb-sdk/tx-control.md). Во [встроенном UI](../../reference/embedded-ui/index.md) режим транзакции также можно выбрать в настройках.
