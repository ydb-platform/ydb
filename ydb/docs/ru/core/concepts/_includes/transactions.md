...
- **Инструкции Data Manipulation Language (DML)**
  DML-запросы (такие как `UPSERT`, `INSERT`, `UPDATE`, `DELETE` и т.д.) автоматически выполняются внутри транзакции с уровнем изоляции Snapshot (для запросов только на чтение) или Serializable (для запросов на чтение-запись). Такая транзакция создается и управляется сервером. Поддерживаются запросы из нескольких инструкций, если все они являются DML. При успешном выполнении изменения коммитятся (фиксируются). При возникновении ошибки все изменения откатываются.
