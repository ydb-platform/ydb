...
* Для вытеснения данных в приватное S3-совместимое хранилище настройте внешний источник данных с `AUTH_METHOD="AWS"` и используйте ссылки на схемные объекты секретов через `AWS_ACCESS_KEY_ID_SECRET_NAME` и `AWS_SECRET_ACCESS_KEY_SECRET_NAME`.
