...
Apache Spark — один из самых популярных инструментов для таких задач, для которого разработан [специализированный коннектор](../../../integrations/query-engines/spark.md) к {{ydb-short-name}}. Если в вашей компании используются другие схожие решения (например, Apache Flink), их также можно задействовать для построения ETL-процессов за поддержки [JDBC-драйвера](../../../reference/languages-and-apis/jdbc-driver/index.md).
