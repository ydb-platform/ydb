...
2. Сжатие данных: колоночная организация хранения позволяет применять сжатие и кодирование данных индивидуально для каждой колонки, что позволяет сжать данные лучше, чем разнородные данные строки. По умолчанию используется алгоритм сжатия `LZ4` без специального кодирования, однако обе настройки могут быть заданы отдельно для каждой колонки. Подробнее в разделе [Параметры запроса CREATE TABLE](../../../yql/reference/syntax/create_table/index.md#parametry-zaprosa).
