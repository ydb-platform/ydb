...
{{ ydb-short-name }} — это горизонтально масштабируемая, распределённая и отказоустойчивая СУБД, разработанная для высокой производительности и работы с объёмами данных до сотен петабайт. Система поддерживает как однодатацентровые, так и геораспределённые (кросс-датацентровые) конфигурации на кластерах из тысяч серверов. Типичный узел кластера способен обрабатывать десятки тысяч запросов в секунду.
...
{{ ydb-short-name }} — это универсальная платформа, подходящая для широкого спектра задач, где требуются масштабируемость, надёжность и гибкость. {{ ydb-short-name }} может выступать как самостоятельная СУБД или как часть распределённой архитектуры. Основные сценарии:

- При использовании NoSQL-систем, когда требуется **строгая консистентность или поддержка транзакций**, включая изменение данных в нескольких строках и таблицах. {{ ydb-short-name }} сочетает масштабируемость NoSQL с возможностями реляционной СУБД по управлению согласованностью и целостностью данных.
...
- В системах с низкой или непредсказуемой нагрузкой — за счёт **режима бессерверных вычислений** и возможности динамического масштабирования, включая автоматическое сжатие ресурсов до минимально необходимых.
