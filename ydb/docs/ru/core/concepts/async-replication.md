...
В данном режиме потоки изменений создаются с включенными [барьерами](cdc.md#barriers). Между барьерами данные буферизируются непосредственно на партициях объектов-реплик. При получении очередной порции барьеров от всех топиков записанные изменения фиксируются и становятся доступными для чтения.
