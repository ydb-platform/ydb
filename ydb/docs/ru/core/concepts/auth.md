...
{{ ydb-short-name }} будет делать попытку получить группы на всех уровнях вложенности, не только те, в которые пользователь входит непосредственно. Если работа {{ ydb-short-name }} настроена против Active Directory, то для поиска всех вложенных групп будет использовано, специфичное для Active Directory правило соответствия, [LDAP_MATCHING_RULE_IN_CHAIN](https://learn.microsoft.com/en-us/windows/win32/adsi/search-filter-syntax?redirectedfrom=MSDN). Такое правило соответствия позволяет получить все вложенные группы одним запросом. Для LDAP серверов на основе OpenLdap, чтобы найти группы, будет выполнен рекурсивный обход графа, в общем случае, делающий больше одного запроса. Как для Active Directory так и для OpenLdap поиск групп будет выполнен только для поддерева, корень которого берется из параметра конфигурации **base_dn**.
...
По умолчанию {{ ydb-short-name }} устанавливает незашифрованное соединение с LDAP сервером. Это означает, что все данные, которые передаются в запросах к LDAP серверу, будут передаваться в открытом виде, в том числе и пароли. Таким способом соединения проще начать пользоваться, но он не безопасен для сохранения информации о паролях. Поэтому в {{ ydb-short-name }} поддержана возможность устанавливать защищенное соединение с LDAP сервером по протоколу TLS. Процесс инициации сессии TLS может происходить одним из двух способов:
* Автоматически. Используется схема соединения `ldaps`
* Явно выполнив запрос `StartTls`, после которого будет установлено зашифрованное соединение

#### LDAPS

Для того чтобы {{ ydb-short-name }} автоматически установила зашифрованное соединение с LDAP сервером необходимо в [параметре конфигурации](../deploy/configuration/config.md#ldap-auth-config) **scheme** выставить значение `ldaps`. TLS диалог будет инициирован на порту, прописанному в конфигурации. Если порт не указан, то для `ldaps` схемы будет применен порт по умолчанию 636. LDAP сервер должен быть настроен на прием соединения на указанных портах.

#### Запрос `StartTls`

Другим способом установления защищенного соединения является выполнение запроса `StartTls`. Такой прием позволяет настроить TLS соединение на порту, который может использоваться и без шифрования. Для включения этого метода соединения необходимо заполнить [секцию](../deploy/configuration/config.md#ldap-auth-config) **use_tls**  в файле конфигурации.
