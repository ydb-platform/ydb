...
#### Преобразование незашифрованного соединения в зашифрованные запросом `StartTls`

Другим способом установления защищенного соединения является выполнение клиентом запроса `StartTls` к LDAP серверу. Такой прием позволяет настроить TLS соединение на порту, который может использоваться и без шифрования. Такой запрос выполняется {{ ydb-short-name }} самостоятельно при включении параметров в [секции](../deploy/configuration/config.md#ldap-auth-config) **use_tls**  в файле конфигурации.
