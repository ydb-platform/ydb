...
- **Гибкость восстановления** — позволяет восстановление на любой момент времени, в который была сделана одна из резервных копий в коллекции.
...
Снимок в определенный момент времени. Служит основой для последующих инкрементальных резервных копий и содержит все данные, необходимые для независимого восстановления.

### Инкрементальная резервная копия {#incremental-backup}

Захватывает только изменения (вставки, обновления, удаления) с момента предыдущей резервной копии в цепочке. Значительно меньше полных резервных копий при небольшой частоте изменений.
...
Создает согласованный снимок всех объектов в коллекции в определенный момент времени. Эта базовая резервная копия содержит все данные и может быть восстановлена независимо без необходимости в других резервных копиях. Полная резервная копия требуется перед созданием любых инкрементальных резервных копий.
...
Захватывает только данные, которые изменились с момента предыдущей резервной копии, что значительно снижает требования к хранилищу. **Важно**: Инкрементальные копии необходимо запускать вручную или использовать внешнюю систему планирования. Для восстановления из инкрементальной резервной копии нужна вся цепочка резервных копий, начиная с начальной полной резервной копии.

[См. руководство по операциям](../maintenance/manual/backup-collections.md#create-backup)

### Экспорт/дамп (опционально) {#export-dump}

Если резервные копии хранятся во внешнем хранилище, выгрузите их туда перед удалением коллекции. Это обеспечивает долгосрочное хранение и возможность восстановления за пределами {{ ydb-short-name }}.

### Удаление коллекции {#backup-collection-cleanup}

Когда коллекция резервных копий больше не нужна, удалите её, чтобы убрать все связанные метаданные и освободить ресурсы. Это безвозвратно удаляет коллекцию и все её резервные копии. Удаляйте коллекцию только после того, как убедитесь, что резервные копии вам больше не нужны и/или они выгружены во внешнее хранилище.
...
Если ваши резервные копии хранятся во внешнем хранилище (файловой системе или объектном хранилище), импортируйте их обратно в кластер с помощью операций импорта {{ ydb-short-name }} CLI. Вам потребуются все резервные копии в цепочке: полная резервная копия и все инкрементальные резервные копии до нужной точки восстановления. Этот шаг необходим только при восстановлении из внешнего хранилища.

[См. экспорт/импорт {{ ydb-short-name }} CLI](../reference/ydb-cli/export-import/index.md)
...
## Эксплуатация и обслуживание {#operational-guidance}
...
Для перемещения резервных копий во внешнее хранилище (файловую систему или S3-совместимое хранилище), загрузите их обратно в кластер с помощью совместимых команд (`ydb export` или `ydb tools dump`) CLI (`ydb import` или `ydb tools restore`). Загрузите все резервные копии в цепочке, начиная с полной резервной копии, за которой следуют инкрементальные резервные копии. Этот шаг необходим только при восстановлении из внешнего хранилища.

[См. экспорт/импорт {{ ydb-short-name }} CLI](../reference/ydb-cli/export-import/index.md) | [См. руководство по мониторингу](../maintenance/manual/backup-collections.md#monitoring) | [См. процедуры очистки](../maintenance/manual/backup-collections.md#cleanup)
...
Резервные копии хранятся в том же кластере {{ ydb-short-name }} с использованием распределенного файлового хранения:
...
- Отслеживать прогресс с помощью команд операций {{ ydb-short-name }} CLI.
...
2. **Отслеживание изменений**: Для инкрементальных резервных копий захватываются только изменения с последней резервной копии и сохраняются в [поток изменений](cdc.md).
3. **Материализация изменений**: При вызове инкрементального резервного копирования поток изменений компактифицируется в таблицы инкрементальной резервной копии.
...
- **Валидацию**: Полнота цепочки проверяется перед операциями.
...
- Простых сценариев резервного копирования без необходимости в инкрементах.
...
- **Нет модификации коллекций**: Нельзя добавлять/удалять таблицы после создания коллекции.
- **Нет частичного восстановления**: Частичное восстановление из коллекций должно управляться извне.
...
- [Общие концепции резервного копирования](backup.md) - Обзор всех подходов резервного копирования в {{ ydb-short-name }}.
