...
Именованный контейнер, который группирует резервные копии для определенного набора объектов базы данных. Коллекции обеспечивают согласованное резервное копирование всех включенных объектов.

### Полная резервная копия {#full-backup}

Полный снимок всех выбранных объектов в определенный момент времени. Служит основой для последующих инкрементальных резервных копий и содержит все данные, необходимые для независимого восстановления.
...
Если ваши резервные копии хранятся во внешнем хранилище (файловой системе или объектном хранилище), импортируйте их обратно в кластер с помощью операций импорта {{ ydb-short-name }} CLI. Импортируйте все резервные копии в цепочке в правильном порядке, начиная с полной резервной копии, за которой следуют инкрементальные резервные копии. Этот шаг необходим только при восстановлении из внешнего хранилища.
