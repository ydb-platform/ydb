...
[Коллекция резервных копий](datamodel/backup-collection.md) — это механизм, который объединяет набор таблиц и управляет цепочкой их полных и инкрементальных резервных копий. Коллекции хранятся внутри кластера и могут быть [экспортированы во внешнее хранилище](datamodel/backup-collection.md#external-storage) для защиты от потери кластера.

Подробнее об архитектуре, ограничениях, операциях и внутреннем устройстве коллекций см. в разделе [Коллекции резервных копий](datamodel/backup-collection.md).
