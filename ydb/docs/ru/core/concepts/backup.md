...
|| Тип хранилища | Тип резервной копии | Сценарии использования | Как использовать |
||---------------|---------------------|------------------------|------------------|
...
|| **Локально в базе данных** | Полные снимки выбранных объектов, инкрементальные резервные копии выбранных объектов | Производственные среды, большие наборы данных | Требуется [коллекции резервных копий](backup-collections.md) |
...
|| Тип резервной копии | Тип хранилища | Сценарии использования | Как использовать | Ограничения |
||---------------------|---------------|------------------------|------------------|-------------|
...
|| **Инкрементальное резервное копирование** | Локально в базе данных, s3-совместимое хранилище\*, файловая система\* | Производственные среды, большие наборы данных, регулярные расписания резервного копирования | Требуется [коллекции резервных копий](backup-collections.md) | Поддерживаются только строковые таблицы |

Инкрементальные резервные копии более эффективны с точки зрения использования хранилища по сравнению с созданием нескольких полных резервных копий, поскольку они захватывают только изменения с момента предыдущей резервной копии, а не дублируют все данные.

\* Чтобы сохранить цепочку инкрементальных резервных копий в s3-совместимое хранилище или файловую систему, необходимо сначала создать коллекцию резервных копий локально, а затем выгрузить её содержимое в S3-совместимое хранилище или файловую систему.

## Коллекции резервных копий {#backup-collections}

Коллекции резервных копий предоставляют способ организации полных и инкрементальных резервных копий структурированным образом, обеспечивая эффективное восстановление на определенный момент времени для производственных нагрузок. Коллекции хранятся в выделенной структуре директорий внутри базы данных, которую можно выгружать во внешнее хранилище с помощью операций [export/import](../reference/ydb-cli/export-import/index.md) или dump/restore. Подробнее см. [Бэкенды хранения](backup-collections.md#storage-backends).
