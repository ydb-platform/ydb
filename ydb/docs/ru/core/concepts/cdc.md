...
* `user` — идентификатор пользователя. Присутствует, если включена настройка `USER_SIDS` . Содержит `SID` пользователя. Если происходит удаление записей по TTL, то указывается значение `ttl@system`.
* `traceId` — идентификатор трассировки open telemetry. Присутствует, если включена настройка `TRACE_IDS`. Содержит внешний `trace_id`.
...
* `userIdentity` — информация о пользователе (вложенное значение `principalId` содержит `SID` пользователя, если происходит удаление записей по TTL, то указывается значение `dynamodb.amazonaws.com`.).
