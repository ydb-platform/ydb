...
* `user` — идентификатор пользователя. Присутствует, если включена настройка `USER_SIDS`. Содержит [SID](glossary.md#sid-access-sid) пользователя. Если происходит удаление записей по TTL, то указывается значение `ttl@system`.
...
* `userIdentity` — информация о пользователе (вложенное значение `principalId` содержит [SID](glossary.md#sid-access-sid) пользователя, если происходит удаление записей по TTL, то указывается значение `dynamodb.amazonaws.com`.).
