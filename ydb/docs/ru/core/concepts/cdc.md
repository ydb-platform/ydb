...
1. Глобального времени [координатора](../concepts/glossary.md#coordinator).
1. Уникального [идентификатора транзакции](../concepts/glossary.md#txid).
...
Для [одношардовых транзакций](../concepts/glossary.md#transactions) может быть использована метка времени существующей распределённой транзакции в очереди выполнения, что позволяет метке времени быть неуникальной. Если в очереди выполнения нет распределённых транзакций, то первым элементом метки времени будет время, полученное от координатора, а уникальный идентификатор транзакции примет максимальное возможное значение `18446744073709551615` (2<sup>64</sup>-1, максимальное значение `Uint64`).
...
* `user` — идентификатор пользователя. Присутствует, если включена настройка `USER_SIDS`. Содержит [SID](glossary.md#sid-access-sid) пользователя. Если происходит удаление записей по [TTL](ttl.md), то указывается значение `ttl@system`.
* `traceId` — идентификатор [трассировки OpenTelemetry](../reference/observability/tracing/external-traces.md). Присутствует, если включена настройка `TRACE_IDS`.
...
* `userIdentity` — информация о пользователе. Присутствует, если включена настройка `USER_SIDS`. Имеет вложенные поля `type` (содержит `"User"` или `"Service"`) и `principalId` (содержит [SID](glossary.md#sid-access-sid) пользователя). Если происходит удаление записей по [TTL](ttl.md), то поле `type` содержит `"Service"`, а поле `principalId` содержит `"dynamodb.amazonaws.com"`.
...
Для настройки времени хранения записей укажите параметр [RETENTION_PERIOD](../yql/reference/syntax/alter_table/changefeed.md) при создании потока изменений. Данные хранятся в течение всего указанного периода независимо от того, были ли они прочитаны потребителями.
