...
Change Data Capture (CDC) обеспечивает захват изменений строк строковой таблицы {{ ydb-short-name }}, формирует из них *поток изменений (changefeed)*, записывает в распределенное хранилище и предоставляет доступ к этим записям для дальнейшей обработки. В качестве распределенного хранилища используется [топик](topic.md), который позволяет эффективно хранить лог изменений таблицы.
...
## Метки-барьеры {#resolved-timestamps}

Метки-барьеры — записи "пустышки" (без данных об изменении или удалении) с [виртуальными метками времени](#virtual-timestamps), которые появляются в каждой партиции топика с заданной периодичностью. Метка-барьер — это гарантия, что любое изменение с виртуальной меткой времени меньшей, чем у барьера, было записано в данную партицию топика.

Метки-барьеры могут быть использованы для обеспечения строгой упорядоченности и глобальной согласованности данных путём буферизации данных между метками.

{% note info %}

По умолчанию метки-барьеры не выгружаются в поток изменений. Для их включения используйте [соответствующий параметр](../yql/reference/syntax/alter_table/changefeed.md) при создании потока.
...
На время первоначального сканирования в таблице приостанавливаются процессы [автоматического партиционирования](datamodel/table.md#partitioning_row_table) и в топик не выгружаются [метки-барьеры](#resolved-timestamps).
...
Запись с меткой-барьером содержит единственное поле `resolved` с виртуальной меткой времени:
