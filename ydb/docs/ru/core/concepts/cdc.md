...
* В потоке изменений поддерживаются записи о следующих видах операций:
  ...
  * обновление — перезапись значений указанных столбцов;
  * замена — перезапись значений указанных столбцов, значения неуказанных столбцов заменяются на значения по умолчанию;
  * удаление.
...
* В режиме `UPDATES` для операций обновления или замены поля `update` и `reset` выполняют роль не только признака операции, но и содержит названия и значения изменившихся столбцов.
