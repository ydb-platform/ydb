...
Барьеры — записи "пустышки" (без данных об изменении или удалении) с [виртуальными метками времени](#virtual-timestamps), которые появляются в каждой партиции топика с заданной периодичностью. Барьер — это гарантия, что любое изменение с виртуальной меткой времени меньшей, чем у барьера, было записано в данную партицию топика.
