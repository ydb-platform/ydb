...
Для одношардовых транзакций, которые не проходят через координатор, время назначается с медиатора, а идентификатор транзакции не назначается и принимает максимальное значение `18446744073709551615` (2<sup>64</sup>-1, максимальное значение `Uint64`). Реальный идентификатор присутствует только у распределённых транзакций, планируемых через координатор.
