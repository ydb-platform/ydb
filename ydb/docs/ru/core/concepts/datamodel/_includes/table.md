...
При старте данные групп колонок с режимом `in_memory` предзагружаются в кэш. Данные групп колонок с режимом `regular` попадают в кэш при первом обращении. При превышении [лимита](../../../reference/configuration/memory_controller_config.md#cache-memory-limits) выделенной памяти сначала вытесняются страницы из `regular`, если этого недостаточно — из `in_memory`. Таким образом, режим `in_memory` предоставляет приоритетное размещение в памяти, но не приводит к её неограниченному потреблению.
