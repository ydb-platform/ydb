...
Для групп колонок можно задавать режимы кэширования. Режимы кэширования в {{ ydb-short-name }} позволяют управлять стратегией хранения данных таблиц в оперативной памяти. Это особенно важно для оптимизации времени ответа при чтении небольших часто используемых таблиц. Поддержаны два режима кэширования:

* `regular` (значение по умолчанию);
* `in_memory`.

При старте [таблетки Data shard](../../../concepts/glossary.md#data-shard) данные групп колонок с режимом `in_memory` предзагружаются в [общий кэш](../../../concepts/glossary.md#shared-cache). Данные групп колонок с режимом `regular` попадают в общий кэш при первом обращении. При превышении [лимита](../../../reference/configuration/memory_controller_config.md#cache-memory-limits) выделенной памяти сначала вытесняются страницы из `regular`, если этого недостаточно — из `in_memory`. Таким образом, режим `in_memory` предоставляет приоритетное размещение в памяти, но не приводит к её неограниченному потреблению.

Рекомендуется использовать режим кэширования `in_memory` для небольших таблиц-справочников, которые полностью помещаются в оперативную память. Это оптимально для ситуаций, когда необходимо обеспечить минимальное время отклика при доступе к данным даже при редких запросах — например, для "lookup-таблиц", содержащих справочную информацию.
