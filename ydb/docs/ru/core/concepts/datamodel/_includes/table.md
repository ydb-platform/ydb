...
Желаемый порог размера партиции. Несмотря на суффикс `_MB`, размер задаётся в мебибайтах (МиБ). Рекомендуемые значения варьируются от `10 МиБ` до `2 ГиБ`. Если этот порог превышается, партиция может быть разделена.
