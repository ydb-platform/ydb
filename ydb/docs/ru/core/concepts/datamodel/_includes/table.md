...
Желаемый порог размера партиции в мебибайтах. Рекомендуемые значения варьируются от `10 МиБ` до `2048 МиБ`. Если этот порог превышается, партиция может быть разделена.
