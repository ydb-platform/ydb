...
Желаемый порог размера партиции в мебибайтах. Рекомендуемые значения варьируются от `10 MiB` до `2048 MiB`. Если этот порог превышается, партиция может быть разделена.
