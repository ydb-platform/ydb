...
* Тип: `Uint64`.
* Значение по умолчанию: `2048 МиБ` ( `2 ГиБ` ).

Желаемый порог размера партиции в мебибайтах. Рекомендуемые значения варьируются от `10 МиБ` до `2000 МиБ`. Если этот порог превышается, партиция может быть разделена.
