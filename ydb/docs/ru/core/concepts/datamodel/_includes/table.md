...
Используя режим `in_memory`, помните, что выделенная на такие таблицы память уменьшает объём, доступный для кэша обычных таблиц, что может негативно сказаться на их производительности (увеличить latency). Поэтому рекомендуется включать этот режим только для действительно небольших и часто используемых (или критичных по времени доступа) таблиц.
