...
Для групп колонок можно задавать режимы кэширования. Режимы кэширования в {{ ydb-short-name }} позволяют управлять стратегией хранения данных таблиц в оперативной памяти. Это особенно важно для оптимизации времени отклика при чтении небольших часто используемых таблиц. Поддержаны два режима кэширования:

* `in_memory`;
* `regular` (значение по умолчанию).

При старте данные групп колонок с режимом  `in_memory` предзагружаются в кэш. Данные групп колонок с режимом `regular` попадают в кэш при первом обращении. При превышении лимита выделенной памяти сначала вытесняются страницы из `regular`, если этого недостаточно — из `in_memory`. Таким образом, режим `in_memory` предоставляет приоритетное размещение в памяти, но не приводит к её неограниченному потреблению.
