...
Независимо от значения параметра [AUTO_PARTITIONING_PARTITION_SIZE_MB](#auto_partitioning_by_size) {{ ydb-short-name }} делает разделение и объединение шардов по размеру в 2 GB. Причем {{ ydb-short-name }} не ограничивает пользовательские настройки, а работает одновременно с ними. Например, если задать [размер шарда](#auto_partitioning_by_size) 100MB и [ограничение](#auto_partitioning_max_partitions_count) в 10 шардов, то когда у таблицы станет 10 шардов по 100MB, она перестанет делиться и размер шардов начнет расти в размерах. Когда какой-то шард превысит размер в 2GB, он разделится и в таблице станет 11 шардов. При уменьшений размера двух соседних шардов до 1GB, то они объединятся и снова в таблице станет 10 шардов.
