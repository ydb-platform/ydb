...
# Правила наименования колонок (столбцов) {#column-naming-rules}
...
- Если [увеличилась нагрузка](#auto_partitioning_by_load), то партиция сначала собирает сэмпл запрашиваемых ключей (читаемых, записываемых и удаляемых) и на основании этого сэмпла выбирает для разделения такой ключ, чтобы нагрузка распределилась поровну между новыми партициями. Таким образом в случае разделения по нагрузке новые партиции могут иметь существенно отличающийся размер.
...
Независимо от значения параметра [AUTO_PARTITIONING_PARTITION_SIZE_MB](#auto_partitioning_by_size), {{ ydb-short-name }} выполняет разделение и объединение партиций, ориентируясь на размер в 2048 МиБ. При этом {{ ydb-short-name }} не ограничивает пользовательские настройки, а работает параллельно с ними. Например, если задать [размер партиции](#auto_partitioning_by_size) 100 МиБ и [ограничение](#auto_partitioning_max_partitions_count) на 10 партиций, то, когда у таблицы станет 10 партиций по 100 МиБ, она перестанет делиться, и размер партиций начнет увеличиваться. Когда размер какой-либо партиции превысит 2048 МиБ, она разделится, и в таблице станет 11 партиций. Если размер двух соседних партиций уменьшится до 1024 МиБ, они объединятся, и в таблице снова станет 10 партиций.
...
Режим автоматического партиционирования по нагрузке. Если в течение нескольких десятков секунд партиция потребляет более 50% CPU, то она ставится в очередь на разделение (split). Если в течение часа суммарная нагрузка на две или более соседних партиции составляет менее 35% одного ядра CPU, то они ставятся в очередь на объединение (merge).

При принятии решения о разделении партиции по нагрузке или об объединении нескольких партиций по нагрузке, {{ ydb-short-name }} рассматривает CPU нагрузку и на лидера данной партиции, и на все её реплики.
...
* Тип: `Uint64`.
* Значение по умолчанию: `2048` (2 ГиБ).

Желаемый порог размера партиции. Несмотря на суффикс `_MB`, размер задаётся в мебибайтах (МиБ). Рекомендуемые значения варьируются от `10 МиБ` до `2000 МиБ`. Если этот порог превышается, партиция может быть разделена.
...
Применение чтения с реплик дает следующие возможности:
...
* Обслуживать читающие запросы с реплик без влияния на модифицирующие запросы, выполняющиеся на партиции. Это может быть полезно как для изоляции разных сценариев, так и для увеличения пропускной способности партиции.
...
В схеме данных таблицы можно указать необходимость запуска реплик для чтения для каждой партиции таблицы. Обращения к репликам для чтения типично происходят не покидая сети датацентра, что позволяет обеспечить время ответа в единицы миллисекунд:

| Имя параметра | Описание | Тип | Допустимые значения | Возможность<br/>изменения | Возможность<br/>сброса |
| ------------- | --------- | --- | ------------------- | --------------------- | ------------------ |
| `READ_REPLICAS_SETTINGS` | `PER_AZ` означает использование указанного количества реплик в каждом AZ, `ANY_AZ` — во всех AZ суммарно. | String | `"PER_AZ:<count>"`, `"ANY_AZ:<count>"` где `<count>` — число реплик. Чтобы удалить реплики, установите значение 0. Например: "PER_AZ:0". | Да | Нет |

Внутреннее состояние каждой реплики восстанавливается в точности по состоянию лидера и полностью консистентно.

Кроме состояния данных в сторадже на реплики отправляется и поток обновлений с лидера. Обновления отправляются в реальном времени, сразу после коммита в лог. Но отправляются асинхронно, что приводит к некоторой задержке применения обновлений на репликах относительно их коммита на лидере (типично в десятки миллисекунд, но может увеличиваться в случае проблем связности в кластере). В связи с этим, чтение с реплик обеспечивается только в [режиме транзакций](../../transactions.md#modes) `StaleReadOnly()`.

Если реплик несколько, то отставание их от лидера может различаться, т.е. хотя каждая реплика каждой из партиции сохраняет внутреннюю консистентность, между разными партициями могут наблюдаться артефакты. Код приложения должен быть к этому готов. По этой же причине на данный момент невозможно и выполнение с реплик кроссшардовых транзакций.
...
{{ ydb-short-name }} позволяет группировать колонки в таблице для оптимизации их хранения и использования. Механизм групп колонок позволяет увеличить производительность операций неполного чтения строк путем разделения хранения колонок таблицы на несколько групп. Наиболее часто используемый сценарий — организация хранения редко используемых атрибутов в отдельной группе колонок. Тогда можно включить сжатие данных и/или хранить их на более медленных накопителях.
...
Для каждой таблицы существует основная группа колонок с именем `default`, в которую входят все колонки, для которых не указана явным образом другая группа колонок. Колонки первичного ключа всегда принадлежат основной группе колонок и не могут быть перемещены в другую группу. Также, если нет других групп колонок, то изменение атрибутов основной группы колонок позволяет задавать атрибуты для всей таблицы.

Для группы колонок устанавливаются атрибуты, влияющие на хранение и кэширование данных:
...
Атрибуты группы колонок устанавливаются при создании таблицы, а также могут быть изменены впоследствии. Изменения атрибутов хранения не применяются немедленно к хранимым данным, вместо этого они применяются при последующем фоновом [LSM compaction](../../glossary.md#compaction). Изменения атрибутов кэширования применяются сразу.

Доступ к данным, хранящимся в полях основной группы колонок, является более быстрым и требует меньше ресурсов, чем доступ к данным той же самой строки таблицы, хранящимся в полях дополнительно созданных групп колонок. Поиск по первичному ключу всегда осуществляется в основной группе колонок. При обращении к полям в других группах колонок помимо поиска по первичному ключу необходимы дополнительные поисковые операции для определения конкретной позиции хранения этих полей. Изменение режима кэширования не влияет на необходимость дополнительных операций для неосновных колонок, поэтому при одинаковых режимах кэширования для основной группы колонок доступ тоже будет быстрее.

Таким образом, вынос части колонок таблицы в отдельную группу колонок позволяет ускорить чтение наиболее важных и часто используемых колонок (входящих в состав основной группы колонок) ценой некоторого замедления доступа к остальным колонкам. Кроме того, на уровне групп колонок осуществляется управление параметрами хранения данных — выбор типа устройств хранения и режима сжатия, а также параметров кэширования.

### Режимы кэширования {#cache-modes}

Для групп колонок можно задавать режимы кэширования. Режимы кэширования в {{ ydb-short-name }} позволяют управлять стратегией кэширования данных таблиц в оперативной памяти. Это особенно важно для оптимизации времени ответа при чтении небольших часто используемых таблиц. Поддержаны два режима кэширования:

* `regular` (значение по умолчанию);
* `in_memory`.

В режиме кэширования `regular` данные групп колонок загружаются в [общий кэш](../../../concepts/glossary.md#shared-cache) только при первом обращении к ним. Если группа колонок использует режим `in_memory`, её данные автоматически предзагружаются в кэш при запуске системы.

Когда суммарный объём данных в кэше превышает установленный [лимит](../../../reference/configuration/memory_controller_config.md#cache-memory-limits), сначала освобождается память, занятая страницами с режимом `regular`. Если этого недостаточно, вытесняются страницы с режимом `in_memory`. Таким образом, режим `in_memory` даёт приоритетное хранение в оперативной памяти, но размер кэша остаётся ограниченным конфигурацией — память не будет использоваться неограниченно.

Режим кэширования `in_memory` может быть полезен в различных сценариях работы. Один из частых сценариев — хранение небольших справочных таблиц (“lookup-таблиц”), которые полностью помещаются в оперативную память. В этом случае использование режима `in_memory` позволяет минимизировать задержки при доступе к данным, так как таблица всегда будет находиться в памяти, даже если обращения к ней происходят нерегулярно.

Однако это не единственный способ применения. Например, в некоторых случаях может быть целесообразно хранить в памяти все данные конкретной таблицы, независимо от её назначения, если бизнес-логика приложения требует постоянного быстрого доступа к полной версии таблицы.

Используя режим `in_memory`, помните, что выделенная на такие таблицы память уменьшает объём, доступный для кэша обычных таблиц, что может негативно сказаться на их производительности (увеличить latency).

Следует отметить, что режим кэширования `in_memory` влияет только на операции чтения. Операции записи при любом режиме пишутся на диск, тем самым гарантии сохранности данных остаются без изменений. Это отличает режимы кэширования {{ ydb-short-name }} от некоторых других in-memory СУБД, которые могут полностью содержать таблицы в памяти ценой снижения гарантий на сохранность данных.
