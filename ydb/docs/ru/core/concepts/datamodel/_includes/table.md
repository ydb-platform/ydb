...
При принятии решения о разделении партиции по нагрузке или об объединении нескольких партиций по нагрузке, YDB рассматривает CPU нагрузку и на лидера данной партиции, и на все её реплики. При разделении, даже если только для одной из реплик партиции (или для лидера) CPU нагрузка превышает порога разделения по нагрузке, то данная партиция будет разделена на две партиции. При этом ключ разделения будет вычислен по текущей нагрузке именно на ту реплику (или лидера), которая перегружена. При объединении, CPU нагрузка на реплику (или на лидера), которая испытывает максимальную нагрузку, считается эффективной CPU нагрузкой для данной партиции. Если суммарная эффективная CPU нагрузка для несколько соседних партиций ниже порога объединения по нагрузке, то эти партиции будут объединены в одну партицию.
