...
* **Секрет со значением** — хранит строку, заданную при создании: пароль, токен, ключ. Это тип по умолчанию.
* **Секрет с делегированием IAM** (`type = "IAM_DELEGATION"`) — не хранит значения. Вместо этого {{ ydb-short-name }} получает разрешение действовать от имени указанного сервисного аккаунта {{ yandex-cloud }} и сама выпускает и обновляет его короткоживущие IAM-токены; значением такого секрета всегда является действующий токен. Подробнее — в разделе [{#T}](iam-delegation-secrets.md).
