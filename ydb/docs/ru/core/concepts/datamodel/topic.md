...
- **streaming** — это тип читателя, при котором партиция эксклюзивно назначается одному потребителю, и все сообщения из этой партиции обрабатываются этим потребителем. В этом случае количество партиций служит мерой параллельности чтения из топика: чем больше партиций в топике, тем больше потребителей могут одновременно читать из него. Типичный пример использования — обработка логов приложения. Читать из топика с использованием streaming-читателя можно по [Topic API](../../reference/ydb-sdk/topic.md) и протоколу [Kafka](../../reference/kafka-api/index.md).
- **shared** — это тип читателя, который позволяет нескольким потребителям совместно обрабатывать записи из одного топика без эксклюзивного назначения партиций каждому из них. Типичный пример использования — обмен сообщениями между микросервисами. Читать из топика с использованием shared-читателя можно по протоколу [SQS](../../reference/sqs-api/index.md).
