...
Для читателя может быть задан временной период `availability_period`, в течение которого ему будут доступны сообщения из топика, которые он ещё не обработал.
...
Параметр `availability_period` не влияет на время хранения обработанных читателем сообщений. Если параметр не задан, время хранения сообщений определяется в [настройках топика](#retention-time).
