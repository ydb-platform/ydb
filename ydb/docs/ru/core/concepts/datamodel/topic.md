...
См. также [{#T}](../../dev/shared-consumer-internals.md).
...
Также ограниченно поддержаны протоколы Kafka API версии 3.4.0 (см. [{#T}](../../reference/kafka-api/index.md)) и Amazon SQS API (см. [{#T}](../../reference/sqs-api/index.md)).
...
Для записи сообщений в топик используется привычный синтаксис (такой де как и при записи в таблицу в одной колонкой типа `String`):
...
Запись в топики ведётся не транзакционно, по этому в топике можно видеть частичные результаты запроса.

{% endnote %}

### Запись в топики другой базы данных

Для записи в топики, находящихся в другой базе данных, нужно использовать [внешние источники данных](external_data_source.md).
