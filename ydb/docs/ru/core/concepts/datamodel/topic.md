...
Сессия чтения — это подключение клиента к топику для получения сообщений от имени читателя. Сессии чтения работают через Topic API. Один читатель может установить несколько сессий чтения: в этом случае партиции топика распределяются между этими сессиями. Для работы с сессиями чтения рекомендуется использовать {{ ydb-short-name }} SDK (см. [Работа с топиками](../../reference/ydb-sdk/topic.md)).

В {{ ydb-short-name }} читатели могут быть следующих типов:

- [**потоковый**](#streaming-consumer) — это тип читателя, при котором партиция эксклюзивно назначается одному потребителю, и все сообщения из этой партиции обрабатываются этим потребителем.
- [**разделяемый (общий)**](#shared-consumer) — это тип читателя, который позволяет нескольким потребителям совместно обрабатывать сообщения из одного топика без эксклюзивного назначения партиций каждому из них.

## Потоковый читатель {#streaming-consumer}

Потоковый читатель предназначен для потоковой обработки данных из топика. В этом режиме несколько экземпляров приложения-потребителя параллельно читают сообщения и обрабатывают их по мере поступления. Типичные сценарии — сбор и анализ логов приложения, построение агрегатов в реальном времени, репликация событий во внешние системы.

Читать из топика с использованием потокового читателя можно по [Topic API](../../reference/ydb-sdk/topic.md) и протоколу [Kafka](../../reference/kafka-api/index.md).

В потоковом режиме каждая [партиция](#partitioning) топика в каждый момент времени назначается не более чем одному потребителю. Все сообщения из назначенной партиции обрабатывает этот потребитель.

За распределение партиций между потребителями отвечает сервер {{ ydb-short-name }}. При подключении и отключении потребителей сервер перераспределяет партиции и стремится распределить их между активными потребителями как можно равномернее.

Количество партиций определяет максимальную степень параллелизма: одновременно читать из топика могут не больше потребителей, чем партиций. Если потребителей больше, чем партиций, часть экземпляров остаётся без назначенных партиций и простаивает до появления свободных партиций или уменьшения числа потребителей.


## Разделяемый (общий) читатель {#shared-consumer}

Разделяемый (общий) читатель в {{ ydb-short-name }} — это модель чтения из топика, когда за читателем закрепляется одно или несколько сообщений из топика, а не партиция топика целиком. Это позволяет одну партицию обрабатывать одновременно большим количеством потребителей.

Типичный пример использования — обмен сообщениями между микросервисами. Читать из топика с использованием разделяемого (общего) читателя можно по протоколу [SQS](../../reference/sqs-api/index.md).

Сообщение за потребителем закрепляется на некоторое время. Если за это время потребитель не сообщил об окончании обработки сообщения и не продлил его время обработки, то это значит, что потребитель не смог обработать сообщение, и сообщение становится доступным для чтения и будет вычитано еще раз. Время обработки настраивается в параметрах читателя, но может быть задано и для каждого запроса на чтение сообщений.

К сообщениям, которые не могут быть обработаны, применяется Dead letter policy. Dead letter policy может быть настроена одним из следующих способов:

- **none** — сообщение после неуспешной обработки возвращается в очередь и после будет отдано для повторного чтения.
- **move** — сообщение после нескольких попыток обработки перемещается в DLQ, в качестве которого может быть указан любой другой топик базы данных.
- **delete** — сообщение после нескольких попыток обработки удаляется.

По умолчанию Dead letter policy выключена (none). Количество попыток обработки сообщения до его перемещения в DLQ (move) или его удаления (delete) настраивается на читателе. Если не удалось переместить сообщение в DLQ, то оно возвращается обратно в очередь (как при выключенной Dead letter policy), а при следующей неудачной попытке обработать сообщение будет снова выполнена попытка переместить сообщение в DLQ. Например, если удалить топик, указанный в качестве DLQ, то сообщения после неудачной обработки будут возвращаться в очередь до тех пор, пока не будет создан топик, указанный в качестве DLQ, либо не будет указан другой существующий топик.

В зависимости от конфигурации разделяемого (общего) читателя чтение из топика может осуществляться как с сохранением порядка сообщений, так и без. Порядок сообщений сохраняется среди сообщений с одинаковым message-group-id, который может быть задан для каждого сообщения при его записи. Если разделяемый (общий) читатель сконфигурирован на чтение с сохранением порядка сообщений, то таким читателем в один момент времени потребителям на обработку выдаётся не более одного сообщения из одной message-group-id.

При помещении сообщения в очередь можно указать задержку перед тем, как сообщение будет отдано на обработку в первый раз. Порядок сообщений определяется временем добавления сообщения в топик — если чтение осуществляется с сохранением порядка сообщений, то отложенное сообщение приостановит выдачу на обработку всех последующих сообщений с таким же message-group-id.
...
В инфлайте одной партиции может находиться до 48000 сообщений. Это означает, что если из одной партиции читает большое количество потребителей (больше 48000), то сообщения для обработки будут получать не более 48000 потребителей, даже если в партиции сообщений больше. Увеличение количества партиций в топике увеличивает общий размер инфлайта — общий инфлайт топика равен количеству партиций, умноженному на 48000 сообщений. Если необходимо обрабатывать большое количество сообщений одновременно, то следует увеличить количество партиций топика.
...
Для разделяемых (общих) читателей с сохранением порядка сообщений это означает, что если в партицию будет записано подряд большое количество сообщений из одной message-group-id (больше 48000 сообщений), то весь инфлайт займут сообщения из одной message-group-id, и сообщения из других групп выдаваться не будут, пока в инфлайте не появятся сообщения из других групп.
...
### Ограничение чтения из одной партиции {#partition-max-in-flight-bytes}

Параметр `partition_max_in_flight_bytes` можно задать при создании сессии чтения.

Этот параметр задает верхнюю границу in-flight - объема данных по одной партиции, которые уже выданы сервером в ответах чтения, но еще не подтверждены коммитом смещения.

- `0` — отдельный лимит in-flight по партиции не применяется;
- положительное значение — при достижении лимита сервер временно прекращает отдавать новые порции по этой партиции до тех пор, пока клиент не выполнит коммит и не уменьшит in-flight.

Слишком маленькое значение при медленной обработке или редких коммитах может вызывать паузы чтения по партиции. В таком случае увеличьте лимит или выполняйте коммиты чаще.
...
## YQL-запросы к топикам {#yql-topics}

Для работы с топиками можно использовать [YQL-запросы](../query_execution/topics.md) с привычными конструкциями: [SELECT](../../yql/reference/syntax/select/index.md) для чтения сообщений из топика и [INSERT](../../yql/reference/syntax/insert_into.md) для записи сообщений в топик.
