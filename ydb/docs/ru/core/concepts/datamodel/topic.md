...
- [**streaming**](#streaming-consumer) — это тип читателя, при котором партиция эксклюзивно назначается одному потребителю, и все сообщения из этой партиции обрабатываются этим потребителем.
- [**shared**](#shared-consumer) — это тип читателя, который позволяет нескольким потребителям совместно обрабатывать записи из одного топика без эксклюзивного назначения партиций каждому из них.
...
## Streaming-читатель {#streaming-consumer}

Streaming-читатель предназначен для потоковой обработки данных из топика. В этом режиме несколько экземпляров приложения-потребителя параллельно читают сообщения и обрабатывают их по мере поступления. Типичные сценарии — сбор и анализ логов приложения, построение агрегатов в реальном времени, репликация событий во внешние системы.

Читать из топика с использованием streaming-читателя можно по [Topic API](../../reference/ydb-sdk/topic.md) и протоколу [Kafka](../../reference/kafka-api/index.md).

пВ streaming-режиме каждая [партиция](#partitioning) топика в каждый момент времени назначается не более чем одному потребителю. Все сообщения из назначенной партиции обрабатывает этот потребитель.
...
## Shared-читатель {#shared-consumer}

Shared-читатель в {{ ydb-short-name }} — это модель чтения из топика, когда за читателям закрепляется одно или несколько сообщений из топика, а не партиция топика целиком. Это позволяет одну партицию обрабатывать одновременно большим количеством потребителей.

Типичный пример использования — обмен сообщениями между микросервисами. Читать из топика с использованием shared-читателя можно по протоколу [SQS](../../reference/sqs-api/index.md).

Сообщение за потребителем закрепляется на некоторое время. Если за это время потребитель не сообщил об окончании обработки сообщения, и не продлил его время обработки, то это значит, что потребитель не смог обработать сообщение, и сообщение распределяется повторно другим потребителям. Врем обработки настраиваться в параметрах консьюмера, но может быть задано и для каждого запроса на чтение сообщений.

К сообщениям, которые не могут быть обработаны, применяетяся Dead lette policy. Dead letter policy может быть настроена одним из следующих способов:

- **выключено** - cообщение после неуспешной обработки возвращается в очередь, и после будет отдано для повторного чтения.
- **переместить в DLQ** - сообщение после нескольких попыток обработки перемещается в DLQ, в кчестве которого может быть указан любой другой топик базы данных.
- **удалить** - сообщение после нескольких попыток обработки удаляется.

По умолчанию Dead letter policy выключено. Количество попыток обработки сообщения до его перемещения в DLQ или его удаления настраивается на читателе. Если не удалось переместить сообщение в DLQ, то оно возаращается обратно в очередь (как при выключенной Dedad letter policy), а при следующей неудачной попытки обработать сообщение будет снова выполнена попытка переместить сообщение в DLQ. Например, если удалить топик указанный в качестве DLQ, то сообщения после неудачной обработки будут возвращаться в очередь до тех пор, пока не будет создан топик, указанный в качестве DLQ, либо не будет указан другой существующий топик.

В зависимости от конфигурации shared-читателя чтения из топика может осуществляться с сохранением порядка сообщение, так и без. Порядок сообщений сохраняется среди сообщений с одинаковым message-group-id, который может быть задан для каждого сообщения при его записи. Если shared-читатель сконфигурирован на чтение с сохранением порядка сообщений, то таким читателем в один момент времени потребеителям на обработку выдается не более одного сообщения из одной message-group-id.

При помещении сообщения в очередь можно указать задержку перед тем как сообщение будет отдано на обработку первый раз.

### Особенность реализации {#shared-consumer-implementation}

Для каждой пары «shared-читатель — партиция топика» сервер поддерживает собственное состояние обработки сообщений. Состояние состоит из непрерывного блока сообщений из партиции топика - инфлайта. Первыйм сообщением инфлайта является сообщение, которое еще не обработано, находится в обработке или ждет перемещения в DLQ. Если первое сообщение успешно обработано, то блок перемещается на следующее сообщение. Сообщения для обработки отдаются только из сообщений находящихся в инфлайте. 

В инфлайте одной партиции может находится до 48 тыс. сообщений. Это означае, что если из одной партиции читать большим количество потребеителей (больше 48 тыс.), то сообщения для обработки будут получать не более 48 тыс. потребителей, даже если в партиции сообщений больше. Увеличение количества партици в топике увеличивает общий размер инфлайта - общий инфлайт топика равен количество партиции * 48 тыс. сообщений. Если необходимо обрабатывать большое количество сообщений одновременно, то следует увеличить количество партиции топика.

Если топик состоит из нескольких партиции, то запросы на чтения равномерно распределяются по всем партициям. Может случится ситуация, что запрос попадет в партицию, в которой отсутствуют сообщения. В этом случае потребитель получит ответ, что сообщений для обработки нет. Если в партиции долгое время нет сообщений для обработки, то она исключается из распределения запросов на чтения. Партиция вернется в распределение сразу как только в ней появятся сообщения для обработки. 

Для shared-читателей с сохранением порядка сообщений это означает, что если в партицию будет записано большое количество сообщений из одной message-group-id (больше 48 тыс. сообщений), то весь инфлайт займут сообщения из одной m,essage-group-id, и сообщения из других групп выдаваться не будут, пока в инфлайте не появятся сообщения из других групп.
...
Также ограниченно поддержан протокол Kafka API версии 3.4.0. (см. [Работа с Kafka API](../../reference/kafka-api/index.md)) и [Amazon SQS](https://en.wikipedia.org/wiki/Amazon_Simple_Queue_Service) (см. [SQS](../../reference/sqs-api/index.md)).
