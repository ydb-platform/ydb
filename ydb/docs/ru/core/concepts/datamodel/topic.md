...
В streaming-режиме каждая [партиция](#partitioning) топика в каждый момент времени назначается не более чем одному потребителю. Все сообщения из назначенной партиции обрабатывает этот потребитель.
