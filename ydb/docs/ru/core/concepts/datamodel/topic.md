...
В потоковом API YDB Topic (`StreamRead`) в `InitRequest` можно задать параметр `partition_max_in_flight_bytes`.

Параметр задает верхнюю границу in-flight - объема данных по одной партиции, которые уже выданы сервером в ответах чтения, но еще не подтверждены коммитом смещения.
