...
Исключение составляют данные, которые еще не были подтверждены ["важным"](#important-consumer) читателем — они будут храниться до тех пор, пока читатель их не прочитает.
Также хранятся сообщения, чтение которых не было подтверждено читателем с явно указанным [временем доступности](#availability-period-consumer), но только пока возраст сообщений не превысит указанного времени.
...
Для читателя может быть задан временной период `availability_period`, в течение которого ему будут доступны сообщения из топика, для которых он ещё не подтвердил прочтение.

Задание параметра позволяет читателю не пропускать обработку важных сообщений при простое. В отличие от признака ["важного"](#important-consumer) читателя, параметр ограничивает максимальный возраст сообщений, которые будут храниться в топике.

Параметр не влияет на время хранения подтверждённых сообщений.
