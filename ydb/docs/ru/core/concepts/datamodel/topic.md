...
Разделяемый (общий) читатель в {{ ydb-short-name }} — это модель чтения из топика, когда за читателям закрепляется одно или несколько сообщений из топика, а не партиция топика целиком. Это позволяет одну партицию обрабатывать одновременно большим количеством потребителей.
