...
Для чтения данных из топика используется привычный синтаксис (такой же как при чтении таблицы):
...
Чтение всегда происходит без использования читателя.
...
### Чтение топиков из другой базы данных

Для чтения топикав, находящихся в другой базе данных нужно использовать [внешние источники данных](external_data_source.md). 
...
- Чтение пользовательских атрибутов не поддерживается. 
