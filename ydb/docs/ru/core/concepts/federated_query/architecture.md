...
Ключевым элементом системы обработки федеративных запросов {{ ydb-full-name }} является понятие [внешнего источника данных](../datamodel/external_data_source.md) (external data source). В настоящее время в качестве таких источников для {{ ydb-short-name }} могут выступать реляционные СУБД или объектные хранилища. В момент выполнения федеративного запроса {{ ydb-short-name }} извлекает данные из внешних систем, размещает их в своей оперативной памяти, а затем выполняет агрегацию и совместный анализ данных из разных источников в соответствии с запросом клиента.
