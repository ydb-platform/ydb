...
Автоматический вывод схемы доступен для всех [форматов данных](formats.md#formats), кроме `raw` и `json_as_string`. Он удобен в случаях, когда схема содержит большое количество полей. Чтобы не вводить эти поля вручную, используйте параметр `WITH_INFER`:
