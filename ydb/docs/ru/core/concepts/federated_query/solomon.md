# Чтение метрик Solomon через внешние источники данных

{% if audience == "tech" %}

Solomon — это сервис, позволяющий собирать и хранить метрики, а также отображать их в виде графиков на дашбордах. Отправляемые в solomon данные представляют из себя значения измеряемых величин (метрики) и метки (labels), их описывающие.

Например, чтобы следить за количеством сбоев приложения, в качестве метрики можно использовать число сбоев за интервал времени. Данные, описывающие это падение: название хоста, версия приложения — являются метками. В интерфейсе Solomon можно проводить различные агрегации метрик по меткам.

Пример чтения метрик из Solomon.

```yql
SELECT
    storagePool, sum(value) as sum_value
FROM
    solomon_prod.kikimr
WITH (
    selectors = @@{cluster = "ydb_ru", service = "vdisks", storagePool = "*", sensor = "ResponsesWithDiskSpaceLightYellowMove", group = "*", host != "cluster"}@@,

    labels = "storagePool",

    from = "2025-09-02T08:52:00Z",
    to = "2025-09-04T09:52:00Z",
