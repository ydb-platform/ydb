...
**Sequence** — это схемный объект, генерирующий монотонно возрастающий ряд целых чисел. Каждый вызов возвращает уникальное значение. Объекты Sequence используются для реализации [серийных типов данных](../yql/reference/types/serial.md). Объект Sequence создаётся автоматически при определении серийной колонки и уничтожается вместе с таблицей.
