...
**Пул ресурсов** - объект схемы который описывает ограничения накладываемые на потребляемые ресурсы (CPU, RAM, максимальная параллельность выполнения запросов) запросами в этом пуле ресурсов. Запрос всегда выполняется в каком-то пуле ресурсов. По умолчанию все запросы выполняются в пуле ресурсов с именем default, который не накладывает каких либо ограничений.

#### Классификатор пулов ресурсов {#resource-pool-classifier}

**Классификатор пулов ресурсов** - для управления раздачей запросов между пулами ресурсов существует классификатор пулов ресурсов. Он описывает правила по которым запросы распределяются между пулами ресурсов. Эти классификаторы глобальные на всю базу данных и применяются на все запросы внутри нее
