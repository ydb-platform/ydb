...
**Табличный сервис** или **TableService** - набор методов, предоставляемый {{ ydb-short-name }} и позволяющий клиентским приложениям выполнять различные операции с [таблицами](#table): создание и удаление таблиц, выполнение запросов [YQL](#YQL), [пакетная загрузка данных](../dev/batch-upload.md) и множество других действий.

### Query service {#query-service}
**Query сервис** или **QueryService** - сервис {{ ydb-short-name }}, предоставляющий возможность выполнять запросы [YQL](#YQL) с помощью [GRPC Streaming RPC](https://grpc.io/docs/what-is-grpc/core-concepts/#server-streaming-rpc). С более подробным описанием можно в разделе [Query Service](query_service.md)

### Topic service {#topic-service}
**Сервис топиков** или **TopicService** - сервис {{ ydb-short-name }}, предоставляющий функционал по работе с [топиками](#topic), такой как создание/удаление топиков, чтение сообщений из топика и запись сообщений в топик.
