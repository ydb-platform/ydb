...
#### Эпоха Node Broker {#node-broker-epoch}

**Эпоха Node Broker** — период времени фиксированной длительности. По завершении каждой эпохи [Node Broker](#node-broker) удаляет из кластера [динамические узлы](#dynamic) с истёкшей [арендой](#node-lease) и начинает новую эпоху.

#### Аренда динамического узла {#node-lease}

**Аренда динамического узла** или **lease** — ограниченный по времени срок, в течение которого зарегистрированный [динамический узел](#dynamic) остаётся в кластере. Чтобы не быть удалённым из кластера, узел до истечения аренды периодически обращается к [Node Broker](#node-broker) и продлевает её. Если узел перестаёт продлевать аренду, по истечении её срока Node Broker удаляет его из кластера.
