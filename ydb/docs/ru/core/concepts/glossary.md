...
**Pile** или **пайл** — это набор узлов, которые могут отказать или быть отключены одновременно. Pile используются в [режиме bridge](#bridge) для разделения кластера на несколько частей, между которыми осуществляется синхронная репликация. Pile может состоять из узлов одного или нескольких регионов.
