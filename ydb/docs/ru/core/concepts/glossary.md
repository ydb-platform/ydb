...
Отдельно выделяются специальные виды **вторичного индекса** - [векторный индекс](#vector-index), [полнотекстовый индекс](#fulltext-index) и [JSON-индекс](#json-index).
...
**JSON-индекс** — это дополнительная структура данных, используемая для ускорения поиска внутри документов в колонке типа `Json` или `JsonDocument`: по существованию пути и по значению в указанном пути, без полного сканирования таблицы. JSON-индекс запрашивается уже существующими функциями `JSON_EXISTS` и `JSON_VALUE` и может быть выбран [оптимизатором](#optimizer) автоматически.

Возможности JSON-индекса и правила его использования описаны в статьях [{#T}](../dev/json-indexes.md) и [{#T}](query_execution/json_search.md).
