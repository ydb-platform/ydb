...
**mTLS** (mutual TLS, взаимный TLS) — режим [TLS](https://ru.wikipedia.org/wiki/Transport_Layer_Security), при котором не только клиент проверяет сертификат сервера, но и сервер запрашивает и проверяет [клиентский сертификат](#client-certificate) клиента при установлении соединения.

### Клиентский сертификат {#client-certificate}

**Клиентский сертификат** (или **сертификат клиента**) — [цифровой сертификат](https://en.wikipedia.org/wiki/X.509), выдаваемый и используемый клиентом — приложением, пользователем или [узлом {{ ydb-short-name }}](#node) — для подтверждения его личности при взаимодействии с {{ ydb-short-name }}.
