...
**Пайл** или **pile** — это набор узлов, которые могут отказать или быть отключены одновременно. Пайлы используются в [режиме bridge](#bridge) для разделения кластера на несколько частей, между которыми осуществляется синхронная репликация. Пайл может состоять из узлов одного или нескольких регионов.

#### Режим bridge {#bridge}

**Режим bridge** — это особый режим работы кластера, при котором данные хранятся с синхронной репликацией между несколькими [пайлами](#pile). Особенности режима описаны в [{#T}](topology.md#bridge), а также в [{#T}](bridge.md).
### Таблица {#table}
