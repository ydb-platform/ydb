...
**Recovery-режим** — режим, в котором таблетка запускается и доступна через [{{ ydb-ui-name }}](../reference/ydb-ui/index.md), но **не отвечает на запросы**, **не выполняет никаких фоновых активностей** и **не читает данные из распределённого хранилища**, что позволяет выполнять операции восстановления. Остальные таблетки продолжают работать в штатном режиме, что позволяет кластеру продолжать функционировать, но будут недоступны операции и UI, связанные с таблеткой, переведённой в Recovery-режим.
