...
Тип запроса, предназначенный для [потоковой обработки](https://en.wikipedia.org/wiki/Stream_processing) неограниченного потока данных. В отличие от обычных запросов, потоковые запросы не имеют максимальной длительности выполнения, автоматически перезапускаются при ошибках и периодически сохраняют своё состояние в виде [чекпоинтов](#checkpoints) для обеспечения отказоустойчивости.
