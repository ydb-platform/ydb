...
**Спиллинг** или **spilling** — это механизм управления памятью в {{ ydb-short-name }}, при котором данные, возникающие в результате промежуточных вычислений и превышающие доступный объём оперативной памяти, временно выгружаются на диск. Спиллинг обеспечивает выполнение запросов, требующих обработки больших объёмов данных, превышающих доступную оперативную память.

Спиллинг в {{ ydb-short-name }} реализуется через **Spilling Service** — [акторный сервис](#actor-service), который предоставляет временное хранилище для блобов данных. Сервис работает по принципу key-value хранилища, где клиенты могут сохранять данные по уникальному идентификатору и получать их обратно по этому идентификатору.

Основные типы спиллинга в {{ ydb-short-name }}:

* **Спиллинг вычислительных ядер** — выгрузка промежуточных результатов вычислений (агрегации, сортировки, join операции) на диск при достижении лимитов памяти;
* **Спиллинг каналов** — выгрузка данных в каналах передачи между различными этапами выполнения запроса при переполнении буферов.

Подробнее о спиллинге см. в [отдельной статье](spilling.md).
