...
{{ ydb-short-name }} предоставляет возможность управления режимами выполнения транзакций. Есть возможность явно определить режим транзакции с конкретными гарантиями изоляции и консистентности, либо использовать неявные транзакции, при которых {{ ydb-short-name }} автоматически определяет подходящее транзакционное поведение в зависимости от типа выполняемых инструкций. Детальное описание доступных режимов транзакций и их характеристик представлено в разделе [{#T}](./transactions.md#modes).
