...
Узел {{ ydb-short-name }} проверяет [аутентификационный токен](#auth-token) [пользователя](#access-user) при первом полученном от него запросе. Если источник, в котором был создан пользователь, подтверждает подлинность пользователя, то создаётся и кэшируется **токен пользователя** для подтверждения будущих запросов от этого пользователя, чтобы не проводить проверку аутентификационного токена заново. Токен пользователя действует только для создавшего его узла {{ ydb-short-name }}.
