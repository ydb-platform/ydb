...
**Токен пользователя** — внутренний результат проверки [аутентификационного токена](#auth-token). Он содержит [SID](#access-sid) пользователя и групп и используется при [авторизации](../security/authorization.md). Токен пользователя хранится только на создавшем его узле и не передаётся клиенту. Подробнее см. в статье [{#T}](../security/caching-authentication-results.md).
