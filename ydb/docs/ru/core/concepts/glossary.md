...
Тип запроса, предназначенный для потоковой обработки неограниченного потока данных. В отличие от обычных запросов, потоковые запросы не имеют максимальной длительности выполнения, автоматически перезапускаются при ошибках и периодически сохраняют своё состояние в виде [чекпоинтов](#checkpoints) для обеспечения отказоустойчивости.

Стриминговые запросы описаны более подробно в отдельной статье [{#T}](streaming-query.md)

### Чекпоинты {#checkpoints}

Периодически сохраняемое состояние [потокового запроса](#streaming-query) (включая агрегации, смещения во входных [топиках](#topic) и номера сообщений в выходных), необходимое для автоматического восстановления его работы после сбоев в распределённой системе. Подробнее про чекпоинты в статье [{#T}](streaming-query.md#checkpoints)
