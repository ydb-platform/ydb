...
{{ ydb-short-name }} имеет несколько [списков уровней доступа](../devops/configuration-management/configuration-v1/config-settings.md#security-access-levels), совместно определяющих набор [уровней доступа](#access-level) в системе.
...
**Контроллер памяти** или **memory controller**— это [актор](#actor), который управляет [лимитами памяти](../devops/configuration-management/configuration-v1/config-settings.md#memory-controller) {{ ydb-short-name }}.
