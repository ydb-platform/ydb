...
Транзакции, которые явно определяются и управляются пользователем, называются **явными транзакциями**. Данный вид транзакций инициируется вручную с выбором уровня изоляции и транзакционного поведения. Они описаны более подробно в [{#T}](transactions.md#modes).
