...
### Вотермарки потоковых запросов {#streaming-queries-watermarks}

Монотонно возрастающая нижняя оценка времён событий в [потоковом запросе](#streaming-query), которые ещё могут поступить в поток. Когда вотермарка достигает значения X, система объявляет, что все события с временем меньше X с высокой вероятностью уже получены. Подробнее про вотермарки в статье [{#T}](./streaming-query/watermarks.md).
