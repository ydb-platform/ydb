...
**PDisk**, **physical disk** или **физический диск** — это компонент, который управляет физическим дисковым накопителем (блочным устройством). Другими словами, PDisk — это подсистема, которая реализует абстракцию, аналогичную специализированной файловой системе поверх блочных устройств (или файлов, эмулирующих блочное устройство для целей тестирования). PDisk обеспечивает контроль целостности данных (включая [кодирование с исправлением ошибок](#erasure-coding) групп секторов для восстановления данных на отдельных повреждённых секторах, контроль целостности с помощью контрольных сумм), прозрачное шифрование всех данных на диске и транзакционные гарантии дисковых операций (подтверждение записи строго после `fsync`).

PDisk разделяет ресурсы блочного устройства (пропускную способность и свободное дисковое пространство) на равные доли - слоты или [VSlot](#slot). Cлоты принадлежат VDisk'ам из разных [групп хранения](#storage-group), совместно использующим данный PDisk. В кластерах с неоднородными по размеру дисками один VDisk может занимать более одного слота.

PDisk размечает блочное устройство на блоки по 128 МБ, именуемые чанками. Часть чанков резервируется для системных нужд, в частности под журнал восстановления. Остальные чанки могут быть аллоцированы VDisk'ами. VDisk получает квоту на чанки пропорционально количеству занимаемых им слотов, однако может её превышать при наличии незанятых чанков.

Совместное использование пропускной способности диска обеспечивает планировщик PDisk. Он стремится обеспечить минимальные задержки для VDisk'ов, соблюдающих выделенную им полосу. При этом VDisk может получить и больше при наличии свободных ресурсов.

#### VDisk {#vdisk}

**VDisk**, **virtual disk** или **виртуальный диск** — это компонент, который реализует хранение данных [распределённого хранилища](#distributed-storage) [LogoBlob](#logoblob) на [PDisk](#pdisk). VDisk хранит все свои данные на PDisk. Один VDisk соответствует одному PDisk, но обычно несколько VDisk связаны с одним PDisk. В отличие от PDisk, который скрывает блоки и журналы за собой, VDisk предоставляет интерфейс на уровне LogoBlob и [LogoBlobID](#logoblobid), например запись LogoBlob, чтение данных LogoBlobID и удаление набора LogoBlob с помощью специальной команды. VDisk является членом [группы хранения](#storage-group). Сам VDisk является локальным, но совокупность VDisk в группе обеспечивает надёжное хранение данных. Все VDisk в группе синхронизируют данные друг с другом и реплицируют данные в случае потерь. Набор VDisk в группе хранения образует распределённый RAID.
