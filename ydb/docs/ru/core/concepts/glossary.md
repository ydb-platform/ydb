...
PDisk делит ресурсы блочного устройства (пропускную способность и свободное место) на равные доли,  называемые слотами, или [VSlot](#slot). Cлоты принадлежат VDisk'ам из разных [группы хранения](#storage-group), совместно использующим данный PDisk. В кластерах [с неоднородными по размеру дисками](../maintenance/manual/managing-non-uniform-drives.md) один VDisk может занимать более одного слота.

PDisk размечает блочное устройство на блоки по 128 МБ, именуемые чанками. Часть чанков резервируется для системных нужд, в частности под журнал восстановления, общий для записей службы PDisk и всех VDisk. Остальные чанки могут быть аллоцированы VDisk'ами. VDisk получает квоту на чанки пропорционально количеству занимаемых им слотов, однако может её превышать при наличии незанятых чанков.
