...
Узел YDB проверяет [аутентификационный токен](#auth-token) [пользователя](#access-user) при первом полученном от него запросе. Если всё в порядке, то создаётся и кэшируется **токен пользователя** для аутентификации будущих запросов от этого пользователя, чтобы не проводить проверку аутентификационного токена заново.
