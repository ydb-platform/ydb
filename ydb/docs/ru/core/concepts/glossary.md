...
**Векторный индекс** или **vector index** — это дополнительная структура данных, используемая для ускорения решения задачи приблизительно векторного поиска, обычно когда данных достаточно много и [подход без индекса](todo mbkkt ref) не работает удовлетворительно. В отличие от первичного индекса, векторные индексы управляются независимо от основных данных таблицы. Таким образом, у таблицы может быть несколько векторных индексов для различных сценариев. Возможности {{ ydb-short-name }} в отношении векторных индексов описаны в отдельной статье [{#T}](vector_indexes.md).
