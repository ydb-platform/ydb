...
Типы баз данных в YDB:

- [тенантная база данных](#tenant-database);
- [корневая база данных кластера](#root-database).

#### Тенантная база данных {#tenant-database}

**Тенантная база данных** — это логический контейнер с независимым пространством имён для пользовательский объектов базы данных.

Тенантные базы данных полностью изолированы друг от друга — они обрабатываются отдельными [узлами баз данных](#database-node), используют отдельные [группы хранения](#storage-group), и могут иметь отдельных [пользователей](#access-user) с разными [правами доступа](#access-right) и [уровнями доступа](#access-level).

#### Корневая база данных кластера {#root-database}

**Корневая база данных кластера** – это системная база данных, создаваемая для нужд {{ ydb-short-name }} в [корне схемы кластера](#scheme-root). Эта база данных содержит служебную информацию, например [пользователей](#access-user), [права](#access-right) и [уровни доступа](#access-level), [тенантные базы данных](#tenant-database), и т.д.
Не рекомендуется создавать на уровне корневой базы данных пользовательские схемные объекты.
...
Узел YDB проверяет [аутентификационный токен](#auth-token) [пользователя](#access-user) при первом полученном от него запросе. Если источник, в котором был создан пользователь, подтверждает подлинность пользователя, то создаётся и кэшируется **токен пользователя** для подтверждения будущих запросов от этого пользователя, чтобы не проводить проверку аутентификационного токена заново.
...
**Корень базы данных** — это путь к базе данных в схеме кластера, который является корнем для схемных объектов базы данных.
...
Уровень доступа определяется принадлежностью субъекта к [спискам уровня доступа](#access-level-list).
