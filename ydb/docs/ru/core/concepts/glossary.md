...
**Клиентский таймаут** или **client-side timeout** — ограничение времени, которое приложение или {{ ydb-short-name }} SDK ждет завершения операции с базой (например, выполнения запроса или приема ответа по вызову gRPC). По истечении этого времени клиент обычно прерывает ожидание: закрывает соединение или поток данных, получает ошибку по транспорту или из SDK — еще до того, как сервер успел вернуть явный ответ (в том числе код ошибки вроде `OVERLOADED`).

Клиентский таймаут не равен лимиту длительности запроса на стороне {{ ydb-short-name }}: из-за особенностей обработки запросов в кластере прерванный на клиенте запрос может некоторое время продолжать выполняться на сервере. Поэтому частые повторы того же запроса сразу после таймаута могут усугублять перегрузку. Подробнее см. в статьях [{#T}](../troubleshooting/performance/queries/retry-cascade.md) и [{#T}](../troubleshooting/performance/queries/overloaded-errors.md); политики повторов в SDK — в разделе [{#T}](../reference/ydb-sdk/error_handling.md).
