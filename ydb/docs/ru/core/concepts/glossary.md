...
В базе данных выделяются ресурсы (CPU, disk, RAM, ...) используемые всеми запросами на равных условиях без каких либо ограничений. Если же необходимо разделять ресурсы в одной базе данных, то для этого в {{ ydb-short-name }} существуют resource pool и resource pool classifier. Один из сценариев применения такой функциональности, это разделение контуров онлайн вычислений и фоновой аналитики. На текущий момент эта функциональность доступна только для dedicated баз данных.

#### Resource Pool {#resource-pool}

**Resource Pool** - объект схемы который описывает ограничения накладываемые на потребляемые ресурсы (CPU, disk, RAM, ...) запросами в этом resource pool. Запрос всегда выполняется в каком-то resource pool. По умолчанию все запросы выполняются в default resource pool, который не накладывает каких либо ограничений на потребляемые ресурсы.

#### Resource Pool Classifier {#resource-pool-classifier}

**Resource Pool Classifier** - для управления раздачей запросов между resource pool существует resource pool classifier. Resource pools classifiers описывают правила по которым запросы распределяются между resource pool. Эти классификаторы глобальные на всю базу данных и применяются на все запросы внутри нее