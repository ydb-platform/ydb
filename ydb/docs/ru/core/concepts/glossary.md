...
Транзакции, которые явно определяются и управляются пользователем, называются **явными транзакциями**. Данный вид транзакций иницируется вручную с выбором уровня изоляции и транзакционными поведением. Они описаны более подробно в отдельной статье [{#T}](./transactions.md#modes).
