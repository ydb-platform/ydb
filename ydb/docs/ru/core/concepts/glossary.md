...
**Спиллинг** или **spilling** — это механизм управления памятью в {{ ydb-short-name }}, при котором промежуточные данные, возникающие в результате вычислений и превышающие доступный объём оперативной памяти узла, временно выгружаются во внешнее хранилище. В {{ ydb-short-name }} для спиллинга в настоящее время используется диск. Спиллинг обеспечивает выполнение запросов, требующих обработки больших объёмов данных, превышающих доступную память узла.
