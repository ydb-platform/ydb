...
**Pile** или **пайл** — это набор узлов, которые могут отказать или быть отключены одновременно с сохранением работоспособности других частей кластера (pile). Pile может сохранять работоспособность при одключении других узлов кластера. Pile используются в [режиме bridge](#bridge) для разделения кластера на несколько частей, между которыми осуществляется синхронная репликация. Pile может состоять из узлов одного или нескольких регионов.
...
#### Пушдаун предикатов {#predicate-pushdown}

**Пушдаун предикатов** или **predicate pushdown** — функциональность, позволяющая {{ ydb-short-name }} передавать различные части федеративного запроса на исполнение во внешний источник данных. Пушдаун предикатов позволяет снизить нагрузку на федеративную {{ ydb-short-name }}, уменьшить количество передаваемых данных между федеративной {{ ydb-short-name }} и внешней системой, а также ускорить выполнение федеративного запроса в целом. Частным случаем пушдауна предикатов является **пушдаун фильтров** или **filter pushdown**, позволяющий выполнить фильтрацию данных внутри внешней системы.

#### Сплит {#split}

**Сплит** или **split** — некоторый фрагмент (партиция, шард) внешней таблицы и одновременно с этим — элементарная единица работы для массивно-параллельной системы обработки федеративных запросов {{ ydb-short-name }}. При извлечении данных из некоторых внешних источников данных {{ ydb-short-name }} умеет выполнять разбиение всего набора данных на сплиты и вычитывать их параллельно, что позволяет значительно ускорить время обработки федеративного запроса. 
