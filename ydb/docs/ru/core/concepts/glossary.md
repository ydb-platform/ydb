...
Транзакции, которые автоматически создаются и управляются {{ ydb-short-name }}, когда режим транзакции не определен, называются **неявными транзакциями**. {{ ydb-short-name }} самостоятельно определяет транзакционное поведение в зависимости от переданного запроса. Они описаны более подробно в [{#T}](./transactions.md#modes).
