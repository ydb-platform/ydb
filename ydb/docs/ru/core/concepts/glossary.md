...
### Чекпоинты потоковых запросов {#streaming_queries_checkpoints}

Периодически сохраняемое состояние [потокового запроса](#streaming-query) (включая агрегации, смещения во входных [топиках](#topic) и номера сообщений в выходных), необходимое для автоматического восстановления его работы после сбоев в распределённой системе. Подробнее про чекпоинты в статье [{#T}](../dev/streaming-query/checkpoints.md)
