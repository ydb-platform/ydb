...
Реплики каждого кольца размещаются в пределах одного **домена отказа** (fail domain) — одной серверной стойки. Реплики из разных колец при этом размещаются в разных стойках. Это гарантирует, что при выходе из строя любой стойки из каждой выборки потеряется **не более одной реплики** — той, что принадлежит кольцу, чьи реплики находились в этой стойке.
