...
### По-запросное включение стоимостного оптимизатора

Чтобы принудительно включить стоимостной оптимизатор для текущего запроса, в тексте запроса необходимо использовать прагму `PRAGMA ydb.CostBasedOptimization = "on";`. Если требуется отключить стоимостной оптимизатор, значение прагмы следует установить в `off`.
