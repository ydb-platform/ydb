...
Чтобы для текущего запроса принудительно включить стоимостной оптимизатор, надо в тексте запроса использовать прагму `PRAGMA ydb.CostBasedOptimization = "on";`. Если требуется отключить стоимостной оптимизатор, значение прагмы надо выставить как `off`.
