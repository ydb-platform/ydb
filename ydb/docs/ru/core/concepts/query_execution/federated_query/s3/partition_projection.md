...
[Партицирование](partitioning.md) позволяет подсказать для {{ydb-full-name}} правила размещения данных в S3-совместимом хранилище данных.

Предположим, что данные в S3 хранятся в следующей структуре каталогов:
...
То есть при работе с партицированными данными выполняется полный листинг содержимого S3-совместимого хранилища данных, что может занимать продолжительное время на бакетах большого размера.

Для оптимизации работы на больших объемах данных следует использовать "расширенное партицирование". В этом режиме не происходит сканирования каталогов S3-совместимого хранилища данных, вместо этого все пути вычисляются заранее и обращение происходит только к ним.

Для работы расширенного партицирования необходимо задать правила работы через специальный параметр - "projection". В этом параметре описываются все правила размещения данных в каталогах S3-совместимого хранилища данных.
...
Оба варианта используют одинаковый набор параметров (`projection.*`, `storage.location.template`, `PARTITIONED_BY`), но отличаются способом их передачи в запрос. Полное описание этих параметров приведено в разделах [{#T}](#field_types) и [{#T}](#storage_location_template) ниже.

{% note warning %}

Синтаксис параметра `PARTITIONED_BY` отличается в зависимости от способа чтения:

* через внешний источник данных: `PARTITIONED_BY = (year, month)` (tuple-синтаксис);
* через внешнюю таблицу: `PARTITIONED_BY = "['year', 'month']"` (строковый литерал).

{% endnote %}

### Чтение через внешний источник данных { #syntax_external_data_source }

При чтении через внешний источник данных правила партицирования передаются непосредственно в запросе через конструкцию `WITH (...)`. Параметры `projection.*` задаются в виде JSON-документа, обёрнутого в [многострочный строковый литерал YQL](https://ydb.tech/docs/ru/yql/reference/syntax/lexer#string-literals) (синтаксис `@@ ... @@`), и передаются через именованный параметр `PROJECTION`. Список колонок, по которым выполняется партицирование, передаётся через параметр `PARTITIONED_BY` как tuple.
...
Данные в бакетах S3-совместимого хранилища данных могут быть размещены в каталогах с произвольными названиями. С помощью настройки `storage.location.template` можно указать правила именования каталогов, где хранятся данные.
