...
Для долгосрочных запросов к ресурсам, расположенным в других облачных базах, поддерживается [IAM-аутентификация с делегированием сервисному аккаунту](../../../security/iam-service-account-auth.md). Описание схемы, параметров и подготовки сервисного аккаунта приведено в общем разделе; ниже описаны специфичные для внешнего источника детали.
...
Далее [делегирование сервисного аккаунта](../../../security/iam-service-account-auth.md#service-account) включается обычным образом.
...
- `SERVICE_ACCOUNT_ID`, `INITIAL_TOKEN_SECRET_PATH`: описаны в разделе [{#T}](../../../security/iam-service-account-auth.md#parameters).
