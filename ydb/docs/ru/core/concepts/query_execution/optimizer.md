...
![Граф запроса](_assets/star_schema.png)
...
Стоимостной оптимизатор использует статистики только по таблице целиком, сбор статистик по отдельным колонкам находится в разработке. {{ ydb-short-name }} собирает и поддерживает статистку в актуальном состоянии в фоновом режиме. Вы можете принудительно запустить сбор статистики, используя команду [ANALYZE](../../yql/reference/syntax/analyze.md).
