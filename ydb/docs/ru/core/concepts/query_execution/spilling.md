...
На уровне Resource Manager порог перехода к спиллингу задаётся параметром [`resource_manager.spilling_percent`](../../reference/configuration/table_service_config.md#spilling-percent): когда пул `query_memory_limit` на узле заполнен на указанный процент, вычислительные операции начинают предпочитать выгрузку данных на диск дальнейшему росту потребления RAM.
