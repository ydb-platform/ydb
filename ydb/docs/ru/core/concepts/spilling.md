...
**Спиллинг** (от англ. *spilling*) - это механизм управления памятью, при котором данные, превышающие доступный объём оперативной памяти, временно выгружаются во внешнее хранилище. Это позволяет продолжать выполнение операций без аварийного завершения и эффективно использовать ресурсы в условиях ограниченной RAM.
...
**Ожидание результатов**: После отправки события вычислительный компонент передает выполнение другим компонентам и сам перестает выполняться, до тех пор, пока блоб не будет физически записан во внешнее хранилище.
...
**Чтение данных**: При необходимости восстановления данных компонент отправляет событие `TEvRead` с идентификатором блоба. Spilling Service читает данные из внешней памяти и возвращает событие `TEvReadResult` с восстановленными данными. Во время ожидания данных вычисления также приостанавливаются.

### Типы спиллинга в {{ ydb-short-name }}

В {{ ydb-short-name }} реализованы два основных типа спиллинга, функционирующие на различных уровнях вычислительного процесса:
