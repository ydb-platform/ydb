...
[Потоковые запросы](glossary.md#streaming-query) предназначены для обработки данных, которые поступают непрерывно (из топика), в реальном времени.
...
- сохранения состояний агрегаций, таких как `GROUP BY HOP` и `MATCH_RECOGNIZE`.

## Использование читателя {#consumer-usage}

При работе с потоковыми запросами возможно два режима чтения из топика: с использованием [читателя](../concepts/datamodel/topic.md#consumer) и без него. 

- **Без использования читателя:**  
   По умолчанию потоковые запросы читают данные из топика напрямую, без привязки к читателю. Такой подход не влияет на функциональность запроса и обычно используется для большинства сценариев. Подробнее: [Чтение без использования читателя](../reference/ydb-sdk/topic.md#no-consumer).
- **С использованием читателя:**  
   Можно явно указать читателя для потокового запроса. Это дает возможность агрегировать и отслеживать метрики на стороне мониторинга топиков по конкретному читателю. Чтобы воспользоваться этим режимом, предварительно создайте читателя через [CLI](../reference/ydb-cli/topic-consumer-add) или при создании топика с помощью [CREATE TOPIC](../yql/reference/syntax/create-topic.md). Пример настройки смотрите в [CREATE STREAMING QUERY](../yql/reference/syntax/create-streaming-query).

Выбор режима не влияет на результат выполнения потокового запроса, но позволяет гибко настраивать мониторинг использования топиков под задачи вашего сервиса.
...
- создание запроса — [CREATE STREAMING QUERY](../yql/reference/syntax/create-streaming-query);
- изменение запроса — [ALTER STREAMING QUERY](../yql/reference/syntax/alter-streaming-query);
- удаление запроса — [DROP STREAMING QUERY](../yql/reference/syntax/drop-streaming-query).

### Состояние запроса {#state}

Для получения текущего состояния (статуса) потокового запроса необходимо обратиться к системной таблице [.sys/streaming_queries](../dev/system-views.md#streaming) — значение статуса отображается в колонке Status.

Для изменения параметров или управления работой потокового запроса (например, приостановить или возобновить выполнение) используется команда `ALTER STREAMING QUERY`. Выполнение этой команды происходит асинхронно.

### Поддерживаемые типы данных в топиках {#data-types}

Данные в топиках хранятся в сыром виде — как набор байт. Читать сообщения из топика можно с помощью встроенных механизмамов парсинга наиболее популярных форматов данных (см. [Форматы данных потоковых запросов](../dev/streaming-query/streaming-query-formats.md)) или в сыром виде. 

Запись можно выполнять только в виде набора байт (например, как строка или JSON).

### Чтение из топика без использования CREATE STREAMING QUERY {#read-from-topic}

Для удобной отладки потоковых запросов есть возможность использовать привычные `SELECT` конструкции с топиками [без использования `CREATE STREAMING QUERY`](../yql/reference/syntax/select/index.md#reading-streaming-queries). При этом важно задать ограничение на количество выходных строк, иначе запрос просто зависнет. В таких запросах отключены чекпоинты и есть ограничение на время работы, и как следствие, такие запросы не способны восстановить свою работоспособность после сбоя. Режим рекомендуется использовать исключительно в отладочных целях. 
...
В потоковых запросах возможно присоединение к потоку данных из S3 с помощью конструкции `JOIN`. При этом поток обязательно должен находиться в левой части `JOIN`. Механизм имеет ограничения, т.к. правая часть `JOIN` полностью помещается в оперативную память процесса. Пример доступен по [ссылке](../dev/streaming-query/S3-enrichment.md)

## Запись в таблицы {#table-write}

Запись результата в таблицу {{ ydb-short-name }} возможна с помощью [UPSERT INTO](../yql/reference/syntax/upsert_into.md). Пример доступен по [ссылке](../dev/streaming-query/table-writing.md)
...
[Чекпоинт](glossary.md#checkpoints) представляет собой состояние рабочего потокового запроса, которое необходимо для восстановления работы запроса в случае сбоев в распределенной системе: выхода из строя узлов, падения процессов при нехватке памяти и др. {{ ydb-short-name }} собирает чекпоинты всех запущенных потоковых запросов периодически и сохраняет в системные таблицы базы.
...
Для снижения накладных расходов (от сохранения чекпоинтов) возможно отключение чекпоинтов. Для этого используется прагма `ydb.DisableCheckpoints`. При этом нужно учитывать отсутствие гарантий на консистентность данных (при пользовательских или внутренних перезапусках запроса). Используется исключительно с целью отладки. Пример доступен по [ссылке](../dev/streaming-query/checkpoints.md)
