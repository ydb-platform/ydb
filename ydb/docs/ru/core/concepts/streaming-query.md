...
- Не поддерживается обогащение из таблиц {{ ydb-short-name }} — потоковые запросы могут использовать только [внешние таблицы S3](federated_query/s3/external_table.md) для обогащения данными (поддержка обогащения из таблиц  {{ ydb-short-name }} планируется к реализации в версии 26.1).
