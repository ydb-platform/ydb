...
Потоковые запросы обеспечивают гарантию **at-least-once**: каждое событие будет записано в приемник как минимум один раз.
...
- `JOIN` двух потоков;
- обогащение из таблиц {{ ydb-short-name }} — потоковые запросы могут использовать только [внешние таблицы S3](query_execution/federated_query/s3/external_table.md) для обогащения данными (будет поддержано в версии 26.1);
