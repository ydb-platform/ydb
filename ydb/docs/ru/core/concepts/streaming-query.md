...
В текущей версии {{ ydb-short-name }} потоковые запросы обеспечивают уровень гарантий доставки данных [at-least-once](https://en.wikipedia.org/wiki/Reliable_messaging#At-least-once_delivery) (как минимум один раз). Это гарантирует отсутствие потерь данных при их фильтрации, обогащении или записи в таблицы через [UPSERT](../yql/reference/syntax/upsert_into.md). В случае системного сбоя или перезапуска запроса данные будут прочитаны из источника и обработаны повторно.

Для запросов с группировкой по окнам результат обработки может варьироваться в зависимости от порядка и времени поступления событий из различных [партиций](datamodel/topic.md#partitioning) топика. Если данные из одной партиции сильно запаздывают относительно других, это может привести к временной неполноте агрегатов. Данное ограничение является временным и планируется к исправлению в версии `26.1`.
