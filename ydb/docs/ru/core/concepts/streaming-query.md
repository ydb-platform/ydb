...
Потоковый запрос создается и продолжает работу бесконечно до явной отмены пользователем. Данные непрерывно поступают в топик, «протекают» через запрос и записываются в приёмник — другой топик или таблицу.
...
- `JOIN` двух потоков;
- обогащение из локальных таблиц {{ ydb-short-name }} — потоковые запросы не могут напрямую выполнять `JOIN` с таблицами той же базы данных;
- чтение и запись локальных топиков — потоковые запросы не могут напрямую работать с топиками той же базы данных.

{% endnote %}

Для работы с локальными таблицами и топиками используйте [внешние источники данных](../concepts/datamodel/external_data_source.md):

- **Обогащение из таблиц:** создайте внешний источник данных на отдельную базу данных {{ ydb-short-name }}, содержащую справочные таблицы, и используйте её для `JOIN`.
- **Работа с топиками:** создайте внешний источник данных, указывающий на ту же или другую базу данных {{ ydb-short-name }}, и обращайтесь к топикам через него.
