...
**Топики** — основной источник потоковых данных. Запрос читает сообщения из одного или нескольких [топиков](datamodel/topic.md) и обрабатывает их по мере поступления. Данные в топик могут записывать:
...
- Не поддерживается обогащение из таблиц {{ ydb-short-name }} — потоковые запросы могут использовать только [внешние таблицы S3](query_execution/federated_query/s3/external_table.md) для обогащения данными (поддержка обогащения из таблиц  {{ ydb-short-name }} планируется к реализации в версии 26.1).
- Не поддерживается чтение и запись локальных топиков — для работы с ними необходимо использовать [внешние источники данных](datamodel/external_data_source.md), указывающие на текущую базу данных (ограничение планируется снять в версии 26.1).
