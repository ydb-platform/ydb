...
В {{ ydb-short-name }} этот компромисс регулируется явно: параметр `WATERMARK` задаёт выражение для вычисления watermark, в том числе величину отставания от времени последнего события. Это позволяет подобрать баланс между актуальностью результатов и полнотой учёта запоздавших событий.

События, поступившие после того, как watermark прошёл соответствующий временной диапазон, считаются опоздавшими и обрабатываются особым образом. Подробнее об обработке опоздавших событий и о настройке watermarks: [{#T}](../../dev/streaming-query/watermarks.md).
