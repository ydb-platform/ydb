...
Размер чекпойнта зависит от сложности запроса, используемых агрегаций и от характера данных. Для оптимального потребления хранилища после очередного успешного сохранения чекпойнта старые удаляются автоматически. Т.е. большую часть времени для одного запроса в хранилище хранится только один чекпойнт.
