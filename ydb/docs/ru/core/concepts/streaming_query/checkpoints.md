...
На старте запроса выполняется компиляция запроса и результат компиляции будет сохранен (в системные таблицы).
В случае завершения запроса с ошибкой, он будет перезапущен автоматически, при этом:

- запрос начнёт выполняться их сохраненнного результат компиляции (без перекомплиляции),
...
Снижения накладных расходов сохраннения чекпойтов для отключения  используется прагма `ydb.DisableCheckpoints`. При этом нужно отменить отсутствие гарантий на консистентность данных (при пользовательных или внутренних перезапусках запроса).
