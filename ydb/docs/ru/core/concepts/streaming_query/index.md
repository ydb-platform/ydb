...
Потоковые запросы служат для потоковой обработки данных. Они осуществляют обработку над неограниченным потоком данных.
...
Чтобы использовать читателя необходимо предварительно его создать и указать его имя в тексте запроса через `PRAGMA pq.Consumer = ...` (см. пример в [CREATE STREAMING QUERY](../../../yql/reference/syntax/create-streaming-query)).
...
Чтение может быть произведено в различных формтах (см. [Форматы данных](formats.md)). Запись можно выполнять только в виде байтового потока, который интерпретируется на принимающей стороне.
...
Для управления (запуск, изменение и останавка) потоковыми запросами предназначена сущность STREAMING QUERY, которую можно создавать и изменять через DDL:
