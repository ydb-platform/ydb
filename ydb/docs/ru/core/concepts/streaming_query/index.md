...
Запросы могут быть в 2-х состояних: запущен или остановлен. При этом в системе запрос не моментально переходит в запущенное или остановленное состояние, также запрос в процессе работы может ожидать перезапуска после завершения с ошибкой, состояние запроса в системе описывается его статусом. Подробную информацию о запросе можно получить через системную таблицу streaming_queries.
