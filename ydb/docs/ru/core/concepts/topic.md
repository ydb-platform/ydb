...
Рекомендуемое максимальное пар <идентификатор источника, идентификатор группы сообщений> - до 100'000 на одну партицию за последние 14 дней.

{% endnote %}

{% cut "Почему и когда важен порядок обработки сообщений" %}
...
Несмотря на то, что упорядочивание сообщений в данном случае не важно, протокол записи сообщений в персистентную очередь требует указание идентификатора источника. {{ydb-short-name}} запоминает связь идентификатора источника и партицию, куда была произведена запись. Если выбирать идентификатор источника случайным образом, это приведет к большому объему различных идентификаторов источников, и, как следствие, к большому объему хранимых данных по взаимосвязям между источниками и партициями, что может привести к перегрузке {{ydb-short-name}}.
