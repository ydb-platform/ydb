...
Партиции бывают активные (все партиции по умолчанию, в них возможны как запись, так и чтение) и неактивные (из них можно только читать). Неактивные партиции появляются после разделения партиции при включенном автопартиционировании (см. ниже). Неактивные партиции удаляются автоматически, когда все сообщения из такой партиции удалены по истечении времени хранения.
...
## Транзакции с учаcтием топиков {#topic-transactions}

{{ ydb-short-name }} поддерживает работу с топиками в рамках транзакции. Операции с топиками в транзакции всегда выполняются с уровнем *Serialazable*.

### Транзакционное чтение из топика {#topic-transactions-read}

Данные в топиках не меняются при чтении из топика. Поэтому при чтении в транзакции из топика непосредственно транзакционной операцией является только изменение офсета. SDK скрывает от пользователя детали, при транзакционном чтении через SDK одновременно происходит чтение и коммит офсета.

### Транзакционная запись в топик {#topic-transactions-write}

При транзакционной записи в топик данные до коммита сохраняются в топике вне партиции и затем публикуются (становятся видимыми) в момент коммита транзакции. Видимости собственных изменений в транзакциях с топиками нет.

### Ограничения при работе с топиками в транзакции {#topic-transactions-constraints}

Транзакции не вносят дополнительных ограничений на работу с топиками. Внутри транзакции можно писать большие объемы данных в топик, писать в несколько партиций, читать многими консьюмерами. Тем не менее рекомендовано так подбирать режим работы с транзакциями, чтобы учесть принципы транзакционной работы с топиками: данные публикуются в момент коммита транзакции, то есть если транзакция длинная, то данные станут видимыми только по завершении транзакции.