...
{{ ydb-short-name }} поддерживает работу с топиками в рамках [транзакций](./transactions.md). Операции с топиками в транзакции всегда выполняются с уровнем *Serializable*.

### Транзакционное чтение из топика {#topic-transactions-read}

Данные в топиках не изменяются при чтении. Поэтому при чтении из топика в транзакции непосредственно транзакционной операцией является только изменение [смещения](./glossary.md#offset). SDK скрывают от пользователя эти детали, и при транзакционном чтении из топика через SDK одновременно выполняется чтение и изменение смещения. Фактически смещение будет изменено в момент коммита транзакции.

### Транзакционная запись в топик {#topic-transactions-write}

При транзакционной записи в топик данные до коммита сохраняются в топике вне партиции и затем публикуются (становятся видимыми) в момент коммита транзакции. При этом в момент комита транзакции данные будут добавлены в конец партиции в последовательные офсеты. Видимости собственных изменений в транзакциях с топиками нет.

### Ограничения при работе с топиками в транзакции {#topic-transactions-constraints}

Транзакции не вносят дополнительных ограничений на работу с топиками. Внутри транзакции можно писать большие объемы данных в топик, писать в несколько партиций, читать многими консьюмерами.

Тем не менее рекомендовано так подбирать режим работы с транзакциями, чтобы учесть особенности транзакционной работы с топиками: данные публикуются в момент коммита транзакции. То есть если транзакция длинная, то данные станут видимыми спустя длительное время.