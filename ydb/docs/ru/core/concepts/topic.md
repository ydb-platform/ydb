...
Данные в топиках не меняются при чтении из топика. Поэтому при чтении в транзакции из топика непосредственно транзакционной операцией является только изменение офсета. SDK скрывает от пользователя детали, и при транзакционном чтении через SDK одновременно происходит чтение и коммит офсета.
