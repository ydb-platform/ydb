...
1. Если на топике включено автопартиционирование, его нельзя выключить, можно только поставить на паузу.
...
{{ ydb-short-name }} поддерживает работу с топиками в рамках транзакции. Все транзакции с топиками имеют уровень *Serialazable*

### Транзакционное чтение из топика {#topic-transactions-read}

Данные в топиках не меняются при чтении из топика, поэтому при транзакционном чтении из топика необходимо только транзакционно поменять офсет. SDK скрывает от пользователя детали, при транзакционном чтении через SDK одновременно происходит чтение и коммит офсета.

### Транзакционная запись в топик {#topic-transactions-write}

При транзакционной записи в топик данные промежуточно сохраняются в служебной партиции и затем публикуются (становятся видимыми) при коммите транзакции.
