...
* Гарантируется, что будут обработаны все сообщения топика. Если при обработке сообщения возникнет ошибка, то трансфер будет остановлен с [критичной ошибкой](#error-handling).
* Порядок обработки сообщений топика гарантируется только в рамках одной [партиции](glossary.md#partition) топика. Рекомендуется группировать сообщения, относящиеся к одной строке таблицы, в одну партицию топика.
* Запись в таблицу происходит без использования транзакции: данные в разных [DataShard](glossary.md#data-shard) [строковых таблиц](glossary.md#row-oriented-table) и [ColumnShard](glossary.md#column-shard) [колоночных таблиц](glossary.md#column-oriented-table) могут становиться видимыми для чтения и модификации не одновременно.
