...
2.  **Построение дерева связей**: Каждый узел инициирует подключение (`bind`) к случайному узлу из общего списка, которому он еще не подчинен. Этот процесс формирует ациклический граф, который на промежуточных стадиях является лесом (набором несвязных деревьев), а в конечном итоге — единым деревом, покрывающим все узлы.
3.  **Предотвращение циклов и обмен топологией**: Механизм защиты от циклов основан на постоянном обмене информацией о текущей топологии. Когда узел `А` пытается подключиться к узлу `Б`, он отправляет `Б` все свое известное поддерево. Узел `Б` отклоняет запрос, если `А` уже является его потомком. Если происходит одновременная попытка взаимного подключения, конфликт разрешается в пользу узла с большим `NodeId`.
