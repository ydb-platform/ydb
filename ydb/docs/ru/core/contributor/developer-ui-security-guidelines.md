...
Статья охватывает [политику безопасности контента](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) (Content Security Policy, CSP), защиту от [межсайтовой подделки запросов](https://ru.wikipedia.org/wiki/Межсайтовая_подделка_запроса) (Cross-Site Request Forgery, CSRF) и безопасный вывод данных в HTML.
...
Встроенным тегам `<script>` нужен атрибут `nonce`. Без него браузер блокирует выполнение скрипта согласно активной политике CSP.
...
В `script-src` не добавляют `'unsafe-inline'`, `'unsafe-eval'` и внешние домены. Если скрипт не работает без `'unsafe-inline'`, его переписывают с nonce, см. [{#T}](#inline-script-nonce).
...
В текущем заголовке CSP нет директивы `style-src`, поэтому встроенные стили — атрибуты `style="..."` и блоки `<style>` — браузером не блокируются. Они широко используются на существующих страницах Developer UI: hive monitoring, pdisk, tablet_flat, graph, cms, columnshard, tracing и т. д.; эти места планируют мигрировать, после чего в заголовок добавят более строгую директиву `style-src`.

Отсутствие блокировки в текущей версии не означает, что встроенные стили можно добавлять в новом коде.

{% note warning %}

Не добавляйте встроенные стили в новом коде — атрибуты `style="..."` и блоки `<style>`.
...
**Статус принудительного применения.** Только строка `script-src` в таблице ниже обеспечивается текущим заголовком CSP. Остальные строки описывают целевую политику, к которой движется кодовая база; её соблюдение в новом коде позволит позже включить более строгий заголовок без поломки UI.
...
| `style-src` | только `'self'`, без внешних таблиц стилей | Нет — директива отсутствует в заголовке (см. [{#T}](#csp-and-nonce)) |
| `font-src` | `'self'`, без внешних шрифтов | Нет — директива отсутствует в заголовке |
| `connect-src` | `'self'`, без внешних `fetch()`/XMLHttpRequest (XHR) | Нет — директива отсутствует в заголовке |
| `frame-src` | `'self'`, без внешних iframe | Нет — директива отсутствует в заголовке |
| `img-src` | `'self'` и `data:`, без внешних URL | Нет — директива отсутствует в заголовке |

### Относительные ссылки в HTML {#relative-links}

Страницы мониторинга могут отдаваться под разными префиксами, поэтому в генерируемом HTML не задаются абсолютные пути. В `href`, `src`, `action`, `formaction`, `fetch()`, `$.ajax()` и аналогичных атрибутах используются только относительные ссылки. Не используются:
...
Если странице нужно сослаться на встроенный ресурс, действует правило об относительных ссылках: пути от корня вроде `/static/js/jquery.min.js` или `/jquery.tablesorter.js` в код не зашиваются.

Если нужна библиотека, которой ещё нет во встроенных ресурсах, её добавляют в [`ydb/core/viewer/`](https://github.com/ydb-platform/ydb/tree/main/ydb/core/viewer) и подключают через обёртку или вспомогательную функцию мониторинга, без внешних ссылок и путей от корня в C++-странице.
...
Встроенные обработчики событий — `onclick="..."`, `onchange="..."` и т. п. — блокируются политикой CSP `script-src` даже при наличии nonce, поскольку nonce относится только к блокам `<script>`, а не к inline-атрибутам.
