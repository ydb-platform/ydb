# Распределённое хранилище {{ ydb-short-name }}

{{ ydb-short-name }} distributed storage — это подсистема {{ ydb-short-name }}, которая отвечает за надёжное хранение данных.

Блобовый интерфейс позволяет хранить *блобы* (бинарные фрагменты размером от 1 байта до 10 мегабайт) с уникальным идентификатором. Подсистема также предоставляет примитивы DDisk и PersistentBuffer для прямого блочного доступа. Эти пути используют общую инфраструктуру управления устройствами и размещения, но имеют разные контракты данных и репликации.

## Компоненты и границы ответственности

| Компонент | Ответственность | С чего начать |
|---|---|---|
| DS proxy | Выполняет запросы к группам блобов, включая erasure-кодирование и обеспечение избыточности на уровне группы. | [Исходный код DS proxy](https://github.com/ydb-platform/ydb/tree/main/ydb/core/blobstorage/dsproxy) |
| VDisk | Хранит части блобов группы и поддерживает их индексы, синхронизацию, сборку мусора и состояние восстановления. | [Исходный код VDisk](https://github.com/ydb-platform/ydb/tree/main/ydb/core/blobstorage/vdisk) |
| PDisk | Владеет физическим устройством, выделяет чанки владельцам и предоставляет журналирование и ввод-вывод устройства. | [Исходный код PDisk](https://github.com/ydb-platform/ydb/tree/main/ydb/core/blobstorage/pdisk) |
| DDisk и PersistentBuffer | Предоставляют адресуемое блочное хранилище и сохраняемые промежуточные записи для клиентов прямого блочного доступа. | [{#T}](distributed-storage/ddisk.md), [{#T}](distributed-storage/persistent-buffer.md) |
| NodeWarden | Применяет конфигурацию хранения на узле и управляет локальными акторами PDisk, VDisk/DDisk и прокси. | [{#T}](distributed-storage/node-warden.md) |
| Контроллер BlobStorage (BSC) | Поддерживает пулы хранения, размещение, конфигурацию групп и закрепление ресурсов за группами прямого блочного доступа. | [Исходный код контроллера](https://github.com/ydb-platform/ydb/tree/main/ydb/core/mind/bscontroller) |

Обычный путь блобовых запросов — `tablet → DS proxy → VDisk → PDisk`. При прямом блочном доступе клиент, например партиция NBS, отправляет запросы непосредственно сервисам DDisk и PersistentBuffer. Этот клиент выбирает политику репликации, кворума, повторных попыток и переноса данных из PB. DDisk не реализует блобовый протокол VDisk или его алгоритм восстановления группы.

[{#T}](distributed-storage/direct-block-groups.md) описывает размещение и выделение ресурсов для прямого блочного доступа. Остальная часть этой страницы посвящена обычному блобовому интерфейсу и группам VDisk.

### Общие контракты

- Сервисные идентификаторы акторов обозначают расположение или сервис. Актор, обслуживающий такой идентификатор, может быть заменён при восстановлении; сам по себе идентификатор не доказывает, что запрос относится к текущему экземпляру.
- Поколение таблетки, поколение группы блобов, раунд владельца PDisk и токен соединения DDisk имеют разных владельцев и назначение. На каждом интерфейсе сохраняйте соответствующее значение защиты от устаревших запросов, не подменяя его другим.
- Закрепление ёмкости пула, локальное выделение чанков и завершённая запись данных — разные состояния. Успешное выделение ресурсов контроллером не подтверждает запись данных.
- Успех запроса ограничен областью ответственности получателя. Запись на локальное устройство или одна успешная реплика не означают автоматического подтверждения надёжного сохранения на уровне группы.
- Правила [жизненного цикла акторов и обмена сообщениями](actor-system/index.md) применяются к таблеткам, сервисам хранения, акторам запросов и завершениям их асинхронных операций ввода-вывода.

### Расположение исходного кода

Начните с `ydb/core/blobstorage`: там находятся `dsproxy/`, `vdisk/`, `pdisk/`, `ddisk/` и `nodewarden/`. Общие для нескольких компонентов определения событий и форматов также находятся в `ydb/core/base` и `ydb/core/protos`; транзакции размещения — в `ydb/core/mind/bscontroller`. Низкоуровневая реализация `TUringRouter` находится в `ydb/library/pdisk_io`, а политика партиции NBS — в `ydb/core/nbs/cloud/blockstore/libs/storage/partition_direct`.
...
Для диагностики распределения чанков, выделенных VDisk, используйте внутренний акторный API, описанный в [{#T}](vdisk-space-report.md).
