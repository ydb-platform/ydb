...
Для `block-8-2` PartId 1..8 содержит данные, PartId 9..10 — чётность; основные позиции — 0..9, handoff-позиции — 10..11. Десять закодированных частей размещаются в подгруппе из двенадцати VDisk. Минимум — двенадцать независимых доменов, дополнительный резерв выбирается для конкретного развёртывания. См. [геометрию, версии и единицы ёмкости Block82](../concepts/block-8-2.md).
