...
Следующий актор читает данные блоками по 32 КиБ при размере чанка 128 МиБ, в течение `120` секунд, одновременно выполняются `64` запроса (постоянная нагрузка):
