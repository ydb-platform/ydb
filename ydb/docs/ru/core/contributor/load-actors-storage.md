...
О блочной нагрузке, подобной NBS, через группы прямого блочного доступа (DBG), PersistentBuffer и DDisk см. [{#T}](load-actors-nbs-dbg-like.md). `StorageLoad` проверяет путь хранения блобов через DSProxy/VDisk.
...
| `MaxInFlightWriteBytes` | Максимальный суммарный объём данных в одновременно выполняемых запросах записи, в байтах; ноль означает отсутствие ограничения. |
...
| `MaxInFlightReadBytes` | Максимальный суммарный объём данных в одновременно выполняемых запросах чтения, в байтах; ноль означает отсутствие ограничения. |
...
До старта основной нагрузки записывается блок данных, объемом `1 ГБ`, состоящий из блобов размером от `1 МБ` до `5 МБ`. Чтобы не перегрузить систему запросами записи, количество одновременно выполняемых запросов ограничивается `5`. После завершения записи этого блока запускается readonly нагрузка с возрастающей частотой: с `10` до `50` запросов в секунду, частота возрастает равномерно на протяжении `300` секунд.
