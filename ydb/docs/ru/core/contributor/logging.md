...
3. Логирование происходит в ходе работы [актора](../concepts/glossary.md#actor), и для работы с контекстом (в частности, отправки сообщений актору логирования) доступна переменная `NActors::TlsActivationContext`.
...
Не записывайте в лог пароли, токены доступа, ключи шифрования, строки подключения и персональные данные. Если значение необходимо для диагностики, перед передачей в макрос удалите чувствительную часть, замаскируйте её или замените необратимым хешем.
