...
[настроен](../reference/configuration/log_config#entry-objects) уровень логирования.
...
7. Если числовое значение параметра человеку удобнее анализировать не в десятичном виде, то лучше это значение указывать как символьную строку и дополнять общепринятыми символами (например, у шестнадцатеричного представления должен быть префикс `0x`, а у двоичного — суффикс 'B' и т.д.). Хорошей практикой является дополнение названия параметра суффиксом `Hex` или т.п.
