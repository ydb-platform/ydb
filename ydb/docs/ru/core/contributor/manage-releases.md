...
Раньше публичные версии сервера {{ ydb-short-name }} часто обозначались тремя числами, например `24.3.3` или `24.2.7`. Четвертое число — номер патча, то есть итерации релиза внутри минорной версии — появилось в публичных релизах линеек `24.3` и `24.4`, например `24.3.11.13` и `24.4.4.2`. Актуальный список версий — на [{#T}](../downloads/ydb-open-source-database.md).

С мажорного релиза `25.1` действует новая схема релизных веток. Релизные теги создаются в минорных ветках `stable-XX-Y-Z`, в которые можно мерджить только исправления ошибок. В мажорную ветку `stable-XX-Y` по согласованию можно мерджить новые фичи. Подробнее — в разделе [{#T}](#release_branch_scheme).
