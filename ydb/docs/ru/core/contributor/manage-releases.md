...
* **Хотфиксная ветка** - ветка для срочного исправления критических ошибок в конкретном релизном теге. Именуется `stable-XX-Y-Z-A-hotfix-N` (где `XX.Y.Z.A` - имя релизного тега, N-порядковый номер хотфикса, если потребовалось более одного обновления), например, `stable-24-1-1-2-hotfix-1`. Такие ветки отводятся только от релизных тегов при необходимости сделать хотфикс. При необходимости сделать хотфикс поверх ранее сделанного хотфикса, ветка для нового хотфикса отводится от ветки с последним хотфиксом. В хотфиксные ветки можно мержить только исправления критических ошибок, которые требуется немедленно выкатить в продакшн.
