...
* **Фича**. К фичам относятся любые добавляющие новую функциональность или улучшающую существующую изменения, не связанные с исправлением ошибок.
...
Активная ветка - ветка, код из которой может попасть в продакшен.
...
3. После согласования из п.2 замерджите исправления во все активные минорные ветки.
...
1. Определите версию базы {{ ydb-short-name }} (соответствует номеру релизного тега), на которой возникла проблема.
2. Найдите подходящую хотфиксную ветку в формате `stable-XX-Y-Z-A-hotfix` (где `XX-Y-Z-A` номер релизного тега).
...
В минорные ветки, начиная с `stable-XX-Y-2`, добавляется не более трех новых фич (не считая включения feature-флагов по дефолту, см. [раздел](#server-compatibility)). Это означает, что фича может не попасть в ближайший релизный тег. Это ограничение введено, чтобы сократить время на отладку релизных тегов.

### Тестирование {#server-testing}

Каждая минорная версия проходит [приемочное тестирование](https://en.wikipedia.org/wiki/Acceptance_testing). После чего код из минорной ветки проходит комплексную проверку, включая развертывание во внутренних кластерах - testing, prestable, и production. 
