...
С мажорного релиза `25.1` действует схема релизных веток, в которой релизные теги создаются в минорных ветках `stable-XX-Y-Z` (в них можно мерджить только исправления ошибок), а в мажорную ветку `stable-XX-Y` по согласованию можно мерджить новые фичи. Подробнее — в разделе [{#T}](#release_branch_scheme). Как устроен номер версии сервера — в статье [{#T}](../devops/concepts/versioning.md). Актуальный список версий — на [{#T}](../downloads/ydb-open-source-database.md).
...
* Полная версия - это комбинация всех четырёх чисел (например, `25.1.2.1`)

Как устроена структура номера версии и какие компоненты могут опускаться в зависимости от контекста, см. в статье [{#T}](../devops/concepts/versioning.md).
