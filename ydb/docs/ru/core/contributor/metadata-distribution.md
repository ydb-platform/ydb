...
Эта статья объясняет, зачем нужны эти подсистемы, как они устроены и как работают. Обзор без деталей ядра для пользователей документации — в разделе [Сервисы распространения метаданных](../concepts/architecture/metadata-services.md). Инструкции по конфигурированию и изменению конфигурации описаны в разделе [Конфигурирование подсистем распространения метаданных](../devops/configuration-management/configuration-v2/state-storage-reconfiguration.md).

## Зачем нужны подсистемы распространения метаданных {#why}

Кластер {{ ydb-short-name }} — это распределённая система, в которой одновременно могут работать миллионы [таблеток](../concepts/glossary.md#tablet) на тысячах узлов. Чтобы компоненты кластера могли взаимодействовать друг с другом, им необходимо знать:
...
**StateStorage** — это распределённый сервис, который хранит актуальное состояние [таблеток](../concepts/glossary.md#tablet): где работает лидер, каков его идентификатор актора, поколение и шаг.

#### Что хранит StateStorage {#state-storage-data}

Для каждой таблетки StateStorage хранит:

- **Текущий лидер** — идентификатор актора ([ActorId](../concepts/glossary.md#actorid)) лидера таблетки.
...
SchemeBoard является **кешем схемных метаданных** для всех компонентов кластера. Когда узел базы данных получает запрос, ему необходимо знать структуру таблиц, с которыми работает запрос. Вместо того чтобы каждый раз обращаться к [SchemeShard](../concepts/glossary.md#scheme-shard) (таблетке, которая является источником истины для схемы), узел читает схему из локального кеша, который синхронизируется через SchemeBoard.
...
### Автоматическая реконфигурация (SelfHeal) {#self-heal}

В кластерах с конфигурацией V2 доступен механизм **SelfHeal подсистем распространения метаданных** — автоматическое управление конфигурациями State Storage, Board и SchemeBoard. Он отслеживает состояние узлов кластера и при необходимости:

- переносит реплики с вышедших из строя узлов на работоспособные;
- может добавлять новые реплики при расширении кластера с учётом настроек и доступных узлов.

SelfHeal работает через тот же механизм групп колец, что и ручная реконфигурация, но выполняет все шаги автоматически. Подробнее — в разделе [SelfHeal подсистем распространения метаданных](../devops/concepts/selfheal-metadata-distribution.md).

### Группы колец в конфигурации двух дата-центров {#two-dc}

В конфигурации с двумя дата-центрами (режим [bridge](../concepts/glossary.md#bridge)) каждый дата-центр (pile) имеет **собственную группу колец**. Это позволяет:
...
Подробнее о режиме bridge описано в разделе [Режим bridge](../concepts/bridge.md).

## Отказоустойчивость и модель отказа {#fault-tolerance}

Подсистемы распространения метаданных спроектированы с учётом [модели отказа](../concepts/glossary.md#fail-domain) {{ ydb-short-name }}, основанной на концепциях доменов отказа (обычно — серверных стоек) и областей отказа (обычно — дата-центров).
...
- [SelfHeal подсистем распространения метаданных](../devops/concepts/selfheal-metadata-distribution.md) — автоматическое управление конфигурациями State Storage, Board и SchemeBoard.
