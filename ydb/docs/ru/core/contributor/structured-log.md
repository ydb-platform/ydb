...
Макрос `YDB_LOG_CTX_COMP` полностью укладывается в данную схему именования: он требует передачи уровня сообщения в качестве аргумента, а так же явного указания контекста исполнения и кода компонента.
...
Если один и тот же набор параметров прикрепляется к различным сообщениям, то для избежания дублирования кода можно заранее создать этот набор параметров, а затем его переиспользовать при логировании различных событий. Для этого предназначен макрос `YDB_LOG_CREATE_MESSAGE`. В качестве аргументов он принимает набор параметров (в виде пар `{key, value}`), а в качестве результата возвращает некий объект, который впоследствии может быть использован при отправке сообщений в журнал.
...
Результат вызова макроса `YDB_LOG_CREATE_MESSAGE` является экземпляром класса `TStructuredMessage`. Этот экземпляр является самодостаточным в том смысле, что внутри себя хранит набор входящих в него параметров, их имена, типы и значения, но при этом не хранит каких-либо ссылок на внешние структуры данных. Соответственно, экземпляры класса `TStructuredMessage` можно хранить в локальных и глобальных переменных, копировать, передавать и т.д. Однако, его главное назначение - использование при логировании сообщений.
...
2. Каждая пара `{key, value}` располагается на отдельной строке.
...
{% cut "Пример логирования с несколькими параметрами:" %}
...
> Если взять текст сообщения и к нему добавить значения параметров в виде `(key=value, key=value, ... )`, то должно получиться осмысленное, законченное, однозначное и понятное человеку текстовое сообщение.
...
```
 ...
 Если использовать упомянутое ранее правило согласования текстов сообщений и названия параметров, то получится предложение `Response timeout elapsed (node=<строка>, request=<число>, wait=<число>, timeout=<число>)`, где непонятно следующее:

 1. Значение `node` - это имя хоста или некоторое внутренее название-идентификатор узла.

 2. Значение `request` - это порядковый номер или числовой идентификатор?

 3. Значение `wait` - что это такое?

 4. В каких единицах задано значение `wait`?
