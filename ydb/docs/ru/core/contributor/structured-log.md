...
### Общие рекомендации по именовананию отдельных параметров
...
3. Значение `wait` - что это такое и как интерпретировать это значение?
...
### Частные рекомендации по именовананию отдельных параметров
...
3. Названия параметров должны состоять из нескольких слов, причём главным должно быть последнее, а каждое предшествующее слово должно уточнять последующее. Например:
...
1. Идентификатором какой сущности является параметр `srcId`.

2. Как трактовать значение `dstshard` - это идентификатор, название или что-либо ещё.
...
5. Если в названии параметра используется общепринятая в {{ydb-short-name}} аббревиатура или термин, то её следует писать в том регистре, в котором принято. Примеры названия таких параметров:
...
7. Если параметр содержит числовую величину, то желательно к названию параметра добавлять суффикс:
...
9. Если параметр содержит текстовую информацию, то желательно к названию параметра добавлять суффикс, позволяющий легче интерпретировать значение параметра (например, `Name`, `Desc`, `Json`, `Base64` и т.п.).

10. Для именования параметров можно применять правила именования локальных переменных (например, для логических признаков следует в начало названия параметра добавлять `is`, `has` или т.п).
