...
При создании Pull Request'а описание будет заполнено текстом из шаблона, который нужно отредактировать следующим образом:
 
>**Changelog Entry.**
>В этот блок следует добавить описание изменения для конечных пользователей системы (см. требования). Содержимое этого блока будет опубликовано в Release Notes в документации.
>
> **Changelog Category.**
В этом блоке нужно оставить одну категорию, которой соответствует изменение (см. как выбрать категорию). Есть категории, для которых сообщения из Changelog Entry не будут опубликованы в Release Notes. Остальные категории определяют раздел Release Notes, в который попадет сообщение из Changelog Entry.
>
> **Additional Information.**
В этот блок нужно добавить ссылку на задачу в трекере (внутреннем или внешнем) и любую дополнительную информацию, которую вы считаете  полезной для code review. Содержимое этого блока не попадет в Release Notes.

#### Как выбрать Changelog Category

Если ваше изменение не видно для конечного пользователя, используйте категории:

Если изменение конечно и видно пользователю, используйте категории, для которых есть соответствующий блок в Release Notes:

#### Требования к Changelog Entry

Сообщение в Changelog Entry должно отвечать следующим требованиям:
- содержать не менее 20 символов, если выбранная категория из списка публикуемых,
- содержать термины, которые используются в глоссарии YDB,
- описывать, что в работе системы изменилось для конечного пользователя,
- написано на английском языке,
- в зависимости от категории изменений нужно добавить в сообщение:
  - New feature, Experimental feature:
    - ссылку на документацию к новой функциональности,
    - описание, как включить функциональность, если она релизится под флагом (если описание больше одного предложения, стоит вынести его в документацию).
  - Performance
    - производительность какой части системы изменилась,
    - насколько изменилась производительность.
   - Bugfix
     - ссылка на ISSUE в github или таск в Трекере, где описано: при каких условиях пользователь мог столкнуться с проблемой, как проблема проявлялась, как была исправлена (необязательно, если логика работы системы принципиально не изменилась).
