...
После создания Pull Request будет запущена [предварительная проверка](#precommit_checks), которая убеждается, что:
...
В `main` и `stable-*` ветках в корне лежит файл CHANGELOG.md. В `main` он содержит изменения для всех выпущенных версий и изменения, которые попадут в следующий релиз. В `stable-*` — изменения, которые были в `main` на момент отщепления, а также изменения, которые были домержены.
