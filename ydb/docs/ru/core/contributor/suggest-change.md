...
|| Not for Changelog
| -
| Изменения, которые не видны конечному пользователю — рефакторинг кода, тесты, CI/CD, исправления ошибок в коде новой функциональности (которая не попала в предыдущие релизные версии).
| Не будет опубликовано в истории изменений. Сообщение заполнять не нужно.
||
|#
...
#### Сообщение из Changelog Entry будет опубликовано в списке изменений

Готовя [стабильный релиз](manage-releases.md#server-stable), [релизная команда {{ ydb-short-name }}](https://github.com/orgs/ydb-platform/teams/release) собирает блоки Changelog Entry из описаний Pull Request, замерженных в соответствующую ветку, и публикует их в [списке изменений](../changelog-server.md) в документации. Для сбора используется скрипт [ydb/tools/release_notes/changelog.py](https://github.com/ydb-platform/ydb/tree/main/ydb/tools/release_notes), который читает блоки Changelog Entry и Changelog Category прямо из описаний Pull Request. Pull Request с категорией, у которой нет [раздела в списке изменений](#choose_category), в список изменений не попадают.

На ревью проверят, что выбрана верная категория и описание соответствует [требованиям](#changelog_entry_req). Если описание не будет отвечать требованиям, то автора Pull Request попросят исправить сообщение.

#### Как изменить сообщение в Changelog Entry

Пока сообщение не опубликовано в списке изменений, достаточно отредактировать блок Changelog Entry в описании Pull Request. Это можно сделать и после мержа — сбор сообщений происходит позже, при подготовке релиза.

Если сообщение уже опубликовано, исправить его можно отдельным Pull Request, который меняет файл списка изменений в документации: `ydb/docs/ru/core/changelog-server.md` и его англоязычную версию `ydb/docs/en/core/changelog-server.md`.
