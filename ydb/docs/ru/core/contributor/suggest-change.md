...
#### Автоматический Cherry-pick (Рекомендуется) {#auto_cherry_pick}

После слияния вашего PR в `main`, вы можете использовать функцию автоматического cherry-pick для простого создания PR с переносом в стабильные ветки.

1. **Предложение**: После слияния PR бот автоматически опубликует комментарий с предложением доступных стабильных веток для cherry-pick.

2. **Команда cherry-pick**: Ответьте на слитый PR комментарием с командой cherry-pick:
   ```
   /cherry-pick stable-25-1 stable-24-4
   ```

3. **Автоматический процесс**: Система:
   - Проверит, что указанные ветки существуют
   - Создаст новые ветки для каждой целевой стабильной ветки
   - Применит ваши изменения через `git cherry-pick`
   - Создаст pull request'ы для каждой ветки
   - Сообщит о любых конфликтах или ошибках

**Примеры использования:**
- Cherry-pick в несколько веток: `/cherry-pick stable-25-1 stable-24-4 stable-23-3`
- Cherry-pick в одну ветку: `/cherry-pick stable-25-1`

**Кто может использовать эту функцию:**
- Автор оригинального PR
- Коллабораторы репозитория с правами записи

**Примечание**: Если cherry-pick не удался из-за конфликтов, вам нужно будет создать PR вручную, используя процесс ниже.

#### Ручной Cherry-pick {#manual_cherry_pick}

Когда есть необходимость вручную перенести патч в стабильную ветку, отведите ветку от ветки stable:
