...
2. **Changelog Category.** В этом блоке нужно оставить одну категорию из списка, которой соответствует изменение. Есть категории, для которых сообщения из Changelog Entry не будут опубликованы в списке изменений. Остальные категории определяют раздел списка изменений, в который попадет сообщение.
...
- отвечать дополнительным требованиям, которые определяются выбранной категории.
