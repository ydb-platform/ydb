...
Хаос-тестирование — это методология проверки устойчивости {{ ydb-short-name }} путём намеренного внесения сбоев в работающий кластер. Цель — убедиться, что {{ ydb-short-name }} корректно переживает реальные отказы: потери узлов, сетевые разделения, проблемы с дисками и другие нештатные ситуации. За внесение этого хаоса отвечает инструмент **[Nemesis](https://github.com/ydb-platform/ydb/tree/main/ydb/tests/stability/nemesis)**.
...
Результат запуска Nemesis оценивается по двум [свойствам](https://en.wikipedia.org/wiki/Safety_and_liveness_properties): **живучесть (liveness)** и **безопасность (safety)**. Критерии успеха и отказа:
