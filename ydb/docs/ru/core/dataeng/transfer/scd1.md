...
## Реализация SCD1 c помощью трансфера {#scd1}
  ...
  С помощью Transfer можно реализовать SCD1-soft: удаление помечается флагом и сохраняется момент удаления. Если нужна физическая очистка данных, ее можно выполнять или через явную команду DELETE или с помощью TTL.
