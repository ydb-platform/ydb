...
- SCD2 append only
  ...
  Существуют различные подходы для построения SCD2 хранилищ. Так как трансфер не может выполнять поиск исторических данных в таблицах, то при условии использовании только трансфера, возможно построение только SCD2 append only хранилищ. В этом режиме данные только добавляются, что позволяет выполнять операции с максимальной эффективностью, но усложняется поиск хранимых данных.
  ...
  **1. Вставка или обновление новой записи (создаст событие с `op = "с"`):**
...
- SCD2 с помощью таблицы хранения промежуточных данных и операции слияния
  ...
  2. Создайте трансфер для переноса данных:
  ...
  4. Создайте периодический запроса подмерживания изменений из таблицы `dimension_scd_changes` в основную таблицу хранения данных в формате SCD2.
