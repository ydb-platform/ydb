...
Node Broker может автоматически генерировать для динамических узлов имена, которые идентифицируют узел в рамках тенанта и отображаются в метриках вместо имени хоста. Если динамический узел был выключен, то по прошествии таймаута его имя может занять новый динамический узел. Благодаря этому имена узлов могут переиспользоваться при смене имен хостов.

Для того чтобы включить автоматическую генерацию имен узлов, необходимо добавить в конфигурацию следующее:
...
Имя узла состоит из префикса и порядкового номера узла в рамках тенанта. По умолчанию, префиксом является `slot-`. Для того, чтобы переопределить префикс, необходимо добавить в конфигурацию следующее:

```yaml
node_broker_config:
  node_name_prefix: <новый префикс>
```
