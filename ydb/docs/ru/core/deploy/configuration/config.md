...
Параметр | Описание
--- | ---
`host` | Имя хоста, на котором работает LDAP сервер. Это устарелый параметр, вместо него должен использоваться параметр `hosts`
...
`enable` | Определяет будет ли попытка установить TLS соединение с использованием запроса `StartTls`. При выставлении значения параметра в `true`, необходимо отключить использование схемы соединения `ldaps`, присвоив параметру `ldap_authentication.scheme` значение `ldap`
