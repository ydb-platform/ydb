...
Node Broker может присваивать динамическим узлам *виртуальные* имена. Виртуальное имя идентифицирует узел в рамках тенанта. Если динамический узел был выключен, то по истечении таймаута его виртуальное имя может быть занято новым динамическим узлом, обслуживающим того же тенанта. Без использования виртуальных имен количество известных имен динамических узлов в кластере может расти бесконечно, если новые динамические узлы всегда получают уникальные имена хостов, что часто встречается в облачных средах.

Чтобы включить виртуальные имена узлов, необходимо добавить в конфигурацию кластера следующее:
...
Виртуальное имя узла состоит из префикса и порядкового номера узла в рамках тенанта. По умолчанию, префиксом является `slot-`. Для того, чтобы переопределить префикс, необходимо добавить в конфигурацию кластер следующее:

```yaml
node_broker_config:
  virtual_node_name_prefix: <новый префикс>
```
