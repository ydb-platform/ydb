...
По умолчанию используется анонимная авторизация. А подключение к кластеру YDB с использованием токена авторизации будет иметь следующий вид:
...
Клиент предоставляет следующий API для выполнения запросов над таблицами:
...
Для выполнения YQL-запросов и чтения результатов используется методы `query.Session.Query`, `query.Session.QueryResultSet` и `query.Session.QueryRow`.
