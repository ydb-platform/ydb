...
Метод `table.Session.CreateTable()` не позволяет создавать колоночные таблицы. Это можно сделать с помощью метода `table.Session.Execute()`, который выполняет YQL-запросы. Работа с методом `table.Session.Execute()` описана в разделе [Получение выборки данных](#query-processing). 
