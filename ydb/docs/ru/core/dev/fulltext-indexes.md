...
* генерирует значения `__ydb_row_id` автоматически при вставке строк (эту колонку вы не задаёте сами);
...
* `__ydb_row_id` генерируется на стороне сервера; не записывайте в неё значения. Её нельзя задать явно, в том числе через `BulkUpsert`.
...
* Поддерживается первичный ключ любого типа. Для таблиц, первичный ключ которых не является одной целочисленной колонкой, автоматически создаётся управляемая системная колонка `__ydb_row_id` и уникальный индекс `uniq__ydb_row_id` (см. [Типы первичного ключа](#primary-key)).
