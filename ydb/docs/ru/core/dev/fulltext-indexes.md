...
* **Любой другой первичный ключ** (например `Utf8` / `String`, нецелочисленный тип или составной ключ из нескольких колонок) — {{ ydb-short-name }} поддерживает системную колонку `__ydb_row_id` типа `Uint64` и использует её в качестве `doc_id`.
...
* автоматически генерирует значение `__ydb_row_id` для каждой строки, в которой эта колонка не указана в `INSERT` / `UPSERT` (обычно вы её не задаёте сами);
* создаёт уникальный [вторичный индекс](../concepts/glossary.md#secondary-index) с именем `uniq__ydb_row_id` по колонке `__ydb_row_id`. Во время запроса этот индекс сопоставляет найденный `__ydb_row_id` с первичным ключом таблицы перед чтением строки из основной таблицы.

Если на таблице создаётся несколько полнотекстовых индексов, все они **переиспользуют** одну и ту же колонку `__ydb_row_id` и индекс `uniq__ydb_row_id` — эти структуры создаются для таблицы только один раз.

{% note warning %}

Колонкой `__ydb_row_id` и индексом `uniq__ydb_row_id` управляет {{ ydb-short-name }}:

* Позвольте {{ ydb-short-name }} заполнять `__ydb_row_id` — не указывайте её в `INSERT` / `UPSERT`, и значение проставится автоматически. Если вы зададите значение явно, оно сохранится как есть, и вы сами отвечаете за его уникальность (индекс `uniq__ydb_row_id` отклоняет дубликаты). `BulkUpsert` не поддерживает задание `__ydb_row_id` и отклоняет запросы, в которых она указана.
* Индекс `uniq__ydb_row_id` нельзя удалить, пока от него зависит хотя бы один полнотекстовый индекс. Сначала удалите зависимые полнотекстовые индексы.
...
* Поддерживается первичный ключ любого типа. Для первичного ключа, не являющегося одной целочисленной колонкой, {{ ydb-short-name }} автоматически создаёт системную колонку `__ydb_row_id` и уникальный индекс `uniq__ydb_row_id` (см. [Типы первичного ключа](#primary-key)).
