...
Полнотекстовый индекс с фильтрацией позволяет выполнять полнотекстовый поиск в рамках логического раздела, заданного значениями дополнительных колонок. Чтобы создать такой индекс, укажите одну или несколько дополнительных колонок перед текстовой колонкой в секции `ON`. Последней колонкой должна быть текстовая; остальные (колонки фильтрации) могут быть любого сравнимого типа:
...
* Для таблиц с полнотекстовыми индексами поддерживается только первичный ключ из одной колонки типа `Uint64`.
...
* [Полнотекстовые индексы с фильтрацией](#filtered): для каждой колонки фильтрации требуется предикат равенства; колонки фильтрации не должны быть колонками первичного ключа.
