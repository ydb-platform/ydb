...
JSON-индекс сужает множество строк по токенам пути (и, для равенства, по токену «путь + значение»). Затем движок исполнения запросов повторно проверяет полный предикат. Запросы с `!=`, `<`, `>=`, `BETWEEN`, `starts with`, `like_regex` и т. п. могут использовать индекс для отсечения по пути, но итоговую точность сравнения обеспечивает пост-фильтр. Результат поиска по индексу всегда совпадает с выполнением того же запроса без индекса.
