...
Индексируемы только выражения на основе `JSON_EXISTS` и `JSON_VALUE` в блоке `WHERE`, объединённые операторами `AND` / `OR` по правилам ниже. Остальные условия ведут себя по-разному в зависимости от оператора.
...
Подробнее о полнотекстовых индексах: [Полнотекстовые индексы](../dev/fulltext-indexes.md).
...
- [Вторичные индексы](../dev/secondary-indexes.md) — общие сведения о глобальных индексах и `VIEW`.
- [Полнотекстовые индексы](../dev/fulltext-indexes.md) — родственный механизм инвертированного поиска по токенам.
