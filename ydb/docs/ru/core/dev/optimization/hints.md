...
Например, в следующем запросе используются три подсказки `Rows`, задающие кардинальность (т.е. ожидаемое количество записей), а также подсказка полного порядка соединений `JoinOrder` и подсказка выбора алгоритма соединения `JoinType`:
...
Если порядок соединений не зафиксирован отдельной подсказкой, оптимизатор построит оба варианта планов: где делаются запросы в левый и правый входы соединения. Если порядок соединений зафиксирован подсказкой, запросы будут делаться в правую сторону соединения.
...
Запустим запрос без подсказок кардинальности и затем посмотрим, как подсказки меняют план запроса.
