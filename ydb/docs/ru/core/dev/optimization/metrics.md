...
{{ ydb-short-name }} — распределённая СУБД для больших объёмов данных. [Кластер](../../concepts/glossary.md#cluster) может включать много [узлов](../../concepts/glossary.md#node); вычислительный граф запроса тогда состоит из сотен и тысяч задач, каждая со своими метриками. Показать все значения по отдельности и по ним же делать выводы нереалистично, поэтому метрики задач **агрегируются по стадии**, попадают в ответ сервиса и на диаграмму уже в агрегированном виде.

На SVG-плане метрики расположены в колонках `Tasks`, `Stages` и `Timeline` — см. [Расположение информации на плане](layout.md). Они соответствуют структуре [плана выполнения запроса](plans.md) и [графа стадий](structure.md). Эта статья завершает [рекомендуемый маршрут](index.md#reading-order) раздела: ниже — как читать числа, полосы и временные графики.
...
Подробнее по колонкам — [Расположение информации на плане](layout.md); по стадиям и операторам на графе — [Структура плана запроса](structure.md).
...
«Загруженность» стадии и всего графа выполнения запроса зависит от многих факторов, в том числе от других запросов на кластере. Для интерпретации CPU используется несколько представлений.
...
Так сопоставляются разные величины — суммарный CPU и обработка строк, нормированная по длительности, — и становится проще искать узкое место в графе выполнения.
...
- Сравните полосы `Stages` и `Timeline` — найдите стадии с наибольшим CPU, памятью или трафиком.
- Проверьте `Tasks` и [перекос данных](#dataskew): неравномерная нагрузка удлиняет стадию.
- Сопоставьте метрики с [структурой плана](structure.md) — узкое место часто совпадает с тяжёлой стадией или каналом.
- Вернитесь к [плану выполнения запроса](plans.md) — сравните оценки `EXPLAIN` с фактическими метриками `ANALYZE`.
