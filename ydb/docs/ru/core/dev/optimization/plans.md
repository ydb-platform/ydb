...
В графическом интерфейсе {{ ydb-short-name }} также можно получить план выполнения, для этого надо использовать кнопку `[Explain]`. В UI доступно больше вариантов плана. По умолчанию показывается закладка `Computation Graph` граф вычислений в виде операторов и связи между ними:
