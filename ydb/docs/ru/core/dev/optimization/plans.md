...
В самой правой колонке показана структура запроса. В ней написано, что сначала сервер планирует (последовательность выполнения идёт снизу вверх):
...
Аналогично, план запроса доступен в UI при обычном выполнении кнопкой `[Run]`. Как и в случае с CLI, в план добавляются актуальные метрики:
