...
Если в качестве первичного ключа используется `Uuid`, предпочтительнее [`Uuid::newV8ColumnKey`](../../yql/reference/udf/list/uuid.md#newv8columnkey): функция формирует ключи, упорядоченные по времени во внутреннем формате байтов {{ ydb-short-name }}. Стандартный [UUIDv7](../../yql/reference/udf/list/uuid.md#rfc-v7) не сохраняет хронологический порядок при mixed-endian хранении в {{ ydb-short-name }}. См. также [UUID в качестве первичного ключа](row-oriented.md#uuid-primary-key).
