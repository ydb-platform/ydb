...
* **`Uuid::newV8RowKey`** — вариант по умолчанию для строчных таблиц с интенсивной вставкой. У каждого ключа случайный 12-битный префикс (4096 групп значений) и встроенное текущее время с точностью до секунды (по модулю 2<sup>31</sup>). Запись распределяется по партициям; строки, созданные в близкие моменты, остаются относительно рядом в пространстве ключей внутри своей «корзины» префикса.
* **`Uuid::newV8ColumnKey`** — хронологический порядок в хранимых байтах. Предпочтителен для колоночных таблиц, которые используют хеш-партиционирование и выигрывают от упорядоченности ключей по времени внутри партиции.
* **`Uuid::newV8RowGroup`** — возвращает список ключей строчной раскладки с общим префиксом (например, для всех строк одной транзакции). Такие ключи обычно попадают в одну партицию, что снижает межпартиционные накладные расходы операции. Для баланса таблицы в целом задавайте новый случайный префикс на каждую транзакцию.
...
{% cut "Пример: таблица событий с UUID-ключом для строчной таблицы" %}
