...
Экспериментальный [модуль `Uuid`](../../yql/reference/udf/list/uuid.md) предоставляет генераторы, настроенные под порядок байтов ключей в {{ ydb-short-name }}:

* **`Uuid::newSharded`** — вариант по умолчанию для таблиц с интенсивной вставкой. У каждого ключа случайный 10-битный префикс (около 1024 корзин партиций) и встроенное текущее время с точностью до секунды. Запись распределяется по партициям; строки, созданные в близкие моменты, остаются относительно рядом в пространстве ключей.
* **`Uuid::newChrono`** — более выраженный хронологический порядок в хранимых байтах. Предпочтителен, когда запросы часто сканируют временной диапазон по первичному ключу или когда ключи индексов должны следовать времени создания.
...
* **`Uuid::newV7` / `Uuid::newV7At`** — стандартный [RFC 9562 UUID версии 7](https://datatracker.ietf.org/doc/html/rfc9562). Используйте при необходимости совместимости с внешними инструментами v7 или [`Uuid::extractTs`](../../yql/reference/udf/list/uuid.md#extract-ts). Для производительности строковых таблиц внутри {{ ydb-short-name }} обычно лучше подходят `newChrono` или `newSharded`.
