...
- {{ ydb-ui-name }}
  ...
  План запроса также можно получить через [{{ ydb-ui-name }}](../../reference/ydb-ui/ydb-monitoring.md). Для этого необходимо перейти на страницу базы данных в раздел `Query` и нажать `Explain`:
