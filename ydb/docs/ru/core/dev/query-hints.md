...
- BroadcastJoin - это тип объединения, при котором один из наборов данных достаточно мал, что позволяет скопировать (разослать) его на все необходимые узлы в кластере. Это позволяет каждому узлу выполнять объединение локально, не передавая данные по сети.
...
- LookupJoin - по каждой строке одного входа делается Lookup в таблицу или индекс, в данный момент поддерживается только для [строковых таблиц](../concepts/datamodel/table.md).
