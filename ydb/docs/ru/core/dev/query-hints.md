...
- ShuffleJoin - когда оба входа большие, данные распределяются на узлы по ключу соединения, и локально выполняют соединение.
- LookupJoin - по каждой строке одного входа делается запрос в таблицу или индекс другого входа, позволяющий очень эффективно выполнить операцию, в данный момент поддерживается только для [строковых таблиц](../concepts/datamodel/table.md).
