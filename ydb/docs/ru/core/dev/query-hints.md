...
Подсказкм оптимизатора позволяют влиять на поведение стоимостного оптимизатора при планировании выполнения SQL-запросов. {{ydb-short-name}} поддерживает четыре типа подсказок для управления соединениями (joins) и статистикой.
...
Если оптимизатор не смог применить одну из указанных подсказок к запросу, пользователь будет оповещен через WARNING.
...
JoinTree - представление бинарного дерева с помощью скобок, например:
  (R S) (T U) 
```

## Требования к CBO

**Важно:** Все подсказки (`Rows`, `Bytes`, `JoinOrder`) работают только с **включенным** [стоимостным оптимизатором](../concepts/optimizer.md), кроме `JoinType` - его можно указывать и для выключенного CBO.
...
**Принцип работы**
Оптимизатор будет рассматривать только те планы, в которых притуствует заданный частичный или полный порядок соединений.
