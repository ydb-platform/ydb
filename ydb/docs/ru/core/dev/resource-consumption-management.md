...
Чтобы выполнить запрос в пуле, пользователь должен иметь [право доступа](../yql/reference/syntax/grant.md#permissions-list) `SELECT` на этот пул. Пример выдачи прав:
...
Классификаторы пула ресурсов глобальны для всей базы данных и применяются ко всем пользователям. Для создания, удаления или изменения классификатора пула ресурсов необходимо иметь [право доступа](../yql/reference/syntax/grant.md#permissions-list) `USE` на всю базу данных, которое можно выдать запросом вида:
...
Для использования классификатора у пользователя должен быть [доступ к пулу ресурсов](#run-access), на который этот классификатор ссылается. Если такого доступа нет, классификатор пропускается и проверяется следующий. В будущих версиях это поведение может быть изменено, поэтому не используйте отсутствие прав на пул как механизм управления тем, какой классификатор сработает.
...
Рассмотрим пример разделения ресурсов между командой аналитиков и условным директором (CEO). Важно, чтобы запросы CEO продолжали выполняться даже тогда, когда аналитические задачи загружают базу данных. Конфигурация для этого сценария может выглядеть следующим образом:
...
- **Пул ресурсов `olap`**: `DATABASE_LOAD_CPU_THRESHOLD=80` означает, что при достижении загрузки CPU базы данных в 80% новые аналитические запросы перестают запускаться и ожидают в очереди, вмещающей до `QUEUE_SIZE=100` запросов.
- **Пул ресурсов `the_ceo`**: параметр `DATABASE_LOAD_CPU_THRESHOLD` не задан, поэтому запросы CEO продолжают запускаться независимо от степени загрузки базы данных.

Это обеспечивает CEO приоритет при перегрузке: пул аналитики ограничивается первым, а пул CEO — нет.

{% note warning %}

Этот механизм управляет только *моментом запуска запросов*. Он не резервирует CPU для одного пула за счёт другого, пока запросы уже выполняются: оба пула выше ограничены значением `TOTAL_CPU_LIMIT_PERCENT_PER_NODE=100`, поэтому выполняющиеся запросы конкурируют за CPU на равных условиях.
