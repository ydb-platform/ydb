...
Для использования классификатора пула ресурсов у пользователя должен быть [доступ к пулу ресурсов](../yql/reference/syntax/create-resource-pool-classifier.md#run-access), на который ссылается данный классификатор.
