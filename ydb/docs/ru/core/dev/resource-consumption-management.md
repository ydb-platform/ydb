...
Классификаторы пула ресурсов глобальны для всей базы данных и применяются ко всем пользователям. Для создания, удаления или изменения классификатора пула ресурсов необходимо иметь [разрешение](./grant.md#permissions-list) `ALL` на всю базу данных, которое можно выдать запросом вида:
