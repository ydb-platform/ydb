...
Если в очереди уже находится $QUEUE\_SIZE = 1000$ запросов, то при отправке 1001-го запроса клиент сразу же получит в ответ ошибку, и этот запрос не будет выполнен.
...
В примере ниже приведён [классификатор пула ресурсов](../concepts/glossary.md#resource-pool-classifier), который отправляет запросы от всех пользователей в пул ресурсов с именем `olap`:
