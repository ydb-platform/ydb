...
Соединению соответствует пятиугольник, образно показывающий направление передачи данных в графе. В данном экземпляре графа выделено соединение (../../_assets/rts-hash-conn-u.svg) между стадиями ```0``` и ```1```.
...
2. Соединение ```H``` это hash shuffle, распределение данных по одному или нескольким полям (список полей будет показан во всплывающем окне при позиционировании курсора мыши над символом соединения)
...
Обе используемые таблицы, ```region``` и ```nation``` очень маленькие, всего 5 и 25 строк соответственно. Поэтому запрос выполняется очень быстро, а обрабатываемый трафик крайне мал. Но это не делает запрос менее ценным с точки зрения исследования его структуры. Для дальнейшего изложения нам потребовался Shuffle Join, который был принудительно выбран через хинты оптимизатора. Если запустить запрос без это прагмы, то план будет немного отличаться, так как из-за небольшого размера оптимизатор построит другую структуру запроса (можете проверить какую).

![Объединение стадий](../../_assets/rts-nation-region.svg){inline=false}

На графе стадия ```2``` (изначально выделена) объединяет данные от стадий ```0``` и ```1```. Стадии всё ещё выстроены вертикально, но связь между ними нелинейная. Вместо этого применяется топологическая сортировка - выход любой стадии связывает её со входом другой стадии, которая строго выше исходной на графе. Таким образом, сохраняется принцип распространения данных снизу вверх. Если стадия принимает входы от двух и более других стадий, то она формирует обрамляющий отступ в левой части изображения, а входящие соединения рисуются (../../_assets/rts-hash-conn-l.svg) пятиугольником направленным влево. Тут два таких соединения ```H``` - hash shuffle.
...
2. При этом для каждой внешней стадии в соответствующих "читающих" вычислительных стадиях (с номерами ```1``` и ```0```) было создано по 3 задачи. В случае большого количества записей даже в одном шарде, трафик будет динамически балансироваться между этими задачами, и все они примерно равномерно участвовали бы в обработке.
3. В данном случае все записи уместились в один отправляемый пакет, который был отправлен из колоночного шарда в какую-то 1 из 3 созданных задач, а остальным просто не досталось никакого трафика, поэтому они ничего и не отправили в статистику.
...
С другой стороны, из таблицы ```nation``` были выбраны все имеющиеся там значения ключа ```n_regionkey``` который использовался для хеширования, эти данные получили только 5 задач. Всего же задач для стадии объединения было создано 8, что больше 1 и 5, поэтому данные были присланы только в 5 задач из 8, а успешное объединение данных выполнила только 1 задача, та, которой повезло получить строку из таблицу ```region```, именно эта задача выдала 5 строк результата, а выход стадии ```2``` также получил отметку ```1```.
