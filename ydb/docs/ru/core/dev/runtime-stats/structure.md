...
![Объединение стадий](../../_assets/rts-tpch-q17.svg)

В этом запросе данные из таблицы ```nation``` объединяются сами с собой, такой запрос еще называется "self join". Если оставить в стороне практическю целесообразность именно этого запроса и просто рассмотреть его структуру, то можно понять следующее:

1. Данные выгружаются из таблицы ```nation```
2. Они достигают стадии ```0``` - в которою они приходят в одном экземпляре
3. После чего данные разветвляются и один экземпляр приходит в стадию ```1```
4. А второй экземпляр тоже приходит в стадию ```1``` но на другой вход

Для программы в стадии ```1``` нет разницы, данные получены из одной и тоже стадии или нескольких (в сложных запросах данные могут быть еще подвергнуты сложным преобразованиям), но для понимания того, как работает запрос, важно чётко прослеждивать вся связи между отдельными стадиями, и графический план помогает пользователю это сделать.

{% note tip %}

Попробуйте выделить стадию ```0```. Вы заметите, что на схеме оказались подсвеченными 2 полосы - большая, которая считается основной и тонкая, тот самый "клон" стадии, необходимый для того, чтобы изобразить DAG в виде дерева.

{% endnote %}

В реальности это одна и та же стадия, которая работает один раз (поэтому номер и имеет два выхода). В нашем случае оба выхода связаны со стадией ```1```