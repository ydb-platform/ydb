...
[Колоночные таблицы](../concepts/datamodel/table.md#column-oriented-tables) поддерживают **локальные Bloom skip индексы**, которые ускоряют селективные запросы за счёт пропуска гранул данных, не содержащих искомых значений. Они создаются с помощью `LOCAL USING bloom_filter` или `LOCAL USING bloom_ngram_filter` и не используют синтаксис `VIEW` и описанные ниже сценарии обновления и удаления. Синтаксис и параметры см. в [Локальные Bloom skip индексы](../yql/reference/syntax/alter_table/indexes.md#local-bloom-column).
