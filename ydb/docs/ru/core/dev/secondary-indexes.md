...
Этот пример показывает общее правило: индекс избавляет от дополнительного чтения основной таблицы, если в нём есть все колонки, необходимые запросу. Глобальный вторичный индекс содержит свои ключевые колонки, колонки из `COVER` и неявно сохранённые колонки первичного ключа основной таблицы (см. [{#T}](../concepts/query_execution/secondary_indexes.md)). Если запрос выбирает другие колонки (например, `SELECT * FROM Table WHERE SubKey2 = 2`), их потребуется дополнительно прочитать из основной таблицы для найденных строк.
