...
Таблицы поддерживают **локальные Bloom skip индексы**, которые ускоряют селективные запросы за счёт пропуска частей данных, не содержащих искомых значений. Они создаются с помощью `LOCAL USING bloom_filter` или `LOCAL USING bloom_ngram_filter`, не используют синтаксис `VIEW`, а их синтаксис, параметры и изменение параметров через `ALTER INDEX` описаны в разделе [Bloom skip индексы](../yql/reference/syntax/alter_table/indexes.md#local-bloom).
