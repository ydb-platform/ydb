...
Данные в строковых таблицах {{ ydb-short-name }} всегда проиндексированы по первичному ключу. Это означает, что получение любой записи из строковой таблицы с заданными значениями полей, составляющих первичный ключ, всегда будет занимать минимальное фиксированное время, вне зависимости от общего количества записей в строковой таблице. Также наличие индекса по первичному ключу позволяет получить любой последовательный диапазон записей по возрастанию или убыванию первичного ключа. Время исполнения такой операции будет зависеть только от количества получаемых записей, но не зависеть от общего количества записей в строковой таблице. Рекомендации по проектированию первичного ключа см. в разделе [{#T}](primary-key/row-oriented.md).
...
Без вторичного индекса запрос с условием только по неключевой колонке, как правило, выполняет полное сканирование таблицы (`FullScan`), при котором читается каждая строка. Например, рассмотрим таблицу:
...
вынужден читать все строки таблицы, так как `email` не входит в первичный ключ. Добавление индекса позволяет выполнять выборку по `email` без полного сканирования таблицы:
...
Для эффективного ограничения результата запроса с помощью [команды `LIMIT`](../yql/reference/syntax/select/limit_offset.md) необходимо использовать однонаправленную сортировку в [команде `ORDER BY`](../yql/reference/syntax/select/order_by.md):

* `WHERE a = $var1 AND b > $var2 ORDER BY a, b DESC LIMIT 1`, будут прочитаны все строки индекса, подходящие под условия фильтра;
* `WHERE a = $var1 AND b > $var2 ORDER BY a DESC, b DESC LIMIT 1`, будет прочитана всего одна строка.
...
2. **Автоматический выбор индекса оптимизатором запросов** — если секция `VIEW` не указана, оптимизатор может самостоятельно принять решение об использовании того или иного вторичного индекса на основе текста запроса. Подробное описание критериев выбора приведено ниже.

{% note warning %}

Если у вас существуют запросы без явного указания индекса (без секции `VIEW`), то при добавлении нового индекса к таблице такие запросы могут начать использовать его автоматически. Это может привести к неожиданному изменению плана и времени выполнения запроса. Чтобы избежать подобных неожиданностей, рекомендуется:

* Явно указывать в запросах, какой индекс должен использоваться, с помощью секции `VIEW`;
* На основе критериев выбора вторичного индекса заранее представлять, может ли добавление нового вторичного индекса повлиять на существующие запросы.
...
Этот пример показывает общее правило: индекс избавляет от необходимости лишний раз обращаться к основной таблице только тогда, когда все колонки запроса входят в сам индекс (напрямую или через `COVER`, см. [покрывающий индекс](../concepts/query_execution/secondary_indexes.md#covering)). Если же запрос выбирает колонки, которых в индексе нет (например, `SELECT * FROM Table WHERE SubKey2 = 2`), даже при использовании индекса потребуется дополнительное чтение из основной таблицы для каждой найденной строки.
...
Существующий вторичный индекс можно заменить атомарно. Это может быть полезно, например, для замены индекса на [покрывающий](../concepts/query_execution/secondary_indexes.md#covering). Для работающих приложений эта операция прозрачна — в момент замены индекса произойдет инвалидация скомпилированных запросов.
