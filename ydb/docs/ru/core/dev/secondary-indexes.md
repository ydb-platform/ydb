...
## Ускорение запросов с помощью вторичных индексов {#speeding-up-queries}

При выполнении запросов по полям, которые не входят в первичный ключ, {{ ydb-short-name }} вынуждена полностью сканировать таблицу (FullScan). Это означает, что система должна прочитать все строки таблицы и проверить условие для каждой из них. При росте объема данных FullScan становится крайне медленным и ресурсоемким, что приводит к:
...
- Рассмотрите использование [покрывающего индекса](../concepts/query_execution/secondary_indexes.md#covering) (covering index) для запросов, которые читают только индексированные поля.
