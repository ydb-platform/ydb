...
При восстановлении запрос откатывается к последнему чекпоинту: возобновляет чтение с сохранённых смещений и восстанавливает состояния агрегаций. События, поступившие между чекпоинтом и сбоем, будут обработаны повторно.

Подробнее о механизме чекпоинтов:
...
В настоящий момент изменение текста запроса без его остановки не поддерживается. Для обновления запроса используется команда [ALTER](../../yql/reference/syntax/alter-streaming-query.md), в этом случае гарантия `at-least-once` не выполняется: часть событий может быть пропущена.
{% else %}
...
Ниже описаны сценарии, в которых это происходит.
...
Это ожидаемое поведение при первом запуске. Все последующие окна получат данные за полный интервал. Данную особенность важно учитывать при пересоздании запроса, так как каждое пересоздание приводит к повторному появлению неполного первого окна.
...
{% endif %}
чекпоинт удаляется вместе с запросом, так как {{ ydb-short-name }} использует внутреннее хранение смещений чтения из источника, то эти смещения удаляются вместе с запросом. Новый запрос не имеет сохранённой позиции и начинает чтение с конца топика. Все события, поступившие в топик между удалением старого запроса и стартом нового, не будут прочитаны.
...
В системах потоковой обработки [watermarks](https://en.wikipedia.org/wiki/Watermark_(data_synchronization)) представляют собой системные метки, определяющие момент, после которого все данные для заданного временного интервала гарантированно получены. {{ ydb-short-name }} в данный момент не поддерживает этот механизм.
...
### Обновление текста запроса с гарантией at-least-once {#update-at-least-once}

Для обновления текста запроса без потери событий можно использовать стратегию с двумя копиями запроса. Обновление выполняется поочерёдно: сначала обновляется один из запросов, затем, после того как он начал корректно обрабатывать данные, обновляется второй запрос. В момент обновления текста одного из запросов его копия продолжает выдавать корректный результат в полном объёме.

Такой алгоритм гарантирует обработку каждого события как минимум один раз (at-least-once). При этом он не устраняет аномалию [неполных агрегатов из-за отсутствия watermarks](#no-watermarks), так как она связана с задержками между партициями внутри каждой отдельной копии запроса.
