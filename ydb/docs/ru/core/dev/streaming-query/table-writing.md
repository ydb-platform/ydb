...
Для записи используется [UPSERT INTO](../../yql/reference/syntax/upsert_into) — вставка новой строки или обновление существующей по первичному ключу. Операция UPSERT идемпотентна по первичному ключу: повторная запись той же строки приводит к обновлению, а не к дублированию. Это важно, потому что потоковые запросы обеспечивают гарантию [at-least-once](../../concepts/streaming-query/streaming-query.md#guarantees) — при восстановлении из [чекпоинта](checkpoints.md) часть событий может быть обработана повторно.
