...
В одном потоковом запросе:

- Нельзя использовать одну и ту же таблицу для [обогащения потока](./enrichment.md) с использованием `JOIN` и для записи результата запроса.
- Нельзя писать в одну и ту же таблицу несколько раз.
