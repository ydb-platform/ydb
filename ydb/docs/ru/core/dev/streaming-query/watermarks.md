...
Вотермарка (watermark) — монотонно возрастающая нижняя оценка времён событий в потоке (подробнее о концепции: [{#T}](../../concepts/streaming-query/watermarks.md)). В данном разделе описана настройка вотермарок в потоковых запросах {{ ydb-short-name }}.
...
Вотермарка используется операциями, зависящими от прогресса времени события в потоке. В {{ ydb-short-name }} к таким операциям относится оконная агрегация [HoppingWindow](../../yql/reference/syntax/select/group-by.md#group-by-hopping_window) — она определяет скользящие временные окна, по которым группируются события. При получении вотермарки `HoppingWindow` закрывает все окна, которые полностью покрыты этим значением.
...
## Вычисление вотермарки {#watermark-computation}

Когда система получает событие, она обновляет вотермарку — **продвигает** его вперёд по временной оси. Вотермарка вычисляется как `максимальное наблюдаемое время события − delay`, где `delay` — величина отставания, заданная в выражении [`WATERMARK`](#configuration) (например, `Interval("PT5S")` в `WATERMARK = __ydb_write_time - Interval("PT5S")`).
...
Параметр `delay` задаёт допустимый «запас» времени для событий, поступающих с задержкой. Например, при `delay` в 5 секунд событие с временем 00:00:48 будет принято, даже если уже пришли события с временем 00:00:50: вотермарка ещё не дошла до 00:00:48. Если то же событие придёт позже, когда вотермарка уже продвинулась за 00:00:48, оно будет признано опоздавшим и отброшено.
...
Если входной топик содержит несколько [партиций](../../concepts/datamodel/topic.md#partitioning), каждая из них продвигает вотермарку независимо. Общая вотермарка запроса не обгоняет самую медленную партицию: окна не закрываются, пока хотя бы одна партиция не достигла соответствующего момента времени.

Если одна из партиций перестаёт получать данные, её вотермарка перестаёт продвигаться вперёд. Такая партиция называется простаивающей (idle). Пока простаивающая партиция учитывается при вычислении общей вотермарки, она тоже перестаёт продвигаться, и результаты не выдаются, несмотря на поступление данных от других партиций.

Чтобы избежать этой блокировки, простаивающая партиция исключается из вычисления общей вотермарки по истечении настраиваемого периода ожидания (параметр `WATERMARK_IDLE_TIMEOUT`, подробнее в разделе [Настройка](#configuration)).

## Настройка {#configuration}

Вотермарки включаются и настраиваются в секции [WITH](../../yql/reference/syntax/select/with.md) при чтении из топика.
...
Ниже приведён пример потокового запроса с вотермаркой и оконной агрегацией. Запрос читает события из топика, фильтрует их по полю `pass` и агрегирует значения `payload` в окнах по 10 секунд с шагом 5 секунд. Вотермарка настроена с отставанием в 5 секунд.
...
- `WATERMARK = __ydb_write_time - Interval("PT5S")` — вотермарка с отставанием 5 секунд. `Interval("PT5S")` задаёт интервал в формате [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations).
...
1. Первое событие (`"a"`, время записи 40с) проходит фильтр (`pass > 0`) и попадает в окна `[35; 45)` и `[40; 50)`. Вотермарка продвигается до 35с и не закрывает ни одного окна.
2. Второе событие (`"b"`, время записи 42с) аналогично попадает в окна `[35; 45)` и `[40; 50)`. Вотермарка продвигается до 37с.
3. Третье событие (`"c"`, время записи 50с) отбрасывается на фильтре (`pass = 0`). Несмотря на это, событие всё равно продвигает вотермарку до 45с. Вотермарка закрывает окно `[35; 45)` — результат `["a", "b"]` выдаётся.
4. Четвёртое событие (`"d"`, время записи 40с) не продвигает вотермарку: он уже находится на отметке 45с. Событие проходит фильтр, но отбрасывается как опоздавшее — его время записи (40с) меньше текущей вотермарки (45с). Хотя окно `[40; 50)` ещё открыто, вотермарка уже обещала, что событий с временем < 45с больше не будет, поэтому `d` не учитывается ни в одном из своих окон.

## См. также

- [{#T}](../../yql/reference/syntax/select/group-by.md#group-by-hopping_window) — оконная функция, использующая вотермарки.
