...
Если входной топик содержит несколько [партиций](../../concepts/datamodel/topic.md#partitioning), каждая партиция генерирует свой водяной знак. Общий водяной знак вычисляется как минимум водяных знаков всех активных партиций.

Если одна из партиций перестаёт получать данные (idle-партиция), её водяной знак перестаёт продвигаться вперёд. Пока эта партиция учитывается в вычислении общего водяного знака, он тоже перестаёт продвигаться, и окна агрегации не закрываются, несмотря на поступление данных от других партиций. Чтобы избежать этой блокировки, idle-партиция исключается из вычисления общего водяного знака по истечении `WATERMARK_IDLE_TIMEOUT`.
...
4. Четвёртое событие (`"d"`, время записи 40с) не порождает нового водяного знака. После третьего события максимальное время записи уже равно 50с, поэтому текущий водяной знак остаётся равным 45с (`50 - 5`). Событие проходит фильтр, но отбрасывается как опоздавшее: его время записи (40с) меньше текущего водяного знака (45с), поэтому окна, к которым оно относилось, уже закрыты.
