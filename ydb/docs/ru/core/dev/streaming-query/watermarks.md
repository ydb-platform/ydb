...
Водяной знак (watermark) — монотонно возрастающая нижняя оценка времён событий в потоке (подробнее о концепции: [{#T}](../../concepts/watermarks.md)). В данном разделе описана настройка водяных знаков в потоковых запросах {{ ydb-short-name }}.
...
Когда система получает событие, она обновляет водяной знак — **продвигает** его вперёд по временной оси. Водяной знак вычисляется как `максимальное наблюдаемое время события − delay`, где `delay` — настраиваемый параметр задержки.
...
О компромиссе между точностью и задержкой выдачи результатов: [{#T}](../../concepts/watermarks.md#tradeoff).
