...
## Cтриминговые запросы {#streaming}

### Просмотр информации о стриминговых запросах {#streaming_queries}

Системное представление `streaming_queries` содержит информацию о текущих [стриминговых запросах](../concepts/streaming_query/index.md) (как запущенных, так и остановленных).

Структура таблицы:

| Колонка           | Описание |
|-------------------|----------|
| `Path`            | Имя запроса.<br />Тип: `Utf8`.|
| `Status`          | Статус выполнения запроса, значение одно из:<br />`CREATING` - запрос создается <br />`CREATED` - запрос создан, но не запущен<br />`STARTING` - запрос запускается<br />`RUNNING` - запрос запущен<br />`STOPPING` - запрос останавливается<br />`STOPPED` - запрос остановлен<br />`SUSPENDED` - запрос завершился с ошибкой и ожидает backoff для ретрая<br />`FAILED` - превышен лимит на число ретраев запроса, требуется ручной перезапуск<br />Тип: `Utf8`|
| `Issues`          | Ошибки выполнения запроса в формате json (для последнего execution)<br />Тип: `Utf8`|
| `Plan`            | План запроса в формате json (для последнего execution)<br />Тип: `Utf8`|
| `Ast`             | Ast запроса (для последнего execution)<br />Тип: `Utf8`|
| `Text`            | Текст запроса<br />Тип: `Utf8`|
| `Run`             | Запущен ли запрос пользователем в данный момент<br />Тип: `Bool`|
| `ResourcePool`    | Имя пула ресурсов, к которому был привязан запрос<br />Тип: `Utf8`|
| `RetryCount`      | Число перезапусков запроса<br />Тип: `Uint64`|
| `LastFailAt`      | Время последнего завершения запроса<br />Тип: `Timestamp`|
| `SuspendedUntil`  | Время, до которого приостановлено исполнение<br />Тип: `Timestamp`|
| `LastExecutionId` | Идентификатор последней операции исполнения запроса<br />Тип: `Utf8`|
| `PreviousExecutionIds` | Предыдущий идентификатор последней операции исполнения запроса<br />Тип: `Utf8`|
