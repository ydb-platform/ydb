...
Кумулятивные метрики (`RowReads`, `RowUpdates`, `LocksAcquired` и т.д.) хранят накопленные значения с момента последнего старта таблетки, обслуживающей партицию.
...
`CPUCores` | Моментальное значение нагрузки на партицию (доля ядра).<br/>Тип: `Double`
...
`RowReads` | Количество точечных чтений.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`RowUpdates` | Количество записанных строк .<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`RowDeletes` | Количество удалённых строк.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`RangeReads` | Количество чтений диапазонов строк.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`RangeReadRows` | Количество строк, прочитанных в диапазонах.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`InFlightTxCount` | Количество транзакций, находящихся в процессе исполнения.<br/>Тип: `Uint64`.
`ImmediateTxCompleted` | Количество завершившихся одношардовых транзакций.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`CoordinatedTxCompleted` | Количество завершившихся координируемых транзакций .<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`TxRejectedByOverload` | Количество транзакций, отменённых по причине слишком высокой нагрузки.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`TxRejectedByOutOfStorage` | Количество транзакций, отменённых из-за нехватки места.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`LocksAcquired` | Количество установленных [блокировок](../contributor/datashard-locks-and-change-visibility.md) "на диапазон ключей" .<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`LocksWholeShard` | Количество установленных блокировок "на всю партицию".<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
`LocksBroken` | Количество сломанных блокировок.<br/>Тип: `Uint64`.<br/>Кумулятивная метрика
...
* `IntervalEnd` - момент закрытия интервала;
* `Rank` - ранг партиции по пиковой нагрузке `CPUCores` в этом интервале.

Например, `top_partitions_one_hour` для часового интервала `"20.12.2024 10:00-11:00"` выдаст 10 строк, отсортированных по порядку убывания `CPUCores`. У них будет `Rank` от 1 до 10 и одинаковый `IntervalEnd` `"20.12.2024 11:00"`.
...
Следующий запрос выводит партиции в указанном интервале времени, с идентификаторами таблеток и числом сломанных блокировок. Запрос выполняется к представлению `.sys/top_partitions_by_tli_one_minute`, которое содержит данные за последние 6 часов с разбиением по минутным интервалам:
