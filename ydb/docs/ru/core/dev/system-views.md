...
| `Query` | Текст запроса. Может быть обрезан до 10 КБ.<br/>Тип: `Utf8`. |
| `AccessCount` | Количество обращений к результату компиляции запроса.<br/>Тип: `Uint64`. |
...
Топ-20 самых популярных запросов по количеству обращений:
