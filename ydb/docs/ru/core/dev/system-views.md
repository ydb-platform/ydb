...
Различные запросы с одним и тем же текстом объединяются. В топе содержится информация о запуске, который имеет максимальное значение определенной характеристики среди всех запросов за один временной период.
...
Топ запросов по времени выполнения. Запрос выполняется к представлению `.sys/top_queries_by_duration_one_minute`, которое содержит данные за последние 6 часов с разбиением по минутным интервалам:
...
Запросы, прочитавшие больше всего байт. Запрос выполняется к представлению `.sys/top_queries_by_read_bytes_one_minute`, которое содержит данные за последние 6 часов с разбиением по минутным интервалам:
