...
| `DurationUs` | Время с начала текущего запроса в микросекундах, включая компиляцию и ожидание в очереди.<br/>Тип: `Uint64`. |
| `CpuTimeUs` | Сумма накопленного времени CPU compute-задач и времени CPU хранилища по полученным отчётам, в микросекундах.<br/>Тип: `Uint64`. |
| `ComputeMemoryBytes` | Текущая учтённая память compute-задач в байтах. Не включает отдельную квоту каналов; не является RSS или пиковым потреблением памяти.<br/>Тип: `Uint64`. |
| `TableReadBytes` | Накопленное число прочитанных байтов по статистике таблиц.<br/>Тип: `Uint64`. |
| `SourceReadBytes` | Накопленное число входящих байтов по статистике источников.<br/>Тип: `Uint64`. |

Показатели ресурсов отражают последние полученные отчёты задач и обновляются независимо от запроса клиентской статистики. До первого отчёта показатели ресурсов равны `NULL`; для сессии без текущего запроса все пять колонок равны `NULL`. CPU и байты суммируются по физическим исполнениям текущего запроса, включая уже завершённые исполнения. Память учитывает только активные исполнения. История завершённых запросов не сохраняется.

`TableReadBytes` и `SourceReadBytes` могут пересекаться: их нельзя складывать или использовать как разделение IO между обычным хранилищем и S3. Для группировки запросов по пулам используйте `WmPoolId`. Сумма `CpuTimeUs` активных запросов не является загрузкой CPU пула за интервал.
...
| `Text`            | Текст запроса<br />Тип: `Utf8`|
| `Run`             | Запущен ли запрос пользователем в данный момент<br />Тип: `Bool`|
