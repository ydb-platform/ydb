...
{% if audience == "tech" or audience == "internal" %}

{% note info %}

На базах данных внутренних кластеров нет локальных пользователей, соответственно, системное представление `auth_users` не содержит записей.
