...
* **Жёсткий лимит размера партиции на уровне кластера** — по умолчанию **2 ГиБ**. Если размер партиции превышает этот лимит, она разделится независимо от [`AUTO_PARTITIONING_MAX_PARTITIONS_COUNT`](auto/index.md#auto-sharding-limits), если не достигнут [лимит числа шардов таблицы в базе](../../../concepts/limits-ydb.md#schema-object).

* **Использование `AUTO_PARTITIONING_MAX_PARTITIONS_COUNT` как жёсткого предела** — при достижении этого значения обычное разделение по размеру или нагрузке прекращается. Однако при превышении жёсткого лимита размера 2 ГиБ партиция всё равно разделяется, поэтому фактическое число партиций может превысить заданный максимум.
