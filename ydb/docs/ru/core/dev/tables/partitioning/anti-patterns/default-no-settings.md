...
При **высокой конкуренции за единичные ключи** (горячий ключ, низкая кардинальность первого компонента) производительность будет ограничена вне зависимости от настроек `AUTO_PARTITIONING_*` — партицирование по нагрузке здесь мало поможет; пересмотрите модель ключа (см. [{#T}](index.md)).
