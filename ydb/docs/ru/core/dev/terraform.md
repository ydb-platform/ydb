...
* Строка соединения `connection_string` — выражение вида `grpc(s)://HOST:PORT/?database=/database/path`, где `grpc(s)://HOST:PORT/` эндпоинт, а `/database/path` — путь БД.
  Например, `grpcs://example.com:{{ ydb-ports.grpcs }}?database=/Root/testdb0`.
...
resource "ydb_topic" "ydb_topic" {
  database_endpoint = "grpcs://example.com:{{ ydb-ports.grpcs }}/?database=/Root/testdb0" #пример подключения к БД
  name              = "test/test1"
  supported_codecs  = ["zstd"]
...
* `database_endpoint` - (обязательный) полный путь до базы данных, например: `"grpcs://example.com:{{ ydb-ports.grpcs }}/?database=/Root/testdb0"`; аналог `connection_string` для строковых таблиц.
