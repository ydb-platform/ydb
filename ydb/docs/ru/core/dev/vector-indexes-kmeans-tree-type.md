...
Векторный индекс с фильтрацией ускоряет запросы с условиями WHERE, позволяя мгновенно получать похожие объекты среди подходящих по фильтру. При использовании такого индекса поиск ближайших векторов производится только среди строк, соответствующих фильтру, что обеспечивает высокую производительность на больших объёмах данных.
...
В YDB фильтрация интегрирована прямо в структуру и алгоритм векторного индекса, так что не подходящие по условиям объекты сразу исключаются из рассмотрения. Архитектурно индекс с фильтрацией добавляет перед векторным индексом классический вторичный индекс для указанных колонок. А для каждого уникального элемента вторичного индекса строится отдельный векторный индекс:

![Архитектура индекса с фильтрацией](../_assets/vector_kmeans_tree_9.png)

Технически вторичный индекс помещается в дополнительную реляционную таблицу prefix table (`indexImplPrefixTable`), а все деревья векторных индексов остаются в таблице level table:

| Колонка | Тип данных | Описание |
| --- | --- | --- |
| `<name>` | `<type>` | Имя и тип колонки для вторичного индекса. В этой дополнительной таблице будет столько таких колонок, сколько было указано при создании индекса с фильтрацией |
