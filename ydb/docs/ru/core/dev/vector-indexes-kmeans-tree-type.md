...
Векторный индекс с фильтрацией ускоряет запросы с условиями WHERE, позволяя мгновенно получать похожие объекты среди подходящих по фильтру. Например, можно выполнить следующий запрос:

```yql
-- входная переменная: идентификатор пользователя
$user=1;
-- входная переменная: вектор-образец
$embedding = Knn::ToBinaryStringFloat([4.0f, 5.0f, 6.0f]);
SELECT *
  FROM table
  -- нужно явно указать использование конкретного векторного индекса
  VIEW index
  -- поддерживается только фильтрация по точному совпадению
  WHERE user_id=$user
  -- функция, аналогичная оператору <-> в pgvector
  ORDER BY Knn::EuclideanDistance(embedding, $embedding) ASC
  LIMIT 10
```

В таком запросе поиск ближайших векторов производится только среди строк, соответствующих конкретному пользователю, что обеспечивает высокую производительность и гибкость реализации бизнес-логики на больших объёмах данных.

Быстрый и точный фильтрованный поиск поддерживается непосредственно на уровне структуры индекса. Для этого фильтруемая колонка должна быть явно указана в [SQL-запросе на создание индекса](../yql/reference/syntax/create_table/vector_index.md), например:

```yql
ALTER TABLE table
  ADD INDEX index
  GLOBAL USING vector_kmeans_tree
  -- список колонок таблицы, используемых для поиска по индексу. Последняя
  -- колонка используется как эмбеддинг, остальные — как колонки для
  -- ускорения фильтрации. Если использовать несколько колонок фильтрации,
  -- то доступны условия поиска вида
  -- WHERE category_id=$category AND user_id=$user.
  ON (user_id, embedding)
  WITH (
    distance=cosine,
    vector_type="float",
...
### Пример структура таблицы `indexImplPostingTable` с покрывающим индексом
...
## Внутреннее устройство индекса с фильтрацией {#index-filter-structure}

Часто возникает необходимость фильтровать объекты по дополнительным атрибутам или метаданным. Если фильтровать до поиска, то после применения фильтра останется набор строк, в которых нужно найти ближайшие к образцу векторы. Обычный ANN-индекс позволяет искать ближайшие векторы только среди всех векторов в таблице, его нельзя применить к произвольному набору строк. Для нахождения ближайших векторов СУБД придётся перебрать все оставшиеся строки и для каждой из них вычислить дистанцию до образца

Если фильтровать после поиска, то подходящих результатов может остаться меньше, чем нужно, и качество поиска упадёт. Нужно повторить поиск с увеличенным лимитом векторного поиска, что тоже медленно.

В YDB фильтрация интегрирована прямо в структуру и алгоритм ANN-индекса, так что не подходящие по условиям объекты сразу исключаются из рассмотрения. Архитектурно индекс с фильтрацией добавляет классический вторичный индекс для префиксов перед векторным индексом. А для каждого уникального элемента вторичного индекса строится отдельный векторный индекс:

![Архитектура индекса с фильтрацией](../_assets/vector_kmeans_tree_9.png)

Технически префиксы помещаются в дополнительную реляционную таблицу prefix table (`indexImplPrefixTable`), а все деревья векторных индексов остаются в таблице level table.

| Колонка | Тип данных | Описание |
| --- | --- | --- |
| `langs` | `Uint32` | Префикс вторичного индекса |
| `__ydb_id` | `Uint64` | Идентификатор корневого кластера для префикса |
