...
### Автоматический подбор параметров {#auto-settings}

Параметры `clusters` и `levels` можно не указывать при создании индекса. В этом случае {{ ydb-short-name }} подбирает их автоматически, исходя из количества векторов в таблице. Явно заданные параметры {{ ydb-short-name }} не переопределяет.
...
Индекс с фильтрацией строит отдельное дерево кластеров для каждого уникального значения фильтруемых колонок, а разные значения часто содержат очень разное количество векторов — от миллионов до нескольких десятков. Единое фиксированное значение `clusters` не может быть оптимальным сразу для всех: при малом значении кластеры последнего уровня получаются слишком большими и перебираются медленно, а при большом — появляется множество почти пустых кластеров, что расходует место и может снизить полноту поиска.

Адаптивное количество кластеров решает эту проблему: когда режим включён, {{ ydb-short-name }} выбирает количество кластеров *для каждого значения фильтруемой колонки* автоматически, исходя из того, сколько векторов это значение фактически содержит. Значения с большим количеством векторов получают больше кластеров (вплоть до значения `clusters`, которое выступает верхней границей), а значения с малым количеством векторов — меньше.
...
* Если при создании индекса с фильтрацией параметры не заданы явно и {{ ydb-short-name }} [подбирает их автоматически](#auto-settings), адаптивное количество кластеров включается по умолчанию.
* Если параметры индекса заданы явно, то `adaptive_clusters` по умолчанию равен `false`; чтобы включить адаптивный режим, задайте `adaptive_clusters` равным `true`.

В адаптивном режиме параметр `clusters` интерпретируется как *максимальное* количество кластеров для одного значения фильтруемой колонки. Фактическое количество для каждого значения вычисляется на основе количества его векторов, параметров `levels` и [`overlap_clusters`](vector-indexes.md#overlap-clusters) и никогда не бывает меньше 2. Значения с очень малым количеством векторов (менее 100) всегда используют минимум — 2 кластера.
