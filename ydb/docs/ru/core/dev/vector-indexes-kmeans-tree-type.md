...
По умолчанию последний кластер хранит не сами векторы для поиска, а только список ключей этих векторов в таблице. Такой список называется Posting Table — это разновидность инвертированного индекса. Хранение ссылок на векторы вместо самих векторов позволяет минимизировать размер векторного индекса.

Однако такой подход означает, что после нахождения последнего кластера {{ ydb-short-name }} должен выполнить `SELECT`-запрос к таблице, чтобы получить несколько сотен элементов (точное количество зависит от параметра `clusters`, который определяет и число кластеров на каждом уровне дерева, и количество векторов в последнем кластере).
...
В первичный ключ этой таблицы входят колонка `__ydb_parent` и колонка, соответствующая первичному ключу индексируемой таблицы (`id` в примере ниже).

| Колонка | Тип данных | Описание |
| --- | --- | --- |
| `__ydb_parent` | `Uint64` | Идентификатор кластера, содержащего текущий вектор |
| `id` | `Uint64` | Первичный ключ таблицы, соответствующий текущему вектору. Название и тип ключа копируются из индексируемой таблицы. |
...
| --- | --- | --- |
| `__ydb_parent` | `Uint64` | Идентификатор кластера, содержащего текущий вектор |
| `id` | `Uint64` | Первичный ключ таблицы, соответствующий текущему вектору |
