...
## Адаптивное количество кластеров {#adaptive-clusters}

Индекс с фильтрацией строит отдельное дерево кластеров для каждого уникального значения фильтруемых колонок. Разные значения часто содержат очень разное количество векторов: в одних категориях могут быть миллионы векторов, в других — всего несколько десятков. Единое фиксированное значение `clusters` не может быть оптимальным сразу для всех:

* Для значения с большим количеством векторов маленькое `clusters` даёт крупные кластеры последнего уровня, перебор которых выполняется медленно.
* Для значения с малым количеством векторов большое `clusters` даёт множество почти пустых кластеров последнего уровня, что расходует место и может даже снизить полноту поиска (векторов слишком мало, чтобы их осмысленно распределить).

Без адаптивного количества кластеров единственный обходной путь — строить несколько отдельных индексов, настроенных под категории разной мощности, что неудобно.

Адаптивное количество кластеров решает эту проблему: когда режим включён, {{ ydb-short-name }} выбирает количество кластеров *для каждого значения фильтруемой колонки* автоматически, исходя из того, сколько векторов это значение фактически содержит. Значения с большим количеством векторов получают больше кластеров (вплоть до значения `clusters`, которое выступает верхней границей), а значения с малым количеством векторов — меньше. Это позволяет одному индексу обеспечивать хорошую производительность поиска для категорий очень разного размера.

Адаптивное количество кластеров управляется [параметром индекса](../yql/reference/syntax/create_table/vector_index.md) `adaptive_clusters`:

* `adaptive_clusters` применяется только к индексам с фильтрацией (индексам с более чем одной индексируемой колонкой). Для индексов без фильтрации этот параметр не действует.
* Если при создании индекса с фильтрацией параметры индекса не заданы явно и {{ ydb-short-name }} подбирает их автоматически, адаптивное количество кластеров включается по умолчанию.
* Если параметры индекса заданы явно, то `adaptive_clusters` по умолчанию равен `false`; установите его в `true`, чтобы включить адаптивный режим.

В адаптивном режиме параметр `clusters` интерпретируется как *максимальное* количество кластеров для одного значения фильтруемой колонки. Фактическое количество для каждого значения вычисляется на основе количества его векторов, параметров `levels` и `overlap_clusters` и никогда не бывает меньше 2. Значения с очень малым количеством векторов (менее 100) всегда используют минимум — 2 кластера.

Пример создания индекса с фильтрацией и адаптивным количеством кластеров:
...
Здесь `clusters=512` — верхняя граница: значение `user` с миллионами векторов может получить количество кластеров, близкое к 512, а значение `user` с несколькими десятками векторов получит только 2.
...
Чтобы не перечитывать центроиды из распределённого хранилища при каждом запросе, содержимое level table можно кэшировать в памяти на каждом узле — см. параметр [`resource_manager.kqp_level_cache_max_size_bytes`](../reference/configuration/table_service_config.md#resource-manager) в `table_service_config`.
