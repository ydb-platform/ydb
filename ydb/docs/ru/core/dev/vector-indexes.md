...
Внутри векторный индекс состоит из скрытых индексных таблиц вида `indexImpl*Table`. В [запросах на выборку](#select) с использованием векторного индекса, индексные таблицы будут отображены в [статистике запросов](query-plans-optimization.md). Подробнее об этих таблицах см. в [статье про устройство векторного индекса](vector-indexes-kmeans-tree-type).

Логика работы и настройка векторного индекса подробно описаны в отдельной
[статье](vector-indexes-kmeans-tree-type.md).
