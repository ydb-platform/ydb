...
При выполнении сценария развертывания {{ ydb-short-name }} на 9 хостах на каждом хосте будут задействованы следующие порты:

{% list tabs %}

- Порты сторадж ноды

  Запускается процесс `ydbd-storage`, который использует порты:
...
В итоге, на каждом хосте задействуются следующие порты: `2135` - `2137`, `5432`, `8765` - `8767`, `19001` - `19003`. Ознакомиться со списком портов {{ ydb-short-name }}, назначенных по умолчанию можно в статье [{#T}](../../reference/ydb-ports.md).
