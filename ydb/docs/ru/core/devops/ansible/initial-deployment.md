...
1. Роль `ydb-static` отвечает за подготовку и запуск статических нод {{ ydb-short-name }}, включая проверку необходимых переменных и секретов, форматирование и подготовку дисков, создание и запуск `systemd unit` для узла хранения, а также инициализацию хранилища и управление доступом к базе данных. Ознакомиться с `systemd` инитом сторадж ноды можно по [ссылке](https://github.com/ydb-platform/ydb-ansible/blob/3b61d8c947d94ad4b89e8460f26391041a47b00e/roles/ydbd_static/templates/ydbd-storage.service).
1. Роль `ydb-dynamic` настраивает и управляет динамическими узлами {{ ydb-short-name }}, включая проверку необходимых переменных, создание конфигурационных файлов и `systemd unit` файлов для каждого динамического узла, запуск этих узлов, получение токена для доступа к {{ ydb-short-name }}, создание базы данных в {{ ydb-short-name }}. Шаблон файла `systemd` инита динамической ноды находится по [ссылке](https://github.com/ydb-platform/ydb-ansible/blob/3b61d8c947d94ad4b89e8460f26391041a47b00e/roles/ydbd_static/templates/ydbd-storage.service).

В шаблонах `systemd` инитов {{ ydb-short-name }} нод используются следующие стандартные порты для {{ ydb-short-name }}:

| Порт сторадж ноды | Порт динамической ноды | Описание |
|-------------------|------------------------|----------|
| `{{ ydb-ports.grpcs }}` | `2136` | Порты для защищенного GRPCS-соединения (TLS).|
| `{{ ydb-ports.interconnect }}` | `19002` | Порты для Interconnect-соединения нод {{ ydb-short-name }}.|
| `{{ ydb-ports.https }}` | `8766` | HTTPS-порты мониторинга кластера.|

В случае развертывания нескольких динамических нод на одном хосте в ручном или автоматическом режиме для каждой динамической ноды значение портов GRPCS, Interconnect, HTTPS увеличивается на 1 пропорционально количеству динамических нод, развернутых на одном хосте. Например, Ansible осуществляет порт-шифтинг динамических нод с помощью автоинкремента `item.offset`.

Ознакомиться со списком портов {{ ydb-short-name }}, назначенных по умолчанию можно в статье [{#T}](../../reference/ydb-ports.md).
