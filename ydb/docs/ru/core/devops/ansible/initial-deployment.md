...
При выполнении сценария развертывания {{ ydb-short-name }} на каждом хосте будут задействованы следующие порты:
...
- Порты динамических нод
  ...
  Запускаются несколько `systemd unit` файлов: `ydbd-database-a` и `ydbd-database-b` для обслуживание разных БД. Они задействуют порты:
...
В итоге, на каждом хосте задействуются следующие порты: `2135` - `2137`, `5432`, `8765` - `8767`, `19001` - `19003`.
