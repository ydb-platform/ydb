...
Коллекции резервных копий обеспечивают инкрементальное резервное копирование и восстановление на любую сохранённую точку в цепочке резервных копий. Концептуальную информацию и описание архитектуры см. в статье [Коллекции резервных копий](../../concepts/datamodel/backup-collection.md).

Коллекции резервных копий рекомендуются для производственных сред с регулярным расписанием резервного копирования и большими объёмами данных, когда инкрементальные изменения намного меньше общего объёма. Для более простых сценариев (разовая миграция, среды разработки, небольшие базы данных) рассмотрите [export/import](#s3) или [dump/restore](#files).

Пошаговые инструкции по настройке и использованию коллекций резервных копий:

- [Коллекции резервных копий](../../concepts/datamodel/backup-collection.md) — архитектура, концепции и ограничения;
- [Рецепты для коллекций резервных копий](../../recipes/backup-collections/index.md) — типовые сценарии и примеры.
