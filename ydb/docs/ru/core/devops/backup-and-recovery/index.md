...
|| [Резервное копирование системных таблеток](#system-tablet-backup) | Файловая система кластера | Полные и инкрементальные копии | Да, кроме отказа оборудования в кластере | Крупные рабочие среды, где при повреждении метаданных кластера недопустимы его пересоздание и миграция баз данных ||
|#
...
Механизм резервного копирования системных таблеток, таких как [Hive](../../concepts/glossary.md#hive), [BSController](../../concepts/glossary.md#bs-controller) и [SchemeShard](../../concepts/glossary.md#scheme-shard), обеспечивает инкрементальное копирование метаданных кластера на локальную файловую систему кластера.

При повреждении метаданных в основном хранилище — например, при полной потере [статической группы](../../concepts/glossary.md#static-group) — кластер необходимо пересоздать с нуля и восстановить каждую базу из резервной копии. Резервное копирование системных таблеток позволяет восстановить метаданные на месте, без пересоздания кластера и миграции баз данных.

Для более простых сценариев, таких как разовые миграции или среды разработки, рассмотрите возможность использования команд [dump/restore](#files).
