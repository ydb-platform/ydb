...
Замена выполняется на одном хосте за раз.
...
- `-l static-node-1.ydb-cluster.com` — укажите адрес хоста, на котором выполняется замена диска; аргумент `-l` ограничивает playbook одним хостом;

- в аргументе `ydb_disk_prepare` значение `ydb_disk_1` — на фактический `label` заменяемого диска.

### Перезапустите замененный PDisk {#restart}
  ...
  - Перезапуском узла хранения через Ansible

    Укажите тот же хост, что в аргументе `-l` на предыдущем шаге.
...
### Проверьте результат {#verify}
  ...
  После подготовки диска и перезапуска **PDisk** в том же разделе **PDisks** диск с новым разделом должен отображаться с зелёной индикацией — признак того, что узел видит диск и он участвует в хранилище без ошибок на уровне мониторинга.
