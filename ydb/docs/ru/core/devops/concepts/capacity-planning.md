...
Во всех расчётах:
1 KB = 1000 байт, 1 MB = 1000 KB, 1 GB = 1000 MB, 1 TB = 1000 GB;
1 KiB = 1024 байт, 1 MiB = 1024 KiB, 1 GiB = 1024 MiB, 1 TiB = 1024 GiB.
...
- `RF` — фактор избыточности, определяется режимом работы кластера (см. [таблицу выше](#storage-model)).
- `Overhead` — накладные расходы хранилища (см. [раздел выше](#metrics)); для грубой оценки принять `2`.
...
- `VDisksInGroup` — количество VDisk в группе хранения, определяется режимом работы кластера (см. [таблицу выше](#storage-model)).
...
2. Определите размер слота:
    ...
    Около 28.08 GB ёмкости PDisk зарезервировано для системных нужд, остальное место поровну распределяется между слотами.

    Данная формула применима для дисков объёмом от ~839 GB (800 GiB). Использование дисков меньшего размера не рекомендуется, если необходимо достижение оптимальной производительности. Подробнее о требованиях к дисковой подсистеме читайте в разделе [{#T}](system-requirements.md).
...
3. Оцените количество групп хранения:
    ...
    При мониторинге рекомендуется ориентироваться на значения VDisk Slot Usage и Capacity Alert. Для существующих групп значения VDisk Raw Usage, VDisk Slot Usage и Capacity Alert можно посмотреть в {{ ydb-short-name }} UI на вкладке **Info → Storage** страницы базы.
...
Допустим, необходимо разместить 100 TB данных (Tablet Storage) в кластере с режимом отказоустойчивости `block-4-2`, используя SSD-диски ёмкостью 3.2 TB.
...
3. TotalGroups = ⌈300 000 / (198.25 × 8 × 0.85)⌉ = **223 группы хранения**
...
Таким образом, для хранения 100 TB данных в режиме `block-4-2` на SSD-дисках ёмкостью 3.2 TB потребуется 10 стоек по 14 дисков (140 дисков) либо 30 серверов по 4 диска (120 дисков).

**Обратный расчёт:**

Далее возьмём конфигурацию из предыдущего примера — 30 серверов по 4 SSD-диска ёмкостью 3.2 TB, и оценим, какой полезный объём данных поместится в таком кластере.
...
Таким образом, кластер из 30 серверов и 120 SSD-дисков по 3.2 TB в режиме `block-4-2` позволяет разместить около 101.1 TB данных (Tablet Storage).
