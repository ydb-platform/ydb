...
Часть ёмкости PDisk зарезервирована для системных нужд, суммарно около 28.4 ГБ. Остальное место поровну распределено между слотами. Количество слотов на одном PDisk должно определяться исходя из параметров оборудования. Для оценки вслепую можно полагать `ExpectedSlotCount = 8` для HDD-дисков и `16` для SSD/NVMe. Исходя из этого размер слота можно определить по формуле:
...
Допустим, необходимо разместить 10 TB данных (Tablet storage) в кластере с режимом отказоустойчивости `block-4-2`, используя SSD-диски ёмкостью 1000 ГБ.
