...
Во всех расчётах применяются обозначения согласно ГОСТ IEC 60027-2-2015:
1 кБ = 1000 байт, 1 МБ = 1000 кБ, 1 ГБ = 1000 МБ, 1 ТБ = 1000 ГБ;
1 КиБ = 1024 байт, 1 МиБ = 1024 КиБ, 1 ГиБ = 1024 МиБ, 1 ТиБ = 1024 ГиБ.
...
2. Определите размер слота:
    ...
    ```text
    SlotSize = (DriveSize - 28.08 ГБ) / ExpectedSlotCount
    ```

    <!-- 1 (SysLog) + 5 (SysReserveSize) + 200 (MaxCommonLogChunks) = -->
    <!-- 206 chunks (130 MiB each: 128 MiB payload + 2 MiB overhead) ≈ 28.08 GB -->

    Около 28.08 ГБ ёмкости PDisk зарезервировано для системных нужд, остальное место поровну распределяется между слотами.

    Данная формула применима для дисков объёмом от ~859 ГБ (800 ГиБ). Использование дисков меньшего размера не рекомендуется, если необходимо достижение оптимальной производительности. Подробнее о требованиях к дисковой подсистеме читайте в разделе [{#T}](system-requirements.md).
...
2. SlotSize = (3 200 − 28.08) / 16 ≈ **198.25 ГБ**
...
5. SlotSize = (3 200 − 28.08) / 16 ≈ **198.25 ГБ**
6. Database Storage = 225 × 8 × 198.25 × 0.85 ≈ **303 323 ГБ**
7. Tablet Storage = 303 323 / (1.5 × 2) ≈ **101 108 ГБ**
