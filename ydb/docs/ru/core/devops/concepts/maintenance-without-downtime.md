...
- **Smart**: режим, который по умолчанию ведёт себя как **Strong**, но автоматически переключается на **Weak**, если уже есть недоступные VDisk-и в затрагиваемых группах хранения или недоступные кольца State Storage, не связанные с текущими работами по обслуживанию.
...
Режим доступности **Smart** является экспериментальной функциональностью и по умолчанию выключен. Для его включения необходимо включить фича флаг `enable_cms_smart_availability_mode` в [конфигурации](../../reference/configuration/feature_flags.md).

{% endnote %}

### Приоритет {#priority}

У задачи обслуживания можно указать приоритет. Меньшее значение означает более высокий приоритет, поддерживаются отрицательные приоритеты (по аналогии с `nice` в Linux).

Приоритет определяет порядок выполнения задач обслуживания. Задачи с более высоким приоритетом имеют преимущество перед задачами с более низким приоритетом: они блокируют их выполнение и могут выполняться поверх. Задачи с одинаковым приоритетом обрабатываются равноправно — ни одна из них не имеет преимущества перед другой.

{% note warning "Экспериментальная функциональность" %}

Выполнение приоритетных задач поверх задач с более низким приоритетом является экспериментальной функциональностью и по умолчанию выключено. Для включения необходимо включить фича флаг `enable_cms_locks_priority` в [конфигурации](../../reference/configuration/feature_flags.md).

{% endnote %}

## Лимиты недоступных узлов {#unavailable-node-limits}

В CMS есть абсолютные и относительные лимиты на количество недоступных узлов для каждой базы данных (тенанта) и для кластера в целом. Лимиты недоступных узлов настраиваются в [конфигурации CMS](../../reference/configuration/cms_config.md).
...
Во внештатных ситуациях, например при возникновении проблем с инфраструктурой, можно временно приостановить новые работы по обслуживанию кластера с помощью параметра `disable_maintenance` в [конфигурации CMS](../../reference/configuration/cms_config.md).
