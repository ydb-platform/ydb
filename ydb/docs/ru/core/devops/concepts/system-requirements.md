...
Для повышения производительности дискового и сетевого ввода-вывода мы рекомендуем выключить `iommu`, указав загрузочный параметр Linux `intel_iommu=off`. Если по какой-то причине требуется IOMMU, следует отключить трансляцию адресов `intel_iommu=on iommu=pt`.
