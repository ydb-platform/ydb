...
Иногда возникает ситуация, когда меняется FQDN узла, при этом сам узел в системе остаётся, но под другим именем. Простая замена имени узла в разделе `hosts` не сработает, так как `BS_CONTROLLER` внутри хранит привязку ресурсов к парам `FQDN:IcPort`, где `IcPort` — номер порта Interconnect, на котором работает узел.
...
4. Заменить FQDN в списке `hosts` в `config.yaml`.
