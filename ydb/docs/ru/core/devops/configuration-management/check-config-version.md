...
1. [Через {{ ydb-ui-name }}](#ydb-ui)
2. [Через метрики кластера](#metrics)

## Через {{ ydb-ui-name }} {#ydb-ui}

Данный способ может быть применен, если сбор метрик с кластера {{ ydb-short-name }} в систему мониторинга не настроен. Вы можете проверить версию конфигурации для конкретного узла или переключаться между узлами через [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md):
