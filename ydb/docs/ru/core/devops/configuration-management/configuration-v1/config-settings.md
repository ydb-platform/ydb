...
Даёт возможность просмотра состояния системы, закрытого от публичного доступа (это большая часть страниц Embedded UI ([YDB Monitoring](../../../reference/embedded-ui/ydb-monitoring.md))), без возможности делать изменения.
    ||
...
В этом разделе настраиваются пороговые значения и таймауты, используемые [сервисом Health Check](../../../reference/ydb-sdk/health-check-api.md) {{ ydb-short-name }}. Эти параметры помогают настраивать возможные [проблемы](../../../reference/ydb-sdk/health-check-api.md#issues), такие как чрезмерные перезапуски или расхождение по времени между динамическими узлами.
