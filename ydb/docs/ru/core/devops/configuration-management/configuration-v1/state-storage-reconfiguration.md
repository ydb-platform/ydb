...
1. Внести требуемые изменения в секции `domains_config` конфигурационного файла `config.yaml` на каждом узле кластера {{ ydb-short-name }}:
    Правила изменения секции `domains_config` см. в разделе [Правила конфигурирования подсистем распространения метаданных](#metadata-subsystems-reconfig-rules).

2. С помощью процедуры [rolling-restart](../../../maintenance/manual/node_restarting.md) последовательно перезапустите все узлы кластера {{ ydb-short-name }} — [статические](../../../concepts/glossary.md#static-node) и [динамические](../../../concepts/glossary.md#dynamic): реплики подсистем метаданных размещаются на статических узлах, к ним обращаются таблетки на динамических. Подробнее об узлах — в разделе [Топология кластера](../../../concepts/topology.md).
    Перед рестартом следующего хоста необходима задержка, чтобы перезапуск на предыдущем хосте завершился и узел снова участвовал в работе кластера.
