...
Основной потребитель CPU — акторы, выполнением которых управляет акторная система. Все акторы, в зависимости от своего типа, выполняются в одном из нескольких доступных в акторной системе пулов потоков (параметр `name`). Конфигурирование заключается в указании количества потоков для каждого из пулов. Следует избегать конфигураций, при которых суммарное количество потоков в пулах превышает количество доступных ядер процессора. При выборе количества потоков в пулах важно учитывать, что PDisk и gRPC API работают вне акторной системы и требуют отдельных ресурсов.
