...
Атрибут `host_config_id` задает числовой идентификатор конфигурации узла. В атрибуте `drive` содержится коллекция описаний подключенных дисков. Каждое описание состоит из двух атрибутов:
...
Одна конфигурация узла с идентификатором 1, с одним диском типа SSD, доступным по пути `/dev/disk/by-partlabel/ydb_disk_ssd_01`:
...
Две конфигурации узлов с идентификаторами 1 (два SSD диска) и 2 (три SSD диска):
...
### Встроенные настройки безопаности {#builtin-security}
...
Основной потребитель CPU — акторы, выполнением которых управляет акторная система. Все акторы, в зависимости от своего типа, выполняются в одном из пулов потоков (параметр `name`). Конфигурирование заключается в указании количества потоков для каждого из пулов, используемых различными подсистемами {{ ydb-full-name }}. Следует избегать конфигураций, при которых суммарное количество потоков в пулах превышает количество доступных ядер процессора. При выборе количества потоков в пулах важно учитывать, что PDisk и gRPC API работают вне акторной системы и требуют отдельных ресурсов.
...
* количество ядер CPU для каждого из пулов потоков акторной системы {{ ydb-full-name }} при использовании ручного конфигурирования
