...
1. В файле `config.yaml` измените значение `host` в существующем элементе списка `hosts`, соответствующем нужному узлу, не изменяя остальные элементы списка и их порядок.
