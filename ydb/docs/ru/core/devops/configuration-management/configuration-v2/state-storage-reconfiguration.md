...
При использовании конфигурации V2 подсистемы распространения метаданных частично поддерживаются автоматически за счёт механизма SelfHeal — см. [SelfHeal State Storage](../../concepts/selfheal-state-storage.md) (перенос и добавление реплик при изменениях топологии). Чтобы отключить это поведение, установите `state_storage_self_heal_config.enable` в `false`, как описано в том же разделе. Отключение не обязательно для нижеприведённых шагов: конфигурация после `ydb admin cluster config replace` применится до очередного срабатывания автоматики.
