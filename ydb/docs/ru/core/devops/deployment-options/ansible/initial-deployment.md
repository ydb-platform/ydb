...
Inventory для `block-8-2` должен описывать минимум двенадцать независимых доменов и закреплять одну поддерживающую сборку для процессов хранения и баз данных. Примера на восемь узлов недостаточно; его правило расчёта числа групп нельзя переносить на группы из двенадцати VDisk. Внешние шаблоны Ansible требуют отдельной адаптации. См. [геометрию и ёмкость Block82](../../../concepts/block-8-2.md).
