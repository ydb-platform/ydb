...
Перед установкой выберите подходящую топологию кластера YDB:

- **Для стартовых установок** рекомендуем использовать топологию `mirror-3dc-3-nodes` — для неё потребуется всего 3 сервера. Это самый простой способ быстро развернуть тестовую или пилотную среду.
- **Для production сред** рекомендуем:
  - `mirror-3-dc` — если у вас есть три датацентра и важна отказоустойчивость на уровне площадок (cross-DC).
  - `block-4-2` — если планируется эксплуатация кластера в одном датацентре.

Подробнее о вариантах топологий и их назначении читайте в разделе [концепции топологий YDB](https://ydb.tech/ru/docs/concepts/topology/).

Выберите топологию, которая соответствует вашим задачам и инфраструктуре.
