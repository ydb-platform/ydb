...
**Для развертывания под аналитическую нагрузку (OLAP)** рекомендуется использовать топологию `mirror-3dc-3-nodes`: 3 сервера, 9 дисков для пользовательских данных и отдельные диски для [спиллинга](../../../../concepts/spilling.md) на каждом узле — по той же схеме, что и для пилотного OLTP-кластера выше. Подробнее о вариантах топологии см. [статью о топологиях](../../../../concepts/topology.md).
...
Дополнительно рекомендуется выделить по одному дополнительному диску на каждый сервер под [спиллинг](../../../../concepts/spilling.md).

{% note info %}

Для OLAP наличие дисков для [спиллинга](../../../../concepts/spilling.md) — обязательно.
