...
**Для развертывания под аналитическую нагрузку (OLAP)** рекомендуется использовать топологию `mirror-3dc-3-nodes`: 3 сервера, 9 дисков для пользовательских данных и отдельные диски для [спиллинга](../../../../concepts/query_execution/spilling.md) на каждом узле — по той же схеме, что и для пилотного OLTP-кластера выше. Подробнее о вариантах топологии см. [статью о топологиях](../../../../concepts/topology.md).

На каждом сервере должен быть хотя бы один отдельный диск для пользовательских данных. Рекомендуется также добавить отдельный небольшой диск под операционную систему. Подробнее о различных вариантах топологии и избыточности читайте в [этой статье](../../../../concepts/topology.md).

Дополнительно рекомендуется выделить по одному дополнительному диску на каждый сервер под [спиллинг](../../../../concepts/query_execution/spilling.md).

{% note info %}

Для OLAP наличие дисков для [спиллинга](../../../../concepts/query_execution/spilling.md) — обязательно.
...
- OLAP
  ...
  * Дополнительные SSD-диски для данных, не менее 120 GB каждый.
  * Дополнительные SSD-диски для спиллинга, не менее 120 GB каждый.
  * Доступ по SSH.
  * Сетевая связность между машинами в кластере.
  * ОС: Ubuntu 18+, Debian 9+.
  * Доступ в интернет для обновления репозиториев и скачивания необходимых пакетов.
