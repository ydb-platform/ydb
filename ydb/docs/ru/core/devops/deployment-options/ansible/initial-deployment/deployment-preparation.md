...
Дополнительно для OLTP рекомендуется выделить по одному дополнительному диску на каждый сервер под спиллинг.

{% note info %}

Для OLAP наличие дисков для спиллинга — обязательно.
