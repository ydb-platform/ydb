...
Зафиксируйте состояние дисковой подсистемы до изменений, чтобы после добавления диска убедиться в успешности операции. Текущий набор PDisk каждого узла показывает [Embedded UI](../../../reference/embedded-ui/ydb-monitoring.md), доступный на порту мониторинга узла хранения.
...
3. Проверьте состояние PDisk кластера:
    ...
    В выводе команды должны присутствовать новые PDisk. Значение `ACTIVE` в колонке `Status` означает, что кластер готов размещать на диске VDisk.

4. Если новые PDisk имеют статус `INACTIVE`, переведите их в состояние `ACTIVE`:
    ...
    * `<pdisk-ids>` — разделённый пробелами список идентификаторов PDisk в формате `[NodeId:PDiskId]`, взятых из вывода команды `pdisk list`, например `[1:2] [2:2] [3:2]`. Указывайте только новые PDisk: каждый узел хранения кластера добавляет в список одну запись.

    Повторно выполните команду `pdisk list` и убедитесь, что все нужные PDisk имеют статус `ACTIVE`. Диск со статусом `INACTIVE` остаётся зарегистрированным в кластере, но новые VDisk на нём не размещаются, поэтому добавление групп хранения завершится ошибкой.
