...
Для `block-8-2` нужны двенадцать независимо размещённых узлов хранения и образ YDB с поддержкой схемы в ресурсах Storage и Database. Шаблон `ydbd_slice` в репозитории называется `12-node-block-8-2`; явно задайте `ydb_image`. Перед применением внешних примеров убедитесь в поддержке схемы оператором и его валидаторами. См. [требования Block82](../../../concepts/block-8-2.md).
