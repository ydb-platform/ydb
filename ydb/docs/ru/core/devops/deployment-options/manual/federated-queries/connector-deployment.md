...
| `logger.enable_sql_query_logging` | Для источников данных, поддерживающих SQL, включает логирование транслированных запросов. Допустимые значения: `true`, `false`. {% note warning %} Включение этой опции может привести к печати конфиденциальных пользовательских данных в логи. {% endnote %} | `false` |
| `metrics_server` | Опциональная секция. Содержит настройки вспомогательного HTTP-сервера, выполняющего доступ к метрикам. | Сервис отключён. |
