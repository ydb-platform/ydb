...
|----------|------------|-----------------------|
| `connector_server` | Обязательная секция. Содержит настройки основного сервера, выполняющего доступ к данным. ||
| `connector_server.endpoint.host` | Имя хоста или IP-адрес, на котором сервер будет принимать TCP-соединения. ||
| `connector_server.endpoint.port` | Номер порта, на котором сервер будет принимать TCP-соединения. ||
| `connector_server.tls` | Опциональная секция. Заполняется, если требуется включение TLS-соединений для основного сервера. | Сервер запускается без TLS. |
...
| `paging.bytes_per_page` | Максимальное количество байт в одной странице. Рекомендуемые значения - от 4 до 8 МиБ, максимальное значение - 48 МиБ. | 4 МиБ |
...
| `datasources.postgresql.open_connection_timeout` | Таймаут открытия соединения с PostgreSQL. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.clickhouse.open_connection_timeout` | Таймаут открытия соединения с ClickHouse. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
| `datasources.clickhouse.ping_connection_timeout` | Таймаут проверки соединения с ClickHouse. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.mysql.open_connection_timeout` | Таймаут открытия соединения с MySQL. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.ms_sql_server.open_connection_timeout` | Таймаут открытия соединения с MS SQL Server. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
| `datasources.ms_sql_server.ping_connection_timeout` | Таймаут проверки соединения с MS SQL Server. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.greenplum.open_connection_timeout` | Таймаут открытия соединения с Greenplum. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.ydb.open_connection_timeout` | Таймаут открытия соединения с {{ ydb-short-name }}. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
| `datasources.ydb.ping_connection_timeout` | Таймаут проверки соединения с {{ ydb-short-name }}. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5s` |
...
| `datasources.ydb.splitting.query_tablet_ids_timeout` | Таймаут для запроса, запрашивающего идентификаторы таблеток колоночной таблицы {{ ydb-short-name }}. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `1m` |
...
| `datasources.ydb.table_metadata_cache.ttl` | Время жизни (TTL) для кешированных значений. Значение должно состоять из числового литерала и суффикса, обозначающего единицы измерения (наносекунды `ns`, микросекунды `us`, миллисекунды `ms`, секунды `s`, минуты `m`, часы `h`). Примеры значений: `100ms`, `5s`, `3h`. | `5m` |
