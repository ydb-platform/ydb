...
| `connector_server.endpoint.port` | Номер порта, на котором север будет принимать TCP-соединения. ||
