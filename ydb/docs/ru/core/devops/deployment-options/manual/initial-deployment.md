...
Для нового кластера `block-8-2` нужны минимум двенадцать независимых доменов и пример `block-8-2.yaml` из выбранной сборки YDB с поддержкой схемы. Используйте эту сборку на узлах хранения и баз данных. Это не преобразует существующую статическую группу. См. [требования Block82](../../../concepts/block-8-2.md).
