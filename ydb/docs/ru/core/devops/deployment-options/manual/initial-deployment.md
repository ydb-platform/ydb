...
- Аутентификация включена
  ...
  Для выполнения команды инициализации кластера (`bootstrap`) с включенной аутентификацией требуется специальный порядок действий. На момент инициализации системные компоненты, отвечающие за проверку аутентификационных токенов, еще не запущены.

  Поэтому для развертывания защищенного кластера необходимо сначала инициализировать его в разрешающем режиме, а затем переключить в защищенный.

  1. Убедитесь, что кластер запущен с `enforce_user_token_requirement: false`, как описано в разделе [«Подготовьте конфигурационные файлы»](#config).

  2. Выполните инициализацию кластера, следуя инструкции из соседней вкладки «Аутентификация выключена».

  3. Получите аутентификационный токен для встроенной учетной записи `root`. После успешной инициализации кластера пароль еще не задан:
  ...
  4. Подготовьте файл конфигурации для защищенного режима. Для этого скопируйте ваш исходный `config.yaml` в новый файл (например, `secure_config.yaml`) и измените в нем значение флага `enforce_user_token_requirement` на `true`:
  ...
  5. Примените новую конфигурацию в кластер с помощью команды `replace`, используя полученный ранее токен:
  ...
  6. Перезапустите узлы кластера для применения конфигурации.
  ...
  7. Установите пароль для учетной записи `root`.
     ...
     Теперь, когда кластер работает в режиме с обязательной аутентификацией, установите пароль для администратора, чтобы завершить настройку безопасности:

      ```bash
      ydb --ca-file ca.crt -e grpcs://<node1.ydb.tech>:2135 -d /Root --token-file token-file \
          yql -s 'ALTER USER root PASSWORD "YOUR_SECURE_PASSWORD"'
      ```
     В дальнейшем для административных действий вы сможете использовать новый пароль или токен.
