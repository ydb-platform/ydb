...
Если на кластере присутствует конфигурация v2, то в выводе команды будет присутствовать опция `ydbd --config-dir`.
...
В результате проделанных действий конфигурация кластера будет переведена на механизм конфигурации v2. Любое изменение конфигурации на существующих узлах выполняется с помощью [команд YDB CLI](../update-config.md). при первоначальном запуске узла, как и прежде, конфигурация будет загружаться из файла, указанного в опции `ydbd --yaml-config`. Однако при повторном запуске узла, актуальная конфигурация будет загружена из директории, указанной в опции `ydbd --config-dir`.
