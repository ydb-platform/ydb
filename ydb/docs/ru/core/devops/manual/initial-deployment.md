...
Название блочных устройств зависит от настроек OS, которые были выставлены системным администратором или поставлены облачным провайдером. Например, имена блочных устройств в Yandex Cloud имеют базу `vd(n)`, где `n` — это инкрементируемый литерал. В AWS имена блочных устройств включают в себя тип устройства (`hdd`,`ssd`, `nvme`) и порядковый номер (`0n1`, `1n1`).
...
В перечисленных выше примерах команд `<node.ydb.tech>` - FQDN сервера, на котором запущен любой динамический узел, обслуживающий базу `/Root/testdb`. При подключение по SSH к динамическому узлу YDB, удобно использовать конструкцию ```grpcs://`hostname -f`:2136``` для получения FQDN.
