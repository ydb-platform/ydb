...
Кластеры, находящиеся за сетевыми барьерами, могут быть недоступны для прямого наблюдения за их состоянием. Для диагностики проблем во внутреннем состоянии такого изолированного кластера можно использовать функцию записи внутреннего состояния в файл.
...
* `--no-sanitize` - пользовательские имена таблиц, колонок, запросы по умолчанию вырезаются из отчета. Эта опция позволяет получить полный отчет.
