...
Указанная директория должна существовать до запуска процесса; {{ ydb-short-name }} не создаёт её автоматически. Если директория отсутствует в момент падения, запись минидампа выполнена не будет.
