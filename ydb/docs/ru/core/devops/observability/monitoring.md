...
- Ansible (с TLS)
  ...
  Проверьте содержимое сгенерированных файлов `ydbd-storage.yml` и `ydbd-database.yml`. Список узлов и портов должен совпадать с фактической топологией кластера, в том числе с узлами, отображаемыми во [{{ ydb-ui-name }} кластера](../../reference/ydb-ui/ydb-monitoring.md).
