...
- Обычная установка
  ...
  Определите значения портов узлов баз данных командой на **хосте**, где запущен соответствующий узел: по SSH или в локальной консоли на этой машине.
  ...
  Если в выводе несколько процессов `ydbd` с разными `--mon-port`, перечислите в `targets` **все** порты динамических узлов, которые нужно опрашивать на данном сервере. На скриншоте выделены примеры — сопоставляйте порты с вашей фактической конфигурацией.
...
Отредактированные файлы положите в **любую** удобную директорию на машине, где запускается Prometheus (рядом с бинарником или отдельно, например `/etc/prometheus`). В параметре `--config.file` укажите **полный или относительный путь** к `prometheus_ydb.yml`. Файлы `prometheus_ydb.yml`, `ydbd-storage.yml` и `ydbd-database.yml` держите в **одной** папке: в шаблоне к `ydbd-storage.yml` и `ydbd-database.yml` обращаются **относительные** пути, и Prometheus ищет их относительно **рабочей директории процесса** при старте. Запускайте Prometheus из этой папки (`cd` в неё перед командой) либо пропишите в `prometheus_ydb.yml` абсолютные пути к файлам целей. После этого [запустите](https://prometheus.io/docs/prometheus/latest/getting_started/#starting-prometheus) Prometheus.
