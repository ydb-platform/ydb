...
Начиная с текущей версии появилась возможность настраивать права доступа к каталогу `.sys` c системными представлениями кластера и отдельных баз, а также к самим системным представлениям. 
При возврате к предыдущей версии все выставленные права на каталог `.sys` и системные представления будут сохранены.

Пользователи, удовлетворяющие следующим условиям, при обновлении с предыдущей версии **потеряют** доступ к системным представлениям:
 * являющиеся владельцами корня базы данных;
 * не имеющие выставленных прав доступа на корне базы.

Для восстановления доступа им необходимо выдать себе права на чтение на корень базы или на каталог `.sys`.

{% endnote %}

## Разграничение прав доступа

Возможность настраивать права доступа к системным представления может помочь при необходимости разграничения доступа к служебной информации, 
хранящейся в системных представлениях, и данным, например, таблиц. 

В случае, если в системе предполагается наличие пользователей, отвечающих за корректную работу базы данных, и которые должны иметь доступ к системам мониторинга, 
но не иметь доступа к данным, хранящимся в базе, 
то для таких пользователей эту модель поведения базы можно настроить путем выдачи таким пользователям прав на чтение только на каталог `.sys`, 
не выдавая права на чтение на корень базы. 

В обратной ситуации, когда от пользователей, которые, например, выполняют аналитические запросы к данным и не занимаются оптимизацией запросов, хочется скрыть наличие системных представлений, 
то это можно сделать, не выдавая им прав на просмотр объектов на корень базы, а выдавая права только на каталог, в котором содержатся данные, с которыми работают пользователи.

Кроме этого данная возможность позволяет задавать различные права доступа к определенному системному представлению для отдельных пользователей и групп пользователей, 
что делает возможным предоставление различным ролям пользователей различных полномочий.
