...
Начиная с версии 25-4 появилась возможность настраивать права доступа к каталогу `.sys` c системными представлениям кластера и отдельных баз, а также к самим системным представлениям.

Эта возможность предоставляется автоматически, но может вызвать неожидаемое поведение у пользователей, которые являются владельцами корня базы данных, но не имеют прав на чтение на корне базы.
Такие пользователи потеряют возможность читать данные из системных представлений, пока не выдадут себе права на чтение на корень базы или на каталог `.sys`.
При возврате к предыдущей версии (25-3) все выставленные права на каталог `.sys` и системные представления сохранятся.

Данная возможность может помочь при необходимости разграничения доступа к служебной информации, хранящейся в системных представлениях, и данным, например, таблиц. 
...
что позволяет предоставить различным ролям пользователей в базе данных различные полномочия.
