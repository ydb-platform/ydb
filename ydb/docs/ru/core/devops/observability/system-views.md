...
Начиная с версии 25.4 появилась возможность настраивать права доступа к каталогу `.sys` — это позволяет гибко настраивать права не только на весь каталог с системными представлениями (как кластера, так и отдельных баз данных), но и на отдельные системные представления.

Если вы обновляете {{ ydb-short-name }} с предыдущей версии, все ранее выставленные права доступа к каталогу `.sys` и к системным представлениям сохранятся. Однако обратите внимание: пользователи, которые являются владельцами корня базы данных, но при этом не имеют явно выданных прав на чтение в корне базы, после обновления **потеряют** доступ к системным представлениям.

Чтобы вернуть доступ, таким пользователям необходимо выдать себе права на чтение либо на корневой каталог базы данных, либо непосредственно на каталог `.sys`.
