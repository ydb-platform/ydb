...
`ErasureSpecies` может иметь значение `block-8-2`; обычная группа содержит двенадцать VSlot. При сравнении разных схем фильтруйте группы по пулу. BSC также публикует отдельные gauge-счётчики `GroupErasureInfo` и `StoragePoolErasureInfo`; `DskUsedBytes` сохраняет единицы физических байтов. См. [метки соответствия и коэффициенты ресурсов](../../concepts/block-8-2.md).
...
| ErasureSpecies      | String   |          | Режим кодирования избыточности для группы (block-4-2, block-8-2, mirror-3-dc, mirror-3of4, ...)                       |
