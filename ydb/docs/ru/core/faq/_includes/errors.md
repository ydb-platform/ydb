...
## Из-за чего возникает ошибка «State of operation is unknown»? {#state-unknown}

Данная ошибка возникает при потере связи с шардом во время пишущей операции — в этом случае невозможно установить её результат. Система не может определить, выполнена ли операция успешно или отменена. Поскольку пишущие операции обычно не являются идемпотентными, повторная отправка (ретраи) невозможна — ошибка относится к категории non-retriable. Такая ситуация может возникнуть при любом перемещении шардов — как во время обновления, так и при балансировке нагрузки. При этом ретраи допустимы для идемпотентных запросов.
