...
Поскольку в памяти YQL работает с неизменяемыми значениями, для изменения JSON нужно сконструировать новое значение из частей старого. Для этого используется [`Yson` UDF](../../yql/reference/udf/list/yson.md) в комбинации с функциями для работы со [словарями](../../yql/reference/builtins/dict.md).
