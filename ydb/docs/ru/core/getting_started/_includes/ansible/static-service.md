...
ExecStart={{ ydb_dir }}/release/{{ ydb_version }}/bin/ydbd server \                     # Запуск процесса YDB с параметром server 
    --log-level 3 --syslog --tcp --yaml-config  {{ ydb_dir }}/cfg/static_config.yaml \  # Передача конфигурационного файла статической ноды
    --grpc-port {{ grpc_port }} \                                                       # Установка значения gRPC порта из переменной {{ grpc_port }}
    --ic-port {{ ic_port }} \                                                           # Установка значения IC порта из переменной {{ ic_port }}
    --mon-port {{ mon_port }} \                                                         # Установка значения порта мониторинга из переменной {{ mon_port }}
    --node static                                                                       # Указатель запуска процесса YDB в качестве статической ноды
