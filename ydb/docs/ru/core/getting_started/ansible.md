...
Переменные из файла `/group_vars/all.yaml` используются в задачах плейбука и в шаблонах файлов, которые копируются на хост. На хост копируются service файл для запуска статической и динамической ноды через systemd (юниты) и конфигурационные файлы для обоих нод, которые указываются в параметрах запуска юнитов. Содержание копируемых файлов с комментариями: 
