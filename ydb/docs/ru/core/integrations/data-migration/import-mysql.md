...
| `BLOB` / `BINARY` | Та же колонка, значение inline в `String` (байты) | Колонка становится `Int64` (идентификатор blob); данные выносятся в **отдельную таблицу** `${schema}/${table}_${field}` со строками `(id, pos, val)` блоками по 64 КиБ |
