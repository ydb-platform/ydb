...
Настройте аккаунт пользователя {{ ydb-short-name }} с правами **только на чтение** [(подробнее об уровнях доступа)](../cluster/access.md) и доступом до баз данных и таблиц, к которым потребуется делать запросы.
...
После установки плагина следуйте [этим инструкциям](https://grafana.com/docs/grafana/latest/datasources/add-a-data-source/), чтобы добавить новый источник данных {{ ydb-short-name }}, и укажите настройки для соединения.

## Настройка через файл конфигурации

Источник данных можно настроить с помощью файла конфигурации. Изучить методику работы, включая все возможные настройки источника данных можно в [документации  Grafana](https://grafana.com/docs/grafana/latest/administration/provisioning/#data-sources).
...
Для запроса данных из баз {{ ydb-short-name }} поддерживает [YQL диалект](../yql/reference/index.md).
Запросы могут содержать макросы, которые упрощают синтаксис и дают возможность динамически изменять параметры.
 В запросе могут содержаться два вида макросов - [уровня grafana](#macros) и уровня {{ ydb-short-name }}. Перед отправкой запроса в {{ ydb-short-name }}, плагин проанализирует текст запроса и заменит макросы уровня Grafana на конкретные значения. После этого макросы уровня {{ ydb-short-name }} будут обработаны на сервере {{ ydb-short-name }}.
...
Чтобы создать многострочный временной ряд, результаты запроса должны содержать в себе как минимум три поля в следующем порядке:
