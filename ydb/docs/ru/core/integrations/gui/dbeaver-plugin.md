...
### Установка через URL P2-репозитория {#installation-url}
...
1. В открывшемся окне **Add Repository** укажите имя репозитория (например, `YDB Plugin`) и вставьте в поле **Location** следующий URL:
    ...
    В процессе установки DBeaver скачивает не только сам плагин, но и его зависимости (OSGi-компоненты) с серверов проекта Eclipse и других внешних репозиториев. Если эти ресурсы недоступны из вашей сети, обратитесь к сетевому администратору или воспользуйтесь [установкой из ZIP-архива](#installation-zip).
...
### Установка из ZIP-архива (GitHub Releases) {#installation-zip}

1. Перейдите на [страницу GitHub Releases](https://github.com/ydb-platform/ydb-dbeaver-plugin/releases) и скачайте файл `ydb-dbeaver-plugin-*.zip` нужной версии.
...
1. DBeaver может показать предупреждение о неподписанном содержимом. Нажмите **Install Anyway**.
...
1. Откроется страница настройки подключения к {{ ydb-name }}. Заполните поля:
    ...
    | **Monitoring URL** | URL [{{ ydb-short-name }} YDB UI](../../reference/ydb-ui/index.md) с путём к базе данных, используется для дашборда (необязательно) | `http://ydb.example.com:8765/monitoring/tenant?name=%2FRoot%2Fdatabase` |
...
Дашборд доступен только при работе с self-hosted инсталляциями {{ ydb-short-name }}, где есть доступ к [{{ ydb-short-name }} YDB UI](../../reference/ydb-ui/index.md). В Yandex Cloud Managed Service for {{ ydb-short-name }} Embedded UI не публикуется, поэтому данные дашборда недоступны — для мониторинга используйте [средства облачной платформы](https://yandex.cloud/ru/docs/ydb/operations/monitoring).
...
Автоматические обновления поддерживаются только при [установке через URL P2-репозитория](#installation-url). DBeaver запоминает источник (URL репозитория) и при публикации новой версии предлагает обновление одним из двух способов:
