...
`loadData`, `loadUpdateData` - загрузка данных из CSV файла. *loadUpdateData* загружает данные командой [UPSERT INTO](../yql/reference/syntax/upsert_into.md). Данные будут автоматически преобразованы в требуемые типы с учетом строгой типизации в {{ ydb-short-name }}.

`insert` - changeset, который осуществляет единичный insert в таблицу командой [INSERT INTO](../yql/reference/syntax/insert_into.md). Например:
