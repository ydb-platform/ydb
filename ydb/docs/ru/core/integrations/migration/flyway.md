...
```bash
flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration -baselineVersion=3 baseline
```
...
```bash
flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration migrate
```
...
Результатом исполнения команды `flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration info` будет подробная информация о состоянии миграций:
...
После применения к текущим миграциям команды `flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration validate`, в логах будет написано, что последняя миграция не была применена к нашей базе данных:
...
```bash
flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration repair
```
...
```bash
flyway -url=jdbc:ydb:grpc://localhost:{{ ydb-ports.grpc }}/local -locations=db/migration -cleanDisabled=false clean
```
