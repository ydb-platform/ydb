...
Адаптер поддерживает все основные типы данных {{ ydb-short-name }} (`Bool`, целочисленные и вещественные типы, `Utf8`, `String`, `Uuid`, `Json`, типы даты и времени и др.), а также {{ ydb-short-name }}-специфичные возможности схемы: вторичные и векторные индексы, TTL, партиционирование и семейства колонок. Полный перечень типов и опций приведён в [руководстве по схеме](https://ydb.js.org/guide/drizzle/schema).
