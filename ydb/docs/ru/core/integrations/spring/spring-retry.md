...
| `maxRetries` | `-1` | Максимальное число повторных попыток после первого неудачного выполнения. Метод может быть выполнен суммарно до `maxRetries + 1` раз. Значение `-1` означает наследование из глобальной конфигурации. Значение `0` не поддерживается — для отключения повтора используйте `enabled = false`. |
