...
[spring-ydb-retry](https://github.com/ydb-platform/ydb-java-dialects/tree/main/spring-ydb-retry) — это модуль автоконфигурации Spring Boot, который добавляет автоматический повтор транзакционных методов при возникновении повторяемых ошибок {{ ydb-short-name }}.
...
{{ ydb-short-name }} является распределённой базой данных, поэтому часть ошибок носит временный (transient) характер: например, может устареть сессия, возникнуть кратковременная перегрузка, временно стать недоступным узел или транзакция может быть прервана из-за конкурентного изменения данных. Корректная обработка таких ошибок требует повторного выполнения транзакции целиком, а не отдельного запроса. Модуль `spring-ydb-retry` берёт эту логику на себя: он перехватывает транзакционные методы, классифицирует ошибки по [кодам статуса {{ ydb-short-name }}](../../reference/ydb-sdk/error_handling.md) и при необходимости повторяет транзакцию с [экспоненциальной задержкой](../../concepts/glossary.md#exponential-backoff) и [джиттером](../../concepts/glossary.md#jitter).

## Возможности {#features}

- Автоматический повтор методов `@Transactional` при повторяемых кодах статуса {{ ydb-short-name }}.
- Аннотация `@YdbTransactional` с настройками повтора на уровне отдельного метода (максимальное число попыток, [backoff](../../concepts/glossary.md#exponential-backoff), [идемпотентность](../../concepts/glossary.md#idempotency)).
- Двухуровневая стратегия задержки (быстрая/медленная) с [джиттером](../../concepts/glossary.md#jitter), подобранная под семантику ошибок {{ ydb-short-name }}.
- Режим [идемпотентности](../../concepts/glossary.md#idempotency) для расширенного покрытия повторами на недетерминированных кодах статуса.
...
Модуль настраивается автоматически через механизм автоконфигурации Spring Boot. Как только зависимость оказывается в classpath, модуль заменяет стандартный [перехватчик транзакций](../../concepts/glossary.md#transaction-interceptor) Spring (бин `transactionInterceptor`) на свою реализацию и автоматически оборачивает логикой повтора все методы, помеченные `@Transactional` и `@YdbTransactional`. Никаких дополнительных аннотаций или явного подключения не требуется — достаточно настроить источник данных {{ ydb-short-name }}:
...
[Повтор транзакции](../../concepts/glossary.md#transaction-retry) означает её повторное выполнение целиком. Это безопасно только для идемпотентных операций — таких, повторный запуск которых приводит к тому же результату (например, `UPSERT` с детерминированным ключом или операции чтения).
