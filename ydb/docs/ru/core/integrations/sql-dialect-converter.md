...
Конвертер запускается в [Yandex Cloud Function](https://yandex.cloud/ru/services/functions) — бессерверном окружении, которое выполняет код по HTTP-запросу. Клиент (плагин DBeaver или любой другой) отправляет исходный запрос и имя исходного диалекта на HTTPS-эндпоинт функции; ответом возвращается результат преобразования. Такая архитектура позволяет обновлять конвертер централизованно без выпуска новых версий клиентских инструментов.
