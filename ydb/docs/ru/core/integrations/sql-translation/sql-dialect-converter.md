...
Этапы выполняются на базе библиотеки [SQLGlot](https://github.com/tobymao/sqlglot) и [плагина {{ ydb-short-name }} для неё](sqlglot.md), который добавляет в SQLGlot отдельный диалект {{ ydb-short-name }}. SQLGlot — SQL-парсер и транспилер с открытым исходным кодом, поддерживающий более двадцати диалектов. Он выполняет роль как парсера исходного запроса, так и генератора YQL.
