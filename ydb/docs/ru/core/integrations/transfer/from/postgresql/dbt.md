# Перенос данных из PostgreSQL в {{ ydb-short-name }} с помощью dbt (dbt-ydb)

dbt материализует SQL-модель в {{ ydb-short-name }}. Источник — External Data Source (PostgreSQL).
