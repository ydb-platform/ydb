...
Новые группы пула `block-8-2` требуют двенадцати независимых доменов отказа. Динамический пул может использовать Block82 без изменения существующей статической схемы. Проверяйте выбранный пул и геометрию созданных групп перед переносом каналов таблеток. См. [конфигурацию Block82](../../concepts/block-8-2.md).
