...
В настоящее время YDB не предоставляет встроенных команд для проверки зависимостей цепочки или проверки целостности цепочки. Вы должны вручную отслеживать, какие резервные копии принадлежат какой цепочке:
- Перечисление резервных копий в хронологическом порядке с помощью `ydb scheme ls .backups/collections/<имя_коллекции>/ | sort`
- Определение типов резервных копий по суффиксу имени каталога (`_full` или `_incremental`)
- Обеспечение удаления полных цепочек (всех резервных копий от полной резервной копии до её инкрементальных)
