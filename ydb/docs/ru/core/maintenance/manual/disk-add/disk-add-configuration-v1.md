# Добавление дисков в кластеры YDB, развёрнутые вручную
...
Если вы настраивали динамическую конфигурацию на кластере, её также необходимо отредактировать.

{% endnote %}

{% cut "Редактирование динамической конфигурации" %}

За дополнительной информацией обращайтесь к документации [динамической конфигурации](../../../devops/configuration-management/configuration-v1/dynamic-config.md).
...
Для применения новой конфигурации выполните [последовательный перезапуск](../node_restarting.md) всех существующих статических узлов кластера, с ожиданием инициализации и восстановления работы каждого перезапускаемого узла.
...
- Перезапуск через systemd
  ...
  3. Выполните перезапуск с помощью команд:
    * `sudo service ydbd-storage restart` — на серверах со статическими узлами;
    * `sudo service ydbd-testdb restart` — на серверах с динамическими узлами;
...
Добавьте новые узлы хранения. Это можно сделать с помощью утилиты `ydb-dstool`. Для выполнения операции потребуется токен аутентификации, обновите его при необходимости командой выше.
...
Если вов время добавления группы возникают ошибки, проверьте состояние PDisks командой:
```bash
