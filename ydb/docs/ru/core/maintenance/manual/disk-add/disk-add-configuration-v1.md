...
* `storage_config_generation: 1` — номер изменения конфигурации, значение приведено для примера: в нём предполагается, что конфигурация ранее не изменялась. Укажите значение, следующее за номером текущей конфигурации вашего кластера.

Остальные секции и настройки конфигурационного файла остаются без изменений.

{% note info %}

Если текущий номер изменения конфигурации неизвестен, узнать его можно из ошибки сверки номера конфигурации: при неверном значении команда инициализации конфигурации возвращает ожидаемый номер в поле `ItemConfigGenerationExpected`. Подробнее об этом читайте в разделе [Добавление статических узлов](../../../devops/configuration-management/configuration-v1/cluster-expansion.md#add-static-node).
...
Если динамическая конфигурация на кластере не настроена, команда завершается, не вернув YAML-конфигурацию: файл `dynamic_config.yaml` остаётся пустым, а YDB CLI печатает диагностическое сообщение. Текст сообщения зависит от версии YDB CLI и от того, какой API поддерживает кластер, например:

```text
No config returned.
```

Ориентируйтесь не на конкретный текст сообщения, а на то, что конфигурация не получена. В этом случае пропустите раздел редактирования динамической конфигурации.
...
На снимке экрана все узлы кластера работают, но новый диск ещё не зарегистрирован: в колонке `PDisks` по-прежнему отображается по три диска на узел хранения.
...
На любом из серверов кластера инициализируйте конфигурацию с новыми дисками командой:
...
2. Проверьте состояние PDisk кластера командой:
  ...
  * `auth_token` — имя файла, содержащего токен аутентификации.
  ...
  В выводе команды должны присутствовать новые PDisk. Проверьте колонку `Status` у всех PDisk, а не только у новых: после перезапуска узлов кластера в статусе `INACTIVE` могут оказаться и ранее добавленные диски. На PDisk со статусом `INACTIVE` новые VDisk не размещаются, поэтому добавление групп хранения завершится ошибкой.
