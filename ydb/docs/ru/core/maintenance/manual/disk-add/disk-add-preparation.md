...
На снимке экрана показана вкладка `Nodes` [Embedded UI](../../../reference/embedded-ui/ydb-monitoring.md): в кластере три узла хранения, у каждого в колонке `PDisks` по три диска. После добавления диска на каждом узле хранения станет на один PDisk больше.
...
В приведённом выше примере `vde` — имя нового диска; в вашей системе оно может быть другим.
...
* `/dev/vde` — имя нового диска, значение приведено для примера; узнайте имя диска в своей системе командой `lsblk`;
* `/dev/disk/by-partlabel/ydb_disk_ssd_04` — метка нового раздела, значение приведено для примера (используется в дальнейшем при добавлении диска в конфигурационный файл);
* `/opt/ydb/bin/ydbd` — путь к исполняемому файлу `ydbd` (в примере используется путь по умолчанию).
...
Выберите инструкцию в соответствии с версией вашей конфигурации:
