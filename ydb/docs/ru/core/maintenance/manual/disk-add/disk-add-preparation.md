...
На снимке экрана показана вкладка `Nodes` [{{ ydb-ui-name }}](../../../reference/ydb-ui/ydb-monitoring.md): в кластере три узла хранения, у каждого в колонке `PDisks` по три диска. После добавления диска на каждом узле хранения станет на один PDisk больше.
