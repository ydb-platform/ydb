...
Если за это время узел или диск снова заработал, SelfHeal не запускается: механизм рассчитан на длительные отказы, а не на кратковременные сбои. Быстрое ручное восстановление не делает SelfHeal бесполезным, просто перенос не успевает начаться.
...
Обычно SelfHeal оставляют включённым. Временно отключить его имеет смысл, например, в таких ситуациях:

* в SelfHeal обнаружен баг, и вместо восстановления он ухудшает состояние данных;
* одновременно отказало много узлов, кластер перегружен, восстановление своими силами займёт дольше часа, а лишняя фоновая репликация от SelfHeal только добавит нагрузку.

После стабилизации снова включите SelfHeal.

## Настройки SelfHeal {#settings}

Параметры ниже задают, как часто CMS опрашивает диски и через сколько циклов наблюдения меняет статус PDisk в [Blob Storage Controller](../../concepts/glossary.md#ds-controller). Произведение интервала опроса и числа циклов даёт задержку до смены статуса (и дальше до старта переноса), в том числе привычные «около часа» при значениях по умолчанию.
