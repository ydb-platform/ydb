...
Проверить, что изменения применились, можно  в разделе `CMS` в [YDB UI](../../reference/ydb-ui/index.md) кластера (доступен на порту 8765): перейдите на вкладку `Sentinel` для просмотра статуса Sentinel и Self Heal State Storage.
