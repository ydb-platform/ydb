...
Для решения этой задачи применяется механизм декомиссии групп хранения. В рамках этого механизма создаётся новая [виртуальная группа хранения](../../concepts/glossary.md#virtual-storage-groups), которая заменяет собой исходную физическую группу. После выполнения операции система переходит в следующее состояние:
...
- Ресурсы, используемые декомиссуемой группой, освобождаются ([VDisk'и](../../concepts/glossary.md#vdisk) удаляются, [слоты PDisk'ов](../../concepts/glossary.md#slot) освобождаются)
...
Во время декомиссии доступ к данным сохраняется — пользователи могут читать и записывать данные, как обычно. Все перемещения данных происходят в фоновом режиме и не мешают работе приложений.
...
- Блокируется запись новых данных в декомиссуемую физическую группу
...
**Что происходит:**
- Копируются метаданные всех блобов (информация о том, какие данные есть, но не сами данные) из декомиссуемой физической группы в оставшиеся физические группы хранения
...
**Что происходит:**
- Читаются данные из декомиссуемой физической группы
...
### Освобождение ресурсов декомиссуемой физической группы

**Что происходит:**
- Все данные скопированы
- Удаляется декомиссуемая физическая группа и её VDisk'и
