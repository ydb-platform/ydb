...
Должен быть развернут [кластер](../../concepts/glossary.md#cluster) {{ ydb-short-name }}. Инструкции по развертыванию доступны в разделе [Первоначальное развёртывание](../../devops/deployment-options/manual/initial-deployment.md).

### Установленный {{ ydb-short-name }} DSTool

Должна быть установлена утилита `ydb-dstool`. Инструкции по установке доступны в [документации по установке {{ ydb-short-name }} DSTool](../../reference/ydb-dstool/install.md).

### Полученный токен аутентификации

Должен быть сгенерирован токен аутентификации для доступа к кластеру:
...
Перед запуском декомиссии необходимо выбрать группу (group ID), которую вы хотите декомиссировать. Это можно сделать через веб-интерфейс кластера или с помощью команды:

{% list tabs %}

- UI (Веб-интерфейс)
  1. Откройте вкладку `Storage` в веб-интерфейсе https://{URL_CLUSTER}:8765/monitoring/cluster/storage
  2. В таблице будут нужные вам group id
