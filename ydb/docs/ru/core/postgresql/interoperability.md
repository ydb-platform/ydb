...
1. Программа отправляет запросы в {{ ydb-short-name }} через стандартные инструменты {{ ydb-short-name }} с указанием PostgreSQL-диалекта.
2. Query processor транслирует PostgreSQL запросы в YQL AST.
3. После обработки запросов результаты собираются и отправляются обратно в программу. При обработке запроса он может распараллеливаться и исполняться на произвольном количестве узлов {{ ydb-name }}.
