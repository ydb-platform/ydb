...
По умолчанию слушатель сетевого протокола PostgreSQL (pgwire) на узлах `ydbd` и в локальном Docker-образе {{ ydb-short-name }} **отключён**. Чтобы включить pgwire и функции совместимости с PostgreSQL на узле кластера, задайте `enable_local_pg_wire: true` в секции [`local_pg_wire_config`](../reference/configuration/local_pg_wire_config.md) файла конфигурации. В Docker установите `YDB_EXPERIMENTAL_PG=1` — см. [{#T}](../reference/docker/start.md).
