...
Доклад посвящен архитектуре аналитического движка {{ ydb-short-name }}, который позволяет выполнять сложные OLAP-запросы поверх транзакционной (OLTP) базы данных без влияния на основную нагрузку. Спикер Алексей Озерицкий детально разбирает, как это достигается.

@[YouTube](https://www.youtube.com/watch?v=sRX7zLT1exQ)
...
@[YouTube](https://www.youtube.com/watch?v=Pm8PONakdP8)
...
## Используем {{ ydb-short-name }}: Возможности YDB UI и CLI {#2023-webinar-ydb-interface}

{% include notitle [practice_tag](../_includes/tags.md#practice) %}


[{{ team.kovalenko.name }}]({{ team.kovalenko.profile }}) ({{ team.kovalenko.position }}) рассказал и показал как запустить нагрузку на кластер с помощью {{ ydb-short-name }} CLI, как найти причины проблем с производительностью запросов, и как с помощью YDB UI оценить состояние кластера.
