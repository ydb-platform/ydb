...
Доклад посвящен архитектуре аналитического движка {{ ydb-short-name }}, который позволяет выполнять сложные OLAP-запросы поверх транзакционной (OLTP) базы данных без влияния на основную нагрузку. Спикер {{ team.denisov.name }} детально разбирает, как это достигается.
