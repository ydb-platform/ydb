...
- Minikube
   ...
   5. Примените манифест для создания кластера {{ ydb-short-name }}:
      ...
      Сначала проверьте, что API-сервер может вызвать admission webhook контроллера и принять манифест, не создавая ресурс:

      ```bash
      kubectl apply --dry-run=server -f samples/minikube/storage.yaml
      ```

      Даже после завершения `rollout status` webhook может быть ещё недоступен. Если проверка сообщает `connection refused` или `no endpoints available` при обращении к webhook, подождите несколько секунд и повторите её. Другие ошибки требуют устранения причины. Продолжайте только после успешной проверки:
   ...
   9. После обработки манифеста создаётся объект StatefulSet, описывающий набор динамических узлов. Для подключения из того же пространства имён Kubernetes используйте отдельные сервисы:

       - `database-minikube-sample-grpc:2135` (gRPC);
       - `database-minikube-sample-status:8765` ({{ ydb-ui-name }}).

       Путь базы данных: `/Root/database-minikube-sample`.
...
- Kind
   ...
   10. После обработки манифеста создаётся объект StatefulSet, описывающий набор динамических узлов. Для подключения из того же пространства имён Kubernetes используйте отдельные сервисы:

       - `database-kind-sample-grpc:2135` (gRPC);
       - `database-kind-sample-status:8765` ({{ ydb-ui-name }}).

       Путь базы данных: `/Root/database-kind-sample`.
...
Самый простой способ выполнить свой первый запрос к {{ ydb-short-name }} — это [{{ ydb-ui-name }}](reference/ydb-ui/index.md). Он запускается по умолчанию на порту 8765 сервера {{ ydb-short-name }}, поэтому, если вы запустили его локально, вам нужно открыть [localhost:8765](http://localhost:8765) в вашем веб-браузере. Если нет, замените `localhost` на имя хоста вашего сервера в этом URL, либо используйте `ssh -L 8765:localhost:8765 my-server-hostname-or-ip.example.com` для настройки проброса порта и все равно откройте [localhost:8765](http://localhost:8765). Вы увидите страницу подобного вида:

![Стартовая страница веб-интерфейса](_assets/web-ui-home.png)

{{ ydb-short-name }} спроектирован как многопользовательская система с возможностью одновременной работы тысяч пользователей с одним кластером. Как следствие, большинство логических сущностей внутри кластера {{ ydb-short-name }} находятся в гибкой иерархической структуре, больше похожей на виртуальную файловую систему Unix, чем на схему с фиксированной глубиной, с которой вы, возможно, знакомы из других систем управления базами данных. Как видите, первый уровень иерархии состоит из баз данных, работающих в одном процессе {{ ydb-short-name }}, которые могут принадлежать разным пользователям. `/Root` предназначена для системных целей. Для выполнения запросов выберите базу данных, созданную на предыдущем шаге:

| Способ установки | База данных |
| --- | --- |
...
Нажмите на соответствующую базу данных, затем введите первый запрос и нажмите кнопку запуска:
