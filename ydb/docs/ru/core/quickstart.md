...
Основная цель существования систем управления базами данных - сохранение данных для последующего извлечения. Как система, базирующаяся на SQL, основной абстракцией {{ ydb-short-name }} для хранения данных является таблица. {{ ydb-short-name }} поддерживает два типа таблиц:
...
Функциональность колоночных таблиц сейчас находится в разработке. На данный момент колоночные таблицы можно создавать, удалять и загружать в них данные несколькими путями.

{% include [ways_to_add_data](./_includes/ways_add_data_to_olap.md) %}

{% endnote %}

Чтобы создать первую таблицу, выполните один из следующих запросов:

{% list tabs %}

- Создать строковую таблицы
   ...
   Опции создания строковой таблицы:
...
- Создать колоночную таблицу
   ...
   Ключевым отличием в создании колоночной таблицы от строковой таблицы является использование блока [дополнительных параметров](./yql/reference/syntax/create_table/with.md) `WITH`, в котором задаётся параметр `STORE = COLUMN`.
