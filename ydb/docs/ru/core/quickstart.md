...
- Docker x86_64
   ...
   2. Запустите Docker-контейнер:
      ...
      Чтобы отключить сохранение данных на диск и сделать все данные волатильными, хранящимися только в оперативной памяти, добавьте в команду запуска Docker-контейнера параметр `-e YDB_USE_IN_MEMORY_PDISKS=true`. Подробнее о [настройке Docker контейнера](./reference/docker/configuration.md).
