...
1. Определите идентификатор системной таблетки, которую требуется восстановить. Идентификатор таблетки можно найти в разделе Tablets в [{{ ydb-ui-name }}](../../reference/embedded-ui/index.md).
...
2. Выберите наиболее актуальную резервную копию, пригодную для восстановления.
   ...
   - Произошло повреждение данных, восстановление невозможно.


### Шаг 3. Перенесите файлы резервной копии {#transfer-backup-files}

1. Определите, на каком хосте запущена таблетка в Recovery-режиме. Для этого откройте [{{ ydb-ui-name }}](../../reference/embedded-ui/index.md) и найдите хост, на котором работает таблетка.
...
3. Убедитесь, что:
    - С таблеткой нет проблем в [HealthCheck](../../reference/ydb-sdk/health-check-api.md).
    - Таблетка не перезапускается.
    - В App таблетки в [{{ ydb-ui-name }}](../../reference/embedded-ui/index.md) отсутствует форма восстановления.

4. Перезапустите все узлы в кластере для синхронизации состояния внутренних кешей в памяти с состоянием таблетки.
