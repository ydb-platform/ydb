...
[YDB CLI](../../reference/ydb-cli/export-import/import-file.md) загружает данные из [CSV](../../reference/ydb-cli/export-import/import-file.md) и [Parquet](../../reference/ydb-cli/export-import/import-file.md) командой `ydb import file`. [Spark](../../integrations/query-engines/spark.md) также читает CSV и Parquet напрямую.
...
Для CSV можно получить черновик DDL по файлу: [ydb tools infer csv](../../reference/ydb-cli/tools-infer.md).

[ydb-importer](../../integrations/data-migration/import-jdbc.md) **не читает** промежуточные CSV/Parquet — он подключается к источнику по JDBC и параллельно переносит данные (см. столбец ydb-importer в таблице). Если промежуточный файл не нужен, ydb-importer часто удобнее, чем выгрузка в Parquet и обратная загрузка.

Parquet-файлы, уже лежащие в **S3**, можно загрузить без YDB CLI и Spark — через [федеративные запросы](from/parquet/federated-queries-s3.md) (`UPSERT INTO … SELECT … FROM external_table`).
...
* [YDB CLI](../../reference/ydb-cli/export-import/import-file.md) — `import file` для CSV и Parquet;
...
Для массовой загрузки **колоночных** таблиц из Parquet на S3 см. [импорт Parquet из S3 через федеративные запросы](from/parquet/federated-queries-s3.md) и [импорт и экспорт колоночных таблиц](../import-export-column-tables.md#objstorage).
