...
* `sku_id` — числовой идентификатор товара. Первичный ключ JSON-индекса должен состоять из одной колонки типа `Uint64`.
...
В индекс попадает токен пути `$.warehouses`, а условие `@.stock > 0` проверяется пост-фильтром на каждой найденной записи. Это эффективно, когда поле `warehouses` присутствует у сравнительно малой части документов.
