...
Эта статья поможет быстро начать работу с [стриминговыми запросами](../concepts/streaming_query/index.md) в {{ ydb-short-name }} на простейшем модельном примере.  Мы будем читать из входного топика сообщения в формате json, фильтровать их, агрегировать и результат записывать в выходной топик.
