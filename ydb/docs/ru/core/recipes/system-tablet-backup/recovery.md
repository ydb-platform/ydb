...
1. Определите идентификатор системной таблетки, которую требуется восстановить. Идентификатор таблетки можно найти в разделе Tablets в [YDB UI](../../reference/ydb-ui/index.md).
...
3. Измените конфигурацию, добавив `boot_mode: RECOVERY` в секцию `bootstrap_config` восстанавливаемой таблетки.
    - При использовании конфигурации V1, необходимо изменить [статическую конфигурацию](../../devops/configuration-management/configuration-v1/static-config.md) на всех узлах, на которых может работать восстанавливаемая таблетка.
...
5. Убедитесь, что:
    ...
    - В App таблетки в [YDB UI](../../reference/ydb-ui/index.md) доступна форма восстановления.
...
1. Определите, на каком хосте запущена таблетка в Recovery-режиме. Для этого откройте [YDB UI](../../reference/ydb-ui/index.md) и найдите узел, на котором работает таблетка.
...
1. Откройте App восстанавливаемой таблетки в [YDB UI](../../reference/ydb-ui/index.md).
...
4. Убедитесь, что:
    - С таблеткой нет проблем в [HealthCheck](../../reference/ydb-sdk/health-check-api.md).
    - Таблетка не перезапускается.
    - В App таблетки в [YDB UI](../../reference/ydb-ui/index.md) отсутствует форма восстановления.
