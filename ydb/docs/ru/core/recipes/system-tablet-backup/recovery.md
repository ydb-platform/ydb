...
1. Определите, на каком хосте запущена таблетка в Recovery-режиме. Для этого откройте [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md) и найдите узел, на котором работает таблетка.
...
1. Откройте App восстанавливаемой таблетки в [{{ ydb-ui-name }}](../../reference/ydb-ui/index.md).
