...
Данная команда создаёт индекс типа `vector_kmeans_tree`. В данном модельном примере указан параметр `distance=cosine`, который указывает что мы будем использовать косинусное расстояние. Это очень важный параметр, расстояние должно быть то, на которое училась модель. Так же есть параметры clusters, levels, overlap_clusters, они определяются автоматически, но если у вас есть большой объем данных и желание их потюнить то здесь есть более детальное описание этих параметров {% include [vector_index_parameters.md](../../yql/reference/syntax/_includes/vector_index_parameters.md) %}.
