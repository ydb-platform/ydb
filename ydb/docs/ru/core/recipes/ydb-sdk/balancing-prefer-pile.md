...
- Java
  ...
  - JDBC
    ...
    В Spring Boot, ORM и прочих сторонних фреймворках вокруг JDBC подключение к {{ ydb-short-name }} задаётся JDBC-строкой подключения; при появлении поддержки политики для пайла настройки будут задаваться через те же параметры [драйвера](../../reference/languages-and-apis/jdbc-driver/properties.md), что и при прямом JDBC (например, `spring.datasource.url`).
