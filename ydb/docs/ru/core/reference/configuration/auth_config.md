...
Значение по умолчанию: `1h`
    ||
|| life_time
| Максимальный период неактивности токена пользователя. Время последнего использования записи обновляет только запрос, совпадающий по ключу кеша (аутентификационному токену и контексту проверки). Если токен пользователя не использовался в течение `life_time`, узел удаляет его из кеша при следующей обработке этой записи.
...
Если срок действия результата истёк, узел удаляет токен пользователя, когда наступает время обработки этой записи в очереди. Для обновляемых токенов записи обычно планируются в соответствии с `refresh_time`, а `refresh_period` задаёт частоту обработки уже готовых записей. Поэтому уменьшение только `expire_time` не отключает кеш и напрямую не определяет частоту удаления или обновления.
