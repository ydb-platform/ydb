...
Для сокращения количества запросов по проверке аутентификационных токенов, каждый узел {{ ydb-short-name }} кеширует результаты аутентификации в **токенах пользователей**. Подробнее о процессе кеширования см. в статье [{#T}](../../security/caching-authentication-results.md).
