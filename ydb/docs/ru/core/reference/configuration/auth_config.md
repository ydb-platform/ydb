...
{{ ydb-short-name }} позволяет настраивать тип аутентификации узлов баз данных при их регистрации в кластере. Настройка это типа осуществляется через параметр `node_registration_token` секции `auth_config`.
