...
| Ожидаемое значение утверждения `iss` (issuer), которое идентифицирует издателя JWT-токена, и базовый URL для OIDC Discovery. Обязательный параметр; должен начинаться с `https://` и не должен заканчиваться символом `/`. Значения `issuer` в Discovery-документе и `iss` в JWT должны в точности совпадать с указанным значением.

Значение по умолчанию: пустая строка
    ||
|| external_idp_config.audience
| Ожидаемое значение утверждения `aud` (audience), которое идентифицирует получателя JWT-токена. Если параметр не задан, получатель токена не проверяется.

Значение по умолчанию: пустая строка
    ||
|| external_idp_config.allowed_clock_skew
| Допустимое расхождение часов при проверке утверждений JWT, связанных со временем: `exp` задаёт время истечения срока действия токена, `nbf` — время начала его действия, `iat` — время выпуска.
