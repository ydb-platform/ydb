...
- "root@builtin" — используется режим аутентификации узлов через токен `root@builtin`. Этот токен является особым, и он всегда соответствует идентификатору защиты [SID](../../concepts/glossary.md#access-sid) `root@builtin`, который прописывается в параметре `register_dynamic_node_allowed_sids` секции [Настройки административных и других привилегий](security_config.md#security-access-levels).

{% note warning %}

Для обеспечения безопасности кластера рекомендуется использовать режим аутентификации узлов через сертификаты TLS (установить значение параметра в пустую строку) вместо использования токена.

{% endnote %}

Значение по умолчанию: не задано (используется режим аутентификации через токен субъекта доступа `root@builtin`)
||
|#
