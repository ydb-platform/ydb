...
{{ ydb-short-name }} позволяет настроить аутентификацию узлов баз данных при их регистрации в кластере. Для настройки аутентификации узлов необходимо указать параметр `node_registration_token` в секции `auth_config`.

#|
|| Параметр | Описание ||
|| node_registration_token
| Токен, используемый узлами баз данных для регистрации в кластере {{ ydb-short-name }}.
...
- Непустая строка — используется режим аутентификации узлов через токен. Узлы баз данных должны предоставить указанный токен при регистрации в кластере.
...
Значение по умолчанию: не задано (используется режим аутентификации через токен, если токен не указан)
||
|#
