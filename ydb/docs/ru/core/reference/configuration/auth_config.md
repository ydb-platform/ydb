...
| Ожидаемое значение поля `iss` (issuer), которое идентифицирует издателя JWT-токена, и базовый URL для OIDC Discovery. Обязательный параметр; должен начинаться с `https://` и не должен заканчиваться символом `/`. Значения `issuer` в Discovery-документе и `iss` в JWT должны в точности совпадать с указанным значением.

Значение по умолчанию: пустая строка
    ||
|| external_idp_config.audience
| Ожидаемое значение поля `aud` (audience), которое идентифицирует получателя JWT-токена. Если параметр не задан, получатель токена не проверяется.

Значение по умолчанию: пустая строка
    ||
|| external_idp_config.allowed_clock_skew
| Допустимое расхождение часов при проверке полей JWT, связанных со временем: `exp` задаёт время истечения срока действия токена, `nbf` — время начала его действия, `iat` — время выпуска.
...
Сторонний IAM-провайдер и внешний IdP по протоколу OIDC используют токены типа `Bearer`. Если параметр `use_access_service` включён, IAM-провайдер имеет приоритет и перехватывает все такие токены. Поэтому одновременное использование аутентификации через IAM-провайдер и внешний IdP по протоколу OIDC не поддерживается.
...
Для сокращения количества [проверок аутентификационных токенов](../../security/authentication.md#token-validation) каждый узел {{ ydb-short-name }} кеширует результаты проверки в [токенах пользователей](../../concepts/glossary.md#user-token). Подробнее см. в статье [{#T}](../../security/caching-authentication-results.md).
...
Значение по умолчанию: `1s`
    ||
|| refresh_time
| Максимальный интервал между успешным обновлением токена пользователя и следующей попыткой обновления. Конкретное время обновления выбирается в диапазоне от `refresh_time/2` до `refresh_time`.

Например, после первого запроса с корректным аутентификационным токеном узел создаёт токен пользователя. Через интервал, выбранный случайно в диапазоне от `refresh_time/2` до `refresh_time`, узел снова проверяет аутентификационный токен. После успешной проверки цикл повторяется. При ретрабельной ошибке узел повторяет проверку с учётом параметров `min_error_refresh_time` и `max_error_refresh_time`, а при постоянной ошибке перестаёт использовать ранее созданный токен пользователя.

Параметр применяется к обновляемым аутентификационным токенам, например к токенам входа по логину и паролю и токенам внешнего провайдера идентификации.
...
Значение по умолчанию: `1h`
    ||
|| expire_time
| Период действия результата успешной проверки для большинства типов аутентификационных токенов. После успешного обновления отсчёт начинается заново. По истечении периода запись удаляется из кеша независимо от `life_time`.

Для токенов входа по логину и паролю и токенов внешнего провайдера идентификации используется срок действия самого аутентификационного токена. Для запросов с подписью ключом доступа применяется отдельный параметр `as_signature_expire_time`.
...
После ретрабельной ошибки первая повторная проверка выполняется немедленно. Если она также завершается ретрабельной ошибкой, задержка перед следующей проверкой выбирается случайным образом в диапазоне от `min_error_refresh_time/2` до `min_error_refresh_time`. После каждой следующей ретрабельной ошибки текущий интервал удваивается, но не превышает разность `max_error_refresh_time - min_error_refresh_time`. Для каждого текущего интервала `D` фактическая задержка выбирается случайным образом в диапазоне от `D/2` до `D`.
...
| Ограничивает увеличение интервала между повторными проверками после ретрабельных ошибок обновления токена пользователя. Не ограничивает общую продолжительность повторных проверок.
