...
- `true` — внутренние пользователи {{ ydb-short-name }} существуют на уровне кластера (добавляются в [корневую базу данных](../../concepts/glossary.md#root-database));
- `false` — внутренние пользователи могут существовать как на уровне кластера, так и на уровне каждой отдельной базы данных (могут добавляться в [тенантные базы данных](../../concepts/glossary.md#tenant-database)).
...
Если указано значение, эквивалентное `"0s"`, пользователь будет заблокирован на неограниченное время.
