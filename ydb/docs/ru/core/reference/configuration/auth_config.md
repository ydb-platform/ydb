...
Например, после первого запроса с корректным аутентификационным токеном узел создаёт токен пользователя. Через интервал от `refresh_time/2` до `refresh_time` узел снова проверяет аутентификационный токен. После успешной проверки цикл повторяется. При временно́й ошибке узел повторяет проверку согласно `min_error_refresh_time` и `max_error_refresh_time`, а при постоянной ошибке перестаёт использовать ранее созданный токен пользователя.
...
Перед первой повторной проверкой задержка выбирается от `min_error_refresh_time/2` до `min_error_refresh_time`. После каждой ретрабельной ошибки текущий интервал удваивается. Его максимальное значение равно `max_error_refresh_time`. Для каждого интервала `D` фактическая задержка выбирается от `D/2` до `D`.
