...
Значение по умолчанию: `true`
    ||
|| domain_login_only
| Флаг определяет границы привилегий внутренних пользователей в кластере {{ ydb-short-name }}.

Возможные значения:

- `true` — внутренние пользователи {{ ydb-short-name }} существуют на уровне кластера и им могут назначаться привилегии на доступ к множеству тенантных баз данных.

    В этом случае пользователи добавляются в [корневую базу данных](../../concepts/glossary.md#root-database).

- `false` — внутренние пользователи могут существовать как на уровне кластера, так и на уровне каждой отдельной базы данных. Границы привилегий внутренних пользователей тенантных баз данных ограничиваются базой данных, в которой они созданы.

    В этом случае пользователи добавляются либо в корневую базу данных, либо в [тенантную базу данных](../../concepts/glossary.md#tenant-database).
