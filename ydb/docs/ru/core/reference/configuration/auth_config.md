...
{{ ydb-short-name }} поддерживает [аутентификацию по клиентскому сертификату](../../security/authentication.md#client-certificate). Правила проверки сертификатов задаются в секции [`client_certificate_authorization`](client_certificate_authorization.md). В секции `auth_config` может указываться только суффикс имён пользователей, аутентифицированных по сертификату:
