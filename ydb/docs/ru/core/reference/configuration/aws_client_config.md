...
| `verify_ssl` | `true` | Проверять TLS-сертификаты при подключении к endpoint AWS (и совместиые API). Отключать стоит только в тестовых средах. |
...
| `tcp_keep_alive_interval_ms` | `30000` | Интервал между keep-alive пакетами в миллисекундах. В Curl значение округляется до секунд; должен быть не меньше 15000. |
| `max_connections_count` | `32` | Максимум одновременных TCP‑соединений, которые может открыть HTTP-клиент в составе AWS SDK. |
| `executor_threads_count` | `32` | Размер пула потоков, которым пользуются клиенты AWS SDK при обращении к одному и тому же endpoint. Задает параллелизм асинхронных запросов. |
