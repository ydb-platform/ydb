...
Ключ | Описание
---- | ---
`request_client_certificate` | Запрос клиентского сертификата при TLS-handshake для gRPCs.<br/>Допустимые значения:<br/><ul><li>`true` — сервер запрашивает клиентский сертификат; если он предъявлен, проверяется на TLS-уровне при [аутентификации устройств](../../security/device-auth.md#interfaces) — соединение с недоверенным сертификатом не устанавливается. Без сертификата соединение устанавливается, но [аутентификация устройств](../../security/device-auth.md#interfaces) и [аутентификация по клиентскому сертификату](../../security/authentication.md#client-certificate) не выполняются. Если сертификат присутствует в TLS-контексте соединения и при этом не передан [аутентификационный токен](../../concepts/glossary.md#auth-token), сервер также использует его для аутентификации по [клиентскому сертификату](../../security/authentication.md#client-certificate).</li><li>`false` — сервер не запрашивает клиентский сертификат при TLS-handshake, соответственно, аутентификация устройств и аутентификация по клиентскому сертификату при такой настройке недоступны.<br/>Значение по умолчанию: `false`.</li></ul>
