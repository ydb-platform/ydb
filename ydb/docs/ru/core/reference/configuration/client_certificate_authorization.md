...
Следующий фрагмент конфигурации требует клиентский сертификат с компонентом `O=YDB` в поле "Subject". Для сертификата с Subject `O=YDB` будет сформирован SID пользователя `O=YDB@cert`, а подключению будет назначена группа `group@cert`:
