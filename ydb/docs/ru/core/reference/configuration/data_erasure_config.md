...
В конфигурации введены параметры `timeout_seconds` и `tenant_data_erasure_config.timeout_seconds`, отвечающие за отведенное время выполнения очистки на уровне кластера и уровне базы. Основное назначение таймаутов очистки это попытаться перезапустить ее, если за отведенное время очистка не успела выполниться или завершилась ошибкой. Перезапуск очистки на уровне кластера бессмысленная операция, поэтому, после истечения времени `timeout_seconds`, новая очистка в базе данных не запускается, но информация о истекшем таймауте отражается в показаниях [метрики](../observability/metrics/index.md#data_erasure). На уровне базы данных очистка в таблетке будет перезапущена через `tenant_data_erasure_config.timeout_seconds`, только если текущая операция очистки в таблетке завершилась ошибкой. Если текущая операции очистки в таблетке просто не уложилась в обозначенный таймаут, то повторного запуска очистки в этой таблетке не производится. 
