...
|| `enable_fast_tablet_move` | — | true/false | Данная опция позволяет не дожидаться остановки предыдущего поколения таблетки при запуске нового. Это позволяет перевозить таблетки между узлами, не создавая периода полной недоступности таблетки. | true ||
|# {wide-content}
...
|| `check_move_expediency` | CheckMoveExpediency | true/false | Проверка целесообразности перемещений таблеток. Отключение этой опции позволяет снизить потребление процессорных ресурсов самим Hive за счёт менее качественной балансировки. | true ||
...
Таким файлом конфигурации можно отключить все виды автобалансировки между узлами для таблеток, участвующих в распределении транзакций, то есть [координаторов](../../concepts/glossary.md#coordinator) и [медиаторов](../../concepts/glossary.md#mediator).
