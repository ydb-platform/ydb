...
С помощью этих опций можно регулировать с какой скоростью [запускаются таблетки](../../contributor/hive.md) и как для них [выбираются узлы](../../contributor/hive.md).
...
|| `max_boot_batch_size` | MaxBootBatchSize | Целое число | Максимальное число таблеток из [очереди запуска](../../contributor/hive.md) Hive, обрабатываемых за раз. | 1000 ||
