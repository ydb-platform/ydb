...
Чтобы домены отказа рассчитывались по уровню `disk_scope`, надо добавить следующую строку в yaml-конфиг:
