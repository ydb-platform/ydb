...
В разделе `http_proxy_config` файла конфигурации {{ ydb-short-name }} включается и конфигурируется AWS совместимых протоколов, которая дает доступ к работе с [{{ ydb-short-name }} Topics](../../concepts/datamodel/topic.md) по [SQS](../../reference/kafka-sqs/index.md).
