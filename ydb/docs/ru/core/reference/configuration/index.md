...
| 4 |
| `attempt_reset_duration` | Время, в течение которого пользователь считается заблокированным., то есть ему запрещено выполнять аутентификацию. Это время начинает отсчитываться после последней неверной попытки ввода пароля. Если введено значение эквивалентное `"0s"`, то пользователь будет считаться заблокированным неограниченное время. | "1h" |
