...
Поля статической схемы и `erasure_species` пулов/каналов также принимают `block-8-2` (идентификатор 19). Нужны минимум двенадцать независимых доменов и поддерживающая сборка на всех процессах. Геометрию статической группы нельзя изменить на месте; динамические пулы могут использовать другую схему. См. [конфигурацию Block82 и метаданные ёмкости](../../concepts/block-8-2.md).
