...
### Конфигурация условий запрета аутентификации {#account-lockout}
...
|Параметр|Описание|Значение по умолчанию|
|:---|:---|:---:|
| `attempt_threshold` | Число попыток ввести правильный пароль до блокирования пользователя. Если пользователь ввел `attempt_threshold` раз подряд неправильный пароль, то ему запрещается аутентифицироваться на время, заданное в параметре `attempt_reset_duration`. Если параметр имеет значение `0`, то число попыток ввода неправильного пароля не ограничено. | 4 |
| `attempt_reset_duration` | Время, в течение которого пользователю запрещено выполнять аутентификацию, пользователь считается заблокированным. Это время начинает отсчитываться после последней попытки ввода пароля, после которой пользователь стал заблокированным. Если введено значение эквивалентное `"0s"`, пустая или некорректная строка, то пользователь будет считаться заблокированным неограниченное время. | "1h" |
