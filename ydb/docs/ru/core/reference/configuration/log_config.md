...
Секция `log_config` управляет тем, как серверный процесс {{ ydb-short-name }} обрабатывает и управляет своими логами. Она позволяет настраивать уровни логирования, форматы и места назначения для различных компонентов.

## Обзор

Логирование является критически важной частью наблюдаемости {{ ydb-short-name }}. Секция `log_config` позволяет настраивать различные аспекты логирования, включая:

- Уровень логирования по умолчанию
- Уровни логирования для конкретных компонентов
- Формат вывода логов
- Ротация и хранение логов
- Интеграция с системными логами или внешними сервисами логирования
...
| `tenant_name` | string | | Имя тенанта для включения в записи логов. |
...
1. При указании имен компонентов в массиве `entry` имена компонентов должны быть закодированы в base64.
2. Уровни логирования указываются в конфигурации как числовые значения, а не строки. Используйте таблицу выше для сопоставления между числовыми значениями и их значениями.
3. Если указан параметр `backend_file_name`, логи записываются в этот файл. Если параметр `sys_log` имеет значение true, логи отправляются в системный регистратор.
4. Параметр `format` определяет, как форматируются записи логов. Формат "full" включает всю доступную информацию, "short" предоставляет более компактный формат, а "json" выводит логи в формате JSON для более простого разбора.