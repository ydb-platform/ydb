...
Значение по умолчанию: `false`.
    ||
|| `enforce_user_token_check_requirement` | Запрещает выполнение запросов с ошибкой аутентификации в режиме опциональной аутентификации (`enforce_user_token_requirement: false`).

В режиме `enforce_user_token_requirement: false` запросы могут не содержать токен. Запросы без токена выполняются в [анонимном режиме](../../security/authentication.md#anonymous) и без проверки прав. Запросы с токеном проходят аутентификацию и проверку прав.

Валидные значения:

- `true`. В случае ошибки аутентификации запроса с токеном, выполнение запроса запрещается.
- `false`. В случае ошибки аутентификации запроса с токеном, запрос выполняется в анонимном режиме.
...
#|
|| Параметр | Описание ||
|| `default_users` | Список пар логин-пароль [пользователей](../../concepts/glossary.md#access-user), которые должны быть созданы на кластере при первом запуске.

{% note warning %}

Первый пользователь в этом списке становится суперпользователем.
...
Оба механизма применяются одновременно: запрашиваемая операция для конкретного [субъекта](../../concepts/glossary.md#access-subject) доступна, если оба механизма его разрешают, и не доступна, если хотя бы один не разрешает.
...
{% cut "Детализация прав" %}

- доступ к [встроенному UI](../embedded-ui/index.md), без возможности делать изменения.
...
- рестарт или старт/стоп таблеток на странице таблетки;
- перемещение vdisk'ов на другие pdisk'и (evict) на странице vdisk;
- рестарт или декомиссия диска на странице pdisk;
- действия на закладке с информацией по операциям;
- действия на закладке с информацией по запросам.
...
- рестарт pdisk без проверки на деградацию групп;
- изменение статуса pdisk без проверки на деградацию групп;
- перемещение vdisk'ов на другие pdisk'и (evict) без проверки на деградацию групп;
- управление базами данных (тенантами) в [CMS](../../concepts/glossary.md#cms);
- изменение динамической конфигурации (кластера или базы);
- управление пулами ресурсов;
- управление metering config;
- изменение настроек таблетки Console;
- запросы к сервису maintenance (list cluster nodes, манипуляции maintenance-задачами);
- некоторые запросы к сервису ClickHouse_internal;
- прямой ALTER TABLE на индексную таблицу без ограничений консистентности с основной таблицей;
- прямой split-merge партиций таблицы;
- прямое выполнение схемной операции по protobuf spec через CLI;
- прямое выполнение локальной транзакции по minikql через CLI;
- прямое выполнение drain/fill ноды через CLI.
...
Рекомендуется включать в списки `*_allowed_sids` группы или отдельные сервисные аккаунты, тогда наделение индивидуальных пользователей соответствующими возможностями не будет требовать изменения общей конфигурации кластера.
...
- администратор должен состоять во `viewer_allowed_sids`, `monitoring_allowed_sids` и в `administration_allowed_sids`.
    ...
    Кроме списков разрешений, администратор должен присутствовать в списках прав доступа корня схемы кластера для доступа ко всем схемным объектам.
