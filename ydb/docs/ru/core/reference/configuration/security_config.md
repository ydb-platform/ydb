...
Даёт возможность просматривать состояние кластера, недоступное публично (включая большую часть страниц [Embedded UI](../embedded-ui/ydb-monitoring/index.md)), без возможности вносить изменения.
    ||
...
- Субъект доступа, не включённый ни в один список уровней доступа, может просматривать только публично доступную информацию о кластере (например, [список баз данных в кластере](../embedded-ui/ydb-monitoring/tab-databases.md#database_list) или [список узлов кластера](../embedded-ui/ydb-monitoring/tab-nodes.md#nodes_list)).
