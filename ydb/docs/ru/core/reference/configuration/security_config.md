...
Более высокий уровень автоматически включает все более низкие, поэтому субъекту достаточно присутствовать только в одном списке.

Списки `bootstrap_allowed_sids` и `register_dynamic_node_allowed_sids` являются отдельными и применяются только к начальной инициализации кластера и регистрации узлов соответственно.

Для любого списка доступ разрешён, если список **пустой** или если токен субъекта содержит хотя бы один SID из этого списка.
...
Даёт возможность просматривать состояние кластера, недоступное публично (включая большую часть страниц [Embedded UI](../embedded-ui/ydb-monitoring.md)), без возможности вносить изменения.
    ||
|| `monitoring_allowed_sids` | Список [SID](../../concepts/glossary.md#access-sid)'ов с уровнем доступа operator.
...
Даёт дополнительные привилегии для мониторинга и изменения состояния кластера. Например, позволяет выполнять резервное копирование, восстановление базы данных или запускать YQL-запросы через Embedded UI.
    ||
|| `administration_allowed_sids` | Список [SID](../../concepts/glossary.md#access-sid)'ов с уровнем доступа administrator.
...
- оператора следует добавлять только в `monitoring_allowed_sids` (он автоматически получает viewer-привилегии);
