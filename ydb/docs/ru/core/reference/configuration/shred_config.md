...
Процесс очистки запускается для всех баз кластера и его можно условно разделить на 2 этапа:

* очистка на уровне внутренних структур баз данных;
* очистка в [слое хранения](../../concepts/glossary.md#distributed-storage-implementation).
...
Параметр `data_erasure_config.inflight_limit` регулирует максимальное число одновременно очищаемых баз.
...
Значение по умолчанию `604800` секунд.
| Секунды
||
...
Другим способом отключения автоматических запусков очистки является выставление параметра `data_erasure_interval_seconds` в значение `0`.
