...
**Описание:** Файловая директория для сохранения файлов спиллинга. Если значение не задано, используется системная временная директория (обычно `/tmp`). Директория должна существовать заранее, {{ ydb-short-name }} её не создаёт — см. [Требования к директории root](#root-requirements).
...
Имя пользователя стоит последним, потому что может содержать `-`: остальные части имени фиксированного вида, поэтому имя пользователя однозначно определяется как весь остаток.
...
Если спиллинг включён, а директория `root` отсутствует или недоступна для записи, узел базы данных не запустится: Spilling Service выполнит 2 повторные попытки с интервалом 1 секунда, после чего процесс `ydbd` аварийно завершится. Убедитесь, что `root` существует и доступен на всех узлах базы данных до включения спиллинга.

{% endnote %}

##### Очистка директорий при старте {#cleanup}

Каждый запуск процесса пишет в свою директорию с новым `spilling_service_id`, а файлы спиллинга нужны только во время работы процесса, поэтому директории прошлых запусков удаляются. Очистка выполняется при старте Spilling Service, асинхронно и только внутри `root`.
...
- любые посторонние файлы и директории внутри `root`.

{% note info %}

В предыдущих версиях {{ ydb-short-name }} директории назывались `node_<node_id>_<spilling_service_id>` и создавались либо внутри `<root>/spilling-tmp-<username>/` (если `root` не задан), либо непосредственно в `root` (если `root` задан). Такие директории этого узла тоже удаляются при старте, в обоих расположениях. Общая директория `spilling-tmp-<username>` и директории `node_<other_node_id>_*` других узлов сохраняются: во время обновления кластера соседние узлы ещё могут писать туда в старом формате.
