...
- `spilling_service_id` — уникальный идентификатор экземпляра, который создаётся при инициализации [Spilling Service](../../contributor/spilling-service.md) один раз при запуске процесса ydbd
...
- `32860791-037c-42b4-b201-82a0a337ac80` — идентификатор экземпляра Spilling Service
...
Удаляются директории вида `spilling-tmp-<node_id>-<username>-*`, где `node_id` и `username` совпадают с текущим процессом, кроме директории текущего запуска. Так после аварийного завершения процесса (например, `SIGKILL` или отключение питания) место освобождается при следующем старте узла.
