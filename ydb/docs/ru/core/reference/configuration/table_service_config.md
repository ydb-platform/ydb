...
- Директория должна иметь достаточные права на запись и чтение для пользователя, под которым запускается `ydbd`. Если [статические](../../concepts/glossary.md#static-node) и [динамические](../../concepts/glossary.md#dynamic) узлы запускаются под разными пользователями ОС и используют один и тот же `root`, у обоих пользователей должны быть права на чтение и запись этой директории (например, у категории `other` права `rwx`, режим `1777`).
