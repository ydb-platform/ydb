...
В подсекции `resource_manager` также задаются лимиты памяти, которые Resource Manager использует при выполнении запросов на узле базы данных. Эти параметры определяют размер пула памяти запросов, начальные лимиты MKQL-программ и порог, при котором спиллинг считается предпочтительным.
...
**Описание:** Максимальный объём оперативной памяти на [узле](../../concepts/glossary.md#node), который Resource Manager может выделить под выполнение запросов (пул памяти запросов). По мере роста потребления задачи запрашивают дополнительную память из этого пула.

#### resource_manager.mkql_light_program_memory_limit

**Тип:** `uint64`  
**По умолчанию:** `1048576` (1 MiB)  
**Описание:** Начальный лимит памяти MKQL-аллокатора для «лёгких» вычислительных задач — задач без map join в программе.

#### resource_manager.mkql_heavy_program_memory_limit

**Тип:** `uint64`  
**По умолчанию:** `31457280` (30 MiB)  
**Описание:** Начальный лимит памяти MKQL-аллокатора для «тяжёлых» вычислительных задач — задач, в программе которых присутствует map join. При нехватке памяти лимит может наращиваться за счёт пула `query_memory_limit`, пока не будет достигнут порог `spilling_percent`.
...
| `mkql_light_program_memory_limit` / `mkql_heavy_program_memory_limit` | Задача | Начальный лимит MKQL-аллокатора |
...
Параметры `mkql_*_program_memory_limit` определяют, с какого объёма стартует каждая задача. `spilling_percent` определяет, когда общий пул запросов на узле настолько заполнен, что дальнейший рост в RAM должен уступить место спиллингу.
...
Непосредственный рантайм-порог, при котором вычислительные операции переключаются на спиллинг, задаётся параметром [`resource_manager.spilling_percent`](#spilling-percent). Он определяет, при каком заполнении пула `query_memory_limit` промежуточные данные запроса начинают выгружаться на диск. Подробнее о связанных лимитах см. раздел [Лимиты памяти выполнения запросов](#query-execution-memory-limits).
