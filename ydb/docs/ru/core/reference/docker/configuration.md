...
{{ ydb-short-name }} в Docker контейнере настраивается через дополнительные переменные окружения, меняющие поведение {{ ydb-short-name }} по умолчанию. [Старт](start.md) Docker контейнера {{ db-short-name }} осуществляется с помощью команды `docker run`, в которой можно указать перменные окружения, используя ключ `-e` для каждой переменной окружения. Ниже приведен список поддерживаемых переменных окружения.
