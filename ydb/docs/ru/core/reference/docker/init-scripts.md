...
Docker-контейнер {{ ydb-short-name }} поддерживает пользовательские скрипты инициализации, позволяющие автоматизировать настройку базы данных. Эта функциональность создана по образцу паттерна инициализации PostgreSQL/Bitnami.
...
| Расширение | Описание |
|---|---|
| `.sh` | Shell-скрипты. Если скрипт исполняемый, он запускается напрямую. В противном случае он подключается. |
...
|---|---|---|
| `YBD_PREINITSCRIPTS_DIR` | `/preinit.d` | Путь к директории скриптов предварительной инициализации |
| `YBD_INITSCRIPTS_DIR` | `/init.d` | Путь к директории скриптов инициализации |
