...
| `/init.d`      | Скрипты в этой директории выполняются только **один раз** после успешного запуска сервера {{ ydb-short-name }}. Создаётся файл-маркер для предотвращения повторного выполнения при перезапуске контейнера. | Выполняются только один раз после успешного запуска сервера. Например, для создание структуры базы данных (таблиц, индексов) и вставки начальных данных.                                                                          |
