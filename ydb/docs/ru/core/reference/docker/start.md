...
docker_args=(
    ...
    -p 5432:5432                    # открытие внешнего доступа к порту, обеспечивающему PostgreSQL-совместимость (требуется YDB_EXPERIMENTAL_PG=1)
    ...
    -e YDB_EXPERIMENTAL_PG=1        # включает сетевой протокол PostgreSQL на порту 5432 (по умолчанию выключен)
