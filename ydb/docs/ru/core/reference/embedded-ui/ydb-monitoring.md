...
На вкладке отображается список групп хранения и их текущее состояние.
...
* **Group ID** — идентификатор группы хранения. По ссылке можно перейти на [страницу Storage](#storage_page);
...
Страница открывается при переходе по **Group ID** с вкладки [Storage](#storage_list).

Ниже показан пример страницы группы хранения.
...
Ниже представлены ключевые атрибуты группы:
...
* **Used Space** — занятое пространство относительно общей емкости;
...
* **Read Throughput** — текущая пропускная способность чтения;
* **Write Throughput** — текущая пропускная способность записи.
