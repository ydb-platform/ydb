...
* `<database>` — [путь к базе данных, где создан пользователь](../../concepts/connect#database) (Необходим только для механизма `PLAIN`).

Для механизма `SCRAM-SHA-256` поиск пользователя выполняется в базе данных, к которой относится указанный в настройках подключения endpoint.
