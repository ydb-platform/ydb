...
## Ydb Data Source { #ydb-data-source }
...
2. **С использованием строки подключения**:
...
1. **YdbDataSource.OpenConnectionAsync**:
   ...
   Открывает обычное подключение к YDB с параметрами, указанными при создании `YdbDataSource` (cм. раздел [повторные попытки](#ydb-data-source)).
...
2. **YdbDataSource.OpenRetryableConnectionAsync**:
   ...
   Открывает подключение с автоматическими повторами операций (retry), учитывающее YDB Retry Policy (см. раздел [повторные попытки](#ydb_retry_policy)).
   ...
   Особенности режима:
    - Интерактивные транзакции не поддерживаются.
    - Команды (`YdbCommand`), созданные из такого подключения, автоматически повторяют одиночные операции при временных ошибках.
    - Попытка использовать транзакцию приведет к исключению (см. раздел [транзакции](#transaction)).
...
3. Не рекомендуется: CreateConnection/конструктор:

   Использование `YdbDataSource.CreateConnection` и конструктора `YdbConnection` (legacy API ADO.NET) не рекомендуется.
   Если всё же нужно — откройте подключение вручную:
...
- `YdbDataSource.DisposeAsync()`: Освобождает источник данных. Закрывает связанные с ним пулы и сетевые каналы для его ConnectionString. После вызова экземпляр использовать нельзя. 
...
Повторные попытки выполняются в соответствии с политиками YDB (см. раздел [повторные попытки](#ydb_retry_policy)).
...
Это допустимый и рекомендуемый для длительных чтений. Используйте транзакции чтения в режиме Snapshot (read-only). Такие транзакции дают консистентный срез, не берут блокировки записи и минимизируют конфликты.
...
## Повторные попытки {#ydb_retry_policy}

Повторы (retry) — важная часть дизайна {{ ydb-short-name }}. Провайдер ADO.NET предоставляет гибкую политику повторов с учетом спецификации YDB.
...
Этот раздел актуален, если вы не используете встроенные повторы провайдера (см. раздел [повторные попытки](#ydb_retry_policy.md)).
