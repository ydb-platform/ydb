...
- Вы можете читать значения и как другие .NET‑типы, используя `YdbDataReader.GetFieldValue<T>()`, а также `GetInt32()` / `GetInt64()` для получения «сырого» числового значения.
...
3. Если ничего из вышеперечисленного не задано, тип серверной части будет определен на основе типа значения CLR.
...
По умолчанию используется тип Decimal(22, 9). Если нужно задать другие значения Precision и Scale, это можно сделать с
помощью кода.
