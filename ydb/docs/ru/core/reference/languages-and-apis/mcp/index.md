...
2. MCP-сервер {{ ydb-short-name }} — это приложение на Python, которое обычно размещается вместе с MCP-клиентом. Существует несколько вариантов установки и запуска MCP-сервера {{ ydb-short-name }}, которые описаны ниже, но все они требуют предварительно установленного окружения Python 3.10+.
...
| `--ydb-auth-mode`       | `YDB_AUTH_MODE`      | `anonymous`              | Режим аутентификации {{ ydb-short-name }}; варианты: `anonymous`, `login-password`                |
| `--log-level`           | —                    | `INFO`                   | Уровень логирования; варианты: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`                    |
