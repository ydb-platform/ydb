...
`DiskTimeAvailable` и стоимость запроса являются оценками доступной и потребленной пропускной способности, соответственно, а не реально измеренным времем, поэтому измеряются обе эти величины в условных единицах, которые по своему физическому смыслу близки к наносекундам.
