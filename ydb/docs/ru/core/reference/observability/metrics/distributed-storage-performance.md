...
Планировщик PDisk управляет порядком выполнения запросов PDisk'ом от его клиентов-VDisk'ов. PDisk честно делит время устройства между своими VDisk'ами, то есть каждому из $n$ VDisk'ов гарантируется $1/n$ секунд работы физического устройства каждую секунду. На основе информации о количестве VDisk'ов-соседей для каждого VDisk'а, которое мы обозначим как $N$, и настраиваемого параметра `DiskTimeAvailableScale` строится оценка доступного время диска, или `DiskTimeAvailable`, по формуле
$$
...
|-----------------------|-------------------|---------------------------------------------------------------------------------------------------|
| `DiskTimeAvailable`   | условные единицы  | Доступное время диска.                                                                            |
| `UserDiskCost`        | условные единицы  | Суммарная стоимость запросов, полученных VDisk'ом из DS Proxy.                                    |
| `InternalDiskCost`    | условные единицы  | Суммарная стоимость запросов, полученных VDisk'ом от другого VDisk'а группы, например, в рамках процесса репликации. |
| `CompactionDiskCost`  | условные единицы  | Суммарная стоимость запросов, отправленных VDisk'ом в рамках системного процесса компакшена.      |
| `DefragDiskCost`      | условные единицы  | Суммарная стоимость запросов, отправленных VDisk'ом в рамках системного процесса дефрагментации.  |
| `ScrubDiskCost`       | условные единицы  | Суммарная стоимость запросов, отправленных VDisk'ом в рамках системного процесса скраббинга.      |
| `BurstDetector_redMs` | мс                | Время в миллисекундах, на протяжении которого Token Bucket находился в состоянии underflow.       |

`DiskTimeAvailable` и стоимость запроса являются оценками доступной и потребленной пропускной способности, соответственно, а не реально измеренным времем, поэтому измеряются обе эти величины в условных единицах.
