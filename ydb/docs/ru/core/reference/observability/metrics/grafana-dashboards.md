...
* ds — используется для выбора источника данных, подключённого к Grafana;
* local-tx — определяет, какие данные будут отображаться в виджете `{{local-tx}}` avg time.

| Имя | Описание |
|---|---|
| hive+balancer | Процессорное время, потребляемое `HIVE_ACTOR` и `HIVE_BALANCER_ACTOR` — двумя самыми важными акторами Hive. |
| hive self-ping time | Время ответа таблеткой Hive на собственные запросы. Высокие значения указывают на сильную загрузку (и медленную отзывчивость) Hive. |
| hive local transaction times | Время работы CPU, потребляемое для выполнения различных типов локальных транзакций в Hive. Отображает структуру нагрузки на Hive. |
| `{{local-tx}}` avg time | Среднее время выполнения одной локальной транзакции типа, выбранного в фильтре local-tx. |
