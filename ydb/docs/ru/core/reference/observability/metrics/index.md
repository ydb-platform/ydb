...
## Метрики реплик {#followers}

Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
`FollowerSyncLatency`<br/>`HIST_RATE`, микросекунды | Задержка применения обновлений на фолловерах относительно их коммита на лидере. <br/>Метки:<br/>- _type_ – тип таблетки, единственное значение – `DataShard`.
