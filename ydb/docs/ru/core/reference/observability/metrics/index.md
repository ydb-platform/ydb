...
Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
`SchemeShard.DataErasureTotal`<br/>`GAUGE`, штуки | Число баз данных, в которых происходит очистка.<br/>Метки:<br/>- _waiting_ - число баз данных , которые ожидают запуска очистки.<br/>- _running_ - число баз данных,в которых, на данный момент, запущена очистка.<br/> - _completed_ - число баз данных, в которых очистка завершена.
`SchemeShard.TenantDataErasureTotal`<br/>`GAUGE`, штуки | Число таблеток в конкретной базе данных, в которой происходит очитка.<br/>Метки:<br/>- _waiting_ - Число таблеток, которые ожидают запуска очистки.<br/>- _running_ - число таблеток, в которых запущена очистка в данный момент.<br/>- _restarting_ - число таблеток, очистка в которых завершилась ошибкой и очистка в них перезапущена.<br/>- _completed_ - число таблеток, очистка в которых завершена.
