...
`topic.read.lag_messages`<br/>`RATE`, штуки | Суммарное по топику количество невычитанных данным читателем сообщений. Метрика служит индикатором отставания читателя. Рост значения означает, что читатель не успевает за потоком сообщений — например, из-за остановки читателя, ребалансировки партиций или всплеска нагрузки на запись.<br/>Метки:<br/>- _topic_ – название топика.<br/>- _consumer_ – имя читателя.
