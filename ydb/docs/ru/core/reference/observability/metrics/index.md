...
`SchemeShard.TenantShredTotal`<br/>`GAUGE`, штуки | Число таблеток в конкретной базе данных, в которой происходит очистка.<br/>Метки:<br/>- _waiting_ - Число таблеток, которые ожидают запуска очистки.<br/>- _running_ - число таблеток, в которых запущена очистка в данный момент.<br/>- _restarting_ - число таблеток, очистка в которых завершилась ошибкой и очистка в них перезапущена.<br/>- _completed_ - число таблеток, очистка в которых завершена.
