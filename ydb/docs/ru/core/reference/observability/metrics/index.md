...
Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
`table.datashard.row_count`<br/>`GAUGE`, штуки | Количество строк в строковых таблицах базы данных.
`table.datashard.size_bytes`<br/>`GAUGE`, байты | Размер данных в строковых таблицах базы.
...
`table.datashard.erase.rows`<br/>`RATE`, штуки |  Количество строк, которые удалены в базе данных из строковых таблиц в определенный период времени.
`table.datashard.erase.bytes`<br/>`RATE`, байты | Размер данных, которые удалены в базе из строковых таблиц в определенный период времени.
...
Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
<!-- `table.columnshard.row_count`<br/>`GAUGE`, штуки | Количество строк в таблицах базы данных. -->
<!-- `table.columnshard.size_bytes`<br/>`GAUGE`, байты | Размер данных в таблицах базы. -->
`table.columnshard.used_core_percents`<br/>`HIST_GAUGE`, % | Гистограммный счетчик. Интервалы заданы в процентах. Показывает количество партиций колоночных таблиц, которые используют вычислительные ресурсы в доле, попадающей в определенный интервал.
`table.columnshard.read.rows`<br/>`RATE`, штуки | Количество строк, которые прочитаны всеми партициями всех колоночных таблиц в базе данных в определенный период времени.
`table.columnshard.read.bytes`<br/>`RATE`, байты | Размер данных, которые прочитаны всеми партициями всех колоночных таблиц в базе в определенный период времени.
...
`table.columnshard.write.bytes`<br/>`RATE`, байты | Размер данных, которые записаны всеми партициями всех колоночных таблиц в базе в определенный период времени.
...
`table.columnshard.scan.bytes`<br/>`RATE`, байты | Размер данных, которые прочитаны через вызов gRPC API `StreamExecuteScanQuery` или `StreamReadTable` всеми партициями всех колоночных таблиц в базе в определенный период времени.
...
`table.columnshard.bulk_upsert.bytes`<br/>`RATE`, байты | Размер данных, которые добавлены через вызов gRPC API `BulkUpsert` во все партиции всех колоночных таблиц в базе в определенный период времени.
`table.columnshard.erase.rows`<br/>`RATE`, штуки |  Количество строк колоночных таблиц, которые удалены в базе данных в определенный период времени.
`table.columnshard.erase.bytes`<br/>`RATE`, байты | Размер данных колоночных таблиц, которые удалены в базе в определенный период времени.
<!-- `table.columnshard.cache_hit.bytes`<br/>`RATE`, байты | Общий объем данных, успешно полученных из памяти (кэша). Больший объем данных, полученных из кэша, свидетельствует об эффективном использовании кэша без доступа к распределенному хранилищу. -->
<!-- `table.columnshard.cache_miss.bytes`<br/>`RATE`, байты | Общий объем данных, которые были запрошены, но не найдены в памяти (кэше), и были прочитаны из распределенного хранилища.  Указывает на потенциальные области для оптимизации кэша. -->
