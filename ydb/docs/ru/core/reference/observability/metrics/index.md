...
----- | -----
`resources.storage.used_bytes`<br/>`IGAUGE`, байты  | Размер пользовательских и служебных данных, сохраненных в распределенном сетевом хранилище. Равен `resources.storage.table.used_bytes` + `resources.storage.topic.used_bytes`.
...
`resources.storage.topic.used_bytes`<br/>`IGAUGE`, байты  | Размер распределенного сетевого хранилища, используемого топиками. Равен сумме `topic.storage_bytes`.
