...
В этом документе приведен справочник по основным метрикам системы {{ ydb-short-name }}, которые используются для мониторинга состояния кластера, диагностики производительности и анализа нагрузки.

Как посмотреть текущие значения метрик во [встроенном веб-интерфейсе](../../../devops/observability/monitoring.md#web-metrics) и настроить их сбор в Prometheus и Grafana описано в разделе [Настройка мониторинга кластера {{ ydb-short-name }}](../../../devops/observability/monitoring.md).
...
`api.grpc.response.bytes`<br/>`RATE`, байты | Размер ответов, которые отправлены базой данных в определенный период времени.<br/>Метки:<br/>- _api_service_ – название сервиса gRPC API, например `table`.<br/>- _method_ – название метода сервиса gRPC API, например `ExecuteDataQuery`.
...
`grpc.topic.stream_write.uncommitted_bytes`<br/>`RATE`, байты | Количество байт, записанных методом `Ydb::TopicService::StreamWrite` в рамках еще не закомиченных транзакций.<br/>Метки:<br/>- _topic_ – название топика.
...
`grpc.topic.stream_write.uncommitted_messages`<br/>`RATE`, штуки | Количество сообщений, записанных методом `Ydb::TopicService::StreamWrite` в рамках еще не закомиченных транзакций.<br/>Метки:<br/>- _topic_ – название топика.
...
`api.kafka.response.count`<br/>`RATE`, штуки | Количество ответов по протоколу Kafka в единицу времени.<br/>Метки:<br/>- _method_ – название метода сервиса Kafka API, например `PRODUCE`, `SASL_HANDSHAKE`.<br/>- _error_code_ – код ответа Kafka.
...
`table.query.compilation.cached_query_count`<br/>`IGAUGE`, штуки | Количество запросов в кэше подготовленных запросов.
...
`topic.write.uncommited_bytes`<br/>`RATE`, байты | Размер данных, записанных в рамках еще не завершенных транзакций.<br/>Метки:<br/>- _topic_ — название топика.
...
`topic.write.uncommitted_messages`<br/>`RATE`, штуки | Количество сообщений, записанных в рамках еще не завершенных транзакций.<br/>Метки:<br/>- _topic_ — название топика.
...
`topic.partition.committed_read_lag_milliseconds_max`<br/>`GAUGE`, миллисекунды | Максимальная (по всем партициям) разница между текущим временем и временем записи самого раннего незакомиченного сообщения. Если метрика больше нуля, значит в топике есть как минимум одно сообщение; оно уже записано, возможно прочитано, но ещё не закоммичено.<br/>Метки:<br/>- _topic_ – название топика.<br/>- _consumer_ – имя читателя.
...
`topic.partition.write.throttled_nanoseconds_max`<br/>`GAUGE`, наносекунды | Максимальное время троттлинга записи (ожидания на квоте) по всем партициям. В пределе если `topic.partition.write.throttled_nanoseconds_max` = 10^9, то это означает, что всю секунду ожидали на квоте.<br/>Метки:<br/>- _topic_ – название топика.
...
Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
`kqp.workload_manager.CpuQuotaManager.AverageLoadPercentage`<br/>`RATE`, штуки | Средняя загрузка базы данных, по этой метрике работает `DATABASE_LOAD_CPU_THRESHOLD`.
...
## Метрики процесса очистки данных {#shred}

Имя метрики<br/>Тип, единицы измерения | Описание<br/>Метки
----- | -----
`SchemeShard.RootShredTotal`<br/>`GAUGE`, штуки | Число баз данных, в которых происходит очистка.<br/>Метки:<br/>- _waiting_ — число баз данных, которые ожидают запуска очистки.<br/>- _running_ — число баз данных, в которых на данный момент запущена очистка.<br/>- _completed_ — число баз данных, в которых очистка завершена.
`SchemeShard.TenantShredTotal`<br/>`GAUGE`, штуки | Число таблеток строковых таблиц и топиков в конкретной базе данных, в которой происходит очистка.<br/>Метки:<br/>- _waiting_ — число таблеток, которые ожидают запуска очистки.<br/>- _running_ — число таблеток, в которых запущена очистка в данный момент.<br/>- _completed_ — число таблеток, очистка в которых завершена.
