...
Этот endpoint указывается в AWS CLI через параметр `--endpoint-url` (или `--endpoint`).
...
```shell
aws --endpoint-url "https://ydb-ru-prestable.yandex.net:8443/Root/my_db" \
...
```shell
ENDPOINT="https://ydb-ru-prestable.yandex.net:8443/Root/my_db"

# создать топик (операция SQS CreateQueue)
aws --endpoint-url "$ENDPOINT" sqs create-queue --queue-name "my_topic"

# получить QueueUrl
QUEUE_URL="$(aws --endpoint-url "$ENDPOINT" sqs get-queue-url --queue-name "my_topic" --query 'QueueUrl' --output text)"
