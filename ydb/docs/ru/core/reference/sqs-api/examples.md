...
В SQS-протоколе операция создания вызывается как `CreateQueue`, но в {{ ydb-short-name }} она создаёт [топик](../../concepts/datamodel/topic.md) с указанным именем и [разделяемым (общим) читателем](../../concepts/datamodel/topic.md#shared-consumer) с именем `sqs_consumer`.
...
Для создания FIFO-очереди используйте суффикс `.fifo` в имени очереди и атрибут `FifoQueue=true`:
...
После выполнения этой команды будет создан [топик](../../concepts/datamodel/topic.md) с именем my_topic.fifo и читателем [разделяемым (общим) читателем](../../concepts/datamodel/topic.md#shared-consumer) с именем `sqs_consumer`, у которого будет включено сохранение порядка сообщений.
