...
В SQS-протоколе операция создания вызывается как `CreateQueue`, но в {{ ydb-short-name }} она создаёт [топик](../../concepts/datamodel/topic.md) с указанным именем и [shared-читателем](../../concepts/datamodel/topic.md#shared-consumer) с именем `sqs_consumer`.
