...
Для FIFO-очередей поддержана дедубликация сообщений по `MessageDeduplicationId`, а если `MessageDeduplicationId` не передан — по содержимому сообщения. Включить дедубликацию по содержимому можно командами `CreateQueue` и `SetQueueAttributes`, указав параметр `ContentBasedDeduplication`.

Дедубликация реализована на окне в 5 минут: сообщение с повторяющимся `MessageDeduplicationId` может быть повторно записано через 5 минут или более.

Есть ограничение на количество сообщений, которые можно записать в партицию FIFO-очереди, — 1000 сообщений в секунду. Если требуется записывать в топик больше сообщений, следует увеличить количество партиций. Ограничение на топик вычисляется как 1000 сообщений/сек/партиция × количество партиций. Например, чтобы записывать 10 тыс. сообщений в секунду, создайте топик с 10 партициями.
