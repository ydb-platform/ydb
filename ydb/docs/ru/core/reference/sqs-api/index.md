...
Для FIFO-очередей поддержана дедубликация сообщений по `MessageDeduplicationId`. Если `MessageDeduplicationId` не передан, дедубликация по содержимому возможна только при включённом `ContentBasedDeduplication` (команды `CreateQueue` и `SetQueueAttributes`).
