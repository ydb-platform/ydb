...
Для записи по Amazon SQS-протоколу поддержана дедубликация сообщений по `DeduplicationMessageId`, а если `DeduplicationMessageId` не передан — по содержимому сообщения. Включить дедубликацию по содержимому можно командами `CreateQueue` и `SetQueueAttributes`, указав параметр `ContentBasedDeduplication`.
