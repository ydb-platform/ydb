...
- **Database** (наличие в `database_allowed_sids`) — даёт право работать с {{ ydb-ui-name }} только как «пользователь базы данных»: можно открыть UI и видеть данные в рамках конкретной базы, но нельзя просматривать общекластерные данные или выполнять операции уровня кластера.
- **Viewer** (наличие в `viewer_allowed_sids`) — даёт право просматривать {{ ydb-ui-name }} без возможности вносить изменения.
- **Monitoring** (наличие в `monitoring_allowed_sids`) — даёт право выполнять в {{ ydb-ui-name }} действия, изменяющие состояние системы.
