...
- `-l`, `--access-list` — показать уровни доступа, выданные учетной записи (administration, monitoring, viewer, database, register node, начальная инициализация);
- `-a`, `--all` — показать всю дополнительную информацию (эквивалентно одновременному указанию `-g` и `-l`).

Уровни доступа, отображаемые при использовании опций `-l` или `-a`, соответствуют [иерархической конфигурации управления доступом](../../../configuration/security_config.md#security-access-levels). Выводятся только те уровни доступа, которые действительно выданы пользователю:

- **Database** (наличие в `database_allowed_sids`) — даёт право работать с web UI только как «пользователь базы данных»: можно открыть UI и видеть данные в рамках конкретной базы, но нельзя просматривать общекластерные данные или выполнять операции уровня кластера.
- **Viewer** (наличие в `viewer_allowed_sids`) — даёт право просматривать web UI без возможности вносить изменения.
- **Monitoring** (наличие в `monitoring_allowed_sids`) — даёт право выполнять в web UI действия, изменяющие состояние системы.
