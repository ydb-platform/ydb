...
`--include PATH`: Пути экспортируемых объектов относительно корневой директории. Данный параметр может быть указан несколько раз для разных путей или один раз списком значений, разделённых запятыми. Если параметры `--item` или `--include` не указаны, будет выгружена вся директория `--root-path`.
