...
`--include PATH`: Объекты схемы данных для включения в экспорт. Директории обходятся рекурсивно. Пути указываются относительно `root-path`. Для включения нескольких объектов допускается как указание перечня путей через запятую в одном значении параметра, так и указание параметра несколько раз. Если не указан, выполняется выгрузка всех несистемных объектов в `root-path`.
