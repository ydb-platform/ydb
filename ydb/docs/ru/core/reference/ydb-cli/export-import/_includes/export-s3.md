...
`--root-path PATH`: Корневая директория для выгружаемых объектов. Если не указана, равна пути, соответствующем базе данных.

`--item STRING`: Описание объекта выгрузки. Параметр `--item` может быть указан несколько раз, если необходимо выполнить выгрузку нескольких объектов. Если параметров `--item` или `--include` не указано, то будет выгружена вся директория `--root-path`. `STRING` задается в формате `<свойство>=<значение>,...`, со следующими свойствами:
