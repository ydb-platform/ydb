...
`--source-path PATH`: Корневая директория для выгружаемых объектов. Если не указана, равна пути, соответствующем базе данных.

`--item STRING`: Описание объекта выгрузки. Параметр `--item` может быть указан несколько раз, если необходимо выполнить выгрузку нескольких объектов. Если параметров `--item` не указано, то будет выгружена вся директория `--source-path`. `STRING` задается в формате `<свойство>=<значение>,...`, со следующими свойствами:
