...
`--include PATH`: Пути экспортируемых объектов относительно корневой директории. Поддерживается указание нескольких параметров `--include` или несколько путей, разделённых запятой, в одном параметре. Если параметров `--item` или `--include` не указано, то будет выгружена вся директория `--root-path`.
