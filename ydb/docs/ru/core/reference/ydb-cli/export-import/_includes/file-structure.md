...
Описанная ниже файловая структура применяется для выгрузки как в файловую систему, так и в S3-совместимое объектное хранилище. При работе с S3 в ключ объекта записывается путь к файлу, а директория выгрузки является префиксом ключа. В случае зашифрованного файла к нему добавляется расширение `.enc`.
...
При выгрузке в S3-совместимое объектное хранилище с указанием общего префикса создаётся файл `SchemaMapping/mapping.json`. В этом файле содержится список путей выгружаемых объектов, отображённых в соответствующие префиксы S3 относительно общего префикса.

В зашифрованных выгрузках это отображение анонимизирует исходные имена объектов базы данных, а также хранит дополнительные данные, используемые для расшифровки.
...
- Один или несколько файлов `data_XX.csv`, содержащих данные таблицы в формате `csv`, где `XX` - порядковый номер файла. Выгрузка начинается с файла `data_00.csv`, каждый следующий файл создается при превышении размера текущего файла в 128 МиБ
