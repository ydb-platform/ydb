...
Команда `import {{ import_storage_cmd }}` запускает на стороне сервера процесс загрузки {% if import_storage_cmd == "s3" %}из S3-совместимого хранилища{% else %}из сетевой файловой системы ([Network File System](https://ru.wikipedia.org/wiki/Network_File_System), NFS) хостов кластера {{ ydb-short-name }}{% endif %} данных и информации об объектах схемы, в описанном в статье [Файловая структура](../file-structure.md) формате:

```bash
{{ ydb-cli }} [connection options] import {{ import_storage_cmd }} [options]
```

{% if import_storage_cmd == "s3" %}
...
Перед использованием команды убедитесь, что NFS смонтирована на всех узлах кластера по одному и тому же абсолютному пути и включён [feature flag](../../../../reference/configuration/feature_flags.md) `enable_fs_backups`. Подробнее см. рецепт [Резервное копирование и восстановление через NFS](../../../../recipes/nfs-backup/nfs-backup.md).
...
В отличие от [команды `tools restore`](../tools-restore.md), команда `import {{ import_storage_cmd }}` всегда создает объекты целиком, поэтому для её успешного выполнения ни один из загружаемых объектов (ни директорий, ни таблиц) не должен существовать.

При необходимости догрузки данных в существующие таблицы из {{ import_storage_name }} вы можете скопировать содержимое {{ import_storage_name }} в {% if import_storage_cmd == "s3" %}файловую систему (например, с помощью [S3cmd](https://s3tools.org/s3cmd)){% else %}локальную файловую систему{% endif %} и воспользоваться [командой `tools restore`](../tools-restore.md).
...
Команда загрузки из NFS требует указания монтированной директории (или поддиректории), общей для всех объектов, участвующих в загрузке. Так как загрузка производится в асинхронном режиме на всех хостах {{ ydb-short-name }}, указанная директория должна быть доступна на каждом хосте кластера и смонтирована в NFS.

`--fs-path PATH`: абсолютный путь до монтированной директории (или поддиректории) на хостах кластера.
...
- `source`, `src` или `s` — {{ import_item_source_desc }}.
...
Некоторые возможности могут быть недоступны при использовании альтернативного синтаксиса (в частности, шифрованные резервные копии{% if import_storage_cmd == "s3" %} или перечисление объектов выгрузки{% endif %}).
...
При успешном исполнении команда `import {{ import_storage_cmd }}` выводит сводную информацию о поставленной в очередь операции загрузки из {{ import_storage_name }}, в заданном опцией `--format` формате. Фактическая загрузка производится сервером асинхронно. В сводной информации выводится ID операции, который может быть использован в дальнейшем для проверки статуса и действий с операцией:
...
- В режиме вывода `pretty` (по умолчанию) идентификатор операции показывается в выделенном псевдографикой поле id:
  ...
  | id                                        | ready | stat...
  ├───────────────────────────────────────────┼───────┼─────...
  | ydb://import/8?id=281474976788395&kind={{ import_operation_kind }} | true  | SUCC...
  ├╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴╴┴╴╴╴╴╴╴╴┴╴╴╴╴╴...
  | Items:
  ...
  ```
...
- В режиме вывода `proto-json-base64` идентификатор находится в атрибуте "id":
  ...
  ```json
  {"id":"ydb://import/8?id=281474976788395&kind={{ import_operation_kind }}","ready":true, ... }
  ```
...
```bash
{{ ydb-cli }} -p quickstart operation get "ydb://import/8?id=281474976788395&kind={{ import_operation_kind }}"
```
...
```bash
{{ ydb-cli }} -p quickstart operation forget "ydb://import/8?id=281474976788395&kind={{ import_operation_kind }}"
```

### Список операций загрузки {#list}

Для получения списка операций загрузки воспользуйтесь командой `operation list {{ import_operation_list }}`:

```bash
{{ ydb-cli }} -p quickstart operation list {{ import_operation_list }}
```
