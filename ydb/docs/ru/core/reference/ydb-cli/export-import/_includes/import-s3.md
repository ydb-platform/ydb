...
При необходимости догрузки данных в существующие [строковые таблицы](../../../../concepts/datamodel/table.md#row-oriented-tables) из S3 вы можете скопировать содержимое S3 в файловую систему (например, с помощью [S3cmd](https://s3tools.org/s3cmd)) и воспользоваться [командой `tools restore`](../tools-restore.md). Команда `tools restore` не загружает данные в [колоночные таблицы](../../../../concepts/datamodel/table.md#column-oriented-tables); для догрузки в существующие колоночные таблицы используйте [внешние источники данных](../../../../concepts/datamodel/external_data_source.md). Подробнее см. в статье [{#T}](../../../../concepts/query_execution/federated_query/import_and_export.md#import).
