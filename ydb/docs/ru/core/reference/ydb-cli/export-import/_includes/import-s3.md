...
`--item STRING`: Описание объекта загрузки. Параметр `--item` может быть указан несколько раз, если необходимо выполнить загрузку нескольких объектов. Если параметров `--item` или `--include` не указано, то будут загружены все объекты, присутствующие по указанному префиксу загрузки. `STRING` задается в формате `<свойство>=<значение>,...`, со следующими обязательными свойствами:
...
`--include PATH`: Пути импортируемых объектов относительно корня выгрузки. Поддерживается указание нескольких параметров `--include` или несколько путей, разделённых запятой, в одном параметре. Если параметров `--item` или `--include` не указано, то будут загружены все объекты, присутствующие по указанному префиксу загрузки.
