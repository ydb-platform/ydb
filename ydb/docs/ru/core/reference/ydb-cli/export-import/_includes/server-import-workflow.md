## Что происходит при импорте {#server-import-workflow}

1. Создаётся серверная асинхронная операция импорта.
2. Сервер считывает метаданные объектов из файлов (`scheme.pb`, `metadata.json` и т.д.) в хранилище выгрузки (бакет S3 по заданному префиксу или каталог в файловой системе).
