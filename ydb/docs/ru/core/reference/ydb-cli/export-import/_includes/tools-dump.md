...
[Конфигурация кластера](../../../../maintenance/manual/config-overview.md) выгружается отдельно с помощью следующих действий:

1) Cкопировать cтатическую конфигурацию с локальных дисков узлов.
2) Получить динамическую конфигурацию с помощью команды `{{ ydb-cli }} admin cluster config fetch`.
...
[Конфигурация базы данных](../../../../maintenance/manual/config-overview.md) выгружается отдельно с помощью следующих действий:

1) Cкопировать cтатическую конфигурацию с локальных дисков узлов.
2) Получить динамическую конфигурацию с помощью команды `{{ ydb-cli }} admin database config fetch`.
