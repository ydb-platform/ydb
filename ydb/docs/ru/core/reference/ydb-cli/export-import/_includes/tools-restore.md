...
Если таблица или директория уже существует в базе данных -- в её схему не будет внесено изменений. Это может привести к тому, что операция загрузки данных не сможет быть выполнена, если не все колонки в загружаемых файлах присутствуют в таблице в базе данных или не подходят по типу.
...
`--restore-acl VAL`: Флаг загрузки ACL, 1 (да) или 0 (нет), по умолчанию 1. Если флаг установлен в 0, то при загрузке будут созданы объекты в схеме с пустым ACL, а их владельцем будет пользователь, запустивший загрузку.
