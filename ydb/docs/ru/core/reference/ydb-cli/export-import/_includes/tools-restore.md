...
`--upload-batch-bytes VAL`: Ограничение объема пакета загружаемых данных, по умолчанию 512KB. `VAL` определяет объем, задается в виде числа с приставкой, например 1MiB. Не рекомендуется устанавливать значение более 15MiB.
...
### Пример вызова команды для оптимальной скорости загрузки
