...
При необходимости догрузки данных в существующие таблицы из NFS вы можете скопировать содержимое NFS в локалькую файловую систему и воспользоваться [командой `tools restore`](./tools-restore.md).
...
Команда загрузки из NFS требует указания монтированной директории (или поддиректории) общей для всех объектов, участвующих в загрузке. Так как загрузка производится в асинхронном режиме на всех хостах {{ ydb-short-name }}, указанная директория должна быть на каждом хосте {{ ydb-short-name }} и монтированна в NFS.
...
- В режиме вывода `proto-json-base64` идентификатор находится в атрибуте "id":
  ...
  ```json
  {"id":"ydb://export/8?id=281474976788395&kind=fs","ready":true, ... }
  ```
