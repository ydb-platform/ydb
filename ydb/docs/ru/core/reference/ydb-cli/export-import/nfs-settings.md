...
4. Данные каждой таблицы записываются из копии на файловую систему в формате [файловой структуры выгрузки](file-structure.md) с параллельной записью с разных узлов кластера.
...
Для использования экспорта и импорта через NFS необходимо включить feature flag `enable_fs_backups` в [конфигурации](../../configuration/feature_flags.md) кластера. По умолчанию этот флаг **выключен**.
...
Экспорт и импорт через NFS используют те же очереди [брокера ресурсов](../../configuration/resource_broker_config.md), что и S3: `queue_backup` для экспорта и `queue_restore` для импорта. Увеличение лимитов ускоряет операции, но увеличивает нагрузку на CPU. Значения по умолчанию и способ настройки описаны в разделе [{#T}](../../configuration/resource_broker_config.md).

### IO-пул акторной системы

Файловые операции (запись при экспорте, чтение при импорте) выполняются акторами в пуле `IO` [акторной системы](../../configuration/actor_system_config.md#tuneconfig). По умолчанию этот пул содержит `1` поток. Поскольку операции с файловой системой являются блокирующими, при интенсивном экспорте или импорте IO-пул может стать узким местом.

Для увеличения количества потоков в IO-пуле измените параметр `threads` в [конфигурации акторной системы](../../configuration/actor_system_config.md#tuneconfig):
...
- Мониторьте загрузку пула через [Embedded UI](../../embedded-ui/ydb-monitoring.md#node_list_page).
...
- Экспорт и импорт через NFS поддерживают те же типы объектов, что и экспорт в S3. Подробный перечень поддерживаемых объектов приведён в документации [команды экспорта](export-nfs.md).
