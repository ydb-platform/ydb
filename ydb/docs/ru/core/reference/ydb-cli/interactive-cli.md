...
* [Режим AI-ассистента](#ai-assistant)
...
`CTRL + T` | Переключает между режимами YQL и AI.
...
`resource_pool` | Определяет ресурсный пул для выполнения последующих запросов. В качестве значения ожидает наименование пула.
...
Выполнение запроса в ресурсном пуле `default`:
...
Интерактивный режим включает режим AI-ассистента — диалогового агента на основе большой языковой модели, который помогает исследовать структуру базы данных, писать и выполнять запросы YQL, искать информацию в документации и вызывать команды {{ ydb-short-name }} CLI. Переключаться между режимами YQL и AI можно горячей клавишей `Ctrl + T` или командой `/switch`.

Подробнее см. [Режим AI-ассистента](interactive-cli-ai.md).
