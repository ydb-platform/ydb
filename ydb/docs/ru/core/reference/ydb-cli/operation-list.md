...
* `kind` — тип операции. Возможные значения:
  ...
  * `incbackup` — операции инкрементального резервного копирования. См. также [руководство по операциям с коллекциями резервных копий](export-import/backup-collections/operations.md);
