...
* **Улучшенная производительность:** Параметризованные запросы значительно повышают производительность при массовом выполнении однотипных операций, которые различаются только входными значениями. Это происходит благодаря кешированию плана запроса. После первой компиляции запроса его план сохраняется в [LRU-кеше](https://en.wikipedia.org/wiki/Cache_replacement_policies) [узла {{ ydb-short-name }}](../../concepts/glossary.md#node). Все последующие вызовы того же запроса с новыми параметрами избегают стадии компиляции и начинают выполняться сразу, что экономит время и ресурсы. Подробнее см. в разделе [Параметризованные запросы и повторная компиляция](../../dev/optimization/parameterized-queries.md).
