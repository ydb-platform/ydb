...
|| `--diagnostics-file` | Путь для сохранения файла с диагностикой (подробно см. Сбор статистики. ||
...
Опция `--diagnostics-file <path_to_diagnostics>` позволяет сохранять расширенную диагностическую информацию о выполнении SQL-запросов в файл <path_to_diagnostics> в формате JSON-файл.

Диагностическая информация формируется только при одновременном указании опции `--stats full`, или при выполнении `EXPLAIN`-запросов. Файл `<path_to_diagnostics>` с диагностической информацией содержит следующие поля:

- `plan` — [план](../../yql/query_plans.md) выполнения запроса.
- `stats` — статистика выполнения запроса.
- `meta` — дополнительная информация о запросе в JSON-формате в составе  следующих полей:
...
Сбор диагностики в файл `diagnostics.json`:
...
Если вы хотите получить диагностические данные, относящиеся к плану запроса, без фактического выполнения запроса, необходимо выполнить `EXPLAIN`-запрос, добавив опцию `--explain`:
...
Обратите внимание, что в диагностическом файле `diagnostics.json` в поле `meta.query_text` содержится такая строка:
