...
`--run-interval` | Интервал запуска операции удаления строк с истекшим TTL. Указывается в секундах. Наименьший допустимый интервал составляет 15 минут (900 секунд). Значение по умолчанию (`3600`) оптимально для старта работы с таблицей с установленным TTL.<br/><br/>При выборе значения `--run-interval` важно учитывать баланс между задержкой удаления просроченных данных и дополнительной нагрузкой на кластер. Слишком маленький интервал приводит к частому запуску фоновых задач удаления и может заметно увеличить нагрузку на CPU и I/O, особенно при большом объёме данных. Слишком большой интервал, наоборот, задерживает физическое удаление строк: просроченные данные дольше занимают место на диске и могут продолжать участвовать в сканированиях.<br/><br/>При любом значении `--run-interval` следует ожидать, что на запрос к таблице могут вернуться строки с истекшим `TTL`.
