...
При выполнении обоих условий - значения из первой строки файла используются в качестве имен колонок таблицы. В противном случае - имена колонок генерируются автоматически (как `column1`, `column2`, и т.д.). Подробнее см. [пример](#example-default) ниже.
