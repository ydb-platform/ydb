...
`--content-based-deduplication` | Включает или отключает дедубликацию сообщений на основе содержимого для топика.<br/>Возможные значения: `true` или `false`. При включении сообщения с идентичным содержимым считаются дубликатами.
