...
`--content-based-deduplication` | Включает дедубликацию сообщений на основе содержимого для топика. При включении сообщения с идентичным содержимым считаются дубликатами, и сохраняется только первое сообщение. Полезно для FIFO-топиков, где требуется семантика exact-once на основе содержимого сообщения, а не явных идентификаторов дедубликации.
