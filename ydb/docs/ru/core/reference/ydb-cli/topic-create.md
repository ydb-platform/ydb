...
`--retention-storage-mb` | Максимальный объем хранения для топика, задается в МБ. При достижении ограничения будут удаляться самые старые данные. В настоящий момент функциональность работает некорректно при включенном автопартиционировании. Дело в том, что потребляемое место контролируется отдельно по каждой партиции топика, а не по топику в целом, то есть заданный в опции размер делится на количество партиций и далее контролируется по партиции. Если количество партиций автоматически меняется, то ограничения по партиции остаются неизменными. Поведение будет исправлено в будущих версиях.<br/>Значение по умолчанию — `0` (ограничение не задано).
