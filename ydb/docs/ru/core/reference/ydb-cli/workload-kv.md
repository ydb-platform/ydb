...
* [upsert](#upsert-kv) — при помощи операции `UPSERT` вставляет в заранее созданную командой `init` строковую таблицу строки, которые представляют из себя кортежи (key1, key2, ... keyK, value1, value2, ... valueN), числа K и N задаются в параметрах.
