...
* Предполагается, что каждая поддиректория соответствует таблице в базе данных. Путь к поддиректории от `suite`-директории совпадает с путем к таблице от корня базы данных.
