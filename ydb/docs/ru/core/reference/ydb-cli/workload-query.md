...
* Если задан `suite` и в нем присутсвует папка `init`, она выполняет обход её и всех её вложенных папок поиском в глубину, на каждом уровне упорядочивая файлы и подпапки по алфавиту.
* Выибирает файлы с расширениями `sql` и `yql` и исполняет их как SQL-запросы последовательно.
...
Предполагается, что на этом этапе выполняются относительно "лёгкие" запросы, например, создание таблиц и индексов. Наполнение их данными происходит далее.
Также в команде `init` присутсвует опция `--clear`, позволяющая очистить базу данных перед новой инициализацией.
...
* Выполняется згрузка данных из файлов в соответствующие таблицы. Загрузка идет во много потоков, порядок загрузки не гарантируется.
