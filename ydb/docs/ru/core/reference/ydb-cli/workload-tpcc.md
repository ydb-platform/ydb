...
|-------------------------|--------------------------------------------------------------------------|-----------------------|
| `--warehouses` или `-w` | Количество складов TPC-C.                                                | 10                    |
| `--compact`             | Выполнить сжатие таблиц после загрузки данных.                  |                       |
...
Результаты теста включают tpmC, эффективность и задержки по типам транзакций.

В JSON-выводе (`--format Json`) для каждого типа транзакции публикуются три набора перцентилей latency:

* `percentiles` — **full**, включая ожидание слота session / inflight;
* `percentiles_ms` — без ожидания в этой очереди (ближе к классическим измерениям BenchBase);
* `percentiles_pure` — только время выполнения запросов внутри транзакции.

Если `--warmup` не задан, длительность прогрева выбирается автоматически по числу складов (около 30 минут для масштабов больше 1000 складов).
