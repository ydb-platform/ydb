...
Поддерживается тестирование глобальных и префиксных [векторных индексов](../../yql/reference/syntax/create_table/vector_index.md).
...
2. Производится оценка полноты:
   ...
   - Если выбранный индекс — префиксный, то оба поиска проводятся среди записей с префиксами, равными префиксу тестовой записи.
...
Для проведения теста требуется таблица с загруженными в неё данными, содержащая векторную колонку и созданный векторный индекс.

Для поиска также требуется тестовая выборка векторов. Её можно разместить в отдельной таблице либо сформировать случайным образом из основной таблицы.
...
- Указывать названия колонок (векторной, префиксной, первичного ключа) и функцию расстояния не нужно — они извлекаются автоматически из определения индекса.
