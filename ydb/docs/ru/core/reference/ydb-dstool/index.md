...
С помощью утилиты {{ ydb-short-name }} DSTool вы можете управлять дисковой подсистемой кластера {{ ydb-short-name }}. Чтобы установить и настроить утилиту, выполните [инструкцию](install.md). Перед работой с кластером настройте [подключение и аутентификацию](connect.md).
