...
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на страницу базы данных, выбрать вкладку `Storage`, по известному `id` группы проверить конфигурацию узлов и дисков.
...
**Действия при срабатывании:** В [YDB {{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на страницу базы данных, выбрать вкладку `Storage`, установить фильтры `Groups` и `Degraded`, по известному `id` группы проверить доступность нод и дисков на нодах.
...
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на страницу базы данных, выбрать вкладку `Storage`, установить фильтры `Nodes` и `Degraded`. По известным `id` ноды и pDisk проверить доступность нод и дисков на нодах.
...
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на страницу базы данных, выбрать вкладку `Storage`, установить фильтры `Nodes` и `Out of Space`, по известным `id` ноды и PDisk просмотреть доступное место.
...
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) на вкладке `Storage` установить фильтр `Nodes`. Проверить `Uptime` нод и их статус. Если `Uptime` небольшой, проверить по логам причины рестарта нод.
...
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на вкладку `Nodes`. Проверить `Uptime` нод и их статус. Если `Uptime` небольшой, необходимо проверить логи для определения причин частых рестартов нод.

#### Tablets are dead, Followers are dead

**Описание:** Таблетки не запущены (или не могут быть запущены).
**Действия при срабатывании:** В [{{ ydb-ui-name }}](../ydb-ui/ydb-monitoring.md) перейти на вкладку `Nodes`. Проверить `Uptime` нод и их статус. Если `Uptime` небольшой, необходимо проверить логи для определения причин рестарта нод.
