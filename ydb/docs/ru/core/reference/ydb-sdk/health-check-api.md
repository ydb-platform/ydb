...
**Действия при срабатывании:** С помощью [YDB Embedded UI](../embedded-ui/ydb-monitoring.md) проверить доступность нод и дисков на нодах из списка в проверке. Например, через [YDB Embedded UI](../embedded-ui/ydb-monitoring.md). При необходимости - увезти вдиски с недоступных дисков/нод с помощью `dstool`
...
**Логика работы:** `HealthCheck` проверяет различные параметры (режим отказоустойчивости, количество отказавших дисков, статус дисков и т. д.) и в зависимости от этого устанавливает соответствующий статус у группы.
**Действия при срабатывании:** Проверить доступность нод и дисков на нодах из списка в проверке.
...
**Описание:** Отсутствует нода с дисками.
...
**Описание:** (Load) Физический хост перегружен. Это указывает на то, что система работает на пределе, скорее всего из-за большого количества процессов, ожидающих операций ввода-вывода.
**Логика работы:**
