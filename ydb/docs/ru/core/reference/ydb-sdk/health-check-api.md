...
Даже при технической исправности и штатной работе базы данных (кластера) не гарантируется успешное выполнение запросов к ней — например, может отсутствовать сетевой доступ к отдельным нодам.

В целях проверки доступности базы данных рекомендуется реализовать следующие действия на уровне приложения:

1. Выполнить запрос `SELECT 1` с применением стандартных для приложения ретраев. Это позволит удостовериться в базовой доступности базы данных — в том, что приложение способно установить соединение и получить ответ в рамках принятой политики ретраев.

2. Для более детальной проверки работоспособности системы — выполнить запросы к конкретным таблицам. Это даст возможность подтвердить наличие доступа к целевым структурам данных и оценить работоспособность базы данных на уровне взаимодействия с таблицами.
