...
- Во всех наших SDK реализована эффективная [клиентская балансировка](../../recipes/ydb-sdk/balancing.md) запросов, позволяющая выжать из пропускной способности кластера {{ ydb-short-name }} максимум, а также обеспечивающая равномерность распределения нагрузки по нодам кластера за счет механизмов клиентской балансировки с учетом выставленных политик балансировки. В случае параметризованных запросов клиентская балансировка не работает, потому что запрос жестко привязан к сессии и узлу {{ ydb-short-name }}

- Также необходимо уметь инвалидировать на уровне клиентского кода подготовленные запросы в зависимости от кодов возвращаемых ошибок. Этот микро-менеджмент statement'ов ложится на пользователя {{ ydb-short-name }}, что усложняет его код и требует аккуратности и большей квалификации программиста. Легко допустить ошибку, сохранив для последующего использования "испорченный" подготовленный запрос.
