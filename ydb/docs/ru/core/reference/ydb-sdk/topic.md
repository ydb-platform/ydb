...
- C++
  ...
  В C++ SDK для записи в топик доступно несколько вариантов:
    ...
    - `ISimpleBlockingWriteSession` - простой синхронный вариант `IWriteSession` для записи в одну партицию топика. Рекомендованный способ работы, если покрывает ваш сценарий.
