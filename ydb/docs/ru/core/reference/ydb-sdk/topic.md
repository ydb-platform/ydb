...
- `IProducer` — более высокоуровневая абстракция для записи. Он выбирает партицию для сообщения по ключу (`Key`) или по явному `Partition`. Гарантирует локальность по ключу (сообщения с одним ключом попадут в одну партицию) и порядок доставки в партиции (сообщения будут доставлены в том же порядке, в котором они отправлены). Также `IProducer` корректно обрабатывает ситуацию автопартиционирования (split/merge) и перестроения партиций.

Акты (ack) от продьюсера приходят в том же порядке, в котором сообщения были отправлены, при использовании `AcksHandler`.

При настройке `ProducerIdPrefix` предоставляет гарантию exactly once на уровне записи.

- `IWriteSession` — низкоуровневый, старый вариант. Сессия привязана к `producer_id`/`message_group_id` и (в рамках одной сессии) пишет в одну партицию. Она умеет корректно обрабатывать ошибки при перестроениях партиций (например, в сценариях автопартиционирования), но не предназначена для одновременной записи в несколько партиций. Обеспечивает порядок доставки сообщений в партиции (в потребителе сообщения будут прочитаны в том же порядке, что и отправлены). При указании `ProducerId` в `TWriteSessionSettings` предоставляет гарантию exactly once на уровне записи.

Если нужна высокопроизводительная запись, используйте `IProducer`.
