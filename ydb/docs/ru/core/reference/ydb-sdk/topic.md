...
- C++
  ...
  В C++ SDK для записи в топик доступно несколько вариантов:
    - `IWriteSession` -- максимально функциональная сессия записи. Позволяет делать тонкую настройку и писать асинхронный код. Использовать стоит, когда остальные интерфейсы не подошли.
    - `ISimpleBlockingWriteSession` -- простой синхронный вариант `IWriteSession` для записи по одному сообщению. Рекомендованный способ работы, если покрывает ваш сценарий.
