...
Запрос был отменён на сервере. Например, пользователь отменил запрос во [{{ ydb-ui-name }}](../ydb-ui/index.md), который выполнялся слишком долго, запрос был сделан с опцией таймаута [cancel_after](../../dev/timeouts.md#cancel), или сессия была закрыта во время выполнения запроса.

Если сессия была закрыта, создайте новую сессию перед отправкой следующего запроса. Если выполнение запроса заняло слишком много времени, попробуйте оптимизировать запрос. Если использовалась опция таймаута `cancel_after`, увеличьте её значение.
