...
Запрос был отменён на сервере. Например, пользователь отменил запрос во [{{ ydb-ui-name }}](../ydb-ui/index.md), который выполнялся слишком долго, или запрос был сделан с опцией таймаута [cancel_after](../../dev/timeouts.md#cancel).
