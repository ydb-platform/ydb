...
### Настройки направлений и формата {#backend-settings}

Каждое направление аудитного потока (`file_backend`, `unified_agent_backend`, `stderr_backend`) поддерживает поля вроде `format`, `file_path`, `log_name` и при необходимости `log_json_envelope`. Их назначение описано в таблицах и примерах YAML в этом разделе.
