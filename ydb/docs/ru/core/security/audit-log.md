...
События аудитного лога генерируются *источниками аудитных событий* — службами или подсистемами {{ ydb-short-name }}, способные их производить. В общем случае, для включения аудита требуется указание одного или нескольких [направлений аудитного потока](#enabling-audit-log) в [конфигурации аудита](#audit-config), однако некоторые источники могут требовать дополнительных параметров в или включения [функционального флага](../reference/configuration/feature_flags.md).
