...
| _[Пользовательские атрибуты](../operations/manage-users-attr.md)_ ||
...
Запись в `stderr` рекомендуется для тестовых инсталляций. На дальнейшую обработку `stderr` могут влиять настройки подсистемы [логирования](./logs.md) {{ ydb-short-name }}.
