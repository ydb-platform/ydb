...
Аутентификация по логину и паролю через сервер {{ ydb-short-name }} доступна только для [внутренних пользователей](../concepts/glossary.md#access-user). В аутентификации внешних пользователей участвуют сервера внешних систем.
...
Для включения аутентификации по логину и паролю, необходимо выставить значения параметров `use_login_provider` и `enable_login_authentication` в `true` в [конфигурации](../reference/configuration/auth_config.md).
