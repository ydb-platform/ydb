...
{{ ydb-short-name }} позволяет предъявлять различные требования к сложности паролей. Если пароль, предоставляемый через команды `CREATE USER` или `ALTER USER`, не соответствует критериям сложности, выполнение команды завершится ошибкой.
...
По умолчанию пользователю даётся 4 попытки ввести правильный пароль. Если попытки исчерпаны, аутентификация будет запрещена на один час. Настроить критерии блокировки пользователя можно в [конфигурации](../reference/configuration/auth_config.md#account-lockout).
