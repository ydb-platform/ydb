...
Аутентификация сервисного аккаунта может быть выполнена двумя способами:

* Предоставление логина и пароля.
  В качестве логина используется параметр конфигурации `bind_dn`, пароль записывается в параметре `bind_password`.
* mTLS аутентификация с помощью SASL EXERNAL механизма.
  SASL EXTERNAL позволяет аутентифицировать сервисный аккаунт с помощью протокола mutual TLS. Такой способ не требует заполнять параметры `bind_dn` и `bind_password` в конфигурации, а предоставлять вместо них сертификат. Такой способ позволяет не хранить пароль для сервисного аккаунта в конфигурации.
  Для того чтобы задействовать возможность аутентифицировать сервисный аккаунт с помощью метода SASL EXTERNAL требуется заполнить следующие параметры конфигурации: `use_tls.cert_file`, `use_tls.key_file` и включить флаг `extended_settings.enable_sasl_external_bind`. Подробнее описано в секции [ldap_authentication](../reference/configuration/auth_config.md#ldap-auth-config).
