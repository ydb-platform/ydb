...
- claim `iss` (issuer, издатель токена), совпадающий со значением `issuer` в конфигурации;
- непустой строковый идентификатор пользователя. Для его получения используется claim, заданный параметром `subject_claim_name`. Если этот claim отсутствует или имеет другой тип, используется стандартный claim `sub` (subject, идентификатор субъекта).

Остальные проверяемые поля необязательны:

- claim `aud` (audience, получатель токена) проверяется, если в конфигурации задан параметр `audience`; рекомендуется всегда задавать ожидаемую audience, чтобы токены, выпущенные для других сервисов, не принимались;
- при проверке временных claims `exp` (expiration time, время истечения срока действия), `nbf` (not before, время начала действия) и `iat` (issued at, время выпуска), если они присутствуют, учитывается допустимое расхождение часов, заданное [параметром `allowed_clock_skew`](../reference/configuration/auth_config.md#external-idp-auth-config). Если `exp` отсутствует, {{ ydb-short-name }} устанавливает для результата аутентификации срок действия 10 минут;
- claim, заданный параметром `groups_claim_name`, содержит список групп пользователя. Он должен быть массивом; из массива извлекаются только строковые элементы. Если claim отсутствует или имеет другой тип, список групп считается пустым.

### Формирование SID

К идентификатору пользователя и каждой группе из JWT добавляется суффикс `@<auth-domain>`. Значение `<auth-domain>` задаётся параметром `external_idp_authentication_domain`; по умолчанию используется `sso`.
...
2. Защита от ошибок конфигурации — не допустить подключения к чужим кластерам {{ ydb-short-name }}, например, при неверном параметре [node-broker](../devops/configuration-management/configuration-v2/node-authorization.md) динамический узел не подключится к чужому кластеру, тогда как при обычном TLS такое соединение могло бы установиться.
