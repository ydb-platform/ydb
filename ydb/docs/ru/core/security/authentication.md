...
{{ ydb-short-name }} позволяет дополнительно настраивать политики паролей пользователей. Пароли, указываемые в таких командах как `CREATE USER` и `ALTER USER`, должны соответствовать действующей политике паролей (ссылка на соотв раздел по политикам паролей)
По умолчанию на пароль не наложены никакие ограничения. Принимается пароль любой длины, в том числе и пустая строка, в пароле может присутствовать произвольное число цифр и букв в различных регистрах, а так же специальные символы из списка `!@#$%^&*()_+{}|<>?=`. Для наложения ограничений на сложность пароля нужно заполнить секцию `password_complexity` в [конфигурации](../reference/configuration/index.md#password-complexity).
