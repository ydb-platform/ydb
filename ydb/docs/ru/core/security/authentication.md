...
{{ ydb-short-name }} имеет возможность накладывать различные требования к сложности пароля. Если пароль, предоставляемый через команды `CREATE USER`, `ALTER USER`, будет не удовлетворять критериям сложности, то выполнение команд закончится ошибкой.
