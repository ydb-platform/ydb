...
После успешной установки сетевого соединения сервер принимает к обработке запросы от клиента. Аутентификационная информация передаётся в виде [аутентификационного токена](../concepts/glossary.md#auth-token) или [клиентского сертификата](#client-certificate). На её основании сервер определяет [SID](../concepts/glossary.md#access-sid) клиента и проверяет его доступы на выполнение запроса.
...
Учетные данные сервисного аккаунта для подключения к LDAP указываются в настройках конфигурации: используйте параметры `bind_dn` и `bind_password`, либо настройте mTLS (подробнее см. в разделе [Аутентификация сервисного аккаунта](#ldap-service-account-auth)).
...
1. Клиент устанавливает TLS-соединение с сервером {{ ydb-short-name }}, передавая клиентский сертификат (и цепочку доверия). Чтобы аутентифицироваться по сертификату, клиент не должен передавать токен: ни в заголовке `Authorization` (HTTP), ни через механизмы SDK/CLI для IAM, логина и пароля и т.п.
2. При обработке запроса сервер извлекает сертификат из TLS-контекста.
3. Если в запросе нет [аутентификационного токена](../concepts/glossary.md#auth-token), сервер использует сертификат для аутентификации и проверяет его по правилам секции [client_certificate_authorization](../reference/configuration/client_certificate_authorization.md). Если клиент передаёт вместе с TLS-сертификатом и аутентификационный токен, то используется только токен. Сертификат при этом не используется для аутентификации (на TLS он всё равно уходит).
4. В итоге успешной проверки сертификата клиенту назначается идентификатор защиты [SID](../concepts/glossary.md#access-sid), который обладает всеми назначенными соответствующему идентификатору [правами](../concepts/glossary.md#access-right).
