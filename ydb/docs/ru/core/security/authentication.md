...
{{ ydb-short-name }} может аутентифицировать пользователей по JWT-токенам, выпущенным внешним [провайдером идентификации](https://openid.net/developers/how-connect-works/) (Identity Provider, IdP), который поддерживает OpenID Connect (OIDC). Провайдер отвечает за аутентификацию пользователя и выдачу токена, а {{ ydb-short-name }} проверяет подпись и поля (claims) полученного токена и формирует [SID](../concepts/glossary.md#access-sid) пользователя и его групп.
...
3. Узел {{ ydb-short-name }} запрашивает OIDC Discovery-документ по адресу `<issuer>/.well-known/openid-configuration`. Значение `issuer` в документе должно в точности совпадать с настроенным адресом провайдера.
4. Из Discovery-документа извлекается адрес `jwks_uri`, по которому {{ ydb-short-name }} периодически получает набор открытых ключей [JWKS](https://www.rfc-editor.org/rfc/rfc7517).
5. По полям `alg` и `kid` заголовка JWT выбирается открытый ключ. {{ ydb-short-name }} проверяет подпись токена, `issuer`, настроенную `audience` и временные поля.
...
Симметричные алгоритмы семейства `HS*` не поддерживаются. Открытый ключ в JWKS должен иметь совпадающие `kty` и `kid` и содержать цепочку сертификатов `x5c`. В текущей реализации открытый ключ извлекается из первого сертификата `x5c`; JWK, содержащие только параметры RSA или EC без `x5c`, пропускаются.

При проверке используются следующие поля токена:

- `iss` должен совпадать со значением `issuer` в конфигурации;
- `aud` проверяется, если в конфигурации задан параметр `audience`; рекомендуется всегда задавать ожидаемую audience, чтобы токены, выпущенные для других сервисов, не принимались;
- для временных claims `exp`, `nbf` и `iat`, если они присутствуют, учитывается допустимое расхождение часов `allowed_clock_skew`;
- claim, заданный параметром `subject_claim_name`, используется как идентификатор пользователя. Claim должен быть строкой. Если он отсутствует или имеет другой тип, используется стандартный claim `sub`. Пустой итоговый идентификатор приводит к отказу в аутентификации;
- claim, заданный параметром `groups_claim_name`, используется как список групп. Он должен быть массивом; из массива извлекаются только строковые элементы. Если claim отсутствует или имеет другой тип, список групп считается пустым.

Поле `exp` не является обязательным. Если оно отсутствует, {{ ydb-short-name }} устанавливает для результата аутентификации срок действия 10 минут.
