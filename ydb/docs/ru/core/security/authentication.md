...
2. Защита от ошибок конфигурации — не допустить подключения к чужим кластерам {{ ydb-short-name }}, например, при неверном параметре [node-broker](../devops/concepts/node-authorization.md) динамический узел не подключится к чужому кластеру, тогда как при обычном TLS такое соединение могло бы установиться.
