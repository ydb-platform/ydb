...
{{ ydb-short-name }} имеет возможность накладывать различные требования к сложности пароля. Если пароль, предоставляемый через команды `CREATE USER`, `ALTER USER`, не будет удовлетворять критериям сложности, то выполнение команд закончится ошибкой.
По умолчанию на пароль не наложены никакие ограничения. Принимается пароль любой длины, в том числе и пустая строка, в пароле может присутствовать произвольное число цифр и букв в различных регистрах, а так же специальные символы из списка `!@#$%^&*()_+{}|<>?=`. Для наложения ограничений на сложность пароля нужно заполнить секцию `password_complexity` в [конфигурации](../reference/configuration/auth_config.md#password-complexity).
