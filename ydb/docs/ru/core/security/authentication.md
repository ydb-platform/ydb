...
Аутентификация устройств — проверка [клиентского сертификата](../concepts/glossary.md#client-certificate) при установлении TLS-соединения; [SID](../concepts/glossary.md#access-sid) при этом не формируется. Если сертификат предъявлен, проверяется цепочка доверия к CA; недоверенный сертификат приводит к отклонению соединения до обработки прикладных запросов. Обязательность предъявления сертификата зависит от интерфейса (см. [ниже](#device-auth-interfaces)).
...
2. Если сертификат предъявлен, сервер проверяет цепочку доверия относительно настроенного центра сертификации (CA). На этом этапе выполняется проверка сертификата на уровне TLS (цепочка доверия к CA, срок действия и т. п.); дополнительные правила сопоставления полей сертификата (например, `require_same_issuer`, Subject и SAN) применяются при [аутентификации по клиентскому сертификату](./authentication.md#client-certificate).
