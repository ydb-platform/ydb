...
Оба механизма применяются одновременно: для конкретного [субъекта](../concepts/glossary.md#access-subject) действие оказывается доступно, если оба механизма его разрешают, и недоступно, если хотя бы один не разрешает.

### Иерархия уровней доступа

Списки уровней доступа образуют иерархию, которая используется в [Embedded UI](../reference/embedded-ui/ydb-monitoring.md), viewer и во многих других общекластерных действиях:

- `database_allowed_sids` — даёт уровень доступа database;
- `viewer_allowed_sids` — даёт уровень доступа viewer;
- `monitoring_allowed_sids` — даёт уровень доступа monitoring;
- `administration_allowed_sids` — даёт уровень доступа administration.

Более высокий уровень автоматически включает все более низкие, поэтому субъекту достаточно присутствовать только в одном списке. Например, администратор автоматически получает привилегии monitoring и viewer.
...
- **Database** (наличие в `database_allowed_sids`) — даёт право работать с Embedded UI только как «пользователь базы данных»: можно открыть UI и видеть данные в рамках конкретной базы, но нельзя просматривать общекластерные данные или выполнять операции уровня кластера. Предназначен для приложений или пользователей, ограниченных одной базой данных. Вызовы без указания базы запрещены.
- **Viewer** (наличие в `viewer_allowed_sids`) — даёт право просматривать Embedded UI без возможности вносить изменения. Даёт возможность просматривать состояние кластера, недоступное публично (включая большую часть страниц [Embedded UI](../reference/embedded-ui/ydb-monitoring.md)).
- **Monitoring** (наличие в `monitoring_allowed_sids`) — даёт право выполнять в Embedded UI действия, изменяющие состояние системы. Например, позволяет выполнять резервное копирование, восстановление базы данных или запускать YQL-запросы через Embedded UI.
...
- **Register node** (наличие в `register_dynamic_node_allowed_sids`) — даёт право регистрировать динамические узлы в кластере. По техническим причинам этот список должен включать `root@builtin`.
- **Bootstrap** (наличие в `bootstrap_allowed_sids`) — даёт право выполнять операции начальной инициализации кластера. Используется для первоначальной инициализации кластера из неинициализированного состояния, когда подсистема аутентификации ещё не функционирует. Начальная инициализация разрешена, если субъект входит в `bootstrap_allowed_sids` или `administration_allowed_sids`.
