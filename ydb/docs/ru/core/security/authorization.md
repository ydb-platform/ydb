...
Списки уровней доступа образуют иерархию, которая используется в [YDB UI](../reference/ydb-ui/ydb-monitoring/index.md), viewer и во многих других общекластерных действиях (порядок от меньших привилегий к большим):
...
- **Viewer** (наличие в `viewer_allowed_sids`) — доступ только на чтение для общекластерного состояния: можно просматривать страницы [YDB UI](../reference/ydb-ui/ydb-monitoring/index.md) и диагностическую информацию, но нельзя запускать действия, изменяющие состояние системы.
