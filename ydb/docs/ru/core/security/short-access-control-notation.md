...
| `W` | (write) запись. Состоит из разрешений на обновление и удаление записей из строковых таблиц, запись атрибутов ACL, создание подкаталогов, создание таблиц, очередей, изменение и удаление объектов, изменении пользовательских атрибутов
...
Например, `+(SR|UR):subject` — разрешение на чтение и обновление записей в строковые таблицы.

| Разрешение | Описание |
|:----:|:----|
| `SR` | (select row) чтение из таблиц
| `UR` | (update row) обновление записей таблиц
| `ER` | (erase row) удаление записей из строковых таблицы
