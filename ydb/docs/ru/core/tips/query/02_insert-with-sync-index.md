...
Синхронные вторичные индексы в {{ ydb-short-name }} обеспечивают консистентность данных, но создают дополнительные ограничения при вставке данных. Основные проблемы:
...
3. **Увеличенная вероятность ошибок TLI (Transaction Locks Invalidated)** - более медленные транзакции повышают вероятность конфликтов блокировок, как и риск эскалации блокировок на уровень партиций индексных таблиц

## Решение

### Стратегия 1: Использование транзакционных UPSERT запросов

Для таблиц с синхронными индексами используйте транзакционные UPSERT запросы вместо BulkUpsert.
...
### Стратегия 2: Оптимизация размера батчей

Разбивайте большие объемы данных на оптимальные батчи:
...
#### ✅ Хороший пример - оптимальные батчи
...
### Стратегия 3: Использование идемпотентных операций

> [!NOTE]
> При наличии на таблице большого количества вторичных индексов вставка данных с помощью оператора INSERT является более производительной. Выбор между UPSERT и INSERT в такой ситуации - результат компромисса между производительностью и обеспечением идемпотентности операций.

Используйте UPSERT вместо INSERT для избежания ошибок дублирования:
...
### Стратегия 4: Для массовой загрузки - временное удаление индексов

Для первоначальной загрузки больших объемов данных:
...
1. **Размер батча**: 100-1000 строк на транзакцию
2. **Сортировка данных**: сортируйте по первичному ключу для минимизации шардов
3. **Ретраи**: всегда обрабатывайте TLI ошибки с экспоненциальной задержкой
4. **Мониторинг**: отслеживайте метрики TLI ошибок и latency
...
Следование этим рекомендациям позволит эффективно работать с таблицами, содержащими синхронные индексы, минимизируя latency и избегая ошибок TLI.
