...
2. На каждом сервере {{ ydb-short-name }} изменить [динамическую конфигурацию](../../../devops/configuration-management/configuration-v1/dynamic-config.md) кластера:
...
3. Чтобы применить новую конфигурацию, перезапустить узлы YDB с помощью утилиты [ydbops](../../../reference/ydbops/rolling-restart-scenario.md), [ansible](../../../devops/deployment-options/ansible/restart.md) или вручную.
...
2. Воспроизведите стандартную нагрузку на кластер {{ ydb-short-name }}:
