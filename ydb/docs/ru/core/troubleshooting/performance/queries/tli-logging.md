...
Когда происходит TLI, сервер записывает в лог записи от каждого компонента (`DataShard` или `SessionActor`) для каждой из сторон конфликта (жертве или нарушителе). Количество записей зависит от количества запросов в транзакциях.
