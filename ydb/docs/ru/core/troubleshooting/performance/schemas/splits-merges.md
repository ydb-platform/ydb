...
При снижении значения `AUTO_PARTITIONING_PARTITION_SIZE_MB` ниже текущего размера многих партиций это может поставить в очередь на разделение сразу множество партиций и кратковременно увеличить задержки. Операции из очереди выполняются с ограниченным параллелизмом. Снижайте порог постепенно и контролируйте метрики.
