...
При снижении значения `AUTO_PARTITIONING_PARTITION_SIZE_MB` на уже большой таблице избегайте резкого уменьшения порога (например, с 2000 МБ до 100 МБ за один шаг): это может вызвать волну одновременных split-операций и кратковременно увеличить задержки. Снижайте порог **постепенно** и наблюдайте за стабилизацией числа партиций между шагами.
