...
{{ ydb-short-name }} поддерживает векторный поиск по векторам типов `Float`, [Float16](https://en.wikipedia.org/wiki/Half-precision_floating-point_format), [BFloat16](https://en.wikipedia.org/wiki/Bfloat16_floating-point_format), `Int8`, `Uint8`, `Bit`. Следовательно, возможно применение скалярного квантования для преобразования данных из `Float` в любой из этих типов.
