...
### Вариант над кортежем

Вариант над кортежем перечисляет типы альтернатив непосредственно. Например,
`Variant<Int32, String>` содержит две альтернативы с индексами `0` и `1`.
В отличие от него, `Variant<Tuple<Int32, String>>` содержит одну альтернативу,
значением которой является кортеж, поэтому существует только индекс `0`.

Индекс передаётся в `Variant()` строкой. [Way](#way) возвращает активный индекс
как `Uint32`. Точечная нотация предоставляет доступ к альтернативе по известному
на этапе компиляции индексу и возвращает `Optional<T>`: значение активной
альтернативы либо `NULL` для неактивной:
...
`Variant()` принимает индекс кортежа строкой, поскольку этот же аргумент
задаёт имя поля для варианта над структурой. Это не изменяет тип результата
`Way()`.

Если запрос должен обрабатывать любую активную альтернативу, используйте
[Visit](#visit).
