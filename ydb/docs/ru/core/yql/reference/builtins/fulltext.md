...
Добавьте `+` перед термом в режиме `Keywords` с `DefaultOperator = "Or"`, чтобы сделать его обязательным для каждого результата. `MinimumShouldMatch` при этом учитывает только оставшиеся необязательные термы:
...
Запрос вернёт документы, содержащие «машинное» (обязательный терм) и хотя бы один из «обучение», «нейронные», «сети» (необязательные). Если все термы начинаются с `+`, запрос работает как строгий AND.
