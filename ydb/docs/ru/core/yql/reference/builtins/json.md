...
2. JsonPath запрос (должен быть явно указан строковым литералом; параметр
   запроса или другое выражение здесь использовать нельзя)
3. **(Опционально)** `PASSING` секция

{% note warning %}

Сам JsonPath запрос нельзя передать как параметр YQL-запроса, в том числе как
параметр, заданный через SDK. `JsonPath` не является типом данных YQL. Поэтому
ни `$path`, ни `CAST($path AS JsonPath)` нельзя использовать как второй аргумент
JSON-функции.

Чтобы передать динамические значения в фиксированный JsonPath запрос,
используйте секцию `PASSING`. Если приложению нужно выбирать разные пути,
выбирайте шаблон запроса из заданного приложением списка разрешённых вариантов.
Не подставляйте недоверенный путь в текст YQL-запроса.
...
В качестве выражения в этой секции можно использовать параметр YQL-запроса:
...
-- При $increment = 5 результат равен 15.
```
