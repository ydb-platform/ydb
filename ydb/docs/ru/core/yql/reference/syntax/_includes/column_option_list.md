...
Настройки сжатия данных колонки. Параметры задаютcя в виде пар ключ=значение, разделенных запятыми.
...
В случае, если COMPRESSION() задается без параметров, колонка будет вести себя аналогично колонке, где сжатие не было задано, то есть колонка будет сжиматься алгоритмом по-умолчанию.
