...
Запрещает значения `NULL` в данной колонке.

{% note warning %}

На текущий момент поддерживается только операция `DROP NOT NULL`, которая снимает ограничение и снова разрешает значения `NULL` в колонке. `SET NOT NULL` находится в разработке.
