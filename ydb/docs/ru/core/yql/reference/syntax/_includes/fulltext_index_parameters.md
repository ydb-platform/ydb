  * общие параметры для всех полнотекстовых индексов:
    * `analyzer` - предустановленная цепочка токенизатора и фильтров:
      * `standard` - токенизатор `standard`, фильтры lowercase и английских стоп-слов
      * `snowball` - токенизатор `standard`, фильтры lowercase, стоп-слов и Snowball; требуется `language` (`english` или `russian`)
      * `keyword` - токенизатор `keyword` без фильтров
    ...
    * `use_filter_stopwords` - фильтр стоп-слов (`true` или `false`); без `language` используется английский язык, поддерживаются `english` и `russian`
    ...
    * `language` - язык для фильтра стоп-слов или стеммера [Snowball](https://snowballstem.org/) (`english` или `russian`)
