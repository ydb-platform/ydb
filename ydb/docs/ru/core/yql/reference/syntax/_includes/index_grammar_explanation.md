...
* `<index_type>` - тип индекса, в настоящее время поддерживаются:
    ...
    * `bloom_filter` — локальный Bloom skip индекс. Доступен только `LOCAL`. См. [ALTER TABLE ADD INDEX](../alter_table/indexes.md#local-bloom-column).
    * `bloom_ngram_filter` — локальный N-граммный Bloom skip индекс. Доступен только `LOCAL`. См. [ALTER TABLE ADD INDEX](../alter_table/indexes.md#local-bloom-column).
