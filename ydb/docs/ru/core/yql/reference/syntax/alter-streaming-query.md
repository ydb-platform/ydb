...
- `FORCE = (TRUE|FALSE)`  нужно ли разрешать изменение запроса, приводящее к невозможности его загрузки из чекпоинта, по умолчанию FALSE (`TRUE` в основном требуется указывать при изменения запроса, приводящим к изменению физического плана исполнения).
Примеры:
