...
`ALTER STREAMING QUERY` изменяет настройки{% if alter_streaming_query == true %} и текст {% endif %} [потоковых запросов](../../../concepts/streaming-query.md), а также управляет их состоянием: запуском, остановкой, возобновлением и перезапуском.
...
* `RUN = (TRUE|FALSE)` — запустить, возобновить или перезапустить запрос либо остановить его.
...
При выполнении `SET (RUN = TRUE)` запрос будет:
- запущен, если он ранее не был запущен;
- возобновлён, если он ранее был запущен и затем остановлен;
- перезапущен, если он уже выполняется.

Если запрос возобновляется или перезапускается, смещения чтения из топика и состояния агрегационных функций восстанавливаются из [чекпоинта](../../../dev/streaming-query/checkpoints.md).
