...
* `availability_period` — определяет время доступности сообщений для читателя. Данные из топика не будут удалены, если их возраст меньше указанного значения и читатель не подтвердил их обработку. Тип значения — `Interval`. Не совместим с параметром `important`. Значение по умолчанию отсутствует.
