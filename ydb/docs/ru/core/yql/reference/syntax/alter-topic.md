...
* `availability_period` — определяет время доступности сообщений для читателя. Время жизни сообщений в топике будет продлено вплоть до этого времени, если этот читатель не подтверждает их обработку. Тип значения — `Interval`. Не совместим с параметром `important`. Значение по умолчанию отсутствует.
