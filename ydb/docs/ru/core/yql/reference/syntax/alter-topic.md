...
* `retention_storage_mb` — ограничение на максимальное место на диске, занимаемое данными топика. При превышении этого значения старые данные будут удаляться, как по retention. В настоящий момент функциональность работает некорректно при включенном автопартиционировании. Дело в том, что потребляемое место контролируется отдельно по каждой партиции топика, а не по топику в целом, то есть заданный в опции размер делится на количество партиций и далее контролируется по партиции. Если количество партиций автоматически меняется, то ограничения по партиции остаются неизменными. Поведение будет исправлено в будущих версиях. Тип значения — `integer`, значение по умолчанию — `0` (не ограничено).
