...
Операция `ADD INDEX` для создания глобальных вторичных индексов (`GLOBAL`, `UNIQUE` и т.п.) поддерживается только для строковых таблиц. Для колоночных таблиц через `ADD INDEX` поддерживаются только специальные локальные Bloom skip индексы, описанные ниже.
