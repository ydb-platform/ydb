...
Для [колоночных таблиц](../../../../concepts/datamodel/table.md#column-oriented-tables) поддерживается добавление **локальных Bloom skip индексов**; см. [Локальные Bloom skip индексы для колоночных таблиц](#local-bloom-column).
...
* `bloom_ngram_filter` — N-граммный фильтр Блума для строковых колонок. Параметры:
  ...
  * `case_sensitive` — необязательный, `true` или `false` (по умолчанию).
