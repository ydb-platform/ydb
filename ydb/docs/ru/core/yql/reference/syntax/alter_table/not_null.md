...
* `SET NOT NULL` выполняется как синхронная SQL-операция (ожидает применения изменения схемы), но при этом создаётся фоновая операция для проверки таблицы на наличие `NULL`-значений.
...
* После запуска операции `SET NOT NULL` и до её завершения в указанную колонку нельзя записывать `NULL`-значения. При попытке записать такие значения, вы получите ошибку: «`SET NOT NULL` operation is currently in progress for this column».
