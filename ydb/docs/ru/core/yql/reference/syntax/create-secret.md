...
* `option` — опция команды:
  ...
  * `inherit_permissions` (Bool) — опция, при включении которой [права](grant.md) на секрет наследуются от директории, в которой секрет создаётся. Отключенная опция оставляет наследование только [права](grant.md) `DESCRIBE SCHEMA`. Владелец секрета получает все возможные права на него в любом случае. По умолчанию — `False`.
