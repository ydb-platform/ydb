...
* `retention_period` — время хранения данных в топике. Тип значения — `Interval`, значение по умолчанию — `24h`.
...
Значение `max_active_partitions`, превышающее `min_active_partitions`, само по
себе не включает автопартиционирование. Чтобы использовать разные минимальную
и максимальную границы, дополнительно задайте `auto_partitioning_strategy`
равным `scale_up`. При стратегии `disabled`, используемой по умолчанию, число
активных партиций топика фиксировано.
...
Чтобы создать топик с разделяемым (общим) читателем, выполните команду:
