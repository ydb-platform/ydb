...
Это ограничение связано с тем, что `CREATE TABLE AS` выводит схему таблицы только из результата `SELECT`: в синтаксисе команды можно задать лишь первичный ключ и параметры `WITH`, но не секции `INDEX` и другие элементы DDL. Сейчас это постоянное ограничение языка YQL; [локальные Блум-индексы](bloom_skip_index.md) и остальные индексы добавляются через `ALTER TABLE` после создания таблицы.
