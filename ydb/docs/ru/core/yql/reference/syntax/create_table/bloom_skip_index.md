...
{% if backend_name == 'YDB' %}[Блум-индексы](../../../../dev/bloom-skip-indexes.md){% else %}Блум-индексы{% endif %} можно задать только как локальные (`LOCAL`). При создании таблицы в секции `INDEX` для них используются типы `bloom_filter` или `bloom_ngram_filter` (по аналогии с [вторичным индексом](secondary_index.md), но с обязательным `LOCAL` и соответствующим `USING`).
