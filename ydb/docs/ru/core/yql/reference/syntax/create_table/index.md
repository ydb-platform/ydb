...
- Создание строковой таблицы
  ...
  Для ключевых и неключевых колонок допускаются только [примитивные](../../types/primitive.md) и [серийные](../../types/serial.md) типы данных.
  ...
  {% if feature_serial %}Колонка с несерийным типом{% else %}Колонка{% endif %} без дополнительных модификаторов приобретает [опциональный тип](../../types/optional.md) и допускает запись `NULL` в качестве значений. Для получения неопционального типа необходимо использовать `NOT NULL`.
  ...
  Серийные колонки автоматически получают ограничение `NOT NULL`. Этот механизм поддерживается только для строковых таблиц.
