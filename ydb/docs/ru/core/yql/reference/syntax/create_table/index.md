...
Определение ключей партиционирования для **строковых** таблиц. Указывает колонки, которые используются для партиционирования данных. Колонки должны быть частью первичного ключа и перечислены в том же порядке, что и в `PRIMARY KEY`.

{% note warning %}

В настоящий момент `PARTITION BY` для строковых таблиц поддерживается только в случае, когда указанные колонки полностью совпадают с первичным ключом таблицы. В остальных случаях использование `PARTITION BY` для строковых таблиц пока не поддерживается.

{% endnote %}

Подробнее о партиционировании строковых таблиц читайте в разделе [{#T}](../../../../concepts/datamodel/table.md#partitioning_row_table).

### PARTITION BY HASH

Определение ключей партиционирования для **колоночных** таблиц. Указывает колонки, по хэшу которых выполняется [партиционирование](../../../../concepts/glossary.md#partition) данных. Колонки должны быть частью первичного ключа.

{% note info %}

`PARTITION BY HASH` является обязательным параметром при создании колоночных таблиц. Подробнее о выборе ключей партиционирования для колоночных таблиц читайте в статье [{#T}](../../../../dev/primary-key/column-oriented.md).
...
- Создание строковой таблицы
  ...
  Пример создания строковой таблицы с явным указанием ключей партиционирования (только если они совпадают с первичным ключом):
  ...
  В настоящий момент `PARTITION BY` для строковых таблиц поддерживается только в случае, когда указанные колонки полностью совпадают с первичным ключом таблицы.
