...
### Предикаты и Bloom skip индекс `bloom_ngram_filter` {#like-with-bloom-ngram}

Для строковых колонок с локальным индексом `bloom_ngram_filter` оптимизатор может использовать фильтр при выполнении предикатов вроде `LIKE` / `ILIKE` по индексируемой колонке (без `VIEW`, в отличие от полнотекстового индекса). Подробности см. в [Bloom skip индексы](../../../dev/bloom-skip-indexes.md).
