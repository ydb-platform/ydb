...
Каждая прагма `TablePathPrefix` начинает действовать с того места, где она указана в тексте запроса. Следующая `TablePathPrefix` изменяет префикс для последующих ссылок на таблицы, не меняя пути в предыдущих выражениях.

Именованные подзапросы, например `$q = SELECT * FROM Input`, сохраняют префикс, действовавший при определении подзапроса. Строковое выражение, используемое как имя таблицы, например `$t = "Input"` с последующим `SELECT * FROM $t`, использует префикс, действующий в месте каждого обращения `FROM $t`. Присваивание строки не фиксирует префикс.
