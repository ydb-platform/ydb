...
При включённом флаге `EnableTablePathPrefixMultiScopes` в `TableServiceConfig` каждая прагма `TablePathPrefix` начинает действовать с того места, где она указана в тексте запроса. Следующая `TablePathPrefix` изменяет префикс для последующих ссылок на таблицы, не меняя пути в предыдущих выражениях.

Флаг включён по умолчанию. Если он выключен, сохраняется прежнее поведение: последующая прагма может также изменить пути к таблицам в предыдущих выражениях.

При включённом флаге именованные подзапросы, например `$q = SELECT * FROM Input`, сохраняют префикс, действовавший при определении подзапроса. Строковое выражение, используемое как имя таблицы, например `$t = "Input"` с последующим `SELECT * FROM $t`, использует префикс, действующий в месте каждого обращения `FROM $t`. Присваивание строки не фиксирует префикс.
