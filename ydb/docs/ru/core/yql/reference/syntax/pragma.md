...
Добавить указанный префикс к путям таблиц. При включённом флаге `EnableTablePathPrefixRelativePaths` в `TableServiceConfig` относительный префикс, например `folder` или `./folder`, отсчитывается от корня базы данных, указанной при подключении. Абсолютный префикс (начинающийся с `/`) отсчитывается от корня кластера.

Флаг включён по умолчанию. Если он выключен, сохраняется прежнее поведение: относительные префиксы отсчитываются от корня кластера.

Пути объединяются по правилам файловой системы: поддерживаются `.` и `..`, завершающий слеш необязателен. Например, при подключении к базе данных `/local`:
...
Каждый относительный префикс отсчитывается от корня базы данных, даже если ранее уже был задан другой префикс. Пустой префикс указывает на корень базы данных.

`..` позволяет получить путь выше корня базы данных. Для базы данных `/Root/database` префикс `../sibling` разрешается в `/Root/sibling`. К полученному пути применяются те же проверки доступа и зависящие от операции ограничения на базу данных, что и к явно заданному абсолютному пути.
