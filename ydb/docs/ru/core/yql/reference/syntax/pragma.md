...
Добавить указанный префикс к путям таблиц. Относительный префикс, например `folder` или `./folder`, отсчитывается от корня базы данных, указанной при подключении. Абсолютный префикс (начинающийся с `/`) отсчитывается от корня кластера.
...
Префикс не добавляется, если имя таблицы указано как абсолютный путь (начинается с `/`). Например, путь к таблице `/other/users` используется без изменений независимо от префикса.
