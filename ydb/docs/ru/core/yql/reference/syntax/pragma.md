...
Добавить указанный префикс к путям таблиц. При включённом флаге `EnableTablePathPrefixRelativePaths` относительный префикс, например `folder` или `./folder`, отсчитывается от корня базы данных, указанной при подключении. Абсолютный префикс (начинающийся с `/`) отсчитывается от корня кластера.
