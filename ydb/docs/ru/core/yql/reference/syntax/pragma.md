...
Относительные префиксы, например `folder` или `./folder`, отсчитываются от базового пути запроса — пути, относительно которого по умолчанию разрешаются имена таблиц. Префикс, начинающийся с `/`, считается абсолютным.

Префикс не добавляется, если имя таблицы в запросе указано как абсолютный путь (начинается с /).
