...
`HOP` группирует данные по перекрывающимся временным окнам ([hopping windows](https://en.wikipedia.org/wiki/Window_function_(SQL)#Hopping_window)). Используется в `GROUP BY` вместе с другими столбцами или выражениями для группировки.
...
Параметр `delay` игнорируется, так как данные в таблице уже доступны целиком и обрабатываются в отсортированном порядке.
...
`HoppingWindow` группирует события по перекрывающимся временным окнам (hopping windows), аналогично [GROUP BY HOP](#group-by-hop). Основное отличие: в потоковых запросах `HoppingWindow` использует механизм [водяных знаков](../../../../dev/streaming-query/watermarks.md) для определения момента закрытия окна вместо фиксированного параметра `delay`.
...
3. События обрабатываются в порядке возрастания `time_extractor`. Допускаются небольшие перестановки порядка входного потока.
