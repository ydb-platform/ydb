...
Алгоритм использует SQL выражение `CAST(ts AS Timestamp)` для определения временной метки для каждого входного события. Другими словами, из каждого нового сообщения будет взято строковое поле `ts` и конвертировано в тип `Timestamp`. Полученное значение будет временем события. Оно определяет окна, в которые попадет событие.

Окна в алгоритме **hopping window** строятся по следующему правилу:

- Берутся временные метки, делящиеся на **hop** (10 секунд): 0, 10, 20 и так далее. Это будут временные метки начал окон.
- К каждой такой временной метке прибавляется **interval** (30 секунд): 30, 40, 50 и так далее. Это будут временные метки концов окон.
- Получились окна: `[0; 30)`, `[10; 40)`, `[20; 50)` и так далее. Если временная метка события лежит в интервале, то событие кладется в окно, добавляется в соответствующие агрегации
- Окно считается завершенным при получении события с временной меткой больше, чем конец этого окна + **delay** (20 секунд). Например, окно `[10; 40)` закрывается при получении события с меткой 60 и более.

Также доступны функции `HOP_START()` и `HOP_END()`, которые возвращают временную метку начала и конца текущего окна типа `Timestamp` соответственно.
...
1. Входной топик партиционируется по ключам группировки, указанным в `GROUP BY`, без учета HOP. Если кроме HOP в `GROUP BY` ничего нет, то входной топик попадает в одну партицию.
2. В каждой партиции окно продвигается независимо от других.
...
Так как события могут перемешиваться, добавлен параметр `delay` для указания задержки закрытия окна. События, приходящие до текущего окна, игнорируются.

### Ограничения

`time_extractor` представляет собой SQL выражение, зависящее только от входных значений столбцов, и должно иметь тип `Timestamp`.

Для задания `hop`, `interval` и `delay` используется строковое выражение, соответствующее стандарту [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601), например, `PT10S` означает 10 секунд, `PT1M` означает 1 минуту. Это формат, который используется для конструирования встроенного типа `Interval` из [строки](../../builtins/basic.md#data-type-literals).

Значения параметров `interval` и `delay` должны делиться на значение параметра `hop`. Параметры `hop` и `interval` должны быть ненулевыми.
