...
{% if select_command == "SELECT STREAM" %}
3. Каждая партиция обрабатывается в порядке возрастания значения выражения `time_extractor`. Допускаются небольшие перестановки строгого порядка возрастания входного потока;
{% else %}
3. Каждая партиция сортируется по возрастанию значения выражения `time_extractor`;
{% endif %}
4. Каждая партиция делится на возможно пересекающиеся подмножества из событий;
5. На каждом подмножестве событий вычисляются заданные агрегатные функции.

{% if select_command == "SELECT STREAM" %}
Так как события в партиции могут слегка перемешиваться, добавлен параметр `delay` для указания задержки закрытия окна. События, приходящие до текущего окна, игнорируются.
{% else %}
Параметр `delay` игнорируется так как данные в одной партиции уже отсортированы.
{% endif %}

Параметры `interval` и `delay` следует задавать кратными параметру `hop`. Некратные интервалы в текущей реализации запрещены.
Параметры `interval` и `hop` должны быть ненулевыми.
