...
Вместо встроенного `Mode` (`rrf`/`linear`) можно передать собственную лямбду объединения, которая вычисляет итоговую оценку из значений ветвей. Доступны два вида лямбд, взаимоисключающих:
