...
1. Входная таблица разбивается на партиции. Каждая партиция состоит из набора строк входной таблицы с одинаковыми значениями в колонках, перечисленных после `PARTITION BY`.
2. Каждая партиция упорядочивается в соответствии с блоком `ORDER BY`.
3. В каждой упорядоченной партиции независимо выполняется распознавание паттерна из `PATTERN`.
