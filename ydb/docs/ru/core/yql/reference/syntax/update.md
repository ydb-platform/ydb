...
Используется для обновления больших таблиц, с которыми не может справиться стандартный `UPDATE` из-за транзакционных ограничений и риска потенциальной отмены блокировок транзакции. Независимо применяет изменения для каждой партиции указанной таблицы, обрабатывая ограниченное количество строк за итерацию (по умолчанию — 10000 строк). Запрос выполняется в нетранзакционном режиме. В случае ошибки изменения не откатываются. Семантика наследуется от стандартного `UPDATE` с ограничениями:
