...
Используется для обновления строк и одновременного возврата их новых значений. Результат содержит все записи, соответствующие условию обновления, вне зависимости от того, потребовалась ли фактическая модификация данных. Это позволяет получить актуальное состояние строк за один запрос, избавляя от необходимости выполнять дополнительный SELECT.
