...
Хотя тип `Interval` и поддерживет литералы только в формате, соответствующем ([ISO 8601](https://ru.wikipedia.org/wiki/ISO_8601)), в YDB есть следующие отличия от стандарта:

* доступны только те интервалы, которые можно представить как конкретное целое число микросекунд (то есть год `Y` и месяц `M` к таковым не относятся из-за разного количества дней);
...
* доступно совмещение литерала недели `W` с другими;
...
SELECT (
    ...
    Interval("P1D")                 -- интервал 1 сутки (24 часа)
);
```
