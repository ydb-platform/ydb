...
Не рекомендуется использовать серийные типы данных в качестве первичного ключа: монотонно возрастающие значения приводят к неравномерному распределению данных и перегрузке отдельных партиций. Подробнее — [{#T}](../../../dev/primary-key/row-oriented.md).
