...
Модуль DateTime добавляет новые функции и типы дат в {{ ydb-short-name }}.

## Внутренний формат представления {#resource}

Добавленные типы дат хранятся в памяти во внутреннем формате представления, который {{ ydb-short-name }} не может обрабатывать самостоятельно, без использования модуля DateTime.

Внутренний формат представления — это непрозрачный указатель на ресурс, который можно передавать между пользовательскими функциями (UDF, user defined function). Тип возвращаемого и принимаемого ресурса объявляется внутри функции строковой меткой. При передаче ресурса YQL проверяет совпадение меток, чтобы предотвратить передачу ресурсов между несовместимыми функциями. В случае несовпадения меток происходит ошибка типизации.

Для преобразования внутреннего формате представления в простой тип {{ ydb-short-name }} используются функции [`Make`](#make). Для преобразования простого типа во внутренний формат представления используются функции [`Split`](#split).
...
Получение компоненты [внутреннего представления](#resource).
...
На данный момент поддерживаются только литералы для детерминированных интервалов. Все размерности больше недели — `M` (месяц) и `Y` (год) — не являются фиксированными, поскольку их продолжительность может быть определена только с помощью внешнего контекста (например, конкретного месяца или года).
