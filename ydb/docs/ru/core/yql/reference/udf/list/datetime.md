...
Модуль DateTime добавляет новые функции и типы дат в {{ ydb-short-name }}. Однако, добавленные типы дат хранятся в памяти во внутреннем формате представления, который {{ ydb-short-name }} не может обрабатывать самостоятельно, без использования модуля DateTime. Для преобразования внутреннего формате представления в простой тип {{ ydb-short-name }} и обратно используются функции [`Split`](#split).
...
Преобразование из простого типа {{ ydb-short-name }} во внутреннее представление модуля DateTime. Преобразование всегда выполняется успешно, при условии что на вход функции подаётся требуемое значение.
...
Сборка простого типа {{ ydb-short-name }} из внутреннего представления модуля DateTime. Преобразование всегда выполняется успешно, при условии что на вход функции подаётся требуемое значение.
