...
3. `mergeLambda` - _(опционально)_ позволяет задать функцию для объединения полей (аргументы: имя поля, Optional значение поля в первой структуре, Optional значение поля во второй структуре); по умолчанию выбирается значение поля из первой структуры, а если в первой отсутствует - то из второй
...
Union добавляет в результат все поля обеих структур; Intersection - поля, которые есть в обеих структурах; Difference - поля которые есть в left, но которых нет в right; SymmetricDifference - все поля, которые есть только в одной из структур.
