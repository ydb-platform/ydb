...
### Примеры создания строковой таблицы с дополнительными параметрами

Создание таблицы с включенным автоматическим партиционированием по размеру партиции и предпочитаемым размером каждой партиции 512 мегабайт:
```sql
CREATE TABLE my_table (
    id Uint64,
    title Utf8,
...
Создание таблицы с включенным [TTL](../../../../../concepts/ttl.md) для автоматического удаления устаревших записей в таблице. Запись удаляется, когда значение одной из колонок (настраиваемой пользователем), имеющей тип: `Date`, `Datetime` или `Timestamp` становится больше текущего времени (Epoch time). Время жизни записей может быть продлено (глобально на всю таблицу) на указанное в функции `Interval` значение:
```sql
CREATE TABLE my_table (
    id Uint64,
    title Utf8,
...
В приведенном примере запись будет удалена сразу как значение в колонки `expire_at` совпадёт с `Epoch time`, так как в функции `Interval` указано значение "PT0S" — 0 секунд. 

{% note info %}

В таблице может быть только одна TTL колонка. Если в строке в TTL-колонке указан NULL — такая строка никогда не будет удалена. 
