...
Типы данных, поддерживаемые в колоночных таблицах, и ограничения на их использование в первичных ключах описаны в разделе [колоночные таблицы](../../../../concepts/datamodel/table.md#column-tables). При создании колоночных таблиц обязательно указывается `PRIMARY KEY` с непустым списком колонок, все из которых должны быть помечены как `NOT NULL`.
