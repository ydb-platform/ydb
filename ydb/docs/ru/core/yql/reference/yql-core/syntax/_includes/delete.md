...
Вместо `DELETE FROM` для удаления данных из колоночных таблиц можно воспользоваться механизмом удаления строк по времени — [TTL](../../../../concepts/ttl.md). TTL можно задать при [создании](../create_table.md) строковой или колоночной таблицы (`CREATE TABLE`) или при их [изменении](../alter_table.md) (`ALTER TABLE`).
