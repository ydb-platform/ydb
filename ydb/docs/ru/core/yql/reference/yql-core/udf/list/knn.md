...
Прежде чем мы начнем говорить о приближенных методах, важно помнить, что вам возможно нужно будет измерить, дат ли вам эти методы достаточную точность/полноту или нет.

Имеется множество методов приблеженного поиска. В данном документе приведен пример приблеженного поиска с помощью скалярной квантизации.

Скалярная квантизация это метод компрессии векторов когда множество значений координаты отображаются в множество меньшей размерности.
{{ ydb-short-name }} поддерживает точный поиск по `Float`, `Int8`, `Uint8`, `Bit` векторам.
Соотвественно возможна скалярная квантизация из `Float` в один из этих типов.

Скалярная квантизация уменьшает время необходимое для чтения/записи, поскольку нам нужно читать/записывать меньше байтов в несколько раз.
