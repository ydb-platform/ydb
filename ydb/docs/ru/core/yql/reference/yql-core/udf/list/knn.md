...
Имеется множество методов приближенного поиска. В данном документе приведен пример приближенного поиска с помощью скалярного квантования.

Скалярная квантование это метод компрессии векторов, когда множество значений координаты отображаются в множество меньшей размерности.
{{ ydb-short-name }} поддерживает точный поиск по `Float`, `Int8`, `Uint8`, `Bit` векторам.
Соотвественно, возможна скалярная квантование из `Float` в один из этих типов.

Скалярная квантование уменьшает время необходимое для чтения/записи, поскольку нам нужно читать/записывать меньше байтов в несколько раз.
Например, для `Float` -> `Bit` в `32` раза.

{% note info %}

Важно помнить, что вам, возможно, нужно будет измерить, дают ли вам эти методы достаточную точность/полноту или нет.
...
Квантование может делать ML model, либо это можно сделать средствами YQL.

Дальше идут примеры квантования средствами YQL.
