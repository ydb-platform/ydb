...
namespace NAsyncTest {
    ...
    Y_UNIT_TEST_SUITE(WaitForEvent) {
        ...
        Y_UNIT_TEST(StartedWaiterSynchronousSelfReply) {
            TVector<TString> sequence;
            ui64 seen = 0;
            ...
            auto actor = runtime.StartAsyncActor(state, [&](auto*) -> async<void> {
                ...
                UNIT_ASSERT_VALUES_EQUAL(ev->Cookie, seen);
                sequence.push_back("returning");
            }, handler);
            ...
            // The self-sent reply is queued in the mailbox, deliver it
            actor.Step();
            ASYNC_ASSERT_SEQUENCE(sequence, "returning", "finished");
            UNIT_ASSERT(!state.Destroyed);
        }

        Y_UNIT_TEST(StartedWaiterPoisonWhileWaiting) {
            TVector<TString> sequence;
            ui64 seen = 0;
            ...
            auto actor = runtime.StartAsyncActor(state, [&](auto*) -> async<void> {
                ...
                ASYNC_ASSERT_NO_RETURN(co_await ActorWaitForEvent<TEvents::TEvWakeup>([&](IActor&, ui64 cookie) {
                    seen = cookie;
                }));
            }, handler);
