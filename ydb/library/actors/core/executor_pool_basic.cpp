...
namespace NActors {
    ...
    TMailbox* TBasicExecutorPool::GetReadyActivationRingQueue(ui64 revolvingCounter) {
        return GetReadyActivationRingQueueImpl(Activations, revolvingCounter);
    }

    TMailbox* TBasicExecutorPool::GetReadyActivationWaker(ui64 revolvingCounter) {
        return GetReadyActivationWakerImpl(Activations, revolvingCounter);
    }

    TMailbox* TBasicExecutorPool::GetReadyActivation(ui64 revolvingCounter) {
        EXECUTOR_POOL_BASIC_DEBUG(EDebugLevel::Executor, "get ready activation");
        return GetReadyActivationWithQueue(Activations, revolvingCounter);
    }
    ...
    void TBasicExecutorPool::WakeUpLoop(i16 currentThreadCount) {
        for (i16 i = 0;;) {
            if (Threads[i].WakeUp()) {
    ...
    void TBasicExecutorPool::ScheduleActivationExRingQueue(TMailbox* mailbox, ui64 revolvingCounter, std::optional<TAtomic> initSemaphore) {
        ScheduleActivationExRingQueueImpl(Activations, mailbox, revolvingCounter, initSemaphore);
    }
    ...
    void TBasicExecutorPool::ScheduleActivationExWaker(TMailbox* mailbox, ui64 revolvingCounter) {
        ScheduleActivationExWakerImpl(Activations, mailbox, revolvingCounter);
    }
...
// version in Ubuntu 12.04
    ...
    TMailbox* TBasicExecutorPool::GetReadyActivationShared(ui64 revolvingCounter) {
        // Reuse the existing executor interface for the pool's scheduling policy.
        // GetReadyActivationWithQueue selects the shared worker accounting path.
        Y_DEBUG_ABORT_UNLESS(TlsThreadContext->IsShared());
        return GetReadyActivation(revolvingCounter);
    }
