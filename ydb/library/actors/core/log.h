...
#define AFL_VERIFY(condition) if (condition) { static_assert(!std::is_array_v<std::remove_cvref_t<decltype(condition)>>, "The value of an array type is always true"); } else [[unlikely]] NActors::TVerifyFormattedRecordWriter(#condition)("fline", TStringBuilder() << TStringBuf(__SOURCE_FILE__) << ":" << __LINE__)
#define AFL_ENSURE(condition) if (condition) { static_assert(!std::is_array_v<std::remove_cvref_t<decltype(condition)>>, "The value of an array type is always true"); } else [[unlikely]] NActors::TEnsureFormattedRecordWriter(#condition)("fline", TStringBuilder() << TStringBuf(__SOURCE_FILE__) << ":" << __LINE__)

#ifndef NDEBUG
/// Assert that depend on NDEBUG macro and outputs message like printf
#define AFL_VERIFY_DEBUG AFL_VERIFY
#else
#define AFL_VERIFY_DEBUG(condition) if (true); else NActors::TVerifyFormattedRecordWriter(#condition)("fline", TStringBuilder() << TStringBuf(__SOURCE_FILE__) << ":" << __LINE__)
#endif
...
#define ACTORS_FORMATTED_LOG(mPriority, mComponent) \
    if (NActors::TlsActivationContext && !IS_LOG_PRIORITY_ENABLED(mPriority, mComponent));\
            ...
            )("fline", TStringBuilder() << TStringBuf(__SOURCE_FILE__) << ":" << __LINE__)
