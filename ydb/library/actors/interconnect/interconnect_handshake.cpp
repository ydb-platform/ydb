    ...
    public:
        ...
        void PerformOutgoingHandshake() {
            ...
            if (!PeerVirtualId) {
                ...
                for (const auto& item : success.GetParams()) {
                    params.emplace(item.GetKey(), item.GetValue());
                }
                auto& actors = Common->ConnectionCheckerActorIds;
                for (size_t i = 0; i < actors.size(); ++i) {
                    Send(actors[i], new TEvInterconnect::TEvNotifyOutgoingConnectionEstablished(i + 1 != actors.size()
                        ? THashMap(params)
                        : std::move(params)));
                }
                ...
                if (auto ev = reply->CastAsLocal<TEvHandshakeReplyOK>()) {
                    ...
                    while (pending--) {
                        ...
                        if (const auto& errorReason = ev->Get()->ErrorReason) {
                            generateError(TStringBuilder() << "connection checker refused to proceed with connection: "
                                << *errorReason);
                        }
                        for (auto&& [key, value] : ev->Get()->ParamsToSend) {
