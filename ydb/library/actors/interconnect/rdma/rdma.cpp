...
void TIbVerbsBuilderImpl::AddSendVerb(const TRcBuf& packet,
    std::function<void(NActors::TActorSystem* as, TEvRdmaIoDone*)> ioCb) noexcept
{
    ...
    AddSendVerb(span, memReg.GetMemRegion(), std::move(ioCb));
}

void TIbVerbsBuilderImpl::AddSendVerb(TContiguousSpan packet, const TMemRegion* memRegion,
    std::function<void(NActors::TActorSystem* as, TEvRdmaIoDone*)> ioCb) noexcept
{
    const TSendSge sg {
        .Data = packet.data(),
