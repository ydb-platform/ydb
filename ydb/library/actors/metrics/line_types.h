...
namespace NActors {
    ...
    namespace NInMemoryMetricsPrivate {
        // Bound each maintenance stage so the manager can yield to other work.
        inline constexpr ui32 MaintenanceBatchSize = 64;
    }
    ...
    struct TInMemoryMetricsConfig {
        ...
        // At capacity, a closed line and its retained history may be evicted.
        ui32 MaxLines = 0;
        // Ready chunks per line, excluding its current chunk (managed backend).
        ui32 ReserveChunks = 1;
        // Bound queued registration/snapshot/label requests before actor dispatch.
        ui32 MaxPendingRequests = 1024;
        // Target free physical chunks; refill below half this level. Zero disables.
        ui32 FreeChunkReservePercent = 5;
