...
/*
template <ui32 MaxSizeBits, ui32 ThreadBits>
struct TMPMCBlockingRingQueue {
    static constexpr std::optional<ui32> TestSize = std::nullopt;
    static constexpr ui32 MaxSize = 1 << MaxSizeBits;
    static constexpr ui32 MaxThreadCount = 1 << ThreadBits;
    ...
    struct alignas(ui64) TSlot {
        static constexpr ui64 EmptyBit = 1ull << 63;
        static constexpr ui64 ClaimBit = 1ull << 62;
        static constexpr ui64 BlockingBit = 1ull << 62;
        static constexpr ui64 BlockingIdMask = ((1ull << ThreadBits) - 1) << (61 - ThreadBits);
        static constexpr ui64 InternalBits = EmptyBit | ClaimBit | BlockingBit | BlockingIdMask;
        ui64 Generation = 0;
        ui64 Value = 0;
        ui64 BlockingId = 0;
        bool IsEmpty = true;
        ...
        static constexpr ui64 MakeBlockedClaim(ui64 generation, ui64 blockingId) {
            return EmptyBit | BlockingBit | (blockingId << (61 - ThreadBits)) | generation;
        }

        static constexpr TSlot Recognise(ui64 slotValue) {
            if (slotValue & ClaimBit) {
                return {.Generation = (~InternalBits & slotValue), .IsClaim=true};
            }
            if (slotValue & BlockedBit) {
                return {.Generation = (~InternalBits & slotValue), .BlockingId=((slotValue & BlockingIdMask) >> (61 - ThreadBits)), .IsClaim=true, .IsBlocked=true};
            }
            if ((slotValue & EmptyBit) == 0) {
                return {.Value= (~InternalBits & slotValue), .IsEmpty=false};
            }
            return {.Generation = (~InternalBits & slotValue)};
        }
    };
