...
namespace { // Tests
    ...
    struct TTestCases {
        ...
        static auto GetHead(const TMPMCRingQueueV1WithStats<SizeBits> &realQueue) {
            return realQueue.Head.load();
        }

        static auto GetHeadGeneration(const TMPMCRingQueueV1WithStats<SizeBits> &realQueue) {
            return realQueue.Head.load() / MaxSize;
        }

        static auto GetTail(const TMPMCRingQueueV1WithStats<SizeBits> &realQueue) {
            return realQueue.Tail.load();
        }

        static auto GetTailGeneration(const TMPMCRingQueueV1WithStats<SizeBits> &realQueue) {
            return realQueue.Tail.load() / MaxSize;
        }

        static void PushesPopsWithShift() {
            TMPMCRingQueueV1WithStats<SizeBits> realQueue;
            ...
            for (ui32 it = 0; it < MaxSize; ++it) {
                for (ui32 idx = 0; idx < MaxSize - 1; ++idx) {
                    ...
                    ui64 emptyCurrentGeneration = (ui64(1) << 63);
                ...
                for (ui32 idx = 0; idx < MaxSize - 1; ++idx) {
                    TString debugString = TStringBuilder() << "iteration:" << it << " pos: " << idx << "/" << MaxSize;
                    ui64 emptyNextGeneration = (ui64(1) << 63);
                    auto head = GetHead(realQueue);
                    auto tail = GetTail(realQueue);
        ...
        static void CheckPushes() {
            ...
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[0].load(), emptyZeroGeneration);
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[1].load(), emptyZeroGeneration);
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[2].load(), emptyZeroGeneration);
            UNIT_ASSERT(realQueue.TryPush(0));
            UNIT_ASSERT(realQueue.TryPush(1));
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[0].load(), 0u);
        ...
        static void CheckOvertakenPushes() {
            ...
            ui64 emptyZeroGeneration = (ui64(1) << 63);
            ui64 overtakenGeneration = (ui64(1) << 62);
            ...
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[0].load(), overtakenGeneration);
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[1].load(), emptyZeroGeneration);
            UNIT_ASSERT_VALUES_EQUAL(realQueue.Buffer[2].load(), emptyZeroGeneration);
...
Y_UNIT_TEST_SUITE(MPMCRingQueueV1SingleThreadTests) {
    ...
    Y_UNIT_TEST(RandomUsage) {
        TMPMCRingQueueV1WithStats<SizeBits> realQueue;
        TestRandomUsage(1024, (ui64(1) << SizeBits), TIdAdaptor<TMPMCRingQueueV1WithStats<SizeBits>>(&realQueue));
    }
}
