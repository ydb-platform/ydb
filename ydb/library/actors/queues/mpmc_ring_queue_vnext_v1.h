...
// OBSERVE
    ...
    NThreading::TPadded<std::atomic<ui64>> ReadInFlight{0};
    ...
    TMPMCRingQueueVNextV1(ui32 readersCount)
    ...
    std::optional<ui32> TryPop() {
        ...
        if (queueMode == EQueueMode::Overtaken) {
            ...
            ui64 currentInflight = ReadInFlight.load(std::memory_order_acquire);
            ui64 waitMultiplier = 1;
            while (true) {
                if (currentInflight < 10) {
                    if (ReadInFlight.compare_exchange_strong(currentInflight, currentInflight + 1, std::memory_order_acq_rel)) {
                        break;
                    }
                }

                if (waitMultiplier < 4 && currentInflight < 4) {
                    SpinLockPause();
                    waitMultiplier *= 2;
                } else {
                    NanoSleep(100 * waitMultiplier);
                    waitMultiplier = Min<ui64>(1024, waitMultiplier * 2);
                }
                currentInflight = ReadInFlight.load(std::memory_order_acquire);
            }
            ...
            if (slot.IsEmpty) {
                ...
                ReadInFlight.fetch_sub(1, std::memory_order_relaxed);
                return std::nullopt;
            }
            ...
            if (currentHead + ReadersCount <= currentTail) {
                ...
                ReadInFlight.fetch_sub(1, std::memory_order_release);
                return std::nullopt;
            }
        }
