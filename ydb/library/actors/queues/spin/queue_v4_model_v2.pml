...
chan overtaken_queue = [ELEMENTS] of { int };
...
push_begin_change_slot:
        atomic {
            ...
            :: buffer[slot_idx].state == SLOT_EMPTY ->
                ...
                buffer[slot_idx].state = SLOT_HAS_VALUE;
            ...
            :: buffer[slot_idx].state == SLOT_OVERTAKEN ->
                ...
                :: buffer[slot_idx].value == 1 ->
                    ...
                    printf("Producer %d: make slot empty slot_idx=%d\n", id, slot_idx);
                :: buffer[slot_idx].value > 1 ->
            ...
            :: else ->
                ...
                buffer[slot_idx].write_overtaken = buffer[slot_idx].write_overtaken + 1;
                success = false;
                goto push_end;
            fi
        }
...
pop_begin_change_slot:
        atomic {
            ...
            :: buffer[slot_idx].state == SLOT_EMPTY ->
                if
                :: buffer[slot_idx].write_overtaken > 0 ->
                    buffer[slot_idx].write_overtaken = buffer[slot_idx].write_overtaken - 1;
                    success = false;
                    printf("Consumer %d: make slot_idx=%d write_overtaken from %d to %d\n", id, slot_idx, buffer[slot_idx].write_overtaken + 1, buffer[slot_idx].write_overtaken);
                :: else ->
                    // Implements: if (slot.IsEmpty) { if (currentSlot.compare_exchange_weak(expected, TSlot::MakeOvertaken())) { break; } }
                    buffer[slot_idx].state = SLOT_OVERTAKEN;
                    buffer[slot_idx].value = 1;
                    success = false;
                    printf("Consumer %d: make slot_idx=%d overtaken\n", id, slot_idx);
                fi
                goto pop_check_tail;
            :: buffer[slot_idx].state == SLOT_HAS_VALUE ->
            ...
            :: buffer[slot_idx].state == SLOT_OVERTAKEN ->
                ...
                buffer[slot_idx].value = buffer[slot_idx].value + 1;
                printf("Consumer %d: failed to pop from slot_idx=%d (slot is overtaken) overtaken_value=%d\n", id, slot_idx, buffer[slot_idx].value);
                success = false;
                goto pop_end;
            fi
        }
    ...
    :: pushed_elements < ELEMENTS ->
        ...
        atomic {
            ...
            :: overflow ->
                ...
                if
                :: pushed_elements == popped_elements && ELEMENTS - planned_pushed_elements == pushed_elements ->
...
ltl all {
    ...
    //[] (buffer[0].state == SLOT_OVERTAKEN -> buffer[0].value != 0) &&
    //[] (buffer[1].state == SLOT_OVERTAKEN -> buffer[1].value != 0) &&
    true
}
