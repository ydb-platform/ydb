...
struct TModulo {
    ...
    static constexpr EnableIfInteger<TRes> Call(arrow20::compute::KernelContext*, TArg0 lhs, TArg1 rhs, arrow20::Status* st) {
        static_assert(std::is_same<TRes, TArg0>::value && std::is_same<TRes, TArg1>::value, "");
        if (ARROW_PREDICT_FALSE(rhs == 0)) {
            *st = arrow20::Status::Invalid("divide by zero");
            return 0;
        }
    ...
    template <typename TRes, typename TArg0, typename TArg1>
    static constexpr EnableIfFloatingPoint<TRes> Call(arrow20::compute::KernelContext*, TArg0 lhs, TArg1 rhs, arrow20::Status* st) {
        static_assert(std::is_same<TRes, TArg0>::value && std::is_same<TRes, TArg1>::value, "");
