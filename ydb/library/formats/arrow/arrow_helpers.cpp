...
TInstant ArrowTimestampToInstant(const arrow20::TimestampArray& timestamp, const ui32 position) {
    int64_t micros = timestamp.Value(position);
    const auto unit = std::static_pointer_cast<arrow20::TimestampType>(timestamp.type())->unit();
    if (unit == arrow20::TimeUnit::SECOND) {
        return TInstant::MicroSeconds(micros * 1000000LL);
    } else if (unit == arrow20::TimeUnit::MILLI) {
        return TInstant::MicroSeconds(micros * 1000LL);
    } else if (unit == arrow20::TimeUnit::MICRO) {
        return TInstant::MicroSeconds(micros);
    } else if (unit == arrow20::TimeUnit::NANO) {
        return TInstant::MicroSeconds(micros / 1000LL);
    }
...
TString DebugString(std::shared_ptr<arrow20::Array> array, const ui32 position, const NKikimr::NScheme::TTypeInfo* logicalType) {
    if (!array) {
        return "_NO_DATA";
    }
    ...
    SwitchType(array->type_id(), [&](const auto& type) {
        ...
        if constexpr (std::is_same_v<typename TWrap::T, arrow20::BooleanType>) {
            result << (column.Value(position) ? "true" : "false");
        } else if constexpr (std::is_same_v<typename TWrap::T, arrow20::TimestampType>) {
            const auto& tsArr = static_cast<const arrow20::TimestampArray&>(column);
            result << ArrowTimestampToInstant(tsArr, position).ToString();
        } else if constexpr (arrow20::has_string_view<typename TWrap::T>()) {
            auto value = column.GetString(position);
            result << TString(value.data(), value.size());
