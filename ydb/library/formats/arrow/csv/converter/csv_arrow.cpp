    ...
    , NotNullColumns(notNullColumns)
{
    ...
    auto SetOptionsForColumns = [&](const auto& col) -> decltype(auto) {
        if (col.Precision > 0) {
            ...
            ConvertOptions.column_types[col.Name] = arrow20::boolean();
        } else {
...
std::shared_ptr<arrow20::RecordBatch> TArrowCSV::ConvertColumnTypes(std::shared_ptr<arrow20::RecordBatch> parsedBatch) const {
    ...
    for (const auto& f : schema->fields()) {
        ...
        } else if (fArr->type()->id() == arrow20::BooleanType::type_id && originalType->id() == arrow20::UInt8Type::type_id) {
            auto boolArray = std::static_pointer_cast<arrow20::BooleanArray>(fArr);
            arrow20::UInt8Builder builder;
