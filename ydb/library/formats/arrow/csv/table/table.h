...
public:
    ...
    static arrow20::Result<TArrowCSV> Create(const std::vector<NYdb::NTable::TTableColumn>& columns, bool header = false);

private:
    static NYdb::TTypeParser ExtractType(const NYdb::TType& type);
    static arrow20::Result<std::shared_ptr<arrow20::DataType>> GetArrowType(const NYdb::TType& type);
    static arrow20::Result<std::shared_ptr<arrow20::DataType>> GetCSVArrowType(const NYdb::TType& type);
};
