...
TConclusion<TString> TSerializer::SerializePayloadToString(const std::shared_ptr<arrow20::Scalar>& scalar) {
    ...
    const bool resultFlag = NArrow::SwitchType(scalar->type->id(), [&](const auto& type) {
        ...
        if constexpr (arrow20::has_c_type<T>()) {
            ...
            using CTypeScalar = typename arrow20::TypeTraits<T>::ScalarType;
            ...
            return true;
        } else if constexpr (arrow20::has_string_view<T>()) {
            using StringScalarType = typename arrow20::TypeTraits<T>::ScalarType;
...
TConclusion<std::shared_ptr<arrow20::Scalar>> TSerializer::DeserializeFromStringWithPayload(
    ...
    const bool resultFlag = NArrow::SwitchType(dataType->id(), [&](const auto& type) {
        ...
        if constexpr (arrow20::has_c_type<T>()) {
            using CType = typename T::c_type;
            using ScalarType = typename arrow20::TypeTraits<T>::ScalarType;
            ...
            using ScalarType = typename arrow20::TypeTraits<T>::ScalarType;
            result = std::make_shared<ScalarType>(arrow20::Buffer::FromString(std::string(data.data(), data.size())), dataType);
            return true;
        } else {
            return false;
        }
