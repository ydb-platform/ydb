...
/*
* Implementation of SASLprep normalization is inspired by PostgreSQL sources
* placed in /yql/essentials/parser/pg_wrapper/postgresql/src/common/saslprep.c
*/
...
*/
constexpr std::array<wchar32, 12> NON_ASCII_SPACE_RANGES = {
    0x00A0, 0x00A0,
    0x1680, 0x1680,
    0x2000, 0x200B,
    0x202F, 0x202F,
...
*/
constexpr std::array<wchar32, 16> COMMONLY_MAPPED_TO_NOTHING_RANGES = {
    0x00AD, 0x00AD,
    0x034F, 0x034F,
    0x1806, 0x1806,
    0x180B, 0x180D,
 ...
 */
constexpr std::array<wchar32, 72> PROHIBITED_OUTPUT_RANGES = {
	0x0000, 0x001F,				/* C.2.1 */
...
constexpr std::array<wchar32, 792> UNASSIGNED_CODEPOINT_RANGES = {
	0x0221, 0x0221,
	0x0234, 0x024F,
	0x02AE, 0x02AF,
	0x02EF, 0x02FF,
...
constexpr std::array<wchar32, 68> RAND_ALCAT_CODEPOINT_RANGES = {
	0x05BE, 0x05BE,
	0x05C0, 0x05C0,
	0x05C3, 0x05C3,
	0x05D0, 0x05EA,
...
constexpr std::array<wchar32, 720> LCAT_CODEPOINT_RANGES = {
	0x0041, 0x005A,
	0x0061, 0x007A,
	0x00AA, 0x00AA,
	0x00B5, 0x00B5,
...
int CodepointRangeCmp(const void* a, const void* b) {
    const wchar32* key = static_cast<const wchar32*>(a);
    const wchar32* range = static_cast<const wchar32*>(b);
...
bool IsCodeInTable(wchar32 code, const wchar32* map, size_t mapsize) {
    Y_ASSERT(mapsize % 2 == 0);
    if (code < map[0] || code > map[mapsize - 1]) {
        return false;
    }

    return bsearch(&code, map, mapsize / 2, sizeof(wchar32) * 2, CodepointRangeCmp);
}
...
/*
 * SASLprep requires the input to be in UTF-8 encoding, but YDB
 * supports many encodings, so we don't blindly assume that. SaslPrep
 * will check if the input looks like valid UTF-8, and returns
 * ESaslPrepReturnCodes::InvalidUTF8 if not.
 *
 ...
 * returns ESaslPrepReturnCodes::Prohibited.
 *
 * On success, returns ESaslPrepReturnCodes::Success, and the normalized string in
 * result.
 */
ESaslPrepReturnCodes SaslPrep(const std::string& str, std::string& result) {
    ...
    */
    if (IsAsciiString(str)) {
        result = str;
        return ESaslPrepReturnCodes::Success;
    }
    ...
    */
    if (!IsUtf(str)) {
        return ESaslPrepReturnCodes::InvalidUTF8;
    }

    std::basic_string<wchar32> wideStr = UTF8ToUTF32<true>(str);
    ...
    */
    ...
    for (size_t i = 0; i < wideStr.size(); ++i) {
        wchar32 code = wideStr[i];
        if (IsCodeInTable(code, NON_ASCII_SPACE_RANGES.data(), NON_ASCII_SPACE_RANGES.size())) {
            wideStr[newSize++] = 0x0020;
        } else if (IsCodeInTable(code, COMMONLY_MAPPED_TO_NOTHING_RANGES.data(), COMMONLY_MAPPED_TO_NOTHING_RANGES.size())) {
            /* map to nothing */
            continue;
        } else {
            wideStr[newSize++] = code;
        }
    }
    ...
    * normalization.
    */
    std::basic_string<wchar32> normalizedWideStr = Normalize<NUnicode::NFKC, wchar32>(wideStr);

    /*
    * 3) Prohibit -- Check for any characters that are not allowed in the
    * input. If any are found, return an error.
    */
    for (const wchar32& code : wideStr) {
        if (IsCodeInTable(code, PROHIBITED_OUTPUT_RANGES.data(), PROHIBITED_OUTPUT_RANGES.size())) {
            return ESaslPrepReturnCodes::Prohibited;
        }

        if (IsCodeInTable(code, UNASSIGNED_CODEPOINT_RANGES.data(), UNASSIGNED_CODEPOINT_RANGES.size())) {
            return ESaslPrepReturnCodes::Prohibited;
        }
    }
    ...
    */
    ...
    for (const wchar32& code : wideStr) {
        if (IsCodeInTable(code, RAND_ALCAT_CODEPOINT_RANGES.data(), RAND_ALCAT_CODEPOINT_RANGES.size())) {
    ...
    if (containsRandALCat) {
        for (const wchar32& code : wideStr) {
            if (IsCodeInTable(code, LCAT_CODEPOINT_RANGES.data(), LCAT_CODEPOINT_RANGES.size())) {
                return ESaslPrepReturnCodes::Prohibited;
            }
        }

        if (!IsCodeInTable(wideStr.front(), RAND_ALCAT_CODEPOINT_RANGES.data(), RAND_ALCAT_CODEPOINT_RANGES.size())
            || !IsCodeInTable(wideStr.back(), RAND_ALCAT_CODEPOINT_RANGES.data(), RAND_ALCAT_CODEPOINT_RANGES.size())) {
            return ESaslPrepReturnCodes::Prohibited;
        }
    }
    ...
    */
    result = WideToUTF8(normalizedWideStr);
    return ESaslPrepReturnCodes::Success;
}
