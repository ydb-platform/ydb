...
// Keep libc's NGROUPS_MAX visible to the rest of the translation unit while
// still allowing linux/uapi headers pulled by liburing to define their own.
// Pull limits.h up front so its include guard is set: liburing drags in
// <atomic> (via liburing/barrier.h) after linux/fs.h has defined NGROUPS_MAX,
// and libc++'s <atomic> reaches <climits>. Without this, musl's limits.h is
// first processed there and redefines NGROUPS_MAX (32 vs the uapi 65536),
// which is -Wmacro-redefined and, under -Werror, a build failure.
#include <limits.h>
