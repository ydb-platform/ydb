...
TDistrictLoadQueryGenerator::TDistrictLoadQueryGenerator(TLoadParams& params, TLog& log, ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::district, log, seed)
{
    WarehouseId = 1;
    DistrictId = 1;
}
...
NYdb::TValue TDistrictLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    for (i32 id = 1; id <= Params.LoadBatchSize; ++id) {
        ...
        rows.AddMember("D_NEXT_O_ID").Int32(EWorkloadConstants::TPCC_CUST_PER_DIST + 1);
