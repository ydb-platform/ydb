...
TItemLoadQueryGenerator::TItemLoadQueryGenerator(TLoadParams& params, TLog& log, ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::item, log, seed)
{
    ItemId = 1;
}

bool TItemLoadQueryGenerator::Finished() {
    return (ItemId > EWorkloadConstants::TPCC_ITEM_COUNT);
}
...
NYdb::TValue TItemLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    i32 partEnd = std::min(static_cast<i32>(EWorkloadConstants::TPCC_ITEM_COUNT),
