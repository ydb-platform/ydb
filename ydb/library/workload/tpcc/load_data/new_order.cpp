...
TNewOrderLoadQueryGenerator::TNewOrderLoadQueryGenerator(TLoadParams& params, ui32 partNum, TLog& log, ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::new_order, log, seed)
    , PartNum(partNum)
{
    ...
    CustomerId = EWorkloadConstants::TPCC_FIRST_UNPROCESSED_O_ID;
}
...
NYdb::TValue TNewOrderLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    for (i32 id = 1; id <= Params.LoadBatchSize; ++id) {
        ...
        CustomerId++;
        if (CustomerId > EWorkloadConstants::TPCC_CUST_PER_DIST) {
            CustomerId = EWorkloadConstants::TPCC_FIRST_UNPROCESSED_O_ID;
            DistrictId++;
