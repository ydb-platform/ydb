...
TOorderLoadQueryGenerator::TOorderLoadQueryGenerator(TLoadParams& params,
                                                     ui32 partNum, TLog& log,
                                                     ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::oorder, log, seed)
    , PartNum(partNum)
    , CustShuffle(EWorkloadConstants::TPCC_CUST_PER_DIST)
{
...
NYdb::TValue TOorderLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    for (i32 id = 1; id <= Params.LoadBatchSize; ++id) {
        ...
        if (CustomerId + 1 < EWorkloadConstants::TPCC_FIRST_UNPROCESSED_O_ID) {
