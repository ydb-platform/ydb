...
TOrderLineLoadQueryGenerator::TOrderLineLoadQueryGenerator(TLoadParams& params, ui32 partNum, TLog& log, ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::order_line, log, seed)
    , PartNum(partNum)
{
...
NYdb::TValue TOrderLineLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    for (i32 id = 1; id <= Params.LoadBatchSize; ++id) {
        ...
        ui64 itemId = UniformRandom32(1, EWorkloadConstants::TPCC_ITEM_COUNT, Rng);
        rows.AddMember("OL_I_ID").Int32(itemId);
        if (itemId < EWorkloadConstants::TPCC_FIRST_UNPROCESSED_O_ID) {
