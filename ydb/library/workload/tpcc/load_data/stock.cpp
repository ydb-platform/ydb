...
TStockLoadQueryGenerator::TStockLoadQueryGenerator(TLoadParams& params, ui32 partNum, TLog& log, ui64 seed)
    : TLoadQueryGenerator(params, ETablesType::stock, log, seed)
    , PartNum(partNum)
{
...
NYdb::TValue TStockLoadQueryGenerator::GetNextBatchLoadDDL() {
    ...
    for (i32 id = 1; id <= Params.LoadBatchSize; ++id) {
        ...
        rows.AddMember("S_YTD").Double(0.0);
        ...
        ItemId++;
        if (ItemId > EWorkloadConstants::TPCC_ITEM_COUNT) {
            ItemId = 1;
            WarehouseId++;
            if (Finished()) {
                break;
