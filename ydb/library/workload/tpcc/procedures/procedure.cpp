...
NThreading::TFuture<void> IProcedure::RunStages() {
    ...
    return Terminal.ApplyByThreadPool(currentStageF, [this, sessionF](const TFuture<void>& future) {
            if (future.HasException() && !Finished() && Transaction.Defined() && Transaction->IsActive()) {
                return Transaction->Rollback().Apply([this, future](const TAsyncStatus& rollbackF) {
                    if (DebugMode && !rollbackF.GetValue().IsSuccess()) {
                        TStringStream stream;
                        stream << "Terminal ID=" << Terminal.GetTerminalId() << ": ";
                        stream << Type << ": Rollback failed";
                        stream << " (Transaction ID=" << Transaction->GetId() << ")";
