...
TTerminal::TTerminal(const TRunParams& params,
    ...
    , FastRetryOption(params.RetryCount,
                      TDuration::Zero(),
                      TDuration::MilliSeconds(EWorkloadConstants::TPCC_FAST_RETRY_RANDOM_DELTA),
                      TDuration::Zero(),
                      TDuration::MilliSeconds(EWorkloadConstants::TPCC_FAST_RETRY_EXP_MULTIPLIER))
    , SlowRetryOption(params.RetryCount,
                      TDuration::Zero(),
                      TDuration::MilliSeconds(EWorkloadConstants::TPCC_SLOW_RETRY_RANDOM_DELTA),
                      TDuration::Zero(),
...
void TTerminal::Process(void*) {
    ...
    // Initialization after receiving the TableClient
    if (Procedure.get() == nullptr) {
        NextProcedure();
    }
...
void TTerminal::FinishAndGetNextProcedure() {
    ...
    Scheduler->Add(MakeIntrusive<TSchedulerTerminalSubtask>(promise), expire);
    
    promise.GetFuture().Apply([this](const TFuture<void>& future) {
        try {
            future.GetValue();
            ...
            if (Params.DebugMode) {
                TStringStream stream;
                stream << "After scheduler terminal subtask: " << ex.what();
                Log.Write(TLOG_ERR, stream.Str());
            }
            throw yexception() << "After scheduler terminal subtask: " << ex.what();
        }
    });
}
