...
public:
    TServiceAccountServiceFixture() {
        ...
        ServiceAccountService = NCloud::CreateServiceAccountService("localhost:" + ToString(servicePort), "service_account_service_ut");
...
Y_UNIT_TEST_SUITE_F(TServiceAccountServiceTest, TServiceAccountServiceFixture) {
    Y_UNIT_TEST(Get) {
        ...
        with_lock (ServiceAccountServiceMock.MetadataMutex) {
            UNIT_ASSERT_STRING_CONTAINS(ServiceAccountServiceMock.CapturedUserAgent, "ydb-service_account_service_ut/");
        }
    }
