...
Y_UNIT_TEST_SUITE(TUserAccountServiceTest) {
    Y_UNIT_TEST(Get) {
        ...
        IActor* userAccountService = NCloud::CreateUserAccountService("localhost:" + ToString(servicePort), "user_account_service_ut");
        ...
        with_lock (userAccountServiceMock.MetadataMutex) {
            UNIT_ASSERT_STRING_CONTAINS(userAccountServiceMock.CapturedUserAgent, "ydb-user_account_service_ut/");
        }
    }
}
