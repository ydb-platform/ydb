...
Y_UNIT_TEST_SUITE(TUserAccountServiceTest) {
    Y_UNIT_TEST(Get) {
        ...
        IActor* userAccountService = NCloud::CreateUserAccountService("localhost:" + ToString(servicePort), "ydb-ua");
        ...
        with_lock (userAccountServiceMock.MetadataMutex) {
            UNIT_ASSERT_STRING_CONTAINS(userAccountServiceMock.CapturedUserAgent, "ydb-ua/");
        }
    }
}
