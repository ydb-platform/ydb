...
bool TDqComputeActorCheckpoints::ShouldIgnoreOldCoordinator(const E& ev, bool verifyOnGenerationFromFuture) {
    ...
    if (CheckpointCoordinator && generation < CheckpointCoordinator->Generation) {
        LOG_WARN_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Ignoring event " << ev->Get()->ToStringHeader() << " from previous coordinator: "
...
void TDqComputeActorCheckpoints::Handle(TEvDqCompute::TEvNewCheckpointCoordinator::TPtr& ev) {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Got TEvNewCheckpointCoordinator event: generation " << newGeneration << ", actorId: " << ev->Sender);
    ...
    if (CheckpointCoordinator) {
        LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Replace stale checkpoint coordinator (generation = " << CheckpointCoordinator->Generation << ") with a new one");
    } else {
        LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Assign checkpoint coordinator (generation = " << newGeneration << ")");
    }
    ...
    AbortCheckpoint();
    if (resumeInputs) {
        LOG_WARN_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Drop pending checkpoint since coordinator is stale");
...
void TDqComputeActorCheckpoints::Handle(TEvDqCompute::TEvInjectCheckpoint::TPtr& ev) {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"TEvInjectCheckpoint");
...
void TDqComputeActorCheckpoints::Handle(TEvDqCompute::TEvRestoreFromCheckpoint::TPtr& ev) {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(checkpoint) << "] " << "TEvRestoreFromCheckpoint, StateLoadPlan = " << StateLoadPlan);
    ...
    default:
        {
            ...
            LOG_ERROR_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(checkpoint) << "] " << message);
            NYql::TIssues issues;
...
void TDqComputeActorCheckpoints::Handle(TEvDqCompute::TEvGetTaskStateResult::TPtr& ev) {
    ...
    if (!ev->Get()->Issues.Empty()) {
        LOG_ERROR_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(checkpoint) << "] " << "TEvGetTaskStateResult error: " << ev->Get()->Issues.ToOneLineString());
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(checkpoint) << "] " << "TEvGetTaskStateResult: restoring state");
...
void TDqComputeActorCheckpoints::AfterStateLoading(const TMaybe<TString>& error) {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(checkpoint) << "] " << "Checkpoint state restored");
}
...
void TDqComputeActorCheckpoints::Handle(NActors::TEvents::TEvPoison::TPtr&) {
    LOG_INFO_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Pass Away");
    PassAway();
}

void TDqComputeActorCheckpoints::Handle(NActors::TEvInterconnect::TEvNodeDisconnected::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Handle disconnected node " << ev->Get()->NodeId);
...
void TDqComputeActorCheckpoints::Handle(NActors::TEvInterconnect::TEvNodeConnected::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Handle connected node " << ev->Get()->NodeId);
...
void TDqComputeActorCheckpoints::Handle(NActors::TEvents::TEvUndelivered::TPtr& ev) {
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Handle undelivered");
    if (EventsQueue.HandleUndelivered(ev) != NYql::NDq::TRetryEventsQueue::ESessionState::WrongSession) {
        LOG_ERROR_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"TEvUndelivered: " << ev->Get()->SourceType);
    }
}
...
void TDqComputeActorCheckpoints::Handle(NActors::TEvents::TEvWakeup::TPtr&) {
    if (CheckpointStartTime && (TActivationContext::Now() - CheckpointStartTime) >= SLOW_CHECKPOINT_DURATION) {
        ...
        LOG_WARN_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<checkpointDiagnostic);
    }
...
void TDqComputeActorCheckpoints::DoCheckpoint() {
    ...
    LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"Performing task checkpoint");
    if (SaveState()) {
        LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"Injecting checkpoint barrier to outputs");
...
bool TDqComputeActorCheckpoints::SaveState() {
    try {
        ...
        AbortCheckpoint();
        LOG_ERROR_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"Failed to save state: " << e.what());
    ...
    LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"CA state saved");
    return true;
}
...
void TDqComputeActorCheckpoints::RegisterCheckpoint(const NDqProto::TCheckpoint& checkpoint, ui64 channelId) {
    ...
    LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"Got checkpoint barrier from channel " << channelId);
...
void TDqComputeActorCheckpoints::OnSinkStateSaved(TSinkState&& state, ui64 outputIndex, const NDqProto::TCheckpoint& checkpoint) {
    ...
    if (checkpoint.GetGeneration() < CheckpointCoordinator->Generation) {
        LOG_WARN_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Ignoring sink[" << outputIndex << "] state saved event from previous coordinator: "
    ...
    LOG_TRACE_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " <<"Sink[" << outputIndex << "] state saved");
...
void TDqComputeActorCheckpoints::TryToSavePendingCheckpoint() {
    ...
    if (PendingCheckpoint.IsReady()) {
        ...
        LOG_DEBUG_S(*NActors::TlsActivationContext, NKikimrServices::KQP_COMPUTE, "[" << GraphId << "] Task: " << Task.GetId() << ". " << "[Checkpoint " << MakeStringForLog(*PendingCheckpoint.Checkpoint) << "] " <<"Task checkpoint is done. Send to storage");
        PendingCheckpoint.Clear();
