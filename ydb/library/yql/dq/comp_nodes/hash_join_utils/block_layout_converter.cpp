...
std::unique_ptr<arrow20::Buffer> MakeBufferWithSize(int size, arrow20::MemoryPool* pool){
...
struct IColumnDataExtractor {
    ...
    virtual TVector<const ui8*> GetColumnsDataConst(std::shared_ptr<arrow20::ArrayData> array) = 0;
    ...
    virtual TVector<const ui8*> GetNullBitmapConst(std::shared_ptr<arrow20::ArrayData> array, TVector<std::shared_ptr<arrow20::Buffer>>& nullBitmapRelocationBuffer) = 0;
    
    // For writing (Unpack): returns mutable pointers to new buffers
    virtual TVector<ui8*> GetColumnsData(std::shared_ptr<arrow20::ArrayData> array) = 0;
    virtual TVector<ui8*> GetNullBitmap(std::shared_ptr<arrow20::ArrayData> array) = 0;
    
    virtual ui32 GetElementSize() = 0;
    virtual NPackedTuple::EColumnSizeType GetElementSizeType() = 0;
    virtual std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) = 0;
...
public:
    TFixedSizeColumnDataExtractor(arrow20::MemoryPool* pool, TType* type)
        : Pool_(pool)
        , Type_(type)
    {}

    TVector<const ui8*> GetColumnsDataConst(std::shared_ptr<arrow20::ArrayData> array) override {
    ...
    TVector<const ui8*> GetNullBitmapConst(std::shared_ptr<arrow20::ArrayData> array, TVector<std::shared_ptr<arrow20::Buffer>>& nullBitmapRelocationBuffer) override {
    ...
    TVector<ui8*> GetColumnsData(std::shared_ptr<arrow20::ArrayData> array) override {
        Y_ENSURE(array->buffers.size() == 2);

		return { reinterpret_cast<ui8*>(array->GetMutableValues<TLayout>(1)) };
    }

    TVector<ui8*> GetNullBitmap(std::shared_ptr<arrow20::ArrayData> array) override {
        Y_ENSURE(array->buffers.size() > 0);
    ...
    std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) override {
        ...
        return arrow20::ArrayData::Make(std::move(type), len, {std::move(nullBitmap), std::move(dataBuffer)});
    }
...
public:
    TResourceColumnDataExtractor(arrow20::MemoryPool* pool, TType* type)
        : Pool_(pool)
        , Type_(type)
    {}
...
public:
    TSingularColumnDataExtractor(arrow20::MemoryPool* pool, TType* type) {
        Y_UNUSED(pool, type);
    }
    ...
    std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) override {
        Y_UNUSED(bytes);
        return arrow20::ArrayData::Make(arrow20::null(), len, {nullptr}, len);
    }
...
public:
    TStringColumnDataExtractor(arrow20::MemoryPool* pool, TType* type)
        : Pool_(pool)
        , Type_(type)
    {}
    ...
    std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) override {
        ...
        return arrow20::ArrayData::Make(std::move(type), len, {std::move(nullBitmap), std::move(offsetBuffer), std::move(dataBuffer)});
    }
...
public:
    TTupleColumnDataExtractor(
        std::vector<IColumnDataExtractor::TPtr> children, arrow20::MemoryPool* pool, TType* type
    )
    ...
    std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) override {
        ...
        std::vector<std::shared_ptr<arrow20::ArrayData>> reservedChildren;
        for (size_t i = 0; i < Children_.size(); i++) {
        ...
        return arrow20::ArrayData::Make(std::move(type), len, {std::move(nullBitmap)}, std::move(reservedChildren));
    }
...
public:
    TTzDateColumnDataExtractor(arrow20::MemoryPool* pool, TType* type) {
        this->Pool_ = pool;
        this->Type_ = type;
...
public:
    TExternalOptionalColumnDataExtractor(
        IColumnDataExtractor::TPtr inner, arrow20::MemoryPool* pool, TType* type
    )
    ...
    std::shared_ptr<arrow20::ArrayData> ReserveArray(const TVector<ui64>& bytes, ui32 len, [[maybe_unused]] bool isBitmapNull = false) override {
        ...
        return arrow20::ArrayData::Make(std::move(type), len, {std::move(nullBitmap)}, {std::move(reservedInner)});
    }
...
struct TColumnDataExtractorTraits {
    ...
    static TResult::TPtr MakePg(const NUdf::TPgTypeDescription& desc, const NUdf::IPgBuilder* pgBuilder, arrow20::MemoryPool* pool, TType* type) {
        Y_UNUSED(pgBuilder);
        if (desc.PassByValue) {
            return std::make_unique<TFixedSize<ui64, true>>(pool, type);
        } else {
            return std::make_unique<TStrings<arrow20::BinaryType, true, NKikimr::NUdf::EDataSlot::String>>(pool, type);
        }
    }

    template <bool IsNull>
    static TResult::TPtr MakeSingular(arrow20::MemoryPool* pool, TType* type) {
        Y_UNUSED(IsNull);
        return std::make_unique<TSingular>(pool, type);
    }

    static TResult::TPtr MakeResource(bool isOptional, arrow20::MemoryPool* pool, TType* type) {
        if (isOptional) {
    ...
    static TResult::TPtr MakeTzDate(bool isOptional, arrow20::MemoryPool* pool, TType* type) {
        if (isOptional) {
...
class TBlockLayoutConverter : public IBlockLayoutConverter {
    auto GetColumns_(const TVector<arrow20::Datum>& columns, TVector<std::shared_ptr<arrow20::Buffer>>& nullBitmapRelocationBuffer) {
...
public:
    ...
    void Pack(const TVector<arrow20::Datum>& columns, TPackResult& packed) override {
        TVector<std::shared_ptr<arrow20::Buffer>> nullBitmapRelocationBuffer;
    ...
    void BucketPack(const TVector<arrow20::Datum>& columns, TPaddedPtr<TPackResult> packs, ui32 bucketsLogNum) override {
        TVector<std::shared_ptr<arrow20::Buffer>> nullBitmapRelocationBuffer;
        auto [columnsData, columnsNullBitmap] = GetColumns_(columns, nullBitmapRelocationBuffer);
        auto tuplesToPack = columns.front().array()->length;
    ...
    void Unpack(const TPackResult& packed, TVector<arrow20::Datum>& columns) override {
