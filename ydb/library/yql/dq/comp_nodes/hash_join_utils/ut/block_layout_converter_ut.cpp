...
Y_UNIT_TEST_SUITE(TBlockLayoutConverterTest) {
    ...
    Y_UNIT_TEST(TestBool) {
        ...
        const auto boolType = data.PgmBuilder.NewDataType(NUdf::EDataSlot::Bool, false);
        TVector<NKikimr::NMiniKQL::TType*> types{boolType};
        TVector<NPackedTuple::EColumnRole> roles{NPackedTuple::EColumnRole::Payload};

        constexpr auto testSize = 137;
        auto builder = MakeArrayBuilder(NMiniKQL::TTypeInfoHelper(), boolType, *data.ArrowPool, testSize, nullptr);
        auto converter = MakeBlockLayoutConverter(NMiniKQL::TTypeInfoHelper(), types, roles, data.ArrowPool);

        for (size_t i = 0; i < testSize; i++) {
            builder->Add(TBlockItem(i % 3 == 0));
        }
        auto datum = builder->Build(true);
        Y_ENSURE(datum.is_array());

        TPackResult packRes;
        converter->Pack(TVector<arrow::Datum>{datum}, packRes);
        ...
        auto reader = MakeBlockReader(NMiniKQL::TTypeInfoHelper(), boolType);
        for (size_t i = 0; i < testSize; i++) {
            UNIT_ASSERT_VALUES_EQUAL(reader->GetItem(*unpacked[0].array(), i).Get<ui8>(), ui8(i % 3 == 0));
        }
    }

    Y_UNIT_TEST(TestBitPackedBool) {
        TBlockLayoutConverterTestData data;
        ...
        constexpr int testSize = 137;
        arrow::BooleanBuilder builder(data.ArrowPool);
        for (int i = 0; i < testSize; ++i) {
            ARROW_OK(builder.Append(i % 3 == 0));
        }
        std::shared_ptr<arrow::Array> array;
        ARROW_OK(builder.Finish(&array));
        UNIT_ASSERT_VALUES_EQUAL(int(array->type_id()), int(arrow::Type::BOOL));

        auto converter = MakeBlockLayoutConverter(NMiniKQL::TTypeInfoHelper(), types, roles, data.ArrowPool);
        TPackResult packRes;
        converter->Pack(TVector<arrow::Datum>{arrow::Datum(array)}, packRes);
        UNIT_ASSERT_VALUES_EQUAL(packRes.NTuples, testSize);
    ...
    Y_UNIT_TEST(TestAllNullOptionalOmitsValuesBuffer) {
        TBlockLayoutConverterTestData data;

        const auto optionalInt32Type = data.PgmBuilder.NewDataType(NUdf::EDataSlot::Int32, true);
        TVector<NKikimr::NMiniKQL::TType*> types{optionalInt32Type};
        TVector<NPackedTuple::EColumnRole> roles{NPackedTuple::EColumnRole::Key};

        constexpr auto testSize = 64;
        auto builder = MakeArrayBuilder(NMiniKQL::TTypeInfoHelper(), optionalInt32Type, *data.ArrowPool, testSize, nullptr);
        auto converter = MakeBlockLayoutConverter(NMiniKQL::TTypeInfoHelper(), types, roles, data.ArrowPool);
        ...
        arrayData->buffers[1] = nullptr;

        TPackResult packRes;
        converter->Pack(TVector<arrow::Datum>{arrow::Datum(arrayData)}, packRes);
        UNIT_ASSERT_VALUES_EQUAL(packRes.NTuples, testSize);

        TVector<arrow::Datum> unpacked;
        converter->Unpack(packRes, unpacked);
        auto reader = MakeBlockReader(NMiniKQL::TTypeInfoHelper(), optionalInt32Type);
        for (size_t i = 0; i < testSize; i++) {
            UNIT_ASSERT_C(!reader->GetItem(*unpacked[0].array(), i), "Row must stay NULL after packing a values-less optional");
        }
    }

    Y_UNIT_TEST(TestBoolPayloadWithOptionalKey) {
        TBlockLayoutConverterTestData data;

        const auto boolType = data.PgmBuilder.NewDataType(NUdf::EDataSlot::Bool, false);
        const auto optionalInt32Type = data.PgmBuilder.NewDataType(NUdf::EDataSlot::Int32, true);
        TVector<NKikimr::NMiniKQL::TType*> types{optionalInt32Type, boolType, boolType};
        TVector<NPackedTuple::EColumnRole> roles{
            NPackedTuple::EColumnRole::Key,
            NPackedTuple::EColumnRole::Payload,
            NPackedTuple::EColumnRole::Payload};

        constexpr auto testSize = 51;
        auto keyBuilder = MakeArrayBuilder(NMiniKQL::TTypeInfoHelper(), optionalInt32Type, *data.ArrowPool, testSize, nullptr);
        auto boolBuilder0 = MakeArrayBuilder(NMiniKQL::TTypeInfoHelper(), boolType, *data.ArrowPool, testSize, nullptr);
        auto boolBuilder1 = MakeArrayBuilder(NMiniKQL::TTypeInfoHelper(), boolType, *data.ArrowPool, testSize, nullptr);
        auto converter = MakeBlockLayoutConverter(NMiniKQL::TTypeInfoHelper(), types, roles, data.ArrowPool);

        for (size_t i = 0; i < testSize; i++) {
            if (i % 2) {
                keyBuilder->Add(TBlockItem());
            } else {
                keyBuilder->Add(TBlockItem(i32(i)));
            }
            boolBuilder0->Add(TBlockItem(false));
            boolBuilder1->Add(TBlockItem(i % 2 == 0));
        }

        TVector<arrow::Datum> columns{keyBuilder->Build(true), boolBuilder0->Build(true), boolBuilder1->Build(true)};
        TPackResult packRes;
        converter->Pack(columns, packRes);
        UNIT_ASSERT_VALUES_EQUAL(packRes.NTuples, testSize);
        ...
        TVector<arrow::Datum> unpacked;
        converter->Unpack(packRes, unpacked);
        UNIT_ASSERT_VALUES_EQUAL(unpacked.size(), 3u);

        auto keyReader = MakeBlockReader(NMiniKQL::TTypeInfoHelper(), optionalInt32Type);
        auto boolReader = MakeBlockReader(NMiniKQL::TTypeInfoHelper(), boolType);
        for (size_t i = 0; i < testSize; i++) {
            auto key = keyReader->GetItem(*unpacked[0].array(), i);
            if (i % 2) {
                UNIT_ASSERT_C(!key, "Expected NULL key");
            } else {
                UNIT_ASSERT_VALUES_EQUAL(key.Get<i32>(), i32(i));
            }
            UNIT_ASSERT_VALUES_EQUAL(boolReader->GetItem(*unpacked[1].array(), i).Get<ui8>(), 0);
            UNIT_ASSERT_VALUES_EQUAL(boolReader->GetItem(*unpacked[2].array(), i).Get<ui8>(), ui8(i % 2 == 0));
        }
    }
