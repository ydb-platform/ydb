...
template <typename TNodeSet> std::shared_ptr<TJoinOptimizerNodeInternal> TDPHypSolver<TNodeSet>::PickBestJoin(
    ...
    for (auto joinAlgo : AllJoinAlgos) {
        if (ctx.IsJoinApplicable(left, right, joinConditions, leftJoinKeys, rightJoinKeys, joinAlgo)){
            auto cost = ComputeJoinStats(*left->Stats, *right->Stats, leftJoinKeys, rightJoinKeys, joinAlgo, joinKind, ctx).Cost;
            if (cost < bestCost) {
                bestCost = cost;
                bestAlgo = joinAlgo;
        ...
        if (isCommutative) {
            if (ctx.IsJoinApplicable(right, left, reversedJoinConditions, rightJoinKeys, leftJoinKeys, joinAlgo)){
                auto cost = ComputeJoinStats(*right->Stats, *left->Stats,  rightJoinKeys, leftJoinKeys, joinAlgo, joinKind, ctx).Cost;
                if (cost < bestCost) {
                    bestCost = cost;
                    bestAlgo = joinAlgo;
