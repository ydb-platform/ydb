...
)
{
    ...
    if (joinTuple.LeftScope().Maybe<TCoAtom>()) {
        ...
        left = BuildDqJoin(joinTuple.LeftScope().Cast<TCoEquiJoinTuple>(), inputs, mode, ctx, typeCtx, lhsLabels, hints, useCBO, hasErrors);
        if (!left) {
            return {};
        }
    }
    ...
    if (joinTuple.RightScope().Maybe<TCoAtom>()) {
        ...
        right = BuildDqJoin(joinTuple.RightScope().Cast<TCoEquiJoinTuple>(), inputs, mode, ctx, typeCtx, rhsLabels, hints, useCBO, hasErrors);
        if (!right) {
            return {};
        }
    }
...
TMaybeNode<TExprBase> DqRewriteEquiJoin(
    ...
    auto result = BuildDqJoin(joinTuple, inputs, mode, ctx, typeCtx, dummy, hints, useCBO, hasErrors);
    if (!result) {
        if (hasErrors) {
            return {};
        }
