...
TExprBase DqBuildExtendStage(TExprBase node, TExprContext& ctx) {
            ...
            if (connectionType && connectionType->GetKind() == ETypeAnnotationKind::List) {
                ...
                if (listItemType && listItemType->GetKind() == ETypeAnnotationKind::Struct) {
                    ...
                    for (auto* item : structType->GetItems()) {
                        ...
                        } else if (item->GetItemType()->GetKind() == ETypeAnnotationKind::Optional) {
                            auto optionalType = item->GetItemType()->Cast<TOptionalExprType>()->GetItemType();
                            if (optionalType->GetKind() == ETypeAnnotationKind::Data) {
                                auto columnName = Build<TCoAtom>(ctx, node.Pos()).Value(item->GetName()).Done();
                                keyColumns.push_back(columnName);
                            }
                        }
    ...
    const bool replaceWithShuffle = (!hasPureExpression && dqConnectionCount == replaces.size());
