...
void TLocalBuffer::Push(TDataChunk&& data) {
    ...
    *Registry->LocalBufferBytes += data.Bytes;
    ...
    if (Storage) {
        if (!SpilledChunkBytes.empty() || InflightBytes.load() > MaxInflightBytes) {
...
bool TLocalBuffer::Pop(TDataChunk& data) {
    ...
    if (!Queue.empty() && Queue.front().SpillingBlobId) {
        TBuffer buffer;
        if (Storage->Get(Queue.front().SpillingBlobId, buffer)) {
            BufferToData(Queue.front(), std::move(buffer));
        } else {
            NeedToNotifyInput.store(true);
            return false;
        }
    ...
    *Registry->LocalBufferLatency += (TInstant::Now() - data.Timestamp).MicroSeconds();
    ...
    while (InflightBytes.load() <= MinInflightBytes && !SpilledChunkBytes.empty()) {
        ...
        Queue.emplace(++TailBlobId);
        TBuffer buffer;
        if (Storage->Get(TailBlobId, buffer)) {
            BufferToData(Queue.back(), std::move(buffer));
        }
    }
...
void TOutputDescriptor::AddPushBytes(ui64 bytes) {
    {
        ...
        PushBytes += bytes;
        if (FillLevel == EDqFillLevel::NoLimit && PushBytes.load() > MaxInflightBytes + RemotePopBytes.load()) {
            FillLevel = EDqFillLevel::HardLimit;
            if (Aggregator) {
...
bool TInputBuffer::Pop(TDataChunk& data) {
    ...
    if (result) {
        ...
        if (data.Finished) {
            LOGA_D("FINISHED TInputBuffer, ChannelId=" << Descriptor->Info.ChannelId);
...
void TInputBuffer::UpdatePushStats() {
    PushStats.Bytes = Descriptor->BufferPushBytes.load();
    PushStats.Chunks = Descriptor->BufferPushChunks.load();
    PushStats.Rows = Descriptor->BufferPushRows.load();
}
...
void TNodeState::Handle(TEvDqCompute::TEvChannelDiscoveryV2::TPtr& ev) {
...

    {
        ...
        if (Queue.empty()) {
            ...
            if (!item->Descriptor->IsTerminatedOrAborted()) {
                if (item->Descriptor->CheckGenMajor(GenMajor, "Inconsistent GenMajor in Ack")) {
...
void TNodeState::Handle(TEvPrivate::TEvSendWaiters::TPtr&) {
   SendFromWaiters(0);
}
...
void TNodeState::TerminateInputDescriptor(const std::shared_ptr<TInputDescriptor>& inputBuffer) {
    ...
    (*InputBufferCount)--;
    auto size = InputDescriptors.size();
    LOG_D("TERMINATED TInputDescriptor, ChannelId=" << inputBuffer->Info.ChannelId << ", InputDescriptors.size()=" << size);
}
...
void TNodeState::StartDiscovery() {
    ...
    Reconcilation.store(GenMajor);
    ActorSystem->Send(new NActors::IEventHandle(PeerActorId, NodeActorId, evDiscovery.Release(), flags));
}
...
std::shared_ptr<TNodeState> TDqChannelService::GetOrCreateNodeState(ui32 nodeId) {
    ...
    if (it != NodeStates.end()) {
        ...
        LOG_D("NODE SESSION CREATED, NodeId=" << nodeId << ", NodeActorId=" << nodeState->NodeActorId << ", MaxInflight=" << Limits.NodeSessionIcInflightBytes << " bytes");
        nodeState->StartDiscovery();
        return nodeState;
    }
}
...
std::shared_ptr<IChannelBuffer> TDqChannelService::GetOutputBuffer(ui64 channelId, IDqChannelStorage::TPtr storage) {
    return std::make_shared<TChannelStub>(channelId, storage);
}

std::shared_ptr<IChannelBuffer> TDqChannelService::GetInputBuffer(ui64 channelId) {
    return std::make_shared<TChannelStub>(channelId, nullptr);
}
...
std::shared_ptr<IChannelBuffer> TDqChannelService::GetOutputBuffer(const TChannelFullInfo& info, IDqChannelStorage::TPtr storage) {
    ...
    return (info.InputActorId.NodeId() == NodeId) ? GetLocalBuffer(info, false, storage) : GetRemoteOutputBuffer(info);
}
...
std::shared_ptr<TOutputBuffer> TDqChannelService::GetRemoteOutputBuffer(const TChannelFullInfo& info) {
    Y_ENSURE(info.InputActorId.NodeId() != NodeId);
...
IDqOutputChannel::TPtr TDqChannelService::GetOutputChannel(const TDqChannelParams& params) {
    ...
    auto buffer  = GetOutputBuffer(params.Desc.ChannelId, params.Storage);
...
bool TFastDqInputChannel::Pop(NKikimr::NMiniKQL::TUnboxedValueBatch& batch, TMaybe<TInstant>& watermark) {
    ...
    TDataChunk chunk;
    if (!Buffer->Pop(chunk)) {
        LOGA_D("FALSE Buffer->Pop() TFastDqInputChannel, ChannelId=" << Buffer->PopStats.ChannelId);
        return false;
    }
    if (chunk.Finished && !Finished) {
        if (!IsLocal) {
            LOGA_D("FINISHED TFastDqInputChannel, ChannelId=" << Buffer->PopStats.ChannelId);
        }
        Finished = true;
    }
    if (chunk.Buffer.Empty()) {
        LOGA_D("EMPTY Buffer->Pop() TFastDqInputChannel, ChannelId=" << Buffer->PopStats.ChannelId);
        return false;
    }
    ...
    LOGA_D("TRUE Buffer->Pop() TFastDqInputChannel, ChannelId=" << Buffer->PopStats.ChannelId << ", Bytes=" << chunk.Bytes << ", Rows=" << chunk.Rows);
    return true;
}
...
bool TFastDqOutputChannel::Bind(NActors::TActorId outputActorId, NActors::TActorId inputActorId) {
    if (auto service = Service.lock()) {
        ...
        auto buffer = service->GetOutputBuffer(TChannelFullInfo(Desc.ChannelId, outputActorId, inputActorId, Desc.SrcStageId, Desc.DstStageId), Storage);
        if (Aggregator) {
...
bool TFastDqInputChannel::Bind(NActors::TActorId outputActorId, NActors::TActorId inputActorId) {
    if (auto service = Service.lock()) {
        ...
        auto buffer = service->GetInputBuffer(TChannelFullInfo(Desc.ChannelId, outputActorId, inputActorId, Desc.SrcStageId, Desc.DstStageId));
                    ...
                    for (auto& [nodeId, state] : ChannelService->NodeStates) {
                        for (auto& [info, descriptor] : state->InputDescriptors) {
                            TABLER() {
                                ...
                                TABLED() {str << info.OutputActorId;}
                                TABLED() {
                                    HREF("/actors/kqp_node?ca=" + ToString(info.InputActorId))  {
