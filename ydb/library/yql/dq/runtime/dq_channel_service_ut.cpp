...
*/
    ...
    Y_UNIT_TEST(IcChannelLateBinding) {
        ...
        std::shared_ptr<IChannelBuffer> senderBuffer1 = service0->GetRemoteOutputBuffer(info1);
        std::shared_ptr<IChannelBuffer> senderBuffer2 = service0->GetRemoteOutputBuffer(info2);
    ...
    Y_UNIT_TEST(IcChannelSessionWaiters) {
...

        {
            ...
            auto service = serviceReply->Service;
            for (ui32 i = 0; i < SENDER_COUNT; i++) {
                senderBuffers[i] = service->GetRemoteOutputBuffer(TChannelFullInfo(i + 1, senders[i], receiver, 1, 2));
            }
        }
