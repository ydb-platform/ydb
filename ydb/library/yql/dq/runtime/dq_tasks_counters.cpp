...
NYql::NUdf::TCounter TDqTaskCountersProvider::GetCounter(const NUdf::TStringRef& module, const NUdf::TStringRef& name, bool deriv) {
    ...
    if (name == NKikimr::NMiniKQL::Counter_Bytes) return NYql::NUdf::TCounter(&opStat->Bytes);
    if (name == NKikimr::NMiniKQL::Counter_Rows) return NYql::NUdf::TCounter(&opStat->Rows);
