...
TStatus AnnotateDqBlockHashJoinCore(const TExprNode::TPtr& node, TExprContext& ctx) {
    ...
    if (leftRenamesNode.ChildrenSize() % 2 != 0) {
        ctx.AddError(TIssue(ctx.GetPosition(leftRenamesNode.Pos()), TStringBuilder() << "Left renames must have even number of elements (pairs)"));
        return IGraphTransformer::TStatus(TStatus::Error);
    }
    if (rightRenamesNode.ChildrenSize() % 2 != 0) {
        ctx.AddError(TIssue(ctx.GetPosition(rightRenamesNode.Pos()), TStringBuilder() << "Right renames must have even number of elements (pairs)"));
        return IGraphTransformer::TStatus(TStatus::Error);
    }
