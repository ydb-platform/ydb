...
ui32 TPlanFormatterBase::GetInputs(const TExprNode& node, TVector<TPinInfo>& inputs) {
    Y_UNUSED(node);
    Y_UNUSED(inputs);
    return 0;
}

ui32 TPlanFormatterBase::GetOutputs(const TExprNode& node, TVector<TPinInfo>& outputs) {
    Y_UNUSED(node);
    Y_UNUSED(outputs);
    return 0;
}
