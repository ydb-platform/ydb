...
Y_UNIT_TEST_SUITE(GenericProviderLookupActor) {
    ...
    class TMockSecuredServiceAccountCredentialsFactory : public NYql::ISecuredServiceAccountCredentialsFactory {
        public:
        TMockSecuredServiceAccountCredentialsFactory(const std::string yqlToken, const TVector<bool>& pattern)
            : YqlToken(yqlToken)
              , Pattern(pattern)
        {
        }
    ...
    std::shared_ptr<NYql::ISecuredServiceAccountCredentialsFactory> CreateMockSecuredServiceAccountCredentialsFactory(const std::string yqlToken, const TVector<bool>& pattern) {
        return std::make_shared<TMockSecuredServiceAccountCredentialsFactory>(yqlToken, pattern);
    }
    ...
    Y_UNIT_TEST(LookupWithAuthErrors) {
        ...
        *lookupSourceSettings.mutable_data_source_instance() = dsi;
        ...
        auto lookupActorFactory = [&holderFactory, connectorMock = std::move(connectorMock), lookupSourceSettings = std::move(lookupSourceSettings)](const NActors::TActorId& caller, std::shared_ptr<NKikimr::NMiniKQL::TScopedAlloc>& alloc, NKikimr::NMiniKQL::TTypeEnvironment& typeEnv) mutable {
            ...
            auto [lookupSource, actor] = NYql::NDq::CreateGenericLookupActor(
                std::move(connectorMock),
                CreateMockSecuredServiceAccountCredentialsFactory("token_value", pattern),
                caller,
                nullptr,
                alloc,
                keyTypeHelper,
