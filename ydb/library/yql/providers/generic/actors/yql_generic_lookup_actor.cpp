...
#define COMMON_LOG \
            {"actorId", SelfId()}
#define COMMON_LOG_CTX \
            {"actorId", selfId}

            YDB_LOG_INFO("New generic proivider lookup source actor",
                COMMON_LOG,
    ...
    private: // events
        ...
        void Handle(TEvListSplitsIterator::TPtr ev) {
            ...
            iterator->ReadNext().Subscribe(
                [
                    ...
                    YDB_LOG_DEBUG_CTX(*actorSystem, "Got TListSplitsResponse from Connector",
                        COMMON_LOG_CTX,
                    );
            ...
            *readRequest.add_splits() = std::move(split);
            ...
            CredentialsProvider->AsyncCredentials().Apply([
                ...
                *readRequest.mutable_data_source_instance()->mutable_credentials() = ExtractFromConstFuture(future);
                ...
                try {
                    YDB_LOG_DEBUG_CTX(*actorSystem, "Got ReadSplitsStreamIterator from Connector",
                        COMMON_LOG_CTX,
                    );
                    auto result = ExtractFromConstFuture(asyncResult);
                    if (result.Status.Ok()) {
        ...
        void Handle(TEvLookupRetry::TPtr ev) {
            if (LocalInFlight == 0) { // already passed away
                YDB_LOG_DEBUG("Retry after PassAway",
                    COMMON_LOG,
                );
                return;
            }
    ...
    private:
        ...
        void Handle(TEvGotCredentials::TPtr ev) {
            ...
            Connector->ListSplits(splitRequest, RequestTimeout).Subscribe([
                ...
                auto result = ExtractFromConstFuture(asyncResult);
                if (result.Status.Ok()) {
                    YDB_LOG_DEBUG_CTX(*actorSystem, "Got TListSplitsStreamIterator",
                        COMMON_LOG_CTX,
                    );
            ...
            state->ReadSplitsIterator->ReadNext().Subscribe(
                [
                   ...
                   state = state
                    ...
                    auto result = ExtractFromConstFuture(asyncResult);
                    if (result.Status.Ok()) {
                        YDB_LOG_DEBUG_CTX(*actorSystem, "Got DataChunk",
                            COMMON_LOG_CTX,
                        );
                        Y_ABORT_UNLESS(result.Response);
                        ...
                        YDB_LOG_DEBUG_CTX(*actorSystem, "Got EOF",
                            COMMON_LOG_CTX,
                        );
        ...
        void ProcessReceivedData(const NConnector::NApi::TReadSplitsResponse& resp, TLookupState::TPtr state) {
            ...
            if (state->FullscanLimit > 0 && height > state->FullscanLimit - state->ResultRows) {
                YDB_LOG_WARN("YQ-5124 Workaround for unimplemented LIMIT invoked",
                    COMMON_LOG,
                    {"addedRows", height},
                    {"fullscanLimit", state->FullscanLimit},
        ...
        static void SendError(NActors::TActorSystem* actorSystem, const NActors::TActorId& selfId, const NConnector::NApi::TError& error) {
            YDB_LOG_ERROR_CTX(*actorSystem, "Got GrpcError from Connector",
                COMMON_LOG_CTX,
        ...
        static void SendRetryOrError(NActors::TActorSystem* actorSystem, const NActors::TActorId& selfId, const NYdbGrpc::TGrpcStatus& status, TLookupState::TPtr state) {
            ...
            if (nextRetry) {
                YDB_LOG_DEBUG_CTX(*actorSystem, "Got retrievable GRPC Error from Connector",
                    COMMON_LOG_CTX,
