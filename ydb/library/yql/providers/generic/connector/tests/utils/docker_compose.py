...
class EndpointDeterminer:
    ...
    def __init__(self, docker_compose_yml: os.PathLike):
        self.docker_compose_bin = os.path.realpath(
            yatest.common.build_path('library/recipes/docker_compose/bin/docker-compose')
        )
        self.docker_compose_yml = docker_compose_yml

    def get_port(self, service_name: str, internal_port: int) -> int:
        cmd = ['docker-compose', '-f', self.docker_compose_yml, 'port', service_name, str(internal_port)]
        try:
