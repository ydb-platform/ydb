...
class KqpRunner(Runner):
    ...
    def run(self, test_name: str, script: str, generic_settings: GenericSettings) -> Result:
        ...
        cmd = f'{self.kqprun_path} -s {scheme_path} -p {script_path} --app-config={app_conf_path} --result-file={result_path} --result-format=full-json --udfs-dir={self.udf_dir} --exclude-linked-udfs '
