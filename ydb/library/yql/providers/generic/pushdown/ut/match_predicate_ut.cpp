...
Y_UNIT_TEST_SUITE(MatchPredicate) {
    ...
    Y_UNIT_TEST(EmptyWhere) {
        UNIT_ASSERT(MatchPredicate(TMap<TString, NYql::TColumnStatistics>{ {
            { "col1", NYql::TColumnStatistics{} },
    ...
    Y_UNIT_TEST(Between) {
        UNIT_ASSERT(MatchPredicate(TMap<TString, NYql::TColumnStatistics>{ {
            { "col1", BuildTimestampStats(TInstant::ParseIso8601("2024-03-01T00:00:00Z"), TInstant::ParseIso8601("2024-03-01T23:59:59Z")) }
        }}, 
        BuildPredicate(
            R"proto(
                between {
    ...
    Y_UNIT_TEST(NotLess) {
        UNIT_ASSERT(!MatchPredicate(TMap<TString, NYql::TColumnStatistics>{ {
            { "col1", BuildTimestampStats(TInstant::ParseIso8601("2024-03-02T00:00:00Z"), TInstant::ParseIso8601("2024-03-02T23:59:59Z")) }
        }}, 
        BuildPredicate(
            R"proto(
                comparison {
