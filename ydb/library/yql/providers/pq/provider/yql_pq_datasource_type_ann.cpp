    ...
    TStatus HandleReadTopic(const TExprNode::TPtr& input, TExprContext& ctx) {
        ...
        if (TPqReadTopic::idx_Watermark < input->ChildrenSize()) {
            ...
            if (!IsPureIsolatedLambda(*lambdaBody.Ptr())) {
                ctx.AddError(TIssue(ctx.GetPosition(watermark->Pos()), "Expected pure expression for watermark definition"));
                return TStatus::Error;
            }

            const auto maybeSub = lambdaBody.Maybe<TCoSub>();
            if (!maybeSub) {
                ctx.AddError(TIssue(ctx.GetPosition(watermark->Pos()), "Expected top-level subtraction"));
                return TStatus::Error;
            }
            ...
            if (!maybeInterval) {
                ctx.AddError(TIssue(ctx.GetPosition(watermark->Pos()), "Expected interval as watermark delay"));
                return TStatus::Error;
            }
