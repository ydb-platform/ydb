...
EStreamType CheckMagic(NDB::ReadBuffer& input) {
    char data[4u];
    YQL_ENSURE_CODELINE(input.read(data, sizeof(data)) == sizeof(data), NFq::TIssuesIds::BAD_REQUEST, "Buffer too small.");
...
{
    YQL_ENSURE_CODELINE(StreamType != EStreamType::Unknown, NFq::TIssuesIds::BAD_REQUEST, "Wrong magic.");
    if (StreamType == EStreamType::Frame) {
        ...
        YQL_ENSURE_CODELINE(!LZ4F_isError(NextToLoad), NFq::TIssuesIds::BAD_REQUEST, "Header error: " << LZ4F_getErrorName(NextToLoad));
    }
}
    ...
    while (NextToLoad) {
        ...
        if (Pos >= Remaining) {
            for (auto toRead = NextToLoad; toRead > 0U;) {
                ...
                YQL_ENSURE_CODELINE(sizeCheck > 0U && sizeCheck <= toRead, NFq::TIssuesIds::BAD_REQUEST, "Cannot access compressed block.");
                toRead -= sizeCheck;
            }
        ...
        while (Pos < Remaining || (decodedBytes == OutBuffer.size())) {
            ...
            YQL_ENSURE_CODELINE(!LZ4F_isError(NextToLoad), NFq::TIssuesIds::BAD_REQUEST, "Decompression error: " << LZ4F_getErrorName(NextToLoad));
            Pos += Remaining;
...
size_t TReadBuffer::DecompressLegacy() {
    ...
    if (const auto sizeCheck = Source.read(InBuffer.data(), 4U)) {
        YQL_ENSURE_CODELINE(sizeCheck == 4U, NFq::TIssuesIds::BAD_REQUEST, "Cannot access block size.");
        blockSize = ReadLE32(InBuffer.data());
        YQL_ENSURE_CODELINE(blockSize <= LZ4_COMPRESSBOUND(LegacyBlockSize), NFq::TIssuesIds::BAD_REQUEST, "Block size out of bounds.");
    } else
        return 0ULL;
    ...
    YQL_ENSURE_CODELINE(decodeSize >= 0, NFq::TIssuesIds::BAD_REQUEST, "Corrupted input detected.");
