...
Y_UNIT_TEST_SUITE(CheckSqlFormatter) {
    ...
    Y_UNIT_TEST(CreateTable) {
        TCases cases = {
            ...
            {"create table user(index idx global using subtype with (setting = foo, another_setting = bar) on (col) cover (col));",
             "CREATE TABLE user (\n\tINDEX idx GLOBAL USING subtype WITH (setting = foo, another_setting = bar) ON (col) COVER (col)\n);\n"}, 
    ...
    Y_UNIT_TEST(AlterTable) {
        TCases cases = {
            ...
            {"alter table user add index idx global using subtype with (setting = foo, another_setting = bar) on (col) cover (col);",
             "ALTER TABLE user\n\tADD INDEX idx GLOBAL USING subtype WITH (setting = foo, another_setting = bar) ON (col) COVER (col);\n"}, 
