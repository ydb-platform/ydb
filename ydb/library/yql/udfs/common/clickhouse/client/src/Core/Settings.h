...
#define FORMAT_FACTORY_SETTINGS(M) \
    ...
    M(Bool, input_format_skip_unknown_fields, false, "Skip columns with unknown names from input data (it works for json_each_row, csv_with_names, tsv_with_names and TSKV formats).", 0) \
    M(Bool, input_format_with_names_use_header, true, "For tsv_with_names and csv_with_names input formats this controls whether format parser is to assume that column data appear in the input exactly as they are specified in the header.", 0) \
    ...
    M(Bool, output_format_json_array_of_rows, false, "Output a JSON array of all rows in json_each_row(Compact) format.", 0) \
    \
