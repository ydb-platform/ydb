...
void RowInputFormatWithNamesAndTypes::addInputColumn(const String & column_name, std::vector<UInt8> & read_columns)
{
...
void RowInputFormatWithNamesAndTypes::readPrefix()
{
    ...
    if (with_names)
    {
        ...
        {
            column_mapping->read_columns.assign(data_types.size(), false);
            auto column_names = format_reader->readNames();
            for (const auto & name : column_names)
                addInputColumn(name, column_mapping->read_columns);
        }
        else
        {
...
{
    for (size_t index = 0; index < ext.read_columns.size(); ++index) {
        if (index >= column_mapping->read_columns.size() || !column_mapping->read_columns[index]) {
