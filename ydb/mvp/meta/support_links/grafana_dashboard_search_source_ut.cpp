...
void ConfigureTokenator(TTestActorRuntime& runtime, const TString& tokenName, const TString& tokenValue) {
    ...
    runtime.GetActorSystem(0)->AppData<NMVP::TMVPAppData>()->Tokenator =
        NMVP::TMvpTokenator::CreateTokenator(tokensConfig, NActors::TActorId());
}
...
public:
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpOutgoingRequest::TPtr event) {
        ...
        const bool hasExpectedAuth = request->Headers.find("OAuth test-token") != TString::npos;
        const TString body = "[]";
        const TString statusLine = (targetsSearchApi && hasTag && hasFolder && hasExpectedAuth)
            ? "HTTP/1.1 200 OK"
...
Y_UNIT_TEST_SUITE(SupportLinksGrafanaDashboardSearchSource) {
    ...
    Y_UNIT_TEST(ResolveRequestsSearchApiWithTagFolderAndAuthorization) {
        TMetaDatabaseTokenNameGuard tokenNameGuard("meta-token");
        TTestActorRuntime runtime;
        ConfigureTokenator(runtime, "meta-token", "OAuth test-token");
    ...
    Y_UNIT_TEST(ResolveBuildsDashboardLinksFromSearchResponse) {
        ...
        const TString body = R"json([
            {"title":"CPU","url":"/d/ydb_cpu/cpu"},
            {"title":"DB overview","uri":"db/db-overview"},
            {"title":"Skip me"}
        ])json";
