    ...
    TString& errorMessage)
{
    ...
    const auto workspaceIt = clusterInfo.find("workspace");
    if (workspaceIt == clusterInfo.end() || workspaceIt->second.empty()) {
        errorMessage = "workspace is required in cluster info for source=grafana/logging";
        return false;
    }

    const auto datasourceIt = clusterInfo.find("datasource");
    if (datasourceIt == clusterInfo.end() || datasourceIt->second.empty()) {
        errorMessage = "datasource is required in cluster info for source=grafana/logging";
        return false;
    }
