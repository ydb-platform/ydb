...
Y_UNIT_TEST_SUITE(SupportLinksGrafanaLoggingSource) {
    ...
    Y_UNIT_TEST(ResolveBuildsDefaultExploreUrlFromScratch) {
        TGrafanaLoggingTestContext context;
        context.ClusterInfo["workspace"] = "ydb-common";
        context.ClusterInfo["datasource"] = "cd868168-09d4-4ece-82db-e646130697e5";
    ...
    Y_UNIT_TEST(ResolveBuildsAbsoluteExploreUrlFromScratch) {
        ...
        context.ClusterInfo["workspace"] = "ydb-common";
        context.ClusterInfo["datasource"] = "ds-42";
    ...
    Y_UNIT_TEST(ResolveReturnsErrorWhenWorkspaceMissing) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(result.Errors[0].Message, "workspace is required in cluster info for source=grafana/logging");
    }
    ...
    Y_UNIT_TEST(ResolveReturnsErrorWhenDatasourceMissing) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(result.Errors.size(), 1u);
        UNIT_ASSERT_VALUES_EQUAL(result.Errors[0].Source, "grafana/logging");
        UNIT_ASSERT_VALUES_EQUAL(result.Errors[0].Message, "datasource is required in cluster info for source=grafana/logging");
    }

    Y_UNIT_TEST(ResolveRejectsUrlWithQueryParameters) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(result.Errors.size(), 1u);
        UNIT_ASSERT_VALUES_EQUAL(result.Errors[0].Source, "grafana/logging");
        UNIT_ASSERT_VALUES_EQUAL(result.Errors[0].Message, "query parameters are not supported in url for source=grafana/logging");
    }
}
