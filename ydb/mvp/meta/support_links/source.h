...
#include <ydb/mvp/meta/support_links/grafana_dashboard.h>
...
class TGrafanaDashboardSearchResolver : public ILinkSource {
public:
    TGrafanaDashboardSearchResolver(TSupportLinkEntry config, const TMetaSettings& metaSettings)
...
std::shared_ptr<ILinkSource> BuildGrafanaDashboardSource(
    TSupportLinkEntry config,
    const TMetaSettings& metaSettings);
std::shared_ptr<ILinkSource> BuildGrafanaDashboardSourceUnchecked(
    TSupportLinkEntry config,
    const TMetaSettings& metaSettings);
    ...
    const TMetaSettings& metaSettings)
{
    ...
    return std::make_shared<TGrafanaDashboardSearchResolver>(std::move(config), metaSettings);
}

inline std::shared_ptr<ILinkSource> MakeLinkSourceWithoutValidation(size_t place, TSupportLinkEntry config, const TMetaSettings& metaSettings) {
    (void)place;
    if (config.GetSource() == NSupportLinks::SOURCE_GRAFANA_DASHBOARD) {
        return BuildGrafanaDashboardSourceUnchecked(std::move(config), metaSettings);
    }
...
inline std::shared_ptr<ILinkSource> MakeGrafanaLinkSource(size_t place, TSupportLinkEntry config) {
    Y_ABORT_UNLESS(InstanceMVP);
    return MakeLinkSourceWithoutValidation(place, std::move(config), InstanceMVP->MetaSettings);
}
