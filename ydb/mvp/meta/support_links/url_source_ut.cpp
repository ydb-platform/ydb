...
Y_UNIT_TEST_SUITE(SupportLinksUrlSource) {
    ...
    Y_UNIT_TEST(ValidationRejectsUnusedTemplateParameterMapping) {
        TUrlSourceTestContext context("https://service.example.net/{host}");
        context.Config.SetSource("url");
        auto* dcMapping = context.Config.AddTemplateParameterMappings();
        ...
        UNIT_ASSERT_EXCEPTION_CONTAINS(
            context.CreateSource(),
            yexception,
            "template_parameter_mappings.parameter=dc is not used in url"
        );
    }
    ...
    Y_UNIT_TEST(ResolveUsesMappedValueForTemplateParameter) {
        ...
        AssertSingleResolvedLink(
            result,
            "https://service.example.net/instance/node-1.example.net?dc=man"
        );
    }
    ...
    Y_UNIT_TEST(ResolveEscapesTemplateValuesUsedInQueryTemplateParameters) {
        ...
        context.Config.SetSource("url");
        auto* ticketMapping = context.Config.AddTemplateParameterMappings();
        ticketMapping->SetParameter("ticket");
        ticketMapping->SetStaticValue("A&B?x=1");
    ...
    Y_UNIT_TEST(ResolveRendersTemplateUsingEmptyStaticValue) {
        TUrlSourceTestContext context("https://service.example.net/instance/{host}?bucket={bucket}");
        context.Config.SetSource("url");
        auto* bucketMapping = context.Config.AddTemplateParameterMappings();
        bucketMapping->SetParameter("bucket");
        bucketMapping->SetStaticValue("");
        ...
        AssertSingleResolvedLink(
            result,
            "https://service.example.net/instance/node-1.example.net?bucket="
        );
    }

    Y_UNIT_TEST(ResolveReturnsErrorWhenMappedTemplateValueIsMissing) {
        TUrlSourceTestContext context("https://service.example.net/instance/{host}?dc={dc}");
        context.Config.SetSource("url");
        auto* dcMapping = context.Config.AddTemplateParameterMappings();
        dcMapping->SetParameter("dc");
