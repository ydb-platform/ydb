...
Y_UNIT_TEST_SUITE(SupportLinksUrlSource) {
    ...
    Y_UNIT_TEST(ValidationRejectsInvalidTemplateFormat) {
        TUrlSourceTestContext context("https://service.example.net/{host-name}");
        context.Config.SetSource("url");
        UNIT_ASSERT_EXCEPTION_CONTAINS(
            context.CreateSource(),
            yexception,
    ...
    Y_UNIT_TEST(ResolveEscapesTemplateValuesUsedInQueryTemplateParameters) {
        ...
        context.Config.SetSource("url");
        auto* ticketMapping = context.Config.AddLinkParameterMappings();
    ...
    Y_UNIT_TEST(ResolveReturnsErrorWhenMappedTemplateValueIsMissing) {
        ...
        UNIT_ASSERT_STRING_CONTAINS(result.Errors[0].Message, "failed to render URL template");
    }
