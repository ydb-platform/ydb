...
class TAuthStartHandler : public NActors::TActor<TAuthStartHandler> {
    using TBase = NActors::TActor<TAuthStartHandler>;

    const NActors::TActorId HttpProxyId;
    const TOpenIdConnectSettings Settings;

public:
    TAuthStartHandler(const NActors::TActorId& httpProxyId, const TOpenIdConnectSettings& settings);
