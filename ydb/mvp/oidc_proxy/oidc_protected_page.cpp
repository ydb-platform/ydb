...
void THandlerSessionServiceCheck::HandleProxy(NHttp::TEvHttpProxy::TEvHttpIncomingResponse::TPtr event) {
    if (event->Get()->Response != nullptr) {
        ...
        return ReplyAndPassAway(CreateResponseFromProtectedResource(response));
    } else {
...
void THandlerSessionServiceCheck::ForwardUserRequest(TStringBuf authHeader, bool secure) {
    ...
    auto httpRequest = CreateRequestToProtectedResource(authHeader, secure);
...
NHttp::THttpOutgoingRequestPtr THandlerSessionServiceCheck::CreateRequestToProtectedResource(TStringBuf authHeader, bool secure) const {
...
NHttp::THttpOutgoingResponsePtr THandlerSessionServiceCheck::CreateResponseFromProtectedResource(const NHttp::THttpIncomingResponsePtr& response) {
    static const TVector<TStringBuf> HEADERS_WHITE_LIST = {
        "Content-Type",
        "Connection",
        "X-Worker-Name",
