...
void THandlerSessionCreate::Bootstrap() {
    ...
    if (decodedState.Payload.RequestedAddress.empty()) {
        BLOG_D("Restore oidc session failed: requested address is missing in state");
        SendUnknownErrorResponseAndDie();
        return;
    }
    Context = TContext({.RequestedAddress = decodedState.Payload.RequestedAddress});
