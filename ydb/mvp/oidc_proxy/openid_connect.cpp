...
NHttp::THttpOutgoingRequestPtr CreateProxiedRequest(const TProxiedRequestParams& params) {
    ...
    if (!params.AuthHeader.empty()) {
        outRequest->Set(AUTH_HEADER_NAME, params.AuthHeader);
    }
...
NHttp::THttpOutgoingResponsePtr CreateProxiedResponse(const TProxiedResponseParams& params) {
    ...
    auto outBody = params.OutBody ? params.OutBody : params.Response->Body;
    NHttp::THeadersBuilder outHeaders;

    NHttp::THeadersBuilder headers(params.Response->Headers);
    for (const auto& header : params.Settings.RESPONSE_HEADERS_WHITE_LIST) {
        if (headers.Has(header)) {
            outHeaders.Set(header, headers.Get(header));
        }
    }
    if (headers.Has(LOCATION_HEADER_NAME)) {
        outHeaders.Set(LOCATION_HEADER_NAME, GetFixedLocationHeader(params.ProtectedPage, headers.Get(LOCATION_HEADER_NAME)));
    }

    TStringBuf contentType = headers.Get("Content-Type").NextTok(';');
    if (contentType == "text/html") {
        TString fixedBody = FixReferenceInHtml(outBody, params.Response->GetRequest()->Host);
        return params.Request->CreateResponse(outStatus, outMessage, outHeaders, fixedBody);
    } else {
