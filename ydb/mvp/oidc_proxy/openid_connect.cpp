...
// Reserve extra space for the authorization code returned by IAM in
// /auth/callback?code=...&state=...
constexpr size_t AUTH_CALLBACK_CODE_RESERVE_BYTES = 200;

bool IsAuthCallbackStateTooLarge(TStringBuf state) {
    TCgiParameters callbackParams;
    callbackParams.InsertUnescaped("code", "");
    callbackParams.InsertUnescaped("state", state);
    const TString callbackUrl = TStringBuilder()
        << GetAuthCallbackUrl()
        << "?"
        << callbackParams.Print();
    return callbackUrl.size() + AUTH_CALLBACK_CODE_RESERVE_BYTES > NHttp::THttpRequestParser::MaxURLSize;
}

TString BuildStateForAuthorizationServerRedirect(const TContext& context, const TString& clientSecret) {
    TString state = context.GetState(clientSecret);
    if (IsAuthCallbackStateTooLarge(state)) {
        // Keep return_to in the temp cookie when the callback URL would exceed the HTTP parser limit.
        state = context.GetState(clientSecret, false);
    }
    return state;
}
...
TString BuildAuthorizationServerRedirectUrl(const NHttp::THttpIncomingRequestPtr& request,
    ...
    authParams.InsertUnescaped("state", BuildStateForAuthorizationServerRedirect(context, settings.ClientSecret));
