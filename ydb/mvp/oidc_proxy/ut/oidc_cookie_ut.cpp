...
Y_UNIT_TEST_SUITE(OidcCookie) {
    ...
    Y_UNIT_TEST(AuthFlowCookieRestoresRequestedAddress) {
        const TString token = MakeToken(1);
        const TString requestedAddress = "/requested/page";

        const TString cookieValue = UpdateAuthFlowCookieValue("", TEST_SECRET, token, requestedAddress);

        UNIT_ASSERT_STRINGS_EQUAL(RestoreRequestedAddressFromAuthFlowCookie(cookieValue, TEST_SECRET), requestedAddress);
    }

    Y_UNIT_TEST(AuthFlowCookieUpdatesRequestedAddressOnNewAttempt) {
        const TString firstToken = MakeToken(1);
        const TString secondToken = MakeToken(2);

        TString cookieValue = UpdateAuthFlowCookieValue("", TEST_SECRET, firstToken, "/requested/first");
        cookieValue = UpdateAuthFlowCookieValue(cookieValue, TEST_SECRET, secondToken, "/requested/second");

        UNIT_ASSERT(HasAuthFlowCookieNonce(cookieValue, TEST_SECRET, firstToken));
        UNIT_ASSERT(HasAuthFlowCookieNonce(cookieValue, TEST_SECRET, secondToken));
        UNIT_ASSERT_STRINGS_EQUAL(RestoreRequestedAddressFromAuthFlowCookie(cookieValue, TEST_SECRET), "/requested/second");
    }
