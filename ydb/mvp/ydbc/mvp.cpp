...
int TMVP::Init() {
    ...
    InitYdbc(ActorSystem, BaseHttpProxyId, Locations);
...
void TMVP::TryGetYdbcOptionsFromConfig(const YAML::Node& config) {
    Cerr << "iiiiii TryGetYdbcOptionsFromConfig " << Endl;
    if (!config["ydbc"]) {
        return;
    }
    auto ydbc_component = config["ydbc"];

    if (ydbc_component["locations"]) {
        for (const auto& ydbc_location: ydbc_component["locations"]) {
            TString name = ydbc_location["name"].as<std::string>("");
            Cerr << "iiiiii name " << name << Endl;
            if (name) {
                TYdbcLocation& location = Locations[name];
                TryGetLocationFromConfig(location, ydbc_location);
            Cerr << "iiiiii location.name " << location.Name << Endl;
            }
        }
    }
}
...
THolder<NActors::TActorSystemSetup> TMVP::BuildActorSystemSetup(int argc, char** argv) {
    ...
    if (!yamlConfigPath.empty()) {
        try {
            ...
            TryGetYdbcOptionsFromConfig(config);
