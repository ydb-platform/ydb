...
class THandlerActorYdbcAllDatabasesStatsCollector : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcAllDatabasesStatsCollector> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcAllDatabasesStatsCollector>;
    ...
    THolder<TEvPrivate::TEvListAllDatabaseResponse> ListAllDatabasesResponse;
    THolder<TEvPrivate::TEvErrorResponse> ErrorResponse;

    THandlerActorYdbcAllDatabasesStatsCollector(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcAllDatabasesStatsCollector::StateWork, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
    ...
    void Handle(TEvPrivate::TEvListAllDatabaseResponse::TPtr event, const NActors::TActorContext& ctx) {
        ListAllDatabasesResponse = event->Release();
        ReplyAndDie(ctx);
    }
    ...
    void ReplyAndDie(const NActors::TActorContext& ctx) {
        ...
        if (ErrorResponse == nullptr) {
            if (ListAllDatabasesResponse != nullptr) {
                NJson::TJsonValue jsonRoot;
                NProtobufJson::Proto2Json(ListAllDatabasesResponse->Databases, jsonRoot, Proto2JsonConfig);
...
class THandlerActorYdbcAllDatabases : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcAllDatabases> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcAllDatabases>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcAllDatabases(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcAllDatabases::StateWork)
        , Location(location)
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "GET" || request->Method == "HEAD" || (request->Method == "POST" && contentType == "application/json")) {
            ctx.Register(new THandlerActorYdbcAllDatabasesStatsCollector(Location, HttpProxyId, event->Sender, event->Get()->Request));
            return;
        }
