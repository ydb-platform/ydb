...
class THandlerActorYdbcDynamoItemPost : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemPost> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemPost>;
    const TYdbcLocation& Location;
    TRequest Request;
    NActors::TActorId HttpProxyId;
    std::unique_ptr<NYdb::NScheme::TSchemeClient> SchemeClient;

    THandlerActorYdbcDynamoItemPost(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcDynamoItemPost::StateResolveDatabase, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
        ...
        if (event->Get()->Error.empty() && status == "200") {
            ...
            if (success) {
                ...
                if (!database.empty()) {
                    ...
                    httpRequest->Set("X-Amz-Target", "DynamoDB_20120810.PutItem");
                    httpRequest->Set<&NHttp::THttpRequest::ContentType>("application/x-amz-json-1.0");
                    httpRequest->Set<&NHttp::THttpRequest::Body>(body);
                    ctx.Send(HttpProxyId, new NHttp::TEvHttpProxy::TEvHttpOutgoingRequest(httpRequest));
                    Become(&THandlerActorYdbcDynamoItemPost::StateRequest);
                    return;
                } else {
                    if (database.empty()) {
...
// TODO(xenoxeno): temporary copy-paste from *ItemPost
class THandlerActorYdbcDynamoItemGet : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemGet> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemGet>;
    const TYdbcLocation& Location;
    TRequest Request;
    NActors::TActorId HttpProxyId;
    std::unique_ptr<NYdb::NScheme::TSchemeClient> SchemeClient;

    THandlerActorYdbcDynamoItemGet(
            const TYdbcLocation& location,
            const NActors::TActorId& httpProxyId,
            const NActors::TActorId& sender,
            const NHttp::THttpIncomingRequestPtr& request)
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcDynamoItemGet::StateResolveDatabase, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
        ...
        if (event->Get()->Error.empty() && status == "200") {
            ...
            if (success) {
                ...
                if (!database.empty() && !path.empty()) {
                    ...
                    NHttp::THttpOutgoingRequestPtr httpRequest = NHttp::THttpOutgoingRequest::CreateRequestPost(Location.GetServerlessProxyUrl(database));
                    Request.ForwardHeaders(httpRequest);
                    httpRequest->Set("X-Amz-Target", "DynamoDB_20120810.GetItem");
                    httpRequest->Set<&NHttp::THttpRequest::ContentType>("application/x-amz-json-1.0");
                    httpRequest->Set<&NHttp::THttpRequest::Body>(body);
                    ctx.Send(HttpProxyId, new NHttp::TEvHttpProxy::TEvHttpOutgoingRequest(httpRequest));
                    Become(&THandlerActorYdbcDynamoItemGet::StateRequest);
                    return;
                } else {
                    if (database.empty()) {
                        message = "Invalid database endpoint";
...
// TODO(xenoxeno): temporary copy-paste from *ItemPost
class THandlerActorYdbcDynamoItemDelete : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemDelete> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDynamoItemDelete>;
    const TYdbcLocation& Location;
    TRequest Request;
    NActors::TActorId HttpProxyId;
    std::unique_ptr<NYdb::NScheme::TSchemeClient> SchemeClient;

    THandlerActorYdbcDynamoItemDelete(
            const TYdbcLocation& location,
            const NActors::TActorId& httpProxyId,
            const NActors::TActorId& sender,
            const NHttp::THttpIncomingRequestPtr& request)
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcDynamoItemDelete::StateResolveDatabase, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
        ...
        if (event->Get()->Error.empty() && status == "200") {
            ...
            if (success) {
                ...
                if (!database.empty() && !path.empty()) {
                    ...
                    NHttp::THttpOutgoingRequestPtr httpRequest = NHttp::THttpOutgoingRequest::CreateRequestPost(Location.GetServerlessProxyUrl(database));
                    Request.ForwardHeaders(httpRequest);
                    httpRequest->Set("X-Amz-Target", "DynamoDB_20120810.DeleteItem");
                    httpRequest->Set<&NHttp::THttpRequest::ContentType>("application/x-amz-json-1.0");
                    httpRequest->Set<&NHttp::THttpRequest::Body>(body);
                    ctx.Send(HttpProxyId, new NHttp::TEvHttpProxy::TEvHttpOutgoingRequest(httpRequest));
                    Become(&THandlerActorYdbcDynamoItemDelete::StateRequest);
                    return;
                } else {
                    if (database.empty()) {
                        message = "Invalid database endpoint";
...
class THandlerActorYdbcDynamoItem : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcDynamoItem> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcDynamoItem>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcDynamoItem(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcDynamoItem::StateWork)
        , Location(location)
        , HttpProxyId(httpProxyId)
    {}
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            ctx.Register(new THandlerActorYdbcDynamoItemPost(Location, HttpProxyId, event->Sender, request));
            return;
        } else if (request->Method == "GET") {
            ctx.Register(new THandlerActorYdbcDynamoItemGet(Location, HttpProxyId, event->Sender, request));
            return;
        } else if (request->Method == "DELETE") {
            ctx.Register(new THandlerActorYdbcDynamoItemDelete(Location, HttpProxyId, event->Sender, request));
            return;
        }
