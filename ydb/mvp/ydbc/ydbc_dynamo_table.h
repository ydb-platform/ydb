...
class THandlerActorYdbcDynamoTablePost : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDynamoTablePost> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDynamoTablePost>;
    const TYdbcLocation& Location;
    TRequest Request;
    NActors::TActorId HttpProxyId;
    std::unique_ptr<NYdb::NScheme::TSchemeClient> SchemeClient;

    THandlerActorYdbcDynamoTablePost(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcDynamoTablePost::StateResolveDatabase, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
        ...
        if (event->Get()->Error.empty() && status == "200") {
            ...
            if (success) {
                ...
                if (!database.empty()) {
                    ...
                    httpRequest->Set("X-Amz-Target", "DynamoDB_20120810.CreateTable");
                    httpRequest->Set<&NHttp::THttpRequest::ContentType>("application/x-amz-json-1.0");
                    httpRequest->Set<&NHttp::THttpRequest::Body>(body);
                    ctx.Send(HttpProxyId, new NHttp::TEvHttpProxy::TEvHttpOutgoingRequest(httpRequest));
                    Become(&THandlerActorYdbcDynamoTablePost::StateRequest);
                    return;
                } else {
                    if (database.empty()) {
...
// TODO(xenoxeno): temporary copy-paste from *TablePost
class THandlerActorYdbcDynamoTableDelete : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDynamoTableDelete> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDynamoTableDelete>;
    const TYdbcLocation& Location;
    TRequest Request;
    NActors::TActorId HttpProxyId;
    std::unique_ptr<NYdb::NScheme::TSchemeClient> SchemeClient;

    THandlerActorYdbcDynamoTableDelete(
            const TYdbcLocation& location,
            const NActors::TActorId& httpProxyId,
            const NActors::TActorId& sender,
            const NHttp::THttpIncomingRequestPtr& request)
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        Become(&THandlerActorYdbcDynamoTableDelete::StateResolveDatabase, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
        ...
        if (event->Get()->Error.empty() && status == "200") {
            ...
            if (success) {
                ...
                if (!database.empty() && !path.empty()) {
                    ...
                    jsonRoot["TableName"] = path;
                    NHttp::THttpOutgoingRequestPtr httpRequest = NHttp::THttpOutgoingRequest::CreateRequestPost(Location.GetServerlessProxyUrl(database));
                    TString body = NJson::WriteJson(jsonRoot);
                    Request.ForwardHeaders(httpRequest);
                    httpRequest->Set("X-Amz-Target", "DynamoDB_20120810.DeleteTable");
                    httpRequest->Set<&NHttp::THttpRequest::ContentType>("application/x-amz-json-1.0");
                    httpRequest->Set<&NHttp::THttpRequest::Body>(body);
                    ctx.Send(HttpProxyId, new NHttp::TEvHttpProxy::TEvHttpOutgoingRequest(httpRequest));
                    Become(&THandlerActorYdbcDynamoTableDelete::StateRequest);
                    return;
                } else {
                    if (database.empty()) {
                        message = "Invalid database endpoint";
...
class THandlerActorYdbcDynamoTable : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcDynamoTable> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcDynamoTable>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcDynamoTable(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcDynamoTable::StateWork)
        , Location(location)
        , HttpProxyId(httpProxyId)
    {}
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            ctx.Register(new THandlerActorYdbcDynamoTablePost(Location, HttpProxyId, event->Sender, request));
            return;
        } else if (request->Method == "DELETE") {
            ctx.Register(new THandlerActorYdbcDynamoTableDelete(Location, HttpProxyId, event->Sender, request));
            return;
        }
