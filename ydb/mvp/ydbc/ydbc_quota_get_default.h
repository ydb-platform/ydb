...
class THandlerActorYdbcQuotaGetDefaultRequest : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcQuotaGetDefaultRequest> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcQuotaGetDefaultRequest>;
    const TYdbcLocation& Location;
    TRequest Request;

    THandlerActorYdbcQuotaGetDefaultRequest(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        yandex::cloud::priv::quota::GetQuotaDefaultRequest cpRequest;
        ...
        NYdbGrpc::TResponseCallback<yandex::cloud::priv::quota::GetQuotaDefaultResponse> responseCb =
            [actorId, actorSystem](NYdbGrpc::TGrpcStatus&& status, yandex::cloud::priv::quota::GetQuotaDefaultResponse&& response) -> void {
            if (status.Ok()) {
                actorSystem->Send(actorId, new TEvPrivate::TEvQuotaDefaultResponse(std::move(response)));
            } else {
        ...
        connection->DoRequest(cpRequest, std::move(responseCb), &yandex::cloud::priv::ydb::v1::QuotaService::Stub::AsyncGetDefault, meta);
        Become(&THandlerActorYdbcQuotaGetDefaultRequest::StateWork, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }

    void Handle(TEvPrivate::TEvQuotaDefaultResponse::TPtr event, const NActors::TActorContext& ctx) {
    ...
    STFUNC(StateWork) {
        switch (ev->GetTypeRewrite()) {
            HFunc(TEvPrivate::TEvQuotaDefaultResponse, Handle);
...
class THandlerActorYdbcQuotaGetDefault : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcQuotaGetDefault> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcQuotaGetDefault>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcQuotaGetDefault(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcQuotaGetDefault::StateWork)
        , Location(location)
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "GET" || (request->Method == "POST" && contentType == "application/json")) {
            ctx.Register(new THandlerActorYdbcQuotaGetDefaultRequest(Location, HttpProxyId, event->Sender, request));
            return;
        }
