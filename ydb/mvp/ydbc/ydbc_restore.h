...
class THandlerActorYdbcRestorePOST : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcRestorePOST> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcRestorePOST>;
    const TYdbcLocation& Location;
    TRequest Request;

    THandlerActorYdbcRestorePOST(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        yandex::cloud::priv::ydb::v1::RestoreBackupRequest cpRequest;
        ...
        connection->DoRequest(cpRequest, std::move(responseCb), &yandex::cloud::priv::ydb::v1::DatabaseService::Stub::AsyncRestore, meta);
        Become(&THandlerActorYdbcRestorePOST::StateWork, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
    ...
    void Handle(TEvPrivate::TEvOperationResponse::TPtr event, const NActors::TActorContext& ctx) {
        ...
        yandex::cloud::priv::ydb::v1::RestoreBackupMetadata meta;
...
class THandlerActorYdbcRestore : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcRestore> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcRestore>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcRestore(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcRestore::StateWork)
        , Location(location)
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            ctx.Register(new THandlerActorYdbcRestorePOST(Location, HttpProxyId, event->Sender, request));
            return;
        }
