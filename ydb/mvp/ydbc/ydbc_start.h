...
class THandlerActorYdbcDatabaseStarter : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDatabaseStarter> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDatabaseStarter>;
    ...
    THandlerActorYdbcDatabaseStarter(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        yandex::cloud::priv::ydb::v1::StartDatabaseRequest cpRequest;
        ...
        connection->DoRequest(cpRequest, std::move(responseCb), &yandex::cloud::priv::ydb::v1::DatabaseService::Stub::AsyncStart, meta);
    ...
    void ReplyAndDie(const NActors::TActorContext& ctx) {
        ...
        if (ErrorResponse == nullptr) {
            if (OperationResponse != nullptr) {
                ...
                yandex::cloud::priv::ydb::v1::StartDatabaseMetadata meta;
...
class THandlerActorYdbcStart : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcStart> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcStart>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcStart(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcStart::StateWork)
        , Location(location)
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            ctx.Register(new THandlerActorYdbcDatabaseStarter(Location, HttpProxyId, event->Sender, request));
            return;
        }
