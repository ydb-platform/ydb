...
class THandlerActorYdbcDatabaseStopper : THandlerActorYdbc, public NActors::TActorBootstrapped<THandlerActorYdbcDatabaseStopper> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorYdbcDatabaseStopper>;
    ...
    THandlerActorYdbcDatabaseStopper(
            const TYdbcLocation& location,
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        yandex::cloud::priv::ydb::v1::StopDatabaseRequest cpRequest;
        ...
        connection->DoRequest(cpRequest, std::move(responseCb), &yandex::cloud::priv::ydb::v1::DatabaseService::Stub::AsyncStop, meta);
        Become(&THandlerActorYdbcDatabaseStopper::StateWork, GetTimeout(), new NActors::TEvents::TEvWakeup());
    }
    ...
    void ReplyAndDie(const NActors::TActorContext& ctx) {
        ...
        if (ErrorResponse == nullptr) {
            if (OperationResponse != nullptr) {
                ...
                yandex::cloud::priv::ydb::v1::StopDatabaseMetadata meta;
...
class THandlerActorYdbcStop : THandlerActorYdbc, public NActors::TActor<THandlerActorYdbcStop> {
public:
    using TBase = NActors::TActor<THandlerActorYdbcStop>;
    const TYdbcLocation& Location;
    NActors::TActorId HttpProxyId;

    THandlerActorYdbcStop(const TYdbcLocation& location, const NActors::TActorId& httpProxyId)
        : TBase(&THandlerActorYdbcStop::StateWork)
        , Location(location)
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            ctx.Register(new THandlerActorYdbcDatabaseStopper(Location, HttpProxyId, event->Sender, request));
            return;
        }
