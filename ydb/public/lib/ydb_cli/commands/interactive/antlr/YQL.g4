...
lambda_body
    : SEMICOLON* (lambda_stmt SEMICOLON+)* RETURN expr SEMICOLON*
    ;
...
or_subexpr
    : and_subexpr (AND and_subexpr)*
    ;

and_subexpr
    : xor_subexpr (XOR xor_subexpr)*
    ;

xor_subexpr
    : eq_subexpr cond_expr?
    ;
...
cond_expr
    ...
    | (ISNULL | NOTNULL | IS NULL_ | (IS)? NOT NULL_)
...
eq_subexpr
    : neq_subexpr ((LESS | LESS_OR_EQ | GREATER | GREATER_OR_EQ) neq_subexpr)*
    ;
...
neq_subexpr
    : bit_subexpr ((SHIFT_LEFT | shift_right | ROT_LEFT | rot_right | AMPERSAND | PIPE | CARET) bit_subexpr)*
...
bit_subexpr
    : add_subexpr ((PLUS | MINUS) add_subexpr)*
    ;

add_subexpr
    : mul_subexpr ((ASTERISK | SLASH | PERCENT) mul_subexpr)*
    ;

mul_subexpr
    : con_subexpr (DOUBLE_PIPE con_subexpr)*
    ;

con_subexpr
    : unary_subexpr
    | unary_op unary_subexpr
    ;
...
unary_subexpr_suffix
    : (key_expr | invoke_expr | (DOT (bind_parameter | DIGITS | an_id_or_type)))* (COLLATE an_id)?
    ;

unary_casual_subexpr
    : (id_expr | atom_expr) unary_subexpr_suffix
    ;

in_unary_casual_subexpr
    : (id_expr_in | in_atom_expr) unary_subexpr_suffix
    ;
...
list_literal
    : LBRACE_SQUARE expr_list? COMMA? RBRACE_SQUARE
    ;
...
dict_literal
    : LBRACE_CURLY expr_dict_list? COMMA? RBRACE_CURLY
    ;
...
cast_expr
    : CAST LPAREN expr AS type_name_or_bind RPAREN
    ;

bitcast_expr
    : BITCAST LPAREN expr AS type_name_simple RPAREN
    ;

exists_expr
    : EXISTS LPAREN (select_stmt | values_stmt) RPAREN
    ;

case_expr
    : CASE expr? when_expr+ (ELSE expr)? END
    ;

lambda
    : smart_parenthesis (ARROW ((LPAREN expr RPAREN) | (LBRACE_CURLY lambda_body RBRACE_CURLY)))?
    ;
...
json_variable
    : expr AS json_variable_name
    ;

json_variables
    : json_variable (COMMA json_variable)*
    ;

json_common_args
    : expr COMMA jsonpath_spec (PASSING json_variables)?
    ;
...
json_value
    : JSON_VALUE LPAREN json_common_args (RETURNING type_name_simple)? (json_case_handler ON (EMPTY | ERROR))* RPAREN
    ;
...
json_exists
    : JSON_EXISTS LPAREN json_common_args json_exists_handler? RPAREN
    ;
...
pure_column_or_named_list
    : LPAREN pure_column_or_named (COMMA pure_column_or_named)* RPAREN
    ;
...
without_column_list
    : without_column_name (COMMA without_column_name)* COMMA?
    ;

named_expr
    : expr (AS an_id_or_type)?
    ;

named_expr_list
    : named_expr (COMMA named_expr)*
    ;

invoke_expr
    : LPAREN (opt_set_quantifier named_expr_list COMMA? | ASTERISK)? RPAREN invoke_expr_tail
    ;
...
key_expr
    : LBRACE_SQUARE expr RBRACE_SQUARE
    ;
...
bind_parameter
    : DOLLAR (an_id_or_type | TRUE | FALSE)
    ;
...
bind_parameter_list
    : bind_parameter (COMMA bind_parameter)*
    ;

named_bind_parameter
    : bind_parameter (AS bind_parameter)?
    ;

named_bind_parameter_list
    : named_bind_parameter (COMMA named_bind_parameter)*
    ;
...
struct_arg
    : type_name_tag COLON type_name_or_bind
    ;

struct_arg_positional
    : type_name_tag type_name_or_bind (NOT? NULL_)?
    | type_name_or_bind AS type_name_tag
    ; //deprecated
...
callable_arg
    : variant_arg (LBRACE_CURLY AUTOMAP RBRACE_CURLY)?
    ;

callable_arg_list
    : callable_arg (COMMA callable_arg)*
    ;

type_name_decimal
    : DECIMAL LPAREN integer_or_bind COMMA integer_or_bind RPAREN
    ;

type_name_optional
    : OPTIONAL LESS type_name_or_bind GREATER
    ;

type_name_tuple
    : TUPLE (LESS (type_name_or_bind (COMMA type_name_or_bind)* COMMA?)? GREATER | NOT_EQUALS2)
    ;

type_name_struct
    : STRUCT (LESS (struct_arg (COMMA struct_arg)* COMMA?)? GREATER | NOT_EQUALS2)
    ;

type_name_variant
    : VARIANT LESS variant_arg (COMMA variant_arg)* COMMA? GREATER
    ;

type_name_list
    : LIST LESS type_name_or_bind GREATER
    ;

type_name_stream
    : STREAM LESS type_name_or_bind GREATER
    ;

type_name_flow
    : FLOW LESS type_name_or_bind GREATER
    ;

type_name_dict
    : DICT LESS type_name_or_bind COMMA type_name_or_bind GREATER
    ;

type_name_set
    : SET LESS type_name_or_bind GREATER
    ;

type_name_enum
    : ENUM LESS type_name_tag (COMMA type_name_tag)* COMMA? GREATER
    ;

type_name_resource
    : RESOURCE LESS type_name_tag GREATER
    ;

type_name_tagged
    : TAGGED LESS type_name_or_bind COMMA type_name_tag GREATER
    ;
...
declare_stmt
    : DECLARE bind_parameter AS type_name (EQUALS literal_value)?
    ;
...
import_stmt
    : IMPORT module_path SYMBOLS named_bind_parameter_list
    ;

export_stmt
    : EXPORT bind_parameter_list
    ;

call_action
    : (bind_parameter | EMPTY_ACTION) LPAREN expr_list? RPAREN
    ;

inline_action
    : BEGIN define_action_or_subquery_body END DO
    ;
...
external_call_settings
    : external_call_param (COMMA external_call_param)*
    ;
...
row_pattern_recognition_clause
    : MATCH_RECOGNIZE LPAREN window_partition_clause? order_by_clause? row_pattern_measures? row_pattern_rows_per_match? row_pattern_common_syntax RPAREN
    ;
...
row_pattern_common_syntax
    : (AFTER MATCH row_pattern_skip_to)? row_pattern_initial_or_seek? PATTERN LPAREN row_pattern RPAREN row_pattern_subset_clause? DEFINE row_pattern_definition_list
    ;

row_pattern_skip_to
    : SKIP_RULE TO NEXT ROW
    | SKIP_RULE PAST LAST ROW
    | SKIP_RULE TO FIRST row_pattern_skip_to_variable_name
    | SKIP_RULE TO LAST row_pattern_skip_to_variable_name
    | SKIP_RULE TO row_pattern_skip_to_variable_name
    ;
...
row_pattern
    : row_pattern_term (PIPE row_pattern_term)*
    ;
...
row_pattern_permute
    : PERMUTE LPAREN row_pattern (COMMA row_pattern)* RPAREN
    ;
...
row_pattern_subset_item
    : row_pattern_subset_item_variable_name EQUALS LPAREN row_pattern_subset_rhs RPAREN
    ;
...
ext_order_by_clause
    : ASSUME? order_by_clause
    ;

group_by_clause
    : GROUP COMPACT? BY opt_set_quantifier grouping_element_list (WITH an_id)?
    ;
...
rollup_list
    : ROLLUP LPAREN ordinary_grouping_set_list RPAREN
    ;

cube_list
    : CUBE LPAREN ordinary_grouping_set_list RPAREN
    ;
...
hopping_window_specification
    : HOP LPAREN expr COMMA expr COMMA expr COMMA expr RPAREN
    ;
...
join_source
    : ANY? flatten_source (join_op ANY? flatten_source join_constraint?)*
    ;

named_column
    : column_name (AS an_id)?
    ;
...
flatten_source
    : named_single_source (FLATTEN ((OPTIONAL | LIST | DICT)? BY flatten_by_arg | COLUMNS))?
    ;

named_single_source
    : single_source row_pattern_recognition_clause? (((AS an_id) | an_id_as_compat) pure_column_list?)? (sample_clause | tablesample_clause)?
    ;
...
tablesample_clause
    : TABLESAMPLE sampling_mode LPAREN expr RPAREN repeatable_clause?
    ;
...
create_view_stmt
    : CREATE VIEW object_ref with_table_settings AS select_stmt
    ;
...
create_object_stmt
    : CREATE OBJECT (IF NOT EXISTS)? object_ref LPAREN TYPE object_type_ref RPAREN create_object_features?
    ;

create_object_features
    : WITH object_features
    ;
...
object_feature_kv
    : an_id_or_type EQUALS object_feature_value
    ;
...
table_partition_by
    : PARTITION BY HASH pure_column_list
    ;
...
table_tablestore
    : TABLESTORE simple_table_ref_core
    ;
...
alter_table_alter_column_family
    : ALTER FAMILY an_id SET an_id family_setting_value
    ;

alter_table_set_table_setting_uncompat
    : SET an_id table_setting_value
    ;

alter_table_set_table_setting_compat
    : SET LPAREN alter_table_setting_entry (COMMA alter_table_setting_entry)* RPAREN
    ;

alter_table_reset_table_setting
    : RESET LPAREN an_id (COMMA an_id)* RPAREN
    ;
...
alter_table_alter_changefeed
    : ALTER CHANGEFEED an_id changefeed_alter_settings
    ;

alter_table_drop_changefeed
    : DROP CHANGEFEED an_id
    ;
...
table_index
    : INDEX an_id table_index_type (WITH LPAREN an_id EQUALS an_id (COMMA an_id EQUALS an_id)* COMMA? RPAREN)? ON LPAREN an_id_schema (COMMA an_id_schema)* RPAREN (COVER LPAREN an_id_schema (COMMA an_id_schema)* RPAREN)?
    ;
...
changefeed
    : CHANGEFEED an_id WITH LPAREN changefeed_settings RPAREN
    ;
...
changefeed_settings_entry
    : an_id EQUALS changefeed_setting_value
    ;
...
family_entry
    : FAMILY an_id family_settings
    ;

family_settings
    : LPAREN (family_settings_entry (COMMA family_settings_entry)*)? RPAREN
    ;

family_settings_entry
    : an_id EQUALS family_setting_value
    ;
...
literal_value_list
    : LPAREN literal_value (COMMA literal_value)* RPAREN
    ;
...
create_group_stmt
    : CREATE GROUP role_name (WITH USER role_name (COMMA role_name)* COMMA?)?
    ;

alter_group_stmt
    : ALTER GROUP role_name ((ADD | DROP) USER role_name (COMMA role_name)* COMMA? | RENAME TO role_name)
    ;

drop_role_stmt
    : DROP (USER | GROUP) (IF EXISTS)? role_name (COMMA role_name)* COMMA?
    ;
...
permission_name_target
    : permission_name (COMMA permission_name)* COMMA?
    | ALL PRIVILEGES?
    ;

create_replication_stmt
    : CREATE ASYNC REPLICATION object_ref FOR replication_target (COMMA replication_target)* WITH LPAREN replication_settings RPAREN
    ;
...
action_or_subquery_args
    : opt_bind_parameter (COMMA opt_bind_parameter)*
    ;

define_action_or_subquery_stmt
    : DEFINE (ACTION | SUBQUERY) bind_parameter LPAREN action_or_subquery_args? RPAREN AS define_action_or_subquery_body END DEFINE
    ;

define_action_or_subquery_body
    : SEMICOLON* (sql_stmt_core (SEMICOLON+ sql_stmt_core)* SEMICOLON*)?
    ;

if_stmt
    : EVALUATE? IF expr do_stmt (ELSE do_stmt)?
    ;
...
set_target_list
    : LPAREN set_target (COMMA set_target)* RPAREN
    ;

// topics
create_topic_stmt
    : CREATE TOPIC topic_ref create_topic_entries? with_topic_settings?
    ;

create_topic_entries
    : LPAREN create_topic_entry (COMMA create_topic_entry)* RPAREN
    ;
...
with_topic_settings
    : WITH LPAREN topic_settings RPAREN
    ;
...
alter_topic_add_consumer
    : ADD topic_create_consumer_entry
    ;

topic_create_consumer_entry
    : CONSUMER an_id topic_consumer_with_settings?
    ;
...
alter_topic_drop_consumer
    : DROP CONSUMER topic_consumer_ref
    ;

topic_alter_consumer_set
    : SET LPAREN topic_consumer_settings RPAREN
    ;
...
alter_topic_set_settings
    : SET LPAREN topic_settings RPAREN
    ;

alter_topic_reset_settings
    : RESET LPAREN an_id (COMMA an_id_pure)* RPAREN
    ;
...
topic_settings_entry
    : an_id EQUALS topic_setting_value
    ;
...
topic_consumer_with_settings
    : WITH LPAREN topic_consumer_settings RPAREN
    ;
...
topic_consumer_settings_entry
    : an_id EQUALS topic_consumer_setting_value
    ;
...
window_specification
    : LPAREN window_specification_details RPAREN
    ;
...
window_partition_clause
    : PARTITION COMPACT? BY named_expr_list
    ;
...
cluster_expr
    : (an_id COLON)? (pure_column_or_named | ASTERISK)
    ;
...
QUOTE_DOUBLE
    : '"'
    ; // This comment for fix syntax highlighting "
...
ID_QUOTED
    : BACKTICK ID_QUOTED_CORE* BACKTICK
    ;
