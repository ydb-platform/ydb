...
Y_UNIT_TEST_SUITE(TTaskQueueTest) {
    ...
    TTransactionTask MakeTransactionTask(int& counter) {
        counter++;
    ...
    Y_UNIT_TEST(ShouldExecuteTerminalTaskWithSleepsAndTransactions) {
        ...
        auto queue = CreateTaskQueue(1, 10, 10, log);
    ...
    TTransactionTask MakeFailingTransactionTask() {
    ...
    Y_UNIT_TEST(ShouldHandleMultipleTerminals) {
        ...
        auto log = std::make_shared<TLog>(CreateLogBackend("cerr", TLOG_ERR));
        auto queue = CreateTaskQueue(4, numTerminals, numTerminals, log);
    ...
    Y_UNIT_TEST(ShouldHandleQueueLimits) {
        ...
        auto queue = CreateTaskQueue(1, maxTerminals, maxTransactions, log);
