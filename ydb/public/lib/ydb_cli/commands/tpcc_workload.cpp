...
void TTPCCRunCommand::Config(TConfig& config) {
    ...
    config.Opts->AddLongOption("retry-count", "Retry count for failed transactions.")
        .DefaultValue(static_cast<int>(EWorkloadConstants::TPCC_RETRY_COUNT)).StoreResult(&RunParams.RetryCount);
