...
int TCommandExportToS3::Run(TConfig& config) {
    ...
    // Along with the encryption feature YDB supported recursive directories handling.
    // So if we use encryption, then we can suppose that YDB already supports expanding of items.
    const bool expandItems = (!encryption || !ExclusionPatterns.empty());
