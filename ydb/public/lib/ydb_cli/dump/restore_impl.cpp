...
constexpr bool IsSystemDir(const TString& dbPath) {
    return dbPath.EndsWith("~")
        || dbPath.EndsWith(".sys")
        || dbPath.EndsWith(".metadata");
}
    ...
    const Ydb::Table::DescribeSystemViewResult& actualProto)
{
    ...
    for (const auto& dumpedCol : dumpedColumns) {
        ...
        TString dumpedTypeStr = dumpedCol.type().SerializeAsString();
        TString actualTypeStr = actualCol.type().SerializeAsString();

        if (dumpedTypeStr != actualTypeStr) {
            return Result<TRestoreResult>(EStatus::SCHEME_ERROR, TStringBuilder()
                << "Column type mismatch for " << TString(dumpedCol.name()).Quote()
                << ": types differ between dump and actual system view");
        }
    }
    ...
    THashSet<TString> actualPrimaryKeysSet;
    for (const auto& key : actualPrimaryKeys) {
        actualPrimaryKeysSet.insert(key);
    }

    for (const auto& dumpedKey : dumpedPrimaryKeys) {
        if (!actualPrimaryKeysSet.contains(dumpedKey)) {
            return Result<TRestoreResult>(EStatus::SCHEME_ERROR, TStringBuilder()
                << "Primary key column " << TString(dumpedKey).Quote()
                << " from dump is missing in actual system view primary key");
        }
    }
        ...
        const TRestoreSettings& settings)
{
    ...
    if (IsSystemDir(dbPath)) {
        if (auto result = CheckExistenceAndType(dbPath, ESchemeEntryType::SysView); !result.IsSuccess()) {
