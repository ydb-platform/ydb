...
void ExplicitTcl(TQueryClient client, const TString& path) {
    ...
    ThrowOnError(client.RetryQuerySync([path](TQueryClient client) {
        return [&client, &path](const TInstant& airDate) -> TStatus {
...
void StreamQuerySelect(TQueryClient client, const TString& path) {
    ...
    ThrowOnError(client.RetryQuerySync([path, &resultSets](TQueryClient client) {
        return [&client, &path](std::vector<TResultSet>& resultSets) -> TStatus {
            resultSets.clear();
            auto query = Sprintf(R"(
                --!syntax_v1
                PRAGMA TablePathPrefix("%s");
