...
namespace NQuantizer {
    ...
    // static std::string_view Int8 = "int8";
    // static std::string_view Uint8 = "uint8";
    static std::string_view Bit = "bit";
}
...
static void CreateFlatBit(TTableClient& client, const TOptions& options) {
...
static void CreateKMeansNone(TTableClient& client, const TOptions& options) {
    ...
    ThrowOnError(client.RetryOperationSync([&](TSession session) {
        auto desc = TTableBuilder()
                        .AddNonNullableColumn(parentPK, EPrimitiveType::Uint64)
                        .AddNonNullableColumn(options.PrimaryKey, EPrimitiveType::Uint64)
                        .AddNullableColumn(options.Embedding, EPrimitiveType::String)
...
struct TBulkWriter {
    TBulkWriter(TTableClient& client, const TOptions& options)
        : Client{client}
        , Options{options}
        , Table{FullName(options, IndexName(options))}
    {
        Rows.BeginList();
    }

    void operator()(TId parentId, TId id, TRawEmbedding rawEmbedding) {
        Rows.AddListItem()
            .BeginStruct()
    ...
    void operator()(TId parentId, TId id, TEmbedding embedding) {
        ...
        *data = 1;
        operator()(parentId, id, rawEmbedding);
    }
