...
struct TBulkWriter {
    ...
    void UseLevel(ui8 level) {
        auto indexTable = FullIndexName(Options);
        Embeddings = Options.LastLevelEmbeddings;
        if (level < Options.Levels) {
            Embeddings = true;
...
static void UpdateKMeansNone(TTableClient& client, const TOptions& options) {
    ...
    TBulkWriter writer{options, client, reader.Rows()};
    ...
    writer.DropLevel(options.Levels);
...
static void TopKFlatBit(TTableClient& client, const TOptions& options) {
    TString query = std::format(R"($TargetEmbeddingFloat = Knn::ToBinaryStringFloat($Target);
