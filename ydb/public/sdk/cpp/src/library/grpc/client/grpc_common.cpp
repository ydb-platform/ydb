...
TString BuildUserAgent(const TStringBuf userAgentPrefix) {
    static const TString revision = GetRevision();
    return userAgentPrefix.empty()
        ? TStringBuilder() << "ydb" << '/' << revision
        : TStringBuilder() << userAgentPrefix << '/' << revision;
}
...
std::shared_ptr<grpc::ChannelInterface> CreateChannelInterface(const TGRpcClientConfig& config, grpc_socket_mutator* mutator) {
    ...
    if (config.UserAgentPrefix.has_value()) {
        args.SetUserAgentPrefix(NYdb::TStringType{BuildUserAgent(config.UserAgentPrefix.value())});
    }
