...
TEST(RelativeDatabase, RepeatedSessionsAndRootQualifiedResources) {
    ...
    // The second root segment is a real directory, not another prefix to strip.
    ...
    const std::vector<std::pair<std::string, std::string>> tables{
        ...
        {database + '/' + root + "/Config", "repeated-root"},
    ...
    for (const auto& spelling : {database, database.substr(1)}) {
        ...
        for (size_t iteration = 0; iteration < 3; ++iteration) {
            ...
            for (const auto& path : {database + "/Config", database.substr(1) + "/Config", std::string("Config")}) {
                AssertReadTable(session, path, "target");
            }
            AssertReadTable(session, database.substr(1) + '/' + root + "/Config", "repeated-root");
