...
void ensureMessageTypesSame(const Descriptor* a, const Descriptor* b, std::unordered_set<std::string>* visitedTypes);
void ensureFieldDescriptorsSame(const FieldDescriptor* a, const FieldDescriptor* b, std::unordered_set<std::string>* visitedTypes) {
    UNIT_ASSERT(a);
    UNIT_ASSERT(b);
...
Y_UNIT_TEST_SUITE(IssueProtoTest) {
    ...
    Y_UNIT_TEST(BinarySerialization) {
        ...
        std::string bin = IssueToBinaryMessage(issueTo);
...
Y_UNIT_TEST_SUITE(ToStreamTest) {
    ...
    void CheckSerializationToStream(const TIssues& issues, const std::string& expected) {
