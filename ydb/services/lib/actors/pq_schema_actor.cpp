...
#include <ydb/services/persqueue_v1/actors/schema/pqv1/common.h>
...
namespace NKikimr::NGRpcProxy::V1 {
    ...
    Ydb::StatusIds::StatusCode FillProposeRequestImpl( // create and alter
            ...
            NKikimrSchemeOp::TModifyScheme& modifyScheme, const TActorContext&,
            bool /*alter*/, TString& error, const TString& path, const TString& database, const TString& localDc
    ) {
        auto result = NPQv1::ApplyCreate(modifyScheme, settings, path, name, database, localDc);
        if (!result) {
            error = result.GetErrorMessage();
            return result.GetStatus();
        }
