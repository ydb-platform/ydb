...
namespace NKikimr::NGRpcProxy::V1 {
    ...
    TString ProcessAlterConsumer(Ydb::Topic::Consumer& consumer, const Ydb::Topic::AlterConsumer& alter) {
        ...
        for (auto& [name, value] : alter.alter_attributes()) {
            (*consumer.mutable_attributes())[name] = value;
        }
    ...
    Ydb::StatusIds::StatusCode ProcessAttributes(const ::google::protobuf::Map<TProtoStringType, TProtoStringType>& attributes, NKikimrSchemeOp::TPersQueueGroupDescription* pqDescr, TString& error, bool alter) {
        ...
        for (const auto& [name, value] : attributes) {
            if (name == "_partitions_per_tablet") {
                try {
                    if (!alter)
            ...
            } else if (name == "_allow_unauthenticated_read") {
                if (value.empty()) {
            ...
            } else if (name == "_allow_unauthenticated_write") {
                if (value.empty()) {
            ...
            } else if (name == "_abc_slug") {
                config->SetAbcSlug(value);
            }  else if (name == "_federation_account") {
                config->SetFederationAccount(value);
            } else if (name == "_abc_id") {
                if (value.empty()) {
                    config->SetAbcId(0);
                } else {
                    try {
            ...
            } else if (name == "_max_partition_storage_size") {
                if (value.empty()) {
            ...
            }  else if (name == "_message_group_seqno_retention_period_ms") {
                ...
                if (!value.empty()) {
                    try {
                        ...
                        error = TStringBuilder() << "Attribute " << name << " is " << value << ", which is not ui64";
                        return Ydb::StatusIds::BAD_REQUEST;
                    }
                }

            } else if (name == "_max_partition_message_groups_seqno_stored") {
                partConfig->SetSourceIdMaxCounts(NKikimrPQ::TPartitionConfig().GetSourceIdMaxCounts());
                if (!value.empty()) {
                    try {
                        i64 count = FromString<i64>(value);
                        if (count < 0) {
                            error = TStringBuilder() << name << "can't be negative, provided " << count;
                            return Ydb::StatusIds::BAD_REQUEST;
                        }
                        if (count > 0) {
            ...
            } else if (name == "_cleanup_policy") {
                config->SetEnableCompactification(value == "compact");
            } else if (name == "_timestamp_type") {
                if (!value || value == NKafka::MESSAGE_TIMESTAMP_CREATE_TIME || value == NKafka::MESSAGE_TIMESTAMP_LOG_APPEND) {
                    config->SetTimestampType(value ? value :  NKafka::MESSAGE_TIMESTAMP_CREATE_TIME);
                } else {
                    error = TStringBuilder() << "Attribute " << name << " is " << value << ", which is an incorrect value.";
                    return Ydb::StatusIds::BAD_REQUEST;
                }
            } else {
                error = TStringBuilder() << "Attribute " << name << " is not supported";
                return Ydb::StatusIds::BAD_REQUEST;
            }
        }
