...
class ISchemeObjectOperationsManager {
public:
    using TPtr = std::shared_ptr<ISchemeObjectOperationsManager>;

    virtual TOperationParsingResult BuildPatchFromSettings(const NYql::TObjectSettingsImpl& settings, NSchemeShard::TSchemeShard& context) const = 0;
};

template <class T>
class TSchemeObjectOperationsManager: public IOperationsManager, ISchemeObjectOperationsManager {
private:
...
public:
    ...
    using TPreprocessingResult = TConclusion<NYql::TObjectSettingsImpl>;
...
protected:
    ...
    virtual NThreading::TFuture<TPreprocessingResult> DoPreprocessSettings(
        const NYql::TObjectSettingsImpl& settings, TInternalModificationContext& context) const = 0;

public:
    TOperationParsingResult BuildPatchFromSettings(const NYql::TObjectSettingsImpl& settings, NSchemeShard::TSchemeShard& context) const override {
...
protected:
        ...
        auto promise = NThreading::NewPromise<TYqlConclusionStatus>();
        {
            ...
            switch (context.GetActivityType()) {
                ...
                case EActivityType::Create:
                    modifyObjectCommand = std::make_shared<TCreateObjectCommand<T>>(
                        patch.GetResult(), manager, std::move(controller), context, settings.GetExistingOk());
                    break;
                ...
                case EActivityType::Drop:
                    modifyObjectCommand = std::make_shared<TDeleteObjectCommand<T>>(
                        patch.GetResult(), manager, std::move(controller), context, settings.GetMissingOk());
                    break;
            ...
            TActivationContext::Send(
                new IEventHandle(NProvider::MakeServiceId(nodeId), {}, new NProvider::TEvObjectsOperation(modifyObjectCommand)));
        }
        return promise;
    }
    ...
    virtual NThreading::TFuture<TYqlConclusionStatus> ExecutePrepared(const NKqpProto::TKqpSchemeOperation& schemeOperation, const ui32 nodeId,
        ...
        switch (schemeOperation.GetOperationCase()) {
            case NKqpProto::TKqpSchemeOperation::kUpsertObject:
                ...
                modifyObjectCommand = std::make_shared<TUpsertObjectCommand<T>>(
                    BuildPatchFromProto(schemeOperation.GetUpsertObject()), manager, std::move(controller), internalContext);
                break;
            case NKqpProto::TKqpSchemeOperation::kCreateObject:
                internalContext.SetActivityType(EActivityType::Create);
                modifyObjectCommand = std::make_shared<TCreateObjectCommand<T>>(BuildPatchFromProto(schemeOperation.GetCreateObject()), manager,
                    std::move(controller), internalContext, schemeOperation.GetCreateObject().GetSuccessOnAlreadyExists());
                break;
            case NKqpProto::TKqpSchemeOperation::kAlterObject:
                internalContext.SetActivityType(EActivityType::Alter);
                modifyObjectCommand = std::make_shared<TUpdateObjectCommand<T>>(
                    BuildPatchFromProto(schemeOperation.GetAlterObject()), manager, std::move(controller), internalContext);
                break;
            case NKqpProto::TKqpSchemeOperation::kDropObject:
                internalContext.SetActivityType(EActivityType::Drop);
                modifyObjectCommand = std::make_shared<TDeleteObjectCommand<T>>(BuildPatchFromProto(schemeOperation.GetDropObject()), manager,
                    std::move(controller), internalContext, schemeOperation.GetDropObject().GetSuccessOnNotExist());
                break;
            default:
