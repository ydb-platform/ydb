...
class TDescribeLogicActor: public NActors::TActorBootstrapped<TDerived>
...
public:
    TDescribeLogicActor(const NActors::TActorId& parent, TDescribeSettings&& settings)
    ...
    void Bootstrap() {
        ...
        DescriberActorId = this->RegisterWithSameMailbox(NPQ::NDescriber::CreateDescriberActor(
            this->SelfId(),
        ...
        this->Become(&TDescribeLogicActor::StateDescribe);
    ...
    void PassAway() override {
        LOG_D("PassAway");
        if (DescriberActorId) {
            this->Send(DescriberActorId, new NActors::TEvents::TEvPoison());
            DescriberActorId = {};
        }
...
protected:
        ...
        Ydb::PersQueue::ErrorCode::ErrorCode issueCode = Ydb::PersQueue::ErrorCode::BAD_REQUEST)
    {
        ...
        this->Send(Parent, response.release());
        IsDead = true;
        PassAway();
    }
    ...
    void Handle(NPQ::NDescriber::TEvDescribeTopicsResponse::TPtr& ev) {
        ...
        this->Become(&TDescribeLogicActor::StateWork);
    }
