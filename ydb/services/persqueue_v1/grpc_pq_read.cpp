...
void TPQReadService::HandleReadInfo(TAutoPtr<NActors::IEventHandle>& evHandle, const TActorContext& ctx) {
    TEvPQReadInfoRequest* ev = static_cast<TEvPQReadInfoRequest*>(evHandle->GetBase());
    ...
    if (HaveClusters && (Clusters.empty() || LocalCluster.empty())) {
        ...
        auto* evBase = evHandle->ReleaseBase().Release();
        Y_UNUSED(evBase); // == ev
        ctx.Register(new TReadInfoActor(ev, *TopicsHandler, SchemeCache, NewSchemeCache, Counters));
    }
}
