...
namespace NKikimr::NSqsTopic::V1 {
    ...
    struct TSendMessageItem {
        ...
        TMaybe<TMessageError> ValidationError;
    };
    ...
    private:
        ...
        void Handle(TEvTabletPipe::TEvClientConnected::TPtr& ev, const TActorContext& ctx) {
            ...
            if (ev->Get()->Status != NKikimrProto::EReplyStatus::OK) {
                ReplyWithError(
                    ctx, TStringBuilder() << "Cannot connect to tablet " << TabletId,
        ...
        void Handle(TEvTabletPipe::TEvClientDestroyed::TPtr& ev, const TActorContext& ctx) {
            Y_UNUSED(ev);
            ReplyWithError(ctx, TStringBuilder() << "Connection to tablet " << TabletId << " is dead", Ydb::StatusIds::UNAVAILABLE, NYds::EErrorCodes::ERROR);
        }
        ...
        void ReplyWithError(const NActors::TActorContext& ctx, const TString& errorText, Ydb::StatusIds::StatusCode statusCode, NYds::EErrorCodes issueCode) {
            auto result = MakeHolder<NSqsTopic::V1::TEvSqsTopic::TEvPartitionActorResult>();
            result->PartitionId = Partition;
            for (ui32 i = 0; i < DataToWrite.size(); ++i) {
        ...
        bool CheckForError(TEvPersQueue::TEvResponse::TPtr& ev, const NActors::TActorContext& ctx) {
            ...
            if (record.HasErrorCode() && record.GetErrorCode() != NPersQueue::NErrorCode::OK) {
                ReplyWithError(ctx, record.GetErrorReason(), Ydb::StatusIds::UNAVAILABLE, NYds::EErrorCodes::ERROR);
                return true;
            }
            if (!ev->Get()->Record.HasPartitionResponse()) {
                ReplyWithError(ctx, "Missing partition response", Ydb::StatusIds::INTERNAL_ERROR, NYds::EErrorCodes::ERROR);
                return true;
            }
            if (ev->Get()->Record.GetPartitionResponse().GetCmdWriteResult().size() != std::ssize(DataToWrite)) {
                ReplyWithError(ctx, std::format("Partition response length mismatch: {} != {}", ev->Get()->Record.GetPartitionResponse().GetCmdWriteResult().size(), std::ssize(DataToWrite)), Ydb::StatusIds::INTERNAL_ERROR, NYds::EErrorCodes::ERROR);
                return true;
            }
            return false;
        }
    ...
    public:
        ...
        void Bootstrap(const NActors::TActorContext& ctx) {
            ...
            if (this->Request().queue_url().empty()) {
                return this->ReplyWithError(Ydb::StatusIds::BAD_REQUEST, static_cast<size_t>(NYds::EErrorCodes::MISSING_PARAMETER),
                                            "No QueueUrl parameter.");
            }
            if (!QueueUrl_.has_value()) {
                return this->ReplyWithError(Ydb::StatusIds::BAD_REQUEST, static_cast<size_t>(NYds::EErrorCodes::INVALID_ARGUMENT),
                                            "Invalid QueueUrl");
            }
        ...
        bool ValidateMessages() {
            if (Items.size() < NSQS::TLimits::MinBatchSize) {
                this->ReplyWithError(Ydb::StatusIds::BAD_REQUEST, static_cast<size_t>(NYds::EErrorCodes::INVALID_ARGUMENT),
                                            std::format("Batch size is less than {}", NSQS::TLimits::MinBatchSize));
                return false;
            }
            if (Items.size() > NSQS::TLimits::MaxBatchSize) {
                this->ReplyWithError(Ydb::StatusIds::BAD_REQUEST, static_cast<size_t>(NYds::EErrorCodes::INVALID_ARGUMENT),
                                            std::format("Batch size is greater than {}", NSQS::TLimits::MaxBatchSize));
                return false;
            }
            ...
            for (auto& item : Items) {
                ...
                if (!unique) {
                      this->ReplyWithError(Ydb::StatusIds::BAD_REQUEST, static_cast<size_t>(NYds::EErrorCodes::INVALID_ARGUMENT),
                                            std::format("Duplicated batch id: {}", item.BatchId.Quote().c_str()));
                    return false;
                }
            }
            return true;
        ...
        void Handle(NPQ::NDescriber::TEvDescribeTopicsResponse::TPtr& ev, const TActorContext& ctx) {
            ...
            AFL_ENSURE(response->Topics.size() == 1)("#topics", response->Topics.size());
            ...
            if (info.Status != NKikimr::NPQ::NDescriber::EStatus::SUCCESS) {
                ...
                return this->ReplyWithError(
                    describerStatus.Error.StatusCode,
                    static_cast<size_t>(describerStatus.Error.IssueCode),
                    describerStatus.Error.Text);
            }
    ...
    static void FillSingleFailedMessage(const TMessageError& error, const TSendMessageItem& item, const TString& method, auto& result) {
        Y_UNUSED(method);
        result.set_code(std::format("status: {}, issue: {}", static_cast<i64>(error.StatusCode), static_cast<i64>(error.IssueCode))); // TODO: use proper names
        result.set_id(item.BatchId);
        result.set_sender_fault(error.StatusCode != Ydb::StatusIds::SUCCESS);
        result.set_message(error.Text);
        Y_ASSERT(result.IsInitialized());
    }
    ...
    public:
        ...
        void ReplyAndDie(const TActorContext& ctx) {
            if (Items.size() != 1) {
                this->ReplyWithError(Ydb::StatusIds::INTERNAL_ERROR, static_cast<size_t>(NYds::EErrorCodes::ERROR), "Unexpected number of messages to write");
            }
            const TSendMessageItem& item = Items.front();
            if (item.ValidationError.Defined()) {
                this->ReplyWithError(item.ValidationError->StatusCode, static_cast<size_t>(item.ValidationError->IssueCode), item.ValidationError->Text);
            }
            for (auto&& message : Result->Get()->Messages) {
                if (message.Error.Defined()) {
                    this->ReplyWithError(message.Error->StatusCode, static_cast<size_t>(message.Error->IssueCode), message.Error->Text);
                }
            }
            if (Result->Get()->Messages.size() != 1) {
                this->ReplyWithError(Ydb::StatusIds::INTERNAL_ERROR, static_cast<size_t>(NYds::EErrorCodes::ERROR), "Unexpected number of messages");
            }
        ...
        void ReplyAndDie(const TActorContext& ctx) {
            ...
            for (const TSendMessageItem& item : Items) {
                ...
                if (item.ValidationError.Defined()) {
                    ...
                    return this->ReplyWithError(Ydb::StatusIds::INTERNAL_ERROR, static_cast<size_t>(NYds::EErrorCodes::ERROR), "Missing write response");
