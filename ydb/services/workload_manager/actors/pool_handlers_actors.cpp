...
public:
    ...
    STRICT_STFUNC(StateFuncBase,
        ...
        IgnoreFunc(NKqp::TEvKqp::TEvCancelQueryResponse);
    ...
    virtual void PassAway() override {
        ...
        this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), new TEvPrivate::TEvStopPoolHandlerResponse(DatabaseId, PoolId));
...
private:
    void HandlePoison() {
        YDB_LOG_WARN("[WorkloadService] Got poison, stop pool handler",
            {"logPrefix", LogPrefix()});
        this->PassAway();
    }
    ...
    void Handle(TEvPrivate::TEvResolvePoolResponse::TPtr& ev) {
        ...
        YDB_LOG_DEBUG("[WorkloadService] Received new request, request",
            {"logPrefix", LogPrefix()},
            {"workerId", workerActorId},
            {"sessionId", sessionId},
            {"text", requestText});
    ...
    void Handle(TEvPrivate::TEvCancelRequest::TPtr& ev) {
        ...
        YDB_LOG_DEBUG("[WorkloadService] Cancel request by deadline",
            {"logPrefix", LogPrefix()},
            {"workerId", request->WorkerActorId},
    ...
    void Handle(TEvTxProxySchemeCache::TEvWatchNotifyUpdated::TPtr& ev) {
        ...
        if (!result) {
            YDB_LOG_WARN("[WorkloadService] Got empty notification",
                {"logPrefix", LogPrefix()});
            return;
        }
        ...
        YDB_LOG_DEBUG("[WorkloadService] Got watch notification",
            {"logPrefix", LogPrefix()});
    ...
    void Handle(TEvTxProxySchemeCache::TEvWatchNotifyDeleted::TPtr& ev) {
        ...
        YDB_LOG_DEBUG("[WorkloadService] Got delete notification",
            {"logPrefix", LogPrefix()});
        SendPoolInfoUpdate(std::nullopt, std::nullopt, Subscribers);
    }
...
protected:
    ...
    void RemoveRequest(TRequest* request) {
        ...
        this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), event.release());

        LocalSessions.erase(request->SessionId);
        if (StopHandler && LocalSessions.empty()) {
            YDB_LOG_INFO("[WorkloadService] All requests finished, stop handler",
                {"logPrefix", LogPrefix()});
            PassAway();
        }
    }
...
private:
    ...
    void ReplyCancel(const TRequest* request) {
        auto ev = std::make_unique<NKqp::TEvKqp::TEvCancelQueryRequest>();
        ev->Record.MutableRequest()->SetSessionId(request->SessionId);
        this->Send(NKqp::MakeKqpProxyID(this->SelfId().NodeId()), ev.release());
    ...
    bool UpdateSchemeboardSubscription(TPathId pathId) {
        ...
        if (WatchPathId) {
            YDB_LOG_INFO("[WorkloadService] Pool path has changed, new path",
                {"logPrefix", LogPrefix()},
                {"id", pathId});
        ...
        YDB_LOG_DEBUG("[WorkloadService] Subscribed on schemeboard notifications",
            {"logPrefix", LogPrefix()},
            {"path", pathId});
    ...
    void UpdatePoolConfig(const NResourcePool::TPoolSettings& poolConfig) {
        ...
        YDB_LOG_DEBUG("[WorkloadService] Pool config has changed, queue in flight",
            {"logPrefix", LogPrefix()},
            {"size", poolConfig.QueueSize},
            {"limit", poolConfig.ConcurrentQueryLimit});
        ...
        if (ShouldResign()) {
            ...
            this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), new TEvPrivate::TEvResignPoolHandler(DatabaseId, PoolId, newHandler));
        }
    }
...
protected:
    ...
    void OnScheduleRequest(TRequest* request) override {
        ...
        YDB_LOG_DEBUG("[WorkloadService] Request placed into pending queue, session request queue",
            {"logPrefix", LogPrefix()},
            {"id", request->SessionId},
            {"text", request->RequestText},
        ...
        if (!PreparingFinished) {
            this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), new TEvPrivate::TEvPrepareTablesRequest(DatabaseId, PoolId));
        }
    ...
    void RefreshState(bool refreshRequired = false) override {
        if (!WaitingNodesInfo && TInstant::Now() - LastNodesInfoRefreshTime > LEASE_DURATION) {
            ...
            this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), new TEvPrivate::TEvNodesInfoRequest());
        }
...
private:
    void HandleRefreshState() {
        ...
        YDB_LOG_TRACE("[WorkloadService] Try to start scheduled refresh",
            {"logPrefix", LogPrefix()});
    ...
    void HandleExternalRefreshState() {
        YDB_LOG_DEBUG("[WorkloadService] Got remote refresh request",
            {"logPrefix", LogPrefix()});
        RefreshState(true);
    }
    ...
    void Handle(TEvPrivate::TEvRefreshPoolStateResponse::TPtr& ev) {
        ...
        YDB_LOG_TRACE("[WorkloadService] successfully refreshed pool state",
            {"logPrefix", LogPrefix()},
            {"flight", GlobalState.RunningRequests},
    ...
    void Handle(TEvPrivate::TEvDelayRequestResponse::TPtr& ev) {
        ...
        if (request) {
            ...
            YDB_LOG_DEBUG("[WorkloadService] successfully delayed request, session",
                {"logPrefix", LogPrefix()},
                {"id", sessionId});
        }
    ...
    void Handle(TEvPrivate::TEvCpuQuotaResponse::TPtr& ev) {
        ...
        if (!ev->Get()->QuotaAccepted) {
            YDB_LOG_DEBUG("[WorkloadService] Skipped request start due to load cpu threshold",
                {"logPrefix", LogPrefix()});
    ...
    void Handle(TEvPrivate::TEvStartRequestResponse::TPtr& ev) {
        ...
        if (SelfId().NodeId() != nodeId) {
            YDB_LOG_DEBUG("[WorkloadService] first request in queue is remote, send notification to node",
                {"logPrefix", LogPrefix()},
                {"nodeId", nodeId});
            ...
            this->Send(NWorkloadManager::MakeServiceId(nodeId), std::move(event));
            RefreshState();
            return;
        }

        YDB_LOG_DEBUG("[WorkloadService] started request, session",
            {"logPrefix", LogPrefix()},
            {"id", sessionId});
    ...
    void Handle(TEvPrivate::TEvCleanupRequestsResponse::TPtr& ev) {
        ...
        for (const TString& sessionId : ev->Get()->SesssionIds) {
            YDB_LOG_TRACE("[WorkloadService] cleanuped request, session",
                {"logPrefix", LogPrefix()},
                {"id", sessionId});
...
private:
    ...
    void RequestCpuQuota(double loadCpuThreshold, EStartRequestCase requestCase) const {
        this->Send(NWorkloadManager::MakeServiceId(this->SelfId().NodeId()), new TEvPrivate::TEvCpuQuotaRequest(loadCpuThreshold / 100.0), 0, static_cast<ui64>(requestCase));
    }
