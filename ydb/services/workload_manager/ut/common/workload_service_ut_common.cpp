#include "workload_service_ut_common.h"
...
public:
    TQueryRunnerActor(std::unique_ptr<NKqp::TEvKqp::TEvQueryRequest> request, TPromise<TQueryRunnerResult> promise, const TQueryRunnerSettings& settings, ui32 targetNodeId)
    ...
    void Bootstrap() {
        ...
        Send(NKqp::MakeKqpProxyID(TargetNodeId_), std::move(Request_));
...
private:
    std::unique_ptr<NKqp::TEvKqp::TEvQueryRequest> Request_;
...
private:
    ...
    std::unique_ptr<NKqp::TEvKqp::TEvQueryRequest> GetQueryRequest(const TString& query, const TQueryRunnerSettings& settings) const {
    ...
    NMonitoring::TDynamicCounterPtr GetWorkloadManagerCounters(ui32 nodeIndex) const {
        return NWorkloadManager::GetWorkloadManagerCounters(GetRuntime()->GetAppData(nodeIndex).Counters);
    }
