...
void CheckDecision(std::function<void(NResourcePool::TPoolSettings&)> setup, EExpected expected) {
    ...
    switch (expected) {
        case EExpected::Bypass:
            UNIT_ASSERT_C(std::holds_alternative<NWorkloadManager::IQueryClassifier::TBypass>(result),
        ...
        case EExpected::ResolvedAdmission: {
            ...
            UNIT_ASSERT_C(!std::get<NWorkloadManager::IQueryClassifier::TResolvedPoolId>(result).SkipAdmission,
