...
Y_UNIT_TEST_SUITE(BackupRestoreS3) {
    ...
    void TestBackupCompression(const TDriver& driver, ui16 s3Port, const char* table,
        TString compression, EStatus expectedStatus)
    {
        const auto exportSettings = NExport::TExportToS3Settings()
            .Endpoint(Sprintf("localhost:%u", s3Port))
            .Scheme(ES3Scheme::HTTP)
            ...
            .AccessKey("test_key")
            .SecretKey("test_secret")
            .AppendItem(NExport::TExportToS3Settings::TItem{.Src = table, .Dst = "table"})
            .Compression(compression);

        NExport::TExportClient exportClient(driver);
        auto response = exportClient.ExportToS3(exportSettings).ExtractValueSync();
        UNIT_ASSERT_VALUES_EQUAL(response.Status().GetStatus(), expectedStatus);
    }

    Y_UNIT_TEST(ZstdCompressionLevelValidation) {
        TS3TestEnv testEnv;
        constexpr const char* table = "/Root/table";

        auto& session = testEnv.GetSession();
        ExecuteDataDefinitionQuery(session, Sprintf(R"(
            CREATE TABLE `%s` (
                Key Uint32,
                Value Utf8,
        ...
        const auto& driver = testEnv.GetDriver();
        auto s3Port = testEnv.GetS3Port();

        TestBackupCompression(driver, s3Port, table, "zstd-0", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd-23", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd-", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd-a", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd--1", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd-2147483648", EStatus::BAD_REQUEST);
        TestBackupCompression(driver, s3Port, table, "zstd--2147483649", EStatus::BAD_REQUEST);

        TestBackupCompression(driver, s3Port, table, "zstd-1", EStatus::SUCCESS);
        TestBackupCompression(driver, s3Port, table, "zstd-11", EStatus::SUCCESS);
        TestBackupCompression(driver, s3Port, table, "zstd-22", EStatus::SUCCESS);
    }
}
