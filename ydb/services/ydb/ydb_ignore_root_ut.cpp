        ...
        if (success) {
            ...
            if constexpr (std::is_same_v<TResponse, Ydb::Operations::GetOperationResponse>) {
                if (!response.operation().ready()
                    && (StatusCode(response) == Ydb::StatusIds::STATUS_CODE_UNSPECIFIED
                        || StatusCode(response) == Ydb::StatusIds::SUCCESS)) {
                    return response;
                }
            }
...
Y_UNIT_TEST_SUITE(YdbIgnoreRoot) {
    ...
    Y_UNIT_TEST_TWIN(MissingAndRelativeDatabases, SingleComponent) {
        for (bool ignoreRoot : {false, true}) {
            ...
            env.TableSession(SingleComponent ? env.Root : env.Name);
            for (const TString& database : {TString("ru"), TString("missing"), TString("team/missing"),
    ...
    Y_UNIT_TEST_TWIN(CancelOperation, SingleComponent) {
        RunCases(SingleComponent, [](TEnvironment& env, const TString& db, bool success) {
            ...
            UNIT_ASSERT(metadata.exec_status() == Ydb::Query::EXEC_STATUS_CANCELLED);
        }, false);
    }
    ...
    Y_UNIT_TEST_TWIN(TableKeepAlive, SingleComponent) {
        RunCases(SingleComponent, [](TEnvironment& env, const TString& db, bool success) {
            ...
            if (success) {
                UNIT_ASSERT(Unpack<Ydb::Table::KeepAliveResult>(response).session_status() ==
