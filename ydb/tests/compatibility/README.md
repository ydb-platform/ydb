...
**Папки по областям** (сюда попадает большинство новых тестов): `indexes`, `olap`, `kv`, `topics`, `udf`, `s3_backups`, `federated_queries`, `result_set_format`, `streaming`. Внутри папки выбери файл по под-области, а сценарий (mixed / restart / rolling) задаётся выбором [фикстуры](#фикстуры), а НЕ папкой.
...
**Правило про новую папку**: Заводи новую папку **только** если тесту нужен принципиально иной ресурсный профиль (как у `stress`) или функциональная область. В остальных случаях — клади в существующую папку по области.
