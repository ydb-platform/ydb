...
class TestTieringRestart(RestartToAnotherVersionFixture):
    ...
    def setup(self, tree):
        ...
        # New persisted ESysViewType values prevent an older SchemeShard from
        # booting. Use virtual system views for downgrades to isolate tiering compatibility.
        disabled_flags = ["enable_real_system_view_paths"] if self.versions[0] > self.versions[1] else []
        # During upgrade/downgrade the tree flag stays disabled so new writes
        # remain readable by the previous release. Also test tree -> tree restarts.
    ...
    def test_tiering_survives_version_change(self, tree):
        ...
        """)
        # Promote small test portions to the final compaction level promptly,
        # as in the functional tiering tests: only optimized portions are evicted.
        self.query(f"""
        ...
        # Two overlapping key ranges make every shard eligible for compaction.
        ...
        # Reading every payload checks the S3 data, including after rollback.
