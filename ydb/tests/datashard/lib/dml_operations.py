...
class DMLOperations:
    ...
    def select_after_insert(
        ...
        for count in range(1, number_of_columns + 1):
            ...
            ), f"Expected one rows, failed in {count} value, table {table_name}"
    ...
    def update(
        ...
        if ttl != "":
            self.create_update(count, "ttl_", ttl, ttl_types[ttl], table_name)
            count += 1

        for type_name in all_types.keys():
            self.create_update(count, "col_", type_name, all_types[type_name], table_name)
            count += 1

        if unique == "":
            for type_name in index.keys():
                self.create_update(count, "col_index_", type_name, index[type_name], table_name)
                count += 1
        else:
            ...
            for i in range(1, number_of_columns + 1):
                self.create_update_unique(number_of_columns + i, i, index, table_name)
        ...
        if ttl != "":
            ...
            assert (
                len(rows) == 1 and rows[0].count == number_of_columns
            ), f"Expected {number_of_columns} rows after insert, faild in ttl_{cleanup_type_name(ttl)}, table {table_name}"
            count_assert += 1
        ...
        for type_name in all_types.keys():
            if type_name != "Json" and type_name != "Yson" and type_name != "JsonDocument":
                ...
                assert (
                    len(rows) == 1 and rows[0].count == number_of_columns
                ), f"Expected {number_of_columns} rows after insert, faild in col_{cleanup_type_name(type_name)}, table {table_name}"
            count_assert += 1
        if unique == "":
            for type_name in index.keys():
                rows = self.query(
                ...
                assert (
                    len(rows) == 1 and rows[0].count == number_of_columns
                ), f"Expected {number_of_columns} rows after insert, faild in col_index_{cleanup_type_name(type_name)}, table {table_name}"
                count_assert += 1
        else:
            ...
            for type_name in index.keys():
                ...
                assert (
                    len(rows) == 1 and rows[0].count == 1
                ), f"Expected {1} rows after insert, faild in col_index_{cleanup_type_name(type_name)}, table {table_name}"
                number_of_columns += 1
    ...
    def upsert(
        ...
        for count in range(1, number_of_columns + 1):
            self.create_upsert(table_name, number_of_columns + 1 - count, count, all_types, pk_types, index, ttl)

        for count in range(number_of_columns + 1, 2 * number_of_columns + 1):
            self.create_upsert(table_name, count, count, all_types, pk_types, index, ttl)
        ...
        for count in range(number_of_columns + 1, 2 * number_of_columns + 1):
            ...
            for type_name in all_types.keys():
                if (
                    type_name != "Json"
                    and type_name != "Yson"
                    and type_name != "JsonDocument"
                    and ((type_name != "Date" and type_name != "Datetime") or count < 106)
                ):
                    create_all_type.append(
                        f"col_{cleanup_type_name(type_name)}={format_sql_value(all_types[type_name](count), type_name)}"
                    )
        ...
        rows = self.query(f"SELECT COUNT(*) as count FROM `{table_name}`")
        assert (
            len(rows) == 1 and rows[0].count == 2 * number_of_columns
        ), f"Expected {2*number_of_columns} rows, after select all line"
    ...
    def delete(
        ...
        if ttl != "":
            self.create_delete(number_of_columns, "ttl_", ttl, ttl_types[ttl], table_name)
            number_of_columns += 1

        for type_name in pk_types.keys():
            if type_name != "Bool":
                self.create_delete(number_of_columns, "pk_", type_name, pk_types[type_name], table_name)
            else:
                self.create_delete(number_of_columns, "pk_", "Int64", pk_types["Int64"], table_name)
            number_of_columns += 1

        for type_name in all_types.keys():
            if type_name != "Bool" and type_name != "Json" and type_name != "Yson" and type_name != "JsonDocument":
                self.create_delete(number_of_columns, "col_", type_name, all_types[type_name], table_name)
            else:
                self.create_delete(number_of_columns, "pk_", "Int64", pk_types["Int64"], table_name)
            number_of_columns += 1
        ...
        for type_name in index.keys():
            if type_name != "Bool":
                self.create_delete(number_of_columns, "col_index_", type_name, index[type_name], table_name)
            else:
                self.create_delete(number_of_columns, "pk_", "Int64", pk_types["Int64"], table_name)
            number_of_columns += 1
    ...
    def assert_type(self, key, data_type: str, values: int, values_from_rows):
        if data_type == "String" or data_type == "Yson":
            assert values_from_rows.decode("utf-8") == key[data_type](values), f"{data_type}"
        elif data_type == "Float" or data_type == "DyNumber":
            assert math.isclose(float(values_from_rows), float(key[data_type](values)), rel_tol=1e-3), f"{data_type}"
        elif data_type == "Interval" or data_type == "Interval64":
            assert values_from_rows == timedelta(microseconds=key[data_type](values)), f"{data_type}"
        elif data_type == "Timestamp" or data_type == "Timestamp64":
            assert values_from_rows == datetime.fromtimestamp(key[data_type](values) / 1_000_000), f"{data_type}"
        elif data_type == "Json" or data_type == "JsonDocument":
            assert str(values_from_rows).replace("'", "\"") == str(key[data_type](values)), f"{data_type}"
        else:
