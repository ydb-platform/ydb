...
from ydb.tests.datashard.lib.types_of_variables import cleanup_type_name, pk_types_parametrized_queries, non_pk_types, index_first_parametrized_queries, index_second_parametrized_queries, ttl_types, \
    index_first_sync, index_second_sync, index_three_sync_parametrized_queries, index_three_sync_not_Bool_parametrized_queries, index_four_sync_parametrized_queries, index_zero_sync
...
class TestParametrizedQueries(TestBase):
    @pytest.mark.parametrize(
        ...
        [
            ("table_index_4_UNIQUE_SYNC", pk_types_parametrized_queries, {},
             index_four_sync_parametrized_queries, "", "UNIQUE", "SYNC"),
            ("table_index_3_UNIQUE_SYNC", pk_types_parametrized_queries, {},
             index_three_sync_not_Bool_parametrized_queries, "", "UNIQUE", "SYNC"),
            ("table_index_2_UNIQUE_SYNC", pk_types_parametrized_queries, {},
             index_second_sync, "", "UNIQUE", "SYNC"),
            ("table_index_1_UNIQUE_SYNC", pk_types_parametrized_queries, {},
             index_first_sync, "", "UNIQUE", "SYNC"),
            ("table_index_0_UNIQUE_SYNC", pk_types_parametrized_queries, {},
             index_zero_sync, "", "UNIQUE", "SYNC"),
            ("table_index_4__SYNC", pk_types_parametrized_queries, {},
             index_four_sync_parametrized_queries, "", "", "SYNC"),
            ("table_index_3__SYNC", pk_types_parametrized_queries, {},
             index_three_sync_parametrized_queries, "", "", "SYNC"),
            ("table_index_2__SYNC", pk_types_parametrized_queries, {},
             index_second_sync, "", "", "SYNC"),
            ("table_index_1__SYNC", pk_types_parametrized_queries, {},
    ...
    def test_parametrized_queries(self, table_name: str, pk_types: dict[str, str], all_types: dict[str, str], index: dict[str, str], ttl: str, unique: str, sync: str):
    ...
    def create_insert(self, table_name: str, value: int, all_types: dict[str, str], pk_types: dict[str, str], index: dict[str, str], ttl: str):
        ...
        for type_name in pk_types.keys():
            parameters[f"$pk_{cleanup_type_name(type_name)}"] = self.create_type_value(
                pk_types, type_name, value)
        for type_name in all_types.keys():
            parameters[f"$col_{cleanup_type_name(type_name)}"] = self.create_type_value(
                all_types, type_name, value)
        for type_name in index.keys():
            parameters[f"$col_index_{cleanup_type_name(type_name)}"] = self.create_type_value(
                index, type_name, value)
        if ttl != "":
            parameters[f"$ttl_{cleanup_type_name(ttl)}"] = self.create_type_value(
                ttl_types, ttl, value)
            ...
            for type_name in all_types.keys():
                if type_name != "Json" and type_name != "Yson" and type_name != "JsonDocument":
                    create_all_type.append(
                        f"col_{cleanup_type_name(type_name)}=$col_{cleanup_type_name(type_name)}")
                    create_all_type_declare.append(
                        f"DECLARE $col_{cleanup_type_name(type_name)} AS {type_name};")
