...
class TestPartitionong(TestBase):
    @pytest.mark.parametrize(
        ...
        [
            ("table_ttl_Date", {"Uint64": lambda i: i, }, {
             **pk_types, **non_pk_types}, {}, "", "", ""),
            ("table_ttl_Date", {"Uint32": lambda i: i, }, {
             **pk_types, **non_pk_types}, {}, "", "", ""),
        ]
    )
    def test_uniform_partitiona(self, table_name: str, pk_types: dict[str, str], all_types: dict[str, str], index: dict[str, str], ttl: str, unique: str, sync: str):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("table_String", {
             "String": lambda i: f"{i}"}, {**pk_types, **non_pk_types}, {}, "", "", ""),
            ("table_Utf8", {"Utf8": lambda i: f"{i}"},
             {**pk_types, **non_pk_types}, {}, "", "", ""),
        ]
    )
    def test_partition_at_keys(self, table_name: str, pk_types: dict[str, str], all_types: dict[str, str], index: dict[str, str], ttl: str, unique: str, sync: str):
        dml = DMLOperations(self)
    ...
    def create_predicate(self, index, dml: DMLOperations):
        def predicate():
            rows = dml.query("""SELECT
                count(*) as count
