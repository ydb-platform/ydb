...
from ydb.tests.datashard.lib.types_of_variables import cleanup_type_name, format_sql_value, pk_types, non_pk_types, index_first, index_second, \
...
class TestSecondaryIndex(TestBase):
    ...
    @pytest.mark.parametrize(
        "table_name, pk_types, all_types, index, ttl, unique, sync",
        [
            ("table_index_4_UNIQUE_SYNC", pk_types, {**pk_types, **non_pk_types},
             index_four_sync, "", "UNIQUE", "SYNC"),
            ("table_index_3_UNIQUE_SYNC", pk_types, {**pk_types, **non_pk_types},
             index_three_sync_not_Bool, "", "UNIQUE", "SYNC"),
            ("table_index_2_UNIQUE_SYNC", pk_types, {**pk_types, **non_pk_types},
             index_second_sync, "", "UNIQUE", "SYNC"),
            ("table_index_1_UNIQUE_SYNC", pk_types, {**pk_types, **non_pk_types},
             index_first_sync, "", "UNIQUE", "SYNC"),
            ("table_index_0_UNIQUE_SYNC", pk_types, {**pk_types, **non_pk_types},
             index_zero_sync, "", "UNIQUE", "SYNC"),
            ("table_index_4__SYNC", pk_types, {**pk_types, **non_pk_types},
             index_four_sync, "", "", "SYNC"),
            ("table_index_3__SYNC", pk_types, {**pk_types, **non_pk_types},
             index_three_sync, "", "", "SYNC"),
            ("table_index_2__SYNC", pk_types, {**pk_types, **non_pk_types},
             index_second_sync, "", "", "SYNC"),
            ("table_index_1__SYNC", pk_types, {**pk_types, **non_pk_types},
             index_first_sync, "", "", "SYNC"),
            ("table_index_0__SYNC", pk_types, {**pk_types, **non_pk_types},
             index_zero_sync, "", "", "SYNC"),
            ("table_index_1__ASYNC", pk_types, {
             **pk_types, **non_pk_types}, index_second, "", "", "ASYNC"),
            ("table_index_0__ASYNC", pk_types, {
             **pk_types, **non_pk_types}, index_first, "", "", "ASYNC"),
        ]
    )
    def test_secondary_index_cover(self, table_name: str, pk_types: dict[str, str], all_types: dict[str, str], index: dict[str, str], ttl: str, unique: str, sync: str):
        dml = DMLOperations(self)
        self.create_table(table_name, pk_types, all_types,
                          index, ttl, unique, sync, dml)
        dml.insert(table_name, all_types, pk_types, index, ttl)
        ...
        for type_name in index.keys():
            if type_name != "Bool":
                for i in range(1, count_col+1):
                    ...
                    def process(session):
                        rows = self.query(select_sql, tx=session.transaction(
                            tx_mode=ydb.QueryStaleReadOnly()))
                        count = 0
                        for data_type in all_types.keys():
                            if data_type != "Date32" and data_type != "Datetime64" and data_type != "Timestamp64" and data_type != 'Interval64':
                                dml.assert_type(
                                    all_types, data_type, i, rows[0][count])
                                count += 1
                    dml.transactional(process)
            else:
                select_sql = f"""
    ...
    def create_table(self, table_name: str, pk_types: dict[str, str], all_types: dict[str, str], index: dict[str, str], ttl: str, unique: str, sync: str, dml: DMLOperations):
        columns = {
            "pk_": pk_types.keys(),
            "col_": all_types.keys(),
        ...
        sql_create_table = self.create_table_sql_request(
            table_name, columns, pk_columns, index_columns, unique, sync)
        dml.query(sql_create_table)

    def create_table_sql_request(self, table_name: str, columns: dict[str, dict[str]], pk_colums: dict[str, dict[str]], index_colums: dict[str, dict[str]], unique: str, sync: str):
        create_columns = []
