...
class TestSplitMerge(TestBase):
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("table_DyNumber", {"DyNumber": lambda i: float(f"{i}e1")}, {**pk_types, **non_pk_types}, {}, "", "", ""),
            ("table_String", {"String": lambda i: f"String {i}"}, {**pk_types, **non_pk_types}, {}, "", "", ""),
            ("table_Utf8", {"Utf8": lambda i: f"Utf8 {i}"}, {**pk_types, **non_pk_types}, {}, "", "", ""),
            (
                "table_UUID",
                {"Utf8": lambda i: UUID("3{:03}5678-e89b-12d3-a456-556642440000".format(i))},
                {**pk_types, **non_pk_types},
                {},
                "",
                "",
            ...
            ("table_Interval", {"Interval": lambda i: i}, {**pk_types, **non_pk_types}, {}, "", "", ""),
            (
                "table_Date32",
            ...
            ("table_Interval64", {"Interval64": lambda i: i}, {**pk_types, **non_pk_types}, {}, "", "", ""),
        ],
    )
    def test_merge_split(
        ...
        dml.query(f"alter table {table_name} set(AUTO_PARTITIONING_PARTITION_SIZE_MB = 1)")
        dml.insert(table_name, all_types, pk_types, index, ttl)
        is_split = wait_for(self.expected_split(True, table_name), timeout_seconds=150)
        ...
        is_merge = wait_for(self.expected_split(False, table_name), timeout_seconds=150)
