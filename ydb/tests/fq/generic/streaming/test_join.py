...
class TestJoinStreaming(TestYdsBase):
    ...
    @pytest.mark.parametrize(
        "wide_channels", [False, True] if DEBUG else [True], ids=["narrow", "wide"] if DEBUG else ["wide"]
    )
    @pytest.mark.parametrize("partitions_count", [1, 3] if DEBUG else [3])
    @pytest.mark.parametrize(
        "streamlookup", [False, True] if DEBUG else [True], ids=["map", "slj"] if DEBUG else ["slj"]
    )
