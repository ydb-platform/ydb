...
from ydb.tests.tools.fq_runner.kikimr_utils import yq_v2, yq_all
...
class TestStatsMode:
    ...
    @yq_v2
    @pytest.mark.parametrize("client", [{"folder_id": "my_folder"}], indirect=True)
    @pytest.mark.parametrize("stat_view", ["stat_none", "stat_basc", "stat_full", "stat_prof", "stat_prod", "stat_tree", "stat_join"])
    def test_view(self, kikimr, s3, client, stat_view):
        resource = boto3.resource(
            "s3",
            endpoint_url=s3.s3_url,
            aws_access_key_id="key",
