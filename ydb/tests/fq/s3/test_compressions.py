...
class TestS3Compressions:
    ...
    def test_compression_inference(self, kikimr, s3, client, filename, compression):
        self.create_bucket_and_upload_file(filename, s3, kikimr)
        client.create_storage_connection("fruitbucket", "fbucket")
...
Pear,15,33'''
    ...
    @yq_v2
    @pytest.mark.parametrize("client", [{"folder_id": "my_folder"}], indirect=True)
    def test_invalid_compression_inference(self, kikimr, s3, client):
        resource = boto3.resource(
