...
class TestConnection(TestYdsBase):
    ...
    def test_test_s3_connection_uri(self, kikimr, s3, client):
        ...
        test_response = client.test_storage_connection(
            " testbucket", check_issues=False)
        assert "Object Storage: The specified bucket does not exist" in str(
            test_response.issues)

    @yq_all
    @pytest.mark.parametrize("client", [{"folder_id": "my_folder"}], indirect=True)
    def test_test_s3_connection_error(self, kikimr, s3, client):
        test_response = client.test_storage_connection(
            "errorbucket", check_issues=False)
        assert "Object Storage: The specified bucket does not exist" in str(
            test_response.issues)
