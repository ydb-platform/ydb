...
class TestStreamingRollingRestart(StreamingTestBase):
    ...
    def test_shared_reading(self, kikimr, entity_name):
        ...
        inp, out, endpoint = self.get_io_names(
            kikimr, "test_shared_reading", local_topics, entity_name, shared=True)

        query_count = 2
        for i in range(query_count):
            name = f"test_shared_reading_{i}_{local_topics!s:.1}"
            sql = f"""
        ...
        for i, _ in enumerate(self.roll(kikimr)):  # every iteration is a step in rolling upgrade process
