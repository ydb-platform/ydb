...
class TestPqRowDispatcher(TestYdsBase):
    ...
    def test_filters_non_optional_field(self, kikimr, client):
        ...
        self.run_and_check(kikimr, client, sql + filter, data, expected, R'predicate: ((IF((CAST(`data` AS Utf8?) IS NOT NULL), CAST(CAST(`data` AS Utf8?) AS String), NULL) REGEXP \".*hello2.*\") IS NOT DISTINCT FROM TRUE)')  # YQ-5727
