...
class TestStaticGroupSelfHealAllowedHosts:

    def test_self_heal_relocates_only_to_allowed_host(self, cluster):
        ...
        assert_that(allowed_node_id in new_group_nodes,
                    "static group vdisk did not move to the allowed host (node %d); group nodes: %s"
                    % (allowed_node_id, sorted(new_group_nodes)))
        assert_that(forbidden_node_id not in new_group_nodes,
                    "static group vdisk moved onto the forbidden host (node %d); group nodes: %s"
