...
MISSING_CLUSTER_PARAMETER_ERROR = (
    ...
    "database requires 'cluster' and 'database'; node requires 'cluster' and 'node_id'; "
    "host requires 'cluster' and 'host'."
)
DATABASE_WITH_NODE_OR_HOST_ERROR = "Invalid identity parameters. Parameter 'database' must not be specified together with 'node_id' or 'host'."
...
class MetaSupportLinksEnv(BaseHttpEnv):
    ...
    def get_support_links(self, cluster_name=None, database=None, node_id=None, host=None):
        params = {}
    ...
    def get_ok_support_links_payload(self, cluster_name=None, database=None, node_id=None, host=None):
        response = self.get_support_links(cluster_name=cluster_name, database=database, node_id=node_id, host=host)
        assert response.status_code == 200, response.text
        return response.json()

    def get_bad_request_support_links_payload(self, cluster_name=None, database=None, node_id=None, host=None):
        response = self.get_support_links(cluster_name=cluster_name, database=database, node_id=node_id, host=host)
