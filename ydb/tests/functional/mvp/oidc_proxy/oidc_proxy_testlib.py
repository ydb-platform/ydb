...
def auth_start_url_from_response(response):
...
def start_local_auth(env, auth_start_url):
    parsed_auth_start = urlparse(auth_start_url)
    response = env.get(
        parsed_auth_start.path + ("?" + parsed_auth_start.query if parsed_auth_start.query else ""),
        allow_redirects=False,
    ...
    assert redirect_url.startswith(env.auth_service.endpoint + "/oauth/authorize"), redirect_url
    state = parse_qs(urlparse(redirect_url).query)["state"][0]
    oidc_cookie = response_cookie_by_name(response, "ydb_oidc_cookie")
    return state, oidc_cookie, response
...
def authenticate_session(oidc_proxy_full_flow_env, start_path):
    ...
    auth_start_url = auth_start_url_from_response(start_response)
    state, oidc_cookie, _ = start_local_auth(oidc_proxy_full_flow_env, auth_start_url)
