...
def assert_background_auth_challenge(response):
    ...
    assert urlparse(response_json["authUrl"]).path == AUTH_START_PATH, response_json
    assert "Location" not in response.headers, response.headers
    assert "Set-Cookie" not in response.headers, response.headers
...
def assert_navigation_auth_redirect(env, response):
    ...
    assert urlparse(redirect_url).path == AUTH_START_PATH, redirect_url
    assert "Set-Cookie" not in response.headers, response.headers
...
def start_navigation_auth_flow(env, start_path):
    ...
    return start_local_auth(env, response.headers["Location"])[:2]
...
def start_background_auth_flow(env):
    ...
    return start_local_auth(env, auth_start_url_from_response(response))[:2]
...
def test_auth_start_rejects_cross_host_return_to(oidc_proxy_env):
    response = oidc_proxy_env.get(
        "/auth/start",
        params={"return_to": "https://evil.test/path"},
        allow_redirects=False,
        headers={"Host": "oidcproxy.net"},
    )
...
def test_parallel_long_navigation_urls_latest_auth_flow_cookie_fallback_wins(oidc_proxy_full_flow_env):
    ...
    callback_response = finish_auth_callback(env, first_state, [first_oidc_cookie, second_oidc_cookie])

    assert callback_response.status_code == 302, callback_response.text
    assert callback_response.headers["Location"] == second_navigation_target, callback_response.headers
