...
def build_programmatic_expired_session_request_headers(host):
...
def start_programmatic_auth_challenge_request(env):
    host = protected_host(env)
    return start_auth_challenge_request(
        env,
        build_programmatic_expired_session_request_headers(host),
    )
...
def assert_programmatic_auth_challenge(response):
...
def test_programmatic_request_returns_json_401_with_auth_url(oidc_proxy_full_flow_env):
    response = start_programmatic_auth_challenge_request(oidc_proxy_full_flow_env)
    assert_programmatic_auth_challenge(response)
