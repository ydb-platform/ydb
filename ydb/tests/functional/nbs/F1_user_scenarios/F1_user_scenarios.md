...
| F1.13 | Block sizes 4 / 8 / 16 / 32 / 64 / 128 KiB (create with `--block-size`) | create succeeds at every size; 4 KiB read-after-write exact and out-of-range rejected. IO at block size > 4 KiB is a known product bug: PBuffer selector stays 4 KiB (`size=8192, selector.Size=4096`) | `F1_13_block_sizes.py` (known_bug xfail for >4 KiB IO) |
...
| F1.15 | Vhost reconnect after partition tablet kill + recovery | socket `/tmp/<disk_id>.sock` is usable again; previously written data readable | none |
...
| F1.25 | Max disk size at every supported block size | create `2³¹` blocks at every size; 4 KiB first/last block read-after-write exact and write past the last block rejected. IO at block size > 4 KiB is the same PBuffer 4 KiB selector bug as F1.13 | `F1_25_max_disk_size.py` (known_bug xfail for >4 KiB IO) |
