...
| F1.26 | Grow an existing disk (`ResizePartition`): 4→8 GiB (new region), 2→4 GiB (same region), same size, shrink, grow under live IO | grow: write past the old end fails before the resize; tablet restarts; old data survives; write/read in the new capacity succeeds. same size: SUCCESS, data and capacity unchanged. shrink: `BAD_REQUEST`, data and capacity unchanged. under IO: in-flight vhost IO fails across the restart; IO resumes afterwards and pre-resize data is intact | `F1_26_resize_grow.py` |
