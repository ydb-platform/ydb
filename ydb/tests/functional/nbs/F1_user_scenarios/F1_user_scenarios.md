...
| F1.25 | Max disk size at every supported block size (`2³¹` blocks) | 4 KiB: create, first / middle / last block read-after-write exact, write past the last block rejected. Eager vchunk metadata at 2^31 blocks for large disks. | `F1_25_max_disk_size.py` (known_bug xfail, not run, per size >4 KiB) |
