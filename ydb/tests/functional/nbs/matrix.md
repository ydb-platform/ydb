...
| F1.13 | Block sizes 4K / 8K / 16K / 32K / 64K / 128K | P0 | — | block size | yes | `F1_13_block_sizes.py` (known_bug xfail: PBuffer 4 KiB selector) |
| F1.14 | Vhost unaligned write | P0 | — | — | yes | `test_nbs_vhost_unaligned_write` |
| F1.15 | Vhost after tablet restart | P0 | `tablet_kill` | — | yes | `F1_vhost/F1_15_vhost_after_tablet_restart.py` (known_bug xfail) |
...
| F1.25 | Max disk size per block size | P0 | — | 2³¹ blocks | yes | `F1_25_max_disk_size.py` (known_bug xfail: PBuffer 4 KiB selector) |
...
| F2.5 | Three DBG hosts down | P1 | `node.stop` ×3 PBuffer nodes | file PDisks | yes | `F2_05_three_dbg_hosts_down.py` |
| F2.6 | PDisk BROKEN | P1 | `pdisk set BROKEN` | file PDisks | yes | `F2_06_pdisk_broken.py` |
...
| F2.14 | Combined faults | P1 | kill + SIGSTOP + stop | file PDisks | yes | `F2_14_combined_faults.py` |
...
| F3.3 | Offline promotes HandOff, watermark 0 | P1 | `node.stop` | file PDisks | yes | `F3_03_offline_promotes_handoff.py` |
| F3.4 | Offline QueryAddHost | P1 | `node.stop` | file PDisks | yes | `F3_04_offline_query_addhost.py` |
| F3.5 | OnDDiskBroken stays Broken | P1 | `pdisk BROKEN` | file PDisks | yes | `F3_05_ondiskbroken_stays_broken.py` |
| F3.6 | Copier starts after promotion | P1 | follows F3.3 | file PDisks | yes | `F3_06_copier_starts_after_promotion.py` |
| F3.7 | Serial 1 MiB ranges | P1 | follows F3.3 | file PDisks | yes | `F3_07_serial_1mib_ranges.py` |
| F3.8 | Fresh DDisk not readable above watermark | P1 | follows F3.3 | file PDisks | yes | `F3_08_fresh_ddisk_not_readable_above_watermark.py` |
| F3.9 | Copy complete, watermark nullopt | P1 | follows F3.3 | file PDisks | yes | `F3_09_copy_complete_watermark_unset.py` |
...
| F3.12 | User writes during copy | P1 | follows F3.3 | file PDisks | yes | `F3_12_user_writes_during_copy.py` |
...
| F3.15 | Copy does not resume incrementally | P1 | `tablet_kill` mid-copy | file PDisks | yes | `F3_15_copy_does_not_resume_incrementally.py` |
| F3.16 | DoStart does not start copiers | P1 | follows F3.15 | file PDisks | yes | `F3_16_dostart_does_not_start_copiers.py` |
| F3.17 | Read fallback to disabled replica | P0 | disable last replica | nbs_cfg | prereq | nbs_cfg |
| F3.18 | Ahead not readable, still copied | P1 | write during copy | file PDisks | yes | `F3_18_ahead_not_readable_still_copied.py` |
| F3.19 | No demote / no rebalance | P1 | follows F3.3+F3.4 | file PDisks | yes | `F3_19_no_demote_no_rebalance.py` |
