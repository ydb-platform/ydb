...
def generate_certificates(certs_tmp_dir):
    ...
    # Combined PEM (cert + key) for monitoring when server expects single file
    server_combined = os.path.join(certs_tmp_dir, 'server_combined.pem')
    with open(server_combined, 'w') as f:
        with open(server_crt) as crt:
            f.write(crt.read())
        with open(server_key) as key:
            f.write(key.read())

    return {
        'server_cert': server_crt,
        'server_key': server_key,
        'server_combined': server_combined,
        'server_ca': ca_crt,
...
def ydb_configurator(ydb_cluster_configuration, certificates):
    ...
    config_generator.monitoring_tls_cert_path = certificates['server_combined']
    ...
    config_generator.yaml_config['monitoring_config']['monitoring_certificate_file'] = certificates['server_combined']
    config_generator.yaml_config['monitoring_config']['monitoring_ca_file'] = certificates['server_ca']
