...
class YandexCloudCommonTests(YandexCloudSqsTestBase, CommonTests):
    pass


class TestCommonSqsYandexCloudMode(get_test_with_sqs_tenant_installation(YandexCloudCommonTests)):
    def _before_test_start(self):
        super(TestCommonSqsYandexCloudMode, self)._before_test_start()

        self._sqs_api = self._create_api_for_user(
            'ignored', raise_on_error=True, force_private=False,
