...
class TestSqsGettingCounters(KikimrSqsTestBase):
    ...
    def test_sqs_action_counters(self):
        ...
        def assert_action_counters(counters):
            successes = self._get_counter_value(counters, {
                'queue': self.queue_name,
                'sensor': 'ReceiveMessage_Success',
            })
            assert successes == 1

            durations = self._get_counter(counters, {
                'queue': self.queue_name,
            ...
            working_durations = self._get_counter(counters, {
                'queue': self.queue_name,
