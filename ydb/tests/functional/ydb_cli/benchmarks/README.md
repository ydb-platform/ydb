...
After few runs statistic may be collected by using script `report/wiki_sql_report.py` (each run must include only one model and only one split). Example for one split `train` and sample rate `0.1`:

```bash
cd report
python3 wiki_sql_report.py train0.1
```
