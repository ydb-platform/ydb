...
# Features that exist on main but not on this stable's SQS-over-topics HTTP API.
skip_unsupported_on_this_branch = pytest.mark.skip(
    reason='Not supported by SQS-over-topics on this stable branch'
)


class AmazonSqsJsonSerializer(JSONSerializer):
    # Query-protocol SQS models use locationName (Attribute, Tag) for flattened
    # maps. http_proxy JSON expects the member names (Attributes, tags).
...
class AmazonSqsJsonParser(JSONParser):
    def _handle_structure(self, shape, value):
        if value is None:
            return None
        final_parsed = {}
        member_shapes = shape.members
        for member_name, member_shape in member_shapes.items():
            raw_value = value.get(member_name)
            if raw_value is None:
                json_name = member_shape.serialization.get('name', member_name)
                raw_value = value.get(json_name)
            if raw_value is not None:
                final_parsed[member_name] = self._parse_shape(member_shape, raw_value)
        return final_parsed


class _AmazonSqsJsonParserFactory:
    def create_parser(self, protocol_name):
        if protocol_name == 'json':
            return AmazonSqsJsonParser()
        from botocore.parsers import ResponseParserFactory
        return ResponseParserFactory().create_parser(protocol_name)
...
class KikimrSqsTopicTestBase(object):
    ...
    def _make_boto_client(self, endpoint_url=None):
        ...
        # This branch's http_proxy speaks Amazon SQS JSON (X-Amz-Target), not query/XML.
        metadata = client.meta.service_model.metadata
        metadata['protocol'] = 'json'
        metadata['jsonVersion'] = '1.0'
        metadata['targetPrefix'] = 'AmazonSQS'
        client._serializer = AmazonSqsJsonSerializer()
        client._endpoint._response_parser_factory = _AmazonSqsJsonParserFactory()
        return client
