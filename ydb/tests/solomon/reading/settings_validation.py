        ...
        with ydb.QuerySessionPool(self.driver) as session_pool:
            data_source_query = f"""
                CREATE EXTERNAL DATA SOURCE local_solomon WITH (
                    ...
                    AUTH_METHOD="NONE",
                    USE_SSL="false",
                    HTTP_ENDPOINT="{self.solomon_http_endpoint}",
                    GRPC_ENDPOINT="{self.solomon_grpc_endpoint}"
                )"""
            session_pool.execute_with_retries(data_source_query)

        # check `program`/`selectors` settings validation
        query = """
            SELECT * FROM local_solomon.my_project;
        """
        self.check_query_error(query, "specify either program or selectors")
