...
class WorkloadVectorIndex(WorkloadBase):
    def __init__(self, client, prefix, stop):
        ...
        self.targets = {
            "similarity": {
                "inner_product": "Knn::InnerProductSimilarity", "cosine": "Knn::CosineSimilarity"
            },
            "distance": {
                "cosine": "Knn::CosineDistance", "manhattan": "Knn::ManhattanDistance", "euclidean": "Knn::EuclideanDistance"
            }
        }
    ...
    def _create_index(self, table_path, vector_type, vector_dimension, levels, clusters, distance = None, similarity = None):
        logger.info(f'Create index vector_type={vector_type}, vector_dimension={vector_dimension}, levels={levels}, clusters={clusters}, distance={distance}, similarity={similarity}')
        if distance is not None:
            create_index_sql = f"""
                ALTER TABLE `{table_path}` ADD INDEX `{self.index_name}` GLOBAL USING vector_kmeans_tree
                ON (embedding)
                WITH (distance={distance}, vector_type={vector_type}, vector_dimension={vector_dimension}, levels={levels}, clusters={clusters});
            """
        else:
            create_index_sql = f"""
                ALTER TABLE `{table_path}` ADD INDEX `{self.index_name}` GLOBAL USING vector_kmeans_tree
                ON (embedding)
                WITH (similarity={similarity}, vector_type={vector_type}, vector_dimension={vector_dimension}, levels={levels}, clusters={clusters});
            """
    ...
    def _upsert_values(self, table_path, vector_type, vector_dimension):
        ...
        for key in range(self.rows_count):
            ...
            values.append(
                f'({key}, Untag({converter.name}([{vector}]), "{converter.vector_type}"))'
            )
    ...
    def _select(self, table_path, vector_type, vector_dimension, distance, similarity):
        ...
        converter = self.to_binary_string_converters[vector_type]
        select_sql = f"""
            $Target = {converter.name}(Cast([{vector}] AS List<{converter.data_type}>));
    ...
    def _select_top(self, table_path, vector_type, vector_dimension, distance, similarity):
        logger.info(f"Select values from table")
        result_set = self._select(table_path, vector_type, vector_dimension, distance, similarity)
        if len(result_set) == 0:
        ...
        for row in rows:
            ...
            if not condition:
                raise Exception(f"The set of rows does not satisfy the condition, prev: {prev}, cur: {cur}")
            prev = cur

    def _wait_inddex_ready_(self, table_path, vector_type, vector_dimension, distance, similarity):
        if distance is not None:
            target = self.targets["distance"][distance]
        else:
            target = self.targets["similarity"][similarity]
        ...
        for i in range(10):
            ...
            try:
                self._select(table_path, vector_type, vector_dimension, distance, similarity)
            ...
            return
        raise Exception(f"Error getting index status")
    ...
    def _check_loop(self, table_path, vector_type, vector_dimension, levels, clusters, distance = None, similarity = None):
        self._create_index(
            ...
            levels=levels, clusters=clusters, distance=distance, similarity=similarity
        )
        self._wait_inddex_ready_(
            table_path=table_path, vector_type=vector_type, vector_dimension=vector_dimension, distance=distance, similarity=similarity
        )
        self._select_top(
            table_path=table_path, vector_type=vector_type, vector_dimension=vector_dimension, distance=distance, similarity=similarity
        )
                    ...
                    for levels in levels_data:
                        for clusters in clusters_data:
                            for distance in distance_data:
                                self._check_loop(
                                    table_path=table_path, vector_type=vector_type, vector_dimension=vector_dimension, levels=levels, clusters=clusters, distance=distance
                                )
                    ...
                    for levels in levels_data:
                        for clusters in clusters_data:
                            for similarity in similarity_data:
                                self._check_loop(
                                    table_path=table_path, vector_type=vector_type, vector_dimension=vector_dimension, levels=levels, clusters=clusters, similarity=similarity
                                )
