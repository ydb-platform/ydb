...
* `start_prometheus.sh` - start prometheus web UI, can be used for counters visualisation (kqprun should be run with flag `-M <monitoring port>`), usage:
    ```(bash)
...
* `cleanup_docker.sh` - stop created docker containers, usage:
    ```(bash)
