...
class Description:
    ...
    def formatted_entry(self) -> str:
        ...
        entry = re.sub(
            r"([^[])#([0-9]{4,})",
            r"\1[#\2](https://github.com/ydb-platform/ydb/issues/\2)",
            self.entry,
        )
        # 2) issue URL w/o markdown link
        entry = re.sub(
            r"([^(])https://github.com/ydb-platform/ydb/issues/([0-9]{4,})",
            r"\1[#\2](https://github.com/ydb-platform/ydb/issues/\2)",
            entry,
        )
...
# This function mirrors the PR description checks in PullRequest.
...
def write_changelog(fd: TextIO, descriptions: Dict[str, List[Description]]):
    ...
    fd.write(
        f"---\nsidebar_position: 1\nsidebar_label: {year}\n---\n\n"
        f"# {year} Changelog\n\n"
        f"### YDB release {TO_REF} ({to_commit}) "
