...
def _discovery_endpoint_ports(output):
    ports = set()
    for line in output.splitlines():
        fields = line.split(maxsplit=1)
        if not fields or not fields[0].startswith(("grpc://", "grpcs://")):
            continue
        try:
            port = urlsplit(fields[0]).port
        except ValueError:
            continue
        if port is not None:
            ports.add(port)
...
class LocalYdbCluster:
    ...
    def _wait_client_endpoints(self, timeout):
        command = [
            self.ydb_cli,
            "--endpoint",
        ...
        expected_ports = {node["grpc_port"] for node in self.dynamic_nodes}
        deadline = time.monotonic() + timeout
        attempts = []
        while True:
            self._check_cancelled()
            ...
            attempts.append(result)
            self.ensure_running("YDB process exited while waiting for client endpoint discovery")
            if result.interrupted:
            ...
            if result.timed_out or result.exit_code:
                ...
                raise BenchmarkError("YDB client endpoint discovery {}: {}".format(status, details))
            discovered_ports = _discovery_endpoint_ports(result.stdout)
            if expected_ports.issubset(discovered_ports):
                self._write_attempts("client-discovery", result, attempts)
                return
            if time.monotonic() >= deadline:
                self._write_attempts("client-discovery", result, attempts)
                missing = ", ".join(map(str, sorted(expected_ports - discovered_ports)))
                raise BenchmarkError(
                    "YDB client endpoint discovery did not publish dynamic node ports in {} seconds: {}".format(
                        timeout, missing
                    )
                )
            time.sleep(1)
    ...
    def add_dynamic_nodes(self, count):
        ...
        self._progress("waiting-for-client-endpoints", dynamic_nodes=final_count)
        self._wait_client_endpoints(min(self.timeout, 600))
