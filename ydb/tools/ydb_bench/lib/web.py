...
function bindEditorControls(){
  ...
  if(editor.model&&document.querySelector('.profile-list')){
    const queue=[];
    for(const profile of editor.model.profiles)for(const affinity of profile.affinity)for(const threads of profile.threads)for(let repeat=1;repeat<=profile.repetitions;repeat++)queue.push(profile.benchmark+' / '+profile.name+' / '+affinity+' / '+threads+' threads / repeat '+repeat);
...
function profileEditor(profile){
  ...
  const parameterFields=benchmark.parameters.map((parameter,index)=>field('parameter-'+index,parameter.name,parameter.type==='integer'?compactIntegerRanges(profile.parameters[parameter.name]||[]):(profile.parameters[parameter.name]||[]).join(', '),parameter.description)).join('');
...
function arrayField(value){
  ...
  if(!parts.length)throw Error('Enter one or more positive integers or ranges.');
  for(const part of parts){
    const match=/^(\d+)(?:\s*-\s*(\d+))?$/.exec(part);
    if(!match)throw Error('Values must be positive integers or ranges such as 1-16.');
    const first=Number(match[1]),last=Number(match[2]||match[1]);
    if(!Number.isSafeInteger(first)||!Number.isSafeInteger(last)||first<1||last<first)throw Error('Ranges must use positive integers in ascending order.');
...
function svgChart(metric,xName,xValues,seriesRows,colors){
  const width=900,height=330,left=78,right=24,top=24,bottom=52,plotWidth=width-left-right,plotHeight=height-top-bottom;
  const values=[];for(const item of seriesRows)for(const x of xValues){const row=item.rows.get(String(x)),value=Number(row?.[metric]);if(Number.isFinite(value))values.push(value)}
  ...
  seriesRows.forEach((item,index)=>{const color=colors[index%colors.length],points=xValues.map(x=>{const row=item.rows.get(String(x)),y=Number(row?.[metric]);return Number.isFinite(y)?{x,y,row}:null}).filter(Boolean);svg+='<polyline class=chart-line stroke="'+color+'" points="'+points.map(point=>xPos(point.x)+','+yPos(point.y)).join(' ')+'"/>';for(const point of points)svg+='<circle class=chart-point fill="'+color+'" cx="'+xPos(point.x)+'" cy="'+yPos(point.y)+'" r="4"><title>'+esc(item.label+'; '+xName+'='+point.x+'; '+metric+'='+point.y)+'</title></circle>'});
...
function bindChartTooltips(container,xName,xValues,seriesRows,metrics,colors){
  ...
  for(const panel of container.querySelectorAll('.chart-panel')){
    ...
    svg.onmousemove=event=>{
      ...
      const values=seriesRows.map((item,index)=>({label:item.label,colorClass:index%chartColors.length,value:Number(item.rows.get(String(selected))?.[metric])})).filter(item=>Number.isFinite(item.value)).sort((leftItem,rightItem)=>rightItem.value-leftItem.value||leftItem.label.localeCompare(rightItem.label));
...
function mountChartBuilder(container,data,scope={}){
  ...
  const state={benchmark:scope.benchmark||'',profile:scope.profile||'',x:data.dimensions.includes('threads')?'threads':data.dimensions[0],ys:new Set(data.metrics.includes('median_msgs_per_sec')?['median_msgs_per_sec']:data.metrics.slice(0,1)),filters:{},lines:null};
  ...
  const resetLines=()=>{state.lines=null};
  function render(){
    ...
    const series=available();if(state.lines===null)state.lines=new Set(series.map(item=>item.id));
    const dimensions=data.dimensions.filter(name=>series.some(item=>item.rows.some(row=>row[name]!==undefined)));if(!dimensions.includes(state.x))state.x=dimensions[0];
    const filters=dimensions.filter(name=>name!==state.x);for(const name of filters){const values=[...new Set(series.flatMap(item=>item.rows.map(row=>row[name]).filter(value=>value!==undefined)).map(String))].sort((a,b)=>Number(a)-Number(b));if(!values.includes(String(state.filters[name])))state.filters[name]=values[0]}
    ...
    let controls='<div class=chart-controls>'+(scope.benchmark?'':select('chart-benchmark','Benchmark',benchmarks,state.benchmark))+(scope.profile?'':select('chart-profile','Profile',profiles,state.profile))+select('chart-x','X axis',dimensions,state.x)+filters.map(name=>select('chart-filter-'+name,name,[...new Set(series.flatMap(item=>item.rows.map(row=>row[name]).filter(value=>value!==undefined)).map(String))].sort((a,b)=>Number(a)-Number(b)),String(state.filters[name]))).join('')+'</div><h3>Y axes</h3><div class=checkboxes>'+data.metrics.map(metric=>'<label><input class=chart-y type=checkbox value="'+esc(metric)+'" '+(state.ys.has(metric)?'checked':'')+'> '+esc(metric)+(data.metric_metadata?.[metric]?.unit?' ('+esc(data.metric_metadata[metric].unit)+')':'')+'</label>').join('')+'</div><h3>Lines</h3><div class=series-picker>'+series.map(item=>'<label><input class=chart-line-select type=checkbox value="'+esc(item.id)+'" '+(state.lines.has(item.id)?'checked':'')+'> '+esc(chartSeriesLabel(item,Boolean(scope.profile)))+(scope.profile?'<small class=series-cpus>'+esc(seriesCpuNote(item))+'</small>':'')+'</label>').join('')+'</div><div id=chart-warning></div><div id=chart-output></div>';
    container.innerHTML=controls;
    const benchmark=container.querySelector('#chart-benchmark');if(benchmark)benchmark.onchange=()=>{state.benchmark=benchmark.value;state.profile='';resetLines();render()};
    const profile=container.querySelector('#chart-profile');if(profile)profile.onchange=()=>{state.profile=profile.value;resetLines();render()};
    container.querySelector('#chart-x').onchange=event=>{state.x=event.target.value;render()};
    for(const name of filters)container.querySelector('#chart-filter-'+CSS.escape(name)).onchange=event=>{state.filters[name]=event.target.value;draw()};
    for(const input of container.querySelectorAll('.chart-y'))input.onchange=()=>{state.ys=new Set([...container.querySelectorAll('.chart-y:checked')].map(item=>item.value));draw()};
    for(const input of container.querySelectorAll('.chart-line-select'))input.onchange=()=>{state.lines=new Set([...container.querySelectorAll('.chart-line-select:checked')].map(item=>item.value));draw()};
    draw()
  }
  function draw(){
    const chosen=available().filter(item=>state.lines.has(item.id));const output=container.querySelector('#chart-output'),warning=container.querySelector('#chart-warning');
    ...
    const indexed=chosen.map(item=>{const rows=new Map;for(const row of item.rows){if(Object.entries(state.filters).every(([name,value])=>name===state.x||row[name]===undefined||String(row[name])===String(value))&&row[state.x]!==undefined)rows.set(String(row[state.x]),row)}return {item,rows,label:chartSeriesLabel(item,Boolean(scope.profile))}});
    const sets=indexed.map(item=>new Set(item.rows.keys())),common=[...sets[0]].filter(value=>sets.every(set=>set.has(value))).sort((a,b)=>Number(a)-Number(b)),union=new Set(sets.flatMap(set=>[...set]));
    ...
    const metrics=[...state.ys];output.innerHTML=legend+metrics.map(metric=>'<section class=chart-panel data-metric="'+esc(metric)+'"><h3>'+esc(metric)+'</h3>'+svgChart(metric,state.x,common,indexed,colors)+'</section>').join('');bindChartTooltips(output,state.x,common,indexed,metrics,colors)
  }
  render()
}
...
def chart_data(output, run_ids):
    ...
    result = []
    dimensions, metrics, metric_metadata = set(), set(), {}
    ...
    return {"series": result, "dimensions": sorted(dimensions), "metrics": sorted(metrics), "metric_metadata": metric_metadata}
