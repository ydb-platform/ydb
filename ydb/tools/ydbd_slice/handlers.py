...
def slice_install(components, nodes, cluster_details, configurator, do_clear_logs, args):
    ...
    if 'kikimr' in components:
        ...
        if 'bin' in components.get('kikimr', []):
            update_kikimr(nodes, configurator.kikimr_bin, configurator.kikimr_compressed_bin, args.direct_copy)

        if 'cfg' in components.get('kikimr', []):
            static_cfg_path = configurator.create_static_cfg()
            update_cfg(nodes, static_cfg_path, args.direct_copy)
...
def update_kikimr(nodes, bin_path, compressed_path, direct_copy):
    bin_directory = os.path.dirname(bin_path)
    nodes.copy(bin_path, slice_kikimr_path, compressed_path=compressed_path, direct_copy=direct_copy)
    for lib in ['libiconv.so', 'liblibaio-dynamic.so', 'liblibidn-dynamic.so']:
        ...
        if os.path.exists(lib_path):
            ...
            nodes.copy(lib_path, remote_lib_path, direct_copy=direct_copy)


def update_cfg(nodes, cfg_path, direct_copy):
    nodes.copy(cfg_path, slice_cfg_path, directory=True, direct_copy=direct_copy)
...
def slice_update(components, nodes, cluster_details, configurator, do_clear_logs, args):
    ...
    if 'kikimr' in components:
        if 'cfg' in components.get('kikimr', []):
            ...
            update_cfg(nodes, static, args.direct_copy)
            deploy_secrets(nodes, args.yav_version)
