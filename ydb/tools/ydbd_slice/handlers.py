...
class Slice:
    ...
    def _clear_config_dir(self):
        cmd = "sudo rm -rf {}".format(self.slice_cfg_dir)
        self.nodes.execute_async(cmd)
    ...
    def slice_install(self):
        ...
        if 'kikimr' in self.components:
            ...
            self.__create_databases()
            # Not really needed, cose old txt files used
...
mon={mon}""".format(
    ...
    def _upload_cfg_v2(self, cfg_path):
        self.nodes.copy(cfg_path, self.slice_cfg_path, directory=True)
        self.nodes.execute_async(f"sudo bash -c 'mkdir -p {self.slice_cfg_dir}; cp {self.slice_cfg_path}/config.yaml {self.slice_cfg_dir}'")
