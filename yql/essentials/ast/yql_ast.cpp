...
inline bool IsListStart(char c) {
    return c == '(';
}
inline bool IsListEnd(char c) {
    return c == ')';
}
inline bool IsCommentStart(char c) {
    return c == '#';
}
inline bool IsQuoteStart(char c) {
    return c == '\'';
}
inline bool IsStringStart(char c) {
    return c == '"';
}
inline bool IsHexStringStart(char c) {
    return c == 'x';
}
inline bool IsMultilineStringStart(char c) {
    return c == '@';
}
...
public:
    ...
    inline const TStringBuf& Str() const {
        return Str_;
    }
    inline ui32 Offset() const {
        return Offset_;
    }
    inline const TPosition& Position() const {
        return Position_;
    }
    inline TMemoryPool& Pool() {
        return *Pool_;
    }
    inline std::unique_ptr<TMemoryPool>&& InnerPool() {
        return std::move(InnerPool_);
    }
...
inline bool IsQuoteNode(const TAstNode& node) {
    return node.GetChildrenCount() == 2 && node.GetChild(0)->GetType() == TAstNode::Atom && node.GetChild(0)->GetContent() == TStringBuf("quote");
}

inline bool IsBlockNode(const TAstNode& node) {
    return node.GetChildrenCount() == 2 && node.GetChild(0)->GetType() == TAstNode::Atom && node.GetChild(0)->GetContent() == TStringBuf("block");
}
