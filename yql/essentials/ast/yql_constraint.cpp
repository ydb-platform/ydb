    ...
    switch (type.GetKind()) {
        ...
        case ETypeAnnotationKind::Dict:
            if (const auto index = TryFromString<ui8>(TStringBuf(path.front()))) {
                switch (*index) {
                    case 0U:
                        return GetSubTypeByPath(tail(path), *type.Cast<TDictExprType>()->GetKeyType());
                    case 1U:
                        return GetSubTypeByPath(tail(path), *type.Cast<TDictExprType>()->GetPayloadType());
                    default:
                        break;
                }
            }
...
size_t GetElementsCount(const TTypeAnnotationNode* type) {
    if (type) {
        switch (type->GetKind()) {
            case ETypeAnnotationKind::Tuple:
                return type->Cast<TTupleExprType>()->GetSize();
            case ETypeAnnotationKind::Multi:
                return type->Cast<TMultiExprType>()->GetSize();
            case ETypeAnnotationKind::Struct:
                return type->Cast<TStructExprType>()->GetSize();
            default:
                break;
        }
    }
...
const TConstraintWithFieldsNode* TStreamingConstraintNode::DoGetSimplifiedForType(const TTypeAnnotationNode&, TExprContext&) const {
    return this;
}
