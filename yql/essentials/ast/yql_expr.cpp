...
void DumpNode(const TExprNode& node, IOutputStream& out, ui32 level, TNodeSet& visited) {
    ...
    if (!visited.emplace(&node).second) {
        if (node.Type() == TExprNode::Callable || node.Type() == TExprNode::List || node.Type() == TExprNode::Lambda || node.Type() == TExprNode::Arguments) {
            out << " ...";
            showChildren = false;
        }
    }
...
TAstNode* ConvertTypeAnnotationToAst(const TTypeAnnotationNode& annotation, TMemoryPool& pool, bool refAtoms) {
    switch (annotation.GetKind()) {
        ...
        case ETypeAnnotationKind::Item: {
            ...
            auto name = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), casted->GetName(), pool, TNodeFlags::ArbitraryContent) : TAstNode::NewAtom(TPosition(), casted->GetName(), pool, TNodeFlags::ArbitraryContent);
        ...
        case ETypeAnnotationKind::Data: {
            ...
            auto datatype = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), annotation.Cast<TDataExprType>()->GetName(), pool) : TAstNode::NewAtom(TPosition(), annotation.Cast<TDataExprType>()->GetName(), pool);
            if (auto params = dynamic_cast<const TDataExprParamsType*>(&annotation)) {
                auto param1 = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), params->GetParamOne(), pool) : TAstNode::NewAtom(TPosition(), params->GetParamOne(), pool);

                auto param2 = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), params->GetParamTwo(), pool) : TAstNode::NewAtom(TPosition(), params->GetParamTwo(), pool);
        ...
        case ETypeAnnotationKind::Resource: {
            ...
            auto restype = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), annotation.Cast<TResourceExprType>()->GetTag(), pool, TNodeFlags::ArbitraryContent) : TAstNode::NewAtom(TPosition(), annotation.Cast<TResourceExprType>()->GetTag(), pool, TNodeFlags::ArbitraryContent);
        ...
        case ETypeAnnotationKind::Tagged: {
            ...
            auto restype = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), annotation.Cast<TTaggedExprType>()->GetTag(), pool, TNodeFlags::ArbitraryContent) : TAstNode::NewAtom(TPosition(), annotation.Cast<TTaggedExprType>()->GetTag(), pool, TNodeFlags::ArbitraryContent);
        ...
        case ETypeAnnotationKind::Error: {
            ...
            auto file = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), err.Position.File, pool, TNodeFlags::ArbitraryContent) : TAstNode::NewAtom(TPosition(), err.Position.File, pool, TNodeFlags::ArbitraryContent);
            auto message = refAtoms ? TAstNode::NewLiteralAtom(TPosition(), err.GetMessage(), pool, TNodeFlags::ArbitraryContent) : TAstNode::NewAtom(TPosition(), err.GetMessage(), pool, TNodeFlags::ArbitraryContent);
...
bool AddParameterDependencies(const TString& url, const TAstNode& node, TContext& ctx) {
    ...
    if (!world.empty()) {
        ...
        for (const auto& name : names) {
            ...
            auto newWorld = TExprNode::TListType{ctx.Expr.NewCallable(node.GetPosition(), "Left!", {ctx.Expr.NewCallable(node.GetPosition(), "Cons!", std::move(args))})};
...
bool CompileDeclare(const TAstNode& node, TContext& ctx, bool checkOnly) {
    ...
    auto parameterExpr = ctx.ProcessNode(node,
                                         ctx.Expr.NewCallable(typePos, "Parameter", {ctx.Expr.NewAtom(node.GetPosition(), nameStr),
...
bool ValidateName(TPosition position, TStringBuf name, TStringBuf descr, TExprContext& ctx) {
    ...
    if (!IsUtf8(name)) {
        ctx.AddError(TIssue(position, TStringBuilder() << TString(descr).to_title() << " name must be a valid utf-8 byte sequence: " << TString{name}.Quote()));
        return false;
    }

    if (name.size() > 16_KB) {
        ctx.AddError(TIssue(position, TStringBuilder() << TString(descr).to_title() << " name length must be less than " << 16_KB));
        return false;
    }
