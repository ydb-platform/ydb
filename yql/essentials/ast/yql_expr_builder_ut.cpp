...
Y_UNIT_TEST(TestRootEmptyListTwice) {
    TExprContext ctx;
    // clang-format off
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).List().List().Build(), yexception);
    // clang-format on
}
...
Y_UNIT_TEST(TestWrongLevelBuild) {
    TExprContext ctx;
    // clang-format off
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).List().Build(), yexception);
    // clang-format on
}
...
Y_UNIT_TEST(TestWrongLevelSeal) {
    TExprContext ctx;
    // clang-format off
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Seal(), yexception);
    // clang-format on
}
...
Y_UNIT_TEST(TestIncompleteRootLambda) {
    TExprContext ctx;
    // clang-format off
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Lambda().Build(), yexception);
    // clang-format on
}
...
Y_UNIT_TEST(TestIncompleteInnerLambda) {
    TExprContext ctx;
    // clang-format off
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).List().Lambda().Seal().Build(), yexception);
    // clang-format on
}
...
Y_UNIT_TEST(TestParamAtRoot) {
    TExprContext ctx;
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Param("aaa").Build(), yexception);
}
...
Y_UNIT_TEST(TestAddAtLambda) {
    ...
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle())
        .Lambda()
            .Add(1, ctx.Builder(TPositionHandle())
                .Atom("ABC")
                .Build())
        .Seal()
        .Build(), yexception);
...
Y_UNIT_TEST(TestIndexedArgAsLambdaBody) {
    TExprContext ctx;
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Lambda().Param("x").Arg(1, "x").Seal().Build(), yexception);
}

Y_UNIT_TEST(TestWrongArgName) {
    TExprContext ctx;
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Lambda().Param("x").Arg("y").Seal().Build(), yexception);
}
...
Y_UNIT_TEST(TestNonIndexedArg) {
    TExprContext ctx;
    UNIT_ASSERT_EXCEPTION(ctx.Builder(TPositionHandle()).Lambda().Param("x").Callable("f").Arg("x").Seal().Seal().Build(), yexception);
}
...
Y_UNIT_TEST(TestApplyPartialLambdaArgAsRoot) {
    ...
    auto res = ctx.Builder(TPositionHandle())
        .Lambda()
            .Param("y")
            .ApplyPartial(lambda->HeadPtr(), lambda->Child(1)->HeadPtr())
                .With(0, "y")
            .Seal()
        .Seal()
        .Build();
...
Y_UNIT_TEST(TestApplyPartialLambdaArgInContainer) {
    ...
    auto res = ctx.Builder(TPositionHandle())
        .Lambda()
            .Param("y")
            .Callable("Func3")
                .ApplyPartial(0, lambda->HeadPtr(), lambda->Child(1)->HeadPtr())
                    .With(0, "y")
                .Seal()
            .Seal()
        .Seal()
...
Y_UNIT_TEST(TestApplyOuterArg) {
    ...
    auto res2 = ctx.Builder(TPositionHandle())
        .Lambda()
            .Callable("Func3")
                .ApplyPartial(0, nullptr, ast->Child(1)->Child(1)->ChildPtr(1))
                    .WithNode(ast->Head().Head(), TExprNode::TPtr(atom))
                .Seal()
            .Seal()
        .Seal()
        .Build();
