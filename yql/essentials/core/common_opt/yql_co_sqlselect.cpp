...
TExprNode::TPtr ExpandSqlSelectImpl(
...
TExprNode::TPtr ExpandSqlSelect(const TExprNode::TPtr& node, TExprContext& ctx, TOptimizeContext& optCtx) {
    ...
    return ExpandSqlSelectImpl(node, /*originalNode=*/node, ctx, optCtx, Nothing(), {}, {});
}
...
TExprNode::TPtr ExpandSqlSelectSublink(
    ...
    const TVector<TString>& outerInputAliases)
{
    return ExpandSqlSelectImpl(node, originalNode, ctx, optCtx, subLinkId, outerInputs, outerInputAliases);
}
