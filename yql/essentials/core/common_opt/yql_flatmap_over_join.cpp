...
// Maps the given `labelNames` collected from join tree to `joinLabels` associated with `EquiJoin` callable.
TVector<std::pair<THashSet<TString>, TExprNode::TPtr>> MapLabelNamesToJoinLabels(const TVector<std::pair<THashSet<TString>, TExprNode::TPtr>>& joinLabels,
    ...
    // We could have a situation with multiple labels associated with one set of join keys, so we want to match in ones.
    ...
    const TPositionHandle& pos)
{
    ...
    // Stores labels as hash set and associated join keys as `TExprNode::TPtr`.
    ...
    // Collect join lables for left child of the `Left` join, they are used in `EquiJoin` for `Left Only` and `Inner`.
