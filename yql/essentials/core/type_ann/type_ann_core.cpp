    ...
    if (atomNode.Flags() & TNodeFlags::BinaryContent) {
        ...
        if (!isValid) {
            if (slot == NKikimr::NUdf::EDataSlot::Bool) {
                ...
                T data;
                isValid = TryFromString(atomNode.Content(), data) && NKikimr::NMiniKQL::IsValidValue(slot, NKikimr::NUdf::TUnboxedValuePod(data));
            }
        }
...
bool IsValidTzData(TExprNode& atomNode, const TStringBuf& type, TExprContext& ctx, NKikimr::NUdf::EDataSlot slot, TMaybe<TString>& plainValue) {
    ...
    if (atomNode.Flags() & TNodeFlags::BinaryContent) {
        ...
        GetNext(atom, ',', value);
        T data;
        isValid = TryFromString(value, data) && NKikimr::NMiniKQL::IsValidValue(slot, NKikimr::NUdf::TUnboxedValuePod(data));
...
std::pair<TExprNode::TPtr, const TTypeAnnotationNode*> MakeRepr(const TExprNode::TPtr& input, const TTypeAnnotationNode* type, TExprContext& ctx) {
    ...
    switch (type->GetKind()) {
        ...
        case ETypeAnnotationKind::Optional: {
            ...
            return {ctx.NewCallable(input->Pos(), "Map", {input,
        ...
        case ETypeAnnotationKind::List: {
            ...
            return {ctx.NewCallable(input->Pos(), "OrderedMap", {input,
                                                                 ctx.NewLambda(input->Pos(), ctx.NewArguments(input->Pos(), {arg}), std::move(inner.first))}),
        ...
        case ETypeAnnotationKind::Dict: {
            ...
            auto mapped = ctx.NewCallable(input->Pos(), "Map", {ctx.NewCallable(input->Pos(), "DictItems", {input}),
...
bool EnsureJsonQueryFunction(const NNodes::TCoJsonQueryBase& function, TContext& ctx, bool& isUniversal) {
    ...
    if (payloadType->GetKind() != ETypeAnnotationKind::Resource || payloadType->Cast<TResourceExprType>()->GetTag() != "JsonNode") {
...
IGraphTransformer::TStatus KeyWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    for (auto& child : input->Children()) {
        ...
        if (child->ChildrenSize() != 1 && child->ChildrenSize() != 2) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Expected tuple of size 1 or 2, but got: " << input->ChildrenSize()));
            return IGraphTransformer::TStatus::Error;
        }
    ...
    if (sourceName) {
        ...
        for (auto& item : structType->GetItems()) {
            ...
            if (columnItemName == columnName) {
                if (goalItemName) {
                    auto issue = TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "column name: " << columnName << " conflicted without correlation name it may be one of: " << goalItemName << ", " << itemName);
                    if (columnOrType) {
...
IGraphTransformer::TStatus TryMemberWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    const TTypeAnnotationNode* memberType = (isStructOptional && !fieldType->IsOptionalOrNull()) ? ctx.Expr.MakeType<TOptionalExprType>(fieldType) : fieldType;

    const TTypeAnnotationNode* resultType = (isStructOptional && !isOptional) ? ctx.Expr.MakeType<TOptionalExprType>(otherType) : otherType;
...
IGraphTransformer::TStatus FlattenByColumnsWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    for (++iter; iter != input->Children().end(); ++iter) {
        ...
        if (argType == TExprNode::List) {
            ...
            if (!aliases.emplace(alias).second) {
                ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(aliasNode->Pos()), TStringBuilder() << "Duplicate flatten alias found: " << alias));
                return IGraphTransformer::TStatus::Error;
            }
            if (flattenByColumns.contains(alias)) {
                ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(columnNameNode->Pos()), TStringBuilder() << "Collision between alias and column name: " << alias));
                return IGraphTransformer::TStatus::Error;
            }
        }
        ...
        if (!flattenByColumns.emplace(TString(columnName), alias).second) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(columnNameNode->Pos()), TStringBuilder() << "Duplicate flatten field found: " << columnName));
            return IGraphTransformer::TStatus::Error;
        }
        if (aliases.contains(columnName)) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(columnNameNode->Pos()), TStringBuilder() << "Collision between alias and column name: " << columnName));
            return IGraphTransformer::TStatus::Error;
        }
    }
    ...
    for (auto& field : type->Cast<TStructExprType>()->GetItems()) {
        ...
        if (flattenIter == flattenByColumns.end()) {
            if (aliases.contains(fieldName)) {
                ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Conflict flatten alias and column name: '" << fieldName << "'"));
                return IGraphTransformer::TStatus::Error;
            }
        ...
        if (mode == "list" && fieldType->GetKind() != ETypeAnnotationKind::List) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Expected (optional) list type in field of struct: '" << fieldName << "', but got: " << *field->GetItemType()));
            return IGraphTransformer::TStatus::Error;
        }

        if (mode == "dict" && fieldType->GetKind() != ETypeAnnotationKind::Dict) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Expected (optional) dict type in field of struct: '" << fieldName << "', but got: " << *field->GetItemType()));
            return IGraphTransformer::TStatus::Error;
        }
        ...
        if (mode == "list" || (mode == "auto" && !fieldOptional && fieldType->GetKind() == ETypeAnnotationKind::List)) {
            ...
            if (!IsAvailable(NFeature::FlattenOptionalByNonOptional, ctx.Types) && !fieldOptional) {
                ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Expected optional type in field of struct: '" << fieldName << "', but got: " << *field->GetItemType()));
                return IGraphTransformer::TStatus::Error;
            }

            flattenItemType = fieldType;
        } else {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Expected list, dict or optional types in field of struct: '" << fieldName << "', but got: " << *field->GetItemType()));
            return IGraphTransformer::TStatus::Error;
        }
    ...
    if (!flattenByColumns.empty()) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(structObj->Pos()), TStringBuilder() << "Column for flatten \"" << flattenByColumns.begin()->first << "\" does not exist"));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus NthWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (index >= tupleType->GetSize()) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Index out of range. Index: " << index << ", size: " << tupleType->GetSize()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus CompareWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    switch (CanCompare<Equality>(input->Head().GetTypeAnn(), input->Tail().GetTypeAnn())) {
        ...
        case ECompareOptions::Uncomparable:
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Uncompatible types in compare: " << *input->Head().GetTypeAnn() << " '" << input->Content() << "' " << *input->Tail().GetTypeAnn()));
            return IGraphTransformer::TStatus::Error;
...
IGraphTransformer::TStatus FullTextBuiltinWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (positionalArguments->ChildrenSize() > 2) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(positionalArguments->Pos()), TStringBuilder() << "Expected at most 2 positional argument(s) for fulltext builtin, but got " << positionalArguments->ChildrenSize() << " arguments. Use named arguments for additional arguments instead."));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus HybridRankBuiltinWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (positionalArguments->ChildrenSize() < 2) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(positionalArguments->Pos()), TStringBuilder() << "HybridRank expects at least 2 positional arguments (fulltext score and vector distance), but got " << positionalArguments->ChildrenSize()));
        return IGraphTransformer::TStatus::Error;
    }
    ...
    if (hasCustomLambda) {
        ...
        {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(lambda->Pos()), TStringBuilder() << "HybridRank " << lambdaName << " must return a numeric score, but got " << *lambda->GetTypeAnn()));
            return IGraphTransformer::TStatus::Error;
        }
    }
...
IGraphTransformer::TStatus ByteAtWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (dataType != EDataSlot::String && dataType != EDataSlot::Utf8) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Head().Pos()), TStringBuilder() << "Expected either String or Utf8, but got: " << *type));
        return IGraphTransformer::TStatus::Error;
    }
            ...
            if (argType->GetKind() == ETypeAnnotationKind::Tuple) {
                if (const auto size = argType->Cast<TTupleExprType>()->GetSize()) {
                    ...
                    for (auto i = 0U; i < size; ++i) {
                        list.emplace_back(
                            input->Head().IsList() ? input->Head().ChildPtr(i) : ctx.Expr.NewCallable(input->Pos(), "Nth", {input->HeadPtr(), ctx.Expr.NewAtom(input->Pos(), ToString(i), TNodeFlags::Default)}));
                    }
...
IGraphTransformer::TStatus WithContextWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (input->Tail().Content() != "Agg" && input->Tail().Content() != "WinAgg") {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Tail().Pos()), TStringBuilder() << "Unexpected context type: " << input->Tail().Content()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus UntagWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (taggedType->GetTag() != input->Tail().Content()) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Tail().Pos()), TStringBuilder() << "Expected tag: " << taggedType->GetTag() << ", but got: " << input->Tail().Content()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus ConvertWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!CanConvert(sourceSlot, targetSlot)) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Cannot convert type " << *input->Head().GetTypeAnn() << " into " << NKikimr::NUdf::GetDataTypeInfo(targetSlot).Name));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus BitCastWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!((EDataSlot::Bool == sourceSlot || IsDataTypeIntegral(sourceSlot) || IsDataTypeDateOrTzDateOrInterval(sourceSlot)) && IsDataTypeIntegral(targetSlot))) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Cannot bit cast type " << *input->Head().GetTypeAnn() << " into " << NKikimr::NUdf::GetDataTypeInfo(targetSlot).Name));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus WidenIntegralWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!(isDecimal || IsDataTypeNumeric(sourceDataType->GetSlot()) || isInterval)) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Expected numeric type, but got " << *input->Head().GetTypeAnn()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus CoalesceWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (SilentInferCommonType(convertedArg2, *rightItemType, convertedArg1, *leftItemType, ctx.Expr, ctx.Types,
        ...
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "uncompatible coalesce types, first type: " << *leftType << ", second type: " << *rightType));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus NanvlWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (isDecimal) {
        ...
        input->SetTypeAnn((isOptional1 || isOptional2) ? (const TTypeAnnotationNode*)ctx.Expr.MakeType<TOptionalExprType>(retDataType) : (const TTypeAnnotationNode*)retDataType);
    } else {
...
IGraphTransformer::TStatus OptionalWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!IsSameAnnotation(*type->Cast<TOptionalExprType>()->GetItemType(), *input->Tail().GetTypeAnn())) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Tail().Pos()), TStringBuilder() << "Mismatch item type, expected: " << *type->Cast<TOptionalExprType>()->GetItemType() << " but got: " << *input->Tail().GetTypeAnn()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus SqlInWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!HasSetting(*options, "tableSource")) {
        ...
        const TTypeAnnotationNode* listItemType = (collectionKind == ETypeAnnotationKind::List) ? collectionType->Cast<TListExprType>()->GetItemType() : nullptr;
...
IGraphTransformer::TStatus DictItemsWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!dictType) {
        ...
        input->SetTypeAnn(ctx.Expr.MakeType<TListExprType>(Mode == EDictItems::Keys ? dictType->GetKeyType() : dictType->GetPayloadType()));
    }
...
IGraphTransformer::TStatus AsDictWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if constexpr (IsStrict) {
        ...
        const auto dictType = IsSet ? ctx.Expr.MakeType<TDictExprType>(commonType, ctx.Expr.MakeType<TVoidExprType>()) : ctx.Expr.MakeType<TDictExprType>(commonType->Cast<TTupleExprType>()->GetItems().front(), commonType->Cast<TTupleExprType>()->GetItems().back());
...
IGraphTransformer::TStatus IfWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if constexpr (IsStrict) {
        ...
        if (IsSameAnnotation(*thenType, *elseType)) {
            ...
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Different types of branches, then type: " << *thenType << ", else type: " << *elseType));
            return IGraphTransformer::TStatus::Error;
        }
...
IGraphTransformer::TStatus StructMergeWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (left->GetTypeAnn() && (left->GetTypeAnn()->GetKind() == ETypeAnnotationKind::Universal ||
                               left->GetTypeAnn()->GetKind() == ETypeAnnotationKind::UniversalStruct) ||
        right->GetTypeAnn() && (right->GetTypeAnn()->GetKind() == ETypeAnnotationKind::Universal ||
    ...
    auto mergeMembers = [&ctx, &buildJustMember, &input, &left, &right, &mergeLambda](const TStringBuf& name, bool hasLeft, bool hasRight) -> TExprNode::TPtr {
        auto leftMaybe = hasLeft ? buildJustMember(left, name) : ctx.Expr.NewCallable(input->Pos(), "Nothing", {ExpandType(input->Pos(), *ctx.Expr.MakeType<TOptionalExprType>(right->GetTypeAnn()->Cast<TStructExprType>()->FindItemType(name)), ctx.Expr)});

        auto rightMaybe = hasRight ? buildJustMember(right, name) : ctx.Expr.NewCallable(input->Pos(), "Nothing", {ExpandType(input->Pos(), *ctx.Expr.MakeType<TOptionalExprType>(left->GetTypeAnn()->Cast<TStructExprType>()->FindItemType(name)), ctx.Expr)});
...
IGraphTransformer::TStatus EnsureTypeKindWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (ToString(input->Head().GetTypeAnn()->GetKind()) != input->Tail().Content()) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()),
                                 TStringBuilder() << "Expected type kind: " << input->Tail().Content() << ", but got: " << input->Head().GetTypeAnn()->GetKind()));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus ToDictWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (const auto inputType = input->Head().GetTypeAnn(); inputType && inputType->GetKind() == ETypeAnnotationKind::Optional && inputType->Cast<TOptionalExprType>()->GetItemType()->GetKind() != ETypeAnnotationKind::Optional) {
        // clang-format off
        output = ctx.Expr.Builder(input->Pos())
            .Callable("Map")
                .Add(0, input->HeadPtr())
...
IGraphTransformer::TStatus UdfWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (input->ChildrenSize() > 7) {
        ...
        for (const auto& child : settings->Children()) {
            ...
            } else if (settingName == "cpu" || settingName == "extraMem" || settingName == "minLang" || settingName == "maxLang")
            {
                if (!EnsureTupleSize(*child, 2, ctx.Expr)) {
                    return IGraphTransformer::TStatus::Error;
                }
...
IGraphTransformer::TStatus NamedApplyWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (convertStatus.Level == IGraphTransformer::TStatus::Error) {
        ...
        if (hasError) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Child(1)->Pos()), TStringBuilder() << "Mismatch positional argument types, expected: " << *static_cast<const TTypeAnnotationNode*>(expectedTupleType) << ", but got: " << *input->Child(1)->GetTypeAnn()));
        } else {
    ...
    if (convertStatus.Level == IGraphTransformer::TStatus::Error) {
        ...
        if (hasError) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Child(2)->Pos()), TStringBuilder() << "Mismatch named argument types, expected: " << *static_cast<const TTypeAnnotationNode*>(expectedStructType) << ", but got: " << *input->Child(2)->GetTypeAnn()));
        } else {
            convertStatus = IGraphTransformer::TStatus::Ok;
        }
    }
    ...
    if (!tupleAutoMaps.empty() || !structAutoMaps.empty()) {
        ...
        for (ui32 i = 0; i < tupleAutoMaps.size(); ++i) {
            ...
            inputArgs.push_back(ctx.Expr.NewCallable(input->Pos(), "Nth", {input->ChildPtr(1),
        ...
        for (ui32 i = 0; i < structAutoMaps.size(); ++i) {
            ...
            inputArgs.push_back(ctx.Expr.NewCallable(input->Pos(), "Member", {input->ChildPtr(2),
        ...
        if (!tupleAutoMaps.empty()) {
            ...
            for (ui32 i = 0; i < tupleType->GetSize(); ++i) {
                ...
                if (it == indexToTupleIndex.end()) {
                    newTupleItems.push_back(ctx.Expr.NewCallable(input->Pos(), "Nth", {input->ChildPtr(1),
        ...
        if (!structAutoMaps.empty()) {
            ...
            for (ui32 i = 0; i < structType->GetSize(); ++i) {
                ...
                if (it == indexToStructIndex.end()) {
                    value = ctx.Expr.NewCallable(input->Pos(), "Member", {input->ChildPtr(2),
                                                                          ctx.Expr.NewAtom(input->Pos(), name)});
                } else {
                    value = args[it->second];
                }

                newStructItems.push_back(ctx.Expr.NewList(input->Pos(), {ctx.Expr.NewAtom(input->Pos(), name), value}));
            }
...
IGraphTransformer::TStatus CastStructWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    for (auto item : newStructType->GetItems()) {
        ...
        if (!oldItem) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Failed to find member with name: " << item->GetName() << " in struct " << *input->Head().GetTypeAnn()));
            return IGraphTransformer::TStatus::Error;
        }

        auto oldItemType = oldStructType->GetItems()[oldItem.GetRef()]->GetItemType();
        if (!IsSameAnnotation(*item->GetItemType(), *oldItemType)) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Pos()), TStringBuilder() << "Failed to convert member with name: " << item->GetName() << " from type " << *oldItemType << " to type " << *item->GetItemType()));
            return IGraphTransformer::TStatus::Error;
        }
    }
...
IGraphTransformer::TStatus DynamicVariantWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    const TTypeAnnotationNode* expectedType = ctx.Expr.MakeType<TOptionalExprType>(
        ctx.Expr.MakeType<TDataExprType>(
            variantType->GetUnderlyingType()->GetKind() == ETypeAnnotationKind::Tuple ? EDataSlot::Uint32 : EDataSlot::Utf8));
...
IGraphTransformer::TStatus SqlAccessWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (input->Head().Content() == "tuple") {
        ...
        if (unpacked->GetKind() == ETypeAnnotationKind::Dict) {
            ...
            output = ctx.Expr.NewCallable(input->Pos(), "Lookup", {ctx.Expr.NewCallable(input->Pos(), "ToIndexDict", {input->ChildPtr(1)}), input->ChildPtr(2)});
...
IGraphTransformer::TStatus SqlExternalFunctionWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    for (const auto& tuple : input->Child(2)->Children()) {
        ...
        if (!usedParams.insert(paramName).second) {
            ...
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(tuple->Pos()), TStringBuilder() << "Unknown param name: " << paramName.Quote()));
            return IGraphTransformer::TStatus::Error;
        }
    }

    if (inputType == nullptr && outputType == nullptr) {
        ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Child(2)->Pos()), TStringBuilder() << "EXTERNAL FUNCTION should have INPUT_TYPE/OUTPUT_TYPE parameter"));
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus SqlProjectWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (!transparentFields.empty()) {
        ...
        for (const auto& fieldName : transparentFields) {
            ...
            auto lambdaBody = ctx.Expr.NewCallable(input->Pos(), "Member", {lambdaArg,
                                                                            ctx.Expr.NewAtom(input->Pos(), fieldName)});
            newProjectItems.push_back(ctx.Expr.NewCallable(
                input->Pos(),
                "SqlProjectItem",
                {ctx.Expr.NewCallable(input->Pos(), "TypeOf", {input->Child(0)}),
                 ctx.Expr.NewAtom(input->Pos(), fieldName),
                 ctx.Expr.NewLambda(
                     input->Pos(),
...
IGraphTransformer::TStatus SqlProjectItemWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    YQL_ENSURE(itemType);
    if (itemType->GetKind() != ETypeAnnotationKind::Universal && itemType->GetKind() != ETypeAnnotationKind::UniversalStruct && !EnsureStructType(input->Head().Pos(), *itemType, ctx.Expr)) {
        return IGraphTransformer::TStatus::Error;
    }
        ...
        if (itemType->GetKind() != ETypeAnnotationKind::Universal &&
            ...
            auto issue = TIssue(ctx.Expr.GetPosition(input->Child(1)->Pos()),
                                TStringBuilder() << "Alias `" << alias << "` shadows column with the same name. It looks like comma is missed here. "
                                                                          "If not, it is recommended to use ... AS `"
...
IGraphTransformer::TStatus QueueCreateWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    input->SetTypeAnn(ctx.Expr.MakeType<TResourceExprType>(TStringBuilder() << NKikimr::NMiniKQL::ResourceQueuePrefix << FormatType(queueType)));
    return IGraphTransformer::TStatus::Ok;
}
...
IGraphTransformer::TStatus WeakFieldWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    if (isDefault) {
        ...
        if (convertStatus.Level == IGraphTransformer::TStatus::Error) {
            ctx.Expr.AddError(TIssue(ctx.Expr.GetPosition(input->Child(1)->Pos()), TStringBuilder() << "Default value not correspond weak field type: " << targetType));
            return IGraphTransformer::TStatus::Error;
        }
    }
    ...
    if (auto pos = structType->FindItem(fullMemberName)) {
        ...
        auto checkType = fieldType->GetKind() == ETypeAnnotationKind::Optional ? fieldType : ctx.Expr.MakeType<TOptionalExprType>(fieldType);
...
IGraphTransformer::TStatus MuxWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    if (inputType->GetKind() == ETypeAnnotationKind::Tuple) {
        ...
        for (size_t i = 0; i < tupleType->GetSize(); ++i) {
            ...
            if (itemType->GetKind() == ETypeAnnotationKind::Struct && AnyOf(itemType->Cast<TStructExprType>()->GetItems(), [](const TItemExprType* structItem) { return structItem->GetName().StartsWith("_yql_sys_"); })) {
...
IGraphTransformer::TStatus JsonValueWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    bool isUniversal1;
    bool isUniversal2;
    if (!EnsureMinArgsCount(*input, 7, ctx.Expr) || !EnsureMaxArgsCount(*input, 8, ctx.Expr) || !EnsureAtomOrUniversal(*input->Child(TCoJsonValue::idx_OnEmptyMode), ctx.Expr, isUniversal1) || !EnsureAtomOrUniversal(*input->Child(TCoJsonValue::idx_OnErrorMode), ctx.Expr, isUniversal2)) {
        return IGraphTransformer::TStatus::Error;
    }
    ...
    if (returningTypeArg) {
        ...
        if (!IsDataTypeNumeric(resultSlot) && !IsDataTypeDate(resultSlot) && resultSlot != EDataSlot::Utf8 && resultSlot != EDataSlot::String && resultSlot != EDataSlot::Bool) {
...
IGraphTransformer::TStatus JsonQueryWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExtContext& ctx) {
    ...
    bool isUniversal2;
    bool isUniversal3;
    if (!EnsureAtomOrUniversal(*input->Child(TCoJsonQuery::idx_WrapMode), ctx.Expr, isUniversal1) || !EnsureAtomOrUniversal(*input->Child(TCoJsonQuery::idx_OnEmpty), ctx.Expr, isUniversal2) || !EnsureAtomOrUniversal(*input->Child(TCoJsonQuery::idx_OnError), ctx.Expr, isUniversal3)) {
        return IGraphTransformer::TStatus::Error;
    }
...
IGraphTransformer::TStatus JsonVariablesWrapper(const TExprNode::TPtr& input, TExprNode::TPtr& output, TContext& ctx) {
    ...
    for (size_t i = 0; i < input->ChildrenSize(); i++) {
        ...
        if (!IsDataTypeNumeric(valueSlot) && !IsDataTypeDate(valueSlot) && valueSlot != EDataSlot::Utf8 && valueSlot != EDataSlot::Bool && valueSlot != EDataSlot::Json) {
    ...
    for (size_t i = 0; i < components.size(); ++i) {
        ...
        if (i % 2 == 0) {
            if (itemType->GetKind() != ETypeAnnotationKind::Data || itemType->Cast<TDataExprType>()->GetSlot() != EDataSlot::Int32) {
                ctx.AddError(TIssue(ctx.GetPosition(pos),
                                    TStringBuilder() << "Expected " << i << "th component of range boundary tuple to be Int32, but got: " << *itemType));
                return false;
            }
            ...
            ctx.AddError(TIssue(ctx.GetPosition(pos),
                                TStringBuilder() << "Expected " << i << "th component of range boundary tuple to be (multi) optional of "
                                                                        "comparable and equatable Data or Pg type, but got: "
                                                 << *itemType));
            return false;
        }
    }
...
public:
    ...
    TMaybe<IGraphTransformer::TStatus> ProcessCore(const TExprNode::TPtr& input, TExprNode::TPtr& output, TExprContext& ctx) {
        ...
        if (status == IGraphTransformer::TStatus::Ok && Types_.DeriveColumnOrder && !Types_.LookupColumnOrder(*input) && input->GetTypeAnn()->GetKind() != ETypeAnnotationKind::Universal) {
