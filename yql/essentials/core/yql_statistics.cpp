...
{
        if (ShufflingOrderingsByJoinLabels.empty()) {
            return "TShufflingOrderingsByJoinLabels{empty}";
        }

        TStringBuilder result;
        result << "TShufflingOrderingsByJoinLabels{" << ShufflingOrderingsByJoinLabels.size() << " entries: ";

        for (size_t i = 0; i < ShufflingOrderingsByJoinLabels.size(); ++i) {
            if (i > 0) result << "; ";

            const auto& [joinLabels, shufflings] = ShufflingOrderingsByJoinLabels[i];
