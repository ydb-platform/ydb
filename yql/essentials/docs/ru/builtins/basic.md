...
Если один из аргументов `Double`, то в выдаче `Double`, иначе `Float`. Если один из аргументов `Optional`, то и в выдаче `Optional`.
...
* Cpu - фактор, описывающий, насколько много CPU потребляет udf. Должно быть положительное число. Значение по умолчанию - "1". Чем оно больше, тем большая параллельность нужна для вызова такой udf. Пример: "4.5".
* ExtraMem - дополнительная память, требуемая udf в байтах. Значение по умолчанию - "0".
  Поддерживаемые форматы:
  1. Количество в байтах (неотрицательное целое число)
  2. Строковые представления с суффиксом: "K", "M", "G" (например, "2048M", "1G", "512K")
...
`Version()` возвращает строку, описывающую текущую версию узла, обрабатывающего запрос. В некоторых случаях, например, во время постепенного обновления кластера, она может возвращать разные строки в зависимости от того, какой узел обрабатывает запрос. Функция не принимает никаких аргументов.
...
Создает экземпляр гомогенного варианта (т.е. содержащего поля/элементы одного типа), причем индекс или поле варианта можно задавать динамически. При несуществующем индексе или имени поля будет возвращен `NULL`.
...
При наличии `AssumeStrict` оптимизатор сможет выполнить сначала фильтрацию, а потом JOIN.
...
Функция `WithSideEffects` или `WithSideEffectsMode` возвращает свой первый аргумент. Функция является требованием для оптимизатора и помечает внутреннее выражение (обычно вызов UDF) как содержащее побочные эффекты.

Побочным эффектом является:

* Влияние вызова одной функции на другую кроме как через передачу данных в аргументах – например, через какое-то глобальное состояние;
* Наблюдаемое поведение за пределами YQL во время выполнения запроса – например, чтение/запись данных во внешних системах.

Побочным эффектом не является:

* Счетчики и логи UDF, доступные в ABI. Иными словами, допускаются оптимизации, которые изменят значения этих счетчиков или записей в логи за счет другого количества вызовов UDF;
* Ошибка выполнения запроса.

Оптимизатор не должен удалять вычисление такого выражения и обычно не должен изменять количество его вычислений, если оно зависит, например, от каждой строки таблицы.

Возможные значения mode:

* `General` - допускаются все виды побочных эффектов, невозможно удаление общих подвыражений;
* `SemilatticeRT` - допускаются только идемпотентные и коммутирующие побочные эффекты, а результат выражения может быть вычислен один раз и переиспользован, что позволяет удалять общие подвыражения;
* `None` - побочных эффектов не допускается за пределами выражения.

Функция `WithSideEffects` это краткая форма вызова функции `WithSideEffectsMode` c mode равным `General`.

Типичный пример побочного эффекта вида `General` - выполнение `UPDATE` в другую систему, и возвращение количества измененных записей как результат выражения.
Типичный пример побочного эффекта вида `SemilatticeRT` - выполнение `UPSERT` в таблицу без неключевых колонок (что является идемпотентным и коммутирующим действием), и возвращение числа отсылаемых записей как результат выражения.
...
Функции доступны начиная с версии [2026.02](../changelog/2026.02.md).

Функция `AsErased` стирает тип значения, приводя любой входной тип в фиксированный. При этом запрещено передавать в `AsErased` [Linear](../types/linear.md) тип (но можно использовать тип `DynamicLinear`).
Функция `PeekErased` проверяет строгое совпадение оригинального типа с заданным. Если типы не совпадают, возвращается пустой `Optional`, иначе - заполненное значение исходного типа.

Типичное применение этих функций - построение рекурсивных функций и/или данных. Например, тип узла дерева и функция для его создания может выглядеть следующим образом:
...
1. По второму аргументу (номеру бита) выбирается соответствующий байт *с начала строки*.
...
Доступна начиная с версии [2026.01](../changelog/2026.01.md). Добавляет в переданное значение возможность содержать `NULL`. Если тип значения уже является опциональным, PostgreSQL типом или `NULL`, возвращает его без изменений. В противном случае — оборачивает значение в [Just](#optional-ops). В отличие от [Just](#optional-ops), не добавляет дополнительный уровень вложенности для уже опциональных значений.
...
Результатом будет соответственно структура или кортеж.
