...
**JSON** - слабо схематизированный [формат данных](https://www.json.org), представленный в YQL типом `Json`. В отличие от реляционных таблиц, JSON может хранить данные, для которых схема не определена. Вот пример валидного JSON:
...
3. `$.comments[1]` - обращение ко второму элементу массива JSON (нумерация элементов начинается с 0)
...
| Проверка булевого выражения на null     | `($.age > 20) is unknown`                       |
...
В отличие от некоторых языков программирования, в JsonPath булевыми значениями считаются не только `true` (истина) и `false` (ложь), но и `null` (неопределенность).

JsonPath считает все значения, полученные из JSON документа, не булевыми. Например, запрос `! $.is_valid_user` (логическое отрицание, примененное к полю `is_valid_user`), является синтаксически неверным поскольку поле `is_valid_user` не является булевым значением (даже если по факту в нем хранится `true` или `false`). Правильный способ написать такой запрос - это явно использовать сравнение с булевым значением, например `$.is_valid_user == false`.

### Логические операции

JsonPath поддерживает некоторые логические операции для работы с булевыми значениями.

Аргументы всех логических операций должны являться единственным булевым значением.
Все логические операции возвращают булевое значение.
...
Все операторы сравнения возвращают булевое значение. Оба аргумента оператора поддерживают наличие нескольких значений.
...
JsonPath поддерживает предикаты - выражения, возвращающие булевое значение, и проверяющие некоторое условие. Их можно использовать, например, в фильтрах.
...
Предикат `is unknown` позволяет проверить, является ли булевое значение `null`.
...
Где `<expression>` - это JsonPath выражение, которое нужно проверить. Допускаются только выражения, возвращающие булевое значение. Скобки вокруг выражения обязательны.
...
Выражение в фильтре должно возвращать булевое значение.
...
В отличие от многих языков программирования, JsonPath не поддерживает создание новых переменных или изменение существующих.
