...
Для этого оператора есть аналогичная функция [Concat](../builtins/basic.md#concat), которая поддерживает произвольное число аргументов.
...
Начиная с версии языка [2026.02](../changelog/2026.02.md) арифметические операторы поддерживают аргументы разных типов `Decimal(N, M)`. Оба аргумента неявно приводятся к общему типу Decimal.

Такое приведение возможно, только если для типов аргументов существует общий тип `Decimal`. Например, для `Decimal(35, 0)` и `Decimal(35, 35)` общего типа не существует: он должен был бы одновременно хранить 35 знаков до и 35 знаков после десятичной точки, тогда как максимальная точность `Decimal` равна 35.

```yql
-- `sum` имеет тип `Decimal(11, 3)` и значение `20`
SELECT
