...
struct TColumnDataPackInfo {
  ...
  TType
      *MKQLType; // Data type of the column in term of compute nodes data flows
  NUdf::EDataSlot DataType =
      NUdf::EDataSlot::Uint32;     // Data type of the column for standard types
                                   // (TDataType)
  ...
  bool IsPresortSupported = false; // True if pg type supports presort and can
                                   // be interpreted as string value
  bool IsIType = false; // True if column need to be processed via IHash,
                        // IEquate interfaces
...
struct TGraceJoinPacker {
  ...
  ui64 TuplesBatchPacked =
      0;                   // Number of tuples packed during current join batch
  ui64 TuplesUnpacked = 0; // Total number of unpacked tuples
  ui64 BatchSize =
      PartialJoinBatchSize; // Batch size for partial table packing and join
  std::chrono::time_point<std::chrono::system_clock>
      StartTime; // Start time of execution
  std::chrono::time_point<std::chrono::system_clock>
      EndTime; // End time of execution
  std::vector<ui64>
      TupleIntVals; // Packed value of all fixed length values of table tuple.
                    // Keys columns should be packed first.
  ...
  std::vector<std::shared_ptr<TValuePacker>>
      Packers;                         // Packers for composite data types
  const THolderFactory &HolderFactory; // To use during unpacking
  std::vector<TColumnDataPackInfo>
      ColumnsPackInfo; // Information about columns packing
  ...
  std::vector<NUdf::TUnboxedValue *>
      TuplePtrs; // Storage for tuple data pointers to use in FetchValues
  std::vector<std::string> TupleStringHolder; // Storage for complex tuple data
                                              // types serialized to strings
  std::vector<NUdf::TUnboxedValue>
      IColumnsHolder; // Storage for interface-based types (IHash, IEquate)
  ...
  ui64 KeyIntColumnsNum =
      0; // Total number of key int columns in original table
  ui64 PackedKeyIntColumnsNum = 0; // Length of ui64 array containing data of
                                   // all key int columns after packing
  ...
  ui64 PackedDataIntColumnsNum = 0; // Length of ui64 array containing data of
                                    // all non-key int columns after packing
  ...
  const NUdf::TLogComponentId
      LogComponent;   // Id of current component for logging. GracejJoin here
  inline void Pack(); // Packs new tuple from TupleHolder and TuplePtrs to
                      // TupleIntVals, TupleStrSizes, TupleStrings
  inline void
  UnPack(); // Unpacks packed values from TupleIntVals, TupleStrSizes,
            // TupleStrings into TupleHolder and TuplePtrs
  TGraceJoinPacker(const std::vector<TType *> &columnTypes,
                   const std::vector<ui32> &keyColumns,
                   const THolderFactory &holderFactory, bool isAny,
...
void TGraceJoinPacker::UnPack() {
  ...
  for (ui64 i = 0; i < ColumnsPackInfo.size(); i++) {
    ...
    if (colType->GetKind() != TType::EKind::Data) {
      ...
      value = Packers[pi.ColumnIdx]->Unpack(
          TStringBuf(TupleStrings[offset], TupleStrSizes[offset]),
          HolderFactory);
      continue;
    }
    ...
    case NUdf::EDataSlot::Decimal: {
      const auto des =
          NYql::NDecimal::Deserialize(buffPtr, sizeof(NYql::NDecimal::TInt128));
      MKQL_ENSURE(!NYql::NDecimal::IsError(des.first),
                  "Bad packed data: invalid decimal.");
      value = NUdf::TUnboxedValuePod(des.first);
      break;
    }
    default: {
      value = MakeString(
          NUdf::TStringRef(TupleStrings[offset], TupleStrSizes[offset]));
    }
    }
  }
}

TGraceJoinPacker::TGraceJoinPacker(const std::vector<TType *> &columnTypes,
                                   const std::vector<ui32> &keyColumns,
                                   const THolderFactory &holderFactory,
                                   bool isAny,
                                   NUdf::TLoggerPtr logger = nullptr,
                                   NUdf::TLogComponentId logComponent = 0)
    : ColumnTypes(columnTypes), HolderFactory(holderFactory), IsAny(isAny),
      Logger(logger), LogComponent(logComponent) {
  ...
  for (ui32 i = 0; i < columnTypes.size(); i++) {
    ...
    ui32 keyColNums = std::count_if(keyColumns.begin(), keyColumns.end(),
                                    [&](ui32 k) { return k == i; });
  ...
  ui64 totalIntColumnsNum = std::count_if(
      ColumnsPackInfo.begin(), ColumnsPackInfo.end(),
      [](TColumnDataPackInfo a) { return !a.IsString && !a.IsPgType; });
  ui64 totalIColumnsNum =
      std::count_if(ColumnsPackInfo.begin(), ColumnsPackInfo.end(),
                    [](TColumnDataPackInfo a) { return a.IsIType; });
  ui64 totalStrColumnsNum = nColumns - totalIntColumnsNum - totalIColumnsNum;

  ui64 keyIntColumnsNum =
      std::count_if(ColumnsPackInfo.begin(), ColumnsPackInfo.end(),
                    [](TColumnDataPackInfo a) {
                      return (a.IsKeyColumn && !a.IsString && !a.IsPgType);
                    });
  ui64 keyIColumnsNum = std::count_if(
      ColumnsPackInfo.begin(), ColumnsPackInfo.end(),
      [](TColumnDataPackInfo a) { return (a.IsKeyColumn && a.IsIType); });
  ...
  std::transform(TupleHolder.begin(), TupleHolder.end(),
                 std::back_inserter(TuplePtrs),
                 [](NUdf::TUnboxedValue &v) { return std::addressof(v); });
  ...
  for (auto &p : ColumnsPackInfo) {
    if (!p.IsString && !p.IsIType) {
      if (prevKeyColumn && !p.IsKeyColumn) {
        currIntOffset =
            ((currIntOffset + sizeof(ui64) - 1) / sizeof(ui64)) * sizeof(ui64);
      }
      ...
      currIOffset++;
      GraceJoin::TColTypeInterface cti{
          MakeHashImpl(p.MKQLType), MakeEquateImpl(p.MKQLType),
          std::make_shared<TValuePacker>(true, p.MKQLType), HolderFactory};
      ColumnInterfaces.push_back(cti);
    }
  }

  PackedKeyIntColumnsNum =
      (keyIntOffset + sizeof(ui64) - 1) / sizeof(ui64) - NullsBitmapSize;
  PackedDataIntColumnsNum = (currIntOffset + sizeof(ui64) - 1) / sizeof(ui64) -
                            PackedKeyIntColumnsNum - NullsBitmapSize;
  ...
  TablePtr = std::make_unique<GraceJoin::TTable>(
      Logger, LogComponent, PackedKeyIntColumnsNum, KeyStrColumnsNum,
      PackedDataIntColumnsNum, DataStrColumnsNum, KeyIColumnsNum,
      DataIColumnsNum, NullsBitmapSize, cti_p, IsAny);
}

class TGraceJoinSpillingSupportState
    : public TComputationValue<TGraceJoinSpillingSupportState> {
  using TBase = TComputationValue<TGraceJoinSpillingSupportState>;
  enum class EOperatingMode { InMemory, Spilling, ProcessSpilled };

public:
  TGraceJoinSpillingSupportState(
      TMemoryUsageInfo *memInfo, IComputationWideFlowNode *flowLeft,
      IComputationWideFlowNode *flowRight, EJoinKind joinKind,
      EAnyJoinSettings anyJoinSettings, const std::vector<ui32> &leftKeyColumns,
      ...
      const std::vector<TType *> &rightColumnsTypes, TComputationContext &ctx,
      const bool isSelfJoin, bool isSpillingAllowed, NUdf::TLoggerPtr logger,
      NUdf::TLogComponentId logComponent)
      : TBase(memInfo), FlowLeft(flowLeft), FlowRight(flowRight),
        JoinKind(joinKind), LeftKeyColumns(leftKeyColumns),
        RightKeyColumns(rightKeyColumns), LeftRenames(leftRenames),
        RightRenames(rightRenames),
        LeftPacker(std::make_unique<TGraceJoinPacker>(
            leftColumnsTypes, leftKeyColumns, ctx.HolderFactory,
            (anyJoinSettings == EAnyJoinSettings::Left ||
             anyJoinSettings == EAnyJoinSettings::Both ||
             joinKind == EJoinKind::RightSemi ||
             joinKind == EJoinKind::RightOnly),
            logger, logComponent)),
        RightPacker(std::make_unique<TGraceJoinPacker>(
            rightColumnsTypes, rightKeyColumns, ctx.HolderFactory,
            (anyJoinSettings == EAnyJoinSettings::Right ||
             anyJoinSettings == EAnyJoinSettings::Both ||
             joinKind == EJoinKind::LeftSemi ||
             joinKind == EJoinKind::LeftOnly),
            logger, logComponent)),
        JoinedTablePtr(
            std::make_unique<GraceJoin::TTable>(logger, logComponent)),
        ...
        IsSpillingAllowed(isSpillingAllowed), Logger(logger),
        LogComponent(logComponent) {
    UDF_LOG(Logger, LogComponent, GRACEJOIN_DEBUG,
            TStringBuilder() << (const void *)&*JoinedTablePtr
                             << "# AnyJoinSettings=" << (int)anyJoinSettings
    ...
    if (ctx.CountersProvider) {
      ...
      CounterOutputRows_ =
          ctx.CountersProvider->GetCounter(id, Counter_OutputRows, false);
    }
  }

  EFetchResult FetchValues(TComputationContext &ctx,
                           NUdf::TUnboxedValue *const *output) {
    while (true) {
      switch (GetMode()) {
...
private:
  bool CanSkipRightOnLeftFinished() const {
    return !IsSelfJoin_ && LeftPacker->TuplesPacked == 0 &&
           GraceJoin::ShouldSkipRightIfLeftEmpty(JoinKind);
  }

  bool CanSkipLeftOnRightFinished() const {
    return !IsSelfJoin_ && RightPacker->TuplesPacked == 0 &&
           GraceJoin::ShouldSkipLeftIfRightEmpty(JoinKind);
  }
  ...
  void SwitchMode(EOperatingMode mode, TComputationContext &ctx) {
    ...
    case EOperatingMode::InMemory: {
      UDF_LOG(Logger, LogComponent, NUdf::ELogLevel::Info,
              TStringBuilder() << (const void *)&*JoinedTablePtr
                               << "# switching Memory mode to InMemory");
    ...
    case EOperatingMode::Spilling: {
      UDF_LOG(Logger, LogComponent, NUdf::ELogLevel::Info,
              TStringBuilder() << (const void *)&*JoinedTablePtr
                               << "# switching Memory mode to Spilling");
    ...
    case EOperatingMode::ProcessSpilled: {
      UDF_LOG(Logger, LogComponent, NUdf::ELogLevel::Info,
              TStringBuilder() << (const void *)&*JoinedTablePtr
                               << "# switching Memory mode to ProcessSpilled");
      ...
      std::sort(SpilledBucketsJoinOrder.begin(), SpilledBucketsJoinOrder.end(),
                [&](ui32 lhs, ui32 rhs) {
                  auto lhs_in_memory =
                      LeftPacker->TablePtr->IsBucketInMemory(lhs) +
                      RightPacker->TablePtr->IsBucketInMemory(lhs);
                  auto rhs_in_memory =
                      LeftPacker->TablePtr->IsBucketInMemory(rhs) +
                      RightPacker->TablePtr->IsBucketInMemory(rhs);
  ...
  EFetchResult FetchAndPackData(TComputationContext &ctx,
                                NUdf::TUnboxedValue *const *output) {
    const NKikimr::NMiniKQL::EFetchResult resultLeft =
        FlowLeft->FetchValues(ctx, LeftPacker->TuplePtrs.data());
    ...
    if (resultLeft == EFetchResult::One) {
      ...
      LeftPacker->Pack();
      {
        auto added = LeftPacker->TablePtr->AddTuple(
            LeftPacker->TupleIntVals.data(), LeftPacker->TupleStrings.data(),
            LeftPacker->TupleStrSizes.data(), LeftPacker->IColumnsHolder.data(),
            *RightPacker->TablePtr);
        if (added == GraceJoin::TTable::EAddTupleResult::Added) {
          ...
          ; // row dropped
        else if (JoinKind == EJoinKind::Inner || JoinKind == EJoinKind::Right ||
                 JoinKind == EJoinKind::RightSemi ||
                 JoinKind == EJoinKind::RightOnly ||
                 JoinKind == EJoinKind::LeftSemi)
          ;    // row dropped
    ...
    if (IsSelfJoin_) {
      ...
      if (!SelfJoinSameKeys_) {
        std::copy_n(LeftPacker->TupleHolder.begin(),
                    LeftPacker->TotalColumnsNum,
                    RightPacker->TupleHolder.begin());
      }
    } else {
    ...
    if (resultRight == EFetchResult::One) {
      ...
      if (!SelfJoinSameKeys_) {
        RightPacker->Pack();
        auto added = RightPacker->TablePtr->AddTuple(
            RightPacker->TupleIntVals.data(), RightPacker->TupleStrings.data(),
            RightPacker->TupleStrSizes.data(),
            RightPacker->IColumnsHolder.data(), *LeftPacker->TablePtr);
        if (added == GraceJoin::TTable::EAddTupleResult::Added) {
          ++RightPacker->TuplesBatchPacked;
        } else if (added == GraceJoin::TTable::EAddTupleResult::AnyMatch)
          ; // row dropped
        else if (JoinKind == EJoinKind::Inner || JoinKind == EJoinKind::Left ||
                 JoinKind == EJoinKind::LeftSemi ||
                 JoinKind == EJoinKind::LeftOnly ||
                 JoinKind == EJoinKind::RightSemi)
          ;    // row dropped
    ...
    if (resultLeft == EFetchResult::Yield ||
  ...
  EFetchResult DoCalculateInMemory(TComputationContext &ctx,
                                   NUdf::TUnboxedValue *const *output) {
    // Collecting data for join and perform join (batch or full)
    while (!*JoinCompleted) {
      if (*PartialJoinCompleted) {
        // Returns join results (batch or full)
        while (JoinedTablePtr->NextJoinedData(LeftPacker->JoinTupleData,
                                              RightPacker->JoinTupleData)) {
        ...
        if (!*HaveMoreRightRows) {
          *PartialJoinCompleted = false;
          ...
          LeftPacker->TablePtr->Clear(); // Clear table content, ready to
                                         // collect data for next batch
        ...
        if (!*HaveMoreLeftRows) {
          *PartialJoinCompleted = false;
          ...
          RightPacker->TablePtr->Clear(); // Clear table content, ready to
                                          // collect data for next batch
          JoinedTablePtr->Clear();
          JoinedTablePtr->ResetIterator();
        }
      ...
      if (IsSpillingAllowed && ctx.SpillerFactory &&
          IsSwitchToSpillingModeCondition()) {
      ...
      if (!*PartialJoinCompleted &&
        ...
        UDF_LOG(Logger, LogComponent, GRACEJOIN_TRACE,
                TStringBuilder()
                    << (const void *)&*JoinedTablePtr << '#' << " HaveLeft "
                    << *HaveMoreLeftRows << " LeftPacked "
                    << LeftPacker->TuplesBatchPacked << " LeftBatch "
                    << LeftPacker->BatchSize << " HaveRight "
                    << *HaveMoreRightRows << " RightPacked "
                    << RightPacker->TuplesBatchPacked << " RightBatch "
                    << RightPacker->BatchSize);

        auto &leftTable = *LeftPacker->TablePtr;
        auto &rightTable =
            SelfJoinSameKeys_ ? *LeftPacker->TablePtr : *RightPacker->TablePtr;
        if (IsSpillingAllowed && ctx.SpillerFactory &&
            !JoinedTablePtr->TryToPreallocateMemoryForJoin(
                leftTable, rightTable, JoinKind, *HaveMoreLeftRows,
                *HaveMoreRightRows)) {
          SwitchMode(EOperatingMode::Spilling, ctx);
          return EFetchResult::Yield;
        }
        ...
        *PartialJoinCompleted = true;
        LeftPacker->StartTime = std::chrono::system_clock::now();
        RightPacker->StartTime = std::chrono::system_clock::now();
        JoinedTablePtr->Join(leftTable, rightTable, JoinKind, *HaveMoreLeftRows,
                             *HaveMoreRightRows);
        JoinedTablePtr->ResetIterator();
  ...
  bool TryToReduceMemoryAndWait() {
    ...
    bool isWaitingLeftForReduce =
        LeftPacker->TablePtr->TryToReduceMemoryAndWait(largestBucketsPairIndex);
    bool isWaitingRightForReduce =
        RightPacker->TablePtr->TryToReduceMemoryAndWait(
            largestBucketsPairIndex);
  ...
  bool IsSpillingFinished() const {
    return LeftPacker->TablePtr->IsSpillingFinished() &&
           RightPacker->TablePtr->IsSpillingFinished();
  }

  bool IsReadyForSpilledDataProcessing() const {
    return LeftPacker->TablePtr->IsSpillingAcceptingDataRequests() &&
           RightPacker->TablePtr->IsSpillingAcceptingDataRequests();
  }

  bool IsRestoringSpilledBuckets() const {
    return LeftPacker->TablePtr->IsRestoringSpilledBuckets() ||
           RightPacker->TablePtr->IsRestoringSpilledBuckets();
  }
  ...
  EFetchResult ProcessSpilledData(TComputationContext &,
                                  NUdf::TUnboxedValue *const *output) {
    while (SpilledBucketsJoinOrderCurrentIndex != GraceJoin::NumberOfBuckets) {
      ...
      ui32 nextBucketToJoin =
          SpilledBucketsJoinOrder[SpilledBucketsJoinOrderCurrentIndex];

      if (LeftPacker->TablePtr->IsSpilledBucketWaitingForExtraction(
              nextBucketToJoin)) {
        LeftPacker->TablePtr->PrepareBucket(nextBucketToJoin);
      }

      if (RightPacker->TablePtr->IsSpilledBucketWaitingForExtraction(
              nextBucketToJoin)) {
      ...
      if (LeftPacker->TablePtr->IsBucketInMemory(nextBucketToJoin) &&
          RightPacker->TablePtr->IsBucketInMemory(nextBucketToJoin)) {
        if (*PartialJoinCompleted) {
          while (JoinedTablePtr->NextJoinedData(LeftPacker->JoinTupleData,
                                                RightPacker->JoinTupleData,
                                                nextBucketToJoin + 1)) {
          ...
          LeftPacker->TuplesBatchPacked = 0;
          LeftPacker->TablePtr->ClearBucket(
              nextBucketToJoin); // Clear content of returned bucket
          LeftPacker->TablePtr->ShrinkBucket(nextBucketToJoin);

          RightPacker->TuplesBatchPacked = 0;
          RightPacker->TablePtr->ClearBucket(
              nextBucketToJoin); // Clear content of returned bucket
          ...
          *PartialJoinCompleted = true;
          ...
          RightPacker->StartTime = std::chrono::system_clock::now();
          if (SelfJoinSameKeys_) {
            JoinedTablePtr->Join(*LeftPacker->TablePtr, *LeftPacker->TablePtr,
                                 JoinKind, *HaveMoreLeftRows,
                                 *HaveMoreRightRows, nextBucketToJoin,
                                 nextBucketToJoin + 1);
          } else {
            JoinedTablePtr->Join(*LeftPacker->TablePtr, *RightPacker->TablePtr,
                                 JoinKind, *HaveMoreLeftRows,
                                 *HaveMoreRightRows, nextBucketToJoin,
                                 nextBucketToJoin + 1);
          }
...
class TGraceJoinWrapper
    : public TStatefulWideFlowCodegeneratorNode<TGraceJoinWrapper> {
  using TBaseComputation =
      TStatefulWideFlowCodegeneratorNode<TGraceJoinWrapper>;

public:
  TGraceJoinWrapper(
      TComputationMutables &mutables, IComputationWideFlowNode *flowLeft,
      IComputationWideFlowNode *flowRight, EJoinKind joinKind,
      EAnyJoinSettings anyJoinSettings, std::vector<ui32> &&leftKeyColumns,
      std::vector<ui32> &&rightKeyColumns, std::vector<ui32> &&leftRenames,
      std::vector<ui32> &&rightRenames, std::vector<TType *> &&leftColumnsTypes,
      std::vector<TType *> &&rightColumnsTypes,
      std::vector<EValueRepresentation> &&outputRepresentations,
      bool isSelfJoin, bool isSpillingAllowed)
      : TBaseComputation(mutables, nullptr, EValueRepresentation::Boxed),
        FlowLeft(flowLeft), FlowRight(flowRight), JoinKind(joinKind),
        ...
        IsSelfJoin_(isSelfJoin), IsSpillingAllowed(isSpillingAllowed) {}
  ...
  EFetchResult DoCalculate(NUdf::TUnboxedValue &state, TComputationContext &ctx,
    ...
    return static_cast<TGraceJoinSpillingSupportState *>(state.AsBoxed().Get())
        ->FetchValues(ctx, output);
  }
#ifndef MKQL_DISABLE_CODEGEN
  ICodegeneratorInlineWideNode::TGenerateResult
  DoGenGetValues(const TCodegenContext &ctx, Value *statePtr,
    ...
    const auto arrayType =
        ArrayType::get(valueType, OutputRepresentations.size());
    const auto fieldsType = ArrayType::get(PointerType::getUnqual(valueType),
                                           OutputRepresentations.size());
    ...
    for (auto i = 0U; i < getters.size(); ++i) {
      pointers.emplace_back(GetElementPtrInst::CreateInBounds(
          arrayType, values,
          {ConstantInt::get(indexType, 0), ConstantInt::get(indexType, i)},
          (TString("ptr_") += ToString(i)).c_str(), atTop));
      initV = InsertValueInst::Create(
          initV, ConstantInt::get(valueType, 0), {i},
          (TString("zero_") += ToString(i)).c_str(), atTop);
      initF = InsertValueInst::Create(
          initF, pointers.back(), {i},
          (TString("insert_") += ToString(i)).c_str(), atTop);

      getters[i] = [i, values, indexType, arrayType,
                    valueType](const TCodegenContext &ctx, BasicBlock *&block) {
        Y_UNUSED(ctx);
        const auto pointer = GetElementPtrInst::CreateInBounds(
            arrayType, values,
            {ConstantInt::get(indexType, 0), ConstantInt::get(indexType, i)},
            (TString("ptr_") += ToString(i)).c_str(), block);
        return new LoadInst(valueType, pointer,
                            (TString("load_") += ToString(i)).c_str(), block);
      };
    }
    ...
    const auto stateType =
        StructType::get(context, fieldsStruct.GetFieldsArray());
    ...
    const auto self = CastInst::Create(
        Instruction::IntToPtr,
        ConstantInt::get(Type::getInt64Ty(context), uintptr_t(this)), ptrType,
        "self", block);
    const auto makeFunc = ConstantInt::get(
        Type::getInt64Ty(context),
        GetMethodPtr<&TGraceJoinWrapper::MakeSpillingSupportState>());
    const auto makeType = FunctionType::get(
        Type::getVoidTy(context),
        {self->getType(), ctx.Ctx->getType(), statePtr->getType()}, false);
    const auto makeFuncPtr =
        CastInst::Create(Instruction::IntToPtr, makeFunc,
                         PointerType::getUnqual(makeType), "function", block);
    CallInst::Create(makeType, makeFuncPtr, {self, ctx.Ctx, statePtr}, "",
                     block);
    ...
    const auto half = CastInst::Create(
        Instruction::Trunc, state, Type::getInt64Ty(context), "half", block);
    const auto stateArg = CastInst::Create(Instruction::IntToPtr, half,
                                           statePtrType, "state_arg", block);

    const auto func = ConstantInt::get(
        Type::getInt64Ty(context),
        GetMethodPtr<&TGraceJoinSpillingSupportState::FetchValues>());
    const auto funcType = FunctionType::get(
        Type::getInt32Ty(context),
        {statePtrType, ctx.Ctx->getType(), fields->getType()}, false);
    const auto funcPtr =
        CastInst::Create(Instruction::IntToPtr, func,
                         PointerType::getUnqual(funcType), "fetch_func", block);
    const auto result = CallInst::Create(
        funcType, funcPtr, {stateArg, ctx.Ctx, fields}, "fetch", block);
...
private:
  ...
  void MakeSpillingSupportState(TComputationContext &ctx,
                                NUdf::TUnboxedValue &state) const {
    NYql::NUdf::TLoggerPtr logger = ctx.MakeLogger();
    NYql::NUdf::TLogComponentId logComponent =
        logger->RegisterComponent("GraceJoin");
    UDF_LOG(logger, logComponent, NUdf::ELogLevel::Debug,
            TStringBuilder() << "State initialized");

    state = ctx.HolderFactory.Create<TGraceJoinSpillingSupportState>(
        FlowLeft, FlowRight, JoinKind, AnyJoinSettings_, LeftKeyColumns,
        RightKeyColumns, LeftRenames, RightRenames, LeftColumnsTypes,
        RightColumnsTypes, ctx, IsSelfJoin_, IsSpillingAllowed, logger,
        logComponent);
  }
...
IComputationNode *WrapGraceJoinCommon(TCallable &callable,
  ...
  const auto leftKeyColumnsNode =
      AS_VALUE(TTupleLiteral, callable.GetInput(leftKeyColumnsNodeIndex));
  const auto rightKeyColumnsNode =
      AS_VALUE(TTupleLiteral, callable.GetInput(rightKeyColumnsNodeIndex));
  const auto leftRenamesNode =
      AS_VALUE(TTupleLiteral, callable.GetInput(leftRenamesNodeIndex));
  const auto rightRenamesNode =
      AS_VALUE(TTupleLiteral, callable.GetInput(rightRenamesNodeIndex));
  const EAnyJoinSettings anyJoinSettings = GetAnyJoinSettings(
      AS_VALUE(TDataLiteral, callable.GetInput(anyJoinSettingsIndex))
          ->AsValue()
          .Get<ui32>());

  const auto leftFlowComponents =
      GetWideComponents(AS_TYPE(TFlowType, leftFlowNode));
  const ui32 rawJoinKind =
      AS_VALUE(TDataLiteral, joinKindNode)->AsValue().Get<ui32>();

  const auto flowLeft = dynamic_cast<IComputationWideFlowNode *>(
      LocateNode(ctx.NodeLocator, callable, 0));
  IComputationWideFlowNode *flowRight = nullptr;
  if (!isSelfJoin) {
    flowRight = dynamic_cast<IComputationWideFlowNode *>(
        LocateNode(ctx.NodeLocator, callable, 1));
  }

  const auto outputFlowComponents = GetWideComponents(
      AS_TYPE(TFlowType, callable.GetType()->GetReturnType()));
  std::vector<EValueRepresentation> outputRepresentations;
  outputRepresentations.reserve(outputFlowComponents.size());
  for (ui32 i = 0U; i < outputFlowComponents.size(); ++i) {
    outputRepresentations.emplace_back(
        GetValueRepresentation(outputFlowComponents[i]));
  }

  std::vector<ui32> leftKeyColumns, leftRenames, rightKeyColumns, rightRenames;
  std::vector<TType *> leftColumnsTypes(leftFlowComponents.begin(),
                                        leftFlowComponents.end());
  std::vector<TType *> rightColumnsTypes;
  if (isSelfJoin) {
    rightColumnsTypes = {leftColumnsTypes};
  } else {
    const auto rightFlowNode = callable.GetInput(rightFlowNodeIndex);
    const auto rightFlowComponents =
        GetWideComponents(AS_TYPE(TFlowType, rightFlowNode));
    rightColumnsTypes = {rightFlowComponents.begin(),
                         rightFlowComponents.end()};
  }

  leftKeyColumns.reserve(leftKeyColumnsNode->GetValuesCount());
  for (ui32 i = 0; i < leftKeyColumnsNode->GetValuesCount(); ++i) {
    leftKeyColumns.emplace_back(
        AS_VALUE(TDataLiteral, leftKeyColumnsNode->GetValue(i))
            ->AsValue()
            .Get<ui32>());
  }
  ...
  for (ui32 i = 0; i < rightKeyColumnsNode->GetValuesCount(); ++i) {
    rightKeyColumns.emplace_back(
        AS_VALUE(TDataLiteral, rightKeyColumnsNode->GetValue(i))
            ->AsValue()
            .Get<ui32>());
  }

  if (isSelfJoin) {
    MKQL_ENSURE(leftKeyColumns.size() == rightKeyColumns.size(),
                "Number of key columns for self join should be equal");
  }
  ...
  os << "[MISHA GJ renames]" << rawJoinKind << std::endl;
  os << "\tLEFT:\n\t";
  ...
  return new TGraceJoinWrapper(
      ...
      anyJoinSettings, std::move(leftKeyColumns), std::move(rightKeyColumns),
      std::move(leftRenames), std::move(rightRenames),
      std::move(leftColumnsTypes), std::move(rightColumnsTypes),
      std::move(outputRepresentations), isSelfJoin, isSpillingAllowed);
}

IComputationNode *WrapGraceJoin(TCallable &callable,
                                const TComputationNodeFactoryContext &ctx) {
...
IComputationNode *WrapGraceSelfJoin(TCallable &callable,
                                    const TComputationNodeFactoryContext &ctx) {
