...
Y_FORCE_INLINE void EnsureInputSize(TStringBuf& input, size_t size) {
    MKQL_ENSURE(input.size() >= size, "premature end of input");
}

template <bool Desc>
Y_FORCE_INLINE void EncodeBool(TVector<ui8>& output, bool value) {
    output.push_back(Desc ? 0xFF ^ ui8(value) : ui8(value));
}

template <bool Desc>
Y_FORCE_INLINE bool DecodeBool(TStringBuf& input) {
...
Y_FORCE_INLINE void EncodeUnsigned(TVector<ui8>& output, TUnsigned value) {
...
template <typename TUnsigned, bool Desc>
Y_FORCE_INLINE TUnsigned DecodeUnsigned(TStringBuf& input) {
    constexpr size_t size = sizeof(TUnsigned);
...
Y_FORCE_INLINE void EncodeSigned(TVector<ui8>& output, TSigned value) {
...
template <typename TSigned, bool Desc>
Y_FORCE_INLINE TSigned DecodeSigned(TStringBuf& input) {
    using TUnsigned = std::make_unsigned_t<TSigned>;
    constexpr size_t size = sizeof(TUnsigned);
    constexpr TUnsigned shift = TUnsigned(1) << (size * 8 - 1);
...
Y_FORCE_INLINE void EncodeFloating(TVector<ui8>& output, TFloat value) {
...
template <typename TFloat, bool Desc>
Y_FORCE_INLINE TFloat DecodeFloating(TStringBuf& input) {
    using TInteger = typename TFloatToInteger<TFloat>::TType;
...
template <bool Desc>
Y_FORCE_INLINE void EncodeString(TVector<ui8>& output, TStringBuf value) {
    size_t part = 0;
...
template <bool Desc>
Y_FORCE_INLINE TStringBuf DecodeString(TStringBuf& input, TVector<ui8>& value) {
    EnsureInputSize(input, 1);
