...
TUnboxedValuePod PeelData(const TDataTypeId nodeType, const TUnboxedValuePod value, const IValueBuilder* valueBuilder, const TSourcePosition& pos) {
    switch (nodeType) {
        case TDataType<char*>::Id:
            return ConvertToString<Strict, AutoConvert, false>(value, valueBuilder, pos);
        case TDataType<TUtf8>::Id:
            return ConvertToString<Strict, AutoConvert, true>(value, valueBuilder, pos);
        case TDataType<bool>::Id:
            return ConvertToBool<Strict, AutoConvert>(value, valueBuilder, pos);
        case TDataType<i8>::Id:
            return ConvertToIntegral<Strict, AutoConvert, i8>(value, valueBuilder, pos);
        case TDataType<i16>::Id:
            return ConvertToIntegral<Strict, AutoConvert, i16>(value, valueBuilder, pos);
        case TDataType<i32>::Id:
            return ConvertToIntegral<Strict, AutoConvert, i32>(value, valueBuilder, pos);
        case TDataType<i64>::Id:
            return ConvertToIntegral<Strict, AutoConvert, i64>(value, valueBuilder, pos);
        case TDataType<ui8>::Id:
            return ConvertToIntegral<Strict, AutoConvert, ui8>(value, valueBuilder, pos);
        case TDataType<ui16>::Id:
            return ConvertToIntegral<Strict, AutoConvert, ui16>(value, valueBuilder, pos);
        case TDataType<ui32>::Id:
            return ConvertToIntegral<Strict, AutoConvert, ui32>(value, valueBuilder, pos);
        case TDataType<ui64>::Id:
            return ConvertToIntegral<Strict, AutoConvert, ui64>(value, valueBuilder, pos);
        case TDataType<float>::Id:
            return ConvertToFloat<Strict, AutoConvert, float>(value, valueBuilder, pos);
        case TDataType<double>::Id:
            return ConvertToFloat<Strict, AutoConvert, double>(value, valueBuilder, pos);
        case TDataType<TYson>::Id:
            return valueBuilder->NewString(SerializeYsonDomToBinary(value)).Release();
        case TDataType<TJson>::Id:
            return valueBuilder->NewString(SerializeJsonDom(value)).Release();
        default:
            break;
    }
    ...
    switch (const auto kind = typeHelper->GetTypeKind(shape)) {
        ...
        case ETypeKind::Tuple:
            if (const auto tupleTypeInspector = TTupleTypeInspector(*typeHelper, shape); auto count = tupleTypeInspector.GetElementsCount()) {
                ...
                items += count;
                do {
                    *--items = MakeStub(typeHelper, tupleTypeInspector.GetElementType(--count), valueBuilder, pos);
                } while (count);
        ...
        case ETypeKind::Struct:
            if (const auto structTypeInspector = TStructTypeInspector(*typeHelper, shape); auto count = structTypeInspector.GetMembersCount()) {
                ...
                items += count;
                do {
                    *--items = MakeStub(typeHelper, structTypeInspector.GetMemberType(--count), valueBuilder, pos);
                } while (count);
                return result.Release();
            }
            return valueBuilder->NewEmptyList().Release();
        ...
        case ETypeKind::Dict: {
            ...
            if (const auto keyKind = typeHelper->GetTypeKind(keyType); ETypeKind::Data == keyKind) {
                switch (const auto keyId = TDataTypeInspector(*typeHelper, keyType).GetTypeId()) {
                    case TDataType<char*>::Id:
                        return PeelDict<Strict, AutoConvert, false>(typeHelper, dictTypeInspector.GetValueType(), value, valueBuilder, pos);
                    case TDataType<TUtf8>::Id:
                        return PeelDict<Strict, AutoConvert, true>(typeHelper, dictTypeInspector.GetValueType(), value, valueBuilder, pos);
                    default:
                        UdfTerminate((::TStringBuilder() << pos << " Unsupported dict key type: " << keyId).c_str());
                }
            } else {
