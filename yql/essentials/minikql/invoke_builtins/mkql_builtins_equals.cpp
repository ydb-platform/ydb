...
template <typename TLeft, typename TRight, bool Aggr>
struct TDiffDateEquals: public TCompareArithmeticBinary<typename TLeft::TLayout, typename TRight::TLayout,
                                                        TDiffDateEquals<TLeft, TRight, Aggr>>,
                        public TAggrEquals {
    ...
    static Value* Gen(Value* left, Value* right, const TCodegenContext& ctx, BasicBlock*& block)
    {
        ...
        return std::is_same<TLeft, TRight>::value
                   ...
                   : GenEquals<TScaledDate, TScaledDate, Aggr>(
...
struct TAggrTzDateEquals: public TArithmeticConstraintsBinary<TLeft, TRight, bool>, public TAggrEquals {
    ...
    static NUdf::TUnboxedValuePod Execute(const NUdf::TUnboxedValuePod& left, const NUdf::TUnboxedValuePod& right) {
        return NUdf::TUnboxedValuePod(Join(
            Equals<TLeft, TRight, Aggr>(left.template Get<TLeft>(), right.template Get<TRight>()),
            Equals<ui16, ui16, Aggr>(left.GetTimezoneId(), right.GetTimezoneId())));
    }
    ...
    static Value* Generate(Value* left, Value* right, const TCodegenContext& ctx, BasicBlock*& block)
    {
        ...
        const auto result = GenJoin(
            GenEquals<TLeft, TRight, Aggr>(lhs, rhs, context, block),
            GenEquals<ui16, ui16, Aggr>(ltz, rtz, context, block), block);
