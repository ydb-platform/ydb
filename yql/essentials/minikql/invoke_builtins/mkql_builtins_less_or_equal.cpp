...
template <typename TLeft, typename TRight, bool Aggr>
struct TDiffDateLessOrEqual: public TCompareArithmeticBinary<typename TLeft::TLayout,
                                                             typename TRight::TLayout, TDiffDateLessOrEqual<TLeft, TRight, Aggr>>,
                             public TAggrLessOrEqual {
    static bool Do(typename TLeft::TLayout left, typename TRight::TLayout right)
    {
        if constexpr (std::is_same<TLeft, TRight>::value) {
            return LessOrEqual<typename TLeft::TLayout, typename TRight::TLayout, Aggr>(left, right);
        } else {
            return LessOrEqual<TScaledDate, TScaledDate, Aggr>(ToScaledDate<TLeft>(left), ToScaledDate<TRight>(right));
        }
    }
    ...
    static Value* Gen(Value* left, Value* right, const TCodegenContext& ctx, BasicBlock*& block)
    {
        ...
        if constexpr (std::is_same<TLeft, TRight>::value) {
            return GenLessOrEqual<typename TLeft::TLayout, typename TRight::TLayout, Aggr>(left, right, context, block);
        } else {
            return GenLessOrEqual<TScaledDate, TScaledDate, Aggr>(GenToScaledDate<TLeft>(left, context, block),
...
template <typename TLeft, typename TRight, bool Aggr>
struct TAggrTzDateLessOrEqual: public TCompareArithmeticBinaryWithTimezone<TLeft, TRight, TAggrTzDateLessOrEqual<TLeft, TRight, Aggr>>,
                               public TAggrLessOrEqual {
    static bool Do(TLeft left, TRight right)
    {
        return LessOrEqual<TLeft, TRight, Aggr>(left, right);
