...
public:
    ...
    public:
        ...
        void Visit(TCallableType& node) override {
            ...
            Owner_.Write(TypeMarker | (char)TType::EKind::Callable | (node.IsMergeDisabled() ? UserMarker2 : 0) | (node.GetPayload() ? UserMarker3 : 0));
...
public:
    ...
    TRuntimeNode Deserialize() {
        ...
        if ((header & CommandMask) != (char)ESystemCommand::Begin && (header & CommandMask) != (char)ESystemCommand::BeginNotImmediate) {
            ThrowCorrupted();
        }
...
private:
    ...
    TNode* ReadOptionalOrSimilarType(char code) {
        switch ((TType::EKind)(code & TypeMask)) {
            case TType::EKind::Optional:
                return ReadOptionalType();
            case TType::EKind::Tagged:
                return ReadTaggedType();
            case TType::EKind::Linear:
                return ReadLinearType();
            default:
                ThrowCorrupted();
        }
    }
    ...
    TNode* ReadVoid(char code) {
        switch ((TType::EKind)(code & TypeMask)) {
            case TType::EKind::Void:
                return Env_.GetVoidLazy();
            case TType::EKind::EmptyList:
                return Env_.GetEmptyListLazy();
            case TType::EKind::EmptyDict:
                return Env_.GetEmptyDictLazy();
            default:
                ThrowCorrupted();
        }
    }
