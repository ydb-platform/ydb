...
2. Bump PostgreSQL version in copy_src.sh (it is recommended not to do big jumps here)

3. Run ./copy_src.sh

4. Assuming compilation and automatic patching were successful, apply local changes collected on step 2:

   patch -p6 < local_changes.patch

   Resolve possible conflicts. Usually, conflicts arise due to some already backported changes

5. Update pg_catalog data

   (cd ../../tools/pg_catalog_dump/ && yag make --build=relwithdebinfo && ./pg_catalog_dump > dump.json)

6. Regenerate Arrow postgresql kernels

   ./generate_kernels.py

7. Make sure that resulting pg_wrapper library compiles and passes minimal tests

   yag make --build=relwithdebinfo -tA -C ut -C test -C ../../sql/pg/ut

8. Verify that all global variables in PostgreSQL-originated sources are accounted for
