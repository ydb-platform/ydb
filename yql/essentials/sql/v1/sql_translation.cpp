...
bool TSqlTranslation::CreateTableIndex(const TRule_table_index& node, TVector<TIndexDescription>& indexes) {
    ...
    if (node.GetRule_table_index_type3().HasBlock2()) {
        ...
        if (subType == "VECTOR_KMEANS_TREE" ||
            subType == "FULLTEXT_PLAIN" ||
            subType == "FULLTEXT_RELEVANCE" ||
            subType == "JSON" ||
            subType == "FULLTEXT_COMPACT" ||
            subType == "FULLTEXT_COMPACT_RELEVANCE" ||
            subType == "JSON_COMPACT") {
            ...
            } else if (subType == "FULLTEXT_COMPACT") {
                indexes.back().Type = TIndexDescription::EType::GlobalFulltextCompact;
            } else if (subType == "FULLTEXT_COMPACT_RELEVANCE") {
                indexes.back().Type = TIndexDescription::EType::GlobalFulltextCompactRelevance;
            } else if (subType == "JSON_COMPACT") {
                indexes.back().Type = TIndexDescription::EType::GlobalJsonCompact;
            } else {
                Y_ABORT("Unreachable");
            }
