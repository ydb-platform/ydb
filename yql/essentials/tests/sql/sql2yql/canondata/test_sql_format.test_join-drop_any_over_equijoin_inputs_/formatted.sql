...
JOIN ANY
    $distinct AS b
ON
    a.key == b.key AND a.subkey == b.subkey AND a.value == b.value
;
...
JOIN ANY
    $distinct AS c
ON
    a.key == c.key AND a.subkey == c.subkey -- drop
RIGHT JOIN ANY
    $unique AS d
ON
    a.key == d.key AND a.subkey == d.subkey -- keep
LEFT JOIN ANY
    $unique AS e
ON
    a.key == e.key AND a.subkey == e.subkey
; -- drop
...
JOIN ANY
    $complex_key AS b
ON
    a.key == b.key.0 AND a.subkey == b.key.1
; -- drop
