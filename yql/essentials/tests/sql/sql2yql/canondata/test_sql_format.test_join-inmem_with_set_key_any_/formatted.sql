...
SELECT
    ListSort(DictItems(bar.g)) AS bar_g,
    bar.h,
    ListSort(DictItems(foo.e)) AS foo_e,
    foo.f,
    ListSort(DictItems(one.a)) AS one_a,
    one.b,
    ListSort(DictItems(two.c)) AS two_c,
    two.d
FROM ANY AS_TABLE($one)
    AS one
INNER JOIN ANY AS_TABLE($two)
    AS two
ON one.a == two.c
LEFT JOIN ANY AS_TABLE($foo)
    AS foo
ON foo.e == two.c
FULL JOIN ANY AS_TABLE($bar)
    AS bar
ON bar.g == one.a
ORDER BY
    bar_g,
