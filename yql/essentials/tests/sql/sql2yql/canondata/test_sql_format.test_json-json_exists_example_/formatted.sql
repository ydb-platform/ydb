...
FROM
    T
WHERE
    JSON_EXISTS (T.J, 'strict $.where' FALSE ON ERROR)
;
...
FROM
    T
WHERE
    JSON_EXISTS (T.J, 'strict $.friends[*].rank')
;
...
FROM
    Filter1
WHERE
    JSON_EXISTS (Filter1.J, 'lax $ ? (@.pay/@.hours > 9)')
;
...
FROM
    Filter1
WHERE
    JSON_EXISTS (Filter1.J, 'strict $ ? (@.pay/@.hours > 9)')
;
...
FROM
    Filter2
WHERE
    JSON_EXISTS (Filter2.J, 'lax $ ? (@.pay/@.hours > 9)')
;
...
FROM
    Filter2
WHERE
    JSON_EXISTS (Filter2.J, 'strict $ ? (@.pay/@.hours > 9)')
;
...
FROM
    Filter2
WHERE
    JSON_EXISTS (Filter2.J, 'lax $ ? (@.hours > 9)')
;
...
FROM
    Filter2
WHERE
    JSON_EXISTS (Filter2.J, 'strict $ ? (@.hours > 9)')
;
