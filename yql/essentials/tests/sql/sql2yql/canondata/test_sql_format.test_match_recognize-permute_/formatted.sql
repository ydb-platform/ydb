...
FROM
    ...
    MEASURES FIRST(A.dt) AS a, FIRST(B.dt) AS b, FIRST(C.dt) AS c ONE ROW PER MATCH AFTER MATCH SKIP TO NEXT ROW PATTERN (PERMUTE (A, B, C)) DEFINE A AS A.event == "A", B AS B.event == "B", C AS C.event == "C") AS MATCHED
;
