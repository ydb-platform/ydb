...
void SetupCallableSettings(const TPyCastContext::TPtr& castCtx, PyObject* value) {
    if (const auto lazyInput = GetOptionalAttribute(value, "_yql_lazy_input")) {
        try {
    ...
    if (const auto convertYson = GetOptionalAttribute(value, "_yql_convert_yson")) {
        try {
    ...
    if (const auto bytesDecodeMode = GetOptionalAttribute(value, "_yql_bytes_decode_mode")) {
        try {
