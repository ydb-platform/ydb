...
//! Translate one YQL `Optional<T>` argument into a `TUnversionedValue` allocated inside the
//! compartment. Strings are copied into compartment memory via CopyIntoCompartment so the wasm
//! UDF can read them through the YT memory64 ABI.
    ...
    EUdfValueType expectedType)
{
    ...
    switch (expectedType) {
        ...
        case EUdfValueType::String: {
            ...
            // The result string lives inside the compartment memory pool whose lifetime
            // ends with `TWasmUdfInvocationContext::WebAssemblyPool.Clear()` at the end of this call,
            // so we MUST copy it out via valueBuilder->NewString before returning.
...
TWasmUdfFunction::TWasmUdfFunction(TWasmDirectoryStatePtr state, const TWasmUdfDescriptor& descriptor)
    ...
    const TUnboxedValuePod* args) const
{
    try {
        ...
        // Phase 1: serialize per-compartment invocations. YT-side compartments are not thread-safe;
        // proper clone-per-thread is Phase 2.
...
TWasmSoModule::TWasmSoModule(TWasmDirectoryStatePtr state, TString moduleName)
    : State_(std::move(state))
...
{
    const auto* locator = State_->Locators.FindPtr(ModuleName_);
    if (!locator) {
        return;
    }
    for (const auto& name : locator->DescriptorOrder) {
    ...
    IFunctionTypeInfoBuilder& builder) const
{
    try {
        ...
        if (!locator->Exports.contains(functionName)) {
