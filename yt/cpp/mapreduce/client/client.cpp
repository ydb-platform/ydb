    ...
    const TCopyOptions& options)
{
    try {
        ...
        if (e.GetError().ContainsErrorCode(NClusterErrorCodes::NObjectClient::CrossCellAdditionalPath)) {
            ...
            std::function<TNodeId(ITransactionPtr)> lambda = [this, &sourcePath, &destinationPath](ITransactionPtr transaction) {
                return NRawClient::CopyWithoutRetries(Context_, transaction->GetId(), sourcePath, destinationPath);
            };
    ...
    const TMoveOptions& options)
{
    try {
        ...
        if (e.GetError().ContainsErrorCode(NClusterErrorCodes::NObjectClient::CrossCellAdditionalPath)) {
            ...
            std::function<TNodeId(ITransactionPtr)> lambda = [this, &sourcePath, &destinationPath](ITransactionPtr transaction) {
                return NRawClient::MoveWithoutRetries(Context_, transaction->GetId(), sourcePath, destinationPath);
            };
            return RetryTransactionWithPolicy<TNodeId>(
                this,
                lambda,
