...
// Exercises the multi-server and multi-channel-factory wiring over a single
// "yt-tcp" backend: a service registered on the multi-server must be reachable
// through a channel produced by the multi-channel factory.
...
{
    TTestProxy proxy(CreateChannel(Format("yt-tcp://%v", GetAddress())));
...
TEST_F(TMultiProtocolRpcTest, SendWithDefaultProtocol)
{
    TTestProxy proxy(CreateChannel(GetAddress()));
    auto req = proxy.SomeCall();
    req->set_a(1);
    auto rspOrError = WaitFor(req->Invoke());
    EXPECT_TRUE(rspOrError.IsOK()) << ToString(rspOrError);
    EXPECT_EQ(101, rspOrError.Value()->b());
}

TEST_F(TMultiProtocolRpcTest, UnsupportedProtocol)
{
    EXPECT_THROW_WITH_SUBSTRING(
        CreateChannel(Format("no-such-protocol://%v", GetAddress())),
