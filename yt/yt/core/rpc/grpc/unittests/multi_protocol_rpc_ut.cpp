...
// Exercises the multi-server and multi-channel-factory wiring over the "grpc"
...
public:
    void SetUp() final
    {
        ...
        auto serverConfig = ConvertTo<TMultiProtocolServerConfigPtr>(NYTree::BuildYsonNodeFluently()
            .BeginMap()
                .Item("grpc").BeginMap()
                    .Item("addresses").BeginList()
                        .Item().BeginMap()
                            .Item("address").Value(GetAddress())
                        .EndMap()
                    .EndList()
                .EndMap()
            .EndMap());
...
{
    TTestProxy proxy(CreateChannel(Format("grpc://%v", GetAddress())));
