...
template <class T>
T IMapNode::GetChildValueOrThrow(TKeyView key) const
{
    return GetChildOrThrow(key)->GetValue<T>();
}

template <class T>
T IMapNode::GetChildValueOrDefault(TKeyView key, const T& defaultValue) const
{
...
template <class T>
std::optional<T> IMapNode::FindChildValue(TKeyView key) const
{
    auto child = FindChild(key);
