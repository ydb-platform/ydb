...
public:
    ...
    void Symbolize(const TSymbolizeOptions& options)
    {
        ...
        for (int index = 0; index < Profile_->location_size(); index++) {
            ...
            if (auto object = SymbolIndex_.FindObject(ip)) {
                location->set_mapping_id(AddObject(object));
            }
        }

        Profile_->add_comment(AddString("generated by ytprof " + GetVersion()));

        if (options.SymbolizeExistingFunctions) {
...
private:
    ...
    std::optional<const void*> AddFunction(const void* locationIp)
    {
        ...
        if (!Functions_.contains(ip)) {
            ...
            function->set_id(reinterpret_cast<ui64>(ip));
            function->set_name(symbolized->Name);
            function->set_system_name(symbolized->SystemName);
        }
    ...
    std::optional<TSymbolizedFunction> SymbolizeFunction(const void* ip)
    {
    ...
    ui64 AddObject(const TSymbolIndex::TObject* object)
    {
        ...
        mapping->set_filename(AddString(object->Name));
        return mapping->id();
    }
};
